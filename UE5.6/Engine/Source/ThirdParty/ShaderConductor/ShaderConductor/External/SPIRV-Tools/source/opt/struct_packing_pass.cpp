...
// Alignments the specified value to a multiple of alignmnet, whereas the
...
uint32_t StructPackingPass::getPackedAlignment(
  ...
  switch (type.kind()) {
    ...
    default: {
      ...
      if (auto* matrixType = type.AsMatrix()) {
        // Rule 5/7
        if (isPackingVec4Padded(packingRules_) ||
      ...
      } else if (auto* vectorType = type.AsVector()) {
        // Rule 1
...
static uint32_t getPadAlignment(const analysis::Type& type, uint32_t packedAlignment) {
  ...
  switch (type.kind()) {
    case analysis::Type::kArray: {
      if (const analysis::Array* arrayType = type.AsArray()) {
        uint32_t size = getPackedArrayStride(*arrayType) * getArrayLength(*arrayType);
    ...
    case analysis::Type::kStruct: {
	  ...
	  for (const analysis::Type* memberType : type.AsStruct()->element_types()) {
    ...
    default: {
      ...
      if (isPackingScalar(packingRules_)) {
        ...
        uint32_t size = 0;
        if (auto* matrixType = type.AsMatrix()) {
          auto* matrixSubType = matrixType->element_type()->AsVector();
...
Pass::Status StructPackingPass::Process() {
  ...
  // Find all structs of interest
  uint32_t structIdToPack = ~0u;
  for (Instruction& instr : context()->module()->debugs2()) {
    if (instr.opcode() == spv::Op::OpName &&
        instr.GetOperand(1).AsString() == structToPack_) {
      structIdToPack = instr.GetOperand(0).AsId();
      break;
    }
  }
  ...
  struct StructMemberInfo {
    ...
    uint32_t offset = 0; // Original offset
    uint32_t packedOffset = 0; // Packed offset
  };
  std::vector<StructMemberInfo> structMembers;

  if (structIdToPack == ~0u) {
  ...
  for (auto
           it =
               std::reverse_iterator{context()->module()->types_values_end()},
           itEnd =
               std::reverse_iterator{context()->module()->types_values_begin()};
       it != itEnd; ++it) {
    if (it->opcode() == spv::Op::OpTypeStruct &&
        it->result_id() == structIdToPack) {
      // Found struct type to pack, now collect all types of its members
      assert(it->NumOperands() > 0 &&
             "Number of operands in OpTypeStruct instruction must not be zero");
      const uint32_t numMembers = it->NumOperands() - 1;
      structMembers.resize(numMembers);
      for (uint32_t i = 0; i < numMembers; ++i) {
        const spv::Id memberTypeId = it->GetOperand(1 + i).AsId();
        if (analysis::Type* memberType =
                context()->get_type_mgr()->GetType(memberTypeId)) {
          structMembers[i].typeId = memberTypeId;
          structMembers[i].type = memberType;
        }
      }
      break;
    }
  ...
  auto isMemberOffsetDecoration = [structIdToPack](Instruction& instr) -> bool {
  ...
  for (auto it = context()->module()->annotation_begin(),
    ...
    if (isMemberOffsetDecoration(*it)) {
      ...
      do {
        ...
        const auto& memberType = *structMembers[memberIndex].type;
