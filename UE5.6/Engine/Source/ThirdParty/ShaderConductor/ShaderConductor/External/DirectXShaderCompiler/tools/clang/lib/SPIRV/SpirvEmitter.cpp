...
SpirvInstruction *SpirvEmitter::castToType(SpirvInstruction *value,
                                           ...
                                           SourceRange range) {
  // UE Change Begin: No-op when both types are equal
  if (fromType == toType)
    return value;
  // UE Change End: No-op when both types are equal
...
SpirvInstruction *SpirvEmitter::doShortCircuitedConditionalOperator(
  ...
  // UE Change Begin: Fix null pointer exception
  if (!trueVal)
    return nullptr;
  // UE Change End: Fix null pointer exception
