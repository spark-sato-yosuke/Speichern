...
namespace uba
{
	...
	public:
		...
		virtual bool Send(Logger& logger, void* connection, const void* data, u32 dataSize, SendContext& sendContext, const tchar* sendHint) = 0;
		...
		using RecvTimeoutCallback = bool(void* context, u32 timeoutMs, const tchar* recvHint, const tchar* hint);
		virtual void SetRecvTimeout(void* connection, u32 timeoutMs, void* context, RecvTimeoutCallback* callback) = 0;
		...
		// Remove connection (user guarantee connection is not referenced anywhere)
		virtual void DeleteConnection(void* connection) = 0;

		virtual void GetTotalSendAndRecv(u64& outSend, u64& outRecv) = 0;
		virtual void Validate(Logger& logger) = 0;
	};
	...
	public:
		HttpConnection();
		~HttpConnection();
		bool Query(Logger& logger, const char* type, StringBufferBase& outResponse, u32& outStatusCode, const char* host, const char* path, const char* header = "", u32 timeoutMs = 3000);

		void SetConnectTimeout(u32 timeOutMs);
