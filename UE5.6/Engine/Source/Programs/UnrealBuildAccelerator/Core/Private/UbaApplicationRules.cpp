...
namespace uba
{
	...
	public:
		...
		virtual bool KeepInMemory(const StringView& fileName, const tchar* systemTemp, bool isRunningRemote, bool isWrite) const override
		{
	...
	public:
		...
		virtual bool NeedsSharedMemory(const tchar* file) const override
		{
			return Contains(file, TC("lnk{")) // This file is shared from link.exe to mt.exe and rc.exe so we need to put it shared memory
			 	...
			 	|| ToView(file).EndsWith(TCV(".manifest")); // lld-link.exe is using a different name for files shared with child processes
		}
	...
	class ApplicationRulesLldLinkExe : public ApplicationRulesVcLink
	{
		...
		virtual bool KeepInMemory(const StringView& fileName, const tchar* systemTemp, bool isRunningRemote, bool isWrite) const override
		{
			return fileName.EndsWith(TCV(".manifest")) && fileName.Contains(systemTemp);
		}
	...
	class ApplicationRulesClangPlusPlusExePlatform1 : public ApplicationRulesClangPlusPlusExe
	{
		...
		//{
		//	return fileName.Contains(TCV("lto-llvm")); // Used by a clang based platform's link time optimization pass. Shared from lto process back to linker process
		//}
	};
	...
	class ApplicationRulesLdExePlatform1 : public ApplicationRules
	{
		...
		//virtual bool IsOutputFile(const StringView& fileName) const override
		//{
		//	return fileName.EndsWith(TCV(".self")) || Equals(file, TC("Symbols.map"));
		//}
		virtual bool KeepInMemory(const StringView& fileName, const tchar* systemTemp, bool isRunningRemote, bool isWrite) const override
		{
			return Super::KeepInMemory(fileName, systemTemp, isRunningRemote, isWrite)
	...
	class ApplicationRulesClangPlusPlusExePlatform2 : public ApplicationRulesClangPlusPlusExe
	{
		...
		virtual bool IsOutputFile(const StringView& fileName) const override
		{
			return fileName.Contains(TCV(".self")) || Super::IsOutputFile(fileName);
		}
