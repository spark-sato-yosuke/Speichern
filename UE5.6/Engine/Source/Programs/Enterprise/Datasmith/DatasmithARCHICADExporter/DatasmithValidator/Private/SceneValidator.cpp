    ...
    for (int32 Index = 0; Index < Count; ++Index)
    {
        ...
        if (Texture.IsValid())
        {
            ...
            if (FPaths::IsRelative(TextureFile))
            {
                ...
                {
                    AddMessage(kBug, TEXT("Texture Name=\"%s\", Label=\"%s\", Path=\"%s\" - Relative path without reference path"), Texture->GetName(), Texture->GetLabel(), *TextureFile);
                    Reported = true;
                }
            }
            if (!Reported && !FPaths::FileExists(TextureFile))
            {
                AddMessage(kBug, TEXT("Texture Name=\"%s\", Label=\"%s\", Path=\"%s\" - File not found"), Texture->GetName(), Texture->GetLabel(), *TextureFile);
            }
        }
    }
}
