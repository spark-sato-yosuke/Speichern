...
GSErrCode FTaskCalledFromEventLoop::DoTasks(GSHandle ParamHandle)
{
	if (ParamHandle)
	{
		...
		#if AC_VERSION > 27
		if (CHCompareCStrings(Param.name, "TaskSharedRef", GS::CaseSensitive) == 0 && Param.type == MDCLPar_pointer)
		#else
		...
		#if AC_VERSION > 27
		else if (CHCompareCStrings(Param.name, "TaskWeakPtr", GS::CaseSensitive) == 0 && Param.type == MDCLPar_pointer)
		#else
