...
utf8_string FElement2String::GetIFCAttributesAsString(const API_Guid& InElementGuid)
{
	...
	if (ElementObjectID.IsOk())
	{
		...
		{
			IFCAttributesString += Utf8StringFormat("\t\tIFCAtributes Nb=%zu\n", IFCAttributes->size());
			for (const auto& IfcAttribute : *IFCAttributes)
			{
				if (IfcAttribute.GetValue().has_value())
				{
					IFCAttributesString += Utf8StringFormat(
						"\t\t\tName=%s, Type=%s, %s=%s\n", IfcAttribute.GetName().ToUtf8(), IfcAttribute.GetValueType().ToUtf8(),
						IfcAttribute.GetValue().has_value() ? "Value" : "Default", IfcAttribute.GetValue().value().ToUtf8());
				}
			}
		}
		else
		{
			UE_AC_DebugF("FElement2String::GetIFCAttributesAsString - Error=%s IFCAPI::PropertyAccessor(*ElementObjectID).GetAttributes()\n", GetErrorName(IFCAttributes.UnwrapErr().kind));
		}
	}
	else
	{
		UE_AC_DebugF("FElement2String::GetIFCAttributesAsString - Error=%s IFCAPI::GetObjectAccessor().CreateElementObjectID\n", GetErrorName(ElementObjectID.UnwrapErr().kind));
	}
	#endif
...
const GS::UniString TrueString("True");
const GS::UniString FalseString("False");
const GS::UniString UnknownString("Unknown");
const GS::UniString SeparatorString(", ");

#if AC_VERSION > 27
GS::UniString FElement2String::GetValueTypePostfix(const GS::UniString& InValueType)
{
	return GS::UniString::Printf(" (%T)", InValueType.ToPrintf());
}

GS::UniString FElement2String::PropertyValueToString(const IFCAPI::v1::Value& InValue)
{
	GS::UniString result;

	auto anyValue = InValue.GetAnyValue();
	...
	if (!result.IsEmpty())
	{
		result.Append(GetValueTypePostfix(InValue.GetType()));
	}

	return result;
}

GS::UniString FElement2String::Concatenate(const GS::Array<GS::UniString>& InStrings, const GS::UniString& InSeparator)
{
	GS::UniString result;

	for (const auto& item : InStrings) 
	{
		if (!result.IsEmpty())
		{
			result.Append(InSeparator);
		}
...
GS::UniString FElement2String::ConcatenatePropertyValues(const std::vector<IFCAPI::Value>& InValues, const GS::UniString& InSeparator)
{
	...
	return Concatenate(valueStrings, InSeparator);
}

GS::UniString FElement2String::PropertyBoundedValueToString(const IFCAPI::PropertyBoundedValue& InBoundedValue)
{
	GS::UniString result;
	GS::UniString lowerBoundValue = PropertyValueToString(InBoundedValue.GetLowerBoundValue());
	GS::UniString upperBoundValue = PropertyValueToString(InBoundedValue.GetUpperBoundValue());
	GS::UniString setPointValue = PropertyValueToString(InBoundedValue.GetSetPointValue());
...
GS::UniString FElement2String::PropertyTableValueToString(const IFCAPI::PropertyTableValue& InTableValue)
{
	GS::Array<GS::UniString> names;
	auto definingValues = InTableValue.GetDefiningValues();
	auto definedValues = InTableValue.GetDefinedValues();
...
utf8_string FElement2String::GetIFCPropertiesAsString(const API_Guid& InElementGuid)
{
	...
	if (ElementObjectID.IsOk())
	{
		...
		{
			IFCPropertiesString += Utf8StringFormat("\t\tIFCProperties Nb=%zu\n", LocalProperties->size());
			for (const auto& Property : *LocalProperties)
			{
				IFCPropertiesString += Utf8StringFormat("\t\t\tSet=%s, Name=%s",
					Property.GetPropertySetName().ToUtf8(),
					Property.GetName().ToUtf8());
				...
				if (std::holds_alternative<IFCAPI::v1::PropertySingleValue>(propertyVariant)) 
				{
					...
					IFCPropertiesString += PropertyValueToString(propertyCast.GetNominalValue()).ToUtf8();
					IFCPropertiesString += "\n";
				}
				else if (std::holds_alternative<IFCAPI::v1::PropertyListValue>(propertyVariant)) 
				{
					...
					IFCPropertiesString += "{";
					IFCPropertiesString += ConcatenatePropertyValues(propertyCast.GetListValues(), SeparatorString).ToUtf8();
					IFCPropertiesString += "}\n";
				}
				else if (std::holds_alternative<IFCAPI::v1::PropertyBoundedValue>(propertyVariant)) 
				{
					...
					IFCPropertiesString += PropertyBoundedValueToString(propertyCast).ToUtf8();
					IFCPropertiesString += "\n";
				...
				else if (std::holds_alternative<IFCAPI::v1::PropertyEnumeratedValue>(propertyVariant)) 
				{
					...
					IFCPropertiesString += "(";
					IFCPropertiesString += Concatenate(names, SeparatorString).ToUtf8();
					IFCPropertiesString += ")\n";
				}
				else 
				{
					const auto& propertyCast = std::get<IFCAPI::v1::PropertyTableValue>(propertyVariant);
					IFCPropertiesString += PropertyTableValueToString(propertyCast).ToUtf8();
				}
			}
		}
		else
		{
			if (LocalProperties.UnwrapErr().kind != APIERR_BADPARS)
			{
	...
	{
		if (ElementObjectID.UnwrapErr().kind != APIERR_BADPARS)
		{
