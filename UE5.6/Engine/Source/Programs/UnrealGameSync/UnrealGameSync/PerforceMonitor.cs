...
#pragma warning restore CA2213
		...
		public PerforceMonitor(IPerforceSettings perforceSettings, ProjectInfo projectInfo, ConfigFile projectConfigFile, DirectoryReference cacheFolder, List<KeyValuePair<FileReference, DateTime>> localConfigFiles, IServiceProvider serviceProvider)
		{
			...
			_cloudStorage = serviceProvider.GetService<ICloudStorage>();
		...
		{
			List<BaseArchiveChannel> newArchives = await BaseArchive.EnumerateChannelsAsync(perforce, _hordeClient, _cloudStorage, LatestProjectConfigFile, _selectedProjectIdentifier, cancellationToken);
