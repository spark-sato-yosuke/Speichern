...
{
	partial class OpenProjectWindow : ThemedForm
	{
...
#pragma warning restore CS0649 // Avoid unused private fields
		...
		{
			Color activeTextColor = Theme.TextColor;
			Color inactiveTextColor = Theme.DimmedText;
			
			Color workspaceTextColor = WorkspaceRadioBtn.Checked? activeTextColor : inactiveTextColor;
			...
			Color localFileTextColor = LocalFileRadioBtn.Checked? activeTextColor : inactiveTextColor;
		...
		protected override void OnHandleCreated(EventArgs e)
		{
			...
			// Override the automatic theming colors for the enabled state
			UpdateEnabledControls();
		}
		...
		private void WorkspaceNewBtn_Click(object sender, EventArgs e)
		{
			...
			if (NewWorkspaceWindow.ShowModal(this, Perforce, null, WorkspaceNameTextBox.Text, _settings, _serviceProvider, out workspaceName))
			{
		...
		private void WorkspacePathBrowseBtn_Click(object sender, EventArgs e)
		{
			...
			if (TryGetWorkspaceName(out workspaceName))
			{
				...
				if (SelectProjectFromWorkspaceWindow.ShowModal(this, Perforce, workspaceName, workspacePath, _serviceProvider, _settings.Global.AllowUEFNProjects, out workspacePath))
				{
		...
		private void LocalFileBrowseBtn_Click(object sender, EventArgs e)
		{
			...
			string filter = "Project files (*.uproject)|*.uproject|Project directory lists (*.uprojectdirs)|*.uprojectdirs|All supported files (*.uproject;*.uprojectdirs)|*.uproject;*.uprojectdirs|All files (*.*)|*.*";

			if (_settings.Global.AllowUEFNProjects)
			{
				filter = "Project files (*.uproject;*.uefnproject)|*.uproject;*.uefnproject|Project directory lists (*.uprojectdirs)|*.uprojectdirs|All supported files (*.uproject;*.uefnproject;*.uprojectdirs)|*.uproject;*.uefnproject;*.uprojectdirs|All files (*.*)|*.*";
			}
