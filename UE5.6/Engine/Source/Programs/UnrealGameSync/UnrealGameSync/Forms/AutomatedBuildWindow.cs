...
{
	partial class AutomatedBuildWindow : ThemedForm
	{
		...
		private AutomatedBuildWindow(string streamName, int changelist, string command, IPerforceSettings defaultPerforceSettings, string? defaultWorkspaceName, string? defaultProjectPath, UserSettings settings, IServiceProvider serviceProvider)
		{
		...
		public static bool ShowModal(IWin32Window owner, IPerforceSettings defaultPerforceSettings, string streamName, string projectPath, int changelist, string command, UserSettings settings, IServiceProvider loggerFactory, [NotNullWhen(true)] out BuildInfo? buildInfo)
		{
			...
			using AutomatedBuildWindow window = new AutomatedBuildWindow(streamName, changelist, command, defaultPerforceSettings, defaultWorkspaceName, defaultProjectPath, settings, loggerFactory);
		...
		private void WorkspaceNameNewBtn_Click(object sender, EventArgs e)
		{
			...
			if (NewWorkspaceWindow.ShowModal(this, Perforce, _streamName, WorkspaceNameTextBox.Text, _settings, _serviceProvider, out workspaceName))
			{
