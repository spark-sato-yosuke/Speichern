			...
			try
			{
				...
				if (launcherSettings.UpdateSource == LauncherUpdateSource.Horde)
				{
					...
					if (syncText == null || syncText != requiredSyncText)
					{
						...
						await rootNode.ExtractAsync(new DirectoryInfo(applicationFolder), new ExtractOptions { Progress = new ExtractStatsLogger(0, rootNode.Length, logger) }, logger, cancellationToken);
