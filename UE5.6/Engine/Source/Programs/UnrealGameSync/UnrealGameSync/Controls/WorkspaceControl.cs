...
{
	partial class WorkspaceControl : UserControl, IMainWindowTabPanel, ICustomThemeHandler
	{
...
#pragma warning restore CA2213
		...
		readonly List<string> _authorsFilter = new List<string>();
		...
		public WorkspaceControl(IWorkspaceControlOwner owner, DirectoryReference appDataFolder, string? apiUrl, OpenProjectInfo openProjectInfo, IServiceProvider serviceProvider, UserSettings settings)
		{
			...
			if (projectConfigFile != null)
			{
				string? cloudHost = projectConfigFile.GetValue("CloudStorage.Host", null);
				string? cloudNamespace = projectConfigFile.GetValue("CloudStorage.Namespace", null);

				if (cloudHost != null && cloudNamespace != null && ProjectInfo.StreamName != null)
				{
					UpdateImportSnapshotDropdown(cloudHost, cloudNamespace, ProjectInfo.StreamName, ProjectInfo.LocalFileName.GetFileNameWithoutAnyExtensions());
					_settings.SnapshotHost = cloudHost;
					_settings.SnapshotNamespace = cloudNamespace;
				}
				else
				{
					OptionsContextMenu_ImportSnapshots.Enabled = false;
					_logger.LogWarning("Unable to determine cloud host and namespace from project's UnrealGameSync.ini. Specify Host=<host> and Namespace=<namespace> in the [CloudStorage] section of your UnrealGameSync.ini to enable snapshot import.");					
				}
			}
			...
			FilterContextMenu_Author_Name.TextBox.GotFocus += new System.EventHandler(FilterContextMenu_Author_Name_GotFocus);
			FilterContextMenu_Author_Name.TextBox.LostFocus += new System.EventHandler(FilterContextMenu_Author_Name_LostFocus);
			FilterContextMenu_Author_Name.TextBox.TextChanged += new System.EventHandler(FilterContextMenu_Author_Name_Changed);
		}
		...
		private void StreamChanged()
		{
			...
			// Reset the current CL we have synced and the current Code CL we have tracked 
			// since these no longer are accurate now we are targetting a new stream
		...
		void StartSync(int changeNumber, bool skipKnownChangeLists)
		{
			StartSync(changeNumber, false, null, skipKnownChangeLists);
		}

		void StartSync(int changeNumber, bool syncOnly, WorkspaceUpdateCallback? callback, bool skipKnownChangeLists)
		{
			WorkspaceUpdateOptions options = WorkspaceUpdateOptions.Sync | WorkspaceUpdateOptions.SyncArchives;

			_settings.Snapshots.Clear();

			foreach (ToolStripMenuItem menuItem in OptionsContextMenu_ImportSnapshots.DropDownItems)
			{
				if (menuItem.Checked)
				{
					if (menuItem.Tag is SnapshotSettings settings)
					{
						_settings.Snapshots.Add(settings);
					}
				}
			}

			if (_settings.Snapshots.Count > 0)
			{
				options |= WorkspaceUpdateOptions.ImportSnapshots;
			}
			
			if (_settings.SyncOptions.UseFastSync is true)
			{
			...
			if (!syncOnly)
			{
				if (_settings.GenerateProjectAfterSync)
				{
			...
			StartWorkspaceUpdate(changeNumber, options, callback, skipKnownChangeLists);
		}

		void StartWorkspaceUpdate(int changeNumber, WorkspaceUpdateOptions options, bool skipKnownChangeLists)
		{
			StartWorkspaceUpdate(changeNumber, options, null, skipKnownChangeLists);
		}

		void StartWorkspaceUpdate(int changeNumber, WorkspaceUpdateOptions options, WorkspaceUpdateCallback? callback, bool skipKnownChangeLists)
		{
			...
			if (options.HasFlag(WorkspaceUpdateOptions.SyncArchives))
			{
				...
				{
					IArchive? archivePath = GetArchiveForChangeNumber(archiveChannel, changeNumber, skipKnownChangeLists);
					if (archivePath == null)
					{
				...
				{
					if (!CanSyncChange(changeNumber, skipKnownChangeLists))
					{
			...
			if (options.HasFlag(WorkspaceUpdateOptions.ImportSnapshots))
			{
				context.Snapshots.AddRange(_settings.Snapshots);
				context.SnapshotHost = _settings.SnapshotHost;
				context.SnapshotNamespace = _settings.SnapshotNamespace;
			}
		...
		bool ShouldShowChange(ChangesRecord change, string[] excludeChanges)
		{
			...
			if( _authorsFilter.Any() )
			{
				// If none of the (potentially partial) username strings in the query are contained in the change's user, ignore this change
				if (!_authorsFilter.Any(username => change.User.Contains(username, StringComparison.OrdinalIgnoreCase)))
				{
					return false;
				}
			}
		...
		{
			return ConfigUtils.GetPresets(_perforceMonitor.LatestProjectConfigFile, SelectedProjectIdentifier);
		}
		...
		{
			Color textColor = ProgramApplicationContext.GetTheme().TextColor;
			TextRenderer.DrawText(graphics, layout._mainText, _buildFont, layout._mainRect, textColor, TextFormatFlags.SingleLine | TextFormatFlags.VerticalCenter | TextFormatFlags.NoPrefix | TextFormatFlags.NoPadding);

			Color linkColor = ProgramApplicationContext.GetTheme().LinkColor;
			if (_mouseOverExpandLink)
			{
				linkColor = ProgramApplicationContext.GetTheme().LinkColorHover;
			}
		...
		private IArchive? GetArchiveForChangeNumber(IArchiveChannel archiveChannel, int changeNumber, bool skipKnownChangeLists)
		{
			...
			return GetArchiveForChangeNumber(archiveChannel, changeNumber, changeNumber, changeNumberToArchive, skipKnownChangeLists);
		}

		private IArchive? GetArchiveForChangeNumber(IArchiveChannel archiveChannel, int changeNumber, int maxChangeNumber, Dictionary<int, IArchive?> changeNumberToArchive, bool skipKnownChangeLists)
		{
			IArchive? archive;
			if (!changeNumberToArchive.TryGetValue(changeNumber, out archive))
			{
				archive = skipKnownChangeLists
				? archiveChannel.TryGetArchiveForChangeNumber(changeNumber, maxChangeNumber)
				: GetArchiveForChangeNumberUncached(archiveChannel, changeNumber, maxChangeNumber);
		...
		private bool CanSyncChange(int changeNumber, bool skipKnownChangeLists)
		{
			if (_perforceMonitor == null)
			{
				return false;
			...
			bool returnValue = selectedArchives.Count == 0 || selectedArchives.All(x => GetArchiveForChangeNumber(x, changeNumber, skipKnownChangeLists) != null);
		...
		private void BuildList_DrawSubItem(object sender, DrawListViewSubItemEventArgs e)
		{
			...
			if (e.Item == _expandItem && e.ColumnIndex == 0)
			{
				// Draw the expand item only once, centered in the full row
				ExpandRowLayout expandLayout = LayoutExpandRow(e.Graphics, e.Item.Bounds);
			...
			bool allowSync = CanSyncChange(change.Number, false);
			int badgeAlpha = allowSync ? 255 : 128;
			Color baseTextColor = ProgramApplicationContext.GetTheme().TextColor;
			Color backgroundColor = ProgramApplicationContext.GetTheme().InputBackgroundColor;
			Color textColor = (allowSync || change.Number == _workspace.PendingChangeNumber || change.Number == _workspace.CurrentChangeNumber || _workspace.State.AdditionalChangeNumbers.Contains(change.Number)) ? baseTextColor : Blend(backgroundColor, baseTextColor, 0.25f);
			...
			if (e.ItemIndex >= BuildList.Items.Count - FadeRange && _numChanges >= _perforceMonitor.CurrentMaxChanges && !IsBisectModeEnabled())
			{
				...
				textColor = Blend(backgroundColor, textColor, opacity);
			}
			...
			else if (e.ColumnIndex == StatusColumn.Index)
			{
				...
				else
				{
					...
					else if (e.ItemIndex == BuildList.HoverItem && allowSync)
					{
						...
						DrawBadge(e.Graphics, syncBadgeRectangle, "Sync", _hoverSync ? ProgramApplicationContext.GetTheme().SyncBadgeColor : ProgramApplicationContext.GetTheme().SyncHoverBadgeColor, true, true);
		...
		public void UpdateSettings()
		{
			UpdateBuildSteps();
			UpdateStatusPanel();
			UpdatePerforceSettings();
		}

		// Note this should be changed to be done on a global level as it's unecessary to do this per workspace tab
		private void UpdatePerforceSettings()
		{
			((PerforceSettings)_perforceSettings).PreferNativeClient = _settings.SyncOptions.UseNativeClient ?? true;
		}
		...
		private List<BadgeInfo> CreateTypeBadges(int changeNumber)
		{
			...
			if (_perforceMonitor.TryGetChangeDetails(changeNumber, out details))
			{
				...
				{
					Color badgeColor = ProgramApplicationContext.GetTheme().CodeBadgeColor;
					badges.Add(new BadgeInfo("Code", "ChangeType", badgeColor));
				}
				if (details.ContainsContent)
				{
					Color badgeColor = ProgramApplicationContext.GetTheme().ContentBadgeColor;
					badges.Add(new BadgeInfo("Content", "ChangeType", badgeColor));
				}
			}
			if (badges.Count == 0)
			{
		...
		private BadgeInfo CreateBadge(int changeNumber, string badgeName, string badgeGroup, BadgeData? badgeData, int maxBadgeChars = -1)
		{
			...
			else
			{
				rightClickHandler = (control, point) =>
				{
					...
					ProgramApplicationContext.GetThemeManager()?.ApplyThemeRecursively(BadgeContextMenu);
				};
			}
		...
		private void DrawBadge(Graphics graphics, Rectangle badgeRect, string badgeText, Color badgeColor, bool mergeLeft, bool mergeRight)
		{
			if (badgeColor.A != 0)
			{
				...
				ApplicationThemeManager? manager = ProgramApplicationContext.GetThemeManager();
				if ( manager != null && manager.Theme.UseDarkMode )
				{
					textColor = ApplicationTheme.IsColorLight(badgeColor) ? Color.Black : Color.White;
				}
				
				TextRenderer.DrawText(graphics, badgeText, _badgeFont, badgeRect, textColor, TextFormatFlags.HorizontalCenter | TextFormatFlags.VerticalCenter | TextFormatFlags.SingleLine | TextFormatFlags.NoPrefix | TextFormatFlags.PreserveGraphicsClipping);
			}
		}
			...
			if (args.Button == MouseButtons.Left)
			{
				...
				if (hitTest.Item != null)
				{
					...
					if (change != null)
					{
						...
						{
							StartSync(change.Number, false, null, false);
						}
					}
				}
			}
...
#pragma warning restore CA2000
			...
			ProgramApplicationContext.GetThemeManager()?.ApplyThemeRecursively(BuildHealthContextMenu);
		}
...
#pragma warning restore CA2000 // Dispose objects before losing scope
		...
		private void ShowToolContextMenu(Rectangle bounds, ContextMenuStrip menuStrip)
		{
			...
			ProgramApplicationContext.GetThemeManager()?.ApplyThemeRecursively(menuStrip);
		}
		...
		private void ShowActionsMenu(Rectangle bounds)
		{
			...
			ProgramApplicationContext.GetThemeManager()?.ApplyThemeRecursively(MoreToolsContextMenu);
		}
				...
				if (hitTest.Item != null)
				{
					...
					else
					{
						...
						else
						{
							...
							if (syncBadgeRectangle.Contains(args.Location) && CanSyncChange(change.Number, false))
							{
								StartSync(change.Number, false, null, false);
							}
						}
			...
			else if (args.Button == MouseButtons.Right)
			{
				...
				if (hitTest.Item != null && hitTest.Item.Tag != null)
				{
					if (BuildList.SelectedItems.Count > 1 && BuildList.SelectedItems.Contains(hitTest.Item))
					{
						...
						ProgramApplicationContext.GetThemeManager()?.ApplyThemeRecursively(BuildListMultiContextMenu);
					}
					else
					{
...
#pragma warning restore CA2000
				...
				ProgramApplicationContext.GetThemeManager()?.ApplyThemeRecursively(BuildListContextMenu);
			}
		}
					...
					if (layoutInfo._buildBadges.Count > 0)
					{
						...
						if (_hoverBadgeUniqueId != newHoverBadgeUniqueId)
						{
							if (badge?._toolTip != null && badge._backgroundColor.A != 0)
							{
								...
								ProgramApplicationContext.GetThemeManager()?.ApplyThemeRecursively(BuildListToolTip);
							}
							else
							{
		...
		private void OptionsButton_Click(object sender, EventArgs e)
		{
			...
			ProgramApplicationContext.GetThemeManager()?.ApplyThemeRecursively(OptionsContextMenu);
		}
		...
		private void GenerateAfterSyncCheckBox_CheckedChanged(object sender, EventArgs e)
		{
			_settings.GenerateProjectAfterSync = GenerateAfterSyncCheckBox.Checked;
			_settings.Save(_logger);
		...
		private void BuildListToolTip_Draw(object sender, DrawToolTipEventArgs e)
		{
			e.DrawBackground();
			e.DrawBorder();
			e.DrawText(TextFormatFlags.LeftAndRightPadding | TextFormatFlags.VerticalCenter);
		}
		...
		{
			GenerateAfterSyncCheckBox.Checked = _settings.GenerateProjectAfterSync;
			...
			ApplicationTheme theme = ProgramApplicationContext.GetTheme();

			if (_workspace == null || _workspace.IsBusy())
			{
				AfterSyncingLabel.ForeColor = theme.DimmedText;
				GenerateAfterSyncCheckBox.ReadOnly = true;
				BuildAfterSyncCheckBox.ReadOnly = true;
				RunAfterSyncCheckBox.ReadOnly = true;
				OpenSolutionAfterSyncCheckBox.ReadOnly = true;
			}
			else
			{
				AfterSyncingLabel.ForeColor = theme.TextColor;
				GenerateAfterSyncCheckBox.ReadOnly = !_hasBuildSteps;
				BuildAfterSyncCheckBox.ReadOnly = !_hasBuildSteps;
				RunAfterSyncCheckBox.ReadOnly = !BuildAfterSyncCheckBox.Checked && !ShouldSyncPrecompiledEditor;
				OpenSolutionAfterSyncCheckBox.ReadOnly = ShouldSyncPrecompiledEditor;
			}
		}

		private void UpdateImportSnapshotDropdown(string host, string ns, string branch, string project, bool allowInteractiveLogin = false)
		{
			OptionsContextMenu_ImportSnapshots.DropDownItems.Clear();

			try
			{
				Task<UnrealCloudDDCBuildIndex> buildIndexTask = Task.Run(() => UnrealCloudDDCBuildIndex.CreateBuildIndexAsync(host, ns, (app, cmdline) => { }, allowInteractiveLogin));
				buildIndexTask.Wait();

				if (!buildIndexTask.IsCompletedSuccessfully)
				{
					OptionsContextMenu_ImportSnapshots.DropDownItems.Add(new ToolStripMenuItem("Unable to determine valid platforms for snapshot import"));
					return;
				}

				UnrealCloudDDCBuildIndex buildIndex = buildIndexTask.Result;
				List<string> cookPlatforms = buildIndex.GetAvailablePlatforms(branch, project);

				foreach (string cookPlatform in cookPlatforms)
				{
					string runtime = cookPlatform.EndsWith("client", StringComparison.OrdinalIgnoreCase) ? "client"
						: cookPlatform.EndsWith("server", StringComparison.OrdinalIgnoreCase) ? "server"
						: "game";

					string platform = cookPlatform.Replace(runtime, "", StringComparison.OrdinalIgnoreCase);

					TextInfo textInfo = new CultureInfo("en-US").TextInfo;

					ToolStripMenuItem item = new ToolStripMenuItem(textInfo.ToTitleCase(platform + " " + runtime));
					item.Enabled = true;
					item.CheckOnClick = true;
					item.Tag = new SnapshotSettings { Platform = platform, Runtime = runtime };

					OptionsContextMenu_ImportSnapshots.DropDown.Closing += (sender, eventArgs) =>
					{
						ToolStripDropDown dropDown = (ToolStripDropDown)sender!;
						Point p = dropDown.PointToClient(Control.MousePosition);
						if (dropDown.DisplayRectangle.Contains(p))
						{
							eventArgs.Cancel = true;
						}
					};

					OptionsContextMenu_ImportSnapshots.DropDownItems.Add(item);
				}
			}
			catch (AggregateException e)
			{
				if (e.GetBaseException() is NotLoggedInException)
				{
					ToolStripMenuItem item = new ToolStripMenuItem("Populate available platforms");
					item.Enabled = true;
					item.Click += (sender, eventArgs) =>
					{
						UpdateImportSnapshotDropdown(host, ns, branch, project, true);
					};
		...
		public void SyncChange(int changeNumber, bool syncOnly, WorkspaceUpdateCallback? callback, bool skipKnownChangeLists)
		{
			if (_workspace != null)
			{
				_owner.ShowAndActivate();
				SelectChange(changeNumber);
				StartSync(changeNumber, syncOnly, callback, skipKnownChangeLists);
			}
		}
		...
		public void SyncLatestChange(WorkspaceUpdateCallback? callback)
		{
			if (_workspace != null)
			{
				...
				{
					SyncChange(changeNumber, false, callback, false);
				}
			}
		}
		...
		private void BuildListContextMenu_Sync_Click(object sender, EventArgs e)
		{
			if (_contextMenuChange != null)
			{
				StartSync(_contextMenuChange.Number, false);
			}
		}
		...
		private void BuildListContextMenu_SyncContentOnly_Click(object sender, EventArgs e)
		{
			if (_contextMenuChange != null)
			{
				StartWorkspaceUpdate(_contextMenuChange.Number, WorkspaceUpdateOptions.Sync | WorkspaceUpdateOptions.ContentOnly, false);
			}
		}
		...
		private void BuildListContextMenu_SyncOnlyThisChange_Click(object sender, EventArgs e)
		{
			if (_contextMenuChange != null)
			{
				StartWorkspaceUpdate(_contextMenuChange.Number, WorkspaceUpdateOptions.SyncSingleChange, false);
			}
		}

		private void BuildListContextMenu_Build_Click(object sender, EventArgs e)
		{
			StartWorkspaceUpdate(_workspace.CurrentChangeNumber, WorkspaceUpdateOptions.SyncArchives | WorkspaceUpdateOptions.Build, false);
		}

		private void BuildListContextMenu_Rebuild_Click(object sender, EventArgs e)
		{
			StartWorkspaceUpdate(_workspace.CurrentChangeNumber, WorkspaceUpdateOptions.SyncArchives | WorkspaceUpdateOptions.Build | WorkspaceUpdateOptions.Clean, false);
		}

		private void BuildListContextMenu_GenerateProjectFiles_Click(object sender, EventArgs e)
		{
			StartWorkspaceUpdate(_workspace.CurrentChangeNumber, WorkspaceUpdateOptions.GenerateProjectFiles, false);
		}
		...
		public void ScheduleTimerElapsed()
		{
			if (_settings.ScheduleEnabled)
			{
				...
				else
				{
					...
					StartWorkspaceUpdate(changeNumber, options, false);
				}
			}
		}
		...
		bool FindChangeToSync(LatestChangeType changeType, out int changeNumber)
		{
			...
			for (int idx = 0; idx < BuildList.Items.Count; idx++)
			{
				...
				{
					EventSummary? summary = _eventMonitor.GetSummaryForChange(change.Number);
					if (CanSyncChange(change.Number, false)
		...
		int FindNewestGoodContentChange(int changeNumber)
		{
			...
			for (int nextIndex = index + 1; nextIndex < _sortedChangeNumbers.Count; nextIndex++)
			{
				...
				if (!CanSyncChange(nextChangeNumber, false))
				{
					break;
				}
		...
		private void UpdateBuildSteps()
		{
			...
			if (_workspace != null)
			{
				...
				if (projectConfigFile != null)
				{
					...
					Dictionary<string, ContextMenuStrip>  nameToMenu         = new Dictionary<string, ContextMenuStrip>(StringComparer.OrdinalIgnoreCase);
					Dictionary<string, ToolStripMenuItem> overflowNameToMenu = new Dictionary<string, ToolStripMenuItem>(StringComparer.OrdinalIgnoreCase);
					foreach (BuildStep step in userSteps)
					{
						if (!String.IsNullOrEmpty(step.StatusPanelLink) && CanRunStep(step))
						{
							bool containsSegment = step.StatusPanelLink.Contains('|', StringComparison.Ordinal);
							if (!containsSegment)
							{
							...
							else
							{
								...
								string[] segments       = step.StatusPanelLink.Split('|');
								int lastSegmentStartIdx = step.StatusPanelLink.LastIndexOf('|');
...
#pragma warning disable CA2000 // warning CA2000: Use recommended dispose pattern to ensure that object created by 'new ToolStripMenuItem(itemName)' is disposed on all paths. If possible, wrap the creation within a 'using' statement or a 'using' declaration. Otherwise, use a try-finally pattern, with a dedicated local variable declared before the try region and an unconditional Dispose invocation on non-null value in the 'finally' region, say 'x?.Dispose()'. If the object is explicitly disposed within the try region or the dispose ownership is transfered to another object or method, assign 'null' to the local variable just after such an operation to prevent double dispose in 'finally'.
								...
								// Create the menu item we'll add to the view
								string itemName = segments[segments.Length - 1].Replace("&", "&&", StringComparison.OrdinalIgnoreCase); // Tool "Name" will be the last part of the segment always
								...
								// Where the item ends up depends on the first segment text
								if (segments[0] == "More...")
								{
									// Overflow Menu
									ToolStripMenuItem? curParentItem = null;

									// Walk the segments to find the parent menu, creating missing records along the way
									string combinedSegmentKey = "";
									for( int idx = 1; idx < segments.Length - 1; ++idx) // Skip the first segment since we know it's `More...`, and stop before the last segment since it's the name
									{
										combinedSegmentKey += segments[idx];
										string segmentItemName = segments[idx].Replace("&", "&&", StringComparison.OrdinalIgnoreCase);

										ToolStripMenuItem? foundParentItem = null;
										if (!overflowNameToMenu.TryGetValue(combinedSegmentKey, out foundParentItem))
										{
											// Missing this segment
											foundParentItem = new ToolStripMenuItem(segmentItemName);
											overflowNameToMenu.Add(combinedSegmentKey, foundParentItem);
											if(curParentItem != null)
											{
												curParentItem.DropDownItems.Add(foundParentItem);
											}
											else
											{
												MoreToolsContextMenu.Items.Insert(moreToolsItemCount++, foundParentItem);
											}
										}
										curParentItem = foundParentItem;
									}

									if (curParentItem != null)
									{
										curParentItem.DropDownItems.Add(newMenuItem);
									}
									else
									{
										MoreToolsContextMenu.Items.Insert(moreToolsItemCount++, newMenuItem); // No parent item, add it to the root
									}
								}
								else
								{
									...
									string menuName = segments[0]; // Not in the overflow menu, so the first segment is the menu name
...
#pragma warning restore CA2000
		...
		private void ShowSyncMenu(Rectangle bounds)
		{
			...
			ProgramApplicationContext.GetThemeManager()?.ApplyThemeRecursively(SyncContextMenu);
		}
		...
		private void SyncContextMenu_EnterChangelist_Click(object sender, EventArgs e)
		{
			...
			{
				StartSync(childWindow.ChangeNumber, true);
			}
		}
...
#pragma warning restore CA2000
			...
			ProgramApplicationContext.GetThemeManager()?.ApplyThemeRecursively(RecentMenu);
		}
		...
		private void FilterButton_Click(object sender, EventArgs e)
		{
			...
			ProgramApplicationContext.GetThemeManager()?.ApplyThemeRecursively(FilterContextMenu);
		}
		...
		private void FilterContextMenu_Author_Name_GotFocus(object? sender, EventArgs e)
		{
			if (FilterContextMenu_Author_Name.Text == AuthorFilterPlaceholderText)
			{
				...
				FilterContextMenu_Author_Name.ForeColor = Color.Black;
		...
		private void FilterContextMenu_Author_Name_LostFocus(object? sender, EventArgs e)
		{
			...
			{
				FilterContextMenu_Author_Name.Text = AuthorFilterPlaceholderText;
				FilterContextMenu_Author_Name.ForeColor = Color.DarkGray;
		...
		private void FilterContextMenu_Author_Name_Changed(object? sender, EventArgs e)
		{
			...
			if (!String.IsNullOrWhiteSpace(_authorFilterText) && _authorFilterText != AuthorFilterPlaceholderText)
			{
				_authorsFilter.Clear();
				_authorsFilter.AddAll( _authorFilterText.Split((char[])[';', '+'], StringSplitOptions.TrimEntries));
			}
			else
			{
				_authorsFilter.Clear();
		...
		{
			themeManager.DefaultApplyThemeToControl(this);

			BuildListToolTip.BackColor = themeManager.Theme.InputBackgroundColor;
			BuildListToolTip.ForeColor = themeManager.Theme.TextColor;
