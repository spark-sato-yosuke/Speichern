...
namespace EpicGames.Core
{
	...
	/// </summary>
	[TypeConverter(typeof(FileFilterConverter))]
	public class FileFilter
	{
		...
		/// </summary>
		readonly List<FileFilterNode> _rules = [];
		...
		/// <summary>
		/// Parse a filter pattern, determining the default rule type from the initial rule
		/// </summary>
		/// <param name="text">Sequence of patterns to parse. Entries may be separated by semicolons.</param>
		public static FileFilter? Parse(string? text)
		{
			FileFilter? filter = null;
			if (text != null)
			{
				string[] rules = text.Split(';', StringSplitOptions.RemoveEmptyEntries | StringSplitOptions.TrimEntries);
				filter = Parse(rules);
			}
			return filter;
		}
		...
		public static FileFilter? Parse(IReadOnlyList<string>? rules)
		{
			FileFilter? filter = null;
			if (rules != null && rules.Count > 0)
			{
				FileFilterType defaultType = rules[0].StartsWith("-", StringComparison.Ordinal) ? FileFilterType.Include : FileFilterType.Exclude;
				filter = new FileFilter(defaultType);
		...
		public bool PossiblyMatches(string folderName)
		{
			...
			FileFilterNode matchingNode = FindMatchingNode(_rootNode, tokens.Union([""]).ToArray(), 0, _defaultNode);
		...
		public List<FileReference> ApplyToDirectory(DirectoryReference directoryName, bool bIgnoreSymlinks)
			=> ApplyToDirectory(directoryName.ToDirectoryInfo(), bIgnoreSymlinks).ConvertAll(x => new FileReference(x));

		/// <summary>
		/// Finds a list of files within a given directory which match the filter.
		/// </summary>
		/// <param name="directoryInfo">File to match</param>
		/// <param name="bIgnoreSymlinks">Whether to ignore symlinks in the output</param>
		/// <returns>List of files that pass the filter</returns>
		public List<FileInfo> ApplyToDirectory(DirectoryInfo directoryInfo, bool bIgnoreSymlinks)
		{
			List<FileInfo> matchingFileNames = [];
			if (directoryInfo.Exists)
			{
				FindMatchesFromDirectory(directoryInfo, "", bIgnoreSymlinks, matchingFileNames);
			}
		...
		public List<FileReference> ApplyToDirectory(DirectoryReference directoryName, string prefixPath, bool bIgnoreSymlinks)
			=> ApplyToDirectory(directoryName.ToDirectoryInfo(), prefixPath, bIgnoreSymlinks).ConvertAll(x => new FileReference(x));
		...
		/// <param name="bIgnoreSymlinks">Whether to ignore symlinks in the output</param>
		/// <returns>List of files that pass the filter</returns>
		public List<FileInfo> ApplyToDirectory(DirectoryInfo directoryInfo, string prefixPath, bool bIgnoreSymlinks)
		{
			List<FileInfo> matchingFileNames = [];
			if (directoryInfo.Exists)
			{
				FindMatchesFromDirectory(directoryInfo, NormalizePath(prefixPath).TrimEnd('/') + "/", bIgnoreSymlinks, matchingFileNames);
			}
			return matchingFileNames;
		}
		...
		/// <returns>True if the file passes the filter</returns>
		void FindMatchesFromDirectory(DirectoryInfo currentDirectory, string namePrefix, bool bIgnoreSymlinks, List<FileInfo> matchingFileNames)
		{
			foreach (FileInfo nextFile in currentDirectory.EnumerateFiles())
			{
				...
				{
					matchingFileNames.Add(nextFile);
				}
			}
	...
	/// <summary>
	/// Type converter for FileFilter objects. Allows parsing filters from command line arguments.
	/// </summary>
	class FileFilterConverter : TypeConverter
	{
		/// <inheritdoc/>
