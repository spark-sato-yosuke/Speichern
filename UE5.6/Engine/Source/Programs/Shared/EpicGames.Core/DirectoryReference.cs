...
namespace EpicGames.Core
{
	...
	public class DirectoryReference : FileSystemReference, IEquatable<DirectoryReference>, IComparable<DirectoryReference>
	{
		...
		public static DirectoryReference GetTempPath() => new DirectoryReference(Path.GetTempPath());
		...
		/// <param name="sourceDir">The source directory</param>
		/// <param name="destDir">The destination directory</param>
		public static void Copy(DirectoryReference sourceDir, DirectoryReference destDir)
		{
			CreateDirectory(destDir);
			
			foreach (string file in Directory.GetFiles(sourceDir.FullName))
			{
				File.Copy(file, Path.Combine(destDir.FullName, Path.GetFileName(file)), true);
			}
			
			foreach (string directory in Directory.GetDirectories(sourceDir.FullName))
			{
				Copy(new DirectoryReference(directory), Combine(destDir, Path.GetFileName(directory)));
			}
		}
		
		/// <summary>
		/// Calculates the total size in bytes of all files in a directory and its subdirectories
		/// </summary>
		/// <param name="dirPath">Path to directory</param>
		/// <returns>The total size in bytes of all accessible files in the directory and its subdirectories</returns>
		public static long GetTotalSize(DirectoryReference dirPath)
		{
			long size = 0;
			foreach (FileReference file in EnumerateFiles(dirPath, "*", SearchOption.AllDirectories))
			{
				try
				{
					size += file.ToFileInfo().Length;
				}
