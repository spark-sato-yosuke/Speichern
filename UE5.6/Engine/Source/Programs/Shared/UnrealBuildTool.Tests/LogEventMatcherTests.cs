	...
	public class LogEventMatcherTests
	{
		...
		class LoggerCapture : ILogger
		{
			...
			public void Log<TState>(LogLevel logLevel, EventId eventId, TState state, Exception? exception, Func<TState, Exception?, string> formatter)
			{
				...
				{
					KeyValuePair<string, object?>[] items = [new KeyValuePair<string, object?>(LogLine, _logLineIndex)];
		...
		[TestMethod]
		public void LinkEventMatcher5()
		{
			string[] lines =
			[
				@"  lld-link: error: Malformed block",
			];

			List<LogEvent> logEvents = Parse(String.Join("\n", lines));
			Assert.AreEqual(1, logEvents.Count);
			CheckEventGroup(logEvents, 0, 1, LogLevel.Error, KnownLogEvents.Linker);
		}
		...
		static List<LogEvent> Parse(string text)
		{
			...
			{
				parser.AddMatchersFromAssembly(typeof(UnrealBuildTool).Assembly);
