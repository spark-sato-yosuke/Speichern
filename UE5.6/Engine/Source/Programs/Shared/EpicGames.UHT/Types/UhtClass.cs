...
namespace EpicGames.UHT.Types
{
	...
	public class UhtClass : UhtStruct
	{
		...
		/// <summary>
		/// Line number containing the VINTERFACES macro
		/// </summary>
		public int VerseInterfacesLine { get; set; } = 0;

		/// <summary>
		/// Verse interfaces directly specified on the interface definition
		/// </summary>
		[JsonConverter(typeof(UhtTypeReadOnlyListSourceNameJsonConverter<UhtClass>))]
		public IReadOnlyList<UhtClass> VerseInterfaces => _verseInterfaces ?? s_emptyVerseInterfaces;

		/// <summary>
		/// Flattened collection of all verse interfaces. 
		/// </summary>
		[JsonConverter(typeof(UhtTypeReadOnlyListSourceNameJsonConverter<UhtClass>))]
		public IReadOnlyList<UhtClass> FlattenedVerseInterfaces => _flattenedVerseInterfaces ?? s_emptyVerseInterfaces;

		/// <summary>
		/// List of all verse interfaces implemented by this interface
		/// </summary>
		[JsonIgnore]
		[SuppressMessage("Usage", "CA2227:Collection properties should be read only", Justification = "<Pending>")]
		public List<UhtToken>? VerseInterfaceIdentifiers { get; set; } = null;

		private static readonly List<UhtClass> s_emptyVerseInterfaces = new();
		private List<UhtClass>? _verseInterfaces = null;
		private List<UhtClass>? _flattenedVerseInterfaces = null;
		...
		protected override void ResolveSuper(UhtResolvePhase resolvePhase)
		{
			...
			if (_verseInterfaces != null)
			{
				foreach (UhtClass verseInterface in _verseInterfaces)
				{
					verseInterface.ResolveSuper(resolvePhase);
				}
			}
			...
			switch (resolvePhase)
			{
				case UhtResolvePhase.Bases:
					...
					// Roll up the verse interfaces
					if (_verseInterfaces != null)
					{
						_flattenedVerseInterfaces = [];
						_flattenedVerseInterfaces.AddRange(_verseInterfaces);
						int stopIndex = _flattenedVerseInterfaces.Count;
						for (int index = 0; index < stopIndex; index++)
						{
							UhtClass verseInterface = _flattenedVerseInterfaces[index];
							if (verseInterface._flattenedVerseInterfaces != null)
							{
								foreach (UhtClass subInterface in verseInterface._flattenedVerseInterfaces)
								{
									if (!_flattenedVerseInterfaces.Contains(subInterface))
									{
										_flattenedVerseInterfaces.Add(subInterface);
									}
								}
							}
						}
					...
					switch (ClassType)
					{
						case UhtClassType.Class:
							{
								// Validate some verse settings
								if (!IsVerseField && VerseInterfacesLine != 0)
								{
									this.LogError($"VINTERFACES can only appear of classes marked as Verse classes");
								}
								...
								foreach (UhtStruct baseStruct in Bases)
								{
									if (baseStruct is UhtClass baseClass)
									{
										...
										if (baseClass.IsVerseField)
										{
											if (IsVerseField)
											{
												this.LogError($"Interface '{baseClass.SourceName}' must be listed in a VINTERFACES() macro");
											}
											else
											{
												this.LogError($"Non-Verse classes can not inherit the Verse interface '{baseClass.SourceName}");
											}
										}
						...
						case UhtClassType.NativeInterface:
							if (AlternateObject is UhtClass interfaceObj && interfaceObj.IsVerseField)
							{
								if (superClass != Session.IInterface)
								{
									this.LogError($"Verse native interface must derived from IInterface");
								}
							}
							break;
		...
		private void ValidateVerseInterfaces()
		{
			foreach (UhtClass verseInterface in VerseInterfaces)
			{
				if (verseInterface.ClassType != UhtClassType.NativeInterface)
				{
					this.LogError($"Class '{verseInterface.SourceName}' is not a native interface");
				}
				if (!verseInterface.IsVerseField)
				{
					this.LogError($"Class '{verseInterface.SourceName}' is not a Verse interface");
				}
			}
		}

		/// <summary>
		/// Return the native interface name from an interface
		/// </summary>
		/// <returns>Native interface name</returns>
		/// <exception cref="UhtIceException"></exception>
		public string GetNativeInterfaceName()
		{
			if (ClassType != UhtClassType.Interface)
			{
				throw new UhtIceException("Can only get the native interface name from interfaces");
			}
			return "I" + SourceName[1..];
		}

		/// <summary>
		/// Return the interface name from a native interface
		/// </summary>
		/// <returns>Interface name</returns>
		/// <exception cref="UhtIceException"></exception>
		public string GetInterfaceName()
		{
			if (ClassType != UhtClassType.NativeInterface)
			{
				throw new UhtIceException("Can only get the interface name from native interfaces");
			}
			return "U" + SourceName[1..];
		}
			...
			switch (phase)
			{
				case UhtResolvePhase.InvalidCheck:
					switch (ClassType)
					{
						...
						case UhtClassType.Interface:
							{
								string nativeInterfaceName = GetNativeInterfaceName();
...
#pragma warning restore IDE0059 // Unnecessary assignment of a value
		...
		protected override UhtValidationOptions Validate(UhtValidationOptions options)
		{
			...
			if (SuperClass != null)
			{
				...
				if (ClassType == UhtClassType.Interface || ClassType == UhtClassType.NativeInterface)
				{
					if (IsVerseField)
					{
						if (ClassType == UhtClassType.Interface && SuperClass != Session.UInterface)
						{
							this.LogError($"Verse Interface class '{SourceName}' must inherit from UInterface'");
						}
						else if (ClassType == UhtClassType.NativeInterface && SuperClass != Session.IInterface)
						{
							this.LogError($"Verse Native Interface class '{SourceName}' must inherit from IInterface'");
						}
					}
					else if (this != Session.UInterface && !SuperClass.ClassFlags.HasAnyFlags(EClassFlags.Interface))
					{
			...
			else
			{
				if (ClassType == UhtClassType.Interface)
				{
					this.LogError($"Interface '{SourceName}' must derive from an interface or UInterface");
				}
			}
				...
				case UhtClassType.Interface:
					{
						...
						// Verify that our supers match
						if (NativeInterface != null && SuperClass != null)
						{
							if (SuperClass == Session.UInterface)
							{
								if (NativeInterface.SuperClass != null && NativeInterface.SuperClass != Session.IInterface)
								{
									this.LogError($"Interface {SourceName} derives from {SuperClass.SourceName}, therefore {NativeInterface.SourceName} can not derive from an another native interface");
								}
							}
							else if (SuperClass.NativeInterface != null)
							{
								if (SuperClass.NativeInterface != NativeInterface.SuperClass)
								{
									this.LogError($"Interface {SourceName} derives from {SuperClass.SourceName}, therefore {NativeInterface.SourceName} must derive from {SuperClass.NativeInterface.SourceName}");
								}
							}
						}
		...
		public override void CollectReferences(IUhtReferenceCollector collector)
		{
			...
			foreach (UhtStruct baseObj in Bases)
			{
				if (baseObj is UhtClass baseClass)
				{
					collector.AddCrossModuleReference(baseClass, false);
				}
			}

			// Add any verse interfaces
			foreach (UhtStruct interfaceObj in FlattenedVerseInterfaces)
			{
				collector.AddCrossModuleReference(interfaceObj, false);
			}
		...
		private void BindVerseInterfaces()
		{
			if (VerseInterfaceIdentifiers != null)
			{
				foreach (UhtToken identifier in VerseInterfaceIdentifiers)
				{
					// We really only case about interfaces, but we can also handle structs
					UhtToken scratch = identifier;
					UhtClass? verseInterface = (UhtClass?)FindType(UhtFindOptions.Class | UhtFindOptions.SourceName | UhtFindOptions.NoSelf, ref scratch);
					if (verseInterface != null)
					{
						_verseInterfaces ??= new List<UhtClass>();
						_verseInterfaces.Add(verseInterface);
					}
					else
					{
						this.LogError($"Verse interface {identifier.Value} not found");
					}
				}
			}
		}
