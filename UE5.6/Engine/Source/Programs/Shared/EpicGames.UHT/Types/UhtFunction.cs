...
namespace EpicGames.UHT.Types
{
	...
	public enum UhtFunctionExportFlags : int
	{
		...
		/// <summary>
		/// True if the function is verse function callable from native code
		/// </summary>
		VerseNativeCallable = 1 << 18,

		/// <summary>
		/// True if the function is a decides function
		/// </summary>
		VerseDecides = 1 << 19,

		/// <summary>
		/// True if the return value is wrapped with TVerseTask
		/// </summary>
		VerseSuspends = 1 << 20,

		/// <summary>
		/// Function can be called from verse
		/// </summary>
		VerseCallable = 1 << 21,

		/// <summary>
		/// True if the function is a no_rollback function
		/// </summary>
		VerseNoRollback = 1 << 22,

		/// <summary>
		/// True if the function is marked as rtfm_always_open
		/// </summary>
		VerseRTFMAlwaysOpen = 1 << 23,
	}
	...
	public class UhtFunction : UhtStruct
	{
		...
		/// <inheritdoc/>
		protected override void AppendVersePathPart(StringBuilder builder, UhtVerseFullNameMode mode)
		{
			// suspends methods don't get the path part added to the name
			if (!FunctionExportFlags.HasAnyFlags(UhtFunctionExportFlags.VerseSuspends))
			{
				base.AppendVersePathPart(builder, mode);
			}
		}

		/// <inheritdoc/>
		protected override void AppendVerseName(StringBuilder builder, UhtVerseFullNameMode mode)
		{
			UhtProperty? returnProperty = ReturnProperty;
			ReadOnlyMemory<UhtType> parameterProperties = ParameterProperties;
			if (Session.IsIncompleteReturn(returnProperty))
			{
				parameterProperties = parameterProperties[..^1];
			}

			builder.Append(VerseName);
			if (!parameterProperties.IsEmpty)
			{
				bool needsWhereClause = false;
				bool isFirst = true;
				builder.Append('(');
				foreach (UhtType type in parameterProperties.Span)
				{
					if (type is UhtProperty property)
					{
						if (!isFirst)
						...
						builder.Append(':').AppendPropertyVerseMangledType(property);
						isFirst = false;
						if (!needsWhereClause)
						{
							needsWhereClause = type is UhtObjectPropertyBase objectProperty && objectProperty.CppForm.NeedsWhereClause();
						}
					}
				}
				...
				if (needsWhereClause)
				{
					...
					foreach (UhtType type in parameterProperties.Span)
					{
						if (type is UhtObjectPropertyBase objectProperty && objectProperty.CppForm.NeedsWhereClause())
						{
							if (!isFirst)
							{
								builder.Append(',');
							}
							builder.Append(' ').Append(objectProperty.SourceName);
							isFirst = false;
						}
					}
				}
		...
		protected override bool ResolveSelf(UhtResolvePhase phase)
		{
			...
			switch (phase)
			{
				...
				case UhtResolvePhase.Final:
					...
					foreach (UhtType type in Children)
					{
						if (type is not UhtProperty property)
						{
							continue;
						}
						...
						if (property is UhtStructProperty structProperty && Outer is UhtClass)
						{
		...
		protected override UhtValidationOptions Validate(UhtValidationOptions options)
		{
			...
			if (DefineScope.HasAnyFlags(UhtDefineScope.Tests) && !Outer!.DefineScope.HasAnyFlags(UhtDefineScope.Tests))
			{
				this.LogError($"Functions are not supported within a {UhtNames.WithTests} scope.");
			}
			...
			switch (FunctionType)
			{
				case UhtFunctionType.Function:
					...
					if (IsVerseField)
					{
						if (FunctionExportFlags.HasAnyFlags(UhtFunctionExportFlags.VerseDecides))
						{
							UhtProperty? returnProperty = ReturnProperty;
							if (returnProperty == null || returnProperty is not UhtOptionalProperty)
							{
								this.LogError($"Verse function '{SourceName}' marked as 'decides' must have a 'TOptional<...>' return value");
							}							
						}

						if (Outer is UhtClass parent && !parent.IsVerseField)
						{
							this.LogError($"Function '{SourceName}' is marked as a Verse function but appears in a class that isn't a Verse class");
						}

						if (!FunctionExportFlags.HasAnyFlags(UhtFunctionExportFlags.VerseNativeCallable | UhtFunctionExportFlags.VerseCallable))
						{
							this.LogError($"Verse function '{SourceName}' must either be marked as NativeCallable or VerseCallable");
						}

						if (VersePath == SourceName)
						{
							this.LogError($"Verse function '{SourceName}' has the same name as the Verse generated name.  Change the source name and then specify the name in the UFUNCTION attributes");
						}
					}
					break;
