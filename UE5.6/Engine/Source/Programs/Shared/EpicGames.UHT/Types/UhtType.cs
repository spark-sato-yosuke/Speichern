...
namespace EpicGames.UHT.Types
{
	...
	/// </summary>
	public enum UhtDefineScopeNames
	{
	...
	public enum UhtDefineScope
	{
		...
		/// <summary>
		/// Exists inside of a WITH_TESTS block
		/// </summary>
		Tests = 1 << 2,
	...
	public static class UhtDefineScopeExtensions
	{
		...
		/// <summary>
		/// Return the compiler directive the matches the given scope
		/// </summary>
		/// <param name="defineScope">Scope to return compiler directive</param>
		/// <param name="defineScopeNames">Used to optionally select WITH_EDITOR instead of WITH_EDITORONLYDATA</param>
		/// <returns></returns>
		public static UhtCompilerDirective GetCompilerDirective(this UhtDefineScope defineScope, UhtDefineScopeNames defineScopeNames)
		{
			switch (defineScope)
			{
				case UhtDefineScope.EditorOnlyData:
					return defineScopeNames == UhtDefineScopeNames.WithEditor ? UhtCompilerDirective.WithEditor : UhtCompilerDirective.WithEditorOnlyData;
				case UhtDefineScope.VerseVM:
					return UhtCompilerDirective.WithVerseVM;
				case UhtDefineScope.Tests:
					return UhtCompilerDirective.WithTests;
				default:
					throw new UhtIceException("A single valid define scope must be passed to GetCompilerDirective()");
			}
		}
	}
	...
	/// </summary>
	[DebuggerDisplay("SourceName = {SourceName}")]
