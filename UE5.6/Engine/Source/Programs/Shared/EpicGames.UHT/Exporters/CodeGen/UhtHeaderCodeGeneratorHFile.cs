		...
		: UhtHeaderCodeGenerator
	{
		...
		public void Generate(IUhtExportFactory factory)
		{
			ref UhtCodeGenerator.HeaderInfo headerInfo = ref HeaderInfos[HeaderFile.HeaderFileTypeIndex];
			{
				...
				if (headerInfo.NeedsVerseString)
				{
					builder.Append("#include \"VerseVM/VVMNativeString.h\"\r\n");
				}
				if (headerInfo.NeedsVerseValue)
				{
					builder.Append("#include \"VerseValue.h\"\r\n");
				}
				if (headerInfo.NeedsVerseCodeGen)
				{
					builder.Append("#include \"VerseInteropMacros.h\"\r\n");
				}
				if (headerInfo.NeedsVerseField)
				{
					builder.Append("#include \"VerseField.h\"\r\n");
				}
				if (headerInfo.NeedsVersePointer)
				{
					builder.Append("#include \"VersePointer.h\"\r\n");
				}
				if (headerInfo.NeedsVerseTask)
				{
					builder.Append("#include \"VerseTask.h\"\r\n");
				}
				...
				foreach (UhtField field in HeaderFile.References.ExportTypes)
				{
					...
					{
						if (!classObj.ClassFlags.HasAnyFlags(EClassFlags.Intrinsic) && classObj.ClassType == UhtClassType.Interface && classObj.IsVerseField)
						{
							using UhtCodeBlockComment blockComment = new(builder, field, "Verse Interface Proxy");
							using UhtMacroBlockEmitter macroBlockEmitter = new(builder, UhtDefineScopeNames.Standard, classObj.DefineScope);
							AppendNativeInterfaceVerseProxy(builder, classObj);
						}
					}
				}
				...
				foreach (UhtField field in HeaderFile.References.ExportTypes)
				{
					...
					{
						using UhtCodeBlockComment blockComment = new(builder, field);
						using UhtMacroBlockEmitter macroBlockEmitter = new(builder, UhtDefineScopeNames.Standard, field.DefineScope);
		...
		private static StringBuilder AppendRigVmStructInfoDefine(StringBuilder builder, UhtScriptStruct scriptStruct)
		{
		...
		private StringBuilder AppendRigVmStructInfoMethods(StringBuilder builder, UhtScriptStruct scriptStruct)
		{
			...
			if (scriptStruct.RigVMStructInfo != null)
			{
				...
				if (!scriptStruct.ScriptStructFlags.HasAnyFlags(EStructFlags.RequiredAPI) && 
				    (Module.Module.AlwaysExportStructs || scriptStruct.ScriptStructExportFlags.HasAnyFlags(UhtScriptStructExportFlags.MinimalAPI)))
				{
					builder.Append(Module.Api);
				}
				builder.Append("virtual const UScriptStruct* GetExecuteContextStruct() const override { return ");
				builder.Append(scriptStruct.RigVMStructInfo.ExecuteContextType);
				builder.Append("::StaticStruct(); } \\\r\n");
				...
				foreach (UhtRigVMMethodInfo methodInfo in scriptStruct.RigVMStructInfo.Methods)
				{
					...
					builder.Append('\t').Append(methodInfo.DllStorage).Append(methodInfo.ReturnType).Append(' ').Append(methodInfo.Name).Append('(').Append(constPrefix).Append(scriptStruct.RigVMStructInfo.ExecuteContextType).Append("& InExecuteContext); \\\r\n");
					builder.Append("\tstatic ").Append(methodInfo.ReturnType).Append(' ').Append(methodInfo.DllStorage).Append("Static").Append(methodInfo.Name).Append("( \\\r\n");
					builder.Append("\t\t");
					if (!scriptStruct.RigVMStructInfo.HasAnyExecuteContextMember)
					{
						builder.Append("const ");
		...
		{
			AppendRigVmStructInfoDefine(builder, scriptStruct);
			...
			if (scriptStruct.ScriptStructFlags.HasAnyFlags(EStructFlags.Native))
			{
				using (UhtMacroCreator macro = new(builder, this, scriptStruct, GeneratedBodyMacroSuffix))
				{
					...
					AppendRigVmStructInfoMethods(builder, scriptStruct);
					...
					if (scriptStruct.MetaData.ContainsKey(UhtNames.StateStreamStaticState) || scriptStruct.MetaData.ContainsKey(UhtNames.StateStreamDynamicState))
					{
						builder.Append("\tenum Fields \\\r\n");
						builder.Append("\t{ \\\r\n");
						int enumValue = 1;
						foreach (UhtProperty property in scriptStruct.Properties)
						{
							builder.Append("\t\tField_").Append(property.SourceName).Append(" = ").Append(enumValue).Append(", \\\r\n");
							enumValue = enumValue << 1;
						}
						builder.Append("\t}; \\\r\n");

						builder.Append("\tuint32 ModifiedFlags = 0; \\\r\n");

						builder.Append('\t').Append(scriptStruct.SourceName).Append("() = default; \\\r\n");

						builder.Append('\t').Append(scriptStruct.SourceName).Append("(FStateStreamCopyContext& Context, const ").Append(scriptStruct.SourceName).Append("& Other) \\\r\n");
						bool isFirst = true;

						builder.Append(isFirst ? "\t:\t" : "\t,\t").Append("ModifiedFlags(Other.ModifiedFlags) \\\r\n");
						isFirst = false;
						...
						foreach (UhtProperty property in scriptStruct.Properties)
						{
							builder.Append(isFirst ? "\t:\t" : "\t,\t").Append(property.SourceName).Append('(');
							if (property is UhtStructProperty structProperty)
							{
								if (structProperty.ScriptStruct.MetaData.ContainsKey(UhtNames.StateStreamHandle))
								{
									hasHandles = true;
									builder.Append("Context, ");
								}
							}
							builder.Append("Other.").Append(property.SourceName);
							builder.Append(") \\\r\n");
							isFirst = false;
						}
						...
						foreach (UhtProperty property in scriptStruct.Properties)
						{
							...
							bool isBoolWithB = (property is UhtBoolProperty && propName.StartsWith("b", StringComparison.Ordinal));
							string cleanPropName = isBoolWithB ? propName[1..] : propName;
							builder.Append("\tinline bool ").Append(cleanPropName).Append("Modified() const { return !!(ModifiedFlags & Field_"). Append(propName).Append("); } \\\r\n");

							builder.Append("\tinline void Set").Append(cleanPropName).Append("(const ");
							property.AppendText(builder, UhtPropertyTextType.Generic);
							builder.Append("& In) { ").Append(propName).Append(" = In; ModifiedFlags |= Field_").Append(propName).Append("; } \\\r\n");

							if (isBoolWithB)
							{
								builder.Append("\tinline bool Get").Append(cleanPropName).Append("() const { return ").Append(propName).Append("; } \\\r\n");
							}
							else
							{
								builder.Append("\tinline const ");
								property.AppendText(builder, UhtPropertyTextType.Generic);
								builder.Append("& Get").Append(propName).Append("() const { return ").Append(propName).Append("; } \\\r\n");
							}
						}

						int dependencyCount = 0;
						builder.Append("\tuint32 GetDependencies(FStateStreamHandle** OutHandles, uint32 Capacity) \\\r\n");
						builder.Append("\t{ \\\r\n");
						if (hasHandles)
						{
							foreach (UhtProperty property in scriptStruct.Properties)
							{
								if (property is UhtStructProperty structProperty)
								{
									if (structProperty.ScriptStruct.MetaData.ContainsKey(UhtNames.StateStreamHandle))
									{
										builder.Append("\t\tOutHandles[").Append(dependencyCount++).Append("] = &").Append(property.SourceName).Append("; \\\r\n");
									}
								}
							}
						}
						builder.Append("\t\treturn ").Append(dependencyCount).Append("; \\\r\n");
						builder.Append("\t} \\\r\n");

						builder.Append("\tvoid Apply(FStateStreamCopyContext& Context, const ").Append(scriptStruct.SourceName).Append("& Other) \\\r\n");
						builder.Append("\t{ \\\r\n");
						foreach (UhtProperty property in scriptStruct.Properties)
						{
							string propName = property.SourceName;
							...
							builder.Append("\t\tif (Other.").Append(cleanPropName).Append("Modified()");
							builder.Append(" && !StateStreamEquals(").Append(propName).Append(", Other.").Append(propName);
							builder.Append(")) \\\r\n");
							builder.Append("\t\t{ \\\r\n");
							...
							if (property is UhtStructProperty structProperty)
							{
								if (structProperty.ScriptStruct.MetaData.ContainsKey(UhtNames.StateStreamHandle))
								{
									builder.Append("\t\t\t").Append(propName).Append(".Apply(Context, Other.").Append(propName).Append("); \\\r\n");
									assignHandled = true;
								}
							}
							if (!assignHandled)
							{
								builder.Append("\t\t\t").Append(propName).Append(" = Other.").Append(propName).Append("; \\\r\n");
							}
							builder.Append("\t\t\tModifiedFlags |= Field_").Append(propName).Append("; \\\r\n");
							builder.Append("\t\t} \\\r\n");
						}
						builder.Append("\t} \\\r\n");

						builder.Append("\tvoid Interpolate(FStateStreamInterpolateContext& Context, const ").Append(scriptStruct.SourceName).Append("& From, const ").Append(scriptStruct.SourceName).Append("& To) \\\r\n");
						builder.Append("\t{ \\\r\n");
						builder.Append("\t\tModifiedFlags = 0; \\\r\n");
						foreach (UhtProperty property in scriptStruct.Properties)
						{
							string propName = property.SourceName;
							bool isBool = property is UhtBoolProperty;
							bool isBoolWithB = isBool && propName.StartsWith("b", StringComparison.Ordinal);
							string cleanPropName = isBoolWithB ? propName[1..] : propName;

							builder.Append("\t\tif (To.").Append(cleanPropName).Append("Modified()) \\\r\n");
							builder.Append("\t\t{ \\\r\n");
							builder.Append("\t\t\tModifiedFlags |= Field_").Append(propName).Append("; \\\r\n");
							...
							if (property is UhtStructProperty structProperty)
							{
								if (structProperty.ScriptStruct.MetaData.ContainsKey(UhtNames.StateStreamHandle))
								{
									builder.Append("\t\t\t").Append(propName).Append(".Apply(Context, To.").Append(propName).Append("); \\\r\n");
									assignHandled = true;
								}
							}
							if (!assignHandled)
							{
								if (isBool)
								{
									builder.Append("\t\t\t").Append(propName).Append(" = To.").Append(propName).Append(";\\\r\n");
								}
								else
								{
									builder.Append("\t\t\tStateStreamInterpolate(Context, ").Append(propName).Append(", From.").Append(propName).Append(", To.").Append(propName).Append("); \\\r\n");
								}
							}
							builder.Append("\t\t} \\\r\n");
						}
					...
					else if (scriptStruct.MetaData.ContainsKey(UhtNames.StateStreamHandle))
					{
						ReadOnlySpan<char> shortName = scriptStruct.SourceName.AsSpan(1, scriptStruct.SourceName.Length - 7);
						builder.Append("\tDECLARE_STATESTREAM_HANDLE(").Append(shortName).Append(") \\\r\n");
					}
					
					Session.Tables!.CodeGeneratorInjectorTable.Inject(builder, scriptStruct, UhtCodeGeneratorInjectionLocation.GeneratedMacro);
				}

				// For back compat we still need to forward declare the struct here
				builder.Append("struct ").Append(scriptStruct.SourceName).Append(";\r\n");

				// Verse declarations
				if (scriptStruct.IsVerseField)
				{
					if (!scriptStruct.FieldExportFlags.HasAnyFlags(UhtFieldExportFlags.NoVerseAlias))
					{
						builder.Append("namespace verse { using ").Append(scriptStruct.EngineName).Append(" = ").Append(scriptStruct.SourceName).Append("; }\r\n");
					}
					builder
						.Append("V_DECLARE_STRUCT_RUNTIME_TRAITS(, ")
						.Append(scriptStruct.SourceName).Append(", \"")
						.AppendVerseUEPackageName(scriptStruct).Append("\", \"")
						.AppendVersePath(scriptStruct, UhtVerseFullNameMode.Default).Append("\")\r\n");
				}

				Session.Tables!.CodeGeneratorInjectorTable.Inject(builder, scriptStruct, UhtCodeGeneratorInjectionLocation.Header);
			}
			return builder;
		}
		...
		private StringBuilder AppendEnum(StringBuilder builder, UhtEnum enumObj)
		{
			...
			{	
				...
				if (Module.Module.AlwaysExportEnums || enumObj.EnumExportFlags.HasAnyFlags(UhtEnumExportFlags.MinimalAPI))
				{
			...
			{
				if (!enumObj.FieldExportFlags.HasAnyFlags(UhtFieldExportFlags.NoVerseAlias))
				{
					builder.Append("namespace verse { using ").Append(enumObj.EngineName).Append(" = ::").Append(enumObj.SourceName).Append("; }\r\n");
				}
				builder.Append("V_DECLARE_IMPORTED_ENUM(").Append(Module.Api.TrimEnd()).Append(", ").Append(enumObj.SourceName).Append(");\r\n");
			}
			return builder;
		}
		...
		private StringBuilder AppendClass(StringBuilder builder, UhtClass classObj)
		{
			...
			// Collect the verse native callable functions
			UhtUsedDefineScopes<UhtFunction> verseNativeCallableFunctions = new(classObj.Functions.Where(x => x.FunctionExportFlags.HasAnyFlags(UhtFunctionExportFlags.VerseNativeCallable)));

			// Output the verse native callback functions
			AppendVerseNativeCallableFunctions(builder, classObj, verseNativeCallableFunctions);
			...
			// Declaration of globla function fetching UClass of type
			builder.Append(classObj.Module.Api).Append("UClass* ").Append(ObjectInfos[classObj.ObjectTypeIndex].UnregisteredSingletonName).Append("();\r\n\r\n");
			...
			if (classObj.ClassFlags.HasAnyFlags(EClassFlags.Interface))
			{
				...
				AppendGeneratedBodyMacroBlock(builder, classObj, nativeInterface!, alternateUsesLegacy, rpcFunctions, verseNativeCallableFunctions,
					autoGetterSetterProperties, sparseScriptStructs.Any(), sparseProperties, getterSetterProperties, hasCallbacks, null);

				// For interfaces, the VINTERFACES macro generates nothing
				if (classObj.VerseInterfacesLine != 0)
				{
					using UhtMacroCreator macro = new(builder, this, classObj.VerseInterfacesLine, VInterfacesMacroSuffix);
				}
			}
			else
			{
				...
				AppendProlog(builder, classObj);
				AppendGeneratedBodyMacroBlock(builder, classObj, classObj, usesLegacy, rpcFunctions, verseNativeCallableFunctions,
					autoGetterSetterProperties, sparseScriptStructs.Any(), sparseProperties, getterSetterProperties, hasCallbacks, 
					usesLegacy ? "GENERATED_UCLASS_BODY" : null);
			}

			// For back compat we still need to forward declare the class here
			builder.Append("class ").Append(classObj.SourceName).Append(";\r\n");
			
			// Verse declarations
			if (classObj.IsVerseField)
			{
				if (!classObj.FieldExportFlags.HasAnyFlags(UhtFieldExportFlags.NoVerseAlias))
				{
					if (classObj.ClassType == UhtClassType.Class)
					{
						builder.Append("namespace verse { using ").Append(classObj.EngineName).Append(" = ").AppendClassSourceNameOrInterfaceProxyName(classObj).Append("; }\r\n");
					}
				}
			}

			Session.Tables!.CodeGeneratorInjectorTable.Inject(builder, classObj, UhtCodeGeneratorInjectionLocation.Header);
			builder.Append("\r\n");
			return builder;
		}

		private static StringBuilder AppendNativeInterfaceVerseProxy(StringBuilder builder, UhtClass classObj)
		{
			builder.Append("class ").AppendClassSourceNameOrInterfaceProxyName(classObj).Append("\r\n");
			builder.Append("{\r\n");
			builder.Append("public:\r\n");
			builder.Append("\tusing StaticClassType = ").Append(classObj.SourceName).Append(";\r\n");
			foreach (UhtClass baseClass in classObj.FlattenedVerseInterfaces)
			{
				AppendNativeInterfaceVerseProxyFunctions(builder, baseClass.AlternateObject as UhtClass);
			}
			AppendNativeInterfaceVerseProxyFunctions(builder, classObj);
			builder.Append('\t').AppendClassSourceNameOrInterfaceProxyName(classObj).Append("(const TNonNullPtr<UObject>& InSelf) : Self(InSelf) {}\r\n");
			builder.Append("\tUObject* _getUObject() const { return Self; }\r\n");
			builder.Append('\t').AppendClassSourceNameOrInterfaceProxyName(classObj).Append("* operator->() { return this; }\r\n");
			builder.Append("private:\r\n");
			builder.Append("\tTNonNullPtr<UObject> Self;\r\n");
			builder.Append("};\r\n");
			foreach (UhtClass baseClass in classObj.FlattenedVerseInterfaces)
			{
				builder
					.Append("template<> struct TIsSubinterfaceOf<")
					.AppendClassSourceNameOrInterfaceProxyName(baseClass)
					.Append(", ")
					.AppendClassSourceNameOrInterfaceProxyName(classObj)
					.Append("> { enum { Value = true }; };\r\n");
			}
		...
		private static StringBuilder AppendNativeInterfaceVerseProxyFunctions(StringBuilder builder, UhtClass? classObj)
		{
			if (classObj != null)
			{
				UhtUsedDefineScopes<UhtFunction> verseNativeCallableFunctions = new(classObj.Functions.Where(x => x.FunctionExportFlags.HasAnyFlags(UhtFunctionExportFlags.VerseNativeCallable)));
		...
		private StringBuilder AppendFieldNotify(StringBuilder builder, UhtClass classObj)
		{
			...
			return builder.AppendSingleMacro(notifyTypes, UhtDefineScopeNames.Standard, this, classObj, FieldNotifyMacroSuffix,
				(builder, filteredTypes) =>
				{
					...
					{
						builder.Append("\tUE_FIELD_NOTIFICATION_DECLARE_FIELD(").Append(GetNotifyTypeName(notifyType)).Append(',').Append(Module.Api).Append(") \\\r\n");
					}
		...
		private StringBuilder AppendAutoGettersSetters(StringBuilder builder, UhtClass classObj, UhtUsedDefineScopes<UhtProperty> autoGetterSetterProperties)
		{
			...
			return builder.AppendMultiScopedMacros(autoGetterSetterProperties, UhtDefineScopeNames.Standard, this, classObj, AutoGettersSettersMacroSuffix,
				(builder, properties) =>
				{
		...
		private StringBuilder AppendSparseDeclarations(StringBuilder builder, UhtClass classObj, IEnumerable<UhtScriptStruct> sparseScriptStructs, UhtUsedDefineScopes<UhtProperty> sparseProperties)
		{
			...
			builder.AppendMultiScopedMacros(sparseProperties, UhtDefineScopeNames.Standard, this, classObj, SparseDataPropertyAccessorsMacroSuffix, 
				(builder, properties) =>
				{
					foreach (UhtProperty property in properties)
					{
		...
		private StringBuilder AppendSparseStructDeclarations(StringBuilder builder, UhtClass classObj, IEnumerable<UhtScriptStruct> sparseScriptStructs)
		{
			using (UhtMacroCreator macro = new(builder, this, classObj, SparseDataMacroSuffix))
			{
				...
				foreach (UhtScriptStruct sparseScriptStruct in sparseScriptStructs)
				{
					...
					builder.Append("UE_DEPRECATED(5.6, \"Use (preferably) Get").Append(sparseDataType).Append("(EGetSparseClassDataMethod::ArchetypeIfNull), or GetMutable").Append(sparseDataType).Append("() instead if you need to mutate the sparse class data (best avoided at runtime)\")\\\r\n");
					builder.Append(api).Append('F').Append(sparseDataType).Append("* Get").Append(sparseDataType).Append("() const; \\\r\n");

					builder.Append(api).Append('F').Append(sparseDataType).Append("* GetMutable").Append(sparseDataType).Append("() const; \\\r\n");
		...
		{
			builder.AppendMultiScopedMacros(functions, UhtDefineScopeNames.WithEditor, this, classObj, usesLegacy ? RpcWrappersMacroSuffix : RpcWrappersNoPureDeclsMacroSuffix,
				(builder, functions) =>
				{
					if (usesLegacy)
					{
		...
		private StringBuilder AppendVerseNativeCallableFunctions(StringBuilder builder, UhtClass classObj, UhtUsedDefineScopes<UhtFunction> functions)
		{
			builder.AppendMultiScopedMacros(functions, UhtDefineScopeNames.WithEditor, this, classObj, VerseNativeCallableWrappersMacroSuffix,
				(builder, functions) =>
				{
					foreach (UhtFunction function in functions)
					{
						builder.Append('\t').AppendVerseNativeCallableSignature(null, function, "").Append("; \\\r\n");
					}
				});
			return builder;
		}

		private static StringBuilder AppendVerseNativeCallableProxyFunctions(StringBuilder builder, UhtClass classObj, UhtUsedDefineScopes<UhtFunction> functions)
		{
			if (classObj.SuperClass != null && classObj.SuperClass.IsVerseField && classObj.SuperClass.ClassType == UhtClassType.Interface)
			{
				AppendVerseNativeCallableProxyFunctions(builder, classObj.SuperClass, new(classObj.SuperClass.Functions.Where(x => x.FunctionExportFlags.HasAnyFlags(UhtFunctionExportFlags.VerseNativeCallable))));
			}

			builder.AppendMultiScoped(functions, UhtDefineScopeNames.WithEditor,
				(builder, functions) =>
				{
					foreach (UhtFunction function in functions)
					{
		...
		private static StringBuilder AppendRpcWrappers(StringBuilder builder, IEnumerable<UhtFunction> functions)
		{
			foreach (UhtFunction function in functions)
			{
				...
				if (function.FunctionExportFlags.HasAnyFlags(UhtFunctionExportFlags.VerseCallable))
				{
					if (function.FunctionExportFlags.HasAnyFlags(UhtFunctionExportFlags.VerseNoRollback))
					{
						builder.Append("\tV_DECLARE_EXEC_NOROLLBACK(").Append(function.VerseName).Append("); \\\r\n");
					}
					else if (function.FunctionExportFlags.HasAnyFlags(UhtFunctionExportFlags.VerseSuspends))
					{
						builder.Append("\tV_DECLARE_EXEC_SUSPENDS(").Append(function.VerseName).Append("); \\\r\n");
					}
					else
					{
						builder.Append("\tV_DECLARE_EXEC(").Append(function.VerseName).Append("); \\\r\n");
					}
					builder.Append("\tV_DECLARE_IMPL_NO_PRED(").Append(function.VerseName).Append("); \\\r\n");
				}
			}
			return builder;
		}
		...
		{
			builder.Append("\t/** Deleted move- and copy-constructors, should never be used */ \\\r\n");
			builder.Append('\t').Append(classObj.SourceName).Append('(').Append(classObj.SourceName).Append("&&) = delete; \\\r\n");
			builder.Append('\t').Append(classObj.SourceName).Append("(const ").Append(classObj.SourceName).Append("&) = delete; \\\r\n");
			return builder;
		}
		...
		private static StringBuilder AppendDestructorDefinition(StringBuilder builder, UhtClass classObj, string api)
		{
			if (!classObj.ClassExportFlags.HasAnyFlags(UhtClassExportFlags.HasDestructor))
			{
				if (classObj.ClassFlags.HasAnyFlags(EClassFlags.Interface))
				{
					builder.Append('\t').Append("virtual ~").Append(classObj.SourceName).Append("() = default; \\\r\n");	
				}
				else
				{
		...
		{
			UhtCodeGenerator.ObjectInfo info = ObjectInfos[classObj.ObjectTypeIndex];
			...
			{
				builder.Append("\tfriend struct ").Append(info.RegisteredSingletonName).Append("_Statics; \\\r\n");
			}
			
			builder.Append('\t').Append("static UClass* GetPrivateStaticClass(); \\\r\n");
			builder.Append("\tfriend ").Append(classObj.Module.Api).Append("UClass* ").Append(info.UnregisteredSingletonName).Append("(); \\\r\n");
			...
			builder.Append(", TEXT(\"").Append(classObj.Package.SourceName).Append("\"), ").Append(info.UnregisteredSingletonName).Append(") \\\r\n");
		...
		{
			string interfaceSourceName = classObj.GetNativeInterfaceName();
			...
			if (classObj.IsVerseField || (classObj.SuperClass != null && classObj.SuperClass.IsChildOf(Session.UInterface)))
			{
		...
		private static StringBuilder AppendNetData(StringBuilder builder, UhtClass classObj, string api)
		{
			...
			builder.Append('\t').Append("DECLARE_VALIDATE_GENERATED_REP_ENUMS(").Append(api[0..^1]).Append(") \\\r\n");
