...
namespace EpicGames.UHT.Exporters.CodeGen
{
	class UhtHeaderCodeGenerator : UhtPackageCodeGenerator
	{
		...
		public const string VerseNativeCallableWrappersMacroSuffix = "VERSE_NATIVE_CALLABLE_WRAPPERS";
		...
		public const string VInterfacesMacroSuffix = "VINTERFACES";
		#endregion
			...
			string? alternateFunctionName, string? extraParam, UhtFunctionExportFlags extraExportFlags, int tabs, string endl)
		{
			...
			if (isDeclaration)
			{
				...
				if (textType != UhtPropertyTextType.EventFunctionArgOrRetVal &&
					...
					exportFlags.HasAnyFlags(UhtFunctionExportFlags.RequiredAPI) && (!isInterface || textType == UhtPropertyTextType.InterfaceFunctionArgOrRetVal))
				{
		...
		{
			return
				function.FunctionFlags.HasAnyFlags(EFunctionFlags.Native) &&
		...
		{
			// Always create a stub for verse native callables
			if (function.FunctionExportFlags.HasAnyFlags(UhtFunctionExportFlags.VerseNativeCallable))
			{
				return true;
			}
	...
	internal static class UhtHaederCodeGeneratorStringBuilderExtensions
	{
		...
		/// <summary>
		/// Append just the overall return type of the verse callable function
		/// </summary>
		/// <param name="builder">Destination builder</param>
		/// <param name="function">Function in question</param>
		/// <param name="isVerseVM">If true, this is for the verse VM where the outer TOptional is stripped</param>
		/// <returns>Builder</returns>
		public static StringBuilder AppendVerseCallableReturnType(this StringBuilder builder, UhtFunction function, bool isVerseVM)
		{
			UhtProperty? returnProperty = function.ReturnProperty;
			...
			{
				if (isVerseVM && returnProperty is UhtOptionalProperty optionalProperty)
				{
					returnProperty = optionalProperty.ValueProperty;
				}
				builder.AppendTokens(returnProperty.TypeTokens.MyTokens);
			}
			return builder;
		}

		/// <summary>
		/// Append just the overall return type of the native callable verse function
		/// </summary>
		/// <param name="builder">Destination builder</param>
		/// <param name="function">Function in question</param>
		/// <returns>Builder</returns>
		public static StringBuilder AppendVerseNativeCallableReturnType(this StringBuilder builder, UhtFunction function)
		{
			UhtProperty? returnProperty = function.ReturnProperty;
			...
			bool isIfNoRuntimeError = returnProperty != null && !isIncompleteReturn;

			if (isSuspends)
			{
				builder.Append("TVerseTask<");
			}
			else if (isIfNoRuntimeError)
			{
				builder.Append("TIfNoRuntimeError<");
			}
			...
			else if (isIncompleteReturn)
			{
				builder.Append("EVerseNativeCallResult");
			}
			else
			{
				builder.AppendTokens(returnProperty.TypeTokens.AllTokens);
			}

			if (isSuspends || isIfNoRuntimeError)
			{
				builder.Append("> ");
			}
			else
		...
		/// <summary>
		/// Append the complete signature of the native callable verse function
		/// </summary>
		/// <param name="builder">Destination builder</param>
		/// <param name="classObj">If supplied, used to append the ClassSourceName:: part of the declaration</param>
		/// <param name="function">Function in question</param>
		/// <param name="classNameSuffix">Appended to the end of the class name.  Used to generate the Proxy implementations</param>
		/// <returns>Builder</returns>
		public static StringBuilder AppendVerseNativeCallableSignature(this StringBuilder builder, UhtClass? classObj, UhtFunction function, string classNameSuffix)
		{
			builder.AppendVerseNativeCallableReturnType(function);

			if (classObj != null)
			{
				builder.AppendClassSourceNameOrInterfaceName(classObj).Append(classNameSuffix).Append("::");
			}
			builder.Append(function.SourceName).Append("(V_EXEC_CONTEXT_PARAM");
			foreach (UhtType type in function.ParameterProperties.Span)
			{
				if (type is UhtProperty property)
				{
					builder.Append(", ").AppendTokens(property.TypeTokens.AllTokens).Append(' ').Append(property.SourceName);
				}
			}
			builder.Append(')');
			return builder;
		...
		public static StringBuilder AppendVerseNativeCallableTypeDef(this StringBuilder builder, UhtFunction function)
		{
			UhtProperty? returnProperty = function.ReturnProperty;
			bool isIncompleteReturn = function.Session.IsIncompleteReturn(returnProperty);
			bool isSuspends = function.FunctionExportFlags.HasAnyFlags(UhtFunctionExportFlags.VerseSuspends);
			if (!isIncompleteReturn && function.FunctionExportFlags.HasAnyFlags(UhtFunctionExportFlags.VerseDecides) && returnProperty is UhtOptionalProperty optionalReturn)
			{
				returnProperty = optionalReturn.ValueProperty;
			}

			builder.Append("TVerseFunction<");
			if (isSuspends)
			{
				builder.Append("FVerseResult(TVerseCall<");
			}
			if (returnProperty == null)
			{
				builder.Append("void");
			...
			builder.Append(isSuspends ? '>' : '(').AppendVerseFunctionArgs(function, false, true, property => builder.AppendTokens(property.TypeTokens.MyTokens), () => builder.Append("FDecidesContext")).Append(")>");
			return builder;
		}

		public static StringBuilder AppendVerseFunctionArgs(this StringBuilder builder, UhtFunction function, bool startWithSeparator, bool decidesBeforeReturn, Action<UhtProperty> propertyAction, Action decidesAction)
		{
			bool insertSeparator = startWithSeparator;
			if (function.Session.IsIncompleteReturn(function.ReturnProperty))
			{
				if (decidesBeforeReturn && function.FunctionExportFlags.HasAnyFlags(UhtFunctionExportFlags.VerseDecides))
				{
					if (insertSeparator)
					{
						builder.Append(", ");
				...
				{
					if (function.ParameterProperties.Span[^1] is UhtProperty property)
					{
						if (insertSeparator)
						{
							builder.Append(", ");
				...
				foreach (UhtType type in function.ParameterProperties.Span[..^1])
				{
					if (type is UhtProperty property)
					{
						if (insertSeparator)
			...
			{
				if (function.FunctionExportFlags.HasAnyFlags(UhtFunctionExportFlags.VerseDecides))
				{
					if (insertSeparator)
					{
						builder.Append(", ");
