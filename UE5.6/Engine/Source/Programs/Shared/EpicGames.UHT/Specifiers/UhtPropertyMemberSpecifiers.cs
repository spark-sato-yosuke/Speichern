...
namespace EpicGames.UHT.Parsers
{
	...
	public static class UhtPropertyMemberSpecifiers
	{
		...
		[UhtSpecifier(Extends = UhtTableNames.PropertyMember, ValueType = UhtSpecifierValueType.Legacy)]
		private static void ExperimentalExternalObjectsSpecifier(UhtSpecifierContext specifierContext)
		{
			UhtPropertySpecifierContext context = (UhtPropertySpecifierContext)specifierContext;
			if ((context.PropertySettings.PropertyFlags & EPropertyFlags.ExperimentalOverridableLogic) == 0)
			{
				context.MessageSite.LogError("ExperimentalExternalObjects can only be used with ExperimentalOverridableLogic");
			}
			context.PropertySettings.PropertyFlags |= EPropertyFlags.ExperimentalExternalObjects;
		}
