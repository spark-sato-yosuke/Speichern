		...
		private static UhtParseResult ParseUClass(UhtParsingScope parentScope, ref UhtToken token)
		{
			UhtClass classObj = new(parentScope.HeaderFile, parentScope.ScopeType, token.InputLine);
			{
...

				{
					...
					{
						classObj.MetaData.Add(UhtNames.IsBlueprintBase, "false");
						classObj.MetaData.Add(UhtNames.DisplayName, classObj.VerseName!);
					...
					if ((compilerDirective & UhtCompilerDirective.WithTests) != 0)
					{
						classObj.DefineScope |= UhtDefineScope.Tests;
					}
