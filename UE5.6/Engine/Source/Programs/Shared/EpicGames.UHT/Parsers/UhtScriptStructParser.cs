	...
	public static class UhtScriptStructParser
	{
		...
		private static UhtParseResult ParseUScriptStruct(UhtParsingScope parentScope, UhtToken keywordToken)
		{
			UhtScriptStruct scriptStruct = new(parentScope.HeaderFile, parentScope.ScopeType, keywordToken.InputLine);
			{
...

				{
					...
					{
						scriptStruct.MetaData.Add(UhtNames.IsBlueprintBase, "false");
					}
					...
					{
						scriptStruct.DefineScope |= UhtDefineScope.Tests;
					}
		...
		private static void ParseRigVM(UhtParsingScope topScope)
		{
...

			{
				...
				topScope.TokenReader
					.OptionalEndsWith("_API", (api) => { methodInfo.DllStorage = api + " "; })
					.Optional("virtual")
