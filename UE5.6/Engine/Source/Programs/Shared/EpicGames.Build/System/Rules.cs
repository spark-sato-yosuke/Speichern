...
namespace UnrealBuildBase
{
	public class Rules
	{
		...
		static Dictionary<DirectoryReference, RulesFileCache> RootFolderToRulesFileCache = [];
		...
		{
			return FindAllRulesFiles([Directory], Type);
		}
		...
		private static void FindAllRulesFilesRecursively(DirectoryItem Directory, RulesFileCache Cache, ThreadPoolWorkQueue Queue)
		{
			...
			if (bIsPlugin)
			{
				...
				// search Build directory as well as it can contain automation projects
				DirectoryItem? BuildDir;
				if (Directory.TryGetDirectory("Build", out BuildDir))
				{
					Queue.Enqueue(() => FindAllRulesFilesRecursively(BuildDir, Cache, Queue));
				}
			}
