...
namespace JobDriver.Utility
{
	...
	static class TempStorage
	{
		...
		/// </summary>
		/// <param name="hordeClient">The job rpc interface</param>
		...
		public static async Task<TempStorageTagManifest> RetrieveTagAsync(IHordeClient hordeClient, JobId jobId, JobStepId stepId, string nodeName, string tagName, DirectoryReference manifestDir, ILogger logger, CancellationToken cancellationToken)
		{
			...
			else
			{
				...
				logger.LogInformation("Reading tag \"{NodeName}\":\"{TagName}\" from temp storage (artifact: {ArtifactId} '{ArtifactName}' ({ArtifactType}), ns: {NamespaceId}, ref: {RefName}, localFile: {LocalFile})", nodeName, tagName, ArtifactId.Parse(artifact.Id), artifactName, artifactType, namespaceId, refName, localFileListLocation);
		...
		public static async Task<FileEntry> ArchiveTagAsync(DirectoryReference manifestDir, string nodeName, string tagName, DirectoryReference workspaceDir, IEnumerable<FileReference> files, TempStorageBlockRef[] blocks, IBlobWriter writer, ILogger logger, CancellationToken cancellationToken)
		{
			...
			return new FileEntry(localFileListLocation.GetFileName(), FileEntryFlags.None, fileNodeData);
		}
		...
		public static async Task<TempStorageBlockManifest> RetrieveBlockAsync(IHordeClient hordeClient, JobId jobId, JobStepId stepId, string nodeName, string blockName, DirectoryReference rootDir, DirectoryReference manifestDir, ILogger logger, CancellationToken cancellationToken)
		{
			...
			else
			{
				...
				logger.LogInformation("Reading block \"{NodeName}\":\"{BlockName}\" from temp storage (artifact: {ArtifactId} '{ArtifactName}' ({ArtifactType}), ns: {NamespaceId}, ref: {RefName}, local: {LocalFile}, blockdir: {BlockDir})", nodeName, blockName, ArtifactId.Parse(artifact.Id), artifactName, artifactType, namespaceId, refName, localManifestFile, blockDirectoryName);
				...
				for (int attempt = 0; ; attempt++)
				{
					...
					options.VerifyOutput = options.VerboseOutput = (attempt > 0);

					await rootDirEntry.Handle.ExtractAsync(rootDir.ToDirectoryInfo(), options, logger, cancellationToken);
					...
					catch (FileNotFoundException ex) when (attempt == 0)
					{
						logger.LogWarning(ex, "File not found: {Message}", ex.Message);
						logger.LogInformation("Running extract again with verbose logging.");
					}
				}
			}
