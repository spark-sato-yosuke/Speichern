...
import backend from '.';
import { AuthMethod, DashboardPreference, DownloadFormat, GetDashboardAgentCategoryResponse, GetDashboardArtifactTypeResponse, GetDashboardConfigResponse, GetDashboardPoolCategoryResponse, GetJobTemplateSettingsResponse, GetUserResponse, UserClaim } from './Api';
...
export class Dashboard {
    ...
    getArtifactTypeInfo(type:string): GetDashboardArtifactTypeResponse | undefined {
        return this.config?.artifactTypeInfo?.find(a => a.type === type);
    }
    ...
    setDownloadFormat(value: DownloadFormat | undefined) {
        this.setPreference(DashboardPreference.DownloadFormat, value);
    }

    get downloadFormat(): DownloadFormat {
        return this.preferences.get(DashboardPreference.DownloadFormat) as DownloadFormat ?? DownloadFormat.Zip;
    }
        ...
        try {
            ...
            if (!this.config) {
                try {
                    ...
                    this.config = { agentCategories: [], poolCategories: [], telemetryViews: [], artifactTypes: [], artifactTypeInfo: [] };
                }
            }
