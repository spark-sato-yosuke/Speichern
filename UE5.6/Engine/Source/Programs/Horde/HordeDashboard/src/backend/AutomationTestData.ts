...
export class TestDataHandler {
    ...
    addStream(streamId: StreamId, queryQuery: boolean = true) {
        ...
        if (this.updateSearch(queryQuery)) {
            this.setUpdated();
        }
    }
    ...
    updateSearch(queueQuery: boolean = true): boolean {
    ...
    async query() {
        ...
        refs.forEach(r => {
            ...
            // Use the latest date for summary, otherwise gets confusing when a CL spans days
