import { DefaultButton, DetailsList, DetailsListLayoutMode, Dropdown, FocusZone, FocusZoneDirection, IColumn, Icon, IconButton, IGroup, GroupHeader, Label, Link as FluentLink, mergeStyleSets, Modal, Pivot, PivotItem, PrimaryButton, ScrollablePane, ScrollbarVisibility, SelectionMode, Spinner, SpinnerSize, Stack, Text, TextField, IDetailsGroupDividerProps, IGroupHeaderProps } from "@fluentui/react";
...
import { DeviceHandler, DeviceStatus, DeviceStatusLabels, DeviceHealthStatus } from "../../backend/DeviceHandler";
import { DeviceEditor } from "./DeviceEditor";
import { DeviceInfoModal } from "./DeviceInfoView";
import { deviceHealthBadge, DeviceGroupHeader } from "./DeviceHealthWidgets";
...
const DevicePanel: React.FC = observer(() => {
   ...
   column.onRender = ((item: DeviceItem, index, column) => {
      ...
      let text = DeviceStatusLabels.get(status)!;
      ...
      if (item.device.cleanStartTime && !item.device.cleanFinishTime) {
         text = "Cleaning";
      }
   ...
   column.onRender = ((item: DeviceItem, index, column) => {
      ...
      let health = automationTab? handler.getDeviceHealth(device.id) : DeviceHealthStatus.Unknown;
      if (health === DeviceHealthStatus.Healthy && handler.getDeviceStatus(device) === DeviceStatus.Problem) {
         health = DeviceHealthStatus.Unreliable;
      }
            ...
            {automationTab && <Stack verticalFill={true} verticalAlign="center">
                  <div style={{ cursor: "pointer" }} onClick={(ev) => {
                     ...
                     <Stack horizontal tokens={{ childrenGap: 8 }}>
                        <Icon style={{ padding: 2 }} iconName="History"/>
                        {!!health && deviceHealthBadge(health, handler.telemetryHoursRange)}
                     </Stack>
                  </div>
            </Stack>}
            {!!r && <Stack>
   ...
   devices.forEach((item, index) => {
      ...
      if (item.userCheckout) {
         ...
         const groupDevices = devices.filter(cd => cd.device.platformId === d.platformId).map(d => d.device.id);
         newGroups.push({
            key: key,
            name: platforms.get(d.platformId)!.name,
            data: {devices: groupDevices},
            startIndex: index,
            count: groupDevices.length,
            level: 0,
            // Collapse if it was collapsed by the user, or filtering is not an exact match, or there is more than one device listed
            isCollapsed: (groups.find(g => g.key === key)?.isCollapsed ?? !exactMatch) && devices.length !== 1
         });
      }
   })
   ...
   const groupTitleRenderer = (props: IGroupHeaderProps): JSX.Element => {
      return <DeviceGroupHeader
         group={props.group!}
         handler={handler}
         devices={props.group!.data!.devices}
      />
   }

   const groupHeaderRenderer = (props: IDetailsGroupDividerProps): JSX.Element => {
      const onRenderName = automationTab ? groupTitleRenderer : undefined;
      return <GroupHeader {...props} onRenderName={onRenderName} />
   }
   ...
   return (<Stack>
      {!!telemState && <DevicePoolTelemetryModal poolsIn={localState.searchState.filterPools} platformsIn={localState.searchState.filterPlatforms} onClose={() => setTelemState(false)} />}
      ...
      {editState.shown && <DeviceEditor handler={handler} deviceIn={editState.device?.device} editNote={editState.editNote} onClose={(device) => { const deviceItem = !!device? { device: device }: undefined; setEditState({ ...editState, shown: false, device: deviceItem }); localState.resetHistoryItem(); }} />}
               ...
               <div className={customStyles.details} style={{ height: "calc(100vh - 280px)", position: 'relative' }} data-is-scrollable>
                  <ScrollablePane scrollbarVisibility={ScrollbarVisibility.always} onScroll={() => { }}>
                     <DetailsList
                        ...
                        items={devices}
                        groups={groups}
                        groupProps={{onRenderHeader: groupHeaderRenderer}}
                        columns={columns}
