...
import { DeviceHandler, DeviceStatusLabels } from "../../backend/DeviceHandler";
...
export const DeviceInfoModal: React.FC<{ handler: DeviceHandler, deviceIn?: GetDeviceResponse | undefined, onEdit: (device: GetDeviceResponse) => void, onClose: () => void }> = ({ handler, deviceIn, onEdit, onClose }) => {
   ...
   const failureRate = jobState.telemetryItems.length == 0? 0 : Math.floor(jobState.telemetryItems.filter(item => !!item.job.problemTimeUtc).length / jobState.telemetryItems.length * 100);
   const status = handler.getDeviceStatus(deviceIn);
            ...
            <Stack horizontal tokens={{ childrenGap: 48 }} >
               ...
               <Stack>
                  <Stack tokens={{ childrenGap: 12 }}>
                     ...
                     <Stack>
                        <TextField label="Pool" value={deviceIn.poolId?.toUpperCase()} disabled={true} readOnly={true} />
                     </Stack>

                     <Stack>
                        <TextField label="Model" value={deviceIn.modelId ? deviceIn.modelId : "Base"} disabled={true} readOnly={true} />
                     </Stack>

                     <Stack>
                        <TextField label="Address" value={deviceIn.address} disabled={true} readOnly={true} />
                     </Stack>

                     <Stack>
                        <TextField label="Status" value={DeviceStatusLabels.get(status)} disabled={true} readOnly={true} />
                     </Stack>

                     <Stack>
                        <TextField label="Last Modified By" value={jobState.modifiedByUser ? jobState.modifiedByUser : ""} disabled={true} readOnly={true} />
                     </Stack>
                     ...
                     <Stack horizontal style={{ paddingTop: 24 }}>
                        ...
                        <Stack style={{ paddingTop: 8, paddingRight: 24 }}>
                           <Link to={`/audit/device/${deviceIn.id}`} ><DefaultButton text="Audit" onClick={(ev) => { }} /></Link>
                        </Stack>
                     </Stack>
                  </Stack>
               </Stack>
               ...
               <Stack>
                  <Stack styles={{ root: { paddingLeft: 4, paddingRight: 0, paddingBottom: 4 } }}>
                     <Stack>
                        <Label>Device Reservations History</Label>
                        {!!failureRate && deviceHealthRateBar(failureRate, 70, 15, `${failureRate}% Failure Rate`)}
                     </Stack>
