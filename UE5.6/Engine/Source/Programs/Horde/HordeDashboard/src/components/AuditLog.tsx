...
class AuditLogHandler {
   ...
   async update() {
      ...
      if (!this.agentId && !this.issueId && !this.templateAudit && !this.deviceId && !this.jobId) {
         return;
      }
...
export const AuditLogPanel: React.FC<{ agentId?: string, deviceId?: string, jobId?: string, issueId?: string, templateAudit?: TemplateAudit }> = observer(({ agentId, deviceId, jobId, issueId, templateAudit }) => {
   ...
   if (!agentId && !deviceId && !issueId && !templateAudit && !jobId) {
      return null;
   }
...
class AuditLine {
   ...
   static create(logType: AuditLogType, entry: AuditLogEntry, search?: string) {
      ...
      if (logType === AuditLogType.Device) {
         instance = new DeviceAuditLine(logType, entry);
      }

      if (logType === AuditLogType.Job) {
         instance = new JobAuditLine(logType, entry);
      }
      ...
      let renderedTags = tags.map((tag, index) => {
         ...
         if (tag === "StepId") {
            ...
            if (stepId && jobId) {
               ...
               element = <Link to={to} >{this.highlight(key, stepId)}</Link>
            }
         ...
         if (tag === "Message") {
            ...
            if (message && message.includes("\n")) {
               element = <pre style={{margin: 0}}>{this.highlight(key, message)}</pre>;
            }
         }
...
const renderAuditEntry = (entry: AuditLogEntry, search?: string) => {
   ...
   if (handler.isDeviceLog) {
      type = AuditLogType.Device;
   }

   if (handler.isJobLog) {
      type = AuditLogType.Job;
   }
