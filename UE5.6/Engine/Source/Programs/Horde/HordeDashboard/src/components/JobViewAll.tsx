...
import { GetLabelStateResponse, GetStepResponse, JobData, JobState, JobStepOutcome, LabelOutcome, LabelState, StepData, StreamData } from "../backend/Api";
...
const JobViewAllInner: React.FC<{ filter: JobFilterSimple }> = observer(({ filter }) => {
      ...
      return <Stack>
         <div id={target}>
            <Link to={url}><Stack className={hordeClasses.badgeNoIcon}>
               <DefaultButton key={label.dashboardName} style={{ backgroundColor: color.primaryColor, fontSize: 9 }} text={label.dashboardName}>
   ...
   const renderLabels = (item: JobItem): JSX.Element => {
      ...
      if (!catergories.has("Other") && defaultLabel) {
         ...
         defaultLabel.dashboardCategory = "Other";
