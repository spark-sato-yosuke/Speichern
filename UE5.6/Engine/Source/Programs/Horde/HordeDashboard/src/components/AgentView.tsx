...
class LocalState {
   ...
   private _updateColumnDefs() {
      this.columnsState.forEach(colState => {
         ...
         // True when colState is for a column type that can be selected for the last column
         const bIsSwapColumn: boolean = (
            colState.key === "software"
         || colState.key === "storage"
         || colState.key.indexOf("systemInfo") !== -1
         || colState.key === "taskTime"
         || colState.key === "comment"
         );
         ...
         colState.columnDef = {
            ...
            styles: bIsSwapColumn ? {
                  cellName: {
                     flexGrow: '1' // Grow the cell name element so there can be space between it's elements
                  }
            } : undefined,
            onRenderHeader: bIsSwapColumn ? () => {
               return <Stack horizontal>
                  <span style={{flexGrow: 1, textAlign: 'center'}}>{colState.displayText}</span>
                  <TooltipHost
                     content="Change column content"
                  >
                     <IconButton
                        iconProps={{ iconName: 'Edit' }}
                        onClick={(ev) => {
                           ev.preventDefault();
                           ev.stopPropagation(); // Consume the input before it causes a sort re-order
