...
export const AutosubmitInfo: React.FC<{ jobDetails: JobDetailsV2 }> = observer(({ jobDetails }) => {
   ...
   if (job.autoSubmitChange) {
      if (dashboard.swarmUrl) {
         ...
         return <Stack style={{ paddingTop: 24 }}><Text>Automatically submitted in {`CL ${job.autoSubmitChange}`}</Text></Stack>
      }
   }
   ...
   if (job.state !== JobState.Complete) {
      ...
      if (job.preflightChange) {
         return <Stack style={{ paddingTop: 24 }}><Checkbox label={`Automatically submit changelist ${job.preflightChange} upon preflight success`}
