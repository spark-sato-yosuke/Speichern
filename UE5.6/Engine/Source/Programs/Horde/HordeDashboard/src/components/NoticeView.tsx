...
import { DefaultButton, DetailsList, DetailsListLayoutMode, Dialog, DialogFooter, DialogType, DirectionalHint, IColumn, Icon, Modal, PrimaryButton, SelectionMode, Spinner, SpinnerSize, Stack, Text, TextField, TooltipHost } from "@fluentui/react";
...
const NoticePanel: React.FC = observer(() => {
   ...
   const renderItem = (item: GetNoticeResponse, index?: number, column?: IColumn) => {
      ...
      if (column?.name === "Start Time") {
         ...
         return <Stack style={{width: 25}}>
               <TooltipHost
                  content={'User notices displayed immediately'}
      ...
      if (column?.name === "Finish Time") {
         ...
         return <Stack style={{width: 25}}>
            <TooltipHost
               content={'User notices displayed until issue is resolved'}
               directionalHint={DirectionalHint.bottomCenter}>
               <Text>{'N/A'}</Text>
            </TooltipHost>
         </Stack>
