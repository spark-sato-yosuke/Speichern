...
class MetricsHandler {
   ...
   getChartLegend(chartName: string): LegendEntry[] {
      ...
      function getPrefix(words: string[]) {
         ...
         const length = words[0].split(":").length;
         ...
         if (words.find(w => w.split(":").length !== length))
         {
            return "";
         }
         ...
         {
            elementMap.set(i, new Set);
         }

         words.forEach(w => {

            const elements = w.split(":");

            elements.forEach((element, index) => {
               elementMap.get(index)!.add(element);
            })
         })
         ...
         {
            if (elementMap.get(count)!.size > 1)
            {
               break;
            }

            replace.push(Array.from(elementMap.get(count)!)[0])            
         }         

         let replaceValue = replace.join(":");
         if (replaceValue) {
            replaceValue += ":";
         }
