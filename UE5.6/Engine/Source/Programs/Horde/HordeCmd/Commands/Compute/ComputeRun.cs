...
namespace Horde.Commands.Compute
{
	...
	class ComputeRun : Command
	{
		...
		[CommandLine("-Pool")]
		[Description("ID of pool to run this task in")]
		...
		{
			await using IBlobWriter writer = storage.CreateBlobWriter(cancellationToken: cancellationToken);

			IHashedBlobRef<DirectoryNode> sandbox = await writer.AddFilesAsync(taskFile.Directory, cancellationToken: cancellationToken);
