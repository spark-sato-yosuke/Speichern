			...
			if (User.HasAdminClaim())
			{
				...
				foreach (SubsystemStatus status in statuses)
				{
					...
					if (update != null)
					{
						...
						{
							string message = $"Server is reporting a {status.Name} issue.";

							// Add links to relevant status pages within Horde
							if (status.Name == "Perforce")
							{
								message += " See[Perforce servers page](/perforce/servers) for more info.";
							}
							else
							{
								message += " See[status page](/serverstatus) for more info.";
							}
