...
namespace HordeServer.Tests.Issues
{
	...
	public class IssueServiceTests : BuildTestSetup
	{
		...
		PerforceServiceStub _perforce = default!;
		UserId _jerryId;
		UserId _bobId;
		UserId _chrisId;
		DirectoryReference _autoSdkDir = default!;
		DirectoryReference _workspaceDir = default!;
		...
		public async Task SetupAsync()
		{
			...
			_chrisId = chris.Id;
			_jerryId = (await UserCollection.FindOrAddUserByLoginAsync("Jerry")).Id;
			_bobId = (await UserCollection.FindOrAddUserByLoginAsync("Bob")).Id;
		...
		public async Task GenericErrorTestAsync()
		{
			...
			// Expected: Creates issue, blames submitters at CL 110, 115, 120
			{
				...
				Assert.AreEqual(_chrisId, suspects[0].AuthorId);
		...
		public async Task DefaultOwnerTestAsync()
		{
			...
			// Expected: Creates issue, blames submitters at CL 110, 115, 120
			{
				...
				Assert.IsTrue(primarySuspects.Contains(_chrisId)); // 120
			}
			...
			// Expected: Creates issue, blames submitter at CL 120
			{
				...
				IIssue issue = issues[0];
				Assert.AreEqual(_chrisId, issue.OwnerId);
		...
		public async Task DeclineIssueTestAsync()
		{
			...
			// Expected: Only suspect is Jerry, but owner is still unassigned
			{
				IReadOnlyList<IIssue> issues = await IssueCollection.FindIssuesAsync();
				Assert.AreEqual(1, issues.Count);
				await IssueService.UpdateIssueAsync(issues[0].Id, declinedById: _chrisId);
		...
		public async Task GauntletTestAsync()
		{
			...
			{
				IJob job = CreateJob(_mainStreamId, 160, "Test Build", _graph);
				await using (TestJsonLogger logger = await CreateLoggerAsync(job, 0, 0))
				{
				...
				IJob job2 = CreateJob(_mainStreamId, 170, "Test Build", _graph);
			...
			{
				IJob job = CreateJob(_mainStreamId, 180, "Test Build", _graph);
				await using (TestJsonLogger logger = await CreateLoggerAsync(job, 0, 0))
				{
					logger.LogWarning(KnownLogEvents.Gauntlet_DeviceEvent, "Failed to setup device");
					logger.LogError(KnownLogEvents.Gauntlet, "Generic Gauntlet failure");
				}
				IJob job2 = CreateJob(_mainStreamId, 190, "Test Build", _graph);
				await using (TestJsonLogger logger = await CreateLoggerAsync(job2, 0, 0))
				{
					logger.LogWarning(KnownLogEvents.Gauntlet_DeviceEvent, "Failed to setup device");
					logger.LogError(KnownLogEvents.Gauntlet, "Generic Gauntlet failure");
