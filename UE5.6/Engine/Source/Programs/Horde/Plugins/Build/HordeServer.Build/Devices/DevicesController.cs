...
namespace HordeServer.Devices
{
	...
	public class DevicesController : ControllerBase
	{
		...
		/// Constructor
		/// </summary>
		public DevicesController(IUserCollection userCollection, DeviceService deviceService, IOptionsMonitor<BuildConfig> buildConfig, ILogger<DevicesController> logger)
		{
		...
		{
			DevicePoolAuthorization? poolAuth = _deviceService.GetUserPoolAuthorization(new DevicePoolId(deviceRequest.PoolId!), User, _buildConfig.CurrentValue);
		...
		{
			List<DevicePoolAuthorization> poolAuth = _deviceService.GetUserPoolAuthorizations(User, _buildConfig.CurrentValue);
		...
		public async Task<ActionResult<object>> GetDeviceAsync(string deviceId)
		{
			...
			DevicePoolAuthorization? poolAuth = _deviceService.GetUserPoolAuthorization(device.PoolId, User, _buildConfig.CurrentValue);
		...
		public async Task<ActionResult> UpdateDeviceAsync(string deviceId, [FromBody] UpdateDeviceRequest update, CancellationToken cancellationToken = default)
		{
			...
			if (update.ModelId != null)
			{
				...
				if (modelIdValue != "Base" && platform.Models?.FirstOrDefault(x => x == modelIdValue) == null)
				{
			...
			await _deviceService.UpdateDeviceAsync(deviceIdValue, poolIdValue, name, address, modelIdValue, update.Notes, update.Enabled, update.Problem, update.Maintenance, internalUser?.Id, update.Message);
		...
		[HttpPut]
		[Authorize]
		[Route("/api/v2/devices/{deviceId}/clean")]		
		public async Task<ActionResult> CleanDeviceAsync(string deviceId, [FromBody] CleanDeviceRequest request)
		{
			...
			await _deviceService.CleanDeviceAsync(device.Id, request.Clean);
		...
		{
			if (!DeviceService.Authorize(DeviceAclAction.DeviceWrite, User, _buildConfig.CurrentValue))
			{
				return Forbid();
			}
		...
		{
			if (!DeviceService.Authorize(DeviceAclAction.DeviceRead, User, _buildConfig.CurrentValue))
			{
				return Forbid();
			}
		...
		/// <summary>
		/// Retrieve historical information about a specific device
		/// </summary>
		/// <param name="deviceId">Id of the device to get information about</param>
		...
		[HttpGet]
		[Route("/api/v1/devices/{deviceId}/history")]
		public async Task<ActionResult> GetDeviceHistoryAsync(DeviceId deviceId, [FromQuery] DateTime? minTime = null, [FromQuery] DateTime? maxTime = null, [FromQuery] int index = 0, [FromQuery] int count = 50)
		{
			if (!DeviceService.Authorize(DeviceAclAction.DeviceRead, User, _buildConfig.CurrentValue))
			{
				return Forbid();
			...
			await _deviceService.GetDeviceLogger(deviceId).FindAsync(Response.BodyWriter, minTime, maxTime, index, count);
		...
		public async Task<ActionResult> PutDeviceErrorAsync(string deviceName, [FromBody] DeviceErrorRequest? request = null)
		{
			IDevice? device = await _deviceService.GetDeviceByNameAsync(deviceName);
			...
			await _deviceService.UpdateDeviceAsync(device.Id, newProblem: true, message: request?.Message);
