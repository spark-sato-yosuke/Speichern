...
namespace HordeServer.Perforce
{
	...
	/// </summary>
	public sealed class PerforceLoadBalancer : IHostedService, IHealthCheck, IAsyncDisposable
	{
		/// <summary>
		...
		record class HealthTask(Task Task, Stopwatch Timer);
		...
		readonly CancellationTokenSource _healthTaskCts = new CancellationTokenSource();
		readonly SemaphoreSlim _healthTaskSema = new SemaphoreSlim(1);
		readonly Dictionary<string, HealthTask> _healthTasks = new Dictionary<string, HealthTask>(StringComparer.OrdinalIgnoreCase);

		/// <summary>
		/// Constructor
		/// </summary>
		public PerforceLoadBalancer(IMongoService mongoService, IRedisService redisService, ILeaseCollection leaseCollection, IClock clock, HttpClient httpClient, IOptionsMonitor<BuildConfig> buildConfig, IHealthMonitor<PerforceLoadBalancer> health, Tracer tracer, ILogger<PerforceLoadBalancer> logger)
		{
		...
		public async ValueTask DisposeAsync()
		{
			...
			if (_healthTasks.Count > 0)
			{
				await _healthTaskCts.CancelAsync();
				foreach (HealthTask healthTask in _healthTasks.Values)
				{
					await healthTask.Task;
				}
				_healthTasks.Clear();
			}

			_healthTaskCts.Dispose();
			_healthTaskSema.Dispose();
		}

		/// <inheritdoc/>
		public async Task<HealthCheckResult> CheckHealthAsync(HealthCheckContext context, CancellationToken cancellationToken = default)
		{
			using (await _healthTaskSema.WaitDisposableAsync(cancellationToken))
			{
				KeyValuePair<string, HealthTask> longestTask = _healthTasks.OrderByDescending(x => x.Value.Timer.Elapsed).FirstOrDefault(x => !x.Value.Task.IsCompleted);
				if (longestTask.Key == null || longestTask.Value.Timer.Elapsed < TimeSpan.FromMinutes(5.0))
				{
					return HealthCheckResult.Healthy();
				}
				else
				{
					return HealthCheckResult.Unhealthy($"Health check for {longestTask.Key} has taken {(int)longestTask.Value.Timer.Elapsed.TotalSeconds}s");
				}
			}
		}
		...
		{
			_logger.LogDebug("Ticking server health status");
			...
			foreach (PerforceCluster cluster in buildConfig.PerforceClusters)
			{
			...
			// Lock the task lookup and update server health
			using (await _healthTaskSema.WaitDisposableAsync(cancellationToken))
			{
				// Remove any health check tasks that have finished
				KeyValuePair<string, HealthTask>[] healthTasks = _healthTasks.ToArray();
				foreach ((string serverAndPort, HealthTask healthTask) in healthTasks)
				{
					if (healthTask.Task.IsCompleted)
					{
						await healthTask.Task;
						_healthTasks.Remove(serverAndPort);
					}
					else if (healthTask.Timer.Elapsed > TimeSpan.FromMinutes(1.0))
					{
						_logger.LogWarning("Health check for {ServerAndPort} is still running after {TimeSecs}s", serverAndPort, (int)healthTask.Timer.Elapsed.TotalSeconds);
					}
				}
				...
				{
					if (!_healthTasks.ContainsKey(entry.ServerAndPort))
					{
						Task task = Task.Run(() => UpdateHealthGuardedAsync(entry, _healthTaskCts.Token), CancellationToken.None);
						_healthTasks.Add(entry.ServerAndPort, new HealthTask(task, Stopwatch.StartNew()));
					}
				}
			}
		...
		static (HealthStatus health, string? message) GetPerforceHealth(List<PerforceServerEntry> servers)
		{
			...
			{
				if (server.Status != PerforceServerStatus.Unknown)
				{
		...
		async Task UpdateHealthGuardedAsync(PerforceServerEntry entry, CancellationToken cancellationToken)
		{
			try
			{
				await UpdateHealthAsync(entry, cancellationToken);
			}
			...
			{
				_logger.LogError(ex, "Error updating health for {ServerAndPort}: {Message}", entry.ServerAndPort, ex.Message);
			}
		}
		...
		async Task<ServerHealth> GetServerHealthViaHttpAsync(string healthCheckUrl, CancellationToken cancellationToken)
		{
			...
			{
				_logger.LogWarning("Failed checking Perforce server health via HTTP. URL {HealthCheckUrl} Reason {Reason}", healthCheckUrl, ex.Message);
		...
		internal static async Task<ServerHealth> GetServerHealthViaP4InfoAsync(string serverAndPort, ILogger logger, CancellationToken cancellationToken)
		{
			...
			{
				logger.LogWarning("Failed checking Perforce server health via info command. Reason {Reason}", ex.Message);
