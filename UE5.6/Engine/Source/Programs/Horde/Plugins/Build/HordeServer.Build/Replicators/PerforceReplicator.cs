...
namespace HordeServer.Replicators
{
	...
	/// </summary>
	public class PerforceReplicator
	{
		...
		public async Task<IReplicator> RunOnceAsync(IReplicator replicator, BuildConfig buildConfig, StreamConfig streamConfig, ReplicatorConfig replicatorConfig, CancellationToken cancellationToken)
		{
			...
			IStorageNamespace store = _storageService.GetNamespace(replicatorConfig.NamespaceId);
			...
			if (replicator.Reset)
			{
				...
				await store.RemoveRefAsync(refName, cancellationToken);
				await store.RemoveRefAsync(incRefName, cancellationToken);
		...
		async Task<IReplicator> WriteInternalAsync(IReplicator replicator, int change, BuildConfig buildConfig, StreamConfig streamConfig, ReplicatorConfig replicatorConfig, BlobSerializerOptions blobOptions, CancellationToken cancellationToken = default)
		{
			...
			ReplicationClient clientInfo = await FindOrAddReplicationClientAsync(buildConfig, streamConfig, cancellationToken);
			...
			while (directories.Count > 0)
			{
				...
				await store.AddRefAsync(incRefName, stateNodeRef, cancellationToken: cancellationToken);
				rootUpdate.Clear();
			...
			await store.AddRefAsync(refName, redirectNodeRef, cancellationToken: cancellationToken);
		...
		async Task<ReplicationClient> FindOrAddReplicationClientAsync(BuildConfig buildConfig, StreamConfig streamConfig, CancellationToken cancellationToken = default)
		{
			...
			if (clientInfo == null)
			{
				...
				PerforceCluster cluster = buildConfig.GetPerforceCluster(streamConfig.ClusterName);
