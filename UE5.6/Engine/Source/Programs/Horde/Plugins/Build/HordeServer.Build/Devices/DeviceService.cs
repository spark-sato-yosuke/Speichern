...
namespace HordeServer.Devices
{
	...
	public sealed class DeviceService : IHostedService, IAsyncDisposable
	{
		...
		/// <summary>
		/// Get the logger for the specified device id
		/// </summary>
		/// <param name="deviceId"></param>
		/// <returns></returns>
		public IAuditLogChannel<DeviceId> GetDeviceLogger(DeviceId deviceId)
		{
			...
			if (!cancellationToken.IsCancellationRequested)
			{
				...
				try
				{
					...
					if (expireNotifications != null && expireNotifications.Count > 0)
					{
						...
						{
							await NotifyDeviceServiceAsync(buildConfig, $"Device {expiredDevice.Item2.PlatformId.ToString().ToUpperInvariant()} / {expiredDevice.Item2.Name} checkout will expire in 24 hours.  Please visit {_serverInfo.DashboardUrl}/devices to renew the checkout if needed.", null, null, null, expiredDevice.Item1, cancellationToken);
						}
					}
		...
		{
			return _buildConfig.CurrentValue.Devices?.Platforms.ConvertAll(x => (IDevicePlatform)x) ?? new List<IDevicePlatform>();
		}
		/// <summary>
		...
		{
			return _buildConfig.CurrentValue.Devices?.Pools.ConvertAll(x => (IDevicePool)x) ?? new List<IDevicePool>();
		}
		...
		/// Update a device
		/// </summary>
		public async Task UpdateDeviceAsync(DeviceId deviceId, DevicePoolId? newPoolId = null, string? newName = null, string? newAddress = null, string? newModelId = null, string? newNotes = null, bool? newEnabled = null, bool? newProblem = null, bool? newMaintenance = null, UserId? modifiedByUserId = null, string? message = null)
		{
			if (newProblem == true)
			{
			...
			await _devices.UpdateDeviceAsync(deviceId, newPoolId, newName, newAddress, newModelId, newNotes, newEnabled, newProblem, newMaintenance, modifiedByUserId, message);
		}
		...
		/// Clean a device
		/// </summary>
		public async Task CleanDeviceAsync(DeviceId deviceId, bool cleaning)
		{
			await _devices.UpdateDeviceCleaningAsync(deviceId, cleaning);
		}
		...
		/// 
		/// </summary>
		public async Task NotifyDeviceServiceAsync(BuildConfig buildConfig, string message, DeviceId? deviceId = null, string? jobId = null, string? stepId = null, UserId? userId = null, CancellationToken cancellationToken = default)
		{
			try
			{
				IDevice? device = null;
				...
				if (jobId != null)
				{
					...
					if (job != null)
					{
						buildConfig.TryGetStream(job.StreamId, out streamConfig);
		...
		/// <returns></returns>
		public static bool Authorize(AclAction action, ClaimsPrincipal user, BuildConfig buildConfig)
		{
			...
			if (buildConfig.Authorize(AdminAclAction.AdminWrite, user))
			{
				return true;
			}
		...
		/// <param name="buildConfig"></param>
		/// <returns></returns>
		public List<DevicePoolAuthorization> GetUserPoolAuthorizations(ClaimsPrincipal user, BuildConfig buildConfig)
		{
			List<DevicePoolAuthorization> authPools = new List<DevicePoolAuthorization>();

			List<IDevicePool> allPools = GetPools();
			IReadOnlyList<ProjectConfig> projects = buildConfig.Projects;
			...
			foreach (ProjectId projectId in projectIds)
			{
				...
				if (buildConfig.TryGetProject(projectId, out ProjectConfig? projectConfig))
				{
		...
		/// <param name="buildConfig"></param>
		/// <returns></returns>
		public DevicePoolAuthorization? GetUserPoolAuthorization(DevicePoolId id, ClaimsPrincipal user, BuildConfig buildConfig)
		{
			List<DevicePoolAuthorization> auth = GetUserPoolAuthorizations(user, buildConfig);
