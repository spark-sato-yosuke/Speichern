...
{
	/// <summary>
	/// Exception raised by errors in <see cref="PerforceService" />
	/// </summary>
	public class PerforceServiceException(string? message) : Exception(message);
	...
	class PerforceService : IPerforceService, IAsyncDisposable
	{
		...
		Credentials GetServiceUserCredentials(PerforceCluster cluster)
		{
			...
			else if (cluster.ServiceAccount != null)
			{
				...
				if (credentials == null)
				{
					throw new PerforceServiceException($"No credentials defined for {cluster.ServiceAccount} on {cluster.Name}");
				}
		...
		{
			PerforceCluster cluster = _buildConfig.CurrentValue.GetPerforceCluster(clusterName);
		...
		async Task<IPerforceServer> GetServerAsync(PerforceCluster cluster, CancellationToken cancellationToken)
		{
			...
			if (server == null)
			{
				throw new PerforceServiceException($"Unable to select server from '{cluster.Name}'");
			}
			return server;
		}
		...
		public async Task<ICommit> GetChangeDetailsAsync(StreamConfig streamConfig, int changeNumber, CancellationToken cancellationToken)
		{
			...
			PerforceCluster cluster = _buildConfig.CurrentValue.GetPerforceCluster(streamConfig.ClusterName);
				...
				for (; ; )
				{
					...
					try
					{
						...
						if (files.Count == 0)
						{
							...
							{
								throw new PerforceServiceException($"Invalid directory name for local client file, disk file path: {diskFilePath}");
							}
							...
							if (!Directory.Exists(directoryName))
							{
								...
								if (!Directory.Exists(directoryName))
								{
									throw new PerforceServiceException($"Unable to create directrory: {directoryName}");
								}
							}
							...
							if (!File.Exists(diskFilePath))
							{
								...
								if (!File.Exists(diskFilePath))
								{
									throw new PerforceServiceException($"Unable to create local change file: {diskFilePath}");
								}
							}
							...
							if (addFiles.Count != 1)
							{
								throw new PerforceServiceException($"Unable to add local change file,  local: {diskFilePath} : workspace: {workspaceFilePath}");
							}
						...
						else
						{
							...
							{
								throw new PerforceServiceException($"Unable to sync file, workspace: {workspaceFilePath}");
							}
							...
							{
								throw new PerforceServiceException($"Unable to edit file, workspace: {workspaceFilePath}");
							}
						...
						{
							throw new PerforceServiceException($"Unable to get depot path for: {workspaceFilePath}");
						}
		...
		protected class CommitSource : ICommitCollection
		{
			...
			public async Task<CommitIdWithOrder> CreateNewAsync(string path, string description, CancellationToken cancellationToken = default)
			{
				...
				catch (PerforceServiceException e)
				{
					throw new CommitCollectionException("Failed creating new VCS change", e);
				}
			}
