...
namespace HordeServer.Devices
{
	...
	public class DeviceCollection : IDeviceCollection
	{
		...
		class DeviceDocument : IDevice
		{
			...
			/// <summary>
			/// The time the last clean started,
			/// </summary>
			[BsonIgnoreIfNull]
			public DateTime? CleanStartTimeUtc { get; set; }

			/// <summary>
			/// The time the least clean finished
			/// </summary>
			[BsonIgnoreIfNull]
			public DateTime? CleanFinishTimeUtc { get; set; }

			/// <summary>
			/// Whether a clean is required
			/// </summary>
			[BsonIgnoreIfNull]
			public bool? CleanRequired { get; set; }
		...
		readonly IAuditLog<DeviceId> _auditLog;
		readonly ILogger<DeviceCollection> _logger;

		/// <summary>
		/// Constructor
		/// </summary>
		public DeviceCollection(IMongoService mongoService, IUserCollection userCollection, ILogger<DeviceCollection> logger, IAuditLog<DeviceId> auditLog)
		{
			_auditLog = auditLog;
			_logger = logger;
		...
		public async Task CheckoutDeviceAsync(DeviceId deviceId, UserId? checkedOutByUserId)
		{
			...
			if (checkedOutByUserId != null)
			{
				...
				updates.Add(updateBuilder.Set(x => x.CleanRequired, false));
			}
			else
			{
				updates.Add(updateBuilder.Set(x => x.CheckoutExpiringNotificationSent, true));
				// Mark device as needing cleaning
				updates.Add(updateBuilder.Set(x => x.CleanRequired, true));
			}
		...
		/// <inheritdoc/>
		public async Task UpdateDeviceAsync(DeviceId deviceId, DevicePoolId? newPoolId, string? newName, string? newAddress, string? newModelId, string? newNotes, bool? newEnabled, bool? newProblem, bool? newMaintenance, UserId? modifiedByUserId = null, string? message = null)
		{
			UpdateDefinitionBuilder<DeviceDocument> updateBuilder = Builders<DeviceDocument>.Update;
			List<UpdateDefinition<DeviceDocument>> updates = new List<UpdateDefinition<DeviceDocument>>();
			...
			IAuditLogChannel<DeviceId> logger = GetDeviceLogger(deviceId);
			IDevice? current = await _devices.Find<DeviceDocument>(x => x.Id == deviceId).FirstOrDefaultAsync();
			IUser? user = null;
			if (modifiedByUserId != null)
			{
				user = await _userCollection.GetCachedUserAsync(modifiedByUserId);
			}
			...
			if (modifiedByUserId != null)
			{
				...
				if (log && current!.ModifiedByUser != modifiedByUserId.ToString())
				{					
					logger.LogInformation("Modified by user {UserName}", user?.Name ?? modifiedByUserId.ToString());
				}				
			}
			...
			if (newPoolId != null)
			{
				...
				if (log && current!.PoolId != newPoolId.Value)
				{
					logger.LogInformation("Assigned to pool {PoolId}", newPoolId.Value.ToString());
				}				
			}
			...
			if (newName != null)
			{
				...
				if (log && current!.Name != newName)
				{
					if (user?.Name == null)
					{
						logger.LogInformation("Renamed to {DeviceName}", newName);
					}
					else
					{
						logger.LogInformation("Renamed to {DeviceName} by {UserName}", newName, user.Name);
					}					
				}				
			}
			...
			if (newEnabled != null)
			{
				...
				if (log && current!.Enabled != newEnabled)
				{
					if (newEnabled.Value)
					{
						if (user?.Name == null)
						{
							logger.LogInformation("Device enabled");
						}
						else
						{
							logger.LogInformation("Device enabled by {UserName}", user.Name);
						}							
					}
					else
					{
						if (user?.Name == null)
						{
							logger.LogInformation("Device disabled");
						}
						else
						{
							logger.LogInformation("Device disabled by {UserName}", user.Name);
						}
					}
				}
			}
			...
			if (newAddress != null)
			{
				...
				if (log && current!.Address != newAddress)
				{
					if (user?.Name == null)
					{
						logger.LogInformation("Device address changed to {DeviceAddress}", newAddress);
					}
					else
					{
						logger.LogInformation("Device address changed to {DeviceAddress} by {UserName}", newAddress, user.Name);
					}
				}
			}
			...
			if (!String.IsNullOrEmpty(newModelId))
			{
				if (newModelId == "Base")
				{
					...
					if (log && current!.ModelId != null)
					{
						if (user?.Name == null)
						{
							logger.LogInformation("Device model changed to Base");
						}
						else
						{
							logger.LogInformation("Device model changed to Base by {UserName}", user.Name);
						}
					}
				}
				else
				{
					...
					if (log && current!.ModelId != newModelId)
					{						
						if (user?.Name == null)
						{
							logger.LogInformation("Device model changed to {DeviceModel}", newModelId);
						}
						else
						{
							logger.LogInformation("Device model changed to {DeviceModel} by {UserName}", newModelId, user.Name);
						}
					}
				}
			}
			...
			if (newMaintenance.HasValue)
			{
				...
				if (log && current!.MaintenanceTimeUtc != maintenanceTime)
				{
					if (maintenanceTime != null)
					{
						if (user?.Name == null)
						{
							logger.LogInformation("Device put into maintenance");
						}
						else
						{
							logger.LogInformation("Device put into maintenance by {UserName}", user.Name);
						}
					}
					else
					{
						if (user?.Name == null)
						{
							logger.LogInformation("Device taken out of maintenance");
						}
						else
						{
							logger.LogInformation("Device taken out of maintenance by {UserName}", user.Name);
						}
					}
				}
			}
			...
			if (newProblem.HasValue && newProblem.Value)
			{
				...
				if (reservation != null)
				{
					if (log)
					{
						 logger.LogInformation("Device reported a problem in step: {StepName} - job {JobId}, step {StepId})", reservation.StepName ?? "Unknown Step", reservation.JobId ?? "Unknown Job Id", reservation.StepId ?? "Unknown Step Id");
					}
				...
				else
				{
					if (log)
					{
						logger.LogInformation("Device reported a problem without reservation information");
					}
				}
			}
		...
		/// <inheritdoc/>
		public async Task UpdateDeviceCleaningAsync(DeviceId deviceId, bool? cleaning = null, bool? cleanRequired = null)
		{
			UpdateDefinitionBuilder<DeviceDocument> updateBuilder = Builders<DeviceDocument>.Update;
			List<UpdateDefinition<DeviceDocument>> updates = new List<UpdateDefinition<DeviceDocument>>();
			...
			if (current == null)
			{
				_logger.LogInformation("Device cleaning updated for non-existant {DeviceId}", deviceId.ToString());
				return;
			}
			...
			if (cleaning.HasValue)
			{
				if (cleaning.Value)
				{
					logger.LogInformation("Starting Clean");
					updates.Add(updateBuilder.Set(x => x.CleanStartTimeUtc, utcNow));
					updates.Add(updateBuilder.Set(x => x.CleanFinishTimeUtc, null));
					updates.Add(updateBuilder.Set(x => x.CleanRequired, false));
				}				
				else
				{
					logger.LogInformation("Finished Clean");
					updates.Add(updateBuilder.Set(x => x.CleanFinishTimeUtc, utcNow));
					updates.Add(updateBuilder.Set(x => x.CleanRequired, false));
				}
			}

			if (cleanRequired.HasValue)
			{
				logger.LogInformation("Marking as requiring a clean: {CleanRequired}", cleanRequired.Value);
				updates.Add(updateBuilder.Set(x => x.CleanRequired, cleanRequired.Value));
			}
		...
		public async Task<List<(UserId, IDevice)>?> ExpireCheckedOutAsync(int checkoutDays)
		{
			...
			if (expiredDevices.Count > 0)
			{
				...
				deviceUpdates.Add(deviceBuilder.Set(x => x.CleanRequired, true));
		...
		IAuditLogChannel<DeviceId> GetDeviceLogger(DeviceId deviceId)
		{
			return _auditLog[deviceId];
		}
	}
}
