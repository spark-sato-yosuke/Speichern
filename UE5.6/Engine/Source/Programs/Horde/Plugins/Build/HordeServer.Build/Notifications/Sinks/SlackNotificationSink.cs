...
namespace HordeServer.Notifications.Sinks
{
	...
	public sealed class SlackNotificationSink : IHostedService, INotificationSink, IAvatarService, IAsyncDisposable
	{
		...
		// Badges to add color accents
		const string YellowgBadge = ":large_yellow_square:";
		const string OrangeBadge = ":large_orange_square:";
		const string RedBadge = ":large_red_square:";
		const string BlackBadge = ":black_square:";
		...
		/// Constructor
		/// </summary>
		public SlackNotificationSink(IMongoService mongoService, IRedisService redisService, IssueService issueService, ICommitService commitService, IUserCollection userCollection, ILogCollection logCollection, IExternalIssueService externalIssueService, IWebHostEnvironment environment, IOptions<BuildServerConfig> settings, IClock clock, IServerInfo serverInfo, IOptionsMonitor<BuildConfig> buildConfig, ILogger<SlackNotificationSink> logger)
		{
		...
		private async Task SendJobCompleteMessageAsync(string recipient, StreamConfig streamConfig, IJob job, IGraph graph, CancellationToken cancellationToken)
		{
			...
			Uri jobLink = new Uri($"{_serverInfo.DashboardUrl}job/{job.Id}");
		...
		{
			// The Slack notification requires the list of user's to notify to be non-null
		...
		{
			Uri labelLink = new Uri($"{_serverInfo.DashboardUrl}job/{job.Id}?label={labelIdx}");
		...
		async ValueTask ProcessIssueQueueAsync(CancellationToken cancellationToken)
		{
			...
			for (; count > 0; count--)
			{
				...
				if (!testedIssueIds.Add(issueId) || !await TryUpdateIssueAsync(buildConfig, issueId, cancellationToken))
				{
		...
		async ValueTask<bool> TryUpdateIssueAsync(BuildConfig buildConfig, int issueId, CancellationToken cancellationToken)
		{
			using (RedisLock issueLock = new RedisLock(_redisService.GetDatabase(), $"{_redisIssueLockPrefix}/{issueId}"))
			{
				...
				{
					await NotifyIssueUpdatedInternalAsync(buildConfig, issueId, cancellationToken);
					return true;
				}
				else
				{
		...
		async Task NotifyIssueUpdatedInternalAsync(BuildConfig buildConfig, int issueId, CancellationToken cancellationToken)
		{
			...
			if (details.Spans.Count > 0)
			{
				...
				{
					StreamConfig? streamConfig;
					if (buildConfig.TryGetStream(span.StreamId, out streamConfig) && streamConfig.TryGetWorkflow(workflowId.Value, out workflow) && !String.IsNullOrEmpty(workflow.TriageChannel))
					{
						bool skip = (issue.Severity == IssueSeverity.Warning && !workflow.TriageWarnings) || (issue.Severity == IssueSeverity.Error && !workflow.TriageErrors);
						if (!skip)
						{
			...
			{
				StreamConfig? streamConfig;
				if (buildConfig.TryGetStream(span.StreamId, out streamConfig))
				{
			...
			if (userIds.Count > 0)
			{
				foreach (UserId userId in userIds)
				{
					...
					{
						await NotifyIssueUpdatedAsync(buildConfig, user, issue, details, cancellationToken);
					}
				}
			}
			...
			if (channels.Count > 0)
			{
				...
				{
					await SendIssueMessageAsync(buildConfig, channel, issue, details, null, DefaultAllowMentions, cancellationToken);
				}
			}
		...
		async Task CreateOrUpdateWorkflowThreadAsync(string triageChannel, IIssue issue, IIssueSpan span, IReadOnlyList<IIssueSpan> spans, WorkflowConfig workflow, CancellationToken cancellationToken)
		{
			...
			if (!isSecondThread)
			{
				...
				if (isNew)
				{
					...
					else
					{
						...
						if (suspectGroups.Length > 0 && suspectGroups.Length <= workflow.MaxMentions)
						{
							...
							Uri? robomergeTrackChangesLink = GetRobomergeTrackChangesUrlForSpan(span);
							if (robomergeTrackChangesLink != null)
							{
								suspectMessage += $" (<{robomergeTrackChangesLink}|Track changes>)";
							}
						...
						{
							string suspectMessage = String.Empty;

							Uri? swarmLink = GetSwarmLinkForSpan(span);
							if (swarmLink != null)
							{
								suspectMessage += $"<{swarmLink}|View changes in Swarm>";
							}

							Uri? commitsViewerLink = GetCommitsViewerLinkForSpan(span);
							if (commitsViewerLink != null)
							{
								suspectMessage += $" (<{commitsViewerLink}|View changes in Commits Viewer>)";
							}
							...
							if (!String.IsNullOrWhiteSpace(suspectMessage))
							{
								await _slackClient.PostMessageToThreadAsync(threadId, suspectMessage, cancellationToken);
							}
				...
				if (issue.FixCommitId != null)
				{
					if (fixFailedSpan == null)
					{
						...
						if (robomergeTrackChangesLink != null)
						{
							fixedMessage += $" (<{robomergeTrackChangesLink}|Track changes>)";
						}
					...
					else
					{
						...
						IIssueStream? fixStream = issue.Streams.FirstOrDefault(x => x.ContainsFix ?? false);
						...
						if (fixStream != null && fixStream.StreamId != fixFailedSpan.StreamId)
						{
							string fixStreamName = spans.FirstOrDefault(x => x.StreamId == fixStream.StreamId)?.StreamName ?? fixStream.StreamId.ToString();
							fixFailedMessage = $"Issue possibly not fixed by {FormatChange(issue.FixCommitId)}.  The fix CL may be in the process of automatically or manually merging from {fixStreamName} to {fixFailedSpan.StreamName}; see {FormatJobStep(fixFailedSpan.LastFailure, fixFailedSpan.NodeName)} at CL {fixFailedSpan.LastFailure.CommitId} in {fixFailedSpan.StreamName}.";
						}
			...
			await UpdateReactionsAsync(triageChannel, issue, fixFailedSpan, eventId, threadId, cancellationToken);
		...
		private async Task UpdateReactionsAsync(string triageChannel, IIssue issue, IIssueSpan? fixFailedSpan, string eventId, SlackMessageId threadId, CancellationToken cancellationToken)
		{
		...
		Uri? GetRobomergeTrackChangesUrlForSpan(IIssueSpan span)
		{
			if (_settings.RobomergeUrl == null || span.LastSuccess == null)
			{
				return null;
			}
			else
			{
				return new Uri(_settings.RobomergeUrl, $"trackchange/{span.FirstFailure.CommitId}");
			}
		}

		Uri? GetCommitsViewerLinkForSpan(IIssueSpan span)
		{
			if (_settings.CommitsViewerUrl == null || span.LastSuccess == null)
			{
				return null;
			}
			else
			{
				string depot_stream = span.StreamName.TrimStart('/');
				string[] depot_stream_split = depot_stream.Split(new[] { '/' }, 2);

				// If for some reason the stream is not split correctly for CV, return null
				if (depot_stream_split.Length != 2)
				{
					return null;
				}

				string query_string = $"?depot={depot_stream_split[0]}&stream={depot_stream_split[1]}" +
					$"&query_range[CL][start_cl]={span.LastSuccess.CommitId.GetPerforceChange() + 1}&query_range[CL][end_cl]={span.FirstFailure.CommitId}";

				return new Uri(_settings.CommitsViewerUrl, query_string);
			}
		}
		...
		async Task NotifyIssueUpdatedAsync(BuildConfig buildConfig, IUser user, IIssue issue, IIssueDetails details, CancellationToken cancellationToken)
		{
			string? slackUserId = await GetSlackUserIdAsync(user, cancellationToken);
			if (slackUserId == null)
			{
			...
			await SendIssueMessageAsync(buildConfig, slackUserId, issue, details, user.Id, DefaultAllowMentions, cancellationToken);
		}

		Uri GetJobUrl(JobId jobId)
		{
			return new Uri(_serverInfo.DashboardUrl, $"job/{jobId}");
		}

		Uri GetStepUrl(JobId jobId, JobStepId stepId)
		{
			return new Uri(_serverInfo.DashboardUrl, $"job/{jobId}?step={stepId}");
		}

		Uri GetIssueUrl(IIssue issue, IIssueStep step)
		{
			return new Uri(_serverInfo.DashboardUrl, $"job/{step.JobId}?step={step.StepId}&issue={issue.Id}");
		}

		async Task SendIssueMessageAsync(BuildConfig buildConfig, string recipient, IIssue issue, IIssueDetails details, UserId? userId, bool allowMentions, CancellationToken cancellationToken)
		{
			...
			Uri issueUrl = _serverInfo.DashboardUrl;
			...
			{
				StreamConfig? streamConfig;
				if (buildConfig.TryGetStream(span.StreamId, out streamConfig) && streamConfig.TryGetWorkflow(workflowId.Value, out WorkflowConfig? workflow) && !String.IsNullOrEmpty(workflow.TriageChannel) && workflow.AllowMentions)
				{
					bool skip = (issue.Severity == IssueSeverity.Warning && !workflow.TriageWarnings) || (issue.Severity == IssueSeverity.Error && !workflow.TriageErrors);
					if (!skip)
					{ 
			...
			if (issue.FixCommitId != null)
			{
				...
				if (fixFailedStep != null)
				{
					...
					string rbTrackChangeText = FormatRobomergeTrackChangeLinkChange(issue.FixCommitId);
					if (!String.IsNullOrWhiteSpace(rbTrackChangeText))
					{
						text += rbTrackChangeText;
					}
				}
				else
				{
		...
		{
			string output = _settings.P4SwarmUrl != null
				? $"<{new Uri(_settings.P4SwarmUrl, $"changes/{change}")}|CL {change}>"
				: $"CL {change}";

			if (_settings.RobomergeUrl != null)
			{
				output += FormatRobomergeTrackChangeLinkChange(change);
			}

			return output;
		}

		string FormatRobomergeTrackChangeLinkChange(CommitId change)
		{
			return _settings.RobomergeUrl != null
				? $" (<{new Uri(_settings.RobomergeUrl, $"trackchange/{change}")}|Track changes>)"
				: String.Empty;
		}
		...
		public async Task SendIssueReportAsync(IssueReportGroup group, CancellationToken cancellationToken)
		{
			...
			{
				await SendIssueReportForStreamAsync(buildConfig, group.Channel, group.Time, report, cancellationToken);
			}
		...
		async Task SendIssueReportForStreamAsync(BuildConfig buildConfig, string channel, DateTime time, IssueReport report, CancellationToken cancellationToken)
		{
			const int MaxIssuesPerMessage = 8;

			StreamConfig? streamConfig;
			if (!buildConfig.TryGetStream(report.StreamId, out streamConfig))
			{
				return;
			}
		...
		async Task UpdateReportsAsync(BuildConfig buildConfig, IIssue issue, IReadOnlyList<IIssueSpan> spans, CancellationToken cancellationToken)
		{
		...
		async Task UpdateReportBlockAsync(string channel, string eventId, DateTime reportTime, StreamConfig streamConfig, TemplateId templateId, List<(IIssue, IIssueSpan?, bool)> issues, string? triageChannel, bool templateHeader, CancellationToken cancellationToken)
		{
			...
			if (templateHeader && !templateId.IsEmpty)
			{
				...
				if (streamConfig.TryGetTemplate(templateId, out templateConfig))
				{
					...
					if (tab != null)
					{
						Uri templateUrl = new Uri(_serverInfo.DashboardUrl, $"stream/{streamConfig.Id}?tab={tab.Title}&template={templateId}");
		...
		public async Task SendDeviceIssueReportAsync(DeviceIssueReport report, CancellationToken cancellationToken)
		{
			if (report.PoolReports.Count > 0)
			{
				...
				{
					List<SectionBlock> blocks = new List<SectionBlock>();
					foreach (DevicePoolMetrics metrics in pool.Metrics)
					{
						StringBuilder builder = new StringBuilder();

						string badge = YellowgBadge;

						if (metrics.AverageLoadPercentage >= 40 || metrics.MaxConcurrentProblemsPercentage >= 50)
						{
							badge = RedBadge;
						}
						else if (metrics.AverageLoadPercentage >= 20 || metrics.MaxConcurrentProblemsPercentage >= 30)
						{
							badge = OrangeBadge;
						}
						else if (metrics.Total == (metrics.Disabled + metrics.Maintenance))
						{
							badge = BlackBadge;
						}
						else if (metrics.Problems == 0 && metrics.SaturationSpikes == 0)
						{
							continue;
						}
						...
						builder.Append($"<{metrics.PlatformURL}|*{metrics.PlatformName}*>\n> ");
						builder.Append($"Average Load: {metrics.AverageLoadPercentage}% | ");

						if (metrics.SaturationSpikes > 0)
						{
							builder.Append($"Saturation Spikes: {metrics.SaturationSpikes} | ");
							builder.Append($"Average Spike Duration: {metrics.SpikeDurationAverage.ToString(@"hh\:mm")}({metrics.SpikeDurationPercentage}%) | ");
						}

						if (metrics.Problems > 0)
						{
							builder.Append($"Max Concurrent Problems: {metrics.MaxConcurrentProblems}({metrics.MaxConcurrentProblemsPercentage}%) | ");
						}

						builder.Append($"Devices: {metrics.Total} | ");
						if (metrics.Disabled > 0)
						{
							builder.Append($"Disabled: {metrics.Disabled} | ");
						}
						if (metrics.Maintenance > 0)
						{
							builder.Append($"Maintenance: {metrics.Maintenance} | ");
						}
						builder.Append($"Streams: {metrics.Streams.Count}");

						SectionBlock section = new SectionBlock(builder.ToString());
						blocks.Add(section);
					}
					if (blocks.Count > 0)
					{
						SlackMessage poolReportMessage = new SlackMessage();
						poolReportMessage.Blocks.Add(new SectionBlock($"<{pool.PoolURL}|*{pool.PoolName}*> *| Device Pool Health*"));
						poolReportMessage.Blocks.AddRange(blocks);
						poolReportMessage.AddDivider();
						await SendMessageAsync(report.Channel, poolReportMessage, cancellationToken);
					}
				}
				...
				{
					SlackMessage message = new SlackMessage();
					message.Blocks.Add(new SectionBlock("*Device Pool Health* | no outstanding usage to report"));
					message.AddDivider();
			...
			if (report.PlatformReports.Count > 0)
			{
				...
				{
					SlackMessage message = new SlackMessage();
					message.Blocks.Add(new SectionBlock($"*{platform.PlatformName} | Device Problems Since Last Update*"));

					foreach (DeviceReport device in platform.DeviceReports.OrderByDescending(r => r.ProblemDelta))
					{
						StringBuilder builder = new StringBuilder($"{RedBadge} <{device.DevicePoolURL}|*{device.DeviceName}*> | {device.DeviceAddress} | {device.PoolName}");
						builder.Append($"\n> {device.ProblemDelta} Problem(s) | {device.ProblemPercent}% Rate");

						if (!String.IsNullOrEmpty(device.LastProblemURL))
						{
							builder.Append($"\n> <{device.LastProblemURL}|{device.LastProblemDesc ?? "??"}>");
						}

						message.Blocks.Add(new SectionBlock(builder.ToString()));
					}
		...
		async ValueTask EscalateAsync(CancellationToken cancellationToken)
		{
			...
			if (issueIds.Length > 0)
			{
				...
				foreach (int issueId in issueIds)
				{
					...
					try
					{
						double? nextTime = await EscalateSingleIssueAsync(buildConfig, issueId, utcNow, cancellationToken);
						if (nextTime == null)
						{
					...
					{
						if (ex.Message.Equals("channel_not_found", StringComparison.Ordinal))
						{
							_logger.LogWarning(ex, "Slack channel not found while escalating issue {IssueId}; cancelling.", issueId);
						}
						else
						{
		...
		async Task<double?> EscalateSingleIssueAsync(BuildConfig buildConfig, int issueId, DateTime utcNow, CancellationToken cancellationToken)
		{
			...
			if (String.IsNullOrEmpty(workflow.TriageChannel) || workflow.EscalateAlias == null || workflow.EscalateTimes.Count == 0 || (!workflow.TriageWarnings && issue.Severity == IssueSeverity.Warning) || (!workflow.TriageErrors && issue.Severity == IssueSeverity.Error))
			{
				return null;
			}
			...
			if (newIssue != null)
			{
				...
				if (user != null)
				{
					...
					if (recipient != null)
					{
						...
						await SendIssueMessageAsync(_buildConfig.CurrentValue, recipient, newIssue, details, userId, DefaultAllowMentions, cancellationToken);
					}
				}
			}
		}
			...
			if (!String.IsNullOrEmpty(_settings.AgentNotificationChannel))
			{
...

				{
					...
					else
					{
						...
						{
							Uri agentUrl = new Uri(_serverInfo.DashboardUrl, $"agents?agentId={agentId}");
