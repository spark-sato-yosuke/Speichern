...
namespace HordeServer.Issues
{
	...
	public sealed class IssueService : IHostedService, IAsyncDisposable
	{
		...
		{
			using TelemetrySpan tracerSpan = _tracer.StartActiveSpan($"{nameof(IssueService)}.{nameof(UpdateCompleteStepsAsync)}");
		...
		async Task<List<NewIssueSpanSuspectData>> FindSuspectsForSpanAsync(StreamConfig streamConfig, IIssueFingerprint fingerprint, CommitId lastSuccess, CommitId firstFailure, CancellationToken cancellationToken)
		{
			...
			if (!String.IsNullOrEmpty(fingerprint.ChangeFilter))
			{
				...
				{
					int rank = await GetChangeScoreAsync(commit, filter, suspectFiles, suspectNotes, suspectSymbols, _tracer);
					if (rank > maxRank)
					{
						suspects.Clear();
						maxRank = rank;
		...
		static async ValueTask<int> GetChangeScoreAsync(ICommit commit, FileFilter filter, HashSet<string> suspectFiles, HashSet<string> suspectNotes, HashSet<string> suspectSymbols, Tracer tracer)
		{
