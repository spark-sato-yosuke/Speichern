...
namespace HordeServer.Devices
{
	...
	public interface IDeviceCollection
	{
		...
		/// <param name="message">Attached a message to the log of the device</param>
		Task UpdateDeviceAsync(DeviceId deviceId, DevicePoolId? newPoolId, string? newName, string? newAddress, string? newModelId, string? newNotes, bool? newEnabled, bool? newProblem, bool? newMaintenance, UserId? modifiedByUserId = null, string? message = null);

		/// <summary>
		/// Updates a device's cleaning status
		/// </summary>
		/// <param name="deviceId">The device id to update</param>
		/// <param name="cleaning">Whether the device is actively being cleaned</param>
		/// <param name="cleanRequired">Whetner the device requires cleaning, for example after being checked in</param>
		/// <returns></returns>
		Task UpdateDeviceCleaningAsync(DeviceId deviceId, bool? cleaning = null, bool? cleanRequired = null);
