...
namespace HordeServer.Jobs
{
	...
	public sealed class JobTaskSource : TaskSourceBase<ExecuteJobTask>, IHostedService, IAsyncDisposable
	{
		...
		internal async ValueTask TickAsync(CancellationToken cancellationToken)
		{
			...
			IReadOnlyList<IStream> streamsList = await _streamCollection.FindAsync(buildConfig.Streams.Select(x => x.Id).ToList(), cancellationToken);
			...
			for (int idx = 0; idx < newJobs.Count; idx++)
			{
				...
				while (newJob != null)
				{
					...
					if (!stream.AgentTypes.TryGetValue(graph.Groups[batch.GroupIdx].AgentType, out IAgentType? agentType))
					{
					...
					else if (!buildConfig.TryGetStream(stream.Id, out StreamConfig? streamConfig) || !streamConfig.TryGetAgentWorkspace(agentType, out AgentWorkspaceInfo? workspace, out AutoSdkConfig? autoSdkConfig))
					{
		...
		public void UpdateQueuedJob(IJob job, IGraph graph)
		{
			...
			_buildConfig.CurrentValue.TryGetStream(job.StreamId, out streamConfig);
		...
		private async Task<CreateLeaseOptions?> TryCreateLeaseAsync(QueueItem item, QueueWaiter waiter, CancellationToken cancellationToken)
		{
			...
			if (newJob != null)
			{
				...
				StreamConfig? streamConfig;
				if (!_buildConfig.CurrentValue.TryGetStream(newJob.StreamId, out streamConfig))
				{
					return null;
				}
				...
				if (item._useAutoSdk)
				{
					PerforceCluster cluster = _buildConfig.CurrentValue.FindPerforceCluster(streamConfig.ClusterName)!;
				...
				if (task != null)
				{
					...
					if (waiter.LeaseSource.TrySetResult(lease))
					{
						...
						await _logCollection.AddAsync(job.Id, leaseId, agent.SessionId, LogType.Json, logId, streamConfig.Acl.ScopeName, cancellationToken);
						return lease;
					}
				}
		...
		async Task<ExecuteJobTask?> CreateExecuteJobTaskAsync(LeaseId leaseId, StreamConfig streamConfig, IJob job, IJobStepBatch batch, IAgent agent, AgentWorkspaceInfo workspace, AgentWorkspaceInfo? autoSdkWorkspace, LogId logId, CancellationToken cancellationToken)
		{
			...
			claims.AddRange(job.Claims.ConvertAll(x => new AclClaimConfig(x.Type, x.Value)));
		...
		public async Task UpdateUgsBadgesAsync(IJob job, IGraph graph, IReadOnlyList<(LabelState, LabelOutcome)> oldLabelStates, IReadOnlyList<(LabelState, LabelOutcome)> newLabelStates, CancellationToken cancellationToken)
		{
			...
			foreach (int labelIdx in updateLabels)
			{
				...
				{
					// Apply the update
					Uri labelUrl = new Uri(_serverInfo.DashboardUrl, $"job/{job.Id}?label={labelIdx}");
