...
#pragma warning restore CA2000
		...
		public void SendEvent(TelemetryStoreId telemetryStoreId, TelemetryEvent telemetryEvent)
		{
			...
			// Only forward internal Horde events to the data router; other apps (like the engine) will write to the data
			// router by themselves, and we want need duplicates.
			if (!String.Equals(telemetryEvent.RecordMeta.AppId, TelemetryRecordMeta.HordeAppId, StringComparison.OrdinalIgnoreCase))
			{
				return;
			}
