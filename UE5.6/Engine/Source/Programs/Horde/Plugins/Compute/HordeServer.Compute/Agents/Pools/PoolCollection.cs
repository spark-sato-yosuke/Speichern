...
#pragma warning restore CS0618 // Type or member is obsolete
		...
		public PoolCollection(IMongoService mongoService, IOptionsMonitor<ComputeConfig> computeConfig)
		{
		...
		public async Task<IReadOnlyList<IPoolConfig>> GetConfigsAsync(CancellationToken cancellationToken)
		{
			...
			pools.AddRange(computeConfig.Pools);
			if (computeConfig.VersionEnum < ConfigVersion.PoolsInConfigFiles)
			{
		...
		public async Task<IPool?> GetAsync(PoolId id, CancellationToken cancellationToken)
		{
			...
			if (computeConfig.TryGetPool(id, out PoolConfig? globalPoolConfig))
			{
				poolConfig = globalPoolConfig;
			}
			else if (computeConfig.VersionEnum < ConfigVersion.PoolsInConfigFiles)
			{
		...
		public async Task<List<PoolId>> GetPoolIdsAsync(CancellationToken cancellationToken)
		{
			...
			pools.AddRange(computeConfig.Pools.Select(x => x.Id));
