...
namespace HordeServer.Compute
{
	...
	public class ComputeTaskSource : TaskSourceBase<ComputeTask>
	{
		...
		/// Constructor
		/// </summary>
		public ComputeTaskSource(AgentRelayService agentRelay, ILeaseCollection leaseCollection, IOptionsMonitor<ComputeConfig> computeConfig, ILogger<ComputeTaskSource> logger)
		{
		...
		async Task<CreateLeaseOptions?> WaitInternalAsync(IAgent agent, CancellationToken cancellationToken)
		{
			...
			try
			{
				lock (_lockObject)
				{
					foreach (ComputeClusterConfig clusterConfig in computeConfig.Clusters)
					{
