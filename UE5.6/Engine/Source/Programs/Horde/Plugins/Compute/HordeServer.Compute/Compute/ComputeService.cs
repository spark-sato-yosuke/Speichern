...
namespace HordeServer.Compute
{
	...
	public class ComputeServiceException : Exception
	{
		...
		/// <inheritdoc/>
		public ComputeServiceException(string? message, Exception? innerException = null, bool showToUser = false) : base(message, innerException)
		{
	...
	/// </summary>
	public class NoComputeResourcesException(string? message, Exception? innerException = null) : ComputeServiceException(message, innerException, true)
	{
	...
	public class AllocateResourceParams
	{
		...
		/// <inheritdoc cref="ConnectionMetadataRequest.InactivityTimeoutMs" />
		public int? InactivityTimeoutMs { get; init; }
	...
	public sealed class ComputeService : IHostedService, IAsyncDisposable
	{
		...
		public async Task<ComputeResource> TryAllocateResourceAsync(AllocateResourceParams arp, CancellationToken cancellationToken)
		{
			...
			span.SetAttribute("requesterPublicIp", arp.RequesterPublicIp);
			...
			span.SetAttribute("inactivityTimeout", arp.InactivityTimeoutMs);
			...
			try
			{
				...
				await foreach (IAgent agent in _agentCollection.FindAsync(poolId, consistentRead: false, cancellationToken: cancellationToken))
				{
					...
					if (match)
					{
						...
						ComputeTask computeTask = CreateComputeTask(assignedResources, log?.Id, arp.Encryption, certificate, arp.ParentLeaseId, protocol, arp.InactivityTimeoutMs);
		...
		private string? ResolvePoolId(string? poolId, IPAddress? ipAddress)
		{
			...
			_computeConfig.CurrentValue.TryGetNetworkConfig(ipAddress ?? IPAddress.Any, out NetworkConfig? networkConfig);
		...
		private async Task<ComputeResource?> TryAssignAsync(AllocateResourceParams arp, IAgent agent, ComputeTask computeTask, ComputeProtocol protocol, LeaseId leaseId)
		{
			...
			string? tunnelAddress = _staticComputeConfig.CurrentValue.ComputeTunnelAddress;
		...
		/// </summary>
		/// <param name="computeConfig">Global config</param>
		/// <param name="requesterIp">IP of client (initiator) requesting a compute resource</param>
		/// <returns>Compute cluster ID</returns>
		/// <exception cref="ComputeServiceException"></exception>
		public static ClusterId FindBestComputeClusterId(ComputeConfig computeConfig, IPAddress? requesterIp)
		{
			if (requesterIp == null || !computeConfig.TryGetNetworkConfig(requesterIp, out NetworkConfig? networkConfig))
			{
		...
		static ComputeTask CreateComputeTask(Dictionary<string, int> assignedResources, LogId? logId, ComputeEncryption encryption,
			byte[] certificateData, LeaseId? parentLeaseId, ComputeProtocol protocol, int? inactivityTimeoutMs)
		{
			...
			computeTask.InactivityTimeoutMs = inactivityTimeoutMs ?? 30000;
			return computeTask;
		}
