...
namespace UnrealBuildTool
{
	 ...
	 *	$S(UnrealArch) = target Unreal arch (arm32, arm64, x86, x64)
	 ...
	 *	
	 * 	<!-- optional additions to GameActivity onTopResumeActivity in GameActivity.java -->
	 *	<gameActivityOnTopResumeActivityAdditions>	</gameActivityOnTopResumeActivityAdditions>
	 *	
	...
	class UnrealPluginLanguage
	{
		...
		private class UPLContext
		{
			...
			public UPLContext(string inFilename, (string UEArch, string NativeArch) Arch, string PluginDir)
			{
				Filename = inFilename;
				...
				StringVariables["UnrealArch"] = Arch.UEArch;
				StringVariables["Architecture"] = Arch.NativeArch;
		...
		public UnrealPluginLanguage(FileReference? InProjectFile, List<string> InXMLFiles, List<(string UEArch, string NativeArch)> InArchs, List<AliasedXMLNamespace>? InXMLNameSpaceAliases, UnrealTargetPlatform InTargetPlatform, ILogger InLogger)
		{
			...
			foreach (string Basename in InXMLFiles)
			{
				...
				if (File.Exists(Filename))
				{
					...
					try
					{
						...
						MergeXML(MergeDoc, Filename, PluginDir, InArchs);
					}
					catch (Exception e)
					{
		...
		public bool MergeXML(XDocument MergeDoc, string Filename, string PluginDir, List<(string UEArch, string NativeArch)> Archs)
		{
			if (MergeDoc == null)
			{
				return false;
			...
			ContextIndex++;
			foreach ((string UEArch, string NativeArch) Arch in Archs)
			{
				UPLContext Context = new UPLContext(Filename, Arch, PluginDir);
				Contexts[Arch.NativeArch + "_" + ContextIndex] = Context;
			}
