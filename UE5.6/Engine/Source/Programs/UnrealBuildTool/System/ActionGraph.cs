			...
			ActionHistory ActionHistory, CppDependencyCache CppDependencies, bool bIgnoreOutdatedImportLibraries, ILogger Logger)
		{
			...
			if (!bIsOutdated)
			{
				foreach (FileItem PrerequisiteItem in RootAction.PrerequisiteItems)
				{
					...
					{
						// If the PrerequisiteItem doesn't exist, it means it's being produced right now and we need to update the ProducedItem that
						// depends on it
						Logger.LogDebug("{StatusDescription}: Prerequisite {PrerequisiteItem} doesn't exist.", RootAction.StatusDescription, PrerequisiteItem.Location);
						bIsOutdated = true;
						break;
					}
				}
