	...
	class VSCodeProjectFileGenerator : ProjectFileGenerator
	{
		...
		protected override void AddTargetForIntellisense(UEBuildTarget Target, ILogger Logger)
		{
			...
			else if (OperatingSystem.IsMacOS())
			{
				...
				SysRootPath = Settings.GetSDKPath(UnrealArch.Host.Value);
			}
			else
			{
				...
				else
				{
					...
					foreach (UnrealTargetConfiguration Config in Configs)
					{
...

						{
							...
							DirectoryReference OutputPath = Info.GetOutputDir(VCSharpProject.ProjectFilePath.Directory);
							FileReference? OutputFile = null;
							HashSet<FileReference> ProjectBuildProducts = new HashSet<FileReference>();
							Info.FindCompiledBuildProducts(OutputPath, ProjectBuildProducts);
		...
		private void WriteConfiguration(string Name, string ProjectName, IEnumerable<FileReference> SourceFiles, CppStandardVersion CppStandard, FileReference CompilerPath, DirectoryReference? SysRootPath, Dictionary<DirectoryReference, string> ModuleCommandLines, JsonFile OutFile, DirectoryReference OutputDirectory)
		{
			...
			string CompilerPrefix = HostPlatform == UnrealTargetPlatform.Win64 ? "msvc" : "clang";
			string ArchSuffix = UnrealArch.Host.Value.bIsX64 ? "x64" : "arm64";
			// make msvc-x64, clang-arm64, etc
			OutFile.AddField("intelliSenseMode", $"{CompilerPrefix}-{ArchSuffix}");
			...
			foreach (string Type in Types)
			{
				OutFile.BeginObject();
				{
					...
					*/
					...
					Dictionary<DirectoryReference, DirectoryReference> SourceFileMap = new();
					if (Unreal.OriginalCompilationRootDirectory != ProjectRoot)
					{
						SourceFileMap.Add(Unreal.OriginalCompilationRootDirectory, ProjectRoot);
					}
					CppRootPaths RootPaths = new();
					RootPaths[CppRootPathFolder.Root] = Unreal.RootDirectory;
					if (BuildProduct.UProjectFile != null)
					{
						RootPaths[CppRootPathFolder.Project] = BuildProduct.UProjectFile.Directory;
					}
					foreach ((uint id, DirectoryReference vfs, DirectoryReference local) entry in RootPaths)
					{
						SourceFileMap.Add(entry.vfs, entry.local);
					}

					if (SourceFileMap.Any())
					{
						OutFile.BeginObject("sourceFileMap");
						{
							foreach (KeyValuePair<DirectoryReference, DirectoryReference> entry in SourceFileMap)
							{
								OutFile.AddField(
									MakeUnquotedPathString(entry.Key, EPathType.Absolute),
									MakeUnquotedPathString(entry.Value, EPathType.Absolute));
							}
						}
						OutFile.EndObject();
					}
		...
		{
			string[] DirAllowList = { "Binaries", "Build", "Config", "Plugins", "Source", "Private", "Public", "Internal", "Classes", "Resources", "Restricted" };
