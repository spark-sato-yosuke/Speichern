...
namespace UnrealBuildTool
{
	internal partial class MicrosoftPlatformSDK : UEBuildPlatformSDK
	{
		...
		{
			ToolChainInstallation? ToolChain = Preference(ToolChains.Where(x => x.Architecture == Architecture && x.HostArchitecture == MicrosoftPlatformSDK.MSVCHostUnrealArch)
			...
			// Allow x64 host toolchains on arm64 if no native arm64 toolchains are available, and not running via wine
			if (!BuildHostPlatform.Current.IsRunningOnWine() && MicrosoftPlatformSDK.MSVCHostUnrealArch == UnrealArch.Arm64 && (ToolChain == null || ToolChain.Error != null))
			{
				ToolChain = Preference(ToolChains.Where(x => x.Architecture == Architecture && x.HostArchitecture != MicrosoftPlatformSDK.MSVCHostUnrealArch)
					.OrderByDescending(x => x.Error == null))
					.ThenBy(x => x.ReleaseChannel)
					.ThenBy(x => x.FamilyRank)
					.ThenByDescending(x => x.IsAutoSdk)
		...
		static void DumpToolChains(IEnumerable<ToolChainInstallation> ToolChains, Func<IOrderedEnumerable<ToolChainInstallation>, IOrderedEnumerable<ToolChainInstallation>> Preference, UnrealArch Architecture, ILogger Logger)
		{
			IOrderedEnumerable<ToolChainInstallation> SortedToolChains = Preference(ToolChains.Where(x => x.Architecture == Architecture)
				...
				.ThenByDescending(x => x.IsAutoSdk)
				.ThenByDescending(x => x.Version)
				.ThenBy(x => x.HostArchitecture.ToString());
			...
			if (SortedToolChains.Any())
			{
				...
				if (AvailableToolChains.Any())
				{
					...
					{
						Logger.LogInformation(" * {ToolChainDir}\n    (Family={Family}, FamilyRank={FamilyRank}, Version={Version}, HostArchitecture={HostArchitecture}, ReleaseChannel={ReleaseChannel}, Architecture={Arch})", ToolChain.BaseDir, ToolChain.Family, ToolChain.FamilyRank, ToolChain.Version, ToolChain.HostArchitecture, ToolChain.ReleaseChannel, ToolChain.Architecture);
					}
				}
				...
				if (BuildHostPlatform.Current.Platform == UnrealTargetPlatform.Win64)
				{
					if (Compiler == WindowsCompiler.Clang)
					{
						...
						foreach (VisualStudioInstallation Installation in FindVisualStudioInstallations(WindowsCompiler.VisualStudio2022, Logger))
						{
							...
							AddClangToolChain(Compiler, DirectoryReference.Combine(Installation.BaseDir, "VC", "Tools", "Llvm", "ARM64"), ToolChains, IsAutoSdk: false, Logger);
						}
					...
					{
						// the UnrealInstrumentation Clang compiler is in-tree
						DirectoryReference UnrealInstrumentationDir = DirectoryReference.Combine(Unreal.EngineDirectory, "Binaries", "Win64", "UnrealInstrumentation");
						AddClangToolChain(Compiler, UnrealInstrumentationDir, ToolChains, IsAutoSdk: false, Logger);

						// Check for AutoSDK paths
						DirectoryReference? AutoSdkDir;
						if (UEBuildPlatformSDK.TryGetHostPlatformAutoSDKDir(out AutoSdkDir))
						{
							DirectoryReference ClangBaseDir = DirectoryReference.Combine(AutoSdkDir, "Win64", "UnrealInstrumentation");
							if (DirectoryReference.Exists(ClangBaseDir))
							{
								foreach (DirectoryReference ToolChainDir in DirectoryReference.EnumerateDirectories(ClangBaseDir))
								{
					...
					{
						// Check for a manual installation to a custom directory
						string? LlvmPath = Environment.GetEnvironmentVariable("LLVM_CUSTOM_PATH");
						if (!String.IsNullOrEmpty(LlvmPath))
						{
							AddClangToolChain(Compiler, new DirectoryReference(LlvmPath), ToolChains, IsAutoSdk: false, Logger);
						}
						...
						{
							throw new BuildException("ClangCustom needs LLVM_CUSTOM_PATH environment variable to be defined");
						}
					}
					...
					foreach (ToolChainInstallation? ToolChain in ToolChains.Where(x => x.Error == null && x.HostArchitecture == MSVCHostUnrealArch))
					{
						ToolChainInstallation? CrossToolChain;
						if (Compiler.IsMSVC())
						{
							// MSVC has the same base dir. Version may not match between Hostx64 and Hostarm64.
							CrossToolChain = ToolChains.FirstOrDefault(x =>
								x.Error == null &&
								x.Architecture == ToolChain.Architecture &&
								x.ReleaseChannel == ToolChain.ReleaseChannel &&
								x.BaseDir == ToolChain.BaseDir &&
								x.HostArchitecture != MSVCHostUnrealArch);
						}
						else
						{
							// Clang the version should match.
							CrossToolChain = ToolChains.FirstOrDefault(x =>
								x.Error == null &&
								x.Architecture == ToolChain.Architecture &&
								x.ReleaseChannel == ToolChain.ReleaseChannel &&
								x.Version == ToolChain.Version &&
								x.HostArchitecture != MSVCHostUnrealArch);
						}

						if (CrossToolChain != null)
						{
							DirectoryReference HostBinDir;
							DirectoryReference CrossBinDir;

							if (Compiler.IsMSVC())
							{
								HostBinDir = DirectoryReference.Combine(ToolChain.BaseDir, "bin", UnrealArchToMSVCHostDirectoryName(ToolChain.HostArchitecture));
								CrossBinDir = DirectoryReference.Combine(CrossToolChain.BaseDir, "bin", UnrealArchToMSVCHostDirectoryName(CrossToolChain.HostArchitecture));
							}
							else
							{
								HostBinDir = DirectoryReference.Combine(ToolChain.BaseDir, MSVCHostUnrealArch.bIsX64 ? "bin" : "bin-woa64");
								CrossBinDir = DirectoryReference.Combine(CrossToolChain.BaseDir, !MSVCHostUnrealArch.bIsX64 ? "bin" : "bin-woa64");
							}
...
#pragma warning restore CS0618 // Type or member is obsolete
		...
		static void FindVisualStudioToolChains(DirectoryReference BaseDir, DirectoryReference? OptionalRedistDir, WindowsCompilerChannel ReleaseChannel, List<ToolChainInstallation> ToolChains, bool IsAutoSdk, ILogger Logger)
		{
			if (DirectoryReference.Exists(BaseDir))
			{
				...
				{
					VersionNumber? Version;
					if (IsValidToolChainDirMSVC(ToolChainDir, UnrealArch.X64, out Version))
					{
						DirectoryReference? RedistDir = FindVisualStudioRedistForToolChain(ToolChainDir, OptionalRedistDir, Version);
						AddVisualCppToolChain(Version, UnrealArch.X64, ReleaseChannel, ToolChainDir, RedistDir, ToolChains, IsAutoSdk, Logger);
					}

					if (IsValidToolChainDirMSVC(ToolChainDir, UnrealArch.Arm64, out Version))
					{
						DirectoryReference? RedistDir = FindVisualStudioRedistForToolChain(ToolChainDir, OptionalRedistDir, Version);
						AddVisualCppToolChain(Version, UnrealArch.Arm64, ReleaseChannel, ToolChainDir, RedistDir, ToolChains, IsAutoSdk, Logger);
					}
				}
			}
		}
		...
		/// </summary>
		/// <param name="Version"></param>
		/// <param name="HostArchitecture"></param>
		...
		static void AddVisualCppToolChain(VersionNumber Version, UnrealArch HostArchitecture, WindowsCompilerChannel ReleaseChannel, DirectoryReference ToolChainDir, DirectoryReference? RedistDir, List<ToolChainInstallation> ToolChains, bool IsAutoSdk, ILogger Logger)
		{
			...
			if (HasX64ToolChain(ToolChainDir, HostArchitecture))
			{
				...
				ToolChains.Add(new ToolChainInstallation(Family, FamilyRank, Version, HostArchitecture, ReleaseChannel, UnrealArch.X64, Error, ToolChainDir, RedistDir, IsAutoSdk));
			}

			if (HasArm64ToolChain(ToolChainDir, HostArchitecture))
			{
				Logger.LogDebug("Found Visual Studio toolchain: {ToolChainDir} (Family={Family}, FamilyRank={FamilyRank}, Version={Version}, HostArchitecture={HostArchitecture}, ReleaseChannel={ReleaseChannel}, Architecture={Arch}, Error={Error}, Redist={RedistDir})", ToolChainDir, Family, FamilyRank, Version, HostArchitecture, ReleaseChannel, UnrealArch.Arm64.ToString(), Error != null, RedistDir);
				ToolChains.Add(new ToolChainInstallation(Family, FamilyRank, Version, HostArchitecture, ReleaseChannel, UnrealArch.Arm64, Error, ToolChainDir, RedistDir, IsAutoSdk));
				...
				if (HasArm64ECToolChain(ToolChainDir))
				{
					...
					ToolChains.Add(new ToolChainInstallation(Family, FamilyRank, Version, HostArchitecture, ReleaseChannel, UnrealArch.Arm64ec, Error, ToolChainDir, RedistDir, IsAutoSdk));
				}
			}
		}
		...
		/// <param name="ToolChainDir"></param>
		/// <param name="CompilerFile"></param>
		/// <param name="ToolChains"></param>
		/// <param name="IsAutoSdk"></param>
		/// <param name="Logger"></param>
		static void AddClangToolChain(WindowsCompiler Compiler, DirectoryReference ToolChainDir, FileReference CompilerFile, List<ToolChainInstallation> ToolChains, bool IsAutoSdk, ILogger Logger)
		{
			if (FileReference.Exists(CompilerFile))
			{
				...
				UnrealArch HostArchitecture;
				if (IsX64Executable(CompilerFile))
				{
					HostArchitecture = UnrealArch.X64;
				}
				else if (IsArm64Executable(CompilerFile))
				{
					HostArchitecture = UnrealArch.Arm64;
				}
				else
				{
					// Unknown binary architecture
					HostArchitecture = UnrealArch.Deprecated;
					Error = $"Clang binary is not x64 or arm64.";
				}

				Logger.LogDebug("Found Clang toolchain: {ToolChainDir} (Version={Version}, HostArchitecture={HostArchitecture}, Rank={Rank}, Error={Error})", ToolChainDir, Version, HostArchitecture, Rank, Error != null);
				ToolChains.Add(new ToolChainInstallation(Family, Rank, Version, HostArchitecture, WindowsCompilerChannel.Any, UnrealArch.X64, Error, ToolChainDir, null, IsAutoSdk));
				ToolChains.Add(new ToolChainInstallation(Family, Rank, Version, HostArchitecture, WindowsCompilerChannel.Any, UnrealArch.Arm64, Error, ToolChainDir, null, IsAutoSdk));
				ToolChains.Add(new ToolChainInstallation(Family, Rank, Version, HostArchitecture, WindowsCompilerChannel.Any, UnrealArch.Arm64ec, Error, ToolChainDir, null, IsAutoSdk));
			}
		}
		...
		{
			string CompilerFilename = Compiler == WindowsCompiler.ClangRTFM ? "verse-clang-cl.exe" : (Compiler == WindowsCompiler.ClangInstrument ? "instr-clang-cl.exe" : "clang-cl.exe");
			AddClangToolChain(Compiler, ToolChainDir, FileReference.Combine(ToolChainDir, "bin", CompilerFilename), ToolChains, IsAutoSdk, Logger);
			AddClangToolChain(Compiler, ToolChainDir, FileReference.Combine(ToolChainDir, "bin-woa64", CompilerFilename), ToolChains, IsAutoSdk, Logger);
		}
		...
		static void AddIntelOneApiToolChain(DirectoryReference ToolChainDir, List<ToolChainInstallation> ToolChains, bool IsAutoSdk, ILogger Logger)
		{
			...
			if (FileReference.Exists(CompilerFile))
			{
				...
				bool Is64Bit = IsX64Executable(CompilerFile);
				...
				Logger.LogDebug("Found Intel OneAPI toolchain: {ToolChainDir} (Version={Version}, HostArchitecture={HostArchitecture}, Rank={Rank}, Error={Error})", ToolChainDir, Version, Is64Bit, Rank, Error != null);
				ToolChains.Add(new ToolChainInstallation(Family, Rank, Version, HostArchitecture, WindowsCompilerChannel.Any, UnrealArch.X64, Error, ToolChainDir, null, IsAutoSdk));
				ToolChains.Add(new ToolChainInstallation(Family, Rank, Version, HostArchitecture, WindowsCompilerChannel.Any, UnrealArch.Arm64, Error, ToolChainDir, null, IsAutoSdk));
				ToolChains.Add(new ToolChainInstallation(Family, Rank, Version, HostArchitecture, WindowsCompilerChannel.Any, UnrealArch.Arm64ec, Error, ToolChainDir, null, IsAutoSdk));
			}
		...
		/// </summary>
		public static UnrealArch MSVCHostUnrealArch => RuntimeInformation.ProcessArchitecture == Architecture.Arm64 ? UnrealArch.Arm64 : UnrealArch.X64;

		/// <summary>
		/// The compiler host directory name for an UnrealArch.
		/// </summary>
		/// <param name="HostArchitecture">The binary architecture for the toolchain</param>
		public static string UnrealArchToMSVCHostDirectoryName(UnrealArch HostArchitecture) => HostArchitecture.bIsX64 ? "Hostx64" : "Hostarm64";

		/// <summary>
		/// The compiler host directory name for the running process architecture.
		/// </summary>
		public static string MSVCHostDirectoryName => UnrealArchToMSVCHostDirectoryName(MSVCHostUnrealArch);

		/// <summary>
		/// Test whether an executable is a specific machine type
		/// </summary>
		/// <param name="File">Executable to test</param>
		/// <param name="MachineType">The machine type to check</param>
		/// <returns></returns>
		static bool IsExecutableMachineType(FileReference File, ushort MachineType)
		{
			using (FileStream Stream = new FileStream(File.FullName, FileMode.Open, FileAccess.Read, FileShare.Read | FileShare.Delete))
			{
				...
				return MachineType == BinaryPrimitives.ReadUInt16LittleEndian(PeHeader.AsSpan(4));
			}
		}

		/// <summary>
		/// Test whether an executable is x64
		/// </summary>
		/// <param name="File">Executable to test</param>
		/// <returns></returns>
		static bool IsX64Executable(FileReference File)
		{
			const ushort IMAGE_FILE_MACHINE_AMD64 = 0x8664;
			return IsExecutableMachineType(File, IMAGE_FILE_MACHINE_AMD64);
		}

		/// <summary>
		/// Test whether an executable is arm64
		/// </summary>
		/// <param name="File">Executable to test</param>
		/// <returns></returns>
		static bool IsArm64Executable(FileReference File)
		{
			const ushort IMAGE_FILE_MACHINE_ARM64 = 0xaa64;
			return IsExecutableMachineType(File, IMAGE_FILE_MACHINE_ARM64);
		}
		...
		static bool IsValidToolChainDirMSVC(DirectoryReference ToolChainDir, UnrealArch HostArchitecture, [NotNullWhen(true)] out VersionNumber? Version)
		{
			FileReference CompilerExe = FileReference.Combine(ToolChainDir, "bin", UnrealArchToMSVCHostDirectoryName(HostArchitecture), "x64", "cl.exe");
			if (!FileReference.Exists(CompilerExe))
			{
				CompilerExe = FileReference.Combine(ToolChainDir, "bin", UnrealArchToMSVCHostDirectoryName(HostArchitecture), "arm64", "cl.exe");
			}
		...
		/// <summary>
		/// Checks if the given directory contains a x64 toolchain. 
		/// </summary>
		/// <param name="ToolChainDir">Directory to check</param>
		/// <param name="HostArchitecture">The host binary architecture to check</param>
		/// <returns>True if the given directory contains a 64-bit toolchain</returns>
		static bool HasX64ToolChain(DirectoryReference ToolChainDir, UnrealArch HostArchitecture)
		{
			return FileReference.Exists(FileReference.Combine(ToolChainDir, "bin", UnrealArchToMSVCHostDirectoryName(HostArchitecture), "x64", "cl.exe"));
		}

		/// <summary>
		/// Checks if the given directory contains an arm64 toolchain.
		/// </summary>
		/// <param name="ToolChainDir">Directory to check</param>
		/// <param name="HostArchitecture">The host binary architecture to check</param>
		/// <returns>True if the given directory contains the arm64 toolchain</returns>
		static bool HasArm64ToolChain(DirectoryReference ToolChainDir, UnrealArch HostArchitecture)
		{
			return FileReference.Exists(FileReference.Combine(ToolChainDir, "bin", UnrealArchToMSVCHostDirectoryName(HostArchitecture), "arm64", "cl.exe"));
		}

		/// <summary>
		/// Checks if the given directory contains an arm64ec toolchain.
		/// </summary>
		/// <param name="ToolChainDir">Directory to check</param>
...
#pragma warning restore CS0618 // Type or member is obsolete
	...
	internal class ToolChainInstallation
	{
		...
		/// <summary>
		/// The host architecture of this ToolChainInstallation (multiple ToolChainInstallation instances may be created, one per host architecture).
		/// </summary>
		public UnrealArch HostArchitecture { get; }
		...
		/// <param name="Version"></param>
		/// <param name="HostArchitecture">The architecture of the binaries in this toolchain</param>
		/// <param name="ReleaseChannel">The release channel of this toolchain</param>
		/// <param name="Architecture">The target architecture this toolchain can compile</param>
		...
		public ToolChainInstallation(VersionNumber Family, int FamilyRank, VersionNumber Version, UnrealArch HostArchitecture, WindowsCompilerChannel ReleaseChannel, UnrealArch Architecture, string? Error, DirectoryReference BaseDir, DirectoryReference? RedistDir, bool IsAutoSdk)
		{
			this.Family = Family;
