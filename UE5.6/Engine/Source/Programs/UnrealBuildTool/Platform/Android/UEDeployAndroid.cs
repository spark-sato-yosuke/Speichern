...
namespace UnrealBuildTool
{
	class UEDeployAndroid : UEBuildDeploy, IAndroidDeploy
	{
		...
		public void SetAndroidPluginData(UnrealArchitectures Architectures, List<string> inPluginExtraData)
		{
			...
			List<(string UEArch, string NativeArch)> Archs = Architectures.Architectures.Select(x => (x.ToString(), GetNDKArch(x))).ToList();
			...
			UPL = new UnrealPluginLanguage(ProjectFile, inPluginExtraData, Archs, ManifestXMLNamespaces, UnrealTargetPlatform.Android, Logger);
		...
		public bool IsPackagingForMetaQuest(string CookFlavor, ConfigHierarchy? Ini = null)
		{
			// make a new one if one wasn't passed in
			Ini ??= GetConfigCacheIni(ConfigHierarchyType.Engine);
			...
			return (result && bPackageForMetaQuest) || bTargetOculusDevices || CookFlavor == "_OpenXR";
		}
		...
		private static void DeleteDirectory(string InPath, ILogger Logger, string SubDirectoryToKeep = "")
		{
			...
			if (String.Equals(Path.GetFileName(InPath), SubDirectoryToKeep, StringComparison.CurrentCultureIgnoreCase))
			{
				return;
			}
		...
		private bool CopyIfDifferent(string SourceFilename, string DestFilename, bool bLog, bool bContentCompare)
		{
			...
			if (!bNeedCopy)
			{
				...
				else
				{
					...
					// Allow 1s slop in case one of the files is on a FAT volume, and the other is not
					else if (Math.Abs((File.GetLastWriteTimeUtc(SourceFilename) - File.GetLastWriteTimeUtc(DestFilename)).TotalSeconds) > 1)
					{
		...
		public int GetStoreVersion(UnrealArch? Architecture)
		{
			if (CachedStoreVersion < 1)
			{
				...
				// override store version with changelist if enabled
				if (bUseChangeListAsStoreVersion)
				{
					...
					int Changelist = 0;
					if (EngineVersion == null || !(Int32.TryParse(EngineChangelist, out Changelist)) || Changelist == 0)
					{
						throw new BuildException("AndroidRuntimeSettings.bUseChangeListAsStoreVersion is enabled but no valid Engine version changelist was found!");
					}
				...
				Logger.LogInformation("GotStoreVersion found v{StoreVersion}. (bUseChangeListAsStoreVersion={UseChangeListAsStoreVersion} IsBuildMachine={IsBuildMachine} EngineChangeList={EngineChangeList})", StoreVersion, bUseChangeListAsStoreVersion, IsBuildMachine, EngineChangelist);
		...
		private List<string> GetSODependencies(string SOFilename, UnrealArch UnrealArch)
		{
			List<string> DependencyList = new List<string>();
			string ReadElfExecutable = AndroidToolChain.GetReadElfExecutablePath(UnrealArch);
			string? Output = Utils.RunLocalProcessAndReturnStdOut(ReadElfExecutable, "--dynamic \"" + SOFilename + "\"");
			if (Output != null)
			{
				// ex:   0x0000000000000001 (NEEDED)         Shared library: [libGLESv3.so]
				int ScanStart = 0;
				int SharedIndex = Output.IndexOf("Shared library: [", ScanStart);
				while (SharedIndex >= 0)
				{
					ScanStart = SharedIndex + 17;
					int EndIndex = Output.IndexOf(']', ScanStart);
					if (EndIndex > 0)
					{
						string SharedLib = Output.Substring(ScanStart, EndIndex - ScanStart);

						// strip off lib (not needed for System.loadLibrary()
						if (SharedLib.StartsWith("lib"))
						{
							SharedLib = SharedLib.Substring(3);
						}
						// strip off .so (not needed for System.loadLibrary()
						if (SharedLib.EndsWith(".so"))
						{
							SharedLib = SharedLib.Substring(0, SharedLib.Length - 3);
						}

						DependencyList.Add(SharedLib);
					}
					else
					{
						// Unpaired [, break due to unexpected result
						break;
					}
					ScanStart = EndIndex + 1;
					SharedIndex = Output.IndexOf("Shared library: [", ScanStart);
				}
			}
		...
		void LogBuildSetup()
		{
			...
			Logger.LogInformation("bBuildForES31: {BuildForES31}", (bBuildForES31 ? "true" : "false"));
			Logger.LogInformation("bSupportsVulkan: {SupportsVulkan}", (bSupportsVulkan ? "true" : "false"));
		}
		...
		private string GetAllBuildSettings(AndroidToolChain ToolChain, UnrealPluginLanguage UPL, bool bForDistribution, bool bMakeSeparateApks, bool bPackageDataInsideApk,
			bool bDisableVerifyOBBOnStartUp, bool bUseExternalFilesDir, bool bDontBundleLibrariesInAPK, bool bUsesZenStreaming, string TemplatesHashCode)
		{
			...
			CurrentSettings.AppendLine(String.Format("bUsesZenStreaming={0}", bUsesZenStreaming));
		...
		private void PickSplashScreenOrientation(string CookFlavor, string UnrealBuildPath, bool bNeedPortrait, bool bNeedLandscape)
		{
			ConfigHierarchy Ini = GetConfigCacheIni(ConfigHierarchyType.Engine);
			...
			bool bPackageForMetaQuest = IsPackagingForMetaQuest(CookFlavor, Ini);
			...
			if (bPackageForMetaQuest)
			{
				...
				if (bShowLaunchImage)
				{
					...
					else if (FilesAreDifferent(LandscapeFilename, OculusSplashTargetPath))
					{
						...
						File.SetLastWriteTimeUtc(OculusSplashTargetPath, File.GetLastWriteTimeUtc(LandscapeFilename));
		...
		private string GenerateManifest(AndroidToolChain ToolChain, string ProjectName, TargetType InTargetType, string EngineDirectory, bool bIsForDistribution, bool bPackageDataInsideApk, string GameBuildFilesPath, bool bHasOBBFiles, bool bDisableVerifyOBBOnStartUp, UnrealArch UnrealArch, string CookFlavor, bool bUseExternalFilesDir, string Configuration, int SDKLevelInt, bool bIsEmbedded, bool bEnableBundle, bool bUsesZenStreaming)
		{
			...
			float MinAspectRatioValue;
			if (!Ini.TryGetValue("/Script/AndroidRuntimeSettings.AndroidRuntimeSettings", "MinAspectRatio", out MinAspectRatioValue))
			{
				MinAspectRatioValue = 0.0f;
			}
			...
			bool bASISEnabled = false;
			Ini.GetBool("/Script/AndroidSingleInstanceServiceEditor.AndroidSingleInstanceServiceRuntimeSettings", "bEnableASISPlugin", out bASISEnabled);
			...
			{
				if(MinAspectRatioValue > 0.0)
				{
					string MinAspectRatioString = MinAspectRatioValue.ToString("f", System.Globalization.CultureInfo.InvariantCulture);
					Text.AppendLine(String.Format("\t\t\tandroid:minAspectRatio=\"{0}\"", MinAspectRatioString));
				}
				Text.AppendLine(String.Format("\t\t\tandroid:maxAspectRatio=\"{0}\"", MaxAspectRatioString));
			}
			...
			bool bNeedMainIntent = !bShowLaunchImage && !bASISEnabled;
			if (bNeedMainIntent || bUsesZenStreaming)
			{
				Text.AppendLine("\t\t\t<intent-filter>");
				if (bNeedMainIntent)
				{
				...
				if (bUsesZenStreaming)
				{
					Text.AppendLine("\t\t\t\t<action android:name=\"com.epicgames.unreal.Zen.StreamingEnabled\" />");
				}
				Text.AppendLine("\t\t\t</intent-filter>");
			}
			...
			else
			{
				if (bPackageForMetaQuest)
				{
					// needed for virtual keyboard to work on MetaQuest
					Text.AppendLine("\t<uses-feature android:name=\"oculus.software.overlay_keyboard\" android:required=\"false\"/>");

					// need for hand tracking on MetaQuest
					Text.AppendLine("\t<uses-feature android:name=\"oculus.software.handtracking\" android:required=\"false\"/>");
					Text.AppendLine("\t<uses-permission android:name=\"com.oculus.permission.HAND_TRACKING\"/>");

					// need for passthrough on MetaQuest
					Text.AppendLine("\t<uses-feature android:name=\"com.oculus.feature.PASSTHROUGH\" android:required=\"false\" />");
				}
			...
			UPL!.SetGlobalContextVariable("CompileSDK", SDKLevelInt.ToString());
		...
		{
			// source must exist
			FileInfo SourceInfo = new(SourceFilename);
			...
			FileInfo DestInfo = new(DestFilename);
			if (!DestInfo.Exists)
			{
				return true;
			}
		...
		private void CreateGradlePropertiesFiles(AndroidToolChain ToolChain, UnrealArch Arch, int MinSDKVersion, int TargetSDKVersion, string CompileSDKVersion, string BuildToolsVersion, string PackageName,
			...
			bool bForDistribution, bool bIsEmbedded, List<string> OBBFiles, bool bJavaPerfettoMarkers)
		{
			...
			GradleProperties.AppendLine(String.Format("NDK_PATH={0}", NDKPath));
			...
			String BuildGradleAdditionsDefault = "";
			if (bJavaPerfettoMarkers)
			{
				BuildGradleAdditionsDefault = @"
dependencies.implementation('androidx.tracing:tracing:1.2.0')
";
			}

			// Add any UPL app buildGradleAdditions
			GradleBuildAdditionsContent.Append(UPL.ProcessPluginNode(NDKArch, "buildGradleAdditions", BuildGradleAdditionsDefault));
		...
		public static bool GetDontBundleLibrariesInAPK(FileReference? ProjectFile, bool? bForceDontBundleLibrariesInAPK, UnrealTargetConfiguration Configuration, bool bIsArchive, bool bFromMSBuild, bool bIsFromUAT, ILogger? Logger)
		{
			...
			{
				Logger?.LogInformation("bDontBundleLibrariesInAPK is force set to {ForceDontBundleLibrariesInAPK} via command line option.", bForceDontBundleLibrariesInAPK.Value);
			...
			{
				Logger?.LogInformation("bDontBundleLibrariesInAPK is set to false, due to not called from MSBuild or UAT (bFromMSBuild={FromMSBuild}, bIsFromUAT={IsFromUAT}), use \"-ForceDontBundleLibrariesInAPK=true\" to override.", bFromMSBuild, bIsFromUAT);
				return false;
			}
			...
			Logger?.LogInformation("bDontBundleLibrariesInAPK is set to {DontBundleLibrariesInAPK}, based on AndroidRuntimeSettings.", bDontBundleLibrariesInAPK);
		...
		private bool GetJavaPerfettoMarkers(ConfigHierarchy Ini, UnrealTargetConfiguration Configuration, bool bDistribution)
		{
			// Ship for distribution should not include markers
			if (bDistribution && Configuration == UnrealTargetConfiguration.Shipping)
			{
				return false;
			}

			bool bJavaPerfettoMarkers = false;
			Ini.GetBool("/Script/AndroidRuntimeSettings.AndroidRuntimeSettings", "bJavaPerfettoMarkers", out bJavaPerfettoMarkers);
			return bJavaPerfettoMarkers;
		}
			...
			UnrealTargetConfiguration Configuration, bool bMakeSeparateApks, bool bIncrementalPackage, bool bDisallowPackagingDataInApk, bool bDisallowExternalFilesDir, bool bSkipGradleBuild, bool bIsArchive, bool bIsFromUAT)
		{
			...
			bool bJavaPerfettoMarkers = GetJavaPerfettoMarkers(Ini, Configuration, bForDistribution);
			...
			//string NDKBuildPath = Environment.ExpandEnvironmentVariables("%NDKROOT%/ndk-build" + (OperatingSystem.IsWindows() ? ".cmd" : ""));
			...
			// get a list of unique architectures enabled for build
			List<(UnrealArch UEArch, string NativeArch)> Archs = Architectures.Architectures.Select(x => (x, GetNDKArch(x))).ToList();
			...
			// LINQ query is subsequently used multiple times, so must copy results
			TemplateFile[] templates = (from template in Directory.EnumerateFiles(JavaTemplateSourceDir, "*.template")
												  let RealName = Path.GetFileNameWithoutExtension(template)
												  select new TemplateFile(SourceFile: template, DestinationFile: GetUnrealTemplateJavaDestination(TemplateDestinationBase, RealName))).ToArray();
			...
			foreach ((UnrealArch UnrealArch, string _) in Archs)
			{
				string UnrealBuildPath = Path.Combine(IntermediateAndroidPath, UnrealArch.ToString());
			...
			string ZenProjectStoreFileName = Path.Combine(Path.GetDirectoryName(ObbFileLocation)!, Path.GetFileNameWithoutExtension(ObbFileLocation), "ue.projectstore");
			bool bUsesZenStreaming = File.Exists(ZenProjectStoreFileName);
			...
			string CurrentBuildSettings = GetAllBuildSettings(ToolChain, UPL!, bForDistribution, bMakeSeparateApks, bPackageDataInsideApk, bDisableVerifyOBBOnStartUp, bUseExternalFilesDir, bDontBundleLibrariesInAPK, bUsesZenStreaming, TemplatesHashCode);
			...
			if (CheckApplicationName(Path.Combine(IntermediateAndroidPath, ArchRemapping[Archs[0].NativeArch]), ProjectName, out ApplicationDisplayName))
			{
			...
			UPL.SetGlobalContextVariable("CookFlavor", CookFlavor);
			UPL.Init(Archs.Select(x => x.NativeArch), bForDistribution, EngineDirectory, IntermediateAndroidPath, ProjectDirectory, Configuration.ToString(), bSkipGradleBuild, bPerArchBuildDir: true, ArchRemapping: ArchRemapping);
			

			Dictionary<string, int> NDKArchToCompileSDK = new Dictionary<string, int>();
			foreach ((UnrealArch _, string NDKArch) in Archs)
			{
				...
				int CompileSDKVersionInt = MinimumSDKLevel;
				Int32.TryParse(CompileSDKVersion, out CompileSDKVersionInt);
				if (!String.IsNullOrEmpty(CompileSDKMin))
				{
					...
					if (bUpdatedCompileSDK)
					{
						...
						Logger.LogInformation("Building Java with SDK API Level 'android-{CompileSDKVersion}' for arch {NDKArch} due to enabled plugin requirements", CompileSDKVersion, NDKArch);
					}
				}
				NDKArchToCompileSDK.Add(NDKArch, CompileSDKVersionInt);
			}

			Tuple<UnrealArch, string>[] BuildList;
			...
			if (!bBuildSettingsMatch)
			{
				...
				BuildList = (from Arch in Architectures.Architectures
							let CompileSDKLevelForArch = NDKArchToCompileSDK[GetNDKArch(Arch)]
							let manifest = GenerateManifest(ToolChain, ProjectName, InTargetType, EngineDirectory, bForDistribution, bPackageDataInsideApk, GameBuildFilesPath, bRequiresOBB, bDisableVerifyOBBOnStartUp, Arch, CookFlavor, bUseExternalFilesDir, Configuration.ToString(), CompileSDKLevelForArch, bSkipGradleBuild, bEnableBundle, bUsesZenStreaming)
							select Tuple.Create(Arch, manifest)).ToArray();
			}
			else
			{
				// LINQ query is subsequently used multiple times, so must copy results
			...
			if (bDontBundleLibrariesInAPK)
			{
				foreach ((UnrealArch Arch, string _) in Archs)
				{
					...
					Utils.LinkOrCopyIfDifferent(Path.Combine(LLDBExtraSymbolsDirectory, "libUnreal.so"), SOName);
			...
			int BuildListComboTotal = BuildList.Length;
			...
			if (BypassGradlePackaging && !bEnableBundle && !bSkipGradleBuild && !bCreateFromScratch)
			{
				...
				foreach (Tuple<UnrealArch, string> build in BuildList)
				{
					...
					if (bVerifySOChanged)
					{
						...
						if (!Utils.LinkOrCopyIfDifferent(FinalSOName, SourceSOName))
						{
					...
					string APKSignerExecutable = Path.Combine(BuildToolsPath, "apksigner" + (OperatingSystem.IsWindows() ? ".bat" : ""));
					...
					Ini.GetBool("/Script/AndroidRuntimeSettings.AndroidRuntimeSettings", "bBuildWithHiddenSymbolVisibility", out bool bBuildWithHiddenSymbolVisibility);
					Ini.GetBool("/Script/AndroidRuntimeSettings.AndroidRuntimeSettings", "bSaveSymbols", out bool bSaveSymbols);
					...
					if (bBuildWithHiddenSymbolVisibility || bSaveSymbols)
					{
						...
						string SymbolifiedSOPath = Path.Combine(SymbolSODirectory, "libUnreal.so");
						MakeDirectoryIfRequired(SymbolifiedSOPath);
						Logger.LogInformation("Writing symbols to {SymbolifiedSOPath}", SymbolifiedSOPath);

						Utils.HardLinkOrCopyIfDifferent(SymbolifiedSOPath, SourceSOName);
					}
			...
			foreach (Tuple<UnrealArch, string> build in BuildList)
			{
				...
				int CompileSDKLevelForArch = NDKArchToCompileSDK[NDKArch];
				string CompileSDKVersion = CompileSDKLevelForArch.ToString();

				UPL.SetGlobalContextVariable("CompileSDK", CompileSDKVersion);
				...
				Logger.LogInformation("bPackageDataInsideApk = {PackageDataInsideApk}", bPackageDataInsideApk);
				if (bPackageDataInsideApk)
				{
				...
				GenerateGradleAARImports(EngineDirectory, UnrealBuildPath, Archs.Select(x => x.NativeArch));
				...
				DetermineScreenOrientationRequirements(Archs[0].NativeArch, out bNeedPortrait, out bNeedLandscape);

				//Now keep the splash screen images matching orientation requested
				PickSplashScreenOrientation(CookFlavor, UnrealBuildPath, bNeedPortrait, bNeedLandscape);
				...
				File.WriteAllText(Path.Combine(IntermediateAndroidPath, Arch + "_AndroidManifest.xml"), Manifest);
				File.WriteAllText(Path.Combine(UnrealBuildPath, "AndroidManifest.xml"), Manifest);

				// Check if included SplashActivity in AndroidManifest
				bool bHasSplashActivity = Manifest.Contains("com.epicgames.unreal.SplashActivity");
				...
				if (bSkipGradleBuild)
				{
					SourceSOName = OutputPath;
					if (!File.Exists(SourceSOName))
					{
						Logger.LogWarning("Did not find compiled .so [{SourceSOName}]", SourceSOName);
					}
				}
				else if (bDontBundleLibrariesInAPK)
				{
					SourceSOName = AndroidToolChain.InlineArchName(OutputPath, Arch);
					string SourceSONameStripped = Path.Combine(Path.GetDirectoryName(SourceSOName)!, Path.GetFileNameWithoutExtension(SourceSOName) + "-stripped" + Path.GetExtension(SourceSOName));
					if (!File.Exists(SourceSONameStripped))
					{
						Logger.LogWarning("Did not find compiled .so [{FinalSONameStripped}]", SourceSONameStripped);
					}
					...
					string? SOPushScriptLocation = Path.GetDirectoryName(SourceSOName)!;

					string FinalSONameStrippedRelative = Path.GetRelativePath(SOPushScriptLocation, SourceSONameStripped);
...
";
					...
					string SOPushScriptName = Path.Combine(Path.GetDirectoryName(SourceSOName)!, "Push_" + Path.GetFileNameWithoutExtension(SourceSOName) + "_so.bat");
				...
				else
				{
					...
					string JniDir = Path.Combine(UnrealBuildPath, "jni", NDKArch);
						Directory.CreateDirectory(JniDir);
					Utils.LinkOrCopyIfDifferent(Path.Combine(JniDir, "libUnreal.so"), SourceSOName);

					string JniLibsDir = Path.Combine(UnrealBuildGradleMainPath, "jniLibs", NDKArch);
					Directory.CreateDirectory(JniLibsDir);
					Utils.LinkOrCopyIfDifferent(Path.Combine(JniLibsDir, "libUnreal.so"), SourceSOName);
				}

				// update GameActivity.java and GameApplication.java if out of date
				UpdateGameActivity(Arch, NDKArch, EngineDirectory, UnrealBuildPath, bDontBundleLibrariesInAPK, bJavaPerfettoMarkers, Configuration, bHasSplashActivity);
				UpdateGameApplication(Arch, NDKArch, EngineDirectory, UnrealBuildPath, bJavaPerfettoMarkers, Configuration, SourceSOName);
				...
				switch (NDKArch)
				{
					default:
					case "arm64-v8a":
						Excludes = new string[] { "armeabi-v7a", "x86", "x86_64" };
						break;
...
//				LocalProperties.AppendLine(string.Format("ndk.dir={0}", Environment.GetEnvironmentVariable("NDKROOT")!.Replace("\\", "/")));
				...
				CreateGradlePropertiesFiles(ToolChain, Arch, MinSDKVersion, TargetSDKVersion, CompileSDKVersion, BuildToolsVersion, PackageName, DestApkName, NDKArch,
					UnrealBuildFilesPath, GameBuildFilesPath, UnrealBuildGradleAppPath, UnrealBuildPath, UnrealBuildGradlePath, bForDistribution, bSkipGradleBuild, RequiredOBBFiles, bJavaPerfettoMarkers);
				...
				if (!bSkipGradleBuild)
				{
					...
					if (!bEnableBundle)
					{
						...
						if (!bHasAndroidStudioProject)
						{
							...
							if (bIsBuildMachine && OperatingSystem.IsWindows())
							{
								string Output;
							...
							string ShellExecutable = OperatingSystem.IsWindows() ? "cmd.exe" : "/bin/sh";
							string ShellParametersBegin = OperatingSystem.IsWindows() ? "/c " : "-c '";
							string ShellParametersEnd = OperatingSystem.IsWindows() ? "" : "'";
				...
				Ini.GetBool("/Script/AndroidRuntimeSettings.AndroidRuntimeSettings", "bBuildWithHiddenSymbolVisibility", out bool bBuildWithHiddenSymbolVisibility);
				Ini.GetBool("/Script/AndroidRuntimeSettings.AndroidRuntimeSettings", "bSaveSymbols", out bool bSaveSymbols);
				// Deprecated check of bBuildWithHiddenSymbolVisibility and forced bSaveSymbols = true. Remove in 5.8+
				bSaveSymbols = true;
				if (bBuildWithHiddenSymbolVisibility || bSaveSymbols)
				{
					// Copy .so with symbols to 
			...
			if (bEnableBundle && !bSkipGradleBuild)
			{
				...
				string ABIFilter = "";
				string[] JavaSrcRootComponents = { "app", "src", "main" };
				...
				foreach (Tuple<UnrealArch, string> build in BuildList)
				{
					...
					foreach (string Filename in SourceFiles)
					{
						...
						string[] DirectoryNameComponents = DirectoryName.Split(Path.DirectorySeparatorChar, StringSplitOptions.RemoveEmptyEntries);
						int BuildDirIndex = Array.IndexOf(DirectoryNameComponents, "build");
						// if a build directory exists
						if (BuildDirIndex >= 0)
						{
							// get the directory component index where app/src/main (JavaSrcRootComponents) begins
							int JavaSrcRootIndex = Array.IndexOf(DirectoryNameComponents, JavaSrcRootComponents[0]);
							for (int c = 1; c < JavaSrcRootComponents.Length && JavaSrcRootIndex >= 0; ++c)
							{
								int NextComponentIndex = Array.IndexOf(DirectoryNameComponents, JavaSrcRootComponents[c]);
								if (NextComponentIndex != JavaSrcRootIndex + c)
								{
									JavaSrcRootIndex = -1;
								}
							}
							// if the said build directory is not beneath the Java src root
							if (JavaSrcRootIndex == -1 || BuildDirIndex < JavaSrcRootIndex)
							{
								continue;
							}
						}
				...
				{
					string NDKArch = Archs[0].NativeArch;
					...
					string ManifestFilename = Path.Combine(UnrealBuildGradlePath, Path.Combine(JavaSrcRootComponents), "AndroidManifest.xml");
		...
		private void UpdateGameActivity(UnrealArch UnrealArch, string NDKArch, string EngineDir, string UnrealBuildPath, bool bDontBundleLibrariesInAPK,
				bool bJavaPerfettoMarkers, UnrealTargetConfiguration Configuration, bool bHasSplashActivity)
		{
...
";
			...
			if (bJavaPerfettoMarkers)
			{
				GameActivityImportAdditionsDefault += "import androidx.tracing.Trace;\n";
			}
			...
			Dictionary<string, string> Replacements = new Dictionary<string, string>{
				...
				{ "//$${gameActivityOnCreateBeforeProcessSystemInfoAdditions}$$", UPL.ProcessPluginNode(NDKArch, "gameActivityOnCreateBeforeProcessSystemInfoAdditions", "")},
				{ "//$${gameActivityProcessSystemInfoThreadBeforeConfigRulesAdditions}$$", UPL.ProcessPluginNode(NDKArch, "gameActivityProcessSystemInfoThreadBeforeConfigRulesAdditions", "")},
				{ "//$${gameActivityProcessSystemInfoThreadAfterConfigRulesAdditions}$$", UPL.ProcessPluginNode(NDKArch, "gameActivityProcessSystemInfoThreadAfterConfigRulesAdditions", "")},
				{ "//$${gameActivityOnCreateBeforeLoadLibrariesAdditions}$$", UPL.ProcessPluginNode(NDKArch, "gameActivityOnCreateBeforeLoadLibrariesAdditions", "")},
				...
				{ "//$${gameActivityOnTopResumeActivityAdditions}$$", UPL.ProcessPluginNode(NDKArch, "gameActivityOnTopResumeActivityAdditions", "")},
				...
				{ "$${gameActivityIsShippingConfig}$$", Configuration == UnrealTargetConfiguration.Shipping ? "true" : "false"},
				{ "$${gameActivityHasSplashActivity}$$", bHasSplashActivity ? "true" : "false"},
			};
			...
			for (int LineIndex = 0; LineIndex < TemplateSrc.Length; ++LineIndex)
			{
				...
				if (!bJavaPerfettoMarkers)
				{
					if (SrcLine.StartsWith("Trace."))
					{
						TemplateSrc[LineIndex] = "";
						Changed = true;
				...
				if (!Changed)
				{
					foreach (KeyValuePair<string, string> KVP in Replacements)
					{
						if (SrcLine.Contains(KVP.Key))
						{
							...
							{
								int TraceStartIndex = KVP.Value.IndexOf("Trace.");
								if (TraceStartIndex == 0 || (TraceStartIndex > 0 && KVP.Value[TraceStartIndex - 1] == '\n'))
								{
									String Addition = KVP.Value;
									do
									{
										int TraceEndIndex = Addition.IndexOf("\n", TraceStartIndex);
										if (TraceEndIndex < 0)
										{
											Addition = Addition.Substring(0, TraceStartIndex);
										}
										else
										{
											Addition = Addition.Substring(0, TraceStartIndex) + Addition.Substring(TraceEndIndex + 1);
										}
										TraceStartIndex = Addition.IndexOf("Trace.", TraceStartIndex);
									} while (TraceStartIndex >= 0);
									SrcLine = SrcLine.Replace(KVP.Key, Addition);
								}
								else
								{
									SrcLine = SrcLine.Replace(KVP.Key, KVP.Value);
				...
				bool bJavaPerfettoMarkers, UnrealTargetConfiguration Configuration, string SOName)
		{
			...
			Dictionary<string, string> Replacements = new Dictionary<string, string>{
				{ "//$${gameApplicationImportAdditions}$$", UPL!.ProcessPluginNode(NDKArch, "gameApplicationImportAdditions", GameApplicationImportAdditionsDefault)},
				...
				{ "$${gameApplicationIsShippingConfig}$$", Configuration == UnrealTargetConfiguration.Shipping ? "true" : "false"},
				{ "//$${gameApplicationEarlyLoadAdditions}$$", GameApplicationEarlyLoad},
			};
		...
		private AndroidAARHandler CreateAARHandler(string EngineDir, string UnrealBuildPath, IEnumerable<string> NDKArches, bool HandleDependencies = true)
		{
		...
		private void PrepareJavaLibsForGradle(string JavaLibsDir, string UnrealBuildGradlePath, string InMinSdkVersion, string InTargetSdkVersion, string CompileSDKVersion, string BuildToolsVersion, string NDKArch)
		{
			...
			foreach (string LibDir in LibDirs)
			{
				...
				BuildGradleContent.AppendLine("\tndkPath NDK_PATH");
				BuildGradleContent.AppendLine("\tcompileSdk COMPILE_SDK_VERSION.toInteger()");
		...
		private void GenerateGradleAARImports(string EngineDir, string UnrealBuildPath, IEnumerable<string> NDKArches)
		{
