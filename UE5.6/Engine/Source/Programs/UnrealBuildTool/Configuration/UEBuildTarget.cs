...
namespace UnrealBuildTool
{
	...
	class UEBuildTarget
	{
		...
		public static UEBuildTarget Create(TargetDescriptor Descriptor, bool bSkipRulesCompile, bool bForceRulesCompile, bool bUsePrecompiled, UnrealIntermediateEnvironment IntermediateEnvironment, ILogger Logger)
		{
			...
			{
				throw new BuildException("Platform {0} is not a valid platform to build. Check that the SDK is installed properly and that you have the necessary platform support files (DataDrivenPlatformInfo.ini, SDK.json, etc).", Descriptor.Platform);
			}
		...
		{
			bool anyErrors = plugin.ValidatePlugin(logger, Rules);
		...
		{
			int errorCount = 0;
			CaptureLogger[] captureLoggers = new CaptureLogger[Modules.Count];
			Parallel.ForEach(Modules.Values.OrderBy(x => x.Name), (module, _, index) =>
			{
				captureLoggers[index] = new();
				UEBuildPlatform.GetBuildPlatform(Platform).ValidateModule(module, Rules, captureLoggers[index]);
				bool anyErrors = UEBuildPlatform.GetBuildPlatform(Platform).ValidateModuleIncludePaths(module, Rules, Modules.Values, captureLoggers[index]);
				anyErrors = ValidateModule(module, captureLoggers[index]) || anyErrors;
				if (anyErrors)
				{
			...
			foreach (CaptureLogger captureLogger in captureLoggers)
			{
		...
		{
			bool anyErrors = module.ValidateModule(logger);
			...
			if (module.Binary != null)
			{
				foreach (UEBuildModule referencedModule in referencedModules.Value)
				{
					...
					{
						logger.LogError(KnownLogEvents.Validation, "Module '{ModuleName}' ({ScopeName}) should not reference module '{DependencyModuleName}' ({DependencyScopeName}). Hierarchy is {Hierarchy}.", module.Name, module.Rules.Context.Scope.Name, referencedModule.Name, referencedModule.Rules.Context.Scope.Name, referencedModule.Rules.Context.Scope.FormatHierarchy());
						anyErrors = true;
					}
				}
			}
			...
			ModuleDescriptor? moduleDescriptor = ProjectDescriptor?.Modules?.FirstOrDefault(x => x.Name == module.Name);
			if (moduleDescriptor != null)
			{
				...
				foreach (UEBuildModuleCPP dependencyModule in referencedModules.Value.OfType<UEBuildModuleCPP>())
				{
					...
					{
						ModuleDescriptor? dependencyModuleInfo = dependencyPluginInfo.Descriptor.Modules?.FirstOrDefault(x => x.Name == dependencyModule.Name);
						...
						if (projectPluginReference != null)
						{
							...
							{
								logger.LogWarning(KnownLogEvents.Validation, "{ProjectName} plugin dependency '{DependencyPluginName}' is not enabled, but module '{ModuleName}' depends on '{DependencyModuleName}'.", projectName, dependencyPluginInfo.Name, module.Name, dependencyModule.Name);
							}
							else if (!projectPluginReference.IsEnabledForTarget(TargetType))
							{
								logger.LogWarning(KnownLogEvents.Validation, "{ProjectName} plugin dependency '{DependencyPluginName}' is not enabled for target '{TargetType}', but module '{ModuleName}' depends on '{DependencyModuleName}'.", projectName, dependencyPluginInfo.Name, TargetType.ToString(), module.Name, dependencyModule.Name);
							}
							else if (!projectPluginReference.IsEnabledForPlatform(Platform))
							{
								logger.LogWarning(KnownLogEvents.Validation, "{ProjectName} plugin dependency '{DependencyPluginName}' is not enabled for platform '{Platform}', but module '{ModuleName}' depends on '{DependencyModuleName}'.", projectName, dependencyPluginInfo.Name, Platform.ToString(), module.Name, dependencyModule.Name);
							}
							else if (!projectPluginReference.IsEnabledForTargetConfiguration(Configuration))
							{
								logger.LogWarning(KnownLogEvents.Validation, "{ProjectName} plugin dependency '{DependencyPluginName}' is not enabled for target configuration '{Configuration}', but module '{ModuleName}' depends on '{DependencyModuleName}'.", projectName, dependencyPluginInfo.Name, Configuration.ToString(), module.Name, dependencyModule.Name);
							}
						}
						else
						{
							...
							{
								logger.LogWarning(KnownLogEvents.Validation, "{ProjectName} does not list plugin '{DependencyPluginName}' as a dependency, but module '{ModuleName}' depends on '{DependencyModuleName}'.", projectName, dependencyPluginInfo.Name, module.Name, dependencyModule.Name);
							}
						}
					}
				}
		...
		public async Task<TargetMakefile> BuildAsync(BuildConfiguration BuildConfiguration, ISourceFileWorkingSet WorkingSet, TargetDescriptor TargetDescriptor, ILogger Logger, bool bInitOnly = false)
		{
			...
			Makefile.HotReloadModuleNames = GetHotReloadModuleNames(TargetDescriptor);
			...
			Task VNITask = VNIExecution.RunVNIAsync(ModulesToGenerateHeadersFor, Makefile, TargetDescriptor, Logger);
#endif
			...
			// Add any relevent platform sdk info
			IEnumerable<UnrealTargetPlatform> SdkPlatforms = Rules.Type == TargetType.Editor || Rules.Type == TargetType.Program ? UnrealTargetPlatform.GetValidPlatforms() : [BuildHostPlatform.Current.Platform, Rules.Platform];
			foreach (UnrealTargetPlatform platform in SdkPlatforms.Distinct())
			{
				Makefile.PlatformSDKInfo.Add(platform.ToString(), UEBuildPlatformSDK.GetSDKForPlatform(platform.ToString())?.GetAllSDKInfo().ToString() ?? String.Empty);
			}
...
#pragma warning restore CA1308 // Normalize strings to uppercase
			...
			void CreateStripAction(string Name, UEBuildBinaryType binaryType, IEnumerable< FileItem> toStrip, DirectoryReference IntermediateDirectory)
			{
				...
				MakefileBuilder.CreateIntermediateTextFile(exportsRsp, arguments);
				...
				exportsAction.WorkingDirectory = Unreal.EngineSourceDirectory;
				exportsAction.StatusDescription = Name;
			...
			{
				CreateStripAction(Binary.OutputFilePaths.First().GetFileNameWithoutExtension(), Binary.Type, Binary.InputObjects, Binary.IntermediateDirectory);
			}
		}
		...
		{
			void SetPerModuleInline(UEBuildBinary binary)
			{
				binary.PrimaryModule.bCreatePerModuleFile = false;

				var queue = new Queue<(UEBuildModule Module, UEBuildModule Source)>();
				var visited = new HashSet<UEBuildModule>();

				// Queue up primary module and all direct dependencies
				queue.Enqueue((binary.PrimaryModule, binary.PrimaryModule));
				foreach (var m in binary.Modules)
				{
					if (visited.Add(m))
					{
						queue.Enqueue((m, m));
				...
				// Search for non-engine PerModuleInlineFile.. 
				while (queue.Count > 0)
				{
					var current = queue.Dequeue();
					if (current.Module.RulesFile.IsUnderDirectory(Unreal.EngineDirectory))
					{
						continue;
					}
					if (current.Module.Rules.PublicDefinitions.Exists(x => x.StartsWith("PER_MODULE_INLINE_FILE")))
					{
						((UEBuildModuleCPP)current.Source).bCreatePerModuleFile = true;
						return;
					}

					foreach (var dep in current.Module.PublicDependencyModules!.Union(current.Module.PrivateDependencyModules!))
					{
						if (visited.Add(dep))
						{
							queue.Enqueue((dep, current.Source));
						}
					}
				}
			...
			void MergeModulesToDLL(string name, IEnumerable<UEBuildModuleCPP> modules)
			{
				...
				IEnumerable<UEBuildModuleCPP> modulesList = modules;
				UEBuildBinary mergedBinary = CreateDynamicLibraryForModules(name, modulesList);
				foreach (UEBuildModuleCPP module in modules)
				{
					module.Binary = mergedBinary;
					module.bCreatePerModuleFile = false;
				}
			...
			void GetPluginDependencies(UEBuildPlugin plugin, ref HashSet<UEBuildPlugin> dependencies, bool recursively = false)
			{
				foreach (PluginReferenceDescriptor dependency in plugin.Info.Descriptor.Plugins ?? new())
				{
					...
					{
						IEnumerable<UEBuildPlugin> dependentPluginCandidates = (plugin.Dependencies ?? new()).Where(x => x.Name == dependency.Name);
			...
			void GetModulesDependenciesRecursively(IEnumerable<UEBuildModuleCPP> Modules, ref HashSet<UEBuildModuleCPP> Needed)
			{
				...
				foreach (UEBuildModuleCPP item in CPPModules)
				{
					...
					if (bAddDependencies)
					{
						...
						{
							GetModulesDependenciesRecursively(item.PrivateDependencyModules.OfType<UEBuildModuleCPP>(), ref Needed);
						}

						if (item.PublicDependencyModules != null && item.PublicDependencyModules.Count > 0)
						{
							GetModulesDependenciesRecursively(item.PublicDependencyModules.OfType<UEBuildModuleCPP>(), ref Needed);
						}
					}
				}
			}
			...
			if (Rules.MergePlugins.Any() && BuildPlugins != null)
			{
				...
				foreach (UEBuildPlugin plugin in unmergablePlugins)
				{
					...
					foreach (UEBuildPlugin dependency in dependencies)
					{
						if (!unmergeablePluginDependencies.ContainsKey(dependency))
						{
							HashSet<string> pluginNames = new();
							pluginNames.Add(plugin.Name);
							unmergeablePluginDependencies.Add(dependency, pluginNames);
						}
						else
						{
							unmergeablePluginDependencies[dependency].Add(plugin.Name);
						}
					}
				}

				// Report unmergeable dependencies
		...
		private HashSet<string> GetHotReloadModuleNames(TargetDescriptor TargetDescriptor)
		{
			...
			{
				// Legacy hot reload cannot reload modules from the executable binary
				IEnumerable<UEBuildBinary> ValidBinaries = TargetDescriptor.HotReloadMode == HotReloadMode.FromEditor || TargetDescriptor.HotReloadMode == HotReloadMode.FromIDE
					? Binaries.Skip(1)
					: Binaries;
				foreach (UEBuildBinary Binary in ValidBinaries)
				{
					IEnumerable<UEBuildModule> GameModules = Binary.FindHotReloadModules();
					if (GameModules.Any())
					{
		...
		public void CreateSharedPCHInstances(ReadOnlyTargetRules Target, UEToolChain ToolChain, List<UEBuildBinary> OriginalBinaries, CppCompileEnvironment GlobalCompileEnvironment, IActionGraphBuilder Graph, ILogger Logger)
		{
			...
			if (Target.CppCompileWarningSettings.PCHPerformanceIssueWarningLevel != WarningLevel.Off)
			{
				bool bFoundIssues = false;
				LogLevel PCHLoggerLevel = Target.CppCompileWarningSettings.PCHPerformanceIssueWarningLevel == WarningLevel.Warning ? LogLevel.Warning : LogLevel.Error;
				string Prefix = Target.CppCompileWarningSettings.PCHPerformanceIssueWarningLevel == WarningLevel.Warning ? "Warning:" : "Error:";
				for (int SharedPCHIndex = NumSharedPCHs - 1; SharedPCHIndex >= 0; SharedPCHIndex--)
				{
					...
					if (SharedPCH.Instances.Any())
					{
						...
						foreach (PrecompiledHeaderInstance UnsafeTypeCastWarningInstance in SharedPCH.Instances.Where(instance => instance.Modules.First().Rules.CppCompileWarningSettings.UnsafeTypeCastWarningLevel == WarningLevel.Warning))
						{
							...
							UpdatedCppCompileEnvironment.CppCompileWarnings.UnsafeTypeCastWarningLevel = WarningLevel.Error;
							foreach (PrecompiledHeaderInstance UnsafeTypeCastErrorInstance in SharedPCH.Instances.Where(instance => instance.Modules.First().Rules.CppCompileWarningSettings.UnsafeTypeCastWarningLevel == WarningLevel.Error))
							{
		...
		private UEBuildPlugin? AddPlugin(PluginReferenceDescriptor Reference, string ReferenceChain, string[] ExcludeFolders, Dictionary<string, UEBuildPlugin> NameToInstance, Dictionary<string, PluginSet> NameToInfos, ILogger Logger)
		{
				...
				if (!Reference.bOptional && !Instance.bDescriptorReferencedExplicitly)
				{
					...
					if (Instance.Descriptor.Plugins != null)
					{
						foreach (PluginReferenceDescriptor NextReference in Instance.Descriptor.Plugins)
						{
							...
							if (Info == null || !Rules.ShouldIgnorePluginDependency(Info, NextReference))
							{
			...
			else
			{
				...
				{
					if (Rules.CppCompileWarningSettings.DisablePluginsConflictWarningLevel == WarningLevel.Error)
					{
						LogValue PluginLogValue = LogValue.SourceFile(Info.File, Info.File.GetFileName());
					...
					else if (Rules.CppCompileWarningSettings.DisablePluginsConflictWarningLevel == WarningLevel.Warning)
					{
					...
					else if (Rules.CppCompileWarningSettings.DisablePluginsConflictWarningLevel != WarningLevel.Off)
					{
		...
		private void SetupUbtPlugins(ILogger Logger)
		{
			...
			if (Rules.bGenerateProjectFiles)
			{
				return;
			}
		...
		protected void SetupBinaries(ILogger Logger)
		{
			...
			if (bIsUniqueBuildProgramWithExternalProject)
			{
				...
				Logger.LogInformation("Overriding output directory for unique build program {Name} from {Output} to {Project}, because it is being built with external project {ProjectFile}", Rules.Name, GetOutputDirectoryForExecutable(ProjectDirectory, Rules.File), ProjectDirectory, ProjectFile);
		...
		public void SetupGlobalEnvironment(UEToolChain ToolChain, CppCompileEnvironment GlobalCompileEnvironment, LinkEnvironment GlobalLinkEnvironment)
		{
			...
			ToolChain.SetUpGlobalEnvironment(Rules, GlobalCompileEnvironment, GlobalLinkEnvironment);
			...
			GlobalCompileEnvironment.CppCompileWarnings = Rules.CppCompileWarningSettings.CloneAsWriteable();
			...
			GlobalCompileEnvironment.MinArm64CpuTarget = Rules.MinArm64CpuTarget;
			...
			if (Rules.bUseHeaderUnitsForPch)
			{
				...
				GlobalCompileEnvironment.Definitions.Add("UE_HEADER_UNITS=");
			}
			...
			// Setup defines for statically selected allocator type
			string[] Mapping = {
			...
			for (int i = 0; i < (int)StaticAllocatorType.None; ++i)
			{
				if (GlobalCompileEnvironment.Definitions.Contains(Mapping[i] + "=0") || GlobalCompileEnvironment.Definitions.Contains(Mapping[i] + "=1"))
				{
					throw new BuildException("Target `{0}` contains a definition for `{1}` allocator define. It conflicts with the new allocator type declatation via UBT. Please use `StaticAllocator = StaticAllocatorType.{2};` in your *.Target.cs instead", TargetName, Mapping[i], ((StaticAllocatorType)i).ToString());
				}

				string EnabledValue = Rules.StaticAllocator == (StaticAllocatorType)i ? "=1" : "=0";
				GlobalCompileEnvironment.Definitions.Add(Mapping[i] + EnabledValue);
			}
		...
		private ModuleRules CreateModuleRulesAndSetDefaults(string ModuleName, string ReferenceChain, ILogger Logger)
		{
			...
			// If compiling with the AutoRTFM compiler, implicitly depend on the AutoRTFM runtime.
			if (Rules.bUseAutoRTFMCompiler && !RulesObject.bDisableAutoRTFMInstrumentation && ModuleName != "AutoRTFM")
			{
				RulesObject.PrivateDependencyModuleNames.Add("AutoRTFM");
			}
			...
			RulesObject.PrivateDefinitions.Add($"UE_MODULE_NAME=\"{ModuleName}\"");
			RulesObject.PrivateDefinitions.Add($"UE_PLUGIN_NAME=\"{(RulesObject.Plugin?.Name ?? String.Empty)}\"");

			// Module validation can be found in UEBuildModuleCPP.ValidateModule()
