...
SESSIONS_TEST_CASE("If I call FindSessions while a search is already running for that user, I get an error", EG_SESSIONS_FINDSESSIONS_TAG)
{
	...
	LoginPipeline
		...
		.EmplaceAsyncLambda([&FindFirstSessionsHelperParams, &FindSecondSessionsHelperParams](FAsyncLambdaResult Promise, const IOnlineServicesPtr& OnlineServices)
			{
...
SESSIONS_TEST_CASE("If I call FindSessions with valid information, I will get a list of ids for all the valid sessions", EG_SESSIONS_FINDSESSIONSEOS_TAG)
{
	...
	constexpr uint32_t ExpectedSessionsFound = 2;
...
SESSIONS_TEST_CASE("Call FindSessions using custom setting as a filter Double", EG_SESSIONS_FINDSESSIONSEOS_TAG)
{
	...
	LoginPipeline
		...
		.EmplaceAsyncLambda([&FifteenthFindSessionsHelperParams, &DoubleSettingValue](FAsyncLambdaResult Promise, const IOnlineServicesPtr& OnlineServices)
			{
				ISessionsPtr SessionsInterface = OnlineServices->GetSessionsInterface();
