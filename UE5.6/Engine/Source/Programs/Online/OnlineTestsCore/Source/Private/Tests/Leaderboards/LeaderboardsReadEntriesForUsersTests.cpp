...
LEADERBOARDS_TEST_CASE("ReadEntriesForUsers succeed", "[MultiAccount]")
{
	...
	LeaderboardsFixture_NUsers(MoveTemp(GetLoginPipeline({ AccountIdA, AccountIdB })), AccountIds, 2)
		.EmplaceAsyncLambda([&](const FAsyncLambdaResult& Promise, const IOnlineServicesPtr& Services) { ReadEntriesForUsers_2UsersFixture(Promise, Services, "Stat_Use_Set", AccountIdA, AccountIdB, LeaderboardEntries); })
		.EmplaceLambda([&](const IOnlineServicesPtr& Services) { CheckEntries_2Users_Stat_Use_Set(LeaderboardEntries, AccountIdA, AccountIdB); })
		.EmplaceAsyncLambda([&](const FAsyncLambdaResult& Promise, const IOnlineServicesPtr& Services) { ReadEntriesForUsers_2UsersFixture(Promise, Services, "Stat_Use_Smallest", AccountIdA, AccountIdB, LeaderboardEntries); })
		.EmplaceLambda([&](const IOnlineServicesPtr& Services) { CheckEntries_2Users_Stat_Use_Smallest(LeaderboardEntries, AccountIdA, AccountIdB); });
...
LEADERBOARDS_TEST_CASE("Verify ReadEntriesForUsers returns a fail message if the local user is not logged in", "[MultiAccount]")
{
	...
	LeaderboardsFixture_NUsers(MoveTemp(GetLoginPipeline({ AccountId })), AccountIds, 2)
		.EmplaceAsyncLambda([&](const FAsyncLambdaResult& Promise, const IOnlineServicesPtr& Services) { ReadEntriesForUsersFixture(Promise, Services, "Stat_Use_Set", { LocalUserInvalid, AccountId }, UE::Online::Errors::InvalidUser(), LeaderboardEntries); });
...
LEADERBOARDS_TEST_CASE("Verify ReadEntriesForUsers returns an empty array of entries if the given UserIds array is empty")
{
	...
	LeaderboardsFixture_NUsers(MoveTemp(GetLoginPipeline({ LocalUser })), AccountIds, 1)
		.EmplaceAsyncLambda([&](const FAsyncLambdaResult& Promise, const IOnlineServicesPtr& Services) {
...
LEADERBOARDS_TEST_CASE("Verify ReadEntriesForUsers returns a fail message if given a UserIds array that contains one valid user ID and one invalid user ID", "[MultiAccount]")
{
	...
	TArray< FAccountId*> AccountIds = { &LocalUser, &AccountIdInvalid };
	LeaderboardsFixture_NUsers(MoveTemp(GetLoginPipeline({ LocalUser })), AccountIds, 1)
		.EmplaceAsyncLambda([&](const FAsyncLambdaResult& Promise, const IOnlineServicesPtr& Services) { ReadEntriesForUsersFixture(Promise, Services, "Stat_Use_Set", { LocalUser, AccountIdInvalid }, UE::Online::Errors::InvalidUser(), LeaderboardEntries); });
	RunToCompletion();
}
