...
void FTestPipeline::operator()(const IOnlineServicesPtr& Subsystem)
{
	double CurrentTime = FPlatformTime::Seconds();
...
void FTestPipeline::EvaluatePlatformTickTime(double&& TickTime)
{
	...
	if (EvaluateTickConfig.bEvaluateTickCheckActive)
	{
		if (SubsystemTickCount >= EvaluateTickConfig.MinimumTickCount)
		{
			...
			//CHECK(AverageTick <= ExpectedAverageTick); // todo: what are these? they are spamming checks in my code
		}
		...
		//CHECK(Tick <= AbsoluteMaximumTick); // todo: what are these? they are spamming checks in my code
	}
}
...
FTestDriver::~FTestDriver()
{
	...
	ForeachSubsystemInstance([this](const IOnlineServicesPtr&  OnlineSubsystem, FTestPipeline&)
		{
...
bool FTestDriver::AddPipeline(FTestPipeline&& Pipeline, const FPipelineTestContext& TestContext)
{
	...
	{
		SubsystemInstances.Emplace(OnlineServices, MoveTemp(Pipeline));
		return true;
	}
}
...
void FTestDriver::RunToCompletion()
{
	...
	ForeachSubsystemInstance([](const IOnlineServicesPtr&  Subsystem, FTestPipeline& Func)
		{
			Func.Start();
		});
	...
	while (!SubsystemInstances.IsEmpty())
	{
		FPlatformProcess::Sleep(TICK_DURATION.GetTotalSeconds());
			...
			ForeachSubsystemInstance([this](const IOnlineServicesPtr& Services, FTestPipeline& CurrentPipeline)
				{
		...
		ForeachSubsystemInstance([](const IOnlineServicesPtr& Services, FTestPipeline& Func)
			{
