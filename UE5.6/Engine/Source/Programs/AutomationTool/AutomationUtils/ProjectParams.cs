...
namespace AutomationTool
{
	...
	[ParamHelp("snapshot", "Imports the most recently published project snapshot (if cooking, this forces the use of -cookincremental)", ParamType = typeof(bool))]
	[ParamHelp("cookincremental", "Cooks incrementally using previously-cooked data or a published snapshot as a base", ParamType = typeof(bool))]
	...
	public class ProjectParams
	{
		...
		public ProjectParams(ProjectParams InParams)
		{
			...
			this.DLCOverrideCryptoSettings = InParams.DLCOverrideCryptoSettings;
			this.DLCOverrideEncryptionKeyGuid = InParams.DLCOverrideEncryptionKeyGuid;
			...
			this.Snapshot = InParams.Snapshot;
			this.IncrementalCook = InParams.IncrementalCook;
			...
			this.UseMergedModules = InParams.UseMergedModules;
			...
			this.bMakeMacNative = InParams.bMakeMacNative;
		}
		...
		public ProjectParams(			
			...
			FileReference DLCOverrideCryptoSettings = null,
			string DLCOverrideEncryptionKeyGuid = null,
			...
			string ZenWorkspaceSharePath = null,
			bool? ZenOmitPakFiles = null,
			...
			)
		{
			...
			this.ZenWorkspaceSharePath = ParseParamValueIfNotSpecified(Command, ZenWorkspaceSharePath, "ZenWorkspaceSharePath", String.Empty);
			this.ZenOmitPakFiles = GetParamValueIfNotSpecified(Command, ZenOmitPakFiles, false, "ZenOmitPakFiles");
			...
			this.UseMergedModules = GetParamValueIfNotSpecified(Command, UseMergedModules, this.UseMergedModules, "UseMergedModules");
			...
			this.Snapshot = GetParamValueIfNotSpecified(Command, Snapshot, this.Snapshot, "Snapshot");

			// -snapshot implies the use of -cookincremental for cooking.
			this.IncrementalCook = this.Snapshot || GetParamValueIfNotSpecified(Command, IncrementalCook, this.IncrementalCook, "cookincremental");
			...
			this.bMakeMacNative = GetParamValueIfNotSpecified(Command, null, this.bMakeMacNative, "macnative");
		...
		/// </summary>
		public string ZenWorkspaceSharePath { private set; get; }

		/// <summary>
		/// Should omit pak files when deploying build to target for Zen pak streaming
		/// </summary>
		public bool ZenOmitPakFiles { private set; get; }
		...
		/// <summary>
		/// Stage: Path to the crypto.json file to use for decrypting ReferenceContainerGlobalFileName, if needed.
		/// </summary>
		...
		/// <summary>
		/// Specifies keys to use rather than those generated from ini files when using a staging manifest (ignored when using chunk manifests)
		/// </summary>
		public EncryptionAndSigning.CryptoSettings DLCOverrideCryptoSettings;

		/// <summary>
		/// Specifies an encryption key for UnrealPak to use when using a staging manifest (not a chunk manifest)
		/// </summary>
		public string DLCOverrideEncryptionKeyGuid;
		...
		/// </summary>
		[Help("snapshot", "Imports the most recently published project snapshot (if cooking, this forces the use of -cookincremental)")]
		public bool Snapshot;

		/// <summary>
		/// Cook: Cooks incrementally using previously-cooked data or a published snapshot as a base
		/// </summary>
		[Help("cookincremental", "Cooks incrementally using previously-cooked data or a published snapshot as a base")]
		public bool IncrementalCook;
		...
		[Help("macnative", "Generate a \"Designed for iPad\" .app that can be run natively on a silicon Mac")]
		public bool bMakeMacNative { get; set; }
