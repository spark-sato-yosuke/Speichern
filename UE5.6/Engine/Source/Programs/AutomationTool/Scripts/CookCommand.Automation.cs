...
namespace AutomationScripts
{
	...
	public partial class Project : CommandUtils
	{
		private static string GetGenericCookCommandletParams(ProjectParams Params)
		{
			...
			if (Params.IncrementalCook)
			{
				CommandletParams += " -cookincremental";
			}
		...
		private static HashSet<string> GetPlatformsToCook(ProjectParams Params)
		{
		...
		{
			var PlatformsToCookSet = GetPlatformsToCook(Params);
		...
		public static void Cook(ProjectParams Params)
		{
			if (Params.Snapshot)
			{
				Logger.LogInformation("********** SNAPSHOT IMPORT STARTED **********");

				var ImportStartTime = DateTime.UtcNow;
				string PlatformsToCook = CombineCommandletParams(GetPlatformsToCook(Params));
				ImportSnapshots(Params, PlatformsToCook);

				Logger.LogInformation("Snapshot import time: {0:0.00} s", (DateTime.UtcNow - ImportStartTime).TotalMilliseconds / 1000);
				Logger.LogInformation("********** SNAPSHOT IMPORT COMPLETED **********");
			}
		...
		private static void ImportSnapshots(ProjectParams Params, string PlatformsToCook)
		{
			string Branch;
			int Changelist;
			if (UnrealBuildTool.BuildVersion.TryRead(UnrealBuildTool.BuildVersion.GetDefaultFileName(), out UnrealBuildTool.BuildVersion CurrentBuildVersion) && CurrentBuildVersion.Changelist != 0)
			{
				Branch = CurrentBuildVersion.BranchName.Replace('+','/');
				Changelist = CurrentBuildVersion.Changelist;
			}
			else if (P4Enabled)
			{
				Branch = P4Env.Branch;
				Changelist = P4Env.Changelist;
			}
			else
			{
				throw new AutomationException($"Cannot get current branch or changelist for snapshot import - could not read from {UnrealBuildTool.BuildVersion.GetDefaultFileName()} and -p4 was not specified on the command line");
			}

			FileReference ZenLaunchExe = new FileReference(Path.Combine(Unreal.EngineDirectory.FullName, "Binaries", HostPlatform.Current.HostEditorPlatform.ToString(), String.Format("ZenLaunch{0}", RuntimePlatform.ExeExtension)));

			if (!FileReference.Exists(ZenLaunchExe))
			{
				CommandUtils.RunUBT(CommandUtils.CmdEnv, Unreal.UnrealBuildToolDllPath, CommandUtils.UBTCommandline(null, "ZenLaunch", HostPlatform.Current.HostEditorPlatform, UnrealTargetConfiguration.Development));
			}

			if (!FileReference.Exists(ZenLaunchExe))
			{
				throw new AutomationException("Failed to find ZenLaunch executable");
			}

			StringBuilder ZenLaunchCommandline = new StringBuilder();
			ZenLaunchCommandline.AppendFormat("{0} -SponsorProcessID={1}", CommandUtils.MakePathSafeToUseWithCommandLine(Params.RawProjectPath.FullName), Environment.ProcessId);
			CommandUtils.Run(ZenLaunchExe.FullName, ZenLaunchCommandline.ToString());

			ConfigHierarchy HostPlatformConfig = Params.EngineConfigs[HostPlatform.Current.HostEditorPlatform];
			if (!HostPlatformConfig.TryGetValueGeneric("StorageServers", "Cloud", out CloudConfiguration cloudConfig))
			{
				throw new AutomationException("Failed to read config values for storage servers");
			}

			UnrealCloudDDCBuildIndex BuildIndex = new UnrealCloudDDCBuildIndex(cloudConfig.Host.Split(";")[0], cloudConfig.BuildsNamespace, cloudConfig.OAuthProviderIdentifier, (string app, string commandLine) => CommandUtils.Run(app, commandLine));

			DirectoryReference PlatformCookedDirectory;
			if (String.IsNullOrEmpty(Params.CookOutputDir))
			{
				PlatformCookedDirectory = new DirectoryReference(Path.Combine(Params.RawProjectPath.Directory.FullName, "Saved", "Cooked", "{Platform}"));
			}
			else
			{
				PlatformCookedDirectory = new DirectoryReference(Params.CookOutputDir);
			}

			List<string> Platforms = PlatformsToCook.Split("+").ToList();
			...
			foreach (string Platform in Platforms)
			{
				...
				{
					EpicGames.ProjectStore.Build Snapshot = BuildIndex.GetBuild(Branch, Params.ShortProjectName, Platform, Runtime, Changelist, true);
					Snapshot.Import(Params.RawProjectPath, PlatformCookedDirectory, false, false);
				}
			}
		}
