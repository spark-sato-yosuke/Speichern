	...
	public partial class Project : CommandUtils
	{
		...
		public struct OrderFile : IComparable<OrderFile>
		{
			...
			public OrderFile(FileReference FileRef, OrderFileType InType, int SpecIndex, int InPriority)
			{
				...
				{
					index++;
					string Suffix = FileName.Substring(index, FileName.Length - index);
		...
		/// <summary>
		/// returns a hash set containing file extensions that will not be encrypted within pak files
		/// or null if no settings could be found.
		/// </summary>
		private static HashSet<string> GetFileExtensionsToSkipEncryption(ConfigHierarchy PlatformGameConfig)
		{
			if (PlatformGameConfig != null)
			{
				List<string> FileExtensionsToSkipEncryptionList = null;
				PlatformGameConfig.GetArray("/Script/UnrealEd.ProjectPackagingSettings", "FileExtensionsToSkipEncryptionInPak", out FileExtensionsToSkipEncryptionList);
				HashSet<string> FileExtensionsToSkipEncryption = (FileExtensionsToSkipEncryptionList is not null) ? new HashSet<string>(FileExtensionsToSkipEncryptionList) : null;
				return FileExtensionsToSkipEncryption;
			}
		...
		private static void WritePakResponseFile(string Filename, Dictionary<string, string> ResponseFile, bool Compressed, bool RehydrateAssets, EncryptionAndSigning.CryptoSettings CryptoSettings, bool bForceFullEncryption, HashSet<string> FileExtensionsToSkipCompression, HashSet<string> FileExtensionsToSkipEncryption)
		{
			using (var Writer = new StreamWriter(Filename, false, new System.Text.UTF8Encoding(true)))
			{
				foreach (var Entry in ResponseFile)
				{
					...
					bool bSkipEncryption = FileExtensionsToSkipEncryption != null && FileExtensionsToSkipEncryption.Contains(Path.GetExtension(Entry.Key));
					if (CryptoSettings != null && !bSkipEncryption)
					{
		...
		static private string GetPakFileSpecificUnrealPakArguments(Dictionary<string, string> UnrealPakResponseFile, FileReference OutputLocation, string AdditionalOptions, bool Compressed, bool RehydrateAssets,  EncryptionAndSigning.CryptoSettings CryptoSettings, String PatchSourceContentPath, string EncryptionKeyGuid, HashSet<string> FileExtensionsToSkipCompression, HashSet<string> FileExtensionsToSkipEncryption)
		{
			...
			WritePakResponseFile(UnrealPakResponseFileName, UnrealPakResponseFile, Compressed, RehydrateAssets, CryptoSettings, bForceEncryption, FileExtensionsToSkipCompression, FileExtensionsToSkipEncryption);
		...
		static public string GetCreateSinglePakArguments(
				Dictionary<string, string> UnrealPakResponseFile,
				FileReference OutputLocation,
				List<OrderFile> PakOrderFileLocations,
				string AdditionalOptions,
				bool Compressed,
				EncryptionAndSigning.CryptoSettings CryptoSettings,
				FileReference CryptoKeysCacheFilename,
				String PatchSourceContentPath,
				string EncryptionKeyGuid, List<OrderFile> SecondaryPakOrderFileLocations,
				bool bUnattended)
		{
			bool RehydrateAssets = false;
			HashSet<string> FileExtensionsToSkipCompression = null;
			HashSet<string> FileExtensionsToSkipEncryption = null;

			StringBuilder CmdLine = new StringBuilder();
			CmdLine.Append(GetPakFileSpecificUnrealPakArguments(UnrealPakResponseFile, OutputLocation, "", Compressed, RehydrateAssets, CryptoSettings, PatchSourceContentPath, EncryptionKeyGuid, FileExtensionsToSkipCompression, FileExtensionsToSkipEncryption));
			CmdLine.Append(" ");
		...
		static private string GetIoStoreCommandArguments(
			...
			HashSet<string> FileExtensionsToSkipEncryption)
		{
			StringBuilder CmdLine = new StringBuilder();
			...
			WritePakResponseFile(UnrealPakResponseFileName, UnrealPakResponseFile, bCompressed, RehydrateAssets, CryptoSettings, bForceEncryption, FileExtensionsToSkipCompression, FileExtensionsToSkipEncryption);
		...
		static public void RunUnrealPak(FileReference ProjectPath, string Arguments, string Scenario)
		{
			// Invoke UnrealPak with the project argument first on the commandline to make it use project-specific configs.
			// This will also use use the project specific Saved directory instead of the program Saved directory.
			Arguments = MakePathSafeToUseWithCommandLine(ProjectPath.FullName) + " " + Arguments;
			Logger.LogInformation("Running UnrealPak with arguments: {Arguments}", Arguments);
			RunAndLog(CmdEnv, GetUnrealPakLocation().FullName, Arguments, Options: ERunOptions.Default | ERunOptions.UTF8Output);

			// try to copy the default UnrealPak.log file to the UAT log folder,
			// this log file may contain more verbose logging than the one generated by RunAndLog() based on stdout/stderr.
			string UnrealPakLogFileName = "UnrealPak_" + Scenario;
			string SourceLogFile = CombinePaths(DirectoryReference.Combine(ProjectPath.Directory, "Saved", "Logs").FullName, "UnrealPak.log");
			string DestLogFile = LogUtils.GetUniqueLogName(CombinePaths(CommandUtils.CmdEnv.LogFolder, UnrealPakLogFileName));
			if (InternalUtils.SafeFileExists(SourceLogFile, true))
			{
				CopyFile_NoExceptions(SourceLogFile, DestLogFile);
			}
		}
		...
		private static List<ZenCookedFile> ReadZenCookedFilesFromZenServer(string Host, int Port, string ProjectId, string OplogId, ProjectParams Params, DeploymentContext SC, bool bAutoLaunch, string PackageStoreFileArgName, string PackageStoreFileArgValue)
		{
			...
			for (; ; )
			{
				...
				string Uri = string.Format("http://{0}:{1}/prj/{2}/oplog/{3}/entries", Host, Port, ProjectId, OplogId);
				Uri += "?trim_by_referencedset=true";
				...
				catch
				{
					...
					{
						string Arguments = String.Format("IoStore -StartZenServerForStage -SponsorProcessID={0} -{1}={2}", Environment.ProcessId, PackageStoreFileArgName, MakePathSafeToUseWithCommandLine(PackageStoreFileArgValue));
						if (Params.Unattended)
						{
							Arguments += " -unattended";
						}

						RunUnrealPak(Params.RawProjectPath, Arguments, "ZenAutoLaunch");
				...
				if (EntryField["key"].AsString() == "EndCook")
				{
					foreach (CbField PackageDataField in EntryField["files"].AsArray())
					{
						...
						if (RelativeFilename.EndsWith(".bin") || RelativeFilename.EndsWith(".ushaderbytecode") ||
							RelativeFilename.EndsWith(".metallib") || RelativeFilename.EndsWith(".metalmap") ||
			...
			return ZenCookedFiles;
		}

		private static void LoadPackageStoreData(ProjectParams Params, DeploymentContext SC)
		{
			...
			FileReference ProjectStoreFile = FileReference.Combine(SC.PlatformCookDir, "ue.projectstore");
			System.IO.FileInfo ProjectStoreFileInfo = ProjectStoreFile.ToFileInfo();
			if (ProjectStoreFileInfo.Exists)
			{
				SC.PackageStoreData = new PackageStoreData();
				SC.PackageStoreData.MarkerFullPath = ProjectStoreFile.FullName;
				byte[] ProjectStoreBytes = FileReference.ReadAllBytes(ProjectStoreFile);
				...
				ProjectStoreData ParsedProjectStore = JsonSerializer.Deserialize<ProjectStoreData>(ProjectStoreBytes, Options)!;

				if ((ParsedProjectStore != null) && (ParsedProjectStore.ZenServer != null))
				{
			...
			{
				SC.PackageStoreData = new PackageStoreData();
				SC.PackageStoreData.ManifestFullPath = PackageStoreManifestFile.FullName;
				return;
			}
		...
		private static FileReference UpdateProjectStore(ProjectParams Params, DeploymentContext SC, FileReference SourceProjectStorePath)
		{
			if (!EpicGames.Core.JsonObject.TryRead(SourceProjectStorePath, out EpicGames.Core.JsonObject ProjectStore))
			{
				return SourceProjectStorePath;
			}

			FileReference UpdatedProjectStorePath = FileReference.Combine(SC.ProjectRoot, "Intermediate", "Staging", SC.StageTargetPlatform.PlatformType.ToString(),
				"updated_" + SourceProjectStorePath.GetFileName());
			if (FileReference.Exists(UpdatedProjectStorePath) && !InternalUtils.SafeDeleteFile(UpdatedProjectStorePath.FullName))
			{
				return SourceProjectStorePath;
			}

			if (!InternalUtils.SafeCreateDirectory(UpdatedProjectStorePath.Directory.FullName))
			{
				return SourceProjectStorePath;
			}

			if (ZenUtils.ShouldSetupPakStreaming(Params, SC))
			{
				AppendWorkspaceToProjectStore(SC, ProjectStore);
			}
			else if (SC.RemapDirectories.Any())
			{
				MakeProjectStoreWithRemappings(SC, ProjectStore);
			}
			File.WriteAllText(UpdatedProjectStorePath.FullName, ProjectStore.ToJsonString());

			return UpdatedProjectStorePath;
		}

		private static void AppendWorkspaceToProjectStore(DeploymentContext SC, EpicGames.Core.JsonObject ProjectStore)
		{
			if (!ProjectStore.TryGetObjectField("zenserver", out EpicGames.Core.JsonObject ZenServerObject))
			{
				return;
			}

			if (!ZenServerObject.TryGetStringField("HostName", out string HostName) || !ZenServerObject.TryGetIntegerField("HostPort", out int HostPort))
			{
				return;
			}

			ZenRunContext ZenServerContext = ZenRunContext.DiscoverServerContext();
			if (ZenServerContext == null)
			{
				return;
			}

			DirectoryReference StageBaseDir = DirectoryReference.Combine(SC.ProjectRoot, "Saved", "StagedBuilds");
			ZenWorkspace BaseWorkspace = ZenServerContext.FindWorkspaceForDir(StageBaseDir);
			DirectoryReference ShareDir = DirectoryReference.Combine(StageBaseDir, SC.FinalCookPlatform);
			if (BaseWorkspace == null)
			{
				return;
			}

			ZenWorkspaceShare WorkspaceShare = ZenServerContext.FindShareForDir(ShareDir);
			if (WorkspaceShare != null)
			{
				DirectoryReference SharePath = DirectoryReference.Combine(StageBaseDir, WorkspaceShare.share_path);
				ZenServerObject.AddOrSetFieldValue("workspaceid", BaseWorkspace.id);
				ZenServerObject.AddOrSetFieldValue("shareid", WorkspaceShare.id);
				ZenServerObject.AddOrSetFieldValue("workspacesharepath", ShareDir.ToString());
				ZenServerObject.AddOrSetFieldValue("operatingmode", ZenOperatingMode.Filesystem);
			}

			ProjectStore.AddOrSetFieldValue("zenserver", ZenServerObject);
		}

		/// <summary>
		/// Updates project store json data with a new field containing the stage RemapDirectories, allowing StorageServer to apply these when requesting files
		/// </summary>
		private static void MakeProjectStoreWithRemappings(DeploymentContext SC, EpicGames.Core.JsonObject ProjectStore)
		{
			string MakeStorageServerPath(StagedDirectoryReference Source)
			{
				string Result = Source.ToString();
				string[] PathFragments = Result.Split(new[] { '\\', '/' }, StringSplitOptions.RemoveEmptyEntries);
				if (PathFragments.Length > 0)
				{
					if (PathFragments[0].Equals("Engine", StringComparison.InvariantCultureIgnoreCase))
					{
						PathFragments[0] = "{engine}";
					}
					else if (PathFragments[0].Equals(SC.ShortProjectName, StringComparison.InvariantCultureIgnoreCase))
					{
						PathFragments[0] = "{project}";
					}
					Result = '/' + string.Join('/', PathFragments) + '/';
				}
				return Result;
			}

			List<EpicGames.Core.JsonObject> JsonRemapDirectories = [];
			foreach (Tuple<StagedDirectoryReference, StagedDirectoryReference> RemapDirectory in SC.RemapDirectories)
			{
				EpicGames.Core.JsonObject JsonRemapDirectory = new();
				JsonRemapDirectory.AddOrSetFieldValue("from", MakeStorageServerPath(RemapDirectory.Item1));
				JsonRemapDirectory.AddOrSetFieldValue("to", MakeStorageServerPath(RemapDirectory.Item2));
				JsonRemapDirectories.Add(JsonRemapDirectory);
			}
			ProjectStore.AddOrSetFieldValue("remapDirectories", JsonRemapDirectories.ToArray());
		}
		...
		private static async Task<byte[]> ReadChunkId(HttpClient Client, ZenServerStoreData StoreData, IoHash ChunkId)
		{
			using var Request = new HttpRequestMessage(HttpMethod.Get, string.Format("http://{0}:{1}/prj/{2}/oplog/{3}/{4}", StoreData.HostName, StoreData.HostPort, StoreData.ProjectId, StoreData.OplogId, ChunkId));
			Request.Headers.Add("Accept", "application/octet-stream");
			return await Client.Send(Request).Content.ReadAsByteArrayAsync();
		}

		private static FileReference ZenCookedFileToFileReference(ProjectParams Params, DeploymentContext SC, ZenCookedFile CookedFile, HttpClient Client, List<Task> TaskList, HashSet<string> TemporaryNonUFSFiles)
		{
			FileReference CookedFileReference = new FileReference(CookedFile.Filename);

			if (CookedFile.ChunkId != IoHash.Zero)
			{
				StagedFileType? CookedFileStageType = GetStagingTypeForCookedFile(CookedFileReference);
				if (CookedFileStageType.HasValue && (CookedFileStageType.Value == StagedFileType.NonUFS) && Params.UsePak(SC.StageTargetPlatform))
				{
					if (!FileReference.Exists(CookedFileReference))
					{
						Task NewTask = Task.Run( async () =>
							{
								InternalUtils.SafeCreateDirectory(CookedFileReference.Directory.FullName, true);
								InternalUtils.SafeWriteAllBytes(CookedFile.Filename, await ReadChunkId(Client, SC.PackageStoreData.ZenServerStore, CookedFile.ChunkId));
							});
						TaskList.Add(NewTask);
					}
					TemporaryNonUFSFiles.Add(CookedFile.Filename);
				}
			}

			return CookedFileReference;
		}

		private static void SynchronizeTemporaryNonUFSFiles(DeploymentContext SC, HashSet<string> TemporaryNonUFSFiles)
		{
			FileReference TemporaryNonUFSFilesManifest = FileReference.Combine(SC.MetadataDir, "TemporaryNonUFSFiles.txt");
			List<string> OnDiskTemporaryNonUFSFiles = new List<string>();
			if (InternalUtils.SafeFileExists(TemporaryNonUFSFilesManifest.FullName))
			{
				OnDiskTemporaryNonUFSFiles = new List<string>(InternalUtils.SafeReadAllLines(TemporaryNonUFSFilesManifest.FullName));
				InternalUtils.SafeDeleteFile(TemporaryNonUFSFilesManifest.FullName);
			}
			foreach (string OnDiskFilename in OnDiskTemporaryNonUFSFiles)
			{
				if (!TemporaryNonUFSFiles.Contains(OnDiskFilename))
				{
					FileReference OnDiskReference = new FileReference(OnDiskFilename);
					StagedFileReference ExistingStagedFile = new StagedFileReference(OnDiskReference.MakeRelativeTo(SC.PlatformCookDir));
					SC.FilesToStage.NonUFSFiles.Remove(ExistingStagedFile);
					InternalUtils.SafeDeleteFile(OnDiskFilename);
				}
			}
			if (TemporaryNonUFSFiles.Any())
			{
				InternalUtils.SafeCreateDirectory(TemporaryNonUFSFilesManifest.Directory.FullName);
				InternalUtils.SafeWriteAllLines(TemporaryNonUFSFilesManifest.FullName, TemporaryNonUFSFiles.ToArray());
			}
		}

		private static List<FileReference> DiscoverCookedFiles(ProjectParams Params, DeploymentContext SC, bool bAlwaysFullyScanCookDir)
		{
			List<FileReference> CookedFiles = new List<FileReference>();

			// When cooking to Zen get the list of cooked package files
			HashSet<string> TemporaryNonUFSFiles = new HashSet<string>();
			LoadPackageStoreData(Params, SC);
			bool bReadZenCookedFiles = (SC.PackageStoreData != null && SC.PackageStoreData.ZenCookedFiles != null);
			if (bReadZenCookedFiles)
			{
				if (!bAlwaysFullyScanCookDir && DirectoryReference.Exists(SC.PlatformCookDir))
				{
				...
				{
					CookedFiles.Add(ZenCookedFileToFileReference(Params, SC, CookedFile, Client, TaskList, TemporaryNonUFSFiles));
				}
				Task.WaitAll(TaskList.ToArray());
			}
			SynchronizeTemporaryNonUFSFiles(SC, TemporaryNonUFSFiles);

			if (bAlwaysFullyScanCookDir || !bReadZenCookedFiles)
			{
		...
		public static void CreateStagingManifest(ProjectParams Params, DeploymentContext SC)
		{
			...
			if (Params.HasDLCName)
			{
				...
				List<FileReference> CookedFiles = DiscoverCookedFiles(Params, SC, /* bAlwaysFullyScanCookDir */ false);
			...
			else
			{
				...
				if (!Params.CookOnTheFly && !Params.SkipCookOnTheFly) // only stage the UFS files if we are not using cook on the fly
				{
					...
					bool bStageVerseSourceFiles = false;
					if (SC.DedicatedServer)
					{
						...
						// Stage Verse source files?
						PlatformGameConfig.GetBool("/Script/UnrealEd.ProjectPackagingSettings", "bIncludeVerseSourceFilesForServer", out bStageVerseSourceFiles);
					}
					...
					List<KeyValuePair<StagedFileReference, FileReference>> GlobalConfigCache = SC.FilesToStage.UFSFiles.Where(x => x.Value.HasExtension(".ini")).ToList();
					Dictionary<StagedFileReference, List<KeyValuePair<StagedFileReference, FileReference>>> PluginConfigCache = new();
					...
					foreach (KeyValuePair<StagedFileReference, FileReference> StagedPlugin in StagedPlugins)
					{
						...
						bool bTrackConfigs = ShouldCreateBinaryConfig(Params, SC) && !Descriptor.bIsPluginExtension;

						// always insert an entry, even if no configs are found, this tells runtime that there are no 
						// configs for this plugin, and not to look (we still may look on disk for downloaded plugins, etc)
						if (bTrackConfigs)
						{
							PluginConfigCache.Add(StagedPlugin.Key, new List<KeyValuePair<StagedFileReference, FileReference>> ());
						}
						...
						if (DirectoryReference.Exists(PluginConfigDirectory))
						{
							...
							// extensions will be added in the child loop below
							if (bTrackConfigs)
							{
								// add any staged files we just added, to the ini cache (not easy to get the staged files list that we just added)
								IEnumerable<KeyValuePair<StagedFileReference, FileReference>> PluginConfigs = SC.FilesToStage.UFSFiles
									.Where(x => x.Value.HasExtension(".ini") && x.Key.IsUnderDirectory(StagedPlugin.Key.Directory));
								PluginConfigCache[StagedPlugin.Key].AddRange(PluginConfigs);
							}
						}
								...
								foreach (FileReference ChildPlugin in Info.ChildFiles)
								{
									...
									if (ChildPlugin.ContainsName(PlatformExtensionToStage, 0))
									{
										...
										if (DirectoryReference.Exists(PlatformConfigDirectory))
										{
											...
											StagedDirectoryReference StagedExtensionDir = StagedDirectoryReference.Combine(StagedPlugin.Key.Directory, "Config", PlatformExtensionToStage);
											SC.StageFiles(StagedFileType.UFS, PlatformConfigDirectory, "*.ini", StageFilesSearch.AllDirectories, StagedExtensionDir);

											if (bTrackConfigs)
											{
												// add any staged files we just added, to the ini cache (not easy to get the staged files list that we just added)
												IEnumerable<KeyValuePair<StagedFileReference, FileReference>> PluginConfigs = SC.FilesToStage.UFSFiles
													.Where(x => x.Value.HasExtension(".ini") && x.Key.IsUnderDirectory(StagedExtensionDir));
												PluginConfigCache[StagedPlugin.Key].AddRange(PluginConfigs);
											}
										}
									}
						...
						// Stage Verse source files?
						if (bStageVerseSourceFiles)
						{
							DirectoryReference PluginContentDirectory = DirectoryReference.Combine(StagedPlugin.Value.Directory, "Content");
							if (DirectoryReference.Exists(PluginContentDirectory))
							{
								// Include Verse source files from the Content folder
								StagedDirectoryReference StagedContentDirecory = StagedDirectoryReference.Combine(StagedPlugin.Key.Directory, "Content");
								SC.StageFiles(StagedFileType.NonUFS, PluginContentDirectory, "*.verse", StageFilesSearch.AllDirectories, StagedContentDirecory);
								SC.StageFiles(StagedFileType.NonUFS, PluginContentDirectory, "*.vmodule", StageFilesSearch.AllDirectories, StagedContentDirecory);
							}

							// Also include Verse source files of C++ modules
							DirectoryReference PluginSourceDirectory = DirectoryReference.Combine(StagedPlugin.Value.Directory, "Source");
							if (DirectoryReference.Exists(PluginSourceDirectory))
							{
								foreach (FileReference BuildCsFile in DirectoryReference.EnumerateFiles(PluginSourceDirectory, "*.Build.cs", SearchOption.AllDirectories))
								{
									DirectoryReference MaybeVerseDirectory = DirectoryReference.Combine(DirectoryReference.FromFile(BuildCsFile), "Verse");
									if (DirectoryReference.Exists(MaybeVerseDirectory))
									{
										string RelativeVerseDirectory = MaybeVerseDirectory.MakeRelativeTo(StagedPlugin.Value.Directory);
										StagedDirectoryReference StagedVerseDirecory = StagedDirectoryReference.Combine(StagedPlugin.Key.Directory, RelativeVerseDirectory);
										SC.StageFiles(StagedFileType.NonUFS, MaybeVerseDirectory, "*.*", StageFilesSearch.AllDirectories, StagedVerseDirecory);
									}
								}
							}
						}
					...
					if (DirectoryReference.Exists(SC.PlatformCookDir))
					{
						List<FileReference> CookedFiles = DiscoverCookedFiles(Params, SC, /* bAlwaysFullyScanCookDir */ true);
						...
						foreach (FileReference CookedFile in CookedFiles)
						{
							...
							{
								if (!Params.UsePak(SC.StageTargetPlatform) || ZenUtils.ShouldSetupPakStreaming(Params, SC))
								{
									FileReference SrcCookedFile = FileReference.Combine(SC.ProjectRoot, "Intermediate", "Staging", SC.StageTargetPlatform.PlatformType.ToString(), "updated_" + CookedFile.GetFileName());

									SrcCookedFile = UpdateProjectStore(Params, SC, CookedFile);

									SC.StageFile(StagedFileType.NonUFS, SrcCookedFile, new StagedFileReference(CookedFile.MakeRelativeTo(SC.PlatformCookDir)));
								}
								continue;
							}
					...
					if (ShouldCreateBinaryConfig(Params, SC) && !Params.SkipPak)
					{
						List<string> ConfigCacheInputFileContents = new List<string>();

						// get the configs that aren't in a plugin
						var GlobalConfigs = GlobalConfigCache.Where(x => !PluginConfigCache.Any(y => y.Value.Any(z => z.Key == x.Key)));
						ConfigCacheInputFileContents.AddRange(GlobalConfigs.Select(x => $"  {x.Value}|{x.Key}"));

						// get the list of plugins that need to be processed
						foreach (KeyValuePair<StagedFileReference, List<KeyValuePair<StagedFileReference, FileReference>>> ConfigInfo in PluginConfigCache)
						{
							// store the plugin name (not path) and any configs it may have had staged
							ConfigCacheInputFileContents.Add(Path.GetFileNameWithoutExtension(ConfigInfo.Key.Name));
							ConfigCacheInputFileContents.AddRange(ConfigInfo.Value.Select(x => $"  {x.Value}|{x.Key}"));
						}

						FileReference ConfigCacheInputFile = FileReference.Combine(SC.ProjectRoot, "Intermediate", "Config", "ConfigCacheInput.txt");
						DirectoryReference.CreateDirectory(ConfigCacheInputFile.Directory);
						File.WriteAllLines(ConfigCacheInputFile.FullName, ConfigCacheInputFileContents.ToArray());
						...
						String UnrealPakParams = String.Format("MakeBinaryConfig -Project=\"{0}\" -Platform={1} -OutputFile=\"{2}\" -StagedPluginsFile=\"{3}\"", SC.RawProjectPath, ConfigHierarchy.GetIniPlatformName(ThisPlatform.IniPlatformType), OutputFile.FullName, ConfigCacheInputFile.FullName);

						RunUnrealPak(Params.RawProjectPath, UnrealPakParams, "MakeBinaryConfig");
			...
			if (bCreatePluginManifest && Params.UsePak(SC.StageTargetPlatform))
			{
				...
				if (Params.HasDLCName)
				{
					CreatePluginManifest(SC, Params.DLCPakPluginFile ? SC.FilesToStage.UFSFiles : SC.FilesToStage.NonUFSFiles, Params.DLCPakPluginFile ? StagedFileType.UFS : StagedFileType.NonUFS, Params.DLCFile.GetFileNameWithoutAnyExtensions());
				}
				else
				{
		...
		private static List<PakFileRules> GetPakFileRules(ProjectParams Params, DeploymentContext SC)
		{
			...
			foreach (string SectionName in PakRulesConfig.SectionNames)
			{
				...
				if (PakRulesConfig.TryGetValues(SectionName, "Files", out FilesEnumberable))
				{
					...
					Logger.LogDebug("Using pakFileRule {0} number of overriden files {1}", PakRules.Name, FilesEnumberable.Count);
				...
				{
					Logger.LogDebug("No files found for pakfilerule {0}", PakRules.Name);
				}
			}
		...
		private static void CreatePakUsingStagingManifest(ProjectParams Params, DeploymentContext SC)
		{
			...
			EncryptionAndSigning.CryptoSettings PakCryptoSettings = Params.DLCOverrideCryptoSettings;
			if (PakCryptoSettings == null)
			{
			 	PakCryptoSettings = EncryptionAndSigning.ParseCryptoSettings(DirectoryReference.FromFile(Params.RawProjectPath), SC.StageTargetPlatform.IniPlatformType, Log.Logger);
			}

			List<CreatePakParams> PakInputs = new List<CreatePakParams>();
			PakInputs.Add(new CreatePakParams(SC.ShortProjectName, UnrealPakResponseFile, Params.Compressed, Params.RehydrateAssets, Params.DLCOverrideEncryptionKeyGuid));
			CreatePaks(Params, SC, PakInputs, PakCryptoSettings);
		}

		private static void PrepareZenWorkspaceEnv(ProjectParams Params, DeploymentContext SC)
		{
			if (!ZenUtils.ShouldSetupPakStreaming(Params, SC))
			{
				return;
			...
			Logger.LogInformation("Preparing zen workspace environment.");

			ZenUtils.ZenLaunch(SC.RawProjectPath);
			...
			FileReference ZenMarkerFile = FileReference.Combine(SC.PlatformCookDir, "ue.projectstore");
			System.IO.FileInfo ProjectStoreFileInfo = ZenMarkerFile.ToFileInfo();
			if (!ProjectStoreFileInfo.Exists)
			{
				return;
			}

			byte[] MarkerFileData = FileReference.ReadAllBytes(ZenMarkerFile);
			JsonSerializerOptions JsonOptions = new();
			JsonOptions.AllowTrailingCommas = true;
			JsonOptions.ReadCommentHandling = JsonCommentHandling.Skip;
			JsonOptions.PropertyNameCaseInsensitive = true;
			JsonOptions.Converters.Add(new JsonStringEnumConverter());
			ProjectStoreData ExtractedProjectStore = JsonSerializer.Deserialize<ProjectStoreData>(MarkerFileData, JsonOptions);

			if (ExtractedProjectStore != null && ExtractedProjectStore.ZenServer != null)
			{
				ExtractedProjectStore.ZenServer.OperatingMode = ZenOperatingMode.Stream;

				if (ShouldCreatePak(Params))
				{
					ZenWorkspaceShare WorkspaceShare = ZenServerContext.CreateWorkspaceAndShare(SC.StageDirectory);
				}
			}
		}
		...
		private static void CreatePakForCrashReporter(ProjectParams Params, DeploymentContext SC)
		{
			...
			if (PakResponseFile.Count < 1)
			{
				return;
			}
			string Arguments = GetCreateSinglePakArguments(PakResponseFile, OutputLocation, null, null, Params.Compressed, null, null, null, null, null, Params.Unattended);
			RunUnrealPak(Params.RawProjectPath, Arguments, "CrashReportClient");
		}
		...
		/// <param name="CryptoSettings"></param>
		private static void CreatePaks(ProjectParams Params, DeploymentContext SC, List<CreatePakParams> PakParamsList, EncryptionAndSigning.CryptoSettings CryptoSettings)
		{
			...
			bool bHasPlatformEngineConfig = Params.EngineConfigs.TryGetValue(SC.StageTargetPlatform.PlatformType, out PlatformEngineConfig);
			...
			HashSet<string> FileExtensionsToSkipEncryptionInPaks = GetFileExtensionsToSkipEncryption(PlatformGameConfig);
			...
			// Only consider enabling the OnDemand override if we are NOT forcing uncompressed data but compression has not been
			// enabled by the normal means.

			string OnDemandCompressionFormat;
			PlatformEngineConfig.GetString("Ias", "CompressionFormatOverride", out OnDemandCompressionFormat);
			
			if (!Params.ForceUncompressed && !Params.Compressed && !string.IsNullOrWhiteSpace(OnDemandCompressionFormat))
			{
				bool bHasOnDemandContainers = false;
				foreach (CreatePakParams PakParams in PakParamsList)
				{
					if (PakParams.bOnDemand)
					{
						PakParams.bCompressed = true;
						bHasOnDemandContainers = true;
					}
				}

				if (bHasOnDemandContainers)
				{
					bAnyCompressed = true;
					CompressionFormats = OnDemandCompressionFormat;
					Logger.LogInformation("Overriding to CompressionFormatOverride = {IasCompressionFormat}", OnDemandCompressionFormat);
					// Note that the compression method will still be set by ini:Game:[/Script/UnrealEd.ProjectPackagingSettings]:PackageCompressionMethod
				}
			}
			...
			if (PlatformGameConfig.TryGetValues("/Script/UnrealEd.ProjectPackagingSettings", "PakOrderFileSpecs", out IReadOnlyList<string> OrderFileConfigValues))
			{
				foreach (string Config in OrderFileConfigValues)
				{
					...
					OrderFileSpecs.Add(new OrderFileSpec { FileNamePattern = FilePattern, OrderType = OrderFile.OrderFileType.Custom, Required = false, Priority = Priority });
				}
			}
					...
					if (!bCopiedExistingPak)
					{
						...
						if (bShouldCreateIoStoreContainerFiles)
						{
							...
							IoStoreCommands.Add(GetIoStoreCommandArguments(
								...
								FileExtensionsToSkipEncryptionInPaks));
						}
			...
			// save out the crypto settings to the Metadata folder; UnrealPak/IasTool uses it immediately, but
			// there seem to be other things in the build that potentially rely on a copy living there
			FileReference CryptoKeysCacheFilename = FileReference.Combine(SC.MetadataDir, "Crypto.json");
			CryptoSettings.Save(CryptoKeysCacheFilename);
			...
			if (IoStoreCommands.Count > 0)
			{
				...
				if (PlatformGameConfig.GetString("/Script/UnrealEd.ProjectPackagingSettings", "FallbackOrdering", out string FallbackOrdering))
				{
					AdditionalArgs += " -FallbackOrdering="+FallbackOrdering;
				}

				bool bAlphaSortClusterPackageLists = false;
				if (PlatformGameConfig.GetBool("/Script/UnrealEd.ProjectPackagingSettings", "bAlphaSortClusterPackageLists", out bAlphaSortClusterPackageLists) && bAlphaSortClusterPackageLists )
				{	
					AdditionalArgs += " -AlphaSortClusterPackageLists";
				}
			...
			// When the ApplyIoStoreOnDemand flag is present then we need to make sure that ondemand data is uploaded.
			// If no upload target has been specified we will default to using the local zen server which is the equivalent
			// of providing '-Upload=LocalZen' as a commandline arg.
			if (Params.ApplyIoStoreOnDemand && string.IsNullOrWhiteSpace(Params.Upload))
			{
				Params.Upload = "localzen";
			}
			...
			if (!string.IsNullOrWhiteSpace(Params.Upload))
			{
				...
				if (Params.Upload.ToLower() == "localzen")
				{
					...
					if (CryptoKeysCacheFilename != null)
					{
						UploadArgs.Append(string.Format(" -CryptoKeys=\"{0}\"", CryptoKeysCacheFilename.FullName));
					}
				}
				else
				{
		...
		private static void RunIoStore(ProjectParams Params, DeploymentContext SC, string CommandsFileName, List<OrderFile> OrderFiles, string AdditionalArgs)
		{
			...
			{
				CommandletParams += String.Format(" -ProjectStore={0}",
					MakePathSafeToUseWithCommandLine(SC.PackageStoreData.MarkerFullPath));
			}

			CommandletParams += String.Format(" -CookedDirectory={0} -Commands={1}",
				MakePathSafeToUseWithCommandLine(SC.PlatformCookDir.ToString()),
				MakePathSafeToUseWithCommandLine(CommandsFileName));

			if (SC.PackageStoreData.ZenCookedFiles == null)
			{
			...
			RunUnrealPak(Params.RawProjectPath, CommandletParams, "CreateIoStoreContainers");
		}
		...
		private static void CreatePaksUsingChunkManifests(ProjectParams Params, DeploymentContext SC)
		{
...

			{
				...
				var RuleNameToFilenames = new Dictionary<string, List<string>>();
				...
				Parallel.ForEach(StagingManifestResponseFile, StagingFile =>
				{
					...
					// Save PAK rules and filenames that might break references
					if (MatchingRule.HasValue && MatchingRule.Value.bOverrideChunkManifest)
					{
						lock (SyncObject)
						{
							List<String> Filenames;
							if (!RuleNameToFilenames.TryGetValue(MatchingRule.Value.Name, out Filenames))
							{
								Filenames = new List<String>();
								RuleNameToFilenames.Add(MatchingRule.Value.Name, Filenames);
							}
				...
				// Save a debug file with Pak rules and filenames that might brake references 
				if (RuleNameToFilenames.Count > 0)
				{
					foreach (KeyValuePair<String, List<String>> KeyValue in RuleNameToFilenames)
					{
						List<String> Filenames = KeyValue.Value;
						Logger.LogInformation("Pak rule '{Arg0}' with {Arg1} matching files, overrides chunk manifest and might cause broken references", KeyValue.Key, Filenames.Count);
					}

					String ChunkManifestOverrideDirectoryPath = Path.Combine(CmdEnv.EngineSavedFolder, "Logs");
					String FilePath = Path.Combine(ChunkManifestOverrideDirectoryPath, "ChunkManifestOverride_DebugFiles.txt");
					Logger.LogInformation("Saving chunk manifest override debug information to '{Arg0}'", FilePath);

					InternalUtils.SafeCreateDirectory(ChunkManifestOverrideDirectoryPath);
					using (var Writer = new StreamWriter(FilePath, false, new System.Text.UTF8Encoding(true)))
					{
						foreach (KeyValuePair<String, List<String>> KeyValue in RuleNameToFilenames)
						{							
							PakFileRules Rule = PakRulesList.First(PakFileRules => PakFileRules.Name == KeyValue.Key);
							List<String> Filenames = KeyValue.Value;
							Writer.WriteLine(String.Format("[{0}]", Rule.Name));
							String Overrides = Rule.OverridePaks != null ? string.Join(",", Rule.OverridePaks) : string.Empty;
							Writer.WriteLine(String.Format("OverridePaks: {0}", Overrides));
							Writer.WriteLine(String.Format("ExcludeFromPaks: {0}", Rule.bExcludeFromPaks ? "True" : "False"));
		...
		private static bool ShouldCreateBinaryConfig(ProjectParams Params, DeploymentContext SC)
		{
			...
			// check the project settings if not set
			ConfigHierarchy PlatformGameConfig = ConfigCache.ReadHierarchy(ConfigHierarchyType.Game, DirectoryReference.FromFile(Params.RawProjectPath), SC.StageTargetPlatform.IniPlatformType, SC.CustomConfig);
			bool bMakeBinaryConfig;
			PlatformGameConfig.GetBool("/Script/UnrealEd.ProjectPackagingSettings", "bMakeBinaryConfig", out bMakeBinaryConfig);
		...
		{
			static IEnumerable<Tuple<string, UnrealTargetConfiguration, UnrealArchitectures>> ExpandDeploymentArchitectures(string Target, UnrealTargetConfiguration Configuration, UnrealArchitectures Architectures, UnrealTargetPlatform Platform )
			{
				List<Tuple<string, UnrealTargetConfiguration, UnrealArchitectures>> Result = new();

				UnrealArchitectureConfig ArchConfig = UnrealArchitectureConfig.ForPlatform(Platform);
				if (ArchConfig.Mode == UnrealArchitectureMode.OneTargetPerArchitecture && Architectures != null && Architectures.bIsMultiArch)
				{
					foreach (UnrealArch Architecture in Architectures.Architectures)
					{
						Result.Add( new(Target, Configuration, new(Architecture)));
					}
				}
				else
				{
					Result.Add( new(Target, Configuration, Architectures));
				}
			...
			foreach (var StagePlatform in PlatformsToStage)
			{
				...
				foreach (var Target in ListToProcess)
				{
					...
					{
						TargetAndConfigPairs.AddRange( ExpandDeploymentArchitectures(Target, UnrealTargetConfiguration.Development, Params.EditorArchitecture, StagePlatform.Type ));
					}
					else if (Params.HasProgramTargets && Params.ProgramTargets.Contains(Target))
					{
						TargetAndConfigPairs.AddRange( ExpandDeploymentArchitectures(Target, UnrealTargetConfiguration.Development, Params.ProgramArchitecture, StagePlatform.Type ));
					}
					else
					{
						...
						{
							TargetAndConfigPairs.AddRange( ExpandDeploymentArchitectures(Target, Config, Arches, StagePlatform.Type ));
						}
					}
				}
				...
				foreach (var TargetAndConfig in TargetAndConfigPairs)
				{
					...
					if (Config != UnrealTargetConfiguration.Development)
					{
						...
						// We need to account for the decorated separator, rather than hardcoding the full file pattern
						foreach (var ProjectTarget in Params.ProjectTargets)
						{
							if (Target == ProjectTarget.TargetName && Params.UseMergedModules)
							{
								TargetRules ValidatedRules = ProjectTarget.Rules;
								TargetRules.ValidateMergedTarget(ValidatedRules, StagePlatform.Type);
								Separator = ValidatedRules.DecoratedSeparator;
								break;
							}
						}
						Exe = Target + Separator + PlatformName + Separator + Config.ToString();
					}
		...
		private static void SetupCustomStageCopyHandler(ProjectParams Params, DeploymentContext SC)
		{
			...
			ConfigHierarchy GameIni = ConfigCache.ReadHierarchy(ConfigHierarchyType.Game, Params.RawProjectPath.Directory, SC.StageTargetPlatform.PlatformType, SC.CustomConfig, Params.AdditionalPakOptions.Split());
			...
			if (ShouldCreatePak(Params) || (Params.Stage && !Params.SkipStage))
			{
				...
				if (!Params.NoClient)
				{
					...
					foreach (var SC in DeployContextList)
					{
						...
						PrepareZenWorkspaceEnv(Params, SC);
