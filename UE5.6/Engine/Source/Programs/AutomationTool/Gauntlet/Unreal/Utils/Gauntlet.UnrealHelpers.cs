...
namespace Gauntlet
{
    ...
    public static class UnrealRoleTypeExtensions
	{
		...
		public static bool IsHost(this UnrealTargetRole Type)
		{
			return Type == UnrealTargetRole.Host;
		}
	...
	public class EpicRoot
	{
		...
		public EpicRoot(string InPath)
		{
			...
			if (BuildHostPlatform.Current.Platform == UnrealTargetPlatform.Mac || BuildHostPlatform.Current.Platform == UnrealTargetPlatform.Linux)
			{
				...
				{
					string PrimaryRoot = Path.Combine(OSMountBase, "epicgames.net", "root");
					string SecondaryRoot = OSMountBase;
					PosixMountPath = Directory.Exists(PrimaryRoot) ? PrimaryRoot : SecondaryRoot;
				}
				else
				{
					PosixMountPath = Path.Combine(OSMountBase, "root");
				}
