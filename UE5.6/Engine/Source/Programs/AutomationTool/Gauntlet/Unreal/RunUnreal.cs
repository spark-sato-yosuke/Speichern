...
namespace Gauntlet
{
	...
	[ParamHelp("ExecCmds", "List commands to execute. NOTE: To provide ExecCmds to specific roles, use -EditorExecCmds, -ClientExecCmds, etc.", MultiSelectSeparator = "+")]
	...
	[ParamHelp("Devices", "List of devices to use for tests using the syntax: <platform>:<address>,<platform>:<address>,...", Action = ParamHelpAttribute.ParamAction.Append)]
	[ParamHelp("DeviceName", "Device name to constrain reservation", Action = ParamHelpAttribute.ParamAction.Append)]
	[ParamHelp("PrefSpec", "Device perfomance specification to constrain reservation (Minimum,Recommended,High)", Action = ParamHelpAttribute.ParamAction.Append)]
	[ParamHelp("PrefModel", "Device model to constrain reservation", Action = ParamHelpAttribute.ParamAction.Append)]
	...
	[ParamHelp("Args", "Extra arguments to pass to all role(s)")]
	...
	[ParamHelp("CookedEditorArgs", "Extra arguments passed to cooked editor role(s)")]
	[ParamHelp("EditorGameArgs", "Extra arguments passed to editor(s) when acting in the game role")]
	[ParamHelp("EditorServerArgs", "Extra arguments passed to editor(s) when acting in the server role")]
	...
	public class RunUnreal : BuildCommand
	{
		...
		public virtual ExitCode RunTests(UnrealTestOptions ContextOptions)
		{
			...
			foreach (ArgumentWithParams PlatformWithParams in ContextOptions.PlatformList)
			{
				...
				foreach (UnrealTargetRole Type in Enum.GetValues(typeof(UnrealTargetRole)))
				{
					...
					bool bPlatformOverride = ContextOptions.RolePlatformOverrides.ContainsKey(Type);
					if (bPlatformOverride)
					{
						Role.Platform = ContextOptions.RolePlatformOverrides[Type];
					}
		...
		IEnumerable<ITestNode> CreateTestList(UnrealTestContext Context, Params DefaultParams, ArgumentWithParams PlatformParams = null)
		{
			...
			foreach (var Test in Context.Options.TestList)
			{
				...
				List<string> DeviceNameArgs = Globals.Params.ParseValues("DeviceName", false);
