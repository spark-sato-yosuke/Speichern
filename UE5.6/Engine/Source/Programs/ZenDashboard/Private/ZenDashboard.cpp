...
#endif
{
	...
	TSharedPtr<SZenWorkspaces> ZenWorkspacesWidget;
	...
	TSharedRef<SWidget> FillToolsMenu()
	{
				...
				FOnGetContent::CreateLambda([this]() -> TSharedRef<SWidget>
					{
						...
						SubMenuBuilder.AddMenuEntry(
							LOCTEXT("ResetStats_ZenServer", "Reset stat counters"),
							LOCTEXT("ResetStats_ZenServer_ToolTip", "Reset the cache stats"),
							FSlateIcon(),
							FUIAction(
								FExecuteAction::CreateRaw(this, &FZenDashboardApp::DeleteDataAndRestartZenServer),
								FCanExecuteAction::CreateRaw(this, &FZenDashboardApp::CanExecuteExclusiveAction)
		...
		MenuBuilder.BeginSection(NAME_None, LOCTEXT("Workspace", "Workspace"));
		{
			MenuBuilder.AddMenuEntry(
				LOCTEXT("LaunchUtilityCommandPrompt", "Manage workspaces"),
				LOCTEXT("LaunchUtilityCommandPrompt_ToolTip", "Manage Zen workspaces: list, create, delete"),
				FSlateIcon(),
				FUIAction(
					FExecuteAction::CreateRaw(this, &FZenDashboardApp::LaunchUtilityCommandPrompt),
					FCanExecuteAction()
...
public:
	...
	void Run()
	{
		...
		TSharedPtr<SWidget> WindowContent = CreateWindowContent();
...
private:
	...
	TSharedPtr<SWidget> CreateWindowContent()
	{
		TSharedPtr<SWidget> WindowContent = 
			SNew(SBorder)
				...
				[
					...
					+ SVerticalBox::Slot()
					.MaxHeight(300)
					.Padding(4.f, 10.f, 4.f, 0.f)
					.FillHeight(1.0f)
					.VAlign(VAlign_Top)
					[
						SAssignNew(ZenWorkspacesWidget, SZenWorkspaces)
						.ZenServiceInstance(ServiceInstanceManager.ToSharedRef(), &UE::Zen::FServiceInstanceManager::GetZenServiceInstance)
						.UpdateFrequency(5.f)
						.Visibility_Lambda([this]() -> EVisibility
						{
							...
							bool bShow = false;
							if (ServiceInstanceManager->GetZenServiceInstance().IsValid()
								&& ServiceInstanceManager->GetZenServiceInstance()->IsServiceRunning())
							{
								ZenWorkspacesWidget->UpdateWorkspaces(DeltaTime);
								bShow = ZenWorkspacesWidget->GetWorkspaceCount() > 0;
							}
