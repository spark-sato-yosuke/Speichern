...
TSharedRef<SDockTab> FOutputLogModule::SpawnOutputLogTab(const FSpawnTabArgs& Args)
{
	...
	NewLog->UpdateOutputLogFilter(GetDefault<UOutputLogSettings>()->OutputLogTabFilter);
...
void FOutputLogModule::FocusOutputLogAndScrollToEnd()
{
	FocusOutputLog();

	if (TSharedPtr<SOutputLog> Log = OutputLog.Pin())
	{
		Log->OnConsoleCommandExecuted();
	}

	if (TSharedPtr<SOutputLog> Log = OutputLogDrawer.Pin())
	{
		Log->OnConsoleCommandExecuted();
	}
}
...
void FOutputLogModule::UpdateOutputLogFilter(const TArray<FName>& CategoriesToShow, const FOutputFilterParams& InParams)
{
	...
	{
		OutputLogFilterCache->ErrorsFilter = InParams.bShowErrors.GetValue() ? ELogLevelFilter::Enabled : ELogLevelFilter::None;
	}
	...
	{
		OutputLogFilterCache->WarningsFilter = InParams.bShowWarnings.GetValue() ? ELogLevelFilter::Enabled : ELogLevelFilter::None;
	}
	if (InParams.bShowLogs.IsSet())
	{
		OutputLogFilterCache->MessagesFilter = InParams.bShowLogs.GetValue() ? ELogLevelFilter::Enabled : ELogLevelFilter::None;
	}
	if (InParams.IgnoreFilterVerbosities.IsSet())
	{
		// Redirect Log/Warning/Error versions to the enum variants.
		// Only using the set for compatability
		OutputLogFilterCache->IgnoreFilterVerbosities.Reset();
		for (ELogVerbosity::Type Verbosity : InParams.IgnoreFilterVerbosities.GetValue())
		{
			switch (Verbosity)
			{
			case ELogVerbosity::Log:
				OutputLogFilterCache->MessagesFilter = ELogLevelFilter::All;
				break;
