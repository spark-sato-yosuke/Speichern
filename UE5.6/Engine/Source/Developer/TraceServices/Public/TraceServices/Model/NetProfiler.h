...
// Mirror from FNetDebugNameId on the runtime side.
typedef uint32 FNetProfilerNameIndexType;

struct FNetProfilerName
{
	const TCHAR* Name;	// Name
	FNetProfilerNameIndexType NameIndex;	// Index in the type array?
};
...
struct FNetProfilerEventType
{
	...
	FNetProfilerNameIndexType NameIndex;	
	uint16 Level;
};
...
struct FNetProfilerObjectInstance
{
	...
	FNetProfilerNameIndexType NameIndex = 0U;	// Index in the Name array
};
...
struct FNetProfilerContentEvent
{
	...
	FNetProfilerNameIndexType NameIndex;	// Identify the name / type, should we store the actual Name as well
