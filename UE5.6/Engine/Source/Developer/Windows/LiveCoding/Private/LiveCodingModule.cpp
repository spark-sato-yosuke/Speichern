...
#if IS_MONOLITHIC
	FString DefaultEngineDir = BuildSettings::GetLiveCodingEngineDir();
#else
...
#if IS_MONOLITHIC
	const TCHAR* LiveCodingProject = BuildSettings::GetLiveCodingProject();
	FString SourceProject = (LiveCodingProject != nullptr)? LiveCodingProject : TEXT("");
#else
	...
	if ((!GIsCompileActive || GTriggerReload) && Reload.IsValid())
	{
		if (GHasLoadedPatch)
		{
			...
			if (Reload->GetEnableReinstancing(false))
			{
				...
				static UObject* DummyDefaultObject = UObject::StaticClass()->GetDefaultObject(false);
				for (TObjectIterator<UClass> It; It; ++It)
				{
					...
					{
						Class->SetDefaultObject(DummyDefaultObject);
					}
				}
			}
			...
			FDelayedAutoRegisterHelper::RunAndClearDelayedAutoRegisterDelegates(EDelayedRegisterRunPhase::LiveCodingReload);
...
bool FLiveCodingModule::StartLiveCoding(ELiveCodingStartupMode StartupMode)
{
	...
	if (!HasStarted())
	{
		...
		GlobalMutex = CreateMutex(NULL, FALSE, *MutexName);
		if (!GlobalMutex)
		{
			State = EState::NotRunning;
			return false;
		...
		if (*VfsPaths)
		{
			// We have to send each entry separately because LC settings strings must be < 256 bytes
			TStringBuilder<256> VfsEntry;

			if (SourceProject.Len() > 0)
			{
				const TCHAR* FirstSemi = FCString::Strchr(VfsPaths, ';');
				FString VfsRoot = FPaths::GetPath(FString(MakeStringView(VfsPaths, FirstSemi - VfsPaths)));
				VfsEntry.Append(VfsRoot).AppendChar('/').Append(FPaths::GetBaseFilename(SourceProject)).AppendChar(';').Append(FPaths::GetPath(SourceProject));
#if PLATFORM_WINDOWS
				VfsEntry.ToString(); // Ensure null terminated
				for (TCHAR* It=VfsEntry.GetData(); *It; ++It)
					if (*It == '/')
						*It = '\\';
#endif
				LppApplySettingString("Vfs", *VfsEntry);
				VfsEntry.Reset();
			}

			const TCHAR* LastSemi = VfsPaths;
			while (true)
			{
				const TCHAR* FirstSemi = FCString::Strchr(LastSemi, ';');
				if (!FirstSemi)
					break;
				const TCHAR* SecondSemi = FCString::Strchr(FirstSemi + 1, ';');
				if (!FirstSemi)
					break;
				VfsEntry.Append(LastSemi, SecondSemi - LastSemi);
				LppApplySettingString("Vfs", *VfsEntry);
				VfsEntry.Reset();
				LastSemi = SecondSemi + 1;
			}
		}
...
void FLiveCodingModule::StopLiveCoding()
{
	if (CallbackCookie)
	{
		LppShutdown();
