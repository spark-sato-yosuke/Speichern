...
void SSettingsEditor::HandleSettingsContainerCategoryModified( const FName& CategoryName )
{
	if ( !bIsActiveTimerRegistered )
	{
		...
		FTSTicker::GetCoreTicker().AddTicker(FTickerDelegate::CreateSP(this, &SSettingsEditor::UpdateCategoriesCallback));
	}
}

bool SSettingsEditor::UpdateCategoriesCallback(float InDeltaTime)
{
	bIsActiveTimerRegistered = false;
