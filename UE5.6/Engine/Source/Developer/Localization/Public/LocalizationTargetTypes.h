...
struct FLocalizationGatherPathRootUtil
{
	...
	static UE_API FString GetResolvedPathRootToken(const ELocalizationGatherPathRoot InPathRoot);
	...
	static UE_API FString GetResolvedPathRoot(const ELocalizationGatherPathRoot InPathRoot, const bool bIsEngineTarget);

	/** Get the display name of the resolved path root (either "Engine/" or "{ProjectName/" depending on the setting and target type */
	static UE_API FText GetResolvedPathRootDisplayName(const ELocalizationGatherPathRoot InPathRoot, const bool bIsEngineTarget);
};
...
struct FGatherTextFromPackagesConfiguration
{
	...
	/*
	 * Worlds (including their associated external actors) in these collections may be processed for gathering. The collections may contain more than just worlds, and will be filtered internally.
	 ...
	 */
	UPROPERTY(config, EditAnywhere, Category = "Filter", AdvancedDisplay)
	TArray<FName> WorldCollections;
...
struct FLocalizationCompilationSettings
{
	...
	/* Should we validate translated text containing rich text tags to see if the translated text have broken rich text tags. (i.e. "Example of <b>broken tag</") */
	UPROPERTY(config, EditAnywhere, Category = "Validation")
	bool ValidateRichTextTags;
};
...
UCLASS(MinimalAPI, Within=LocalizationTargetSet)
...
public:
	...
	UE_API bool IsMemberOfEngineTargetSet() const;
	UE_API bool UpdateWordCountsFromCSV();
	UE_API void UpdateStatusFromConflictReport();
	UE_API bool RenameTargetAndFiles(const FString& NewName);
	UE_API bool DeleteFiles(const FString* const Culture = nullptr) const;
};

UCLASS(MinimalAPI, Within=LocalizationSettings)
