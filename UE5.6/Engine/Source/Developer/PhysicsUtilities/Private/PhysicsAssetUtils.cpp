...
void RemoveBodyFromCollisionTableInternal(UPhysicsAsset* const PhysAsset, const int32 BodyIndex, const bool bDecrementFollowingIndices)
{
	check(PhysAsset);
			...
			for (int32 j = 0; j < i; j++)
			{
				...
				if (PhysAsset->CollisionDisableTable.Find(Key))
				{
					if (i != BodyIndex && j != BodyIndex)
					{
						...
						if (bDecrementFollowingIndices)
						{
							// Decrement index of all bodies with index greater than the supplied BodyIndex. This is useful when the BodyIndex is going to be removed from the physics asset.
							if (NewI > BodyIndex) { --NewI; }
							if (NewJ > BodyIndex) { --NewJ; }
						}
...
void RemoveBodyConstraintsInternal(UPhysicsAsset* const PhysAsset, const int32 BodyIndex)
{
...
void CreateNewBodyAtIndexInternal(UPhysicsAsset* const PhysAsset, const FName InBodyName, const FPhysAssetCreateParams& Params, const int32 BodyIndex)
{
	...
	PhysAsset->SkeletalBodySetups[BodyIndex] = NewBodySetup;
	...
	if (Params.bDisableCollisionsByDefault)
	{
		...
		{
			PhysAsset->DisableCollision(i, BodyIndex);
		}
	}
}
...
int32 GenerateSkinnedTriangleMesh(UPhysicsAsset* PhysicsAsset, USkeletalMesh* SkeletalMesh, int32 LODIndex, const FPhysAssetCreateParams& Params, FText& OutErrorMessage, bool bShowProgress)
{
	if (!ensure(PhysicsAsset))
	{
		OutErrorMessage = FText(NSLOCTEXT("GenerateSkinnedTriangleMesh", "MissingPAFailure", "Failed to generate the skinned triangle mesh: missing input PhysicsAsset"));
		return INDEX_NONE;
	}
	if (!ensure(SkeletalMesh))
	{
		OutErrorMessage = FText(NSLOCTEXT("GenerateSkinnedTriangleMesh", "MissingSKMFailure", "Failed to generate the skinned triangle mesh: missing input Skeletal Mesh"));
		return INDEX_NONE;
	}

	// Gather data from the SkelMesh.
	const FSkeletalMeshRenderData* const RenderData = SkeletalMesh->GetResourceForRendering();
	if (!ensure(RenderData))
	{
		OutErrorMessage = FText::Format(NSLOCTEXT("GenerateSkinnedTriangleMesh", "MissingRenderDataFailure", "Failed to generate the skinned triangle mesh: missing Render Data on Skeletal Mesh '{0}'"), FText::FromString(SkeletalMesh->GetName()));
		return INDEX_NONE;
	}
	if (!ensure(RenderData->LODRenderData.IsValidIndex(LODIndex)))
	{
		OutErrorMessage = FText::Format(NSLOCTEXT("GenerateSkinnedTriangleMesh", "MissingLODRenderDataFailure", "Failed to generate the skinned triangle mesh: missing LOD Render Data on Skeletal Mesh '{0}' and LOD {1}"), FText::FromString(SkeletalMesh->GetName()), LODIndex);
		return INDEX_NONE;
	}

	const FSkeletalMeshLODRenderData& LodRenderData = RenderData->LODRenderData[LODIndex];
	const int32 NumOrigVertices = LodRenderData.StaticVertexBuffers.PositionVertexBuffer.GetNumVertices();
	if (NumOrigVertices == 0)
	{
		// This implies the actual mesh has no positions, not that they're just missing on the CPU.
		// Rather than generate an empty object, generate an error
		OutErrorMessage = FText::Format(NSLOCTEXT("GenerateSkinnedTriangleMesh", "EmptyLODRenderDataFailure", "Failed to generate the skinned triangle mesh: empty LOD Render Data on Skeletal Mesh '{0}' at LOD {1}"), FText::FromString(SkeletalMesh->GetName()), LODIndex);
		return INDEX_NONE;
	}
	if (!ensure(NumOrigVertices == LodRenderData.SkinWeightVertexBuffer.GetNumVertices()))
	{
		OutErrorMessage = FText::Format(NSLOCTEXT("GenerateSkinnedTriangleMesh", "InvalidLODRenderData", "Failed to generate the skinned triangle mesh: invalid LOD Render Data on Skeletal Mesh '{0}' at LOD {1}"), FText::FromString(SkeletalMesh->GetName()), LODIndex);
		return INDEX_NONE;
	}
	if (!ensure(LodRenderData.StaticVertexBuffers.PositionVertexBuffer.GetVertexData() && LodRenderData.SkinWeightVertexBuffer.GetDataVertexBuffer() && LodRenderData.SkinWeightVertexBuffer.GetDataVertexBuffer()->GetWeightData()))
	{
		OutErrorMessage = FText::Format(NSLOCTEXT("GenerateSkinnedTriangleMesh", "MissingCPURenderDataFailure", "Failed to generate the skinned triangle mesh: missing CPU LOD Render Data on Skeletal Mesh '{0}' and LOD {1}"), FText::FromString(SkeletalMesh->GetName()), LODIndex);
		return INDEX_NONE;
	}	

	// Weld mesh verts to get rid of duplicates within a section. BoneData is taken from lower indexed duplicate. This matches existing cloth behavior.
	TArray<FSkinWeightInfo> SkinWeightInfos;
	LodRenderData.SkinWeightVertexBuffer.GetSkinWeights(SkinWeightInfos);
	check(SkinWeightInfos.Num() == NumOrigVertices);

	TArray<int32> OrigToWeldedVertices;
	TArray<Chaos::FVec3f> WeldedPositions;
	TArray<Chaos::FWeightedInfluenceData> BoneData;
	OrigToWeldedVertices.Reserve(NumOrigVertices);
	WeldedPositions.Reserve(NumOrigVertices);
	BoneData.Reserve(NumOrigVertices);
	for (const FSkelMeshRenderSection& Section : LodRenderData.RenderSections)
	{
		constexpr Chaos::FRealSingle ThreshSq = UE_THRESH_POINTS_ARE_SAME * UE_THRESH_POINTS_ARE_SAME;
		const int32 WeldedSectionStartIndex = WeldedPositions.Num();

		for (uint32 OrigVertexIndex = Section.BaseVertexIndex; OrigVertexIndex < Section.BaseVertexIndex + Section.NumVertices; ++OrigVertexIndex)
		{
			const Chaos::FVec3f Position(LodRenderData.StaticVertexBuffers.PositionVertexBuffer.VertexPosition(OrigVertexIndex));
			bool bIsUnique = true;
			int32 WeldedIndex = INDEX_NONE;
			for (int32 WeldedVertexIndex = WeldedSectionStartIndex; WeldedVertexIndex < WeldedPositions.Num(); ++WeldedVertexIndex)
			{
				if (Chaos::FVec3f::DistSquared(Position, WeldedPositions[WeldedVertexIndex]) <= ThreshSq)
				{
					bIsUnique = false;
					WeldedIndex = WeldedVertexIndex;
					break;
				}
			}
			if (bIsUnique)
			{
				WeldedIndex = WeldedPositions.Emplace(Position);

				// Copy bone data
				const FSkinWeightInfo& SkinWeightInfo = SkinWeightInfos[OrigVertexIndex];
				static_assert(MAX_TOTAL_INFLUENCES <= Chaos::FWeightedInfluenceData::MaxTotalInfluences);
				Chaos::FWeightedInfluenceData& NewBoneData = BoneData.AddDefaulted_GetRef();
				check(NewBoneData.NumInfluences == 0);
				for (int32 InfluenceIdx = 0; InfluenceIdx < MAX_TOTAL_INFLUENCES; ++InfluenceIdx)
				{
					if (SkinWeightInfo.InfluenceWeights[InfluenceIdx] > 0)
					{
						NewBoneData.BoneIndices[NewBoneData.NumInfluences] = Section.BoneMap[SkinWeightInfo.InfluenceBones[InfluenceIdx]];
						NewBoneData.BoneWeights[NewBoneData.NumInfluences] = (float)SkinWeightInfo.InfluenceWeights[InfluenceIdx] * UE::AnimationCore::InvMaxRawBoneWeightFloat;
						++NewBoneData.NumInfluences;
					}
				}
			}
			OrigToWeldedVertices.Emplace(WeldedIndex);
		}
	}
	check(OrigToWeldedVertices.Num() == NumOrigVertices);

	// Get UsedBones names and convert BoneIndices from RefSkeleton global indexing to local indexing.
	const FReferenceSkeleton& RefSkeleton = SkeletalMesh->GetRefSkeleton();
	
	// This is copied from WeightedLevelSetImplicitObjectBuilder... Todo refactor all this to be shared.
	TSet<int32> UsedBoneSet;
	for (int32 Idx = 0; Idx < BoneData.Num(); ++Idx)
	{
		const Chaos::FWeightedInfluenceData& Data = BoneData[Idx];
		for (uint8 InfIdx = 0; InfIdx < Data.NumInfluences; ++InfIdx)
		{
			UsedBoneSet.Add(Data.BoneIndices[InfIdx]);
		}
	}
	const TArray<int32> UsedBoneIndices = UsedBoneSet.Array();
	// Calculate the root bone index
	const int32 RootBoneIndex = CalculateCommonRootBoneIndex(RefSkeleton, UsedBoneIndices);
	const FTransform RootRefBaseInv(FMatrix(SkeletalMesh->GetRefBasesInvMatrix()[RootBoneIndex]));
	FTransform ReferenceRootTransform = RootRefBaseInv.Inverse();
	...
	TMap<int32, int32> UsedBoneReverseLookup;
	for (int32 ArrayIdx = 0; ArrayIdx < UsedBoneIndices.Num(); ++ArrayIdx)
	{
		UsedBones[ArrayIdx] = RefSkeleton.GetBoneName(UsedBoneIndices[ArrayIdx]);
		ReferenceRelativeTransforms[ArrayIdx] = ReferenceRootTransform * FTransform(FMatrix(SkeletalMesh->GetRefBasesInvMatrix()[UsedBoneIndices[ArrayIdx]]));
		UsedBoneReverseLookup.Add(UsedBoneIndices[ArrayIdx], ArrayIdx);
	}

	for (int32 Idx = 0; Idx < BoneData.Num(); ++Idx)
	{
		Chaos::FWeightedInfluenceData& Data = BoneData[Idx];
		for (uint8 InfIdx = 0; InfIdx < Data.NumInfluences; ++InfIdx)
		{
			Data.BoneIndices[InfIdx] = (uint16)UsedBoneReverseLookup[Data.BoneIndices[InfIdx]];
		}

		WeldedPositions[Idx] = Chaos::FVec3f(RootRefBaseInv.TransformPosition((FVector)WeldedPositions[Idx]));
	}

	// Copy over triangles with updated indices from welding.
	TArray<uint32> OrigIndices;
	LodRenderData.MultiSizeIndexContainer.GetIndexBuffer(OrigIndices);
	check(OrigIndices.Num() % 3 == 0);
	const int32 NumTriangles = OrigIndices.Num() / 3;
	TArray<Chaos::TVec3<int32>> Elements;
	Elements.Reserve(NumTriangles);
	for (int32 TriIndex = 0; TriIndex < NumTriangles; ++TriIndex)
	{
		Elements.Emplace(
			OrigToWeldedVertices[OrigIndices[TriIndex * 3]],
			OrigToWeldedVertices[OrigIndices[TriIndex * 3 + 1]],
			OrigToWeldedVertices[OrigIndices[TriIndex * 3 + 2]]);
	}

	// Build TriangleMesh.
	Chaos::FTriangleMesh TriangleMesh(MoveTemp(Elements), 0, WeldedPositions.Num());

	// Make the SkinnedTriangleMesh
	TRefCountPtr<Chaos::FSkinnedTriangleMesh> SkinnedTriangleMesh(new Chaos::FSkinnedTriangleMesh(
		MoveTemp(TriangleMesh),
		MoveTemp(WeldedPositions),
		MoveTemp(BoneData),
		MoveTemp(UsedBones),
		MoveTemp(ReferenceRootTransform),
		MoveTemp(ReferenceRelativeTransforms)));

	// Make the SkinnedTriangleMeshElem
	FKSkinnedTriangleMeshElem SkinnedTriangleMeshElem;
	SkinnedTriangleMeshElem.SetSkinnedTriangleMesh(MoveTemp(SkinnedTriangleMesh));


	// Get or create a new body for RootBoneIndex.
	const FName RootBoneName = RefSkeleton.GetBoneName(RootBoneIndex);
	const int32 NewBodyIndex = CreateNewBody(PhysicsAsset, RootBoneName, Params);
	UBodySetup* NewBodySetup = PhysicsAsset->SkeletalBodySetups[NewBodyIndex];
	check(NewBodySetup->BoneName == RootBoneName);
	NewBodySetup->AggGeom.SkinnedTriangleMeshElems.Emplace(MoveTemp(SkinnedTriangleMeshElem));
...
int32 CreateNewBody(UPhysicsAsset* const PhysAsset, const FName InBodyName, const FPhysAssetCreateParams& Params)
{
	check(PhysAsset);
	...
	BodyIndex = PhysAsset->SkeletalBodySetups.AddDefaulted();
	CreateNewBodyAtIndexInternal(PhysAsset, InBodyName, Params, BodyIndex);

	return BodyIndex; // Return index of new body.
}

void RecreateBody(UPhysicsAsset* const PhysAsset, const FName InBodyName, const FPhysAssetCreateParams& Params, const int32 BodyIndex)
{
	RemoveBodyFromCollisionTableInternal(PhysAsset, BodyIndex, false);
	PhysAsset->SkeletalBodySetups[BodyIndex] = TObjectPtr<USkeletalBodySetup>(); // Reset TObjectPtr to body setup that will be replaced.
	CreateNewBodyAtIndexInternal(PhysAsset, InBodyName, Params, BodyIndex);
}

void DestroyBody(UPhysicsAsset* const PhysAsset, const int32 bodyIndex)
{
	RemoveBodyFromCollisionTableInternal(PhysAsset, bodyIndex, true);
	RemoveBodyConstraintsInternal(PhysAsset, bodyIndex);
