...
bool FTargetPlatformBase::ShouldStripNaniteFallbackMeshes() const
{
	return false;
}
...
EOfflineBVHMode FTargetPlatformBase::GetSkeletalMeshOfflineBVHMode() const
{
	return EOfflineBVHMode::Disabled;
}
...
FName FTargetPlatformBase::GetFallbackASTCHDR() const
{
	static IConsoleVariable* CVarFallbackForASTCHDRProfile = IConsoleManager::Get().FindConsoleVariable(TEXT("cook.FallbackForASTCHDRProfile"));
	static const FName NameRGBA16F(TEXT("RGBA16F"));
	static const FName NameASTC_RGB_LDR(TEXT("ASTC_RGB"));

	switch (CVarFallbackForASTCHDRProfile->GetInt())
	{
	case 0:
