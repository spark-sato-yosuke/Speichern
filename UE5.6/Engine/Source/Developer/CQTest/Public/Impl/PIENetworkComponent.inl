...
inline FPIENetworkComponent<NetworkDataType>& FPIENetworkComponent<NetworkDataType>::SpawnOnServer(const FActorSpawnParameters& SpawnParameters, TFunction<void(ActorToSpawn&)> BeforeReplicate, TOptional<FTimespan> Timeout) {
	...
	.UntilServer(TEXT("Waiting for NetGUID"), [this, ServerActor](NetworkDataType& State) {
		FNetworkGUID NetGUID = State.World->GetNetDriver()->GetNetGuidCache()->GetNetGUID(*ServerActor);
		if (!NetGUID.IsValid())
		{
			return false;
		}
...
inline bool FPIENetworkComponent<NetworkDataType>::ReplicateToClients(NetworkDataType& ClientState)
{
	for(const auto& [NetGUID, StorageOffset] : SpawnedActors)
	{
		...
		AActor* ClientActor = Cast<AActor>(ClientState.World->GetNetDriver()->GetNetGuidCache()->GetObjectFromNetGUID(NetGUID, true));
