...
* @param bCacheOptimize			Whether or not to cache optimize the index buffer in each section.
*/
...
public:
		...
		, bTooManyVerts(false)
	{
		...
		if (!InBuildOptions.bComputeNormals || !InBuildOptions.bComputeTangents)
		{
			...
			// TODO: Does Nanite even use this data? Likely only with explicit tangents!
	...
	inline uint32 GetWedgeIndex(uint32 FaceIndex, uint32 TriIndex) const
	{
		return Faces[FaceIndex].iWedge[TriIndex];
	}

	inline uint32 GetVertexIndex(uint32 WedgeIndex) const
	{
		return Wedges[WedgeIndex].iVertex;
	}

	inline uint32 GetVertexIndex(uint32 FaceIndex, uint32 TriIndex) const
	{
		return Wedges[Faces[FaceIndex].iWedge[TriIndex]].iVertex;
	}

	inline const FVector3f& GetVertexPosition(uint32 WedgeIndex) const
	{
		return Points[Wedges[WedgeIndex].iVertex];
	}

	inline const FVector3f& GetVertexPosition(uint32 FaceIndex, uint32 TriIndex) const
	{
		return Points[Wedges[Faces[FaceIndex].iWedge[TriIndex]].iVertex];
	}

	inline const FVector2f& GetVertexUV(uint32 FaceIndex, uint32 TriIndex, uint32 UVIndex) const
	{
		return Wedges[Faces[FaceIndex].iWedge[TriIndex]].UVs[UVIndex];
	}

	inline uint32 GetFaceSmoothingGroups(uint32 FaceIndex) const
	{
	...
	inline uint32 GetNumWedges() const
	{
		return Wedges.Num();
	}

	inline TArray<FVector3f>& GetTangentArray(uint32 Axis)
	{
		if (Axis == 0)
		{
			return TangentX;
	...
	inline void ValidateTangentArraySize() const
	{
	...
	inline SMikkTSpaceInterface* GetMikkTInterface()
	{
		return &MikkTInterface;
	}

	inline void* GetMikkTUserData()
	{
...
public:
		...
		)
	{
		...
		for (int32 i = 0; i < VertIndexAndZ.Num(); i++)
		{
			...
			for (int32 j = i + 1; j < VertIndexAndZ.Num(); j++)
			{
				...
				const FVector3f& PositionA = BuildData.GetVertexPosition(VertIndexAndZ[i].Index);
				const FVector3f& PositionB = BuildData.GetVertexPosition(VertIndexAndZ[j].Index);
		...
		)
	{
		...
		TriangleTangentX.Reset(NumTriangles);
		TriangleTangentY.Reset(NumTriangles);
		TriangleTangentZ.Reset(NumTriangles);

		//Currently GetSafeNormal do not support 0.0f threshold properly
		float RealComparisonThreshold = FMath::Max(ComparisonThreshold, FLT_MIN);
		
		TArray<FVector3f> TriangleTangents;
		TriangleTangents.Reserve(NumTriangles * 3);

		for (int32 TriangleIndex = 0; TriangleIndex < NumTriangles; TriangleIndex++)
		{
			const FVector3f& Position0 = BuildData.GetVertexPosition(TriangleIndex, 0);
			const FVector3f& Position1 = BuildData.GetVertexPosition(TriangleIndex, 1);
			const FVector3f& Position2 = BuildData.GetVertexPosition(TriangleIndex, 2);

			const int32 UVIndex = 0;
			const FVector2f& UV0 = FVector2f(BuildData.GetVertexUV(TriangleIndex, 0, UVIndex));
			const FVector2f& UV1 = FVector2f(BuildData.GetVertexUV(TriangleIndex, 1, UVIndex));
			const FVector2f& UV2 = FVector2f(BuildData.GetVertexUV(TriangleIndex, 2, UVIndex));

			CalculateTriangleTangentInternal(Position0, UV0, Position1, UV1, Position2, UV2, TriangleTangents, RealComparisonThreshold);
			checkSlow(TriangleTangents.Num() == 3);
	...
	void AddAdjacentFace(SkeletalMeshBuildData& BuildData, TBitArray<>& FaceAdded, TMap<int32, TArray<int32>>& VertexIndexToAdjacentFaces, int32 FaceIndex, TArray<int32>& TriangleQueue, FAdjacentFaceContext& Context, const bool bConnectByEdge)
	{
		checkSlow(FaceAdded.Num() == (int32)BuildData.GetNumFaces());

		Context.AdjacentFaceCommonVertices.Reset();
		...
		for (int32 Corner = 0; Corner < 3; Corner++)
		{
			...
			for (int32 AdjacentFaceArrayIndex = 0; AdjacentFaceArrayIndex < AdjacentFaces.Num(); ++AdjacentFaceArrayIndex)
			{
				...
				if (!FaceAdded[AdjacentFaceIndex] && AdjacentFaceIndex != FaceIndex)
				{
					...
					if (bConnectByEdge)
					{
						int32& AdjacentFaceCommonVerticeCount = Context.AdjacentFaceCommonVertices.FindOrAdd(AdjacentFaceIndex);
	...
	void Skeletal_FillPolygonPatch(SkeletalMeshBuildData& BuildData, TArray<int32>& FaceIndexToPatchIndex, const bool bConnectByEdge)
	{
		int32 NumTriangles = BuildData.GetNumFaces();
		...
		VertexIndexToAdjacentFaces.Reserve(NumTriangles*2);
		...
		FAdjacentFaceContext FaceContext;
		FaceContext.AdjacentFaceCommonVertices.Reserve(100);
		...
		for (int32 FaceIndex = 0; FaceIndex < NumTriangles; ++FaceIndex)
		{
			...
			while (TriangleQueue.Num() > 0)
			{
				...
				AddAdjacentFace(BuildData, FaceAdded, VertexIndexToAdjacentFaces, CurrentTriangleIndex, TriangleQueue, FaceContext, bConnectByEdge);
			}
			PatchIndex++;
		}
	}

	bool IsTriangleMirror(SkeletalMeshBuildData& BuildData, const TArray<FVector3f>& TriangleTangentZ, const uint32 FaceIdxA, const uint32 FaceIdxB)
	{
	...
	bool PrepareSourceMesh(const FString& SkeletalMeshName, SkeletalMeshBuildData& BuildData)
	{
		check(Stage == EStage::Uninit);
	...
	bool GenerateSkeletalRenderMesh(SkeletalMeshBuildData& BuildData)
	{
		...
		TArray<int32>	WedgeInfluenceIndices; // GW-TODO: Reserve
		TMap<uint32, uint32> VertexIndexToInfluenceIndexMap; // GW-TODO: Reserve
		...
		for (int32 FaceIndex = 0; FaceIndex < BuildData.Faces.Num(); FaceIndex++)
		{
			...
			for (int32 VertexIndex = 0; VertexIndex < 3; VertexIndex++)
			{
				...
				// Normalize overridden tangents.  It's possible for them to import un-normalized.
				TangentX.Normalize();
				TangentY.Normalize();
				TangentZ.Normalize();
				...
				{
					Vertex.UVs[UVIndex] = Wedge.UVs[UVIndex];
				}
		...
		for (const auto& Kvp : BuildData.LODModel.SkinWeightProfiles)
		{
			const FImportedSkinWeightProfileData& ImportedProfileData = Kvp.Value;
...
)
{
	...
	auto UpdateOverlappingVertices = [](FSkeletalMeshLODModel& InLODModel)
	{
		...
		for (int32 SectionIdx = 0; SectionIdx < InLODModel.Sections.Num(); SectionIdx++)
		{
			...
			MaxSectionVertices = FMath::Max(MaxSectionVertices, CurSection.SoftVertices.Num());
		}

		TArray<FIndexAndZ> VertIndexAndZ;
		VertIndexAndZ.Reserve(MaxSectionVertices);
	...
	BuildSkeletalModelFromChunks(BuildData.LODModel, BuildData.RefSkeleton, BuildData.Chunks, BuildData.PointToOriginalMap, BuildOptions.bCacheOptimize);
