...
struct FSourceControlProjectInfo
{
	...
	friend bool operator==(const FSourceControlProjectInfo& A, const FSourceControlProjectInfo& B)
	{
		return A.ProjectDirectory == B.ProjectDirectory
			&& A.ContentDirectories == B.ContentDirectories;
	}
 ...
 * 
 * @note While this is on scope, ISourceControlModule::IsEnabled() will return false, 
 *       but ISourceControlModule::GetProvider() will still return the current provider
 */
struct FScopedDisableSourceControl
{
public:
	SOURCECONTROL_API explicit FScopedDisableSourceControl(const bool InDisable = true);
	SOURCECONTROL_API ~FScopedDisableSourceControl();

	UE_NONCOPYABLE(FScopedDisableSourceControl);
...
public:
	 ...
	 * @note It is called automatically for RegisterCustomProjectsDelegate and UnregisterCustomProjectsDelegate, but outside of that 
	         it is the responsibility of the code that registered FSourceControlCustomProjectsDelegate to handle change notifications.
	 */
	virtual FSimpleMulticastDelegate& OnCustomProjectsChanged() = 0;
	...
	/**
	 * Gets a pointer to the source control module instance, if loaded.
	 *
	 * @return A pointer to the source control module, or null if not loaded.
	 */
	static inline ISourceControlModule* GetPtr()
	{
		static FName SourceControlModule("SourceControl");
		return FModuleManager::GetModulePtr<ISourceControlModule>(SourceControlModule);
	}
};
