...
FScopedDisableSourceControl::FScopedDisableSourceControl(const bool InDisable)
	: bDisabled(InDisable)
{
	checkf(IsInGameThread(), TEXT("FScopedDisableSourceControl should only be used from the game thread!"));
	if (bDisabled)
	{
		++FSourceControlModule::Get().TemporarilyDisabledCount;
	}
}
...
{
	return TemporarilyDisabledCount == 0 && GetProvider().IsEnabled();
}
...
void FSourceControlModule::RegisterCustomProjectsDelegate(FSourceControlCustomProjectsDelegate InCustomProjectsDelegate)
{
	...
	OnCustomProjectsChanged().Broadcast();
}
...
FSimpleMulticastDelegate& FSourceControlModule::OnCustomProjectsChanged()
{
	return CustomProjectsChangedDelegate;
}
