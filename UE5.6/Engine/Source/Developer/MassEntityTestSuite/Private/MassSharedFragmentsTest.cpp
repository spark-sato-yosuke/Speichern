...
struct FSharedFragment_ForEach : FExecutionTestBase
{
	virtual bool InstantTest() override
	{
		...
		{
			EntityManager->GetOrCreateSharedFragment<FTestSharedFragment_Int>(InitValue);
		}
...
struct FConstSharedFragment_ForEach : FExecutionTestBase
{
	virtual bool InstantTest() override
	{
		...
		for (int32 InitValue : TestInitValues)
		{
			EntityManager->GetOrCreateConstSharedFragment<FTestConstSharedFragment_Int>(InitValue);
		}
