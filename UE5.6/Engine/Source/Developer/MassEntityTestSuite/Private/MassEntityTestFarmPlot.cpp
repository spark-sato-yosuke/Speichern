...
// UFarmWaterProcessor
//-----------------------------------------------------------------------------
void UFarmWaterProcessor::ConfigureQueries(const TSharedRef<FMassEntityManager>& EntityManager)
{
...
//-----------------------------------------------------------------------------
void UFarmHarvestTimerSystem_Flowers::ConfigureQueries(const TSharedRef<FMassEntityManager>& EntityManager)
{
	...
	EntityQuery.ForEachEntityChunk(Context, [this](FMassExecutionContext& Context) 
		{
			...
			{
				if (TimerList[EntityIt].NumSecondsLeft > 0)
				{
					--TimerList[EntityIt].NumSecondsLeft;

					if (WaterList[EntityIt].CurrentWater > WellWateredThreshold)
					{
						++FlowerList[EntityIt].NumBonusTicks;
					}
				}
			}
		});
...
//-----------------------------------------------------------------------------
void UFarmHarvestTimerSystem_Crops::ConfigureQueries(const TSharedRef<FMassEntityManager>& EntityManager)
{
	EntityQuery.AddRequirement<FHarvestTimerFragment>(EMassFragmentAccess::ReadWrite);
	EntityQuery.AddRequirement<FFarmWaterFragment>(EMassFragmentAccess::ReadOnly);
	EntityQuery.AddRequirement<FFarmCropFragment>(EMassFragmentAccess::None, EMassFragmentPresence::All);
}
	...
	EntityQuery.ForEachEntityChunk(Context, [this](FMassExecutionContext& Context) 
		{
			...
			for (FMassExecutionContext::FEntityIterator EntityIt = Context.CreateEntityIterator(); EntityIt; ++EntityIt)
			{
				const uint32 TimeToSubtract = (WaterList[EntityIt].CurrentWater > WellWateredThreshold) ? 2 : 1;
				TimerList[EntityIt].NumSecondsLeft = (TimerList[EntityIt].NumSecondsLeft >= TimeToSubtract) ? (TimerList[EntityIt].NumSecondsLeft - TimeToSubtract) : 0;
			}
		});
}
...
//-----------------------------------------------------------------------------
void UFarmHarvestTimerExpired::ConfigureQueries(const TSharedRef<FMassEntityManager>& EntityManager)
{
	...
	EntityQuery.ForEachEntityChunk(Context, [this](FMassExecutionContext& Context) 
		{
			...
			for (FMassExecutionContext::FEntityIterator EntityIt = Context.CreateEntityIterator(); EntityIt; ++EntityIt)
			{
				if (TimerList[EntityIt].NumSecondsLeft == 0)
				{
					Context.Defer().AddTag<FFarmJustBecameReadyToHarvestTag>(Context.GetEntity(EntityIt));
				}
			}
		});
}
...
//-----------------------------------------------------------------------------
void UFarmHarvestTimerSetIcon::ConfigureQueries(const TSharedRef<FMassEntityManager>& EntityManager)
{
	...
	EntityQuery.ForEachEntityChunk(Context, [this](FMassExecutionContext& Context) 
		{
			...
			for (FMassExecutionContext::FEntityIterator EntityIt = Context.CreateEntityIterator(); EntityIt; ++EntityIt)
			{
				const FFarmGridCellData& GridCells = GridCoordList[EntityIt];
				...
				VisualList[EntityIt].HarvestIconIndex = HarvestIconISMC->AddInstance(IconTransform);

				FMassEntityHandle ThisEntity = Context.GetEntity(EntityIt);
