...
public:
	...
	virtual bool ShouldFindEditorForAsset() const override { return AssetDefinitionPtr.Get()->ShouldFindEditorForAsset(); }
...
namespace UE::AssetTools::Private
{
	...
	static bool bEnableEpicInternalAssetFeature = true;
	static FAutoConsoleVariableRef CVarEnableEpicInternalAssetFeature(
		TEXT("AssetTools.EnableEpicInternalAssetFeature"),
		bEnableEpicInternalAssetFeature,
		TEXT("Enables the Experimental Epic Internal Asset Feature UI (False: disabled, True: enabled")
	);
			...
			for (const ReportPackageData& PackageData : *(PackageDataToMigrate.Get()))
			{
				...
				if (bIsNameChangeCausedByExistingAssetInDestination)
				{
					...
					{ 
						...
						if (!UAssetToolsImpl::CanShowDialogs() || !PackageMigrationImplContext.Options.bPrompt || LastResponse == EAppReturnType::YesAll || LastResponse == EAppReturnType::NoAll)
						{
	...
	, CreateAssetsAsExternallyReferenceable(true)
{
	...
	AllocatedCategoryBits.Add(TEXT("FX"), FAdvancedAssetCategory(EAssetTypeCategories::FX, LOCTEXT("FXAssetCategory", "FX")));
...
EAssetTypeCategories::Type UAssetToolsImpl::RegisterAdvancedAssetCategory(FName CategoryKey, FText CategoryDisplayName)
{
	...
	if (Result == EAssetTypeCategories::Misc)
	{
		if (NextUserCategoryBit != 0)
		{
			...
			UE_LOG(LogAssetTools, Verbose, TEXT("RegisterAssetTypeCategory(\"%s\", \"%s\") used up bit 0x%x, Offset: %d"), *CategoryKey.ToString(), *CategoryDisplayName.ToString(), NextUserCategoryBit, FMath::FloorLog2(Result));
...
UObject* UAssetToolsImpl::CreateAsset(const FString& AssetName, const FString& PackagePath, UClass* AssetClass, UFactory* Factory, FName CallingContext)
{
	...
	if( NewObj )
	{
		Pkg->SetAssetAccessSpecifier(ShouldCreateAssetsAsExternallyReferenceableForPath(PackageName) ? EAssetAccessSpecifier::Public : EAssetAccessSpecifier::Private);
	...
	if ( SourceObjects.Num() == 1 )
	{
		if (UObject* SourceObject = SourceObjects[0])
		{
			if (UFactory* Factory = FactoryConstructor(SourceObject))
			{
				...
				const TOptional<FString> DefaultAssetName = GetDefaultAssetNameForClass(CreateAssetType);

				const FString BasePackageName = DefaultAssetName.IsSet() ? DefaultAssetName.GetValue() : SourceObject->GetName();
				const FString Suffix = DefaultAssetName.IsSet() ? FString() : DefaultSuffix;

				const FString PackagePath = FPackageName::GetLongPackagePath(SourceObject->GetPackage()->GetName());

				FString Name;
				FString PackageName;
				CreateUniqueAssetName(PackagePath / BasePackageName, Suffix, PackageName, Name);
...
UObject* UAssetToolsImpl::CreateAssetWithDialog(UClass* AssetClass, UFactory* Factory, FName CallingContext)
{
	if (Factory != nullptr)
	{
		...
		// Determine an appropriate name
		const TOptional<FString> DefaultAssetName = GetDefaultAssetNameForClass(AssetClass);
		const FString BaseAssetName = DefaultAssetName.IsSet() ? DefaultAssetName.GetValue() : Factory->GetDefaultNewAssetName();

		FString PackageName;
		FString AssetName;
		CreateUniqueAssetName(AssetPath / BaseAssetName, TEXT(""), PackageName, AssetName);
...
bool UAssetToolsImpl::CanShowDialogs()
{
	return !FApp::IsUnattended() && FSlateApplication::IsInitialized() && !GIsRunningUnattendedScript && !IsRunningCommandlet();
}
	...
	if (ValidateFlattenedAdvancedCopyDestinations(SourceAndDestPackages))
	{
		...
		if (UE::AssetTools::Private::bEnableHeaderPatching)
		{
			...
			while (!PatchAssetsCompletionTask.Wait(FTimespan::FromSeconds(0.5)))
			{
				...
				LoopProgress->CompletedWork = (float)FPlatformAtomics::AtomicRead_Relaxed((volatile int32*)&PatchAssetsCompletedCount);
...
void UAssetToolsImpl::MigratePackages(const TArray<FName>& PackageNamesToMigrate, const FString& DestinationPath, const struct FMigrationOptions& Options) const
{
	...
	const bool bPromptUserToSave = CanShowDialogs() && Options.bPrompt;
...
TArray<UObject*> UAssetToolsImpl::ImportAssetsInternal(const TArray<FString>& Files, const FString& RootDestinationPath, TArray<TPair<FString, FString>> *FilesAndDestinationsPtr, const FAssetImportParams& Params) const
{
		...
		if (bUseInterchangeFramework)
		{
			...
			if (InterchangeManager.CanTranslateSourceData(ScopedSourceData.GetSourceData(), Params.bSceneImport))
			{
				...
				TFunction<void(UE::Interchange::FImportResult&)> AppendAndBroadcastImportResultIfNeeded =
					// Note: ImportStatus captured by value so that the lambda keeps the shared ptr alive
					[bSceneImport = Params.bSceneImport, ImportStatus, AppendImportResult, bSyncToBrowser](UE::Interchange::FImportResult& Result)
					{
	...
	if(ReturnObjects.Num() && bSyncToBrowser)
	{
...
bool UAssetToolsImpl::CanCreateAsset(const FString& AssetName, const FString& PackageName, const FText& OperationText) const
{
	...
	// Check for delegates that disallow the particular path and name
	if (!IsObjectPathAllowed(PackageName, &Reason))
	{
		FMessageDialog::Open(EAppMsgType::Ok, Reason);
		return false;
	}
	...
	if( ExistingObject != nullptr )
	{
		...
		if (!CanShowDialogs())
		{
			UE_LOG(LogAssetTools, Error, TEXT("The asset '%s' already exists in package '%s'. CanCreateAsset cannot ask the user as the application is running unattended and will return false"), *AssetName, *PackageName);
			return false;
		}
...
void UAssetToolsImpl::PerformMigratePackages(TArray<FName> PackageNamesToMigrate, const FString DestinationPath, const FMigrationOptions Options) const
{
	...
	if(CanShowDialogs() && Options.bPrompt)
	{
...
void UAssetToolsImpl::MigratePackages_ReportConfirmed(TSharedPtr<TArray<ReportPackageData>> PackageDataToMigrate, const FString DestinationPath, TSet<FName> ExcludedDependencies, TMap<FName, TSet<FName>> PackageToExternalObjectPackages, const FMigrationOptions Options) const
{
	...
	if (!CanShowDialogs() || !DestinationPath.IsEmpty())
	{
	...
	else
	{
		...
		{
			if (CanShowDialogs() && Options.bPrompt && Options.OrphanFolder.IsEmpty())
			{
				...
				else
				{
					...
					if ( SrcFilename.Split( TEXT("/Content/"), nullptr, &SubFolder ) )
					{
						...
						if ( IFileManager::Get().FileSize(*DestFilename) > 0 )
						{
							...
							EAppReturnType::Type Response;
							if (!CanShowDialogs() || !Options.bPrompt || LastResponse == EAppReturnType::YesAll || LastResponse == EAppReturnType::NoAll)
							{
								Response = LastResponse;
							}
							else
...
bool UAssetToolsImpl::IsAssetReadOnly(const FAssetData& AssetData) const
{
	// Cooked assets are always read-only
	if (AssetData.HasAnyPackageFlags(PKG_Cooked | PKG_FilterEditorOnly))
	{
		return true;
	}

	// If this asset's package isn't in a writable folder, then it's read-only
	if (!WritableFolderPermissionList->PassesStartsWithFilter(AssetData.PackagePath))
	{
		return true;
	}
...
FAssetData UAssetToolsImpl::FindAssetByVersePath(const UE::Core::FVersePath& VersePath, bool bIncludeOnlyOnDiskAssets, bool bSkipARFilteredAssets) const
{
	...
	UE::AssetRegistry::EEnumerateAssetsFlags EnumerateAssetsFlags =
		(bIncludeOnlyOnDiskAssets ? UE::AssetRegistry::EEnumerateAssetsFlags::None : UE::AssetRegistry::EEnumerateAssetsFlags::OnlyOnDiskAssets) |
		(bSkipARFilteredAssets ? UE::AssetRegistry::EEnumerateAssetsFlags::None : UE::AssetRegistry::EEnumerateAssetsFlags::AllowUnfilteredArAssets);

	FAssetData AssetData;

	IPluginManager::Get().EnumerateVersePathMountPoints(VersePath, [&VersePath, bIncludeOnlyOnDiskAssets, bSkipARFilteredAssets, &AssetRegistry, EnumerateAssetsFlags, &AssetData](FStringView MountedAssetPath, FStringView LeafPath)
	{
		auto GetAssetByObjectPath = [bIncludeOnlyOnDiskAssets, bSkipARFilteredAssets, &AssetRegistry](const FStringView& PackageName, const FStringView& AssetName) -> FAssetData
		{
			if (PackageName.Len() >= NAME_SIZE || AssetName.Len() >= NAME_SIZE)
			{
				return {};
			}

			// If we can't find the package name, it must not exist, no need to pollute with random clipboard strings.
			FName PackageFName(PackageName, FNAME_Find);
			if (PackageFName.IsNone())
			{
				return {};
			}

			// If we can't find the asset name, it must not exist, no need to pollute with random clipboard strings.
			FName AssetFName(AssetName, FNAME_Find);
			if (AssetFName.IsNone())
			{
				return {};
			}

			return AssetRegistry.GetAssetByObjectPath(
				FSoftObjectPath::ConstructFromAssetPath(FTopLevelAssetPath(PackageFName, AssetFName)), bIncludeOnlyOnDiskAssets, bSkipARFilteredAssets);
		};

		// The last part of the leaf path should be the top level object name.
		FStringView RelativePath;
		FStringView AssetName;
		int32 LastSlashIndex = INDEX_NONE;
		if (LeafPath.FindLastChar(TEXT('/'), LastSlashIndex))
		{
			RelativePath = FStringView(LeafPath).Left(LastSlashIndex + 1);
			AssetName = FStringView(LeafPath).Mid(LastSlashIndex + 1);
		}
		else
		{
			AssetName = LeafPath;
		...
		{
			// Can't resolve an empty name, keep checking each plug-in.
			return true;
		}

		{
			// Typically, the short package name and top level object name match, so check that first.
			FNameBuilder PackageNameBuilder;
			PackageNameBuilder.Append(MountedAssetPath);
			PackageNameBuilder.Append(RelativePath);
			PackageNameBuilder.Append(AssetName);

			FNameBuilder AssetNameBuilder;
			AssetNameBuilder.Append(AssetName);

			AssetData = GetAssetByObjectPath(PackageNameBuilder, AssetNameBuilder);
			if (AssetData.IsValid())
			{
				// Found a match, stop checking subsequent plug-ins.
				return false;
			}

			// It could be that we have a cooked package that only contains compiled blueprints, so check that next.
			AssetNameBuilder.Append(TEXT("_C"));

			AssetData = GetAssetByObjectPath(PackageNameBuilder, AssetNameBuilder);
			if (AssetData.IsValid())
			{
				// If it is a compiled blueprint, GetVersePath will return a Verse path without the _C.
				if (VersePath == AssetData.GetVersePath())
				{
					// Found a match, stop checking subsequent plug-ins.
					return false;
				}
		...
		// Since we don't know the short package name, and the Verse path could point to a top level object whose name doesn't
		// match the short package name, we need to scan the package path for other possible assets.
		{
			FNameBuilder PackagePathBuilder;
			PackagePathBuilder.Append(MountedAssetPath);
			PackagePathBuilder.Append(RelativePath);

			// Remove trailing '/'.
			PackagePathBuilder.RemoveSuffix(1);

			if (PackagePathBuilder.Len() < NAME_SIZE)
			{
				// If we can't find the package path, it must not exist in the path tree, no need to pollute with random clipboard strings.
				FName PackagePath(PackagePathBuilder.ToView(), FNAME_Find);
				if (!PackagePath.IsNone())
				{
					FARFilter ARFilter;
					ARFilter.bIncludeOnlyOnDiskAssets = bIncludeOnlyOnDiskAssets;
					ARFilter.PackagePaths.Add(PackagePath);

					AssetRegistry.EnumerateAssets(ARFilter, [VerseClassPath = UVerseClass::StaticClass()->GetClassPathName(), &VersePath, &AssetData](const FAssetData& PossibleAssetData)
					{
						// We will check Verse classes later on.
						if (VerseClassPath != PossibleAssetData.AssetClassPath && VersePath == PossibleAssetData.GetVersePath())
						{
							AssetData = PossibleAssetData;
						}
						return !AssetData.IsValid();
					}, EnumerateAssetsFlags);

					// Check if EnumerateAssets found a match.
					if (AssetData.IsValid())
					{
						// Found a match, stop checking subsequent plug-ins.
						return false;
		...
		// Keep checking each plug-in.
		return true;
	});

	// Check if EnumerateVersePathMountPoints found a match.
	if (!AssetData.IsValid())
	{
		// Check if we have any Verse classes that match the Verse path.
		// These can have arbitrary verse paths since the package Verse path is defined by the UE module (ModuleRules.SetupVerse).
		FARFilter ARFilter;
		ARFilter.bIncludeOnlyOnDiskAssets = bIncludeOnlyOnDiskAssets;
		ARFilter.ClassPaths.Add(UVerseClass::StaticClass()->GetClassPathName());

		AssetRegistry.EnumerateAssets(ARFilter, [&VersePath, &AssetData](const FAssetData& PossibleAssetData)
		{
			if (VersePath == PossibleAssetData.GetVersePath())
			{
				AssetData = PossibleAssetData;
			}
			return !AssetData.IsValid();
...
bool UAssetToolsImpl::ShowingContentVersePath() const
{
	if (ShowingContentVersePathDelegate.IsBound())
	{
		return ShowingContentVersePathDelegate.Execute();
	}
	return false;
}

UE::AssetTools::FShowingContentVersePath& UAssetToolsImpl::GetOnShowingContentVersePath()
{
	return ShowingContentVersePathDelegate;
}
