		...
		if (bWithDialog)
		{
			...
			if (!NewAssetsAdded.IsEmpty())
			{
				...
				case ELocalizedVariantsInclusion::Exclude:
					AssetsAndVariants = InAssetsRenameData;
					break;
...
bool FAssetRenameManager::FixReferencesAndRename(const TArray<FAssetRenameData>& AssetsAndNames, bool bAutoCheckout, bool bWithDialog, FScopedSlowTask& RenamingSlowTask) const
{
	...
	if (bSoftReferencesOnly || UpdatePackageStatus(AssetsToRename))
	{
		...
		// If revision control is disabled, this still checks if the packages are not read-only.
		const FText& CheckOutSlowTask = ISourceControlModule::Get().IsEnabled()
			? LOCTEXT("CheckOutPackagesSlowTask", "Check out packages...")
			: LOCTEXT("PreparePackagesSlowTask", "Preparing packages...");
		RenamingSlowTask.EnterProgressFrame(0.1f, CheckOutSlowTask);
...
void FAssetRenameManager::DetectReferencingCollections(TArray<FAssetRenameDataWithReferencers>& AssetsToRename) const
{
	...
	for (FAssetRenameDataWithReferencers& AssetToRename : AssetsToRename)
	{
		if (AssetToRename.Asset.IsValid())
		{
			...
			{
				ReferencingCollections.Reset();
				CollectionContainer->GetCollectionsContainingObject(FSoftObjectPath(AssetToRename.Asset.Get()), ReferencingCollections);
...
void FAssetRenameManager::SetupPublicAssets(TArray<FAssetRenameDataWithReferencers>& AssetsToRename) const
{
	...
	for (FAssetRenameDataWithReferencers& RenameData : AssetsToRename)
	{
		...
		// Make sure the asset won't be referencing a private asset in a different mount point after the move
		{
			...
			for (FName DependencyPackageName : DependencyPackageNames)
			{
...

				{
					...
					{
						if (DependencyAssetDatas[0].GetAssetAccessSpecifier() == EAssetAccessSpecifier::Private)
						{
							RenameData.bRenameFailed = true;
		...
		if (Package->GetAssetAccessSpecifier() == EAssetAccessSpecifier::Public)
		{
			continue;
		}
		...
		{
			ReferencingAssetInDifferentMountPoint = OldPackageName;
		}
		if (ReferencingAssetInDifferentMountPoint.IsEmpty())
		{
			for (FName It : RenameData.NotRenamedReferencingPackageNames)
			{
				...
				{
					ReferencingAssetInDifferentMountPoint = ReferencingPackageName;
					break;
				}
			}
		}
...
void FAssetRenameManager::PerformAssetRename(TArray<FAssetRenameDataWithReferencers>& AssetsToRename, const TArray<UPackage*>& ReferencingPackagesToSave, FScopedSlowTask& RenamingSlowTask) const
{
	...
	{
		RenamingSlowTask.EnterProgressFrame(0.5f, FText::GetEmpty());
	}

	// Now branch the files in source control if possible
	if (ISourceControlModule::Get().IsEnabled())
	{
		RenamingSlowTask.EnterProgressFrame(2.9f, UpdatingRevisionControlSlowTask);
		FScopedSlowTask BuildingRelationshipSlowTask(2.9f, LOCTEXT("BranchingRenamedAssetsSlowTask", "Building a relationship between the renamed files in Revision Control..."));

		ProgressStep = 2.9f / static_cast<float>(AssetsToRename.Num());
		TMap<UPackage*, UPackage> PackagesToBranch;
		TArray<FString> LongPackageNamesToCheckout;
		for (const FAssetRenameDataWithReferencers& RenameData : AssetsToRename)
		{
			...
			if (!RenameData.bOnlyFixSoftReferences)
			{
				...
				if (SourceControlState.IsValid() && SourceControlState->IsSourceControlled())
				{
					...
					if (!SourceControlState->IsAdded())
					{
						...
						// Also mark the file for edit so that it can be saved later
						LongPackageNamesToCheckout.Add(*RenameData.NewObjectPath.GetLongPackageName());
					}
				}
			}
			SourceControlHelpers::CheckOutFiles(LongPackageNamesToCheckout, true);
		}
	}
	else
	{
		RenamingSlowTask.EnterProgressFrame(2.9f, FText::GetEmpty());
	}

	// Save all renamed assets and any redirectors that were left behind
	{
		RenamingSlowTask.EnterProgressFrame(1.0f, LOCTEXT("SavingRenamedAssetSlowTask", "Saving renamed assets..."));
		...
		if (PackagesToSave.Num() > 0)
		{
			...
			FEditorFileUtils::PromptForCheckoutAndSave(PackagesToSave, bCheckDirty, bPromptToSave, &FailedPackages, bAlreadyCheckedOut);

			ISourceControlModule::Get().QueueStatusUpdate(Filenames);
		}

		// Revert any previously branched files that did not actually save
		if (ISourceControlModule::Get().IsEnabled())
		{
			RenamingSlowTask.EnterProgressFrame(1.0f, UpdatingRevisionControlSlowTask);

			TArray<FString> FilesToRevert = USourceControlHelpers::PackageFilenames(FailedPackages);
		...
		{
			RenamingSlowTask.EnterProgressFrame(1.0f, FText::GetEmpty());
		}
	}
...
int32 FAssetRenameManager::ReportFailures(const TArray<FAssetRenameDataWithReferencers>& AssetsToRename, bool bWithDialog) const
{
	...
	if (FailedRenames.Num() > 0)
	{
		if (bWithDialog)
		{
			SFileListReportDialog::OpenListDialog(LOCTEXT("FailedRenamesDialog", "Failed Renames"),
