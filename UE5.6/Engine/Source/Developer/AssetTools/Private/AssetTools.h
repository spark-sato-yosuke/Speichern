...
public:
	...
	virtual bool IsAssetReadOnly(const FAssetData& AssetData) const override;
	virtual FAssetData FindAssetByVersePath(const UE::Core::FVersePath& VersePath, bool bIncludeOnlyOnDiskAssets = false, bool bSkipARFilteredAssets = true) const override;
	virtual bool ShowingContentVersePath() const override;
	virtual UE::AssetTools::FShowingContentVersePath& GetOnShowingContentVersePath() override;
	...
	virtual bool IsObjectPathAllowed(const FString& FullPathWithName, FText* OutErrorMessage = nullptr) const override;
	virtual void RegisterIsObjectPathAllowedDelegate(const FName OwnerName, FIsObjectPathAllowed Delegate) override;
	virtual void UnregisterIsObjectPathAllowedDelegate(const FName OwnerName) override;
	...
	virtual TOptional<FString> GetDefaultAssetNameForClass(const UClass* Class) override;
	virtual void RegisterDefaultAssetNameForClass(const UClass* Class, const FString& DefaultName) override;
	virtual void UnregisterDefaultAssetNameForClass(const UClass* Class) override;
...
public:
	...
	/** Check if the Application can currently show dialogs, meaning the App is attended, Slate is initialized and we are not running an unattended or automation scripts */
...
private:
	...
	/** Default names to use when creating new assets */
	TMap<const UClass*, FString> DefaultNewAssetNames;
	...
	UE::AssetTools::FShowingContentVersePath ShowingContentVersePathDelegate;

	TMap<FName, FIsNameAllowed> IsNameAllowedDelegates;
	TMap<FName, FIsObjectPathAllowed> IsObjectPathAllowedDelegates;
