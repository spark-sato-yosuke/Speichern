...
const TArray<FTargetInfo>& FDesktopPlatformBase::GetTargetsForProject(const FString& ProjectFile) const
{
	...
	// we only want this game's targets, not every program under the sun (note if running UnrealEditor without a project, this will
	// still list all the programs). This matches what GenerateProjectFiles outputs when it performs this step
	Arguments += TEXT(" -DontIncludeParentAssembly");
