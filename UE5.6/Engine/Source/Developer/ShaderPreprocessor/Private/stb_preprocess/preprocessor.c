...
static err_charbuf12 error_char(char ch)
{
	err_charbuf12 b;
	if (ch >= 32 && ch < 127)
		if (ch == '\'' || ch == '\\')
			snprintf(b.buf, sizeof(b.buf), "'\\%c'", ch);
		else
			snprintf(b.buf, sizeof(b.buf), "'%c'", ch);
	else if (ch == 0)
		snprintf(b.buf, sizeof(b.buf), "EOF");
	else
		snprintf(b.buf, sizeof(b.buf), "value %d", ch);  // "value -100\0" = 11 (6 + 1 + 3 + 1)
	return b;
}
...
//     4. snprintf() is used if number is > 9999, on the assumption this is rare
...
static char* preprocessor_line_dtoa(char q[INT_STR_SIZE], int number)
{
	...
	{
		return q + snprintf(q, INT_STR_SIZE, "%d", number);
	}
}
...
static void output_line_directive(parse_state* cs)
{
	...
	size_t expansion_length =	1 +					// possible newline
								6 +					// "#line "
								INT_STR_SIZE +	// max size of line number string
								2 +					// space + open quote for filename
								filename_len +
								2;					// close quote for filename + newline
...
#define do_error_char_code(ps, code, text, c, ...) do_error_code(ps, code, text, error_char(c).buf, ##__VA_ARGS__)
...
#define do_error_char(ps, text, c, ...) do_error(ps, text, error_char(c).buf, ##__VA_ARGS__)
...
invalid_char_error:
{
	...
	do_error_char_code(ps, PP_RESULT_invalid_char, error_messages[state], *p, m->symbol_name);
	...
	if (md->num_parameters < 0)
	{
		...
		switch (md->num_parameters)
		{
			...
			case MACRO_NUM_PARAMETERS_defined:
			{
				...
				*q = 0;
				...
				if (parenthesized)
				{
					...
					if (*p != ')')
					{
						do_error_char(cs, "unexpected character %s at end of 'defined('", *p);
						return;
					}
					++p;
			...
			case MACRO_NUM_PARAMETERS_line:
			{
				int len;
				q = identifier;
				arrsetcap(cs->dest, arrlen(cs->dest) + INT_STR_SIZE);
				len = snprintf(cs->dest + arrlen(cs->dest), INT_STR_SIZE, "%d", cs->src_line_number);
			...
			case MACRO_NUM_PARAMETERS_counter:
			{
				...
				q = identifier;
				arrsetcap(cs->dest, arrlen(cs->dest) + INT_STR_SIZE);
				len = snprintf(cs->dest + arrlen(cs->dest), INT_STR_SIZE, "%d", c->counter);
				arrsetlen(cs->dest, arrlen(cs->dest) + len);
				++c->counter;
				break;
			}
...
static const char* parse_pp_identifier_in_directive(parse_state* ps, const char* text, char* identifier, size_t identifier_buffer_length, char* directive, size_t* idlen)
{
	...
	*idlen = 0;
	...
	if (!char_is_pp_identifier_first(*p))
	{
		do_error_char_code(ps, PP_RESULT_invalid_char, "Invalid character %s after '#%s'", *p, directive);
		return p;
	}
	identifier[0] = *p++;
...
static int process_include(parse_state* cs, const char* start, conditional_state* cons)
{
	...
	if (!(*p == '<' || *p == '"'))
	{
		do_error_char(cs, "Expected filename after '#include', found character %s", *p);
	...
	else
	{
		...
		if (*p != 0)
		{
			do_error_char(cs, "Unexpected character %s after filename in '#include'", *p);
			return RETURN_BEHAVIOR_return;
		}
...
static void process_directive(parse_state* cs, conditional_state* cons)
{
	...
	{  // mask to try to avoid range-checked switch
		...
		case 29:
		{
			...
			switch (*p)
			{
				...
				default:
					...
					if (!char_is_pp_identifier_first(*p))
					{
						do_error_char_code(cs, PP_RESULT_invalid_char, "Invalid character %s after '#'", *p);
					}
					else
					{
		...
		case HASH_ifdef:
		{
			...
			if (*p != 0)
			{
				do_error_char_code(cs, PP_RESULT_unexpected_character_after_end_of_directive, "Expected end of line in #ifdef, but found character %s", *p);
			}
			break;
		}
		...
		case HASH_if:
		{
			...
			p = preprocessor_skip_whitespace_simple(p);
			result = evaluate_if(cs, p, &result_code);

			if (result_code != CE_RESULT_ok)
			{
				char* err_msg;
				int ce_result;
				switch (result_code)
				{
				case CE_RESULT_division_by_zero:
					err_msg = "Division by zero occurred when evaluating conditional expression";
					ce_result = PP_RESULT_conditional_expression_division_by_zero;
					break;
				case CE_RESULT_overflow:
					err_msg = "Overflow occurred when evaluating conditional expression";
					ce_result = PP_RESULT_conditional_expression_overflow;
					break;
				case CE_RESULT_syntax_error:
					err_msg = "Syntax error occurred when evaluating conditional expression";
					ce_result = PP_RESULT_syntax_error_conditional_expression;
					break;
				}
				
				do_error_explicit(cs, ce_result, cs->src_line_number, err_msg);
				return;
			}
		...
		handle_else_after_disabled_block:
		{
			...
			if (*p != 0)
			{
				if (do_error_char_code(cs, PP_RESULT_unexpected_character_after_end_of_directive, "Expected end of line in #else, but found character %s.", *p))
				{
					return;
				}
			}
		...
		case HASH_endif:
		{
			...
			if (*p != 0)
			{
				if (do_error_char_code(cs, PP_RESULT_unexpected_character_after_end_of_directive, "Expected end of line in #endif, but found character %s.", *p))
				{
					return;
				}
			}
