 ...
 */
class FAndroidTargetPlatformControls
...
public:
	 ...
	 * Default constructor.
	 */
	ANDROIDTARGETPLATFORMCONTROLS_API FAndroidTargetPlatformControls(bool bInIsClient, ITargetPlatformSettings* TargetPlatformSettings, const TCHAR* FlavorName, const TCHAR* OverrideIniPlatformName = nullptr);

	/**
	 * Destructor
	 */
	ANDROIDTARGETPLATFORMCONTROLS_API virtual ~FAndroidTargetPlatformControls();
...
public:
	...
	ANDROIDTARGETPLATFORMCONTROLS_API virtual void GetAllDevices(TArray<ITargetDevicePtr>& OutDevices) const override;
	...
	ANDROIDTARGETPLATFORMCONTROLS_API virtual ITargetDevicePtr GetDefaultDevice() const override;

	ANDROIDTARGETPLATFORMCONTROLS_API virtual ITargetDevicePtr GetDevice(const FTargetDeviceId& DeviceId) override;

	ANDROIDTARGETPLATFORMCONTROLS_API virtual bool IsSdkInstalled(bool bProjectHasCode, FString& OutDocumentationPath) const override;

	ANDROIDTARGETPLATFORMCONTROLS_API virtual int32 CheckRequirements(bool bProjectHasCode, EBuildConfiguration Configuration, bool bRequiresAssetNativization, FString& OutTutorialPath, FString& OutDocumentationPath, FText& CustomizedLogMessage) const override;

	ANDROIDTARGETPLATFORMCONTROLS_API virtual void GetPlatformSpecificProjectAnalytics(TArray<struct FAnalyticsEventAttribute>& AnalyticsParamArray) const override;
...
#if WITH_ENGINE
	ANDROIDTARGETPLATFORMCONTROLS_API virtual void GetTextureFormats(const UTexture* InTexture, TArray< TArray<FName> >& OutFormats) const override;
	ANDROIDTARGETPLATFORMCONTROLS_API virtual FName FinalizeVirtualTextureLayerFormat(FName Format) const override;
	ANDROIDTARGETPLATFORMCONTROLS_API virtual void GetAllTextureFormats(TArray<FName>& OutFormats) const override;
#endif //WITH_ENGINE

	ANDROIDTARGETPLATFORMCONTROLS_API virtual bool SupportsVariants() const override;
	...
	ANDROIDTARGETPLATFORMCONTROLS_API virtual bool ShouldExpandTo32Bit(const uint16* Indices, const int32 NumIndices) const override;
	//~ End ITargetPlatform Interface

	ANDROIDTARGETPLATFORMCONTROLS_API virtual void InitializeDeviceDetection();

protected:

	ANDROIDTARGETPLATFORMCONTROLS_API virtual FAndroidTargetDevicePtr CreateTargetDevice(const ITargetPlatformControls& InTargetPlatform, const FString& InDeviceId, const FString& InAndroidVariant) const;
...
protected:
	...
	ANDROIDTARGETPLATFORMCONTROLS_API bool HandleTicker(float DeltaTime);

	ANDROIDTARGETPLATFORMCONTROLS_API virtual FAndroidTargetDeviceRef CreateNewDevice(const FAndroidDeviceInfo& DeviceInfo);
...
// XR using the ASTC TPC now
class FAndroid_OpenXRTargetPlatformControls : public FAndroid_ASTCTargetPlatformControls
{
public:
	FAndroid_OpenXRTargetPlatformControls(bool bIsClient, ITargetPlatformSettings* TargetPlatformSettings)
		: FAndroid_ASTCTargetPlatformControls(bIsClient, TargetPlatformSettings, TEXT("OpenXR"))
	{
	}

	virtual FString CookingDeviceProfileName() const override
	{
		return TEXT("Android_OpenXR");
	}
};
