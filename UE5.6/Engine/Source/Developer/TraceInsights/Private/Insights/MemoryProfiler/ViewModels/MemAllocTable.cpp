...
void FMemAllocTable::AddDefaultColumns()
{
	...
	// Hierarchy Column
	{
		...
		HierarchyColumn.SetShortName(LOCTEXT("AllocationColumnName", "Hierarchy"));
		HierarchyColumn.SetTitleName(LOCTEXT("AllocationColumnTitle", "Allocation Hierarchy"));
		HierarchyColumn.SetDescription(LOCTEXT("AllocationColumnDesc", "Hierarchy of the allocation's tree"));
	}
	...
	// Start Event Index Column
	{
		...
		class FMemAllocStartEventIndexValueGetter : public FAggregatedValueGetter
		{
		public:
	...
	// End Event Index Column
	{
		...
		class FMemAllocEndEventIndexValueGetter : public FAggregatedValueGetter
		{
		public:
			virtual const TOptional<FTableCellValue> GetNodeValue(const FBaseTreeNode& Node) const override
			{
	...
	// Event Distance Column
	{
		...
		class FMemAllocEventDistanceValueGetter : public FAggregatedValueGetter
		{
		public:
			virtual const TOptional<FTableCellValue> GetNodeValue(const FBaseTreeNode& Node) const override
			{
	...
	// Start Time Column
	{
		...
		class FMemAllocStartTimeValueGetter : public FAggregatedValueGetter
		{
		public:
			virtual const TOptional<FTableCellValue> GetNodeValue(const FBaseTreeNode& Node) const override
			{
	...
	// End Time Column
	{
		...
		class FMemAllocEndTimeValueGetter : public FAggregatedValueGetter
		{
		public:
			virtual const TOptional<FTableCellValue> GetNodeValue(const FBaseTreeNode& Node) const override
			{
	...
	// Duration Column
	{
		...
		class FMemAllocDurationValueGetter : public FAggregatedValueGetter
		{
		public:
			virtual const TOptional<FTableCellValue> GetNodeValue(const FBaseTreeNode& Node) const override
			{
	...
	// Alloc Thread Column
	{
		...
		class FAllocThreadValueGetter : public FAggregatedValueGetter
		{
		public:
			virtual const TOptional<FTableCellValue> GetNodeValue(const FBaseTreeNode& Node) const override
			{
	...
	// Address Column
	{
		...
		class FMemAllocAddressValueGetter : public FAggregatedValueGetter
		{
		public:
			virtual const TOptional<FTableCellValue> GetNodeValue(const FBaseTreeNode& Node) const override
			{
	...
	// Memory Page Column
	{
		...
		class FMemAllocPageValueGetter : public FAggregatedValueGetter
		{
		public:
			virtual const TOptional<FTableCellValue> GetNodeValue(const FBaseTreeNode& Node) const override
			{
	...
	// Alloc Count Column
	{
		...
		class FMemAllocCountValueGetter : public FAggregatedValueGetter
		{
		public:
			virtual const TOptional<FTableCellValue> GetNodeValue(const FBaseTreeNode& Node) const override
			{
	...
	// Compressed Swap Size
	{
		...
		class FSwapCompressedSizeColumnValueGetter : public FAggregatedValueGetter
		{
		public:
			virtual const TOptional<FTableCellValue> GetNodeValue(const FBaseTreeNode& Node) const override
			{
	...
	// Swap Size Column
	{
		...
		class FSizeInSwapColumnValueGetter : public FAggregatedValueGetter
		{
		public:
			virtual const TOptional<FTableCellValue> GetNodeValue(const FBaseTreeNode& Node) const override
			{
				if (Node.Is<FMemAllocInSwapNode>())
				{
					const FMemAllocInSwapNode& MemAllocInSwapNode = Node.As<FMemAllocInSwapNode>();
	...
	// Size Column
	{
		...
		class FMemAllocSizeValueGetter : public FAggregatedValueGetter
		{
		public:
			virtual const TOptional<FTableCellValue> GetNodeValue(const FBaseTreeNode& Node) const override
			{
				if (Node.Is<FMemAllocNode>())
				{
					...
					if (Alloc)
					{
						...
						{
							const FMemAllocTable& MemAllocTable = MemAllocNode.GetMemTableChecked();
							return FTableCellValue(static_cast<int64>(MemAllocTable.GetPlatformPageSize()));
						}
						else
						{
							return FTableCellValue(static_cast<int64>(Alloc->GetSize()));
		...
		public:
			virtual const TOptional<FTableCellValue> GetValue(const FTableColumn& Column, const FBaseTreeNode& Node) const override
			{
				if (Node.IsGroup())
				{
					...
					{
						const FMemTagTableTreeNode& MemTagNode = Node.As<FMemTagTableTreeNode>();
						int64 LLMSize = MemTagNode.GetLLMSize();
						return FTableCellValue(LLMSize);
					}
					else if (Node.Is<FTableTreeNode>())
					{
						const FTableTreeNode& TableTreeNode = Node.As<FTableTreeNode>();
						if (TableTreeNode.HasAggregatedValue(Column.GetId()))
						{
			...
			virtual const TOptional<FTableCellValue> GetValue(const FTableColumn& Column, const FBaseTreeNode& Node) const override
			{
				if (Node.IsGroup())
				{
					if (Node.Is<FMemTagTableTreeNode>())
					{
						...
						int64 LLMSize = MemTagNode.GetLLMSize();
						int64 Size = 0;
						if (MemTagNode.HasAggregatedValue(FMemAllocTableColumns::SizeColumnId))
						{
							const FTableCellValue& Value = MemTagNode.GetAggregatedValue(FMemAllocTableColumns::SizeColumnId);
	...
	// LLM Tag Column
	{
		...
		class FMemTagValueGetter : public FAggregatedValueGetter
		{
		public:
			virtual const TOptional<FTableCellValue> GetNodeValue(const FBaseTreeNode& Node) const
			{
	...
	// Package Column
	{
		...
		class FPackageMetadataValueGetter : public FAggregatedValueGetter
		{
		public:
			virtual const TOptional<FTableCellValue> GetNodeValue(const FBaseTreeNode& Node) const
			{
	...
	// Asset Column
	{
		...
		class FAssetMetadataValueGetter : public FAggregatedValueGetter
		{
		public:
			virtual const TOptional<FTableCellValue> GetNodeValue(const FBaseTreeNode& Node) const
			{
	...
	// Class Name Column
	{
		...
		class FClassNameMetadataValueGetter : public FAggregatedValueGetter
		{
		public:
			virtual const TOptional<FTableCellValue> GetNodeValue(const FBaseTreeNode& Node) const
			{
	...
	public:
		...
		virtual TSharedPtr<IToolTip> GetCustomTooltip(const FTableColumn& Column, const FBaseTreeNode& Node) const override
		{
			if (!Node.Is<FMemAllocNode>())
			{
				return FTableCellValueFormatter::GetCustomTooltip(Column, Node);
			}
			const FMemAllocNode& MemAllocNode = Node.As<const FMemAllocNode>();
		...
		virtual FText CopyTooltip(const FTableColumn& Column, const FBaseTreeNode& Node) const override
		{
			...
			{
				return FTableCellValueFormatter::CopyTooltip(Column, Node);
			}
		}
	...
	public:
		...
		virtual TSharedPtr<IToolTip> GetCustomTooltip(const FTableColumn& Column, const FBaseTreeNode& Node) const override
		{
			if (!Node.Is<FMemAllocNode>())
			{
				return FTextValueFormatter::GetCustomTooltip(Column, Node);
			}
			const FMemAllocNode& MemAllocNode = Node.As<const FMemAllocNode>();
	...
	// Top Function (Alloc Callstack) Column
	{
		...
		class FFunctionValueGetter : public FAggregatedValueGetter
		{
		public:
			virtual const TOptional<FTableCellValue> GetNodeValue(const FBaseTreeNode& Node) const override
			{
	...
	// Top Source File (Alloc Callstack) Column
	{
		...
		class FSourceValueGetter : public FAggregatedValueGetter
		{
		public:
			virtual const TOptional<FTableCellValue> GetNodeValue(const FBaseTreeNode& Node) const override
			{
	...
	// Alloc Callstack Id Column
	{
		...
		class FCallstackIdValueGetter : public FAggregatedValueGetter
		{
		public:
			virtual const TOptional<FTableCellValue> GetNodeValue(const FBaseTreeNode& Node) const override
			{
	...
	// Alloc Callstack Size Column
	{
		...
		class FCallstackSizeValueGetter : public FAggregatedValueGetter
		{
		public:
			virtual const TOptional<FTableCellValue> GetNodeValue(const FBaseTreeNode& Node) const override
			{
