...
#define LOCTEXT_NAMESPACE "UE::Insights::TimingProfiler::FrameTimingTrack"
...
void FFrameSharedState::OnBeginSession(Timing::ITimingViewSession& InSession)
{
...
void FFrameSharedState::OnEndSession(Timing::ITimingViewSession& InSession)
{
	if (&InSession != TimingView)
	{
		return;
...
void FFrameSharedState::Tick(Timing::ITimingViewSession& InSession, const TraceServices::IAnalysisSession& InAnalysisSession)
{
	if (&InSession != TimingView)
	{
		return;
...

	{
		...
		int32 Order = FTimingTrackOrder::First + 300;
...
void FFrameSharedState::ExtendOtherTracksFilterMenu(Timing::ITimingViewSession& InSession, FMenuBuilder& InOutMenuBuilder)
{
	if (&InSession != TimingView)
	{
		return;
...
{
	return FString::Printf(TEXT("%s Frame %" UINT64_FMT " (%s)"), FFrameTrackDrawHelper::FrameTypeToString(FrameType), InFrameIndex, *FormatTimeAuto(InFrameDuration));
}
