...
SFrameTrack::~SFrameTrack()
{
	...
	if (ProfilerWindow.IsValid())
	{
		TArray<TSharedPtr<FTimerFrameStatsTrackSeries>> TimerSeries;
		for (TSharedPtr<FFrameTrackSeries>& Series : AllSeries)
		{
			if (Series.IsValid() &&
				Series->Type == EFrameTrackSeriesType::TimerFrameStats)
			{
				TimerSeries.Add(StaticCastSharedPtr<FTimerFrameStatsTrackSeries>(Series));
			}
		}
		AllSeries.Reset();
		for (TSharedPtr<FTimerFrameStatsTrackSeries>& Series : TimerSeries)
		{
			ProfilerWindow->OnTimerAddedToGraphsChanged(Series->TimerId);
		}
	}
}
	...
	if (Session.IsValid())
	{
		...
		for (int32 Index = 0; Index < AllSeries.Num(); ++Index)
		{
			...
			if (TimingWindow.IsValid())
			{
				...
				if (TimingView.IsValid())
				{
					...
					if (ThreadSharedState.IsValid())
					{
						...
						FFrameStatsHelper::ComputeFrameStatsForTimer(Frames, TimerSeries->TimerId, Timelines);
			...
			if (!bTimingViewExists)
			{
				...
				FFrameStatsHelper::ComputeFrameStatsForTimer(Frames, TimerSeries->TimerId);
			}
