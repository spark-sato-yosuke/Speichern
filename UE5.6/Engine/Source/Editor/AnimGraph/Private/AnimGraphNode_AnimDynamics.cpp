...
void UAnimGraphNode_AnimDynamics::UpdatePreviewDynamicsNode(const int32 PhysicsBodyIndex)
{
	// Update the preview node from details panel and reset physics simulation.

	if(FAnimNode_AnimDynamics* const ActivePreviewNode = GetPreviewDynamicsNode())
	{
		if(ActivePreviewNode->PhysicsBodyDefinitions.IsValidIndex(PhysicsBodyIndex) && Node.PhysicsBodyDefinitions.IsValidIndex(PhysicsBodyIndex))
		{
			ActivePreviewNode->PhysicsBodyDefinitions[PhysicsBodyIndex] = Node.PhysicsBodyDefinitions[PhysicsBodyIndex];
			ResetSim();
		}
	}
}
...
void UAnimGraphNode_AnimDynamics::Serialize(FArchive& Ar)
{
	...
	{
		// Initialize first physics body using deprecated parameters then re-initialize chain.
...
void UAnimGraphNode_AnimDynamics::OnPhysicsBodyDefCustomizeDetails(TSharedRef<IPropertyHandle> ElementProperty, int32 ElementIndex, IDetailChildrenBuilder& ChildrenBuilder, IDetailLayoutBuilder* DetailLayout)
{
	...
	ElementProperty->SetOnChildPropertyValueChanged(FSimpleDelegate::CreateLambda([this, ElementIndex]() { this->UpdatePreviewDynamicsNode(ElementIndex); }));
}
