...
void UAnimGraphNode_BlendListBase::Serialize(FArchive& Ar)
{
	Super::Serialize(Ar);

	if (Ar.IsLoading())
	{
		FAnimNode_BlendListBase* Node = static_cast<FAnimNode_BlendListBase*>(GetFNode());

		if (Node != nullptr)
		{
PRAGMA_DISABLE_DEPRECATION_WARNINGS
			// Handle deprecation
			if (Node->bResetChildOnActivation_DEPRECATED)
			{
				Node->ChildUpateMode = EBlendListChildUpdateMode::ResetChildOnActivate;
				Node->bResetChildOnActivation_DEPRECATED = false;
			}
