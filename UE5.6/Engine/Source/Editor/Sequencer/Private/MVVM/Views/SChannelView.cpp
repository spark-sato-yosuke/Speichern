...
EViewDependentCacheFlags FChannelViewKeyCachedState::CompareTo(const FChannelViewKeyCachedState& Other) const
{
	...
	if (Other.NonLinearTransform.Pin() != NonLinearTransform.Pin())
	{
...
FReply SChannelView::OnMouseButtonDown(const FGeometry& MyGeometry, const FPointerEvent& MouseEvent)
{
	...
	// Create keys with middle mouse button only if there are no modifier keys down (to prevent zoom scrolling and creating keys at the same time)
...
void SChannelView::CreateKeysUnderMouse(const FVector2D& MousePosition, const FGeometry& AllottedGeometry, const TSet<FSequencerSelectedKey>& InPressedKeys, TArray<FSequencerSelectedKey>& OutKeys)
{
	...
	else
	{
		...
		FAddKeyOperation::FromKeyAreas(TrackModel->GetTrackEditor().Get(), ValidKeyAreasUnderCursor).Commit(CurrentTime.FrameNumber, *LegacyGetSequencer(), &Results);

		for (FAddKeyResult& Result : Results)
		{
			for (TViewModelPtr<FChannelModel> Channel : Model->GetDescendantsOfType<FChannelModel>(true))
			{
				if (Channel->GetChannel() == Result.KeyArea->GetChannel().Get())
				{
					OutKeys.Add(FSequencerSelectedKey(*SectionObject, Channel, Result.KeyHandle));
					break;
				}
			}
		}
