...
void FSequencerCommands::RegisterCommands()
{
	...
	UI_COMMAND( ToggleRerunConstructionScripts, "Allow Construction Scripts to Rerun Every Frame", "Blueprint Actors need to have Run Construction Script in Sequencer active in their Class Settings.", EUserInterfaceActionType::ToggleButton, FInputChord() );
	...
	UI_COMMAND( TogglePreviewCameraCutsInSimulate, "Preview Camera Cuts in Simulate", "Toggle whether camera cuts are evaluated in SIE, or after ejecting from the player controller in PIE", EUserInterfaceActionType::ToggleButton, FInputChord() );
	...
	UI_COMMAND( ToggleSnapKeyTimesToElements, "Snap Keys to Elements", "Snap keys to other keys, sections, markers, playback range, selection range, playhead, and previous position", EUserInterfaceActionType::ToggleButton, FInputChord() );
	UI_COMMAND( ToggleSnapSectionTimesToElements, "Snap Sections to Elements", "Snap sections to other sections, keys, markers, playback range, selection range, and playhead", EUserInterfaceActionType::ToggleButton, FInputChord() );
	...
	UI_COMMAND( ToggleForceWholeFrames, "Force Whole Frames", "Enables or disables keys/sections/playhead on whole frames", EUserInterfaceActionType::ToggleButton, FInputChord() );
	...
	UI_COMMAND( BrowseToObject, "Browse to Object", "Browse to the selected section's object", EUserInterfaceActionType::Button, FInputChord(EModifierKey::Control, EKeys::B) );
	...
	UI_COMMAND( ToggleLinkFiltersWithCurveEditor, "Link Filters with Curve Editor", "Link the curve editor tree view with the sequencer filters", EUserInterfaceActionType::ToggleButton, FInputChord());
	...
	UI_COMMAND( RebindPossessableReferences, "Rebind Possessable References", "Rebinds all possessables in the current sequence to ensure they're using the most robust referencing mechanism.", EUserInterfaceActionType::Button, FInputChord() );
	...
	UI_COMMAND(ToggleLimitViewportSelection, "Limit Viewport Selection", "Toggles viewport selectability between Sequencer only objects and all objects.", EUserInterfaceActionType::ToggleButton, FInputChord());
	...
	UI_COMMAND(ToggleSidebarSelectionDrawerOpen, "Sidebar Selection Drawer Open", "Toggles the visible state of the sidebar selection drawer. Undocks and closes the sidebar drawer if docked.", EUserInterfaceActionType::Button, FInputChord(EModifierKey::Control, EKeys::E));
	UI_COMMAND(ToggleSidebarDrawerDock, "Sidebar Drawer Docked", "Toggles the docked state of the sidebar current drawer. Undocks the docked sidebar drawer if docked or docks the sidebar drawer if there is one open and no currently docked drawer.", EUserInterfaceActionType::Button, FInputChord(EModifierKey::Alt, EKeys::D));

	UI_COMMAND(AlignSelectionToPlayhead, "Align Selection To Playhead", "Aligns layer bars, sections, or key selections to the current playhead location", EUserInterfaceActionType::Button, FInputChord());

	UI_COMMAND(ToggleTrackSelectionPin, "Toggle Track Pin", "Toggle the pinned state of Sequencer track selection", EUserInterfaceActionType::Button, FInputChord());
	UI_COMMAND(ToggleTrackSelectionLock, "Toggle Track Lock", "Toggle the locked state of Sequencer track selection", EUserInterfaceActionType::Button, FInputChord());
	UI_COMMAND(ToggleTrackSelectionDeactive, "Toggle Track Deactive", "Toggle the deactive state of Sequencer track selection", EUserInterfaceActionType::Button, FInputChord());
	UI_COMMAND(ToggleTrackSelectionMute, "Toggle Track Mute", "Toggle the muted state of Sequencer track selection", EUserInterfaceActionType::Button, FInputChord());
	UI_COMMAND(ToggleTrackSelectionSolo, "Toggle Track Solo", "Toggle the soloed state of Sequencer track selection", EUserInterfaceActionType::Button, FInputChord());
}
