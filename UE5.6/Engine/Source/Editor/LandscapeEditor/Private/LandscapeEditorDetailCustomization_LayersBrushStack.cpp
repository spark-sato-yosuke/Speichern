...
void FLandscapeEditorCustomNodeBuilder_LayersBrushStack::GenerateChildContent(IDetailChildrenBuilder& ChildrenBuilder)
{
	...
	{
		const int32 BrushCount = LandscapeEdMode->GetBrushesForCurrentLayer().Num();
		...
		TSharedPtr<SDragAndDropVerticalBox> BrushesList = SNew(SDragAndDropVerticalBox)
			.OnCanAcceptDropAdvanced(this, &FLandscapeEditorCustomNodeBuilder_LayersBrushStack::HandleCanAcceptDrop)
		...
		const TSharedPtr<SWidget> BrushesListWidget = SNew(SVerticalBox)
			+ SVerticalBox::Slot()
			.AutoHeight()
		...
		// Display a disabled message when the current edit layer does not support BP brushes
		const FText DisabledToolTip = FText::Format(LOCTEXT("LandscapeEditLayerBlueprintBrushes_Disabled", "The type of layer {0} ({1}) doesn't support blueprint brushes."),
			FText::FromName(EditLayer->GetName()), EditLayer->GetClass()->GetDisplayNameText());

		const TSharedPtr<SWidget> ErrorMessage = SNew(SMultiLineEditableTextBox)
			.IsReadOnly(true)
		...
		ChildrenBuilder.AddCustomRow(LOCTEXT("LandscapeEditLayerBlueprintBrushes", "Edit Layer Blueprint Brushes"))
			.WholeRowContent()
			[
				EditLayer->SupportsBlueprintBrushes() ? BrushesListWidget.ToSharedRef() : ErrorMessage.ToSharedRef()
			];
...
FReply FLandscapeEditorCustomNodeBuilder_LayersBrushStack::HandleDragDetected(const FGeometry& MyGeometry, const FPointerEvent& MouseEvent, int32 SlotIndex, SVerticalBox::FSlot* Slot)
{
	if (FEdModeLandscape* LandscapeEdMode = GetEditorMode())
	{
		if (const ULandscapeEditLayerBase* EditLayer = LandscapeEdMode->GetCurrentEditLayerConst())
		{
			const TArray<ALandscapeBlueprintBrushBase*>& BrushStack = LandscapeEdMode->GetBrushesForCurrentLayer();
			if (!EditLayer->IsLocked() && BrushStack.IsValidIndex(SlotIndex))
			{
...
TOptional<SDragAndDropVerticalBox::EItemDropZone> FLandscapeEditorCustomNodeBuilder_LayersBrushStack::HandleCanAcceptDrop(const FDragDropEvent& DragDropEvent, SDragAndDropVerticalBox::EItemDropZone DropZone, int32 SlotIndex, SVerticalBox::FSlot* Slot)
{
...
FReply FLandscapeEditorCustomNodeBuilder_LayersBrushStack::HandleAcceptDrop(FDragDropEvent const& DragDropEvent, SDragAndDropVerticalBox::EItemDropZone DropZone, int32 SlotIndex, SVerticalBox::FSlot* Slot)
{
	...
	if (DragDropOperation.IsValid())
	{
		...
		if (Landscape)
		{
			...
			if (Landscape->ReorderLayerBrush(LandscapeEdMode->GetSelectedEditLayerIndex(), StartingBrushIndex, DestinationBrushIndex))
			{
