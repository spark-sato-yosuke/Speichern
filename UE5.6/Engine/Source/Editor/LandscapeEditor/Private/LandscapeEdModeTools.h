...
struct FXYOffsetmapAccessor
{
	...
	{
		// NOTE: GetXYOffsetData may shrink X1/Y1/X2/Y2 to reflect the sub-range that contains valid data
		LandscapeEdit->GetXYOffsetData(X1, Y1, X2, Y2, Data);

		// NOTE: GetHeightData may shrink X1/Y1/X2/Y2 to reflect the sub-range that contains valid data
		// In general, GetHeight should not further shrink the range, as anywhere the XYOffsets are valid, the Heights should also be valid.
	...
	void GetDataFast(int32 X1, int32 Y1, int32 X2, int32 Y2, TMap<FIntPoint, FVector>& Data)
	{
		...
		// NOTE: this can modify X1/Y1/X2/Y2 to reflect the region with valid height data, which may be a subset of the original range
		// We rely on the fact that generally anywhere the XYOffsets are valid, the Heights should also be valid
		// So in theory we should write the Z value for every XY Offset that was queried above.
		TMap<FIntPoint, uint16> NewHeights;
		LandscapeEdit->GetHeightData(X1, Y1, X2, Y2, NewHeights);
		for (int32 Y = Y1; Y <= Y2; ++Y)
		{
...
private:
	...
	{
		check(InLayerVisibility.Num() == Landscape->GetEditLayersConst().Num());
		for (int i = 0; i < InLayerVisibility.Num(); ++i)
		{
			if (ULandscapeEditLayerBase* EditLayer = Landscape->GetEditLayer(i))
			{
				if (EditLayer->IsVisible() != InLayerVisibility[i])
				{
					// Pass bInModify and bInBroadcastDataChange = false here, because we don't want to call Modify on the landscape, we don't actually want to truly change 
					//  the layer visibility, only temporarily for the purpose of the intermediate render : 
					EditLayer->SetVisible(InLayerVisibility[i], /*bInModify = */ false, /*bInBroadcastDataChange = */false);
