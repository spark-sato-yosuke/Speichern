...
void FLandscapeEditorDetailCustomization_ImportExport::CustomizeDetails(IDetailLayoutBuilder& DetailBuilder)
{
	...
	ImportExportModeRow.Visibility(MakeAttributeLambda([]() { return FLandscapeEditorDetailCustomization_ImportExport::GetImportExportVisibility(true); }));
...
FReply FLandscapeEditorDetailCustomization_ImportExport::OnImportExportButtonClicked()
{
	...
	if (ULandscapeInfo* LandscapeInfo = LandscapeEdMode->CurrentToolTarget.LandscapeInfo.Get())
	{
		FIntRect LandscapeExtent = LandscapeInfo->GetCompleteLandscapeExtent();
		...
		else
		{
			...
			// Reset the import gizmo location to landscape minx/miny
			LandscapeEdMode->UISettings->ImportLandscape_GizmoLocalPosition = LandscapeExtent.Min;
		}
	}
