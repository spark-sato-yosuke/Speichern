...
class FLandscapeBrushCircle : public FLandscapeBrush
{
	...
	// Components which previously were under the area of the brush.
...
protected:
	...
	FLandscapeBrushCircle(FEdModeLandscape* InEdMode, UMaterialInterface* InBrushMaterial)
		...
		, BrushMaterial(UE::Landscape::CreateToolLandscapeMaterialInstanceConstant(InBrushMaterial))
		, bCanPaint(false)
		, EdMode(InEdMode)
	{
	}
...
public:
	...
	{
		// bCanPaint is evaluated at an old mouse position, and also does not take interactor positions into account,
		// so cannot be relied upon to be correct vis-a-vis overlapping unloaded components.  We check those again below.
		if (!bCanPaint)
		{
		...
		// check if region to be modified overlaps with an unloaded component
		// TODO [chris.tchou] : The +1 shouldn't be necessary, but without it, edges can be modified by the brush. Possibly a bug in the brush apply code.
		bool bHasUnloadedComponents = LandscapeInfo->HasUnloadedComponentsInRegion(InclusiveBounds.Min.X, InclusiveBounds.Min.Y, InclusiveBounds.Max.X+1, InclusiveBounds.Max.Y+1);
		if (bHasUnloadedComponents)
		{
			return FLandscapeBrushData();
		}
...
public:
		...
		, EdMode(InEdMode)
	{
		...
		BrushMaterial = UE::Landscape::CreateToolLandscapeMaterialInstanceConstant(BaseBrushMaterial);
	}
...
public:
		...
		, EdMode(InEdMode)
	{
		...
		BrushMaterial = UMaterialInstanceDynamic::Create(UE::Landscape::CreateToolLandscapeMaterialInstanceConstant(GizmoMaterial), nullptr);
	}
...
public:
	...
	virtual void Tick(FEditorViewportClient* ViewportClient, float DeltaTime) override
	{
		...
		if (const ALandscapeProxy* Proxy = EdMode->CurrentToolTarget.LandscapeInfo.IsValid() ? EdMode->CurrentToolTarget.LandscapeInfo->GetLandscapeProxy() : nullptr)
		{
