...
struct FLandscapeTargetLayerAssetFilePath
{
	...
	UPROPERTY(Category = "Target Layers", EditAnywhere, NonTransactional, meta=(InlineEditConditionToggle))
	bool bUseAssetDirectoryPath = false;

	UPROPERTY(Category = "Target Layers", EditAnywhere, NonTransactional, meta = (EditCondition = bUseAssetDirectoryPath, LongPackageName))
	FDirectoryPath DirectoryPath;

	FLandscapeTargetLayerAssetFilePath()
		: bUseAssetDirectoryPath(false)
		, DirectoryPath()
	{
	}
};
...
public:
	...
	// The landscape layers that will be created via New tab. Only layer names referenced in the material assigned above are shown here. Modify the material to add more layers.
	...
	TArray<FLandscapeImportLayer> NewLandscape_Layers;
	...
	// Apply brush continuously while held stationary, after a small delay
	UPROPERTY(Category = "Brush Settings", EditAnywhere, NonTransactional, meta=(DisplayName = "Apply Without Moving", ShowForTargetTypes = "Heightmap", ShowForTools = "Sculpt,Erase,Smooth,Flatten,Erosion,HydraErosion,Noise,Mask,CopyPaste", ShowForBrushes = "BrushSet_Circle,BrushSet_Alpha,BrushSet_Gizmo,BrushSet_Pattern,BrushSet_Component"))
	bool bApplyWithoutMovingSculpt;

	// Apply brush continuously while held stationary, after a small delay
	UPROPERTY(Category = "Brush Settings", EditAnywhere, NonTransactional, meta=(DisplayName = "Apply Without Moving", ShowForTargetTypes = "Weightmap", ShowForTools = "Paint,Smooth,Flatten,Noise,Mask", ShowForBrushes = "BrushSet_Circle,BrushSet_Alpha,BrushSet_Pattern,BrushSet_Component"))
	bool bApplyWithoutMovingPaint;   // Internally, store a separate "Apply without Moving" value for paint mode
	...
	void RefreshImportLayersList(bool bRefreshFromTarget = false, bool bRegenerateThumbnails = false);
	void AutoFillTargetLayers(const FString& DefaultTargetLayerAssetPath, const bool bCreateAssetForEmptyLayers);
