...
UObject* USoundCueFactoryNew::FactoryCreateNew(UClass* Class, UObject* InParent, FName Name, EObjectFlags Flags, UObject* Context, FFeedbackContext* Warn)
{
	...
	if (SoundObjects.Num() > 0)
	{
		...
		else
		{
			...
			if (SoundObjects.Num() > RandomNode->GetMaxChildNodes())
			{
				UE_LOG(LogAudio, Warning, TEXT("RandomNode is limited to %d input Sound Sources. Excess Sound Sources in SoundCue %s will be left unconnected."), RandomNode->GetMaxChildNodes(), *Name.ToString());
			}
