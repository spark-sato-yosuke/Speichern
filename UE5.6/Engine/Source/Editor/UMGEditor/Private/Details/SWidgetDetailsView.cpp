...
{
	if (IsWidgetCDOSelected() || SelectedObjects.Num() > 1)
	{
...
FReply SWidgetDetailsView::OnAddComponentButtonClicked(TSharedPtr<FWidgetBlueprintEditor> InBlueprintEditor)
{
	...
	if (ChosenClass)
	{
		...
		{
			if (const UWidget* SelectedWidget = Cast<UWidget>(SelectedObject.Get()))
			{
				FUIComponentUtils::AddComponent(InBlueprintEditor.ToSharedRef(), ChosenClass, SelectedWidget->GetFName());
			}
		}
	}
