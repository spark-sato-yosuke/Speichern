...
public:
	...
	static void OpenLegacyProjectLauncher()
	{
		FGlobalTabmanager::Get()->TryInvokeTab(FTabId("LegacyProjectLauncher"));
	}
	...
	static void CookOrPackage(FName IniPlatformName, EPrepareContentMode Mode)
	{
		...
		if (PackagingSettings->GetUseZenStoreEffective())
		{
		...
		FString Architecture = PlatformsSettings->GetArchitectureForPlatform(IniPlatformName);
		if (!Architecture.IsEmpty())
		{
			switch (BuildTargetInfo->Type)
			{
				case EBuildTargetType::Server:
				{
					BuildCookRunParams += FString::Printf(TEXT(" -serverarchitecture=%s"), *Architecture);
				}
				break;

				case EBuildTargetType::Editor:
				{
					BuildCookRunParams += FString::Printf(TEXT(" -editorarchitecture=%s"), *Architecture);
				}
				break;

				default:
				{
					BuildCookRunParams += FString::Printf(TEXT(" -clientarchitecture=%s"), *Architecture);
				}
				break;
			}
		}
	...
	static void SetActiveArchitecture(const PlatformInfo::FTargetPlatformInfo* VanillaInfo, FString Architecture)
	{
		UPlatformsMenuSettings* PlatformsSettings = GetMutableDefault<UPlatformsMenuSettings>();
		PlatformsSettings->SetArchitectureForPlatform(VanillaInfo->IniPlatformName, Architecture);
		PlatformsSettings->SaveConfig();
	}

	static bool CanSetActiveArchitecture(const PlatformInfo::FTargetPlatformInfo* Info, FString Architecture)
	{
		return true;
	}

	static bool SetActiveArchitectureIsChecked(const PlatformInfo::FTargetPlatformInfo* Info, FString Architecture)
	{
		return GetDefault<UPlatformsMenuSettings>()->GetArchitectureForPlatform(Info->IniPlatformName) == Architecture;
	}
...
{
	FString DeviceId = DeviceProxy ? DeviceProxy->GetTargetDeviceId(DeviceProxy->GetVanillaPlatformId(NAME_None)) : FString();
	...
	else
	{
			...
			Platform->SupportsFeature(ETargetPlatformFeatures::SupportsMultipleConnectionTypes))
		{
			...
			else
			{
				...
				Icon = FSlateIcon(FAppStyle::Get().GetStyleSetName(), FDataDrivenPlatformInfoRegistry::GetPlatformInfo(IniPlatformName).GetIconStyleName(EPlatformIconSize::Normal));
			}
...
// Special fake UUID used to run iOS app natively on local Apple Silicon mac, see also FIOSDeviceHelper::LocalMacDummyUUID
static const FString LocalMacDummyUUID = TEXT("10CA18AC-10CA18AC10CA18AC");
...
static void FormatSdkInfo(const FString& PlatformOrDevice, const FTurnkeySdkInfo& SdkInfo, FText& OutInfo, FText& OutToolTip)
{
	...
	if (PlatformOrDevice == TEXT("IOS@") + LocalMacDummyUUID)
	{
		OutInfo = LOCTEXT("SdkInfo_LocalMac", "Run native iOS app on local Mac");
		OutToolTip = LOCTEXT("SdkInfo_LocalMacToolTip", "Use this device to run native iOS app on local Mac. It is still built for iOS platform");
		return;
	}
#endif
...
static void MakeTurnkeyPlatformMenu(UToolMenu* ToolMenu, FName IniPlatformName, ITargetDeviceServicesModule* TargetDeviceServicesModule)
{
	...
	const ITargetPlatform* TargetPlatform = GetTargetPlatformManager()->FindTargetPlatform(IniPlatformName);
	...
	if (VanillaInfo != nullptr)
	{
		...
		const UProjectPackagingSettings::FConfigurationInfo& DefaultConfigInfo = UProjectPackagingSettings::ConfigurationInfo[static_cast<int32>(PackagingSettings->BuildConfiguration)];
		ConfigSection.AddMenuEntry(
			NAME_None,
			FText::Format(LOCTEXT("DefaultConfiguration",  "Use Project Setting ({0})"), DefaultConfigInfo.Name),
			DefaultConfigInfo.ToolTip,
			FSlateIcon(),
			FUIAction(
		...
		{
			// gather all possible architectures
			TArray<FString> Architectures;
			TargetPlatform->GetPossibleArchitectures(Architectures);
			
			if (Architectures.Num() > 1)
			{
				FToolMenuSection& ArchitectureSection = ToolMenu->AddSection("ArchitectureSelection", LOCTEXT("TurnkeySection_ArchitectureSelection", "Architecture Selection"));

				ArchitectureSection.AddMenuEntry(
					NAME_None,
					LOCTEXT("DefaultArchitectureName", "Project Default"),
					FText(),
					FSlateIcon(),
					FUIAction(
						FExecuteAction::CreateStatic(&FTurnkeySupportCallbacks::SetActiveArchitecture, VanillaInfo, FString()),
						FCanExecuteAction::CreateStatic(&FTurnkeySupportCallbacks::CanSetActiveArchitecture, VanillaInfo, FString()),
						FIsActionChecked::CreateStatic(&FTurnkeySupportCallbacks::SetActiveArchitectureIsChecked, VanillaInfo, FString())
					),
					EUserInterfaceActionType::RadioButton
				);

				for (const FString& Architecture : Architectures)
				{
					bool bIsHostArchitecture = (TargetPlatform->IniPlatformName() == FPlatformProperties::IniPlatformName()) && (Architecture == TargetPlatform->GetHostArchitecture());

					FText ArchLabel = bIsHostArchitecture 
						? FText::Format(LOCTEXT("HostArchLabel", "{0} (this platform)"), FText::FromString(Architecture)) 
						: FText::FromString(Architecture);
					...
					ArchitectureSection.AddMenuEntry(
						...
						FSlateIcon(),
						FUIAction(
							FExecuteAction::CreateStatic(&FTurnkeySupportCallbacks::SetActiveArchitecture, VanillaInfo, Architecture),
							FCanExecuteAction::CreateStatic(&FTurnkeySupportCallbacks::CanSetActiveArchitecture, VanillaInfo, Architecture),
							FIsActionChecked::CreateStatic(&FTurnkeySupportCallbacks::SetActiveArchitectureIsChecked, VanillaInfo, Architecture)
						),
						EUserInterfaceActionType::RadioButton
					);
				}
...
static void LaunchOnDevice(const FString& DeviceId, const FString& DeviceName, bool bUseTurnkey, FStringView Architecture)
{
	FTurnkeyEditorSupport::LaunchRunningMap(DeviceId, DeviceName, GetProjectPathForTurnkey(), bUseTurnkey, false, Architecture);
}

static void LaunchOnSimulator(const FString& DeviceId, const FString& DeviceName, bool bUseTurnkey, FStringView Architecture)
{								
	FTurnkeyEditorSupport::LaunchRunningMap(DeviceId, DeviceName, GetProjectPathForTurnkey(), false, true, Architecture);
}

static void PrepareLaunchOn(const FString& DeviceId, const FString& DeviceName)
{
	FTurnkeyEditorSupport::PrepareToLaunchRunningMap(DeviceId, DeviceName);
}

static void HandleLaunchOnDeviceActionExecute(const FString& DeviceId, const FString& DeviceName, bool bUseTurnkey, FStringView Architecture)
{
	PrepareLaunchOn(DeviceId, DeviceName);
	LaunchOnDevice(DeviceId, DeviceName, bUseTurnkey, Architecture);
}

static void HandleLaunchOnSimulatorActionExecute(const FString& DeviceId, const FString& DeviceName, bool bUseTurnkey, FStringView Architecture)
{
	LaunchOnSimulator(DeviceId, DeviceName, bUseTurnkey, Architecture);
}
