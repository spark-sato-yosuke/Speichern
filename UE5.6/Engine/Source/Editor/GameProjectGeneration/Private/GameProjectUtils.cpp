...
namespace
{
	...
	void AddUserInterfaceEngineConfigValues(const FProjectInformation& InProjectInfo, TArray<FTemplateConfigValue>& ConfigValues)
	{
		if (InProjectInfo.bIsBlankTemplate)
		{
			ConfigValues.Emplace(TEXT("DefaultEngine.ini"),
	...
	void AddUserInterfaceGameConfigValues(const FProjectInformation& InProjectInfo, TArray<FTemplateConfigValue>& ConfigValues)
	{
		if (InProjectInfo.bIsBlankTemplate)
		{
			ConfigValues.Emplace(TEXT("DefaultGame.ini"),
				TEXT("/Script/CommonUI.CommonUISettings"),
				TEXT("CommonButtonAcceptKeyHandling"),
				TEXT("TriggerClick"),
				true /* ShouldReplaceExistingValue */);
		}
	}
	...
	if (TemplateDefs != NULL)
	{
		...
		if (!TemplateDefs->Variants.IsEmpty() && InProjectInfo.Variant != NAME_None)
		{
			...
			if (!Variant)
			{
				FFormatNamedArguments Args;
				Args.Add(TEXT("TemplateName"), FText::FromString(SrcFolder));
				OutFailReason = FText::Format(LOCTEXT("UnknownVariantError", "Unknown variant specified for '{TemplateName}'."), Args);
				return false;
			}
			...
			if( bVariantCopied == false )
			{
				...
				Args.Add(TEXT("TemplateName"), FText::FromString(SrcFolder));
				OutFailReason = FText::Format(LOCTEXT("VariantSharedResourceError", "Error adding variant '{VariantName}' shared resources for '{TemplateName}'."), Args);
				return false;
			}
		}
	}
