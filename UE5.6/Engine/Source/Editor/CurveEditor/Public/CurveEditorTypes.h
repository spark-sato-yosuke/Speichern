...
{
	/** Default constructor is needed by e.g. TArray::operator<< */
	FCurveModelID() : ID(0) {}
	...
	/**
	 * @return Whether this ID is valid (was constructed with Unique)
	 */
	...
	/**
	 * Save a curve model ID
	 */
	FORCEINLINE friend FArchive& operator<<(FArchive& Ar, FCurveModelID& Handle)
	{
		Ar << Handle.ID;
		return Ar;
	}
