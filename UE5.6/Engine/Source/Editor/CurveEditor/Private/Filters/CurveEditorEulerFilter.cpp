...
bool UCurveEditorEulerFilter::CanApplyFilter_Impl(TSharedRef<FCurveEditor> InCurveEditor)
{
	const TMap<FCurveModelID, FKeyHandleSet>& Selection = InCurveEditor->GetSelection().GetAll();
	const bool bHasRotationCurves = !Selection.IsEmpty()
		&& Algo::AnyOf(InCurveEditor->GetSelection().GetAll(),
		[&InCurveEditor](const TPair<FCurveModelID, FKeyHandleSet>& Pair)
		{
			const FCurveModel* Curve = InCurveEditor->FindCurve(Pair.Key);
			return Curve && Curve->GetIntentionName().Contains("Rotation");
		});
	return bHasRotationCurves;
}
