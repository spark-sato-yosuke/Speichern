...
void UAnimationModifier::ApplyToAnimationSequence(UAnimSequence* AnimSequence) const
{
	FEditorScriptExecutionGuard ScriptGuard;

	checkf(AnimSequence, TEXT("Invalid Animation Sequence supplied"));

	//Make sure Anim sequence compression is done
...
{
	if (CurrentAnimSequence->IsCompressedDataOutOfDate())
	{
	...
	if (UClass* ModifierClass = ModifierSubClass.Get())
	{
		...
		for (UAnimationModifiersAssetUserData* AssetUserData : TObjectRange<UAnimationModifiersAssetUserData>{})
		{
			...
			if (AnimSequence)
			{
				...
				ScopedBracket = MakeUnique<IAnimationDataController::FScopedBracket>(Controller, LOCTEXT("ApplyingAnimationModifier", "Applying Animation Modifiers"));
			}
