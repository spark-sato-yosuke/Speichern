...
SGraphPin::SGraphPin()
	...
	, CachedNodeOffset(FVector2f::ZeroVector)
...
void SGraphPin::Construct(const FArguments& InArgs, UEdGraphPin* InPin)
{
	...
	else
	{
		...
		if (ValueWidget != SNullWidget::NullWidget)
		{
			...
			LabelAndValue->AddSlot()
				.Padding(bIsInput ? FMargin(InArgs._SideToSideMargin, 0.0f, 0.0f, 0.0f) : FMargin(0.0f, 0.0f, InArgs._SideToSideMargin, 0.0f))
				.VAlign(VAlign_Center)
				[
...
FReply SGraphPin::OnDrop( const FGeometry& MyGeometry, const FDragDropEvent& DragDropEvent )
{
	...
	if (Operation->IsOfType<FGraphEditorDragDropAction>() && GetIsConnectable())
	{
		...
		if (OwnerNode.IsValid())
		{
			...
			{
				FVector2f PinOffsetPosition = MyGeometry.AbsolutePosition - NodeWidget->GetTickSpaceGeometry().AbsolutePosition;
...
{
	CachedNodeOffset = AllottedGeometry.AbsolutePosition / AllottedGeometry.Scale - UE::Slate::CastToVector2f(OwnerNodePtr.Pin()->GetUnscaledPosition());
...
UE::Slate::FDeprecateVector2DResult SGraphPin::GetNodeOffset() const
{
...
{
	FVector2f OutDesiredLocation = UE::Slate::CastToVector2f(InOutDesiredLocation);
	TSharedPtr<SGraphNode> OwnerNode = OwnerNodePtr.Pin();
	if (OwnerNode.IsValid())
	{
		...
		if (GraphPanel.IsValid())
		{
			...
			OutDesiredLocation = OwnerNode->GetPosition2f() + CachedNodeOffset;

			// Shift the desired location to the right edge of the pin's geometry.
			OutDesiredLocation.X += GetTickSpaceGeometry().Size.X;
			...
			if (Inspector.IsValid() && Inspector->ValueInspectorWidget.IsValid())
			{
				...
				{
					OutDesiredLocation.Y -= VerticalOffsetWithSearchFilter;
				}
				else
				{
					OutDesiredLocation.Y -= VerticalOffsetWithoutSearchFilter;
				}
			}

			// Convert our desired location from graph coordinates into panel space.
			OutDesiredLocation -= GraphPanel->GetViewOffset();
			OutDesiredLocation *= GraphPanel->GetZoomAmount();

			// Finally, convert the modified location from panel space into screen space.
			OutDesiredLocation = GraphPanel->GetTickSpaceGeometry().LocalToAbsolute(OutDesiredLocation);
		}
	}

	InOutDesiredLocation = FVector2D(OutDesiredLocation);
}

void SGraphPin::GetInteractiveTooltipLocation(FVector2f& InOutDesiredLocation) const
{
PRAGMA_DISABLE_DEPRECATION_WARNINGS
	FVector2D TempVector2f = FVector2D(InOutDesiredLocation);
	GetInteractiveTooltipLocation(TempVector2f);
	InOutDesiredLocation = UE::Slate::CastToVector2f(TempVector2f);
