...
void SPinComboBox::Construct( const FArguments& InArgs )
{
	...
	[
		...
		[
			...
			.Padding(0.0f) // No padding here so that the inner box controls matching width with padding
			[
				SNew(SBox)
				.MaxDesiredHeight(450.0f)
				.MinDesiredWidth(this, &SPinComboBox::GetMenuDesiredWidth)
				.Padding(3.0f, 6.0f)
				[
...
FOptionalSize SPinComboBox::GetMenuDesiredWidth() const
{
	if (ComboButton)
	{
		return ComboButton->GetDesiredSize().X;
	}
	return FOptionalSize(150.0f);
}
