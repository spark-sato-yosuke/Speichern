...
FVector2f GetNodeSize(const SGraphEditor& GraphEditor, const UEdGraphNode* Node)
{
	FSlateRect Rect;
...
UE::Slate::FDeprecateVector2DResult SGraphEditorImpl::GetPasteLocation2f() const
{
...
void SGraphEditorImpl::Construct( const FArguments& InArgs )
{
	...
	OnCreateActionMenuAtLocation = InArgs._GraphEvents.OnCreateActionMenuAtLocation;
	...
	[
		...
		[
			...
			.OnDropActors(InArgs._GraphEvents.OnDropActors)
			.OnDropStreamingLevels( InArgs._GraphEvents.OnDropStreamingLevels )
			.OnVerifyTextCommit( InArgs._GraphEvents.OnVerifyTextCommit )
			.OnTextCommitted( InArgs._GraphEvents.OnTextCommitted )
			.OnSpawnNodeByShortcutAtLocation( InArgs._GraphEvents.OnSpawnNodeByShortcutAtLocation)
	...
	GraphPanel->RestoreViewSettings(FVector2f::ZeroVector, -1.0f);
				...
				struct SCommentUtility
				{
					static void CreateComment(const UEdGraphSchema* Schema, UEdGraph* Graph)
					{
						if (Schema && Graph)
						{
							...
							if (Action.IsValid())
							{
								Action->PerformAction(Graph, nullptr, FVector2f());
							}
						}
					}
				};
...
void SGraphEditorImpl::RegisterContextMenu(const UEdGraphSchema* Schema, FToolMenuContext& MenuContext) const
{
	...
	if (bDidRegisterGraphSchemaMenu)
	{
		...
		{
			Menu->AddDynamicSection("SubstrateRootActions", FNewToolMenuDelegate::CreateLambda([](UToolMenu* InMenu)
			{
				if (UGraphNodeContextMenuContext * ContextObject = InMenu->FindContext<UGraphNodeContextMenuContext>())
				{
					if (const UEdGraphSchema* GraphSchema = ContextObject->Graph->GetSchema())
					{
						if (ContextObject->Node->IsA<UMaterialGraphNode_Root>() && !ContextObject->Pin)
						{
							const UMaterialGraphNode_Root* RootNode = Cast<UMaterialGraphNode_Root>(ContextObject->Node);

							TSharedRef<SButton> SubstrateConvertButton = SNew(SButton)
								.Text(LOCTEXT("SubstrateConvertButtonLabel", "\x26A0 Convert To Substrate \x26A0"))
								.ToolTipText(LOCTEXT("SubstrateConvertButtonToolTip", "Convert the root node to a Substrate graph. This material will then use Substrate more advanced features and topology processes.\nTo go back to the previous Non-Substrate material, you can use CTRL+Z or you will need to manually relink the graph."))
								.ButtonStyle(FAppStyle::Get(), "Button")
								.OnClicked_Lambda([RootNode]()
								{
									if (RootNode && RootNode->Material)
									{
										RootNode->Material->UserRequestsConvertMaterialToSubstrateMaterial();
									}
									return FReply::Handled();
								});
						
							FToolMenuSection& SubstrateSection = InMenu->AddSection("Substrate", LOCTEXT("SubstrateMenuHeader", "Substrate"));
							SubstrateSection.AddEntry(FToolMenuEntry::InitWidget("NodeCommentBox", SubstrateConvertButton, FText::GetEmpty()));
						}
					}
				}
			}));
...
FActionMenuContent SGraphEditorImpl::GraphEd_OnGetContextMenuFor(const FGraphContextMenuArguments& SpawnInfo)
{
	...
	if (EdGraphObj != NULL)
	{
		...
		else if (IsEditable.Get())
		{
			...
			{
				if(OnCreateActionMenuAtLocation.IsBound())
				{
					Result = OnCreateActionMenuAtLocation.Execute(
						EdGraphObj, 
...
void SGraphEditorImpl::SetViewLocation( const FVector2f& Location, float ZoomAmount, const FGuid& BookmarkId ) 
{
...
void SGraphEditorImpl::GetViewLocation( FVector2f& Location, float& ZoomAmount ) 
{
	if( GraphPanel.IsValid() &&  EdGraphObj && (!IsLocked() || !GraphPanel->HasDeferredObjectFocus()))
	{
...
void DistributeNodes(TArray<FAlignmentData>& InData, bool bIsHorizontal)
{
	...
	InData.Sort([](const FAlignmentData& A, const FAlignmentData& B) {
		return A.TargetProperty + FMath::RoundToInt(A.TargetOffset / 2) < B.TargetProperty + FMath::RoundToInt(B.TargetOffset / 2);
	});
	...
	const float SpaceToDistributeIn = static_cast<float>(InData.Last().TargetProperty) - InData[0].GetTarget();
	const float PaddingAmount = ((SpaceToDistributeIn - TotalWidthOfNodes) / static_cast<float>(InData.Num() - 1));
	...
	if (InData.Num() > 1)
	{
		...
		if (Graph)
		{
			...
			if (Schema)
			{
				for (int32 Index = 1; Index < InData.Num() - 1; ++Index)
				{
					...
					FVector2f Target2DPosition = Entry.Node->GetPosition();
		...
		for (int32 Index = 1; Index < InData.Num() - 1; ++Index)
		{
			...
			Entry.Node->Modify();
			Entry.TargetProperty = UE::LWC::FloatToIntCastChecked<int32>(TargetPosition);
...
/** Distribute the specified array of node data evenly and align left when aligning vertically, and align top when aligning horizontally */
void StackNodes(TArray<FAlignmentData>& InData, bool bIsHorizontal)
{
	// Sort the data
	InData.Sort([](const FAlignmentData& A, const FAlignmentData& B) {
	...
	float TargetPosition = static_cast<float>(InData[0].TargetProperty);
	float StackAxisPosition = 0.0f;
	float PaddingAmount = static_cast<float>(SNodePanel::GetSnapGridSize());

	if (bIsHorizontal)
	{
		StackAxisPosition = InData[0].Node->GetNodePosY();
		PaddingAmount *= 2.0f;
	}
	else
	{
		StackAxisPosition = InData[0].Node->GetNodePosX();
	}
	...
	if (InData.Num() > 1)
	{
		...
		if (Graph)
		{
			...
			if (Schema)
			{
				for (int32 Index = 0; Index < InData.Num(); ++Index)
				{
					...
					FVector2f AdjustedTargetPosition = Entry.Node->GetPosition();

					if (bIsHorizontal)
					{
						AdjustedTargetPosition.X = TargetPosition;
						AdjustedTargetPosition.Y = StackAxisPosition;
					}
					else
					{
						AdjustedTargetPosition.X = StackAxisPosition;
						AdjustedTargetPosition.Y = TargetPosition;
					}

					Schema->SetNodePosition(Entry.Node, AdjustedTargetPosition);
		...
		// fall back to the old approach if there isn't a schema
		for (int32 Index = 0; Index < InData.Num() - 1; ++Index)
		{
			FAlignmentData& Entry = InData[Index];
...
void SGraphEditorImpl::OnStackNodesH()
{
	TArray<FAlignmentData> AlignData;
	for (UObject* It : GetSelectedNodes())
	{
	...
	if (AlignData.Num() > 1)
	{
		const FScopedTransaction Transaction(FGraphEditorCommands::Get().StackNodesHorizontally->GetLabel());
		StackNodes(AlignData, true);
	}
}

void SGraphEditorImpl::OnStackNodesV()
{
	TArray<FAlignmentData> AlignData;
	for (UObject* It : GetSelectedNodes())
	{
	...
	if (AlignData.Num() > 1)
	{
		const FScopedTransaction Transaction(FGraphEditorCommands::Get().StackNodesVertically->GetLabel());
		StackNodes(AlignData, false);
	}
}
