...
public:
	...
	UE::Slate::FDeprecateVector2DResult GetShadowDeltaSize() const;
...
public:
	...
	UPROPERTY(config, EditAnywhere, Category=Splines, AdvancedDisplay)
	FDeprecateSlateVector2D ForwardSplineTangentFromHorizontalDelta;

	/** The amount that the vertical delta affects the generated tangent handle of splines (when the wire is moving forward) */
	UPROPERTY(config, EditAnywhere, Category=Splines, AdvancedDisplay)
	FDeprecateSlateVector2D ForwardSplineTangentFromVerticalDelta;
	...
	UPROPERTY(config, EditAnywhere, Category=Splines, AdvancedDisplay)
	FDeprecateSlateVector2D BackwardSplineTangentFromHorizontalDelta;

	/** The amount that the vertical delta affects the generated tangent handle of splines (when the wire is moving backwards) */
	UPROPERTY(config, EditAnywhere, Category=Splines, AdvancedDisplay)
	FDeprecateSlateVector2D BackwardSplineTangentFromVerticalDelta;

	/** Compute a tangent given the start and end points of the spline */
	UE::Slate::FDeprecateVector2DResult ComputeSplineTangent(const UE::Slate::FDeprecateVector2DParameter& Start, const UE::Slate::FDeprecateVector2DParameter& End) const;
