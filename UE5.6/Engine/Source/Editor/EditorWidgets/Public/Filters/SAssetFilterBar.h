...
namespace UE::Editor::Widgets
{
	...
	struct EDITORWIDGETS_API FFilterCategoryMenu
	{
		...
		FFilterCategoryMenu(const FName& InSectionExtensionHook, const FText& InSectionHeading);
	};

	EDITORWIDGETS_API TMap<TSharedPtr<FFilterCategory>, FFilterCategoryMenu> BuildCategoryToMenuMap(
		TMap<FName, TSharedPtr<FFilterCategory>>& InAssetFilterCategories,
		TConstArrayView<TSharedRef<FCustomClassFilterData>> InCustomClassFilters,
		const FOnFilterAssetType& InOnFilterAssetType);
}
...
public:
	...
	virtual void SaveSettings()
	{
		...
		SaveSettingsInternal(FilterBarConfig);
	}
...
protected:
	...
	virtual void SaveSettingsInternal(FFilterBarSettings* InSettingsToSave)
	{
		check(InSettingsToSave);

		SaveCustomTextFilters(InSettingsToSave);
		SaveFilters(InSettingsToSave);
	...
	void LoadFilters(const FFilterBarSettings* FilterBarConfig)
	{
		...
		for(const TSharedRef<FCustomClassFilterData> &CustomClassFilter : this->CustomClassFilters)
		{
			...
			{
				// If the filter is already in use, we don't want to add a duplicate but it isn't unknown - so remove it from the UnknownTypeFilters
				UnknownTypeFilters.Remove(FilterName);
			}
		}
	}
...
protected:
	...
	void OnCopyFilters() const
	{
		if (const FFilterBarSettings* SettingsToCopyFrom = GetConstConfig())
		{
			FString Output = TEXT("");
			FFilterBarSettings::StaticStruct()->ExportText(Output, SettingsToCopyFrom, nullptr, nullptr, (PPF_ExportsNotFullyQualified | PPF_Copy | PPF_Delimited | PPF_IncludeTransient), nullptr);
	...
	void OnPasteFilters()
	{
		if (FFilterBarSettings* SettingsToPasteTo = GetMutableConfig())
		{
			FString Output = TEXT("");
			FPlatformApplicationMisc::ClipboardPaste(Output);

			FFilterBarSettings::StaticStruct()->ImportText(*Output, SettingsToPasteTo, nullptr, 0, GLog, GetPathNameSafe(FFilterBarSettings::StaticStruct()));
	...
	bool CanPasteFilters() const
	{
		...
		FCustomizableTextObjectFactory Factory(GWarn);
		if (!Factory.CanCreateObjectsFromText(Output))
		{
			return false;
		}

		FFilterBarSettings Value;
		return FFilterBarSettings::StaticStruct()->ImportText(*Output, &Value, nullptr, 0, GLog, GetPathNameSafe(FFilterBarSettings::StaticStruct())) != nullptr;
	}
