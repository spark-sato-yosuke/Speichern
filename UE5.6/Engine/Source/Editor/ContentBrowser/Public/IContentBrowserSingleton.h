...
struct FContentBrowserConfig
{
	...
	FCollectionRef SelectedCollection;

	UE_DEPRECATED(5.6, "Use SelectedCollection instead.")
	...
	FContentBrowserConfig()
		...
		, SelectedCollection(nullptr, NAME_None, ECollectionShareType::CST_Local)
	...
	PRAGMA_DISABLE_DEPRECATION_WARNINGS
		FContentBrowserConfig& operator=(const FContentBrowserConfig&) = default;
	PRAGMA_ENABLE_DEPRECATION_WARNINGS
};
...
struct FAssetPickerConfig
{
	...
	/** The names of columns to hide by default in the list view, by default the Type column is hidden */
	...
	TArray<FCollectionRef> CollectionsFilter;

	UE_DEPRECATED(5.6, "Use CollectionsFilter instead.")
	...
	/** If set, will use this name for filter bar saving purposes instead of the SaveSettingsName. */
	...
	FAssetPickerConfig()
		...
		, ListHiddenColumnNames( { TEXT("Class") } )
		, ThumbnailLabel( EThumbnailLabel::ClassName )
		, ThumbnailScale(0.1f)
		...
		, bUseSectionsForCustomFilterCategories(false)
	{
		...
		{
			// Default size in the new style AssetPicker will be Large
	...
	FAssetPickerConfig(const FAssetPickerConfig&) = default;
	FAssetPickerConfig(FAssetPickerConfig&&) = default;

	FAssetPickerConfig& operator=(const FAssetPickerConfig&) = default;
	FAssetPickerConfig& operator=(FAssetPickerConfig&&) = default;
...
struct FCollectionPickerConfig
{
	...
	FCollectionPickerConfig()
		...
		, CollectionContainer(FCollectionManagerModule::GetModule().Get().GetProjectCollectionContainer())
...
public:
	...
	*/
	...
	/** Returns the Internal Content Permission List */
	virtual const TSharedPtr<FPathPermissionList>& GetShowEpicInternalContentPermissionList() = 0;
	...
	/** Adds a Custom Text Filter to the Filter List */
	virtual void AddCustomTextFilter(const FCustomTextFilterData& FilterData, bool bApplyFilter) = 0;

	/** Deletes the specified Custom Text Filter */
	virtual void DeleteCustomTextFilterByLabel(const FText& FilterLabel) = 0;

	/** Modifies the specified Custom Text Filter */
	virtual void ModifyCustomTextFilterByLabel(const FCustomTextFilterData& FilterData, const FText& FilterLabel) = 0;

	/** Gets whether the AssetView in the Content Browser is done filtering */
	virtual bool IsAssetViewDoneFiltering() = 0;

	/** Sets the AssetSearchBox text */
	virtual void SetSearchText(const FText& NewText) = 0;

	/** Toggle the Favorite status of the given folder list */
	virtual void ToggleFolderFavorite(const TArray<FString>& FolderPaths) = 0;

	/** Sets the Content Browser filter layout to either vertical or horizontal */
	virtual void SetFilterLayout(EFilterBarLayout InFilterBarLayout) = 0;

	/** Gets the current Content Browser filter layout */
	virtual EFilterBarLayout GetFilterLayout() = 0;

	/** Gets a pointer to the active filter container widget (Horizontal or Vertical), nullptr if neither is active */
	virtual TSharedPtr<SWidget> GetActiveFilterContainer() = 0;

	/** Docks the current content browser drawer as a tabbed content browser in a layout */
	virtual void DockContentBrowserDrawer() = 0;

	/** Factory delegate that instantiates a content source  */
	DECLARE_DELEGATE_RetVal(TSharedRef<UE::Editor::ContentBrowser::IContentSource>, FContentSourceFactory)

	/** Register a content source by name, causing it to appear in the content sources bar as selectable (if the bar is enabled)  */
	virtual void RegisterContentSourceFactory(const FName& InContentSourceName, const FContentSourceFactory& InContentSourceFactory) = 0;

	/** Unregister a content source by name */
	virtual void UnregisterContentSourceFactory(const FName& InContentSourceName) = 0;

	/** Execute a delegate for each registered content source factory */
	virtual void ForEachContentSourceFactory(const TFunctionRef<void(const FName&, const FContentSourceFactory&)>& InFunction) = 0;

	/** Delegate fired when a new content source factory is registered/unregistered */
	DECLARE_MULTICAST_DELEGATE(FOnContentSourceFactoriesChanged)
	virtual FOnContentSourceFactoriesChanged& OnContentSourceFactoriesChanged() = 0;
};
