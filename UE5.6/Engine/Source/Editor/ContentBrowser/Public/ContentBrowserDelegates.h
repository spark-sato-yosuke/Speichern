...
DECLARE_MULTICAST_DELEGATE_OneParam(FOnDeleteFolders, const TArray<FContentBrowserItemPath>& /*PathsToDelete*/);
...
struct FAssetViewDragAndDropExtender
{
	struct FPayload
	{
		FPayload(TSharedPtr<FDragDropOperation> InDragDropOp, const TArray<FName>& InPackagePaths, const TArray<FCollectionRef>& InCollectionSources);
	...
	private:
		TArray<FCollectionRef> TempCollectionSources;
		TArray<FCollectionNameType> TempCollections;

	public:
		const TArray<FName>& PackagePaths;
		const TArray<FCollectionRef>& CollectionSources;
