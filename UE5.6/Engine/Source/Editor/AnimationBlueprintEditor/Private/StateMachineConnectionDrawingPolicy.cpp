...
void FStateMachineConnectionDrawingPolicy::Internal_DrawLineWithArrow(const FVector2f& StartAnchorPoint, const FVector2f& EndAnchorPoint, const FConnectionParams& Params)
{
	...
	{
		StartHovered = (StartPoint - LocalMousePosition).Length() < RelinkHandleHoverRadius;
		EndHovered = (EndPoint - LocalMousePosition).Length() < RelinkHandleHoverRadius;
		FVector2f HoverIndicatorPosition = StartHovered ? StartPoint : EndPoint;
		...
		if (RelinkConnections.IsEmpty() && Params.bUserFlag2)
		{
			...
			else
			{
				...
				const FVector2f CircleCenter = EndPoint - UnitDelta * 2.0f;
