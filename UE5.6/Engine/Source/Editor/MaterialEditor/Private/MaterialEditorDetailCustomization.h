...
public:
	...
	/** Makes a new instance of this detail layout class for a specific detail view requesting it */
	static TSharedRef<class IDetailCustomization> MakeInstance(bool bInIncludeScalars);

	/** Constructor requires a delegate to populate group names with */
	FMaterialExpressionCollectionParameterDetails(bool bInIncludeScalars);
...
private:
	/** Populates the group name that this parameter details uses */
	void PopulateParameters(const FString& FilterText);
...
private:
	...
	/** Text entry to filter parameters */
	TSharedPtr<class SSearchBox> ParameterSearchBox;

	/** A list of all parameter names to choose from */
	TArray<TSharedPtr<FString>> ParametersSource;

	/** Whether to include scalars in this detail customization */
	bool bIncludeScalars;
};
...
/**
 * Customizes the details of a Color Ramp node, adding a gradient slider to the panel
 */
class FMaterialExpressionColorRampCustomization : public IDetailCustomization
{
public:
