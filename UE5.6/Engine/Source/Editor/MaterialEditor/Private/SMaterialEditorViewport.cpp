...
TSharedPtr<SWidget> SMaterialEditor3DPreviewViewport::BuildViewportToolbar()
{
	...
	// clang-format off
	const TSharedRef<SWidget> NewViewportToolbar =
		SNew(SBox)
		// We need this visibility override because the material editor doesn't return its old viewport toolbar
		// via the MakeViewportToolbar override, but adds it manually to the viewport overlay.
		// Therefore, the central code that controls visibility of the old and new viewport toolbars get confused,
		// and we must set the visibility override here.
		.Visibility_Lambda(
		[this]() -> EVisibility
		{
...
void SMaterialEditor3DPreviewViewport::PopulateViewportOverlays(TSharedRef<class SOverlay> Overlay)
{
	...
	// only show topology widget for material instances
	if (Substrate::IsMaterialLayeringSupportEnabled() && Cast<UMaterialInstance>(MaterialEditorPtr.Pin()->GetMaterialInterface()))
	{
		Overlay->AddSlot()
			// clang-format off
			.VAlign(VAlign_Top)
			.HAlign(HAlign_Right)
			.Padding(FMargin(5.0f, 5.0f, 50.0f, 5.0f))
			[
				SAssignNew(SubstrateWidget, SMaterialEditorTopologyWidget, MaterialEditorPtr)
			];
		// clang-format on
	}
