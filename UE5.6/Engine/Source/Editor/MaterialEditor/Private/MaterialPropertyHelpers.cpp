	...
	if (InAssetData.TagsAndValues.Contains(FilterTag) || InAssetData.AssetName == NAME_None)
	{
		...
		case EMaterialParameterAssociation::LayerParameter:
			if (InMaterialFunction->Layers[Index] != LayerFunction)
			{
				...
				{
					// update name of the layer
					InMaterialFunction->EditorOnly.LayerNames[Index] = FText::FromName(InAssetData.AssetName);
				}
			}
			break;
		case EMaterialParameterAssociation::BlendParameter:
			if (InMaterialFunction->Blends[Index] != LayerFunction)
			{
				...
				InMaterialFunction->UnlinkLayerFromParent(Substrate::IsMaterialLayeringSupportEnabled() ? Index : Index + 1); // Blend indices are offset by 1 in legacy, no blend for base layer
			}
			break;
		default:
			break;
