...
class FMaterialStats : public FGCObject, public TSharedFromThis<FMaterialStats>
{
	...
	/** If the new translator is enabled this will contain the source for the old translator.  Useful for comparing. */
	FString PreviousTranslatorHLSLCode;

	/** Cached IR from the new translator if enabled */
	FString MaterialIR;
...
private:
	...
	TSharedRef<SWidget> BuildShaderCodeWidget(FText TextLabel, TFunction<FText(void)>&& InShaderCodeCallback, TFunction<FReply(void)>&& InCopyCodeCallback, TAttribute<EVisibility> InVisibilityAttribute);
