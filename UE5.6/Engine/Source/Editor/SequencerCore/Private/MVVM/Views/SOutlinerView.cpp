...
void SOutlinerView::UpdateOutlinerColumns()
{
	...
	const int32 NumLeftGutter = CreateOutlinerColumnsForGroup(CurrentIndex, EOutlinerColumnGroup::LeftGutter);
	CurrentIndex += NumLeftGutter;
	
	const int32 NumToggles = CreateOutlinerColumnsForGroup(CurrentIndex, EOutlinerColumnGroup::AssetToggles);
	CurrentIndex += NumToggles;

	const int32 NumMuteSoloToggles = CreateOutlinerColumnsForGroup(CurrentIndex, EOutlinerColumnGroup::TransientToggles);
	CurrentIndex += NumMuteSoloToggles;

	const int32 NumCenter = CreateOutlinerColumnsForGroup(CurrentIndex, EOutlinerColumnGroup::Center);
	CurrentIndex += NumCenter;

	const int32 NumRightGutter = CreateOutlinerColumnsForGroup(CurrentIndex, EOutlinerColumnGroup::RightGutter);
	CurrentIndex += NumRightGutter;
	...
	const float DefaultGroupPadding = 2.f;
	if (NumToggles > 0)
	{
		const int32 FirstIndex = NumLeftGutter;
		const int32 LastIndex = FirstIndex + NumToggles - 1;

		ColumnMetaData->Columns[FirstIndex].CellPadding.Left += DefaultGroupPadding;
		ColumnMetaData->Columns[LastIndex].CellPadding.Right += DefaultGroupPadding;
	}

	if (NumMuteSoloToggles > 0)
	{
		const int32 FirstIndex = NumLeftGutter + NumToggles;
		const int32 LastIndex = FirstIndex + NumMuteSoloToggles - 1;
	...
	if (NumCenter > 0)
	{
		const int32 FirstIndex = NumLeftGutter + NumToggles + NumMuteSoloToggles;
		const int32 LastIndex = FirstIndex + NumCenter - 1;
	...
	if (InsertIndex < ColumnMetaData->Columns.Num())
	{
		...
		auto InsertSeparatorAfterGroup = [this, &NumSeparators, &InsertIndex](const int32 InIndex)
		{
			InsertIndex += InIndex;
			if (InsertIndex < ColumnMetaData->Columns.Num())
			{
		...
		if (NumToggles > 0)
        {
        	InsertSeparatorAfterGroup(NumToggles);
        }

		if (NumMuteSoloToggles > 0)
		{
			InsertSeparatorAfterGroup(NumMuteSoloToggles);
		}

		if (NumCenter > 0)
		{
			InsertSeparatorAfterGroup(NumCenter);
		}
	}
}
...
TSharedPtr<SWidget> SOutlinerView::OnContextMenuOpening()
{
	...
	if (auto It = GetSelectedItems(); It.Num() > 0) 
	{
		TWeakViewModelPtr<IOutlinerExtension> WeakSelectedItem = It[0];
		TViewModelPtr<IOutlinerExtension> SelectedItem = WeakSelectedItem.Pin();
...
void SOutlinerView::OnExpansionChanged(TWeakViewModelPtr<IOutlinerExtension> InWeakItem, bool bIsExpanded)
{
	...
	// Also, expand/collapse any selected items if this item is selected
	TArray<TWeakViewModelPtr<IOutlinerExtension>> WeakSelectedItems = GetSelectedItems();
	if (WeakSelectedItems.Contains(InWeakItem))
	{
		for (TWeakViewModelPtr<IOutlinerExtension> WeakItem : WeakSelectedItems)
		{
			TViewModelPtr<IOutlinerExtension> Item = WeakItem.Pin();
			if (Item && Item != InWeakItem)
			{
...
TSharedPtr<STrackLane> SOutlinerView::FindOrCreateParentLane(TViewModelPtr<IOutlinerExtension> InDataModel)
{
	...
	for (const TViewModelPtr<ITrackAreaExtension>& ParentTrackArea : InDataModel.AsModel()->GetAncestorsOfType<ITrackAreaExtension>())
	{
		...
		if (TrackAreaParameters.LaneType == ETrackAreaLaneType::Nested)
		{
			...
			if (auto AncestorsIt = ParentTrackArea.AsModel()->GetAncestorsOfType<IOutlinerExtension>(true); AncestorsIt)
			{
				const TViewModelPtr<IOutlinerExtension>& OutlinerExtItem = *AncestorsIt;
