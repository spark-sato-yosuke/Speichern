...
protected:
	...
	{
		// Iterate by index with null checking to support mutation during iteration
		for (int32 Index = 0; Index < DynamicExtensions.Num(); ++Index)
		{
			if (DynamicExtensions[Index].Extension)
			{
				DynamicExtensions[Index].Extension->OnPostInitialize();
			}
		}
		// Clean up nulls
		for (int32 Index = DynamicExtensions.Num()-1; Index >= 0; --Index)
		{
			if (!DynamicExtensions[Index].Extension)
			{
				DynamicExtensions.RemoveAtSwap(Index, 1);
			}
		}
	}
	...
	void ReinitializeExtensions()
	{
		...
		for (int32 Index = 0; Index < DynamicExtensions.Num(); ++Index)
		{
			if (DynamicExtensions[Index].Extension)
			{
				DynamicExtensions[Index].Extension->OnReinitialize();
			}
		}
		// Clean up nulls
		for (int32 Index = DynamicExtensions.Num()-1; Index >= 0; --Index)
...
private:
	...
	struct FDynamicExtensionInfo
	{
		...
		TSharedPtr<IDynamicExtension> Extension;
	};
