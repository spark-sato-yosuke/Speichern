			...
			if (SelectedSockets.Num())
			{
				for (UStaticMeshSocket* SelectedSocket : SelectedSockets)
				{
					...
					if (MoveMode == UE::Widget::WM_Rotate)
					{
						...
						const FRotator CurrentRot = SelectedSocket->RelativeRotation;
			...
			else if (GetWidgetMode() == UE::Widget::WM_Translate)
			{
				if( InInputState.IsLeftMouseButtonPressed() && (Widget->GetCurrentAxis() & EAxisList::XYZ) )
				{
					...
					if ( bAltDown )
					{
						...
						StaticMeshEditorPtr.Pin()->DuplicateSelectedSockets();
					}
				}
...
UE::Widget::EWidgetMode FStaticMeshEditorViewportClient::GetWidgetMode() const
{
	...
	else if (StaticMeshEditorPtr.Pin()->HasSelectedSockets())
	{
		return WidgetMode;
	}
...
bool FStaticMeshEditorViewportClient::CanSetWidgetMode(UE::Widget::EWidgetMode NewMode) const
{
	if (!Widget->IsDragging())
	{
		...
		{
			return StaticMeshEditorPtr.Pin()->HasSelectedSockets();
		}
	}
	return false;
}
...
bool FStaticMeshEditorViewportClient::CanCycleWidgetMode() const
{
	if (!Widget->IsDragging())
	{
		const bool bHasSelectedSockets = StaticMeshEditorPtr.Pin()->HasSelectedSockets();
		const bool bSelectedPrim = StaticMeshEditorPtr.Pin()->HasSelectedPrims();
		if ((bHasSelectedSockets || bSelectedPrim || IsCustomModeUsingWidget()))
		{
			return true;
		}
	}
...
FVector FStaticMeshEditorViewportClient::GetWidgetLocation() const
{
	...
	{
		const UStaticMeshSocket* SelectedSocket = StaticMeshEditorPtr.Pin()->GetSelectedSockets()[0];
		FMatrix SocketTM;
...
FMatrix FStaticMeshEditorViewportClient::GetWidgetCoordSystem() const 
{
	...
	if (StaticMeshEditorPtr.Pin()->HasSelectedSockets())
	{
		//FMatrix SocketTM;
		//SelectedSocket->GetSocketMatrix(SocketTM, StaticMeshComponent);
		const UStaticMeshSocket* SelectedSocket = StaticMeshEditorPtr.Pin()->GetSelectedSockets()[0];
...
void FStaticMeshEditorViewportClient::Draw(const FSceneView* View,FPrimitiveDrawInterface* PDI)
{
	...
	UBodySetup* MeshBodySetup = StaticMesh->GetBodySetup();
	const bool bHasCookedPhysicsState = MeshBodySetup && !MeshBodySetup->bNeverNeedsCookedCollisionData;

	// Draw simple shapes if we are showing simple, or showing complex but using simple as complex
	if (StaticMesh->GetBodySetup() && bHasCookedPhysicsState && (bShowSimpleCollision || (bShowComplexCollision && StaticMesh->GetBodySetup()->CollisionTraceFlag == ECollisionTraceFlag::CTF_UseSimpleAsComplex)))
	{
	...
	if (bShowComplexCollision && bHasCookedPhysicsState && StaticMesh->ComplexCollisionMesh && StaticMesh->GetBodySetup()->CollisionTraceFlag != ECollisionTraceFlag::CTF_UseSimpleAsComplex)
	{
	...
	if( bShowSockets )
	{
		const FColor UnselectedSocketColor = FColor(255, 128, 128);
		const FColor SelectedSocketColor = FColor(255, 220, 220);
		...
		for(int32 i=0; i < StaticMesh->Sockets.Num(); i++)
		{
			...
			if(Socket)
			{
				const bool bIsSocketSelected = StaticMeshEditor->GetSelectedSockets().Contains(Socket);

				FMatrix SocketTM;
				Socket->GetSocketMatrix(SocketTM, StaticMeshComponent);
				PDI->SetHitProxy( new HSMESocketProxy(i) );
				DrawWireDiamond(PDI, SocketTM, 5.f, bIsSocketSelected ? SelectedSocketColor : UnselectedSocketColor, SDPG_Foreground);
				PDI->SetHitProxy( NULL );
			}
		}
	}
...
void FStaticMeshEditorViewportClient::DrawCanvas(FViewport& InViewport, FSceneView& View, FCanvas& Canvas)
{
	...
	if (!StaticMeshEditor.IsValid() || !StaticMeshEditorViewport.IsValid() || !StaticMesh || StaticMesh->IsCompiling())
	{
		return;
	}
		...
		for (int32 i = 0; i < StaticMesh->Sockets.Num(); i++)
		{
			...
			if (Socket != nullptr)
			{
				...
				if (proj.W > 0.f)
				{
					...
					if (bManipulating && StaticMeshEditor->GetSelectedSockets().Contains(Socket))
					{
	...
	if (StaticMesh->IsNaniteEnabled() && StaticMesh->GetRenderData())
	{
		const Nanite::FResources& Resources = *StaticMesh->GetRenderData()->NaniteResourcesPtr.Get();
		if (Resources.RootData.Num() > 0)
		{
			...
			// Ray Tracing Fallback Mesh Information
			const FStaticMeshRayTracingProxy* RayTracingProxy = StaticMesh->GetRenderData()->RayTracingProxy;
			if (RayTracingProxy && !RayTracingProxy->bUsingRenderingLODs && RayTracingProxy->LODs.Num() > 0)
			{
				const int32 PreviewLODIndex = FMath::Clamp(RayTracingProxy->PreviewLODLevel, 0, RayTracingProxy->LODs.Num() - 1);
				const FStaticMeshRayTracingProxyLOD& PreviewLOD = RayTracingProxy->LODs[PreviewLODIndex];
				const FStaticMeshVertexBuffers* VertexBuffers = RayTracingProxy->LODs[PreviewLODIndex].VertexBuffers;
				const FRawStaticIndexBuffer* IndexBuffer = RayTracingProxy->LODs[PreviewLODIndex].IndexBuffer;

				uint32 RayTracingFallbackTriangleCount = 0;
				uint32 RayTracingFallbackVertexCount = 0;

				if (VertexBuffers && IndexBuffer)
				{
					RayTracingFallbackTriangleCount = IndexBuffer->GetNumIndices() / 3;
					RayTracingFallbackVertexCount = VertexBuffers->PositionVertexBuffer.GetNumVertices();
				}

				if (RayTracingFallbackTriangleCount > 0 && RayTracingFallbackVertexCount > 0)
				{
					TextItems.Emplace(FText::Format(
						NSLOCTEXT("UnrealEd", "RayTracingFallback_Triangles_F", "Ray Tracing Fallback Triangles:  {0}"),
						FText::AsNumber(RayTracingFallbackTriangleCount)));
					TextItems.Emplace(FText::Format(
						NSLOCTEXT("UnrealEd", "RayTracingFallback_Vertices_F", "Ray Tracing Fallback Vertices:  {0}"),
						FText::AsNumber(RayTracingFallbackVertexCount)));
				}
			}
		}
	}
	...
	{
		UBodySetup* MeshBodySetup = StaticMesh->GetBodySetup();
		TextItems.Emplace(FText::Format(NSLOCTEXT("UnrealEd", "NumPrimitives_F", "Num Collision Primitives:  {0}"), FText::AsNumber(MeshBodySetup->AggGeom.GetElementCount())));

		if(MeshBodySetup->bNeverNeedsCookedCollisionData)
		{
			TextItems.Emplace(NSLOCTEXT("UnrealEd", "NumPrimitivesCookWarning", "\tbNeverNeedsCookedCollisionData Enabled, no collision data will be used."), true);
		}
	}
	...
	if (bDrawUVs && StaticMesh->GetRenderData() && StaticMesh->GetRenderData()->LODResources.Num() > 0)
	{
...
bool FStaticMeshEditorViewportClient::InputAxis(const FInputKeyEventArgs& Args)
{
	bool bResult = true;
...
void FStaticMeshEditorViewportClient::ProcessClick(class FSceneView& InView, class HHitProxy* HitProxy, FKey Key, EInputEvent Event, uint32 HitX, uint32 HitY)
{
	...
	if( HitProxy )
	{
		if(HitProxy->IsA( HSMESocketProxy::StaticGetType() ) )
		{
			...
			if(Socket)
			{
				...
				{
					//Select the socket if it is unselected and vice versa
					if (StaticMeshEditorPtr.Pin()->GetSelectedSockets().Contains(Socket))
					{
						StaticMeshEditorPtr.Pin()->RemoveSelectedSocket(Socket);
					}
					else
					{
						StaticMeshEditorPtr.Pin()->AddSelectedSocket(Socket);
					}
				}
			}
		...
		{
			TArray<UStaticMeshSocket*> Sockets = StaticMeshEditorPtr.Pin()->GetSelectedSockets();
	...
	if (ClearSelectedSockets && StaticMeshEditorPtr.Pin()->HasSelectedSockets())
	{
