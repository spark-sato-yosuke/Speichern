...
TOptional<FTextFormat> FMathStructCustomization::OnGetValueToolTipTextFormat(TWeakPtr<IPropertyHandle> WeakHandlePtr) const
{
	if (TSharedPtr<IPropertyHandle> PropertyHandle = WeakHandlePtr.Pin())
	{
		...
		{
			TooltipText = PropertyHandle->GetPropertyDisplayName();
		}
		ToolTipTextFormatString.Append(TooltipText.ToString());
