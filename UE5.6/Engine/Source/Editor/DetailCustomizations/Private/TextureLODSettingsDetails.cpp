...
void FTextureLODGroupLayout::GenerateChildContent(IDetailChildrenBuilder& ChildrenBuilder)
{
	...
	{
		ChildrenBuilder.AddCustomRow(LOCTEXT("LODBiasVT_Row", "LOD Bias VT"))
			.NameContent()
			[
				SNew(STextBlock)
					.Font(IDetailLayoutBuilder::GetDetailFont())
					.Text(LOCTEXT("LODBiasVT_Name", "LOD Bias VT"))
			]
			.ValueContent()
			[
				SNew(SSpinBox<int32>)
					...
					.MinValue(-MAX_TEXTURE_MIP_COUNT)
					.MaxValue(MAX_TEXTURE_MIP_COUNT)
					.Value(this, &FTextureLODGroupLayout::GetLODBiasVT)
					.OnValueChanged(this, &FTextureLODGroupLayout::OnLODBiasVTChanged)
					.OnValueCommitted(this, &FTextureLODGroupLayout::OnLODBiasVTCommitted)
			];
	}
...
int32 FTextureLODGroupLayout::GetLODBiasVT() const
{
	return LodGroup->LODBias_VT;
}

void FTextureLODGroupLayout::OnLODBiasVTChanged(int32 NewValue)
{
	LodGroup->LODBias_VT = NewValue;
}

void FTextureLODGroupLayout::OnLODBiasVTCommitted(int32 NewValue, ETextCommit::Type TextCommitType)
{
	OnLODBiasVTChanged(NewValue);
}
