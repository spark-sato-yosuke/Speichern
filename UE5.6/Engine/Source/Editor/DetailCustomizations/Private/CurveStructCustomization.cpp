...
void FCurveStructCustomization::CustomizeHeader( TSharedRef<IPropertyHandle> InStructPropertyHandle, FDetailWidgetRow& HeaderRow, IPropertyTypeCustomizationUtils& StructCustomizationUtils )
{
	...
	if (StructPtrs.Num() == 1)
	{
		...
		FCurveOwnerInterface* CurveOwner = nullptr;
		bool bCanEdit = true;
		if (RuntimeCurve && RuntimeCurve->ExternalCurve)
		{
			CurveOwner = RuntimeCurve->ExternalCurve;
			bCanEdit = false;
		}
		else
		{
		...
		CurveWidget->SetCurveOwner(CurveOwner, bCanEdit);
		CurveWidget->SetPropertyUtils(StructCustomizationUtils.GetPropertyUtilities());

		// If any property on the chain gets reset while the Curve Owner is set  
		TSharedPtr<IPropertyHandle> RootHandle = StructPropertyHandle;
...
FReply FCurveStructCustomization::OnCurvePreviewDoubleClick( const FGeometry& InMyGeometry, const FPointerEvent& InMouseEvent )
{
	if (InMouseEvent.GetEffectingButton() == EKeys::LeftMouseButton)
	{
		...
		else
		{
			...
			FVector2f AdjustedSummonLocation = FSlateApplication::Get().CalculatePopupWindowPosition( Anchor, FCurveStructCustomization::DEFAULT_WINDOW_SIZE, true, FVector2f::ZeroVector, Orient_Horizontal );
