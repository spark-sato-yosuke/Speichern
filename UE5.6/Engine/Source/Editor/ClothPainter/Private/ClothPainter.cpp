...
{
	// Brush settings share their values by getting/setting per-property values in the config via Constructor/PostEditChange.
	BrushSettings = NewObject<UPaintBrushSettings>(GetTransientPackage());
	BrushSettings->AddToRoot();
	BrushSettings->bOnlyFrontFacingTriangles = false;
	PaintSettings = GetMutableDefault<UClothPainterSettings>();
	PaintSettings->LoadConfig();
