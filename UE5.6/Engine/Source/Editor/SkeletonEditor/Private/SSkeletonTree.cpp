...
void SSkeletonTree::Construct(const FArguments& InArgs, const TSharedRef<FEditableSkeleton>& InEditableSkeleton, const FSkeletonTreeArgs& InSkeletonTreeArgs)
{
	...
	{
		if (TSharedPtr<IPersonaPreviewScene> PreviewScenePtr = PreviewScene.Pin())
		{
			PreviewScenePtr->RegisterOnLODChanged(FSimpleDelegate::CreateSP(this, &SSkeletonTree::OnLODSwitched));
			PreviewScenePtr->RegisterOnPreviewMeshChanged(FOnPreviewMeshChanged::CreateSP(this, &SSkeletonTree::OnPreviewMeshChanged));
			PreviewScenePtr->RegisterOnSelectedBonesChanged(FOnSelectedBonesChanged::CreateSP(this, &SSkeletonTree::HandleSelectedBonesChanged));
			PreviewScenePtr->RegisterOnSelectedSocketChanged(FOnSelectedSocketChanged::CreateSP(this, &SSkeletonTree::HandleSelectedSocketChanged));
			PreviewScenePtr->RegisterOnDeselectAll(FSimpleDelegate::CreateSP(this, &SSkeletonTree::HandleDeselectAll));

			RegisterOnSelectionChanged(FOnSkeletonTreeSelectionChanged::CreateRaw(PreviewScenePtr.Get(), &IPersonaPreviewScene::HandleSkeletonTreeSelectionChanged));
		}
	}
...
void SSkeletonTree::BindCommands()
{
	...
	CommandList.MapAction(
		...
		FExecuteAction::CreateSP(this, &SSkeletonTree::OnResetBoneTransforms, bSelectedOnly ) );

	CommandList.MapAction(
		MenuActions.ResetAllBonesTransforms,
		FExecuteAction::CreateSP(this, &SSkeletonTree::OnResetBoneTransforms, !bSelectedOnly ) );
...
void SSkeletonTree::OnResetBoneTransforms(const bool bSelectedOnly)
{
	if (GetPreviewScene().IsValid())
	{
		...
		const TArray<FAnimNode_ModifyBone>& ModifyBones = PreviewInstance->GetBoneControllers();
		if (ModifyBones.IsEmpty())
		{
			return;
		}
		...
		if (!bSelectedOnly)
		{
			GEditor->BeginTransaction(LOCTEXT("SkeletonTree_ResetAllBonesTransforms", "Reset All Bone Transforms"));
			{
...
void SSkeletonTree::SetSelectedBones(const TArray<FName>& InBoneNames, ESelectInfo::Type InSelectInfo)
{
	...
	if (!bSelecting)
	{
		...
		const TArray<TSharedPtr<ISkeletonTreeItem>> SkeletonTreeItems = LinearItems.FilterByPredicate([&InBoneNames](const TSharedPtr<ISkeletonTreeItem>& SkeletonRow)
		{
			if (SkeletonRow->GetFilterResult() == ESkeletonTreeFilterResult::Hidden)
			{
				return false;
			}

			if (SkeletonRow->IsOfType<FSkeletonTreeBoneItem>() || SkeletonRow->IsOfType<FSkeletonTreeVirtualBoneItem>())
			{
				return InBoneNames.Contains(SkeletonRow->GetRowItemName());	
			}

			return false;
		});

		if (SkeletonTreeItems.IsEmpty())
		{
			return;
		}

		ExpandTreeOnSelection(SkeletonTreeItems[0]);
		constexpr bool bSelected = true;
		SkeletonTreeView->SetItemSelection(SkeletonTreeItems, bSelected, InSelectInfo);
		SkeletonTreeView->RequestScrollIntoView(SkeletonTreeItems[0]);
	}
}
...
void SSkeletonTree::SelectItemsBy(TFunctionRef<bool(const TSharedRef<ISkeletonTreeItem>&, bool&)> Predicate, const ESelectInfo::Type SelectInfo) const
{
	...
	if(ItemsToSelect.Num() > 0)
	{
		...
		for (const TPair<TSharedPtr<ISkeletonTreeItem>, bool>& ItemPair : ItemsToSelect)
		{
			...
			SkeletonTreeView->SetItemSelection(Item, true, SelectInfo);
			if (ItemPair.Value)
			{
...
void SSkeletonTree::ResetBoneTransforms(const bool bSelectedOnly)
{
	OnResetBoneTransforms(bSelectedOnly);
}
...
void SSkeletonTree::HandleSelectedBonesChanged(const TArray<FName>& InBoneNames, ESelectInfo::Type InSelectInfo)
{
	SetSelectedBones(InBoneNames, InSelectInfo);
}
