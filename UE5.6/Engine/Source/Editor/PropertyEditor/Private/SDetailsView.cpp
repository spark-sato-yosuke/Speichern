...
{
	bool GPanelResolveInstanceDataObjectsDefault = false;
	static FAutoConsoleVariableRef CVarDetailsPanelResolveInstanceDataObjectsDefault(
	TEXT("DetailsPanel.ResolveInstanceDataObjects.Default"),
		GPanelResolveInstanceDataObjectsDefault,
		TEXT("Default value for the ResolveInstanceDataObjects DetailsView init parameter if not specified.  Will only apply to newly created details panels"),
		ECVF_Default);
}
...
void SDetailsView::Construct(const FArguments& InArgs, const FDetailsViewArgs& InDetailsViewArgs)
{
	...
	[
		...
		[
			SNew(STextBlock)
			.Text(LOCTEXT("AllItemsFiltered", "All results have been filtered.\nTry changing your active filters above."))
			.AutoWrapText(true)
		...
		+ SOverlay::Slot()
		[
			SAssignNew(ExternalOverlayContainer, SBox)
			.Visibility(DisplayManager.IsValid() && DisplayManager->SupportsDetailTreeOverlay() ? EVisibility::SelfHitTestInvisible : EVisibility::Collapsed)
			.Content()
			[
				DisplayManager.IsValid() && DisplayManager->SupportsDetailTreeOverlay() ? DisplayManager->ConstructDetailTreeOverlay() : SNullWidget::NullWidget
			]
		]
	];
...
void SDetailsView::PostSetObject(const TArray<FDetailsViewObjectRoot>& Roots)
{
	...
	if (DisplayManager && DisplayManager->OverrideCreateCategoryNodes().IsSet())
	{
		InitParams.bCreateCategoryNodes = DisplayManager->OverrideCreateCategoryNodes().GetValue();
	}
	...
	InitParams.bResolveInstanceDataObjects = DetailsViewArgs.bResolveInstanceDataObjects.Get(UE::PropertyEditor::GPanelResolveInstanceDataObjectsDefault);
...
{
	TSharedPtr<FDetailsDisplayManager> OldDisplayManager = DisplayManager;
	if (ObjectFilter.IsValid())
	{
	...
	if (OldDisplayManager != DisplayManager && ExternalOverlayContainer.IsValid())
	{
		if (DisplayManager->SupportsDetailTreeOverlay())
		{
			ExternalOverlayContainer->SetVisibility(EVisibility::SelfHitTestInvisible);
			ExternalOverlayContainer->SetContent(DisplayManager->ConstructDetailTreeOverlay());
		}
		else
		{
			ExternalOverlayContainer->SetVisibility(EVisibility::Collapsed);
			ExternalOverlayContainer->SetContent(SNullWidget::NullWidget);
		}
	}
}
