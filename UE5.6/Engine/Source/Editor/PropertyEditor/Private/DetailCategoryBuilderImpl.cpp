...
void FDetailCategoryImpl::AddCustomBuilder(TSharedRef<IDetailCustomNodeBuilder> InCustomBuilder, bool bForAdvanced)
{
	...
	{
		if (TSharedPtr<IDetailsViewPrivate> DetailsView = GetDetailsViewSharedPtr())
		{
			if (DetailsView->IsCustomBuilderFavorite(NewCustomization.CustomBuilderRow->GetOriginalPath()))
			{
...
void FDetailCategoryImpl::RequestItemExpanded(TSharedRef<FDetailTreeNode> TreeNode, bool bShouldBeExpanded)
{
	if (TSharedPtr<IDetailsViewPrivate> DetailsView = GetDetailsViewSharedPtr())
	{		
		DetailsView->RequestItemExpanded(TreeNode, bShouldBeExpanded);
	}
}
...
TSharedPtr<IDetailsViewPrivate> FDetailCategoryImpl::GetDetailsViewSharedPtr() const
{
	TSharedPtr<FDetailLayoutBuilderImpl> ParentLayout = GetParentLayoutImpl();
	if (ParentLayout.IsValid())
	{
		return StaticCastSharedPtr<IDetailsViewPrivate>(ParentLayout->GetDetailsViewSharedPtr());
	}
...
TSharedRef<ITableRow> FDetailCategoryImpl::GenerateWidgetForTableView(const TSharedRef<STableViewBase>& OwnerTable, bool bAllowFavoriteSystem)
{
	...
	TSharedPtr<FComplexPropertyNode> Node = DetailLayoutBuilder.Pin()->GetRootNode();
	if (Node.IsValid())
	{
		if (FObjectPropertyNode* Object = Node->AsObjectNode())
		{
			Objects.Reserve(Object->GetNumObjects());
			for(int32 Index = 0, End = Object->GetNumObjects(); Index < End; ++Index)
			{
				if (UObject* CategoryObject = Object->GetUObject(Index))
				{
	...
	for (const FDetailLayoutCustomization& Customization : CustomizationList)
	{
		if (Customization.IsValidCustomization())
		{
			...
			if (!IsCustomProperty(Customization.GetPropertyNode()) || Customization.bCustom || bFavoriteCategory)
			{
				...
				{
					if (!FPropertyEditorPermissionList::Get().DoesDetailTreeNodePassFilter(NewNode->GetParentBaseStructure(), NewNode) || !PassesInlineFilters(NewNode.Get()))
					{
						continue;
					}
				}
