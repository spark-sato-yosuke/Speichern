...
protected:
	...
	{
		FAnimatingNodeCollection()
			: BatchId(FGuid::NewGuid())
			, CreationTime(FPlatformTime::Seconds())
		{
		}

		/** Creates a collection for the given node(s). If a BatchId is not specified, it's calculated based on the provided nodes. */
		explicit FAnimatingNodeCollection(const TConstArrayView<TSharedPtr<FPropertyPath>>& InNodePaths, const FGuid& InBatchId = FGuid());

		TArray<TSharedPtr<FPropertyPath>> NodePaths;

		/** Required id for tracking and removal (Property Nodes aren't reliable), which also allow multiple nodes to be part of the same animation. */
		FGuid BatchId;

		FTimerHandle NodeTimer;

		/** Utility function that provides a deterministic Id for the given NodePath. */
		static FGuid GetIdForNodePath(const TSharedPtr<FPropertyPath>& InNodePath);
		...
		/** Effectively resets the stored animation time. */
		void ResetElapsedSeconds();

		float GetElapsedSeconds() const;

	private:
        /** The time at which this FAnimatingNodeCollection was created, useful for resuming animation on widget reconstruction. */
	...
	/** Returns the owning FAnimatingNodeCollection for the provided PropertyNode, if any. */
	FAnimatingNodeCollection* GetAnimatingNodeCollectionForPropertyNode(const TSharedPtr<FPropertyNode>& InNode);
	...
	/** Updates the property map while keeping the expansion state of all items */
	void UpdatePropertyMapsWithItemExpansions();
