...
public:
	...
	/**
	 * Parses the input text and finds the corrisponding asset in the asset registry.
	 * Supports various types of paths, such as export text, object and package paths.
	 * @returns True if OutAssetData contains a valid asset, or the input text was None.
	 */
	static bool ParseAssetText(const FString& InText, FAssetData& OutAssetData);
...
private:
	...
	/** 
	 * Get the icon for the Browse button
	 */
	const FSlateBrush* GetOnBrowseIcon() const;
	 ...
	 */
	bool CanPasteFromText(const FString& InTag, const FString& InText, FAssetData& OutAssetData) const;
	 ...
	 */
	void PasteFromText(const FString& InTag, const FAssetData& InAssetData);
