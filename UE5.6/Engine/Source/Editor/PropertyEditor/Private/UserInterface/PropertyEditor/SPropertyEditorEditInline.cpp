...
{
	return InTreeNode
		&& (InTreeNode->HasNodeFlags(EPropertyNodeFlags::EditInlineNew) || InTreeNode->HasNodeFlags(EPropertyNodeFlags::DynamicInstance))
...
TSharedRef<SWidget> SPropertyEditorEditInline::GenerateClassPicker(TSharedRef<IPropertyHandle> PropertyHandle, FOnClassPicked OnPicked, TSharedPtr<IClassViewerFilter> AdditionalClassFilter) // static
{
	...
	if (ClassFilter->ObjProperty)
	{
		// Verse generated interfaces are not fully supported by yet, for now they are stored in FObjectProperty metadata
		ClassFilter->InterfaceThatMustBeImplemented = ClassFilter->ObjProperty->GetOwnerProperty()->GetClassMetaData(TEXT("ObjectMustImplement"));
	}
	...
	Options.bShowNoneOption = !Property->HasAllPropertyFlags(CPF_NoClear) && !bContainerHasNoClear && !Property->HasAllPropertyFlags(CPF_NonNullable);
...
void SPropertyEditorEditInline::OnClassPicked(UClass* InClass, TSharedRef<IPropertyHandle> PropertyHandle, EPropertyValueSetFlags::Type Flags) // static
{
	...
	if( ObjectNode )
	{
		...
		if (InClass)
		{
			...
			{
				FStringView ClassPath, ObjectPath;
				if (FPackageName::ParseExportTextPath(DefaultValue, &ClassPath, &ObjectPath))
				{
					if (InClass->GetPathName() == ClassPath)
					{
						NewObjectName = FPackageName::ObjectPathToObjectName(ObjectPath);
		...
		for ( TPropObjectIterator Itor( ObjectNode->ObjectIterator() ) ; Itor ; ++Itor )
		{
			...
			if (InClass)
			{
				FStringView CurClassPath, CurObjectName;
				if (PrevPerObjectValues.IsValidIndex(NewValues.Num()) && PrevPerObjectValues[NewValues.Num()] != FName(NAME_None).ToString())
				{
					FStringView CurObjectPath;
					if (FPackageName::ParseExportTextPath(PrevPerObjectValues[NewValues.Num()], &CurClassPath, &CurObjectPath))
					{
						CurObjectName = FPackageName::ObjectPathToObjectName(CurObjectPath);
					}
				}

				if (CurObjectName == NewObjectName && InClass->GetPathName() == CurClassPath)
				{
