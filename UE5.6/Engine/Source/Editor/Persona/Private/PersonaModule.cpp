...
void FPersonaModule::StartupModule()
{
...

	{
		...
		PropertyModule.RegisterCustomPropertyTypeLayout("BlendProfileInterfaceWrapper", FOnGetPropertyTypeCustomizationInstance::CreateStatic(&FBlendProfileInterfaceWrapperCustomization::MakeInstance));
...
{
	TSharedRef<IMorphTargetViewer> ViewerWidget = CreateDefaultMorphTargetViewerWidget(InPreviewScene, OnPostUndo);
	return MakeShareable(new FMorphTargetTabSummoner(InHostingApp, ViewerWidget));
}

TSharedRef<FWorkflowTabFactory> FPersonaModule::CreateMorphTargetTabFactory(
	const TSharedRef<class FWorkflowCentricApplication>& InHostingApp, TSharedRef<IMorphTargetViewer> InViewerWidget) const
{
	return MakeShareable(new FMorphTargetTabSummoner(InHostingApp, InViewerWidget));
}

TSharedRef<IMorphTargetViewer> FPersonaModule::CreateDefaultMorphTargetViewerWidget(
	const TSharedRef<IPersonaPreviewScene>& InPreviewScene,
	FSimpleMulticastDelegate& OnPostUndo) const
{
	return SNew(SMorphTargetViewer, InPreviewScene, OnPostUndo);	
}
...
TSharedRef<FWorkflowTabFactory> FPersonaModule::CreateAttributeViewerTabFactory(const TSharedRef<FWorkflowCentricApplication>& InHostingApp, const TSharedRef<IPersonaPreviewScene>& InPreviewScene) const
{
	return MakeShareable(new FAnimAttributeViewerTabSummoner(InHostingApp, InPreviewScene));
}
...
TSharedRef<SWidget> FPersonaModule::CreateCurvePicker(TSharedRef<IEditableSkeleton> InEditableSkeleton, FOnCurvePicked InOnCurvePicked, FIsCurveNameMarkedForExclusion InIsCurveNameMarkedForExclusion)
{
	return SNew(SAnimCurvePicker, &InEditableSkeleton->GetSkeleton())
		.OnCurvesPicked_Lambda([InOnCurvePicked](const TConstArrayView<FName> InCurves)
		{
			check(InCurves.Num() == 1);
			InOnCurvePicked.ExecuteIfBound(InCurves[0]);
		})
		.IsCurveNameMarkedForExclusion(InIsCurveNameMarkedForExclusion);
}

TSharedRef<SWidget> FPersonaModule::CreateMultiCurvePicker(const USkeleton* InSkeleton, FOnCurvesPicked InOnCurvesPicked, FIsCurveNameMarkedForExclusion InIsCurveNameMarkedForExclusion)
{
	return SNew(SAnimCurvePicker, InSkeleton)
		.bEnableMultiselect(true)
		.OnCurvesPicked(InOnCurvesPicked)
		.IsCurveNameMarkedForExclusion(InIsCurveNameMarkedForExclusion);
}
...
TSharedRef<SWidget> FPersonaModule::CreateSkeletonNotifyPicker(FOnNotifyPicked InOnNotifyPicked, TSharedPtr<IEditableSkeleton> InEditableSkeleton, TSharedPtr<FAssetEditorToolkit> InHostingApp, bool bInShowSyncMarkers, bool bInShowNotifies, bool bInShowCompatibleSkeletonAssets, bool bInShowOtherAssets)
{
	return SNew(SSkeletonAnimNotifies, InHostingApp)
		.OnItemSelected(InOnNotifyPicked)
		.EditableSkeleton(InEditableSkeleton)
		.ShowSyncMarkers(bInShowSyncMarkers)
		.ShowNotifies(bInShowNotifies)
		.ShowCompatibleSkeletonAssets(bInShowCompatibleSkeletonAssets)
		.ShowOtherAssets(bInShowOtherAssets);
}

void FPersonaModule::RegisterNotifyHostAsset(const FTopLevelAssetPath& InAssetClassPath, const FNotifyHostAssetParameters& InParameters)
{
	NotifyHostParameters.Add(InAssetClassPath, InParameters);
	NotifyHostAssetVersion++;
}

void FPersonaModule::UnregisterNotifyHostAsset(const FTopLevelAssetPath& InAssetClassPath)
{
	NotifyHostParameters.Remove(InAssetClassPath);
}

const FPersonaModule::FNotifyHostAssetParameters* FPersonaModule::FindNotifyHostAsset(UObject* InAsset)
{
	check(InAsset);
	UClass* AssetClass = InAsset->GetClass();
	while(AssetClass)
	{
		if(const FNotifyHostAssetParameters* FoundParameters = NotifyHostParameters.Find(AssetClass->GetClassPathName()))
		{
...
TArray<FTopLevelAssetPath> FPersonaModule::GetAllNotifyHostAssetClassPaths() const
{
	TArray<FTopLevelAssetPath> AssetClassPaths;
	NotifyHostParameters.GenerateKeyArray(AssetClassPaths);
	return AssetClassPaths;
}

uint32 FPersonaModule::GetNotifyHostAssetVersion() const
{
	return NotifyHostAssetVersion;
}
...
void FPersonaModule::RegisterBlendProfilePickerExtender(TSharedPtr<IBlendProfilePickerExtender> Extender)
{
	CustomBlendProfiles.Add(Extender);
}

void FPersonaModule::UnregisterBlendProfilePickerExtender(const FName ExtenderId)
{
	CustomBlendProfiles.RemoveAll([ExtenderId](const TSharedPtr<IBlendProfilePickerExtender>& Extender)
		{
			return Extender->GetId() == ExtenderId;
		});
}

const TArray<TSharedPtr<IBlendProfilePickerExtender>>& FPersonaModule::GetCustomBlendProfiles() const
{
	return CustomBlendProfiles;
}
