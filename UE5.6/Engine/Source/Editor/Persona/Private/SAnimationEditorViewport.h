...
public:
	...
	bool IsPhysicsEditor() const { return bShowPhysicsMenu; }
protected:
...
public:
	...
	TSharedPtr<class FAnimationViewportClient> GetAnimationViewportClient() const;
	...
	void OnFloorOffsetChanged(float InNewValue);
	void OnFloorOffsetCommitted(float InNewValue, ETextCommit::Type InCommitType);

	const TSharedPtr<IPinnedCommandList>& GetPinnedCommands();
