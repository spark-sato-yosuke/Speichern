...
const FName FPersonaTabs::AnimAttributeViewID("AnimAttributeViewerTab");
...
void FPersonaModeSharedData::Save(const TSharedRef<FAnimationViewportClient>& InFromViewport)
{
	...
	const TSharedRef<FAnimationEditorPreviewScene> AnimationEditorPreviewScene =
		StaticCastSharedRef<FAnimationEditorPreviewScene>(InFromViewport->GetPreviewScene());
	PlaybackSpeedMode = AnimationEditorPreviewScene->GetAnimationPlaybackSpeedMode();
}
...
void FPersonaModeSharedData::Restore(const TSharedRef<FAnimationViewportClient>& InToViewport)
{
	...
	const TSharedRef<FAnimationEditorPreviewScene> AnimationEditorPreviewScene =
		StaticCastSharedRef<FAnimationEditorPreviewScene>(InToViewport->GetPreviewScene());
	AnimationEditorPreviewScene->SetAnimationPlaybackSpeedMode(PlaybackSpeedMode);
...
FMorphTargetTabSummoner::FMorphTargetTabSummoner(TSharedPtr<class FAssetEditorToolkit> InHostingApp, TSharedRef<IMorphTargetViewer> InViewerWidget)
	: FWorkflowTabFactory(FPersonaTabs::MorphTargetsID, InHostingApp)
	, ViewerWidget(InViewerWidget)
{
...
FPreviewViewportSummoner::FPreviewViewportSummoner(TSharedPtr<class FAssetEditorToolkit> InHostingApp, const FPersonaViewportArgs& InArgs, int32 InViewportIndex)
	...
	, TimelineDelegates(InArgs.TimelineDelegates)
{
...
/////////////////////////////////////////////////////
// FAnimAttributeViewerTabSummoner

FAnimAttributeViewerTabSummoner::FAnimAttributeViewerTabSummoner(TSharedPtr<class FAssetEditorToolkit> InHostingApp, const TSharedRef<IPersonaPreviewScene>& InPreviewScene)
	: FWorkflowTabFactory(FPersonaTabs::AnimAttributeViewID, InHostingApp)
	, PreviewScene(InPreviewScene)
{
	TabLabel = LOCTEXT("AnimAttributeViewTabTitle", "Animation Attributes");	
	TabIcon = FSlateIcon(FAppStyle::GetAppStyleSetName(), "AnimGraph.Attribute.Attributes.Icon");

	EnableTabPadding();
	bIsSingleton = true;

	ViewMenuDescription = LOCTEXT("AnimAttributeTabView", "Animation Attributes");
	ViewMenuTooltip = LOCTEXT("AnimAttributeTabView_ToolTip", "Shows the animation attribute viewer");
}

TSharedRef<SWidget> FAnimAttributeViewerTabSummoner::CreateTabBody(const FWorkflowTabSpawnInfo& Info) const
{
	return SNew(SAnimAttributeViewer, PreviewScene.Pin().ToSharedRef());
}
