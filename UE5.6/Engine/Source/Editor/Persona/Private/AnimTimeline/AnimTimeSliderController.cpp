...
FReply FAnimTimeSliderController::OnMouseButtonUp( SWidget& WidgetOwner, const FGeometry& MyGeometry, const FPointerEvent& MouseEvent )
{
	...
	else if ( bHandleLeftMouseButton )
	{
		...
		case DRAG_NONE:
			{
				// DRAG_NONE is an expected case because Controller's OnMouseButtonUp 
				// is called from SSequenceTimeSliders's OnMouseButtonUp
				break;
			}
		default:
			{
				// if default case is called, then MouseDragType was not properly set
				checkNoEntry();
	...
	// reset distance dragged regardless if OnMouseButtonUp was handled
	DistanceDragged = 0.f;
	
	return FReply::Unhandled();
}

FReply FAnimTimeSliderController::OnMouseButtonDoubleClick(SWidget& OwnerWidget, const FGeometry& MyGeometry, const FPointerEvent& MouseEvent)
{
	return FReply::Unhandled();
}
...
FReply FAnimTimeSliderController::OnMouseMove( SWidget& WidgetOwner, const FGeometry& MyGeometry, const FPointerEvent& MouseEvent )
{
	...
	else if (bHandleLeftMouseButton)
	{
		...
		// only capture mouse if MouseDragType is valid
		if ( MouseDragType != DRAG_NONE && DistanceDragged != 0.f )
		{
			DistanceDragged = 0.f;
