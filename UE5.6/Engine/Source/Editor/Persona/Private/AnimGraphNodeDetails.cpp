...
void FAnimGraphNodeDetails::CustomizeDetails(class IDetailLayoutBuilder& DetailBuilder)
{
	...
	if(DetailBuilder.GetDetailsViewSharedPtr() == nullptr)
	{
		return;
	}
...
void FBoneReferenceCustomization::CustomizeHeader( TSharedRef<IPropertyHandle> StructPropertyHandle, class FDetailWidgetRow& HeaderRow, IPropertyTypeCustomizationUtils& StructCustomizationUtils )
{
	...
	// record selected objects in case they are needed to find the skeleton
	IPropertyUtilities* PropertyUtility = StructCustomizationUtils.GetPropertyUtilities().Get();
	SelectedObjects = PropertyUtility->GetSelectedObjects();
...
USkeleton* FBoneReferenceCustomization::GetSkeleton() const 
{
	...
	// optionally add selected objects which may manage the skeleton provider
	if (Objects.IsEmpty())
	{
		for (const TWeakObjectPtr<UObject>& SelectedObject : SelectedObjects)
		{
			Objects.Add(SelectedObject.Get());
		}
	}
