...
void FAnimationRecorder::SetCurrentFrameTimeGetter(TFunction<TOptional<FQualifiedFrameTime>()> InGetCurrentTimeFunction)
{
	GetCurrentFrameTimeFunction = MoveTemp(InGetCurrentTimeFunction);
}
...
bool FAnimationRecorder::Record(USkeletalMeshComponent* Component, FTransform const& ComponentToWorld, const TArray<FTransform>& SpacesBases, const FBlendedHeapCurve& AnimationCurves, int32 FrameToAdd)
{
	if (ensure(AnimationObject))
	{
		...
		TOptional<FQualifiedFrameTime> CurrentTime;
		
		// When set, use the custom time getter. Otherwise fall back to FApp::GetCurrentFrameTime.
		if (GetCurrentFrameTimeFunction)
		{
			CurrentTime = GetCurrentFrameTimeFunction();
		}
		else
		{
			CurrentTime = FApp::GetCurrentFrameTime();
		}
