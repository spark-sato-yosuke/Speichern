...
void FMainMenu::RegisterWindowMenu()
{
	...
	// Get content section
	Menu->AddSection("GetContent", NSLOCTEXT("MainAppMenu", "GetContentHeader", "Get Content"));
	...
	// Layout section
	{
		...
		const FName FullscreenSeparatorName = "FullscreenSeparator";
		// Separator
		Section.AddSeparator(FullscreenSeparatorName);

		// Fullscreen
		FToolMenuEntry& FullscreenEntry = Section.AddMenuEntry(FMainFrameCommands::Get().ToggleFullscreen);
		FullscreenEntry.InsertPosition.Position = EToolMenuInsertType::After;
		FullscreenEntry.InsertPosition.Name = FullscreenSeparatorName;
#endif
	}
}
...
void FMainMenu::RegisterToolsMenu()
{
	...
	// NOTE: This menu should be in sync with SourceControlMenuHelpers's SourceControl menu.

	FToolMenuSection& SourceControlSection = Menu->AddSection("Source Control", LOCTEXT("SourceControlHeading", "Revision Control"));

	SourceControlSection.AddDynamicEntry("SubmitTab", FNewToolMenuSectionDelegate::CreateLambda([](FToolMenuSection& InSection)
	{
		ISourceControlModule& SourceControlModule = ISourceControlModule::Get();

		// Access to the legacy 'Submit Files' dialog. Git / Subversion require this.
		if (!SourceControlModule.GetProvider().UsesChangelists() && SourceControlModule.GetProvider().GetName() != TEXT("Unreal Revision Control"))
		{
			InSection.AddMenuEntry(
	...
	SourceControlSection.AddDynamicEntry("ChangelistsTab", FNewToolMenuSectionDelegate::CreateLambda([](FToolMenuSection& InSection)
	{
		ISourceControlModule& SourceControlModule = ISourceControlModule::Get();

		if (SourceControlModule.GetProvider().UsesChangelists() || SourceControlModule.GetProvider().UsesUncontrolledChangelists())
		{
			InSection.AddMenuEntry(
				FMainFrameCommands::Get().ViewChangelists,
				TAttribute<FText>(),
				TAttribute<FText>(),
				FSlateIcon(FRevisionControlStyleManager::Get().GetStyleSetName(), "RevisionControl.ChangelistsTab")
			);
		}
	}));

	SourceControlSection.AddDynamicEntry("SnapshotHistoryTab", FNewToolMenuSectionDelegate::CreateLambda([](FToolMenuSection& InSection)
	{
		ISourceControlModule& SourceControlModule = ISourceControlModule::Get();

		if (SourceControlModule.GetProvider().GetName() == TEXT("Unreal Revision Control"))
		{
			InSection.AddMenuEntry(
				FMainFrameCommands::Get().ViewSnapshotHistory,
				TAttribute<FText>(),
				TAttribute<FText>(),
				FSlateIcon(FRevisionControlStyleManager::Get().GetStyleSetName(), "RevisionControl.SnapshotHistoryTab")
			);
		}
	}));

	SourceControlSection.AddDynamicEntry("CheckOutModifiedFilesTab", FNewToolMenuSectionDelegate::CreateLambda([](FToolMenuSection& InSection)
	{
		ISourceControlModule& SourceControlModule = ISourceControlModule::Get();

		if (SourceControlModule.GetProvider().UsesCheckout())
		{
			InSection.AddMenuEntry(
				FMainFrameCommands::Get().CheckOutModifiedFiles,
				TAttribute<FText>(),
				TAttribute<FText>(),
				FSlateIcon(FRevisionControlStyleManager::Get().GetStyleSetName(), "RevisionControl.Actions.CheckOut")
			);
		}
	}));
	...
	SourceControlSection.AddDynamicEntry("ConnectToSourceControl", FNewToolMenuSectionDelegate::CreateLambda([](FToolMenuSection& InSection)
	{
		...
		if (SourceControlModule.IsEnabled() && SourceControlModule.GetProvider().IsAvailable())
		{
			InSection.AddMenuEntry(
				...
				TAttribute<FText>(),
				TAttribute<FText>(),
				FSlateIcon(FRevisionControlStyleManager::Get().GetStyleSetName(), "RevisionControl.Actions.ChangeSettings")
			);
		}
		else
		{
			InSection.AddMenuEntry(
				...
				TAttribute<FText>(),
				TAttribute<FText>(),
				FSlateIcon(FRevisionControlStyleManager::Get().GetStyleSetName(), "RevisionControl.Actions.Connect")
			);
		}
	}));
}
