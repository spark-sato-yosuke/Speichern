...
USCS_Node* FSubobjectData::FindSCSNodeForInstance(const UActorComponent* InstanceComponent, UClass* ClassToSearch)
{
	...
	{
		USCS_Node* Result = nullptr;
		UBlueprintGeneratedClass::ForEachGeneratedClassInHierarchy(ClassToSearch, [InstanceComponent, &Result](const UBlueprintGeneratedClass* BPGClass)
			{
				if (BPGClass->SimpleConstructionScript != nullptr)
				{
					Result = BPGClass->SimpleConstructionScript->FindSCSNode(InstanceComponent->GetFName());
					if (Result != nullptr)
					{
						// Stop traversing when we find the node.
						return false;
					}
				}
