...
UE_DEFINE_COMMANDS_TLS(FLevelViewportCommands)
...
void FLevelViewportCommands::RegisterCommands()
{
	...
	UI_COMMAND(
		ToggleImmersive,
		"Immersive View",
		"Switches this viewport between immersive view and regular view",
		EUserInterfaceActionType::ToggleButton,
		PLATFORM_MAC ? FInputChord(EModifierKey::Control, EKeys::F11) : FInputChord(EKeys::F11)
	);
	...
	FUICommandInfo::MakeCommandInfo(
		...
		TEXT("Default Viewport"),
		LOCTEXT("DefaultViewportLabel", "Default Viewport"),
		LOCTEXT("DefaultViewport_Tooltip", "Reconfigures this viewport to the default arrangement"),
		FSlateIconFinder::FindIconForClass(ACameraActor::StaticClass()),
		EUserInterfaceActionType::RadioButton,
		FInputChord()
	);

	UI_COMMAND( ToggleViewportToolbar, "Show Viewport Toolbar", "Sets the visibility of the viewport toolbar", EUserInterfaceActionType::ToggleButton, FInputChord(EModifierKey::Control | EModifierKey::Shift, EKeys::T) );
	...
	UI_COMMAND( ToggleAllowConstrainedAspectRatioInPreview, "Constrained Aspect Ratio Preview", "Allows this viewport to have a Constrained Aspect Ratio in Preview", EUserInterfaceActionType::ToggleButton, FInputChord() );
	...
	UI_COMMAND(
		PlaySettings,
		"Open Play Preferences...",
		"Opens the play settings",
		EUserInterfaceActionType::Button,
		FInputChord()
	);
...
void FLevelViewportCommands::RegisterShowVolumeCommands()
{
	...
	for (int32 VolumeClassIndex = 0; VolumeClassIndex < VolumeClasses.Num(); ++VolumeClassIndex)
	{
		...
		{
			FText DisplayName;
			if (!FEngineShowFlags::FindShowFlagDisplayName(VolumeName.ToString(), DisplayName))
			{
				DisplayName = VolumeClasses[VolumeClassIndex]->GetDisplayNameText();
			}
