...
bool UK2Node_CallFunction::IsLatentForMacros() const
{
	UFunction* TargetFunction = GetTargetFunction();
	return (TargetFunction && TargetFunction->HasMetaData(FBlueprintMetadata::MD_Latent));
}
...
void UK2Node_CallFunction::GetNodeContextMenuActions(class UToolMenu* Menu, class UGraphNodeContextMenuContext* Context) const
{
	...
	if (CanToggleNodePurity() && !Context->bIsDebugging)
	{
