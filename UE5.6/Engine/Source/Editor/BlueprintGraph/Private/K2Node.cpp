...
{
	// Only mark as dirty if inside a transactional scope; otherwise, there's no need to re-save the package
...
void UK2Node::DestroyPinList(TArray<UEdGraphPin*>& InPins)
{
	...
	{
		Pin->Modify(/*bAlwaysMarkDirty =*/ false);
		Pin->BreakAllPinLinks(!Blueprint->bIsRegeneratingOnLoad, /*bAlwaysMarkDirty =*/ false);
