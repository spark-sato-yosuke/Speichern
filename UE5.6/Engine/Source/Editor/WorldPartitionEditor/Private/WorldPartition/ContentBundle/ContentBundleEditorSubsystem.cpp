...
void UContentBundleEditingSubmodule::CaptureActorEditorContextState(UWorld* InWorld, UActorEditorContextStateCollection* InStateCollection) const
{
	if (EditingContentBundleGuid.IsValid())
	{
		UActorEditorContextContentBundleState* State = NewObject<UActorEditorContextContentBundleState>(InStateCollection);
		State->ContentBundleGuid = EditingContentBundleGuid;
		InStateCollection->AddState(State);
	}
}

void UContentBundleEditingSubmodule::RestoreActorEditorContextState(UWorld* InWorld, const UActorEditorContextStateCollection* InStateCollection)
{
	TSharedPtr<FContentBundleEditor> ContentBundleToActivate;

	if (const UActorEditorContextContentBundleState* State = InStateCollection->GetState<UActorEditorContextContentBundleState>())
	{
		ContentBundleToActivate = GetEditorContentBundle(State->ContentBundleGuid);
	}

	if (ContentBundleToActivate.IsValid())
	{
