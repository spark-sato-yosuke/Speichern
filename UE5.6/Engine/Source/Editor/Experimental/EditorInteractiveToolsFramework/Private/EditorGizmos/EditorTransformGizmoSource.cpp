...
bool UEditorTransformGizmoSource::GetVisible(const EViewportContext InViewportContext) const
{
	if (const FEditorViewportClient* ViewportClient = GetViewportClient(InViewportContext)) 
	{
		...
		if (InViewportContext == EViewportContext::Hovered)
		{
			const bool bEngineShowFlagsModeWidget = ViewportClient->EngineShowFlags.ModeWidgets && !ViewportClient->EngineShowFlags.Game;
...
bool UEditorTransformGizmoSource::CanInteract(const EViewportContext InViewportContext) const
{
	if (const FEditorViewportClient* ViewportClient = GetViewportClient(InViewportContext))
	{
...
const FRotationContext& UEditorTransformGizmoSource::GetRotationContext() const
{
...
const FEditorViewportClient* UEditorTransformGizmoSource::GetViewportClient(const EViewportContext InViewportContext) const
{
	if (InViewportContext == EViewportContext::Hovered)
	{
		if (FEditorViewportClient* Hovered = GetModeTools().GetHoveredViewportClient())
		{
			return Hovered;
		}
	}
