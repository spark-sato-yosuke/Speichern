...
void FComponentElementEditorViewportInteractionCustomization::GizmoManipulationDeltaUpdate(const TTypedElement<ITypedElementWorldInterface>& InElementWorldHandle, const UE::Widget::EWidgetMode InWidgetMode, const EAxisList::Type InDragAxis, const FInputDeviceState& InInputState, const FTransform& InDeltaTransform, const FVector& InPivotLocation)
{
	...
	if (USceneComponent* SceneComponent = Cast<USceneComponent>(Component))
	{
		...
		auto ShouldApplyDeltaDirectly = [ToolkitHostPtr = GetToolkitHost()]()
		{
			if (ToolkitHostPtr)
			{
				const FEditorModeTools& ModeManager = ToolkitHostPtr->GetEditorModeManager();
				return ModeManager.GetRotationContext().bUseExplicitRotator;
			}
			return false;
		};

		const FRotator* DeltaRotationPtr = &DeltaRotation;
		if (ShouldApplyDeltaDirectly())
		{
			//
			const FRotator RelativeRot = SceneComponent->GetRelativeRotation();
			SceneComponent->SetRelativeRotationExact(RelativeRot + DeltaRotation);

			//
			FVector NewCompLocation = SceneComponent->GetRelativeLocation();
			NewCompLocation -= InPivotLocation;
			NewCompLocation = FRotationMatrix(DeltaRotation).TransformPosition(NewCompLocation);
			NewCompLocation += InPivotLocation;
			SceneComponent->SetRelativeLocation(NewCompLocation);

			//
			DeltaRotationPtr = nullptr;
		}
		
		FComponentElementEditorViewportInteractionCustomization::ApplyDeltaToComponent(SceneComponent, /*bDelta*/true, &DeltaTranslation, DeltaRotationPtr, &DeltaScale3D, InPivotLocation, InInputState);
	}
}
