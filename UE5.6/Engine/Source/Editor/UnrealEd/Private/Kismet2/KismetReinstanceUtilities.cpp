...
bool GUseLegacyRootComponentPreservation = false;
static FAutoConsoleVariableRef CVarUseLegacyRootComponentPreservation(
	TEXT("bp.UseLegacyRootComponentPreservation"),
	GUseLegacyRootComponentPreservation,
	TEXT("Use the legacy re-instancing behavior to preserve root components, ignoring data in previous instances")
);
...
class FReferenceReplace : public FArchiveReplaceObjectAndStructPropertyRef<UObject>
{
public:
	FReferenceReplace(UObject* InSearchObject, const TMap<UObject*, UObject*>& InReplacementMap, const TMap<FSoftObjectPath, UObject*>& InWeakReferencesMap)
	...
	FReferenceReplace(UObject* InSearchObject, const TMap<UObject*, UObject*>& InReplacementMap, const TMap<FSoftObjectPath, UObject*>& InWeakReferencesMap, EArchiveReplaceObjectFlags Flags)
		: FArchiveReplaceObjectAndStructPropertyRef<UObject>(InSearchObject, InReplacementMap, EArchiveReplaceObjectFlags::DelayStart), WeakReferencesMap(InWeakReferencesMap)
	{
	...
	static void FindAndReplaceReferences(const TArray<UObject*>& SourceObjects, const TSet<UObject*>* ObjectsThatShouldUseOldStuff, const TArray<UObject*>& ObjectsToReplace, const TMap<UObject*, UObject*>& OldToNewInstanceMap, const TMap<FSoftObjectPath, UObject*>& ReinstancedObjectsWeakReferenceMap)
	{
		...
		if (Targets.Num())
		{
			...
			for (UObject* Obj : Targets)
			{
				...
				{
					FReferenceReplace ReplaceAr(Obj, OldToNewInstanceMap, ReinstancedObjectsWeakReferenceMap);
				}
			}
		}
...
{
	if( InClassToReinstance != nullptr && InClassToReinstance->GetDefaultObject(false) )
	{
		...
		{
			ensure( ClassToReinstance->GetDefaultObject(false)->GetClass() == DuplicatedClass );
			ClassToReinstance->GetDefaultObject(false)->Rename(nullptr, GetTransientPackage(), REN_DoNotDirty | REN_DontCreateRedirectors | REN_AllowPackageLinkerMismatch);
		}
		
		// Note that we can't clear ClassToReinstance->GetDefaultObject(false) even though
...
void FBlueprintCompileReinstancer::FinalizeFastReinstancing(TArray<UObject*>& ObjectsToReplace)
{
	...
	UE::Private::FReplaceReferenceHelper::IncludeCDO(DuplicatedClass, ClassToReinstance, OldToNewInstanceMap, SourceObjects, OriginalCDO);

	if (IsClassObjectReplaced())
	{
		UE::Private::FReplaceReferenceHelper::IncludeClass(DuplicatedClass, ClassToReinstance, OldToNewInstanceMap, SourceObjects, ObjectsToReplace);
	}

	UE::Private::FReplaceReferenceHelper::FindAndReplaceReferences(SourceObjects, &ObjectsThatShouldUseOldStuff, ObjectsToReplace, OldToNewInstanceMap, ReinstancedObjectsWeakReferenceMap);
...
void FBlueprintCompileReinstancer::MoveDependentSkelToReinst(UClass* const OwnerClass, TMap<UClass*, UClass*>& NewSkeletonToOldSkeleton)
{
	...
	for (int32 i = NewParentIndex; i < ClassHierarchy.Num(); ++i)
	{
		...
		UObject* OldCDO = CurClass->GetDefaultObject(false);
...
UClass* FBlueprintCompileReinstancer::MoveCDOToNewClass(UClass* OwnerClass, const TMap<UClass*, UClass*>& OldToNewMap, bool bAvoidCDODuplication)
{
	...
	UObject* OldCDO = OwnerClass->GetDefaultObject(false);
	...
	if(OldCDO)
	{
		...
		if(bAvoidCDODuplication)
		{
			OwnerClass->SetDefaultObject(nullptr);
			...
			CopyOfOwnerClass->SetDefaultObject(OldCDO);
		}
...
void FBlueprintCompileReinstancer::ReplaceObjectHelper(UObject*& OldObject, UClass* OldClass, UObject*& NewUObject, UClass* NewClass, TMap<UObject*, UObject*>& OldToNewInstanceMap, const TMap<UClass*, UClass*>& OldToNewClassMap, TMap<UObject*, FName>& OldToNewNameMap, int32 OldObjIndex, TArray<UObject*>& ObjectsToReplace, TArray<UObject*>& PotentialEditorsForRefreshing, TSet<AActor*>& OwnersToRerunConstructionScript, TFunctionRef<TArray<TObjectPtr<USceneComponent>>&(USceneComponent*)> GetAttachChildrenArray, bool bIsComponent, bool bArchetypesAreUpToDate)
{
	...
	UPackage* OldExternalPackage = OldObject->GetExternalPackage();
	...
	if (!OldToNewNameMap.Contains(OldObject))
	{
		...
		if (bIsComponent && OldObject->HasAllFlags(RF_ArchetypeObject) && OldObject->GetOuter()->IsA<UBlueprintGeneratedClass>())
		{
			...
			UE::Private::FArchetypeReinstanceHelper::GetArchetypeObjects(OldObject, OldArchetypeObjects, RF_InheritableComponentTemplate);

			// Find a unique object name that does not conflict with anything in the scope of all outers in the template chain.
			const FString OldArchetypeName = UE::Private::FArchetypeReinstanceHelper::FindUniqueArchetypeObjectName(OldArchetypeObjects).ToString();
						...
						
	{
		...
		const bool bUseInstancingGraph = PrePopulateInstancingGraphForArchetype(InstancingGraph, OldObject, NewArchetype, &OldToNewInstanceMap);
		
		FMakeClassSpawnableOnScope TemporarilySpawnable(NewClass);
		NewUObject = NewObject<UObject>(DestinationOuter, NewClass, NewName, RF_NoFlags, NewArchetype, false, bUseInstancingGraph ? &InstancingGraph : nullptr, OldExternalPackage);
	}
	...
	PreCreateSubObjectsForReinstantiation(OldToNewClassMap, OldObject, NewUObject, CreatedInstanceMap, &OldToNewInstanceMap, &OrderedListOfObjectToCopy);
	...
	FOverridableManager::Get().RestoreOverrideState(OldObject, NewUObject);
...
void FBlueprintCompileReinstancer::ReplaceInstancesOfClass_Inner(const TMap<UClass*, UClass*>& InOldToNewClassMap, const FReplaceInstancesOfClassParameters& Params)
{
...

	{
		...
		if (IsInGameThread())
		{
			if(GEditor && GEditor->GetSelectedActors())
			{
				...
				for (int32 SelectionIdx = 0; SelectionIdx < SelectedActors->Num(); ++SelectionIdx)
				{
					...
					{
						FBlueprintCompileReinstancer::CheckAndSaveOuterPackageToCleanList(CleanPackageList, SelectedActor);
					}
				}
		...
		if(Params.ConstructionVersioningData)
		{
			check(bArchetypesAreUpToDate); // not supported, can't be
			check(Params.InstancesThatShouldUseOldClass == nullptr); // not supported, could be.. but why?

			ReinstanceWithVersioningData(
				{
					.ObjectsToReplaceScratch = ObjectsToReplace,
					.CachedArchetypeObjects = CachedArchetypeObjects,
					.OldToNewInstanceMap = OldToNewInstanceMap,
					.InOldToNewClassMap = InOldToNewClassMap,
					.OldToNewNameMap = OldToNewNameMap,
					.OldToNewTemplates = Params.OldToNewTemplates,
					.PotentialEditorsForRefreshing = PotentialEditorsForRefreshing,
					.OwnersToRerunConstructionScript = OwnersToRerunConstructionScript,
					.ReplacementActors = ReplacementActors,
					.ReinstancedObjectsWeakReferenceMap = ReinstancedObjectsWeakReferenceMap,
					.ConstructionVersioningData = *Params.ConstructionVersioningData,
					.ObjectsReplaced = ObjectsReplaced,
					.CleanPackageList = CleanPackageList,
					.bSelectionChanged = bSelectionChanged,
					.bFixupSCS = bFixupSCS
				}
			);
		}
		...
		for (TPair<UClass*, UClass*> OldToNewClass : InOldToNewClassMap)
		{
			...
			check(OldClass != NewClass || IsReloadActive());
			{
				...
				CacheArchetypes(ObjectsToReplace, CacheManager, CachedArchetypeObjects);
				...
				for (int32 OldObjIndex = 0; OldObjIndex < ObjectsToReplace.Num(); ++OldObjIndex)
				{
					...
					if (OldActor == nullptr)
					{
						CheckAndSaveOuterPackageToCleanList(CleanPackageList, OldObject);
		...
		for (TPair<UClass*, UClass*> OldToNewClass : InOldToNewClassMap)
		{
...

			{
				...
				for (int32 OldObjIndex = 0; OldObjIndex < ObjectsToReplace.Num(); ++OldObjIndex)
				{
					...
					{
						// Note: For OFPA, each instance may be stored in its own external package.
						CheckAndSaveOuterPackageToCleanList(CleanPackageList, OldActor);

						UObject* NewUObject = nullptr;
						if (OldActor->GetLevel() && OldActor->GetWorld() && !OldActor->GetWorld()->IsCleanedUp())
						{
							CheckAndSaveAttachedActorsOuterPackageToCleanList(CleanPackageList, OldActor);
	...
	for (TPair<UClass*, UClass*> OldToNewClass : InOldToNewClassMap)
	{
		...
		UE::Private::FReplaceReferenceHelper::IncludeCDO(OldClass, NewClass, OldToNewInstanceMap, OldCDOSourceObjects, InOriginalCDO, Params.OldToNewTemplates);
		if (bReplaceReferencesToOldCDOs)
		{
			...
			{
				UE::Private::FReplaceReferenceHelper::IncludeClass(OldClass, NewClass, OldToNewInstanceMap, SourceObjects, ObjectsReplaced);
			}
		}
	}
	...
	// Don't run expensive FindAndReplaceReferences when no objects were replaced if we have versioning data.
	// SourceObjects will always be non zero when using this code path but we can't afford to do global
	// reference replacement at all times:
	if(!Params.ConstructionVersioningData || ObjectsReplaced.Num() > 0 ) 
	{
		UE::Private::FReplaceReferenceHelper::FindAndReplaceReferences(SourceObjects, ObjectsThatShouldUseOldStuff, ObjectsReplaced, OldToNewInstanceMap, ReinstancedObjectsWeakReferenceMap);
	}
...
void FBlueprintCompileReinstancer::CopyPropertiesForUnrelatedObjects(UObject* OldObject, UObject* NewObject, bool bClearExternalReferences, bool bForceDeltaSerialization /* = false */, bool bOnlyHandleDirectSubObjects/* = false */, TMap<UObject*, UObject*>* OldToNewInstanceMap /*=nullptr*/, const TMap<UClass*,UClass*>* OldToNewClassMap /*=nullptr*/)
{
	...
	Params.bPreserveRootComponent = GUseLegacyRootComponentPreservation || 
		// If the native parent class has changed, we may need to conform the root component
		// this behavior may not be exactly correct but it's better than preserving 
		// the root component that was likely set by our old native parent:
		(FBlueprintEditorUtils::FindFirstNativeClass(OldObject->GetClass()) !=
		FBlueprintEditorUtils::FindFirstNativeClass(NewObject->GetClass()));
	if(!Params.bPreserveRootComponent)
	{
		// If the old actor has a bad root component then don't copy it
		// and just preserve the one created by the native constructor:
		if(const AActor* OldActor = Cast<AActor>(OldObject))
		{
			if(const USceneComponent* OldComponent = OldActor->GetRootComponent())
			{
				// this flag's name is somewhat confusing in this context, setting it to true
				// will preserve the root component created by the NewActor's constructor,
				// rather than the one on the old actor:
				Params.bPreserveRootComponent = (OldComponent->GetOwner() != OldActor);
			}
		}
	}
	...
	TArray< TTuple<UObject*, UObject*>>* OrderedListOfObjectToCopy /*= nullptr*/)
{
	...
	if (!FOverridableManager::Get().IsEnabled(OldObject) || !OldObject->HasAnyFlags(RF_NeedPostLoad))
	{
		...
		UE::Private::FReplaceReferenceHelper::GetOwnedSubobjectsRecursive(OldObject, OldInstancedSubObjects);
	}
...
bool FBlueprintCompileReinstancer::PrePopulateInstancingGraphForArchetype(FObjectInstancingGraph& InstancingGraph, TNotNull<UObject*> OldInstance, UObject* InArchetype, TMap<UObject*, UObject*>* OldToNewInstanceMap)
{
	checkf(!InstancingGraph.HasDestinationRoot(), TEXT("Can only pre-populate an empty InstancingGraph"));

	// Check if we are re-instancing from an archetype, otherwise nothing to do
	if (!InArchetype || InArchetype->HasAnyFlags(RF_ClassDefaultObject))
	{
		return false;
	}

	// Only needed for objects that are using OS for now
	if (!FOverridableManager::Get().IsEnabled(OldInstance))
	{
		return false;
	}

	// Find the root of the instantiation
	UObject* InstancingGraphRoot = OldInstance;
	do
	{
		if (UObject** NewInstancingGraphRoot = OldToNewInstanceMap ? OldToNewInstanceMap->Find(InstancingGraphRoot) : nullptr)
		{
			InstancingGraphRoot = *NewInstancingGraphRoot;
		}
		InstancingGraphRoot = InstancingGraphRoot->GetOuter();
	} while (InstancingGraphRoot && !InstancingGraphRoot->GetArchetype()->HasAnyFlags(RF_ClassDefaultObject));

	// Sanitization of the found instancing root
	if (!InstancingGraphRoot || InstancingGraphRoot->IsA<UPackage>())
	{
		UE_LOG(LogClass, Warning, TEXT("Object %s was created from an archetype we could not find the root of the instancing graph."), *GetPathNameSafe(OldInstance));
		return false;
	}

	auto GatherMatchingSubObjects = [&InstancingGraph, OldInstance](UObject* Instance, UObject* Template, auto GatherMatchingSubObjectsRecurse) -> void
	{
		InstancingGraph.AddNewObject(Instance, Template);

		ForEachObjectWithOuter(Instance, [OldInstance, Template, GatherMatchingSubObjectsRecurse](UObject* SubObject)
		{
			// Skip the object that is about to be re-instantiated
			if (SubObject == OldInstance)
			{
				return;
			}

			// Just handle subobject that are created from archetype, skip the others
			UObject* SubObjectArchetype = SubObject->GetArchetype();
			if (SubObjectArchetype->HasAnyFlags(RF_ClassDefaultObject))
			{
				return;
			}

			// Sanitization of the archetype
			if (SubObjectArchetype->GetOuter() != Template)
			{
				UE_LOG(LogClass, Warning, TEXT("Missmatched while trying to rebuild instancing graph for '%s', expecting the outer of archetype %s to be %s but was %s"), 
						*GetPathNameSafe(OldInstance), *GetPathNameSafe(SubObjectArchetype), *GetPathNameSafe(Template), *GetPathNameSafe(SubObjectArchetype->GetOuter()));
				return;
			}

			GatherMatchingSubObjectsRecurse(SubObject, SubObjectArchetype, GatherMatchingSubObjectsRecurse);

		}, /*bIncludeNestedObjects*/false);
	};

	GatherMatchingSubObjects(InstancingGraphRoot, InstancingGraphRoot->GetArchetype(), GatherMatchingSubObjects );
		...
		if (Archetype->HasAnyFlags(RF_ClassDefaultObject))
		{
			...
			else
			{
				...
				else if (OldSubObjectClass->HasAnyClassFlags(CLASS_NewerVersionExists))
				{
					...
					if (UE::FPropertyBagRepository::IsPropertyBagPlaceholderObjectFeatureEnabled(UE::FPropertyBagRepository::EPlaceholderObjectFeature::ReplaceMissingReinstancedTypes))
					{
...
void FBlueprintCompileReinstancer::CacheArchetypes(
...
FBlake3Hash FBlueprintCompileReinstancer::CalculateConstructionHashIfNecessary(const UClass* ForClass)
{
	TArray<UObject*> Count;
	GetObjectsToReinstance(ForClass, Count);
	if(Count.Num() == 0)
	{
		return FBlake3Hash();
	}

	return CalculateConstructionHash(ForClass);
}

FBlake3Hash FBlueprintCompileReinstancer::CalculateConstructionHash(const UClass* ForClass)
{
	check(ForClass && ForClass->GetDefaultObject(false));

	// must capture all template data - just grab the subobjects of the class and exclude the ufunctions.
	// UFunctions should be excluded as on load of an editor asset they will not include bytecode, but
	// after compile they will so if we include them in the class version we'll always have to reinstance. 
	// This can be impactful as we're frequently 'noop' reinstnacing level script actors.
	TArray<UObject*> Objects;
	constexpr bool bIncludeNestedObjects = false;
	GetObjectsWithOuter(ForClass, Objects, bIncludeNestedObjects, RF_Transient, EInternalObjectFlags::Garbage );
	Objects.SetNum(Algo::RemoveIf(Objects, [](const UObject* Obj)
	{
		return Obj->IsA<UFunction>();
	}));
	Objects.Sort([](const UObject& A, const UObject& B) { return A.GetFName().LexicalLess(B.GetFName()); });
	Objects.Add(ForClass->GetDefaultObject(false));
	
	FUtf8String TemplateData = UE::JsonObjectGraph::Stringify(
		Objects,
		FJsonStringifyOptions(EJsonStringifyFlags::FilterEditorOnlyData));

	FBlake3 Hash;
	Hash.Update(TemplateData.GetCharArray().GetData(), TemplateData.GetCharArray().Num());
	return Hash.Finalize();
}

void FBlueprintCompileReinstancer::ReinstanceWithVersioningData(const FReinstancingState& ReinstancingState)
{
	// if an OldToNewTemplates map has been provided, include all the instances in the replacement map
	// this is used to make reinstancing during PIE more robust:
	if (ReinstancingState.OldToNewTemplates)
	{
		for (const TPair<UClass*, TMap<UObject*, UObject*>>& Mapping : *ReinstancingState.OldToNewTemplates)
		{
			ReinstancingState.OldToNewInstanceMap.Append(Mapping.Value);
		}
	}
	// Include a mapping from the old class to the new class as well - this will
	// prevent mutation of the old classes by reference replacement, and aslo patch
	// leaks to REINST classes:
	for (const TPair<UClass*, UClass*>& Mapping : ReinstancingState.InOldToNewClassMap)
	{
		ReinstancingState.OldToNewInstanceMap.Add(Mapping.Key, Mapping.Value);
	}

	TMap<UClass*, UClass*> UnchangedClasses;
	TMap<UClass*, UClass*> NormalReinstancingClasses;
	TMap<UClass*, UClass*> ActorReinstancingClasses;
	CategorizeClassesAndInstances(
		ReinstancingState.InOldToNewClassMap, 
		UnchangedClasses, 
		NormalReinstancingClasses, 
		ActorReinstancingClasses, 
		ReinstancingState);
	RestoreClass(UnchangedClasses, ReinstancingState);
	ReinstanceNormalObjects(NormalReinstancingClasses, ReinstancingState);
	ReinstanceActors(ActorReinstancingClasses, ReinstancingState);
}

void FBlueprintCompileReinstancer::CategorizeClassesAndInstances(
	const TMap<UClass*, UClass*>& InOldToNewClassMap
	, TMap<UClass*, UClass*>& UnchangedClasses
	, TMap<UClass*, UClass*>& NormalReinstancingClasses
	, TMap<UClass*, UClass*>& ActorReinstancingClasses
	, const FBlueprintCompileReinstancer::FReinstancingState& ReinstancingState)
{
	for (TPair<UClass*, UClass*> OldToNewClass : InOldToNewClassMap)
	{
		const UClass* const OldClass = OldToNewClass.Key;
		const UClass* const NewClass = OldToNewClass.Value;

		const bool bIsComponent = NewClass->IsChildOf<UActorComponent>();
		// If any of the class changes are of an actor component to scene component or reverse then we will fixup SCS of all actors affected
		if (bIsComponent && !ReinstancingState.bFixupSCS)
		{
			ReinstancingState.bFixupSCS = (NewClass->IsChildOf<USceneComponent>() != OldClass->IsChildOf<USceneComponent>());
		}

		// if there are no instances, nothing to do:
		TArray<UObject*>& ObjectsToReplace = ReinstancingState.ObjectsToReplaceScratch;
		GetObjectsToReinstance(OldClass, ObjectsToReplace);
		if(ObjectsToReplace.Num() == 0)
		{
			continue; // unambiguously no reinstancing necessary
		}
		
		// if layout and default values match, we can reinstance fast:
		bool bClassMatchesOldClass = false;
		bool bUseActorReinstancing = false;
		// if we have a script constructor we would have to check it for equivalence here
		// but there are many obstacles to script constructors:
		const FBlake3Hash* ConstructionVersion = ReinstancingState.ConstructionVersioningData.Find(NewClass);
		// anim blueprints have uncontrolled backpointers to CDO data and therefore instances must
		// be reconstructed after compilation:
		if( ConstructionVersion && 
			FStructUtils::TheSameLayout(OldClass, NewClass) &&
			CalculateConstructionHash(NewClass) == *ConstructionVersion)
		{
			bClassMatchesOldClass = true;

			// we can only do fast reinstancing when ownership of objects is clear,
			// otherwise disable it - this avoids n^2 archetype lookups for each 
			// instance, which may be still profitable but not much.. we
			// still have to pay for deep hierarchies here, which is not ideal
			// but is only O(n) and hierarchy depth is never going to be >1000
			// at some point we'll need a flag to identify notionally free
			// objects (e.g. not created by a ctor, native or otherwise) - could
			// rely on some kind of 'is editable' flag but we're fighting with perspectives
			// there, where editability is associated with a property and has no
			// tracking on the object...:
			const auto OuteredToRegeneratedObject = [](const UObject* Obj)
			{
				UObject* OuterIter = Obj->GetOuter();
				while(OuterIter)
				{
					if(OuterIter->GetClass()->HasAnyClassFlags(CLASS_NewerVersionExists))
					{
						return true;
					}
			...
			{
				if(OuteredToRegeneratedObject(Object))
				{
					bClassMatchesOldClass = false;
					break;
				}
			}
		}
		// actors require late reinstancing, but if reinstancing fast
		// they should be ok, as finalization happens at the same time
		if(NewClass->IsChildOf(AActor::StaticClass()))
		{
			bUseActorReinstancing = true;
		}

		if(bClassMatchesOldClass)
		{
			UnchangedClasses.Add(OldToNewClass.Key, OldToNewClass.Value);
		}
		else if(bUseActorReinstancing)
		{
			ActorReinstancingClasses.Add(OldToNewClass.Key, OldToNewClass.Value);
		}
		else
		{
			NormalReinstancingClasses.Add(OldToNewClass.Key, OldToNewClass.Value);
		}

	}
}

void FBlueprintCompileReinstancer::RestoreClass(
	TMap<UClass*, UClass*>& OldToNewClassMap, 
	const FBlueprintCompileReinstancer::FReinstancingState& ReinstancingState)
{
	// update classes
	for (TPair<UClass*, UClass*> OldToNewClass : OldToNewClassMap)
	{
		const UClass* const OldClass = OldToNewClass.Key;
		UClass* const NewClass = OldToNewClass.Value;
		TArray<UObject*>& ObjectsToReplace = ReinstancingState.ObjectsToReplaceScratch;
		GetObjectsToReinstance(OldClass, ObjectsToReplace);

		const UBlueprintGeneratedClass* BPGC = Cast<UBlueprintGeneratedClass>(NewClass);

		for (UObject* Object : ObjectsToReplace)
		{
			Object->SetClass(NewClass); // class layouts match, so we can just set the object back to its original class
			
			// run reference replacement on the object, in case it has stored references to regenerated archetype data:
			UE::Private::FReferenceReplace ReplaceAr(
				Object, 
				ReinstancingState.OldToNewInstanceMap,
				ReinstancingState.ReinstancedObjectsWeakReferenceMap);
			BPGC->ReinitializeObjectAfterCompile(Object);
		}
	}
}

void FBlueprintCompileReinstancer::ReinstanceNormalObjects(
	TMap<UClass*, UClass*>& OldToNewClassMap,
	const FBlueprintCompileReinstancer::FReinstancingState& ReinstancingState)
{
	FEditorCacheArchetypeManager& CacheManager = FEditorCacheArchetypeManager::Get();
	TArray<UObject*> EmptyList;
	const int32 OldObjIndex = INDEX_NONE; // not needed as archetypes are up to date in this codepath
	const bool bArchetypesAreUpToDate = true;

	for (TPair<UClass*, UClass*> OldToNewClass : OldToNewClassMap)
	{
		UClass* const OldClass = OldToNewClass.Key;
		UClass* const NewClass = OldToNewClass.Value;
		...
		CacheArchetypes(ObjectsToReplace, CacheManager, ReinstancingState.CachedArchetypeObjects);

		for (UObject* OldObject : ObjectsToReplace)
		{
			check(IsValid(OldObject)); // is RF_MirroredGarbage in GetObjectsToReinstance not enough?
			if(!ShouldReinstanceObject(OldObject, ReinstancingState.InOldToNewClassMap, bIsComponent))
			{
				continue;
			}

			CheckAndSaveOuterPackageToCleanList(ReinstancingState.CleanPackageList, OldObject);
			UObject* NewUObject = nullptr;
			ReplaceObjectHelper(
				OldObject, 
				OldClass, 
				...
				ReinstancingState.OldToNewInstanceMap, 
				ReinstancingState.InOldToNewClassMap, 
				ReinstancingState.OldToNewNameMap, 
				OldObjIndex, 
				EmptyList, 
				ReinstancingState.PotentialEditorsForRefreshing, 
				ReinstancingState.OwnersToRerunConstructionScript, 
				&FDirectAttachChildrenAccessor::Get, 
				bIsComponent, 
				bArchetypesAreUpToDate);
			UpdateObjectBeingDebugged(OldObject, NewUObject);
			ReinstancingState.ObjectsReplaced.Add(OldObject);
		}
	}
}

void FBlueprintCompileReinstancer::ReinstanceActors(
	TMap<UClass*, UClass*>& OldToNewClassMap, 
	const FBlueprintCompileReinstancer::FReinstancingState& ReinstancingState)
{
	TArray<UObject*> EmptyList;
	const int32 OldObjIndex = INDEX_NONE; // not needed as archetypes are up to date in this codepath
	const bool bArchetypesAreUpToDate = true;
	const bool bIsComponent = false;
	const bool bPreserveRootComponent = true;
	...
	for (TPair<UClass*, UClass*> OldToNewClass : OldToNewClassMap)
	{
		...
		{
			check(IsValid(OldObject)); // is RF_MirroredGarbage in GetObjectsToReinstance not enough?
			AActor* OldActor = CastChecked<AActor>(OldObject);
			if(!ShouldReinstanceActor(OldActor))
			{
				continue;
			}
			...
			if (OldActor->GetLevel() && OldActor->GetWorld() && !OldActor->GetWorld()->IsCleanedUp())
			{
				CheckAndSaveAttachedActorsOuterPackageToCleanList(ReinstancingState.CleanPackageList, OldActor);
				ReplaceActorHelper(
					OldActor, 
					OldClass, 
					NewUObject, 
					...
					ReinstancingState.ReinstancedObjectsWeakReferenceMap, 
					ActorAttachmentData, 
					ReinstancingState.ReplacementActors, 
					bPreserveRootComponent, 
					ReinstancingState.bSelectionChanged);
			}
			else
			{
				// Actors that are not in a level cannot be reconstructed, reinstance them as normal objects:
				ReplaceObjectHelper(
					OldObject, 
					OldClass,
					NewUObject,
...
void FBlueprintCompileReinstancer::GetObjectsToReinstance(const UClass* OldClass, TArray<UObject*>& ObjectsToReplace)
{
	ObjectsToReplace.Reset();
	const bool bIncludeDerivedClasses = false;
	GetObjectsOfClass(OldClass, ObjectsToReplace, bIncludeDerivedClasses, RF_ClassDefaultObject | RF_MirroredGarbage);
}

void FBlueprintCompileReinstancer::UpdateObjectBeingDebugged(UObject* InOldObject, UObject* InNewObject)
{
...
void FBlueprintCompileReinstancer::CheckAndSaveOuterPackageToCleanList(TSet<UPackage*>& CleanPackageList, const UObject* InObject)
{
	check(InObject);
...
void FBlueprintCompileReinstancer::CheckAndSaveAttachedActorsOuterPackageToCleanList(TSet<UPackage*>& CleanPackageList, const AActor* Actor)
{
	...
	for (AActor* AttachedActor : AttachedActors)
	{
		if (AttachedActor)
		{
			CheckAndSaveOuterPackageToCleanList(CleanPackageList, AttachedActor);
		}
	}
}

bool FBlueprintCompileReinstancer::ShouldReinstanceObject(
	...
	const bool bIsComponent)
{
	// Skipping any default sub object that outer is going to be replaced
	// This isn't needed for the actor loop as the only outer for actor is a level
	if ((OldObject->IsDefaultSubobject() || OldObject->HasAnyFlags(RF_DefaultSubObject)) && 
		InOldToNewClassMap.Contains(OldObject->GetOuter()->GetClass()))
	{
		return false;
	}

	// Skip archetype instances, EXCEPT for component templates - this component
	// check is likely not needed, but leaving it for now:
	if ( !bIsComponent && OldObject->IsTemplate() )
	{
		return false;
	}
...
bool FBlueprintCompileReinstancer::ShouldReinstanceActor(const AActor* OldActor)
{
	// Skip archetype instances, EXCEPT for child actor templates
	const bool bIsChildActorTemplate = OldActor->GetOuter()->IsA<UChildActorComponent>();
	if (!bIsChildActorTemplate && OldActor->IsTemplate())
	{
		return false;
	}
	return true;
