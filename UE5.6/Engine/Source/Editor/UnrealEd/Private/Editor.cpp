...
FEditorDelegates::FOnEditorLoadDefaultStartupMap		FEditorDelegates::OnEditorLoadDefaultStartupMap;
FEditorDelegates::FOnEditorInitialized					FEditorDelegates::OnEditorInitialized;
FSimpleMulticastDelegate								FEditorDelegates::OnEditorPreExit;
...
namespace EditorUtilities
{
	...
	{
		// Copy the property. Go through all child properties to find UObject.
		//If the UObject is a PIE object, then replace it with a valid object.
		//If the object is replaced, then rehash the set/map that contained that object.

		TArray<FPropertyVisitorPath> PropertiesToRehash;
		auto VisitPred = [InTargetObject, &PropertiesToRehash](const FPropertyVisitorContext& Context) -> EPropertyVisitorControlFlow
			{
				...
				const void* SourcePtr = Data.PropertyData;
				void* TargetPtr = Path.GetPropertyDataPtr(InTargetObject);
				check(TargetPtr);

				EPropertyVisitorControlFlow Result = EPropertyVisitorControlFlow::StepInto;
				bool bCopyPropertyValue = Path.Num() == 1;
				...
				{
					bool bObjectValueChanged = false;

					UObject* const SourceObjectPropertyValue = ObjectProperty->GetObjectPropertyValue(SourcePtr);
					if (SourceObjectPropertyValue && SourceObjectPropertyValue->GetOutermost()->HasAnyPackageFlags(PKG_PlayInEditor))
					{
						// Don't copy any PIE object.
						...
						if (SourceObjectPropertyValue->IsA(AActor::StaticClass()))
						{
							...
							if (EditorWorldActor)
							{
								ObjectProperty->SetObjectPropertyValue(TargetPtr, EditorWorldActor);
								bObjectValueChanged = true;
							}
						}
						...
						else if (SourceObjectPropertyValue->IsA(UActorComponent::StaticClass()) && InTargetObject->IsA(AActor::StaticClass()))
						{
							...
							{
								ObjectProperty->SetObjectPropertyValue(TargetPtr, EditorWorldComponent);
								bObjectValueChanged = true;
							}
						}
					}

					// TSet and TMap needs to rehash if modified.
					// TObjectPtr should not be used for keys (use FObjectKey instead).
					// TSoftObjectPtr are valid keys and could be modified above.
					if (bObjectValueChanged)
					{
						bCopyPropertyValue = false;
						const TArray<FPropertyVisitorInfo>& Infos = Path.GetPath();
						for (int32 Index = Infos.Num() - 1; Index >= 0; --Index)
						{
							if (CastField<FSetProperty>(Infos[Index].Property)
								|| CastField<FMapProperty>(Infos[Index].Property))
							{
								PropertiesToRehash.AddUnique(FPropertyVisitorPath(MakeArrayView(Infos.GetData(), Index+1)));
							}
						}
					}

					Result = EPropertyVisitorControlFlow::StepOver;
				}
				
				if (bCopyPropertyValue)
				{
					// For all the other property types. Only copy if first level.
					Property->CopyCompleteValue(TargetPtr, SourcePtr);
				}

				// As an optimization skip over container's of int/double/... only step into property that can contains UObject.
				auto CanPropertyContainsObject = [](const FProperty* Property)
					{
						const bool bPropertyCannotContainsObject = CastField<FNumericProperty>(Property)
						...
						return !bPropertyCannotContainsObject;
					};
				if (const FSetProperty* const SetProperty = CastField<FSetProperty>(Property))
				{
					if (!CanPropertyContainsObject(SetProperty->ElementProp))
					{
						Result = EPropertyVisitorControlFlow::StepOver;
					}
				}
				else if (const FMapProperty* const MapProperty = CastField<FMapProperty>(Property))
				{
					if (!CanPropertyContainsObject(MapProperty->KeyProp)
						&& !CanPropertyContainsObject(MapProperty->ValueProp))
					{
						Result = EPropertyVisitorControlFlow::StepOver;
					}
				}
				else if (const FArrayProperty* const ArrayProperty = CastField<FArrayProperty>(Property))
				{
					if (!CanPropertyContainsObject(ArrayProperty->Inner))
					{
						Result = EPropertyVisitorControlFlow::StepOver;
					}
				}
		...
		UStruct* PropertyOwner = InProperty->GetOwnerStruct();
		check(PropertyOwner);
		FPropertyVisitorPath Path;
		void* SourceObjectPtr = const_cast<UObject*>(InSourceObject);
		FPropertyVisitorData Data(SourceObjectPtr, /*ParentStructData*/nullptr);
		FPropertyVisitorContext Context(Path, Data);
		PropertyVisitorHelpers::VisitProperty(PropertyOwner, InProperty, Context, VisitPred);

		// Rehash TSet and TMap in reverse order in case one contains the other.
		for (int32 Index = PropertiesToRehash.Num() - 1; Index >= 0; --Index)
		{
			const FPropertyVisitorPath& PathToRehash = PropertiesToRehash[Index];
			const FProperty* Property = PathToRehash.Top().Property;
			void* TargetPtr = PathToRehash.GetPropertyDataPtr(InTargetObject);
			if (const FSetProperty* const SetProperty = CastField<FSetProperty>(Property))
			{
				FScriptSetHelper SetHelper(SetProperty, TargetPtr);
				SetHelper.Rehash();
			}
			else
			{
				const FMapProperty* const MapProperty = CastFieldChecked<FMapProperty>(Property);
				FScriptMapHelper MapHelper(MapProperty, TargetPtr);
				MapHelper.Rehash();
			}
		}
	}
	...
	int32 CopyActorProperties( AActor* SourceActor, AActor* TargetActor, const FCopyOptions& Options )
	{
		...
		for (const TPair<UActorComponent*, UActorComponent*>& ComponentPair : SourceTargetComponentPairs)
		{
			...
			if (!IsValid(TargetComponent)) // The Target Component might not be valid if the component list has been regenerated (see AActor::PostEditChangeProperty)
			{
				TargetComponent = FindMatchingComponentInstance(SourceComponent, TargetActor);
			}
					...
					if( bIsSafeToCopy )
					{
						...
						if( !bIsPreviewing )
						{
							...
							if( Options.Flags & ECopyOptions::CallPostEditChangeProperty )
							{
								FPropertyChangedEvent PropertyChangedEvent( Property );
								TargetActor->PostEditChangeProperty( PropertyChangedEvent );
								// When simulating, a call to AActor::PostEditChangeProperty will reset and reconstruct the Actor's component array
								// which will make the TargetComponent invalid, so we need to update the variable to point to the newly created one
