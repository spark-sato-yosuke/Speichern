...
bool UWorldPartitionResaveActorsBuilder::RunInternal(UWorld* World, const FCellInfo& InCellInfo, FPackageSourceControlHelper& PackageHelper)
{
	...
	if (!WorldPartition)
	{
		if (!World->PersistentLevel->bUseExternalActors)
		{
			UE_LOG(LogWorldPartitionResaveActorsBuilder, Error, TEXT("Cannot work on maps not using one file per actor."));
			return false;
		}

		if (bSwitchActorPackagingSchemeToReduced)
		{
			UE_LOG(LogWorldPartitionResaveActorsBuilder, Error, TEXT("Cannot switch actor packaging scheme on non-partitioned worlds."));
			return false;
		}

		if (bEnableActorFolders)
		{
			UE_LOG(LogWorldPartitionResaveActorsBuilder, Error, TEXT("Cannot enable actor folders non-partitioned worlds."));
			return false;
		}

		if (UActorDescContainer* LevelContainer = UActorDescContainerSubsystem::GetChecked().RegisterContainer(UActorDescContainer::FInitializeParams{ World->GetPackage()->GetLoadedPath().GetPackageFName() }))
		{
			TArray<UPackage*> PackagesToSave;
			for (FActorDescList::TIterator<> ActorDescIterator(LevelContainer); ActorDescIterator; ++ActorDescIterator)
			{
				if (AActor* Actor = FindObject<AActor>(World->PersistentLevel, *ActorDescIterator->GetActorName().ToString()))
				{
					...
					if (bResaveDirtyActorDescsOnly)
					{
						...
						if (!ActorDescIterator->IsResaveNeeded() && !ActorDescIterator->ShouldResave(NewActorDesc.Get()))
						{
							continue;
						}
						...
						{
							DirtyActorDescsOld.Add(ActorDescIterator->ToString(FWorldPartitionActorDesc::EToStringMode::ForDiff));
			...
			UActorDescContainerSubsystem::GetChecked().UnregisterContainer(LevelContainer);
