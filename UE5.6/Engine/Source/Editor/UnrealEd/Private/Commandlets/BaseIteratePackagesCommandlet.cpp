...
{
	TRACE_CPUPROFILER_EVENT_SCOPE(UBaseIteratePackagesCommandlet::InitializeParameters);
...
void UBaseIteratePackagesCommandlet::LoadAndSaveOnePackage(const FString& Filename)
{
	...
	else
	{
		...
		if (bSavePackage)
		{
			...
			{
				UE_LOG(LogIteratePackagesCommandlet, Warning, TEXT("Package %s had no linker. Skipping PreloadOperations"), *PackagePath.GetPackageName());
			}
			...
			if (bSavePackage == true)
			{
				...
				const bool bIsEmpty = ObjectsInOuter.Num() == 0;
...
int32 UBaseIteratePackagesCommandlet::Main( const FString& Params )
{
	...
	// Start an async scan of the asset registry. The asset registry being loaded is required to set EPackageLocationFilter::FileSystem to support loading uncooked.
	FAssetRegistryModule& AssetRegistryModule = FModuleManager::LoadModuleChecked<FAssetRegistryModule>(TEXT("AssetRegistry"));
	IAssetRegistry& AssetRegistry = AssetRegistryModule.Get();
	AssetRegistry.SearchAllAssets(false);

	bForceFinishAllCompilationBeforeGC = false;
	bRequireAssetRegistryLoadedBeforeIteration = true;
	...
	// UE_DEPRECATED(5.6)
	if (Switches.Contains(TEXT("KeepPackageGUIDOnSave")))
	{
		UE_LOG(LogIteratePackagesCommandlet, Warning,
			TEXT("KeepPackageGUIDOnSave is no longer supported, because package save no longer records arbitrary guids but instead records a hash of the package's bytes - PackageSavedHash."));
	}
	...
	if (!FilterByCollection.IsEmpty())
	{
		...
		if (!CollectionManager.TryParseCollectionPath(FilterByCollection, &CollectionContainer, &CollectionName, &ShareType) ||
	...
	if(bRequireAssetRegistryLoadedBeforeIteration)
	{
		// Ensure the asset registry has loaded, before iterating.
		TRACE_CPUPROFILER_EVENT_SCOPE(AssetRegistry.WaitForCompletion);
