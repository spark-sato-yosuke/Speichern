...
void FSummarizeBookmarksProviderEditorWorkflows::AppendBookmark(uint64 BookmarkPoint, double Time, uint32 CallstackId, const uint8* FormatArgs)
{
	FBookmarkSpec& BookmarkSpec = GetSpec(BookmarkPoint);
	...
	FSummarizeBookmarksProviderEditorWorkflows::AppendBookmark(BookmarkPoint, Time, CallstackId, FormatBuffer);
}

void FSummarizeBookmarksProviderEditorWorkflows::AppendBookmark(uint64 BookmarkPoint, double Time, uint32 CallstackId, const TCHAR* Text)
{
	FString Name(Text);
