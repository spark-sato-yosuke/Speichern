...
static TArray<FString> ParseParamStringList(const TMap<FString, FString>& ParamVals, const TCHAR* ParamName)
{
	TArray<FString> StrList;

	const FString* Param = ParamVals.Find(ParamName);
	if (!Param)
	{
		return StrList;
	}

	Param->ToLower().ParseIntoArray(StrList, TEXT(","));
...
int32 DumpStableKeysFile(const FString& Token, const TMap<FString, FString>& ParamVals)
{
	...
	TArray<FString> OutputHashesToSearch = ParseParamStringList(ParamVals, TEXT("outputhash"));
	TArray<FString> NamesToSearch = ParseParamStringList(ParamVals, TEXT("objname"));
	...
	for (const auto& Pair : StableMap)
	{
		if (!OutputHashesToSearch.IsEmpty() && !OutputHashesToSearch.Contains(Pair.Key.OutputHash.ToString().ToLower()))
		{
			continue;
		}
		...
		{
			const FString* Found = Algo::FindByPredicate(NamesToSearch, [ObjName = Pair.Key.ClassNameAndObjectPath.ToString().ToLower()](const FString& PartialName) { return ObjName.Contains(PartialName); });
			if (!Found)
			{
				continue;
			}
...
int32 UShaderPipelineCacheToolsCommandlet::StaticMain(const FString& Params)
{
	...
	UE_LOG(LogShaderPipelineCacheTools, Warning, TEXT("Usage: Dump SCLInfo.%s [...] [-outputhash=LIST] [-objname=LIST] - dumps stable keys file, optionally only dumping entries that match the specified output hashes and/or object names (hash and name lists are comma-separated).\n"), ShaderStableKeysFileExt);
