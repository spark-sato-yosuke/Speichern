...
int32 UGenerateTextLocalizationResourceCommandlet::Main(const FString& Params)
{
	...
	// Get whether to validate Rich Text tags.
	{
		bool bValidateRichTextTags = false;
		GetBoolFromConfig(*SectionName, TEXT("bValidateRichTextTags"), bValidateRichTextTags, GatherTextConfigPath);
		GenerateFlags |= (bValidateRichTextTags ? EGenerateLocResFlags::ValidateRichTextTags : EGenerateLocResFlags::None);
	}
