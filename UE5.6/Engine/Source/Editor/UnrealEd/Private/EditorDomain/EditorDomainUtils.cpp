...
void PrecacheClassDigest(const FTopLevelAssetPath& ClassPath);
...
FPackageDigest CalculatePackageDigest(const FAssetPackageData& PackageData, FName PackageName)
{
	...
	while (NextClass < ImportedClassPaths.Num())
	{
		...
		if (NextClass < ImportedClassPaths.Num())
		{
			...
			if (bHasTriedPrecacheClassDigests || !IsInGameThread() || UE::IsSavingPackage())
			{
...
bool TryAppendClassDigests(const TArrayView<UE::Cook::FCookDependency>& Dependencies, FBlake3& Writer, UE::Cook::FCookDependencyContext& Context)
{
	FClassDigestMap& ClassDigests = GetClassDigests();

	FReadScopeLock ClassDigestsScopeLock(ClassDigests.Lock);

	for (UE::Cook::FCookDependency& ClassDependency : Dependencies)
	{
		FStringView ClassPath = ClassDependency.GetClassPath();
		FTopLevelAssetPath AssetPath(ClassPath);
		if (!AssetPath.IsValid())
		{
			Context.LogError(FString::Printf(TEXT("ClassDependency failed: %s is not a valid TopLevelAssetPath."), *AssetPath.ToString()));
			continue;
		}

		FClassDigestData* ExistingData = ClassDigests.Map.Find(AssetPath);
		if (!ExistingData && (!IsInGameThread() || GIsSavingPackage))
		{
			Context.LogError(FString::Printf(TEXT("%s is not a valid class."), *AssetPath.ToString()));
			return false;
		}
		else if (!ExistingData)
		{
			// PrecacheClassDigest will try to acquire a write lock. It can't upgrade a read lock to a write lock. That's why
			// I release the read lock here.
			ClassDigests.Lock.ReadUnlock();

			PrecacheClassDigest(AssetPath);

			ClassDigests.Lock.ReadLock();

			ExistingData = ClassDigests.Map.Find(AssetPath);
			if (!ExistingData)
			{
				Context.LogError(FString::Printf(TEXT("%s is not a valid class."), *AssetPath.ToString()));
				return false;
		...
		FClassDigestData* NativeData = ExistingData;
		if (ExistingData->ClosestNative != AssetPath)
		{
			NativeData = ClassDigests.Map.Find(ExistingData->ClosestNative);
			checkf(NativeData, TEXT("Classes are only stored in a ClosestNative field if they exist"));
		}
		...
		ClassDependency.SetValue(NativeData->InclusiveSchemaHash);
		Writer.Update(ClassDependency.GetRawValue(), UE::Cook::FCookDependency::ValueSizeInBytes);
	}
	...
	if (!ParentClassPath.IsNull())
	{
		...
		if (!ParentDigest)
		{
			UE_LOG(LogEditorDomain, Display,
				TEXT("Parent class %s of class %s not found. Allow flags for editordomain and incremental cooking will be invalid."),
		...
		else
		{
			if (!ParentDigest->bConstructionComplete)
			{
				...
				{
					UE_LOG(LogEditorDomain, Display,
						TEXT("Cycle detected in parents of class %s. Allow flags for editordomain and incremental cooking will be invalid."),
						*ClassPath.ToString());
				}
			}
...
TSet<FTopLevelAssetPath> ConstructTargetIncrementalClassBlockList()
{
	TSet<FTopLevelAssetPath> Result;
	TArray<FString> DenyListArray;
	GConfig->GetArray(TEXT("CookSettings"), TEXT("IncrementalClassDenyList"), DenyListArray, GEditorIni);
...
{
	TSet<FTopLevelAssetPath> BlockListClassPaths = ConstructTargetIncrementalClassBlockList();
	...
	GConfig->GetArray(TEXT("CookSettings"), TEXT("IncrementalClassAllowList"), AllowListLeafNames, GEditorIni);
	...
	GConfig->GetArray(TEXT("CookSettings"), TEXT("IncrementalClassScriptPackageAllowList"), ScriptPackageAllowListLines, GEditorIni);
	for (const FString& Line : ScriptPackageAllowListLines)
	{
		...
		{
			UE_LOG(LogEditorDomain, Error, TEXT("Invalid value for Editor.ini:[CookSettings]:IncrementalClassScriptPackageAllowList:")
				TEXT("\n\t%s")
				TEXT("\n\tExpected form\n\t+IncrementalClassScriptPackageAllowList=<Allow|Deny>,../../../Engine"),
				*Line);
			continue;
		}
	...
	// For every known /Script/ package, test whether it is allow-listed in IncrementalClassScriptPackageAllowList,
		...
		else
		{
			...
			else
			{
				...
				else
				{
					...
					// e.g. CoreOnline
					UE_LOG(LogEditorDomain, Verbose,
						TEXT("Could not find module for script package %s. Packages with classes from this script package will not be incrementally skippable."),
						*PackageName);
					continue;
				}
			}
	...
	FClassDigestMap& ClassDigests = GetClassDigests();
	{
		...
		// The previous loop only set the flag bTargetIterativeEnabled to target classes of redirection, not to the source class.
		// This is an issue in the case where class Source was renamed to class Target but a package was not resaved so it still contains references to class Source.
		// So when bTargetIterativeEnabled is set to true for class Target, I need to find class Source and set the flag for it as well.
		for (TPair<FTopLevelAssetPath, FClassDigestData>& Item : ClassDigests.Map)
		{
			FClassDigestData& OldClassDigest = Item.Value;
			if (!OldClassDigest.bIsRedirected)
			{
				continue;
			}
			
			FCoreRedirectObjectName OldName(Item.Key);
			FCoreRedirectObjectName NewName = FCoreRedirects::GetRedirectedName(ECoreRedirectFlags::Type_Class | ECoreRedirectFlags::Type_Struct, OldName);
			...
			FTopLevelAssetPath NewNameClassPath(NewName.PackageName, NewName.ObjectName);
			FClassDigestData* NewClassDigest = ClassDigests.Map.Find(NewNameClassPath);
			if (NewClassDigest)
			{
				OldClassDigest.bTargetIterativeEnabled = NewClassDigest->bTargetIterativeEnabled;
			}
		}
	}
}
...
bool TrySavePackage(UPackage* Package)
{
	...
	if (bStorageResultValid)
	{
		UE::Cook::FBuildResultDependenciesMap ExistingResultDependencies;
		UE::Cook::FIncrementalCookAttachments CookAttachments =
			UE::Cook::FIncrementalCookAttachments::Collect(
				Package, nullptr /* TargetPlatform */, MoveTemp(ExistingResultDependencies),
				false /* bHasSaveResult */, TConstArrayView<FName>() /* UntrackedSoftPackageReferences */,
				nullptr /* GenerationHelper */, false /* bGenerated */, TArray<FName>(), TConstArrayView<UObject*>(),
				TConstArrayView<UObject*>(), TConstArrayView<UE::SavePackageUtilities::FPreloadDependency>(),
				TConstArrayView<UE::Cook::FReplicatedLogData>());
		TArray<IPackageWriter::FCommitAttachmentInfo> Attachments;
		CookAttachments.AppendCommitAttachments(Attachments);
