...
FText UEdGraphNode_Comment::GetTooltipText() const
{
	...
	{
		FText DetailsTooltipText = FText::GetEmpty();
		if (!NodeDetails.IsEmpty()) 
		{
			DetailsTooltipText = FText::Format(FText::FromString("\n\nDetails:\n{0}"), NodeDetails);
		}
		CachedTooltip.SetCachedText(FText::Format(NSLOCTEXT("K2Node", "CommentBlock_Tooltip", "Comment:\n{0}{1}"), FText::FromString(NodeComment), DetailsTooltipText), this);
	}
...
void UEdGraphNode_Comment::ResizeNode(const FVector2f& NewSize)
{
	if (bCanResizeNode) 
	{
