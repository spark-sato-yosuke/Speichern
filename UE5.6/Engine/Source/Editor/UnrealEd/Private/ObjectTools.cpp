...
template <class T>
class FArchiveReplaceObjectRefDataTableRows : public FArchiveReplaceObjectRef<T>
{
public:

	FArchiveReplaceObjectRefDataTableRows(UObject* InSearchObject, const TMap<T*, T*>& InReplacementMap)
		: FArchiveReplaceObjectRef<T>(InSearchObject, InReplacementMap, EArchiveReplaceObjectFlags::DelayStart)
	{
	...
	virtual void SerializeObject(UObject* ObjectToSerialize) override
	{
		UDataTable* DataTableObject = Cast<UDataTable>(ObjectToSerialize);

		if (DataTableObject)
		{
			if (DataTableObject->RowStruct != nullptr && DataTableObject->RowStruct->RefLink != nullptr)
			{
				for (const TPair<FName, uint8*>& Pair : DataTableObject->GetRowMap())
				{
					if (uint8* RowData = Pair.Value)
					{
						DataTableObject->RowStruct->SerializeItem(*this, RowData, nullptr);
					}
				}
			}
		}
		...
		{
			FArchiveReplaceObjectRef<T>::SerializeObject(ObjectToSerialize);
		}
	}
};
...
namespace ObjectTools
{
	...
	UObject* DuplicateSingleObject(UObject* Object, const FPackageGroupName& PGN, TSet<UPackage*>& InOutPackagesUserRefusedToFullyLoad, bool bPromptToOverwrite, TMap<TSoftObjectPtr<UObject>, TSoftObjectPtr<UObject>>* DuplicatedObjects /*= nullptr*/)
	{
		...
		if( DupObject )
		{
			...
			DupObject->GetOutermost()->SetAssetAccessSpecifier(Object->GetOutermost()->GetAssetAccessSpecifier());
			...
			for (UObject* CurObject : ObjectsToReplaceWithin)
			{
				...
				if (CurObject && CurObject->IsValidLowLevel())
				{
					...
					if (BPObjectToUpdate)
					{
						...
						FArchiveReplaceObjectAndStructPropertyRef<UObject> ReplaceInBPClassDefaultObject_Ar(BPObjectToUpdate->GeneratedClass->GetDefaultObject(false), ReplacementMap, EArchiveReplaceObjectFlags::IncludeClassGeneratedByRef);
					}

					UDataTable* DataTableObjectToUpdate = Cast<UDataTable>(CurObject);
					if (DataTableObjectToUpdate)
					{
						FArchiveReplaceObjectRefDataTableRows<UObject> ReplaceInDataTableRows_Ar(DataTableObjectToUpdate, ReplacementMap);
					}
	...
	void ForceReplaceReferences(TArrayView<FReplaceRequest> Requests, TSet<UObject*>& ObjectsToReplaceWithin)
	{
		FForceReplaceInfo ReplaceInfo;
		ForceReplaceReferences(Requests, ObjectsToReplaceWithin, ReplaceInfo, false);
	}
	 ...
	 */
	void ShowReferencedObjs(UObject* Object)
	{
		ShowReferencedObjs(Object, nullptr, NAME_None, ECollectionShareType::CST_Private);
	}
	...
	void ShowReferencedObjs(UObject* Object, const FString& CollectionName, ECollectionShareType::Type ShareType)
	{
		...
		{
			ShowReferencedObjs(Object, nullptr, NAME_None, ShareType);
		}
		else
		{
			ShowReferencedObjs(Object, &FCollectionManagerModule::GetModule().Get().GetProjectCollectionContainer().Get(), FName(CollectionName), ShareType);
		}
	}
	 ...
	 * @param	ShareType			The share type of any created collection
	 */
	void ShowReferencedObjs( UObject* Object, ICollectionContainer* CollectionContainer, FName CollectionName, ECollectionShareType::Type ShareType )
	{
		if( Object )
		{
			GEditor->GetSelectedObjects()->Deselect( Object );
			...
			if ( NumReferencedObjects > 1 || ( NumReferencedObjects == 1 && !ReferencedObjects.Contains( Object ) ) )
			{
				...
				else if (ensure(CollectionContainer))
				{
					...
					{
						FContentHelper ContentHelper(CollectionContainer->AsShared());
						if (ContentHelper.Initialize() == true)
						{
							ContentHelper.ClearCollection(CollectionName, ShareType);
							const bool CollectionCreated = ContentHelper.SetCollection(CollectionName, ShareType, ObjectsToAdd);

							// Notify the user whether the collection was successfully created
							FNotificationInfo Info( FText::Format( NSLOCTEXT("ObjectTools", "SuccessfulAddCollection", "{0} sucessfully added as a new collection."), FText::FromName(CollectionName)) );
	...
	void CleanupAfterSuccessfulDelete (const TArray<UPackage*>& PotentialPackagesToDelete, bool bPerformReferenceCheck)
	{
		...
		for ( int32 PackageIdx = PackagesToDelete.Num() - 1; PackageIdx >= 0; --PackageIdx )
		{
			...
			auto ShouldSkipReferenceCheck = [](const UPackage* InPackage)
			{
				auto IsExternalPackage = [](const UPackage* Package) -> bool
				{
					bool bIsExternalPackage = false;
					ForEachObjectWithPackage(Package, [Package, &bIsExternalPackage](UObject* InObject)
					{
						bIsExternalPackage = InObject->GetOutermostObject()->GetPackage() != Package;
						return !bIsExternalPackage;
					});
					return bIsExternalPackage;
				};
				...
				return IsExternalPackage(InPackage);
			};

			if ( Package != nullptr && bPerformReferenceCheck && !ShouldSkipReferenceCheck(Package) )
			{
	...
	{
		TArray<FAssetData> AllAssetsToDelete;
		const UEditorProjectAssetSettings* Settings = GetDefault<UEditorProjectAssetSettings>();
		bool bAddExtraLocalizedVariants = (Settings == nullptr || Settings->bRenameLocalizedVariantsAlongsideSourceAsset);
		if (bAddExtraLocalizedVariants && bShowConfirmation)
		{
			if (!AddExtraLocalizedVariantsToDelete(AssetsToDelete, AllAssetsToDelete))
			{
				// Either the user requested to cancel the current operation or an error occurred that requires interrupting the current operation...
				return 0;
			}
		}
		else
		{
			AllAssetsToDelete = AssetsToDelete;
		}

		TArray<TWeakObjectPtr<UPackage>> PackageFilesToDelete;
		TArray<UObject*> ObjectsToDelete;
		for ( int i = 0; i < AllAssetsToDelete.Num(); i++ )
		{
			const FAssetData& AssetData = AllAssetsToDelete[i];
	...
	int32 PrivatizeAssets(const TArray<FAssetData>& AssetsToPrivatize, bool bShowConfirmation, const EAssetAccessSpecifier InAssetAccessSpecifier)
	{
		...
		{
			return PrivatizeObjects(ObjectsToPrivatize, bShowConfirmation, EAllowCancelDuringPrivatize::AllowCancel, InAssetAccessSpecifier);
		}
	...
	bool AddExtraLocalizedVariantsToDelete(const TArray<FAssetData>& InAssetsToDelete, TArray<FAssetData>& OutAssetsWithVariantsToDelete)
	{
		IAssetTools& AssetTools = FModuleManager::LoadModuleChecked<FAssetToolsModule>("AssetTools").Get();
		TSharedPtr<ILocalizedAssetTools> LocalizedAssetTools = AssetTools.GetLocalizedAssetTools();

		TArray<FName> PackageNames;
		Algo::Transform(InAssetsToDelete, PackageNames, [](const FAssetData& InAssetData) { return InAssetData.PackageName; });

		TMap<FName, TArray<FName>> VariantsBySourcesOnDisk;
		TMap<FName, TArray<FName>> VariantsBySourcesOnlyInRevisionControl;
		TArray<FName> PackagesNotFound;
		bool bNeedToCheckInRevisionControl = USourceControlPreferences::RequiresRevisionControlToRenameLocalizableAssets();
		ELocalizedAssetsResult Result = LocalizedAssetTools->GetLocalizedVariants(PackageNames, VariantsBySourcesOnDisk, bNeedToCheckInRevisionControl, VariantsBySourcesOnlyInRevisionControl, &PackagesNotFound);
		bool bRevisionControlWasNeeded = (Result == ELocalizedAssetsResult::RevisionControlNotAvailable);

		// We have all the assets and variants that we need
		TArray<FName> PackagesAndVariants;
		for (const TPair<FName, TArray<FName>>& LocalizedVariantsWithSourceAsset : VariantsBySourcesOnDisk)
		{
			PackagesAndVariants.Add(LocalizedVariantsWithSourceAsset.Key); // Add Source Asset
			for (const FName& LocalizedVariant : LocalizedVariantsWithSourceAsset.Value)
			{
				PackagesAndVariants.Add(LocalizedVariant); // Add all Localized Variants
			}
		}
		for (const FName& OtherAsset : PackagesNotFound)
		{
			PackagesAndVariants.Add(OtherAsset); // Add the other assets
		}

		// Error processing
		if (!VariantsBySourcesOnlyInRevisionControl.IsEmpty())
		{
			// Files in Revision Control are needed
			TArray<FText> FilesNeeded;
			for (const TPair<FName, TArray<FName>>& LocalizedVariantsWithSourceAssetInSCC : VariantsBySourcesOnlyInRevisionControl)
			{
				for (const FName& LocalizedVariantInSCC : LocalizedVariantsWithSourceAssetInSCC.Value)
				{
					FilesNeeded.Add(FText::AsCultureInvariant(LocalizedVariantInSCC.ToString()));
				}
			}

			UE_LOG(LogObjectTools, Error, TEXT("A file that needs to be renamed was detected in Revision Control but not on your disk."));
			LocalizedAssetTools->OpenFilesInRevisionControlRequiredDialog(FilesNeeded);

			if (&OutAssetsWithVariantsToDelete != &InAssetsToDelete)
			{
				OutAssetsWithVariantsToDelete = InAssetsToDelete;
			}
			return false;
		}
		else if (bRevisionControlWasNeeded)
		{
			// Revision Control is needed. Extraction must fail.
			UE_LOG(LogObjectTools, Error, TEXT("%s"), *LocalizedAssetTools->GetRevisionControlIsNotAvailableWarningText().ToString());
		...
		for (const FName& Package : PackagesAndVariants)
		{
			if (!PackageNames.Contains(Package))
			{
				FString PackageNameStr = Package.ToString();
				UE_LOG(LogObjectTools, Display, TEXT("Added asset to extraction (while checking for localized variants): %s"), *PackageNameStr);
				NewAssetsAdded.Add(FText::AsCultureInvariant(PackageNameStr));
			}
		}
		...
		ELocalizedVariantsInclusion InclusionResult = LocalizedAssetTools->OpenIncludeLocalizedVariantsListDialog(NewAssetsAdded);
		switch (InclusionResult)
		{
		...
		default:
			// The inclusion is already done at this point. Only the exclude/cancel options need to do some revert work.
			break;
		}
		...
		// Don't add localized variants if the asset registry is not ready
		...
		// Iterate over each package path to convert to FAssetData
		for (const FName& PackagePath : PackagesAndVariants)
		{
			TArray<FAssetData> FoundAssets;
			AssetRegistry.GetAssetsByPackageName(PackagePath, FoundAssets);
			OutAssetsWithVariantsToDelete.Append(FoundAssets);
		}
	...
	int32 PrivatizeObjects(const TArray<UObject*>& InObjectsToPrivatize, bool bShowConfirmation, EAllowCancelDuringPrivatize AllowCancelDuringPrivatize, const EAssetAccessSpecifier InAssetAccessSpecifier)
	{
		const FScopedBusyCursor BusyCursor;
		...
		TSharedRef<FAssetPrivatizeModel> PrivatizeModel = MakeShared<FAssetPrivatizeModel>(ObjectsToPrivatize, InAssetAccessSpecifier);
			...
			else
			{
				...
				else
				{
					...
					if (NewPackage != OldPackage)
					{
						...
						NewPackage->SetAssetAccessSpecifier(OldPackage->GetAssetAccessSpecifier());
	...
	FText GetUserFacingFunctionName(const UFunction* Function, bool bAllowFriendlyNames)
	{
		if (Function != nullptr)
		{
			// Function->GetDisplayNameText() and Function->GetMetaDataText(NAME_DisplayName,...) are equivalent.
			// However, we cannot simply use those functions since they enforce friendly names and do not work on SKEL classes.
			// We implement the same functionality a third time.
			static const FName NAME_DisplayName{ TEXT("DisplayName") };

			FString FunctionDisplayName = Function->GetName();
			FString FunctionFriendlyName;
			if (const FString* FoundMetaData = Function->FindMetaData(NAME_DisplayName))
			{
				FunctionDisplayName = *FoundMetaData;
			}
			else
			{
				FunctionFriendlyName = FName::NameToDisplayString(FunctionDisplayName, false);
			}

			// There is a long-term Localization vision that no code is localized, but we are choosing to always localize in UE5.
			if (constexpr bool bUseLocalization = true)
			{
				// We are sometimes calling this function on a SKEL_ClassName_C Actor rather than the instance
				// So we should keep traversing the hierarchy finding the right function name
				const UFunction* BaseMostFunction = Function;
				for (; BaseMostFunction->GetSuperFunction(); BaseMostFunction = BaseMostFunction->GetSuperFunction());

				static const FTextKey LocalizationNamespace = TEXT("UObjectDisplayNames");
				FString LocalizationKey = BaseMostFunction->GetFullGroupName(false);
				FText OutLocalizedText;
				if (FText::FindTextInLiveTable_Advanced(LocalizationNamespace, LocalizationKey, OutLocalizedText, FunctionFriendlyName.IsEmpty() ? &FunctionDisplayName : &FunctionFriendlyName))
				{
					return OutLocalizedText;
				}
			}

			// Functions can opt to not use friendly names because they can be manually input by a user in the editor (and it would otherwise not adhere to their name)
			// There is a long-term goal of removing friendly names from the Engine.  However, we keep them in some cases as it helps the nodes
			// be decipherable in a zoomed-out view.
			const bool bUseFriendlyNames = bAllowFriendlyNames && GEditor && GetDefault<UEditorStyleSettings>()->bShowFriendlyNames;
			if (bUseFriendlyNames && !FunctionFriendlyName.IsEmpty())
			{
				return FText::FromString(FunctionFriendlyName);
			}
			else
			{
				return FText::FromString(FunctionDisplayName);
			}
		}
...
namespace ThumbnailTools
{
	...
	bool LoadThumbnailsFromPackageDirectly(const FString& InPackageFileName, const TSet<FName>& InObjectFullNames, FThumbnailMap& InOutThumbnails, const FString& InPackageFileNameToOpen)
	{
		// Create a file reader to load the file
		TUniquePtr<FArchive> FileReader(IFileManager::Get().CreateFileReader(*InPackageFileNameToOpen));
	...
	TArray<FLoadThumbnailsFromPackage>& GetLoadThumbnailsFromPackageDelegate()
	{
		static TArray<FLoadThumbnailsFromPackage> LoadThumbnailsFromPackageDelegates;
		return LoadThumbnailsFromPackageDelegates;
	}

	FDelegateHandle AddLoadThumbnailsFromPackageDelegate(FLoadThumbnailsFromPackage InDelegate)
	{
		return GetLoadThumbnailsFromPackageDelegate().Add_GetRef(InDelegate).GetHandle();
	}

	void RemoveLoadThumbnailsFromPackageDelegate(const FDelegateHandle InHandle)
	{
		GetLoadThumbnailsFromPackageDelegate().RemoveAll([InHandle](const FLoadThumbnailsFromPackage& Entry)
		{
			return Entry.GetHandle() == InHandle || !Entry.IsBound();
		});
	}
	...
	bool LoadThumbnailsFromPackage( const FString& InPackageFileName, const TSet< FName >& InObjectFullNames, FThumbnailMap& InOutThumbnails )
	{
		...
		if (LoadThumbnailsFromPackageDirectly(InPackageFileName, InObjectFullNames, InOutThumbnails, InPackageFileName))
		{
			return true;
		}
		...
		{
			// Delegates
			bool bDelegatesNeedCleanup = false;
			for (const FLoadThumbnailsFromPackage& DelegateEntry : GetLoadThumbnailsFromPackageDelegate())
			{
				if (DelegateEntry.IsBound())
				{
					if (DelegateEntry.Execute(InPackageFileName, InObjectFullNames, InOutThumbnails))
					{
						return true;
					}
				}
				...
				{
					bDelegatesNeedCleanup = true;
				}
			}

			if (bDelegatesNeedCleanup)
			{
				RemoveLoadThumbnailsFromPackageDelegate(FDelegateHandle());
			}
		}
