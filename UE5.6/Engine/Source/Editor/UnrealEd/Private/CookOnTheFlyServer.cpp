		...
		if (CurrentTime >= SaveBusyWarnTimeSeconds)
		{
			...
			for (TArray<UObject*>* ObjectArray : {  &NonExpectedObjects, &ExpectedObjects })
			{
				for (UObject* Object : *ObjectArray)
				{
					...
					FStringView AdditionalDebugInfoStr = AdditionalDebugInfo.ToView();
	...
	UE::Cook::FPackageData& PackageData, UE::Cook::FCookerTimer& Timer, bool bPrecaching)
{
	...
	if (PackageData.GetSaveSubState() <= ESaveSubState::Generation_CallPopulate)
	{
		...
		{
			Result = TryPopulateGeneratedPackage(GenerationHelper, Info);
		}
		if (Result != EPollStatus::Success)
		{
			return Result;
	...
	TArray<ICookPackageSplitter::FGeneratedPackageForPopulate>& GeneratedPackagesForPopulate)
{
	...
	if (PackageData.GetSaveSubState() <= ESaveSubState::Generation_CallObjectsToMove)
	{
		...
		{
			if (!GenerationHelper.TryCallPopulateGeneratorPackage(GeneratedPackagesForPopulate))
			{
				return EPollStatus::Error;
			}
		}
		...
		if (!Referencer)
		{
				...
				Instigator.Category != EInstigator::StartupSoftObjectPath)
			{
				FString Message = FString::Printf(
					TEXT("Unsolicited package %s was loaded outside of any cook operation on a referencer package; we don't know why it was loaded. ")
...
void UCookOnTheFlyServer::PumpSaves(UE::Cook::FTickStackData& StackData, uint32 DesiredQueueLength, int32& OutNumPushed, bool& bOutBusy)
{
	...
	{
		PumpRuntimeSaves(StackData, DesiredQueueLength, OutNumPushed, bOutBusy);
	}
	else
	{
		// After we enter the build dependencies phase, we no longer save packages, we just commit them with dependencies but no cooked data.
		PumpBuildDependencySaves(StackData, DesiredQueueLength, OutNumPushed, bOutBusy);
	}
}

void UCookOnTheFlyServer::PumpRuntimeSaves(UE::Cook::FTickStackData& StackData, uint32 DesiredQueueLength, int32& OutNumPushed, bool& bOutBusy)
{
	using namespace UE::Cook;
	OutNumPushed = 0;
	bOutBusy = false;
...
void UCookOnTheFlyServer::PumpBuildDependencySaves(UE::Cook::FTickStackData& StackData, uint32 DesiredQueueLength, int32& OutNumPushed, bool& bOutBusy)
{
	using namespace UE::Cook;
	OutNumPushed = 0;
	bOutBusy = false;
...
void UCookOnTheFlyServer::KickBuildDependencies(UE::Cook::FTickStackData& StackData)
{
	...
	check(!IsCookWorkerMode()); // KickBuildDependencies should only be called on the Director.
	if (CookDirector)
	{
...
public:
		...
		const FString& ResolvedMetadataPath, UE::Cook::FDeterminismManager* InDeterminismManager)
	{
		...
		case EDiffMode::IncrementalValidate:
			CookedPackageWriter = new FIncrementalValidatePackageWriter(COTFS,
