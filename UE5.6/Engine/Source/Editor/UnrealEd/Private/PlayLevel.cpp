...
void UEditorEngine::EndPlayMap()
{
	...
	for (int32 WorldIdx = WorldList.Num()-1; WorldIdx >= 0; --WorldIdx)
	{
		...
		if (ThisContext.WorldType == EWorldType::PIE)
		{
			...
			if (ThisContext.World())
			{
				...
				// Destroy the PendingNetGame netdriver if it never finished connecting.
				if (ThisContext.PendingNetGame && ThisContext.PendingNetGame->NetDriver)
				{
					if (ThisContext.PendingNetGame->NetDriver->ServerConnection)
					{
						CancelPending(ThisContext);
					}
					else
					{
						DestroyNamedNetDriver(ThisContext.PendingNetGame, ThisContext.PendingNetGame->NetDriver->NetDriverName);
					}
				}
	...
	// Only remove from list after all PIE worlds have been torn down, to prevent callbacks from trying to access removed worldcontexts
	for (int32 WorldIdx = WorldList.Num() - 1; WorldIdx >= 0; --WorldIdx)
	{
		FWorldContext& ThisContext = WorldList[WorldIdx];
		if (ThisContext.WorldType == EWorldType::PIE)
	...
	for (TObjectIterator<UGameInstance> It; It; ++It)
	{
		if (It->HasAnyInternalFlags(EInternalObjectFlags::Garbage))
		{
			LeakedObjectsSet.Add(*It);
		}
	}
	...
	if( ensure(LevelViewport.IsValid()) )
	{
		...
		else
		{
			...
			if ( SlatePlayInEditorSession.EditorPlayer.IsValid() && SlatePlayInEditorSession.EditorPlayer.Get()->PlayerController )
			{
				...
				ViewRotation.Roll = 0.0; // Clear roll since editor camera controls has no way to adjust roll
