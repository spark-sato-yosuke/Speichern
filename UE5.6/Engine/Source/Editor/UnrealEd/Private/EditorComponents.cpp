...
static void GetAxisColors(FLinearColor Out[3], bool bIsPerspective)
{
	Out[0] = AxisDisplayInfo::GetAxisColor(EAxisList::Forward);
	Out[1] = AxisDisplayInfo::GetAxisColor(EAxisList::Left);
	Out[2] = AxisDisplayInfo::GetAxisColor(EAxisList::Up);

	// less prominent axis lines for perspective views
	if (bIsPerspective)
	{
		for (int i = 0; i < 3; ++i)
		{
...
void FGridWidget::DrawNewGrid(const FSceneView* View, FPrimitiveDrawInterface* PDI)
{
	...
	GetAxisColors(AxisColors, bIsPerspective);
	...
	if(bIsPerspective)
	{
		...
		Radii *= FMath::Max( 1.0, FMath::Abs(CameraPos.Z) / 1000.0 );
	}
	else
	{
