...
public:
	...
	/**
	 * Overridable method for derived classes to add additional world to be process by world builder
	 * This is called after `RunInternal` is executed.
	 */
	UNREALED_API virtual bool ShouldProcessAdditionalWorlds(UWorld* InWorld, TArray<FString>& OutPackageNames) const { return false; }
...
protected:
	...
	bool IsForceIncludedDataLayer(const UDataLayerInstance* InDataLayerInstance) const;
	bool IsForceExcludedDataLayer(const UDataLayerInstance* InDataLayerInstance) const;

	int32 IterativeCellSize = 102400;
	int32 IterativeCellOverlapSize = 0;
	FBox  IterativeWorldBounds;

	UE_DEPRECATED(5.6, "Data layers to include must now be specified using IncludedDataLayers")
	TSet<FName> DataLayerShortNames;
	UE_DEPRECATED(5.6, "Data layers to exclude must now be specified using ExcludedDataLayers")
	TSet<FName> ExcludedDataLayerShortNames;

	TSet<TObjectPtr<UDataLayerAsset>> IncludedDataLayers;
	TSet<TObjectPtr<UDataLayerAsset>> ExcludedDataLayers;

	bool bLoadNonDynamicDataLayers = true;
	bool bIncludeInitiallyActivatedRuntimeDataLayers = false;
	bool bExcludeNonInitiallyActivatedRuntimeDataLayers = false;

	UE_DEPRECATED(5.6, "This has been renamed to bExcludeNonInitiallyActivatedRuntimeDataLayers")
