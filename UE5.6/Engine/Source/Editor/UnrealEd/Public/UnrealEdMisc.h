...
public:
	 ...
	 */
	UE_DEPRECATED(5.6, "Please use FGlobalTabmanager::Get()->SetCanSavePersistentLayouts instead.")
	UNREALED_API void AllowSavingLayoutOnClose(bool bIsEnabled);

	/** Returns true if saving layout on close is allowed. */
	UE_DEPRECATED(5.6, "Please use FGlobalTabmanager::Get()->CanSavePersistentLayouts instead.")
	UNREALED_API bool IsSavingLayoutOnClosedAllowed();
...
private:
	...
	/** Pointer to the EditorAxisDisplayInfo implementation. */
	TUniquePtr<FEditorAxisDisplayInfo> EditorAxisDisplayInfo;
	...
	/** Handle used to register the "OnProfileGPU" delegate in the RHI. */
	FDelegateHandle OnProfileGPUHandle;
};
