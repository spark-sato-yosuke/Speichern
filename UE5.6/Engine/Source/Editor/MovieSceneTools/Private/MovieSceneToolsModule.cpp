...
void FMovieSceneToolsModule::StartupModule()
{
	...
	if (GIsEditor)
	{
		...
		ISequencerCoreModule& SequencerCoreModule = FModuleManager::Get().LoadModuleChecked<ISequencerCoreModule>("SequencerCore");
		SequencerCoreModule.RegisterModelType(UMovieSceneScalingAnchors::StaticClass(), FScalingAnchorsModel::ID);
	}
...
void FMovieSceneToolsModule::FixupDynamicBindingsEvent(ULevelSequence* LevelSequence)
{
	if (LevelSequence)
	{
		if (UBlueprint* DirectorBlueprint = LevelSequence->GetDirectorBlueprint())
		{
			...
			FKismetEditorUtilities::CompileBlueprint(DirectorBlueprint, EBlueprintCompileOptions::SkipGarbageCollection);
		}
	}
}
