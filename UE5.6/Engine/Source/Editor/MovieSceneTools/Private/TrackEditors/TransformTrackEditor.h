...
private:
	...
	void ProcessKeyOperation(UObject* ObjectToKey, TArrayView<const UE::Sequencer::FKeySectionOperation> SectionsToKey, ISequencer& InSequencer, FFrameNumber KeyTime, TArray<UE::Sequencer::FAddKeyResult>* OutResults = nullptr);
	...
	/*In the case the object is being animated with root motion, return the offset from the keyed transform value that root motion is currently applying.*/
	FTransform GetRootMotionOffset(UObject* AnimatedObject) const;

	/*In the case the object is being animated with root motion, updates the offset based on the current object position.*/
	void UpdateRootMotionOffset(UObject* AnimatedObject);
