...
#ifndef ENABLE_PAKFILE_USE_DIRECTORY_TREE
...
{ 
	...
	struct FPakEntryFilenameSetKeyFuncs;
}
namespace UE::IoStore { class IFileIoDispatcherBackend; }
		...
		else
		{
			...
			else
			{
				...
				for (int32 Index = 1; Index < CompressionMethods.Num(); Index++)
				{
					...
					FCStringAnsi::Strncpy(MethodString, TCHAR_TO_ANSI(*CompressionMethods[Index].ToString()), CompressionMethodNameLen);
				}
				Ar.Serialize(Methods, BufferSize);
			}
		}
...
namespace UE::PakFile::Private
{
	...
	typedef TDirectoryTree<FPakDirectory> FDirectoryTreeIndex;
#else
	struct FNullDirectoryTreeIndex
	{
	};
	typedef FNullDirectoryTreeIndex FDirectoryTreeIndex;
#endif
}
...
private:
	...
	using FDirectoryTreeIndex = UE::PakFile::Private::FDirectoryTreeIndex;

	FDirectoryTreeIndex DirectoryTreeIndex;
	...
	FDirectoryTreeIndex PrunedDirectoryTreeIndex;
...
public:
	...
	enum class EIteratorType : uint8
	{
		PathHash,
		DirectoryIndex,
		DirectoryTree,
	...
	private:
		...
		TOptional<FDirectoryIndex::TConstIterator> DirectoryIndexIt;
#if ENABLE_PAKFILE_USE_DIRECTORY_TREE
		/**
		 * Outer iterator over Directories when using the FDirectoryTreeIndex.
		 * A TDirectoryTree::FConstIterator has a large struct size. Store it as a TUniquePtr rather than a TOptional
		 * so that the struct size of our struct can remain small.
		 */
		TUniquePtr<FDirectoryTreeIndex::FConstIterator> TreeIt;
#endif
		/** Inner iterator over Files when using the FDirectoryIndex or FDirectoryTreeIndex. */
		TOptional<FPakDirectory::TConstIterator> FileIt;
		/** Iterator when using the FPathHashIndex. */
		TOptional<FPathHashIndex::TConstIterator> PathHashIt;
		...
		/* Which type of internal iterator this iterator and its pakfile are using. */
	...
	public:
		...
		// PakFile iterators can be large, because they have to walk a tree, so do not copy them, move construct only.
		...
		PAKFILE_API explicit operator bool() const;
	...
	private:
		...
		/** With EIteratorType::PathHash, we have only a single iterator. */
		FPathHashIndex::TConstIterator& GetPathHashIt();
		const FPathHashIndex::TConstIterator& GetPathHashIt() const;

		/**
		 * With EIteratorType::DirectoryIndex or EIteratorType::DirectoryTree, we have an outer iterator that
		 * traverses directories in the tree, and an inner iterator that traverses files in the directory.
		 */
		bool IsDirectoryItValid() const;
		void IncrementDirectoryIt();
		FStringView GetDirectoryItKey() const;
		const FPakDirectory& GetDirectoryItValue() const;
		FPakDirectory::TConstIterator& GetFileIt();
		const FPakDirectory::TConstIterator& GetFileIt() const;

		/** With EIteratorType::DirectoryIndex, GetDirectoryIndexIt is the outer iterator of directories. */
		FDirectoryIndex::TConstIterator& GetDirectoryIndexIt();
		const FDirectoryIndex::TConstIterator& GetDirectoryIndexIt() const;

#if ENABLE_PAKFILE_USE_DIRECTORY_TREE
		/** With EIteratorType::DirectoryTree, GetTreeIt is the outer iterator of directories. */
		FDirectoryTreeIndex::FConstIterator& GetTreeIt();
		const FDirectoryTreeIndex::FConstIterator& GetTreeIt() const;
#endif
	};
	...
	{
		FStringView Path(InOutPath), Filename;
		const bool bResult = SplitPathInline(Path, Filename);
...
private:
	...
	/**
	 * Returns whether the current PakFile is using a TDirectoryTree for storage of its DirectoryTreeIndex rather than a TMap.
	 * Returns false if !ENABLE_PAKFILE_USE_DIRECTORY_TREE or if PakFile TDirectoryTrees are disabled.
	 */
	PAKFILE_API bool ShouldUseDirectoryTree() const;
	...
	/**
	  ...
	  * @param DirectoryTreeIndex Optional FDirectoryTreeIndex into which to insert the (Filename, FPakEntryLocation)
	  ...
	  * @param PakFileVersion Version of the pakfile containing the index, to support legacy formats
	  */
	static void AddEntryToIndex(const FString& Filename, const FPakEntryLocation& EntryLocation,
		const FString& MountPoint, uint64 PathHashSeed, FDirectoryIndex* DirectoryIndex,
		FDirectoryTreeIndex* DirectoryTreeIndex, FPathHashIndex* PathHashIndex,
		TMap<uint64, FString>* CollisionDetection, int32 PakFileVersion);
	...
	bool LoadIndexInternal(FArchive& Reader, FDirectoryIndex& OutDirectoryTMap,
	...
	void LoadIndexInternal_DirectoryIndex(FArchive& Ar, FDirectoryIndex& OutDirectoryTMap,
		FDirectoryTreeIndex& OutDirectoryTree, bool bLoadIntoDirectoryTMap, bool bLoadIntoDirectoryTree);
	// This is an internal function that should only be called by FPakFile or by PakFileUtilities.cpp; it can change without deprecation.
public:
	PAKFILE_API static void SaveIndexInternal_DirectoryIndex(FArchive& Ar, const FDirectoryIndex& DirectoryTMap);
private:

	/* Legacy index loading function for PakFiles saved before FPakInfo::PakFile_Version_PathHashIndex */
	bool LoadLegacyIndex(FArchive& Reader, FDirectoryIndex& OutDirectoryTMap);
	...
	static void PruneDirectoryIndexInternal(FDirectoryIndex* InOutDirectoryIndex,
		FDirectoryTreeIndex* InOutDirectoryTreeIndex, FDirectoryIndex* PrunedDirectoryIndex,
		FDirectoryTreeIndex* PrunedDirectoryTreeIndex, const FString& MountPoint);

	template <typename ShouldVisitFunc>
	struct FVisitFilter
	{
		FVisitFilter(const ShouldVisitFunc& InShouldVisit, bool bInIncludeFiles, bool bInIncludeDirectories, bool bInRecursive);
		const ShouldVisitFunc& ShouldVisit;
	...
	void FindPrunedFilesAtPathInternal(const TCHAR* InPath, ContainerType& OutFiles, const FVisitFilter<ShouldVisitFunc>& VisitFilter) const;

	/**
	 * Lookup the FPakEntryLocation stored in the given DirectoryIndex or DirectoryTreeIndex,
	 * return nullptr if not found
	 */
	PAKFILE_API const FPakEntryLocation* FindLocationFromIndex(const FString& FullPath,
	 ...
	 */
	template <typename ShouldVisitFunc, class ContainerType>
	void FindFilesAtPathInIndex(const FDirectoryIndex& TargetIndex, const FDirectoryTreeIndex& TargetTreeIndex,
		ContainerType& OutFiles, const FString& FullSearchPath,
		const FVisitFilter<ShouldVisitFunc>& VisitFilter) const;

#if ENABLE_PAKFILE_USE_DIRECTORY_TREE
	template <typename ShouldVisitFunc, class ContainerType>
	void FindFilesAtPathInTreeIndexInternal(FStringView RelSearchPath, const FDirectoryTreeIndex& TargetTreeIndex,
	...
	template <class ContainerType>
	bool ValidateDirectoryTreeSearchConsistency(const ContainerType& FilesTree, const TArray<FString>& DirectoriesInPakTree, const ContainerType& FilesIndexed, const TArray<FString>& DirectoriesInPakIndexed) const;
#endif	// UE_BUILD_SHIPPING
#endif	// ENABLE_PAKFILE_USE_DIRECTORY_TREE

	template <typename ShouldVisitFunc, class ContainerType>
	void FindFilesAtPathInIndexInternal(const FStringView& RelSearchPath, const FDirectoryIndex& TargetIndex,
		ContainerType& OutFiles, TArray<FString>& OutDirectories,
		const FString& FullSearchPath, const FVisitFilter<ShouldVisitFunc>& VisitFilter) const;

	template <typename ShouldVisitFunc, class ContainerType>
	static void FindFilesAtPathInPakDirectoryInternal(const FString& MountPoint, FStringView RelPathInIndex,
	...
	const FPakDirectory* FindPrunedDirectoryInIndexInternal(const FString& RelativePathFromMount,
...
{
	// Pak filename
	const TCHAR* PakFilename = nullptr;
	// Search order
	uint32 PakOrder = 0;
...
class FPakPlatformFile : public IPlatformFile
{
	...
	mutable FTransactionallySafeCriticalSection PakListCritical;
	...
	TSharedPtr<UE::IoStore::IFileIoDispatcherBackend> IoDispatcherFileBackend;
	...
	*/
	PAKFILE_API static int32 GetPakOrderFromPakFilePath(const FStringView PakFilePath);
	 ...
	 */
	friend struct UE::PakFile::Private::FHandleMountPaksExDelegate;
...
public:
	...
	*/
	FORCEINLINE void GetMountedChunkIds(TSet<int32>& OutChunkIds)
	{
		UE::TScopeLock ScopedLock(PakListCritical);
		OutChunkIds.Empty(PakFiles.Num());
		for (FPakListEntry& Entry : PakFiles)
		{
			OutChunkIds.Add(Entry.PakFile->PakGetPakchunkIndex());
		}
	}
	...
	*/
	 ...
	 * @param OutPakListEntry for internal use
	 * @param OutIoMountStatus Status of the pak or associated iostore mount
	 */
	 ...
	 * In most cases, this should not be called directly. Instead use the mounting delegates in FCoreDelegates
	 *
	 * @param MountArgs mount arguments
	 * @param OutIoMountStatus Status of the pak or associated iostore mount
	 * @param OutPakListEntry for internal use
	 */
	...
	virtual FString GetFilenameOnDisk(const TCHAR* Filename) override
	{
		...
		if (FindFileInPakFiles(Filename, &PakFile, &FileEntry))
		{
			...
			if (PakDirectory != nullptr)
			{
				...
				{
					FPakEntry PakEntry;
					if (PakFile->GetPakEntry(FileIt.Value(), &PakEntry) != FPakFile::EFindResult::NotFound && PakEntry.Offset == FileEntry.Offset)
					{
						const FString& RealFilename = FileIt.Key();
	...
	PAKFILE_API virtual IAsyncReadFileHandle* OpenAsyncRead(const TCHAR* Filename, bool bAllowWrite = false) override;
	...
	/**
	 * Requests that all the known paks re-evaluate and set their PakchunkIndex value.
	 * Used to allow the AssetManager inform the pak file system about potential name pak chunk overrides.
	 */
