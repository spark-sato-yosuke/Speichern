...
void FVirtualTextureSpace::ApplyUpdates(FVirtualTextureSystem* System, FRDGBuilder& GraphBuilder, FRDGExternalAccessQueue& ExternalAccessQueue)
{
	...
	if (UpdateBuffer == nullptr || TotalNumUpdates * sizeof(FPageTableUpdate) > UpdateBuffer->GetSize())
	{
		...
		const FRHIBufferCreateDesc CreateDesc =
			FRHIBufferCreateDesc::CreateVertex(TEXT("VirtualTexture_PageTableUpdateBuffer"), NewBufferSize)
			.AddUsage(EBufferUsageFlags::ShaderResource | EBufferUsageFlags::Volatile)
			.DetermineInitialState();

		UpdateBuffer = RHICmdList.CreateBuffer(CreateDesc);
		UpdateBufferSRV = RHICmdList.CreateShaderResourceView(
			UpdateBuffer, 
