...
static bool SupportsGlobalDistanceFieldCoverage(EShaderPlatform Platform)
{
	return RHISupportsVolumeTextureAtomics(Platform);
}
	...
	bool bLumenEnabled)
{
	...
	if (View.ViewState)
	{
...

		{
			...
			if (bLumenEnabled && SupportsGlobalDistanceFieldCoverage(View.GetShaderPlatform()))

			{
				if (!GlobalDistanceFieldData.CoverageAtlasTexture
			...
			{
				if (GlobalDistanceFieldData.CoverageAtlasTexture != nullptr)
				{
					bSharedDataReallocated = true;
				}
				GlobalDistanceFieldData.CoverageAtlasTexture = nullptr;
			}
