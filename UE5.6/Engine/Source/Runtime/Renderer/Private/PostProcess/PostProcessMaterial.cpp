	...
	bool bNeuralPostProcessPrepass = false)
{
	...
	while (MaterialProxy)
	{
		...
		if (Material && Material->GetMaterialDomain() == MD_PostProcess)
		{
			...
			if (GetMaterialShaders(*Material, bManualStencilTest, bNeuralPostProcessPrepass, OutVertexShader, OutPixelShader))
			{
				break;
			}
		}
...
)
{
	...
	auto AddPSOInitializer = [&](bool bManualStencilTest)
	{
		...
		bool bNeuralPostProcessPrepass = false;
		if (!GetMaterialShaders(Material, bManualStencilTest, bNeuralPostProcessPrepass, VertexShader, PixelShader))
		{
			return;
		}
