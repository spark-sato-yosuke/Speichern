...
TAutoConsoleVariable<bool> CVarTSRThinGeometryCoverageHair(
	TEXT("r.TSR.ThinGeometryDetection.Coverage.Hair"), true,
	TEXT("Include hair as thin geometry and accumulate the coverage."),
	ECVF_RenderThreadSafe);
...
TAutoConsoleVariable<int32> CVarTSRThinGeometryDetectionWeightRelaxationSky(
	TEXT("r.TSR.ThinGeometryDetection.WeightRelaxation.Sky"), 1,
...
{
	return (CVarTSRThinGeometryDetection.GetValueOnRenderThread()!=0 && CVarTSRThinGeometryDetectionWeightRelaxation.GetValueOnRenderThread()!=0
...
class FTSRMeasureThinGeometryCoverageCS : public FTSRShader
{
	...
	class FHairAsThinGeometryDim : SHADER_PERMUTATION_BOOL("DIM_HAIR_AS_THIN_GEOMETRY");
	using FPermutationDomain = TShaderPermutationDomain<FHairAsThinGeometryDim>;
...
{
	return CVarTSRThinGeometryDetection.GetValueOnRenderThread() != 0;
}
...
{
	const bool bRecordThinGeometryCoverage = CVarTSRThinGeometryDetection.GetValueOnRenderThread() != 0; 
	...
	bool HairAsThinGeometry = CVarTSRThinGeometryCoverageHair.GetValueOnRenderThread();
	FTSRMeasureThinGeometryCoverageCS::FPermutationDomain PermutationVector;
	PermutationVector.Set<FTSRMeasureThinGeometryCoverageCS::FHairAsThinGeometryDim>(HairAsThinGeometry);
	...
	FComputeShaderUtils::AddPass(
		...
		RDG_EVENT_NAME("TSR MeasureThinGeometryCoverage(#%d%s) %dx%d", 
			...
			HairAsThinGeometry ? TEXT(" Hair"): TEXT(""),
			ViewRect.Width(), 
			ViewRect.Height()),
...
FTSRPassConfig GetTSRMainPassConfig(const FViewInfo& View)
{
	...
	PassConfig.ThinGeometryDetectionEnable =  static_cast<bool>(CVarTSRThinGeometryDetection.GetValueOnRenderThread());
	...
	if (IsVisualizeTSREnabled(View))
	{
		...
		if (bIsOverviewVisualize)
		{
			...
			if (Visualization == EVisualizeId::Overview || Visualization == EVisualizeId::ShowFlag)
			{
				...
				if (PassConfig.ThinGeometryDetectionEnable)
				{
					Tiles[4 * 2 + 3] = Visualize(EVisualizeId::ThinGeometry, TEXT("ThinGeometry"));
				}
			}
