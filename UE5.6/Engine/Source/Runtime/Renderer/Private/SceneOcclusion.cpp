...
public:
	virtual void InitRHI(FRHICommandListBase& RHICmdList) override
	{
		...
		const FRHIBufferCreateDesc CreateDesc =
			FRHIBufferCreateDesc::CreateIndex(TEXT("FOcclusionQueryIndexBuffer"), SizeInBytes, Stride)
...
FOcclusionQueryBatcher::FOcclusionQueryBatcher(class FSceneViewState* ViewState,uint32 InMaxBatchedPrimitives, uint32 InNumInstances)
...
static void PrepareDirectionalLightShadowOcclusionQuery(uint32& BaseVertexIndex, TRHIBufferInitializer<FVector3f>& DestinationBuffer, const FViewInfo& View, const FProjectedShadowInfo& ProjectedShadowInfo)
{
	const FViewMatrices& ViewMatrices = View.GetShadowViewMatrices();
	...
	float HalfFOV = ViewMatrices.IsPerspectiveProjection() ? FMath::Atan(1.0f / ProjectionMatrix.M[0][0]) : PI / 4.0f;
...
static void PrepareProjectedShadowOcclusionQuery(uint32& BaseVertexIndex, TRHIBufferInitializer<FVector3f>& DestinationBuffer, const FViewInfo& View, const FProjectedShadowInfo& ProjectedShadowInfo)
{
	// The shadow transforms and view transforms are relative to different origins, so the world coordinates need to
	// be translated.
	const FVector4f PreShadowToPreViewTranslation(FVector3f(View.ViewMatrices.GetPreViewTranslation() - ProjectedShadowInfo.PreShadowTranslation), 0);
	...
	for (uint32 Z = 0; Z < 2; Z++)
	{
		for (uint32 Y = 0; Y < 2; Y++)
		{
			for (uint32 X = 0; X < 2; X++)
			{
				...
				const uint16 CubeVertexIndex = GetCubeVertexIndex(X, Y, Z);

				DestinationBuffer.WriteValueAtIndex(BaseVertexIndex + CubeVertexIndex, ProjectedVertex);
			}
		}
	}
...
static bool AllocatePlanarReflectionOcclusionQuery(const FViewInfo& View, const FPlanarReflectionSceneProxy* SceneProxy, int32 NumBufferedFrames, FRHIRenderQuery*& OcclusionQuery)
{
	...
	if (View.GetCullingFrustum().IntersectBox(SceneProxy->WorldBounds.GetCenter(), SceneProxy->WorldBounds.GetExtent()))
	{
...
static void PreparePlanarReflectionOcclusionQuery(uint32& BaseVertexIndex, TRHIBufferInitializer<FVector3f>& DestinationBuffer, const FViewInfo& View, const FPlanarReflectionSceneProxy* SceneProxy)
{
	...
	DestinationBuffer.WriteValueAtIndex(BaseVertexIndex + 0, FVector3f(PrimitiveBoxMin.X, PrimitiveBoxMin.Y, PrimitiveBoxMin.Z));
	DestinationBuffer.WriteValueAtIndex(BaseVertexIndex + 1, FVector3f(PrimitiveBoxMin.X, PrimitiveBoxMin.Y, PrimitiveBoxMax.Z));
	DestinationBuffer.WriteValueAtIndex(BaseVertexIndex + 2, FVector3f(PrimitiveBoxMin.X, PrimitiveBoxMax.Y, PrimitiveBoxMin.Z));
	DestinationBuffer.WriteValueAtIndex(BaseVertexIndex + 3, FVector3f(PrimitiveBoxMin.X, PrimitiveBoxMax.Y, PrimitiveBoxMax.Z));
	DestinationBuffer.WriteValueAtIndex(BaseVertexIndex + 4, FVector3f(PrimitiveBoxMax.X, PrimitiveBoxMin.Y, PrimitiveBoxMin.Z));
	DestinationBuffer.WriteValueAtIndex(BaseVertexIndex + 5, FVector3f(PrimitiveBoxMax.X, PrimitiveBoxMin.Y, PrimitiveBoxMax.Z));
	DestinationBuffer.WriteValueAtIndex(BaseVertexIndex + 6, FVector3f(PrimitiveBoxMax.X, PrimitiveBoxMax.Y, PrimitiveBoxMin.Z));
	DestinationBuffer.WriteValueAtIndex(BaseVertexIndex + 7, FVector3f(PrimitiveBoxMax.X, PrimitiveBoxMax.Y, PrimitiveBoxMax.Z));
...
{
	SCOPED_NAMED_EVENT(FSceneRenderer_AllocateOcclusionTests, FColor::Emerald);
	...
	for (int32 ViewIndex = 0; ViewIndex < Views.Num(); ViewIndex++)
	{
		...
		if (View.bIsSinglePassStereo && IStereoRendering::IsASecondaryPass(View.StereoPass))
		{
			continue;
		}
	...
	uint32 DownsampleFactor)
{
	...
	for (int32 ViewIndex = 0; ViewIndex < Views.Num(); ViewIndex++)
	{
		...
		// Set stereo viewports while accounting for the downsample factor
		if (View.IsInstancedStereoPass())
		{
			if (View.bIsMultiViewportEnabled)
			{
				const FIntRect ViewRect = GetDownscaledRect(View.ViewRect, DownsampleFactor);
				const FIntRect SecondaryViewRect = GetDownscaledRect(View.GetInstancedView()->ViewRect, DownsampleFactor);
				RHICmdList.SetStereoViewport(ViewRect.Min.X, SecondaryViewRect.Min.X, ViewRect.Min.Y, SecondaryViewRect.Min.Y, 0.0f, ViewRect.Max.X, SecondaryViewRect.Max.X, ViewRect.Max.Y, SecondaryViewRect.Max.Y, 1.0f);
			}
			else
			{
				const FIntRect ViewRect = GetDownscaledRect(View.ViewRectWithSecondaryViews, DownsampleFactor);
		...
		if (NumVertices > 0)
		{
			...
			const FRHIBufferCreateDesc CreateDesc =
				FRHIBufferCreateDesc::CreateVertex<FVector3f>(TEXT("ViewOcclusionTests"), NumVertices)
				.AddUsage(EBufferUsageFlags::Volatile)
				.SetInitialState(ERHIAccess::VertexOrIndexBuffer)
				.SetInitActionInitializer();

			TRHIBufferInitializer<FVector3f> Initializer = RHICmdList.CreateBufferInitializer(CreateDesc);

			{
				for (FProjectedShadowInfo const* Query : ViewQuery.CSMQueryInfos)
				{
					PrepareDirectionalLightShadowOcclusionQuery(BaseVertexOffset, Initializer, View, *Query);
					checkSlow(BaseVertexOffset <= NumVertices);
				}

				for (FProjectedShadowInfo const* Query : ViewQuery.ShadowQueryInfos)
				{
					PrepareProjectedShadowOcclusionQuery(BaseVertexOffset, Initializer, View, *Query);
					checkSlow(BaseVertexOffset <= NumVertices);
				}

				for (FPlanarReflectionSceneProxy const* Query : ViewQuery.ReflectionQueryInfos)
				{
					PreparePlanarReflectionOcclusionQuery(BaseVertexOffset, Initializer, View, Query);
					checkSlow(BaseVertexOffset <= NumVertices);
				}
			}

			FBufferRHIRef VertexBufferRHI = Initializer.Finalize();
...
void FSceneRenderer::FenceOcclusionTests(FRDGBuilder& GraphBuilder)
{
	if (DoOcclusionQueries() && IsRunningRHIInSeparateThread())
	{
		AddPass(GraphBuilder, RDG_EVENT_NAME("FenceOcclusionTests"), [this](FRDGAsyncTask, FRHICommandList& RHICmdList)
		{
			...
			// We implement a queue of buffered fences, so we can avoid waiting on queries for another view family that's rendering
			// in the same frame.  Here we push a new fence into the queue of buffered fences. We assume that the queue isn't full,
			// since WaitOcclusionTests (called earlier in the frame) will always pop at least one fence if the queue is full.
...
void FOcclusionFeedback::ReadbackResults(FRHICommandListImmediate& RHICmdList)
{
	...
	if (!bReady)
	{
			...
			&& ResultsOcclusionFrameCounter < TailOcclusionBuffer.OcclusionFrameCounter)
		{
			...
			FRenderThreadIdleScope IdleScope(ERenderThreadIdleTypes::WaitingForGPUQuery);
			TailOcclusionBuffer.ReadbackBuffer->Wait(RHICmdList, FRHIGPUMask::All());
			bReady = true;
...
public:
	...
	BEGIN_SHADER_PARAMETER_STRUCT(FParameters, )
		...
		SHADER_PARAMETER(uint32, InstancedViewIndex)
	END_SHADER_PARAMETER_STRUCT()
};
...
void FOcclusionFeedback::SubmitOcclusionDraws(FRHICommandList& RHICmdList, FViewInfo& View)
{
	...
	for (int32 BatchIndex = 0, NumBatches = BatchOcclusionQueries.Num(); BatchIndex < NumBatches; BatchIndex++)
	{
		...
		VSParams.View = View.GetShaderParameters();
		...
		// Because the RHI does not yet support instance divisors, Occlusion Feedback supports instanced stereo 
		// by issuing a draw call for each eye. But both draw calls mark the same primitives, so it does not need to duplicate that state. 
		const uint32 InstanceCount = View.bIsInstancedStereoEnabled ? 2 : 1;
		for (uint32 InstancedViewIndex = 0; InstancedViewIndex < InstanceCount; ++InstancedViewIndex)
		{
			VSParams.InstancedViewIndex = InstancedViewIndex;
