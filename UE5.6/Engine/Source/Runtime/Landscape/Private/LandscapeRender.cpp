...
static TAutoConsoleVariable<int32> CVarRayTracingLandscapeVertexMask(
	...
	1,
	TEXT("If landscape should use vertex mask when building ray tracing dynamic geometry"),
	FConsoleVariableDelegate::CreateStatic(&OnCVarNeedingRenderStateInvalidationChanged),
	ECVF_RenderThreadSafe
);
...
struct FLandscapeRayTracingState
{
	...
	FLandscapeRayTracingImpl* Pimpl = nullptr;
	...
	FLandscapeRayTracingState* ViewStateNext = nullptr;
	FLandscapeRayTracingState** ViewStatePrev = nullptr;
...
#endif
{
	...
	SharedBuffersKey = (SubsectionSizeLog2 & 0xf) | ((NumSubsections & 0xf) << 4) | ((NumRayTracingSections & 0xf) << 8) | (XYOffsetmapTexture == nullptr ? 0 : 1 << 31);
	...
	else
	{
		for (const FMaterialRelevance& MaterialRelevance : MaterialRelevances)
		{
			...
			if (EditToolRenderData.SelectedType)
			{
				...
				{
					Result.bDynamicRelevance = true;
					ToolRelevance |= SelectionRegionMaterial->GetRelevance_Concurrent(FeatureLevel);
				}
				...
				{
					Result.bDynamicRelevance = true;
					ToolRelevance |= SelectionColorMaterial->GetRelevance_Concurrent(FeatureLevel);
				}
			}
			...
			{
				Result.bDynamicRelevance = true;
				ToolRelevance |= MaskRegionMaterial->GetRelevance_Concurrent(FeatureLevel);
			}

			if (GLandscapeViewMode == ELandscapeViewMode::LayerContribution)
			{
				Result.bDynamicRelevance = true;
				ToolRelevance |= ColorMaskRegionMaterial->GetRelevance_Concurrent(FeatureLevel);
			}

			if (CVarLandscapeShowDirty.GetValueOnRenderThread() && LandscapeDirtyMaterial)
			{
				Result.bDynamicRelevance = true;
				ToolRelevance |= LandscapeDirtyMaterial->GetRelevance_Concurrent(FeatureLevel);
			}
