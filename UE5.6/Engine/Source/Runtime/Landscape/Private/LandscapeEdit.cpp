	...
	if (GIsEditor && !HasAnyFlags(RF_ClassDefaultObject))
	{
		...
		if (Info)
		{
			...
			for (const FWeightmapLayerAllocationInfo& Allocation : LocalWeightmapLayerAllocations)
			{
				...
				{
					if (Landscape->HasTargetLayer(Allocation.LayerInfo->LayerName))
					{
						if (ULandscapeLayerInfoObject* NewLayerInfo = Landscape->GetTargetLayers()[Allocation.LayerInfo->LayerName].LayerInfoObj)
						{
			...
			{
				TArray<FString> LayerStrings;
				Algo::Transform(LayersToDelete, LayerStrings, [](ULandscapeLayerInfoObject* LayerInfo) { return LayerInfo ? LayerInfo->LayerName.ToString() : FString(TEXT("null")); });
			...
			if (!LayersToRemap.IsEmpty())
			{
				...
				Algo::Transform(LayersToRemap, LayerStrings, [](const TPair<ULandscapeLayerInfoObject*, ULandscapeLayerInfoObject*> &LayerToRemap)	
					{ 
						...
						return FString::Format(TEXT("{0} ({1} -> {2})"), {OldLayerInfo->LayerName.ToString(), OldLayerInfo->GetFullName(), NewLayerInfo->GetFullName()});
					});
				...
				for (FWeightmapLayerAllocationInfo& Allocation : LocalWeightmapLayerAllocations)
				{
					...
					{
						check(Allocation.GetLayerName() == (*NewLayerInfo)->LayerName);
...
#define MAX_HEIGHTMAP_TEXTURE_SIZE 512
	...
	if (CanHaveLayersContent())
	{
		...
		// CreateDefaultLayer or DuplicateLayerAndMoveBrushes must set the SelectedEditLayerIndex during import
		check(LandscapeActor->GetSelectedEditLayerIndex() != INDEX_NONE);
			...
			if (ImportWeightData != nullptr)
			{
				for (const FLandscapeImportLayerInfo& MaterialLayerInfo : *ImportWeightData)
				{
					if (MaterialLayerInfo.LayerInfo != nullptr && MaterialLayerInfo.LayerData.Num() > 0)
					{
						...
						if (!HasTargetLayer(MaterialLayerInfo.LayerInfo->LayerName))
						{
							AddTargetLayer(MaterialLayerInfo.LayerInfo->LayerName, FLandscapeTargetLayerSettings(MaterialLayerInfo.LayerInfo));
						}
					}
				}
			}
...
void ULandscapeInfo::ReplaceLayer(ULandscapeLayerInfoObject* FromLayerInfo, ULandscapeLayerInfoObject* ToLayerInfo)
{
	if (ensure(FromLayerInfo != ToLayerInfo))
	{
		...
		const FName NewName = ToLayerInfo != nullptr ? ToLayerInfo->LayerName : FromLayerInfo->LayerName;
...
void ALandscape::SplitHeightmap(ULandscapeComponent* Comp, ALandscapeProxy* TargetProxy, FMaterialUpdateContext* InOutUpdateContext, TArray<FComponentRecreateRenderStateContext>* InOutRecreateRenderStateContext, bool InReregisterComponent)
{
	...
	check(SrcProxy->HasLayersContent() == DstProxy->CanHaveLayersContent());
	const bool bWithLayersContent = SrcProxy->HasLayersContent() && DstProxy->CanHaveLayersContent();
	SrcProxy->Modify();
	DstProxy->Modify();
...

	{
		...
		// Don't bother copying the data if we're using edit layers, because that will be done in the next UpdateLayersContent. 
		//  It would also have a weird, pervert effect, because when edit layers are in use, the "InGenerateMips = true" argument passed to the following call to SetHeightData (default value) is actually
		//  ignored. This leads to the heightmap's FTextureSource having non-zero values on mip0 and zeroes on all the other mip levels, which is wrong because then FTextureSource's higher mips won't match what 
		//  the GPU texture's higher mips will contain after completion of UpdateLayersContent. You would expect that it would actually get fixed after the GPU readback, but this is not actually happening, because the 
		//  computed hash (see ULandscapeTextureHash::CalculateTextureHash64 in the following lines) only takes into account mip0! So when the readback occurs, it will consider that the FTextureSource and 
		//  the GPU texture are in sync (even though it's not the case since their mips 1 and above are different) and will *not* update FTextureSource's higher mips accordingly!
		if (!bWithLayersContent)
		{
			LandscapeEdit.SetHeightData(Comp->GetSectionBase().X, Comp->GetSectionBase().Y, Comp->GetSectionBase().X + Comp->ComponentSizeQuads, Comp->GetSectionBase().Y + Comp->ComponentSizeQuads, 
	...
	if (bWithLayersContent)
	{
		...
		const uint64 Hash = ULandscapeTextureHash::CalculateTextureHash64(NewHeightmapTexture, ELandscapeTextureType::Heightmap);
...
void ALandscapeProxy::PostEditChangeProperty(FPropertyChangedEvent& PropertyChangedEvent)
{
	...
	if (ULandscapeSubsystem* Subsystem = GetWorld()->GetSubsystem<ULandscapeSubsystem>(); Subsystem != nullptr && bChangedMaterial)
	{
