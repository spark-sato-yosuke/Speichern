...
public:
	...
	ConstResultType Get() const UE_LIFETIMEBOUND // this will give false positives when getting from an rvalue TFuture<T&> or TFuture<void> until C++20
	{
...
public:
	 ...
	 */
#ifndef __cpp_concepts
	MutableResultType GetMutable() UE_LIFETIMEBOUND // this will give false positives when getting from an rvalue TFuture<T&> or TFuture<void> until C++20
	{
...
public:
...
#ifndef __cpp_concepts
	template <
		typename DummyResultType = ResultType
		UE_REQUIRES(!std::is_void_v<DummyResultType>)
	>
#endif
	FORCEINLINE void SetValue(const SetType& Result)
	...
	template <
		typename DummyResultType = ResultType
		UE_REQUIRES(!std::is_void_v<DummyResultType> && !std::is_lvalue_reference_v<DummyResultType>)
	>
#endif
	FORCEINLINE void SetValue(SetType&& Result)
#ifdef __cpp_concepts
