	...
	virtual void Serialize(void* Data, int64 Num) override
	{
		...
		if( NumBytesToAdd > 0 )
		{
			...
			if constexpr (IndexSize < 64)
			{
				...
				{
					UE_LOG(LogSerialization, Fatal, TEXT("TMemoryWriter with IndexSize=%d does not support data larger than %" INT64_FMT " bytes. Archive name: %s."), IndexSize, (int64)MaxValue, *ArchiveName.ToString());
				}
			}
