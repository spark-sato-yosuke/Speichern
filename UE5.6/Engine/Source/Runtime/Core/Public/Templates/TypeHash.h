...
template <
	typename... Types
	UE_REQUIRES((std::is_convertible_v<Types, uint32> && ...))
>
[[nodiscard]] FORCEINLINE constexpr uint32 HashCombine(uint32 A, Types... Vals)
{
	if constexpr (sizeof...(Types) > 0)
	{
		A = HashCombine(A, HashCombine((uint32)Vals...));
	}
	return A;
}
 ...
 */
[[nodiscard]] inline constexpr uint32 HashCombineFast(uint32 A, uint32 B)
{
...
template <
	...
	UE_REQUIRES((std::is_convertible_v<Types, uint32> && ...))
>
[[nodiscard]] FORCEINLINE constexpr uint32 HashCombineFast(uint32 A, Types... Vals)
{
	if constexpr (sizeof...(Types) > 0)
	{
		A = HashCombineFast(A, HashCombineFast((uint32)Vals...));
	}
	return A;
}
...
template <typename T>
[[nodiscard]] FORCEINLINE uint32 GetTypeHashHelper(const T& V) { return static_cast<uint32>(GetTypeHash(V)); }

// Use this when in a scope that has AppendHash to dispatch AppendHash using unqualified lookup.
template <typename BuilderType, typename ArgType>
inline void DispatchAppendHash(BuilderType& Builder, const ArgType& Arg)
{
	AppendHash(Builder, Arg);
}
