...
public:
	...
	[[nodiscard]] TEnumAsByte() = default;
	[[nodiscard]] TEnumAsByte(const TEnumAsByte&) = default;
	 ...
	 */
	[[nodiscard]] FORCEINLINE TEnumAsByte( EnumType InValue )
	 ...
	 * @param InValue value to construct with.
	 */
	[[nodiscard]] explicit FORCEINLINE TEnumAsByte( int32 InValue )
		: Value(static_cast<uint8>(InValue))
	{
	}
	 ...
	 * @param InValue value to construct with.
	 */
	[[nodiscard]] explicit FORCEINLINE TEnumAsByte( uint8 InValue )
		: Value(InValue)
	{
	}
...
public:
	 ...
	 */
	[[nodiscard]] bool operator==( EnumType InValue ) const
	{
	 ...
	 * @return true if the two values are equal, false otherwise.
	 */
	[[nodiscard]] bool operator==(TEnumAsByte InValue) const
	{
		return Value == InValue.Value;
	}

	/** Implicit conversion to EnumType. */
	[[nodiscard]] operator EnumType() const
	{
...
public:
	 ...
	 */
	[[nodiscard]] EnumType GetValue() const
	{
		return (EnumType)Value;
	}
	 ...
	 * @return The enumeration value.
	 */
	[[nodiscard]] uint8 GetIntValue() const
	{
		return Value;
	}
...
template<class T>
[[nodiscard]] FORCEINLINE uint32 GetTypeHash(const TEnumAsByte<T>& Enum)
{
