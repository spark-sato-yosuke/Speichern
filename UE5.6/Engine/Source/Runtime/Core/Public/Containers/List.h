...
public:
	...
	[[nodiscard]] FORCEINLINE bool operator==(const TLinkedListIteratorBase& Rhs) const { return CurrentLink == Rhs.CurrentLink; }
	[[nodiscard]] FORCEINLINE bool operator!=(const TLinkedListIteratorBase& Rhs) const { return CurrentLink != Rhs.CurrentLink; }
protected:
...
public:
	 ...
	 */
	[[nodiscard]] FORCEINLINE bool IsLinked()
	{
		return PrevLink != nullptr;
	}

	[[nodiscard]] FORCEINLINE ContainerType** GetPrevLink() const
	{
		return PrevLink;
	}

	[[nodiscard]] FORCEINLINE ContainerType* GetNextLink() const
	{
		return NextLink;
	}

	[[nodiscard]] FORCEINLINE ContainerType* Next()
	{
		return NextLink;
	}
...
private:
	...
	[[nodiscard]] FORCEINLINE friend TIterator      begin(      ContainerType& List) { return TIterator     (&List); }
	[[nodiscard]] FORCEINLINE friend TConstIterator begin(const ContainerType& List) { return TConstIterator(const_cast<ContainerType*>(&List)); }
	[[nodiscard]] FORCEINLINE friend TIterator      end  (      ContainerType& List) { return TIterator     (nullptr); }
	[[nodiscard]] FORCEINLINE friend TConstIterator end  (const ContainerType& List) { return TConstIterator(nullptr); }
};
...
public:
	...
	[[nodiscard]] bool operator==(const TDoubleLinkedListIterator& Rhs) const { return CurrentNode == Rhs.CurrentNode; }
	[[nodiscard]] bool operator!=(const TDoubleLinkedListIterator& Rhs) const { return CurrentNode != Rhs.CurrentNode; }
...
public:
	...
	public:
		...
		/** Constructor */
		[[nodiscard]] TDoubleLinkedListNode( const ElementType& InValue )
	 ...
	 * @see GetTail
	 */
	[[nodiscard]] TDoubleLinkedListNode* GetHead() const
	{
		return HeadNode;
	}
	 ...
	 * @see GetHead
	 */
	[[nodiscard]] TDoubleLinkedListNode* GetTail() const
	{
		return TailNode;
	}
	 ...
	 */
	[[nodiscard]] TDoubleLinkedListNode* FindNode( const ElementType& InElement )
	{
...
private:
	...
	[[nodiscard]] friend TIterator      begin(      TDoubleLinkedList& List) { return TIterator     (List.GetHead()); }
	[[nodiscard]] friend TConstIterator begin(const TDoubleLinkedList& List) { return TConstIterator(List.GetHead()); }
	[[nodiscard]] friend TIterator      end  (      TDoubleLinkedList& List) { return TIterator     (nullptr); }
	[[nodiscard]] friend TConstIterator end  (const TDoubleLinkedList& List) { return TConstIterator(nullptr); }
};
...
public:
	...
	[[nodiscard]] TList(const ElementType &InElement, TList<ElementType>* InNext = nullptr)
	{
		Element = InElement;
		Next = InNext;
	}
