...
public:
	...
	template <
		...
		UE_REQUIRES(TIsCharEncodingCompatibleWith_V<CharType, FromType>)
	>
	[[nodiscard]] static int32 ConvertedLength(const CharType* Source, int32 SourceLen)
	{
...
namespace StringConv
{
	...
	[[nodiscard]] FORCEINLINE bool IsValidCodepoint(const uint32 Codepoint)
	{
	...
	[[nodiscard]] FORCEINLINE bool IsHighSurrogate(const uint32 Codepoint)
	{
	...
	[[nodiscard]] FORCEINLINE bool IsLowSurrogate(const uint32 Codepoint)
	{
		return Codepoint >= LOW_SURROGATE_START_CODEPOINT && Codepoint <= LOW_SURROGATE_END_CODEPOINT;
	}

	[[nodiscard]] FORCEINLINE uint32 EncodeSurrogate(const uint16 HighSurrogate, const uint16 LowSurrogate)
	{
	...
	[[nodiscard]] FORCEINLINE bool IsEncodedSurrogate(const uint32 Codepoint)
	{
...
namespace UE::Core::Private
{
	...
	{
		[[nodiscard]] FCountingOutputIterator()
			: Counter(0)
		{
		}

		[[nodiscard]] const FCountingOutputIterator& operator*() const
		{
			return *this;
		}
		...
		[[nodiscard]] friend int32 operator-(FCountingOutputIterator Lhs, FCountingOutputIterator Rhs)
		{
	...
	public:
		...
		[[nodiscard]] static int32 Utf8FromCodepoint(uint32 Codepoint, BufferType OutputIterator, uint32 OutputIteratorByteSizeRemaining)
		{
		...
		template <
			...
			UE_REQUIRES(TIsCharEncodingCompatibleWith_V<SrcBufferType, FromType>)
		>
		[[nodiscard]] static FORCEINLINE int32 ConvertedLength(const SrcBufferType* Source, int32 SourceLen)
		{
...
public:
	...
	template <
		...
		UE_REQUIRES(TIsCharEncodingCompatibleWith_V<SrcBufferType, FromType>)
	>
	[[nodiscard]] static int32 ConvertedLength(const SrcBufferType* Source, const int32 SourceLen)
	{
		return FPlatformString::ConvertedLength<ToType>((const FromType*)Source, SourceLen);
	}
	[[nodiscard]] static int32 ConvertedLength(const LegacyFromType* Source, const int32 SourceLen)
	{
		return FPlatformString::ConvertedLength<ToType>((const FromType*)Source, SourceLen);
	}
};
...
public:
	 ...
	 */
	template <typename BufferType>
	[[nodiscard]] static int32 Utf16FromCodepoint(uint32 Codepoint, BufferType OutputIterator, uint32 OutputIteratorNumRemaining)
	{
		// Ensure we have at least one character in size to write
...
private:
	[[nodiscard]] static uint32 CodepointFromUtf16(const FromType*& SourceString, const uint32 SourceLengthRemaining)
	{
...
public:
	...
	template <
		...
		UE_REQUIRES(TIsCharEncodingCompatibleWith_V<SrcBufferType, FromType>)
	>
	[[nodiscard]] explicit TStringConversion(const SrcBufferType* Source)
	{
		if (Source)
		{
	...
	[[nodiscard]] explicit TStringConversion(const LegacyFromType* Source)
	...
	template <
		...
		UE_REQUIRES(TIsCharEncodingCompatibleWith_V<SrcBufferType, FromType>)
	>
	[[nodiscard]] TStringConversion(const SrcBufferType* Source, int32 SourceLen)
	{
		if (Source)
		{
	...
	[[nodiscard]] TStringConversion(const LegacyFromType* Source, int32 SourceLen)
		: TStringConversion((const FromType*)Source, SourceLen)
	{
	}

	[[nodiscard]] explicit TStringConversion(UE::Core::Private::FFromStrCast, const FromType* Source, int32 SourceLen)
	{
	...
	>
	[[nodiscard]] TStringConversion(FromRangeType&& Source)
	 ...
	 */
	[[nodiscard]] FORCEINLINE const ToType* Get() const UE_LIFETIMEBOUND
	{
		return Ptr;
	}
	 ...
	 */
	[[nodiscard]] FORCEINLINE int32 Length() const
	{
		return StringLength;
	}
...
[[nodiscard]] inline auto GetData(const TStringConversion<Converter, DefaultConversionSize>& Conversion) -> decltype(Conversion.Get())
{
	return Conversion.Get();
}

template <typename Converter, int32 DefaultConversionSize>
[[nodiscard]] inline auto GetNum(const TStringConversion<Converter, DefaultConversionSize>& Conversion) -> decltype(Conversion.Length())
{
...
public:
	template <
		...
		UE_REQUIRES(TIsCharEncodingCompatibleWith_V<SrcBufferType, FromType>)
	>
	[[nodiscard]] explicit TStringPointer(const SrcBufferType* Source)
	{
		if (Source)
		{
	...
	template <
		...
		UE_REQUIRES(TIsCharEncodingCompatibleWith_V<SrcBufferType, FromType>)
	>
	[[nodiscard]] explicit TStringPointer(const SrcBufferType* Source, int32 SourceLen)
	{
		if (Source)
		{
			if (SourceLen > 0 && ((const FromType*)Source)[SourceLen-1] == 0)
	...
	[[nodiscard]] explicit TStringPointer(UE::Core::Private::FFromStrCast, const FromType* Source, int32 SourceLen)
		: Ptr((const ToType*)Source)
		, StringLength(SourceLen)
	{
	}
	...
	>
	[[nodiscard]] TStringPointer(FromRangeType&& Source)
	 ...
	 * Move constructor
	 */
	[[nodiscard]] TStringPointer(TStringPointer&& Other) = default;
...
[[nodiscard]] inline auto GetData(const TStringPointer<FromType, ToType>& Pointer) -> decltype(Pointer.Get())
{
	return Pointer.Get();
}

template <typename FromType, typename ToType>
[[nodiscard]] inline auto GetNum(const TStringPointer<FromType, ToType>& Pointer) -> decltype(Pointer.Length())
{
...
[[nodiscard]] FORCEINLINE auto StringCast(const From* Str)
{
 ...
 */
template <typename To, int32 DefaultConversionSize = DEFAULT_STRING_CONVERSION_SIZE, typename From>
[[nodiscard]] FORCEINLINE auto StringCast(const From* Str, int32 Len)
{
	if constexpr (TIsCharEncodingCompatibleWith_V<From, To>)
	{
 ...
 */
template <typename To, int32 DefaultConversionSize = DEFAULT_STRING_CONVERSION_SIZE, typename From>
[[nodiscard]] FORCEINLINE auto StrCast(const From* Str, int32 Len)
{
	if constexpr (TIsCharEncodingCompatibleWith_V<From, To>)
	{
...
[[nodiscard]] FORCEINLINE To CharCast(From Ch)
{
	To Result;
...
public:
	[[nodiscard]] FORCEINLINE TStringPassthru(ToType* InDest, int32 InDestLen, int32 InSrcLen)
		: Dest   (InDest)
		, DestLen(InDestLen)
		, SrcLen (InSrcLen)
	{
	...
	[[nodiscard]] FORCEINLINE TStringPassthru(TStringPassthru&& Other)
	{
	...
	[[nodiscard]] FORCEINLINE FromType* Get() UE_LIFETIMEBOUND
	{
...
{
public:
	[[nodiscard]] FORCEINLINE explicit TPassthruPointer(T* InPtr)
		: Ptr(InPtr)
	{
	}
	...
	[[nodiscard]] FORCEINLINE T* Get() const UE_LIFETIMEBOUND
	{
		return Ptr;
	}
...
[[nodiscard]] FORCEINLINE auto StringMemoryPassthru(To* Buffer, int32 BufferSize, int32 SourceLength)
{
	if constexpr (TIsCharEncodingCompatibleWith_V<From, To>)
	{
...
[[nodiscard]] FORCEINLINE TArray<ToType> StringToArray(const FromType* Src, int32 SrcLen)
{
...
template <typename ToType, typename FromType>
[[nodiscard]] FORCEINLINE TArray<ToType> StringToArray(const FromType* Str)
{
