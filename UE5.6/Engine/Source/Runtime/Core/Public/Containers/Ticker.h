...
public:
	...
	[[nodiscard]] static CORE_API FTSTicker& GetCoreTicker();
...
private:
	...
	struct FElement
	{
		...
		[[nodiscard]] CORE_API FElement();
		/** This is the ctor that the code will generally use. */
		[[nodiscard]] CORE_API FElement(double InFireTime, float InDelayTime, const FTickerDelegate& InDelegate);
		/** This is the ctor that the code will generally use. */
		[[nodiscard]] CORE_API FElement(double InFireTime, float InDelayTime, TUniqueFunction<bool(float)>&& InFunction);
...
public:
	...
	*/
	[[nodiscard]] CORE_API FTSTickerObjectBase(float InDelay = 0.0f, FTSTicker& Ticker = FTSTicker::GetCoreTicker());
