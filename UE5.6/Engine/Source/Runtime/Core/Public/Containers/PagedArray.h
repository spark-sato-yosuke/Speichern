...
protected:
	 ...
	 */
	[[nodiscard]] TIteratorBase(InPageType* Data, SizeType Offset)
	...
	[[nodiscard]] FORCEINLINE bool Equals(const TIteratorBase& Other) const
	{
...
private:
	[[nodiscard]] static constexpr SizeType GetPageIndex(SizeType Index)
	{
		return Index / PageTraits::Capacity;
	}

	[[nodiscard]] static constexpr SizeType GetPageOffset(SizeType Index)
	{
		return Index % PageTraits::Capacity;
	}

	[[nodiscard]] static constexpr SizeType NumRequiredPages(SizeType Count)
	{
...
public:
	...
	[[nodiscard]] static constexpr SizeType MaxPerPage()
	{
		return PageTraits::Capacity;
	}

	[[nodiscard]] TPagedArray() = default;

	[[nodiscard]] TPagedArray(TPagedArray&& Other) : Pages(MoveTemp(Other.Pages)), Count(Other.Count)
	{
		Other.Count = 0;
	}

	[[nodiscard]] TPagedArray(const TPagedArray& Other) = default;

	[[nodiscard]] TPagedArray(std::initializer_list<ElementType> InList)
	{
		CopyToEmpty(InList.begin(), static_cast<SizeType>(InList.size()));
	}

	[[nodiscard]] TPagedArray(const ElementType* InSource, SizeType InSize)
	{
	...
	void SetMem(uint8 ByteValue)
	{
		for (PageType& Page : Pages)
		{
			...
			std::memset((void*)Page.GetData(), ByteValue, Page.Num() * sizeof(ElementType));
		}
	}
	 ...
	 */
	void Pop(EAllowShrinking AllowShrinking = EAllowShrinking::Default)
	{
		CheckValidIndex(0);
		const SizeType LastIndex = Num() - 1;
		const SizeType LastPageIndex = GetPageIndex(LastIndex);
		const SizeType LastIndexInPage = GetPageOffset(LastIndex);
		Pages[LastPageIndex].RemoveAt(LastIndexInPage, EAllowShrinking::No);
	...
	void RemoveAtSwap(SizeType Index, EAllowShrinking AllowShrinking = EAllowShrinking::Default)
	{
		...
		{
			Pages[TargetPageIndex].RemoveAtSwap(TargetIndexInPage, EAllowShrinking::No);
	...
	// Friend operators
	[[nodiscard]] bool operator==(const TPagedArray& Right) const
	{
		if (Num() != Right.Num())
		{
			return false;
	...
	[[nodiscard]] FORCEINLINE bool operator!=(const TPagedArray& Right) const
	{
		return !(*this == Right);
	}
...
private:
	...
	[[nodiscard]] FORCEINLINE ConstIteratorType GetIterator(SizeType Offset) const
	{
		return ConstIteratorType(InPlace, Pages.GetData(), Offset);
	}

	[[nodiscard]] FORCEINLINE IteratorType GetIterator(SizeType Offset)
	{
