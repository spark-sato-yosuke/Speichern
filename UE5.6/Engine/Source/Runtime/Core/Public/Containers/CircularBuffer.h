...
public:
	 ...
	 */
	[[nodiscard]] explicit TCircularBuffer(uint32 Capacity)
	{
	 ...
	 */
	[[nodiscard]] TCircularBuffer(uint32 Capacity, const ElementType& InitialValue)
	{
		checkSlow(Capacity <= 0xffffffffU);
...
public:
	 ...
	 */
	[[nodiscard]] FORCEINLINE ElementType& operator[](uint32 Index)
	{
	 ...
	 * @param Index The index of the element to return.
	 */
	[[nodiscard]] FORCEINLINE const ElementType& operator[](uint32 Index) const
	{
		return Elements[Index & IndexMask];
	}
...
public:
	 ...
	 */
	[[nodiscard]] FORCEINLINE uint32 Capacity() const
	{
		return Elements.Num();
	}
	 ...
	 */
	[[nodiscard]] FORCEINLINE uint32 GetNextIndex(uint32 CurrentIndex) const
	{
	 ...
	 */
	[[nodiscard]] FORCEINLINE uint32 GetPreviousIndex(uint32 CurrentIndex) const
	{
		return ((CurrentIndex - 1) & IndexMask);
	}
