...
private:
	...
	[[nodiscard]] FORCEINLINE const ElementType* GetData() const
	{
		return (const ElementType*)Storage.GetAllocation();
	}

	[[nodiscard]] FORCEINLINE ElementType* GetData()
	{
		return (ElementType*)Storage.GetAllocation();
	}

	[[nodiscard]] FORCEINLINE ConstIteratorType GetIterator(SizeType HeadOffset) const
	{
		return ConstIteratorType(InPlace, GetData(), Max(), HeadOffset);
	}

	[[nodiscard]] FORCEINLINE IteratorType GetIterator(SizeType HeadOffset)
	{
