...
public:
	...
	[[nodiscard]] TArrayView(const TArrayView&) = default;
...
public:
	...
	>
	[[nodiscard]] FORCEINLINE TArrayView(OtherRangeType&& Other)
	...
	>
	[[nodiscard]] FORCEINLINE TArrayView(OtherRangeType&& Other UE_LIFETIMEBOUND)
		: DataPtr(std::conditional_t<
			TIsCompatibleRangeType<OtherRangeType>::Value,
			TIsCompatibleRangeType<OtherRangeType>,
			TIsReinterpretableRangeType<OtherRangeType>
	...
	>
	[[nodiscard]] FORCEINLINE TArrayView(OtherElementType* InData UE_LIFETIMEBOUND, SizeType InCount)
		: DataPtr(InData)
		, ArrayNum(InCount)
	{
		check(ArrayNum >= 0);
	 ...
	 */
	[[nodiscard]] FORCEINLINE TArrayView(std::initializer_list<ElementType> List UE_LIFETIMEBOUND)
	...
	[[nodiscard]] explicit TArrayView(FIntrusiveUnsetOptionalState)
		: DataPtr(nullptr)
		, ArrayNum(-1)
	{
	}
...
public:
	 ...
	 */
	[[nodiscard]] FORCEINLINE static constexpr size_t GetTypeSize()
	{
	 ...
	 */
	[[nodiscard]] FORCEINLINE static constexpr size_t GetTypeAlignment()
	{
	 ...
	 */
	[[nodiscard]] FORCEINLINE ElementType& Last(SizeType IndexFromTheEnd = 0) const
	{
	 ...
	 */
	[[nodiscard]] FORCEINLINE bool Find(const ElementType& Item, SizeType& Index) const
	{
	 ...
	 * @returns True if found. False otherwise.
	 */
	[[nodiscard]] FORCEINLINE bool FindLast(const ElementType& Item, SizeType& Index) const
	{
	...
	[[nodiscard]] SizeType FindLastByPredicate(Predicate Pred, SizeType StartIndex) const
	{
	...
	*/
	template <typename Predicate>
	[[nodiscard]] FORCEINLINE SizeType FindLastByPredicate(Predicate Pred) const
	{
	 ...
	 */
	template <typename KeyType>
	[[nodiscard]] ElementType* FindByKey(const KeyType& Key) const
	{
	 ...
	 */
	template <typename Predicate>
	[[nodiscard]] ElementType* FindByPredicate(Predicate Pred) const
	{
		for (ElementType* RESTRICT Data = GetData(), *RESTRICT DataEnd = Data + ArrayNum; Data != DataEnd; ++Data)
		{
			if (::Invoke(Pred, *Data))
	 ...
	 */
	template <typename Predicate>
	[[nodiscard]] TArray<std::remove_const_t<ElementType>> FilterByPredicate(Predicate Pred) const
	{
...
public:
	 ...
	 */
	[[nodiscard]] FORCEINLINE ElementType*                         begin () const { return GetData(); }
	[[nodiscard]] FORCEINLINE ElementType*                         end   () const { return GetData() + Num(); }
	[[nodiscard]] FORCEINLINE TReversePointerIterator<ElementType> rbegin() const { return TReversePointerIterator<ElementType>(GetData() + Num()); }
	[[nodiscard]] FORCEINLINE TReversePointerIterator<ElementType> rend  () const { return TReversePointerIterator<ElementType>(GetData()); }
...
>
[[nodiscard]] auto MakeArrayView(OtherRangeType&& Other)
{
...
template <
	...
	UE_REQUIRES(TIsContiguousContainer<CVUnqualifiedOtherRangeType>::Value && !TIsTArrayView_V<CVUnqualifiedOtherRangeType>)
>
[[nodiscard]] auto MakeArrayView(OtherRangeType&& Other UE_LIFETIMEBOUND)
{
	return TArrayView<std::remove_pointer_t<decltype(GetData(DeclVal<OtherRangeType&>()))>>(Forward<OtherRangeType>(Other));
}

template<typename ElementType>
[[nodiscard]] TArrayView<ElementType> MakeArrayView(ElementType* Pointer UE_LIFETIMEBOUND, int32 Size)
{
	return TArrayView<ElementType>(Pointer, Size);
}

template <typename ElementType>
[[nodiscard]] TArrayView<const ElementType> MakeArrayView(std::initializer_list<ElementType> List UE_LIFETIMEBOUND)
{
...
template <
	...
	UE_REQUIRES(TIsContiguousContainer<CVUnqualifiedOtherRangeType>::Value && TIsTArrayView_V<CVUnqualifiedOtherRangeType>)
>
[[nodiscard]] auto MakeConstArrayView(OtherRangeType&& Other)
{
...
template <
	...
	UE_REQUIRES(TIsContiguousContainer<CVUnqualifiedOtherRangeType>::Value && !TIsTArrayView_V<CVUnqualifiedOtherRangeType>)
>
[[nodiscard]] auto MakeConstArrayView(OtherRangeType&& Other UE_LIFETIMEBOUND)
{
	return TArrayView<const std::remove_pointer_t<decltype(GetData(DeclVal<OtherRangeType&>()))>>(Forward<OtherRangeType>(Other));
}
...
template <typename ElementType>
[[nodiscard]] TArrayView<const ElementType> MakeConstArrayView(std::initializer_list<ElementType> List UE_LIFETIMEBOUND)
{
	return TArrayView<const ElementType>(List.begin(), List.size());
}
...
>
[[nodiscard]] bool operator==(RangeType&& Lhs, TArrayView<ElementType> Rhs)
{
...
template <
	...
	UE_REQUIRES(std::is_convertible_v<decltype(GetData(std::declval<RangeType&>())), const ElementType*>)
>
[[nodiscard]] bool operator==(TArrayView<ElementType> Lhs, RangeType&& Rhs)
{
	return (Rhs == Lhs);
}
...
[[nodiscard]] FORCEINLINE TArray<InElementType, InAllocatorType>::TArray(const TArrayView<OtherElementType, OtherSizeType>& Other)
{
