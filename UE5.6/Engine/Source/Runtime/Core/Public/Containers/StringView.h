...
public:
	...
	[[nodiscard]] friend inline bool operator==(TStringView Lhs, TStringView Rhs)
	{
	...
	>
	[[nodiscard]] friend inline auto operator==(TStringView Lhs, CharRangeType&& Rhs)
	{
	...
	template <
		...
		UE_REQUIRES(std::is_convertible_v<CharRangeType&&, TStringView>)
	>
	[[nodiscard]] friend inline auto operator==(CharRangeType&& Lhs, TStringView Rhs)
	{
	...
	[[nodiscard]] friend inline bool operator==(TStringView Lhs, const CharType* Rhs)
	{
		return TStringView::PrivateEquals(Lhs, Rhs);
	}

	[[nodiscard]] friend inline bool operator==(const CharType* Lhs, TStringView Rhs)
	{
		return Rhs == Lhs;
	}
...
template <typename CharPtrOrRangeType,
	// This uses enable_if instead of concepts because it tries to instantiate TStringView<void> with concepts.
	// This can use concepts once TStringView constrains its CharType to character types.
	std::enable_if_t<!std::is_pointer_v<CharPtrOrRangeType> || TIsCharType_V<std::remove_pointer_t<CharPtrOrRangeType>>>* = nullptr>
