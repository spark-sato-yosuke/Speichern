...
enum EConsoleVariableFlags
{
	...
	// When this is set, this will stomp on existing tagged array type values in place, instead of adding to the end of the array
	...
	// Set by scalability groups from Scalability.ini (lower priority than game settings so it's easier to override partially). THIS IS AN ARRAY TYPE
	...
	// Per device settings using the DeviceProfiles.ini hierarchy (e.g. specific iOS device, higher priority than per project to do device specific settings). THIS IS AN ARRAY TYPE
...
{
	// Store a mapping from EShaderPlatform (as int) to a Platform that is currently cooking for that ShaderPlatform
	CORE_API void RegisterShaderPlatformToPlatformMapping(int ShaderPlatform, FName PlatformName);
	// Store a mapping from Platform to a DevicveProfile that is currently being used when cooking for that Platform
	CORE_API void RegisterPlatformToDeviceProfileMapping(FName PlatformName, FName DeviceProfileName);

	// Receives the mapping previously set with RegisterShaderPlatformToPlatformMapping
	// Returns NAME_None if no mapping exists
	CORE_API FName GetShaderPlatformToPlatformMapping(int ShaderPlatform);
	// Receives the mapping previously set with RegisterPlatformToDeviceProfileMapping
	// Returns NAME_None if no mapping exists
	CORE_API FName GetPlatformToDeviceProfileMapping(FName PlatformName);
}
...
public:
	...
	/**
	 * If IsShadowObject() returns true, this function will return the "real" variable the shadow cvar is shadowing. Generally not needed,
	 * as most functions (like GetInt, Set, etc) will operate on the real variable already
	 */
...
struct IConsoleManager
{
	...
	*/
	 ...
	 * @param Object to lookup. Must not be null.
	...
	/**
	 * Applies the cvars from the platform/DeviceProfile pair (can be blank to use the platform's named DP) and sets at the given SetBy priority, with the PreviewModeTag
	 * to be unset later. RequiredFlags must be present ont he CVar to be set, unless DisallowedFlags is present, then it will not set it
	 */
	virtual void StompPlatformCVars(FName PlatformName, const FString& DeviceProfileName, FName Tag, EConsoleVariableFlags SetBy, EConsoleVariableFlags RequiredFlags, EConsoleVariableFlags DisallowedFlags) = 0;
...
/**
 * Utility class to modify a console variable's value for a certain scope and restore it to its previous value afterwards.
 * Type parameter may be any supported by IConsoleVariable::GetValue, i.e. bool, int32, float, FString.
 */
template <typename T>
struct TGuardConsoleVariable
{
	T PreviousValue;
	...
	{
		if (ensureMsgf(ConsoleVariable, TEXT("TGuardConsoleVariable: Trying to set console variable \"%s\" but it doesn't exist"), ConsoleVariableName))
		{
			ConsoleVariable->GetValue(PreviousValue);
	...
	~TGuardConsoleVariable()
	{
		if (ConsoleVariable)
		{
			ConsoleVariable->Set(PreviousValue);
		}
	}
};
...
public:
	 ...
	 * @param Flags bitmask combined from EConsoleVariableFlags
	 */
	FAutoConsoleVariableRef(const TCHAR* Name, FName& RefValue, const TCHAR* Help, uint32 Flags = ECVF_Default)
		: FAutoConsoleObject(IConsoleManager::Get().RegisterConsoleVariableRef(Name, RefValue, Help, Flags))
	{
	}
...
public:
	...
	TAutoConsoleVariable(IConsoleVariable* ExistingObject)
		: FAutoConsoleObject(ExistingObject)
	{
...
/**
 * A class that can be used to shadow (alias) a CVar, where both names are valid to use.
 * To deprecate a CVar, use FAutoConsoleVariableDeprecated.
 * To deprecate a CCmd, use FAutoConsoleCommandDeprecated.
 * 
 ...
 *
 * FAutoConsoleVariableDeprecated ShadowCVar(TEXT("r.oldname"), TEXT("r.newname"), TEXT("5.5"));
 * FAutoConsoleVariableDeprecated ShadowCVar(TEXT("r.oldname"), TEXT("r.newname"), TEXT("5.5"), EShadowCVarBehavior::Assert);
 *
 */
...
public:
	 ...
	 * @param LookupFailureBehavior How to handle failure of looking up the CVarToShadow name - defaults to ensure
	 * @param DeprecationMessage Optional deprecation message - defaults to empty, auto-generated.
	 */
 ...
 */
template<typename T>
class TAutoRangedConsoleVariable : public TAutoConsoleVariable<T>
{
public:
