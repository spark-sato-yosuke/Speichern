...
private:
	...
	inline bool ShouldSkipCompactionUnderAutoRTFM() const
	{
		// As a performance optimization for the AutoRTFM runtime, we *always*
		// skip compacting a multicast delegate when called via the AutoRTFM
		// runtime. We skip calls both in the bodies of transactions themselves,
		// and also those that are called as part of on-commit or on-abort calls.
		return AutoRTFM::IsTransactional() || AutoRTFM::IsCommittingOrAborting();
	}
