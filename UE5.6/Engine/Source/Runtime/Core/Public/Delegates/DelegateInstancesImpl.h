...
public:
	...
	bool HasSameObject(FDelegateUserObjectConst InUserObject) const final
	{
		...
		if (InUserObject.IsUObject())
		{
			return UserObjectPtr.HasSameObject(InUserObject.GetUObject());
		}
#endif
	...
	{
#if UE_WITH_REMOTE_OBJECT_HANDLE
		return !UserObjectPtr.IsValid(true);
#else
...
template <ESPMode SPMode, typename RetValType, typename... ParamTypes, typename UserPolicy, typename FunctorType, typename... VarTypes>
class TBaseSPLambdaDelegateInstance<SPMode, RetValType(ParamTypes...), UserPolicy, FunctorType, VarTypes...> : public TCommonDelegateInstanceState<RetValType(ParamTypes...), UserPolicy, VarTypes...>
{
private:
	...
	using UserClass = const void;

public:
	template <typename InFunctorType, typename... InVarTypes>
	explicit TBaseSPLambdaDelegateInstance(const TWeakPtr<UserClass, SPMode>& InContextObject, InFunctorType&& InFunctor, InVarTypes&&... Vars)
		: Super(Forward<InVarTypes>(Vars)...)
...
public:
	...
	bool HasSameObject(FDelegateUserObjectConst InUserObject) const final
	{
		...
		if (InUserObject.IsUObject())
		{
			return UserObject.HasSameObject(InUserObject.GetUObject());
		}
#endif
...
template <typename RetValType, typename... ParamTypes, typename UserPolicy, typename FunctorType, typename... VarTypes>
class TWeakBaseFunctorDelegateInstance<RetValType(ParamTypes...), UserPolicy, FunctorType, VarTypes...> : public TCommonDelegateInstanceState<RetValType(ParamTypes...), UserPolicy, VarTypes...>
{
private:
	static_assert(std::is_same_v<FunctorType, typename TRemoveReference<FunctorType>::Type>, "FunctorType cannot be a reference");
	...
	// A dependency on the template parameter is required here to stop early instantiation of types which are
	// defined in CoreUObject.
...
public:
	...
	{
#if UE_WITH_REMOTE_OBJECT_HANDLE
		if (InContextObject.IsUObject())
		{
			return ContextObject.HasSameObject(InContextObject.GetUObject());
		}
#endif
	...
	{
#if UE_WITH_REMOTE_OBJECT_HANDLE
		return !ContextObject.IsValid(true);
#else
