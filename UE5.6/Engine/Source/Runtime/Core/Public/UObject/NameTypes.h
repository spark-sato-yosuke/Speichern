...
public:
	...
	[[nodiscard]] inline bool operator==(FIntrusiveUnsetOptionalState I) const
	{
	...
	* @return the new FName based upon the calling FName, but with any Whitespace characters potentially problematic for
	 ...
	 */
	[[nodiscard]] CORE_API static int32 GetNameEntryMemorySize();

	/**
	 * @return Estimated remaining size the name entry table is willing to allocate.
	 */
	[[nodiscard]] CORE_API static int32 GetNameEntryMemoryEstimatedAvailable();

	/**
	* @return Size of Name Table object as a whole
	*/
	[[nodiscard]] CORE_API static int32 GetNameTableMemorySize();
...
private:
    ...
    template <typename T>
    [[nodiscard]] friend FORCEINLINE auto operator==(FName N, T O) -> decltype(FName::Equals(N, O))
    {
    	return FName::Equals(N, O);
    }

    template <typename T>
    [[nodiscard]] friend FORCEINLINE auto operator==(T O, FName N) -> decltype(FName::Equals(N, O))
    {
    	return FName::Equals(N, O);
    }
