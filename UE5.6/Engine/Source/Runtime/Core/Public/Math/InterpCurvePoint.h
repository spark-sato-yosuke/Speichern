...
public:
	 ...
	 */
	[[nodiscard]] FInterpCurvePoint() = default;
	 ...
	 */
	[[nodiscard]] FInterpCurvePoint( const float In, const T &Out );
	 ...
	 */
	[[nodiscard]] FInterpCurvePoint( const float In, const T &Out, const T &InArriveTangent, const T &InLeaveTangent, const EInterpCurveMode InInterpMode );
	 ...
	 */
	[[nodiscard]] explicit FORCEINLINE FInterpCurvePoint(EForceInit);

public:

	/** @return true if the key value is using a curve interp mode, otherwise false */
	[[nodiscard]] FORCEINLINE bool IsCurveKey() const;
...
public:
	 ...
	 */
	[[nodiscard]] friend bool operator==( const FInterpCurvePoint& Point1, const FInterpCurvePoint& Point2 )
	{
