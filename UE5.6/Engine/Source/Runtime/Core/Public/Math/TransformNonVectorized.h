...
public:
	...
	*/
	...
	/** Default constructor. */
	[[nodiscard]] FORCEINLINE TTransform()
	...
	*/
	[[nodiscard]] FORCEINLINE explicit TTransform(const TVector<T>& InTranslation)
	...
	*/
	[[nodiscard]] FORCEINLINE explicit TTransform(ENoInit)
	{
	...
	*/
	[[nodiscard]] FORCEINLINE explicit TTransform(const TQuat<T>& InRotation)
		: Rotation(InRotation),
	...
	*/
	[[nodiscard]] FORCEINLINE explicit TTransform(const TRotator<T>& InRotation)
		: Rotation(InRotation),
		Translation(TVector<T>::ZeroVector),
		Scale3D(TVector<T>::OneVector)
	{
	...
	*/
	[[nodiscard]] FORCEINLINE TTransform(const TQuat<T>& InRotation, const TVector<T>& InTranslation, const TVector<T>& InScale3D = TVector<T>::OneVector)
		: Rotation(InRotation),
		Translation(InTranslation),
		Scale3D(InScale3D)
	{
	...
	* @param InScale3D The value to use for the scale component
	*/
	[[nodiscard]] FORCEINLINE TTransform(const TRotator<T>& InRotation, const TVector<T>& InTranslation, const TVector<T>& InScale3D = TVector<T>::OneVector)
		: Rotation(InRotation),
		Translation(InTranslation),
		Scale3D(InScale3D)
	{
	...
	*/
	[[nodiscard]] FORCEINLINE explicit TTransform(const TMatrix<T>& InMatrix)
	{
	...
	[[nodiscard]] FORCEINLINE TTransform(const TVector<T>& InX, const TVector<T>& InY, const TVector<T>& InZ, const TVector<T>& InTranslation)
	{
	...
	*/
	[[nodiscard]] FORCEINLINE TMatrix<T> ToMatrixWithScale() const
	{
		TMatrix<T> OutMatrix;
	...
	*/
	[[nodiscard]] FORCEINLINE TMatrix<T> ToInverseMatrixWithScale() const
	{
		// todo: optimize
	...
	*/
	[[nodiscard]] FORCEINLINE TTransform<T> Inverse() const
	{
	...
	*/
	[[nodiscard]] FORCEINLINE TMatrix<T> ToMatrixNoScale() const
	{
		TMatrix<T> OutMatrix;
	...
	*/
	[[nodiscard]] FORCEINLINE TTransform<T> operator+(const TTransform<T>& Atom) const
	{
	...
	[[nodiscard]] FORCEINLINE TTransform<T> operator*(T Mult) const
	{
	...
	*/
	[[nodiscard]] FORCEINLINE TTransform<T> operator*(const TTransform<T>& Other) const;
	...
	*/
	[[nodiscard]] FORCEINLINE TTransform<T> operator*(const TQuat<T>& Other) const;
	...
	*/
	...
	[[nodiscard]] FORCEINLINE static bool AnyHasNegativeScale(const TVector<T>& InScale3D, const TVector<T>& InOtherScale3D);
	...
	[[nodiscard]] FORCEINLINE T GetMaximumAxisScale() const;
	[[nodiscard]] FORCEINLINE T GetMinimumAxisScale() const;
	...
	*/
	...
	[[nodiscard]] FORCEINLINE TVector4<T> TransformFVector4NoScale(const TVector4<T>& V) const;
	[[nodiscard]] FORCEINLINE TVector<T> TransformPosition(const TVector<T>& V) const;
	[[nodiscard]] FORCEINLINE TVector<T> TransformPositionNoScale(const TVector<T>& V) const;
	...
	[[nodiscard]] FORCEINLINE TVector<T> InverseTransformPositionNoScale(const TVector<T> &V) const;
	[[nodiscard]] FORCEINLINE TVector<T> TransformVector(const TVector<T>& V) const;
	[[nodiscard]] FORCEINLINE TVector<T> TransformVectorNoScale(const TVector<T>& V) const;
	...
	*/
	...
	[[nodiscard]] FORCEINLINE TVector<T> InverseTransformVectorNoScale(const TVector<T> &V) const;
	 ...
	 */
	[[nodiscard]] FORCEINLINE TQuat<T> TransformRotation(const TQuat<T>& Q) const;
	...
	*/
	[[nodiscard]] FORCEINLINE TQuat<T> InverseTransformRotation(const TQuat<T>& Q) const;

	[[nodiscard]] FORCEINLINE TTransform<T> GetScaled(T Scale) const;
	[[nodiscard]] FORCEINLINE TTransform<T> GetScaled(TVector<T> Scale) const;
	[[nodiscard]] FORCEINLINE TVector<T> GetScaledAxis(EAxis::Type InAxis) const;
	[[nodiscard]] FORCEINLINE TVector<T> GetUnitAxis(EAxis::Type InAxis) const;
	[[nodiscard]] FORCEINLINE void Mirror(EAxis::Type MirrorAxis, EAxis::Type FlipAxis);
	[[nodiscard]] FORCEINLINE static TVector<T> GetSafeScaleReciprocal(const TVector<T>& InScale, FReal Tolerance = UE_SMALL_NUMBER);

	// temp function for easy conversion
	[[nodiscard]] FORCEINLINE TVector<T> GetLocation() const
	{
		return GetTranslation();
	}

	[[nodiscard]] FORCEINLINE TRotator<T> Rotator() const
	{
		return Rotation.Rotator();
	}

	/** Calculate the determinant of this transformation */
	[[nodiscard]] FORCEINLINE FReal GetDeterminant() const
	{
...
public:
	...
	[[nodiscard]] FORCEINLINE static bool AreRotationsEqual(const TTransform<T>& A, const TTransform<T>& B, FReal Tolerance = UE_KINDA_SMALL_NUMBER)
	{
	...
	[[nodiscard]] FORCEINLINE static bool AreTranslationsEqual(const TTransform<T>& A, const TTransform<T>& B, FReal Tolerance = UE_KINDA_SMALL_NUMBER)
	{
	...
	[[nodiscard]] FORCEINLINE static bool AreScale3DsEqual(const TTransform<T>& A, const TTransform<T>& B, FReal Tolerance = UE_KINDA_SMALL_NUMBER)
	{
	...
	[[nodiscard]] FORCEINLINE bool RotationEquals(const TTransform<T>& Other, FReal Tolerance = UE_KINDA_SMALL_NUMBER) const
	{
	...
	[[nodiscard]] FORCEINLINE bool TranslationEquals(const TTransform<T>& Other, FReal Tolerance = UE_KINDA_SMALL_NUMBER) const
	{
	...
	[[nodiscard]] FORCEINLINE bool Scale3DEquals(const TTransform<T>& Other, FReal Tolerance = UE_KINDA_SMALL_NUMBER) const
	{
		return AreScale3DsEqual(*this, Other, Tolerance);
	}

	// Test if all components of the transforms are equal, within a tolerance.
	[[nodiscard]] FORCEINLINE bool Equals(const TTransform<T>& Other, FReal Tolerance = UE_KINDA_SMALL_NUMBER) const
	{
	...
	[[nodiscard]] FORCEINLINE bool Identical(const TTransform<T>* Other, uint32 PortFlags) const
	{
		return Equals(*Other, 0.f);
	}

	// Test if rotation and translation components of the transforms are equal, within a tolerance.
	[[nodiscard]] FORCEINLINE bool EqualsNoScale(const TTransform<T>& Other, FReal Tolerance = UE_KINDA_SMALL_NUMBER) const
	{
	...
	*/
	[[nodiscard]] FORCEINLINE static TVector<T> AddTranslations(const TTransform<T>& A, const TTransform<T>& B)
	{
		return A.Translation + B.Translation;
	}
	...
	* @return A.Translation - B.Translation.
	*/
	[[nodiscard]] FORCEINLINE static TVector<T> SubtractTranslations(const TTransform<T>& A, const TTransform<T>& B)
	{
		return A.Translation - B.Translation;
	}
	...
	*/
	[[nodiscard]] FORCEINLINE bool IsRotationNormalized() const
	{
	...
	*/
	[[nodiscard]] FORCEINLINE TQuat<T> GetRotation() const
	{
		DiagnosticCheckNaN_Rotate();
		return Rotation;
	}
	...
	*/
	[[nodiscard]] FORCEINLINE TVector<T> GetTranslation() const
	{
		DiagnosticCheckNaN_Translate();
		return Translation;
	}
	...
	*/
	[[nodiscard]] FORCEINLINE TVector<T> GetScale3D() const
	{
		DiagnosticCheckNaN_Scale3D();
		return Scale3D;
	}
