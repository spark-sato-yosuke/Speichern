...
public:
	...
	FORCEINLINE_DEBUGGABLE void DiagnosticCheckNaN_Scale3D() const
	{
	...
	FORCEINLINE_DEBUGGABLE void DiagnosticCheckNaN_Translate() const
	{
	...
	FORCEINLINE_DEBUGGABLE void DiagnosticCheckNaN_Rotate() const
	{
	...
	FORCEINLINE_DEBUGGABLE void DiagnosticCheckNaN_All() const
	{
	...
	FORCEINLINE_DEBUGGABLE void DiagnosticCheck_IsValid() const
	{
	 ...
	 */
	[[nodiscard]] FORCEINLINE_DEBUGGABLE TTransform()
	{
		// Rotation = {0,0,0,1)
	 ...
	 */
	[[nodiscard]] FORCEINLINE_DEBUGGABLE explicit TTransform(const TVector<T>& InTranslation) 
	{
	 ...
	 */
	[[nodiscard]] FORCEINLINE_DEBUGGABLE explicit TTransform(const TQuat<T>& InRotation) 
	{
		// Rotation = InRotation
	 ...
	 */
	[[nodiscard]] FORCEINLINE_DEBUGGABLE explicit TTransform(const TRotator<T>& InRotation) 
	{
	 ...
	 */
	[[nodiscard]] FORCEINLINE_DEBUGGABLE TTransform(const TQuat<T>& InRotation, const TVector<T>& InTranslation, const TVector<T>& InScale3D = TVector<T>(1.f, 1.f, 1.f))
	{
		// Rotation = InRotation
		Rotation =  VectorLoadAligned( &InRotation );
		// Translation = InTranslation
	 ...
	 * @param InScale3D The value to use for the scale component
	 */
	[[nodiscard]] FORCEINLINE_DEBUGGABLE TTransform(TransformVectorRegister InRotation, TransformVectorRegister InTranslation, TransformVectorRegister InScale3D)
		: Rotation(InRotation),
		Translation(InTranslation),
		Scale3D(InScale3D)
	{
	 ...
	 * @param InScale3D The value to use for the scale component
	 */
	[[nodiscard]] FORCEINLINE_DEBUGGABLE TTransform(const TRotator<T>& InRotation, const TVector<T>& InTranslation, const TVector<T>& InScale3D = TVector<T>(1.f, 1.f, 1.f))
	{
		TQuat<T> InQuatRotation = TQuat<T>::MakeFromRotator(InRotation);
		// Rotation = InRotation
		Rotation =  VectorLoadAligned( &InQuatRotation );
	 ...
	 */
	[[nodiscard]] FORCEINLINE_DEBUGGABLE explicit TTransform(ENoInit) 
	{
	 ...
	 */
	[[nodiscard]] FORCEINLINE_DEBUGGABLE explicit TTransform(const TMatrix<T>& InMatrix)
	{
	...
	[[nodiscard]] FORCEINLINE_DEBUGGABLE TTransform(const TVector<T>& InX, const TVector<T>& InY, const TVector<T>& InZ, const TVector<T>& InTranslation)
	{
	...
	[[nodiscard]] FORCEINLINE_DEBUGGABLE TMatrix<T> ToMatrixWithScale() const
	{
	...
	*/
	[[nodiscard]] FORCEINLINE_DEBUGGABLE TMatrix<T> ToInverseMatrixWithScale() const
	{
		// todo: optimize
	...
	*/
	[[nodiscard]] FORCEINLINE_DEBUGGABLE TTransform Inverse() const
	{
	...
	*/
	[[nodiscard]] FORCEINLINE_DEBUGGABLE TMatrix<T> ToMatrixNoScale() const
	{
		TMatrix<T> OutMatrix;
		TransformVectorRegister DiagonalsXYZ;
		TransformVectorRegister Adds;
	...
	FORCEINLINE_DEBUGGABLE void Blend(const TTransform<T>& Atom1, const TTransform<T>& Atom2, float Alpha)
	{
	...
	FORCEINLINE_DEBUGGABLE void BlendWith(const TTransform<T>& OtherAtom, float Alpha)
	{
#if !(UE_BUILD_SHIPPING || UE_BUILD_TEST) && WITH_EDITORONLY_DATA
		// Check that all bone atoms coming from animation are normalized
	 ...
	 */
	[[nodiscard]] FORCEINLINE_DEBUGGABLE TTransform<T> operator+(const TTransform<T>& Atom) const
	{
	...
	FORCEINLINE_DEBUGGABLE TTransform<T>& operator+=(const TTransform<T>& Atom)
	{ 
	...
	[[nodiscard]] FORCEINLINE_DEBUGGABLE TTransform<T> operator*(const ScalarRegister& MultScalar) const
	{		
	...
	FORCEINLINE_DEBUGGABLE TTransform<T>& operator*=(const ScalarRegister& MultScalar)
	{			
		const TransformVectorRegister Mult = MultScalar.Value;
	...
	[[nodiscard]] FORCEINLINE_DEBUGGABLE TTransform<T>	operator*(const TTransform<T>& Other) const;
	FORCEINLINE_DEBUGGABLE void			operator*=(const TTransform<T>& Other);
	[[nodiscard]] FORCEINLINE_DEBUGGABLE TTransform<T>	operator*(const TQuat<T>& Other) const;
	FORCEINLINE_DEBUGGABLE void			operator*=(const TQuat<T>& Other);

	[[nodiscard]] FORCEINLINE_DEBUGGABLE static bool AnyHasNegativeScale(const TVector<T>& InScale3D, const TVector<T>& InOtherScale3D);
	FORCEINLINE_DEBUGGABLE void ScaleTranslation(const TVector<T>& InScale3D);
	FORCEINLINE_DEBUGGABLE void ScaleTranslation(const T& Scale);
	FORCEINLINE_DEBUGGABLE void RemoveScaling(T Tolerance=UE_SMALL_NUMBER);
	[[nodiscard]] FORCEINLINE_DEBUGGABLE float GetMaximumAxisScale() const;
	[[nodiscard]] FORCEINLINE_DEBUGGABLE float GetMinimumAxisScale() const;
	...
	[[nodiscard]] FORCEINLINE_DEBUGGABLE TVector4<T>	TransformFVector4(const TVector4<T>& V) const;
	[[nodiscard]] FORCEINLINE_DEBUGGABLE TVector4<T>	TransformFVector4NoScale(const TVector4<T>& V) const;
	[[nodiscard]] FORCEINLINE_DEBUGGABLE TVector<T>		TransformPosition(const TVector<T>& V) const;
	[[nodiscard]] FORCEINLINE_DEBUGGABLE TVector<T>		TransformPositionNoScale(const TVector<T>& V) const;


	/** Inverts the transform and then transforms V - correctly handles scaling in this transform. */
	[[nodiscard]] FORCEINLINE_DEBUGGABLE TVector<T>		InverseTransformPosition(const TVector<T> &V) const;
	[[nodiscard]] FORCEINLINE_DEBUGGABLE TVector<T>		InverseTransformPositionNoScale(const TVector<T> &V) const;
	[[nodiscard]] FORCEINLINE_DEBUGGABLE TVector<T>		TransformVector(const TVector<T>& V) const;
	[[nodiscard]] FORCEINLINE_DEBUGGABLE TVector<T>		TransformVectorNoScale(const TVector<T>& V) const;

	/** 
	 *	Transform a direction vector by the inverse of this matrix - will not take into account translation part.
	 *	If you want to transform a surface normal (or plane) and correctly account for non-uniform scaling you should use TransformByUsingAdjointT with adjoint of matrix inverse.
	 */
	[[nodiscard]] FORCEINLINE_DEBUGGABLE TVector<T> InverseTransformVector(const TVector<T> &V) const;
	[[nodiscard]] FORCEINLINE_DEBUGGABLE TVector<T> InverseTransformVectorNoScale(const TVector<T> &V) const;
	...
	*/
	[[nodiscard]] FORCEINLINE_DEBUGGABLE TQuat<T> TransformRotation(const TQuat<T>& Q) const;
	...
	*/
	[[nodiscard]] FORCEINLINE_DEBUGGABLE TQuat<T> InverseTransformRotation(const TQuat<T>& Q) const;

	[[nodiscard]] FORCEINLINE_DEBUGGABLE TTransform<T>	GetScaled(T Scale) const;
	[[nodiscard]] FORCEINLINE_DEBUGGABLE TTransform<T>	GetScaled(TVector<T> Scale) const;
	[[nodiscard]] FORCEINLINE_DEBUGGABLE TVector<T>		GetScaledAxis(EAxis::Type InAxis) const;
	[[nodiscard]] FORCEINLINE_DEBUGGABLE TVector<T>		GetUnitAxis(EAxis::Type InAxis) const;
	FORCEINLINE_DEBUGGABLE void		Mirror(EAxis::Type MirrorAxis, EAxis::Type FlipAxis);
	[[nodiscard]] FORCEINLINE_DEBUGGABLE static TVector<T>	GetSafeScaleReciprocal(const TVector<T>& InScale, T Tolerance=UE_SMALL_NUMBER);


	[[nodiscard]] FORCEINLINE_DEBUGGABLE TVector<T> GetLocation() const
	{
		return GetTranslation();
	}

	[[nodiscard]] FORCEINLINE_DEBUGGABLE TRotator<T> Rotator() const
	{
		TQuat<T> OutRotation;
	...
	[[nodiscard]] FORCEINLINE_DEBUGGABLE T GetDeterminant() const
	{
	...
	FORCEINLINE_DEBUGGABLE void SetLocation(const TVector<T>& Origin)
	{		
...
private:
	...
	FORCEINLINE_DEBUGGABLE static bool Private_AnyHasNegativeScale(TransformVectorRegister InScale3D, TransformVectorRegister InOtherScale3D)
	{
		return !!VectorAnyLesserThan(VectorMin(InScale3D, InOtherScale3D), (TransformVectorRegister)GlobalVectorConstants::FloatZero);
	}

	FORCEINLINE_DEBUGGABLE bool Private_RotationEquals(TransformVectorRegister InRotation, const FReal ToleranceScalar = UE_KINDA_SMALL_NUMBER) const
	{			
	...
	FORCEINLINE_DEBUGGABLE bool Private_TranslationEquals(TransformVectorRegister InTranslation, const FReal ToleranceScalar = UE_KINDA_SMALL_NUMBER) const
	{			
		const TransformVectorRegister Tolerance = VectorLoadFloat1(&ToleranceScalar);
	...
	FORCEINLINE_DEBUGGABLE bool Private_Scale3DEquals(TransformVectorRegister InScale3D, const FReal ToleranceScalar = UE_KINDA_SMALL_NUMBER) const
	{
		const TransformVectorRegister Tolerance = VectorLoadFloat1(&ToleranceScalar);
		// !( (FMath::Abs(X-V.X) > Tolerance) || (FMath::Abs(Y-V.Y) > Tolerance) || (FMath::Abs(Z-V.Z) > Tolerance) )
...
public:
	...
	[[nodiscard]] FORCEINLINE_DEBUGGABLE static bool AreRotationsEqual(const TTransform<T>& A, const TTransform<T>& B, FReal Tolerance=UE_KINDA_SMALL_NUMBER)
	{
	...
	[[nodiscard]] FORCEINLINE_DEBUGGABLE static bool AreTranslationsEqual(const TTransform<T>& A, const TTransform<T>& B, FReal Tolerance=UE_KINDA_SMALL_NUMBER)
	{
	...
	[[nodiscard]] FORCEINLINE_DEBUGGABLE static bool AreScale3DsEqual(const TTransform<T>& A, const TTransform<T>& B, FReal Tolerance=UE_KINDA_SMALL_NUMBER)
	{
	...
	[[nodiscard]] FORCEINLINE_DEBUGGABLE bool RotationEquals(const TTransform<T>& Other, FReal Tolerance=UE_KINDA_SMALL_NUMBER) const
	{
	...
	[[nodiscard]] FORCEINLINE_DEBUGGABLE bool TranslationEquals(const TTransform<T>& Other, FReal Tolerance=UE_KINDA_SMALL_NUMBER) const
	{
	...
	[[nodiscard]] FORCEINLINE_DEBUGGABLE bool Scale3DEquals(const TTransform<T>& Other, FReal Tolerance=UE_KINDA_SMALL_NUMBER) const
	{
		return AreScale3DsEqual(*this, Other, Tolerance);
	}

	// Test if all components of the transforms are equal, within a tolerance.
	[[nodiscard]] FORCEINLINE_DEBUGGABLE bool Equals(const TTransform<T>& Other, FReal Tolerance=UE_KINDA_SMALL_NUMBER) const
	{
	...
	[[nodiscard]] FORCEINLINE_DEBUGGABLE bool Identical(const TTransform<T>* Other, uint32 PortFlags) const
	{
		return Equals(*Other, 0.f);
	}

	// Test if rotation and translation components of the transforms are equal, within a tolerance.
	[[nodiscard]] FORCEINLINE_DEBUGGABLE bool EqualsNoScale(const TTransform<T>& Other, FReal Tolerance=UE_KINDA_SMALL_NUMBER) const
	{
		return Private_TranslationEquals(Other.Translation, Tolerance) && Private_RotationEquals(Other.Rotation, Tolerance);
	}

	FORCEINLINE_DEBUGGABLE static void Multiply(TTransform<T>* OutTransform, const TTransform<T>* A, const TTransform<T>* B);
	/**
	 * Sets the components
	 ...
	 */
	FORCEINLINE_DEBUGGABLE void SetComponents(const TQuat<T>& InRotation, const TVector<T>& InTranslation, const TVector<T>& InScale3D) 
	{
		Rotation = VectorLoadAligned(&InRotation);
	 ...
	 *   Scale3D = (1,1,1)
	 */
	FORCEINLINE_DEBUGGABLE void SetIdentity()
	{
		// Rotation = {0,0,0,1)
	 ...
	 *   Scale3D = (0,0,0)
	 */
	FORCEINLINE_DEBUGGABLE void SetIdentityZeroScale()
	{
		// Rotation = {0,0,0,1)
		Rotation = GlobalVectorConstants::Float0001; //VectorSet_W1( VectorZero() );
		// Translation = {0,0,0,0)
	 ...
	 */
	FORCEINLINE_DEBUGGABLE void MultiplyScale3D(const TVector<T>& Scale3DMultiplier)
	{
	 ...
	 */
	FORCEINLINE_DEBUGGABLE void SetTranslation(const TVector<T>& NewTranslation)
	{
	...
	FORCEINLINE_DEBUGGABLE void CopyTranslation(const TTransform<T>& Other)
	{
	 ...
	 */
	FORCEINLINE_DEBUGGABLE void ConcatenateRotation(const TQuat<T>& DeltaRotation)
	{		
	 ...
	 */
	FORCEINLINE_DEBUGGABLE void AddToTranslation(const TVector<T>& DeltaTranslation)
	{		
	 ...
	 */
	[[nodiscard]] FORCEINLINE_DEBUGGABLE static TVector<T> AddTranslations(const TTransform<T>& A, const TTransform<T>& B)
	{
		TVector<T> Result;
	 ...
	 * @return A.Translation - B.Translation.
	 */
	[[nodiscard]] FORCEINLINE_DEBUGGABLE static TVector<T> SubtractTranslations(const TTransform<T>& A, const TTransform<T>& B)
	{
		TVector<T> Result;
	 ...
	 */
	FORCEINLINE_DEBUGGABLE void SetRotation(const TQuat<T>& NewRotation)
	{
	...
	FORCEINLINE_DEBUGGABLE void CopyRotation(const TTransform<T>& Other)
	{
	 ...
	 */
	FORCEINLINE_DEBUGGABLE void SetScale3D(const TVector<T>& NewScale3D)
	{
	...
	FORCEINLINE_DEBUGGABLE void CopyScale3D(const TTransform<T>& Other)
	{
	 ...
	 * @param NewScale3D The new value for the Scale3D component
	 */
	FORCEINLINE_DEBUGGABLE void SetTranslationAndScale3D(const TVector<T>& NewTranslation, const TVector<T>& NewScale3D)
	{
		Translation = VectorLoadFloat3_W0(&NewTranslation);
		Scale3D = VectorLoadFloat3_W0(&NewScale3D);
	...
	*/
	FORCEINLINE_DEBUGGABLE void Accumulate(const TTransform<T>& SourceAtom)
	{
	...
	*/
	FORCEINLINE_DEBUGGABLE void Accumulate(const TTransform<T>& Atom, const ScalarRegister& BlendWeightScalar)
	{
	 ...
	 */
	FORCEINLINE_DEBUGGABLE void AccumulateWithShortestRotation(const TTransform<T>& DeltaAtom, const ScalarRegister& BlendWeightScalar)
	{
		const TransformVectorRegister BlendWeight(BlendWeightScalar.Value);
	...
	* @param BlendWeight The weight to multiply Atom by before it is accumulated.
	*/
	FORCEINLINE_DEBUGGABLE void AccumulateWithAdditiveScale(const TTransform<T>& Atom, const ScalarRegister& BlendWeightScalar)
	{
		const TransformVectorRegister BlendWeight(BlendWeightScalar.Value);
	 ...
	 */
	FORCEINLINE_DEBUGGABLE void LerpTranslationScale3D(const TTransform<T>& SourceAtom1, const TTransform<T>& SourceAtom2, const ScalarRegister& AlphaScalar)
	{
	 ...
	 */
	FORCEINLINE_DEBUGGABLE void NormalizeRotation()
	{
	 ...
	 */
	[[nodiscard]] FORCEINLINE_DEBUGGABLE bool IsRotationNormalized() const
	{		
	 ...
	 */
	FORCEINLINE_DEBUGGABLE static void BlendFromIdentityAndAccumulate(TTransform<T>& FinalAtom, const TTransform<T>& SourceAtom, const ScalarRegister& BlendWeightScalar)
	{
		const TransformVectorRegister BlendWeight(BlendWeightScalar.Value);
	 ...
	 */
	[[nodiscard]] FORCEINLINE_DEBUGGABLE TQuat<T> GetRotation() const
	{
		DiagnosticCheckNaN_Rotate();
		TQuat<T> OutRotation;
		VectorStoreAligned(Rotation, &OutRotation);
	 ...
	 */
	[[nodiscard]] FORCEINLINE_DEBUGGABLE TVector<T> GetTranslation() const
	{
		DiagnosticCheckNaN_Translate();
	 ...
	 */
	[[nodiscard]] FORCEINLINE_DEBUGGABLE TVector<T> GetScale3D() const
	{
		DiagnosticCheckNaN_Scale3D();
	 ...
	 */
	FORCEINLINE_DEBUGGABLE void CopyRotationPart(const TTransform<T>& SrcBA)
	{
	 ...
	 */
	FORCEINLINE_DEBUGGABLE void CopyTranslationAndScale3D(const TTransform<T>& SrcBA)
	{
...
private:
	FORCEINLINE_DEBUGGABLE void ToMatrixInternal(TransformVectorRegister& OutDiagonals, TransformVectorRegister& OutAdds, TransformVectorRegister& OutSubtracts) const
	{
#if !(UE_BUILD_SHIPPING || UE_BUILD_TEST) && WITH_EDITORONLY_DATA
	...
	FORCEINLINE_DEBUGGABLE void ToMatrixInternalNoScale(TransformVectorRegister& OutDiagonals, TransformVectorRegister& OutAdds, TransformVectorRegister& OutSubtracts) const
	{
#if !(UE_BUILD_SHIPPING || UE_BUILD_TEST) && WITH_EDITORONLY_DATA
		// Make sure Rotation is normalized when we turn it into a matrix.
	 ...
	 * so we hardcode as 0
	 */
	[[nodiscard]] static FORCEINLINE_DEBUGGABLE TransformVectorRegister	GetSafeScaleReciprocal(TransformVectorRegister InScale, const ScalarRegister& Tolerance = ScalarRegister(GlobalVectorConstants::SmallNumber))
	{		
	...
	[[nodiscard]] FORCEINLINE_DEBUGGABLE TTransform<T> InverseFast() const
	{
		// Inverse QST (A) = QST (~A)
		// Since A*~A = Identity, 
	...
	* @param  B Transform B.
	*/
	FORCEINLINE_DEBUGGABLE static void MultiplyUsingMatrixWithScale(TTransform<T>* OutTransform, const TTransform<T>* A, const TTransform<T>* B);
	/**
	...
	*/
	FORCEINLINE_DEBUGGABLE static void ConstructTransformFromMatrixWithDesiredScale(const TMatrix<T>& AMatrix, const TMatrix<T>& BMatrix, TransformVectorRegister DesiredScale, TTransform<T>& OutTransform);
	/**
...
template<typename T>
FORCEINLINE_DEBUGGABLE bool TTransform<T>::AnyHasNegativeScale(const TVector<T>& InScale3D, const TVector<T>& InOtherScale3D)
{
...
template<typename T>
FORCEINLINE_DEBUGGABLE void TTransform<T>::ScaleTranslation(const TVector<T>& InScale3D)
{
	TransformVectorRegister VectorInScale3D = VectorLoadFloat3_W0(&InScale3D);
...
template<typename T>
FORCEINLINE_DEBUGGABLE void TTransform<T>::ScaleTranslation(const T& InScale)
{
...
template<typename T>
FORCEINLINE_DEBUGGABLE void TTransform<T>::RemoveScaling(T Tolerance/*=UE_SMALL_NUMBER*/)
{
	Scale3D = GlobalVectorConstants::Float1110;
	NormalizeRotation();	
...
template<typename T>
FORCEINLINE_DEBUGGABLE void TTransform<T>::MultiplyUsingMatrixWithScale(TTransform<T>* OutTransform, const TTransform<T>* A, const TTransform<T>* B)
{
...
template<typename T>
FORCEINLINE_DEBUGGABLE void TTransform<T>::ConstructTransformFromMatrixWithDesiredScale(const UE::Math::TMatrix<T>& AMatrix, const UE::Math::TMatrix<T>& BMatrix, TransformVectorRegister DesiredScale, TTransform<T>& OutTransform)
{
...
template<typename T>
FORCEINLINE_DEBUGGABLE void TTransform<T>::Multiply(TTransform<T>* OutTransform, const TTransform<T>* A, const TTransform<T>* B)
{
 ...
 */
template<typename T>
FORCEINLINE_DEBUGGABLE TTransform<T> TTransform<T>::GetScaled(T InScale) const
{
	TTransform<T> A(*this);
 ...
 */
template<typename T>
FORCEINLINE_DEBUGGABLE TTransform<T> TTransform<T>::GetScaled(TVector<T> InScale) const
{
	TTransform<T> A(*this);
...
template<typename T>
FORCEINLINE_DEBUGGABLE TVector4<T> TTransform<T>::TransformFVector4NoScale(const TVector4<T>& V) const
{
	DiagnosticCheckNaN_All();
...
template<typename T>
FORCEINLINE_DEBUGGABLE TVector4<T> TTransform<T>::TransformFVector4(const TVector4<T>& V) const
{
	DiagnosticCheckNaN_All();
...
template<typename T>
FORCEINLINE_DEBUGGABLE TVector<T> TTransform<T>::TransformPosition(const TVector<T>& V) const
{
	DiagnosticCheckNaN_All();
...
template<typename T>
FORCEINLINE_DEBUGGABLE TVector<T> TTransform<T>::TransformPositionNoScale(const TVector<T>& V) const
{
	DiagnosticCheckNaN_All();
...
template<typename T>
FORCEINLINE_DEBUGGABLE TVector<T> TTransform<T>::TransformVector(const TVector<T>& V) const
{
	DiagnosticCheckNaN_All();
...
template<typename T>
FORCEINLINE_DEBUGGABLE TVector<T> TTransform<T>::TransformVectorNoScale(const TVector<T>& V) const
{
	DiagnosticCheckNaN_All();
...
template<typename T>
FORCEINLINE_DEBUGGABLE TVector<T> TTransform<T>::InverseTransformPosition(const TVector<T> &V) const
{
	DiagnosticCheckNaN_All();
...
template<typename T>
FORCEINLINE_DEBUGGABLE TVector<T> TTransform<T>::InverseTransformPositionNoScale(const TVector<T> &V) const
{
	DiagnosticCheckNaN_All();
...
// do backward operation when inverse, translation -> rotation -> scale
template<typename T>
FORCEINLINE_DEBUGGABLE TVector<T> TTransform<T>::InverseTransformVector(const TVector<T> &V) const
{
	DiagnosticCheckNaN_All();
...
// do backward operation when inverse, translation -> rotation
template<typename T>
FORCEINLINE_DEBUGGABLE TVector<T> TTransform<T>::InverseTransformVectorNoScale(const TVector<T> &V) const
{
	DiagnosticCheckNaN_All();
...
template<typename T>
FORCEINLINE_DEBUGGABLE TQuat<T> TTransform<T>::TransformRotation(const TQuat<T>& Q) const
{
	return GetRotation() * Q;
}

template<typename T>
FORCEINLINE_DEBUGGABLE TQuat<T> TTransform<T>::InverseTransformRotation(const TQuat<T>& Q) const
{
	return GetRotation().Inverse() * Q;
}

template<typename T>
FORCEINLINE_DEBUGGABLE TTransform<T> TTransform<T>::operator*(const TTransform<T>& Other) const
{
	TTransform<T> Output;
...
template<typename T>
FORCEINLINE_DEBUGGABLE void TTransform<T>::operator*=(const TTransform<T>& Other)
{
	Multiply(this, this, &Other);
}

template<typename T>
FORCEINLINE_DEBUGGABLE TTransform<T> TTransform<T>::operator*(const TQuat<T>& Other) const
{
...
template<typename T>
FORCEINLINE_DEBUGGABLE void TTransform<T>::operator*=(const TQuat<T>& Other)
{
...
// x = 0, y = 1, z = 2
template<typename T>
FORCEINLINE_DEBUGGABLE TVector<T> TTransform<T>::GetScaledAxis(EAxis::Type InAxis) const
{
	if ( InAxis == EAxis::X )
	{
...
// x = 0, y = 1, z = 2
template<typename T>
FORCEINLINE_DEBUGGABLE TVector<T> TTransform<T>::GetUnitAxis(EAxis::Type InAxis) const
{
	if ( InAxis == EAxis::X )
	{
...
template<typename T>
FORCEINLINE_DEBUGGABLE void TTransform<T>::Mirror(EAxis::Type MirrorAxis, EAxis::Type FlipAxis)
{
 ...
 */
template<typename T>
FORCEINLINE_DEBUGGABLE TVector<T> TTransform<T>::GetSafeScaleReciprocal(const TVector<T>& InScale, T Tolerance)
{
