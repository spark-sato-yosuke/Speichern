...
public:
	 ...
	 */
	[[nodiscard]] TSphere(TVector<T> InV, T InW)
		: Center(InV)
		, W(InW)
	{ }
	 ...
	 */
	[[nodiscard]] explicit FORCEINLINE TSphere(EForceInit)
		: Center(ForceInit)
		, W(0.0f)
	{ }
	 ...
	 */
	[[nodiscard]] CORE_API TSphere(const TVector<T>* Points, int32 Count);
	 ...
	 * @param Count How many points are in the list.
	 */
	[[nodiscard]] CORE_API TSphere(const TSphere<T>* Spheres, int32 Count);

	// Conversion from other variant type.
	template<typename FArg UE_REQUIRES(!std::is_same_v<T, FArg>)>
	[[nodiscard]] explicit TSphere(const TSphere<FArg>& From)
		: TSphere<T>(TVector<T>(From.Center), T(From.W))
	{
	}
	 ...
	 */
	[[nodiscard]] bool Equals(const TSphere<T>& Sphere, T Tolerance = UE_KINDA_SMALL_NUMBER) const
	{
	...
	*/
	[[nodiscard]] bool operator==(const TSphere<T>& Other) const
	{
	 ...
	 */
	[[nodiscard]] TSphere<T> operator+(const TSphere<T>& Other) const
	{
	 ...
	 */
	[[nodiscard]] bool IsInside(const TSphere<T>& Other, T Tolerance = UE_KINDA_SMALL_NUMBER) const
	{
	...
	*/
	[[nodiscard]] bool IsInside(const FVector& In, T Tolerance = UE_KINDA_SMALL_NUMBER) const
	{
	 ...
	 */
	[[nodiscard]] FORCEINLINE bool Intersects(const TSphere<T>& Other, T Tolerance = UE_KINDA_SMALL_NUMBER) const
	{
	 ...
	 */
	[[nodiscard]] TSphere<T> TransformBy(const TMatrix<T>& M) const;
	 ...
	 * @return Result of transformation.
	 */
	[[nodiscard]] TSphere<T> TransformBy(const FTransform& M) const;
	...
	*/
	...
	{
		return FString::Printf(TEXT("Center=(%s), Radius=(%s)"), *Center.ToString(), *LexToSanitizedString(W));
	}
