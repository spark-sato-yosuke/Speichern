...
public:
	 ...
	 *
	 * @param Sphere The sphere being enclosed.
	 */
...
public:
	 ...
	 */
	[[nodiscard]] FORCEINLINE bool operator==( const TBox<T>& Other ) const
	{
	 ...
	 */
	[[nodiscard]] FORCEINLINE TBox<T> operator+( const TVector<T>& Other ) const
	{
	 ...
	 * @return A new bounding volume.
	 */
	[[nodiscard]] FORCEINLINE TBox<T> operator+( const TBox<T>& Other ) const
	{
		return TBox<T>(*this) += Other;
	}
	 ...
	 */
    [[nodiscard]] FORCEINLINE TVector<T>& operator[]( int32 Index )
	{
	 ...
	 */
    [[nodiscard]] FORCEINLINE const TVector<T>& operator[]( int32 Index ) const
	{
		check((Index >= 0) && (Index < 2));
...
public:
	 ...
	 * @return The distance.
	 */
	[[nodiscard]] FORCEINLINE T ComputeSquaredDistanceToPoint( const TVector<T>& Point ) const
	{
	 ...
	 */
	[[nodiscard]] FORCEINLINE T ComputeSquaredDistanceToBox(const TBox<T>& Box) const
	{
...
template<typename T>
FORCEINLINE FString TBox<T>::ToCompactString() const
{
