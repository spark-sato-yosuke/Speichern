...
template<typename T>
[[nodiscard]] inline UE::Math::TVector2<T> Concatenate(const UE::Math::TVector2<T>& LHS, const UE::Math::TVector2<T>& RHS)
{
	return LHS + RHS;
}
...
template<typename T>
[[nodiscard]] inline UE::Math::TVector2<T> Inverse(const UE::Math::TVector2<T>& Transform)
{
	return -Transform;
}
...
template<typename T>
[[nodiscard]] inline UE::Math::TVector2<T> TransformPoint(const UE::Math::TVector2<T>& Transform, const UE::Math::TVector2<T>& Point)
{
...
template<typename T>
[[nodiscard]] inline const UE::Math::TVector2<T>& TransformVector(const UE::Math::TVector2<T>& Transform, const UE::Math::TVector2<T>& Vector)
{
	return Vector;
}
...
[[nodiscard]] inline UE::Math::TVector2<PositionType> TransformPoint(float Transform, const UE::Math::TVector2<PositionType>& Point)
{
	return Transform * Point;
}

template<typename PositionType>
[[nodiscard]] inline UE::Math::TVector2<PositionType> TransformPoint(double Transform, const UE::Math::TVector2<PositionType>& Point)
{
	return Transform * Point;
}
...
[[nodiscard]] inline UE::Math::TVector2<VectorType> TransformVector(float Transform, const UE::Math::TVector2<VectorType>& Vector)
{
	return Transform * Vector;
}

template<typename VectorType>
[[nodiscard]] inline UE::Math::TVector2<VectorType> TransformVector(double Transform, const UE::Math::TVector2<VectorType>& Vector)
{
	return Transform * Vector;
}
...
public:
	...
	[[nodiscard]] TScale2() : Scale(1.0f, 1.0f) {}
	/** Ctor. initialize from a uniform scale. */
	[[nodiscard]] explicit TScale2(T InScale) :Scale(InScale, InScale) {}
	/** Ctor. initialize from a non-uniform scale. */
	[[nodiscard]] explicit TScale2(T InScaleX, T InScaleY) :Scale(InScaleX, InScaleY) {}
	/** Ctor. initialize from an FVector defining the 3D scale. */
	template<typename ArgType>
	[[nodiscard]] explicit TScale2(const UE::Math::TVector2<ArgType>& InScale) :Scale(InScale) {}
	
	/** Transform 2D Point */
	template<typename ArgType>
	[[nodiscard]] UE::Math::TVector2<ArgType> TransformPoint(const UE::Math::TVector2<ArgType>& Point) const
	{
	...
	/** Transform 2D Vector*/
	template<typename ArgType>
	[[nodiscard]] UE::Math::TVector2<ArgType> TransformVector(const UE::Math::TVector2<ArgType>& Vector) const
	{
		return TransformPoint(Vector);
	}

	/** Concatenate two scales. */
	[[nodiscard]] TScale2 Concatenate(const TScale2& RHS) const
	{
		return TScale2(Scale * RHS.Scale);
	}
	/** Invert the scale. */
	[[nodiscard]] TScale2 Inverse() const
	{
		return TScale2(1.0f / Scale.X, 1.0f / Scale.Y);
	}

	/** Equality. */
	[[nodiscard]] bool operator==(const TScale2& Other) const
	{
	...
	[[nodiscard]] const Vector2Type& GetVector() const
	{
		return Scale;
	}
...
public:
	 ...
	 */
	template<typename VType>
	[[nodiscard]] static TShear2 FromShearAngles(const UE::Math::TVector2<VType>& InShearAngles)
	{
	 ...
	 */
	[[nodiscard]] inline TMatrix2x2<T> Concatenate(const TShear2& RHS) const;
	 ...
	 * [XX 1]                      [-XX 1]
	 */
	[[nodiscard]] TMatrix2x2<T> Inverse() const;


	/** Equality. */
	[[nodiscard]] bool operator==(const TShear2& Other) const
	{
