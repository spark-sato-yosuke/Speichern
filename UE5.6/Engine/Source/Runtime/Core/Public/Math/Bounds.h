...
struct TBounds
{
	...
	[[nodiscard]] FORCEINLINE TBounds<T> operator+( const TBounds<T>& Other ) const
	{
		return TBounds<T>(*this) += Other;
	}

	[[nodiscard]] FORCEINLINE bool Intersect( const TBounds<T>& Other ) const
	{
		int Separated;
	...
	[[nodiscard]] FORCEINLINE bool Contains( const TBounds<T>& Other ) const
	{
	...
	[[nodiscard]] FORCEINLINE T DistSqr( const TVector<T>& Point ) const
	{
	...
	[[nodiscard]] FORCEINLINE TVector<T> GetCenter() const
	{
		return (Max + Min) * 0.5f;
	}

	[[nodiscard]] FORCEINLINE TVector<T> GetExtent() const
	{
		return (Max - Min) * 0.5f;
	}

	[[nodiscard]] FORCEINLINE TVector<T> GetSize() const
	{
		return (Max - Min);
	}

	[[nodiscard]] FORCEINLINE T GetSurfaceArea() const
	{
	...
	template< typename U >
	[[nodiscard]] FORCEINLINE auto ToAbsolute( const TVector<U>& Offset ) const
	{
	...
	template< typename U >
	[[nodiscard]] FORCEINLINE TBounds< float > ToRelative( const TVector<U>& Offset ) const
	{
		using CommonType = typename std::common_type<T, U>::type;
