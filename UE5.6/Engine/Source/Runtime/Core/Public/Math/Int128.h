...
public:
	...
	[[nodiscard]] FORCEINLINE uint32 GetQuadPart(uint32 Part) const
	{
		switch (Part)
		{
		case 3: return Hi >> 32;
    ...
    [[nodiscard]] FORCEINLINE FUInt128(const FUInt128&) = default;
    [[nodiscard]] FORCEINLINE FUInt128(FUInt128&&) = default;
	...
	[[nodiscard]] FORCEINLINE FUInt128()
		: Hi(0)
		, Lo(0)
	{}

	/** Constructor. Initializes this uint128 with a uint64 value. */
	[[nodiscard]] FORCEINLINE FUInt128(uint64 A)
		: Hi(0)
		, Lo(A)
	{}

	/** Constructor. Initializes this uint128 with two uint64 values. */
	[[nodiscard]] FORCEINLINE FUInt128(uint64 A, uint64 B)
		: Hi(A)
		, Lo(B)
	{}

	/** Constructor. Initializes this uint128 with four uint32 values. */
	[[nodiscard]] FORCEINLINE FUInt128(uint32 A, uint32 B, uint32 C, uint32 D)
		: Hi(((uint64)A << 32) | B)
		, Lo(((uint64)C << 32) | D)
	{}

	/** this > Other */
	[[nodiscard]] FORCEINLINE bool IsGreater(const FUInt128& Other) const
	{
		if (Hi == Other.Hi)
		{
			return Lo > Other.Lo;
	...
	/** this >= Other */
	[[nodiscard]] FORCEINLINE bool IsGreaterOrEqual(const FUInt128& Other) const
	{
		if (Hi == Other.Hi)
		{
			return Lo >= Other.Lo;
	...
	/** this < Other */
	[[nodiscard]] FORCEINLINE bool IsLess(const FUInt128& Other) const
	{
		if (Hi == Other.Hi)
		{
			return Lo < Other.Lo;
	...
	/** this <= Other */
	[[nodiscard]] FORCEINLINE bool IsLessOrEqual(const FUInt128& Other) const
	{
		if (Hi == Other.Hi)
		{
			return Lo <= Other.Lo;
	...
	/** this == Other */
	[[nodiscard]] FORCEINLINE bool IsEqual(const FUInt128& Other) const
	{
	 ...
	 */
	[[nodiscard]] FORCEINLINE bool operator>(const FUInt128& Other) const
	{
	...
	[[nodiscard]] FORCEINLINE bool IsGreaterThanZero() const
	{
		return !IsZero();
	}
