...
public:
	 ...
	 */
	[[nodiscard]] TRotationTranslationMatrix(const TRotator<T>& Rot, const TVector<T>& Origin);

	// Conversion to other type.
	template<typename FArg UE_REQUIRES(!std::is_same_v<T, FArg>)>
	[[nodiscard]] explicit TRotationTranslationMatrix(const TRotationTranslationMatrix<FArg>& From)
		: TMatrix<T>(From)
	{
	}
...
{
#if PLATFORM_ENABLE_VECTORINTRINSICS && (!PLATFORM_CPU_ARM_FAMILY)
