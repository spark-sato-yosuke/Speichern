...
struct FLinearColor
{
	 ...
	 * @param Color The sRGB color that needs to be converted into linear space.
	 */
	[[nodiscard]] FORCEINLINE static FLinearColor FromSRGBColor(const FColor& Color)
	{
	...
	[[nodiscard]] FORCEINLINE float& Component(int32 Index)
	{
	...
	[[nodiscard]] FORCEINLINE const float& Component(int32 Index) const
	{
	PRAGMA_DISABLE_DEPRECATION_WARNINGS
		return RGBA[Index];
	PRAGMA_ENABLE_DEPRECATION_WARNINGS
	...
	[[nodiscard]] FORCEINLINE FLinearColor operator+(const FLinearColor& ColorB) const
	{
		return FLinearColor(
			this->R + ColorB.R,
			this->G + ColorB.G,
	...
	[[nodiscard]] FORCEINLINE FLinearColor operator*(float Scalar) const
	{
		return FLinearColor(
			this->R * Scalar,
			this->G * Scalar,
	...
	[[nodiscard]] FORCEINLINE FLinearColor GetClamped(float InMin = 0.0f, float InMax = 1.0f) const
	{
		FLinearColor Ret;
	...
	[[nodiscard]] FORCEINLINE bool operator==(const FLinearColor& ColorB) const
	{
		return this->R == ColorB.R && this->G == ColorB.G && this->B == ColorB.B && this->A == ColorB.A;
	}
	[[nodiscard]] FORCEINLINE bool operator!=(const FLinearColor& Other) const
	{
		return this->R != Other.R || this->G != Other.G || this->B != Other.B || this->A != Other.A;
	}

	// Error-tolerant comparison.
	[[nodiscard]] FORCEINLINE bool Equals(const FLinearColor& ColorB, float Tolerance=UE_KINDA_SMALL_NUMBER) const
	{
	...
	[[nodiscard]] static CORE_API FLinearColor FGetHSV(uint8 H,uint8 S,uint8 V);
	...
	*/
	[[nodiscard]] FORCEINLINE FColor QuantizeRound() const;
	...
	*/
	[[nodiscard]] FORCEINLINE FColor QuantizeFloor() const;
	...
	*/
	...
	[[nodiscard]] FORCEINLINE FColor Quantize() const;
	...
	*/
	...
	[[nodiscard]] FORCEINLINE FColor ToFColor(const bool bSRGB) const;
	 ...
	 */
	[[nodiscard]] FORCEINLINE float GetMax() const
	{
	 ...
	 */
	[[nodiscard]] FORCEINLINE float GetMin() const
	{
	 ...
	 */
	[[nodiscard]] static FORCEINLINE float Clamp01NansTo0(float InValue)
	{
	...
	[[nodiscard]] friend FORCEINLINE uint32 GetTypeHash( const FLinearColor& LinearColor )
	{
...
public:
	...
	// Operators.
	[[nodiscard]] FORCEINLINE bool operator==( const FColor &C ) const
	{
	...
	*/
	 ...
	 */
	[[nodiscard]] FORCEINLINE FLinearColor ReinterpretAsLinear() const
	{
	 ...
	 */
	[[nodiscard]] FORCEINLINE uint32 ToPackedARGB() const
	{
	 ...
	 */
	[[nodiscard]] FORCEINLINE uint32 ToPackedABGR() const
	{
	 ...
	 */
	[[nodiscard]] FORCEINLINE uint32 ToPackedBGRA() const
	{
	...
	[[nodiscard]] friend FORCEINLINE uint32 GetTypeHash( const FColor& Color )
	{
