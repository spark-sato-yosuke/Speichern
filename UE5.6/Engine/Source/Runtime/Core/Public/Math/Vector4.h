...
public:
	...
	[[nodiscard]] FORCEINLINE TVector4(const UE::Math::TVector<T>& InVector, FArg InW)
		: X(InVector.X)
		, Y(InVector.Y)
		, Z(InVector.Z)
		, W((T)InW)
	...
	template<typename FArg UE_REQUIRES(std::is_arithmetic_v<FArg>)>
	[[nodiscard]] FORCEINLINE TVector4(const UE::Math::TVector4<T>& InVector, FArg OverrideW)
		: X(InVector.X)
		, Y(InVector.Y)
		, Z(InVector.Z)
		, W((T)OverrideW)
	 ...
	 */
	[[nodiscard]] FORCEINLINE TVector4(const FLinearColor& InColor)
		: X(InColor.R)
		, Y(InColor.G)
		, Z(InColor.B)
		, W(InColor.A)
	 ...
	 * @param InOverrideW
	 */
	[[nodiscard]] FORCEINLINE TVector4(const FLinearColor& InColor, T InOverrideW)
		: X(InColor.R)
		, Y(InColor.G)
		, Z(InColor.B)
		, W(InOverrideW)
...
public:
	 ...
	 */
	[[nodiscard]] FORCEINLINE T& operator[](int32 ComponentIndex);
	 ...
	 */
	[[nodiscard]] FORCEINLINE T operator[](int32 ComponentIndex) const;
	 ...
	 */
	[[nodiscard]] FORCEINLINE TVector4<T> operator-() const;
	 ...
	 */
	[[nodiscard]] FORCEINLINE TVector4<T> operator+(const TVector4<T>& V) const;
     ...
     */
	template<typename FArg UE_REQUIRES(std::is_arithmetic_v<FArg>)>
	[[nodiscard]] FORCEINLINE TVector4<T> operator+(FArg Bias) const
	{
	 ...
	 */
	template<typename FArg UE_REQUIRES(std::is_arithmetic_v<FArg>)>
	[[nodiscard]] FORCEINLINE TVector4<T> operator*(FArg Scale) const
	{
...
public:
	...
	*/
	 ...
	 */
	[[nodiscard]] FORCEINLINE TVector4 GetSafeNormal(T Tolerance = UE_SMALL_NUMBER) const;
	 ...
	 */
	[[nodiscard]] FORCEINLINE TVector4 GetUnsafeNormal3() const;
	 ...
	 */
	[[nodiscard]] FORCEINLINE TRotator<T> Rotation() const
	{
...
template<typename T>
template<typename IntType>
FORCEINLINE TVector4<T>::TVector4(const TIntVector4<IntType>& InVector)
	: X((T)InVector.X)
	, Y((T)InVector.Y)
	, Z((T)InVector.Z)
	, W((T)InVector.W)
...
template<typename T>
FORCEINLINE TVector4<T> TVector4<T>::ComponentMin(const TVector4<T>& Other) const
{
    return TVector4<T>(FMath::Min(X, Other.X), FMath::Min(Y, Other.Y), FMath::Min(Z, Other.Z), FMath::Min(W, Other.W));
}

template<typename T>
FORCEINLINE TVector4<T> TVector4<T>::ComponentMax(const TVector4<T>& Other) const
{
...
template<typename T>
FORCEINLINE bool TVector4<T>::IsNearlyZero(T Tolerance) const
{
	return
			FMath::Abs(X)<=Tolerance
		&&	FMath::Abs(Y)<=Tolerance
