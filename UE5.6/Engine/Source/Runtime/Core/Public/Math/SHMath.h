...
public:
	...
	[[nodiscard]] TSHVector(float V0, float V1, float V2, float V3)
	{
		FMemory::Memzero(V,sizeof(V));
	...
	[[nodiscard]] explicit TSHVector(const FVector4f& Vector)
	{
		FMemory::Memzero(V,sizeof(V));
	...
	[[nodiscard]] explicit TSHVector(const TSHVector<OtherOrder>& Other)
	{
		if (Order <= OtherOrder)
		{
	...
	[[nodiscard]] friend FORCEINLINE TSHVector operator*(const TSHVector& A,const float& B)
	{
	...
	[[nodiscard]] friend FORCEINLINE TSHVector operator/(const TSHVector& A,const float& Scalar)
	{
	...
	/** Addition operator. */
	[[nodiscard]] friend FORCEINLINE TSHVector operator+(const TSHVector& A,const TSHVector& B)
	{
		TSHVector Result;
		for(int32 BasisIndex = 0;BasisIndex < NumSIMDVectors;BasisIndex++)
		{
	...
	[[nodiscard]] friend FORCEINLINE float Dot(const TSHVector& A,const TSHVector& B)
	{
	...
	[[nodiscard]] float CalcIntegral() const
	{
	...
	[[nodiscard]] bool AreFloatsValid() const
	{
		bool bValid = true;
	...
	[[nodiscard]] FVector GetMaximumDirection() const
	{
	...
	[[nodiscard]] static TSHVector CalcDiffuseTransfer(const FVector& Normal)
	{
	...
	[[nodiscard]] static TSHVector SHBasisFunction(const FVector& Vector)
	{
		TSHVector Result;
	...
	[[nodiscard]] static TSHVector AmbientFunction()
	{
	...
	[[nodiscard]] static float FindWindowingLambda(const TSHVector& Vector, float TargetLaplacian)
	{
		// "Stupid Spherical Harmonics (SH) Tricks"
...
template<> 
[[nodiscard]] inline TSHVector<2> TSHVector<2>::SHBasisFunction(const FVector& Vector) 
{
	TSHVector<2> Result;
...
template<> 
[[nodiscard]] inline TSHVector<3> TSHVector<3>::SHBasisFunction(const FVector& Vector) 
{
	TSHVector<3> Result;
	Result.V[0] = 0.282095f; 
	Result.V[1] = -0.488603f * (float)Vector.Y;	// LWC_TODO: Precision loss
...
public:
	...
	[[nodiscard]] TSHVector<MaxSHOrder> GetLuminance() const
	{
	...
	/** Calculates the integral of the function over the surface of the sphere. */
	[[nodiscard]] FLinearColor CalcIntegral() const
	{
		FLinearColor Result;
	...
	/** Scalar multiplication operator. */
	/** Changed to float& from float to avoid LHS **/
	[[nodiscard]] friend FORCEINLINE TSHVectorRGB operator*(const TSHVectorRGB& A, const float& Scalar)
	{
		TSHVectorRGB Result;
		Result.R = A.R * Scalar;
		Result.G = A.G * Scalar;
	...
	/** Scalar multiplication operator. */
	/** Changed to float& from float to avoid LHS **/
	[[nodiscard]] friend FORCEINLINE TSHVectorRGB operator*(const float& Scalar,const TSHVectorRGB& A)
	{
		TSHVectorRGB Result;
		Result.R = A.R * Scalar;
		Result.G = A.G * Scalar;
	...
	[[nodiscard]] friend FORCEINLINE TSHVectorRGB operator*(const TSHVectorRGB& A,const FLinearColor& Color)
	{
		TSHVectorRGB Result;
	...
	/** Color multiplication operator. */
	[[nodiscard]] friend FORCEINLINE TSHVectorRGB operator*(const FLinearColor& Color,const TSHVectorRGB& A)
	{
		TSHVectorRGB Result;
		Result.R = A.R * Color.R;
		Result.G = A.G * Color.G;
	...
	/** Division operator. */
	[[nodiscard]] friend FORCEINLINE TSHVectorRGB operator/(const TSHVectorRGB& A,const float& InB)
	{
		TSHVectorRGB Result;
		Result.R = A.R / InB;
		Result.G = A.G / InB;
	...
	/** Addition operator. */
	[[nodiscard]] friend FORCEINLINE TSHVectorRGB operator+(const TSHVectorRGB& A,const TSHVectorRGB& InB)
	{
		TSHVectorRGB Result;
		Result.R = A.R + InB.R;
		Result.G = A.G + InB.G;
	...
	/** Dot product operator. */
	[[nodiscard]] friend FORCEINLINE FLinearColor Dot(const TSHVectorRGB& A,const TSHVector<MaxSHOrder>& InB)
	{
		FLinearColor Result;
