...
public:
     ...
     */
    [[nodiscard]] explicit FORCEINLINE TVector(T InF);

	[[nodiscard]] FORCEINLINE constexpr TVector(T InF, TVectorConstInit);
     ...
     */
    [[nodiscard]] FORCEINLINE TVector(T InX, T InY, T InZ);
     ...
     * @param InZ Z Coordinate.
     */
    [[nodiscard]] explicit FORCEINLINE TVector(const TVector2<T> V, T InZ);
     ...
     */
    [[nodiscard]] FORCEINLINE TVector(const UE::Math::TVector4<T>& V);
     ...
     */
    [[nodiscard]] explicit FORCEINLINE TVector(EForceInit);
     ...
     */
    [[nodiscard]] FORCEINLINE TVector<T> operator^(const TVector<T>& V) const;
	 ...
	 * @return The cross product.
	 */
	[[nodiscard]] FORCEINLINE TVector<T> Cross(const TVector<T>& V2) const;
     ...
     * @return The cross product.
     */
    [[nodiscard]] FORCEINLINE static TVector<T> CrossProduct(const TVector<T>& A, const TVector<T>& B);
     ...
     */
    [[nodiscard]] FORCEINLINE T operator|(const TVector<T>& V) const;
	 ...
	 * @return The dot product.
	 */
	[[nodiscard]] FORCEINLINE T Dot(const TVector<T>& V) const;
     ...
     * @return The dot product.
     */
    [[nodiscard]] FORCEINLINE static T DotProduct(const TVector<T>& A, const TVector<T>& B);
	...
	[[nodiscard]] FORCEINLINE TVector<T> operator-(FArg Bias) const
	{
     ...
     */
	template<typename FArg UE_REQUIRES(std::is_arithmetic_v<FArg>)>
	[[nodiscard]] FORCEINLINE TVector<T> operator*(FArg Scale) const
	{
     ...
     */
   [[nodiscard]] bool operator==(const TVector<T>& V) const;
     ...
     */
    [[nodiscard]] bool Equals(const TVector<T>& V, T Tolerance=UE_KINDA_SMALL_NUMBER) const;
     ...
     * @return true if the vectors are equal within tolerance limits, false otherwise.
     */
    [[nodiscard]] bool AllComponentsEqual(T Tolerance=UE_KINDA_SMALL_NUMBER) const;
     ...
     */
    [[nodiscard]] FORCEINLINE TVector<T> operator-() const;
...
public:
     ...
     */
    [[nodiscard]] FORCEINLINE bool IsUnit(T LengthSquaredTolerance = UE_KINDA_SMALL_NUMBER) const;
     ...
     * @see GetSafeNormal()
     */
    [[nodiscard]] FORCEINLINE TVector<T> GetUnsafeNormal() const;

    /**
     * Gets a normalized copy of the vector, checking it is safe to do so based on the length.
     * Returns zero vector by default if vector length is too small to safely normalize.
     *
     * @param Tolerance Minimum squared vector length. Must be positive.
     ...
     */
    [[nodiscard]] FORCEINLINE TVector<T> GetSignVector() const;
    ...
    * @see GetSafeNormal2D()
    */
    [[nodiscard]] FORCEINLINE TVector<T> GetUnsafeNormal2D() const;
     ...
     * @return The cosine.
     */
    [[nodiscard]] FORCEINLINE T CosineAngle2D(TVector<T> B) const;
     ...
     */
    [[nodiscard]] FORCEINLINE TVector<T> ProjectOnTo(const TVector<T>& A) const ;
     ...
     * @return Projected vector.
     */
    [[nodiscard]] FORCEINLINE TVector<T> ProjectOnToNormal(const TVector<T>& Normal) const;
     ...
     */
	[[nodiscard]] FORCEINLINE UE::Math::TRotator<T> Rotation() const
	{
    ...
    */
     ...
     */
    [[nodiscard]] static FORCEINLINE T Dist(const TVector<T> &V1, const TVector<T> &V2);
    [[nodiscard]] static FORCEINLINE T Distance(const TVector<T> &V1, const TVector<T> &V2)
	{
		return Dist(V1, V2);
	}
    ...
    */
    [[nodiscard]] static FORCEINLINE T DistXY(const TVector<T> &V1, const TVector<T> &V2);
    [[nodiscard]] static FORCEINLINE T Dist2D(const TVector<T> &V1, const TVector<T> &V2)
	{
		return DistXY(V1, V2);
	}
     ...
     */
    [[nodiscard]] static FORCEINLINE T DistSquared(const TVector<T> &V1, const TVector<T> &V2);
     ...
     */
    [[nodiscard]] static FORCEINLINE T DistSquaredXY(const TVector<T> &V1, const TVector<T> &V2);
    [[nodiscard]] static FORCEINLINE T DistSquared2D(const TVector<T> &V1, const TVector<T> &V2)
	{
     ...
     */
    [[nodiscard]] static FORCEINLINE T BoxPushOut(const TVector<T>& Normal, const TVector<T>& Size);

    /**
     * Min, Max, Min3, Max3 like FMath
     */
    [[nodiscard]] static FORCEINLINE TVector<T> Min( const TVector<T>& A, const TVector<T>& B );
    [[nodiscard]] static FORCEINLINE TVector<T> Max( const TVector<T>& A, const TVector<T>& B );

    [[nodiscard]] static FORCEINLINE TVector<T> Min3( const TVector<T>& A, const TVector<T>& B, const TVector<T>& C );
    [[nodiscard]] static FORCEINLINE TVector<T> Max3( const TVector<T>& A, const TVector<T>& B, const TVector<T>& C );
