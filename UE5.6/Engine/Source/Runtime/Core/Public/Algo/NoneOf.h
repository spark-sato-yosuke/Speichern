...
namespace Algo
{
	...
	[[nodiscard]] FORCEINLINE bool NoneOf(const RangeType& Range)
	{
	...
	[[nodiscard]] FORCEINLINE bool NoneOf(const RangeType& Range, ProjectionType Projection)
	{
		for (const auto& Element : Range)
		{
	 ...
	 */
	template <typename RangeType, typename ProjectionType>
	[[nodiscard]] FORCEINLINE bool NoneOf(const RangeType& Range, ProjectionType Projection, ENoRef NoRef)
	{
		for (const auto Element : Range)
		{
			if (Invoke(Projection, Element))
