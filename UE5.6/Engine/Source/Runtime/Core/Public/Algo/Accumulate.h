...
struct TPlus
{
	[[nodiscard]] FORCEINLINE T operator()(const T& A, const T& B) { return A + B; }
};
template<>
struct TPlus<void>
{
	template<typename U, typename V>
	[[nodiscard]] FORCEINLINE auto operator()(U&& A, V&& B) -> decltype(A + B) { return A + B; }
};
...
namespace Algo
{
	...
	*/
	...
	[[nodiscard]] FORCEINLINE T Accumulate(const A& Input, T Init, OpT Op)
	{
	...
	[[nodiscard]] FORCEINLINE T Accumulate(const A& Input, T Init)
	{
	...
	*/
	...
	[[nodiscard]] FORCEINLINE T TransformAccumulate(const A& Input, MapT MapOp, T Init, OpT Op)
	{
		T Result = MoveTemp(Init);
		for (const auto& InputElem : Input)
		{
	...
	*/
	...
	[[nodiscard]] FORCEINLINE T TransformAccumulate(const A& Input, MapT MapOp, T Init)
	{
