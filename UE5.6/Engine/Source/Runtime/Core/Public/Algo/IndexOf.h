...
namespace Algo
{
	...
	[[nodiscard]] FORCEINLINE auto IndexOf(const RangeType& Range, const ValueType& Value)
	{
		static_assert(TIsContiguousContainer<RangeType>::Value, "Must only be used with contiguous containers");
		return AlgoImpl::IndexOfBy(Range, Value, FIdentityFunctor());
	}
	 ...
	 */
	template <typename RangeType, typename ValueType, typename ProjectionType>
	[[nodiscard]] FORCEINLINE auto IndexOfBy(const RangeType& Range, const ValueType& Value, ProjectionType Proj)
	{
		static_assert(TIsContiguousContainer<RangeType>::Value, "Must only be used with contiguous containers");
		return AlgoImpl::IndexOfBy(Range, Value, MoveTemp(Proj));
	}
	 ...
	 */
	template <typename RangeType, typename PredicateType>
	[[nodiscard]] FORCEINLINE auto IndexOfByPredicate(const RangeType& Range, PredicateType Pred)
	{
		static_assert(TIsContiguousContainer<RangeType>::Value, "Must only be used with contiguous containers");
		return AlgoImpl::IndexOfByPredicate(Range, MoveTemp(Pred));
	}
}
