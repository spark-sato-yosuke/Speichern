...
private:
	CORE_API static void VARARGS LogfImplV(const ANSICHAR* File, int32 Line, const FLogCategoryName& Category, ELogVerbosity::Type Verbosity, void* ReturnAddress, const TCHAR* Fmt, va_list Args);
...
CORE_API void BasicFatalLogWithProgramCounter(const FLogCategoryBase& Category, const FStaticBasicLogRecord* Log, void* ProgramCounter, ...);
