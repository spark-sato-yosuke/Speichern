...
public:
	...
	FORCEINLINE bool AcquireReadAccess() const
	{
		...
		checkf(!ErrorDetected || GIsAutomationTesting, TEXT("Acquiring a read access while there is already a write access"));
	 ...
	 * @return true if no errors were detected
	 */
	FORCEINLINE bool UpgradeReadAccessToWriteAccess() const
	{
		...
		{
			checkf(GIsAutomationTesting, TEXT("Upgrading a write access while there are ongoing read or write access"));
			return false;
		}
