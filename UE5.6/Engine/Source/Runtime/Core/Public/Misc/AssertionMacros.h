...
// Lite ensures use an alternate implementation that avoid lambdas which are expensive in the amount of
// debug information generated. However, this comes at the cost of ensures breaking inside an implementation
// function rather than on the line that the ensure is on. On Clang, this limitation can be mitigated by
// using UE_NODEBUG, so that the break happens where the code is inlined rather than where it is defined,
// but no such option exists for MSVC, so we disable lite ensures by default on that compiler.
#ifndef UE_USE_LITE_ENSURES
	#ifdef __clang__
		#define UE_USE_LITE_ENSURES 1
	#else
		#define UE_USE_LITE_ENSURES 0
	#endif
#endif
...
private:
	...
	static CORE_API bool CheckVerifyFailedImpl2V(const ANSICHAR* Expr, const ANSICHAR* File, int32 Line, const TCHAR* Format, va_list Args);
 ...
 */
	...
	CORE_API bool UE_COLD UE_DEBUG_SECTION EnsureFailedV(std::atomic<bool>& bExecuted, const FStaticEnsureRecord* Ensure, void* ProgramCounter, va_list Args);
	...
	template <uint64 Uid>
	std::atomic<bool> bGEnsureHasExecuted = false;

	UE_NODEBUG constexpr uint32 FileHashForEnsure(const char* Filename)
	{
		// Based on djb2
		uint32 Result = 5381;
		for (;;)
	...
	UE_NODEBUG FORCEINLINE bool BreakAndReturnFalse()
	{
		// This is tagged with UE_NODEBUG and FORCEINLINE to ensure that the break happens at the usage of ensure(), not inside this function.
		PLATFORM_BREAK();
		return false;
	}
	...
	#define UE_ENSURE_IMPL(Always, InExpression) \
		(LIKELY(!!(InExpression)) \
			|| (::UE::Assert::Private::ExecCheckImplInternal(::bGEnsureHasExecuted<((uint64)FileHashForEnsure(__FILE__) << 32) | (uint64)__LINE__>, Always, __FILE__, __LINE__, #InExpression) \
			&& BreakAndReturnFalse()))
#else
	#define UE_ENSURE_IMPL(Always, InExpression) \
		(LIKELY(!!(InExpression)) \
