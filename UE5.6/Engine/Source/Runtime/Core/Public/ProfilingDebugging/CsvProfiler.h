...
// Stat macros. These are disabled if CSV_PROFILER_MINIMAL is 1 
	...
	#define CSV_SCOPED_TIMING_STAT_CONDITIONAL(Category,StatName, Condition) \
		TRACE_CSV_PROFILER_INLINE_STAT(#StatName, CSV_CATEGORY_INDEX(Category)); \
		FScopedCsvStatConditional _ScopedCsvStat_ ## StatName (#StatName, CSV_CATEGORY_INDEX(Category), Condition, "CSV_"#StatName);
...
public:
	...
	CORE_API static void RecordEndOfPipeCustomStat(const FName& StatName, uint32 CategoryIndex, double Value, const ECsvCustomStatOp CustomStatOp);
	...
	CORE_API void GetFrameExecCommands(TArray<FString>& OutFrameCommands);
	...
	DECLARE_MULTICAST_DELEGATE_TwoParams(FOnCSVEvent, const FString& /* CategoryName */, const FString& /* EventText */);
	FOnCSVEvent& OnCSVProfileEvent() { return OnCSVProfileEventDelegate; }
...
private:
	...
	float ProcessStatData(uint32* OutNumStatEntriesProcessed = nullptr);
	...
	FOnCSVEvent OnCSVProfileEventDelegate;
...
class FScopedCsvStatConditional
{
public:
	FScopedCsvStatConditional(const char* InStatName, uint32 InCategoryIndex, bool bInCondition, const char* InNamedEventName = nullptr)
		: StatName(InStatName)
		, CategoryIndex(InCategoryIndex)
