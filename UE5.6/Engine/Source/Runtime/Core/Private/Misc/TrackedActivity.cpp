...
public:
	...
	void SendEvent(FTrackedActivity::EEvent Event, FActivity& Activity)
	{
		if (!EventFunc)
			return;
		TStringBuilder<256> Temp;
		FTrackedActivity::FInfo ActivityInfo{GetInfo(Activity, EventFuncMaxDepth, Temp)};
		
		// This callback mechanism is used to update the console output windows. These are wildly AutoRTFM-unsafe
		// (since they are manipulating HWNDs directly, etc.), so we always run the callback in the open.
		UE_AUTORTFM_OPEN
		{
			EventFunc(Event, ActivityInfo);
		};
	}
};
