...
namespace UE::SourceLocation
{
	...
	{
		Out.Reset();
		// Using ANSICHAR format string - FUtf8String::Appendf's use of TCheckedFormatString
		// fails if UE_UTF8TEXT_IS_CONSTEXPR == 0 and UTF8TEXT is used.
		Private::AppendFull<ANSICHAR>(Out, Location);
	}
	...
	void FFileAndLineAdapter::ToString(FUtf8String& Out) const
	{
		...
		// Using ANSICHAR format string - FUtf8String::Appendf's use of TCheckedFormatString
		// fails if UE_UTF8TEXT_IS_CONSTEXPR == 0 and UTF8TEXT is used.
		Private::AppendFileAndLine<ANSICHAR>(Out, Location);
	}
