...
{
	AppendString(ValueStr, EGuidFormats::Digits);
...
bool FGuid::ImportTextItem(const TCHAR*& Buffer, int32 PortFlags, class UObject* Parent, FOutputDevice* ErrorText)
{
	...
	if (FPlatformString::Strnlen(Buffer, GuidStrLen) < GuidStrLen)
	{
		return false;
	}

	if (!ParseExact(FStringView(Buffer, GuidStrLen), EGuidFormats::Digits, *this))
	{
		return false;
	}
...
bool FGuid::Parse(const TCHAR* GuidString, FGuid& OutGuid)
{
	return Parse(FStringView(GuidString), OutGuid);
}
...
bool FGuid::Parse(FStringView GuidString, FGuid& OutGuid)
{
...
bool FGuid::ParseExact(const TCHAR* GuidString, EGuidFormats Format, FGuid& OutGuid)
{
	return ParseExact(FStringView(GuidString), Format, OutGuid);
}
...
bool FGuid::ParseExact(FStringView GuidString, EGuidFormats Format, FGuid& OutGuid)
{
	...
	if (Format == EGuidFormats::Short)
	{
		...
		if (FBase64::GetDecodedDataSize(GuidString.GetData(), GuidString.Len()) != sizeof(Data))
		{
		...
		TStringBuilder<22> GuidCopy;
		GuidCopy += GuidString;
		Algo::Replace(MakeArrayView(GuidCopy), TEXT('-'), TEXT('+'));
		Algo::Replace(MakeArrayView(GuidCopy), TEXT('_'), TEXT('/'));

		// Decode the data
		if (!FBase64::Decode(GuidCopy.GetData(), GuidCopy.Len(), reinterpret_cast<uint8*>(&Data)))
		{
	...
	if (Format == EGuidFormats::Base36Encoded)
	{
		...
		FUInt128 Value;
		for (const TCHAR GuidChar : GuidString)
		{
			Value *= 36;
	...
	TStringBuilder<32> NormalizedGuidString;
	...
	FStringView NormalizedGuidStringView = NormalizedGuidString.ToView();
	for (const TCHAR GuidChar : NormalizedGuidStringView)
	{
		if (!FChar::IsHexDigit(GuidChar))
		{
			return false;
		}
	}
	...
	OutGuid = FGuid(
		FParse::HexNumber(NormalizedGuidStringView.Mid(0, 8)),
		FParse::HexNumber(NormalizedGuidStringView.Mid(8, 8)),
		FParse::HexNumber(NormalizedGuidStringView.Mid(16, 8)),
		FParse::HexNumber(NormalizedGuidStringView.Mid(24, 8))
	);
