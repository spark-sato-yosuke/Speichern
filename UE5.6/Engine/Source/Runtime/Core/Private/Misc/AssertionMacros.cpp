...
// Stack walking can race with ASAN's usage of DbgHelp (which is not thread safe), so we're
// not going to report callstacks for failed ensures and checks if ASAN is enabled.
#if PLATFORM_WINDOWS && PLATFORM_HAS_ASAN_INCLUDE
#	if defined(__SANITIZE_ADDRESS__)
#		define ASSERT_DISABLE_BACKTRACE_DUE_TO_ASAN 1
...
#ifndef ASSERT_DISABLE_BACKTRACE_DUE_TO_ASAN
...
public:
	...
	inline void TerminateWith(const TCHAR* Text)
	{
		const int32 TextLen = FCString::Strlen(Text);
		if (Index + TextLen >= Capacity)
		{
			Index = Capacity - TextLen - 1;
		}
		Append(Text);
	...
	{
		const int32 TextLen = FCString::Strlen(Text);
		const int32 CopyLen = FPlatformMath::Min(TextLen + 1, Capacity - Index);
		if (CopyLen > 1)
		{
			FCString::Strncpy(GErrorHist + Index, Text, CopyLen);
			Index += CopyLen - 1;
		}
	}
	...
	{
		if (const int32 TextLen = TCString<CharType>::Strlen(Text))
		{
	...
	va_list DescriptionArgs)
{
	...
	{
#if ASSERT_DISABLE_BACKTRACE_DUE_TO_ASAN
		FCStringAnsi::Snprintf(StackTrace, UE_ARRAY_COUNT(StackTrace), "(callstack disabled due to ASAN)");
#else
...
{
	va_list DescriptionArgs{};
	StaticFailDebugV(Error, /*Expression*/ "", File, Line, /*bIsEnsure*/ false, ProgramCounter, Message, /*DescriptionFormat*/ nullptr, DescriptionArgs);
}
...
{
	StaticFailDebugV(Error, /*Expression*/ "", File, Line, /*bIsEnsure*/ false, ProgramCounter, /*Message*/ nullptr, DescriptionFormat, DescriptionArgs);
}
...
void FDebug::LogAssertFailedMessageImplV(const ANSICHAR* Expr, const ANSICHAR* File, int32 Line, void* ProgramCounter, const TCHAR* Fmt, va_list Args)
{
	...
	if( !GIsCriticalError )
	{
		StaticFailDebugV(TEXT("Assertion failed: "), Expr, File, Line, /*bIsEnsure*/ false, ProgramCounter, /*Message*/ nullptr, Fmt, Args);
	}
}
...
FORCENOINLINE void FDebug::EnsureFailed(const ANSICHAR* Expr, const ANSICHAR* File, int32 Line, void* ProgramCounter, const TCHAR* Msg)
{
	...
	va_list DescriptionArgs{};
	StaticFailDebugV(TEXT("Ensure condition failed: "), Expr, File, Line, /*bIsEnsure*/ true, ProgramCounter, Msg, nullptr, DescriptionArgs);
	...
	else
	{
		...
		if (StackTrace != NULL)
		{
			...
			FCString::StrncatTruncateDest(ErrorMsg, UE_ARRAY_COUNT(ErrorMsg), ANSI_TO_TCHAR(StackTrace));
		...
		else
		{
			...
			bShouldSendNewReport = !!GEnsuresAreErrors;
...
UE_AUTORTFM_ALWAYS_OPEN
bool FORCENOINLINE FDebug::CheckVerifyFailedImpl2V(
	const ANSICHAR* Expr,
	const ANSICHAR* File,
	int32 Line,
	const TCHAR* Format,
	va_list Args)
{
	va_list Args2;
	va_copy(Args2, Args);
	FDebug::LogAssertFailedMessageImplV(Expr, File, Line, PLATFORM_RETURN_ADDRESS(), Format, Args2);
	va_end(Args2);
	...
	...)
{
	va_list Args;
	va_start(Args, Format);
	bool bResult = CheckVerifyFailedImpl2V(Expr, File, Line, Format, Args);
	va_end(Args);
...
void UE_COLD UE_DEBUG_SECTION VARARGS LowLevelFatalErrorHandler(const ANSICHAR* File, int32 Line, const TCHAR* Format, ...)
{
	va_list Args;
	va_start(Args, Format);
	StaticFailDebugV(TEXT("LowLevelFatalError"), /*Expression*/ "", File, Line, /*bIsEnsure*/ false, PLATFORM_RETURN_ADDRESS(), /*Message*/ nullptr, Format, Args);
	va_end(Args);
...
bool UE_COLD UE_DEBUG_SECTION UE::Assert::Private::ExecCheckImplInternal(std::atomic<bool>& bExecuted, bool bAlways, const ANSICHAR* File, int32 Line, const ANSICHAR* Expr)
{
	if (((bAlways && GEnsureAlwaysEnabled) || !bExecuted.load(std::memory_order_relaxed)) && FPlatformMisc::IsEnsureAllowed())
	{
		...
		va_list Args = {};
		const bool bResult = CheckVerifyImpl(bExecuted, bAlways, File, Line, PLATFORM_RETURN_ADDRESS(), Expr, TEXT(""), Args);
		return bResult;
	}
...
UE_AUTORTFM_ALWAYS_OPEN
bool UE_COLD UE_DEBUG_SECTION UE::Assert::Private::EnsureFailedV(std::atomic<bool>& bExecuted, const FStaticEnsureRecord* Ensure, void* ProgramCounter, va_list Args)
{
	...
	const bool bResult = CheckVerifyImpl(bExecuted, Ensure->bAlways, Ensure->File, Ensure->Line, ProgramCounter, Ensure->Expression, Ensure->Format, Args);
	return bResult;
}
...
bool UE_COLD UE_DEBUG_SECTION VARARGS UE::Assert::Private::EnsureFailed(std::atomic<bool>& bExecuted, const FStaticEnsureRecord* Ensure, ...)
{
	...
	const bool bResult = EnsureFailedV(bExecuted, Ensure, PLATFORM_RETURN_ADDRESS(), Args);
	va_end(Args);
