...
CORE_API FMalloc*			UE::Private::GMalloc	= nullptr;				/* Memory allocator */
CORE_API FMalloc* const&	GMalloc	                = UE::Private::GMalloc;	/* Provides backwards compatibility for external users of un-namespaced GMalloc. */
...
bool					PRIVATE_GIsRunningHybridCookedEditor= false;				/** Whether the editor is running in hybrid cooked editor mode */
bool					PRIVATE_GHasInitializedHybridCookedEditor = false;			/** Whether PRIVATE_GIsRunningHybridCookedEditor has been initialized */
...
TSAN_ATOMIC(bool)		GIsCriticalError				= false;					/* An appError() has occured */
...
#if PLATFORM_UNIX
uint8** CORE_API GNameBlocksDebug = FNameDebugVisualizer(FClangKeepDebugInfo{}).GetBlocks();
...
bool IsThisNotNull(const void* This, const ANSICHAR* Function)
{
	...
	UE_LOG(LogCore, Warning, TEXT("'%hs' was called with a null 'this' pointer - this is no longer supported."), Function);
	return false;
}
