	...
	if (bHasSearchedForPlatforms == false)
	{
		...
		for (int32 Index = 0; Index < NumFiles; Index++)
		{
			...
			if (IniFile.Contains(TEXT("DataDrivenPlatformInfo")))
			{
				...
				FString IniParent;
				bool bSettingPresent = IniFile.GetString(TEXT("DataDrivenPlatformInfo"), TEXT("IniParent"), IniParent);
				// we want an empty string in here if there is nothing already in it for the platform, however if we have a project
				// DDPI.ini file, and it doesn't have the setting, it should not overwrite the good value with empty string
				// so, add empty string if not in map, but only overwrite the engine value if the project value was present (even
				// if that value is empty string: "IniParent=")
				if (!IniParents.Contains(PlatformString) || bSettingPresent)
				{
