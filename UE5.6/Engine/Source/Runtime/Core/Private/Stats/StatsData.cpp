...
void FStatsThreadState::ScanForAdvance(const FStatMessagesArray& Data)
{
	...
	for (const FStatMessage& Item : Data)
	{
		...
		else if (Op == EStatOperation::AdvanceFrameEventEndOfPipe)
		{
			check(Item.NameAndInfo.GetField<EStatDataType>() == EStatDataType::ST_int64);
			int64 NewEndOfPipeFrame = Item.GetValue_int64();

			if (NewEndOfPipeFrame < 0)
			{
				NewEndOfPipeFrame = -NewEndOfPipeFrame;
				BadFrames.Add(NewEndOfPipeFrame - 1);
			}

			if (CurrentEndOfPipeFrame > STAT_FRAME_SLOP && CurrentEndOfPipeFrame + 1 != NewEndOfPipeFrame)
			{
				// this packet has multiple advances in it. They are all bad.
				check(CurrentEndOfPipeFrame + 1 < NewEndOfPipeFrame);
				for (int64 Frame = CurrentEndOfPipeFrame + 1; Frame <= NewEndOfPipeFrame; Frame++)
				{
					BadFrames.Add(Frame - 1);
				}
			}
			CurrentEndOfPipeFrame = NewEndOfPipeFrame;
		}
	}
...
void FStatsThreadState::ScanForAdvance(FStatPacketArray& NewData)
{
	...
	for (FStatPacket* Packet : NewData.Packets)
	{
		switch (Packet->ThreadType)
		{
			...
			case EThreadType::EndOfPipe:
				Packet->AssignFrame( CurrentEndOfPipeFrame );
				break;
	...
	for (FStatMessage& Item : Data)
	{
		...
		if (!Item.NameAndInfo.GetFlag(EStatMetaFlags::ShouldClearEveryFrame))
		{
			if (!(
				...
				Op != EStatOperation::AdvanceFrameEventEndOfPipe
				))
			{
			...
			else
			{
				...
				if (!Result)
				{
					UE_LOG(
						LogStats, Error,
						TEXT("Stat %s was cleared every frame, but we don't have metadata for it. Data loss.")
						TEXT(" (N.B. One cause for this can be duplicately named stats)"),
						*Item.NameAndInfo.GetRawName().ToString()
					);
				}
				else
				{
...
void FStatsThreadState::AddToHistoryAndEmpty(FStatPacketArray& NewData)
{
	...
	int64 LatestFinishedFrame = FMath::Min<int64>(CurrentGameFrame, CurrentRenderFrame, CurrentEndOfPipeFrame) - 1;
...
FName FStatsThreadState::GetStatThreadName( const FStatPacket& Packet ) const
{
	...
	else if ( Packet.ThreadType == EThreadType::EndOfPipe )
	{
		static const FName NAME_EndOfPipe = TEXT("EndOfPipe");
		ThreadName = NAME_EndOfPipe;
	}
...
void FStatsThreadState::Condense(int64 TargetFrame, TArray<FStatMessage>& OutStats) const
{
	...
	OutStats.Emplace(FStatConstants::AdvanceFrame.GetEncodedName(), EStatOperation::AdvanceFrameEventEndOfPipe, TargetFrame, false);
	FRawStatStackNode Root;
