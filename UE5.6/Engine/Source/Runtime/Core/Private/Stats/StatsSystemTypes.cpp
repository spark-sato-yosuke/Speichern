...
static void DebugLeakTest()
{
	if (CVarEnableLeakTest.GetValueOnGameThread() == 1)
	{
		if (GFrameCounter == 60)
		{
			UE::Stats::DirectStatsCommand(TEXT("stat namedmarker Frame-060"), true);
		}

		if (GFrameCounter == 120)
		{
			UE::Stats::DirectStatsCommand(TEXT("stat namedmarker Frame-120"), true);
		}


		if (GFrameCounter == 240)
		{
			UE::Stats::DirectStatsCommand(TEXT("stat namedmarker Frame-240"), true);
		}
		...
		if (GFrameCounter == 300)
		{
			UE_LOG(LogTemp, Warning, TEXT("Stat_ReallocTest: %" SIZE_T_FMT " / %i"), Array.GetAllocatedSize(), Initial);
		}
...
const TStatIdData* FThreadSafeStaticStatBase::DoSetup(FName InStatName, const TCHAR* InStatDesc, const char* InGroupName, const char* InGroupCategory, const TCHAR* InGroupDesc, bool bDefaultEnable, bool bShouldClearEveryFrame, EStatDataType::Type InStatType, bool bCycleStat, bool bSortByName, FPlatformMemory::EMemoryCounterRegion InMemoryRegion) const
{
	// send meta data, we don't use normal messages because the stats thread might not be running yet
	FStartupMessages::Get().AddMetadata(InStatName, InStatDesc, InGroupName, InGroupCategory, InGroupDesc, bShouldClearEveryFrame, InStatType, bCycleStat, bSortByName, InMemoryRegion);

	TStatIdData const* LocalHighPerformanceEnable(IStatGroupEnableManager::Get().GetHighPerformanceEnableForStat(InStatName, InGroupName, InGroupCategory, bDefaultEnable, bShouldClearEveryFrame, InStatType, InStatDesc, bCycleStat, bSortByName, InMemoryRegion).GetRawPointer());
...
class FStatGroupEnableManager : public IStatGroupEnableManager
{
	...
	FTransactionallySafeCriticalSection SynchronizationObject;
...
public:
	...
	{
		UE::TScopeLock ScopeLock(SynchronizationObject);
	...
	virtual TStatId GetHighPerformanceEnableForStat(FName StatShortName, const char* InGroup, const char* InCategory, bool bDefaultEnable, bool bShouldClearEveryFrame, EStatDataType::Type InStatType, TCHAR const* InDescription, bool bCycleStat, bool bSortByName, FPlatformMemory::EMemoryCounterRegion MemoryRegion = FPlatformMemory::MCR_Invalid) override
	{
		...
		FString StatDescription;
		{
			...
			{
				Result = &StatIDs.Last().Emplace_GetRef(NameToMinimalName(Stat));
			}
			else
			{
			...
			// Get the wide stat description.
			{
				...
				TCString<WIDECHAR>::Strncpy(StatDescWide.Get(), StatDescriptionWide.Get(), StatDescriptionWideLength + 1);
			...
			// Get the ansi stat description.
			{
				...
				TCString<ANSICHAR>::Strncpy(StatDescAnsi.Get(), StatDescriptionAnsi.Get(), StatDescriptionAnsiLength + 1);
				Result->StatDescriptionAnsi = MoveTemp(StatDescAnsi);

				MemoryAllocated += StatDescriptionAnsiLength * sizeof(ANSICHAR);
			}

			UE_AUTORTFM_ONCOMMIT(this, MemoryAllocated)
			{
				// Safe to defer this because FStatGroupEnableManager is a singleton, see IStatGroupEnableManager::Get()
...
CORE_API const char* FStatNameAndInfo::GpuStatCategory = "STATCAT_GPU";
...
private:
	UE::Tasks::FTask Barrier;
	FGraphEventRef Event;
};

void FLastFrameBarrier::Reset()
{
	Event = {};
	Barrier = {};
}

void FLastFrameBarrier::Wait()
{
	if (FPlatformProcess::SupportsMultithreading())
	{
		Barrier.Wait();
		Barrier = GStatsPipe.Launch(UE_SOURCE_LOCATION, [] {});
	}
	else
	{
		if (Event.GetReference())
		{
			FTaskGraphInterface::Get().WaitUntilTaskCompletes(Event, ENamedThreads::GameThread_Local);
		}

		Event = TGraphTask<FNullGraphTask>::CreateTask(NULL, ENamedThreads::GameThread).
...
static FLastFrameBarrier	LastFrameBarriers[MAX_STAT_LAG];
...
private:
	void Process()
	{
		...
		else
		{
			...
			bShouldProcess = bReadyToProcess && (FPlatformTime::Seconds() - LastTime > 0.005f || IncomingData.Packets.Num() > MaxIncomingPackets || UE::Stats::FStats::EnabledForCommandlet());
		}
		...
		if (bShouldProcess)
		{
			...
			INC_DWORD_STAT_BY(STAT_StatFramePacketsRecv, NowData.Packets.Num());
			{
				...
				{
					UE::TScopeLock Lock(FStartupMessages::Get().CriticalSection);
...
FThreadStats* FThreadStatsPool::GetFromPool()
{
	...
	if (void* Address = Pool.Pop())
	{
		return new (Address) FThreadStats;
	}

	return new FThreadStats;
}
...
FEndOfPipeStats FEndOfPipeStats::EndOfPipeStats;

FThreadStats::FThreadStats(EThreadType::Type InThreadType)
	: Packet(InThreadType)
	, CurrentGameFrame(UE::Stats::FStats::GameThreadStatsFrame.Load(EMemoryOrder::Relaxed))
{
	if (Packet.ThreadType == EThreadType::Invalid)
	{
		Packet.SetThreadProperties();
	}

	if (Packet.ThreadType != EThreadType::EndOfPipe)
	{
...
{
	bool bNewPrimaryEnable = 
		   WillEverCollectData()
		&& (!IsRunningCommandlet() || UE::Stats::FStats::EnabledForCommandlet())
		&& IsThreadingReady()
		&& PrimaryEnableCounter.GetValue();
...
bool FThreadStats::DetectAndUpdateCurrentGameFrame()
{
	if (Packet.ThreadType == EThreadType::Other)
	{
		FPlatformMisc::MemoryBarrier();
		int32 Frame = UE::Stats::FStats::GameThreadStatsFrame;
...
void FThreadStats::CheckForCollectingStartupStats()
{
	...
	while (1)
	{
		...
		for (int32 Index = 0; Index < CmdsArray.Num(); Index++)
		{
			...
			UE::Stats::DirectStatsCommand(*StatCmd);
		}
	...
	{
		UE::Stats::DirectStatsCommand(TEXT("stat group enable LinkerLoad"));
		UE::Stats::DirectStatsCommand(TEXT("stat group enable AsyncLoad"));
		UE::Stats::DirectStatsCommand(TEXT("stat group enable LoadTimeVerbose"));
		UE::Stats::DirectStatsCommand(TEXT("stat dumpsum -start -ms=250 -num=240"), true);
	}
	else if (FParse::Param(FCommandLine::Get(), TEXT("LoadTimeFile")) || UE::Stats::FStats::HasLoadTimeFileForCommandletToken())
	{
		UE::Stats::DirectStatsCommand(TEXT("stat group enable LinkerLoad"));
		UE::Stats::DirectStatsCommand(TEXT("stat group enable AsyncLoad"));
		UE::Stats::DirectStatsCommand(TEXT("stat group enable LoadTimeVerbose"));
		UE::Stats::DirectStatsCommand(TEXT("stat startfile"), true);
	}
	else if (UE::Stats::FStats::HasLoadTimeStatsForCommandletToken())
	{
		UE::Stats::DirectStatsCommand(TEXT("stat group enable LinkerLoad"));
		UE::Stats::DirectStatsCommand(TEXT("stat group enable AsyncLoad"));
		UE::Stats::DirectStatsCommand(TEXT("stat group enable LoadTimeVerbose"));
		UE::Stats::DirectStatsCommand(TEXT("stat dumpsum -start"), true);
	}
	...
	if (FStatsMallocProfilerProxy::HasMemoryProfilerToken())
	{
		...
		UE::Stats::DirectStatsCommand(TEXT("stat startfileraw"), true);
	}
...
void FThreadStats::StopThread()
{
	...
	if (IsThreadingReady())
	{
		if (UE::Stats::FStats::HasLoadTimeStatsForCommandletToken())
		{
			// Dump all the collected stats to the log, if any.
			UE::Stats::DirectStatsCommand(TEXT("stat dumpsum -stop -ms=100"), true);
		}
	...
	// release memory before the allocator is destroyed
	for (FLastFrameBarrier& Barrier : LastFrameBarriers)
	{
		Barrier.Reset();
	}
}
	...
	{
		{
			...
			LastFrameBarriers[EventIndex].Wait();
			CurrentEventIndex++;
		}
