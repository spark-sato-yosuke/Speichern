...
struct FMacApplicationInfo
{
	void Init()
	{
		...
		FCStringAnsi::Strncpy(AppNameUTF8, TCHAR_TO_UTF8(*AppName), PATH_MAX + 1);
		...
		FCStringAnsi::Strncpy(OSVersionUTF8, TCHAR_TO_UTF8(*OSVersion), PATH_MAX + 1);
		...
		FCStringAnsi::Strncpy(AppLogPath, TCHAR_TO_UTF8(*LogPath), PATH_MAX + 1);

		FString UserCrashVideoPath = IFileManager::Get().ConvertToAbsolutePathForExternalAppForWrite(*CrashVideoPath);
		FCStringAnsi::Strncpy(CrashReportVideo, TCHAR_TO_UTF8(*UserCrashVideoPath), PATH_MAX + 1);
		...
		FCStringAnsi::Strncpy(CrashReportPath, TCHAR_TO_UTF8(*ReportPath), PATH_MAX + 1);
#if WITH_EDITOR
...
#endif
		FCStringAnsi::Strncpy(CrashReportClient, TCHAR_TO_UTF8(*ReportClient), PATH_MAX + 1);
...
void FMacPlatformMisc::PlatformInit()
{
	...
	FString LogPath = FGenericPlatformOutputDevices::GetAbsoluteLogFilename();
	FCStringAnsi::Strncpy(GMacAppInfo.AppLogPath, TCHAR_TO_UTF8(*LogPath), PATH_MAX + 1);
...
{
	static FString BonjourName = TEXT("");
	if (BonjourName.IsEmpty())
	{
		CFStringRef LocalHostNameRef = SCDynamicStoreCopyLocalHostName(NULL);
		if (LocalHostNameRef != nullptr)
		{
			// Per the Bonjour docs, https://developer.apple.com/library/archive/documentation/Cocoa/Conceptual/NetServices/Articles/domainnames.html
			...
			FPlatformString::CFStringToTCHAR(LocalHostNameRef, Result);
			BonjourName = FString(Result);
			CFRelease(LocalHostNameRef);
		}
	}
...
int32 FMacPlatformMisc::XCodeVersionCompare(uint8 Major, uint8 Minor, uint8 Revision)
{
	uint8 TargetValues[3] = {Major, Minor, Revision};
	NSInteger ComponentValues[3] = {GMacAppInfo.XcodeVersion.majorVersion, GMacAppInfo.XcodeVersion.minorVersion, GMacAppInfo.XcodeVersion.patchVersion};
...
void FMacCrashContext::GenerateInfoInFolder(char const* const InfoFolder) const
{
	...
	if(!mkdir(CrashInfoFolder, 0766))
	{
		...
		FCStringAnsi::StrncatTruncateDest(FilePath, PATH_MAX, "/minidump.dmp");
		CopyMinidump(FilePath, GMacAppInfo.PLCrashReportPath);
		
		// generate "info.txt" custom data for our server
		FCStringAnsi::Strncpy(FilePath, CrashInfoFolder, PATH_MAX);
		FCStringAnsi::StrncatTruncateDest(FilePath, PATH_MAX, "/info.txt");
		...
		FCStringAnsi::Strncpy(FilePath, CrashInfoFolder, PATH_MAX);
		FCStringAnsi::StrncatTruncateDest(FilePath, PATH_MAX, "/" );
		FCStringAnsi::StrncatTruncateDest(FilePath, PATH_MAX, FGenericCrashContext::CrashContextRuntimeXMLNameA );
		SerializeAsXML( *FString(FilePath) );
		
		// copy log
		FCStringAnsi::Strncpy(FilePath, CrashInfoFolder, PATH_MAX);
		FCStringAnsi::StrncatTruncateDest(FilePath, PATH_MAX, "/");
		FCStringAnsi::StrncatTruncateDest(FilePath, PATH_MAX, (!GMacAppInfo.AppName.IsEmpty() ? GMacAppInfo.AppNameUTF8 : "Unreal"));
		FCStringAnsi::StrncatTruncateDest(FilePath, PATH_MAX, ".log");
		...
		FCStringAnsi::Strncpy(FilePath, CrashInfoFolder, PATH_MAX);
		FCStringAnsi::StrncatTruncateDest(FilePath, PATH_MAX, "/");
		FCStringAnsi::StrncatTruncateDest(FilePath, PATH_MAX, FGenericCrashContext::CrashConfigFileNameA);
		...
		if ( !GMacAppInfo.bIsSandboxed && GIsGPUCrashed && GMacAppInfo.KernelErrorDir )
		{
			...
			while(readdir_r(GMacAppInfo.KernelErrorDir, &DirEntry, &DirResult) == 0 && DirResult == &DirEntry)
			{
				if (strstr(DirEntry.d_name, ".gpuRestart"))
				{
					...
					FCStringAnsi::StrncatTruncateDest(FilePath, PATH_MAX, DirEntry.d_name);
		...
		if ( access(GMacAppInfo.CrashReportVideo, R_OK|F_OK) == 0 )
		{
			FCStringAnsi::Strncpy(FilePath, CrashInfoFolder, PATH_MAX);
			FCStringAnsi::StrncatTruncateDest(FilePath, PATH_MAX, "/");
			FCStringAnsi::StrncatTruncateDest(FilePath, PATH_MAX, "CrashVideo.avi");
