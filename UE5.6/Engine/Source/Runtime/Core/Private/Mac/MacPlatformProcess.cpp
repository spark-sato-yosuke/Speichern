...
FProcHandle FMacPlatformProcess::CreateProcInternal(const TCHAR* URL, const TCHAR* Parms, bool bLaunchDetached, bool bLaunchHidden, bool bLaunchReallyHidden, uint32* OutProcessID, int32 PriorityModifier, const TCHAR* OptionalWorkingDirectory, void* PipeStdOutChild, void* PipeStdErrChild, void *PipeStdInChild)
{
	...
	{
		UE_LOG(LogHAL, Fatal, TEXT("FMacPlatformProcess::CreateProc: posix_spawn(\"%s\") failed (%d, %s)"), *ProcessPath, PosixSpawnErrNo, UTF8_TO_TCHAR(strerror(PosixSpawnErrNo)));
...
const TCHAR* FMacPlatformProcess::BaseDir()
{
	...
	if (!Result[0])
	{
		...
		else
		{
			...
			FCString::Strncpy(Result, *FString(BasePath), MAC_MAX_PATH);
			FCString::StrncatTruncateDest(Result, MAC_MAX_PATH, TEXT("/"));
		}
	}
	return Result;
}
...
{
    extern void MainThreadCall(dispatch_block_t Block, bool const bWait, NSString* WaitMode);
