...
template <>
struct TCbJsonWriterConstants<UTF8CHAR> : TCbJsonWriterConstants<ANSICHAR>
{
};
...
{
	using FConstants = TCbJsonWriterConstants<CharType>;
...
{
	OutValue = FWideString(Field.AsString());
	return !Field.HasError();
}
...
class TCbJsonWriter
{
	using FConstants = UE::Serialization::Private::TCbJsonWriterConstants<CharType>;
...
private:
	...
	{
		UE::Serialization::Private::AppendQuotedJsonString(Builder, Value);
	}
	...
	void AppendBase64String(FMemoryView Value)
	{
		...
		FBase64::Encode(static_cast<const uint8*>(Value.GetData()), uint32(Value.GetSize()),
			reinterpret_cast<CharType*>(Builder.GetData() + EncodedIndex));
		Builder << '"';
	}
...
{
	TCbJsonWriter<UTF8CHAR> Writer(Builder);
...
{
	TCbJsonWriter<UTF8CHAR> Writer(Builder, ANSITEXTVIEW(""), ANSITEXTVIEW(""), ANSITEXTVIEW(""));
	Writer.WriteField(Field);
}
