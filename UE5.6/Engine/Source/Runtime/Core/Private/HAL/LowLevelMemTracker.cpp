...
#elif LLM_ALLOW_UOBJECTCLASSES_TAGS
	30000, // When using uobjects clases, you will have at least 20K+ LLM titles.
#else
	5000,
#endif
...
#elif LLM_ALLOW_UOBJECTCLASSES_TAGS
		if (FParse::Param(*Command, TEXT("UObjectClasses")))
		{
			TagSet = ELLMTagSet::UObjectClasses;
		}
#endif // LLM_ALLOW_UOBJECTCLASSES_TAGS
...
public:
	...
	void TrackAllocation(const void* Ptr, int64 Size, ELLMTracker Tracker, ELLMAllocType AllocType, 
		const FTagData* TagData, const FTagData* AssetTagData, const FTagData* AssetClassTagData, const FTagData* UObjectClassesTagData, bool bTrackInMemPro);
...
public:
	...
	public:
		FLowLevelAllocInfo()
		{
#if LLM_ENABLED_FULL_TAGS && (LLM_ALLOW_ASSETS_TAGS || LLM_ALLOW_UOBJECTCLASSES_TAGS)
			FMemory::Memset(Tag, InvalidCompressedTagValue);
#endif
		}

		void SetTag(const FTagData* InTag, FLowLevelMemTracker& InLLMRef
#if LLM_ALLOW_ASSETS_TAGS || LLM_ALLOW_UOBJECTCLASSES_TAGS
...
#if LLM_ALLOW_ASSETS_TAGS
#define TAG_SET_MAX 3
#else
#define TAG_SET_MAX 2
#endif
		static_assert(static_cast<uint32>(ELLMTagSet::Max) <= TAG_SET_MAX, "Every new TagSet enum adds 4 bytes of LLM overhead memory for every active allocation. Do not add them without measuring the cost of the overhead in the cook and runtime of large projects.");
#endif // LLM_ALLOW_ASSETS_TAGS
#else
...
static FName TagName_UntaggedUObjectClasses(TEXT("UntaggedUObjectClasses"));
...
void FLowLevelMemTracker::BootstrapInitialise()
{
	...
	FScopeLock BootstrapScopeLock(&BootstrapLock);
	if (bInitialisedTracking)
	{
		return;
	}
	// just make sure this part isn't reentrant.
	if (bBeganInitialisedTracking != false)
	{
		UE_LOG(LogInit, Fatal, TEXT("FLowLevelMemTracker::BootstrapInitialise is not reentrant"));
	}
	bBeganInitialisedTracking = true;
...
void FLowLevelMemTracker::ProcessCommandLine(const TCHAR* CmdLine)
{
	...
	FString SetList;
	static_assert((uint8)ELLMTagSet::Max == TAG_SET_MAX,
...
#elif LLM_ALLOW_UOBJECTCLASSES_TAGS
			if (Set == TEXT("UObjectClasses"))
			{
				ActiveSets[static_cast<int32>(ELLMTagSet::UObjectClasses)] = true;
			}
#else
...
#if LLM_ALLOW_UOBJECTCLASSES_TAGS
	const FTagData* UObjectClassesTagData = State->GetTopTag(ELLMTagSet::UObjectClasses);
#else
	const FTagData* UObjectClassesTagData = nullptr;
#endif

	// track on the thread state
	State->TrackAllocation(Ptr, Size, Tracker, AllocType, ActiveTagData, AssetTagData, AssetClassTagData, UObjectClassesTagData, bTrackInMemPro);
...
#endif
#if LLM_ALLOW_UOBJECTCLASSES_TAGS
		// If there is no scoped tag for UObjectClasses, don't set it
		if (UObjectClassesTagData != nullptr)
		{
			AllocInfo.SetTag(UObjectClassesTagData, LLMRef, ELLMTagSet::UObjectClasses);
		}
#endif
...
void FLLMTracker::TrackFree(const void* Ptr, ELLMAllocType AllocType, bool bTrackInMemPro)
{
...
#if LLM_ALLOW_UOBJECTCLASSES_TAGS	
	const FTagData* UObjectClassesTagData = AllocInfo.GetTag(LLMRef, ELLMTagSet::UObjectClasses);
#else
	const FTagData* UObjectClassesTagData = nullptr;
#endif // LLM_ALLOW_UOBJECTCLASSES_TAGS
	...
	State->TrackFree(Ptr, Size, Tracker, AllocType, TagData, AssetTagData, AssetClassTagData, UObjectClassesTagData, bTrackInMemPro);
}
...
#if LLM_ALLOW_UOBJECTCLASSES_TAGS
	if (UObjectClassesTagData)
	{
		IncrTag(UObjectClassesTagData, Size);
	}
#endif // LLM_ALLOW_UOBJECTCLASSES_TAGS
	...
	UE::LLM::ESizeParams SizeParams)
{
	...
	{
		const FTagData* TagData = It.Key;
		if (!TagData->IsStatsReportable() || !LLMRef.IsTagSetActive(TagData->GetTagSet()))
		{
			continue;
		}
		if (OverrideUntaggedTagData != nullptr && TagData->GetTagSet() == ELLMTagSet::None && TagData->GetName() == TagName_Untagged)
		{
	...
	for (const TPair<const FTagData*, FTrackerTagSizeData>& It : TagSizes)
	{
		...
		if (OverrideUntaggedTagData && TagData->GetTagSet() == ELLMTagSet::None && TagData->GetName() == TagName_Untagged)
		{
			continue; // Handled by OverrideUntaggedTagData.
		}
