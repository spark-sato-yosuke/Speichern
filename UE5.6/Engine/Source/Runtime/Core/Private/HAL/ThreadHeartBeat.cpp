...
FThreadHeartBeatClock::FThreadHeartBeatClock(double InMaxTimeStep)
	: MaxTimeStepCycles(FPlatformTime::SecondsToCycles64(InMaxTimeStep))
{
...
double FThreadHeartBeatClock::Seconds()
{
	...
	uint64 CyclesPerSecond = FPlatformTime::SecondsToCycles64(1.0);
	...
	return (double)Seconds + FPlatformTime::ToSeconds64(RemainderCycles);
}
	...
	*((uint32*)3) = 0xe0000001;
	...
	if (CallstackCRC != LastHangCallstackCRC || ThreadThatHung != LastHungThreadId)
	{
		...
		if (bHangsAreFatal)
		{
			...
			FPlatformMisc::RequestExit(true, TEXT("FThreadHeartbeat::OnHang"));
		}
#else
		if (bHangsAreFatal)
		{
...
void FGameThreadHitchHeartBeatThreaded::ResumeHeartBeat()
{
	...
	// Defer heartbeat-resume to the end of our transaction, if one is running.
