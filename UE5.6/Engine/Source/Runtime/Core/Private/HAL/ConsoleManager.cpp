...
public:
	...
	bool Track(const TCHAR* InValue, EConsoleVariableFlags Flags, FName Tag)
	{
		...
		{
			// untagged array types must only have one entry with no tag, but we push it to the end
			if (Tag == NAME_None)
			{
...
public:
	...
	virtual void Set(const TCHAR* InValue, EConsoleVariableFlags SetBy, FName Tag)
	{
		...
		if (this->TrackHistory(InValue, SetBy, Tag) == false)
		{
			return;
		}
...
{
	if (PreprocessSet(InValue, SetBy, Tag))
	{
