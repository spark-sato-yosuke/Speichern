...
FTaskTagScope::FTaskTagScope(ETaskTag InTag) : Tag(InTag)
{
...
#	endif
	...
	if (!EnumHasAllFlags(Tag, ETaskTag::EParallelThread))
	{
		...
		checkf(IsOK || IsEngineExitRequested(), TEXT("Only Scopes tagged with ETaskTag::EParallelThread can be tagged multiple times. ActiveNamedThreads(%x) cannot be tagged multiple times in the same callstack with the current Tag(%x)"), OldTag, Tag);
	}
...
{
PRAGMA_DISABLE_DEPRECATION_WARNINGS
	bool newValue = (GRenderThreadId == 0)
	...
	{
		const uint32 CurrentThreadId = FPlatformTLS::GetCurrentThreadId();
		bool oldValue = (GRenderThreadId == 0) ? (CurrentThreadId == GGameThreadId) || FTaskTagScope::IsRunningDuringStaticInit() : (CurrentThreadId == GRenderThreadId);
		ensureMsgf(oldValue == newValue, TEXT("oldValue(%i) newValue(%i) If this check fails make sure that there is a FTaskTagScope(ETaskTag::ERenderingThread) as deep as possible on the current callstack, you can see the current value in ActiveNamedThreads(%x), GRenderingThread(%p)"), oldValue, newValue, FTaskTagScope::GetCurrentTag(), GRenderingThread);
		newValue = oldValue;
	}
#endif
	return newValue;
...
{
	return FTaskTagScope::IsCurrentTag(ETaskTag::EParallelRenderingThread)
		|| FTaskTagScope::IsCurrentTag(ETaskTag::ERenderingThread)
...
PRAGMA_DISABLE_DEPRECATION_WARNINGS
	...
	{
		const uint32 CurrentThreadId = FPlatformTLS::GetCurrentThreadId();
		bool oldValue = (GRenderThreadId == 0) ?  true : CurrentThreadId != GGameThreadId;
		ensureMsgf(oldValue == newValue, TEXT("oldValue(%i) newValue(%i) If this check fails make sure that there is a FTaskTagScope(ETaskTag::EParallelRenderingThread) as deep as possible on the current callstack, you can see the current value in ActiveNamedThreads(%x), GRenderingThread(%p)"), oldValue, newValue, FTaskTagScope::GetCurrentTag(), GRenderingThread);
		newValue = oldValue;
	}
#endif
	return newValue;
