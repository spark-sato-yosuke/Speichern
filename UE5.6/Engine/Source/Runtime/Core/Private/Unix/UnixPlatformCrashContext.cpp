...
#define HANDLE_CASE(a,b) case a: ErrorString += TEXT(#a ": " b); break;
	...
	case SIGSEGV:
		...
		{
			ErrorString += FString::Printf(TEXT("SIGSEGV: unaligned memory access at address 0x%016llx (SIMD vectors?)"), (uint64)Info->si_addr);
		}
		else
		{
...
void FUnixCrashContext::InitFromSignal(int32 InSignal, siginfo_t* InInfo, void* InContext)
{
	...
	FCString::StrncatTruncateDest(SignalDescription, UE_ARRAY_COUNT( SignalDescription ), *DescribeSignal(Signal, Info, Context));
}
...
void FUnixCrashContext::InitFromDiagnostics(const void* InAddress)
{
	...
	FCString::Strncpy(SignalDescription, ErrorMessage, UE_ARRAY_COUNT(SignalDescription));
...
void FUnixCrashContext::CaptureStackTrace(void* ErrorProgramCounter)
{
	...
	if (!bCapturedBacktrace)
	{
		...
		FCString::StrncatTruncateDest( GErrorHist, UE_ARRAY_COUNT(GErrorHist), UTF8_TO_TCHAR(StackTrace) );
...
void PlatformCrashHandler(int32 Signal, siginfo_t* Info, void* Context)
{
	...
	// Expose OOM root cause during generic crash (eg. SIGSEGV on nullptr).
	if (Type == ECrashContextType::Crash && FPlatformMemory::bIsOOM)
	{
