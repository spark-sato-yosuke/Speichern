...
void FGenericPlatformMisc::BeginNamedEventFrame()
{
...
void FGenericPlatformMisc::CustomNamedStat(const TCHAR* Text, float Value, const TCHAR* Graph, const TCHAR* Unit)
{
...
void FGenericPlatformMisc::CustomNamedStat(const ANSICHAR* Text, float Value, const ANSICHAR* Graph, const ANSICHAR* Unit)
{
	FRAMEPRO_DYNAMIC_CUSTOM_STAT(Text, Value, Graph, Unit, FRAMEPRO_COLOUR(255,255,255));
}

#endif // ENABLE_GENERIC_NAMED_EVENTS
	...
	bool& bOutSetPhysicalCountToLogicalCount)
{
	...
	// Limit the number of threads as we have a limit of 256 context id in the shadow memory structure.
	constexpr int32 InstrumentationCoreLimit = 32;
	PhysicalCoreLimit = PhysicalCoreLimit == 0 ? InstrumentationCoreLimit : FMath::Min(PhysicalCoreLimit, InstrumentationCoreLimit);
	LogicalCoreLimit  = LogicalCoreLimit  == 0 ? InstrumentationCoreLimit : FMath::Min(LogicalCoreLimit,  InstrumentationCoreLimit);
#endif
...
bool FGenericPlatformMisc::FileExistsInPlatformPackage(const FString& RelativePath)
{
	...
	{
		UE_LOG(LogGenericPlatformMisc, Warning, TEXT("FileExistsInPlatformPackage: expected a relative path, but received %s"), *RelativePath);
		return false;
	}
...
{
	return TEXTVIEW("pakchunk");
}

int32 FGenericPlatformMisc::GetPakchunkIndexFromPakFile(FStringView InFilename)
{
	FStringView ChunkIdentifier(GetPakFilenamePrefix());
	const FString NamedChunkIdentifier(FString(GetPakFilenamePrefix()) + NAMED_PAK_CHUNK_DELIMITER_CHAR);
	FStringView BaseFilename = FPathViews::GetBaseFilename(InFilename);
	int32 ChunkNumber = INDEX_NONE;

	if (FCoreDelegates::PakChunkIdOverrideDelegate.IsBound() && BaseFilename.StartsWith(*NamedChunkIdentifier))
	{
		const int32 StartOfChunkName = NamedChunkIdentifier.Len();
		int32 CharCount = 0;
		while ((CharCount + StartOfChunkName) < BaseFilename.Len() && BaseFilename[StartOfChunkName + CharCount] != NAMED_PAK_CHUNK_DELIMITER_CHAR)
		{
			CharCount++;
		}

		FString ChunkName = FString(BaseFilename.Mid(StartOfChunkName, CharCount));
		int32 ChunkId = FCoreDelegates::PakChunkIdOverrideDelegate.Execute(ChunkName);
		if (ChunkId != INDEX_NONE)
		{
			return ChunkId;
		}
	}
	else if (BaseFilename.StartsWith(ChunkIdentifier))
	{
...
{
	static TMap<FString, FString> Temp;
	return Temp;
}

const FString* FGenericPlatformMisc::GetConfigRulesVariable(const FString& Key) 
{
	return FPlatformMisc::GetConfigRuleVars().Find(Key);
}
