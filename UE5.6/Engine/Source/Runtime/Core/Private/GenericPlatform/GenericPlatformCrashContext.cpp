...
namespace NCached
{
	...
	static TArray<FString> FilePaths;
...
FString FGenericCrashContext::GenerateNewCrashGuidBase()
{
	...
	const FString GuidRoot = FString::Printf(TEXT("%s%s-%s"), CrashGUIDRootPrefix, *IniPlatformName, *Guid.ToString(EGuidFormats::Digits));
	NCached::Set(NCached::Session.CrashGUIDRoot, *GuidRoot);
...
void FGenericCrashContext::Initialize()
{
	...
	{
		UE_LOG(LogInit, Error, TEXT("Symbols label too long (%d) for field size(%" SIZE_T_FMT "), truncated. This may cause problems with crash report symbolication."),
	...
	const FString GuidRoot = GenerateNewCrashGuidBase();
	UE_LOG(LogInit, Log, TEXT("Session CrashGUID >====================================================\n         Session CrashGUID >   %s\n         Session CrashGUID >===================================================="), *GuidRoot);
	...
	FCoreDelegates::OnPostFork.AddLambda([](EForkProcessRole Role)
	{
		if (Role == EForkProcessRole::Child)
		{
			...
			// Generate a new unique CrashGuidBase for this child
			const FString GuidRoot = GenerateNewCrashGuidBase();
			UE_LOG(LogInit, Log, TEXT("Session CrashGUID >====================================================\n                                       Session CrashGUID >   %s\n                                       Session CrashGUID >===================================================="), *GuidRoot);			
			SerializeTempCrashContextToFile();
		}
...
void FGenericCrashContext::InitializeFromContext(const FSessionContext& Session, const TCHAR* EnabledPluginsStr, const TCHAR* EngineDataStr, const TCHAR* GameDataStr, const TCHAR* FilePathData, const FGPUBreadcrumbsSharedContext* GPUBreadcrumbs)
{
	...
	{
		TArray<FString> Tokens;
		FString(FilePathData).ParseIntoArray(Tokens, TokenDelim, 2, true);

		for (FString& Token : Tokens)
		{
			NCached::FilePaths.Add(Token);
		}
	}
...
void FGenericCrashContext::CopySharedCrashContext(FSharedCrashContext& Dst)
{
	...
	FStringBuilderBase DynamicDataBuilder;
	DynamicDataBuilder.Reserve(CR_MAX_DYNAMIC_BUFFER_CHARS);

	Dst.EnabledPluginsOffset = (uint32)DynamicDataBuilder.Len();
	...
	{
		DynamicDataBuilder.Append(Plugin);
		DynamicDataBuilder.Append(CR_PAIR_DELIM);
	}
#else
	Dst.EnabledPluginsNum = 0;
#endif

	Dst.EngineDataOffset = (uint32)DynamicDataBuilder.Len();
	Dst.EngineDataNum = NCached::EngineData.Num();
	for (const TPair<FString, FString>& Pair : NCached::EngineData)
	{
		DynamicDataBuilder.Append(Pair.Key);
		DynamicDataBuilder.Append(CR_PAIR_EQ);
		DynamicDataBuilder.Append(Pair.Value);
		DynamicDataBuilder.Append(CR_PAIR_DELIM);
	}

	Dst.GameDataOffset = (uint32)DynamicDataBuilder.Len();
	...
	Dst.FilePathsOffset = (uint32)DynamicDataBuilder.Len();
	Dst.FilePathsNum = NCached::FilePaths.Num();
	for (const FString& FilePath : NCached::FilePaths)
	{
		DynamicDataBuilder.Append(FilePath);
		DynamicDataBuilder.Append(CR_PAIR_DELIM);
	}

	FCString::Strncpy(Dst.DynamicData, DynamicDataBuilder.ToString(), CR_MAX_DYNAMIC_BUFFER_CHARS - 1);
	Dst.DynamicData[CR_MAX_DYNAMIC_BUFFER_CHARS - 1] = 0;
...
void FGenericCrashContext::SerializeTempCrashContextToFile()
{
	...
	// Add the engine data
	BeginSection(SessionBuffer, EngineDataTag);
	for (const TPair<FString, FString>& Pair : NCached::EngineData)
	{
		AddCrashPropertyInternal(SessionBuffer, *Pair.Key, *Pair.Value);
	}
	EndSection(SessionBuffer, EngineDataTag);

	// Add the game data
	BeginSection(SessionBuffer, GameDataTag);
	for (const TPair<FString, FString>& Pair : NCached::GameData)
	{
		AddCrashPropertyInternal(SessionBuffer, *Pair.Key, *Pair.Value);
	}
	EndSection(SessionBuffer, GameDataTag);
...
{
	FScopeLock Lock(&NCached::CriticalSection);
	NCached::FilePaths.Add(AbsoluteSourcePath);
}
...
void FGenericCrashContext::CopyPlatformSpecificFiles(const TCHAR* OutputDirectory, void* Context)
{
	...
	for (const FString& AbsoluteSource : NCached::FilePaths)
	{
		if (IFileManager::Get().FileExists(*AbsoluteSource))
		{
			const FString AbsoluteDest = FPaths::Combine(OutputDirectory, FPaths::GetCleanFilename(AbsoluteSource));

			// Try to move file, may fail due to any number of reasons
			if (!IFileManager::Get().Move(*AbsoluteDest, *AbsoluteSource))
			{
				UE_LOG(LogCrashContext, Error, TEXT("Error moving crash context file '%s' to '%s'"), *AbsoluteSource, *AbsoluteDest);
			}
		}
	}
}
