...
bool FIoChunkEncoding::Decode(const FIoChunkDecodingParams& Params, FMemoryView EncodedBlocks, FMutableMemoryView OutRawData, EIoDecodeFlags Flags)
{
	...
	const bool bForceTempBuffer = EnumHasAnyFlags(Flags, EIoDecodeFlags::WriteOnly);
	...
	for (uint32 BlockIndex = FirstBlockIndex; BlockIndex <= LastBlockIndex; BlockIndex++)
	{
		...
		{
			if (RawBlockReadSize == RawBlockSize && !bForceTempBuffer)
			{
...
FIoChunkBlockDecodeResult FIoChunkEncoding::DecodeBlock(const FIoChunkBlockDecodingParams Params, FMutableMemoryView EncodedBlock, FMutableMemoryView DecodedBlock)
{
	// Assume that the underlying memory size is aligned to AES block size 
	const uint32 AlignedEncodedSize = Align(uint32(EncodedBlock.GetSize()), FAES::AESBlockSize);

	if (Params.BlockHash.IsEmpty() == false)
	{
		check(Params.HashFunction == EIoSignatureHashFunction::Sha1);
		FSHAHash ShaHash;
		FSHA1::HashBuffer(EncodedBlock.GetData(), AlignedEncodedSize, ShaHash.Hash);
		FMemoryView BlockHash(ShaHash.Hash, sizeof(ShaHash.Hash));

		if (0 != Params.BlockHash.CompareBytes(BlockHash))
		{
			return FIoChunkBlockDecodeResult
			{
				.ErrorCode = EIoErrorCode::SignatureError
			};
		}
	}

	if (Params.EncryptionKey.IsEmpty() == false)
	{
		FAES::FAESKey AESKey;
		FMutableMemoryView(AESKey.Key, FAES::FAESKey::KeySize).CopyFrom(Params.EncryptionKey);
		FAES::DecryptData((uint8*)EncodedBlock.GetData(), AlignedEncodedSize, AESKey);
	}

	if (Params.CompressionFormat.IsNone())
	{
		return FIoChunkBlockDecodeResult
		{
			.DecodedBlock	= FMutableMemoryView(EncodedBlock.GetData(), DecodedBlock.GetSize()),
			.ErrorCode		= EIoErrorCode::Ok
		};
	}

	if (LIKELY(FCompression::UncompressMemory(
		Params.CompressionFormat,
		DecodedBlock.GetData(),
		IntCastChecked<int32>(DecodedBlock.GetSize()),
		EncodedBlock.GetData(),
		IntCastChecked<int32>(EncodedBlock.GetSize()))))
	{
		return FIoChunkBlockDecodeResult
		{
			.DecodedBlock	= FMutableMemoryView(DecodedBlock.GetData(), DecodedBlock.GetSize()),
			.ErrorCode		= EIoErrorCode::Ok
		};
	}
	...
	return FIoChunkBlockDecodeResult
	{
		.ErrorCode = EIoErrorCode::CompressionError
	};
}
...
////////////////////////////////////////////////////////////////////////////////
void FIoChunkBlockDecoder::Set(IIoChunkBlockDecoder& InInstance)
{
	check(Instance == nullptr);
	Instance = &InInstance;
}

IIoChunkBlockDecoder& FIoChunkBlockDecoder::Get()
{
	check(Instance);
	return *Instance;
}

void FIoChunkBlockDecoder::Release()
{
	Instance = nullptr;
}

IIoChunkBlockDecoder* FIoChunkBlockDecoder::Instance = nullptr;
