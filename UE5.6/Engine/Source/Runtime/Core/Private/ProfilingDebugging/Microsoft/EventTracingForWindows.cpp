...
#include COMPILED_PLATFORM_HEADER(PlatformEventTracingForWindows.h) // IWYU pragma: export
...
void FPlatformEvents::TraceEventCallback(PEVENT_RECORD Event)
{
	...
	if (Opcode == FContextSwitchEvent::Type)
	{
		...
		// On some platforms we are getting Event->UserDataLength > sizeof(FContextSwitchEvent), the delta in size is undocumented 
