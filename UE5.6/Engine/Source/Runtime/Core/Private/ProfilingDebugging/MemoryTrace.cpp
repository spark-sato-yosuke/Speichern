...
UE_TRACE_EVENT_BEGIN(Memory, UpdateAlloc) // added in UE 5.6
	UE_TRACE_EVENT_FIELD(uint64, Address)
	UE_TRACE_EVENT_FIELD(uint32, CallstackId)
	UE_TRACE_EVENT_FIELD(uint8, RootHeap)
UE_TRACE_EVENT_END()

UE_TRACE_EVENT_BEGIN(Memory, MemorySwapOp) // added in UE 5.5
			...
			if (SubArg.SubStr(0, 7).Equals("-trace=", ESearchCase::IgnoreCase))
			{
				...
				while (!ChannelsOrPresets.IsEmpty())
				{
					...
					if (Channel.Equals("memalloc", ESearchCase::IgnoreCase))
					{
						...
						// The tags and memory scopes are also emitted on the "memalloc" channel.
					...
					{
						// The "memtag" channel is only used to enable LLM tracing.
					}
					else if (Channel.Equals("probing", ESearchCase::IgnoreCase))
					{
						Mode |= EMemoryTraceInit::Probing;
					}
...
FMalloc* MemoryTrace_CreateInternal(FMalloc* InMalloc, EMemoryTraceInit Mode)
{
	...
	if (EnumHasAnyFlags(Mode, EMemoryTraceInit::Probing))
	{
		MemoryTrace_InitMemoryProbe();
	}
...
////////////////////////////////////////////////////////////////////////////////
void MemoryTrace_UpdateAlloc(uint64 Address, HeapId RootHeap, uint32 ExternalCallstackId)
{
	if (!GTraceAllowed)
	{
		return;
	...
	UE_TRACE_LOG(Memory, UpdateAlloc, MemAllocChannel)
		<< UpdateAlloc.Address(uint64(Address))
		<< UpdateAlloc.CallstackId(CallstackId)
		<< UpdateAlloc.RootHeap(uint8(RootHeap));
