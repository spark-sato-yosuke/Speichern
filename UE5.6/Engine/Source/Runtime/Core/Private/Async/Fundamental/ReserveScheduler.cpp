...
TUniquePtr<FThread> FReserveScheduler::CreateWorker(FThread::EForkable IsForkable, FYieldedWork* ReserveEvent, EThreadPriority Priority)
{
	...
	(
		*FString::Printf(TEXT("Reserve Worker #%d"), WorkerId),
		[this, ReserveEvent]
		{
			FSchedulerTls::FTlsValues& TlsValues = FSchedulerTls::GetTlsValuesRef();
			TlsValues.ActiveScheduler = this;
			TlsValues.LocalQueue = nullptr;
			...
			while (true)
			{
				...
				ReserveEvent->SleepEvent->Wait();
				{
					...
					TlsValues.WorkerType = ReserveEvent->bPermitBackgroundWork ? FSchedulerTls::EWorkerType::Background : FSchedulerTls::EWorkerType::Foreground;
			...
			TlsValues.WorkerType = EWorkerType::None;
			TlsValues.ActiveScheduler = nullptr;
...
bool FReserveScheduler::DoReserveWorkUntil(FConditional&& Condition)
{
	if (FYieldedWork* WorkerEvent = EventStack.Pop())
	{
		...
		WorkerEvent->bPermitBackgroundWork = FSchedulerTls::GetTlsValuesRef().IsBackgroundWorker();
