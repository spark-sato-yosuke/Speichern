...
namespace Tasks2Tests
{
	...
	bool FTasksLocalGlobalPriorities::RunTest(const FString& Parameters)
	{
		...
		// Avoid any kind of task stealing.
		LowLevelTasks::FScheduler::Get().RestartWorkers(0, 1);
		...
		RootTask.Init(
			...
			[&]()
			{
				// Both task are launched from inside a worker with no additional wake up so that both task should end up being run in the same worker
		...
		while (!LocalTask.IsCompleted()) {};
		while (!GlobalTask.IsCompleted()) {};
		verify(GlobalRun == true);
