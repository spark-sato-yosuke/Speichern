...
namespace UE::FIOSPlatformMisc
{
	...
	nw_path_monitor_t PathMonitor = nullptr;
	nw_path_t CachedNetworkPath = nullptr;
	ENetworkConnectionType LastReportedNetworkType = ENetworkConnectionType::Unknown;
...
void FIOSPlatformMisc::PlatformInit()
{
	...
	nw_path_monitor_set_update_handler(
		...
		^(nw_path_t InPath)
		{
			UE_LOG(LogInit, Log, TEXT("Network path changed"));
...
void FIOSPlatformMisc::SetAllowedDeviceOrientation(EDeviceScreenOrientation NewAllowedDeviceOrientation)
{
	...
	// Notify the view controller about a change in supported interface orientations.
	...
	dispatch_async(dispatch_get_main_queue(),
		^{
			...
			{
// Deprecated iOS16 - suppress warning if deployment target	>= iOS16
