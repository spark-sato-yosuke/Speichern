...
uint64 FWindowsPlatformFile::FileJournalGetMaximumSize(const TCHAR* VolumeOrPath, ELogVerbosity::Type* OutErrorLevel,
	FString* OutError) const
{
	...
	EFileJournalResult Result = CreateJournalWindowsHandle(VolumeOrPath, JournalHandle, NormalizedVolumeName, OutError);
	if (Result != EFileJournalResult::Success)
	{
		if (OutErrorLevel)
		{
	...
	return JournalDescriptor.MaximumSize;
}
...
bool FWindowsPlatformFile::FileJournalIterateDirectory(const TCHAR* Directory,
	FDirectoryJournalVisitorFunc Visitor, FString* OutError)
{
	using namespace UE::WindowsPlatformFileJournal::Private;
	...
	{
		return IPlatformFile::FileJournalIterateDirectory(Directory, Visitor, OutError);
	}
	...
	if (Handle == INVALID_HANDLE_VALUE)
	{
		if (OutError)
		{
			...
			*OutError = FString::Printf(TEXT("FileJournal: CreateFile('%s') failed; FileJournal feature is unavailable. LastError=%u (0x%08x) (%s)."),
	...
	if (!Result)
	{
		if (OutError)
		{
			...
			*OutError = FString::Printf(TEXT("Unexpected failure of GetFileInformationByHandleEx('%s') with error code %u (0x%08x) (%s)."),
				*Normalized, LastError, LastError, *JournalGetLastErrorString(LastError));
		}
	...
	for (;;) // For each call to GetFileInformationByHandleEx
	{
		...
		for (;;) // For every FILE_ID_EXTD_DIR_INFO in the buffer returned by GetFileInformationByHandleEx
		{
			...
			if (RelPath != TEXTVIEW(".") && RelPath != TEXTVIEW(".."))
			{
				...
				FileData.bIsReparsePoint = !!(Info->FileAttributes & FILE_ATTRIBUTE_REPARSE_POINT);
...
FFileJournalData FWindowsPlatformFile::FileJournalGetFileData(const TCHAR* FilenameOrDirectory, FString* OutError)
{
	using namespace UE::WindowsPlatformFileJournal::Private;
	using namespace UE::WindowsPlatformFile::Private;
	...
	OutResult.bIsReparsePoint = !!(Info->FileAttributes & FILE_ATTRIBUTE_REPARSE_POINT);
	FILETIME LastWriteTime;
	LastWriteTime.dwLowDateTime = Info->LastWriteTime.LowPart;
	LastWriteTime.dwHighDateTime = Info->LastWriteTime.HighPart;
				...
				case ERROR_JOURNAL_ENTRY_DELETED:
					if (OutError)
					{
						*OutError = FString::Printf(TEXT("NTFS Journal has wrapped for volume '%s'; the number of files written to this volume exceeded the buffer size of the journal and some entries were dropped.")
							...
							TEXT("\r\n<SizeInBytes> defaults to 33554432 (32MB), Recommended <SizeInBytes> is 0x40000000 (1GB). You can see the current value via `fsutil usn queryJournal %s`, 'Maximum Size' field."),
		...
		case ERROR_JOURNAL_NOT_ACTIVE:
			if (OutError)
			{
				...
				*OutError = FString::Printf(TEXT("NTFS Journal is not active for volume '%s'. ")
					...
					TEXT("Recommended <SizeInBytes> is 0x40000000 (1GB)."),
	...
	FString* OutError) const
{
	...
	return IPlatformFile::FileJournalGetMaximumSize(VolumeOrPath, OutErrorLevel, OutError);
}
...
FFileJournalData FWindowsPlatformFile::FileJournalGetFileData(const TCHAR* FilenameOrDirectory, FString* OutError)
{
	return IPlatformFile::FileJournalGetFileData(FilenameOrDirectory, OutError);
}
