...
// MiniDumpWriteDump hangs in ASAN builds and stack walking can race with ASAN's usage of DbgHelp (which is not thread safe).
#if PLATFORM_HAS_ASAN_INCLUDE
#	if defined(__SANITIZE_ADDRESS__)
#		define DISABLE_DEBUG_INFO_DUE_TO_ASAN 1
...
#ifndef DISABLE_DEBUG_INFO_DUE_TO_ASAN
...
namespace {
	...
	{
#if DISABLE_DEBUG_INFO_DUE_TO_ASAN
		return false;
#else
...
namespace UE::Core::Private
{
	...
	static TAutoConsoleVariable<bool> CVarScreenshotOnGPUCrash(
		TEXT("WindowsPlatformCrashContext.ScreenshotOnGPUCrash"),
		false,
		TEXT("Take a screenshot of the game window to include in crash reports for GPU crashes (Windows only, cooked client only)\n"),
		ECVF_Default
	);
}
...
CORE_API void FWindowsPlatformCrashContext::AddOptionalAttachment(const FString& OptionalAttachmentFilepath)
{
	OptionalAttachmentFilepaths.Add(OptionalAttachmentFilepath);
}

void FWindowsPlatformCrashContext::CopyOptionalAttachments(const TCHAR* BaseOutputDirectory) const
{
	// If there are any optional attachments, copy them to the appropriate subdirectory.
	if (OptionalAttachmentFilepaths.IsEmpty())
	{
		return;
	}

	FString OptionalAttachmentsDirectory = FPaths::Combine(BaseOutputDirectory, CrashReportOptionalAttachmentsDirName);
	if (!IFileManager::Get().MakeDirectory(*OptionalAttachmentsDirectory))
	{
		UE_LOG(LogWindows, Error, TEXT("Error creating crash report's optional attachments directory %s"), *OptionalAttachmentsDirectory);
		return;
	}

	bool bHasOptionalAttachments = false;
	for (const FString& OptionalAttachmentFilepath : OptionalAttachmentFilepaths)
	{
		FString OutputFilepath = FPaths::Combine(OptionalAttachmentsDirectory, FPaths::GetCleanFilename(OptionalAttachmentFilepath));
		if (IFileManager::Get().Move(*OutputFilepath, *OptionalAttachmentFilepath))
		{
			bHasOptionalAttachments = true;

		}
		else
		{
			UE_LOG(LogWindows, Error, TEXT("Error moving crash report's optional attachment file %s to output directory"), *OptionalAttachmentFilepath);
		}
	}

	if (bHasOptionalAttachments)
	{
		FString OptionalAttachmentsXML = FPaths::Combine(OptionalAttachmentsDirectory, FPlatformCrashContext::CrashContextRuntimeXMLNameW);
		WriteOptionalAttachmentsXML(OptionalAttachmentsXML);
	}
}

void FWindowsPlatformCrashContext::WriteOptionalAttachmentsXML(const FString& Filepath) const
{
	FString Buffer;
	...
	// Use the same GUID from the base crash report.
	BeginSection(Buffer, RuntimePropertiesTag);
	AddCrashProperty(Buffer, CrashGuidTag, (const TCHAR*)CrashGUID);
	EndSection(Buffer, RuntimePropertiesTag);

	// Mark this as an update ("patch") request.
	BeginSection(Buffer, RequestDataTag);
	AddCrashProperty(Buffer, TEXT("Type"), TEXT("PATCH"));
	EndSection(Buffer, RequestDataTag);

	AddFooter(Buffer);

	if (!FFileHelper::SaveStringToFile(Buffer, *Filepath, FFileHelper::EEncodingOptions::AutoDetect))
	{
		UE_LOG(LogWindows, Error, TEXT("Error writing optional attachments XML file %s"), *Filepath);
	}
}
...
void FWindowsPlatformCrashContext::AddPlatformSpecificProperties() const
{
	...
	AddCrashProperty(TEXT("PlatformSupportsWindows11"), FWindowsPlatformMisc::SupportsWindows11());
	AddCrashProperty(TEXT("IsRunningOnBattery"), FPlatformMisc::IsRunningOnBattery());
	AddCrashProperty(TEXT("IsUsingBatterySaver"), FWindowsPlatformMisc::IsUsingBatterySaver());
...
void FWindowsPlatformCrashContext::CopyPlatformSpecificFiles(const TCHAR* OutputDirectory, void* Context)
{
	...
	CopyOptionalAttachments(OutputDirectory);
}
...
bool CreateCrashReportClientPath(TCHAR* OutClientPath, int32 MaxLength)
{
	auto CreateCrashReportClientPathImpl = [&OutClientPath, MaxLength](const TCHAR* CrashReportClientExeName) -> bool
	{
		...
		*OutClientPath = TCHAR('\0');
		FCString::StrncatTruncateDest(OutClientPath, MaxLength, EngineDir);
		FCString::StrncatTruncateDest(OutClientPath, MaxLength, TEXT("Binaries/"));
		FCString::StrncatTruncateDest(OutClientPath, MaxLength, BinariesDir);
		FCString::StrncatTruncateDest(OutClientPath, MaxLength, TEXT("/"));
		FCString::StrncatTruncateDest(OutClientPath, MaxLength, CrashReportClientExeName);
...
FProcHandle LaunchCrashReportClient(void** OutWritePipe, void** OutReadPipe, uint32* OutCrashReportClientProcessId)
{
	...
	*OutReadPipe = PipeChildOutRead;
...

	{
		...
		FCString::StrncatTruncateDest(CrashReporterClientArgs, CR_CLIENT_MAX_ARGS_LEN, PidStr);
	}

	{
		...
		FCString::StrncatTruncateDest(CrashReporterClientArgs, CR_CLIENT_MAX_ARGS_LEN, AnalyticsSummaryGuid);
	}

#if DISABLE_CRC_SUBMIT_AND_RESTART_BUTTON
	// Prevent showing the button if the application cannot be restarted.
	FCString::StrncatTruncateDest(CrashReporterClientArgs, CR_CLIENT_MAX_ARGS_LEN, TEXT(" -HideSubmitAndRestart"));
#endif
	...
	// FCommandline has not yet been initialized, instead we need to use the OS provided methods.
	{
		...
		if (ArgList != nullptr)
		{
			for (int It = 0; It < ArgCount; ++It)
			{
				...
				{
					FCString::StrncatTruncateDest(CrashReporterClientArgs, CR_CLIENT_MAX_ARGS_LEN, TEXT(" -abslog="));
					FCString::StrncatTruncateDest(CrashReporterClientArgs, CR_CLIENT_MAX_ARGS_LEN, Path);
				}
			...
			#endif
				{
					FCString::StrncatTruncateDest(CrashReporterClientArgs, CR_CLIENT_MAX_ARGS_LEN, TEXT(" -nullrhi"));
				}

				// Pass through any unattended flag
				if (FParse::Param(ArgList[It], TEXT("unattended")))
				{
					FCString::StrncatTruncateDest(CrashReporterClientArgs, CR_CLIENT_MAX_ARGS_LEN, TEXT(" -unattended"));
				}

				// Whether to clean up crash reports after send
				if (FParse::Param(ArgList[It], TEXT("CleanCrashReports")))
				{
					FCString::StrncatTruncateDest(CrashReporterClientArgs, CR_CLIENT_MAX_ARGS_LEN, TEXT(" -CleanCrashReports"));
				}
			}
		}
		...
		{
			FCString::StrncatTruncateDest(CrashReporterClientArgs, CR_CLIENT_MAX_ARGS_LEN, TEXT(" -ExpectLowIntegrityLevel"));
		}
	}
...
#if WITH_EDITOR // Disaster recovery is only enabled for the Editor. Start the server even if in -game, -server, commandlet, the client-side will not connect (its too soon here to query this executable config).
	{
		...
		FCString::StrncatTruncateDest(CrashReporterClientArgs, CR_CLIENT_MAX_ARGS_LEN, *DisasterRecoveryServiceCommandLine);
	}
#endif
...
// background with the only purpose to capture the monitored process exit code and send the analytic summary event once the process died.
		...
		if (bGenerateRuntimeCallstack)
		{
			...
			ANSICHAR* StackTrace = (ANSICHAR*)UE::Private::GMalloc->Malloc(StackTraceSize);
			StackTrace[0] = 0;
...
#if DISABLE_DEBUG_INFO_DUE_TO_ASAN
			FCStringAnsi::Snprintf(StackTrace, StackTraceSize, "(callstack disabled due to ASAN)");
#else
			...
			UE::Private::GMalloc->Free(StackTrace);
		}
