...
TTransform<T> TTransform<T>::GetRelativeTransformReverse(const TTransform<T>& Other) const
{
	...
	// Scale = S(B) / S(A)
	// Rotation = Q(B) * Q(A)(-1)
	// Translation = T(B) - [Q(B) * Q(A)(-1) * S(B)/S(A) * T(A) * Q(A) * Q(B)(-1)]
	...
	// RotatedScaledTranslation
	TransformVectorRegister VRS = VectorQuaternionRotateVector(VRotation, VectorMultiply(VScale3D, Translation));

	// Translation = T(B) - [Q(B) * Q(A)(-1) * S(B)/S(A) * T(A) * Q(A) * Q(B)(-1)]
	TransformVectorRegister VTranslation = VectorSet_W0(VectorSubtract(Other.Translation, VRS));
