...
#if UE_ENABLE_ICU
			{
				...
				// Scoping to allow all locals to leave scope after we serialize at the end
				{
					...
					// Swap to French-Canadian to check if rebuilding works (verify each numerical component)
					{
						...
						(void)FormattedTestLayer2.ToString();
...
static void RunTestCasesForSystem_Core_Misc_TextStringification()
{
	FInternationalization& I18N = FInternationalization::Get();
...
#define TEST_EX(Text, Str, ImportCpp)         DoSingleTest(Text, TEXT(Str), TEXT(#Text), ImportCpp)
...
#if UE_ENABLE_ICU
	#define TEST_ICU(Text, Str)               DoSingleTest(Text, TEXT(Str), TEXT(#Text), true)
	#define TEST_EX_ICU(Text, Str, ImportCpp) DoSingleTest(Text, TEXT(Str), TEXT(#Text), ImportCpp)
#else
	#define TEST_ICU(Text, Str)              
	#define TEST_EX_ICU(Text, Str, ImportCpp)
#endif
	...
	TEST_ICU(LOCGEN_NUMBER(-10, "en"), "LOCGEN_NUMBER(-10, \"en\")");
	...
	TEST_ICU(LOCGEN_PERCENT(0.1, "en"), "LOCGEN_PERCENT(0.100000, \"en\")");

	TEST(LOCGEN_CURRENCY(125, "USD", ""), "LOCGEN_CURRENCY(125, \"USD\", \"\")");
	TEST_EX_ICU(FText::AsCurrency(1.25f, TEXT("USD"), nullptr, FInternationalization::Get().GetCulture(TEXT("en"))), "LOCGEN_CURRENCY(125, \"USD\", \"en\")", false);

	TEST_ICU(LOCGEN_DATE_UTC(1526342400, EDateTimeStyle::Short, "", "en-GB"), "LOCGEN_DATE_UTC(1526342400, EDateTimeStyle::Short, \"\", \"en-GB\")");
	TEST(LOCGEN_DATE_LOCAL(1526342400, EDateTimeStyle::Medium, ""), "LOCGEN_DATE_LOCAL(1526342400, EDateTimeStyle::Medium, \"\")");

	TEST_ICU(LOCGEN_TIME_UTC(1526342400, EDateTimeStyle::Long, "", "en-GB"), "LOCGEN_TIME_UTC(1526342400, EDateTimeStyle::Long, \"\", \"en-GB\")");
	TEST(LOCGEN_TIME_LOCAL(1526342400, EDateTimeStyle::Full, ""), "LOCGEN_TIME_LOCAL(1526342400, EDateTimeStyle::Full, \"\")");

	TEST_ICU(LOCGEN_DATETIME_UTC(1526342400, EDateTimeStyle::Short, EDateTimeStyle::Medium, "", "en-GB"), "LOCGEN_DATETIME_UTC(1526342400, EDateTimeStyle::Short, EDateTimeStyle::Medium, \"\", \"en-GB\")");
	...
	TEST_ICU(LOCGEN_DATETIME_CUSTOM_UTC(1526342400, "%A, %B %e, %Y", "", "en-GB"), "LOCGEN_DATETIME_CUSTOM_UTC(1526342400, \"%A, %B %e, %Y\", \"\", \"en-GB\")");
...
{
	RunTestCasesForSystem_Core_Misc_TextStringification();
}

static void RunTestCasesForSystem_Core_Misc_TextFormatArgModifiers()
{
...
{
	RunTestCasesForSystem_Core_Misc_TextFormatArgModifiers();
}
...
IMPLEMENT_SIMPLE_AUTOMATION_TEST(FAutoRTFMTextTests, "AutoRTFM + FText", \
...
bool FAutoRTFMTextTests::RunTest(const FString& Parameters)
{
	if (!AutoRTFM::ForTheRuntime::IsAutoRTFMRuntimeEnabled())
	{
		ExecutionInfo.AddEvent(FAutomationEvent(EAutomationEventType::Info, TEXT("SKIPPED 'FAutoRTFMTextTests' test. AutoRTFM disabled.")));
		return true;
	}

	// Verify that FText works properly when used in an AutoRTFM transaction, both committing and aborting.
	AutoRTFM::Commit([]
	{
		FTextNumericParsingTestClass Instance;
		Instance.TextNumericParsingTest();
