...
TEST_CASE_NAMED(FTVariantGetDefaultTest, "System::Core::Misc::TVariant::GetDefault", "[ApplicationContextMask][EngineFilter]")
{
	// TVariant returns provided default when requested (valid) type isn't the one actually stored
	{
		const TVariant<int, float, double> ConstVar{ TInPlaceType<float>(), 20.0f }; // Set float
		check(!ConstVar.IsType<int>()); // Not int

		static constexpr int DefaultInt = 55;
		check(ConstVar.Get<int>(DefaultInt) == DefaultInt); // Get int, should return default value
	}
	...
	// TVariant returns stored value when requested type is the one actually stored
	{
		static constexpr int StoredInt = 15;
		const TVariant<int, float, double> ConstVar{ TInPlaceType<int>(), StoredInt }; // Set int
		check(ConstVar.IsType<int>()); // Is int

		static constexpr int DefaultInt = 55;
		check(ConstVar.Get<int>(DefaultInt) == StoredInt); // Get int, should return stored value, not the provided default value
	}
