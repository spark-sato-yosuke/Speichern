...
namespace UE { namespace TasksTests
{
	...
	// StressTest RestartWorkers while having task performing oversubscription
	TEST_CASE_NAMED(FTaskGraphOversubscribeDuringRestartWorkers, "System::Core::Async::Tasks::RestartWorkersAndOversubscription", "[.][ApplicationContextMask][EngineFilter]")
	{
		for (int32 Index = 0; Index < 10000; ++Index)
		{
			FTask Task = Launch(TEXT("Task"),
				[&]()
				{
			...
			// This reproed very reliably before the fix.
	...
	// StressTest RestartWorkers while having a thread trying to launch tasks on workers
	TEST_CASE_NAMED(FTaskGraphLaunchFromThreadDuringRestartWorkers, "System::Core::Async::Tasks::RestartWorkersAndExternalThreads", "[.][ApplicationContextMask][EngineFilter]")
	{
	...
	TEST_CASE_NAMED(FAsymmetricThreadFence, "System::Core::Async::AsymmetricThreadFence", "[.][ApplicationContextMask][EngineFilter]")
	{
		for (int32 Index = 0; Index < 10000; ++Index)
		{
			std::atomic<int> A = 0;
			...
			// If you use AsymmetricThreadFenceLight in both threads, you'll see you can end up with a race
			// because nothing publishes the A value before reading the B value. Using an heavy fence
			// on one side ensures that the test can't fail even when one side is only using relaxed operations
			// along with a simple compiler barrier.
			UE::Tasks::FTask T1 =
				UE::Tasks::Launch(
					TEXT("T1"),
					[&A, &B, &R0]()
					{
						...
						R0.store(B.load(std::memory_order_relaxed), std::memory_order_relaxed);
					}
				);

			B.store(1, std::memory_order_relaxed);
			FPlatformMisc::AsymmetricThreadFenceLight();
			R1.store(A.load(std::memory_order_relaxed), std::memory_order_relaxed);

			T1.Wait();

			// Both value should never be 0 but it can happen when no synchronization is used due to the presence of a store buffer.
			verify(R1 == 1 || R0 == 1);
		}
	}
#endif
