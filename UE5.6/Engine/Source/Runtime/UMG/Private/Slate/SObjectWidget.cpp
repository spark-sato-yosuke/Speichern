...
SObjectWidget::SObjectWidget() = default;
...
FReply SObjectWidget::OnTouchFirstMove(const FGeometry& MyGeometry, const FPointerEvent& InTouchEvent)
{
	if (CanRouteEvent())
	{
		return WidgetObject->NativeOnTouchFirstMove(MyGeometry, InTouchEvent);
	}
	return FReply::Unhandled();
}
