...
void UWidgetComponent::CollectPSOPrecacheData(const FPSOPrecacheParams& BasePrecachePSOParams, FMaterialInterfacePSOPrecacheParamsList& OutParams)
{
	if (MaterialInstance)
	{
		...
		{
			FVertexDeclarationElementList VtxElements;
			//Position Float3, 2x PackedNormal, 1x UVs Float2, Color, InstancingID
			VtxElements.Add(FVertexElement(0, 0, VET_Float3, 0, sizeof(FVector3f)));
			VtxElements.Add(FVertexElement(1, 0, VET_PackedNormal, 1, sizeof(FPackedNormal) * 2));
			VtxElements.Add(FVertexElement(1, sizeof(FPackedNormal), VET_PackedNormal, 2, sizeof(FPackedNormal) * 2));
			VtxElements.Add(FVertexElement(2, 0, VET_Float2, 3, sizeof(FVector2f)));
			VtxElements.Add(FVertexElement(3, 0, VET_Color, 4, sizeof(FColor)));
			VtxElements.Add(FVertexElement(4, 0, VET_UInt, 13, sizeof(uint32), true));
			ComponentParams.VertexFactoryDataList.Add(FPSOPrecacheVertexFactoryData(&FLocalVertexFactory::StaticType, VtxElements));
		}
	}
}
...
void UWidgetComponent::UpdateMaterialInstance()
{
	...
	OnMaterialInstanceUpdated.Broadcast();
...
FCollisionShape UWidgetComponent::GetCollisionShape(float Inflation) const
{
	if ( Space != EWidgetSpace::Screen )
	{
		const FVector BoxHalfExtent = ( FVector(0.01f, CurrentDrawSize.X * 0.5f, CurrentDrawSize.Y * 0.5f) * GetComponentTransform().GetScale3D() );
...
{
	// Don't do any work if Slate is not initialized
	if (FSlateApplication::IsInitialized() && IsValidChecked(this))
	{
		if (Space == EWidgetSpace::World)
		{
