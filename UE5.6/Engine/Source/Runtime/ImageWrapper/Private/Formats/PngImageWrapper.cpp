...
public:
	...
	TArray<png_text> MetadataKeyValuePairs;
	TArray<FUtf8String> MetadataStrings;
...
void FPngImageWrapper::Compress(int32 Quality)
{
	...
	if (!CompressedData.Num())
	{
...

		{
			...
			PNGGuard.MetadataKeyValuePairs.Reserve(TextBlocks.Num());
			PNGGuard.MetadataStrings.Reserve(TextBlocks.Num() * 2);

			for (auto& Block : TextBlocks)
			{
				png_text info {};
				info.compression = PNG_TEXT_COMPRESSION_NONE;

				PNGGuard.MetadataStrings.Emplace((*Block.Key));
				info.key = (char*)*PNGGuard.MetadataStrings.Last();

				PNGGuard.MetadataStrings.Emplace((*Block.Value));
				info.text = (char*)*PNGGuard.MetadataStrings.Last();

				PNGGuard.MetadataKeyValuePairs.Add(info);
			}

			png_set_text(png_ptr, info_ptr, PNGGuard.MetadataKeyValuePairs.GetData(), PNGGuard.MetadataKeyValuePairs.Num());
...
void FPngImageWrapper::UncompressPNGData(const ERGBFormat InFormat, const int32 InBitDepth)
{
	...
	// Check if LibPNG can handle the size
	if (Height > PNG_UINT_32_MAX / sizeof(png_bytep))
	{
		RawData.Empty();
		UE_LOG(LogImageWrapper, Error, TEXT("PNG image too high to process with libPNG"));
		return;
	}
...
#endif
	{
		...
		// Anything allocated on the stack after this point will not be destructed correctly in the case of an error
		{
			...
			{
				png_set_rgb_to_gray_fixed(png_ptr, PNG_ERROR_ACTION_WARN, -1, -1);
			}
...
bool FPngImageWrapper::SupportsMetadata() const
{
	return true;
}

void FPngImageWrapper::AddMetadata(const FString& InKey, const FString& InValue)
{
	TextBlocks.Add(InKey, InValue);
}

bool FPngImageWrapper::TryGetMetadata(const FString& InKey, FString& OutValue) const
{
	if (const FString* Value = TextBlocks.Find(InKey))
	{
		OutValue = *Value;
		return true;
	}
...
void FPngImageWrapper::user_read_compressed(png_structp png_ptr, png_bytep data, png_size_t length)
{
	...
	{
		UE_LOG(LogImageWrapper, Warning, TEXT("Bad PNG read position: offset %" INT64_FMT " num %lld length: %llu"), ctx->ReadOffset, ctx->CompressedData.Num(), length);
...
void FPngImageWrapper::user_warning_fn(png_structp png_ptr, png_const_charp warning_msg)
{
	...
	// libPNG 1.6+ has some very spammy warnings about incorrect color profile metadata that is unfortunately
	// extremely common in the wild, so manually detect and downgrade the verbosity of that
	if (FPlatformString::Strncmp(warning_msg, "iCCP:", 5) == 0)
	{
		UE_LOG(LogImageWrapper, Verbose, TEXT("PNG Warning(%s) %s"), LocalDebugImageName != nullptr ? LocalDebugImageName : TEXT(""), ANSI_TO_TCHAR(warning_msg));
	}
	else
	{
