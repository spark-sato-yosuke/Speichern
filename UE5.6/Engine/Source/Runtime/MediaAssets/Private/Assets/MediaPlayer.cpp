...
FLazyName UMediaPlayer::MediaInfoNameKeyframeInterval(TEXT("KeyframeInterval"));
...
UMediaTimeStampInfo* UMediaPlayer::GetTimeStamp() const
{
	...
	if (TimeStampInfo)
	{
		...
		TimeStampInfo->SequenceIndex = TimeStamp.GetIndexValue();
	}
...
{
	if (PlayerFacade->IsSeeking())
	{
		UE_LOG(LogMediaAssets, VeryVerbose, TEXT("%s.Scrub %s"), *GetFName().ToString(), *Time.ToString(TEXT("%h:%m:%s.%t")));
		PlayerFacade->SetNextSeek(Time);
		return true;	// todo: add a failed seek event?
	}
	return Seek(Time);
}
...
void UMediaPlayer::HandlePlayerMediaEvent(EMediaEvent Event)
{
	...
	case EMediaEvent::MediaBuffering:
		OnBufferingStart.Broadcast();
		break;

	case EMediaEvent::MediaBufferingComplete:
		OnBufferingCompleted.Broadcast();
		break;
	}
}
