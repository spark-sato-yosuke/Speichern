...
{
	SetMediaPlayer(NewMediaPlayer, /*SampleQueue*/ nullptr);
}

void UMediaTexture::SetMediaPlayer(UMediaPlayer* NewMediaPlayer, TSharedPtr<FMediaTextureSampleQueue> InSampleQueue)
{
	CurrentPlayer = NewMediaPlayer;
	UpdatePlayerAndQueue(MoveTemp(InSampleQueue));
}
...
FTextureResource* UMediaTexture::CreateResource()
{
	...
	const uint8 MaxNumMips = static_cast<uint8>(GMaxTextureMipCount);
	return new FMediaTextureResource(*this, Dimensions, Size, ClearColor, CurrentGuid.IsValid() ? CurrentGuid : DefaultGuid, EnableGenMips, MaxNumMips, ColorspaceOverride);
}
...
{
	using namespace UE::MediaTexture::Private;
		...
		if (CurrentPlayerPtr->GetPlayerFacade()->GetPlayer().IsValid())
		{
			if (CurrentPlayerPtr->GetPlayerFacade()->GetPlayer()->GetPlayerFeatureFlag(IMediaPlayer::EFeatureFlag::UsePlaybackTimingV2))
			{
				...
				*/
				...
				TextureNumMips = static_cast<int32>(CalcTextureNumMips(*Sample, EnableGenMips));
				bIsSampleValid = true;
			}
			else
			{
...
void UMediaTexture::UpdatePlayerAndQueue(TSharedPtr<FMediaTextureSampleQueue> InSampleQueue)
{
	if (UMediaPlayer* CurrentPlayerPtr = CurrentPlayer.Get())
	{
		...
		if (CurrentGuid != PlayerGuid)
		{
			...
			SampleQueue = InSampleQueue ? MoveTemp(InSampleQueue) : MakeShared<FMediaTextureSampleQueue, ESPMode::ThreadSafe>();
