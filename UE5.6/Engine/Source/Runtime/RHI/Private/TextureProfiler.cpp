...
{
	LLM_SCOPE_BYNAME("Debug/TextureProfiler");
...
const char* FTextureProfiler::GetTextureNameString(FName TextureName)
{
	...
	if (TextureNameString == nullptr)
	{
		if(TextureName.GetDisplayNameEntry()->IsWide())
		{
			...
			FCStringAnsi::Strncpy(const_cast<char*>(TextureNameString), ConvertedTextureNameString.Get(), Length + 1);
		}
		else
		{
			...
			TextureNameString = new char[Length + 1];
			FCStringAnsi::Strncpy(const_cast<char*>(TextureNameString), NewTextureNameString, Length + 1);
		}
	}
