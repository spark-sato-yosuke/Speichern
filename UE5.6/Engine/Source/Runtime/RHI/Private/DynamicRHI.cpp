...
void RHIInit(bool bHasEditorToken)
{
	...
	// Set CSV profiler metadata so every capture can see this
	// GDynamicRHI is checked above
	CSV_METADATA(TEXT("VerbatimRHIName"), GDynamicRHI->GetName());
	// there's already an unfortunate precedent to concoct RHIName for PCD3D_ES31 and such platforms
	CSV_METADATA(TEXT("RHIName"), GMaxRHIFeatureLevel == ERHIFeatureLevel::ES3_1 ? *(FString(GDynamicRHI->GetName()) + TEXT("_ES31")) : GDynamicRHI->GetName());
	CSV_METADATA(TEXT("RHIFeatureLevel"), *LexToString(GMaxRHIFeatureLevel));
	CSV_METADATA(TEXT("ShaderPlatform"), *LexToString(GetFeatureLevelShaderPlatform(GMaxRHIFeatureLevel)));
}
...
FRHIPooledRenderQuery FDefaultRHIRenderQueryPool::AllocateQuery()
{
	...
	UE::TScopeLock Lock(QueriesMutex);
	if (Queries.Num() > 0)
	{
...
FRHITransition* RHICreateTransition(const FRHITransitionCreateInfo& CreateInfo)
{
	...
	FRHITransition* Transition = new (FRHICmdListBaseLinearAllocator::Malloc(FRHITransition::GetTotalAllocationSize(), (uint32)FRHITransition::GetAlignment())) FRHITransition(CreateInfo.SrcPipelines, CreateInfo.DstPipelines, CreateInfo.Flags);
