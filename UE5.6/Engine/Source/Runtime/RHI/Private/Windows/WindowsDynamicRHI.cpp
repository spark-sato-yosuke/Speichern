...
// Whether to default to Performance Mode on low-end machines (D3D11 RHI, ES31 feature level).
// Looks at a few configuration options that control thresholds to enable this as the default.
static bool ShouldDefaultToPerformanceMode()
{
	static TOptional<bool> ForceD3D11_ES31;
	if (ForceD3D11_ES31.IsSet())
	{
		return ForceD3D11_ES31.GetValue();
	}
	...
	for (const FString& DeviceDefaultRHIString : DeviceDefaultRHIList)
	{
		...
		{
			UE_LOG(LogRHI, Log, TEXT("Found RHIName %s with DeviceName %s: default feature level to %s"), *RHIName, *DeviceName, GetLogName(ERHIFeatureLevel::ES3_1));
		...
		{
			UE_LOG(LogRHI, Log, TEXT("Found RHIName %s with DeviceName %s, VendorId %s, and DeviceId %s: default feature level to %s"), *RHIName, *DeviceName, *VendorId, *DeviceId, GetLogName(ERHIFeatureLevel::ES3_1));
...
static TOptional<ERHIFeatureLevel::Type> GetPreferredFeatureLevel(EWindowsRHI ChosenRHI, const FParsedWindowsDynamicRHIConfig& Config)
{
	...
	if (!GIsEditor)
	{
		...
		else if (Config.IsFeatureLevelTargeted(ChosenRHI, ERHIFeatureLevel::ES3_1))
		{
			...
			{
				bDefaultES31 = ShouldDefaultToPerformanceMode();
			}
...
//  * Performance mode (D3D11) if the machine is low-end (see ShouldDefaultToPerformanceMode) and if we are not running the editor;
...
{
	if (!GIsEditor && !CVarIgnorePerformanceModeCheck.GetValueOnAnyThread())
	{
		bool bDefaultD3D11PerfMode = ShouldDefaultToPerformanceMode();
		if (bDefaultD3D11PerfMode)
		{
