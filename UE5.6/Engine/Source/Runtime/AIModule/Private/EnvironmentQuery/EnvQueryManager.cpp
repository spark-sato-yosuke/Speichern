...
int32 UEnvQueryManager::RunQuery(const TSharedPtr<FEnvQueryInstance>& QueryInstance, FQueryFinishedSignature const& FinishDelegate)
{
	...
	UE_VLOG_UELOG(QueryInstance->Owner.Get(), LogEQS, Verbose, TEXT("%hs: Query: %s - Owner: %s"),
		__FUNCTION__,
...
TSharedPtr<FEnvQueryInstance> UEnvQueryManager::PrepareQueryInstance(const FEnvQueryRequest& Request, EEnvQueryRunMode::Type RunMode)
{
	...
	if (!QueryInstance.IsValid())
	{
		UE_VLOG_UELOG(Request.Owner.Get(), LogEQS, Warning, TEXT("Error creating query instance for QueryTemplate: %s - Owner: %s"),
			...
			&& (QueriesFinishedDuringUpdate + NumRunningQueriesAbortedSinceLastUpdate + NumQueriesRunningAsync < NumRunningQueries))
		{
			...
			else
			{
				...
				if (QueryInstancePtr->IsFinished())
				{
					...
					{
						UE_VLOG_UELOG(QueryInstance->Owner.Get(), LogEQS, Warning, TEXT("Query %s (Owner: %s) has finished in %.2f ms, exceeding the configured limit of %.2f ms. Execution details: %s"), 
					...
					// the time spent for time-slicing purposes, because that's NOT the EQS manager doing work.
					{
						...
						UE_VLOG_UELOG(QueryInstance->Owner.Get(), LogEQS, Verbose, TEXT("%hs: Finished Query: %s - Owner: %s"),
							__FUNCTION__,
							*QueryInstance->QueryName,
							*GetNameSafe(QueryInstance->Owner.Get()));
						...
						if (ResultHandlingDuration > HandlingResultTimeWarningSeconds)
						{
							...
							UE_VLOG_UELOG(QueryInstance->Owner.Get(), LogEQS, Warning,
				...
				{
					UE_VLOG_UELOG(QueryInstance->Owner.Get(), LogEQS, Warning, TEXT("Query %s (Owner: %s) has taken %.2f ms so far, exceeding the configured limit of %.2f ms. Execution details: %s"), 
						*QueryInstancePtr->QueryName, *GetNameSafe(QueryInstancePtr->Owner.Get()), 
						1000.f * QueryInstancePtr->TotalExecutionTime, 1000.f * ExecutionTimeWarningSeconds, 
						*QueryInstancePtr->GetExecutionTimeDescription());
...
void UEnvQueryManager::LogQueryInfo(bool bDisplayThresholdWarning) const
{
	...
	{
		UE_VLOG_UELOG(this, LogEQS, Warning, TEXT("The number of EQS queries (%d) has reached the warning threshold (%d).  Logging queries."), RunningQueries.Num(), QueryCountWarningThreshold);
	}
	else
	{
		UE_VLOG_UELOG(this, LogEQS, Warning, TEXT("The number of EQS queries is (%d).  Logging queries."), RunningQueries.Num());
	}
	...
	for (const TSharedPtr<FEnvQueryInstance>& RunningQuery : RunningQueries)
	{
		...
		{
			UE_VLOG_UELOG(this, LogEQS, Warning, TEXT("Query: %s - Owner: %s - %s"),
		...
		{
			UE_VLOG_UELOG(this, LogEQS, Warning, TEXT("Invalid query found in list!"));
		}
	}
}
#endif
...
void UEnvQueryManager::OnWorldCleanup()
{
	if (RunningQueries.Num() > 0)
	{
		...
		for (int32 Index = 0; Index < RunningQueriesCopy.Num(); Index++)
		{
			...
			if (QueryInstance->IsFinished() == false)
			{
				UE_VLOG_UELOG(QueryInstance->Owner.Get(), LogEQS, Verbose, TEXT("Query failed due to world cleanup: Query: %s - Owner: %s"),
					*QueryInstance->QueryName,
					*GetNameSafe(QueryInstance->Owner.Get()));
...
TSharedPtr<FEnvQueryInstance> UEnvQueryManager::CreateQueryInstance(const UEnvQuery* Template, EEnvQueryRunMode::Type RunMode)
{
	...
	{
		UE_CVLOG_UELOG(Template != nullptr && Template->Options.Num() == 0, this, LogEQS, Warning, TEXT("Query [%s] doesn't have any valid options!"), *Template->GetName());
		return nullptr;
	}
	...
	if (InstanceTemplate == NULL)
	{
		...
		const FString NewInstanceName = RunModeEnum
			? FString::Printf(TEXT("%s_%s"), *MakeUniqueObjectName(this, Template->GetClass(), Template->GetFName()).ToString(), *RunModeEnum->GetNameStringByValue(RunMode))
			: FString::Printf(TEXT("%s_%d"), *MakeUniqueObjectName(this, Template->GetClass(), Template->GetFName()).ToString(), uint8(RunMode));
		...
		for (int32 OptionIndex = 0; OptionIndex < LocalTemplate->Options.Num(); ++OptionIndex, ++SourceOptionIndex)
		{
				...
				MyOption->Generator->IsValidGenerator() == false)
			{
				if (MyOption == nullptr)
				{
					UE_VLOG_UELOG(this, LogEQS, Error, TEXT("Trying to spawn a query with broken Template (null option): %s, option %d"),
						*GetNameSafe(LocalTemplate), OptionIndex);
				} 
				else if (MyOption->Generator == nullptr)
				{
					UE_VLOG_UELOG(this, LogEQS, Error, TEXT("Trying to spawn a query with broken Template (generator:MISSING): %s, option %d"),
						*GetNameSafe(LocalTemplate), OptionIndex);
				}
				else
				{
					UE_VLOG_UELOG(this, LogEQS, Error, TEXT("Trying to spawn a query with broken Template (generator:%s itemType:%s): %s, option %d"),
			...
			for (int32 TestIndex = SortedTests.Num() - 1; TestIndex >= 0; TestIndex--)
			{
				...
				{
					UE_VLOG_UELOG(this, LogEQS, Warning, TEXT("Query [%s] can't use test [%s] in option %d [%s], removing it"),
...
{
	UE_VLOG_UELOG(this, LogEQS, Log, TEXT("Applying new FEnvQueryManagerConfig: %s"), *NewConfig.ToString());
...
void FEQSDebugger::StoreQuery(const TSharedPtr<FEnvQueryInstance>& QueryInstance)
{
	...
	for (TMap<FObjectKey, TArray<FEnvQueryInfo>>::TIterator It(StoredQueries); It; ++It)
	{
