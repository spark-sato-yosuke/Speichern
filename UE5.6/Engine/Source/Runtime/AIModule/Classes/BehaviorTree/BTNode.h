...
class UBTNode : public UObject, public IGameplayTaskOwnerInterface
{
	...
	AIMODULE_API void PreSave(FObjectPreSaveContext SaveContext) override;

	/** @return the external asset associated with this node that should be open on double click. If set, DebugContext contains the info of the node in the current debug step. */
	AIMODULE_API virtual const UObject* GetAssociatedAsset(TOptional<FBehaviorTreeNodeDebugContext> DebugContext) const;
#endif
