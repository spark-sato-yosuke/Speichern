...
protected:
	...
	UE_DEPRECATED(5.6, "RefreshStimulus is deprecated. Use ConditionallyStoreSuccessfulStimulus() instead.")
	AIMODULE_API virtual void RefreshStimulus(FAIStimulus& StimulusStore, const FAIStimulus& NewStimulus) final;

	/**
	 * Updates the stimulus entry in StimulusStore, if NewStimulus is more recent or stronger
	 * @return whether the detected change in stimulus values should be considered a "perception change"
	 *		(and, in consequence, emit appropriate notifications) - defaults to `false`
	 */
	AIMODULE_API virtual bool ConditionallyStoreSuccessfulStimulus(FAIStimulus& StimulusStore, const FAIStimulus& NewStimulus);
