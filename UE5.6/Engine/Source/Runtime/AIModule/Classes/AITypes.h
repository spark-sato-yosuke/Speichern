...
namespace FAISystem
{
	extern AIMODULE_API const FRotator InvalidRotation;
	extern AIMODULE_API const FQuat InvalidOrientation;
	inline constexpr FVector InvalidLocation = FVector(FLT_MAX, UE::Math::TVectorConstInit{});
	inline constexpr FVector InvalidDirection = FVector(0, UE::Math::TVectorConstInit{});
...
{
	constexpr bool StopOnOverlap = true;
	constexpr bool UsePathfinding = true;
	constexpr bool IgnorePathfinding = false;
}
...
protected:
	...
	explicit FAIGenericID(typename TCounter::Type InIndex)
		: Index(InIndex)
	{}
...
public:
	...
	UE_DEPRECATED(5.6, "AIGenericId is not convertible to its base type anymore.")
	operator typename TCounter::Type() const { return Index; }

	bool IsValid() const { return Index != InvalidID().Index; }

	bool operator==(const FAIGenericID Other) const { return Index == Other.Index; }
	bool operator!=(const FAIGenericID Other) const { return Index != Other.Index; }
...
private:
	static constexpr uint32 AnyRequestID = 0;
	static constexpr uint32 InvalidRequestID = uint32(-1);
...
struct FAIMoveRequest
{
	...
	const bool ShouldStartFromPreviousPath() const { return bStartFromPreviousPath; }
