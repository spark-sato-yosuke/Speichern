...
public:
	 ...
	 *	@remark This value is reset if Seek is called directly.
	 */
...
private:
	...
	void ExecuteNextSeek();
	void PrepareSampleQueueForSequenceIndex();
...
private:
	...
	private:
		...
		/** The most recently set time range to block on */
		TRange<FTimespan> CurrentTimeRange;

		/** Last user provided BlockOnRange value */
		mutable TRange<FTimespan> LastProcessedTimeRange;
	...
	/** When multiple consecutive seek are issued while seeking, we keep track of the most recent request. */
	TOptional<FTimespan> NextSeekTime;

	/** The next sequence index value we expect to see on the samples provided by a v2 timing enabled player. */
	TOptional<int32> NextSequenceIndex;
