...
#ifndef UE_WINDOWS_SPLASH_USE_TEXT_OUTLINE
...
void FWindowsPlatformSplash::SetSplashText( const SplashTextType::Type InType, const FText& InText )
{
	...
	if( GSplashScreenThread )
	{
		...
		if( InType == SplashTextType::CopyrightInfo || GIsEditor )
		{
			...
			{
				// Update splash text
				if (!InText.IdenticalTo(GSplashScreenText[InType], ETextIdenticalModeFlags::LexicalCompareInvariants))
				{
