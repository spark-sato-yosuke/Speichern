...
public:
	...
	uint32 ThreadProc()
	{
		...
		SetWindowLongPtr(CommandHwnd, GWLP_USERDATA, (LONG_PTR)this);
		CommandHwndWndProcPtr = (WNDPROC)SetWindowLongPtr(CommandHwnd, GWLP_WNDPROC, (LONG_PTR)StaticCommandHwndWndProc);
		...
		UpdateSize(FromScale(Rect.right), FromScale(Rect.bottom), false);
		...
		while (bLoop)
		{
			if (bWasVisible != bIsVisible)
			{
				if (bIsVisible)
				{
					SetWindowPos(MainHwnd, 0, ConsolePosX, ConsolePosY, ConsoleWidth, ConsoleHeight, SWP_SHOWWINDOW|SWP_NOZORDER|SWP_NOACTIVATE|SWP_NOOWNERZORDER|SWP_FRAMECHANGED);
				}
				else
				{
...

			{
				...
				static LPGetDpiForWindow GetDpiForWindow = (LPGetDpiForWindow)(void*)GetProcAddress(GetModuleHandle(L"user32.dll"), "GetDpiForWindow");
				if (GetDpiForWindow != nullptr)
				{
					uint32 dpi = GetDpiForWindow(MainHwnd);
					static uint32 lastDpi = dpi;
					if (lastDpi != dpi)
					{
						DpiScale = double(dpi) / 96;
						ShowWindow(MainHwnd, SW_HIDE);
						lastDpi = dpi;
						bWasVisible = false;

						ConsolePosX = ToScale(ConsolePosX);
						ConsolePosY = ToScale(ConsolePosY);
						ConsoleWidth = ToScale(ConsoleWidth);
						ConsoleHeight = ToScale(ConsoleHeight);

						SystemParametersInfo(SPI_GETNONCLIENTMETRICS, sizeof(NonClientMetrics), &NonClientMetrics, 0);
						Font = (HFONT)CreateFontIndirect(&NonClientMetrics.lfMessageFont);

						FontHeight = -MulDiv(8, dpi, 72);
						LogFont = (HFONT)CreateFontW(FontHeight, 0, 0, 0, FW_NORMAL, false, false, false, ANSI_CHARSET, OUT_DEFAULT_PRECIS, CLIP_DEFAULT_PRECIS, ANTIALIASED_QUALITY, FIXED_PITCH | FF_MODERN, FontName);
						...
						auto UpdateFont = [](HWND hwnd, HFONT font) { SendMessageW(hwnd, WM_SETFONT, (WPARAM)font, 0); };
						
						UpdateFont(IncludeFilterHwnd, Font);
						UpdateFont(ExcludeFilterHwnd, Font);
						UpdateFont(ClearLogButtonHwnd, Font);
						UpdateFont(CheckpointButtonHwnd, Font);
						UpdateFont(CommandHwnd, Font);
						UpdateFont(RunCommandHwnd, Font);

						for (Activity& A : Activities)
						{
							if (A.NameHwnd)
								UpdateFont(A.NameHwnd, Font);
							if (A.StatusHwnd)
								UpdateFont(A.StatusHwnd, Font);
						}

						UpdateFont(LogHwnd, LogFont);
						SendMessageW(LogHwnd, LB_SETITEMHEIGHT, 0, MAKELPARAM(LogFontHeight, 0));

						WindowBorderPen = CreatePen(PS_INSIDEFRAME, int(ceil(DpiScale)), BorderColor);
					}
				}
			}
	...
	void UpdateSize(int32 ClientWidth, int32 ClientHeight, bool bRedraw)
	{
		...
		SetWindowPosScale(ClearLogButtonHwnd, 0, ButtonStart, Y - 2, 80, 22, FilterFlags);
		SetWindowPosScale(CheckpointButtonHwnd, 0, ButtonStart + 94, Y - 2, 135, 22, FilterFlags);
		...
		SetWindowPosScale(IncludeFilterHwnd, 0, X, Y - 1, WidthForEditBoxes, 20, FilterFlags);
		X += WidthForEditBoxes + 20;
		SetWindowPosScale(ExcludeFilterHwnd, 0, X, Y - 1, WidthForEditBoxes, 20, FilterFlags);

		SetWindowPosScale(LogHwnd, 0, 7, LogTop, ClientWidth - 33, LogHeight, Flags);
		SetWindowPosScale(LogScrollHwnd, 0, ClientWidth - 26, LogTop, 19, LogHeight, Flags);
		SetWindowPosScale(CommandHwnd, 0, 8, CommandY, ClientWidth - ButtonWidth - 20, 20, CommandFlags);
		SetWindowPosScale(RunCommandHwnd, 0, ClientWidth - ButtonWidth - 7, CommandY - 1, ButtonWidth, 22, CommandFlags);
		...
		return TraverseActivityPositions(ClientWidth, ClientHeight, [&](Activity& A, int32 X, int32 Y, int32 Width, int32 RowIndex)
			{
				...
				SetWindowPosScale(A.NameHwnd, 0, X, Y, NameWidth, 18, Flags);
				SetWindowLong(A.NameHwnd, GWLP_USERDATA, RowIndex);
				int32 StatusWidth = FMath::Max(Width - NameWidth, 0);
				SetWindowPosScale(A.StatusHwnd, 0, X + NameWidth, Y, StatusWidth, 18, Flags);
	...
	{
		FConsoleWindow* ThisPtr = (FConsoleWindow*)GetWindowLongPtr(hWnd, GWLP_USERDATA);
		return ThisPtr->CommandHwndWndProc(hWnd, Msg, wParam, lParam);
	}
	...
	{
		int32 ButtonHeight = ClientRect.right + ToScale(1);
	...
	LRESULT ScrollBarWinProc(HWND hWnd, UINT Msg, WPARAM wParam, LPARAM lParam)
	{
		...
		case WM_ERASEBKGND:
		{
			...
			int32 ButtonHeight = Rect.right + ToScale(1);
			...
			POINT vertices[] = { {X - Three, Y + One}, {X, Y - Two}, {X + Three, Y + One} };
			Polygon(hdc, vertices, 3);

			Y = Rect.bottom - Y;
			POINT vertices2[] = { {X - Three, Y - Two}, {X, Y + One}, {X + Three, Y - Two} };
	...
	LRESULT HitTestNCA(HWND hWnd, LPARAM lParam)
	{
		...
		WindowRect = FromScale(WindowRect);
		int MouseX = FromScale((int)LOWORD(lParam));
		int MouseY = FromScale((int)HIWORD(lParam));
		...
		if (MouseY >= WindowRect.top && MouseY < WindowRect.top + 27)
		{
			bOnResizeBorder = (MouseY < (WindowRect.top - FrameRect.top));
			Row = 0;
		}
		else if (MouseY < WindowRect.bottom && MouseY >= WindowRect.bottom - BorderWidth)
			Row = 2;

		if (MouseX >= WindowRect.left && MouseX < WindowRect.left + BorderWidth)
			Col = 0;
		else if (MouseX < WindowRect.right && MouseX >= WindowRect.right - BorderWidth)
			Col = 2;
	...
	LRESULT CommandHwndWndProc(HWND hWnd, UINT Msg, WPARAM wParam, LPARAM lParam)
	{
		...
		case WM_KEYUP:
		{
			if (wParam == VK_UP)
			{
				TCHAR CurrentCommand[1024] = { '\0' };
				GetWindowText(CommandHwnd, CurrentCommand, 1024);

				FString Command;
				Owner->GetLastCommand(Command);

				if (Command == CurrentCommand)
				{
					Owner->GetPreviousCommand(Command);
				}
				SetWindowText(CommandHwnd, *Command);
			}
			else if (wParam == VK_DOWN)
			{
				FString NextCommand;
				Owner->GetNextCommand(NextCommand);
				SetWindowText(CommandHwnd, *NextCommand);
			}
			break;
		}
		}

		return CommandHwndWndProcPtr(hWnd, Msg, wParam, lParam);
	}
	...
	template<typename T>
	T ToScale(T Value) const { return T(Value*DpiScale); }
	template<typename T>
	T FromScale(T Value) const { return T(Value/DpiScale); }

	RECT ToScale(const RECT& Rect)
	{
		return { ToScale(Rect.left), ToScale(Rect.top), ToScale(Rect.right), ToScale(Rect.bottom) };
	}

	RECT FromScale(const RECT& Rect)
	{
		return { FromScale(Rect.left), FromScale(Rect.top), FromScale(Rect.right), FromScale(Rect.bottom) };
	}

	void SetWindowPosScale(HWND hWnd, HWND hWndInsertAfter, int X, int Y, int Cx, int Cy, UINT Flags)
	{
		::SetWindowPos(hWnd, hWndInsertAfter, ToScale(X), ToScale(Y), ToScale(Cx), ToScale(Cy), Flags);
	}
	...
	void DrawNonClientArea(HWND hWnd, WPARAM wParam)
	{
		...
		{
			region = CreateRectRgn(windowRect.left, windowRect.top, windowRect.right, windowRect.bottom);
		}
		else
		{
		...
		RECT rect = FromScale(windowRect);
		...
		FillRect(dc, ToScale(CaptionRect), BackgroundBrush);

		SelectObject(dc, GetStockObject(NULL_BRUSH));
		SelectObject(dc, WindowBorderPen);
		Rectangle(dc, 0, 0, windowRect.right - windowRect.left, windowRect.bottom - windowRect.top);

		DrawIconEx(dc, ToScale(3), ToScale(3), Icon, ToScale(21), ToScale(21), 0, NULL, DI_NORMAL);
		...
		DrawTextW(dc, *ConsoleTitle, ConsoleTitle.Len(), ToScale(CaptionRect), DT_SINGLELINE | DT_VCENTER | DT_END_ELLIPSIS);
		...
		if (NcButtonHot != -1 && (NcButtonDown == -1 || NcButtonHot == NcButtonDown))
		{
			...
			FillRect(dc, ToScale(ButtonRect), Brush);
		}
		...
		if (bConsoleExpanded)
		{
			MoveToScale(dc, MiddleX - 4, MiddleY - 4);
			LineToScale(dc, MiddleX, MiddleY);
			LineToScale(dc, MiddleX + 5, MiddleY - 5);
			MoveToScale(dc, MiddleX - 4, MiddleY);
			LineToScale(dc, MiddleX, MiddleY + 4);
			LineToScale(dc, MiddleX + 5, MiddleY - 1);
		}
		else
		{
			MoveToScale(dc, MiddleX - 4, MiddleY);
		...
		MiddleX += ButtonWidth;
		MoveToScale(dc, MiddleX - 4, MiddleY);
		LineToScale(dc, MiddleX + 5, MiddleY);
		...
		{
			RectangleScale(dc, MiddleX, MiddleY - 3, MiddleX + 8, MiddleY + 5);
			SelectObject(dc, BackgroundBrush);
			RectangleScale(dc, MiddleX - 3, MiddleY, MiddleX + 5, MiddleY + 8);
		}
		else
		{
			RectangleScale(dc, MiddleX - 5, MiddleY - 5, MiddleX + 5, MiddleY + 5);
		}
	...
	LRESULT MainWinProc(HWND hWnd, UINT Msg, WPARAM wParam, LPARAM lParam)
	{
		...
		case WM_SIZE:
		{
			...
			case SIZE_MAXIMIZED:
			case SIZE_RESTORED:
				UpdateSize(FromScale(LOWORD(lParam)), FromScale(HIWORD(lParam)), true);
				RECT WindowRect;
				GetWindowRect(MainHwnd, &WindowRect);
		...
		case WM_DRAWITEM:
		{
			...
			if (pdis->CtlType == ODT_STATIC)
			{
				...
				GetTextRectW(Hdc, Str, StrLen, &ModifiedRect, DT_SINGLELINE | DT_END_ELLIPSIS | DT_CALCRECT);
		...
		case WM_PAINT:
		{
			...
			TraverseActivityPositions(FromScale(Rect.right), FromScale(Rect.bottom), [&](Activity& A, int32 X, int32 Y, int32 Width, int32 RowIndex)
				{
					if (A.Light == 0)
						return;
					...
					RoundRect(Hdc, ToScale(X - CircleSize + XOffset), ToScale(Y + HalfDiff + YOffset), ToScale(X + XOffset), ToScale(Y + FontHeight - HalfDiff + YOffset), ToScale(CircleSize), ToScale(CircleSize));
				});
			EndPaint(hWnd, &ps);
			return 0;
		}
		...
		case WM_LBUTTONUP:
		{
			...
			if (Res == HTEXPAND)
			{
				...
				RECT Rect;
				GetClientRect(MainHwnd, &Rect);
				UpdateSize(FromScale(Rect.right), FromScale(Rect.bottom), true);
		...
		case WM_NCCALCSIZE:
		{
			...
			ncParams->rgrc[0].top += ToScale(27);
			ncParams->rgrc[0].left += ToScale(1);
			ncParams->rgrc[0].bottom -= ToScale(1);
			ncParams->rgrc[0].right -= ToScale(1);
			return 0;
		}
		case WM_NCPAINT:
		{
			DrawNonClientArea(hWnd, wParam);
			return 0;
		}
		case WM_NCACTIVATE:
	...
	, Window(nullptr)
{
	...
	if (FParse::Param(FCommandLine::Get(), TEXT("SuppressConsoleOutputVerboseLogging")))
	{
...
void FWindowsConsoleOutputDevice2::SaveToINI()
{
	...
	int32 ConsoleWidth = Window->FromScale(Window->ConsoleWidth);
	int32 ConsoleHeight = Window->FromScale(Window->ConsoleHeight);
	int32 ConsolePosX = Window->FromScale(Window->ConsolePosX);
	int32 ConsolePosY = Window->FromScale(Window->ConsolePosY);
	...
	// Workaround to file not saving. Now requires file to exist before calling LoadFile to save
	if (!IFileManager::Get().FileExists(*Filename))
	{
		FFileHelper::SaveStringToFile(FStringView(), *Filename);
	}
	...
	Config.SetArray(Selection, TEXT("CommandHistory"), CommandHistory, Filename);
...
void FWindowsConsoleOutputDevice2::Show( bool ShowWindow )
{
	if (ShowWindow)
	{
		...
		Config.GetArray(Selection, TEXT("CommandHistory"), CommandHistory, Filename);
		CommandHistoryCursor = CommandHistory.Num();
		...
		FParse::Value(FCommandLine::Get(), TEXT("ConsoleIncludeFilter="), ConsoleIncludeFilterStr);
...
void FWindowsConsoleOutputDevice2::Serialize( const TCHAR* Data, ELogVerbosity::Type Verbosity, const class FName& Category, const double Time )
{
	...
	if ((Verbosity & ELogVerbosity::VerbosityMask) > AllowedLogVerbosity)
		return;
...
bool FWindowsConsoleOutputDevice2::GetPreviousCommand(FString& OutCommand)
{
	if (CommandHistoryCursor > 0)
	{
		...
		if (CommandHistoryCursor < CommandHistory.Num())
		{
			OutCommand = CommandHistory[CommandHistoryCursor];
			return true;
		}
	}
	return false;
...
{
	if (CommandHistoryCursor < (CommandHistory.Num() - 1))
	{
		CommandHistoryCursor++;
		OutCommand = CommandHistory[CommandHistoryCursor];
		return true;
...
{	
	if (CommandHistoryCursor >= 0 && CommandHistoryCursor < CommandHistory.Num() && CommandHistory[CommandHistoryCursor] == InCommand)
	{
		// The command was probably executed from the command history. No need to store again.
		return;
	}

	CommandHistoryCursor = CommandHistory.Add(InCommand);
	SaveToINI();
}

bool FWindowsConsoleOutputDevice2::GetLastCommand(FString& OutCommand)
{
	if (CommandHistoryCursor >= 0 && CommandHistoryCursor < CommandHistory.Num())
	{
		OutCommand = CommandHistory[CommandHistoryCursor];
		return true;
	}
