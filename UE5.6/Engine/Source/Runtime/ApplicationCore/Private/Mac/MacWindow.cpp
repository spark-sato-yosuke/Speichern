...
bool FMacWindow::IsMinimized() const
{
	SCOPED_AUTORELEASE_POOL;
	// @TODO: UE-227397
	// Currently using cached property to avoid triggering a MainThreadChecker issue.
	return WindowHandle == nullptr || WindowHandle->bMinimized;
}
...
bool FMacWindow::IsPointInWindow( int32 X, int32 Y ) const
{
	...
	if (!WindowHandle->bMinimized)
	{
		...
		if (bScreensHaveSeparateSpaces)
		{
			// @TODO: UE-227397
			// Currently using cached property to avoid triggering a MainThreadChecker issue.
			NSRect ScreenFrame = WindowHandle->CachedScreen.frame;
...
bool FMacWindow::IsForegroundWindow() const
{
	...
	if (WindowHandle)
	{
		return WindowHandle->bKeyWindow;
	}
	return false;
}
...
float FMacWindow::GetDPIScaleFactor() const
{
	// @TODO: UE-227397
	// Currently using cached property to avoid triggering a MainThreadChecker issue.
	if (WindowHandle && FPlatformApplicationMisc::IsHighDPIModeEnabled())
	{
		return (float)WindowHandle->CachedBackingScaleFactor;
	}
	return 1.0f;
}
...
void FMacWindow::UpdateFullScreenState(bool bToggleFullScreen)
{
	...
	}, true, UnrealFullscreenEventMode);
