...
{
#if WITH_EDITOR
	if (!InText.IdenticalTo(SplashText[ InType ], ETextIdenticalModeFlags::LexicalCompareInvariants))
	{
...
* @param	InText		Text to display
*/
void FLinuxPlatformSplash::SetSplashText( const SplashTextType::Type InType, const FText& InText )
{
	if (GSplashState)
	{
		...
		{
			GSplashState->SetSplashText(InType, InText);
		}
		GSplashState->Pump();
	}
}
