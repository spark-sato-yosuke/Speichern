...
bool AndroidEnableMediaPlayerBRIR = true;
static FAutoConsoleVariableRef CVarAndroidEnableMediaPlayerBRIR(
	TEXT("Android.EnableMediaPlayerBRIR"),
	AndroidEnableMediaPlayerBRIR,
	TEXT("Determines how the Android Media Player handles video frame data. True for using ImageReader/HardwareBuffer. False for copying data through the CPU memory. (Default: true)"),
	ECVF_Default);
...
FJavaAndroidMediaPlayer::FJavaAndroidMediaPlayer(bool swizzlePixels, bool vulkanRenderer, bool needTrackInfo)
	...
	, GetVideoLastMediaFrameDataMethod(GetClassMethod("getVideoLastMediaFrameData", "()Lcom/epicgames/unreal/BitmapRenderer$FrameData;"))
{
...
void FJavaAndroidMediaPlayer::SelectBitmapRenderer()
{
	if (BRSelection == BitmapRendererSelection::Unknown)
	{
		if (UsingVulkanRenderer && AndroidEnableMediaPlayerBRIR && FAndroidMisc::IsExternalMemoryAndroidHardwareBufferExtensionLoaded())
		{
...
bool FJavaAndroidMediaPlayer::IsUsingBitmapRendererImageReader()
{
	SelectBitmapRenderer();
	return (BRSelection == BitmapRendererSelection::ImageReader);
}

jobject FJavaAndroidMediaPlayer::GetVideoLastMediaFrameData()
{
	...
	JNIEnv* JEnv = FAndroidApplication::GetJavaEnv();
	jobject localFrameData = JEnv->CallObjectMethod(Object, GetVideoLastMediaFrameDataMethod.Method);
	if (JEnv->ExceptionCheck())
	{
		JEnv->ExceptionDescribe();
		JEnv->ExceptionClear();
	...
	if (localFrameData == nullptr)
	{
		return nullptr;
	}

	jobject globalFrameData = FAndroidApplication::GetJavaEnv()->NewGlobalRef(localFrameData);

	JEnv->DeleteLocalRef(localFrameData);

	return globalFrameData;
}
