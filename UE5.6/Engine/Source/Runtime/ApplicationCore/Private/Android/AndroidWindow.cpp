...
{
	TSharedRef<FAndroidWindow> Window = MakeShareable( new FAndroidWindow() );
	int Num = FAndroidWindowManager::Get().AddWindow(Window);

	if( Num == 1)  // special case for native activity, the first window gets any pre-existing ANative window. if there is not yet a pending window then the event thread will set it later.
	{
		// lock both the FAndroidWindow and the pending window to transfer the pending window across
		FNativeAccessor MyANativeAccessor = Window->GetANativeAccessor(true);
		FAndroidWindowManager::FPendingWindowAccessor PendingAccessor = FAndroidWindowManager::GetPendingWindowAccessor();

		MyANativeAccessor.SetANativeWindow(PendingAccessor.GetANativeWindow());
		PendingAccessor.SetANativeWindow(nullptr);
	}
...
{
	FAndroidWindowManager::Get().RemoveWindow(SharedThis(this));
}
...
FPlatformRect FAndroidWindow::GetScreenRect(bool bUseEventThreadWindow)
{
	...
	if (bComputeRect)
	{
		...
		if (!bIsOculusMobileApp && (GAndroidWindowOverride == nullptr))
		{
