...
#ifndef WITH_IOS_UNIVERSAL_LINKS
...
- (void)InitializeAudioSession
{
	[[NSNotificationCenter defaultCenter] addObserverForName:AVAudioSessionInterruptionNotification object:nil queue:nil usingBlock:^(NSNotification *notification)
	{
		...
		UE_CLOG(InterruptionType == AVAudioSessionInterruptionTypeBegan, LogIOSAudioSession, Log, TEXT("AudioSession interruption notification: Began"));
		UE_CLOG(InterruptionType == AVAudioSessionInterruptionTypeEnded, LogIOSAudioSession, Log, TEXT("AudioSession interruption notification: Ended"));
		...
		[FIOSAsyncTask CreateTaskWithBlock : ^ bool(void)
		{
			...
			switch (InterruptionType)
			{
				case AVAudioSessionInterruptionTypeBegan:
					...
					FCoreDelegates::AudioInterruptionDelegate.Broadcast(true);
					break;
					
				case AVAudioSessionInterruptionTypeEnded:
					FCoreDelegates::AudioInterruptionDelegate.Broadcast(false);
...
- (void)ToggleAudioSession:(bool)bActive
{
		...
		[[AVAudioSession sharedInstance] categoryOptions] != TestOptions)
	{
		...
		if (ActiveError)
		{
			UE_LOG(LogIOSAudioSession, Error, TEXT("Failed to set AVAudioSession category to Category:%s (current %s) Mode:%s (current %s) Options:%x (current %x)! [Error = %s]."), 
				   *FString(Category), *FString([[AVAudioSession sharedInstance] category]), 
				   *FString(Mode), *FString([[AVAudioSession sharedInstance] mode]), 
				   Options, [[AVAudioSession sharedInstance] categoryOptions] & ~AVAudioSessionCategoryOptionMixWithOthers, 
				   *FString([ActiveError description]));
		}
	}
...
{
	return self.PendingActivationProtocol;
}
...
-(BOOL)application:(UIApplication*)application continueUserActivity:(NSUserActivity*)userActivity restorationHandler:(void(^)(NSArray<id<UIUserActivityRestoring>>* _Nullable)) restorationHandler 
{
	...
	{
		self.PendingActivationProtocol = userActivity.webpageURL.absoluteString;

		if ([self.PendingActivationProtocol length] != 0)
		{
			if (!ProtocolActivationTickerHandle.IsValid())
			{
				ProtocolActivationTickerHandle = FTSTicker::GetCoreTicker().AddTicker(FTickerDelegate::CreateLambda(
					[self](float)
					{
						...
						{
							FCoreDelegates::OnActivatedByProtocol.Broadcast(FString([[IOSAppDelegate GetDelegate] PendingActivationProtocol]), PLATFORMUSERID_NONE);

							self.PendingActivationProtocol = @"";
...
#endif // WITH_IOS_UNIVERSAL_LINKS
...
- (UIInterfaceOrientationMask)application:(UIApplication *)application supportedInterfaceOrientationsForWindow:(UIWindow*)window
{
	...
	if (View != nil &&
		(View->SupportedInterfaceOrientations != UIInterfaceOrientationMaskAll) &&
		(View->SupportedInterfaceOrientations & Mask) != View->SupportedInterfaceOrientations)
	{
		// Return all possible allowed orientations, that includes all orientations from IOSRuntimeSettings plus
		// the orientations set at runtime via Blueprint. The iOS View Controller will then restrict to the desired
		// orientation(s) with the "supportedInterfaceOrientations" function.
		return Mask | View->SupportedInterfaceOrientations;
	}
...
#if WITH_ENGINE
	if (FTaskGraphInterface::IsRunning() && self.bEngineInit)
	{
		FGraphEventRef RTBlockTask = FFunctionGraphTask::CreateAndDispatchWhenReady([]()
																					{
...
{
	UE_LOG(LogTemp, Display, TEXT("Received RemoteNotification"));
...
-(void)application:(UIApplication *)application didFailToRegisterForRemoteNotificationsWithError:(NSError *)error
{
	...
	UE_LOG(LogTemp, Warning, TEXT("FAILED to register for remote notificaiton: %s"), *errorDescription);
