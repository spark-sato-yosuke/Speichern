...
uint32 TranslateGCKeyCodeToASCII(GCKeyCode KeyCode)
{
    ...
    if ( KeyCode <= GCKeyCodeNonUSBackslash ) // Only implemented up to '§' GCKeyCode 100
    {
        ...
        else if (KeyCode == GCKeyCodeDeleteOrBackspace) { c = 8;}
        ...
        else if (KeyCode == GCKeyCodeNonUSBackslash) { c = 167;} // Section (§) in FGenericPlatformInput::GetStandardPrintableKeyMap
        
        UE_LOG(LogIOS, Log, TEXT("char: %c - GCKeycode: %d"), TCHAR(c), KeyCode);
	}
...
{
	uint32 c = '?';
	if (KeyCode == GCKeyCodeReturnOrEnter) { c = KEYCODE_ENTER;}
	else if (KeyCode == GCKeyCodeEscape) { c = KEYCODE_ESCAPE;}
	else if (KeyCode == GCKeyCodeDeleteOrBackspace) { c = KEYCODE_BACKSPACE;}
	else if (KeyCode == GCKeyCodeTab) { c = KEYCODE_TAB;}
	else if (KeyCode == GCKeyCodeLeftArrow) { c = KEYCODE_LEFT;}
	else if (KeyCode == GCKeyCodeRightArrow) { c = KEYCODE_RIGHT;}
	else if (KeyCode == GCKeyCodeDownArrow) { c = KEYCODE_DOWN;}
	else if (KeyCode == GCKeyCodeUpArrow) { c = KEYCODE_UP;}
	else if (KeyCode == GCKeyCodeLeftControl) { c = KEYCODE_LEFT_CONTROL;}
	else if (KeyCode == GCKeyCodeLeftShift) { c = KEYCODE_LEFT_SHIFT;}
	else if (KeyCode == GCKeyCodeLeftAlt) { c = KEYCODE_LEFT_ALT;}
	else if (KeyCode == GCKeyCodeCapsLock) {c = KEYCODE_CAPS_LOCK;}
	else if (KeyCode == GCKeyCodeLeftGUI) { c = KEYCODE_LEFT_COMMAND;}
	else if (KeyCode == GCKeyCodeRightControl) { c = KEYCODE_RIGHT_CONTROL;}
	else if (KeyCode == GCKeyCodeRightShift) { c = KEYCODE_RIGHT_SHIFT;}
	else if (KeyCode == GCKeyCodeRightAlt) { c = KEYCODE_RIGHT_ALT;}
	else if (KeyCode == GCKeyCodeRightGUI) { c = KEYCODE_RIGHT_COMMAND;}
	else if (KeyCode == GCKeyCodeF1) { c = KEYCODE_F1;}
	else if (KeyCode == GCKeyCodeF2) { c = KEYCODE_F2;}
	else if (KeyCode == GCKeyCodeF3) { c = KEYCODE_F3;}
	else if (KeyCode == GCKeyCodeF4) { c = KEYCODE_F4;}
	else if (KeyCode == GCKeyCodeF5) { c = KEYCODE_F5;}
	else if (KeyCode == GCKeyCodeF6) { c = KEYCODE_F6;}
	else if (KeyCode == GCKeyCodeF7) { c = KEYCODE_F7;}
	else if (KeyCode == GCKeyCodeF8) { c = KEYCODE_F8;}
	else if (KeyCode == GCKeyCodeF9) { c = KEYCODE_F9;}
	else if (KeyCode == GCKeyCodeF10) { c = KEYCODE_F10;}
	else if (KeyCode == GCKeyCodeF11) { c = KEYCODE_F11;}
	else if (KeyCode == GCKeyCodeF12) { c = KEYCODE_F12;}
	else if (KeyCode == GCKeyCodeF13) { c = KEYCODE_F13;}
	else if (KeyCode == GCKeyCodeF14) { c = KEYCODE_F14;}
	else if (KeyCode == GCKeyCodeF15) { c = KEYCODE_F15;}
	else if (KeyCode == GCKeyCodeF16) { c = KEYCODE_F16;}
	else if (KeyCode == GCKeyCodeF17) { c = KEYCODE_F17;}
	else if (KeyCode == GCKeyCodeF18) { c = KEYCODE_F18;}
	else if (KeyCode == GCKeyCodeF19) { c = KEYCODE_F19;}
	else if (KeyCode == GCKeyCodeF20) { c = KEYCODE_F20;}
	else { c = TranslateGCKeyCodeToASCII(KeyCode); }

	UE_LOG(LogIOS, Log, TEXT("KeyCode: %d - GCKeycode: %d"), c, KeyCode);
...
void FIOSInputInterface::HandleKeyboardConnection(GCKeyboard* Keyboard)
{
    Keyboard.keyboardInput.keyChangedHandler = ^(GCKeyboardInput * _Nonnull keyboard, GCControllerButtonInput * _Nonnull key, GCKeyCode keyCode, BOOL pressed) {
        if ( !(FIOSInputInterface::IsKeyboardInhibited()) )
        {
            ...
            DeferredEvent.keycode = TranslateNonCharKeys(keyCode);
            DeferredEvent.charcode = TranslateGCKeyCodeToASCII(keyCode);
...
void FIOSInputInterface::ProcessEvent(const FDeferredIOSEvent& Event)
{
    if (Event.type != EIOSEventType::Invalid)
    {
        switch (Event.type)
        {
            ...
            {
                MessageHandler->OnKeyDown(Event.keycode, Event.charcode, false);
                break;
            }
            case EIOSEventType::KeyUp:
            {
                MessageHandler->OnKeyUp(Event.keycode, Event.charcode, false);
                break;
            }
