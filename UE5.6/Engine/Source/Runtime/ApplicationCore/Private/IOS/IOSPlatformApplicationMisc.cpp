...
EScreenPhysicalAccuracy FIOSPlatformApplicationMisc::ComputePhysicalScreenDensity(int32& ScreenDensity)
{
	...
	double ContentScaleFactor = 0;
	if (const UIView* View = [IOSAppDelegate GetDelegate].IOSView)
	{
	...
	else if (ScreenDensity > 0)
	{
		if (ContentScaleFactor != 0)
		{
			// The view may not use the full resolution of the screen so we scale the screen density accordingly
			ScreenDensity = FMath::TruncToInt(ScreenDensity / ContentScaleFactor);
		}
