...
void FJavaWrapper::FindClassesAndMethods(JNIEnv* Env)
{
	...
	AndroidThunkJava_GetIntentDataAsString = FindMethod(Env, GameActivityClassID, "AndroidThunkJava_GetIntentDataAsString", "()Ljava/lang/String;", bIsOptional);
	...
	AndroidThunkJava_HasExtraIgnoreView = FindMethod(Env, GameActivityClassID, "HasExtraIgnoreView", "()Z", bIsOptional);;
	...
	InputDeviceInfo_IsExternal = FJavaWrapper::FindField(Env, InputDeviceInfoClass, "isExternal", "Z", bIsOptional);
...
jmethodID FJavaWrapper::AndroidThunkJava_GetIntentDataAsString;
...
jmethodID FJavaWrapper::AndroidThunkJava_HasExtraIgnoreView;
...
jfieldID FJavaWrapper::InputDeviceInfo_IsExternal;
...
bool AndroidThunkCpp_GetInputDeviceInfo(int32 deviceId, FAndroidInputDeviceInfo &results)
{
	if (JNIEnv* Env = FAndroidApplication::GetJavaEnv())
	{
		...
		else if (deviceInfo)
		{
			...
			FScopedJavaObject JavaInputDeviceName = NewScopedJavaObject(Env, (jstring)Env->GetObjectField(*deviceInfo, FJavaWrapper::InputDeviceInfo_Name));
			const auto chars = Env->GetStringUTFChars(*JavaInputDeviceName, 0);
			results.Name = chars; // UTF8_TO_TCHAR(chars)
			Env->ReleaseStringUTFChars(*JavaInputDeviceName, chars);
			...
			results.IsExternal = (bool)Env->GetBooleanField(*deviceInfo, FJavaWrapper::InputDeviceInfo_IsExternal);
			return true;
		}
	}
...
bool AndroidThunkCpp_HasExtraIgnoreView()
{
	bool Result = false;
	if (JNIEnv* Env = FAndroidApplication::GetJavaEnv())
	{
		Result = FJavaWrapper::CallBooleanMethod(Env, FJavaWrapper::GameActivityThis, FJavaWrapper::AndroidThunkJava_HasExtraIgnoreView);
	}
	return Result;
}
...
JNI_METHOD void Java_com_epicgames_unreal_GameActivity_nativeHandleNewIntentData(JNIEnv* jenv, jobject thiz, jstring JavaIntent)
{
	FAndroidMisc::HandleNewIntentUri(FJavaHelper::FStringFromParam(jenv, JavaIntent));
}
...
FString AndroidThunkCpp_GetIntentDataAsString()
{
	FString Result = FString("");
	if (JNIEnv* Env = FAndroidApplication::GetJavaEnv())
	{
		Result = FJavaHelper::FStringFromLocalRef(Env, (jstring)FJavaWrapper::CallObjectMethod(Env, FJavaWrapper::GameActivityThis, FJavaWrapper::AndroidThunkJava_GetIntentDataAsString));
	}
	return Result;
}
...
JNI_METHOD void Java_com_epicgames_unreal_GameActivity_nativeVirtualKeyboardSendKey(JNIEnv* jenv, jobject thiz, jint keyCode)
{
	if (FTaskGraphInterface::IsRunning())
	{
		// Dispatch when ready to delay the submit when the text is being changed by another task (e.g Java_com_epicgames_unreal_GameActivity_nativeVirtualKeyboardChanged)
		FFunctionGraphTask::CreateAndDispatchWhenReady([keyCode = keyCode, CurrentVirtualKeyboardWidget = VirtualKeyboardWidget]()
		{
			if (CurrentVirtualKeyboardWidget.IsValid() && CurrentVirtualKeyboardWidget == VirtualKeyboardWidget)
			{
...
bool AndroidThunkCpp_GetSharedPreferenceBoolTypeSafe(const FString& Group, const FString& Key, bool& OutValue)
{
	if (JNIEnv* Env = FAndroidApplication::GetJavaEnv())
	{
		auto GroupArg = FJavaHelper::ToJavaString(Env, Group);
		auto KeyArg = FJavaHelper::ToJavaString(Env, Key);
		bool bResult = FJavaWrapper::CallBooleanMethod(Env, FJavaWrapper::GameActivityThis, FJavaWrapper::AndroidThunkJava_GetSharedPreferenceBoolean, *GroupArg, *KeyArg, false);

		// SharedPreferences.getBoolean will throw an exception if there is a type mismatch
		if (!Env->ExceptionCheck())
		{
			OutValue = bResult;
			return true;
...
bool AndroidThunkCpp_GetSharedPreferenceIntTypeSafe(const FString& Group, const FString& Key, int32& OutValue)
{
	if (JNIEnv* Env = FAndroidApplication::GetJavaEnv())
	{
		auto GroupArg = FJavaHelper::ToJavaString(Env, Group);
		auto KeyArg = FJavaHelper::ToJavaString(Env, Key);
		int32 Result = FJavaWrapper::CallIntMethod(Env, FJavaWrapper::GameActivityThis, FJavaWrapper::AndroidThunkJava_GetSharedPreferenceInt, *GroupArg, *KeyArg, 0);

		// SharedPreferences.getInt will throw an exception if there is a type mismatch
		if (!Env->ExceptionCheck())
		{
			OutValue = Result;
			return true;
...
bool AndroidThunkCpp_GetSharedPreferenceStringTypeSafe(const FString& Group, const FString& Key, FString& OutValue)
{
	if (JNIEnv* Env = FAndroidApplication::GetJavaEnv())
	{
		auto GroupArg = FJavaHelper::ToJavaString(Env, Group);
		auto KeyArg = FJavaHelper::ToJavaString(Env, Key);
		auto ResultObject = FJavaWrapper::CallObjectMethod(Env, FJavaWrapper::GameActivityThis, FJavaWrapper::AndroidThunkJava_GetSharedPreferenceString, *GroupArg, *KeyArg, NULL);

		// SharedPreferences.getString will throw an exception if there is a type mismatch
		if (Env->ExceptionCheck())
		{
			Env->ExceptionClear();
			return false;
		}
		else if (ResultObject && !Env->IsSameObject(ResultObject, NULL))
		{
			OutValue = FJavaHelper::FStringFromLocalRef(Env, (jstring)ResultObject);
			return true;
		}
	}
...
void AndroidThunkCpp_DeleteSharedPreference(const FString& Group, const FString& Key)
{
	if (JNIEnv* Env = FAndroidApplication::GetJavaEnv())
	{
		auto GroupArg = FJavaHelper::ToJavaString(Env, Group);
		auto KeyArg = FJavaHelper::ToJavaString(Env, Key);
		FJavaWrapper::CallVoidMethod(Env, FJavaWrapper::GameActivityThis, FJavaWrapper::AndroidThunkJava_DeleteSharedPreference, *GroupArg, *KeyArg);
	}
}

void AndroidThunkCpp_DeleteSharedPreferenceGroup(const FString& Group)
{
	if (JNIEnv* Env = FAndroidApplication::GetJavaEnv())
	{
		auto GroupArg = FJavaHelper::ToJavaString(Env, Group);
		FJavaWrapper::CallVoidMethod(Env, FJavaWrapper::GameActivityThis, FJavaWrapper::AndroidThunkJava_DeleteSharedPreferenceGroup, *GroupArg);
	}
}
...
JNI_METHOD void Java_com_epicgames_asis_AsisGameActivity_nativeSetGlobalActivity(JNIEnv* jenv, jobject thiz, jboolean bUseExternalFilesDir, jboolean bPublicLogFiles, jstring internalFilePath, jstring externalFilePath, jboolean bOBBinAPK, jstring APKFilename /*, jobject googleServices*/)
{
	STANDALONE_DEBUG_LOG(TEXT("nativeSetGlobalActivity: Entering asis nativeSetGlobalActivity, GameActivityThis=%p\n"), FJavaWrapper::GameActivityThis);
	if (FJavaWrapper::GameActivityThis != nullptr)
	{
		STANDALONE_DEBUG_LOG(TEXT("nativeSetGlobalActivity(asis): Error GameActivityThis is already set GameActivityThis=%p\n"), FJavaWrapper::GameActivityThis);
		//jenv->DeleteGlobalRef(FJavaWrapper::GameActivityThis);
		GGameActivityThis = FJavaWrapper::GameActivityThis = nullptr;
	}
	if (!FJavaWrapper::GameActivityThis)
	{
		...
		STANDALONE_DEBUG_LOG(TEXT("nativeSetGlobalActivity(asis): jenv=%p, set GameActivityThis=%p, bUseExternalFilesDir=%d, bOBBinAPK=%d\n"), jenv, FJavaWrapper::GameActivityThis, bUseExternalFilesDir, bOBBinAPK);
		// This call is only to set the correct GameActivityThis
		FAndroidApplication::InitializeJavaEnv(GJavaVM, JNI_CURRENT_VERSION, FJavaWrapper::GameActivityThis);
		// Rescan methods since we are switching to the asis version of GameActivity
		...
		if (bUseExternalFilesDir)
		{
			...
			GFilePathBase = GExternalFilePath;
#endif
			FPlatformMisc::LowLevelOutputDebugStringf(TEXT("nativeSetGlobalActivity(asis): GFilePathBase Path override to'%s'\n"), *GFilePathBase);
		}
		FPlatformMisc::LowLevelOutputDebugStringf(TEXT("nativeSetGlobalActivity(asis): InternalFilePath found as '%s'\n"), *GInternalFilePath);
		FPlatformMisc::LowLevelOutputDebugStringf(TEXT("nativeSetGlobalActivity(asis): ExternalFilePath found as '%s'\n"), *GExternalFilePath);
	}
