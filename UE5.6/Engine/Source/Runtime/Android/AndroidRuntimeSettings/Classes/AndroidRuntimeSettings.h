...
public:
	...
	// Minimum supported aspect ratio (width / height). Android will automatically letterbox application on devices with smaller aspect ratio
	UPROPERTY(GlobalConfig, EditAnywhere, Category = "APK Packaging", Meta = (DisplayName = "Minimum supported aspect ratio."))
	...
	UPROPERTY(GlobalConfig, EditAnywhere, Category = Build, meta = (DisplayName = "Force Emulator profile selection in non-shipping builds"))
	bool bForceEmulatorProfileSelectionInNonShippingBuilds;
	...
	UPROPERTY(GlobalConfig, EditAnywhere, Category = AdvancedBuild, meta = (DisplayName = "Enable compression of debug symbols"))
	bool bCompressDebugSymbols;
...
// 	bool bSplitIntoSeparateApks;
	...
	/** If enabled and using the ARM ASTC encoder this will cause the ASTC encoder to optimize the encode to LZ compression in the same manner as Oodle. 
	Refer to the texture encoding settings (e.g. Final Uses RDO) for details on how this works and its benefits.
	*/
	UPROPERTY(Config, EditAnywhere, Category = MultiTextureFormats, meta = (DisplayName = "Enable ASTC RDO", ConfigHierarchyEditable))
	bool bASTCUseRDO; // This name must match for all ASTC platforms so it's the same string in the ini
