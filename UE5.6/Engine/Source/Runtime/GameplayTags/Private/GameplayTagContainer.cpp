...
bool FQueryEvaluator::EvalExpr(FGameplayTagContainer const& Tags, bool bSkip)
{
	...
	ensureAlwaysMsgf(false, TEXT("Encountered invalid query expression type: %s"), *UEnum::GetValueAsString(ExprType));
	return false;
}
...
bool FGameplayTagContainer::Serialize(FStructuredArchive::FSlot Slot)
{
	...
	if (UnderlyingArchive.IsSaving())
	{
		...
		if (UnderlyingArchive.IsCooking())
		{
			...
			{
				SaveContext.AddCookLoadDependency(UGameplayTagsManager::Get().CreateCookDependency());
			}
		}
#endif
	}
		...
		if (Ar.IsSaving())
		{
			...
			if (TagToken.IsValid())
			{
				...
				if (!NetFieldExport.bExported)
				{
					...
					UE_LOG(LogGameplayTags, Verbose, TEXT("Replay> Exported Tag %s as NetFieldIndex %u"), *NetFieldExport.ExportName.ToString(), TagTokenIndex);
				}
