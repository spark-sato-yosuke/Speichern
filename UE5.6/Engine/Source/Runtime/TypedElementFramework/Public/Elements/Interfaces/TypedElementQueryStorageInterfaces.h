...
namespace UE::Editor::DataStorage
{
	...
	protected:
		...
		// Required as the interface doesn't not have direct access to any temporary handling of memory in the backend.
	...
	enum class EDirectQueryExecutionFlags : uint32
	{
		...
		IgnoreActivationCount = 1 << 2, //< If set, a direct call will not check activatable queries if they're set.
	...
	struct IQueryContext : public ICommonQueryWithEnvironmentContext
	{
		...
		/** Retrieves the row for a mapped object. Returns an invalid row handle if the key wasn't found. */
		virtual RowHandle LookupMappedRow(const FMapKeyView& Index) const = 0;
