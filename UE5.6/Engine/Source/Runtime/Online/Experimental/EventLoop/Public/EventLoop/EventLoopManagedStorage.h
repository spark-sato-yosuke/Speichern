...
private:
	...
	TMpscQueue<FAddRequest> AddRequests;
	/** Requests to remove entries from storage. Processed in Update(). */
	TMpscQueue<FRemoveRequest> RemoveRequests;
	/**
...
public:
	...
	FExternalHandle Add(ElementType&& Data)
	{
		...
		AddRequests.Enqueue(FAddRequest{Handle, MoveTemp(Data)});
		return Handle;
	}
	...
	{
		RemoveRequests.Enqueue(FRemoveRequest{Handle, MoveTemp(OnRemoveComplete)});
	}
