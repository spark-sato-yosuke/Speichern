...
FDefaultUserAgentBuilder::FDefaultUserAgentBuilder()
	...
	, ProjectComments()
#ifdef OVERRIDE_HEADER_PLATFORM_NAME
#define _TEXTIFY(T) #T
	, PlatformName(TEXT(_TEXTIFY(OVERRIDE_HEADER_PLATFORM_NAME)))
	, PlatformVersion(TEXT(_TEXTIFY(OVERRIDE_HEADER_PLATFORM_VERSION)))
#undef _TEXTIFY
#else
...
FString FGenericPlatformHttp::GetMimeType(const FString& FilePath)
{
	...
	if ( ExtensionMimeTypeMap.Num() == 0 )
	{
		...
		ExtensionMimeTypeMap.Add(TEXT(".log"), TEXT("text/plain"));
