...
static void VectorVMFreeOptimizerIntermediateData(FVectorVMOptimizerContext *OptContext)
{
...
static void GenerateConstantTableString(const FVectorVMOptimizerContext& Context, FString& OpsConstantTable)
{
...
static bool GenerateInstructionString(const FVectorVMOptimizerContext& Context, uint32 InstrIndex, FString& InstructionString)
{
...
void GenerateHumanReadableScript(const FVectorVMOptimizerContext& Context, FString& VMScript)
{
...
void FreeVectorVMOptimizeContext(FVectorVMOptimizerContext *OptContext)
{
	//save init data
	...
	VectorVMOptimizerErrorCallback* ErrorCallbackFn = OptContext->Error.CallbackFn;
	//save Hash Data
	...
	FMemory::Memset(OptContext, 0, sizeof(FVectorVMOptimizerContext));
	//restore init data
...
static uint32 VectorVMOptimizerSetError_(FVectorVMOptimizerContext* OptContext, uint32 Flags, uint32 LineNum)
{
...
static uint16 VectorVMOptimizeRemapConst(FVectorVMOptimizerContext* OptContext, uint16 ConstIdx)
{
	check((ConstIdx & 3) == 0);
...
static void VVMEnsureInstructionAlloced(FVectorVMOptimizerContext* OptContext, uint32 NumInstructions) {
...
static FVectorVMOptimizeInstruction *VVMPushNewInstruction(FVectorVMOptimizerContext* OptContext, EVectorVMOp OpCode)
{
...
static bool RegDupeCheck(FVectorVMOptimizerContext* OptContext, FVectorVMOptimizeInstruction *Ins0, int Ins0OutputRegIdx, FVectorVMOptimizeInstruction *Ins1, int FirstRegIdx, int LastRegIdx) {
...
static void VVMEnsureRegAlloced(FVectorVMOptimizerContext* OptContext, bool AllocSSA, uint32 *NumRegisterUsageAlloced, uint32 NumRegisters)
{
...
static uint32 VVMPushRegUsage_(FVectorVMOptimizerContext* OptContext, FVectorVMOptimizeInstruction *Instruction, uint16 RegIdx, uint32 Type, uint32 IOFlag, uint32 *NumRegisterUsageAlloced)
{
...
static void VVMOptWriteReg_(FVectorVMOptimizerContext* OptContext, uint16 Idx, uint16 *FinalTempRegRemap, uint8 *OptimizedBytecode, int NumOptimizedBytesWritten)
{
	uint16 Reg = OptContext->Intermediate.RegisterUsageBuffer[Idx];
	uint8 Type = OptContext->Intermediate.RegisterUsageType[Idx];
	if (Reg == 0xFFFF || Type == VVM_RT_INVALID)
	{ /* invalid reg */
...
static void VVMSetParentInstructionIndices(FVectorVMOptimizerContext* OptContext)
{
...
static bool VVMIsInstructionDependentOnInstruction(FVectorVMOptimizerContext* OptContext, FVectorVMOptimizeInstruction *Ins, uint16 *InstructionIdxStack, int DependentIdx)
{
...
static int VVMGetInstructionDependencyChain(FVectorVMOptimizerContext* OptContext, FVectorVMOptimizeInstruction *Ins, uint16 *InstructionIdxStack)
{	
	int NumInstructions = 0;
	int NumInstructionsChecked = 0;
...
static bool VVMDoesInstructionHaveDependencies(FVectorVMOptimizerContext* OptContext, FVectorVMOptimizeInstruction *Ins)
{
	for (int i = 0; i < Ins->NumInputRegisters; ++i)
	{
		uint16 SSAReg  = OptContext->Intermediate.SSARegisterUsageBuffer[Ins->RegPtrOffset + i];
...
uint32 OptimizeVectorVMScript(const uint8 *InBytecode, int InBytecodeLen, FVectorVMExtFunctionData *ExtFnIOData, int NumExtFns, FVectorVMOptimizerContext* OptContext, uint64 HashId, uint32 Flags)
{
	FreeVectorVMOptimizeContext(OptContext);
