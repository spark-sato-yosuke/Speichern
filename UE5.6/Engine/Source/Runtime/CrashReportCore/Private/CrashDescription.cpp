...
FCrashPropertyXmlNode& FCrashPropertyXmlNode::operator=(const TArray<FCrashThreadInfo>& ThreadsInfo)
{
	if (ThreadsInfo.Num() <= 0)
	{
		return *this;
	}
	
	bSet = true;
	FXmlNode* RuntimePropertiesNode = Owner->XmlFile->GetRootNode()->FindChildNode( FGenericCrashContext::RuntimePropertiesTag );
	RuntimePropertiesNode->AppendChildNode(TEXT("Threads"));
	FXmlNode* ThreadsNode = RuntimePropertiesNode->GetChildrenNodes().Last();
	for (int32 Idx = 0; Idx < ThreadsInfo.Num(); ++Idx)
	{
		ThreadsNode->AppendChildNode(TEXT("Thread"));
		FXmlNode* ThreadNode = ThreadsNode->GetChildrenNodes().Last();
		
		const FCrashThreadInfo& ThreadInfo = ThreadsInfo[Idx];
		ThreadNode->AppendChildNode(TEXT("CallStack"), ThreadInfo.PortableCallStackString);
		ThreadNode->AppendChildNode(TEXT("IsCrashed"), ThreadInfo.IsCrashing ? TEXT("true") : TEXT("false"));
		// TODO: do we need thread register states?
		ThreadNode->AppendChildNode(TEXT("Registers"));
		ThreadNode->AppendChildNode(TEXT("ThreadID"), FString::Printf(TEXT("%d"), ThreadInfo.ThreadId));
		ThreadNode->AppendChildNode(TEXT("ThreadName"), ThreadInfo.ThreadName);
	}
...
/**
  ...
  * @Owner Wes.Hunt
  *
  * @EventParam Result - If the HTTP end point could successfully be reached
  * @EventParam ResponseCode - HTTP Response code from the end point.
  * @EventParam Duration - Time to compress and send request.
  * @EventParam PayloadSize - Compressed size of payload (attached files)
  * @EventParam ReportCount - Number of reports uploaded (if more than one above values are accumulative or last observed)
  *
  */
...
void FPrimaryCrashProperties::SendPostUploadAnalytics(double Duration, bool bResult, int32 ResponseCode, uint32 PayloadSize, uint32 ReportCount)
{
	if (FCrashReportAnalytics::IsAvailable())
	{
		const TArray<FAnalyticsEventAttribute> CrashAttributes = {
			FAnalyticsEventAttribute(TEXT("Duration"), Duration),
			FAnalyticsEventAttribute(TEXT("Result"), bResult),
			FAnalyticsEventAttribute(TEXT("ResponseCode"), ResponseCode),
			FAnalyticsEventAttribute(TEXT("PayloadSize"), PayloadSize),
			FAnalyticsEventAttribute(TEXT("ReportCount"), ReportCount),
		};
...
void FCrashContext::SetupPrimaryCrashProperties() 
{
	...
	if (bIsValid)
	{
		...
		GetCrashProperty( CrashScreenshotWidth, FGenericCrashContext::EngineDataTag, TEXT("CrashScreenshotWidth"));
		GetCrashProperty( CrashScreenshotHeight, FGenericCrashContext::EngineDataTag, TEXT("CrashScreenshotHeight"));
