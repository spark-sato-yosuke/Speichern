...
static TAutoConsoleVariable<int32> CVarHDRAcesVersion(
	TEXT("r.HDR.Aces.Version"),
	1,
	TEXT("Version of the ACES transform:\n")
	TEXT("1: ACES 1.3 (default)\n")
	TEXT("2: ACES 2.0\n"),
	ECVF_RenderThreadSafe
);
...
{
	// Note: Renderer settings console variable requires project restart, so we can statically cache this check.
	static const bool bIsDefaultBackBufferLinear = []() -> bool
		{
			...
			return CVarDefaultBackBufferPixelFormat ? CVarDefaultBackBufferPixelFormat->GetValueOnAnyThread() == 3 /* EDefaultBackBufferPixelFormat::DBBPF_FloatRGBA */ : false;
		}();
	
	return bIsDefaultBackBufferLinear && !IsHDREnabled();
}
