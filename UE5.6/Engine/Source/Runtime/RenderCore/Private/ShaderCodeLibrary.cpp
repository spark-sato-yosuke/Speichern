...
static FString ShaderTypeInfoExtension = TEXT(".stinfo");
...
static FString GetShaderTypeInfoFilename(const FString& BaseDir, const FString& LibraryName, FName Platform)
{
...
void FShaderMapResource_SharedCode::ReleaseRHI()
{
	...
	if (GetNumRefs() > 0)
	{
		// There shouldn't be any references left to this resource at this point. Diagnose what shader libraries are still holding a reference to this shader map resource.
		DiagnoseLeakingShaderMapResourceRefs(this);
...
struct FEditorShaderCodeArchive
{
	...
	int32 AddShaderCode(FShaderCodeStats& CodeStats,
			...
			for (uint32 i = 0; i < ShaderMapEntry.NumShaders; ++i)
			{
				...
				for (uint64 OtherShaderTypeHash : OtherShaders.ShaderTypes[OtherShaderIndex].Data)
				{
					...
					if (HashIndex >= SerializedShaderTypes.Num() || SerializedShaderTypes[HashIndex] != OtherShaderTypeHash)
					{
						...
						FShaderCodeStats::FPerTypeStats& TypeCodeStats = CodeStats.ShaderTypeStats.FindOrAdd(OtherShaderTypeHash);
						int32 CodeSize = SerializedShaders.ShaderEntries[ShaderIndex].Size;
						TypeCodeStats.NumShaders++;
						TypeCodeStats.ShadersSize += CodeSize;
					}
	...
	void AddShaderCodeLibraryByName(const FString& BaseDir, const FString& InLibraryName, FShaderCodeStats& CodeStats)
	{
		EShaderPlatform Platform = ShaderFormatToLegacyShaderPlatform(FormatName);
		FString CodeArchiveFileName = GetCodeArchiveFilename(BaseDir, InLibraryName, FormatName);
		if (FArchive* PrevCookedAr = CreateShaderFileReader(*CodeArchiveFileName))
		{
			uint32 Version = 0;
			*PrevCookedAr << Version;
			...
			if (Version == GShaderCodeArchiveVersion)
			{
				...
				*PrevCookedAr << PrevCookedShaders;
				...
				FString AssetInfoFileName = GetShaderAssetInfoFilename(BaseDir, InLibraryName, FormatName);
				...
				FString TypeInfoFileName = GetShaderTypeInfoFilename(BaseDir, InLibraryName, FormatName);
				...
				if (ShaderTypeInfoReader)
				{
					*ShaderTypeInfoReader << PrevCookedShaders.ShaderTypes;
					...
					UE_LOG(LogShaderLibrary, Display, TEXT("Loaded shader type info %s for the shader library %s: %d entries"),
						*TypeInfoFileName,
						*CodeArchiveFileName,
				...
				{
					UE_LOG(LogShaderLibrary, Warning, TEXT("Could not load shader type info %s for the shader library %s"),
						*TypeInfoFileName,
						*CodeArchiveFileName
					);
				}
				...
				{
					AddShaderCode(CodeStats, PrevShaderMapIndex, PrevCookedShaders, PrevCookedShadersCodeStart, *PrevCookedAr);
				}
			}
	...
	void AddShaderCodeLibraryFromDirectory(const FString& BaseDir, FShaderCodeStats& CodeStats)
	{
		AddShaderCodeLibraryByName(BaseDir, LibraryName, CodeStats);
	}

	void AddExistingShaderCodeLibrary(FString const& OutputDir, FShaderCodeStats& CodeStats)
	{
		check(LibraryName.Len() > 0);
		...
		for (const FString& ShaderFileName : ShaderFiles)
		{
			...
			if (FindShaderFormat.FindNext())
			{
				...
				{
					AddShaderCodeLibraryByName(OutputDir, FoundLibraryName, CodeStats);
				}
			}
		}
	}

	void FinishPopulate(const FString& OutputDir, FShaderCodeStats& CodeStats)
	{
		AddExistingShaderCodeLibrary(OutputDir, CodeStats);
	}
	...
	bool SaveToDisk(const FString& OutputDir, const FString& MetaOutputDir, bool bSaveOnlyAssetInfo = false, TArray<FString>* OutputFilenames = nullptr)
	{
		...
		if (bSuccess && SerializedShaders.GetNumShaderMaps() > 0)
		{
			...
			FString TypeInfoIntermediatePath = GetShaderTypeInfoFilename(FPaths::ProjectSavedDir() / TEXT("Shaders") / FormatName.ToString(), LibraryName, FormatName);
			...
			if (!bSaveOnlyAssetInfo)
			{
				...
				FArchive* ShaderTypeFileWriter = IFileManager::Get().CreateFileWriter(*TypeInfoIntermediatePath, FILEWRITE_NoFail);
				if (ShaderTypeFileWriter)
				{
					*ShaderTypeFileWriter << SerializedShaders.ShaderTypes;
					ShaderTypeFileWriter->Close();
					delete ShaderTypeFileWriter;
					
					// Copy to output location - support for iterative native library cooking
					if (!CopyFile(GetShaderTypeInfoFilename(OutputDir, LibraryName, FormatName), TypeInfoIntermediatePath, OutputFilenames))
					{
						bSuccess = false;
					}
					else if (MetaOutputDir.Len())
					{
						if (!CopyFile(GetShaderTypeInfoFilename(MetaOutputDir / TEXT("../ShaderLibrarySource"), LibraryName, FormatName), TypeInfoIntermediatePath, nullptr))
						{
							bSuccess = false;
						}
					}
	...
	static bool CreatePatchLibrary(FName FormatName, FString const& LibraryName, TArray<FString> const& OldMetaDataDirs, FString const& NewMetaDataDir, FString const& OutDir, bool bNativeFormat, bool bNeedsDeterministicOrder)
	{
		...
		if (bOK)
		{
			...
			{
				FString Empty;
				OutLibrary.FinishPopulate(OutDir, OutStats);
...
public:
	...
	void GatherShaderMapResourceReferences(const FShaderMapResource_SharedCode* InResource, TArray<FString>& OutLibrariesWithRefToResource)
	{
		...
		for (const TTuple<FString, TUniquePtr<UE::ShaderLibrary::Private::FNamedShaderLibrary>>& NamedLibraryPair : NamedLibrariesStack)
		{
			...
			for (const TUniquePtr<FShaderLibraryInstance>& LibraryInstance : Library->Components)
			{
				for (int32 LibraryResourceIndex = 0; LibraryResourceIndex < LibraryInstance->GetNumResources(); ++LibraryResourceIndex)
				{
					if (LibraryInstance->GetResource(LibraryResourceIndex).GetReference() == InResource)
					{
	...
	void FinishPopulateShaderCode(const FString& ShaderCodeDir, const FString& MetaOutputDir, const TArray<FName>& ShaderFormats)
	{
		for (FName ShaderFormatName : ShaderFormats)
		{
			...
			if (CodeArchive)
			{
				CodeArchive->FinishPopulate(ShaderCodeDir, CodeStats);
			}
...
// Used to diagnose leaking references to shader map resources in FShaderMapResource_SharedCode::ReleaseRHI()
void DiagnoseLeakingShaderMapResourceRefs(const FShaderMapResource_SharedCode* InResource)
{
	TArray<FString> LibsWithRefToResource;
	FShaderLibrariesCollection::Impl->GatherShaderMapResourceReferences(InResource, LibsWithRefToResource);
	FString Info;
	for (const FString& Name : LibsWithRefToResource)
	{
		if (!Info.IsEmpty())
		{
			Info += TEXT(", ");
	...
	if (!Info.IsEmpty())
	{
		UE_LOG(LogShaderLibrary, Warning, TEXT("Leaking reference to shader map resource detected; Resource \"%s\" still referenced in: %s"), *InResource->GetFriendlyName(), *Info);
	}
}
...
bool FShaderLibraryCooker::MergeShaderCodeArchive(const TArray<FString>& CookedMetadataDirs, const FString& OutputDir, TArray<FString>& OutWrittenFiles)
{
	...
	for (const FString& MetadataDir : CookedMetadataDirs)
	{
		...
		for (const FString& ByteCodeFile : ShaderBytecodeFiles)
		{
			...
			{
				ShaderCodeArchives[ByteCodeFile].Key.AddShaderCodeLibraryFromDirectory(ShaderCodeDir, ShaderCodeArchives[ByteCodeFile].Value);
			}
			else
			{
				...
				if (ensureMsgf(FindShaderFormat.FindNext(), TEXT("Unable to parse out shader format from %s"), *ByteCodeFile))
				{
					...
					NewCodeArchive.AddShaderCodeLibraryFromDirectory(ShaderCodeDir, NewCodeStats);
	...
	for (TPair<FString, TPair<FEditorShaderCodeArchive, FShaderCodeStats>>& ShaderCodeArchivePair : ShaderCodeArchives)
	{
		...
		if (ShaderCodeArchivePair.Value.Key.SaveToDisk(OutShaderCodeDir, TEXT(""), false, &CreatedFiles))
		{
		...
		CreatedFiles.Reset();
		if (ShaderCodeArchivePair.Value.Key.SaveToDisk(OutContentDir, TEXT(""), false, &CreatedFiles))
		{
			OutWrittenFiles.Append(MoveTemp(CreatedFiles));
		}
		else
