...
FPooledRenderTarget* FRenderTargetPool::CreateRenderTarget(FRHICommandListBase& RHICmdList, const FRHITextureCreateInfo& Desc, uint32 DescHash, const TCHAR* Name)
{
	...
	if (EnumHasAnyFlags(Desc.Flags, TexCreate_UAV))
	{
		...
		if (GRHISupportsUAVFormatAliasing)
		{
			...
			Result->RenderTargetItem.UAV = RHICmdList.CreateUnorderedAccessView(
				Result->GetRHI(),
				FRHIViewDesc::CreateTextureUAV()
					.SetDimensionFromTexture(Result->GetRHI())
					.SetFormat(AliasFormat));
		}
		else
		{
			checkf(Desc.UAVFormat == PF_Unknown || Desc.UAVFormat == Desc.Format, TEXT("UAV aliasing is not supported by the current RHI."));
			Result->RenderTargetItem.UAV = RHICmdList.CreateUnorderedAccessView(Result->GetRHI(), FRHIViewDesc::CreateTextureUAV().SetDimensionFromTexture(Result->GetRHI()));
		}
	}
...
{
	const FPooledRenderTarget* RenderTarget;
	uint32 SizeInKB;

	bool operator<(const FRenderTargetWithSize& Other) const
	{
		if (SizeInKB != Other.SizeInKB)
		{
			// Sort larger sizes first
			return SizeInKB > Other.SizeInKB;
		}
		int32 NameOrdinalCompare = FCString::Strcmp(RenderTarget->GetDesc().DebugName, Other.RenderTarget->GetDesc().DebugName);
		if (NameOrdinalCompare != 0)
		{
			return NameOrdinalCompare < 0;
		}
...
void FRenderTargetPool::DumpMemoryUsage(FOutputDevice& OutputDevice)
{
	...
	TArray<FRenderTargetWithSize> SortedRenderTargets;
	SortedRenderTargets.Reserve(PooledRenderTargets.Num());
	...
	for (int32 i = 0; i < PooledRenderTargets.Num(); ++i)
	{
		...
		if (Element)
		{
			SortedRenderTargets.Add({ Element, ComputeSizeInKB(*Element) });
		}
	}

	SortedRenderTargets.Sort();

	OutputDevice.Logf(TEXT("Pooled Render Targets:"));
	for (FRenderTargetWithSize& SortedRenderTarget : SortedRenderTargets)
	{
		if (SortedRenderTarget.RenderTarget->UnusedForNFrames > 2)
		{
			UnusedAllocationInKB += SortedRenderTarget.SizeInKB;
		}

		const FPooledRenderTarget* Element = SortedRenderTarget.RenderTarget;
		OutputDevice.Logf(
			TEXT("  %6.3fMB %4dx%4d%s%s %2dmip(s) %s (%s) Unused frames: %d"),
			SortedRenderTarget.SizeInKB / 1024.0f,
