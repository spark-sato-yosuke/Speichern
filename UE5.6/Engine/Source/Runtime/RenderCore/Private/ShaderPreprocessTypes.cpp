...
static TAutoConsoleVariable<bool> CVarEnableDebugHashAssociation(
	TEXT("r.ShaderCompiler.EnableDebugHashAssociation"),
	true,
	TEXT("If true, a // DebugHash_<hash> comment will be appended to all finalized shader source matching the DebugHash_<hash>.txt file output with "
		 "shader debug artifacts. This can be used to find debug artifacts from shader source visible in Pix/RenderDoc/etc when symbols are enabled. "
		 "Note that currently the only known reason to disable this is when comparing shader source or symbols across streams where versions may differ "
		 "but source would otherwise match."),
	ECVF_ReadOnly);
...
struct FPreprocessConstants
{
	FPreprocessConstants()
	{
		...
		if (CVarEnableDebugHashAssociation.GetValueOnAnyThread())
		{
...
private:
	...
	static constexpr FShaderSource::FViewType DebugHashCommentStart = ANSITEXTVIEW("// ");
	static constexpr FShaderSource::FViewType DebugHashCommentEnd = ANSITEXTVIEW("\n");
	static constexpr FShaderSource::FViewType LineDirectiveStart = ANSITEXTVIEW("#line 1 \"");
	static constexpr FShaderSource::FViewType LineDirectiveEnd = ANSITEXTVIEW("\"\n");
...
static FPreprocessConstants& GetPreprocessConstants()
{
	static FPreprocessConstants PreprocessConstants;
	return PreprocessConstants;
}

FShaderSource::FViewType GetShaderSourceDebugHashPrefix()
{
	return GetPreprocessConstants().DebugHashPrefix;
}

FStringView GetShaderSourceDebugHashPrefixWide()
{
	return GetPreprocessConstants().DebugHashPrefixWide;
}

void FShaderDiagnosticRemapper::Remap(FShaderCompilerError& Diagnostic) const
{
	if (!Diagnostic.ErrorLineString.IsEmpty() && !Diagnostic.ErrorVirtualFilePath.IsEmpty() && Diagnostic.ErrorVirtualFilePath == GetPreprocessConstants().FilenameSentinel.GetData())
	{
	...
	int32 FilenameIndex = Diagnostic.StrippedErrorMessage.Find(GetPreprocessConstants().FilenameSentinel.GetData());
	...
	while (FilenameIndex >= 0)
	{
		...
		int32 LineNumberStart = FilenameIndex + GetPreprocessConstants().FilenameSentinel.Len();
		...
		int32 RemappedLen = OriginalMessage.Len() - GetPreprocessConstants().FilenameSentinel.Len() + RemapData.Filename.Len();
		...
		int32 SeparatorStart = FilenameIndex + GetPreprocessConstants().FilenameSentinel.Len();
		...
		FilenameIndex = Diagnostic.StrippedErrorMessage.Find(GetPreprocessConstants().FilenameSentinel.GetData());
	}
}
...
void FShaderPreprocessOutput::StripCode(bool bCopyOriginalPreprocessdSource)
{
	...
	FShaderSource PreprocessedSourceStripped(GetPreprocessConstants().StrippedCodeHeader, PreprocessedSource.Len());
	FShaderSource::CharType* OutStrippedData = PreprocessedSourceStripped.GetData();
	FShaderSource::CharType* OutStripped = OutStrippedData + GetPreprocessConstants().StrippedCodeHeader.Len();
