...
struct FShaderFileCacheEntry
{
	...
	bool IsEmpty() const
	{
		return Source.IsEmpty() && !StrippedSource.IsValid() && !Dependencies.IsValid();
	}
};
		...
		bResult = 
			GShaderFileCache.FindOrTryProduceAndApply(VirtualFilePath,
				[&VirtualFilePath, &OutCompileErrors](FShaderFileCacheEntry& CachedFile)
				{
					...
					if (!ShaderFilePath.IsEmpty() && FFileHelper::LoadFileToString(CachedFile.Source, *ShaderFilePath, FFileHelper::EHashOptions::EnableVerify | FFileHelper::EHashOptions::ErrorMissingHash))
					{
				...
				[&bFoundValidFile, &OutFileContents, &OutStrippedContents](const FShaderFileCacheEntry& CachedFile)
				{
					if (OutFileContents)
					{
					...
					if (FMissingShaderFileCacheGuard::IsEnabled())
					{
						bFoundValidFile = !CachedFile.IsEmpty();
					}
				}
			);
	}

	// When caching missing files, the map find will always return true, so the result is based on whether it is an empty entry or not.
	if (FMissingShaderFileCacheGuard::IsEnabled())
	{
		bResult = bFoundValidFile;
	}
...
void FShaderCompileJob::OnComplete()
{
	...
	// always set the preprocess time on the output struct, regardless of whether this job hit the cache or actually compiled, since
	// we always run preprocessing as a first step when the job was submitted.
	...
	if (bHasSymbols || Input.DumpDebugInfoEnabled())
	{
	...
	if (Input.DumpDebugInfoEnabled())
	{
		...
		if (!PreprocessOutput.bSucceeded)
		{
			...
			ShaderFormat->OutputDebugDataMinimal(Input);
		}
		else if (CVarDumpDebugInfoForCacheHits.GetValueOnAnyThread() || Output.CompileTime > 0.0f)
