...
FORCEINLINE FRDGBufferRef CreateByteAddressUploadBuffer(
	...
	FRDGBufferInitialDataSizeCallback&& InitialDataSizeCallback)
{
	FRDGBufferRef Buffer = GraphBuilder.CreateBuffer(FRDGBufferDesc::CreateByteAddressUploadDesc(4), Name, MoveTemp(NumElementsCallback));
 ...
 */
template <typename ArrayType>
FORCEINLINE FRDGBufferRef CreateByteAddressUploadBuffer_Impl(
	FRDGBuilder& GraphBuilder,
	const TCHAR* Name,
	TRDGBufferArrayCallback<ArrayType>&& ArrayCallback)
{
	return CreateByteAddressUploadBuffer(GraphBuilder, Name,
		/*NumElementsCallback = */[ArrayCallback]() { const auto& Array = ArrayCallback(); return Array.Num() * Array.GetTypeSize() / 4; },
		/*InitialDataCallback = */[ArrayCallback]() { return ArrayCallback().GetData(); },
		/*InitialDataSizeCallback = */[ArrayCallback]() { const auto& Array = ArrayCallback(); return Array.Num() * Array.GetTypeSize(); });
}
...
FORCEINLINE FRDGBufferRef CreateByteAddressUploadBuffer(
	...
	GetArrayRefCallback&& ArrayCallback)
{
	return CreateByteAddressUploadBuffer_Impl<Type>(GraphBuilder, Name, MoveTemp(ArrayCallback));
}
...
// Same as AllocatePooledBuffer but will rebase the allocated buffer under the current LLM tag.
#if ENABLE_LOW_LEVEL_MEM_TRACKER || UE_MEMORY_TRACE_ENABLED
RENDERCORE_API TRefCountPtr<FRDGPooledBuffer> AllocatePooledBufferCurrentLLMTag(
	FRHICommandListBase& RHICmdList,
	const FRDGBufferDesc& Desc,
	const TCHAR* Name,
	ERDGPooledBufferAlignment Alignment = ERDGPooledBufferAlignment::Page);
#else // #if ENABLE_LOW_LEVEL_MEM_TRACKER
inline TRefCountPtr<FRDGPooledBuffer> AllocatePooledBufferCurrentLLMTag(
	FRHICommandListBase& RHICmdList,
	const FRDGBufferDesc& Desc,
	const TCHAR* Name,
	ERDGPooledBufferAlignment Alignment = ERDGPooledBufferAlignment::Page)
{
	return AllocatePooledBuffer(Desc, Name, Alignment);
}
