...
FORCEINLINE float GetBasisDeterminantSign(const FVector& XAxis, const FVector& YAxis, const FVector& ZAxis)
{
	...
	// RotDeterminant is the determinant of the upper left 3x3 matrix and equivalent to the full 4x4 determinant for this matrix
	// at one fourth the cost. 
	return (Basis.RotDeterminant() < 0) ? -1.0f : +1.0f;
}
