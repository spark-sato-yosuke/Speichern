...
public:
	...
	{}
		...
		SLATE_ARGUMENT(bool, bUseCopyToClipboard)
	...
	*/
	...
	TSharedRef<SWidget> ConstructInternals( const FArguments& InArgs ) 
	{
		...
		TSharedRef<SWidget> CopyToClipboardButton = SNullWidget::NullWidget;

		if (InArgs._bUseCopyToClipboard)
		{
			SAssignNew(CopyToClipboardButton, SBorder)
			...
			[
				...
				.OnClicked(this, &SNotificationItemImpl::OnCopyTextToClipboard)
				.ContentPadding(0.f)
				[
					SNew(SImage)
...
protected:
	...
	/* Used to fetch the text to copy to the clipboard */
	FText GetClipboardText() const
	{
		FTextBuilder ClipboardTextBuilder;
		if (Text.IsSet())
		{
			ClipboardTextBuilder.AppendLine(Text.Get());
		}

		if (SubText.IsSet())
		{
			ClipboardTextBuilder.AppendLine(SubText.Get());
		}

		return ClipboardTextBuilder.ToText();
	}
...
private:
	...
	FReply OnCopyTextToClipboard() const
	{
		FPlatformApplicationMisc::ClipboardCopy(*GetClipboardText().ToString());
		return FReply::Handled();
	}
	...
	if (FSlateNotificationManager::Get().AreNotificationsAllowed())
	{
		...
		else
		{
			...
			NewItem = SNew(SNotificationItemImpl)
				...
				.bUseCopyToClipboard(Info.bUseCopyToClipboard)
				.WidthOverride(Info.WidthOverride)
