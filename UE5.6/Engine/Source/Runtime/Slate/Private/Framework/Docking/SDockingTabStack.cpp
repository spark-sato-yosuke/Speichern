...
void SDockingTabStack::CloseTabsInDirectionFromForegroundTab(ETabsToClose TabsToClose, ECloseTabsDirection Direction)
{
	TSharedPtr<SDockTab> ForegroundTab = TabWell->GetForegroundTab();
	if (ForegroundTab.IsValid())
	{
		const int32 ForegroundTabIndex = TabWell->GetForegroundTabIndex();
		const int32 Increment = GetIncrementFromCloseTabsDirection(Direction);

		int32 DestroyIndex = ForegroundTabIndex + Increment;
		while (DestroyIndex >= 0 && DestroyIndex < TabWell->GetNumTabs())
		{
			...
			const bool bCanClose =
				(TabsToClose == ETabsToClose::CloseAllTabs) ||
				(TabsToClose == ETabsToClose::CloseDocumentTabs && VisualTabRole == ETabRole::DocumentTab) ||
				(TabsToClose == ETabsToClose::CloseDocumentAndMajorTabs && (VisualTabRole == ETabRole::DocumentTab || VisualTabRole == ETabRole::MajorTab));

			// Need to increment index if we failed to close a tab to the right or if we're closing tabs to the left regardless of if we succeeded
			if (!bCanClose || !Tab->RequestCloseTab() || Direction == ECloseTabsDirection::Left)
			{
				DestroyIndex += Increment;
			}
		}
	}
}
...
TSharedRef<SWidget> SDockingTabStack::MakeContextMenu()
{
	...
	FMenuBuilder MenuBuilder( bCloseAfterSelection, NULL, TSharedPtr<FExtender>(), bCloseSelfOnly, &FCoreStyle::Get() );
	{
		...
		MenuBuilder.BeginSection("DockingTabStackCloseTabs");
		{
			...
			const ETabsToClose TabsToClose = ETabsToClose::CloseDocumentAndMajorTabs;

			MenuBuilder.AddMenuEntry(
				LOCTEXT("CloseTabsToTheLeft", "Close Tabs to the Left"),
				LOCTEXT("CloseTabsToTheLeftTooltip", "Closes all tabs to the left of the active tab."),
				FSlateIcon(),
				FUIAction(
					FExecuteAction::CreateSP( this, &SDockingTabStack::CloseTabsInDirectionFromForegroundTab, TabsToClose, ECloseTabsDirection::Left),
					FCanExecuteAction::CreateSP( this, &SDockingTabStack::CanCloseTabsInDirectionOfForegroundTab, ECloseTabsDirection::Left)
				)
			);
			...
			MenuBuilder.AddMenuEntry(
				...
				FSlateIcon(),
				FUIAction(
					FExecuteAction::CreateSP( this, &SDockingTabStack::CloseTabsInDirectionFromForegroundTab, TabsToClose, ECloseTabsDirection::Right),
					FCanExecuteAction::CreateSP( this, &SDockingTabStack::CanCloseTabsInDirectionOfForegroundTab, ECloseTabsDirection::Right)
				)
			);
