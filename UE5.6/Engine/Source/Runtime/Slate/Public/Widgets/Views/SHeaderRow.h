...
public:
	...
	public:
		...
		SLATE_BEGIN_ARGS(FColumn)
			...
			/** Allow to provide a custom handler when a splitter is double-clicked in Manual column size mode. */
			SLATE_EVENT( FPointerEventHandler, OnColumnSplitterDoubleClick )
			...
			SLATE_ATTRIBUTE( bool, IsSorting )
		...
		FColumn( const FArguments& InArgs )
			...
			, OnColumnSplitterDoubleClick( InArgs._OnColumnSplitterDoubleClick )
