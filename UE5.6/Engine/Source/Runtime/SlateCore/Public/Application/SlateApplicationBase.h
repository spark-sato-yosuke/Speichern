...
enum class EPopupCursorOverlapMode : uint8
{
...
public:
	 ...
	 *
	 * @param  InFocusPath WidgetPath to the Widget to being focused
	 ...
	 * @param InFocusPath WidgetPath to the Widget to being focused.
	 * @param InCause The reason that focus is changing.
	 * @return true if the widget or a descendant is now focused, false otherwise.
	 */
...
private:
	...
	/** The set of active timer handles. */
	TSet<TWeakPtr<FActiveTimerHandle>> ActiveTimerHandles;
...
public:
	...
	{
		check(CurrentBaseApplication.IsValid());
...
#endif
#if WITH_EDITOR
	FOnConstrainedAspectRatioChanged OnConstrainedAspectRatioChanged;
#endif
protected:
#if WITH_ACCESSIBILITY
