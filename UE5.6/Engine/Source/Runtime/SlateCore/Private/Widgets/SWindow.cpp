...
void SWindow::Construct(const FArguments& InArgs)
{
	...
	this->FocusUserIndex = InArgs._FocusUserIndex;
...
bool SWindow::OnIsActiveChanged( const FWindowActivateEvent& ActivateEvent )
{
	...
	if (bWasDeactivated)
	{
		...
		else if (SupportsKeyboardFocus())
		{
			...
			WidgetFocusedOnDeactivate = (FocusUserIndex == INDEX_NONE) ? FSlateApplicationBase::Get().GetKeyboardFocusedWidget() : FSlateApplicationBase::Get().GetUserFocusedWidget(FocusUserIndex);
		...
		if (ActivateEvent.GetActivationType() == FWindowActivateEvent::EA_Activate)
		{
			...
			if (PinnedWidgetToFocus.IsValid())
			{
				...
				if (FSlateWindowHelper::FindPathToWidget(JustThisWindow, PinnedWidgetToFocus.ToSharedRef(), WidgetToFocusPath))
				{
					...
					{
						FSlateApplicationBase::Get().SetUserFocus(FocusUserIndex, WidgetToFocusPath, EFocusCause::WindowActivate);
					}
				}
			}
			...
			else if (SupportsKeyboardFocus())
			{
				...
				if (FSlateWindowHelper::FindPathToWidget(JustThisWindow, WindowWidgetToFocus, WindowWidgetPath))
				{
					...
					{
						FSlateApplicationBase::Get().SetUserFocusAllowingDescendantFocus(FocusUserIndex, WindowWidgetPath, EFocusCause::WindowActivate);
					}
				}
			}
		}
