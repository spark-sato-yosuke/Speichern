	...
	if (bUsePerformanceThreshold)
	{
		...
		for (int32 Index = FrameRootHandles.Num() - 1; Index >= 0; --Index)
		{
			...
			else
			{
				if (bFirstItem)
				{
					UE_LOG(LogSlateDebugger, Log, TEXT("Slate Performance Threshold reached at frame %" UINT64_FMT), GFrameCounter);
					UE_TRACE_SLATE_BOOKMARK(TEXT("Slate Performance Threshold %" UINT64_FMT), GFrameCounter);
