...
TSharedRef<const FCompositeFont> FLegacySlateFontInfoCache::GetDefaultFont()
{
	if (!DefaultFont.IsValid())
	{
		...
		// Explicit fallback for glyphs missing from Roboto.
		{
			FCompositeSubFont& SubFont = MutableDefaultFont->SubTypefaces[MutableDefaultFont->SubTypefaces.AddDefaulted()];
			APPEND_RANGE(SubFont, BoxDrawing);
			APPEND_RANGE(SubFont, BlockElements);
			APPEND_RANGE(SubFont, HalfwidthAndFullwidthForms);
			APPEND_FONT(SubFont.Typeface, "Regular", "DroidSansFallback.ttf", EFontHinting::Default);
		}
		...
		// Simplified Chinese (editor-only)
		if (GIsEditor)
		{
			FCompositeSubFont& SubFont = MutableDefaultFont->SubTypefaces[MutableDefaultFont->SubTypefaces.AddDefaulted()];
			SubFont.Cultures = TEXT("zh-Hans");
			APPEND_RANGE(SubFont, CJKCompatibility);
			APPEND_RANGE(SubFont, CJKCompatibilityForms);
			APPEND_RANGE(SubFont, CJKCompatibilityIdeographs);
			APPEND_RANGE(SubFont, CJKCompatibilityIdeographsSupplement);
			...
			APPEND_RANGE(SubFont, EnclosedCJKLettersAndMonths);
			APPEND_FONT(SubFont.Typeface, "Regular", "DroidSansFallback.ttf", EFontHinting::Default);
			APPEND_FONT(SubFont.Typeface, "Italic", "DroidSansFallback.ttf", EFontHinting::Default);
			APPEND_FONT(SubFont.Typeface, "Bold", "DroidSansFallback.ttf", EFontHinting::Default);
			APPEND_FONT(SubFont.Typeface, "BoldItalic", "DroidSansFallback.ttf", EFontHinting::Default);
			APPEND_FONT(SubFont.Typeface, "BoldCondensed", "DroidSansFallback.ttf", EFontHinting::Default);
			APPEND_FONT(SubFont.Typeface, "BoldCondensedItalic", "DroidSansFallback.ttf", EFontHinting::Default);
			APPEND_FONT(SubFont.Typeface, "Black", "DroidSansFallback.ttf", EFontHinting::Default);
			APPEND_FONT(SubFont.Typeface, "BlackItalic", "DroidSansFallback.ttf", EFontHinting::Default);
			APPEND_FONT(SubFont.Typeface, "Light", "DroidSansFallback.ttf", EFontHinting::Default);
			APPEND_FONT(SubFont.Typeface, "VeryLight", "DroidSansFallback.ttf", EFontHinting::Default);
		}
