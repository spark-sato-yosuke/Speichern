...
static int32 SdfTextDebugVisual = static_cast<int32>(ESdfTextDebugVisual::Disabled);

FAutoConsoleVariableRef CVarSlateSdfTextDebugVisual(
	TEXT("UI.SlateSDFText.DebugVisual"),
	SdfTextDebugVisual,
	TEXT("Slate SDF text debug visuals - 0: Disabled, 1: Raw SDF, 2: Text color, 3: Colored boxes."));

static const FColor DebugVisualBitmapTextColor(0x80, 0x80, 0x80); // Gray
static const FColor DebugVisualMsdfTextColor(0xff, 0x00, 0xff); // Magenta
static const FColor DebugVisualSdfTextColor(0x00, 0xff, 0xff); // Cyan
static const FColor DebugVisualApproximateSdfTextColor(0xff, 0x80, 0x00); // Orange
#endif // ENABLE_SDF_DEBUG_VISUAL
...
FSlateBatchData::FSlateBatchData()
	...
	, bRequiresVirtualTextureFeedback(false)
{}
...
{
	return bRequiresVirtualTextureFeedback;
}
...
void FSlateBatchData::MergeRenderBatches()
{
	...
	if(RenderBatches.Num())
	{
		...
		for (int32 BatchIndex = 0; BatchIndex < BatchIndices.Num(); ++BatchIndex)
		{
			...
			if (const FSlateShaderResource* ShaderResource = CurBatch.GetShaderResource())
			{
				bRequiresVirtualTextureFeedback |= ShaderResource->RequiresVirtualTextureFeedback();
			}
		...
		MaxNumFinalIndices = FMath::Max(MaxNumFinalIndices, FinalIndexData.Num());
	}
}
...
int32 FSlateElementBatcher::BuildShapedTextSequence(const FShapedTextBuildContext& Context)
{
	...
	FColor FontTint = Context.FontTint;
	FColor Tint = FColor::White;

	const UObject* FontMaterial = Context.FontMaterial;
...
#if ENABLE_SDF_DEBUG_VISUAL
	TArray<TPair<FMargin, FColor> > DebugVisualBoxes;

	switch (SdfTextDebugVisual)
	{
		case static_cast<int32>(ESdfTextDebugVisual::Disabled):
		default:
			// No effect
			break;
		case static_cast<int32>(ESdfTextDebugVisual::Raw):
		case static_cast<int32>(ESdfTextDebugVisual::Color):
			FontTint = FColor::White;
				...
				if (FontAtlasTexture == nullptr || NextAtlasDataTextureIndex != FontTextureIndex || bIsSdfGlyph != bPrevSdfGlyph || (bIsSdfGlyph && (NextSdfEmSpread != SdfEmSpread || NextSdfPixelSpread != SdfPixelSpread || NextSdfBias != SdfBias)))
				{
					...
					switch (SdfTextDebugVisual)
					{
						...
						case static_cast<int32>(ESdfTextDebugVisual::Raw):
							if (bIsSdfGlyph)
							{
								ShaderType = ContentType == ESlateFontAtlasContentType::Alpha ? ESlateShader::GrayscaleFont : ESlateShader::ColorFont;
							}
							break;
						case static_cast<int32>(ESdfTextDebugVisual::Color):
							switch (GlyphToRender.FontFaceData->RasterizationMode)
							{
								case EFontRasterizationMode::Bitmap:
								default:
									FontTint = DebugVisualBitmapTextColor;
									break;
								case EFontRasterizationMode::Msdf:
									FontTint = DebugVisualMsdfTextColor;
									break;
								case EFontRasterizationMode::Sdf:
									FontTint = DebugVisualSdfTextColor;
									break;
								case EFontRasterizationMode::SdfApproximation:
									FontTint = DebugVisualApproximateSdfTextColor;
									break;
							}
							break;
					}
					...
					Tint = ContentType == ESlateFontAtlasContentType::Color ? FColor::White : FontTint;
...
#if ENABLE_SDF_DEBUG_VISUAL
			if (SdfTextDebugVisual == static_cast<int32>(ESdfTextDebugVisual::Boxes))
			{
				FColor BoxColor;
				switch (GlyphToRender.FontFaceData->RasterizationMode)
				{
					case EFontRasterizationMode::Bitmap:
					default:
						BoxColor = DebugVisualBitmapTextColor;
						BoxColor.A = 0x60;
						break;
					case EFontRasterizationMode::Msdf:
						BoxColor = DebugVisualMsdfTextColor;
						break;
					case EFontRasterizationMode::Sdf:
						BoxColor = DebugVisualSdfTextColor;
						break;
					case EFontRasterizationMode::SdfApproximation:
						BoxColor = DebugVisualApproximateSdfTextColor;
						break;
				}
				DebugVisualBoxes.Add(TPair<FMargin, FColor>(FMargin(UpperLeft.X, UpperLeft.Y, LowerRight.X, LowerRight.Y), BoxColor));
			}
#endif // ENABLE_SDF_DEBUG_VISUAL
		}
		else if (!bIsVisible)
...
#if ENABLE_SDF_DEBUG_VISUAL
	if (SdfTextDebugVisual == static_cast<int32>(ESdfTextDebugVisual::Boxes) && !DebugVisualBoxes.IsEmpty())
	{
		RenderBatch = &CreateRenderBatch(Context.LayerId,
			FShaderParams(),
			nullptr,
			ESlateDrawPrimitive::LineList,
			ESlateShader::Default,
			Context.DrawElement->GetDrawEffects(),
			ESlateBatchDrawFlag::None,
			*Context.DrawElement);
		RenderBatch->ReserveVertices(6*DebugVisualBoxes.Num());
		RenderBatch->ReserveIndices(8*DebugVisualBoxes.Num());
		for (const TPair<FMargin, FColor>& Box : DebugVisualBoxes)
		{
			FColor TransparentColor = Box.Value;
			TransparentColor.A = 0;
			RenderBatch->AddVertex(FSlateVertex::Make<Rounding>(RenderTransform, FVector2f(Box.Key.Left, .5f*(Box.Key.Top + Box.Key.Bottom)), FVector4f(), FVector2f(0.0f, 0.0f), TransparentColor));
			RenderBatch->AddVertex(FSlateVertex::Make<Rounding>(RenderTransform, FVector2f(Box.Key.Left, Box.Key.Bottom), FVector4f(), FVector2f(0.0f, 0.0f), Box.Value));
			RenderBatch->AddVertex(FSlateVertex::Make<Rounding>(RenderTransform, FVector2f(Box.Key.Right, Box.Key.Bottom), FVector4f(), FVector2f(0.0f, 0.0f), TransparentColor));
			RenderBatch->AddVertex(FSlateVertex::Make<Rounding>(RenderTransform, FVector2f(Box.Key.Right, .5f*(Box.Key.Top + Box.Key.Bottom)), FVector4f(), FVector2f(0.0f, 0.0f), TransparentColor));
			RenderBatch->AddVertex(FSlateVertex::Make<Rounding>(RenderTransform, FVector2f(Box.Key.Right, Box.Key.Top), FVector4f(), FVector2f(0.0f, 0.0f), Box.Value));
			RenderBatch->AddVertex(FSlateVertex::Make<Rounding>(RenderTransform, FVector2f(Box.Key.Left, Box.Key.Top), FVector4f(), FVector2f(0.0f, 0.0f), TransparentColor));
		}
		for (int32 I = 0; I < DebugVisualBoxes.Num(); ++I)
		{
			RenderBatch->AddIndex(static_cast<SlateIndex>(6*I+0));
			RenderBatch->AddIndex(static_cast<SlateIndex>(6*I+1));
			RenderBatch->AddIndex(static_cast<SlateIndex>(6*I+1));
			RenderBatch->AddIndex(static_cast<SlateIndex>(6*I+2));
			RenderBatch->AddIndex(static_cast<SlateIndex>(6*I+3));
			RenderBatch->AddIndex(static_cast<SlateIndex>(6*I+4));
			RenderBatch->AddIndex(static_cast<SlateIndex>(6*I+4));
			RenderBatch->AddIndex(static_cast<SlateIndex>(6*I+5));
		}
	}
#endif // ENABLE_SDF_DEBUG_VISUAL
