...
FVector2f ClosestPointOnSlateRotatedRect(const FVector2f &Point, const FSlateRotatedRect& RotatedRect)
{
	...
	FVector2f Corners[NumOfCorners];
	...
	for (int32 i = 0; i < NumOfCorners; ++i)
	{
		...
		const FVector2f ClosestPoint = FMath::ClosestPointOnSegment2D(Point, Corners[i], Corners[(i + 1) % NumOfCorners]);

		//get the distance between the two
		const float TestDist = FVector2f::DistSquared(Point, ClosestPoint);
