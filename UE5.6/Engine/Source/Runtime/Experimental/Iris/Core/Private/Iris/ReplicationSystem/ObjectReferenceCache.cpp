...
static bool bIrisEnableRemoveReferenceFallbackPath = true;
...
FNetRefHandle FObjectReferenceCache::PreRegisterObjectReferenceHandle(const UObject* Object)
{
	if (IsAuthority())
	{
		FNetObjectReference Ref;
		CreateObjectReferenceInternal(Object, Ref);
		FCachedNetObjectReference* CachedRef = ReferenceHandleToCachedReference.Find(Ref.GetRefHandle());
		if (CachedRef)
		{
			CachedRef->bIsPreRegistered = true;
		}
	...
	{
		UE_LOG_REFERENCECACHE(Error, TEXT("FObjectReferenceCache::PreRegisterObjectReferenceHandle attempted to create new handle for object %s but this is not the reference authority (it's a client)."), *GetFullNameSafe(Object));
		ensureMsgf(false, TEXT("FObjectReferenceCache::PreRegisterObjectReferenceHandle attempted to create new handle without having reference authority."));
	}
...
void FObjectReferenceCache::AddPreRegisteredReference(FNetRefHandle RefHandle, const UObject* Object)
{
	if (!IsAuthority())
	{
		...
		AddRemoteReference(CompleteRefHandle, Object);

		if (FCachedNetObjectReference* CacheObjectPtr = ReferenceHandleToCachedReference.Find(CompleteRefHandle))
		{
			CacheObjectPtr->bIsPreRegistered = true;
		}
	}
	else
	{
		UE_LOG_REFERENCECACHE(Error, TEXT("FObjectReferenceCache::AddPreRegisteredReference attempted to associate handle %s with object %s on the reference authority (server)."), *RefHandle.ToString(), *GetFullNameSafe(Object));
		ensureMsgf(false, TEXT("FObjectReferenceCache::AddPreRegisteredReference attempted to associate a handle with an object on the reference authority."));
	}
}
...
void FObjectReferenceCache::RemoveReference(FNetRefHandle RefHandle, const UObject* Object)
{
	...
	if (ensure(RefHandle.IsDynamic()))
	{
		...
		if (It)
		{
			...
			{
				// RemoveReference can be called multiple times for the same NetRefHandle. The first call will clear out the object references.
				ensureMsgf(CachedItemToRemove.ObjectKey == nullptr || Object == CachedItemToRemove.ObjectKey, TEXT("ObjectReferenceCache::RemoveReference: %s, Object 0x%p doesn't match cached reference 0x%p."), *RefHandle.ToString(), Object, CachedItemToRemove.ObjectKey);
			}
			...
			{
				UE_LOG_REFERENCECACHE(Verbose, TEXT("ObjectReferenceCache::RemoveReference: %s, Object: (0x%p). Destroying cache since Path is invalid (%s)"),
					*RefHandle.ToString(), Object, *CachedItemToRemove.RelativePath.ToString());
				It.RemoveCurrent();
			}
			else
			{
				UE_LOG_REFERENCECACHE(Verbose, TEXT("ObjectReferenceCache::RemoveReference: %s, Object: (0x%p). Clearing cache reference to (0x%p)"),
					*RefHandle.ToString(), Object, CachedItemToRemove.ObjectKey);
		...
		if (Object)
		{
			...
			if (ObjectIt)
			{
				...
				{
					UE_LOG_REFERENCECACHE(Verbose, TEXT("ObjectReferenceCache::RemoveReference: %s removed from ObjectToNetReferenceHandle Object: (0x%p)."), 
						*RefHandle.ToString(), Object);
		...
		else if (bIrisEnableRemoveReferenceFallbackPath)
		{
			...
			for (auto RefHandleIt = ObjectToNetReferenceHandle.CreateIterator(); RefHandleIt; ++RefHandleIt)
			{
				if (RefHandleIt.Value() == RefHandle)
				{
					...
					// It's actually bad if we succesfully remove a reference in this horribly slow fallback path. It means it's still need it and we need to figure out how to get rid of it.
					UE_LOG_REFERENCECACHE(Error, TEXT("ObjectReferenceCache::RemoveReference: %s. Removed RefHandle from ObjectToNetReferenceHandle."), *RefHandle.ToString());
					ensureMsgf(false, TEXT("FObjectReferenceCache::RemoveReference %s: Slow fallback path for null instance is still needed. It needs further investigation."), *RefHandle.ToString());
					break;
				}
			}
...
UObject* FObjectReferenceCache::GetPreRegisteredObjectFromReferenceHandle(FNetRefHandle RefHandle)
{
	FCachedNetObjectReference* CachedObject = ReferenceHandleToCachedReference.Find(RefHandle);

	UObject* ResolvedObject = (CachedObject && CachedObject->bIsPreRegistered) ? CachedObject->Object.Get() : nullptr;
...
bool FObjectReferenceCache::IsNetRefHandlePreRegistered(FNetRefHandle RefHandle) const
{
	const FCachedNetObjectReference* CacheObjectPtr = ReferenceHandleToCachedReference.Find(RefHandle);
	return CacheObjectPtr ? CacheObjectPtr->bIsPreRegistered : false;
}
...
void FObjectReferenceCache::ReadReference(FNetSerializationContext& Context, FNetObjectReference& OutRef)
{
	...
	if (ensureMsgf(NetRefHandle.IsDynamic() || ReferenceHandleToCachedReference.Find(NetRefHandle), TEXT("FObjectReferenceCache::ReadReference: Handle %s must already be known or be dynamic"), *NetRefHandle.ToString()))
	{
...
const TCHAR* FObjectReferenceCache::GetObjectRelativePath(FNetRefHandle NetRefHandle) const
{
	if (const FCachedNetObjectReference* CacheObject = ReferenceHandleToCachedReference.Find(NetRefHandle))
	{
		const TCHAR* ResolvedToken = StringTokenStore->ResolveToken(CacheObject->RelativePath);
