...
public:
	...
	struct FBuildParameters
	{
		...
		EDescriptorType DescriptorType = EDescriptorType::Invalid;
		bool bIsInitState = false;
		bool bAllMembersAreReplicated = false;
		bool bShouldValidateIsFullyPushModel = false;
	};
	...
	{
		StructInfo.SizeAndAlignment.Size = Size;
		StructInfo.SizeAndAlignment.Alignment = Alignment;
	}
	...
	{
		StructInfo.SerializerType = SerializerType;
	}
...
private:
	...
	static uint32 GetRepIndexCount(const TConstArrayView<FMemberProperty>& Members);
	...
	void ValidateIsFullyPushModelClassDescriptor(const FBuilderContext& Context, const FReplicationStateDescriptor* Descriptor);
	...
	static TSet<FName> UniqueClassNames;
};

TSet<FName> FPropertyReplicationStateDescriptorBuilder::UniqueClassNames;
...
bool FPropertyReplicationStateDescriptorBuilder::ValidateStructPropertyDescriptor(const FMemberProperty& MemberProperty, const FReplicationStateDescriptor* Descriptor)
{
	...
	{
		UE_LOG_DESCRIPTORBUILDER_WARNING(TEXT("Found replicated struct %s that might contain members not covered by replication, Struct GetPropertiesSize(),%d accumulated size %" SIZE_T_FMT), *(Struct->GetName()), Struct->GetPropertiesSize(), CurrentExpectedOffset);
		return false;
	}
...
bool FPropertyReplicationStateDescriptorBuilder::ShouldValidateIsFullyPushModel(const UClass* Class)
{
#if UE_BUILD_SHIPPING
	return false;
#else
	// If we have processed this class before there's no need to verify it's push basedness again.
	bool bAlreadyInSet = false;
	UniqueClassNames.Emplace(Class->GetFName(), &bAlreadyInSet);
	if (bAlreadyInSet)
	{
		return false;
	}
	...
	if (ReplicationStateDescriptorConfig->EnsureAllClassesAreFullyPushModel())
	{
		return true;
	}

	if (const FReplicationStateDescriptorClassPushModelConfig* ClassConfig = ReplicationStateDescriptorConfig->GetEnsureFullyPushModelClassNames().FindByPredicate([ClassName = Class->GetFName()](const FReplicationStateDescriptorClassPushModelConfig& ClassConfig) { return ClassConfig.ClassName == ClassName; }))
	{
		return true;
	}
...
void FPropertyReplicationStateDescriptorBuilder::ValidateIsFullyPushModelClassDescriptor(const FBuilderContext& Context, const FReplicationStateDescriptor* Descriptor)
{
	if (EnumHasAnyFlags(Descriptor->Traits, EReplicationStateTraits::HasPushBasedDirtiness))
	{
		return;
	}

	// A class and its descriptors is expected to be validated at most once. We want to report all of the failing descriptors because the user has explicitly requested us to.
	ensureAlwaysMsgf(false, TEXT("ReplicationStateDescriptor %s for class %s does not have the HasPushBasedDirtiness trait. This prevents the class from being fully push based. See log for additional details."),
		ToCStr(Descriptor->DebugName), *WriteToString<128>(Context.BuildParams.PathName ? Context.BuildParams.PathName->ToView() : TEXTVIEW("UNKNOWN")));

	// Try to figure out the cause of the lack of push based trait
	if (!Members.IsEmpty())
	{
		TStringBuilder<2048> Message;
		Message.Appendf(TEXT("Members of ReplicationStateDescriptor %s lacking HasPushBasedDirtiness trait:\n"), ToCStr(Descriptor->DebugName));
		for (const FMemberProperty& Member : Members)
		{
			if (!EnumHasAnyFlags(Member.Traits, EMemberPropertyTraits::HasPushBasedDirtiness))
			{
				if (Member.Property)
				{
					Message.Appendf(TEXT("Member property name: %s\n"), ToCStr(Member.Property->GetName()));
				}
				else if (const FReplicationStateMemberDebugDescriptor* DebugDescriptor = Descriptor->MemberDebugDescriptors)
				{
					const SIZE_T MemberIndex = &Member - Members.GetData();
					Message.Appendf(TEXT("Member debug name: %s\n"), ToCStr(DebugDescriptor[MemberIndex].DebugName));
				}
				else
				{
					Message.Appendf(TEXT("Member index: %u\n"), unsigned(&Member - Members.GetData()));
				}
			}
		}
		UE_LOG_DESCRIPTORBUILDER_WARNING(TEXT("%s"), Message.ToString());
	}
	else
	{
		UE_LOG_DESCRIPTORBUILDER_WARNING(TEXT("ReplicationStateDescriptor %s for class %s does not have the HasPushBasedDirtiness trait. No additional info."),
			ToCStr(Descriptor->DebugName), *WriteToString<128>(Context.BuildParams.PathName ? Context.BuildParams.PathName->ToView() : TEXTVIEW("")));
	}
}
...
TRefCountPtr<const FReplicationStateDescriptor> FPropertyReplicationStateDescriptorBuilder::CreateDescriptorForProperty(const FString& DescriptorName, const FProperty* Property, const FReplicationStateDescriptorBuilder::FParameters& Parameters) const
{
	...
	{	
		if (const FReplicationStateDescriptorRegistry::FDescriptors* Result = Parameters.DescriptorRegistry->Find(Property))
		{
	...
	TRefCountPtr<const FReplicationStateDescriptor> Descriptor = Builder.Build(DescriptorName, Parameters.DescriptorRegistry, BuildParameters);
...
void FPropertyReplicationStateDescriptorBuilder::BuildMemberCache(FBuilderContext& Context, FReplicationStateDescriptorRegistry* DescriptorRegistry)
{
	...
	bool bAllMembersAreReplicated = Context.BuildParams.bAllMembersAreReplicated;
	...
	for (FMemberProperty& Member : Members)
	{
		...
		// Disqualify any member having a serializer with an Apply from being considered as AllMembersAreReplicated as we need to be sure Apply is called as expected.
		if (EnumHasAnyFlags(Serializer->Traits, ENetSerializerTraits::HasApply))
		{
	...
	// 
	Context.BuildParams.bAllMembersAreReplicated = bAllMembersAreReplicated;
}
...
void FPropertyReplicationStateDescriptorBuilder::BuildMemberRepIndexToMemberDescriptors(FReplicationStateMemberRepIndexToMemberIndexDescriptor* MemberRepIndexToMemberIndexDescriptors, FReplicationStateDescriptor* Descriptor) const
{
	...
	for (const FMemberProperty& Info : Members)
	{
		ArrayIndex = (LastProperty && Info.Property == LastProperty ? uint16(ArrayIndex + 1) : uint16(0));
		LastProperty = Info.Property;
		CA_ASSUME(Info.Property != nullptr);

		const SIZE_T MemberIndex = &Info - Members.GetData();
		const uint16 RepIndex = Info.Property->RepIndex + ArrayIndex;
...
void FPropertyReplicationStateDescriptorBuilder::BuildMemberDebugDescriptors(FReplicationStateMemberDebugDescriptor* MemberDebugDescriptors, FReplicationStateDescriptor* Descriptor, const FBuilderContext& Context) const
{
	...
	TCHAR StringBuffer[FName::StringBufferSize];
	...
	for (const FMemberProperty& Info : Members)
	{
		...
		{
			Property->GetFName().ToString(StringBuffer);
...
EReplicationStateTraits FPropertyReplicationStateDescriptorBuilder::BuildReplicationStateTraits(const FBuilderContext& Context) const
{
	...
	// Struct traits
	if (Context.BuildParams.DescriptorType == EDescriptorType::Struct)
	{
		if (StructInfo.SerializerType == EStructNetSerializerType::DerivedFromCustom)
		{
			Traits |= EReplicationStateTraits::IsDerivedFromStructWithCustomSerializer;
		}
		else if (StructInfo.SerializerType == EStructNetSerializerType::Custom)
		{
			Traits |= EReplicationStateTraits::IsStructWithCustomSerializer;
		}
	}
	...
	// For the main replication state for functions we do EReplicationStateTraits::HasPushBasedDirtiness even though it does not have any members.
	if (Context.MemberFunctionCache.Num() && Members.Num() == 0)
	{
		Traits |= EReplicationStateTraits::HasPushBasedDirtiness;
	}
...
FPropertyReplicationStateDescriptorBuilder::Build(const FString& StateName, FReplicationStateDescriptorRegistry* DescriptorRegistry, const FBuildParameters& BuildParams)
{
	...
	if (Buffer)
	{
		...
		// BaseStruct
		if (BuildParams.DescriptorType == EDescriptorType::Struct)
		{
			if (StructInfo.SerializerType == EStructNetSerializerType::DerivedFromCustom)
			{
			...
			{
				Descriptor->BaseStruct = Cast<const UScriptStruct>(StructInfo.Struct);
				ensure(Descriptor->BaseStruct);
			}
		}
		...
		// Append MemberCount, FunctionCount and ChangeMaskBitcount to get some validation of the descriptor.
		UpperCaseNameForHash.Appendf(TEXT("%u%u%u"), Descriptor->MemberCount, Descriptor->FunctionCount, Descriptor->ChangeMaskBitCount);
		...
		UE_LOG_DESCRIPTORBUILDER(Verbose, TEXT("Generated identifier(0x%" UINT64_x_FMT ") From %s:%s"), Descriptor->DescriptorIdentifier.Value, *WriteToString<128>(PathLen ? BuildParams.PathName->ToView() : TEXTVIEW("")), *StateName);
		...
		if (Descriptor->InternalSize && BuildParams.DefaultStateSourceData)
		{
			...
			if (CalculateDefaultStateChecksum(Descriptor, BuildParams.ReplicationSystem, Descriptor->DefaultStateBuffer, DefaultStateHash))
			{
				...
				UE_LOG_DESCRIPTORBUILDER(Verbose, TEXT("DefaultStateHash for identifier (0x%" UINT64_x_FMT ") is 0x%" UINT64_x_FMT), Descriptor->DescriptorIdentifier.Value, Descriptor->DescriptorIdentifier.DefaultStateHash);
			}
		}

		if (Context.BuildParams.bShouldValidateIsFullyPushModel)
		{
			ValidateIsFullyPushModelClassDescriptor(Context, Descriptor);
		}
...
uint32 FPropertyReplicationStateDescriptorBuilder::GetRepIndexCount(const TConstArrayView<FMemberProperty>& Members)
{
	uint16 MaxRepIndex = 0;
	{
		...
		for (const FMemberProperty& Member : Members)
		{
			ArrayIndex = (LastProperty && Member.Property == LastProperty ? uint16(ArrayIndex + 1) : uint16(0));
			LastProperty = Member.Property;
			CA_ASSUME(Member.Property != nullptr);
			MaxRepIndex = FPlatformMath::Max(MaxRepIndex, static_cast<uint16>(Member.Property->RepIndex + ArrayIndex));
		}
	}
...
TRefCountPtr<const FReplicationStateDescriptor> FReplicationStateDescriptorBuilder::CreateDescriptorForStruct(const UStruct* InStruct, const FParameters& Parameters)
{
	...
	if (Parameters.DescriptorRegistry)
	{	
		if (const FReplicationStateDescriptorRegistry::FDescriptors* Result = Parameters.DescriptorRegistry->Find(InStruct))
		{
			// For structs we do only expect a single entry
			ensure(Result->Num() == 1);
	...
	Builder.SetStructSerializerType(SerializerType);
...
TRefCountPtr<const FReplicationStateDescriptor> FReplicationStateDescriptorBuilder::CreateDescriptorForFunction(const UFunction* Function, const FParameters& Parameters)
{
	...
	if (Parameters.DescriptorRegistry)
	{	
		if (const FReplicationStateDescriptorRegistry::FDescriptors* Result = Parameters.DescriptorRegistry->Find(Function))
		{
	...
	BuildParameters.bAllMembersAreReplicated = true;
	TRefCountPtr<const FReplicationStateDescriptor> Descriptor = Builder.Build(Function->GetName(), Parameters.DescriptorRegistry, BuildParameters);
...
SIZE_T FReplicationStateDescriptorBuilder::CreateDescriptorsForClass(FResult& CreatedDescriptors, UClass* InObjectClass, const FParameters& Parameters)
{
	...
	{
		UE_LOG(LogIris, Error, TEXT("Trying to create descriptor for invalid class %s"), *GetNameSafe(InObjectClass));
		return 0U;
	}
	...
	if (Parameters.DescriptorRegistry)
	{
		...
		{
			UE_LOG_DESCRIPTORBUILDER(VeryVerbose, TEXT("Found existing descriptors for Class %s : ArcheTypeOrKey: %s ptr: 0x%p"), *InObjectClass->GetName(), *ObjectClassOrArchetypeUsedAsKey->GetPathName(), ObjectClassOrArchetypeUsedAsKey);
	...
	if (bGetLifeTimeProperties)
	{
		...
		// Keep track of non user registered properties
		FNetBitArray RegisteredProperty(InObjectClass->ClassReps.Num());
		...
		LifeTimeProperties.SetNum(InObjectClass->ClassReps.Num());
		for (const FLifetimeProperty& LifetimeProperty : LifeTimePropertiesOriginalOrder)
		{
			...
			RegisteredProperty.SetBit(LifetimeProperty.RepIndex);
		}

		RegisteredProperty.ForAllUnsetBits([&LifeTimeProperties, InObjectClass](uint32 Index)
		{
			LifeTimeProperties[Index].RepIndex = static_cast<uint16>(Index);

			if (UE::Net::bAutoRegisterReplicatedProperties == false)
			{
				// Never replicate this property when auto-registration is disabled
				LifeTimeProperties[Index].Condition = COND_Never;

				if (UE::Net::bEnsureForMissingProperties)
				{
					ensureMsgf(false, TEXT("Property %s::%s (SourceClass: %s) was not registered in GetLifetimeReplicatedProps. This property will not be replicated. Use DISABLE_REPLICATED_PROPERTY if not replicating was intentional."), 
							   *GetNameSafe(InObjectClass), *GetNameSafe(InObjectClass->ClassReps[Index].Property), *GetNameSafe(InObjectClass->ClassReps[Index].Property->GetOwnerClass()));
				}
			}
		});
	}
	...
	for (const FRepRecord& RepRecord : InObjectClass->ClassReps)
	{
		...
		const int32 RepIndex = static_cast<int32>(Property->RepIndex);
	
		// If we are building a class descriptor for a single property we only include the property matching the index
		if (bSingleProperty && (RepIndex != Parameters.SinglePropertyIndex))
		{
			continue;
		}
	...
	// Add special Actor properties
	{
		...
		if (InObjectClass->IsChildOf(ActorClass))
		{
			...
			{
				UE_LOG(LogIris, Error, TEXT("FPropertyReplicationStateDescriptorBuilder: Failed to find expected property %s.%s"), ToCStr(InObjectClass->GetName()), *FName(ReplicationStateDescriptorBuilder_NAME_NetCullDistanceSquared).ToString());
				ensure(false);
			}
		}
	}
	...
	if (!bSingleProperty)
	{
		...
		{
			if (!ensureMsgf(ClassIndex > 0, TEXT("Class hieararchy depth exceeds %" SIZE_T_FMT), MaxClassHierarchyDepth))
			{
				break;
			}
	...
	const bool bShouldValidateIsFullyPushModel = FPropertyReplicationStateDescriptorBuilder::ShouldValidateIsFullyPushModel(InObjectClass);
	...
	for (uint32 BuilderTypeIndex = 0; BuilderTypeIndex < BuilderTypeCount; ++BuilderTypeIndex)
	{
		if (Builders[BuilderTypeIndex].HasDataToBuild())
		{
			...
			BuildParameters.bShouldValidateIsFullyPushModel = bShouldValidateIsFullyPushModel;
	...
	{
		// If bReplicateCustomDeltaPropertiesInRepIndexOrder is false then we use the GetLifetimeReplicatedPropsOrder instead, which is the RepLayout legacy behavior.
		if (!UE::Net::bReplicateCustomDeltaPropertiesInRepIndexOrder)
		{
	...
	{
		UE_LOG_DESCRIPTORBUILDER(VeryVerbose, TEXT("Registering descriptors for  Class %s : ArcheTypeOrKey: %s ptr: 0x%p"), *InObjectClass->GetName(), *ObjectClassOrArchetypeUsedAsKey->GetPathName(), ObjectClassOrArchetypeUsedAsKey);
