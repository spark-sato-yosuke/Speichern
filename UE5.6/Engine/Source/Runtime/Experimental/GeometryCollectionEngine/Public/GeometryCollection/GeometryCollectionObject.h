...
struct FGeometryCollectionProxyMeshData
{
	...
	/** 
	* Transform of the mesh relative to the geometry collection it belongs to 
	...
	*/
	...
	TArray<FTransform3f> MeshTransforms;

	GEOMETRYCOLLECTIONENGINE_API const FTransform3f& GetMeshTransform(int32 MeshIndex) const;
};
...
public:
	...
	GEOMETRYCOLLECTIONENGINE_API virtual bool IsReadyForFinishDestroy() override;
	...
	*/
	...
	/** Create root proxies from geometryt sources */
	GEOMETRYCOLLECTIONENGINE_API void SetRootProxiesFromGeometrySources();
	...
	*/
	...
	/** When using remove on sleep , automatically crumble partial cluster into smaller pieces when possible */
	UPROPERTY(EditAnywhere, BlueprintReadOnly, Category = Removal, meta = (EditCondition = "bRemoveOnMaxSleep"))
	bool bAutomaticCrumblePartialClusters;
	...
	*/
	...
	FString DataflowTerminal_DEPRECATED = "GeometryCollectionTerminal";
#endif

	UPROPERTY(EditAnywhere, BlueprintReadWrite, Category = "Dataflow", DisplayName = "DataFlow Overrides", meta = (DisplayAfter = "DataflowInstance"))
	...
	//~ Begin IDataflowGeneratedAssetInterface
	virtual const FDataflowInstance& GetDataflowInstance() const override { return DataflowInstance; };
	virtual FDataflowInstance& GetDataflowInstance() override { return DataflowInstance; };
	//~ End IDataflowGeneratedAssetInterface
	...
	const TArray<int32>& GetBreadthFirstTransformIndices() const { return BreadthFirstTransformIndices; }

	const TArray<int32>& GetAutoInstanceTransformRemapIndices() const { return AutoInstanceTransformRemapIndices; }
...
private:
	...
	/** A fence to track render resource release before destroy. */
	FRenderCommandFence RenderResourceReleaseFence;

	float GetMassOrDensityInternal(bool& bOutIsDensity, bool bCached) const;

	void MigrateDeprecatedRootProxyData();
