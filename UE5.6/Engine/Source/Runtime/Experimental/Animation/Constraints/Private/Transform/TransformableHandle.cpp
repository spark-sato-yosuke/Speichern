...
void UTransformableComponentHandle::TickTarget() const
{
	...
	if (TransformableHandleUtils::SkipTicking())
	{
		...
		{
			TransformableHandleUtils::EvaluateComponent(Component.Get()); 
		}
	}
	else
	{
...
void UTransformableComponentHandle::PreEvaluate(const bool bTickSkeletalMeshes) const
{
	if (!Component.IsValid())
	{
		return;
...
FTransform UTransformableComponentHandle::GetGlobalTransform() const
{
	...
	using namespace TransformableHandleUtils;
	if (SkipTicking())
	{
		const UE::Anim::FAnimationEvaluator& AnimationEvaluator = EvaluateComponent(Component.Get());
		if (AnimationEvaluator.IsValid())
		{
			return AnimationEvaluator.GetGlobalTransform(SocketName);
		}
	}
...
FTickPrerequisite LookForPrimaryPrerequisite(USceneComponent* Component, const bool bAllowThis)
{
	...
	if (IsValidTickFunction(&Component->PrimaryComponentTick) && bAllowThis)
	{
	...
	if (USceneComponent* Parent = Component->GetAttachParent())
	{
		...
		constexpr bool bAllowTickFromParent = true;
		return LookForPrimaryPrerequisite(Parent, bAllowTickFromParent);
	}
...
FTickPrerequisite UTransformableComponentHandle::GetPrimaryPrerequisite(const bool bAllowThis) const
{
	if (Component.IsValid())
	{
		return LookForPrimaryPrerequisite(Component.Get(), bAllowThis);
	}
...
void UTransformableComponentHandle::ResolveBoundObjects(FMovieSceneSequenceID LocalSequenceID, TSharedRef<UE::MovieScene::FSharedPlaybackState> SharedPlaybackState, UObject* SubObject)
{
	...
	auto ReplaceComponent = [this](USceneComponent* InNewComponent)
	{
		if (InNewComponent != Component)
		{
	...
	if (auto BoundObjectsView = ConstraintBindingID.ResolveBoundObjects(LocalSequenceID, SharedPlaybackState); BoundObjectsView.Num() > 0)
	{
		...
		if (const AActor* Actor = Cast<AActor>(ParentObject.Get()))
		{
			...
			ReplaceComponent(Index != INDEX_NONE ? Components[Index] : Actor->GetRootComponent());
		}
