...
public:
		...
		const TMap<FString, TConstArrayView<int32>>& FaceIntMaps)
	{
		...
		{
			KinematicCollisions.SetColliderThickness(FMath::Max(GetSelfCollisionKinematicColliderThickness(PropertyCollection), 0.f));
		}
		if (IsSelfCollisionKinematicColliderStiffnessMutable(PropertyCollection))
		{
			KinematicCollisions.SetStiffness(FMath::Clamp((FSolverReal)GetSelfCollisionKinematicColliderStiffness(PropertyCollection), 0.f, 1.f));
		}
