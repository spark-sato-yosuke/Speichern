...
namespace Chaos
{
	...
	public:
		...
		*/
		...
		int32 GenerateCollision(const FReal CullDistance, const FReal Dt, const FCollisionContext& Context) { return GenerateCollision(FRealSingle(Dt), FRealSingle(CullDistance), FVec3f(0), Context); }
		...
		*/
		UE_DEPRECATED(5.4, "Use single precision version")
		int32 GenerateCollisionCCD( const bool bEnableCCDSweep, const FReal CullDistance, const FReal Dt, const FCollisionContext& Context) { return GenerateCollisionCCD(FRealSingle(Dt), FRealSingle(CullDistance), FVec3f(0), bEnableCCDSweep, Context); }
	...
	public:
		...
		*/
		 ...
		 * @param Visitor functor with signature ECollisionVisitorResult(FPBDCollisionConstraint& Constraint)
		*/
		...
		/**
		 * @brief Call a lambda on each active collision constraint
		 * @param Visitor functor with signature ECollisionVisitorResult(const FPBDCollisionConstraint& Constraint)
		 */
		template<typename TLambda>
		...
		*/
		...
		UE_DEPRECATED(5.6, "Use SetConvexOptimizationIsActive which takes an OnDestroyingCollision functor")
		void SetConvexOptimizationIsActive(bool bSetConvexOptimizationIsActive)
		{
			SetConvexOptimizationIsActive(bSetConvexOptimizationIsActive, [](FPBDCollisionConstraint*) {});
		}


		/**
		 * @brief Override whether convex optimization if used this mid-phase. Used by the MidPhase modifier and gets reset every frame.
		 * NOTE: when this setting is changed, collisions will be destroyed so any external systems referencing them will need to 
		 * drop their references.
		 */
		CHAOS_API virtual void SetConvexOptimizationIsActive(bool bSetConvexOptimizationIsActive, TFunctionRef<void(FPBDCollisionConstraint*)> OnDestroyingCollision) {}
		...
		*/
		...
		union FFlags
		{
			...
			struct
			{
				...
				uint16 bIsConvexOptimizationUsed : 1;   // True if we generated collisions with convex optimization active (used to detect changes to bIsConvexOptimizationActive in modifiers). See FGenericParticlePairMidPhase::GenerateCollisionsImpl
				uint16 bIsSleeping : 1;  // True is the particles in the midphase are sleeping
	...
	private:
		...
		CHAOS_API virtual void SetConvexOptimizationIsActive(bool bSetConvexOptimizationIsActive, TFunctionRef<void(FPBDCollisionConstraint*)> OnDestroyingCollision) override final;
