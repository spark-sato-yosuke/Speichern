...
private:
	...
	template <typename T>
	T* NewDataHelper(TArray<TUniquePtr<T>>& Backing, TSpscQueue<T*>& Queue)
	{
		T* Result;
	...
	TSpscQueue<TInputType*> InputPool;
	TArray<TUniquePtr<TInputType>> InputBacking;

	TSpscQueue<TOutputType*> OutputPool;
	TArray<TUniquePtr<TOutputType>> OutputBacking;
	TSpscQueue<TOutputType*> OutputQueue;	//holds the outputs in order
