...
namespace Chaos::Private
{
	...
	*/
	...
	private:
		...
		// Partial wake thresholds
		FRealSingle WakeLinearThresholdSq = 0;
		FRealSingle WakeAngularThresholdSq = 0;
	...
	*/
	...
	private:
		...
		CHAOS_API void RandomizeIslandEdges(FPBDIsland* Island);
		...
		// Partial Island Sleeping
		CHAOS_API void ProcessPartialIslandSleep(FPBDIsland* Island, const FRealSingle Dt);
		CHAOS_API void SleepNonMovingParticles(FPBDIsland* Island, const FRealSingle Dt);
		CHAOS_API void PropagateParticleSleepToConstraints(FPBDIsland* Island);

		// Partial Island Waking
		CHAOS_API void ProcessPartialIslandWake(FPBDIsland* Island);
		CHAOS_API bool PropagatePartialIslandWake(TArray<FPBDIslandParticle*>& NodeQueue, TArray<FGeometryParticleHandle*>& ParticlesToWakeUp, const int32 NumNodesConstraintWake, const bool bTriggerNodesAlwaysPropagate);
		CHAOS_API bool ProcessTriggerParticle(FPBDIslandParticle* Node, TArray<FPBDIslandParticle*>& NodeQueue, const bool bAlwaysTrigger);
		CHAOS_API bool ProcessParticleWake(FPBDIslandParticle* Node, const FPBDIslandParticle* OtherNode, TArray<FPBDIslandParticle*>& NodeQueue, TArray<FGeometryParticleHandle*>& WakeUpQueue, const bool bAlwaysPropagate);
		CHAOS_API void SyncConstraintSleepStateToParticles(FPBDIsland* Island);
		CHAOS_API void WakeUpParticles(TArray<FGeometryParticleHandle*>& ParticlesToWake, const bool bResetSleepCounter = true);
		CHAOS_API void ResetParticleSleepCounter(FPBDIsland* Island);
		CHAOS_API void ResetEdgeFlags(FPBDIsland* Island);
		CHAOS_API void ResetNodeFlags(FPBDIsland* Island);
		...
		CHAOS_API void UpdateAllParticleSleepMetrics(const FRealSingle Dt);

		CHAOS_API int32 GetNextVisitEpoch();
		CHAOS_API bool ShouldAssignLevels() const;
