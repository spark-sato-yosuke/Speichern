...
public:
	...
	UE_DEPRECATED(5.6, "No longer supported, use new constructors instead")
	CHAOS_API FXPBDAnisotropicEdgeSpringConstraints(
		const FSolverParticlesRange& Particles,
		const FTriangleMesh& TriangleMesh,
		const TArray<TVec3<FVec2f>>& FaceVertexPatternPositions,
...
private:
	...
	void InitFromPatternData(int32 ParticlesSize, bool bUse3dRestLengths, const TArray<TVec3<FVec2f>>& FaceVertexPatternPositions, const FTriangleMesh& TriangleMesh, bool bSavePreResizedSimPosition3D, const TArray<FVector3f>& PreResizedSimPosition3D, FSolverReal MeshScale);
...
public:
	...
	FXPBDAnisotropicSpringConstraints(
		...
		: EdgeConstraints(
			...
			GetSavePreResizedSimPosition3D(PropertyCollection, bDefaultSavePreResizedSimPosition3D),
		...
		, SavePreResizedSimPosition3DIndex(PropertyCollection)
	{}
...
private:
	...
	UE_CHAOS_DECLARE_PROPERTYCOLLECTION_NAME(SavePreResizedSimPosition3D, bool);
};
