...
class IPhysicsReplicationAsync;
...
namespace Chaos
{
	...
	public:
		...
		{
			MarshallingManager.RemoveDirtyProxyFromHistory_Internal(ProxyBaseIn);
		}
		...
		inline TSimCallbackObjectType* CreateAndRegisterSimCallbackObject_External(ConstructorArgTypes&&... ConstructorArgs)
		{
			auto NewCallbackObject = new TSimCallbackObjectType(Forward<ConstructorArgTypes>(ConstructorArgs)...);
		...
		* and particles that are in sync doesn't need collision data since we just step through their states in history." */
		...
		UE_DEPRECATED(5.6, "Use SetPhysicsReplication_Internal")
			void SetPhysicsReplication(FPhysicsReplicationAsync* InPhysicsReplication)
		{
			PhysicsReplication_DEPRECATED = InPhysicsReplication;
		}

		UE_DEPRECATED(5.6, "Use GetPhysicsReplication_Internal which return the interface instead of the implementation")
			FPhysicsReplicationAsync* GetPhysicsReplication()
		{
			return PhysicsReplication_DEPRECATED;
		}

		void SetPhysicsReplication_Internal(IPhysicsReplicationAsync* InPhysicsReplication)
		{
			PhysicsReplicationAsync = InPhysicsReplication;
		}

		IPhysicsReplicationAsync* GetPhysicsReplication_Internal()
		{
			return PhysicsReplicationAsync;
		}

		void SetPhysicsReplicationLOD_Internal(IPhysicsReplicationLODAsync* InPhysicsReplicationLOD)
		{
			PhysicsReplicationLODAsync = InPhysicsReplicationLOD;
		}
	...
	private:
		...
		/** Scene lock object for internal threads */
		TUniquePtr<FPhysSceneLock> InternalDataLock;
#endif
...
#if UE_WITH_REMOTE_OBJECT_HANDLE
		UE_INTERNAL FPhysSceneLock& GetInternalDataLock() const
		{
			return *InternalDataLock;
		}
#endif
