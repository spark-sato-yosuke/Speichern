...
namespace Chaos
{
		...
		const FConstraintSolverBody& Body1)
	{
		...
		const FReal ContactWeight0 = Body0.InvM();
		const FReal ContactWeight1 = Body1.InvM();
		...
		const FVec3 WorldContact = (ContactWeight0 * WorldContact0 + ContactWeight1 * WorldContact1) / (ContactWeight0 + ContactWeight1);
	...
	void FPBDCollisionContainerSolver::AddConstraints(const TArrayView<Private::FPBDIslandConstraint*>& IslandConstraints)
	{
		for (Private::FPBDIslandConstraint* IslandConstraint : IslandConstraints)
		{
			...
			if (!Constraint->IsSleeping())
			{
				...
				FPBDCollisionConstraint& CollisionConstraint = Constraint->AsUnsafe<FPBDCollisionConstraintHandle>()->GetContact();

				AddConstraint(CollisionConstraint);
			}
		}
	}
