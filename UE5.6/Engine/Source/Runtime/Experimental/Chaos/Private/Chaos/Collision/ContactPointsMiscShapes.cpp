...
namespace Chaos
{
	...
	FContactPoint SphereSphereContactPoint(const FSphere& Sphere1, const FRigidTransform3& Sphere1Transform, const FSphere& Sphere2, const FRigidTransform3& Sphere2Transform, const FRealSingle CullDistance)
	{
		...
		const FRealSingle R1 = Sphere1.GetRadiusf();
		const FRealSingle R2 = Sphere2.GetRadiusf();

		const FRigidTransform3f Sphere1ToSphere2f = FRigidTransform3f(Sphere1Transform.GetRelativeTransformNoScale(Sphere2Transform));

		const FVec3f Center1 = Sphere1ToSphere2f.TransformPosition(Sphere1.GetCenterf());
		const FVec3f Center2 = Sphere2.GetCenterf();
		const FVec3f Direction = Center1 - Center2;
		const FRealSingle SizeSq = Direction.SizeSquared();
		const FRealSingle CullDistanceSq = FMath::Square(CullDistance + R1 + R2);
		if (SizeSq < CullDistanceSq)
		{
			const FRealSingle Size = FMath::Sqrt(SizeSq);
			const FVec3f Normal = Size > UE_KINDA_SMALL_NUMBER ? Direction / Size : FVec3f(0, 0, 1);
			const FRealSingle NewPhi = Size - (R1 + R2);

			Result.ShapeContactPoints[0] = Sphere1.GetCenterf() - Sphere1ToSphere2f.InverseTransformVector(R1 * Normal);
			Result.ShapeContactPoints[1] = Sphere2.GetCenterf() + R2 * Normal;
			Result.ShapeContactNormal = Normal;
			Result.Phi = NewPhi;
		}
		return Result;
	}
	...
	FContactPoint SpherePlaneContactPoint(const FSphere& Sphere, const FRigidTransform3& SphereTransform, const TPlane<FReal, 3>& Plane, const FRigidTransform3& PlaneTransform)
	{
		...
		FReal SphereRadius = Sphere.GetRadiusf();

		FVec3 SpherePosWorld = SphereTransform.TransformPosition(FVec3(Sphere.GetCenterf()));
	...
	FContactPoint SphereBoxContactPoint(const FSphere& Sphere, const FRigidTransform3& SphereTransform, const FImplicitBox3& Box, const FRigidTransform3& BoxTransform)
	{
		...
		const FVec3 SphereWorld = SphereTransform.TransformPosition(FVec3(Sphere.GetCenterf()));	// World-space sphere pos
		...
		FReal Phi = PhiToSphereCenter - Sphere.GetRadiusf();

		FVec3 NormalWorld = BoxTransform.TransformVectorNoScale(NormalBox);
		FVec3 LocationWorld = SphereWorld - (Sphere.GetRadiusf()) * NormalWorld;
	...
	FContactPoint SphereCapsuleContactPoint(const FSphere& A, const FRigidTransform3& ATransform, const FCapsule& B, const FRigidTransform3& BTransform)
	{
		...
		FVector A1 = ATransform.TransformPosition(FVector(A.GetCenterf()));
		FVector B1 = BTransform.TransformPosition(FVector(B.GetX1f()));
		FVector B2 = BTransform.TransformPosition(FVector(B.GetX2f()));
		...
		{
			FReal NewPhi = DeltaLen - (A.GetRadiusf() + B.GetRadiusf());
			FVec3 Dir = Delta / DeltaLen;
			FVec3 LocationA = A1 + Dir * A.GetRadiusf();
			FVec3 LocationB = P2 - Dir * B.GetRadiusf();
			FVec3 Normal = -Dir;
	...
	{
		return GJKImplicitContactPoint<FSphere>(FSphere(A), ATransform, B, BTransform, CullDistance);
	}
	...
	FContactPoint SphereTriangleMeshSweptContactPoint(const FSphere& A, const FRigidTransform3& ATransform, const TriMeshType& B, const FRigidTransform3& BStartTransform, const FVec3& Dir, const FReal Length, const FReal IgnorePenetration, const FReal TargetPenetration, FReal& TOI)
	{
		...
		{
			return GJKImplicitScaledTriMeshSweptContactPoint<FSphere>(A, ATransform, *ScaledTriangleMesh, BStartTransform, Dir, Length, IgnorePenetration, TargetPenetration, TOI);
		}
		else if (const FTriangleMeshImplicitObject* TriangleMesh = B.template GetObject<const FTriangleMeshImplicitObject>())
		{
			return GJKImplicitSweptContactPoint(FSphere(A), ATransform, *TriangleMesh, BStartTransform, Dir, Length, IgnorePenetration, TargetPenetration, TOI);
		}
	...
	{
		FContactPointf Result;

		const FRigidTransform3f CapsuleAToCapsuleBf = FRigidTransform3f(ATransform.GetRelativeTransformNoScale(BTransform));

		const FVector3f A1 = CapsuleAToCapsuleBf.TransformPosition(A.GetX1f());
		const FVector3f A2 = CapsuleAToCapsuleBf.TransformPosition(A.GetX2f());
		const FVector3f B1 = B.GetX1f();
		const FVector3f B2 = B.GetX2f();
		FVector3f P1, P2;
		FMath::SegmentDistToSegmentSafe(A1, A2, B1, B2, P1, P2);

		const FVec3f Delta = P2 - P1;
		const FRealSingle DeltaLen = Delta.Size();
		if (DeltaLen > UE_KINDA_SMALL_NUMBER)
		{
			const FRealSingle NewPhi = DeltaLen - (A.GetRadiusf() + B.GetRadiusf());
			const FVec3f Dir = Delta / DeltaLen;
			const FVec3f Normal = -Dir;
			const FVec3f LocationA = P1 + Dir * A.GetRadiusf();
			const FVec3f LocationB = P2 - Dir * B.GetRadiusf();

			Result.ShapeContactPoints[0] = CapsuleAToCapsuleBf.InverseTransformPosition(LocationA);
			Result.ShapeContactPoints[1] = LocationB;
			Result.ShapeContactNormal = Normal;
			Result.Phi = NewPhi;
		}
