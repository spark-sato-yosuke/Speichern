...
namespace Chaos
{
	...
	inline FImplicitObject* GetInstancedImplicitHelper(FImplicitObject* Implicit0)
	{
		...
		else if (Implicit0OuterType == TImplicitObjectInstanced<FSphere>::StaticType())
		{
			return const_cast<FSphere*>(Implicit0->template GetObject<TImplicitObjectInstanced<FSphere>>()->GetInstancedObject());
		}
	...
	{
		if (FImplicitObjectRef CurrentGeometry = GetGeometry())
		{
			if (FImplicitObjectUnion* Union = CurrentGeometry->AsA<FImplicitObjectUnion>())
			{
	...
	void Chaos::TGeometryParticle<T, d>::PrepareBVHImpl()
	{
		...
		{
			if (FImplicitObjectRef Implicit = MNonFrequentData.Read().GetGeometry())
			{
				if (const FImplicitObjectUnion* Union = Implicit->AsA<FImplicitObjectUnion>())
				{
	...
	void Chaos::TGeometryParticle<T, d>::LockGeometry()
	{
		if (MNonFrequentData.IsDirty(MDirtyFlags))
		{
			if (FImplicitObjectRef Implicit = MNonFrequentData.Read().GetGeometry())
			{
				if (FImplicitObjectUnion* Union = Implicit->AsA<FImplicitObjectUnion>())
				{
					Union->SetLockedRecursive(true);
				}
			}
