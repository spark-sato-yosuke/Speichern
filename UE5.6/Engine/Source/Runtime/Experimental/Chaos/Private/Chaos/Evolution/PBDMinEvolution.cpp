...
namespace Chaos
{
	...
	void FPBDMinEvolution::Rewind(FReal Dt, FReal RewindDt)
	{
		...
		for (TTransientPBDRigidParticleHandle<FReal, 3>& Particle : Particles.GetActiveParticlesView())
		{
			...
			else if (Particle.ObjectState() == EObjectStateType::Kinematic)
			{
				// We can't rewind velocities for kinematics, and anyway they will get recalculated
				// anyway when the kinematic targets are applied.
	...
	void FPBDMinEvolution::Integrate(FReal Dt)
	{
		...
		for (TTransientPBDRigidParticleHandle<FReal, 3>& Particle : Particles.GetActiveParticlesView())
		{
			if (Particle.ObjectState() == EObjectStateType::Dynamic)
			{
				...
				if (SimulationSpaceSettings.bEnabled)
				{
					const FVec3 WxR = FVec3::CrossProduct(SimulationSpace.AngularVelocity, XCoM);
					const FVec3 WxWxR = FVec3::CrossProduct(SimulationSpace.AngularVelocity, WxR);
					const FVec3 WxV = FVec3::CrossProduct(SimulationSpace.AngularVelocity, V);
					const FVec3 AxR = FVec3::CrossProduct(SimulationSpace.AngularAcceleration, XCoM);

					// Calculate the phantom forces
					const FVec3 CoriolisAcc = SimulationSpaceSettings.CoriolisAlpha * 2.0f * WxV;
					const FVec3 CentrifugalAcc = SimulationSpaceSettings.CentrifugalAlpha * WxWxR;
					const FVec3 EulerAcc = SimulationSpaceSettings.EulerAlpha * AxR;
					const FVec3 LinearAcc = SimulationSpaceSettings.LinearAccelerationAlpha * SimulationSpace.LinearAcceleration;
					const FVec3 AngularAcc = SimulationSpaceSettings.AngularAccelerationAlpha * SimulationSpace.AngularAcceleration;
					const FVec3 LinearDragAcc = SimulationSpaceSettings.ExternalLinearEtherDrag * (V + SimulationSpace.LinearVelocity + WxR);

					// Apply the phantom forces
					DV -= (LinearAcc + LinearDragAcc + CoriolisAcc + CentrifugalAcc + EulerAcc) * Dt;
					DW -= AngularAcc * Dt;

					// Pass on some part of world-space motion to the local-space drag calculation below.
					// E.g., for LinearVelocityAlpha=1.0, and if LinearDrag is maxed out below, then the particle's local space velocity 
					// would end up being exactly negative SimulationSpace.LinearVelocity. If LinearVelocityAlpha=0.0, then the 
					// simulation-space velocity does not affect local drag.
					TargetV = -SimulationSpaceSettings.LinearVelocityAlpha * SimulationSpace.LinearVelocity;
					TargetW = -SimulationSpaceSettings.AngularVelocityAlpha * SimulationSpace.AngularVelocity;
				}

				// New velocity
				const FReal LinearDamping = FMath::Min(FReal(1), Particle.LinearEtherDrag() * Dt);
				const FReal AngularDamping = FMath::Min(FReal(1), Particle.AngularEtherDrag() * Dt);
				V = FMath::Lerp(V + DV, TargetV, LinearDamping);
				W = FMath::Lerp(W + DW, TargetW, AngularDamping);

				if (Particle.GyroscopicTorqueEnabled())
				{
					W = Utilities::GetAngularVelocityAdjustedForGyroscopicTorques(RCoM, Particle.I(), W, Dt);
				}
