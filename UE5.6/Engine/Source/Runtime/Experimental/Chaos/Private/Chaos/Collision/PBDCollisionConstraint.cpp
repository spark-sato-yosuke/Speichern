...
namespace Chaos
{
	...
	FPBDCollisionConstraint FPBDCollisionConstraint::MakeTriangle(const FImplicitObject* Implicit0)
	{
		...
		Constraint.InitMarginsAndTolerances(Implicit0->GetCollisionType(), ImplicitObjectType::Triangle, Implicit0->GetMarginf(), FRealSingle(0));
		return Constraint;
	}
		...
		const bool bInUseManifold)
	{
		...
		const FRealSingle Margin0 = GetImplicit0()->GetMarginf();
		const FRealSingle Margin1 = GetImplicit1()->GetMarginf();
	...
	bool FPBDCollisionConstraint::TryRestoreManifold()
	{
		...
		if (!Flags.bCanRestoreManifold || Flags.bModifierApplied)
		{
			return false;
		}
	...
	int32 FPBDCollisionConstraint::FindSimpleSavedManifoldPoint(const int32 ManifoldPointIndex) const
	{
		if (bChaos_Manifold_EnableFrictionRestore)
		{
			...
			if (!ManifoldPoint.Flags.bDisabled)
			{
				const int32 NumAllowedSavedPoints = SavedManifoldPoints.Num();
				...
				// Ignore points farther than NearDistanceTolerance
				FRealSingle BestDistanceSq = NearDistanceToleranceSq;

				int32 MatchAllowedPointIndex = INDEX_NONE;
				for (int32 PointIndex = 0; PointIndex < NumAllowedSavedPoints; ++PointIndex)
				{
					const int32 MatchPointIndex = (PointIndex + ManifoldPointIndex) % NumAllowedSavedPoints;
					const FSavedManifoldPoint& SavedManifoldPoint = SavedManifoldPoints[MatchPointIndex];
					...
					if (DistanceSq < ExactDistanceToleranceSq)
					{
						...
						MatchAllowedPointIndex = MatchPointIndex;
						break;
					}
	...
	void FPBDCollisionConstraint::AssignSavedManifoldPoints()
	{
		...
		int32 NumAllowedPointIndices = 0;
		if (!bChaos_Collision_SimpleAssignContact)
		{
			...
			NumAllowedPointIndices = AllowedPointIndices.Num();
		}
		...
		for (int32 PointIndex = 0, PointEndIndex = ManifoldPoints.Num(); PointIndex < PointEndIndex; ++PointIndex)
		{
			...
			//if (!ManifoldPoints[PointIndex].Flags.bWasRestored)
			{
				...
				int32 SavedManifoldPointIndex = INDEX_NONE;
				if (bChaos_Collision_SimpleAssignContact)
				{
					SavedManifoldPointIndex = FindSimpleSavedManifoldPoint(PointIndex);
				}
				else
				{
					SavedManifoldPointIndex = FindSavedManifoldPoint(PointIndex, AllowedPointIndices.GetData(), NumAllowedPointIndices);
				}
				...
				// Nothing to do if no saved friction point because we already set the anchor to the most recently detected contact point
