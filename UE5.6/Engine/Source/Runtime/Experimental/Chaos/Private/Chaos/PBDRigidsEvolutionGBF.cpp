...
void FPBDRigidsEvolutionGBF::AdvanceOneTimeStepImpl(const FReal Dt, const FSubStepInfo& SubStepInfo)
{
...

	{
		...
		CCDManager.ApplyCCD(Dt, CollisionConstraints.GetConstraintAllocator().GetCCDConstraints(), Particles.GetActiveParticlesView().Num());
	}
	...
	if(Dt > 0)
	{
		...
		// Post-solve CCD fixup to prevent the constraint solve from pushing CCD objects our of the world
		{
			...
			CCDManager.ProjectCCD(Dt, CollisionConstraints.GetConstraintAllocator().GetCCDConstraints());
		}
	}
	...
	
	{
		...
		CVD_TRACE_JOINT_CONSTRAINTS(CVDDC_JointLinearConstraints, JointConstraints.LinearConstraints);
		CVD_TRACE_JOINT_CONSTRAINTS(CVDDC_JointNonLinearConstraints, JointConstraints.NonLinearConstraints);
	...
	ParticlesView.ParallelFor([&](auto& GeomParticle, int32 Index)
		{
			...
			if (PBDParticle && PBDParticle->ObjectState() == EObjectStateType::Dynamic)
			{
				...
				// Apply gyroscopic as a velocity change. This could be applied before the clamps, or after,
				// but the user might be surprised to see an angular velocity that exceeds the clamp
				// so do it before.
				if (Particle.GyroscopicTorqueEnabled())
				{
					W = Utilities::GetAngularVelocityAdjustedForGyroscopicTorques(Particle.RCom(), Particle.I(), W, Dt);
				}
...
void FPBDRigidsEvolutionGBF::ApplyMidPhaseModifier(FReal Dt)
{
	if (MidPhaseModifiers != nullptr)
	{
		FMidPhaseModifierAccessor MidPhaseModifier(*this);
		for (ISimCallbackObject* ModifierCallback : *MidPhaseModifiers)
		{
			ModifierCallback->MidPhaseModification_Internal(MidPhaseModifier);
		}
	}
}
	...
	, bIsDeterministic(false)
{
	...
	JointConstraints.LinearConstraints.SetUseSimd(bChaosRigids_UseSimdForJointsSolver); //NonLinear Joints don't have simd code so it's skipped. 
	...
	AddConstraintContainer(JointConstraints.LinearConstraints, ChaosSolverJointPriority);
	AddConstraintContainer(JointConstraints.NonLinearConstraints, ChaosSolverJointPriority);
...
void FPBDRigidsEvolutionGBF::SetParticleTransform(FGeometryParticleHandle* InParticle, const FVec3& InPos, const FRotation3& InRot, const bool bIsTeleport, const bool bWakeUp)
{
	...
	OnParticleMoved(InParticle, PrevX, PrevR, bIsTeleport, bWakeUp);
...
void FPBDRigidsEvolutionGBF::SetParticleTransformSwept(FGeometryParticleHandle* InParticle, const FVec3& InPos, const FRotation3& InRot, const bool bIsTeleport)
{
	...
	if (TOI > 0)
	{
		...
		SetParticleTransform(InParticle, NewPos, InRot, bIsTeleport, true);
	}
}
...
void FPBDRigidsEvolutionGBF::OnParticleMoved(FGeometryParticleHandle* InParticle, const FVec3& PrevX, const FRotation3& PrevR, const bool bIsTeleport, const bool bWakeUp)
{
...
TArray<FGeometryParticleHandle*> FPBDRigidsEvolutionGBF::GetConnectedParticles(FGeometryParticleHandle* InParticle, const TArray<FParticleID>& ExcludeConnections)
{
	...
	struct FHashMapTraits
	{
		...
		static const FGeometryParticleHandle* GetElementID(const FGeometryParticleHandle* Element) { return Element; }
	};
