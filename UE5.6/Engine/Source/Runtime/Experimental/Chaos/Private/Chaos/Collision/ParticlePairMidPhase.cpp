...
extern bool bChaosCCDResetTOIFix;
...
namespace Chaos
{
	...
	namespace CVars
	{
		...
		// This should be enabled but it exposes a bug that must be fixed first: collisions with 
		// invalid particle pointers are being activated causing a crash in CCDUtilities::ApplyCorrections
		...
		// See FGenericParticlePairMidPhase::SetConvexOptimizationIsActive
	...
	void FSingleShapePairCollisionDetector::WakeCollision(const int32 SleepEpoch, const int32 CurrentEpoch)
	{
		...
		{
			if (bChaosMidPhaseActivateWakingConstraints)
			{
				// Update the epoch and add to the active list
				Constraint->ConcreteContainer()->GetConstraintAllocator().AddActiveConstraint(Constraint.Get());
				LastUsedEpoch = Constraint->GetContainerCookie().LastUsedEpoch;
			}
			else
			{
			...
			// NOTE: this relies on the shape world transforms being up-to-date. They are usually updated in Integrate which
	...
	void FParticlePairMidPhase::Reset()
	{
		...
		Flags.bIsConvexOptimizationUsed = false;
	...
	void FGenericParticlePairMidPhase::SetConvexOptimizationIsActive(bool bSetConvexOptimizationIsActive, TFunctionRef<void(FPBDCollisionConstraint*)> OnDestroyingCollision)
	{
		Flags.bIsModified = true;
		Flags.bIsConvexOptimizationActive = bSetConvexOptimizationIsActive;

		// We need to reset collisions if we enable/disable use of the convex optimizer because we are switching 
		// to a different set of collision shapes. This is necessary because the "key" used to identify a shape 
		// would not be unique between the two sets of shapes so we would end up with mismatched collisions in the map.
		// 
		// NOTE: Collisions must be removed from the constraint graph before being destroyed. The caller is
		// responsible for this because we don't know about the graph here (and shouldn't because the solver
		// may not be using a graph).
		// @todo(chaos): see if we can clean this up a bit?
		// 
		// NOTE: bIsConvexOptimizationActive can be changed in a modifier which gets reset every frame so we
		// use bIsConvexOptimizationUsed to remember what we actually used rather than just resetting on change.
		if (CVars::bChaosMidPhaseModifierConvexOptimizerFix)
		{
			if (Flags.bIsConvexOptimizationActive != Flags.bIsConvexOptimizationUsed)
			{
				// The caller should remove collisions from the graph here
				for (TMap<uint64, FPBDCollisionConstraintPtr>::ElementType& KVP : Constraints)
				{
					FPBDCollisionConstraintPtr& Constraint = KVP.Value;
					OnDestroyingCollision(Constraint.Get());
				}

				// NOTE: An assert here would indicate that the OnDestroyingCollisions callback did not 
				// remove collisions from the constraint graph.
				Constraints.Reset();
			}
		}
	}
		...
		const FCollisionContext& Context)
	{
		...
		// bIsConvexOptimizationActive can be changed in a modifier which gets reset every frame so we
		// need to remember what we actually used when generating collisions, so that we know if we
		// have to destroy those collisions if it changes again (see SetConvexOptimizationIsActive)
		// NOTE: We don't have to handle the case where bHasConvexOptimizer0 or bHasConvexOptimizer1
		// changes because we already reset collisions whenever geometry is replaced or rebuilt.
		...
		Flags.bIsConvexOptimizationUsed = (bHasConvexOptimizer0 || bHasConvexOptimizer1);
		...
		const FCollisionContext& Context)
	{
		...
		for (int32 ConstraintIndex = 0; ConstraintIndex < NumNewConstraints; ++ConstraintIndex)
		{
			...
			// We should test the original probe flag, because the modifier could have changed it at this point
			// Reseting flag will occur only when constraint will be activated.
			if (!Constraint->BoundsTestFlags.bIsProbe)
			{
				if (!bUseCCDSweep)
				{
		...
		const FCollisionContext& Context)
	{
		...
		const FVec3 SphereWorldPos0 = ShapeWorldTransform0.TransformPositionNoScale(FVec3(Sphere0.GetCenterf()));
		const FVec3 SphereWorldPos1 = ShapeWorldTransform1.TransformPositionNoScale(FVec3(Sphere1.GetCenterf()));
		const FVec3 DR = SphereWorldPos0 - SphereWorldPos1;
		const FReal DRLenSq = DR.SizeSquared();
		const FReal CullSeparation = Sphere0.GetRadiusf() + Sphere1.GetRadiusf() + CullDistance;
