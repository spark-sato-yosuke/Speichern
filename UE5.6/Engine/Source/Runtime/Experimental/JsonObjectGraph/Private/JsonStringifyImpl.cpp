...
void FJsonStringifyImpl::WriteStructToJson(const void* StructInstance, const void* DefaultInstance, const UScriptStruct* Struct, const UScriptStruct* DefaultStruct)
{
	...
	// the type stored in the json:
	const bool bWriteStructType = InstancedStructType && 
		// UPropertyBag is a transient non native struct, and is regenerated on load. It 
		// is not meant to have persisent identity - so we don't want to even try
		// to write it out:
		(!InstancedStructType->HasAnyFlags(RF_Transient) || InstancedStructType->IsNative());
	...
	FPendingScope ObjectStartIdentifier( 
		this, 
		[Impl = this, Struct, bWriteStructType]() 
		{ 
			Impl->Writer->WriteObjectStartInline(); 
			if(bWriteStructType)
			{
				Impl->Writer->WriteUtf8Value(UE_JSON_SCRIPTSTRUCT_TCHAR, Impl->WriteObjectReference(Struct));
			}
		}, 
