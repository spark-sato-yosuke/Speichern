...
struct FImageViewStrided : public FImageInfo
{
	...
	FORCEINLINE int64 GetStrideBytes()  const { return StrideBytes; }
	
	// if IsStrideWidth, then you can convert back to FImageView
	FORCEINLINE bool IsStrideWidth() const { return StrideBytes == SizeX * GetBytesPerPixel(); }
	...
	FORCEINLINE uint8 * GetRowPointer(int32 Y,int32 Slice = 0) const
	{
		checkSlow( Y >= 0 && Y < SizeY );
		checkSlow( Slice >= 0 && Slice < NumSlices );
	...
	FORCEINLINE const FImageViewStrided GetPortion(int64 PortionStartX,int64 PortionSizeX, int64 PortionStartY,int64 PortionSizeY) const
	{
