...
private:
	...
	struct FTreeNode
	{
		...
		/**
		 * Recursively search the tree to find the closest parent of the given relative directory name;
		 * returns *this if the first component of the relativepath is not in any child node.
		 */
		FTreeNode& FindClosestParentNode(FStringView InRelPath, FStringView& OutRemainingRelPath);
...
{
public:
	static constexpr int32 SingleThreadFilesPerBatch = 3;
	static constexpr int32 ExpectedMaxBatchSize = 100;
	static constexpr int32 MinSecondsToElapseBeforeCacheWrite = 30;
	static constexpr int32 MinAssetReadsBeforeCacheWrite = 100000;
	static constexpr int32 CacheShardAssetCount = 250000;
	static constexpr uint32 CacheSerializationMagic = 0xA46B0FDA; // Versioning and integrity checking
	 ...
	 */
	static inline int32 GARGatherThreads = 60;
