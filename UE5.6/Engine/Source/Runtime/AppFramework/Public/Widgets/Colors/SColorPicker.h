...
protected:
	...
	UE_DEPRECATED(5.6, "HideSmallTrash is deprecated. The delete button for theme colors is managed by the SThemeColorBlocksBar widget.")
	APPFRAMEWORK_API void HideSmallTrash();

	UE_DEPRECATED(5.6, "ShowSmallTrash is deprecated. The delete button for theme colors is managed by the SThemeColorBlocksBar widget.")
...
private:
	...
	// Callback for getting the array of colors for a color spin box gradient.
	APPFRAMEWORK_API TArray<FLinearColor> GetGradientColors(EColorPickerChannels Channel) const;

	// Callback for getting the alpha display mode of the color preview swatches.
	APPFRAMEWORK_API EColorBlockAlphaDisplayMode HandleColorPreviewAlphaMode() const;
	...
	// Callback for setting the visibility of the themes panel.
	APPFRAMEWORK_API FReply ToggleThemePanelVisibility();

	// Callback for getting the icon to display on the Show/Hide themes panel button.
	APPFRAMEWORK_API const FSlateBrush* HandleThemePanelButtonImageBrush() const;

	// Callback for getting the visibility of the themes panel.
	APPFRAMEWORK_API EVisibility HandleThemesPanelVisibility() const;

	// Callback for getting the tooltip for the color theme panel
	APPFRAMEWORK_API FText GetColorThemePanelToolTipText() const;
	...
	// Callback for value changes in the color wheel picker.
	APPFRAMEWORK_API void HandleColorWheelValueChanged(FLinearColor NewValue);
	...
	APPFRAMEWORK_API FText HandleHexBoxText() const;

	// Callback for committed text in the hex input box (sRGB gamma).
	APPFRAMEWORK_API void HandleHexInputTextCommitted(const FText& Text, ETextCommit::Type CommitType);
	...
	APPFRAMEWORK_API FReply HandleNewColorPreviewClicked();

	// Callback for clicking the old color preview block.
	APPFRAMEWORK_API FReply HandleOldColorPreviewClicked();

	// Callback for getting the visibility of the old/new color buttons
	APPFRAMEWORK_API EVisibility HandleColorPreviewButtonVisibility() const;

	// Callback for setting the visibility of the new color preview button image
	APPFRAMEWORK_API void SetNewColorPreviewImageVisibility(EVisibility InButtonVisibility);

	// Callback for setting the visibility of the old color preview button image
	APPFRAMEWORK_API void SetOldColorPreviewImageVisibility(EVisibility InButtonVisibility);

	// Callback for getting the visibility of the new color preview button image
	APPFRAMEWORK_API EVisibility GetNewColorPreviewImageVisibility() const;

	// Callback for getting the visibility of the old color preview button image
	APPFRAMEWORK_API EVisibility GetOldColorPreviewImageVisibility() const;
	...
	// Build the menu widget for selecting the hexadecimal text mode
	APPFRAMEWORK_API TSharedRef<SWidget> MakeHexModeMenu();

	// Get the text to display on the hexadecimal text mode button
	APPFRAMEWORK_API FText HandleHexModeButtonText() const;

	// Callback for when the hexadecimal text mode changes
	APPFRAMEWORK_API void OnHexModeSelected(EColorPickerHexMode InHexMode);
...
private:
	...
	/** Holds the color picker's hex mode. */
	EColorPickerHexMode HexMode;
	...
	/** Whether or not to display color as sRGB */
	bool bUseSRGB = false;
	...
	/** Visibility of the new color preview button image (visible when hovered) */
	EVisibility NewColorPreviewImageVisibility;

	/** Visibility of the old color preview button image (visible when hovered) */
	EVisibility OldColorPreviewImageVisibility;
	...
	/** Whether or not the color themes panel is visible or collapsed */
