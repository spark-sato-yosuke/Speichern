...
float FAnimNode_BlendSpacePlayerBase::GetCurrentAssetLength() const
{
	if(const FBlendSampleData* HighestWeightedSample = GetHighestWeightedSample())
	{
		...
		if (CurrentBlendSpace != nullptr)
		{
			...
			if (ensure(Sample.Animation != nullptr))
			{
...
bool FAnimNode_BlendSpacePlayer::SetOverridePositionWhenJoiningSyncGroupAsLeader(bool InOverridePositionWhenJoiningSyncGroupAsLeader)
{
	...
	if (bool* OverridePositionPtr = GET_INSTANCE_ANIM_NODE_DATA_PTR(bool, bOverridePositionWhenJoiningSyncGroupAsLeader))
	{
		*OverridePositionPtr = InOverridePositionWhenJoiningSyncGroupAsLeader;
		return true;
	}
