...
{
	check(SRV);
	CurrentState->SetSRV(true, DescriptorSet, BindingIndex, SRV);
}
...
{
	check(UAV);
	CurrentState->SetUAV(true, DescriptorSet, BindingIndex, UAV);
}
...
void FVulkanPendingComputeState::PrepareForDispatch(FVulkanCommandListContext& Context)
{
	...
	if (CurrentPipeline->UsesBindless())
	{
		CurrentState->UpdateBindlessDescriptors(Context);
		CurrentPipeline->Bind(Context.GetCommandBuffer().GetHandle());
	}
	else
	{
		const bool bHasDescriptorSets = CurrentState->UpdateDescriptorSets(Context);
		...
		CurrentPipeline->Bind(CommandBufferHandle);
		if (bHasDescriptorSets)
		{
			CurrentState->BindDescriptorSets(CommandBufferHandle);
		}
	}
}
...
void FVulkanPendingGfxState::PrepareForDraw(FVulkanCommandListContext& Context)
{
	...
	check(Context.GetCommandBuffer().bHasPipeline);

	if (CurrentPipeline->UsesBindless())
	{
		check(!CurrentPipeline->bHasInputAttachments); // todo-jn: bindless + InputAttachments
		UpdateDynamicStates(Context.GetCommandBuffer());
		CurrentState->UpdateBindlessDescriptors(Context);
	}
	else
	{
		...
		if (bHasDescriptorSets)
		{
			CurrentState->BindDescriptorSets(Context.GetCommandBuffer().GetHandle());
		}
	}
	...
	if (bDirtyVertexStreams)
	{
		...
		if (TemporaryIA.NumUsed > 0)
		{
			...
			VulkanRHI::vkCmdBindVertexBuffers(Context.GetCommandBuffer().GetHandle(), 0, TemporaryIA.NumUsed, TemporaryIA.VertexBuffers, TemporaryIA.VertexOffsets);
		}
...
void FVulkanPendingGfxState::InternalUpdateDynamicStates(FVulkanCommandBuffer& CommandBuffer)
{
	const bool bNeedsUpdateViewport = !CommandBuffer.bHasViewport || Viewports.Num() != CommandBuffer.CurrentViewports.Num() ||
		(FMemory::Memcmp((const void*)CommandBuffer.CurrentViewports.GetData(), (const void*)Viewports.GetData(), Viewports.Num() * sizeof(VkViewport)) != 0);
	...
	if (bNeedsUpdateViewport)
	{
		...
		VulkanRHI::vkCmdSetViewport(CommandBuffer.GetHandle(), 0, FlippedViewports.Num(), FlippedViewports.GetData());

		CommandBuffer.CurrentViewports = Viewports;
		CommandBuffer.bHasViewport = true;
	}

	const bool bNeedsUpdateScissor = !CommandBuffer.bHasScissor || Scissors.Num() != CommandBuffer.CurrentScissors.Num() || 
		(FMemory::Memcmp((const void*)CommandBuffer.CurrentScissors.GetData(), (const void*)Scissors.GetData(), Scissors.Num() * sizeof(VkRect2D)) != 0);
	if (bNeedsUpdateScissor)
	{
		VulkanRHI::vkCmdSetScissor(CommandBuffer.GetHandle(), 0, Scissors.Num(), Scissors.GetData());
		CommandBuffer.CurrentScissors = Scissors;
		CommandBuffer.bHasScissor = true;
	}

	const bool bNeedsUpdateStencil = !CommandBuffer.bHasStencilRef || (CommandBuffer.CurrentStencilRef != StencilRef);
	if (bNeedsUpdateStencil)
	{
		VulkanRHI::vkCmdSetStencilReference(CommandBuffer.GetHandle(), VK_STENCIL_FRONT_AND_BACK, StencilRef);
		CommandBuffer.CurrentStencilRef = StencilRef;
		CommandBuffer.bHasStencilRef = true;
	}

	CommandBuffer.bNeedsDynamicStateSet = false;
}
