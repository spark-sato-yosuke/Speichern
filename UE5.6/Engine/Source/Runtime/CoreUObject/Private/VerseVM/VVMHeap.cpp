...
static void HandlePASAssertionFailure(const char* Filename, int Line, const char* Function, const char* Expression, const uint64_t* MaybeExtra)
{
#if DO_CHECK || DO_GUARD_SLOW || DO_ENSURE
	if (MaybeExtra)
	{
		FDebug::CheckVerifyFailedImpl2(Expression, Filename, Line, TEXT("libpas assert in %hs failed with extra detail %" PRIu64), Function, *MaybeExtra);
	}
	else
	{
		FDebug::CheckVerifyFailedImpl2(Expression, Filename, Line, TEXT("libpas assert in %hs failed"), Function);
	}
#else
	if (MaybeExtra)
	{
		UE_LOG(LogVerseGC, Fatal, TEXT("Assertion failed: %hs [File:%hs] [Line: %i] \nlibpas assert in %hs failed with extra detail %" PRIu64 "\n"), Expression, Filename, Line, Function, *MaybeExtra);
	}
	else
	{
		UE_LOG(LogVerseGC, Fatal, TEXT("Assertion failed: %hs [File:%hs] [Line: %i] \nlibpas assert in %hs failed\n"), Expression, Filename, Line, Function);
	}
#endif
}

static void HandlePASPanic(const char* Format, va_list ArgList)
{
	TAnsiStringBuilder<256> Builder;
	Builder.AppendV(Format, ArgList);
	UE_LOG(LogVerseGC, Fatal, TEXT("libpas panic: %hs"), *Builder);
}

static void HandlePASOOM()
{
	// pas doesn't tell us the size/alignment that failed allocation, so just use some generic values.
...
void FHeap::Initialize()
{
	if (!bIsInitialized)
	{
		// First, register our libpas panic/assertion failure handlers.
		pas_set_assertion_failure_callback(HandlePASAssertionFailure);
		pas_set_panic_callback(HandlePASPanic);
		pas_set_oom_callback(HandlePASOOM);
...
void FHeap::RunPreMarking(FIOContext Context)
{
	...
	{
		if (SamplingProfiler->ProcessingMutex.TryLock())
		{
			TArray<FSampledFrame> Samples = MoveTemp(SamplingProfiler->Samples);
			SamplingProfiler->ProcessSamples(Samples);
			SamplingProfiler->ProcessingMutex.Unlock();
		}
	}
}
...
void FHeap::MarkRoots(FIOContext Context)
{
	...
	{
		// Take a snapshot of the global heap roots so that we don't hold GlobalRootMutex while marking them.
		// Holding the mutex while marking the roots would risk a deadlock if any marking visitor locks a mutex
		// that might be held while locking GlobalRootMutex (e.g. to add a root).
		// Taking a snapshot is sound under the assumption that no code does a CheckForHandshake between
		// initializing a FGlobalHeapRoot object's outgoing references and adding it to the GlobalRoots array.
		// Any FGlobalHeapRoot object added to the array during marking must then have initialized its outgoing
		// references at a point where the write barrier was active.
		TArray<FGlobalHeapRoot*> RootsSnapshot;
		{
			TUniqueLock Lock(GlobalRootMutex);
			RootsSnapshot = *GlobalRoots;
		}

		FMarkStackVisitor Visitor(MyMarkStack);
		for (FGlobalHeapRoot* Root : RootsSnapshot)
		{
			Root->Visit(Visitor);
		}
	}
...
bool FHeap::AttemptToTerminate(FIOContext Context)
{
	...
	Context.SoftHandshake([](FHandshakeContext TargetContext) {
		...
		{
			TargetContext.MarkConservativeStack();
		}
	...
	if (FSamplingProfiler* SamplingProfiler = GetSamplingProfiler())
	{
		FMarkStack TempMarkStack;
		SamplingProfiler->MarkReferencedCells(TempMarkStack);
		{
			TUniqueLock Lock(Mutex);
			MarkStack->Append(MoveTemp(TempMarkStack));
		}
	}
