...
public:
...
#if WITH_EDITORONLY_DATA
		, bHasRequiredParam(Property->HasAnyPropertyFlags(CPF_RequiredParm))
#endif
	...
	bool HasRequiredParam() const
	{
#if WITH_EDITORONLY_DATA && CACHE_UNVERSIONED_PROPERTY_SCHEMA
		return bHasRequiredParam;
#else
		return Property->HasAnyPropertyFlags(CPF_RequiredParm);
#endif
	}
...
struct FUnversionedStructSchema
{
	...
	static void AppendClassMetaData(const UStruct* Struct, FBlake3& HashBuilder)
	{
		...
		if (Struct != UPackage::StaticClass())
		{
			if (const UPackage* Package = Cast<UPackage>(Outermost))
			{
				...
				FMetaData& MetaDataObject = const_cast<UPackage*>(Package)->GetMetaData();
...
static std::atomic<const FUnversionedStructSchema*>& GetUnversionedSchema(const UStruct* Struct, bool bSkipEditorOnly)
{
#if WITH_EDITORONLY_DATA
...
{
	std::atomic<const FUnversionedStructSchema*>* ExistingSchemaPtr = &GetUnversionedSchema(Struct, bSkipEditorOnly);
	if (const FUnversionedStructSchema* ExistingSchema = ExistingSchemaPtr->load(std::memory_order_acquire))
	{
	...
	auto DedupSchema = [ExistingSchemaPtr, CreatedSchema]() -> const FUnversionedStructSchema*
	{
		const FUnversionedStructSchema* ExistingSchema = nullptr;
		if (!ExistingSchemaPtr->compare_exchange_strong(ExistingSchema, CreatedSchema, std::memory_order_acq_rel, std::memory_order_acquire))
		{
			FUnversionedStructSchema::Delete(CreatedSchema);
			check(ExistingSchema);
			return ExistingSchema;
		}
	...
	if (AutoRTFM::IsClosed())
	{
		...
		// and get back to try to do the CAS on commit. 
		// This implies that we might destroy the CreatedSchema that got used during the transaction
		// but as long as nothing holds a pointer to it, all should be fine. If not, ASAN will catch it.
		// OnCommit -> Dedup created schema using CAS.
		// OnAbort  -> Created schema should get deleted.
		// So we should not have any leaks remaining in any scenarios.
		// We also assume that GC doesn't run during transactions and that the UStruct will remain valid until the commit.
		AutoRTFM::OnCommit([DedupSchema]() { DedupSchema(); });

		// For now return our created schema and we'll dedup it on commit.
		return *CreatedSchema;
	}
...
{
#if CACHE_UNVERSIONED_PROPERTY_SCHEMA
	FUnversionedStructSchema::Delete(const_cast<FUnversionedStructSchema*>(Struct->UnversionedGameSchema.load(std::memory_order_acquire)));
	Struct->UnversionedGameSchema = nullptr;
#if WITH_EDITORONLY_DATA
	FUnversionedStructSchema::Delete(const_cast<FUnversionedStructSchema*>(Struct->UnversionedEditorSchema.load(std::memory_order_acquire)));
		...
		if (Header.HasValues())
		{
			...
			if (Header.HasNonZeroValues())
			{
...
#if WITH_EDITORONLY_DATA
				bool bTrackInitializedProperties = false;

				// We cannot mark all properties as initialized when loading 'dense' property values.
				if (UnderlyingArchive.DoDelta() && !UnderlyingArchive.IsTransacting())
				{
					bTrackInitializedProperties = UE::FInitializedPropertyValueState(Struct, Data).IsTracking();
				}
#endif
				...
				for (FUnversionedHeader::FIterator It(Header, Schema); It; It.Next())
				{
					if (It.IsNonZero())
					{
#if WITH_EDITOR || UE_WITH_REMOTEOBJECTARCHIVE_DEBUGGING// Skip this scope to save time in the runtime; it is only needed for reference collection in editor
						FSerializedPropertyScope SerializedProperty(UnderlyingArchive, It.GetSerializer().GetProperty());
#endif

#if WITH_EDITORONLY_DATA
						if (bTrackInitializedProperties && It.GetSerializer().HasRequiredParam())
						{
							UE::FInitializedPropertyValueState(Struct, Data).Set(It.GetSerializer().GetProperty());
						}
#endif
