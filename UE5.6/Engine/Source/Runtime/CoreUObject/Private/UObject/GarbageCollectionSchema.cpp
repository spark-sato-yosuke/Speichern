...
void DumpSchemaStats(FOutputDevice& Out)
{
	...
	auto GatherStats = [&Classes, &Structs, &NumClasses](FSchemaView Schema)
		{
			++NumClasses;
			FSchemaStats& ClassStat = Classes.TypeStats.FindOrAdd(Schema.GetWords());
			if (ClassStat.NumMembers == 0 && !Schema.IsEmpty())
			{
				ClassStat = FSchemaStats(Schema, Structs.TypeStats);
	...
	{
		GatherStats(It->ReferenceSchema.Get());
	}

#if WITH_VERSE_VM || defined(__INTELLISENSE__)
	for (TObjectIterator<UVerseStruct> It; It; ++It)
	{
		GatherStats(It->ReferenceSchema.Get());
	}
#endif
...
uint32 CountSchemas(uint32& OutNumWords)
{
	...
	auto CountSchema = [&ClassStats, &StructStats](FSchemaView Schema)
		{
			if (!Schema.IsEmpty())
			{
				FSchemaStats& Stat = ClassStats.FindOrAdd(Schema.GetWords());
				if (Stat.NumMembers == 0)
				{
					Stat = FSchemaStats(Schema, StructStats);
				}
			}
		};
	...
	for (TObjectIterator<UClass> It; It; ++It)
	{
		CountSchema(It->ReferenceSchema.Get());
	}
