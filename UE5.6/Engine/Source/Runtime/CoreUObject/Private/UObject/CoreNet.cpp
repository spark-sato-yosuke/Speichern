...
FArchive& FNetBitWriter::operator<<(UObject*& Object)
{
	...
	UE_CLOG(PackageMap->IsUsingRemoteObjectReferences(), LogCoreNet, Warning, TEXT("Sending raw object reference when NetDriver expects remote references (%s)"), *GetFullNameSafe(Object));
#endif
...
{
#if UE_WITH_REMOTE_OBJECT_HANDLE
	if (PackageMap->IsUsingRemoteObjectReferences())
	{
		FRemoteObjectReference RemoteReference(Value);
		bool bResult = false;
		RemoteReference.NetSerialize(*this, PackageMap, bResult);
		return *this;
	}
#endif
...
FArchive& FNetBitWriter::operator<<(struct FWeakObjectPtr& WeakObjectPtr)
{
	...
	if (PackageMap->IsUsingRemoteObjectReferences())
	{
		FRemoteObjectReference RemoteReference(WeakObjectPtr);
		bool bResult = false;
		RemoteReference.NetSerialize(*this, PackageMap, bResult);
		return *this;
	}
...
{
#if UE_WITH_REMOTE_OBJECT_HANDLE
	UE_CLOG(PackageMap->IsUsingRemoteObjectReferences(), LogCoreNet, Warning, TEXT("Received raw object reference when NetDriver expects remote references (%s)"), *GetFullNameSafe(Object));
#endif
...
FArchive& FNetBitReader::operator<<(struct FWeakObjectPtr& WeakObjectPtr)
{
	...
	if (PackageMap->IsUsingRemoteObjectReferences())
	{
		...
		bool bResult = false;
		RemoteReference.NetSerialize(*this, PackageMap, bResult);
		WeakObjectPtr = RemoteReference.ToWeakPtr();
		return *this;
	}
#endif
	return FArchiveUObject::SerializeWeakObjectPtr(*this, WeakObjectPtr);
