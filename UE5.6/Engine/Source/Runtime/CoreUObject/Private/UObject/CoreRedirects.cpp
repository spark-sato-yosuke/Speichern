...
void FCoreRedirects::RecordAddedObjectRedirector(const FSoftObjectPath& Source, const FSoftObjectPath& Dest)
{
	...
	if (!!(Context.GetFlags() & FCoreRedirectsContext::EFlags::UseRedirectionSummary))
	{
		...
		Context.GetRedirectionSummary().Add(ConvertedToCoreRedirect, false /* bIsWildcardMatch */);
	}
}
...
void FCoreRedirects::RecordRemovedObjectRedirector(const FSoftObjectPath& Source, const FSoftObjectPath& Dest)
{
	...
	if (!!(Context.GetFlags() & FCoreRedirectsContext::EFlags::UseRedirectionSummary))
	{
		FCoreRedirect ConvertedToCoreRedirect(ECoreRedirectFlags::Type_Object,
			FCoreRedirectObjectName(Source), FCoreRedirectObjectName(Dest));
		Context.GetRedirectionSummary().Remove(ConvertedToCoreRedirect, false /* bIsWildcardMatch */);
	}
}
	...
	FScopeCoreRedirectsWriteLockedContext& LockedContext)
{
	...
	{
		LockedContext.Get().GetRedirectionSummary().Add(NewRedirect, bIsWildcardMatch);
	}
#endif
	...
	FScopeCoreRedirectsWriteLockedContext& LockedContext)
{
	...
	if (bRemovedRedirect)
	{
		...
		if (!!(LockedContext.Get().GetFlags() & FCoreRedirectsContext::EFlags::UseRedirectionSummary))
		{
			LockedContext.Get().GetRedirectionSummary().Remove(RedirectToRemove, bIsWildcardMatch);
		}
#endif
	}
...
#define CLASS_REDIRECT(OldName, NewName) DiscardNoDiscard(Redirects.Emplace_GetRef(ECoreRedirectFlags::Type_Class, TEXT(OldName), TEXT(NewName)))
#define CLASS_REDIRECT_INSTANCES(OldName, NewName) DiscardNoDiscard(Redirects.Emplace_GetRef(ECoreRedirectFlags::Type_Class | ECoreRedirectFlags::Category_InstanceOnly, TEXT(OldName), TEXT(NewName)))
#define STRUCT_REDIRECT(OldName, NewName) DiscardNoDiscard(Redirects.Emplace_GetRef(ECoreRedirectFlags::Type_Struct, TEXT(OldName), TEXT(NewName)))
#define ENUM_REDIRECT(OldName, NewName) DiscardNoDiscard(Redirects.Emplace_GetRef(ECoreRedirectFlags::Type_Enum, TEXT(OldName), TEXT(NewName)))
#define PROPERTY_REDIRECT(OldName, NewName) DiscardNoDiscard(Redirects.Emplace_GetRef(ECoreRedirectFlags::Type_Property, TEXT(OldName), TEXT(NewName)))
#define FUNCTION_REDIRECT(OldName, NewName) DiscardNoDiscard(Redirects.Emplace_GetRef(ECoreRedirectFlags::Type_Function, TEXT(OldName), TEXT(NewName)))
#define PACKAGE_REDIRECT(OldName, NewName) DiscardNoDiscard(Redirects.Emplace_GetRef(ECoreRedirectFlags::Type_Package, TEXT(OldName), TEXT(NewName)))
