		...
		else if (bIsLoading)
		{
			...
			else
			{
				...
				checkf(
					...
					TEXT("Value tagged with ID %s was saved as %i bytes, but loading it as a %s loaded " INT64_FMT " bytes."),
...
void FOptionalProperty::InstanceSubobjects(void* Data, void const* DefaultData, TNotNull<UObject*> InOwner, struct FObjectInstancingGraph* InstanceGraph)
{
	if (Data
		&& IsSet(Data)
		&& (ValueProperty->ContainsInstancedObjectProperty() || (InstanceGraph && InstanceGraph->CanUseDynamicInstancing())))
	{
...
EConvertFromTypeResult FOptionalProperty::ConvertFromType(const FPropertyTag& Tag, FStructuredArchive::FSlot Slot, uint8* ContainerData, UStruct* DefaultsStruct, const uint8* DefaultsContainer)
{
	if (Slot.GetArchiveState().UEVer() >= EUnrealEngineObjectUE5Version::PROPERTY_TAG_COMPLETE_TYPE_NAME)
	{
		...
		if (Tag.Type == NAME_OptionalProperty)
		{
			...
			ValueTag.SetProperty(GetValueProperty());
...
EPropertyVisitorControlFlow FOptionalProperty::Visit(FPropertyVisitorContext& Context, const TFunctionRef<EPropertyVisitorControlFlow(const FPropertyVisitorContext& /*Context*/)> InFunc) const
{
	...
	if (RetVal == EPropertyVisitorControlFlow::StepInto && IsSet(Context.Data.PropertyData))
	{
		if(!IsSet(Context.Data.PropertyData))
		{
		...
		FPropertyVisitorScope Scope(Context.Path, FPropertyVisitorInfo(ValueProperty));
		RetVal = ValueProperty->Visit(Context, InFunc);
	}
	return RetVal;
}
