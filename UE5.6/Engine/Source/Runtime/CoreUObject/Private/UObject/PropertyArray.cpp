...
void FArrayProperty::SerializeItem(FStructuredArchive::FSlot Slot, void* Value, void const* Defaults) const
{
	...
	if (bExperimentalOverridableLogic)
	{
		...
		else
		{
			...
			// We filter out external objects if:
			//	- We're an array of objects.
			//	- We have the ExperimentalExternalObjects flag.
			//	- We're writing to a persistent archive.
			//	- We have at least one external entity object.
			// For now, this will be considered as a full array replacement.
			FObjectProperty* ObjectProperty = CastField<FObjectProperty>(Inner);
			const bool bFilterExternalObjects = ObjectProperty && HasAnyPropertyFlags(CPF_ExperimentalExternalObjects) && UnderlyingArchive.IsPersistent() && !UnderlyingArchive.HasAnyPortFlags(PPF_DuplicateForPIE);
			...
			if (!bReplaceArray)
			{
				...
				if (!bReplaceArray)
				{
					...
					{
						auto FindObject = [InnerObjectProperty, bFilterExternalObjects](const FOverriddenPropertyNodeID ObjectToFind, FScriptArrayHelper& ArrayHelper) -> int32
						{
							const int32 ArrayNum = ArrayHelper.Num();
							for (int i = 0; i < ArrayNum; ++i)
							{
								if (UObject* CurrentObject = InnerObjectProperty->GetObjectPropertyValue(ArrayHelper.GetElementPtr(i)); CurrentObject && (!bFilterExternalObjects || !CurrentObject->IsPackageExternal()))
								{
								...
								case EOverriddenPropertyOperation::Remove:
									{
										...
										{
											// We don't support removing external entities from overriden arrays yet
											UObject* DefaultObject = InnerObjectProperty->GetObjectPropertyValue(DefaultsArrayHelper.GetElementPtr(DefaultIndex));
											checkf(DefaultObject && !DefaultObject->IsPackageExternal(), TEXT("Unsupported remove operation on item %s(0x%p)"), *GetNameSafe(DefaultObject), DefaultObject);

											RemovedIndices.Add(DefaultIndex);
										}
										else
										{
											UE_LOG(LogOverridableObject, VeryVerbose, TEXT("Unable to save deleted item %s"), *Pair.Key.ToDebugString());
										}
										break;
									}
								case EOverriddenPropertyOperation::Add:
									{
										...
										if (Index != INDEX_NONE)
										{
											...
											{
												UObject* Object = InnerObjectProperty->GetObjectPropertyValue(ArrayHelper.GetElementPtr(Index));
										...
										{
											UE_LOG(LogOverridableObject, VeryVerbose, TEXT("Unable to save added item %s"), *Pair.Key.ToDebugString());
										}
										break;
									}
								default:
...
{
	if (Data
		&& (Inner->ContainsInstancedObjectProperty() || (InstanceGraph && InstanceGraph->CanUseDynamicInstancing())))
	{
		FScriptArrayHelper ArrayHelper(this, Data);
...
EPropertyVisitorControlFlow FArrayProperty::Visit(FPropertyVisitorContext& Context, const TFunctionRef<EPropertyVisitorControlFlow(const FPropertyVisitorContext& /*Conmtext*/)> InFunc) const
{
	...
	if (RetVal == EPropertyVisitorControlFlow::StepInto)
	{
		...
		FPropertyVisitorScope Scope(Context.Path, FPropertyVisitorInfo(Inner));

		const int32 ArrayNum = ArrayHelper.Num();
		for (int32 ContainerIndex = 0; ContainerIndex < ArrayNum; ContainerIndex++)
		{
			Context.Path.Top().SetIndex(ContainerIndex, EPropertyVisitorInfoType::ContainerIndex);
