...
struct FPropertyTypeNameNodeProxy
{
	...
	friend inline void AppendHash(FBlake3& Builder, const FPropertyTypeNameNodeProxy& Proxy)
	{
		const FPropertyTypeNameNode* Node = Proxy.First;
		for (int32 Remaining = 1; Remaining > 0; --Remaining, ++Node)
		{
			AppendHash(Builder, Node->Name);
			Builder.Update(&Node->InnerCount, sizeof(Node->InnerCount));
			Remaining += Node->InnerCount;
		}
	}
...
FPropertyTypeName::FPropertyTypeName(const FProperty* Property)
	: FPropertyTypeName([](const FProperty* P)
	{
		FPropertyTypeNameBuilder Builder;
		P->SaveTypeName(Builder);
...
{
	const FPropertyTypeNameNode* First = GPropertyTypeNameTable.ResolveByIndex(TypeName.Index);
	AppendHash(Builder, FPropertyTypeNameNodeProxy{First});
}
