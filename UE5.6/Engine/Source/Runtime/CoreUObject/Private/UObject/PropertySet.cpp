 ...
 */
void FSetProperty::InstanceSubobjects(void* Data, void const* DefaultData, TNotNull<UObject*> InOwner, FObjectInstancingGraph* InstanceGraph)
{
	if (!Data)
	{
		return;
	...
	const bool bUsesDynamicInstancing = InstanceGraph && InstanceGraph->CanUseDynamicInstancing();
	const bool bInstancedElement = ElementProp->ContainsInstancedObjectProperty() || bUsesDynamicInstancing;
...
EConvertFromTypeResult FSetProperty::ConvertFromType(const FPropertyTag& Tag, FStructuredArchive::FSlot Slot, uint8* Data, UStruct* DefaultsStruct, const uint8* Defaults)
{
	...
	InnerPropertyTag.SetProperty(ElementProp);
	...
	if (!bConversionSucceeded)
	{
		UE_LOG(LogClass, Warning, TEXT("Set Element Type mismatch in %s - Previous (%s) Current (%s) for package: %s"),
			*WriteToString<32>(Tag.Name), *WriteToString<32>(InnerPropertyTag.GetType()), *WriteToString<32>(UE::FPropertyTypeName(ElementProp)), *UnderlyingArchive.GetArchiveName());
	}
...
EPropertyVisitorControlFlow FSetProperty::Visit(FPropertyVisitorContext& Context, const TFunctionRef<EPropertyVisitorControlFlow(const FPropertyVisitorContext& /*Conmtext*/)> InFunc) const
{
	...
	if (RetVal == EPropertyVisitorControlFlow::StepInto)
	{
		...
		FScriptSetHelper SetHelper(this, Context.Data.PropertyData);

		FPropertyVisitorScope Scope(Context.Path, FPropertyVisitorInfo(ElementProp));
		for (FScriptSetHelper::FIterator It(SetHelper); It; ++It)
		{
			// visit element
			Context.Path.Top().SetIndex(It.GetLogicalIndex(), EPropertyVisitorInfoType::ContainerIndex);

			FPropertyVisitorContext SubContext = Context.VisitPropertyData(SetHelper.GetElementPtr(It));

			RetVal = ElementProp->Visit(SubContext, InFunc);
