		...
		if (EnumHasAnyFlags(SerializationControl, EClassSerializationControlExtension::OverridableSerializationInformation))
		{
			...
			if (UnderlyingArchive.IsLoading())
			{
				...
				if (!UnderlyingArchive.IsTransacting())
				{
					...
					{
						ControlContext.OverriddenProperties = FOverridableManager::Get().RestoreOverrideOperation((UObject*)Data, Operation, /*bNeedsSubobjectTemplateInstantiation*/true);
					}
					else
					{
			...
			while (true)
			{
						...
						if (bSerializeValue || bRestoreOverrideOperation)
						{
							...
							if (FProperty* TempProperty = CastField<FProperty>(TempField.Get()); TempProperty && TempProperty->LoadTypeName(Tag.GetType(), &Tag))
							{
								...
								if (bRestoreOverrideOperation)
								{
									...
									{
										OverriddenProperties->RestoreOverriddenPropertyOperation(Tag.OverrideOperation, UnderlyingArchive.GetSerializedPropertyChain(), TempProperty);
									}
								}
							}
						}
				...
				if (bAdvanceProperty)
				{
					...
					if (!UnderlyingArchive.IsTransacting())
					{
						if (FOverriddenPropertySet* OverriddenProperties = FOverridableSerializationLogic::GetOverriddenProperties())
						{
							OverriddenProperties->RestoreOverriddenPropertyOperation(Tag.OverrideOperation, UnderlyingArchive.GetSerializedPropertyChain(), Property);
						}
					}
...
void UStruct::InstanceSubobjectTemplates( TNotNull<void*> Data, void const* DefaultData, UStruct* DefaultStruct, TNotNull<UObject*> Owner, FObjectInstancingGraph* InstanceGraph )
{
...
bool UStruct::IsChildOf( const UStruct* SomeBase ) const
{
	...
	PRAGMA_DISABLE_DEPRECATION_WARNINGS
	check(IsThisNotNull(this, "UStruct::IsChildOf"));
	PRAGMA_ENABLE_DEPRECATION_WARNINGS
...
bool UClass::ShouldUseDynamicSubobjectInstancing() const
{
	// If any parent class supports dynamic instancing, this class also inherits that trait.
	const UClass* CurrentClass = this;
	while (CurrentClass)
	{
		if (CurrentClass->SupportsDynamicInstancedReference())
		{
			return true;
		}
