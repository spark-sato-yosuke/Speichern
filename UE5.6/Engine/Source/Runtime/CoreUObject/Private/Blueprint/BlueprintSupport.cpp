...
static FFlushCompilationQueueFPtr FlushCompilationQueueFPtr = nullptr;
...
void FBlueprintSupport::FlushCompilationQueue(FUObjectSerializeContext* InLoadContext)
{
	if (FlushCompilationQueueFPtr)
	{
		(*FlushCompilationQueueFPtr)(InLoadContext);
	}
}
...
bool FLinkerLoad::RegenerateBlueprintClass(UClass* LoadClass, UObject* ClassDefaultObject)
{
	...
	if (CVarEnableFullBlueprintPreloading.GetValueOnAnyThread())
	{
		...
		// 
		// Some known objects, specifically FMetaData and UBlueprint, have a cyclic relationship with the current class.
	...
	check(!bAlreadyRegenerating || (LoadClass->GetDefaultObject(false) == ClassDefaultObject));
	LoadClass->SetDefaultObject(ClassDefaultObject);
	...
	if (!bWasSubsequentlyRegenerated)
	{
		...
		for (UClass* SuperClass : ClassChainOrdered)
		{
			...
			if (SuperClassSourceObject && SuperClassSourceObject->HasAnyFlags(RF_BeingRegenerated))
			{
				...
				RegenerateBlueprintClass(SuperClass, SuperClass->GetDefaultObject(false));
			}
		}
	...
	if (FindOuter != nullptr || Import.OuterIndex.IsNull())
	{
		if (UObject* ClassPackage = FindObject<UPackage>(/*Outer =*/nullptr, *Import.ClassPackage.ToString()))
		{
			if (UClass* ImportClass = FindObject<UClass>(ClassPackage, *Import.ClassName.ToString()))
			{
				...
				if (!ImportClass->HasAnyFlags(RF_NeedLoad) && ImportClass->GetDefaultObject(false))
				{
...
bool FLinkerLoad::DeferExportCreation(const int32 Index, UObject* Outer)
{
	...
	const UObject* ClassDefaultObject = LoadClass->GetDefaultObject(false);
	if (!bIsPlaceholderClass
		&& ((ClassLinker == nullptr) || !ClassLinker->IsBlueprintFinalizationPending())
		&& (!ClassDefaultObject || ClassDefaultObject->HasAnyFlags(RF_LoadCompleted) || !ClassDefaultObject->HasAnyFlags(RF_WasLoaded)))
	{
		return false;
	}
	...
	{
		bool bAlreadyResolvingExport = IsExportBeingResolved(Index);
	...
	if (bIsCDOExport)
	{
		DEFERRED_DEPENDENCY_CHECK(LoadClass->GetDefaultObject(false) == nullptr);
		LoadClass->SetDefaultObject(Placeholder);
	}
#endif // USE_CIRCULAR_DEPENDENCY_LOAD_DEFERRING
...
void FLinkerLoad::FinalizeBlueprint(UClass* LoadClass)
{
	...
	if (UClass* SuperClass = LoadClass->GetSuperClass())
	{
		...
		if ((SuperLinker != nullptr) && SuperLinker->IsBlueprintFinalizationPending())
		{
			...
			UObject* SuperCDO = SuperLinker->DeferredCDOIndex != INDEX_NONE ? ToRawPtr(SuperLinker->ExportMap[SuperLinker->DeferredCDOIndex].Object) : SuperClass->GetDefaultObject(false);
	...
	if (IsBlueprintFinalizationPending())
	{
		...
		UObject* CDO = DeferredCDOIndex != INDEX_NONE ? ToRawPtr(ExportMap[DeferredCDOIndexCopy].Object) : LoadClass->GetDefaultObject(false);
...
#endif
		{
			UObject* OldCDO = LoadClass->GetDefaultObject(false);
			if (RegenerateBlueprintClass(LoadClass, CDO))
			{
				...
				// stale one)
				if (OldCDO == LoadClass->GetDefaultObject(false))
				{
					LoadClass->SetDefaultObject(CDO);
				}
			}
		}
	}
...
void FLinkerLoad::ResolveDeferredExports(UClass* LoadClass)
{
	...
	if (DeferredCDOIndex != INDEX_NONE)
	{
		...
		{
			LoadClass->SetDefaultObject(nullptr);
			...
			DEFERRED_DEPENDENCY_CHECK(LoadClass->GetDefaultObject(false) == ExportObj);
		}
	}

	UObject* BlueprintCDO = DeferredCDOIndex != INDEX_NONE ? ToRawPtr(ExportMap[DeferredCDOIndex].Object) : LoadClass->GetDefaultObject(false);
...
{
	if (HasAnyInternalFlags(EInternalObjectFlags::AutoRTFMConstructionAborted))
	{
		// Object was constructed in a transaction that was aborted.
		// Return as the object is unlikely to be in a valid state.
		return;
	}
...
bool FDeferredCdoInitializationTracker::DeferPreload(UObject* Object)
{
	...
	else
	{
		...
		else if (Object->HasAnyFlags(RF_InheritableComponentTemplate))
		{
			...
			DEFERRED_DEPENDENCY_CHECK(OwningClass && OwningClass->GetDefaultObject(false));
			if (OwningClass)
			{
				bDeferPostload = ShouldDeferSubObjectPreload(OwningClass->GetDefaultObject(false));
			}
		}
	}
