...
struct FLongPackagePathsSingleton
{
		...
		FMountPoint::EMountFlags Flags)
	{
		...
		return FMountPoint(FString(RootPath), MoveTemp(RelativeContentPath), FString(AbsolutePathBuilder), Flags);
	}
	...
	{	
		...
		{
			UE::TWriteScopeLock ScopeLock(MountLock);
	...
	{
		UE::TReadScopeLock ScopeLock(MountLock);
...
#endif
	{
		...
		auto LocalInsertMountPoint = [this](const FString& RootPath, const FString& ContentPathRelative, EMountFlags InFlags)
		{
			...
			FMountPoint& MountPoint = *MountPoints.Add_GetRef(MakeUnique<FMountPoint>(
				FString(RootPath), FString(ContentPathRelative), FString(AbsolutePathBuilder),
				InFlags));
...
void FPackageName::InternalFilenameToLongPackageName(FStringView InFilename, FStringBuilderBase& OutPackageName)
{
	...
	{
		UE::TReadScopeLock ScopeLock(Paths.MountLock);
	...
	EFlexNameType* OutFlexNameType, EErrorCode* OutFailureReason)
{
	...
	if (OutExtension)
	{
		*OutExtension = ExtensionType == EPackageExtension::Custom ? *CustomExtension : LexToString(ExtensionType);
	}
	return true;
}
...
{
	return FLongPackagePathsSingleton::Get().MountPointExists(FStringView(RootPath));
}

FName FPackageName::GetPackageMountPoint(FStringView InPackagePath, bool InWithoutSlashes)
{
...
{
	return GetPackageMountPoint(FStringView(InPackagePath), InWithoutSlashes);
}

FName FPackageName::GetPackageMountPoint(const TCHAR* InPackagePath, bool InWithoutSlashes)
{
	return GetPackageMountPoint(FStringView(InPackagePath), InWithoutSlashes);
}
	...
	if (FPackageName::MountPointExists(PackageRoot))
	{
		...
		if (ensure(ConvertedToPath))
		{
			...
			if (PackageLocations == EPackageLocationFilter::None)
			{
				...
				SkippedPackagePath.SetHeaderExtension(EPackageExtension::EmptyString); // Suppress the warning from FPackagePath about unknown asset
...
FPackageName::EPackageLocationFilter FPackageName::DoesPackageExistEx(const FPackagePath& PackagePath, EPackageLocationFilter Filter, bool bMatchCaseOnDisk, FPackagePath* OutPackagePath)
{
	...
	if (AssetRegistry)
	{
		...
		UE::AssetRegistry::EExists ExistsStatus = AssetRegistry->TryGetAssetPackageData(PackageName, AssetPackageData, CorrectCasePackageName);
		if (ExistsStatus == UE::AssetRegistry::EExists::Exists)
		{
			EPackageLocationFilter PackageLocation = AssetPackageData.GetPackageLocation();

			// If the registry doesn't have this information at all, we can't trust it (e.g. reading older cache version)
			if (PackageLocation != EPackageLocationFilter::None)
			{
				PackageLocation = EPackageLocationFilter((uint8)Filter & (uint8)PackageLocation);
				if (!OutPackagePath)
				{
				...
				*OutPackagePath = FPackagePath::FromPackageNameUnchecked(MoveTemp(CorrectCasePackageName));
				if ((uint8)PackageLocation & (uint8)EPackageLocationFilter::FileSystem)
				{
					// If an OutPackagePath was provided but a custom or unspecified extension was used we must use the disk
					// scanning code futher down to allow the PackageResourceManager to handle the unspecified and custom extension cases
				...
				{
					// IODispatcher packagepaths use the EmptyString HeaderExtension to indicate they are an IODispatcher path.
					// We do need to provide the EmptyString extension, rather than leaving it unspecified, to suppress wasted effort and
					// warnings from FPackagePath about missing extensions.
					// For IODispatcher paths we respect an existing header extension and don't overwrite it. Unknown whether that's
					// important; it has been that way for a while so some systems may be relying on it.
...
FPackageName::EPackageLocationFilter FPackageName::InternalDoesPackageExistEx(const FPackagePath& PackagePath, EPackageLocationFilter Filter, bool bMatchCaseOnDisk, FPackagePath* OutPackagePath)
{
	...
	{
		UE_LOG(LogPackageName, Error, TEXT("DoesPackageExist: DoesPackageExist FAILED: '%s' is not a long packagename name. Reason: %s"), *PackageName, *Reason.ToString());
		return EPackageLocationFilter::None;
	}
...
{
	// Ignore empty paths, or paths that specify a package rather than an object within the package
	if (!AssetPath.IsValid() || AssetPath.GetAssetName().IsNone())
	{
		return {};
	}

	TStringBuilder<128> PackageNameBuilder;
	AssetPath.GetPackageName().ToString(PackageNameBuilder);
	...
	FStringView RelativePath = PackageName.RightChop(MountPointName.Len() + 1);

	// Remove the leaf name
	int32 IndexOfLastSlash = INDEX_NONE;
	if (RelativePath.FindLastChar(TEXT('/'), IndexOfLastSlash))
	{
		if (!ensureMsgf(IndexOfLastSlash != RelativePath.Len() - 1, TEXT("Top level asset path '%s' missing leaf name."), *PackageNameBuilder))
		{
			return {};
		}

		RelativePath.LeftInline(IndexOfLastSlash);
	}
	if (!ensureMsgf(IndexOfLastSlash != INDEX_NONE, TEXT("Top level asset path '%s' should have at least two forward slashes."), *PackageNameBuilder))
	{
		return {};
	}

	TStringBuilder<128> AssetNameBuilder;
	AssetPath.GetAssetName().ToString(AssetNameBuilder);

	FString VerseModule = FPaths::Combine(PluginVersePath, RelativePath, AssetNameBuilder);

	UE::Core::FVersePath Result;
#if NO_LOGGING
	UE::Core::FVersePath::TryMake(Result, MoveTemp(VerseModule));
#else
	if (!UE::Core::FVersePath::TryMake(Result, VerseModule))
	{
		...
		if (!bAlreadyInSet)
		{
			UE_LOG(LogCore, Display, TEXT("Unable to make a VersePath for object '%s' with path '%s'"), *AssetPath.ToString(), *VerseModule);
		}
	}
#endif
...
void FPackageName::SplitPackageName(FStringView InPackageName, FStringView* OutPackageRoot,
	FStringView* OutPackagePath, FStringView* OutPackageLeafName, EPathFormatFlags Flags)
{
	auto SetOutput = [OutPackageRoot, OutPackagePath, OutPackageLeafName]
	(FStringView InPackageRoot, FStringView InPackagePath, FStringView InPackageLeafName)
		{
			if (OutPackageRoot)
			{
				*OutPackageRoot = InPackageRoot;
			}
			if (OutPackagePath)
			{
				*OutPackagePath = InPackagePath;
			}
			if (OutPackageLeafName)
			{
				*OutPackageLeafName = InPackageLeafName;
			}
		};

	if (!InPackageName.StartsWith(TEXT("/")))
	{
		SetOutput(FStringView(), FStringView(), FStringView());
		return;
	}

	int32 SecondSlashIndex;
	if (!InPackageName.RightChop(1).FindChar('/', SecondSlashIndex))
	{
		SetOutput(FStringView(), FStringView(), FStringView());
		return;
	}
	check(SecondSlashIndex != INDEX_NONE);
	// Change back to an index relative to the beginnning of InPackageName rather than relative to InPackageName[1].
	SecondSlashIndex += 1; 

	FStringView PackageRoot(InPackageName.Left(SecondSlashIndex + 1));
	FStringView RemainingPath = InPackageName.RightChop(SecondSlashIndex + 1);
	FStringView PackagePath;
	FStringView PackageNameLeaf;

	// Don't use FPathViews, because it has different rules for edge cases than we do, and adds some complexity
	// around Extension to convert between its rules and our rules.

	int32 LastSlashIndex;
	if (!RemainingPath.FindLastChar('/', LastSlashIndex))
	{
		// "/root/leaf"
		PackageNameLeaf = RemainingPath;
	}
	else
	{
		if (LastSlashIndex == RemainingPath.Len() - 1)
		{
			// "/root/path/"
			PackagePath = RemainingPath;
		}
		else
		{
			// "/root/path/leaf"
			PackagePath = RemainingPath.Left(LastSlashIndex + 1);
			PackageNameLeaf = RemainingPath.RightChop(LastSlashIndex + 1);
		}
	}

	// Convert to the requested output format
	if (!EnumHasAnyFlags(Flags, EPathFormatFlags::MountPointLeadingSlash))
	{
		PackageRoot.RightChopInline(1);
	}
	if (!EnumHasAnyFlags(Flags, EPathFormatFlags::MountPointTrailingSlash))
	{
		PackageRoot.LeftChopInline(1);
	}
	SetOutput(PackageRoot, PackagePath, PackageNameLeaf);
}

void FPackageName::SplitPackageName(const FString& InLongPackageName, FString* OutPackageRoot,
	FString* OutPackagePath, FString* OutPackageLeafName, EPathFormatFlags Flags)
{
	FStringView PackageRoot;
	FStringView PackagePath;
	FStringView PackageLeafName;
	SplitPackageName(InLongPackageName, &PackageRoot, &PackagePath, &PackageLeafName, Flags);
	if (OutPackageRoot)
	{
	...
	if (OutPackageLeafName)
	{
		*OutPackageLeafName = PackageLeafName;
	}
}
...
{
	if (!IsValidPath(InLongPackageName))
	{
	...
	SplitPackageName(InLongPackageName, &OutPackageRoot, &OutPackagePath, &OutPackageName,
		bStripRootLeadingSlash ? EPathFormatFlags::MountPointTrailingSlash : EPathFormatFlags::MountPointSlashes);
	return true;
}

FString FPackageName::SplitPackageNameRoot(FName InPackageName, FString* OutRelativePath, EPathFormatFlags Flags)
{
	...
	FStringView Root = SplitPackageNameRoot(PackageName, &RelativePath, Flags);
...
{
	constexpr FStringView EngineTransientPrefix(TEXTVIEW("/Engine/Transient"));
	return InPackageName.StartsWith(EngineTransientPrefix, ESearchCase::IgnoreCase) &&
		(InPackageName.Len() == EngineTransientPrefix.Len() || InPackageName[EngineTransientPrefix.Len()] == '/');
}
