...
enum EBulkDataFlags : uint32
{
	...
	/** DEPRECATED */
	BULKDATA_HasAsyncReadPending UE_DEPRECATED(5.5, "This flag no longer does anything")  = 1 << 29,
...
public:
	...
	* Deprecated
	*
	* @return As of 5.5 this will always return true as ::StartAsyncLoading was removed
	*/
	UE_DEPRECATED(5.5, "No longer provides any functionality (assume that this will always return true")
...
private:
	...
	UE_DEPRECATED(5.5, "No longer provides any functionality")
...
public:
	...
	{
		// Safely clean up any entries that the current FFormatContainer has before creating/adding copies from the provided container.
		FlushData();

		for (const TPair<FName, FByteBulkData*>& Format : Other.Formats)
		{
			FByteBulkData* Copy = new FByteBulkData(*Format.Value);
			Formats.Add(Format.Key, Copy);
		}
	}
...
public:
	...
	EStatus GetStatus() const { return Handle.IsValid() ? Handle->GetStatus() : EStatus::None; }

	/** Returns whether the request is associated with a pending or completed request. */
	bool IsNone() const { return FBulkDataRequest::EStatus::None == GetStatus(); }

	/** Returns whether the request is pending. */
	bool IsPending() const { return FBulkDataRequest::EStatus::Pending == GetStatus(); }

	/** Returns whether the request completed successfully. */
	bool IsOk() const { return FBulkDataRequest::EStatus::Ok == GetStatus(); }

	/** Returns whether the request has been completed. */
	bool IsCompleted() const { return GetStatus() > EStatus::Pending; }

	/** Cancel the pending request. Cancelling an invalid request will immediately return. */
	void Cancel() { if (Handle.IsValid()) { Handle->Cancel(); } }

	/** Reset the request handle to an invalid state. Will cancel and wait if the request is not completed. */
	void Reset() { Handle.SafeRelease(); }

protected:
	friend class FHandleBase;
	friend class FChunkBatchReadRequest;
	friend class FBulkDataBatchRequest;
