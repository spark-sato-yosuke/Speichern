...
PRAGMA_ENABLE_DEPRECATION_WARNINGS
	...
	{
		if (PackageFlags & PKG_NotExternallyReferenceable)
		{
			return EAssetAccessSpecifier::Private;
		}
		else if (PackageFlags & PKG_AccessSpecifierEpicInternal)
		{
...
public:
...
#else
	UE_DEPRECATED(5.6, "No longer supported in non-editor configurations, to save memory.")
	COREUOBJECT_API FIoHash GetPackageSavedHash() const;
	UE_DEPRECATED(5.6, "No longer supported in non-editor configurations, to save memory.")
	COREUOBJECT_API void SetPackageSavedHash(const FIoHash& InHash);
#endif
	...
	{	
		// Clear existing location flags
		Flags = Flags & ~FLAG_PACKAGE_LOCATION_MASK;

		static_assert((uint8)FPackageName::EPackageLocationFilter::MaxBit <= 2, "We are storing PackageLocation in two bits");
		check(PackageLocation <= FPackageName::EPackageLocationFilter::ValidMask);
		Flags |= uint32(PackageLocation) << FLAG_PACKAGE_LOCATION_TO_ENUM_SHIFT_SIZE;
	}

	FPackageName::EPackageLocationFilter GetPackageLocation() const
	{
		return FPackageName::EPackageLocationFilter((Flags & FLAG_PACKAGE_LOCATION_MASK) >> FLAG_PACKAGE_LOCATION_TO_ENUM_SHIFT_SIZE);
	}
...
private:
	...
	enum : uint32
	{
		FLAG_LICENSEE_VERSION						= 1u << 0,
		FLAG_HAS_VIRTUALIZED_PAYLOADS				= 1u << 1,

		FLAG_PACKAGE_LOCATION_IODISPATCHER			= 1u << 30,
		FLAG_PACKAGE_LOCATION_FILESYSTEM			= 1u << 31,

		FLAG_PACKAGE_LOCATION_TO_ENUM_SHIFT_SIZE	= 30u,
		FLAG_PACKAGE_LOCATION_MASK					= uint32(FPackageName::EPackageLocationFilter::ValidMask) << FLAG_PACKAGE_LOCATION_TO_ENUM_SHIFT_SIZE
	};
};
