...
public:
	...
	[[nodiscard]] UE_API explicit FPropertyTypeName(const FProperty* Property);
	 ...
	 * - GetParameterCount() -> 2
	 */
	[[nodiscard]] UE_API int32 GetParameterCount() const;
	 ...
	 */
	[[nodiscard]] UE_API FPropertyTypeName GetParameter(int32 ParamIndex) const;
	 ...
	 */
	[[nodiscard]] inline FName GetParameterName(int32 ParamIndex) const
	{
	...
	[[nodiscard]] UE_API bool IsStruct(FName StructName) const;
	/** Returns true if this is EnumProperty or ByteProperty with a first parameter of EnumName. */
	[[nodiscard]] UE_API bool IsEnum(FName EnumName) const;
...
public:
	...
	[[nodiscard]] UE_API FPropertyTypeName Build() const;
	 ...
	 */
	[[nodiscard]] UE_API bool TryParse(FStringView Name);
