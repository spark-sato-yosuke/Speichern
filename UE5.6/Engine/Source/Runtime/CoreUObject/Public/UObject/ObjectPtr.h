...
{
public:
	[[nodiscard]] FObjectPtr()
		: Handle(UE::CoreUObject::Private::MakeObjectHandle(nullptr))
	{
	}

	[[nodiscard]] explicit FObjectPtr(ENoInit)
	{
	}

	[[nodiscard]] FORCEINLINE FObjectPtr(TYPE_OF_NULLPTR)
		: Handle(UE::CoreUObject::Private::MakeObjectHandle(nullptr))
	{
	}
...
#if UE_OBJECT_PTR_GC_BARRIER
	[[nodiscard]] FObjectPtr(FObjectPtr&& InOther)
		: Handle(MoveTemp(InOther.Handle))
	{
		ConditionallyMarkAsReachable(*this);
	}

	[[nodiscard]] FObjectPtr(const FObjectPtr& InOther)
...
#else
	[[nodiscard]] FObjectPtr(FObjectPtr&&) = default;
	[[nodiscard]] FObjectPtr(const FObjectPtr&) = default;
	...
	[[nodiscard]] FORCEINLINE bool operator==(FObjectPtr Other) const { return (Handle == Other.Handle); }
	[[nodiscard]] FORCEINLINE bool operator!=(FObjectPtr Other) const { return (Handle != Other.Handle); }
...
public:
	...
	[[nodiscard]] explicit FORCEINLINE TObjectPtr(ENoInit)
		: ObjectPtr(NoInit)
	{
	}

	[[nodiscard]] FORCEINLINE TObjectPtr(TYPE_OF_NULLPTR)
		: ObjectPtr(nullptr)
	{
	}

	[[nodiscard]] explicit FORCEINLINE TObjectPtr(FObjectPtr ObjPtr)
		: ObjectPtr(ObjPtr)
	{
	}
	...
	>
	[[nodiscard]] FORCEINLINE TObjectPtr(const TObjectPtr<U>& Other)
		: ObjectPtr(Other.ObjectPtr)
	{
	...
	>
	[[nodiscard]] FORCEINLINE TObjectPtr(const U& Object)
	...
	>
	[[nodiscard]] FORCEINLINE bool operator==(const TObjectPtr<U>& Other) const
	{
#if UE_WITH_OBJECT_HANDLE_TYPE_SAFETY
	...
	>
	[[nodiscard]] FORCEINLINE bool operator==(const U& Other) const
	{
	...
	template <typename U>
	[[nodiscard]] FORCEINLINE auto operator!=(const U& Other) const -> decltype(!(*this == Other))
	{
		return !(*this == Other);
	}
#endif
...
>
[[nodiscard]] FORCEINLINE bool operator==(const U& Other, const TObjectPtr<T>& Ptr)
{
