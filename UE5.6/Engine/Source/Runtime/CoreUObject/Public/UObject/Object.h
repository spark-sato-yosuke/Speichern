...
namespace UE::Cook { enum class ECookEvent : uint32; }
...
public:
	...
	*							  Also allows storage of variables like bCleanupIsRequired for use in PostSaveRoot
	 ...
	 * @see UE::Cook::ECookEvent for the possible events.
	 */
	virtual void OnCookEvent(UE::Cook::ECookEvent CookEvent, UE::Cook::FCookEventContext& Context)
	{
	}
...
public:
	...
	COREUOBJECT_API virtual void PostRename(UObject* OldOuter, const FName OldName);
	...
	/**
	 * Called when an object is about to be migrated and after it's been migrated to another process.
	 */
	virtual void PostMigrate(const struct FUObjectMigrationContext& MigrationContext) {}

	virtual bool IsMigrationRoot() const { return false; }
#endif
	...
	*/
     ...
     * @remark A unique name will be generated if NewName is null and if either is true: NewOuter is null or NewOuter equals the current outer.
	 * 
...
consider using the AssetRegistry's AssetAddedEvent which fires on the game thread.")
	virtual void PostLoadAssetRegistryTags(const FAssetData& InAssetData, TArray<FAssetRegistryTag>& OutTagsAndValuesToUpdate) const final {}
...
public:
	...
	/** The metadata tags to be transferred from the FMetaData to the Asset Registry */
...
public:
	 ...
	 *
	 * @param Function function to call
	 * @param Parms arguments to the function call
	 * @param OutParms out parameter information (irrelevant for RPC's)
	 * @param Stack stack frame for the function call
	 * @return returns true if the function was processed by a NetDriver
	 */
