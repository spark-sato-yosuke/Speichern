...
struct FAccessContext : FContext
{
	...
	const FNativeFrame* NativeFrame() const
	{
	...
	void PushNativeFrame(VFailureContext* FailureContext, const VNativeFunction* Callee, const FOp* PC, VFrame* Frame, VTask* Task, const TFunctor& F)
	{
		GetImpl()->PushNativeFrame(FailureContext, Callee, PC, Frame, Task, F);
	}
...
struct FRunningContext : FAccessContext
{
	...
	template <typename TFunctor>
	bool EnterVM(TFunctor F) { return EnterVM_Internal(F, EEnterVMMode::Auto); }

	// Creates a new transaction and ensure a new matching failure context is created to match
	template <typename TFunctor>
	bool TransactThenEnterVM(TFunctor F)
	{
		bool bResult = false;
		AutoRTFM::TransactThenOpen([&] {
			bResult = EnterVM_Internal(F, EEnterVMMode::NewTransaction);
		});
		return bResult;
	}
...
private:
	...
	template <typename TFunctor>
	bool EnterVM_Internal(TFunctor F, EEnterVMMode Mode);
...
template <typename TFunctor>
void FContextImpl::PushNativeFrame(VFailureContext* FailureContext, const VNativeFunction* Callee, const FOp* PC, VFrame* Frame, VTask* Task, const TFunctor& F)
{
