...
struct FInputModeUIOnly : public FInputModeDataBase
{
	...
	ENGINE_API virtual ~FInputModeUIOnly();
...
struct FInputModeGameAndUI : public FInputModeDataBase
{
	...
	ENGINE_API virtual ~FInputModeGameAndUI();
...
struct FInputModeGameOnly : public FInputModeDataBase
{
	...
	ENGINE_API virtual ~FInputModeGameOnly();
...
public:
	...
	UE_DEPRECATED(5.6, "Use the version which takes in a DeviceId and Timestamp instead.")
	ENGINE_API virtual bool InputTouch(uint32 Handle, ETouchType::Type Type, const FVector2D& TouchLocation, float Force, FDateTime DeviceTimestamp, uint32 TouchpadIndex) final;

	/** Handles a touch screen action */
	ENGINE_API virtual bool InputTouch(
	...
	UE_DEPRECATED(5.6, "Use the version which takes in a DeviceId and Timestamp instead.")
	ENGINE_API virtual bool InputMotion(const FVector& Tilt, const FVector& RotationRate, const FVector& Gravity, const FVector& Acceleration) final;

	/** Handles motion control */
	ENGINE_API virtual bool InputMotion(const FInputDeviceId DeviceId, const FVector& Tilt, const FVector& RotationRate, const FVector& Gravity, const FVector& Acceleration, const uint64 Timestamp);
	...
	ENGINE_API virtual void PostMigrate(const FUObjectMigrationContext& MigrationContext) override;
#endif
...
public:
	...
	ENGINE_API virtual class UPlayer* GetNetOwningPlayerAnyRole() override;
...
protected:
	...
	ENGINE_API virtual void ProcessTouchHitResult(const FInputDeviceId DeviceId, uint32 Handle, ETouchType::Type Type, const FVector2D& TouchLocation, const FHitResult& HitResult, const bool bHit);
...
private:
	...
	// This is a server-specific variable so it should not be migrated between servers and remain non-UPROPERTY().
	bool bSendClientRestartRPC = false;

	/** The player id associated with this player controller. */
	UPROPERTY()
	FUniqueNetIdRepl CachedConnectionPlayerId;

	/** An optional client handshake id used by the connection associated with this player controller. */
	UPROPERTY(DuplicateTransient)
	uint32 ClientHandshakeId = 0;
};
