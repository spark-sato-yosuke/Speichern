...
public:
	...
	/** DEPRECTED, use RemoteViewPitch16 instead. */
	UE_DEPRECATED(5.6, "This property is deprecated, use GetRemoteViewPitch()/RemoteViewPitch16 instead.")
	UPROPERTY(replicated)
...
public:
	...
	/** Replicated pitch value so we can see where remote clients are looking. */
	FORCEINLINE uint16 GetRemoteViewPitch() const { return RemoteViewPitch16; }

	/**
	 * Set Pawn ViewPitch, so we can see where remote clients are looking.
	 * Maps 360.0 degrees into 16 bits
	...
	/** Sets controller for this actor */
	ENGINE_API void SetController(AController* InController);
	...
	ENGINE_API virtual UPlayer* GetNetOwningPlayerAnyRole() override;
