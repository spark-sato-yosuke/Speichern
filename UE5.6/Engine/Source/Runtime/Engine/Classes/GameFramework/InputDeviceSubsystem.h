...
public:
	...
	*/
	...
	ENGINE_API FInputDeviceId GetMostRecentlyUsedInputDeviceId(const FPlatformUserId InUserId, const EHardwareDevicePrimaryType OfType = EHardwareDevicePrimaryType::Unspecified) const;
	
	UFUNCTION(BlueprintCallable, Category = "Input Devices")
	ENGINE_API FHardwareDeviceIdentifier GetInputDeviceHardwareIdentifier(const FInputDeviceId InputDevice) const;

	/**
	 * Returns the latest used input device for a specific type of hardware
	 */
	ENGINE_API FInputDeviceId GetLatestDeviceOfType(const FPlatformUserId InUserId, const EHardwareDevicePrimaryType OfType) const;
...
protected:
	...
	*/
	...
	struct FLatestInputDeviceData
	{
		...
		/**
		 * A map of the latest used input device of each type for a single user.
		 * This allows us to query the most recently used device of a specific type for the player
		 */
		TMap<EHardwareDevicePrimaryType, FInputDeviceId> LatestDevices;
	};
	
	/** A map of platform user's to their most recent hardware device identifiers */
