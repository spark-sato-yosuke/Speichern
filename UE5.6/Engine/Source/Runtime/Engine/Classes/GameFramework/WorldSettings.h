...
public:
	...
	/**
	 * Allows game logic to temporarily disable all forms of time dilation (when using GetEffectiveTimeDilation().
	 * When false, logic is still allowed to set time dilation values, but their values will be ignored when calling
	 * GetEffectiveTimeDilation() until this is set back to true.
	 *
	 * Transient because it's used for temporary overrides and shouldn't be saved/loaded from level packages. Not
	 * replicated because in multiplayer a server should modify the real time dilation, instead of ignoring it on
	 * a client (which will lead to position desyncs due to delta time mismatches).
	*/
	UPROPERTY(Transient)
	uint8 bAllowTimeDilation : 1;
...
public:
	...
	// Set a custom gravity value for this level, in distance units per second squared.
...
public:
	...
	/** Lowest acceptable cinematic time dilation. */
	UPROPERTY(config, EditAnywhere, Category = Tick, AdvancedDisplay, meta = (UIMin = "0", ClampMin = "0"))
	float MinCinematicTimeDilation;

	/** Highest acceptable cinematic time dilation. */
	UPROPERTY(config, EditAnywhere, Category = Tick, AdvancedDisplay, meta = (UIMin = "0", ClampMin = "0"))
	float MaxCinematicTimeDilation;
...
#if WITH_EDITORONLY_DATA
	/** Array of editor-only user data stored with the asset */
	UPROPERTY(EditAnywhere, AdvancedDisplay, Instanced, Category = AssetUserData)
...
public:
	...
	/**
	 * Multiplies all time dilation sources together and returns the effective overall time dilation. When bAllowTimeDilation is
	 * set to false, this will return 1.0 regardless of the time dilation sources. See SetAllowTimeDilation(...)/bAllowTimeDilation.
	 */
	...
	/** Sets whether or not Time Dilation is allowed. When true, GetEffectiveTimeDilation() will return the time dilation as determined by the various dilation factors, when false it will return 1.0f. */
	ENGINE_API virtual void SetAllowTimeDilation(bool NewAllowTimeDilation);

	/** Sets the cinematic time dilation value (subject to clamping). Returns the final value that was set (which may be clamped) even if bAllowTimeDilation is set to false. */
	ENGINE_API virtual float SetCinematicTimeDilation(float NewCinematicTimeDilation);
...
#if WITH_EDITORONLY_DATA
	// Bookmark that will be applied when the level is loaded in the editor.
	UPROPERTY(EditAnywhere, Category=Bookmark)
	TObjectPtr<class UBookmarkBase> DefaultBookmark;
#endif 
...
public:
	...
	DECLARE_MULTICAST_DELEGATE_OneParam(FOnDefaultBookmarkChanged, AWorldSettings*);
	static ENGINE_API FOnDefaultBookmarkChanged OnDefaultBookmarkChanged;
#endif
...
#if WITH_EDITOR
	/**
	 * Get the bookmark that will be applied when the map is opened in the editor.
	 */
	ENGINE_API class UBookmarkBase* GetDefaultBookmark() const;

	/**
	 * Set the bookmark that will be applied when the map is opened in the editor.
	 */
	ENGINE_API void SetDefaultBookmark(class UBookmarkBase* InDefaultBookmark);
#endif
