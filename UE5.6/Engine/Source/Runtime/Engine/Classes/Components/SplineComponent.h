...
PRAGMA_DISABLE_INTERNAL_WARNINGS
	UPROPERTY(Transient, DuplicateTransient, EditAnywhere, Category=Points)
	FSpline Spline;
PRAGMA_ENABLE_INTERNAL_WARNINGS
	
public:

	// Gives the name of the backing spline property to allow users
	// the ability to correctly trigger property change events.
	ENGINE_API static FName GetSplinePropertyName();
...
public:
	...
	// Safe for caching spline data in the legacy format. Will auto convert data if necessary.
	ENGINE_API FSplineCurves GetSplineCurves() const;
	ENGINE_API void SetSpline(const FSplineCurves& InSplineCurves);

	ENGINE_API int32 GetVersion() const;
	...
	ENGINE_API bool CanResetToDefault() const;
	...
	ENGINE_API void SetQuaternionAtSplinePoint(int32 PointIndex, const FQuat& InQuaternion, ESplineCoordinateSpace::Type CoordinateSpace, bool bUpdateSpline = true);
	...
	UFUNCTION(BlueprintCallable, Category=Spline)
	ENGINE_API float FindInputKeyOnSegmentClosestToWorldLocation(const FVector& WorldLocation, int32 Index) const;
	...
	/**
	 * Get a vector attribute value by parameter.
	 * @param InKey The parameter to use when evaluating the specified attribute channel.
	 * @param PropertyName The name of the attribute channel to evaluate.
	 * @return The attribute value.
	 */
	...
	// todo: deprecate
	FInterpCurveVector& GetSplinePointsPosition() { return WarninglessSplineCurves().Position; }
	FInterpCurveQuat& GetSplinePointsRotation() { return WarninglessSplineCurves().Rotation; }
	FInterpCurveVector& GetSplinePointsScale() { return WarninglessSplineCurves().Scale; }
...
private:
	...
	/**
	 * This function takes on the roles of the previously existing functions GetPositionPointSafe, GetRotationPointSafe, and GetScalePointSafe.
	 * Specifically, it makes sure that a provided index is valid by
	 * 1) Specially handling Index == NumPoints if bClosedLoop is true in order to allow users to imagine the connecting point as an extra virtual point
	 * 2) Preventing out of bound accesses by clamping the index. This clamping happens after 1).
	 *
	 * This function is only used where the 3 aforementioned functions were called. It should probably be used more widely and consistently in the future.
	 */
	int32 GetClampedIndex(int32 Index) const
	{
		// Treat Index == GetNumberOfSplinePoints() as Index == 0, then clamp to valid index range.
		return FMath::Clamp(bClosedLoop && Index == GetNumberOfSplinePoints() ? 0 : Index, 0, GetNumberOfSplinePoints() - 1);
	}
