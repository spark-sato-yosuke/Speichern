...
private:
	...
	/** Internally used to know when CalcBounds is called by UpdateBounds. */
	uint8 bIsUpdatingBounds : 1;
...
public:
	...
	ENGINE_API virtual bool SetCustomData(int32 InstanceIndexStart, int32 InstanceIndexEnd, TConstArrayView<float> CustomDataFloats, bool bMarkRenderStateDirty = false);
...
public:
	...
	void OnPostPopulatePerInstanceData() { OnPostLoadPerInstanceData(); }
...
protected:
	...
	UE_DEPRECATED(5.5, "Use version with FBox")
	 ...
	 * @param BoundTransform The bounds transform
	 * @param InstanceBounds The instance bounds
	 */
...
private:
	...
	FBoxSphereBounds GetOrCalcBounds(const FTransform& InBoundTransform, EBoundsCacheType InType) const;

	UPROPERTY()
	mutable FBoundsCacheElement CachedBounds[(int)EBoundsCacheType::Count];
};
