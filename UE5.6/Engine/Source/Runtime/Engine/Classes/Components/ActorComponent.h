...
public:
	...
	const UE::FTimeout& GetIncrementalUpdateTimeout() const { return Timeout; }
	int32 GetIncrementalUpdateAddPrimitiveBatchSize() const { return IncrementalUpdateAddPrimitiveBatchSize; }
	int32 GetIncrementalUpdateNumComponentsToUpdate() const { return IncrementalUpdateNumComponentsToUpdate; }
	...
	UE_DEPRECATED(5.5, "This function is not used anymore")
	int32 Count() const { return AddPrimitiveBatches.Num(); }

	void Process();

private:
	bool OnIncrementalRegisterComponentsDone();

	UWorld* World;
	UE::FTimeout Timeout;
	int32	IncrementalUpdateAddPrimitiveBatchSize;
	int32	IncrementalUpdateNumComponentsToUpdate;
	FAsyncRegisterLevelContext* AsyncRegisterLevelContext;
	
	FPrimitiveBatch AddPrimitiveBatches;
	FPrimitiveBatch SendRenderDynamicDataPrimitives;
...
class UActorComponent : public UObject, public IInterface_AssetUserData, public IAsyncPhysicsStateProcessor
{
	GENERATED_BODY()
...
private:
	...
	/** If non-null, physics state creation/destruction is done asynchronously by FPhysScene_Chaos::ProcessAsyncPhysicsStateJobs. */
	FPhysScene* PhysicsStateAsyncCreationScene;
	...
	enum struct EPhysicsStateAsyncCreationState : uint8
	{
		None,
		Creating,
		Destroying
	...
	/** Whether the component is creating/destroying its physics state asynchronously (or not) */
	EPhysicsStateAsyncCreationState PhysicsStateAsyncCreationState : 2;

#if UE_WITH_REMOTE_OBJECT_HANDLE
	uint8 bComponentTickEnabled : 1 = false;
#endif
...
public:
	...
	{
		return PhysicsStateAsyncCreationScene && (PhysicsStateAsyncCreationState == EPhysicsStateAsyncCreationState::Creating);
	}

	/** Whether asynchronous physics state destruction is running or not. */
	bool IsAsyncDestroyPhysicsStateRunning() const
	{
		return PhysicsStateAsyncCreationScene && (PhysicsStateAsyncCreationState == EPhysicsStateAsyncCreationState::Destroying);
	}
...
protected:
	...
	ENGINE_API virtual bool ShouldAsyncCreatePhysicsState(UWorld* WorldContext) const;

	/** Returns whether this component should destroy its physics state asynchronously. */
	ENGINE_API virtual bool ShouldAsyncDestroyPhysicsState() const;
...
public:
	...
	ENGINE_API virtual bool IsAsyncPhysicsStateCreated() const override final;
	ENGINE_API virtual UObject* GetAsyncPhysicsStateObject() const override final;
	ENGINE_API virtual bool OnAsyncCreatePhysicsState(const UE::FTimeout& Timeout) override;
	ENGINE_API virtual void OnAsyncCreatePhysicsStateEnd_GameThread() override;
	virtual bool OnAsyncDestroyPhysicsState(const UE::FTimeout& Timeout) override { return true; }
	...
	*/
	...
	/**
	 * Pre-registers a component with a specific world. Only called on components that return ShouldIncrementalPreRegister().
	 * Will initialize the necessary before calling OnPreRegister().
	 * @param InWorld - The world to pre-register the component with.
	 */
	...
	FORCEINLINE bool IsPreRegistering() const
	{
		return RegistrationState == EComponentRegistrationState::PreRegistering;
	}

	/** See if this component is currently pre-registered */
	FORCEINLINE bool IsPreRegistered() const
	{
		return RegistrationState == EComponentRegistrationState::PreRegistered;
	}

	/** See if this component is currently pre-unregistering */
	FORCEINLINE bool IsPreUnregistering() const
	{
		return RegistrationState == EComponentRegistrationState::PreUnregistering;
	}

	/** See if this component is currently pre-unregistered */
	FORCEINLINE bool IsPreUnregistered() const
	{
		return RegistrationState == EComponentRegistrationState::PreUnregistered;
	}
	...
	/** Override to allow a component subclass to compute an hash for the purpose of HLOD generation. */
	virtual void ComputeHLODHash(class FHLODHashBuilder& HashBuilder) const	{}
#endif
...
#if WITH_EDITOR
	friend struct SkeletalMeshUCSModifiedDataCleanupHelper;
#endif
