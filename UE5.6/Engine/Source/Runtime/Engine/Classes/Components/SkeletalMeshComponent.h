...
namespace EPhysicsTransformUpdateMode
{
	enum Type : int
	{
		SimulationUpatesComponentTransform UMETA(DisplayName="Simulation Updates Component Transform"), 
...
class USkeletalMeshComponent : public USkinnedMeshComponent, public IInterface_CollisionDataProvider
{
	...
	friend struct FRigUnit_AnimNextRemapPose;
...
public:
	...
	/** Sets the SkeletalMeshAsset to the current SkeletalMesh. */
	...
	UPROPERTY(EditAnywhere, BlueprintReadOnly, Setter = SetAnimInstanceClass_Internal, Category = Animation, meta=(EditCondition = bEnableAnimation))
...
public:
	...
	UE_DEPRECATED(5.6, "Direct access to this property is deprecated, please use GetAnimCurves or GetCurveValue instead. We will move to private in the future.")
...
public:
	...
	*/
	...
	ENGINE_API bool GetCurveValue(FName CurveName, float DefaultValue, float& Value) const;

	/** Get the current set of curve values */
	ENGINE_API const FBlendedHeapCurve& GetAnimCurves() const;
...
public:
	...
	*/
	...
	ENGINE_API void SetEnableAnimation(bool bNewEnableAnimation);
...
private:
	...
	/** previous root body scale to compare the difference when the actor scale is updated. */
	FVector3f PrevRootRigidBodyScale;
...
public:
	...
	*/
	...
	ENGINE_API virtual void GetUpdateClothSimulationData_AnyThread(TMap<int32, FClothSimulData>& OutClothSimulData, FMatrix& OutLocalToWorld, float& OutClothBlendWeight) const override;
...
public:
	...
	DECLARE_MULTICAST_DELEGATE_OneParam(FOnSkeletalMeshUnregisterd, const USkeletalMeshComponent*)
	FOnSkeletalMeshUnregisterd OnSkeletalMeshUnregistered;
	typedef FOnSkeletalMeshUnregisterd::FDelegate FOnSkeletalMeshUnregisteredDelegate;

	/** Register / Unregister delegates called when the mesh component is unregistered. */
	ENGINE_API FDelegateHandle RegisterOnSkeletalMeshUnregistered(const FOnSkeletalMeshUnregisteredDelegate& Delegate);
	ENGINE_API void UnregisterOnSkeletalMeshUnregistered(FDelegateHandle Handle);
...
protected:
	/**
	 * If PhysicsTransformUpdateMode is set to SimulationUpdatesComponentTransform, this returns the 
...
public:
	...
	*/
	...
	/** Update rigid body scaling when the actor is set with a new uniform scale. */
	void UpdateRigidBodyScaling(ETeleportType TeleportType);
...
public:
	...
	UE_DEPRECATED(5.6, "These should not be used as they rely on a global delegate, ANY change in LOD from ANY skeletal mesh component triggers the callback")
...
private:
	// NOTE: We anticipate that this will go away in the future, it is meant to be a temporary solution
	/** Register/Unregister for OnLODRequiredBonesUpdate callback */
	ENGINE_API FDelegateHandle RegisterOnLODRequiredBonesUpdate_Member(const FOnLODRequiredBonesUpdate& Delegate);
	ENGINE_API void UnregisterOnLODRequiredBonesUpdate_Member(const FDelegateHandle& DelegateHandle);
	friend class UE::AnimNext::FDataRegistry;
...
private:
	...
	/** Multicaster fired when this component finalizes the regeneration of the required bones list for the current LOD */
	static ENGINE_API FOnLODRequiredBonesUpdateMulticast OnLODRequiredBonesUpdate_Static;
	FOnLODRequiredBonesUpdateMulticast OnLODRequiredBonesUpdate_Member;
