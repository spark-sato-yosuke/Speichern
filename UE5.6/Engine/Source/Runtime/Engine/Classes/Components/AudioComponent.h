...
class UAudioComponent : public USceneComponent, public ISoundParameterControllerInterface, public FQuartzTickableObject
{
	...
	ENGINE_API void SetOverrideAttenuation(bool bInOverrideAttenuation);
...
public:
	...
	UPROPERTY(EditAnywhere, BlueprintReadWrite, Category = Sound, meta = (ClampMin = "0.0", UIMin = "0.0", EditCondition = "bEnableHighPassFilter", DisplayAfter = "LowPassFilterFrequency"))
	...
	UFUNCTION(BlueprintSetter)
	ENGINE_API void SetAttenuationSettings(USoundAttenuation* InAttenuationSettings);
	...
	UFUNCTION(BlueprintSetter)
	ENGINE_API void SetAttenuationOverrides(const FSoundAttenuationSettings& InAttenuationOverrides);
	...
	/** The interval at which to re-test occlusion (in seconds).  Values < 0.f will only check occlusion once when we start playback */
	...
	*/
	...
	static ENGINE_API FTransactionallySafeCriticalSection AudioIDToComponentMapLock;
...
public:
	 ...
	 */
	UFUNCTION(BlueprintCallable, Category = "Audio|Components|Audio")
	ENGINE_API void SetHighPassFilterEnabled(bool InHighPassFilterEnabled);
	 ...
	 */
	UFUNCTION(BlueprintCallable, Category = "Audio|Components|Audio")
	ENGINE_API void SetHighPassFilterFrequency(float InHighPassFilterFrequency);
