...
class UVolumetricCloudComponent : public USceneComponent
{
	...
	UMaterialInterface* GetMaterial() const { return MaterialLifetimePtr.Get(); }
	...
	UFUNCTION(BlueprintCallable, Category = "Rendering")
	ENGINE_API void SetVisibleInRealTimeSkyCaptures(bool bValue);
...
private:
	...
	// The material is assigned to a regular pointer so that the garbage collector to not get ride of it.
	UPROPERTY()
	TObjectPtr<UMaterialInterface> MaterialLifetimePtr;
