...
struct FClusterNode_DEPRECATED
{
	...
	FClusterNode_DEPRECATED()
		...
		, FirstChild(-1)
		, BoundMax(-MAX_flt, -MAX_flt, -MAX_flt)
		, LastChild(-1)
		, FirstInstance(-1)
		, LastInstance(-1)
	{
...
class UHierarchicalInstancedStaticMeshComponent : public UInstancedStaticMeshComponent
{
	...
	// This can help for implementations (foliage) where we can have instances with offsets to large for single float precision.
	UPROPERTY()
...
protected:
	...
	ENGINE_API virtual void BuildTree();
	ENGINE_API virtual void BuildTreeAsync();
