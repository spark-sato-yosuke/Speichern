...
enum class EFirstPersonPrimitiveType : uint8
{
	...
	/** Primitive represents a first person primitive in world space. This is the primitive that other players would see and is used to cast shadow onto the ground among other things.
	...
	WorldSpaceRepresentation UMETA(DisplayName = "World Space Representation"),
};
...
public:
	...
	UPROPERTY(EditAnywhere, AdvancedDisplay, BlueprintReadOnly, Category = Rendering, meta=(EditCondition="FirstPersonPrimitiveType != EFirstPersonPrimitiveType::WorldSpaceRepresentation"))
	uint8 bOwnerNoSee:1;
	 ...
	 */
	UPROPERTY(EditDefaultsOnly, BlueprintReadWrite, AdvancedDisplay, Category=Rendering)
	...
	UPROPERTY(EditAnywhere, BlueprintReadOnly, Category=Lighting, AdvancedDisplay, meta=(EditCondition="CastShadow && (FirstPersonPrimitiveType != EFirstPersonPrimitiveType::FirstPerson)", DisplayName = "Dynamic Shadow"))
	...
	UPROPERTY(EditAnywhere, BlueprintReadOnly, Category=Lighting, AdvancedDisplay, meta=(EditCondition="CastShadow && (FirstPersonPrimitiveType != EFirstPersonPrimitiveType::FirstPerson)", DisplayName = "Static Shadow"))
	 ...
	 */
	UPROPERTY(EditAnywhere, AdvancedDisplay, BlueprintReadOnly, Category=Lighting, meta=(EditCondition="CastShadow && (FirstPersonPrimitiveType != EFirstPersonPrimitiveType::FirstPerson)", DisplayName = "Volumetric Translucent Shadow"))
	 ...
	 */
	UPROPERTY(EditAnywhere, AdvancedDisplay, BlueprintReadOnly, Category=Lighting, meta=(EditCondition="CastShadow && (FirstPersonPrimitiveType != EFirstPersonPrimitiveType::FirstPerson)", DisplayName = "Far Shadow"))
	 ...
	 */
	UPROPERTY(EditAnywhere, AdvancedDisplay, BlueprintReadOnly, Category=Lighting, meta=(EditCondition="CastShadow && (FirstPersonPrimitiveType != EFirstPersonPrimitiveType::WorldSpaceRepresentation)", DisplayName = "Hidden Shadow"), Interp)
...
#if WITH_EDITOR
	ENGINE_API virtual void ComputeHLODHash(class FHLODHashBuilder& HashBuilder) const override;
#endif
...
public:	
	...
	FThreadSafeCounter* GetAttachmentCounter() { return &GetSceneData().AttachmentCounter; }
...
public:
	...
	*/
	...
	ENGINE_API virtual bool ShouldComponentIgnoreHitResult(FHitResult const& TestHit, EMoveComponentFlags MoveFlags);
	...
	/** Set custom primitive data, two floats at once, from index DataIndex to index DataIndex + 1. This sets the run-time data only, so it doesn't serialize. */
	ENGINE_API void SetCustomPrimitiveDataVector2f(int32 DataIndex, FVector2f Value);
	...
	/** Set custom primitive data, three floats at once, from index DataIndex to index DataIndex + 2. This sets the run-time data only, so it doesn't serialize. */
	ENGINE_API void SetCustomPrimitiveDataVector3f(int32 DataIndex, FVector3f Value);
	...
	/** Set custom primitive data, four floats at once, from index DataIndex to index DataIndex + 3. This sets the run-time data only, so it doesn't serialize. */
	ENGINE_API void SetCustomPrimitiveDataVector4f(int32 DataIndex, FVector4f Value);
	...
	UFUNCTION(BlueprintCallable, Category="Rendering|Material")
	ENGINE_API void SetCustomPrimitiveDataFloatArray(int32 DataIndex, const TArray<float>& Values);

	/** Set custom primitive data, an array of floats at once, from index DataIndex to index DataIndex + Values.Num(). This sets the run-time data only, so it doesn't serialize. */
	ENGINE_API void SetCustomPrimitiveDataFloatArray(int32 DataIndex, TConstArrayView<float> Values);
	...
	/** Set default custom primitive data, two floats at once, from index DataIndex to index DataIndex + 1, and marks the render state dirty */
	ENGINE_API void SetDefaultCustomPrimitiveDataVector2f(int32 DataIndex, FVector2f Value);
	...
	/** Set default custom primitive data, three floats at once, from index DataIndex to index DataIndex + 2, and marks the render state dirty */
	ENGINE_API void SetDefaultCustomPrimitiveDataVector3f(int32 DataIndex, FVector3f Value);
	...
	/** Set default custom primitive data, four floats at once, from index DataIndex to index DataIndex + 3, and marks the render state dirty */
	ENGINE_API void SetDefaultCustomPrimitiveDataVector4f(int32 DataIndex, FVector4f Value);
	...
	UFUNCTION(BlueprintCallable, Category="Rendering|Material")
	ENGINE_API void SetDefaultCustomPrimitiveDataFloatArray(int32 DataIndex, const TArray<float>& Values);

	/** Set default custom primitive data, an array floats at once, from index DataIndex to index DataIndex + Values.Num(), and marks the render state dirty */
	ENGINE_API void SetDefaultCustomPrimitiveDataFloatArray(int32 DataIndex, TConstArrayView<float> Values);
...
protected:
	...
	ENGINE_API void SetCustomPrimitiveDataInternal(int32 DataIndex, TConstArrayView<float> Values);

	/** Insert an array of floats into the CustomPrimitiveData defaults, starting at the given index */
	ENGINE_API void SetDefaultCustomPrimitiveData(int32 DataIndex, TConstArrayView<float> Values);
...
public:
	...
	*/
	 ...
	 * @return The proxy object.
	 */
	virtual HHitProxy* CreateMeshHitProxy(int32 SectionIndex, int32 MaterialIndex) const
	{
		return nullptr;
	}
#endif
...
public:
	...
	ENGINE_API virtual void OnAsyncDestroyPhysicsStateEnd_GameThread() override;
	ENGINE_API virtual void CollectBodySetupsWithPhysicsMeshesToCreate(TSet<UBodySetup*>& OutBodySetups) const override;
...
public:
	...
	UFUNCTION(BlueprintPure, Category = "Physics")
	ENGINE_API virtual bool GetGyroscopicTorqueEnabled() const;
...
protected:
	virtual bool CanBeUsedInPhysicsReplication(const FName BoneName = NAME_None) const { return true; }
...
public:
	...
	/** Whether the physics is static for this component. */
	FORCEINLINE bool IsStaticPhysics() const { return Mobility != EComponentMobility::Movable && bStaticWhenNotMoveable; }

	/** Wheter the component is relevant for first person */
	FORCEINLINE bool IsFirstPersonRelevant() const { return FirstPersonPrimitiveType != EFirstPersonPrimitiveType::None; }
protected:

	UFUNCTION()
	ENGINE_API virtual void HandlePostMigrationPhysicsStateChanged(UPrimitiveComponent* ChangedComponent, EComponentPhysicsStateChange StateChange);

#if UE_WITH_REMOTE_OBJECT_HANDLE

	ENGINE_API bool CanSendMigratedPhysicsState(const FUObjectMigrationContext* MigrationContext, EObjectMigrationSendType ObjMigrationSendType) const;

	ENGINE_API virtual void MigratePhysicsState(FArchive& Ar) override;
	
	ENGINE_API virtual void ApplyMigratedPhysicsState(Chaos::FSerializedDataBufferPtr MigratedStateData = nullptr) override;

	ENGINE_API virtual void PostMigrate(const struct FUObjectMigrationContext& MigrationContext) override;
#endif

protected:
	void AddNavigationModifier(FNavigationRelevantData& OutData) const;
};
