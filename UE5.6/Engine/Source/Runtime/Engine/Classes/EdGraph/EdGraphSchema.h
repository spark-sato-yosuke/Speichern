...
public:
	...
	UE_DEPRECATED(5.6, "Use the version of the function accepting FVector2f; this API no longer interfaces directly with double-precision scalars and vectors.")
	...
	PRAGMA_DISABLE_DEPRECATION_WARNINGS
		return PerformAction(ParentGraph, FromPin, FVector2D(Location), bSelectNewNode);
	...
	virtual UEdGraphNode* PerformAction(class UEdGraph* ParentGraph, TArray<UEdGraphPin*>& FromPins, const FVector2D Location, bool bSelectNewNode = true) UE_SLATE_DEPRECATED_VECTOR_VIRTUAL_FUNCTION
	{
		...
		if (FromPins.Num() > 0)
		{
			NewNode = PerformAction(ParentGraph, FromPins[0], UE::Slate::CastToVector2f(Location), bSelectNewNode);
		}
		else
		{
			NewNode = PerformAction(ParentGraph, nullptr, UE::Slate::CastToVector2f(Location), bSelectNewNode);
		}
	...
	{
	PRAGMA_DISABLE_DEPRECATION_WARNINGS
		return PerformAction(ParentGraph, FromPins, FVector2D(Location), bSelectNewNode);
	PRAGMA_ENABLE_DEPRECATION_WARNINGS
	}
...
struct FEdGraphSchemaAction_NewNode : public FEdGraphSchemaAction
{
	...
	ENGINE_API virtual UEdGraphNode* PerformAction(class UEdGraph* ParentGraph, UEdGraphPin* FromPin, const FVector2f& Location, bool bSelectNewNode = true) override;
	ENGINE_API virtual UEdGraphNode* PerformAction(class UEdGraph* ParentGraph, TArray<UEdGraphPin*>& FromPins, const FVector2f& Location, bool bSelectNewNode = true) override;
	...
	static ENGINE_API UEdGraphNode* CreateNode(class UEdGraph* ParentGraph, UEdGraphPin* FromPin, const UE::Slate::FDeprecateVector2DParameter& Location, class UEdGraphNode* InNodeTemplate);
};
...
public:
	...
	public:
		 ...
		 */
		ENGINE_API void PerformAction( class UEdGraph* ParentGraph, TArray<UEdGraphPin*>& FromPins, const UE::Slate::FDeprecateVector2DParameter& Location );
...
{
	virtual ~FGraphSchemaSearchTextDebugInfo() = default;
...
class UEdGraphSchema : public UObject
{
	...
	*/
...
#if WITH_EDITORONLY_DATA
	/**
	 * @return	True if the schema would like for failed connections to show the node panel menu so the user can pick an appropriate node
	 *
	 * Implementaion notes: Consider returning true if your node context menu handles muliple context pins for node indexing and creation.
	 * When the user attempts to connect two incompatible pins this will allow them to find a node which fullfills the requirements.
	 */
	ENGINE_API bool ShouldShowPanelContextMenuForIncompatibleConnections() const;
#endif
	...
	UE_DEPRECATED(5.6, "Use the version of the function accepting FVector2f; this API no longer interfaces directly with double-precision scalars and vectors.")
	virtual void OnPinConnectionDoubleCicked(UEdGraphPin* PinA, UEdGraphPin* PinB, const FVector2D& GraphPosition) const UE_SLATE_DEPRECATED_VECTOR_VIRTUAL_FUNCTION { }
	virtual void OnPinConnectionDoubleCicked(UEdGraphPin* PinA, UEdGraphPin* PinB, const FVector2f& GraphPosition) const
	{
	PRAGMA_DISABLE_DEPRECATION_WARNINGS
	 ...
	 */
	UE_DEPRECATED(5.6, "Use the version of the function accepting FVector2f; this API no longer interfaces directly with double-precision scalars and vectors.")
	ENGINE_API virtual void SetNodePosition(UEdGraphNode* Node, const FVector2D& Position) const UE_SLATE_DEPRECATED_VECTOR_VIRTUAL_FUNCTION;
	ENGINE_API virtual void SetNodePosition(UEdGraphNode* Node, const FVector2f& Position) const;
	...
	*/
	 ...
	 */
	virtual FReply TrySetGraphCategory(const UEdGraph* InGraph, const FText& InCategory) const { return FReply::Unhandled(); }

	/** Called when asset(s) are dropped onto a graph background. */
	UE_DEPRECATED(5.6, "Use the version of the function accepting FVector2f; this API no longer interfaces directly with double-precision scalars and vectors.")
	virtual void DroppedAssetsOnGraph(const TArray<struct FAssetData>& Assets, const FVector2D& GraphPosition, UEdGraph* Graph) const UE_SLATE_DEPRECATED_VECTOR_VIRTUAL_FUNCTION {}
	virtual void DroppedAssetsOnGraph(const TArray<struct FAssetData>& Assets, const FVector2f& GraphPosition, UEdGraph* Graph) const
	{
	PRAGMA_DISABLE_DEPRECATION_WARNINGS
		return DroppedAssetsOnGraph(Assets, FVector2D(GraphPosition), Graph);
	PRAGMA_ENABLE_DEPRECATION_WARNINGS
	}

	/** Called when asset(s) are dropped onto the specified node */
	UE_DEPRECATED(5.6, "Use the version of the function accepting FVector2f; this API no longer interfaces directly with double-precision scalars and vectors.")
	virtual void DroppedAssetsOnNode(const TArray<struct FAssetData>& Assets, const FVector2D& GraphPosition, UEdGraphNode* Node) const UE_SLATE_DEPRECATED_VECTOR_VIRTUAL_FUNCTION {}
	virtual void DroppedAssetsOnNode(const TArray<struct FAssetData>& Assets, const FVector2f& GraphPosition, UEdGraphNode* Node) const
	{
	PRAGMA_DISABLE_DEPRECATION_WARNINGS
		return DroppedAssetsOnNode(Assets, FVector2D(GraphPosition), Node);
	PRAGMA_ENABLE_DEPRECATION_WARNINGS
	}

	/** Called when asset(s) are dropped onto the specified pin */
	UE_DEPRECATED(5.6, "Use the version of the function accepting FVector2f; this API no longer interfaces directly with double-precision scalars and vectors.")
	virtual void DroppedAssetsOnPin(const TArray<struct FAssetData>& Assets, const FVector2D& GraphPosition, UEdGraphPin* Pin) const UE_SLATE_DEPRECATED_VECTOR_VIRTUAL_FUNCTION {}
	virtual void DroppedAssetsOnPin(const TArray<struct FAssetData>& Assets, const FVector2f& GraphPosition, UEdGraphPin* Pin) const
	{
	PRAGMA_DISABLE_DEPRECATION_WARNINGS
		return DroppedAssetsOnPin(Assets, FVector2D(GraphPosition), Pin);
	PRAGMA_ENABLE_DEPRECATION_WARNINGS
	}
    ...
    */
	 ...
	 */
	UE_DEPRECATED(5.6, "Use the version of the function accepting FVector2f; this API no longer interfaces directly with double-precision scalars and vectors.")
	virtual bool RequestVariableDropOnPanel(UEdGraph* InGraph, FProperty* InVariableToDrop, const FVector2D& InDropPosition, const FVector2D& InScreenPosition) const UE_SLATE_DEPRECATED_VECTOR_VIRTUAL_FUNCTION
	{
		return false;
	}
	virtual bool RequestVariableDropOnPanel(UEdGraph* InGraph, FProperty* InVariableToDrop, const FVector2f& InDropPosition, const FVector2f& InScreenPosition) const
	{
	PRAGMA_DISABLE_DEPRECATION_WARNINGS
		return RequestVariableDropOnPanel(InGraph, InVariableToDrop, FVector2D(InDropPosition), FVector2D(InScreenPosition));
	PRAGMA_ENABLE_DEPRECATION_WARNINGS
	}
	 ...
	 */
	UE_DEPRECATED(5.6, "Use the version of the function accepting FVector2f; this API no longer interfaces directly with double-precision scalars and vectors.")
	virtual bool RequestVariableDropOnNode(UEdGraph* InGraph, FProperty* InVariableToDrop, UEdGraphNode* InNode, const FVector2D& InDropPosition, const FVector2D& InScreenPosition) const UE_SLATE_DEPRECATED_VECTOR_VIRTUAL_FUNCTION
	{
		return false;
	}
	virtual bool RequestVariableDropOnNode(UEdGraph* InGraph, FProperty* InVariableToDrop, UEdGraphNode* InNode, const FVector2f& InDropPosition, const FVector2f& InScreenPosition) const
	{
	PRAGMA_DISABLE_DEPRECATION_WARNINGS
		return RequestVariableDropOnNode(InGraph, InVariableToDrop, InNode, FVector2D(InDropPosition), FVector2D(InScreenPosition));
	PRAGMA_ENABLE_DEPRECATION_WARNINGS
	}
	 ...
	 */
	UE_DEPRECATED(5.6, "Use the version of the function accepting FVector2f; this API no longer interfaces directly with double-precision scalars and vectors.")
	virtual bool RequestVariableDropOnPin(UEdGraph* InGraph, FProperty* InVariableToDrop, UEdGraphPin* InPin, const FVector2D& InDropPosition, const FVector2D& InScreenPosition) const UE_SLATE_DEPRECATED_VECTOR_VIRTUAL_FUNCTION
	{
		return false;
	}
	virtual bool RequestVariableDropOnPin(UEdGraph* InGraph, FProperty* InVariableToDrop, UEdGraphPin* InPin, const FVector2f& InDropPosition, const FVector2f& InScreenPosition) const
	{
	PRAGMA_DISABLE_DEPRECATION_WARNINGS
		return RequestVariableDropOnPin(InGraph, InVariableToDrop, InPin, FVector2D(InDropPosition), FVector2D(InScreenPosition));
	PRAGMA_ENABLE_DEPRECATION_WARNINGS
	}
...
#if WITH_EDITORONLY_DATA
private:
	virtual bool ShouldShowPanelContextMenuForIncompatibleConnectionsImpl() const { return false; }
protected:
