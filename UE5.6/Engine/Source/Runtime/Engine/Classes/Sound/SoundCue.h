...
{
	using Super = Audio::FParameterTransmitterBase;
public:
	ENGINE_API FSoundCueParameterTransmitter(Audio::FParameterTransmitterInitParams&& InParams);

	ENGINE_API virtual ~FSoundCueParameterTransmitter();
	...
	ENGINE_API virtual void AddReferencedObjects(FReferenceCollector& InCollector) override;
	...
	/** The SoundCueParameter keeps a running track of parameter changes from SetParameters(...). 
	 * When this method is called, the accumulated parameters are returned and the internal
	 * tracking of accumulated parameters is reset. */
	ENGINE_API TArray<FAudioParameter> ReleaseAccumulatedParameterUpdates();

	UE_DEPRECATED(5.6, "Do not access ParamsToSet. This will be moved to be a private member.")
	TArray<FAudioParameter> ParamsToSet;

	UE_DEPRECATED(5.6, "Do not access Transmitters. This will be moved to be a private member.")
