...
class UAudioSettings : public UDeveloperSettings
{
	...
	/**
	 * The default interval at which to re-test occlusion (in seconds).  Values < 0.f will only check occlusion once.
	 *   NOTE! Will be overridden by sounds coming from an audio component!
	 */
	UPROPERTY(config, EditAnywhere, Category = Occlusion)
	FPerPlatformFloat DefaultOcclusionCheckInterval = .1f;

	/** The default compression quality (e.g. for new SoundWaves) */
	UPROPERTY(config)
	int32 DefaultCompressionQuality_DEPRECATED = 80;
...
public:
	...
	UE_DEPRECATED(5.5, "Default Compression Quality has moved to SoundWave class defaults")
