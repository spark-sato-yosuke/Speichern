...
enum class EVirtualizationMode : uint8
{
	...
	/** EXPERIMENTAL - Sound fully virtualizes, but attempts to seek to the proper part of its looping duration when realized.
	 * The playback behavior is similar to PlayWhenSilent, but does not take up a voice slot.
	 */
	SeekRestart UE_EXPERIMENTAL(5.6, "This virtualization mode is experimental and may not function as expected in all use cases.")
};
...
public:
	...
	// //~ Begin IAudioPropertiesSheetAssetUserInterface
    ENGINE_API virtual void AllowPropertyParsing(const FProperty& TargetProperty) override;
    ENGINE_API virtual void IgnorePropertyParsing(const FProperty& PropertyToIgnore) override;
    ENGINE_API virtual bool ShouldParseProperty(const FProperty& TargetProperty) const override;
	//~ End IAudioPropertiesSheetAssetUserInterface
#endif
...
private:
	...
	void BindToPropertySheetChanges();
	void UnbindFromPropertySheetChanges();

#if WITH_EDITORONLY_DATA
    UPROPERTY()
    TArray<FName> LocalAudioProperties;
#endif
...
public:
	...
	*/
	...
	virtual bool IsProcedurallyGenerated() const { return false; }
