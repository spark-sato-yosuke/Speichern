...
inline const TCHAR* LexToString(const EDecompressionType InType)
{
#define CASE_TO_STRING(X) case X: return TEXT(#X);
	switch(InType)
	{
		FOREACH_ENUM_EDECOMPRESSIONTYPE(CASE_TO_STRING);
		default: break;
	}
	return TEXT("Unknown");
...
namespace Audio
{
	inline FName ToName(ESoundAssetCompressionType InDecoderType)
	{
	...
	inline ESoundAssetCompressionType ToSoundAssetCompressionType(EDefaultAudioCompressionType InDefaultCompressionType)
	{
...
#if WITH_EDITOR
	ENGINE_API TArray<FSoundWaveCuePoint> GetSoundWaveCuePoints() const;
	ENGINE_API void SetSoundWaveCuePoints(const TArray<FSoundWaveCuePoint>& InCuePoints);
#endif //WITH_EDITOR
...
public:
	...
	*/
	...
	ENGINE_API TArray<uint8> CreateNewCueData(const char ChunkName[4]);
	ENGINE_API void OverwriteSoundWaveDataWithTransformationData();
	ENGINE_API FWaveformTransformationInfo GetWaveformTransformationInfo() const;
...
protected:
	...
	ESoundWaveCuePointOrigin CuePointOrigin;
#endif
...
public:
	...
	TMap<FGuid, FSoundWaveCloudStreamingPlatformSettings>& GetCloudStreamingPlatformSettings() { return PlatformSettings; }
	const TMap<FGuid, FSoundWaveCloudStreamingPlatformSettings>& GetCloudStreamingPlatformSettings() const { return PlatformSettings; }
#endif // WITH_EDITORONLY_DATA
...
public:
	...
	*/
	struct FEditorAudioBulkData
	{
		...
		enum class EPayloadFlags : uint8
		{
			None,
			ReturnCopyWithTransformationData = 0x01
		};
		...
		ENGINE_API TFuture<FSharedBuffer> GetPayload(const EPayloadFlags Flags = EPayloadFlags::None) const;
...
public:
	...
	virtual bool IsProcedurallyGenerated() const override { return bProcedural; }
...
public:
	...
	FPerPlatformFloat GetSizeOfFirstAudioChunkInSeconds() const { return SizeOfFirstAudioChunkInSeconds; }
#endif // #if WITH_EDITORONLY_DATA
	...
	void SetSampleRate(const int32 InSampleRate) { SampleRate = static_cast<float>(InSampleRate); }
	void SetNumFrames(const int32 InNumFrames) { NumFrames = InNumFrames; }
