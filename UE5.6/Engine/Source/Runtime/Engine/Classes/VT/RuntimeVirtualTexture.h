...
protected:
	...
	*/
	...
	UPROPERTY(EditAnywhere, AdvancedDisplay, Category = Performance, meta = (EditCondition = bUseCustomPriority))
	EVTProducerPriority CustomPriority;

	UPROPERTY(EditAnywhere, AdvancedDisplay, Category = Performance, meta = (InlineEditConditionToggle)) 
	bool bUseCustomPriority = false;
	...
	UPROPERTY(EditAnywhere, BlueprintReadWrite, Category = Settings, meta = (DisplayName = "Texture Group"), AssetRegistrySearchable)
...
public:
	...
	/** Public getter for the custom material data. */
	FVector4f GetCustomMaterialData() const { return CustomMaterialData; }
	...
	ENGINE_API EVTProducerPriority GetDefaultPriority() const;
	bool GetUseCustomPriority() const
	{
		return bUseCustomPriority;
	}
