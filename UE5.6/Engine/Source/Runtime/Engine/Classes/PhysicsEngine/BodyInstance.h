...
public:
	ENGINE_API TInitBodiesHelperBase(TArray<FBodyInstance*, InAllocatorType>& InBodies, TArray<FTransform, InAllocatorType>& InTransforms,
		...
		const FInitBodySpawnParams& InSpawnParams, FPhysicsAggregateHandle InAggregate, IPhysicsBodyInstanceOwner* InBodyInstanceOwner = nullptr);
...
struct FInitBodiesHelper : public TInitBodiesHelperBase<InAllocatorType>
{
	...
	FInitBodiesHelper(TArray<FBodyInstance*, InAllocatorType>& InBodies, TArray<FTransform, InAllocatorType>& InTransforms, class UBodySetup* InBodySetup,
		class UPrimitiveComponent* InComponent, class UObject* InSourceObject, FPhysScene* InRBScene,
		const FInitBodySpawnParams& InSpawnParams, FPhysicsAggregateHandle InAggregate, IPhysicsBodyInstanceOwner* InBodyInstanceOwner = nullptr)
		: TInitBodiesHelperBase<InAllocatorType>(InBodies, InTransforms, InBodySetup, InComponent, InSourceObject, InRBScene, InSpawnParams, InAggregate, InBodyInstanceOwner)
	{
...
struct FInitBodiesHelperWithData : public FInitBodiesHelperBase
{
	...
	: FInitBodiesHelperBase(OwnedBodies, OwnedTransforms, InBodySetup, InPrimitiveComp, InRBScene, InSpawnParams, InAggregate, InBodyInstanceOwner), OwnedBodies(MoveTemp(InBodies)), OwnedTransforms(MoveTemp(InTransforms)) //-V1050
	{
		//Compute all the needed constants
	...
	: FInitBodiesHelperBase(OwnedBodies, OwnedTransforms, InHelper.BodySetup, InHelper.PrimitiveComp, InHelper.PhysScene, InHelper.SpawnParams, InHelper.Aggregate), OwnedBodies(InHelper.OwnedBodies), OwnedTransforms(InHelper.OwnedTransforms, InHelper.BodyInstanceOwner) //-V1050
	{
		ensure(false);
	}
...
public:
	...
	// TODO_CHAOSAPI: Deprecate ActorHandle Ref API
	...
	inline void SetPhysicsActorHandle(FPhysicsActorHandle InHandle) { SetPhysicsActor(InHandle); }

	// TODO_CHAOSAPI: Deprecate public ActorHandle
...
public:
	...
	*	@param BodyInstanceOwner The owning object of the body instance (optional and only used if PrimComp is not set)
	*/
	 ...
	 *	@param BodyInstanceOwner The associated owning object of the body instance (optional and only used if PrimitiveComp is not set)
	 */
	...
	// Contains the payload necessary for FBodyInstance::AsyncTermBody to release the body's resources asynchronously (outside of game thread).
	struct FAsyncTermBodyPayload
	{
		UE_API FAsyncTermBodyPayload();
		FPhysicsActorHandle GetPhysicsActor() const { return ActorHandle; }
	 ...
	 * @return the payload used by AsyncTermBody to release the resources.
	 */
	 ...
	 * @param Payload body resources to release.
	 */
	...
	static ENGINE_API UPhysicalMaterial* GetSimplePhysicalMaterial(const FBodyInstance* BodyInstance, TWeakObjectPtr<UPrimitiveComponent> Owner, TWeakObjectPtr<UBodySetup> BodySetupPtr, IPhysicsBodyInstanceOwner* BodyInstanceOwner = nullptr);
	...
	bool IsInertiaConditioningEnabled() const { return bInertiaConditioning; }
	...
	bool GetOverrideMaxDepenetrationVelocity() const { return bOverrideMaxDepenetrationVelocity; }
	...
	*/
	...
	float GetMaxDepenetrationVelocity() const { return MaxDepenetrationVelocity; }
...
public:
	...
	UPhysicalMaterial* GetPhysMaterialOverride() const { return PhysMaterialOverride; }
...
public:
	...
	friend ENGINE_API FArchive& operator<<(FArchive& Ar,FBodyInstance& BodyInst);
...
private:
	...
	ENGINE_API bool OverlapTestForBodiesImpl(const FVector& Position, const FQuat& Rotation, const TArray<FBodyInstance*, AllocatorType>& Bodies, bool bTraceComplex = false) const;
