...
enum TextureCompressionSettings : int
{
	...
	TC_Grayscale				UMETA(DisplayName = "Grayscale (G8/16, RGB8 sRGB) from source R"),
	TC_Displacementmap			UMETA(DisplayName = "Displacementmap (G8/16) from source A", ToolTip = "For Nanite displacement use TC_Alpha"),
	...
	TC_Alpha					UMETA(DisplayName = "Alpha (no sRGB, BC4) from source R", ToolTip = "Single channel compressed; can also use for displacement and gray"),
	TC_DistanceFieldFont		UMETA(DisplayName = "DistanceFieldFont (G8) from source A"),
...
{
	TSE_None		= 0 UMETA(DisplayName = "None", ToolTip = "The source encoding is not overridden."),
...
FORCEINLINE bool IsHDR(ETextureSourceFormat Format)
{
	return (Format == TSF_BGRE8 || Format == TSF_RGBA16F || Format == TSF_RGBA32F || Format == TSF_R16F || Format == TSF_R32F);
}

FORCEINLINE bool IsHDR(TextureCompressionSettings CompressionSettings)
{
...
FORCEINLINE bool IsUncompressed(TextureCompressionSettings CompressionSettings)
{
...
FORCEINLINE bool ShouldUseGreyScaleEditorVisualization(TextureCompressionSettings CompressionSettings)
{
