...
protected:
	...
	virtual ENGINE_API void InternalProcessRemoteFunction(
		class AActor* Actor,
...
public:
	...
	*/
	...
	/** 
	 * Amount of time (in seconds) before we close the channel of a replicated actor when it goes out of relevancy. Ex: If the client moves farther then the actor's netculldistance, the actor will still be replicated to that client for the duration of the RelevantTimeout.
	 * Default value is 5 seconds
	 * Note: ReplicationGraph and Iris do not use this value directly and instead have their unique config.
	 */
	UPROPERTY(Config)
	...
	*/
	...
	/** @todo document */
	//UE_DEPRECATED(5.6, "Variable will be made private. Use GetNetGuidCache() methods instead.")
...
public:
	...
	UE_DEPRECATED(5.6, "SetPendingDestruction will be made private. Use RequestNetDriverDestruction instead")
	ENGINE_API void SetPendingDestruction(bool bDestroy);

	/**
	 * Get const GuidCache
	 * Note: GuidCache/FNetworkGUIDs are not used when using Iris replication.
	*/
	const TSharedPtr<class FNetGUIDCache>& GetNetGuidCache() const
	{
		// Disabled until we have addressed the issues found.
		//ensureMsgf(!IsUsingIrisReplication(), TEXT("Using NetGuidCache/FNetworkGUIDs is not supported when using Iris replication."));
	...
	*/
	...
	{
		// Disabled until we have addressed the issues found.
		//ensureMsgf(!IsUsingIrisReplication(), TEXT("Using NetGuidCache/FNetworkGUIDs is not supported using Iris replication."));
...
private:
	...
	/** Accumulated delta time since the creation of the NetDriver */
	double ElapsedTime;
...
private:
	...
	TMap<FObjectKey, TSharedPtr<FReplicationChangelistMgr>>	ReplicationChangeListMap;
...
public:
	...
	virtual bool CanDowngradeActorRole(UNetConnection* Connection, AActor* Actor) const { return true; };

private:

	bool bExecuteRPCFunctions = true;
	bool bCallPreReplication = true;
	bool bReplicatingAutonomousAsSimulated = false;
...
public:
	 ...
	 * @param Parms parameters in a UObject memory layout
	 * @param OutParms out parameter information (irrelevant for RPC's)
	 * @param Stack stack frame the UFunction is called in
	 * @param SubObject optional: sub object to actually call function on
	 */
	ENGINE_API virtual void ProcessRemoteFunction(class AActor* Actor, class UFunction* Function, void* Parms, struct FOutParmRec* OutParms, struct FFrame* Stack, class UObject* SubObject = nullptr );

	/** Return a reference to the database that holds metrics calcluated by the networking system. */
	TObjectPtr<UNetworkMetricsDatabase> GetMetrics() { return NetworkMetricsDatabase; };
...
public:
	...
	ENGINE_API virtual void AddNetworkActor(AActor* Actor);
...
private:
	FOnNetTokenStoreReady OnNetTokenStoreReadyDelegate;
public:
	...
	{
		return OnNetTokenStoreReadyDelegate;
	}
	 ...
	 * 
	 * Func is a function that is called with the current instance of UNetConnection in the enumeration, and
	 * should return true if the enumeration must stop (i.e. no more calls to Func) or false for the enumeration
	 * to continue.
	 */
	...
	/**
	 * Explicitly generates and assigns a new network ID (FNetRefHandle if using Iris, FNetworkGUID if using generic replication)
	 * to Object and returns it. This can be used in combination with AssignNetIDToObject to associate actor instances
	 * that are spawned manually for replication, as opposed to the default behavior of letting the engine spawn
	 * the client's instance of a replicated actor automatically.
	 * This function would typically be called on a server, and the caller is responsible for communicating
	 * the returned ID to the remote/client side and calling AssignNetIDToObject to associate the ID with
	 * the client instance of the object.
	 * Only actors or Iris root objects are supported.
	 * @see UNetDriver::AssignNetIDToObject
	 */
	ENGINE_API UE::Net::FNetIDVariant CreateNetIDForObject(UObject* Object);

	/**
	 * Explicitly assigns NetID to Object. Can be called on clients that have received a NetID generated on
	 * the server by a call to CreateNetIDForObject. These functions can be used to associate actor instances
	 * that are spawned manually for replication, as opposed to the default behavior of letting the engine spawn
	 * the client's instance of a replicated actor automatically.
	 * Only actors or Iris root objects are supported.
	 * @see UNetDriver::CreateNetIDForObject
	 */
...
public:
	...
	{
		bReplicateTransactionally = bInReplicateTransactionally;
	}

	/** Returns true if this driver should run certain operations via UE::RemoteExecutor::ExecuteTransactional */
	bool ShouldReplicateTransactionally() const
	{
		return bReplicateTransactionally;
	}

	void SetDeferTransactionalSend(bool bInDeferTransactionalSend)
	{
		bDeferTransactionalSend = bInDeferTransactionalSend;
	}

	bool ShouldDeferTransactionalSend() const
	{
		return bDeferTransactionalSend;
	}

	/** Returns true if references to UObjects should be serialized as FRemoteObjectReference */
	bool IsUsingRemoteObjectReferences() const { return bUseRemoteObjectRefs; }
	...
	{
		bSupportNoPawnConnection = bInSupportNoPawnConnection;
	}
	...
	TObjectPtr<UNetConnection> FindConnection(struct FUniqueNetIdRepl& PlayerId);
...
protected:
	...
	ENGINE_API virtual void ForwardRemoteFunction(UObject* RootObject, UObject* SubObject, UFunction* Function, void* Parms);
...
private:
	...
	/** When using remote objects with UE_WITH_REMOTE_OBJECT_HANDLE, determines whether or not property replication happens within a transaction */
	bool bReplicateTransactionally = true;

	/** When using remote objects with UE_WITH_REMOTE_OBJECT_HANDLE, determines whether or not references to UObjects (either from TObjectPtr or TWeakObjectPtr) are serialized as FRemoteObjectReference */
	bool bUseRemoteObjectRefs = false;
	
	/** When using remote objects with UE_WITH_REMOTE_OBJECT_HANDLE, determines whether or not we defer the actual network send until the transaction commits */
	bool bDeferTransactionalSend = true;

	/** Server support for NMT_JoinNoPawn and NMT_JoinNoPawnSplit. */
	bool bSupportNoPawnConnection = false;
};
