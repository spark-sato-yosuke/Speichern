...
{
public:
	FNetFieldExport() : Blob ( {} ), Handle( 0 ), CompatibleChecksum( 0 ), bExported( false ),  bDirtyForReplay( true ), bExportBlob( false ), bIncompatible( false )
	{
	}
...
public:
	...
	const void RemoveUnmappedStablyNamedGuidsWithOuter(FNetworkGUID OuterGUID) { UnmappedStablyNamedGuids_OuterToInner.Remove(OuterGUID); }
...
public:
	...
	ENGINE_API virtual bool IsUsingRemoteObjectReferences() const override;
