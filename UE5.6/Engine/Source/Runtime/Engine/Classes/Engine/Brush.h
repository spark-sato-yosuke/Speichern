...
public:
	...
	ENGINE_API virtual bool IsComponentRelevantForNavigation(UActorComponent* Component) const override;
...
public:
	...
	/** @return true if its Brush Component should ever participate in BSP Rebuild */
	bool ENGINE_API CanEverAffectBSP() const;

	/** @return true if its Brush Component should be considered when building the level's static navigable geometry */
