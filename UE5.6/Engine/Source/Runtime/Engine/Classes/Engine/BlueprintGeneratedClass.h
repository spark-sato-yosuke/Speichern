...
public:
	...
	ENGINE_API UEdGraphNode* FindSourceNodeFromCodeLocation(const UFunction* Function, int32 CodeOffset, bool bAllowImpreciseHit) const;
...
public:
	...
	/** **** Experimental ****
	 * This will enable dynamic subobject instancing, which will use default data to determine which
	 * properties need instancing at construction/serialization time rather than reflected property
	 * 'Instanced' semantics. */
	UPROPERTY()
	uint8 bSupportsDynamicInstancedReference:1;
...
public:
...
#if WITH_EDITOR
	/** Customization point for fixing up backpointers to the class or CDO after compile for instances that can be reused (e.g. no layout change) */
	ENGINE_API void ReinitializeObjectAfterCompile(UObject* Object) const;
	virtual void HandleReinitializeObjectAfterCompile(UObject* Object) const {}
	...
	//~ UObject interface
	ENGINE_API virtual UE::Core::FVersePath GetVersePath() const override;
...
protected:
	...
	ENGINE_API virtual bool SupportsDynamicInstancedReference() const override;
