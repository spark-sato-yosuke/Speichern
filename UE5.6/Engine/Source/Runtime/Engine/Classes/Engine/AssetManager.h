...
class FAssetManagerEditorModule;
class FTimerManager;
class UAssetManagerSettings;
namespace UE::AssetRegistry { struct FShouldSetManagerContext; }
 ...
 * Asset Manager will automatically create a FStreamableDownloadCachePin for each primary asset requested with download params.
 * Cache pins can be retrieved with FStreamableHandle::GetDownloadCachePins()
 * Cache pins will be released along with the primary asset or when bundle state changes.
 */
...
enum class EUpdateManagementDatabaseFlags : uint32
{
	None = 0,							// No flags
	BuildChunkMap = (1 << 0),			// if the CachedChunkMap should be built or not.
	ForceRefresh = (1 << 1),			// When set the database will be forcibly refreshed.
};
...
public:
	 ...
	 * @param DelegateToCall	Delegate that will be called on completion
	 * @param Priority			Async loading priority for this request
	 * @param Location			[optional] Is not intended for direct use, the parameter catches call site source location and passes it down to the streaming manager. See FStreamingManager.
	 ...
	 * These assets will stay in memory until explicitly unloaded.
	 * You can wait on the returned streamable request or poll as needed.
	 * If there is no work to do, returned handle will be null and complete delegate will get called.
	 *
	 * @param AssetsToLoad		List of primary assets to load
	 * @param LoadBundles		List of bundles to load for those assets
	 * @param LoadParams		Contains callbacks and async load priority
	 * @param Location			[optional] Is not intended for direct use, the parameter catches call site source location and passes it down to the streaming manager. See FStreamingManager.
	 * @return					Streamable Handle that can be used to poll or wait. You do not need to keep this handle to stop the assets from being unloaded
	 */
	ENGINE_API virtual TSharedPtr<FStreamableHandle> LoadPrimaryAssets(
		const TArray<FPrimaryAssetId>& AssetsToLoad,
	...
	virtual TSharedPtr<FStreamableHandle> LoadPrimaryAsset(
		...
		UE::FSourceLocation Location = UE::FSourceLocation::Current());
	/** Single asset wrapper, see LoadPrimaryAssets */
	ENGINE_API virtual TSharedPtr<FStreamableHandle> LoadPrimaryAsset(
		const FPrimaryAssetId& AssetToLoad,
		const TArray<FName>& LoadBundles,
		FAssetManagerLoadParams&& LoadParams,
		UE::FSourceLocation Location = UE::FSourceLocation::Current());
	...
	virtual TSharedPtr<FStreamableHandle> LoadPrimaryAssetsWithType(
		...
		UE::FSourceLocation Location = UE::FSourceLocation::Current());
	/** Loads all assets of a given type, useful for cooking */
	ENGINE_API virtual TSharedPtr<FStreamableHandle> LoadPrimaryAssetsWithType(
		FPrimaryAssetType PrimaryAssetType,
		const TArray<FName>& LoadBundles,
		FAssetManagerLoadParams&& LoadParams,
		UE::FSourceLocation Location = UE::FSourceLocation::Current());
	 ...
	 * @param RemoveAllBundles	[optional] If true, remove all existing bundles even if not in remove list
	 * @param DelegateToCall	[optional] Delegate that will be called on completion
	 ...
	 * @return					Streamable Handle that can be used to poll or wait. You do not need to keep this handle to stop the assets from being unloaded
	 */
	ENGINE_API virtual TSharedPtr<FStreamableHandle> ChangeBundleStateForMatchingPrimaryAssets(
		const TArray<FName>& NewBundles,
		const TArray<FName>& OldBundles,
		FAssetManagerLoadParams&& LoadParams,
		UE::FSourceLocation Location = UE::FSourceLocation::Current());
	 ...
	 * @return					Streamable Handle that must be stored to keep the preloaded assets from being freed
	 */
	ENGINE_API virtual TSharedPtr<FStreamableHandle> PreloadPrimaryAssets(
		const TArray<FPrimaryAssetId>& AssetsToLoad,
		const TArray<FName>& LoadBundles,
		bool bLoadRecursive,
		FAssetManagerLoadParams&& LoadParams,
	...
	/**
	 * Downloads data for a set of assets in a specific bundle state, and returns a handle with a FStreamableDownloadCachePin.
	 * The FStreamableDownloadCachePin must be kept active to hold the data in the download cache.
	 * Does not load any assets.
	 *
	 * @param AssetsToLoad		List of primary assets to load
	 * @param LoadBundles		List of bundles to load for those assets
	 * @param bLoadRecursive	If true, this will call RecursivelyExpandBundleData and recurse into sub bundles of other primary assets loaded by a bundle reference
	 * @param LoadParams		Contains callbacks and download params. Download params are required.
	 * @param Location			[optional] Is not intended for direct use, the parameter catches call site source location and passes it down to the streaming manager. See FStreamingManager.
	 * @return					Streamable Handle
	 */
	ENGINE_API virtual TSharedPtr<FStreamableHandle> PredownloadPrimaryAssets(
		const TArray<FPrimaryAssetId>& AssetsToLoad,
		const TArray<FName>& LoadBundles,
		bool bLoadRecursive,
		FAssetManagerLoadParams&& LoadParams,
	...
	TSharedPtr<FStreamableHandle> LoadAssetList(
		...
		UE::FSourceLocation&& Location = UE::FSourceLocation::Current());
	/**
	 * lvalue reference overload to load non primary assets with the primary streamable manager.
	 * This will not auto release the handle, release it if needed.
	 * Prefer using rvalue reference version as it will not create an extra copy of your input array.
	 * 
	 * @param AssetList			List of non primary assets to load
	 * @param LoadParams		Contains callbacks and async load priority
	 * @param DebugName			[optional] Name of this handle, either FString or anything that can construct FString, will be reported in debug tools, will report Source Location if DebugName is not specified explicitly
	 ...
	 * Load non primary assets with the primary streamable manager.
	 * This will not auto release the handle, release it if needed.
	 * This is a entry point for the set of overloads. Overide it to have your custom behavior.
	 * 
	 * @param AssetList			List of non primary assets to load
	 * @param LoadParams		Contains callbacks and async load priority
	 * @param DebugName			[optional] Name of this handle, either FString or anything that can construct FString, will be reported in debug tools, will report Source Location if DebugName is not specified explicitly
	 ...
	 * @return					Streamable Handle that must be stored to keep the preloaded assets from being freed
	 */
	ENGINE_API virtual TSharedPtr<FStreamableHandle> LoadAssetList(
		TArray<FSoftObjectPath>&& AssetList,
		FAssetManagerLoadParams&& LoadParams,
		FString&& DebugName = FString(TEXT("LoadAssetList")),
		UE::FSourceLocation&& Location = UE::FSourceLocation::Current());
	...
	/**
	 * Download non primary assets with the primary streamable manager.
	 * Does not load assets after downloading.
	 * This will not auto release the handle, release it if needed.
	 * 
	 * @param AssetList			List of non primary assets to load
	 * @param LoadParams		Contains callbacks and async load priority
	...
	TSharedPtr<FStreamableHandle> DownloadAssetList(
		...
		UE::FSourceLocation&& Location = UE::FSourceLocation::Current());
	/** rvalue reference overload for Asset List */
	ENGINE_API TSharedPtr<FStreamableHandle> DownloadAssetList(
		TArray<FSoftObjectPath>&& AssetList,
		FAssetManagerLoadParams&& LoadParams,
		FString&& DebugName = FString(TEXT("DownloadAssetList")),
		UE::FSourceLocation&& Location = UE::FSourceLocation::Current());
	...
	ENGINE_API IAssetRegistry& GetAssetRegistry() const;

	/** Return settings object */
	ENGINE_API const UAssetManagerSettings& GetSettings() const;
	...
	ENGINE_API FTimerManager* GetTimerManager() const;
	 ...
	 */
	virtual void ModifyDLCBasePackages(const ITargetPlatform* TargetPlatform, TArray<FName>& PlatformBasedPackages, TSet<FName>& PackagesToClearResults) const {};
	 ...
	 */
	ENGINE_API virtual bool GetPackageChunkIds(FName PackageName, const ITargetPlatform* TargetPlatform, TArrayView<const int32> ExistingChunkList, TArray<int32>& OutChunkList, TArray<int32>* OutOverrideChunkList = nullptr) const;
	...
	virtual TSet<int32> GetEncryptedChunkIDsForPackage(FName InPackageName) const { return TSet<int32>(); }
	...
	ENGINE_API virtual bool GetPrimaryAssetSetChunkIds(const TSet<FPrimaryAssetId>& PrimaryAssetSet, const ITargetPlatform* TargetPlatform, TArrayView<const int32> ExistingChunkList, TArray<int32>& OutChunkList) const;
	...
	UE_DEPRECATED(5.6, "UpdateManagementDatabase as been updated to take InFlags instead.")
	ENGINE_API virtual void UpdateManagementDatabase(bool bForceRefresh);
	ENGINE_API virtual void UpdateManagementDatabase(EUpdateManagementDatabaseFlags InFlags = EUpdateManagementDatabaseFlags::BuildChunkMap);
	...
	/**
	 * Called post cook to build the chunk map only with the packages which have been cooked.
	 * Provides the StartupPackages which will go into chunk 0.
	 *
	 * It's expected this function will set the value of CachedChunkMap which can be used for package chunk assignment.
	 */
	UE_INTERNAL ENGINE_API void UpdateCachedChunkMapAfterCook(const TSet<FName>& AllCookedPackages, const TSet<FName>& StartupPackages);
	 ...
	 * TSoftObjectPtr<UCurveTable> CurveTableReference;
	 *
	...
	/** 
	  ...
	  */
	virtual void PreSaveAssetRegistry(const ITargetPlatform* TargetPlatform, const TSet<FName>& InCookedPackages) {}
	...
	/**
	 * Allows the asset manager to override the chunkID to be a string rather than integer value.
	 * Used at cook time and runtime to determine how to map chunkids to the potential string override.
	 * This does not update the internal asset registry number only the generated pak chunk file name.
	 * The AssetRegistryGenerator will request this mapping during Chunk Generation time.
	 * All strings must be be alpha numeric with no spaces.
	 *
	 * When overriding it's recommended to setup PlatformChunk mapping via the InstallBundle rules.
	 * When a pak chunk does not have a parseable number the default platform chunk will be 0 unless InstallBundle rules override that.
	 *
	 * 
	 * @param ChunkIdStringOverride - Out parameter for the override information of a chunkID to string.
	 */
	virtual void GetPakChunkIdToStringMapping(TMap<int32, FString>& ChunkIdStringOverride) {}
	/**
	 * Helper to make it easy to get the ChunkId based on the string override for a pakchunk.
	 * see "GetPakChunkIdToStringMapping" for more information
	 *
	 * @param ChunkIdStringOverride - Out parameter for getting the chunkID based on the string override.
	 */
...
protected:
	...
	ENGINE_API virtual void GetAssetDataForPathInternal(IAssetRegistry& AssetRegistry, const FString& AssetPath, OUT FAssetData& OutAssetData) const;
	...
	UE_DEPRECATED(5.6, "Use NotifyOnAssetStateChangeCompleted instead")
	virtual void OnAssetStateChangeCompleted(FPrimaryAssetId PrimaryAssetId, TSharedPtr<FStreamableHandle> BoundHandle, FStreamableDelegate WrappedDelegate) { checkf(false, TEXT("Deprecated")); }
	...
	UE_DEPRECATED(5.6, "Function was renamed to LoadAssetList that accepts FAssetManagerLoadParams struct.")
	virtual TSharedPtr<FStreamableHandle> LoadAssetListInternal(TArray<FSoftObjectPath>&& AssetList, FStreamableDelegateWithHandle&& DelegateToCall, TAsyncLoadPriority Priority, FString&& DebugName)
	{
	...
	ENGINE_API virtual EAssetSetManagerResult::Type ShouldSetManager(UE::AssetRegistry::FShouldSetManagerContext& Context) const;
	UE_DEPRECATED(5.6, "Override the version that takes an FShouldSetManagerContext instead.")
	...
	ENGINE_API virtual void GatherPublicAssetsForPackage(FName PackagePath, TSet<FSoftObjectPath>& AssetsToCook) const;
	 ...
	 */
	UE_DEPRECATED(5.6, "BuildChunkMap now takes 2 additional arguments")
	ENGINE_API virtual TMap<int32, FAssetManagerChunkInfo> BuildChunkMap(const TSet<FName>& PackagesToUpdateChunksFor) const;

	/**
	 * Used by UpdateManagementDatabase and at the end of the cook to build the CachedChunkMap.
	 * Should return a new cachedChunkMap.
	 *
	 * @param PackagesToUpdateChunksFor - the packages needing chunk assignment as part of the management database update.
	 * @param StartupPackages - All the known startup packages. When called from UpdateManagementDatabase, this will be empty.
	 * @param PackageThatWereCooked - All the known packages that were cooked. When called from UpdateManagementDatabase, this will be empty.
	 * @return a new CachedChunkMap to be used.
	 */
	ENGINE_API virtual TMap<int32, FAssetManagerChunkInfo> BuildChunkMap(const TSet<FName>& PackagesToUpdateChunksFor, const TSet<FName>& StartupPackages, const TSet<FName>& PackageThatWereCooked) const;
...
private:
	...
	mutable const UAssetManagerSettings* CachedSettings;

	friend FCompiledAssetManagerSearchRules;
};

inline TSharedPtr<FStreamableHandle> UAssetManager::LoadPrimaryAssets(const TArray<FPrimaryAssetId>& AssetsToLoad, const TArray<FName>& LoadBundles, FStreamableDelegate DelegateToCall, TAsyncLoadPriority Priority, UE::FSourceLocation Location)
{
	FAssetManagerLoadParams LoadParams;
	LoadParams.OnComplete = UE::StreamableManager::Private::WrapDelegate(DelegateToCall);
	LoadParams.Priority = Priority;

	return LoadPrimaryAssets(AssetsToLoad, LoadBundles, MoveTemp(LoadParams), MoveTemp(Location));
}

inline TSharedPtr<FStreamableHandle> UAssetManager::LoadPrimaryAsset(const FPrimaryAssetId& AssetToLoad, const TArray<FName>& LoadBundles, FStreamableDelegate DelegateToCall, TAsyncLoadPriority Priority, UE::FSourceLocation Location)
{
	FAssetManagerLoadParams LoadParams;
	LoadParams.OnComplete = UE::StreamableManager::Private::WrapDelegate(DelegateToCall);
	LoadParams.Priority = Priority;

	return LoadPrimaryAsset(AssetToLoad, LoadBundles, MoveTemp(LoadParams), MoveTemp(Location));
}

inline TSharedPtr<FStreamableHandle> UAssetManager::LoadPrimaryAssetsWithType(FPrimaryAssetType PrimaryAssetType, const TArray<FName>& LoadBundles, FStreamableDelegate DelegateToCall, TAsyncLoadPriority Priority, UE::FSourceLocation Location)
{
	FAssetManagerLoadParams LoadParams;
	LoadParams.OnComplete = UE::StreamableManager::Private::WrapDelegate(DelegateToCall);
	LoadParams.Priority = Priority;

	return LoadPrimaryAssetsWithType(PrimaryAssetType, LoadBundles, MoveTemp(LoadParams), MoveTemp(Location));
}

inline TSharedPtr<FStreamableHandle> UAssetManager::ChangeBundleStateForPrimaryAssets(const TArray<FPrimaryAssetId>& AssetsToChange, const TArray<FName>& AddBundles, const TArray<FName>& RemoveBundles, bool bRemoveAllBundles, FStreamableDelegate DelegateToCall, TAsyncLoadPriority Priority, UE::FSourceLocation Location)
{
	FAssetManagerLoadParams LoadParams;
	LoadParams.OnComplete = UE::StreamableManager::Private::WrapDelegate(MoveTemp(DelegateToCall));
	LoadParams.Priority = Priority;

	return ChangeBundleStateForPrimaryAssets(AssetsToChange, AddBundles, RemoveBundles, bRemoveAllBundles, MoveTemp(LoadParams), MoveTemp(Location));
}

inline TSharedPtr<FStreamableHandle> UAssetManager::ChangeBundleStateForMatchingPrimaryAssets(const TArray<FName>& NewBundles, const TArray<FName>& OldBundles, FStreamableDelegate DelegateToCall, TAsyncLoadPriority Priority, UE::FSourceLocation Location)
{
	FAssetManagerLoadParams LoadParams;
	LoadParams.OnComplete = UE::StreamableManager::Private::WrapDelegate(DelegateToCall);
	LoadParams.Priority = Priority;

	return ChangeBundleStateForMatchingPrimaryAssets(NewBundles, OldBundles, MoveTemp(LoadParams), MoveTemp(Location));
}

inline TSharedPtr<FStreamableHandle> UAssetManager::PreloadPrimaryAssets(const TArray<FPrimaryAssetId>& AssetsToLoad, const TArray<FName>& LoadBundles, bool bLoadRecursive, FStreamableDelegate DelegateToCall, TAsyncLoadPriority Priority, UE::FSourceLocation Location)
{
	FAssetManagerLoadParams LoadParams;
	LoadParams.OnComplete = UE::StreamableManager::Private::WrapDelegate(MoveTemp(DelegateToCall));
	LoadParams.Priority = Priority;

	return PreloadPrimaryAssets(AssetsToLoad, LoadBundles, bLoadRecursive, MoveTemp(LoadParams), MoveTemp(Location));
}

inline TSharedPtr<FStreamableHandle> UAssetManager::LoadAssetList(
	const TArray<FSoftObjectPath>& AssetList,
	...
	UE::FSourceLocation&& Location)
{
	...
	return LoadAssetList(CopyTemp(AssetList), MoveTemp(LoadParams), MoveTemp(DebugName), MoveTemp(Location));
}
	...
	UE::FSourceLocation&& Location)
{
	...
	return LoadAssetList(MoveTemp(AssetList), MoveTemp(LoadParams), MoveTemp(DebugName), MoveTemp(Location));
}
...
inline TSharedPtr<FStreamableHandle> UAssetManager::DownloadAssetList(
	const TArray<FSoftObjectPath>& AssetList,
	FAssetManagerLoadParams&& LoadParams,
	FString&& DebugName,
	UE::FSourceLocation&& Location)
{
	return DownloadAssetList(CopyTemp(AssetList), MoveTemp(LoadParams), MoveTemp(DebugName), MoveTemp(Location));
}
