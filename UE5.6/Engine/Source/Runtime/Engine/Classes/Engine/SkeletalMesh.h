...
{
	inline FText GeoAndSkinningText()
	{
...
public:
	...
	UE_API ~FScopedSkeletalMeshRenderData();
...
public:
	...
	struct FCommitMeshDescriptionParams
	{
		...
		/** Update the vertex color enabled state/GUID if the mesh description contains non-default vertex colors */
		bool bUpdateVertexColors = true;
...
public:
	...
	/* Build a LOD model for the given render data. USkinnedAsset Interface. */
	ENGINE_API virtual void BuildLODModel(FSkeletalMeshRenderData& RenderData, const ITargetPlatform* TargetPlatform, int32 LODIndex) override;
#endif
...
private:
	/*
	 * Max LOD level that post-process anim graphs are evaluated.
	 ...
	 */
	UPROPERTY(EditAnywhere, Category = Animation, meta = (DisplayName = "Post-Process LOD Threshold"))
	int32 PostProcessAnimBPLODThreshold = INDEX_NONE;

public:
	UE_DEPRECATED(5.6, "Use GetPostProcessAnimGraphLODThreshold() instead.")
	int32 GetPostProcessAnimBPLODThreshold() const { return GetPostProcessAnimGraphLODThreshold(); }
	UE_DEPRECATED(5.6, "Use SetPostProcessAnimGraphLODThreshold() instead.")
	void SetPostProcessAnimBPLODThreshold(int32 InPostProcessAnimBPLODThreshold) { SetPostProcessAnimGraphLODThreshold(InPostProcessAnimBPLODThreshold); }
	UE_DEPRECATED(5.6, "Use ShouldEvaluatePostProcessAnimGraph() instead.")
	bool ShouldEvaluatePostProcessAnimBP(int32 LODLevel) const { return ShouldEvaluatePostProcessAnimGraph(LODLevel); }
...
protected:
	...
	UPROPERTY(EditAnywhere, BlueprintReadOnly, Category = "Deformer", BlueprintGetter=GetTargetMeshDeformers)
	TObjectPtr<UMeshDeformerCollection> TargetMeshDeformers;
	 ...
	 */
	UPROPERTY(VisibleAnywhere, Category = Mesh, BlueprintGetter = GetForwardAxis)
	TEnumAsByte<EAxis::Type> ForwardAxis = EAxis::Y;
...
public:
	...
	{
		WaitUntilAsyncPropertyReleased(ESkeletalMeshAsyncProperties::TargetMeshDeformers, ESkinnedAssetAsyncPropertyLockType::WriteOnly);
		TargetMeshDeformers = InMeshDeformerCollection;
	}
	...
	{
		WaitUntilAsyncPropertyReleased(ESkeletalMeshAsyncProperties::TargetMeshDeformers, ESkinnedAssetAsyncPropertyLockType::ReadOnly);
		return TargetMeshDeformers;
	}

	/** True if this mesh LOD builds the half edge buffer. USkinnedAsset interface.	*/
	ENGINE_API virtual bool HasHalfEdgeBuffer(int32 LODIndex)const override;
