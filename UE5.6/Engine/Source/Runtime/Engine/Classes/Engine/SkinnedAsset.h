...
public:
	...
	ENGINE_API virtual UMeshDeformerCollection* GetTargetMeshDeformers() const
	PURE_VIRTUAL(USkinnedAsset::GetTargetMeshDeformers, return nullptr;);
	
	ENGINE_API virtual bool HasHalfEdgeBuffer(int32 LODIndex) const
	PURE_VIRTUAL(USkinnedAsset::HasHalfEdgeBuffer, return false;);
	...
	*/
	...
	UE_DEPRECATED(5.6, "Use or override the overload that takes FSkeletalMeshRenderData instead")
	virtual void BuildLODModel(const ITargetPlatform* TargetPlatform, int32 LODIndex) final {}
	
	/* Build a LOD model for the given render data. */
	virtual void BuildLODModel(FSkeletalMeshRenderData& RenderData, const ITargetPlatform* TargetPlatform, int32 LODIndex) {}
