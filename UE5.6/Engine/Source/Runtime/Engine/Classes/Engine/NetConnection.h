...
private:
	...
	/** An optional unique identifier set by the client when establishing a connection. */
...
public:
	...
	/**
	* Delegate that is called when a connection has been successfully completed on a client and
	* calls HandleClientPlayer(). 
	* 
	* In the case of UChildConnection the delegate on the parent connection will be called.
	*
	* @param ChildGameServerConnection 		A child UNetConnection (will be equal to ParentGameServerConnection on a root connection).
	* @param PlayerController				The controller replicated from the server.
	* @param ParentGameServerConnection		A parent UNetConnection.
	*/
	DECLARE_MULTICAST_DELEGATE_ThreeParams(FOnConnectionOpened, 
										   UNetConnection* 		/* ChildGameServerConnection */, 
										   APlayerController* 	/* PlayerController */, 
										   UNetConnection* 		/* ParentGameServerConnection */);
	FOnConnectionOpened& GetOnConnectionOpened() { return OnConnectionOpened; };

private:

	FOnConnectionOpened OnConnectionOpened;
	TFunction<class ULocalPlayer*(UNetConnection*, APlayerController*)> PlayerConnectionFunction;
...
public:
	...
	UE_DEPRECATED(5.6, "Replaced with a version that doesn't have a saturate option. Call Saturate() directly if passing true before." )
	ENGINE_API virtual int32 IsNetReady( bool bSaturate);

	/** Returns true if the connection is ready to send and has not exceeded the bandwidth limit yet. */
	ENGINE_API virtual bool IsNetReady() const;
