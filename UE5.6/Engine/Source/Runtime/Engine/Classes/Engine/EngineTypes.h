...
struct FViewLightingChannels
{
	...
	inline uint8 GetMaskForStruct() const
	{
		return (uint8)((bViewChannel0 << 0) | (bViewChannel1 << 1) | (bViewChannel2 << 2) | (bViewChannel3 << 3) | (bViewChannel4 << 4));
	}

	static inline constexpr uint8 GetMaskDefault() { return 0x1; }
};
...
UENUM()
enum class ENaniteGenerateFallback : uint8
{
...
struct FMeshNaniteSettings
{
	...
	UPROPERTY(BlueprintReadWrite, EditAnywhere, Category = NaniteSettings, meta = (DisplayName = "Generate Fallback Mesh"))
	...
	UPROPERTY(VisibleAnywhere, Category = NaniteSettings)
	...
	bool operator==(const FMeshNaniteSettings& Other) const
	{
		...
		return bEnabled == Other.bEnabled
			...
			&& BoneWeightPrecision == Other.BoneWeightPrecision
			...
			&& GenerateFallback == Other.GenerateFallback
			...
			&& VoxelLevel == Other.VoxelLevel
			&& RayBackUp == Other.RayBackUp
			&& bSeparable == Other.bSeparable
			&& bVoxelNDF == Other.bVoxelNDF
			&& bVoxelOpacity == Other.bVoxelOpacity
			&& NaniteAssemblyData == Other.NaniteAssemblyData;
	}
...
struct FMeshRayTracingProxySettings
{
	...
	UPROPERTY(BlueprintReadWrite, EditAnywhere, Category = RayTracing)
	uint8 bEnabled : 1;
	...
	UPROPERTY(EditAnywhere, BlueprintReadWrite, Category = RayTracing, meta = (NoSpinbox = true, ClampMin = "0.0", ClampMax = "1.0", UIMin = "0.0", UIMax = "1.0"))
	float LOD1PercentTriangles = 0.15f;
...
public:
	...
	{
		return bEnabled == Other.bEnabled &&
			Other.FallbackTarget == FallbackTarget &&
			Other.FallbackPercentTriangles == FallbackPercentTriangles &&
			Other.FallbackRelativeError == FallbackRelativeError &&
			Other.LOD1PercentTriangles == LOD1PercentTriangles &&
			Other.FoliageOverOcclusionBias == FoliageOverOcclusionBias;
	}
...
enum class EPackageAutoSaveType : uint8
{
...
ENGINE_API void SerializeRayTracingProxySettingsForDDC(FArchive& Ar, FMeshRayTracingProxySettings& Settings);
