...
class UKismetStringLibrary : public UBlueprintFunctionLibrary
{
	...
	UFUNCTION(BlueprintPure, meta=(DisplayName = "To String (IntVector2)", CompactNodeTitle = "->", BlueprintAutocast), Category="Utilities|String")
	static ENGINE_API FString Conv_IntVector2ToString(FIntVector2 InIntVec2);
	...
	static ENGINE_API FString BuildString_IntVector(const FString& AppendTo, const FString& Prefix, FIntVector InIntVector2, const FString& Suffix);

	/**
	 * Converts an IntVector2->string, creating a new string in the form AppendTo+Prefix+InIntVector+Suffix
	 * @param AppendTo - An existing string to use as the start of the conversion string
	 * @param Prefix - A string to use as a prefix, after the AppendTo string
	 * @param InIntVector - The intVector2 value to convert. Uses the standard FVector::ToString conversion
	 * @param Suffix - A suffix to append to the end of the conversion string
	 * @return A new string built from the passed parameters
	 */
	UFUNCTION(BlueprintPure, meta = (DisplayName = "Build String (IntVector2)"), Category = "Utilities|String")
	static ENGINE_API FString BuildString_IntVector2(const FString& AppendTo, const FString& Prefix, FIntVector2 InIntVector, const FString& Suffix);
	...
	*/
	 ...
	 * 
	 * @param First - The string to treat as basis
	 * @param Second - The string to treat as change
     * @return the LCS difference between the First and Second string
	 */
	UFUNCTION(BlueprintCallable, Category = "Utilities|String")
	static ENGINE_API FString DiffString(const FString& First, const FString& Second);
#endif	// WITH_EDITOR
