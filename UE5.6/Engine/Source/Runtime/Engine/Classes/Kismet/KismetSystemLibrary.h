...
UENUM()
enum class EEditorPropertyValueState : uint8
{
...
class UKismetSystemLibrary : public UBlueprintFunctionLibrary
{
	...
	/**
	 * Raises an error in the form of a Blueprint exception.
	 * Logs the error to the Output and Message Log with information pertaining to where the error was raised.
	 *
	 * @param ErrorMessage		The error message to display
	 */
	UFUNCTION(BlueprintCallable, Category = "Utilities|Debugging", meta=(Keywords = "raise error", DevelopmentOnly))
	static void RaiseScriptError(const FString& ErrorMessage = FString(TEXT("An error occurred")));
...
#if WITH_EDITOR
	/** Returns true if the object is cooked */
	UFUNCTION(BlueprintPure, Category = "Utilities")
	static ENGINE_API bool IsObjectCooked(const UObject* Object);

	/** Returns true if the object has its editor-only data */
	UFUNCTION(BlueprintPure, Category = "Utilities")
	static ENGINE_API bool ObjectHasEditorOnlyData(const UObject* Object);

	/**
	 * Duplicate an object to a new instance
	 *
	...
	static ENGINE_API UObject* DuplicateObject(const UObject* SourceObject, UObject* Outer, const FName Name = NAME_None);
#endif	// WITH_EDITOR
	...
	*/
	 ...
	 * @return				true if there was an overlap that passed the filters, false otherwise.
	 */
	UFUNCTION(BlueprintCallable, Category="Collision", meta=(WorldContext="WorldContextObject", AutoCreateRefTerm="ActorsToIgnore", DisplayName="Box Overlap Actors With Orientation"))
	...
	/**
	 * Returns an array of components that overlap the given box.
	 * @param WorldContext	World context
	 * @param BoxPos		Center of box.
	 ...
	 * @return				true if there was an overlap that passed the filters, false otherwise.
	 */
	UFUNCTION(BlueprintCallable, Category="Collision", meta=(WorldContext="WorldContextObject", AutoCreateRefTerm="ActorsToIgnore", DisplayName="Box Overlap Components With Orientation"))
	 ...
	 * @return				true if there was an overlap that passed the filters, false otherwise.
	 */
	UFUNCTION(BlueprintCallable, Category="Collision", meta=(WorldContext="WorldContextObject", AutoCreateRefTerm="ActorsToIgnore", DisplayName="Capsule Overlap Actors With Orientation"))
	 ...
	 * @return				true if there was an overlap that passed the filters, false otherwise.
	 */
	UFUNCTION(BlueprintCallable, Category="Collision", meta=(WorldContext="WorldContextObject", AutoCreateRefTerm="ActorsToIgnore", DisplayName="Capsule Overlap Components With Orientation") )
