...
static bool bUseAsyncOcclusionTracesCVar = false;
FAutoConsoleVariableRef CVarUseAsyncOcclusionTraces(
	TEXT("au.UseAsyncOcclusionTraces"),
...
static int32 FocusDependsOnSpatializationCVar = 0;
FAutoConsoleVariableRef CVarFocusDependsOnSpatialization(
	TEXT("au.FocusData.FocusDependsOnSpatialization"),
	FocusDependsOnSpatializationCVar,
	TEXT("When set to 1, focus factor on attenuation settings will not be enabled if spatialization is not enabled on the attenuation setting.\n")
	TEXT("0: Focus and Spatialization are orthogonal, 1: Focus will be disabled if spatialization is not also enabled"),
	ECVF_Default);

static int32 IncludeModulationInConcurrencyEvaluationCVar = 0;
FAutoConsoleVariableRef CVarIncludeModulationInConcurrencyEvaluation(
	TEXT("au.Concurrency.IncludeVolumeModulation"),
...
static int32 bAudioLinkSpatializeAllSoundsCVar = 0;
FAutoConsoleVariableRef CVarAudioLinkSpatializeAllSounds(
	TEXT("au.AudioLink.SpatializeAllSounds"),
	bAudioLinkSpatializeAllSoundsCVar,
	TEXT("0 (off), !=0 (on)"),
	ECVF_Default);

static int32 OverrideSeekStartTimeCVar = 0;
FAutoConsoleVariableRef CVarOverrideSeekStartTime(
	TEXT("au.Virtualization.OverrideSeekStartTimeMode"),
	OverrideSeekStartTimeCVar,
	TEXT("Optionally force all sounds with virtualization mode set to 'Seek Start Time' to another Virtualization Mode.\n")
...
FActiveSound::FActiveSound()
	...
	, VirtualizationModeOverride(EVirtualizationMode::Restart)
	, bEnableVirtualizationModeOverride(false)
...
#endif // ENABLE_AUDIO_DEBUG
{
	...
	check(AudioSettings);
	OcclusionCheckInterval = AudioSettings->DefaultOcclusionCheckInterval.GetValue();
}
...
FActiveSound* FActiveSound::CreateVirtualCopy(const FActiveSound& InActiveSoundToCopy, FAudioDevice& InAudioDevice)
{
	...
	check(Sound);
	if (InActiveSoundToCopy.GetVirtualizationMode() == EVirtualizationMode::Restart)
	{
...
void FActiveSound::AddReferencedObjects(FReferenceCollector& Collector)
{
	...
	{
		InstanceTransmitter->AddReferencedObjects(Collector);
	}
...
bool FActiveSound::IsPlayWhenSilent() const
{
	...
	return GetVirtualizationMode() == EVirtualizationMode::PlayWhenSilent;
}
...
{
	EVirtualizationMode Mode = EVirtualizationMode::Disabled;

	if (Sound)
	{
		Mode = Sound->VirtualizationMode;
	}
	
	if (bEnableVirtualizationModeOverride)
	{
		Mode = VirtualizationModeOverride;
	}

	if (Mode == EVirtualizationMode::SeekRestart)
	{
		switch (OverrideSeekStartTimeCVar)
		{
		case 1:
			Mode = EVirtualizationMode::Disabled;
			break;
		case 2:
			Mode = EVirtualizationMode::PlayWhenSilent;
			break;
		case 3:
			Mode = EVirtualizationMode::Restart;
			break;
		default:
			break;
		}
...
{
	VirtualizationModeOverride = Mode;
	bEnableVirtualizationModeOverride = true;
}
...
{
	TArray<TSet<TObjectPtr<USoundModulatorBase>>> ModulatorSets;
	for (const TPair<UPTRINT, FWaveInstance*>& Pair : GetWaveInstances())
	{
		if (const FWaveInstance* WaveInstance = Pair.Value)
		{
			TSet<TObjectPtr<USoundModulatorBase>> VolumeModulators = WaveInstance->GetEffectiveModulationSettings(EModulationDestination::Volume).Modulators;
			if (!VolumeModulators.IsEmpty())
			{
				ModulatorSets.Add(VolumeModulators);
			}
		}
	}

	return ModulatorSets;
}
...
void FActiveSound::UpdateWaveInstances(TArray<FWaveInstance*> &InWaveInstances, const float DeltaTime)
{
	...
	if (FadeOut == EFadeOut::None || ComponentVolumeFader.IsActive())
	{
		...
		if (bHasAttenuationSettings || bAudioLinkSpatializeAllSoundsCVar)
		{
	...
	else if (ThisSoundsWaveInstances.Num() > 0)
	{
		...
		if (VolumeConcurrency >= 0.0f)
		{
			...
			const bool bUseModulationInConcurrency = (IncludeModulationInConcurrencyEvaluationCVar == 2) || (IncludeModulationInConcurrencyEvaluationCVar == 1 && IsOneShot());
			if (bUseModulationInConcurrency)
			{
				// If there are no modulators set on any wave instance, then MaxSourceModulationValue may still be negative (see line below),
				// so we take the absolute value in that case (1.0).
				VolumeConcurrency *= FMath::Abs(MaxSourceModulationValue);

				// We set the value as negative to mark it as "read", so the next place to update this value will always update it.
				// By Multiplying by -1, we can reset the "read state" while preserving the previous value.
				if (MaxSourceModulationValue >= 0.0f)
				{
					MaxSourceModulationValue *= -1.0f;
				}
			}
...
void FActiveSound::CheckOcclusion(const FVector ListenerLocation, const FVector SoundLocation, const FSoundAttenuationSettings* AttenuationSettingsPtr)
{
	...
	{
		// If our occlusion check interval is less than 0, we only want to check occlusion once.  Otherwise, retest when the interval passes.
		const bool bOcclusionCheckDue = OcclusionCheckInterval >= 0.f && (PlaybackTime - LastOcclusionCheckTime) > OcclusionCheckInterval;
		const bool bNeedsInitialOcclusionCheck = LastOcclusionCheckTime < 0.f;
		...
		if (!bAsyncOcclusionPending && (bNeedsInitialOcclusionCheck || bOcclusionCheckDue))
		{
			...
			if (!bHasCheckedOcclusion || !bUseAsyncOcclusionTracesCVar)
			{
...
void FActiveSound::UpdateFocusData(float DeltaTime, const FAttenuationListenerData& ListenerData, FAttenuationFocusData* OutFocusData)
{
	...
	// This cvar is used to recover the incorrect state of spatialization and focus being dependent on each other
	// The original design of this system was each attenuation setting category was orthogonal (i.e. focus should be applicable independent of spatialization)
	// This cvar exists to allow a swap back to the old state if needed
	if (FocusDependsOnSpatializationCVar == 1 && !ListenerData.AttenuationSettings->bSpatialize)
	{
		return;
	}
...
void FActiveSound::UpdateAttenuation(float DeltaTime, FSoundParseParameters& ParseParams, int32 ListenerIndex, const FSoundAttenuationSettings* SettingsAttenuationNode)
{
	...
	ParseParams.bUseSpatialization |= Settings->bSpatialize || bAudioLinkSpatializeAllSoundsCVar;
