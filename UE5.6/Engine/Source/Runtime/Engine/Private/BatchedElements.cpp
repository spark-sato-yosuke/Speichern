...
void FBatchedElements::DrawPointElements(FRHICommandList& RHICmdList, const FMatrix& Transform, const uint32 ViewportSizeX, const uint32 ViewportSizeY, const FVector& CameraX, const FVector& CameraY) const
{
	...
	if( Points.Num() > 0 )
	{
		...
		const FRHIBufferCreateDesc Desc =
			FRHIBufferCreateDesc::CreateVertex<FSimpleElementVertex>(TEXT("FBatchedElements_Points"), NumVertices)
			.AddUsage(EBufferUsageFlags::Volatile)
			.SetInitialState(ERHIAccess::VertexOrIndexBuffer)
			.SetInitActionInitializer();

		TRHIBufferInitializer<FSimpleElementVertex> Initializer = RHICmdList.CreateBufferInitializer(Desc);
		...
		for(int32 PointIndex = 0; PointIndex < Points.Num(); PointIndex++)
		{
			...
			Initializer[VertIdx + 0] = FSimpleElementVertex(Point.Position + WorldPointX - WorldPointY,FVector2D(1,0),Point.Color,Point.HitProxyColor);
			Initializer[VertIdx + 1] = FSimpleElementVertex(Point.Position + WorldPointX + WorldPointY,FVector2D(1,1),Point.Color,Point.HitProxyColor);
			Initializer[VertIdx + 2] = FSimpleElementVertex(Point.Position - WorldPointX - WorldPointY,FVector2D(0,0),Point.Color,Point.HitProxyColor);
			Initializer[VertIdx + 3] = FSimpleElementVertex(Point.Position + WorldPointX + WorldPointY,FVector2D(1,1),Point.Color,Point.HitProxyColor);
			Initializer[VertIdx + 4] = FSimpleElementVertex(Point.Position - WorldPointX - WorldPointY,FVector2D(0,0),Point.Color,Point.HitProxyColor);
			Initializer[VertIdx + 5] = FSimpleElementVertex(Point.Position - WorldPointX + WorldPointY,FVector2D(0,1),Point.Color,Point.HitProxyColor);
	...
	if( HasPrimsToDraw() )
	{
			...
			&& (Filter & EBlendModeFilter::OpaqueAndMasked))
		{
			...
			if( LineVertices.Num() > 0 )
			{
				...
				FBufferRHIRef VertexBufferRHI = UE::RHIResourceUtils::CreateVertexBufferFromArray(RHICmdList, TEXT("Lines"), BUF_Volatile, MakeConstArrayView(LineVertices));
			...
			if ( ThickLines.Num() > 0 )
			{
				...
				*/
				...
				while (LineIndex < ThickLines.Num())
				{
					...
					const FRHIBufferCreateDesc Desc =
						FRHIBufferCreateDesc::CreateVertex<FSimpleElementVertex>(TEXT("ThickLines"), VertPerLine * NumLinesThisBatch)
						.AddUsage(EBufferUsageFlags::Volatile)
						.SetInitialState(ERHIAccess::VertexOrIndexBuffer)
						.SetInitActionInitializer();

					TRHIBufferInitializer<FSimpleElementVertex> InitialData = RHICmdList.CreateBufferInitializer(Desc);
					FSimpleElementVertex* ThickVertices = InitialData.GetWritableData();
					...
					FBufferRHIRef VertexBufferRHI = InitialData.Finalize();
			...
			if (WireTris.Num() > 0)
			{
				...
				const FRHIBufferCreateDesc CreateDesc =
					FRHIBufferCreateDesc::CreateVertex(TEXT("WireTris"), WireTriVerts.GetResourceDataSize())
					.AddUsage(BUF_Volatile)
					.SetInitActionResourceArray(&WireTriVerts)
					.SetInitialState(ERHIAccess::VertexOrIndexBuffer);
		...
		if( Sprites.Num() > 0 )
		{
			...
			if (ValidSpriteCount > 0)
			{
				...
				const FRHIBufferCreateDesc Desc =
					FRHIBufferCreateDesc::CreateVertex<FSimpleElementVertex>(TEXT("Sprites"), ValidSpriteCount * 6)
					.AddUsage(EBufferUsageFlags::Volatile)
					.SetInitialState(ERHIAccess::VertexOrIndexBuffer)
					.SetInitActionInitializer();

				TRHIBufferInitializer<FSimpleElementVertex> Initializer = RHICmdList.CreateBufferInitializer(Desc);
				FSimpleElementVertex* SpriteList = Initializer.GetWritableData();
		...
		if( MeshElements.Num() > 0)
		{
			...
			FBufferRHIRef VertexBufferRHI = UE::RHIResourceUtils::CreateVertexBufferFromArray(RHICmdList, TEXT("MeshElements"), BUF_Volatile, MakeConstArrayView(MeshVertices));
			...
			for(int32 MeshIndex = 0;MeshIndex < MeshElements.Num();MeshIndex++)
			{
				...
				if (Filter & MeshFilter)
				{
					...
					FBufferRHIRef IndexBufferRHI = UE::RHIResourceUtils::CreateIndexBufferFromArray(RHICmdList, TEXT("MeshElements"), EBufferUsageFlags::Volatile, MakeConstArrayView(MeshElement.Indices));
