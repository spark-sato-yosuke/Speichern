...
static float ConcurrencyMinDistanceDeltaCVar = 0.0f;
FAutoConsoleVariableRef CVarConcurrencyMinDistanceDelta(
	TEXT("au.Concurrency.MinDistanceDelta"),
...
FActiveSound* FSoundConcurrencyManager::GetEvictableSoundStopFarthest(const FActiveSound& NewActiveSound, const FConcurrencyGroup& ConcurrencyGroup, bool bIsRetriggering) const
{
	...
	// Get the mininum distance used to calculate distance of a sound
	float MinDistanceSquared = FMath::Max(ConcurrencyMinDistanceDeltaCVar, 0.0f);
	...
	for (FActiveSound* ActiveSound : ActiveSounds)
	{
		...
		// Stop Farthest
		if (DistanceToActiveSoundSq > (DistanceToStopSoundSq + MinDistanceSquared))
		{
		...
		// Stop Farthest then Oldest
		if (Rule == EMaxConcurrentResolutionRule::StopFarthestThenOldest)
		{
			const bool bIsSameDistance = FMath::Abs(DistanceToActiveSoundSq - DistanceToStopSoundSq) <= MinDistanceSquared;
			if (bIsSameDistance)
			{
