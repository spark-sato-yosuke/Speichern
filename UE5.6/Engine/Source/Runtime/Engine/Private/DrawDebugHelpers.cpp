...
{
	static void OnEnableDrawDebugHelpersChanged(IConsoleVariable* EnableDrawDebugHelpersVar)
	{
		// Clear out any persistently-drawn items if option is disabled
		if (!EnableDrawDebugHelpersVar->GetBool())
		{
			for (const FWorldContext& WorldContext : GEngine->GetWorldContexts())
			{
				FlushPersistentDebugLines(WorldContext.World());
				FlushDebugStrings(WorldContext.World());
			}
		}
	}

	static bool bEnableDrawDebugHelpers = true;
	static FAutoConsoleVariableRef CVarEnableDrawDebugHelpers(
		TEXT("r.EnableDrawDebugHelpers"),
		bEnableDrawDebugHelpers,
		TEXT(	"If true, then any 'DrawDebug' helper functions will be enabled.\nIf false, then the 'DrawDebug' helper functions will exit early and any persistent items will be removed.\nThis is only considered when ENABLE_DRAW_DEBUG is true"),
		FConsoleVariableDelegate::CreateStatic(&OnEnableDrawDebugHelpersChanged));
}

// Checks the value of the r.EnableDrawDebugHelpers cvar, and returns early if it is false.
#define RETURN_IF_DEBUG_DRAW_DISABLED		if (!UE::Private::bEnableDrawDebugHelpers) { return; }
...
void FlushPersistentDebugLines( const UWorld* InWorld )
{
	if (GEngine->GetNetMode(InWorld) != NM_DedicatedServer)
	{
		if (InWorld)
		{
			if (ULineBatchComponent* WorldPersistentLineBatcher = InWorld->GetLineBatcher(UWorld::ELineBatcherType::WorldPersistent))
			{
				WorldPersistentLineBatcher->Flush();
			}
			if (ULineBatchComponent* ForegroundPersistentLineBatcher = InWorld->GetLineBatcher(UWorld::ELineBatcherType::ForegroundPersistent))
			{
