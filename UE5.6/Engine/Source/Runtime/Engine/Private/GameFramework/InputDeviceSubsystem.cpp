...
FHardwareDeviceIdentifier UInputDeviceSubsystem::GetMostRecentlyUsedHardwareDevice(const FPlatformUserId InUserId) const
{
	...
	{
		if (const FHardwareDeviceIdentifier* ExistingDeviceId = LatestInputDeviceIdentifiers.Find(FoundDevice->LatestDeviceId))
		{
...
FInputDeviceId UInputDeviceSubsystem::GetMostRecentlyUsedInputDeviceId(const FPlatformUserId InUserId, const EHardwareDevicePrimaryType OfType /*= EHardwareDevicePrimaryType::Unspecified*/) const
{
	if (const FLatestInputDeviceData* FoundDevice = LatestUserDeviceIdentifiers.Find(InUserId))
	{
		// If we didn't specific a hardware device type, then just return the latest one that has been used
		if (OfType == EHardwareDevicePrimaryType::Unspecified)
		{
			return FoundDevice->LatestDeviceId;	
		}

		// Attempt to find a device of that specific type
		if (const FInputDeviceId* FoundDeviceOfType = FoundDevice->LatestDevices.Find(OfType))
		{
			return *FoundDeviceOfType;
		}
	}
...
FInputDeviceId UInputDeviceSubsystem::GetLatestDeviceOfType(const FPlatformUserId InUserId, const EHardwareDevicePrimaryType OfType) const
{
	// TODO: In the future, we may provide an interface to specify a specify a specific
	// input device to play force feedback on. We would implement that here. For example, it could
	// be useful to be able to play different force feedback events on multiple devices for a given user
	// or otherwise allow a game to specify how a force feedback effect behaves when played.
	// For example, you may want to play a force feedback event on multiple input devices if you have a racing sim
	// or HOTAS setup, with multiple HID's connected at once. 
	
	// Query what the most recently used devices are for the given user...
	if (const FLatestInputDeviceData* FoundDevice = LatestUserDeviceIdentifiers.Find(InUserId))
	{
		// And then return the most recently used device of this type for them.
		if (const FInputDeviceId* FoundDeviceOfType = FoundDevice->LatestDevices.Find(OfType))
		{
			return *FoundDeviceOfType;
		}
...
{	
	// Store or update the most recent hardware information about this FInputDeviceId.
	// We always want this to be up to date no matter what. This data doesn't care about the owning platform user,
	// it is only about the actual Input Device itself.
	LatestInputDeviceIdentifiers.Add(InDeviceId, InHardwareId);

	// Next, we will track information per platform user about which input devices that user has been
	// We want to broadcast the OnInputHardwareDeviceChanged delegate if the user's most recent input device
	...
	FLatestInputDeviceData& LatestUserData = LatestUserDeviceIdentifiers.FindOrAdd(OwningUserId);

	// Check the last known input device ID. If it is the same as the one we are currently
	// processing then there is no need to broadcast the "changed" delegate,  because it hasnt.
	bool bRequiresDelegateUpdate = true;
	if (LatestUserData.LatestDeviceId == InDeviceId)
	{
		bRequiresDelegateUpdate = false;
	}

	// Keep track of the latest hardware device and the latest of this type for the given user
	LatestUserData.LatestDevices.FindOrAdd(InHardwareId.PrimaryDeviceType, InDeviceId);
	LatestUserData.LatestDeviceId = InDeviceId;
	
	if (OnInputHardwareDeviceChanged.IsBound() && bRequiresDelegateUpdate)
	{
