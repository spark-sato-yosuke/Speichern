...
void StatelessConnectHandlerComponent::Outgoing(FBitWriter& Packet, FOutPacketTraits& Traits)
{
	...
	// Try to reuse memory from previous Outgoing call if there is enough space
	FBitWriter NewPacket = MoveTemp(OldPacket);
	if (NewPacket.GetMaxBits() < AdjustedSize + 1)
	{
		// Otherwise allocate at least MAX_PACKET_SIZE bytes, just like PacketHandler does, so packet memory can be reused later
		NewPacket = FBitWriter(FMath::Max(AdjustedSize + 1, MAX_PACKET_SIZE * 8), true);
	}
	else
	{
		NewPacket.Reset();
	...
	// Remember bit writer memory of old packet, so it can be re-used next time
	OldPacket = MoveTemp(Packet);
