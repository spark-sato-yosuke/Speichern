...
#else
	UE_LOG(LogLeakDetector, Log, TEXT("Started Tracking Allocations > %d KiB"), FilterSize / 1024);
...
static FAutoConsoleCommand LeakReporterStartCommand(
	...
	TEXT("Starts tracking allocations. Args -report=[secs] -size=[filter (KiB)]"),
	FConsoleCommandWithArgsDelegate::CreateLambda([](const TArray<FString>& Args) {
		FString ArgString = FString::Join(Args, TEXT(" "));
		int32 Size = 0;
		float ReportTime = 0;
		FParse::Value(*ArgString, TEXT("size="), Size); // KiB
		FParse::Value(*ArgString, TEXT("report="), ReportTime); // Seconds

		// FMallocLeakReporter expects FilterSize in bytes
		FMallocLeakReporter::Get().Start(Size * 1024, ReportTime);

		UE_LOG(LogConsoleResponse, Display, TEXT("Tracking allocations >= %d KiB and reporting every %f seconds"), Size, ReportTime);
	})
);
