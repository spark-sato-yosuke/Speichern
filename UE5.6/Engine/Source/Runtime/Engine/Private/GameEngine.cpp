...
void UGameEngine::CreateGameViewport( UGameViewportClient* GameViewportClient )
{
	...
	if (FParse::Value(FCommandLine::Get(), TEXT("SAVEWINPOS="), SaveWinPos) && SaveWinPos > 0 )
	{
		...
		const int32 SaveWinPosIndex = SaveWinPos - 1;
		const int32 MaxSlots = GetGameUserSettings()->GetMaxNumWindowPositionSaveSlots();

		if (SaveWinPosIndex < MaxSlots)
		{
			Window->SetOnWindowMoved(FOnWindowMoved::CreateWeakLambda(this, [SaveWinPosIndex, this](const TSharedRef<SWindow>& WindowBeingMoved)
				{
					this->OnGameWindowMoved_SavePosition(WindowBeingMoved, SaveWinPosIndex);
				}));
		}
		else
		{
			UE_LOG(LogEngine, Warning, TEXT("SaveWinPos index %d exceeds the maximum of %d. See cvar GameUserSettings.WindowPositionsMaxSize. Window position won't be saved."), SaveWinPosIndex, MaxSlots - 1);
		}
	}
...
TSharedRef<SWindow> UGameEngine::CreateGameWindow()
{
	...
	if (FParse::Value(FCommandLine::Get(), TEXT("SAVEWINPOS="), SaveWinPos) && SaveWinPos > 0)
	{
		...
		// Backwards compatibility for the move from a single saved window position to an array to handle multiple instances.
		// Old behavior: If SaveWinPos > 0 and WindowPosX and WindowPosY config values exist, use those values for the window position.
		// New behavior: If SaveWinPos > 0, use this value as a slot number to save multiple positions in the WindowPositions array.
		// For compatibility, if WindowPosX and WindowPosY exist in the config, use the old behavior.
		// Upon re-saving the config, those values will be moved into the new WindowPositions array.
		...
		{
			TArray<FString> PositionStrings;
			GConfig->GetArray(*GameUserSettingsCategory, TEXT("WindowPositions"), PositionStrings, GGameUserSettingsIni);

			// SaveWinPos is 1-based, the WindowPositions array is 0-based.
			int32 Index = SaveWinPos - 1;
			if (PositionStrings.IsValidIndex(Index))
			{
				FIntPoint Position;
				if (Position.InitFromString(PositionStrings[Index]))
				{
					WinX = Position.X;
					WinY = Position.Y;
					AutoCenterType = EAutoCenter::None;
				...
				{
					UE_LOG(LogEngine, Warning, TEXT("Saved position for SaveWinPos=%d invalid, ignoring."), SaveWinPos);
				}
			}
		}
	}
...
void UGameEngine::OnGameWindowMoved_SavePosition(const TSharedRef<SWindow>& WindowBeingMoved, int32 SaveWinPosIndex)
{
	const FSlateRect WindowRect = WindowBeingMoved->GetRectInScreen();

	// Reload config in case another running instance has modified it since the last load.
	// Prevents this instance from clobbering other SaveWinPos slots.
	GetGameUserSettings()->ReloadWindowPositions();

	// The -SaveWinPos=X command-line option is 1-based, but the GameUserSettings array is 0-based.
	GetGameUserSettings()->SetWindowPosition(WindowRect.Left, WindowRect.Top, SaveWinPosIndex);
	GetGameUserSettings()->SaveConfig();
}

void UGameEngine::OnGameWindowMoved(const TSharedRef<SWindow>& WindowBeingMoved)
{
	OnGameWindowMoved_SavePosition(WindowBeingMoved, 0);
}
...
{
#if PLATFORM_WINDOWS
	static IConsoleVariable* ScreenshotOnCrash = IConsoleManager::Get().FindConsoleVariable(TEXT("WindowsPlatformCrashContext.ScreenshotOnGPUCrash"));
	if (GIsEditor || !ScreenshotOnCrash || !ScreenshotOnCrash->GetBool())
	{
		return;
	}

	// Find the game window and attempt to retrieve its pixels.
	uint32 Width = 0, Height = 0;
	TArray<FColor> Pixels;

	TSharedPtr<SWindow> Window = GameViewportWindow.Pin();
	if (Window)
	{
		TSharedPtr<FGenericWindow> NativeWindow = Window->GetNativeWindow();
		if (NativeWindow && NativeWindow->IsDefinitionValid())
		{
			const TSharedPtr<FWindowsWindow> WindowsWindow = StaticCastSharedPtr<FWindowsWindow>(NativeWindow);
			Pixels = WindowsWindow->GetWindowPixels(Width, Height);
		}
	}
	...
	if (!Pixels.IsEmpty())
	{
		...
		TArray64<uint8> JPGData;
		FImageView Image((void*)Pixels.GetData(), Width, Height, ERawImageFormat::BGRA8);
		FImageUtils::CompressImage(JPGData, TEXT("jpg"), Image, 70);

		FString ScreenshotPath = FPaths::Combine(FPaths::ProjectLogDir(), TEXT("CrashCapture.jpg"));
		if (FFileHelper::SaveArrayToFile(JPGData, *ScreenshotPath))
		{
			FWindowsPlatformCrashContext::SetEngineData(TEXT("CrashScreenshotWidth"), FString::FromInt(Width));
			FWindowsPlatformCrashContext::SetEngineData(TEXT("CrashScreenshotHeight"), FString::FromInt(Height));
			FWindowsPlatformCrashContext::AddOptionalAttachment(ScreenshotPath);
		}
		else
		{
			UE_LOG(LogEngine, Warning, TEXT("Error saving crash screenshot to file: %s"), *ScreenshotPath);
		}
	}
#endif
}
...
void UGameEngine::Tick( float DeltaSeconds, bool bIdleMode )
{
	...
	for (int32 WorldIdx = 0; WorldIdx < WorldList.Num(); ++WorldIdx)
	{
		...
		// streamingServer
		if( GIsServer == true && !GUseUnifiedTimeBudgetForStreaming)
		{
	...
	const bool bIsRenderingScene = !bIdleMode && !IsRunningDedicatedServer() && !IsRunningCommandlet() && FEmbeddedCommunication::IsAwakeForRendering();

	if (bIsRenderingScene && !bRenderingSuspended)
	{
		RedrawViewports();
	}
	...
	if (bIsRenderingScene)
	{
		...
		else if (!GUseUnifiedTimeBudgetForStreaming)
		{
			// Still need to call UpdateLevelStreaming() even when not rendering
			// unless we are using the unified streaming budget
