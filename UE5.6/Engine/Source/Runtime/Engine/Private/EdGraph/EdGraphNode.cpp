...
class UEdGraph* UEdGraphNode::GetGraph() const
{
	...
	if (Graph == nullptr && IsValidChecked(this))
	{
...
FLinearColor UEdGraphNode::GetNodeTitleTextColor() const
{
	FLinearColor ReturnTextColor = FLinearColor::White;
	if (!IsNodeEnabled() || IsDisplayAsDisabledForced() || IsNodeUnrelated())
	{
...
void UEdGraphNode::ResizeNode(const FVector2f& NewSize)
{
PRAGMA_DISABLE_DEPRECATION_WARNINGS
	ResizeNode(FVector2D(NewSize));
...
#if WITH_EDITORONLY_DATA
void UEdGraphNode::CreateDeterministicGuid()
{
	NodeGuid = FGuid::NewDeterministicGuid(GetPathName());
}
#endif//WITH_EDITORONLY_DATA
