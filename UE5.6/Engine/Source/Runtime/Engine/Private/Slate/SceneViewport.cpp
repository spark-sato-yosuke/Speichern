...
void FSceneViewport::ApplyModifierKeys(const FModifierKeysState& InKeysState, const uint64 Timestamp)
{
	if (ViewportClient && GetSizeXY() != FIntPoint::ZeroValue)
	{
		...
		{
			ViewportClient->InputKey(FInputKeyEventArgs(this, DefaultInputDevice, EKeys::LeftAlt, IE_Pressed, Timestamp));
		}
		if (InKeysState.IsRightAltDown())
		{
			ViewportClient->InputKey(FInputKeyEventArgs(this, DefaultInputDevice, EKeys::RightAlt, IE_Pressed, Timestamp));
		}
		if (InKeysState.IsLeftControlDown())
		{
			ViewportClient->InputKey(FInputKeyEventArgs(this, DefaultInputDevice, EKeys::LeftControl, IE_Pressed, Timestamp));
		}
		if (InKeysState.IsRightControlDown())
		{
			ViewportClient->InputKey(FInputKeyEventArgs(this, DefaultInputDevice, EKeys::RightControl, IE_Pressed, Timestamp));
		}
		if (InKeysState.IsLeftShiftDown())
		{
			ViewportClient->InputKey(FInputKeyEventArgs(this, DefaultInputDevice, EKeys::LeftShift, IE_Pressed, Timestamp));
		}
		if (InKeysState.IsRightShiftDown())
		{
			ViewportClient->InputKey(FInputKeyEventArgs(this, DefaultInputDevice, EKeys::RightShift, IE_Pressed, Timestamp));
		}
	}
}
...
void FSceneViewport::ProcessAccumulatedPointerInput()
{
	...
	if (NumMouseSamplesX > 0 || NumMouseSamplesY > 0)
	{
		...
		ViewportClient->InputAxis(MouseXArgs);
		...
		ViewportClient->InputAxis(MouseYArgs);
	}
...
FReply FSceneViewport::OnMouseButtonDown( const FGeometry& InGeometry, const FPointerEvent& InMouseEvent )
{
	...
	if (ViewportClient && GetSizeXY() != FIntPoint::ZeroValue)
	{
		...
		{
			if (!ViewportClient->InputKey(FInputKeyEventArgs(this, InMouseEvent.GetInputDeviceId(), InMouseEvent.GetEffectingButton(), IE_Pressed, 1.0f, InMouseEvent.IsTouchEvent(), InMouseEvent.GetEventTimestamp())))
			{
				CurrentReplyState = FReply::Unhandled();
			}
		}
...
FReply FSceneViewport::OnMouseButtonUp( const FGeometry& InGeometry, const FPointerEvent& InMouseEvent )
{
	...
	if( ViewportClient && GetSizeXY() != FIntPoint::ZeroValue )
	{
		if (!ViewportClient->InputKey(FInputKeyEventArgs(this, InMouseEvent.GetInputDeviceId(), InMouseEvent.GetEffectingButton(), IE_Released, 1.0f, InMouseEvent.IsTouchEvent(), InMouseEvent.GetEventTimestamp())))
		{
			CurrentReplyState = FReply::Unhandled(); 
		}
...
FReply FSceneViewport::OnMouseWheel( const FGeometry& InGeometry, const FPointerEvent& InMouseEvent )
{
	...
	if( ViewportClient  && GetSizeXY() != FIntPoint::ZeroValue  )
	{
		...
		ViewportClient->InputKey(FInputKeyEventArgs(this, InMouseEvent.GetInputDeviceId(), ViewportClientKey, IE_Pressed, 1.0f, InMouseEvent.IsTouchEvent(), InMouseEvent.GetEventTimestamp()));
		ViewportClient->InputKey(FInputKeyEventArgs(this, InMouseEvent.GetInputDeviceId(), ViewportClientKey, IE_Released, 1.0f, InMouseEvent.IsTouchEvent(), InMouseEvent.GetEventTimestamp()));
		ViewportClient->InputAxis(FInputKeyEventArgs(this, InMouseEvent.GetInputDeviceId(), EKeys::MouseWheelAxis, InMouseEvent.GetWheelDelta(), FApp::GetDeltaTime(), 1, InMouseEvent.GetEventTimestamp()));
	}
	return CurrentReplyState;
}
...
FReply FSceneViewport::OnMouseButtonDoubleClick( const FGeometry& InGeometry, const FPointerEvent& InMouseEvent )
{
	...
	if( ViewportClient && GetSizeXY() != FIntPoint::ZeroValue  )
	{
		...
		if (!ViewportClient->InputKey(FInputKeyEventArgs(this, InMouseEvent.GetInputDeviceId(), InMouseEvent.GetEffectingButton(), IE_DoubleClick, 1.0f, InMouseEvent.IsTouchEvent(), InMouseEvent.GetEventTimestamp())))
		{
			CurrentReplyState = FReply::Unhandled(); 
		}
	}
...
FReply FSceneViewport::OnTouchStarted( const FGeometry& MyGeometry, const FPointerEvent& TouchEvent )
{
	...
	if( ViewportClient )
	{
		...
		if(ViewportClient->InputTouch( this, TouchEvent.GetInputDeviceId(), TouchEvent.GetPointerIndex(), ETouchType::Began, TouchPosition, TouchEvent.GetTouchForce(), TouchEvent.GetTouchpadIndex(), TouchEvent.GetEventTimestamp()) )
		{
...
FReply FSceneViewport::OnTouchMoved( const FGeometry& MyGeometry, const FPointerEvent& TouchEvent )
{
	...
	if( ViewportClient )
	{
		...
		if( !ViewportClient->InputTouch( this, TouchEvent.GetInputDeviceId(), TouchEvent.GetPointerIndex(), ETouchType::Moved, CachedCursorPos, TouchEvent.GetTouchForce(), TouchEvent.GetTouchpadIndex(), TouchEvent.GetEventTimestamp()) )
		{
			CurrentReplyState = FReply::Unhandled(); 
		}
	}
...
FReply FSceneViewport::OnTouchEnded( const FGeometry& MyGeometry, const FPointerEvent& TouchEvent )
{
	...
	if( ViewportClient )
	{
		...
		if( !ViewportClient->InputTouch( this, TouchEvent.GetInputDeviceId(), TouchEvent.GetPointerIndex(), ETouchType::Ended, CurCursorPos, 0.0f, TouchEvent.GetTouchpadIndex(), TouchEvent.GetEventTimestamp()) )
		{
			CurrentReplyState = FReply::Unhandled(); 
		}
...
FReply FSceneViewport::OnTouchForceChanged(const FGeometry& MyGeometry, const FPointerEvent& TouchEvent)
{
	...
	if (ViewportClient)
	{
		...
		if (!ViewportClient->InputTouch(this, TouchEvent.GetInputDeviceId(), TouchEvent.GetPointerIndex(), ETouchType::ForceChanged, TouchPosition, TouchEvent.GetTouchForce(), TouchEvent.GetTouchpadIndex(), TouchEvent.GetEventTimestamp()))
		{
			CurrentReplyState = FReply::Unhandled();
		}
	}
...
FReply FSceneViewport::OnTouchFirstMove(const FGeometry& MyGeometry, const FPointerEvent& TouchEvent)
{
	...
	if (ViewportClient)
	{
		...
		if (!ViewportClient->InputTouch(this, TouchEvent.GetInputDeviceId(), TouchEvent.GetPointerIndex(), ETouchType::FirstMove, TouchPosition, TouchEvent.GetTouchForce(), TouchEvent.GetTouchpadIndex(), TouchEvent.GetEventTimestamp()))
		{
			CurrentReplyState = FReply::Unhandled();
		}
	}
...
FReply FSceneViewport::OnTouchGesture( const FGeometry& MyGeometry, const FPointerEvent& GestureEvent )
{
	...
	if( ViewportClient )
	{
		...
		if(!ViewportClient->InputGesture(this, GestureEvent.GetInputDeviceId(), GestureEvent.GetGestureType(), GestureEvent.GetGestureDelta(), GestureEvent.IsDirectionInvertedFromDevice(), GestureEvent.GetEventTimestamp()))
		{
			CurrentReplyState = FReply::Unhandled();
		}
	}
...
FReply FSceneViewport::OnMotionDetected( const FGeometry& MyGeometry, const FMotionEvent& MotionEvent )
{
	...
	if(ViewportClient) 
	{
		...
		if(!ViewportClient->InputMotion( this, MotionEvent.GetInputDeviceId(), MotionEvent.GetTilt(), MotionEvent.GetRotationRate(), MotionEvent.GetGravity(), MotionEvent.GetAcceleration(), MotionEvent.GetEventTimestamp()))
		{
			CurrentReplyState = FReply::Unhandled(); 
		}
	}
...
FReply FSceneViewport::OnKeyUp( const FGeometry& InGeometry, const FKeyEvent& InKeyEvent )
{
	...
	if (Key.IsValid())
	{
		...
		if (ViewportClient && GetSizeXY() != FIntPoint::ZeroValue)
		{
			...
			if (!ViewportClient->InputKey(FInputKeyEventArgs(this, InKeyEvent.GetInputDeviceId(), Key, IE_Released, 1.0f, false, InKeyEvent.GetEventTimestamp())))
			{
				CurrentReplyState = FReply::Unhandled();
			}
		}
...
FReply FSceneViewport::OnAnalogValueChanged(const FGeometry& MyGeometry, const FAnalogInputEvent& InAnalogInputEvent)
{
	...
	if (Key.IsValid())
	{
		...
		if (ViewportClient)
		{
			...
			if (!ViewportClient->InputAxis(Args))
			{
				CurrentReplyState = FReply::Unhandled();
			}
		}
