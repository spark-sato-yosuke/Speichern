	...
	) const
{
	...
	if (LocalVertexFactory->SupportsManualVertexFetch(FeatureLevel) || IsStaticLightingAllowed())
	{
...
//     0: ignored only in editor (Default)
//     1: ignored in editor and on PC >= SM5
//     2: ignored always
...
{
	// Compile local vertex factory permutations while in the editor, always. Important for editor features like preview 
	// Additionally, allow ignoring via CVar to change what scenarios respect the bIsUsedWithStaticMesh flag
	if (EnumHasAnyFlags(Parameters.Flags, EShaderPermutationFlags::HasEditorOnlyData) || ShouldIgnore(Parameters.Platform))
	{
		return true;
	}

	// Otherwise compile when the Material has selected that it can be used with StaticMesh
...
void FLocalVertexFactory::InitRHI(FRHICommandListBase& RHICmdList)
{
	...
	if (RHISupportsManualVertexFetch(GMaxRHIShaderPlatform) || IsStaticLightingAllowed()) // for LODLightmapDataIndex
	{
