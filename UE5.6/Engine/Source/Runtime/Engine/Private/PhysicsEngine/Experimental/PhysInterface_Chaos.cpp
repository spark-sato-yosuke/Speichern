...
FPhysScene* FPhysInterface_Chaos::GetCurrentSceneForComponent(UPrimitiveComponent* InComponent)
{
	if (!InComponent)
	{
		return nullptr;
	...
	FBodyInstance* BodyInstance = InComponent->GetBodyInstance();
	if (BodyInstance && BodyInstance->GetPhysicsActorHandle())
	{
		return static_cast<FPhysScene*>(GetCurrentScene(BodyInstance->GetPhysicsActorHandle()));
	}

	if (UWorld* ComponentWorld = InComponent->GetWorld())
	{
		return ComponentWorld->GetPhysicsScene();
	}
...
void FPhysInterface_Chaos::UpdateLinearDrive_AssumesLocked(const FPhysicsConstraintHandle& InConstraintRef, const FLinearDriveConstraint& InDriveParams, bool InInitialize)
{
	if (InConstraintRef.IsValid() && InConstraintRef.Constraint->IsType(Chaos::EConstraintType::JointConstraintType))
	{
		if (Chaos::FJointConstraint* Constraint = static_cast<Chaos::FJointConstraint*>(InConstraintRef.Constraint))
		{
			...
			Constraint->SetLinearDriveForceMode(InDriveParams.bAccelerationMode ? Chaos::EJointForceMode::Acceleration : Chaos::EJointForceMode::Force);
...
void FPhysInterface_Chaos::UpdateAngularDrive_AssumesLocked(const FPhysicsConstraintHandle& InConstraintRef, const FAngularDriveConstraint& InDriveParams, bool InInitialize)
{
	if (InConstraintRef.IsValid() && InConstraintRef.Constraint->IsType(Chaos::EConstraintType::JointConstraintType))
	{
		if (Chaos::FJointConstraint* Constraint = static_cast<Chaos::FJointConstraint*>(InConstraintRef.Constraint))
		{
			...
			Constraint->SetAngularDriveForceMode(InDriveParams.bAccelerationMode ? Chaos::EJointForceMode::Acceleration : Chaos::EJointForceMode::Force);
			if (InDriveParams.AngularDriveMode == EAngularDriveMode::TwistAndSwing)
			{
			...
			else
			{
				...
				Constraint->SetAngularDriveMaxTorque(Chaos::FVec3(InDriveParams.SlerpDrive.MaxForce, InDriveParams.SlerpDrive.MaxForce, InDriveParams.SlerpDrive.MaxForce));
			}
		}
	}
}
...
void FPhysInterface_Chaos::ExecuteShapeWrite(FBodyInstance* InInstance, FPhysicsShapeHandle& InShape, TFunctionRef<void(FPhysicsShapeHandle& InShape)> InCallable)
{
	...
	{
		FScopedSceneLock_Chaos SceneLock(InInstance->GetPhysicsActor(), EPhysicsInterfaceScopedLockType::Write);
		InCallable(InShape);
	}
}


bool FPhysInterface_Chaos::ExecuteWrite(FPhysicsActorHandle& InActorReference, TFunctionRef<void(FPhysicsActorHandle& Actor)> InCallable, EPhysicsInterfaceScopedThreadContext ThreadContext, EPhysicsInterfaceScopedTransactionMode TransactionMode)
{
	if (InActorReference)
	{
		FScopedSceneLockWithContext_Chaos SceneLock(InActorReference, EPhysicsInterfaceScopedLockType::Write, ThreadContext, TransactionMode);
		InCallable(InActorReference);
		return true;
	}
...
bool FPhysInterface_Chaos::ExecuteRead(FPhysicsActorHandle& InActorReference, TFunctionRef<void(FPhysicsActorHandle& Actor)> InCallable, EPhysicsInterfaceScopedThreadContext ThreadContext)
{
	if (InActorReference)
	{
		FScopedSceneLockWithContext_Chaos SceneLock(InActorReference, EPhysicsInterfaceScopedLockType::Read, ThreadContext, EPhysicsInterfaceScopedTransactionMode::Normal);
		InCallable(InActorReference);
		return true;
	}
...
bool FPhysInterface_Chaos::ExecuteWrite(const FPhysicsConstraintHandle& InConstraintRef, TFunctionRef<void(const FPhysicsConstraintHandle& Constraint)> InCallable, EPhysicsInterfaceScopedThreadContext ThreadContext, EPhysicsInterfaceScopedTransactionMode TransactionMode)
{
	if (InConstraintRef.IsValid())
	{
		FScopedSceneLockWithContext_Chaos SceneLock(&InConstraintRef, EPhysicsInterfaceScopedLockType::Write, ThreadContext, TransactionMode);
		InCallable(InConstraintRef);
		return true;
	}
...
bool FPhysInterface_Chaos::ExecuteRead(const FPhysicsConstraintHandle& InConstraintRef, TFunctionRef<void(const FPhysicsConstraintHandle& Constraint)> InCallable, EPhysicsInterfaceScopedThreadContext ThreadContext)
{
	if (InConstraintRef.IsValid())
	{
		FScopedSceneLockWithContext_Chaos SceneLock(&InConstraintRef, EPhysicsInterfaceScopedLockType::Read, ThreadContext, EPhysicsInterfaceScopedTransactionMode::Normal);
		InCallable(InConstraintRef);
		return true;
	}
	...
	{
		{
			...
			FPhysicsCommand::ExecuteRead(InInstance->GetPhysicsActor(), [&](const FPhysicsActorHandle& Actor)
			{
				...
				if(FPhysicsActorHandle RigidBody = TargetInstance->GetPhysicsActor())
				{
					FRaycastHit BestHit;
	...
	else
	{
		...
		FPhysicsCommand::ExecuteRead(TargetInstance->GetPhysicsActor(), [&](const FPhysicsActorHandle& Actor)
		{
			if (Actor && InInstance->OwnerComponent.Get())
			{
				...
				const Chaos::FVec3 Dir = DeltaMag > 0 ? (Delta / DeltaMag) : Chaos::FVec3(1, 0, 0);

				FSweepHit Hit;
				Hit.Distance = TNumericLimits<float>::Max();
				...
				for (int32 ShapeIdx = 0; ShapeIdx < NumShapes; ShapeIdx++)
				{
					...
					if ((bSweepComplex && bShapeIsComplex) || (!bSweepComplex && bShapeIsSimple))
					{
						...
						// This is what it needs to be, update later...
						const FPhysicsGeometry& QueryGeometry = ShapeAdapter.GetGeometry();
						const bool bShapeHit = Chaos::Utilities::CastHelper(QueryGeometry, ActorTM,
							[&](const auto& Downcast, const auto& FullActorTM)
							{
								return Chaos::SweepQuery(*Shape->GetGeometry(), FullActorTM, Downcast, StartTM, Dir, DeltaMag, Distance, WorldPosition, WorldNormal, FaceIdx, FaceNormal, 0.f, true);
							}
						);

						// we should eventually have the Hit structure to use a Chaos::FReal equivalent instead;
						const float FloatDistance = static_cast<float>(Distance);
						if (bShapeHit && FloatDistance < Hit.Distance)
						{
							...
							ConvertQueryImpactHit(OwnerComponentInst->GetWorld(), Hit, OutHit, DeltaMag, QueryFilter, InStart, InEnd, &QueryGeometry, StartTransform, false, false);
							bSweepHit = true;
						}
					}
				}
...
{
	const FBodyInstance* TargetInstance = InInstance->WeldParent ? InInstance->WeldParent : InInstance;
	FPhysicsActorHandle RigidBody = TargetInstance->GetPhysicsActor();
...
bool FPhysInterface_Chaos::GetSquaredDistanceToBody(const FBodyInstance* InInstance, const FVector& InPoint, float& OutDistanceSquared, FVector* OutOptPointOnBody)
{
	...
	FPhysicsCommand::ExecuteRead(UseBI->GetPhysicsActor(), [&](const FPhysicsActorHandle& Actor)
	{
		bEarlyOut = false;
