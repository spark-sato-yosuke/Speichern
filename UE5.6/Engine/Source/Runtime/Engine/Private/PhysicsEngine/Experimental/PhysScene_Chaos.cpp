...
#if UE_BUILD_SHIPPING
inline constexpr bool GEmitSyncBodiesUObjectStats = false;
#else

static bool GEmitSyncBodiesUObjectStats = false;
static FAutoConsoleVariableRef CVar_EmitSyncBodiesUObjectStats(
	TEXT("p.Chaos.EmitSyncBodiesUObjectStats"),
	GEmitSyncBodiesUObjectStats,
	TEXT("Emits profiler scopes for uobjects updating in SyncBodies"));
...
#endif
{
	...
	// Create physics replication manager
	CreatePhysicsReplication();

	// Create physics replication LOD manager
...
{
	UE_CHAOS_ASYNC_INITBODY_WRITESCOPELOCK(CollisionEventRegistrationsLock);
...
void FPhysScene_Chaos::PushPendingSerializedStateForObject(UObject* InComponent, Chaos::FSerializedDataBufferPtr&& InState)
{
	if (InComponent && ensure(InState))
	{
		PendingMigratedPhysicsStateByComponent.Emplace(InComponent, MoveTemp(InState));
	}
}

Chaos::FSerializedDataBufferPtr FPhysScene_Chaos::PopPendingSerializedStateForObject(UObject* InComponent)
{
	if (PendingMigratedPhysicsStateByComponent.IsEmpty())
	{
		return nullptr;
	}

	Chaos::FSerializedDataBufferPtr FoundStateRef = nullptr;
	PendingMigratedPhysicsStateByComponent.RemoveAndCopyValue(InComponent, FoundStateRef);

	return MoveTemp(FoundStateRef);
}
...
IPhysicsReplication* FPhysScene_Chaos::CreatePhysicsReplication()
{
	...
	PhysicsReplication = nullptr;
	if (PhysicsReplicationFactory.IsValid())
	{
		PhysicsReplication = PhysicsReplicationFactory->CreatePhysicsReplication(this);
	}

	// Check that replication manager was created, else create the default implementation
	if (PhysicsReplication == nullptr)
	{
		PhysicsReplication = MakeUnique<FPhysicsReplication>(this);
	}
...
{
	return PhysicsReplicationLOD.Get();
}
...
IPhysicsReplicationLOD* FPhysScene_Chaos::CreatePhysicsReplicationLOD()
{
	...
	if (PhysicsReplicationFactory.IsValid())
	{
		PhysicsReplicationLOD = PhysicsReplicationFactory->CreatePhysicsReplicationLOD(this);
	}

	// Check that replication LOD manager was created, else create the default implementation
	if (PhysicsReplicationLOD == nullptr)
	{
		PhysicsReplicationLOD = MakeUnique<FPhysicsReplicationLOD>(this);
	}

	// Return ptr to the new physics replication LOD 
	return PhysicsReplicationLOD.Get();
}
...
const FBodyInstance* FPhysScene_Chaos::GetBodyInstanceFromProxyAndShape(IPhysicsProxyBase* InProxy, int32 InShapeIndex) const
{
	...
	if(ProxyInstance && InShapeIndex != INDEX_NONE)
	{
		...
		FPhysicsInterface::GetAllShapes_AssumedLocked(ProxyInstance->GetPhysicsActor(), ShapeHandles);
...
void FPhysScene_Chaos::HandleCollisionEvents(const Chaos::FCollisionEventData& Event)
{
	...
	if (NumCollisions > 0 && LastEventDispatchTime < Event.CollisionData.TimeCreated)
	{
		...
		UE_CHAOS_ASYNC_INITBODY_READSCOPELOCK(CollisionEventRegistrationsLock);
...
{
	UE_CHAOS_ASYNC_INITBODY_WRITESCOPELOCK(PhysicsProxyComponentMapsLock);
	TObjectPtr<UPrimitiveComponent>* const Component = PhysicsProxyToComponentMap.Find(InObject);
	if (Component)
	{
		PendingMigratedPhysicsStateByComponent.Remove(*Component);
...
{
	FPhysicsInterface::AddForce_AssumesLocked(BodyInstance->GetPhysicsActor(), Force, bAllowSubstepping, bAccelChange);
}
...
{
	FPhysicsInterface::AddForceAtPosition_AssumesLocked(BodyInstance->GetPhysicsActor(), Force, Position, bAllowSubstepping, bIsLocalForce);
}
...
{
	FPhysicsInterface::AddRadialForce_AssumesLocked(BodyInstance->GetPhysicsActor(), Origin, Radius, Strength, Falloff, bAccelChange, bAllowSubstepping);
}
...
{
	FPhysicsInterface::AddTorque_AssumesLocked(BodyInstance->GetPhysicsActor(), Torque, bAllowSubstepping, bAccelChange);
}
...
void FPhysScene_Chaos::SetKinematicTarget_AssumesLocked(FBodyInstance* BodyInstance, const FTransform& TargetTM, bool bAllowSubstepping)
{
	...
	FPhysicsInterface::SetKinematicTarget_AssumesLocked(BodyInstance->GetPhysicsActor(), TargetTM);
}

bool FPhysScene_Chaos::GetKinematicTarget_AssumesLocked(const FBodyInstance* BodyInstance, FTransform& OutTM) const
{
	OutTM = FPhysicsInterface::GetKinematicTarget_AssumesLocked(BodyInstance->GetPhysicsActor());
	return true;
}
	...
	T_TRANSFORMCONTAINER& TeleportTransforms)
{
	...
	for (int32 i = 0; i < NumBodies; i++)
	{
		...
		FPhysicsActorHandle ActorHandle = BodyInst->GetPhysicsActor();
...
{
	return Chaos::CVars::bEnableAsyncInitBody;
}

bool FPhysScene_Chaos::SupportsAsyncPhysicsStateDestruction()
{
	return SupportsAsyncPhysicsStateCreation();
}

bool FPhysScene_Chaos::PushAsyncCreatePhysicsStateJob(IAsyncPhysicsStateProcessor* Processor)
{
	if (SupportsAsyncPhysicsStateCreation())
	{
		check(IsInGameThread());
		check(!Processor->IsAsyncPhysicsStateCreated());
		check(Processor->AllowsAsyncPhysicsStateCreation());

		if (!AsyncPhysicsStateJobQueue.IsValid())
		{
			AsyncPhysicsStateJobQueue = MakeUnique<FPhysScene_AsyncPhysicsStateJobQueue>(this);
		}
		AsyncPhysicsStateJobQueue->AddJob({ Processor, FPhysScene_AsyncPhysicsStateJobQueue::EJobType::CreatePhysicsState });
		return true;
	}
...
{
	if (SupportsAsyncPhysicsStateDestruction())
	{
		check(IsInGameThread());
		check(Processor->IsAsyncPhysicsStateCreated());
		check(Processor->AllowsAsyncPhysicsStateDestruction());

		if (!AsyncPhysicsStateJobQueue.IsValid())
		{
			AsyncPhysicsStateJobQueue = MakeUnique<FPhysScene_AsyncPhysicsStateJobQueue>(this);
		}
		AsyncPhysicsStateJobQueue->AddJob({ Processor, FPhysScene_AsyncPhysicsStateJobQueue::EJobType::DestroyPhysicsState });
	}
	return true;
}
...
void FPhysScene_Chaos::RemoveAsyncPhysicsStateJobs(IAsyncPhysicsStateProcessor* Processor)
{
	...
	if (AsyncPhysicsStateJobQueue.IsValid())
	{
		AsyncPhysicsStateJobQueue->RemoveJob({ Processor, FPhysScene_AsyncPhysicsStateJobQueue::EJobType::CreatePhysicsState });
		AsyncPhysicsStateJobQueue->RemoveJob({ Processor, FPhysScene_AsyncPhysicsStateJobQueue::EJobType::DestroyPhysicsState });
	}
}

void FPhysScene_Chaos::ProcessAsyncPhysicsStateJobs(bool bWaitForCompletion)
{
	check(IsInGameThread());
	if (AsyncPhysicsStateJobQueue.IsValid())
	{
		AsyncPhysicsStateJobQueue->Tick(bWaitForCompletion);
	}
}
		...
		for (const TPair<TWeakObjectPtr<USkeletalMeshComponent>, FDeferredKinematicUpdateInfo>& DeferredKinematicUpdate : DeferredKinematicUpdateSkelMeshes)
		{
			...
			if (!SkelComp->bEnablePerPolyCollision)
			{
				...
				for (int32 i = 0; i < NumBodies; i++)
				{
					...
					if (!BodyInst->IsInstanceSimulatingPhysics() || Info.TeleportType != ETeleportType::None)
					{
						const int32 BoneIndex = BodyInst->InstanceBoneIndex;
						if (BoneIndex != INDEX_NONE)
						{
				...
				for (int32 i = 0; i < NumBodies; i++)
				{
					...
					if (!BodyInst->IsInstanceSimulatingPhysics() || Info.TeleportType != ETeleportType::None)
					{
						...
						if (BoneIndex != INDEX_NONE)
						{
							...
							{
								UE_LOG(LogChaos,Log,TEXT("Bone Index %d out of bounds (%d Bones)"), BoneIndex, ComponentSpaceTransforms.Num());
							}
						}
					}
				}
...
void FPhysScene_Chaos::RegisterAsyncPhysicsTickActor(AActor* Actor)
{
	...
	{ 
		UE_LOG(LogChaos, Log, TEXT("RegisterAsyncPhysicsTickActor %s @ 0x%" SIZE_T_x_FMT), *AActor::GetDebugName(Actor), (SIZE_T)Actor);
	}
}
...
void FPhysScene_Chaos::UnregisterAsyncPhysicsTickActor(AActor* Actor)
{
	if (AsyncPhysicsTickCallback)
	{
		...
		{
			UE_LOG(LogChaos, Log, TEXT("UnregisterAsyncPhysicsTickActor %s @ 0x%" SIZE_T_x_FMT), *AActor::GetDebugName(Actor), (SIZE_T)Actor);
		}
