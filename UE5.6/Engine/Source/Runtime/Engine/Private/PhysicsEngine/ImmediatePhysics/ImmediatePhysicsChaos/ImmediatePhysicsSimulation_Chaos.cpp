...
namespace ImmediatePhysics_Chaos
{
	...
	public:
			...
			, bActorsDirty(false)
		{
			...
			Evolution.AddConstraintContainer(Joints.LinearConstraints, 0);
			Evolution.AddConstraintContainer(Joints.NonLinearConstraints, 0);
		}
		...
		// The motion of the simulation space for calculation of phantom forces
		Chaos::FSimulationSpace SimulationSpace;

		// The simulation space transform, used for debug draw
		...
		int32 MaxNumRollingAverageStepTimes; // Set to 1 to disable smoothing
	...
	{
		return CreateActor(ImmediatePhysics::MakeActorSetup(EActorType::StaticActor, BodyInstance, BodyInstance->GetUnrealWorldTransform()));
	}

	FActorHandle* FSimulation::CreateKinematicActor(FBodyInstance* BodyInstance, const FTransform& Transform)
	{
		return CreateActor(ImmediatePhysics::MakeActorSetup(EActorType::KinematicActor, BodyInstance, Transform));
	}

	FActorHandle* FSimulation::CreateDynamicActor(FBodyInstance* BodyInstance, const FTransform& Transform)
	{
		return CreateActor(ImmediatePhysics::MakeActorSetup(EActorType::DynamicActor, BodyInstance, Transform));
	}

	FActorHandle* FSimulation::CreateActor(EActorType ActorType, FBodyInstance* BodyInstance, const FTransform& Transform)
	{
		return CreateActor(ImmediatePhysics::MakeActorSetup(ActorType, BodyInstance, Transform));
	}

	FActorHandle* FSimulation::CreateActor(FActorSetup&& ActorSetup)
	{
		FActorHandle* ActorHandle = new FActorHandle(Implementation->Particles, Implementation->ParticlePrevXs, Implementation->ParticlePrevRs, MoveTemp(ActorSetup));
		int ActorIndex = Implementation->ActorHandles.Add(ActorHandle);

		ActorHandle->GetParticle()->AuxilaryValue(Implementation->ParticleMaterials) = MakeSerializable(ActorSetup.Material);
		ActorHandle->GetParticle()->AuxilaryValue(Implementation->PerParticleMaterials) = MoveTemp(ActorSetup.Material);
	...
	void FSimulation::SetIsKinematic(FActorHandle* ActorHandle, bool bKinematic)
	{
		...
		if (bKinematic != bWasKinematic)
		{
			...
			Implementation->bActorsDirty = true;
			// Force constraint sorting to be updated
			Implementation->Joints.LinearConstraints.MarkJointsDirty();
			Implementation->Joints.NonLinearConstraints.MarkJointsDirty();
		}
	}
	...
	{
		return CreateJoint(ImmediatePhysics::MakeJointSetup(ConstraintInstance, Body1, Body2));
	}

	FJointHandle* FSimulation::CreateJoint(const FJointSetup& JointSetup)
	{
		if (JointSetup.ConstraintSettings.bUseLinearSolver)
		{
			FJointHandle* JointHandle = new FJointHandle(&(Implementation->Joints.LinearConstraints), JointSetup);
			Implementation->JointHandles.Add(JointHandle);
			return JointHandle;
		}
		else
		{
			FJointHandle* JointHandle = new FJointHandle(&(Implementation->Joints.NonLinearConstraints), JointSetup);
			Implementation->JointHandles.Add(JointHandle);
			return JointHandle;
		}
	}
	...
	{
		bool bCouldBeDynamic = ActorHandle->CouldBeDynamic();
		using namespace Chaos;
		...
		for (FActorHandle* OtherActorHandle : Implementation->ActorHandles)
		{
			...
			if ((OtherActorHandle != ActorHandle) && (bCouldBeDynamic || OtherActorHandle->CouldBeDynamic()))
			{
	...
	void FSimulation::SetSimulationSpaceSettings(const bool bEnabled, const FReal DampingAlpha, const FVector& ExternalLinearEtherDrag)
	{
		using namespace Chaos;

		FSimulationSpaceSettings& SimSpaceSettings = Implementation->Evolution.GetSimulationSpaceSettings();
		SimSpaceSettings.bEnabled = bEnabled;
		SimSpaceSettings.LinearVelocityAlpha = DampingAlpha;
		SimSpaceSettings.AngularVelocityAlpha = DampingAlpha;
	...
	void FSimulation::SetSimulationSpaceSettings(const Chaos::FSimulationSpaceSettings& SimulationSpaceSettings)
	{
		Implementation->Evolution.SetSimulationSpaceSettings(SimulationSpaceSettings);
	}

	const Chaos::FSimulationSpaceSettings& FSimulation::GetSimulationSpaceSettings() const
	{
		return Implementation->Evolution.GetSimulationSpaceSettings();
	}
	...
	void FSimulation::SetSolverSettings(const FReal FixedDt, const FReal CullDistance, const FReal MaxDepenetrationVelocity, const int32 UseLinearJointSolver, const int32 PositionIts, const int32 VelocityIts, const int32 ProjectionIts, const int32 bUseManifoilds)
	{
		...
		if (UseLinearJointSolver >= 0)
		{
			if (UseLinearJointSolver != 0)
			{
				if (Implementation->Joints.NonLinearConstraints.NumConstraints() > 0)
				{
					Implementation->Joints.MergeNonLinearConstraintsIntoLinearConstraints();
				}
			}
			else
			{
				if (Implementation->Joints.LinearConstraints.NumConstraints() > 0)
				{
					Implementation->Joints.MergeLinearConstraintsIntoNonLinearConstraints();
				}
			}
			Implementation->Joints.LinearConstraints.SetUseSimd(bChaosImmediate_UseSimdForLinearSolver); 
		}
	...
	void FSimulation::SetMaxNumRollingAverageStepTimes(const int32 MaxNumRollingAverageStepTimes)
	{
		if (MaxNumRollingAverageStepTimes >= 0)
		{
			Implementation->MaxNumRollingAverageStepTimes = MaxNumRollingAverageStepTimes;
		}
	}
	...
	{
		if (Implementation->NumRollingAverageStepTimes > 1)
		{
		...
		{
			// Adjust things so that if we go back to smoothing the step time, we do so gracefully
			Implementation->NumRollingAverageStepTimes = Implementation->MaxNumRollingAverageStepTimes;
			Implementation->RollingAverageStepTime = FMath::Min(DeltaTime, MaxStepTime);
		}
	...
	void FSimulation::Simulate(
		FReal InDeltaTime, FReal MaxStepTime, int32 MaxSubSteps, const FVector& InGravity,
		Chaos::FPBDJointSolverSettings* JointSolverSettings, 
		Chaos::FPBDCollisionSolverSettings* CollisionSolverSettings,
		Chaos::FCollisionDetectorSettings* CollisionDetectorSettings)
	{
		SCOPE_CYCLE_COUNTER(STAT_ImmediateSimulate_Chaos);
		CVD_SCOPE_TRACE_SOLVER_FRAME(FSimulation, *this);
		CVD_TRACE_SOLVER_SIMULATION_SPACE(Implementation->SimulationSpaceTransform);
		...
		// These are overrides, so only apply if > 0. 
		SetSolverSettings(
		...
		SetMaxNumRollingAverageStepTimes(ChaosImmediate_Evolution_DeltaTimeCount);

		if (JointSolverSettings)
		{
			Implementation->Joints.LinearConstraints.SetSettings(*JointSolverSettings);
			Implementation->Joints.NonLinearConstraints.SetSettings(*JointSolverSettings);
		}
		else
		{
			FPBDJointSolverSettings JointsSettings = Implementation->Joints.LinearConstraints.GetSettings();
			...
			Implementation->Joints.LinearConstraints.SetSettings(JointsSettings);
			Implementation->Joints.NonLinearConstraints.SetSettings(JointsSettings);
		}

		if (CollisionSolverSettings)
		{
			Implementation->Collisions.SetSolverSettings(*CollisionSolverSettings);
		}
		else
		{
		...
		if (CollisionDetectorSettings)
		{
			Implementation->Collisions.SetDetectorSettings(*CollisionDetectorSettings);
		}
		else
		{
			FCollisionDetectorSettings DetectorSettings = Implementation->Collisions.GetDetectorSettings();
	...
	void FSimulation::UpdateInertiaConditioning(const FVector& Gravity)
	{
		...
		const bool bUseInertiaConditioning = Implementation->Joints.LinearConstraints.NumConstraints() > 0 ? bChaosImmediate_LinearInertiaConditioningEnabled : bChaosImmediate_NonLinearInertiaConditioningEnabled;
			...
			for (FActorHandle* Actor : Implementation->ActorHandles)
			{
				...
				if (Particle->InertiaConditioningDirty())
				{
					...
					if (bWantInertiaConditioning)
					{
						...
						{
							if (!FPBDJointUtilities::GetLinearIsFree(Joint->GetConstraint()->GetSettings()))
							{
	...
	void FSimulation::DebugDrawStaticParticles()
	{
		...
		if (FDebugDrawQueue::IsDebugDrawingEnabled() && bChaosImmediate_DebugDrawShowStatics)
		{
			...
			{
				DebugDraw::DrawParticleTransforms(Implementation->SimulationSpaceTransform, Implementation->Particles.GetActiveStaticParticlesView(), &ChaosImmPhysDebugDebugDrawSettings);
			}
			if (bChaosImmediate_DebugDrawShapes)
			{
				DebugDraw::DrawParticleShapes(Implementation->SimulationSpaceTransform, Implementation->Particles.GetActiveStaticParticlesView(), 1.0f, &ChaosImmPhysDebugDebugDrawSettings);
			}
			if (bChaosImmediate_DebugDrawBounds)
			{
				DebugDraw::DrawParticleBounds(Implementation->SimulationSpaceTransform, Implementation->Particles.GetActiveStaticParticlesView(), 0.0f, &ChaosImmPhysDebugDebugDrawSettings);
			}
		}
#endif
	}
	...
	void FSimulation::DebugDrawKinematicParticles()
	{
		...
		if (FDebugDrawQueue::IsDebugDrawingEnabled() && bChaosImmediate_DebugDrawShowKinematics)
		{
			if (bChaosImmediate_DebugDrawParticles)
			{
				DebugDraw::DrawParticleTransforms(Implementation->SimulationSpaceTransform, Implementation->Particles.GetActiveKinematicParticlesView(), &ChaosImmPhysDebugDebugDrawSettings);
			}
			if (bChaosImmediate_DebugDrawShapes)
			{
				DebugDraw::DrawParticleShapes(Implementation->SimulationSpaceTransform, Implementation->Particles.GetActiveKinematicParticlesView(), 1.0f, &ChaosImmPhysDebugDebugDrawSettings);
			}
			if (bChaosImmediate_DebugDrawBounds)
			{
				DebugDraw::DrawParticleBounds(Implementation->SimulationSpaceTransform, Implementation->Particles.GetActiveKinematicParticlesView(), 0.0f, &ChaosImmPhysDebugDebugDrawSettings);
			}
		}
#endif
	}
	...
	void FSimulation::DebugDrawDynamicParticles()
	{
		...
		if (FDebugDrawQueue::IsDebugDrawingEnabled() && bChaosImmediate_DebugDrawShowDynamics)
		{
			if (bChaosImmediate_DebugDrawParticles)
			{
				DebugDraw::DrawParticleTransforms(Implementation->SimulationSpaceTransform, Implementation->Particles.GetActiveParticlesView(), &ChaosImmPhysDebugDebugDrawSettings);
			}
			if (bChaosImmediate_DebugDrawShapes)
			{
				DebugDraw::DrawParticleShapes(Implementation->SimulationSpaceTransform, Implementation->Particles.GetActiveParticlesView(), 1.0f, &ChaosImmPhysDebugDebugDrawSettings);
			}
			if (bChaosImmediate_DebugDrawBounds)
			{
				DebugDraw::DrawParticleBounds(Implementation->SimulationSpaceTransform, Implementation->Particles.GetActiveParticlesView(), 0.0f, &ChaosImmPhysDebugDebugDrawSettings);
			}
		}
#endif
	}

	const Chaos::FCollisionDetectorSettings& FSimulation::GetCollisionDetectorSettings() const
	{
		return Implementation->Collisions.GetDetectorSettings();
	}

	void FSimulation::SetCollisionDetectorSettings(const Chaos::FCollisionDetectorSettings& Settings)
	{
		Implementation->Collisions.SetDetectorSettings(Settings);
	}

	Chaos::FVec3 FSimulation::FCollisionData::GetCollisionAccumulatedImpulse() const
	{
		return Collision.GetAccumulatedImpulse();
	}

	int32 FSimulation::FCollisionData::GetNumManifoldPoints() const
	{
		return Collision.NumManifoldPoints();
	}

	const void FSimulation::FCollisionData::GetManifoldPointData(
		...
		Chaos::FVec3&            PlaneLocation) const
	{
		...
		Depth = Collision.GetManifoldPoint(PointIndex).InitialPhi;

		const FConstGenericParticleHandle Particle0 = Collision.GetParticle0();
		const FConstGenericParticleHandle Particle1 = Collision.GetParticle1();
		...
		const FManifoldPoint& ManifoldPoint = Collision.GetManifoldPoint(PointIndex);
		const FManifoldPointResult& ManifoldPointResult = Collision.GetManifoldPointResult(PointIndex);

		const int32 ContactPlaneOwner = 1;
		const int32 ContactPointOwner = 1 - ContactPlaneOwner;
		const Chaos::FRigidTransform3 PlaneTransform = (ContactPlaneOwner == 0)
			? Collision.GetShapeRelativeTransform0() * WorldActorTransform0
			: Collision.GetShapeRelativeTransform1() * WorldActorTransform1;
		const Chaos::FRigidTransform3 PointTransform = (ContactPlaneOwner == 0)
			? Collision.GetShapeRelativeTransform1() * WorldActorTransform1
			: Collision.GetShapeRelativeTransform0() * WorldActorTransform0;

		PlaneNormal = PlaneTransform.TransformVectorNoScale(
			FVec3(ManifoldPoint.ContactPoint.ShapeContactNormal));
		PointLocation = PointTransform.TransformPosition(
			FVec3(ManifoldPoint.ContactPoint.ShapeContactPoints[ContactPointOwner]));
		PlaneLocation = PlaneTransform.TransformPosition(
			FVec3(ManifoldPoint.ContactPoint.ShapeContactPoints[ContactPlaneOwner]));
	}

	void FSimulation::VisitCollisions(
		TFunction<void(const FCollisionData&)> Visitor, Chaos::ECollisionVisitorFlags VisitorFlags) const
	{
		using namespace Chaos;
		const Chaos::Private::FCollisionConstraintAllocator& ConstraintAllocator = Implementation->Collisions.GetConstraintAllocator();
		ConstraintAllocator.VisitConstCollisions(
			[&](const FPBDCollisionConstraint& Collision)
			{
				Visitor(FCollisionData(Collision));
	...
	void FSimulation::DebugDrawConstraints()
	{
		...
		if (FDebugDrawQueue::IsDebugDrawingEnabled())
		{
			...
			{
				DebugDraw::DrawCollisions(Implementation->SimulationSpaceTransform, Implementation->Collisions.GetConstraintAllocator(), 1.0f, &ChaosImmPhysDebugDebugDrawSettings);
			}
			if (bChaosImmediate_DebugDrawJoints)
			{
				DebugDraw::DrawJointConstraints(Implementation->SimulationSpaceTransform, Implementation->Joints.LinearConstraints, 1.0f, ChaosImmediate_DebugDrawJointFeatures, &ChaosImmPhysDebugDebugDrawSettings);
				DebugDraw::DrawJointConstraints(Implementation->SimulationSpaceTransform, Implementation->Joints.NonLinearConstraints, 1.0f, ChaosImmediate_DebugDrawJointFeatures, &ChaosImmPhysDebugDebugDrawSettings);
			}
		}
#endif
	}
	...
	void FSimulation::DebugDrawSimulationSpace()
	{
		...
		{
			DebugDraw::DrawSimulationSpace(Implementation->SimulationSpaceTransform, Implementation->SimulationSpace, &ChaosImmPhysDebugDebugDrawSettings);
		}
#endif
	}
