...
static FAutoConsoleVariableRef CVarParallelGatherDynamicRayTracingInstances(
	TEXT("r.RayTracing.DynamicInstances.Parallel"),
	GParallelGatherDynamicRayTracingInstances,
	TEXT("Enables parallel processing of the gather dynamic ray tracing instances."),
	ECVF_RenderThreadSafe
);

bool IsParallelGatherDynamicRayTracingInstancesEnabled()
{
	return GParallelGatherDynamicRayTracingInstances &&
		// parallel GDRTI creates RHI resources on a task threads which is not supported by some RHIs (eg. OpenGL)
		GSupportsParallelRenderingTasksWithSeparateRHIThread;
}
...
{
	return CVarCacheWPOPrimitives.GetValueOnAnyThread() != 0;
}
...
,	bIsFirstPersonWorldSpaceRepresentation(InProxyDesc.bIsFirstPersonWorldSpaceRepresentation)
,	bSupportsRuntimeVirtualTexture(false)
,	bImplementsStreamableAssetGathering(false)
...
,	bForceMipStreaming(InProxyDesc.bForceMipStreaming)
...
,	LevelName(InProxyDesc.GetLevel() ? InProxyDesc.GetLevel()->GetOutermost()->GetFName() : NAME_None)
#if WITH_EDITOR
...
,	MinDrawDistance(InProxyDesc.MinDrawDistance)
{
	...
	if (ULevel* Level = InProxyDesc.GetLevel())
	{
		...
		{
			SetPrimitiveColor(FLinearColor::White);
		}
#endif
	}
	...
	{
		for (URuntimeVirtualTexture* RuntimeVirtualTexture : InProxyDesc.GetRuntimeVirtualTextures())
		{
			if (RuntimeVirtualTexture != nullptr)
			{
				RuntimeVirtualTextures.Add(RuntimeVirtualTexture->GetUniqueID());
				RuntimeVirtualTextureMaterialTypes.AddUnique(RuntimeVirtualTexture->GetMaterialType());
			}
		}
	}
	...
	bAnyMaterialHasPixelAnimation = false;
	{
		...
		{
			if (MaterialInterface && MaterialInterface->IsUsingWorldPositionOffset_Concurrent(FeatureLevel))
			{
...
void FPrimitiveSceneProxy::BuildUniformShaderParameters(FPrimitiveUniformShaderParametersBuilder &Builder) const
{
	...
	Builder = FPrimitiveUniformShaderParametersBuilder{}
		.Defaults()
			...
			.VisibleInLumenScene(IsVisibleInLumenScene() || IsLandscapeNaniteProxy())
			...
			.MaterialCacheTextureDescriptor(GetMaterialCacheTextureDescriptor())
	...
	if (IsNaniteMesh())
	{
		...
		uint32 NaniteAssemblyTransformOffset = INDEX_NONE;
		...
		Builder.NaniteResourceID(NaniteResourceID)
			...
			.NaniteAssemblyTransformOffset(NaniteAssemblyTransformOffset)
...
void FPrimitiveSceneProxy::UpdateInstances_RenderThread(FRHICommandListBase& RHICmdList, const FBoxSphereBounds& InBounds, const FBoxSphereBounds& InLocalBounds, const FBoxSphereBounds& InStaticMeshBounds)
{
	...
	ensureMsgf(false, TEXT("Incorrect call to UpdateInstances_RenderThread, please update the code to call the correct version."));
...
void FPrimitiveSceneProxy::UpdateInstances_RenderThread(FRHICommandListBase& RHICmdList, const FBoxSphereBounds& InBounds, const FBoxSphereBounds& InLocalBounds)
{
	TRACE_CPUPROFILER_EVENT_SCOPE_STR("FPrimitiveSceneProxy::UpdateInstances_RenderThread");
	// Update the cached bounds.
	Bounds = InBounds;
...
{
	return GIsEditor && bUseEditorCompositing && !View->bIsGameView;
}
...
{
	check(IsInRenderingThread());
	if (DrawInGame != bInDrawInGame)
	{
		DrawInGame = bInDrawInGame;
...
void FPrimitiveSceneProxy::SetDrawDistance_RenderThread(float InMinDrawDistance, float InMaxDrawDistance, float InVirtualTextureMaxDrawDistance)
{
	...
	// Modify max draw distance for main pass if we are using virtual texturing
	if (WritesVirtualTexture() && InVirtualTextureMaxDrawDistance > 0.f)
	{
...
bool FPrimitiveSceneProxy::DrawInRuntimeVirtualTextureOnly(bool bEditor) const
{
	...
	// Conditional path tests against the collected flags from all runtime virtual textures in the scene.
...
bool FPrimitiveSceneProxy::IsShown(const FSceneView* View) const
{
	...
	if(View->Family->EngineShowFlags.Editor)
	{
		...
		if (DrawInRuntimeVirtualTextureOnly(true) && !View->bIsVirtualTexture && !View->Family->EngineShowFlags.VirtualTexturePrimitives && !IsSelected())
		{
			return false;
		}
	}
...
#endif
	{
		...
		if (DrawInRuntimeVirtualTextureOnly(false) && !View->bIsVirtualTexture)
		{
			return false;
		}

		const bool bOwnersContain = View->ViewActor.IsPartOf(Owners);
...
bool FPrimitiveSceneProxy::IsShadowCast(const FSceneView* View) const
{
	...
	if (!CastsHiddenShadow())
	{
		...
		if (DrawInRuntimeVirtualTextureOnly(View->Family->EngineShowFlags.Editor) && !View->bIsVirtualTexture)
		{
			return false;
		}
...
void FPrimitiveSceneProxy::UpdateVisibleInLumenScene()
{
	...
	static const auto LumenUseFarFieldOcclusionOnlyCVar = IConsoleManager::Get().FindTConsoleVariableDataInt(TEXT("r.LumenScene.FarField.OcclusionOnly"));
	...
	bool bAffectsLumen = AffectsDynamicIndirectLighting();
	...
	if (bLumenUsesHardwareRayTracing)
	{
		...
		if (IsRayTracingAllowed() && HasRayTracingRepresentation())
		{
			...
			if (IsRayTracingFarField() && LumenUseFarFieldOcclusionOnlyCVar->GetValueOnAnyThread() != 0)
			{
				bAffectsLumen = false;
			}
		}
