	...
	if (SubLevelBrickPositions.Num() > 0)
	{
...

		{
			const FRHIBufferCreateDesc CreateDesc =
				FRHIBufferCreateDesc::CreateVertex(TEXT("SubLevelBrickPositionsBuffer"), SubLevelBrickPositions.Num() * SubLevelBrickPositions.GetTypeSize())
				.AddUsage(BUF_Static | BUF_ShaderResource)
				.SetInitActionResourceArray(&SubLevelBrickPositions)
				.DetermineInitialState();

			SubLevelBrickPositionsBuffer = RHICmdList.CreateBuffer(CreateDesc);
			SubLevelBrickPositionsSRV = RHICmdList.CreateShaderResourceView(
		...
		{
			const FRHIBufferCreateDesc CreateDesc =
				FRHIBufferCreateDesc::CreateVertex(TEXT("IndirectionTextureOriginalValuesBuffer"), IndirectionTextureOriginalValues.Num() * IndirectionTextureOriginalValues.GetTypeSize())
				.AddUsage(BUF_Static | BUF_ShaderResource)
				.SetInitActionResourceArray(&IndirectionTextureOriginalValues)
				.DetermineInitialState();

			IndirectionTextureOriginalValuesBuffer = RHICmdList.CreateBuffer(CreateDesc);
			IndirectionTextureOriginalValuesSRV = RHICmdList.CreateShaderResourceView(
