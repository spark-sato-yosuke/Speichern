...
void AActor::GatherCurrentMovement()
{
	if (IsReplicatingMovement() || (RootComponent && RootComponent->GetAttachParent()))
	{
		...
		if (RootPrimComp && RootPrimComp->IsSimulatingPhysics())
		{
			...
			int ServerFrame = 0; 
			if (bShouldUsePhysicsReplicationCache && World && !World->IsRecordingClientReplay())
			{
...
void AActor::SetReplicatedComponentNetCondition(const UActorComponent* ReplicatedComponent, ELifetimeCondition NetCondition)
{
	...
	if (FReplicatedComponentInfo* ComponentInfo = ReplicatedComponentsInfo.FindByKey(ReplicatedComponent))
	{
		...
		{
#if UE_WITH_IRIS
			UE::Net::FReplicationSystemUtil::SetNetConditionForActorComponent(ReplicatedComponent, NetCondition);
#endif
...
#if UE_WITH_IRIS
	if (GetIsReplicated() && (ActorHasBegunPlay != EActorBeginPlayState::HasNotBegunPlay))
	{
		...
		if (AActor* Actor = EveryChildren.Pop(EAllowShrinking::No))
		{
			...
			if (Actor->GetIsReplicated())
			{
				...
				if (RefHandle.IsValid())
				{
					...
					const bool bEnableAutonomousCondition = Actor->GetRemoteRole() == ROLE_AutonomousProxy;
...
void AActor::RegisterAsFocalPointInPhysicsReplicationLOD() const
{
	if (UPrimitiveComponent* RootPrimComp = Cast<UPrimitiveComponent>(RootComponent))
	{
		RootPrimComp->RegisterAsFocalPointInPhysicsReplicationLOD();
	}
}
...
void AActor::UnregisterAsFocalPointInPhysicsReplicationLOD() const
{
	if (UPrimitiveComponent* RootPrimComp = Cast<UPrimitiveComponent>(RootComponent))
	{
		RootPrimComp->UnregisterAsFocalPointInPhysicsReplicationLOD();
	}
}
