...
bool FDataTableImporterJSON::AddRow(bool bRemoveDuplicate, FName* OutRowName)
{
	if (JSONData.IsEmpty())
	{
		ImportProblems.Add(TEXT("Input data is empty."));
	...
	TSharedPtr<FJsonValue> ParsedTableRow;
	{
		const TSharedRef< TJsonReader<TCHAR> > JsonReader = TJsonReaderFactory<TCHAR>::Create(JSONData);
		if (!FJsonSerializer::Deserialize(JsonReader, ParsedTableRow) || !ParsedTableRow.IsValid())
		{
			ImportProblems.Add(FString::Printf(TEXT("Failed to parse the JSON data. Error: %s"), *JsonReader->GetErrorMessage()));
			return false;
		}
	...
	TSharedPtr<FJsonObject> ParsedTableRowObject = ParsedTableRow->AsObject();
	if (!ParsedTableRowObject.IsValid())
	{
		ImportProblems.Add(TEXT("Row is not a valid JSON object."));
		return false;
	}

	return ReadRow(ParsedTableRowObject.ToSharedRef(), 0, bRemoveDuplicate, OutRowName);
}

bool FDataTableImporterJSON::ReadRow(const TSharedRef<FJsonObject>& InParsedTableRowObject, const int32 InRowIdx, bool bRemoveDuplicate, FName* OutRowName)
{
	// Get row name
	...
	else if (OutRowName != nullptr)
	{
		*OutRowName = RowName;
	}

	// Check its not a duplicate
	if (DataTable->GetRowMap().Find(RowName) != nullptr)
	{
		if (bRemoveDuplicate)
		{
			DataTable->RemoveRowInternal(RowName);
		}
		else if (!DataTable->AllowDuplicateRowsOnImport())
		{
