...
namespace UE::Input
{
	static int32 DeviceMappingPolicy = 1;
	static FAutoConsoleVariableRef CVarDeviceMappingPolicy(TEXT("input.DeviceMappingPolicy"),
	DeviceMappingPolicy,
	TEXT("The policy defining the behavior of a newly connected input device. See EInputDeviceMappingPolicy\n")
	TEXT("0 - UseManagedPlatformLogin\n")
	TEXT("1 - PrimaryUserSharesKeyboardAndFirstGamepad (PC Default)\n")
	TEXT("2 - CreateUniquePlatformUserForEachDevice\n")
	TEXT("3 - MapAllDevicesToPrimaryUser\n"),
	ECVF_Default);
};
...
UInputPlatformSettings::UInputPlatformSettings()
	...
	, DeviceMappingPolicy(EInputDeviceMappingPolicy::PrimaryUserSharesKeyboardAndFirstGamepad)
...
#if WITH_EDITOR
void UInputPlatformSettings::PostEditChangeChainProperty(FPropertyChangedChainEvent& PropertyChangedEvent)
{
	Super::PostEditChangeChainProperty(PropertyChangedEvent);
	
	const FName MemberPropertyName = PropertyChangedEvent.PropertyChain.GetActiveMemberNode()->GetValue()->GetFName();
	
	if (MemberPropertyName == GET_MEMBER_NAME_CHECKED(UInputPlatformSettings, DeviceMappingPolicy))
	{
		IPlatformInputDeviceMapper::Get().HandleInputDevicePolicyChanged(DeviceMappingPolicy);
	}
}
