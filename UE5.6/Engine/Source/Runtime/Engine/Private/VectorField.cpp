...
public:
	...
	virtual void InitRHI(FRHICommandListBase& RHICmdList) override
	{
		if (VolumeData && GSupportsTexture3D)
		{
			...
			FResourceBulkDataArrayView BulkDataInterface(VolumeData, DataSize);
...
void UVectorFieldStatic::UpdateCPUData(bool bDiscardData)
{
	if (bAllowCPUAccess)
	{
		...
		{
			UE_LOG(LogVectorField, Error, TEXT("Vector field bulk data size is different than expected. Expected %d bytes, got %" INT64_FMT "."), SizeX*SizeY*SizeZ, SourceData.GetBulkDataSize());
			FMemory::Free(Ptr);
			return;
		}
...
public:
	...
	virtual void InitRHI(FRHICommandListBase& RHICmdList) override
	{
		if (GSupportsTexture3D)
		{
			...
			check(SizeY > 0);
			check(SizeZ > 0);
			UE_LOG(LogVectorField,Verbose,TEXT("InitRHI for 0x%016" PTRINT_x_FMT " %dx%dx%d"),(PTRINT)this,SizeX,SizeY,SizeZ);
			...
			if (GetFeatureLevel() >= ERHIFeatureLevel::SM5)
			{
				VolumeTextureUAV = RHICmdList.CreateUnorderedAccessView(VolumeTextureRHI, FRHIViewDesc::CreateTextureUAV().SetDimensionFromTexture(VolumeTextureRHI));
			}
		}
	}
