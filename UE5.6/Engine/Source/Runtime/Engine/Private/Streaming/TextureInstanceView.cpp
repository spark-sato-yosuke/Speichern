...
void FBounds4::Set(int32 Index, const FBoxSphereBounds& Bounds, uint32 InPackedRelativeBox, float InLastRenderTime, const FVector& RangeOrigin, float InMinDistanceSq, float InMinRangeSq, float InMaxRangeSq)
{
...
void FBounds4::UnpackBounds(int32 Index, const UPrimitiveComponent* Component)
{
	check(Component);
	check(Index >= 0 && Index < 4);
...
void FBounds4::FullUpdate(int32 Index, const FVector& NewOrigin, const FVector& NewBoxExtent, float NewSphereRadius, float InLastRenderTime)
{
	check(Index >= 0 && Index < 4);
	...
	const FRenderAssetStreamingSettings& Settings)
{
	...
	UpdateBoundSizes(ViewInfos,ViewInfoExtras,
		LastUpdateTime,
		View->GetMaxTexelFactor(),
		Settings,
		View->GetBoundsView(),
		BoundsViewInfo,
...
void FRenderAssetInstanceAsyncView::UpdateBoundSizes(
	const TArray<FStreamingViewInfo>& ViewInfos,
	const FStreamingViewInfoExtraArray& ViewInfoExtras,
	float LastUpdateTime,
	float MaxTexelFactor,
	const FRenderAssetStreamingSettings& Settings,
	const TConstArrayView<FBounds4>& Bounds4,
	...
	float& OutMaxLevelRenderAssetScreenSize)
{
	...
	const int32 NumBounds4 = Bounds4.Num();

	const VectorRegister LastUpdateTime4 = VectorSet(LastUpdateTime, LastUpdateTime, LastUpdateTime, LastUpdateTime);

	OutBoundsViewInfos.Empty(NumBounds4 * 4);
	OutBoundsViewInfos.AddUninitialized(NumBounds4 * 4);
	...
	for (int32 Bounds4Index = 0; Bounds4Index < NumBounds4; ++Bounds4Index)
	{
		...
		// Store results
		FBoundsViewInfo* BoundsVieWInfo = &OutBoundsViewInfos[Bounds4Index * 4];
	...
	if (Settings.MinLevelRenderAssetScreenSize > 0)
	{
		...
		OutMaxLevelRenderAssetScreenSize = MaxTexelFactor * ViewMaxNormalizedSizeResult;
	}
}
