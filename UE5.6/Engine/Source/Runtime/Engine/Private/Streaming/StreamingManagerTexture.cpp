...
{
	static bool bAllowParallelForDuringIncrementalUpdate = false;
	static FAutoConsoleVariableRef CVarAllowParallelForDuringIncrementalUpdate(
		TEXT("r.Streaming.AllowParallelRenderAssetStreamingManagerIncrementalUpdate"),
		bAllowParallelForDuringIncrementalUpdate,
		TEXT("Whether it is allowed for RenderAssetStreamingManager to use more cores using ParallelFor when doing the incremental update. ")
		TEXT("When enabled, texture streaming built data is not used to retrieve the primitive streaming render asset information."),
		ECVF_Default);

	static int32 GWorkerCountForParallelForDuringIncrementalUpdate = 0;
	static FAutoConsoleVariableRef CVarWorkerCountForParallelForDuringIncrementalUpdate(
		TEXT("r.Streaming.WorkerCountForParallelRenderAssetStreamingManagerIncrementalUpdate"),
		GWorkerCountForParallelForDuringIncrementalUpdate,
		TEXT("Set the max number of workers when r.Streaming.AllowParallelRenderAssetStreamingManagerIncrementalUpdate is enabled. ")
		TEXT("Only taked into account if value > 1 (GameThread counts as 1 worker). "),
		ECVF_Default);
}
...
bool FRenderAssetStreamingManager::IsParallelForAllowedDuringIncrementalUpdate(int32 NumElementsToProcess, int32& OutParallelForMinBatchSize)
{
#if !WITH_EDITOR
	static int32 MinNumElementsToProcess = 8;
	bool bIsAllowed = UE::Private::RenderAssetStreamingManager::bAllowParallelForDuringIncrementalUpdate && FApp::ShouldUseThreadingForPerformance() && (NumElementsToProcess >= MinNumElementsToProcess);
	OutParallelForMinBatchSize = (bIsAllowed && (UE::Private::RenderAssetStreamingManager::GWorkerCountForParallelForDuringIncrementalUpdate > 0) && (NumElementsToProcess > UE::Private::RenderAssetStreamingManager::GWorkerCountForParallelForDuringIncrementalUpdate)) ? FMath::DivideAndRoundUp(NumElementsToProcess, UE::Private::RenderAssetStreamingManager::GWorkerCountForParallelForDuringIncrementalUpdate) : 1;
	return bIsAllowed;
#else
	// Some functions like UMaterialInterface::FindTextureStreamingDataIndexRange are not thread safe in editor
	OutParallelForMinBatchSize = 1;
	return false;
#endif
}
...
void FRenderAssetStreamingManager::ConditionalUpdateStaticData()
{
		...
		PreviousSettings != Settings)
	{
		...
		if (!FSimpleStreamableAssetManager::IsEnabled() && (bDebugSettingsChanged || PreviousSettings.MaterialQualityLevel != Settings.MaterialQualityLevel))
		{
...
{
	// FSimpleStreamableAssetManager::UnregisterAsset can be called outside critical Section
	FSimpleStreamableAssetManager::UnregisterAsset(RenderAsset);
...
{
	TRACE_CPUPROFILER_EVENT_SCOPE(UpdateStreamingRenderAssets);
	if ( StageIndex == 0 )
	{
		CurrentUpdateStreamingRenderAssetIndex = 0;
...
void FRenderAssetStreamingManager::UpdateResourceStreaming( float DeltaTime, bool bProcessEverything/*=false*/ )
{
	...
	float StreamingPoolStat = ((float)(DisplayedStats.RequiredPool + (GPoolSizeVRAMPercentage > 0 ? 0 : DisplayedStats.NonStreamingMips)));
	CSV_CUSTOM_STAT(TextureStreaming, StreamingPool, StreamingPoolStat / (1024.0f * 1024.0f), ECsvCustomStatOp::Set);
	...
	float DesiredDataLoadedPct = StreamingPoolStat == 0.0f ? 0.0f : 100.0f * (float)DisplayedStats.WantedMips / StreamingPoolStat;
	CSV_CUSTOM_STAT(TextureStreaming, DesiredDataLoadedPercent, DesiredDataLoadedPct, ECsvCustomStatOp::Set);
...
void FRenderAssetStreamingManager::GetObjectReferenceBounds(const UObject* RefObject, TArray<FBox>& AssetBoxes)
{
	...
	if (RenderAsset && !FSimpleStreamableAssetManager::IsEnabled())
	{
		for (FLevelRenderAssetManager *LevelManager : LevelRenderAssetManagers)
		{
			if (LevelManager == nullptr)
	...
	{
		FSimpleStreamableAssetManager::GetAssetReferenceBounds(RenderAsset, AssetBoxes);
	}
}
...
bool FRenderAssetStreamingManager::HandleStreamingManagerMemoryCommand( const TCHAR* Cmd, FOutputDevice& Ar, UWorld* InWorld )
{
	...
	MemSize += FSimpleStreamableAssetManager::GetAllocatedSize();
...
bool FRenderAssetStreamingManager::HandleInvestigateRenderAssetCommand(const TCHAR* Cmd, FOutputDevice& Ar, UWorld* InWorld)
{
	...
	if (InvestigateAssetName.Len())
	{
		...
		FSimpleStreamableAssetManager::FScopedLock ScopedLock(FSimpleStreamableAssetManager::GetCriticalSection(), FSimpleStreamableAssetManager::IsEnabled());
		if (FSimpleStreamableAssetManager::IsEnabled())
		{
		...
		for (int32 AssetIndex = 0; AssetIndex < StreamingRenderAssets.Num(); ++AssetIndex)
		{
			...
			if (AssetName.Contains(InvestigateAssetName))
			{
				...
				if (Texture && !Texture->IsCurrentlyVirtualTextured() && Texture->GetRunningPlatformData() && *Texture->GetRunningPlatformData())
				{
				...
				FString BiasDesc;
				{
					...
					if (!Settings.bUseAllMips)
					{
						...
						const bool bIsVirtualTexture = Texture && Texture->IsCurrentlyVirtualTextured();
						const int32 GroupLODBias = bIsVirtualTexture ? LODGroupInfo.LODBias_VT : LODGroupInfo.LODBias;
						if (GroupLODBias)
						{
							if (FPlatformProperties::RequiresCookedData())
							{
								BiasDesc += FString::Printf(TEXT(" [LODGroup.Bias:0(%d)]"), GroupLODBias);
							}
							else
							{
								BiasDesc += FString::Printf(TEXT(" [LODGroup.Bias:%d]"), GroupLODBias);
								CumuBias += GroupLODBias;
							}
						}
						...
						const int32 MipCountBeforeMaxRes = ResourceState.MaxNumLODs - RenderAsset->NumCinematicMipLevels -
							...
							(FPlatformProperties::RequiresCookedData() ? 0 : (GroupLODBias + (Texture ? Texture->LODBias : 0)));
...
void FRenderAssetStreamingManager::AddReferencedObjects(FReferenceCollector& Collector)
{
	/* [Note] UStreamableRenderAsset is registering/unregistering from manager
	 * during the creation/destruction - providing reference to GC will prevent
	 * Asset destruction -> unregistration but in rare case of background task
	 * FRenderAssetStreamingMipCalcTask::DoWork when in spans for more than one frame
	 * it might try touch Asset that just has been removed so to prevent this
	 * we provide GC with referenced assets only if there is a background task running
	 */
	if(!AsyncWork->IsDone())
	{
		for (TObjectPtr<UStreamableRenderAsset>& ReferencedRenderAsset : ReferencedRenderAssets)
		{
			Collector.AddReferencedObject(ReferencedRenderAsset);
		}
	}
}
FString FRenderAssetStreamingManager::GetReferencerName() const
{
	return FString(TEXT("FRenderAssetStreamingManager"));
}
