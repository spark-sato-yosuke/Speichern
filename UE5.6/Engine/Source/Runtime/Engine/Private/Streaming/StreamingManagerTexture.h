 ...
 */
struct FRenderAssetStreamingManager final : public IRenderAssetStreamingManager, FGCObject
{
...
//BEGIN: Thread-safe functions and data
		...
		*/
		...
		/** 
		 * KeyFunc to ensure lookups in the TSet below can be done with raw pointers.
		 * Under certain circumstances, lookups can be performed on ReferencedRenderAssets with invalid pointers (freed resources).
		 * Constructing a TObjectPtr<UStreamableRenderAsset>() from an invalid pointer is unsupported.
		 */
		struct FPointerKeyFunc : BaseKeyFuncs<UStreamableRenderAsset*, const UStreamableRenderAsset*, false>
		{
			static KeyInitType	GetSetKey(ElementInitType& Element) { return Element; }
			static KeyInitType	GetSetKey(const TObjectPtr<UStreamableRenderAsset>& Element) { return Element.Get(); }			
		...
		/** All the textures/meshes referenced in StreamingRenderAssets. Used to handle deleted textures/meshes. */
		TSet<TObjectPtr<UStreamableRenderAsset>, FPointerKeyFunc> ReferencedRenderAssets;
