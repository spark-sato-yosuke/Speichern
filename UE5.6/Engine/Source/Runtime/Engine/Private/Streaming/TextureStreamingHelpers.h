...
struct FRenderAssetStreamingStats
{
...
#if WITH_EDITOR
	int64 RequiredPool_Editor;	// Estimated memory in bytes used by the Editor
#endif
	...
	int64 VirtualTexturePool;			// Pool size used for virtual texture budget
	int32 NumVirtualTextures;			// Number of virtual textures managed by the streamer
	int64 RequiredVirtualTexturePool;	// Estimated memory in bytes the streamer would use if there was no limit
	int64 UsedVirtualTexturePool;		// Estimated memory in bytes the streamer actually use
};
