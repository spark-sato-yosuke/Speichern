...
TRACE_DECLARE_INT_COUNTER(StreamableAssets, TEXT("StreamableAssets/RegisteredAssets"));
...
void FAsyncRenderAssetStreamingData::UpdatePerfectWantedMips_Async(FStreamingRenderAsset& StreamingRenderAsset, const FRenderAssetStreamingSettings& Settings, bool bOutputToLog) const
{
	...
	else
	{
		...
		if (FSimpleStreamableAssetManager::IsEnabled() && RenderAsset->SimpleStreamableAssetManagerIndex != INDEX_NONE)
		{
			// No need to iterate more if render asset is already at maximum resolution.
			const bool bAlreadyAtMaximumResolution = MaxSize_VisibleOnly >= MaxAssetSize || MaxNumForcedLODs >= StreamingRenderAsset.MaxAllowedMips;
			if (!bAlreadyAtMaximumResolution)
			{
				float TmpMaxSize = MaxSize;
				float TmpMaxVisibleOnly = MaxSize_VisibleOnly;
				int32 TmpMaxNumForcedLODs = MaxNumForcedLODs;
				
				FSimpleStreamableAssetManager::GetRenderAssetScreenSize(AssetType, RenderAsset, TmpMaxSize, TmpMaxVisibleOnly, TmpMaxNumForcedLODs, MaxAssetSize, StreamingRenderAsset.MaxAllowedMips, bOutputToLog ? TEXT("Static") : nullptr);
...
void FRenderAssetStreamingMipCalcTask::UpdateBudgetedMips_Async()
{
	...
	for (FStreamingRenderAsset& StreamingRenderAsset : StreamingRenderAssets)
	{
		...
		if (StreamingRenderAsset.IsVirtualTexture())
		{
			continue;
		}
...
void FRenderAssetStreamingMipCalcTask::UpdateBudgetedVirtualTextureMips_Async()
{
	TArray<FStreamingRenderAsset>& StreamingRenderAssets = StreamingManager.AsyncUnsafeStreamingRenderAssets;
	const FRenderAssetStreamingSettings& Settings = StreamingManager.Settings;
	int64 TargetMemoryBudget = FMath::Max(Settings.VirtualTexturePoolSize * 1024 * 1024, 0);
	const bool bDisableStreaming = TargetMemoryBudget == 0;

	int64 MemoryBudgeted = 0;
	TArray<int32> AssetIndices;

	// Iterate the virtual texture streaming assets, set their current budget, and collect candidates for budget reduction.
	for (int32 AssetIndex = 0; AssetIndex < StreamingRenderAssets.Num() && !IsAborted(); ++AssetIndex)
	{
		FStreamingRenderAsset& StreamingRenderAsset = StreamingRenderAssets[AssetIndex];
		if (StreamingRenderAsset.RenderAsset && StreamingRenderAsset.IsVirtualTexture())
		{
			if (bDisableStreaming)
			{
				StreamingRenderAsset.BudgetedMips = StreamingRenderAsset.MinAllowedMips;
			}
			else
			{
				MemoryBudgeted += StreamingRenderAsset.UpdateRetentionPriority_Async(Settings.bPrioritizeMeshLODRetention);
				// Don't consider the virtual texture root pages as part of our budget.
				MemoryBudgeted -= StreamingRenderAsset.GetSize(StreamingRenderAsset.MinAllowedMips);
	...
	// Try to drop mips when overbudget.
	if (MemoryBudgeted > TargetMemoryBudget && !IsAborted())
	{
		// Sort to prioritize mip retention.
		AssetIndices.Sort(FCompareRenderAssetByRetentionPriority(StreamingRenderAssets));
		// Drop mips to reach target budget.
		TryDropMips(AssetIndices, MemoryBudgeted, TargetMemoryBudget);
	}
}
...
void FRenderAssetStreamingMipCalcTask::DoWork()
{
	...
	{
		TRACE_COUNTER_SET(StreamableAssets, StreamingRenderAssets.Num());		
	...
	UpdateBudgetedVirtualTextureMips_Async();
...
void FRenderAssetStreamingMipCalcTask::UpdateStats_Async()
{
	...
	UDeviceProfile* PreviewDeviceProfile = DeviceProfileManager.GetPreviewDeviceProfile();
	if (PreviewDeviceProfile)
	{
		const FString* PoolSizeString = PreviewDeviceProfile->GetAllExpandedCVars().Find(TEXT("r.Streaming.PoolSize"));
		if (PoolSizeString != nullptr)
		{
			int64 PoolSizePreview = FCString::Atoi(**PoolSizeString);
			MemoryBudget = PoolSizePreview * 1024 * 1024;
		}
	}
#endif
	Stats.StreamingPool = MemoryBudget;
	...
	Stats.VirtualTexturePool = Settings.VirtualTexturePoolSize * 1024 * 1024;
	Stats.NumVirtualTextures = 0;
	Stats.RequiredVirtualTexturePool = 0;
	Stats.UsedVirtualTexturePool = 0;
	...
	for (FStreamingRenderAsset& StreamingRenderAsset : StreamingRenderAssets)
	{
		...
		if (StreamingRenderAsset.IsVirtualTexture())
		{
			++Stats.NumVirtualTextures;
			Stats.RequiredVirtualTexturePool += RequiredSize;
			Stats.UsedVirtualTexturePool += ResidentSize;
			// For virtual textures we don't include the root pages in the budget since they are managed completely 
			// by the virtual texture system. We can't even assume that root pages are always locked because the virtual 
			// texture system usually only locks them when a material using the texture allocates a VT.
			const int64 RootPageSize = StreamingRenderAsset.GetSize(StreamingRenderAsset.MinAllowedMips);
			Stats.RequiredVirtualTexturePool -= RootPageSize;
			Stats.UsedVirtualTexturePool -= RootPageSize;
			continue;
		}

#if WITH_EDITOR
		if (StreamingRenderAsset.IsTexture() && StreamingRenderAsset.RenderAsset->bMarkAsEditorStreamingPool)
		{
			// Remove from the non streaming budget what is actually taken by streaming.
			Stats.NonStreamingMips -= RequiredSize;

			// Editor resources should be forced resident
			Stats.RequiredPool_Editor += RequiredSize;
			continue;
		}
#endif
...
void FRenderAssetStreamingMipCalcTask::UpdateCSVOnlyStats_Async()
{
	...
	for (const FStreamingRenderAsset& StreamingRenderAsset : StreamingRenderAssets)
	{
		if (IsAborted()) break;
		if (!StreamingRenderAsset.RenderAsset) continue;
		if (StreamingRenderAsset.IsVirtualTexture()) continue;
