...
{
	TRACE_CPUPROFILER_EVENT_SCOPE(FDynamicRenderAssetInstanceManager::IncrementalUpdate);
	...
	if (PendingComponents.Num())
	{
		const int32 ToProcessComponentCount = PendingComponents.Num();
		int32 MinBatchSize = 1;
		if (FRenderAssetStreamingManager::IsParallelForAllowedDuringIncrementalUpdate(ToProcessComponentCount, MinBatchSize))
		{
			TArray<FRenderAssetInstanceState::FPreAddComponentIgnoreBoundsPayload> Payloads;
			Payloads.SetNum(ToProcessComponentCount);
			ParallelFor(TEXT("PreAddComponentsIgnoreBounds"), ToProcessComponentCount, MinBatchSize, [&](int32 Index)
			{
				FTaskTagScope Scope(ETaskTag::EParallelGameThread);
				const UPrimitiveComponent* Component = PendingComponents[Index];
				check(Component);
				if (!Component->bAttachedToStreamingManagerAsDynamic && CanManage(Component) && Component->IsRegistered() && (!Component->IsRenderStateCreated() || Component->SceneProxy))
				{
					...
					FRenderAssetInstanceState::PreAddComponentIgnoreBounds(Component, LevelContext, Payloads[Index]);
				}
				else
				{
					Payloads[Index].Result = EAddComponentResult::Fail;
				}
			});
			{
				for (FRenderAssetInstanceState::FPreAddComponentIgnoreBoundsPayload& Payload : Payloads)
				{
					if (State->AddComponentIgnoreBounds(Payload) == EAddComponentResult::Success)
					{
						Payload.Component->bAttachedToStreamingManagerAsDynamic = true;
					}
				}
			}
		}
