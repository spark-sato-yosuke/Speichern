...
{
	TRACE_CPUPROFILER_EVENT_SCOPE(FStreamingTextureLevelContext::UpdateContext);
	TextureGuidToLevelIndex = nullptr;
	bUseRelativeBoxes = false;
	BuildDataTimestamp = 0;
	ComponentBuildData = nullptr;
...
{
	return !bForceNoUseBuiltData && bIsBuiltDataValid && GUseTextureStreamingBuiltData != 0;
}
...
void FStreamingTextureLevelContext::ProcessMaterial(const FBoxSphereBounds& ComponentBounds, const FPrimitiveMaterialInfo& MaterialData, float ComponentScaling, TArray<FStreamingRenderAssetPrimitiveInfo>& OutStreamingTextures, bool bIsComponentBuildDataValid, const UPrimitiveComponent* DebugComponent)
{
	...
	auto ProcessTexture = [this, &ComponentBounds, &MaterialData, ComponentScaling, &OutStreamingTextures](UTexture* Texture, const FTextureSamplingInfo* TextureSamplingInfo = nullptr)
	{
#if WITH_EDITOR
...
#endif
		{
			float TextureDensity = 0.f;
#if !WITH_EDITOR
			if (TextureSamplingInfo)
			{
				TextureDensity = MaterialData.Material->GetTextureDensityWithCache(*TextureSamplingInfo, *MaterialData.UVChannelData);
			}
			else
#endif
			{
				TextureDensity = MaterialData.Material->GetTextureDensity(Texture->GetFName(), *MaterialData.UVChannelData);
			}
	...
	{
		TArray<UTexture*, TInlineAllocator<64>> StreamableTextures;
		...
		for (UTexture* Texture : StreamableTextures)
		{
			if (int32* BuildDataIndex = GetBuildDataIndexRef(Texture))
			{
			...
			{
				// Otherwise create an entry using the available data.
	...
	{
		if (const FMaterialCachedTexturesSamplingInfo* CachedTexturesSamplingInfo = MaterialData.Material->GetCachedTexturesSamplingInfo())
		{
			for (const FTextureSamplingInfo& SamplingInfo : CachedTexturesSamplingInfo->TextureSamplingInfos)
			{
				ProcessTexture(SamplingInfo.Texture, &SamplingInfo);
			}
		}
		else
#endif
