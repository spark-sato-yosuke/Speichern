...
#if WITH_EDITOR
DECLARE_MEMORY_STAT_POOL(TEXT("Editor Required Pool"), STAT_Streaming15_EditorRequiredPool, STATGROUP_Streaming, FPlatformMemory::MCR_StreamingPool);
#endif
...
static TAutoConsoleVariable<int32> CVarStreamingPoolSizeForVirtualTextures(
	TEXT("r.Streaming.PoolSizeForVirtualTextures"),
	0,
	TEXT("The maximum size in MB of the virtual texture pool to use for streaming using distance based streaming.")
	TEXT("0 (default) means that the virtual texture streaming is only GPU feedback driven."),
	ECVF_Scalability | ECVF_ExcludeFromPreview);
...
static TAutoConsoleVariable<int32> CVarStreamingStressTestExtraIOLatency(
	TEXT("r.Streaming.StressTest.ExtraIOLatency"),
	10,
	TEXT("An extra latency in milliseconds for each stream-in request when doing the stress test."),
	ECVF_Cheat);

static TAutoConsoleVariable<int32> CVarStreamingStressTestRayTracingExtraIOLatency(
	TEXT("r.Streaming.StressTest.RayTracing.ExtraIOLatency"),
	0,
	TEXT("An extra latency in milliseconds for each ray tracing stream-in request when doing the stress test."),
	ECVF_Cheat);
...
void FRenderAssetStreamingSettings::Update()
{
	...
	VirtualTexturePoolSize = CVarStreamingPoolSizeForVirtualTextures.GetValueOnAnyThread();
	...
	if (CVarStreamingStressTest.GetValueOnAnyThread() != 0)
	{
		...
		ExtraIOLatencyRayTracing = CVarStreamingStressTestRayTracingExtraIOLatency.GetValueOnAnyThread();
	}
    else
#endif
    {
...
void FRenderAssetStreamingStats::Apply()
{
...
#if WITH_EDITOR
	SET_MEMORY_STAT(STAT_Streaming15_EditorRequiredPool, RequiredPool_Editor);
#endif

	SET_MEMORY_STAT(STAT_Streaming16_DesiredDataLoadedPct, (bHasBudgetLimit && StreamingPool != 0) ? 100.0f * float(WantedMips) / float(StreamingPool) : 0.0f);
	
	SET_MEMORY_STAT(STAT_Streaming17_VirtualTexturePool, VirtualTexturePool);
	SET_MEMORY_STAT(STAT_Streaming18_VirtualTextureRequired, RequiredVirtualTexturePool);
	SET_MEMORY_STAT(STAT_Streaming19_VirtualTextureUsed, UsedVirtualTexturePool);
	...
	INC_DWORD_STAT_BY(STAT_Streaming06_VirtualTextureCount, NumVirtualTextures);
