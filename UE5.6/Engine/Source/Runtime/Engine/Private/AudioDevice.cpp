...
static int32 DisableOwnerNotify = 1;
FAutoConsoleVariableRef CVarDisableOwnerNotify(
	TEXT("au.DisableOwnerNotify"),
	DisableOwnerNotify,
	TEXT("Disables SetOwner notifications.\n")
	TEXT("0: Enabled, 1: Disabled"),
	ECVF_Default);
...
UE_TRACE_EVENT_BEGIN(Audio, SoundStart)
...
UE_TRACE_EVENT_BEGIN(Audio, SoundStop)
	UE_TRACE_EVENT_FIELD(uint32, DeviceId)
	UE_TRACE_EVENT_FIELD(uint64, Timestamp)
	UE_TRACE_EVENT_FIELD(uint32, PlayOrder)
UE_TRACE_EVENT_END()

UE_TRACE_EVENT_BEGIN(Audio, VirtualLoopStopOrRealize)
	UE_TRACE_EVENT_FIELD(uint32, DeviceId)
	UE_TRACE_EVENT_FIELD(uint64, Timestamp)
	UE_TRACE_EVENT_FIELD(uint32, PlayOrder)
UE_TRACE_EVENT_END()
...
#if UE_AUDIO_PROFILERTRACE_ENABLED
		FTraceAuxiliary::OnTraceStarted.AddRaw(this, &FAudioDevice::OnTraceStarted);
#endif
	}
}
...
void FAudioDevice::AddReferencedObjects(FReferenceCollector& Collector)
{
	 ...
	 *
	 * The FAudioThread is suspended during GC to avoid accesing objects during
	 * or after their destruction. But the FAudioThread does NOT suspend during 
	 * AddReferencedObjects(...). Any object that needs to be accessed on the FAudioThread
	 * but needs protection from GC should be adding there references to the reference collector. 
	 * 
	 * In FAudioDevice, there are two primary strategies used to achieve this, where systems
	 * generally choose one strategy depending on their system's needs, though sometimes require
	 * both strategies in more complex scenarios. 
	 *
	 * Strategy 1: Manage the lifetime of the object entirely on the GT, where GT logic can
	 * ensure that the object will be protected from GC any time the object is accessed
	 * on the FAudioThread.  
	 *
	 * Strategy 2: Manage the lifetime of the object on the AT and utilize the AudioThreadReferenceCritSec
	 * to protect against modifying the object while it is being read during `AddReferencedObjects(...)`. 
	 */
	...
	{
		Collector.AddPropertyReferencesWithStructARO(FReverbSettings::StaticStruct(), &ActivatedReverbPair.Value.ReverbSettings);
	}

	if (PendingActivatedReverb)
	{
		Collector.AddPropertyReferencesWithStructARO(FReverbSettings::StaticStruct(), &PendingActivatedReverb->ReverbSettings);
	}
	...
	{
		// Lock to protect against modification of objects which are owned by the AudioThread, but need
		// their references tracked to protect against GC. 
		FScopeLock AudioThreadReferenceLock(&AudioThreadReferenceCritSec);

		Collector.AddPropertyReferencesWithStructARO(FReverbSettings::StaticStruct(), &HighestPriorityActivatedReverb_AudioThread.ReverbSettings);

		for (FAudioVolumeSubmixSendSettings& Settings : CurrentAudioVolumeSettings.SubmixSendSettings)
		{
			Collector.AddPropertyReferencesWithStructARO(FAudioVolumeSubmixSendSettings::StaticStruct(), &Settings);
		}
		for (FAudioVolumeSubmixOverrideSettings& Settings : CurrentAudioVolumeSettings.SubmixOverrideSettings)
		{
			Collector.AddPropertyReferencesWithStructARO(FAudioVolumeSubmixOverrideSettings::StaticStruct(), &Settings);
		}
	}
}
...
bool FAudioDevice::HandleAudioMemoryInfo(const TCHAR* Cmd, FOutputDevice& Ar)
{
	...
	if (SoundWaveClass != nullptr)
	{
		...
		for (TObjectIterator<USoundWave> It; It; ++It)
		{
			...
			// Calculate streaming cache size usage. 
			uint32 MaxSizeInCache = 0;
			uint32 MaxUnevictableSize = 0;
			SoundWave->GetChunkSizeStats(MaxSizeInCache, MaxUnevictableSize);
...
#if UE_AUDIO_PROFILERTRACE_ENABLED
void FAudioDevice::OnTraceStarted(FTraceAuxiliary::EConnectionType TraceType, const FString& TraceDestination)
{
	for (const FActiveSound* ActiveSound : ActiveSounds)
	{
		...
		SendSoundStartedTraceMessage(*ActiveSound);
	}

	for (auto& [ActiveSound, VirtualLoop] : VirtualLoops)
	{
		VirtualLoop.OnTraceStarted();
	}
}
#endif // UE_AUDIO_PROFILERTRACE_ENABLED
...
void FAudioDevice::UpdateHighestPriorityReverb()
{
	...
	if (ActivatedReverbs.Num() > 0)
	{
		...
		// Use shared ptr to ensure that pending reverb is protected from GC before it gets applied on the audio thread.
		PendingActivatedReverb = MakeShared<FActivatedReverb>(ActivatedReverbs.CreateConstIterator().Value());
		FAudioThread::RunCommandOnAudioThread([AudioDevice, NewActivatedReverbWeakPtr = TWeakPtr<FActivatedReverb>(PendingActivatedReverb)]()
		{
			if (TSharedPtr<FActivatedReverb> NewActivatedReverb = NewActivatedReverbWeakPtr.Pin())
			{
				FScopeLock AudioThreadReferenceLock(&AudioDevice->AudioThreadReferenceCritSec);
				AudioDevice->bHasActivatedReverb = true;
				AudioDevice->HighestPriorityActivatedReverb_AudioThread = *NewActivatedReverb;
			}
		}, GET_STATID(STAT_AudioUpdateHighestPriorityReverb));
	}
	else
	{
		PendingActivatedReverb.Reset();
		FAudioThread::RunCommandOnAudioThread([AudioDevice]()
		{
			FScopeLock AudioThreadReferenceLock(&AudioDevice->AudioThreadReferenceCritSec);
			AudioDevice->bHasActivatedReverb = false;
			AudioDevice->HighestPriorityActivatedReverb_AudioThread = FActivatedReverb();
		}, GET_STATID(STAT_AudioUpdateHighestPriorityReverb));
	}
}
...
void FAudioDevice::NotifySubsystemsSetOwner(FActiveSound& ActiveSound, const UObject* Owner) const
{
	SubsystemCollection.ForEachSubsystem<IActiveSoundUpdateInterface>([&ActiveSound, Owner](IActiveSoundUpdateInterface* ActiveSoundUpdate)
	{
		ActiveSoundUpdate->NotifyActiveSoundOwner(ActiveSound, Owner);
		return true;
	});
}
...
void FAudioDevice::UpdateAudioVolumeEffects()
{
	...
	{
		// Lock for modifications to CurrentAudioVolumeSettings and contention with AddReferencedObjects(...)
		FScopeLock AudioThreadReferenceLock(&AudioThreadReferenceCritSec);
		...
		if (Effects)
		{
			...
			if (bHasActivatedReverb && (HighestPriorityActivatedReverb_AudioThread.Priority > PlayerAudioVolumeSettings.Priority || bUsingDefaultReverb))
			{
				CurrentAudioVolumeSettings.ReverbSettings = HighestPriorityActivatedReverb_AudioThread.ReverbSettings;
			}
...
#if UE_AUDIO_PROFILERTRACE_ENABLED
void FAudioDevice::SendSoundStartedTraceMessage(const FActiveSound& InActiveSound) const
{
	const bool bChannelEnabled = UE_TRACE_CHANNELEXPR_IS_ENABLED(AudioMixerChannel);
	TObjectPtr<USoundBase> SoundBasePtr = InActiveSound.GetSound();

	if (bChannelEnabled && SoundBasePtr)
	{
		TObjectPtr<UObject> SoundObjectPtr = FSoftObjectPath(InActiveSound.GetSound()->GetPathName()).ResolveObject();
		if (SoundObjectPtr == nullptr)
		{
			return;
		}

		const FString SoundClassName = SoundObjectPtr->GetClass()->GetName();

		const TObjectPtr<UClass> SoundSuperClass = SoundObjectPtr->GetClass()->GetSuperClass();
		const FString SuperClassName = SoundSuperClass ? SoundSuperClass->GetName() : FString();

		UE_TRACE_LOG(Audio, SoundStart, AudioMixerChannel)
			<< SoundStart.DeviceId(InActiveSound.AudioDevice->DeviceID)
			<< SoundStart.Timestamp(FPlatformTime::Cycles64())
			<< SoundStart.PlayOrder(InActiveSound.PlayOrder)
			<< SoundStart.Name(*SoundObjectPtr->GetPathName())
			<< SoundStart.SoundClassName(*SoundClassName)
			<< SoundStart.SoundSuperClassName(*SuperClassName);
	}
}
#endif // UE_AUDIO_PROFILERTRACE_ENABLED
...
void FAudioDevice::AddNewActiveSoundInternal(const FActiveSound& InNewActiveSound, TArray<FAudioParameter>&& InDefaultParams, FAudioVirtualLoop* InVirtualLoopToRetrigger)
{
	...
	if (InVirtualLoopToRetrigger)
	{
		...
		// Attempt to seek the ActiveSound based on virtualization time
		if (ActiveSound->GetVirtualizationMode() == EVirtualizationMode::SeekRestart)
		{
			ActiveSound->RequestedStartTime = ActiveSound->PlaybackTime;
		}
	}
...
void FAudioDevice::InitSoundParams(FActiveSound& InOutActiveSound, TArray<FAudioParameter> InDefaultParams, bool bInIsVirtualLoopRealizing) const
{
	...
	{
		// The default parameters should be empty when realizing a virtualized loop. Prior
		// parameters are already set on the instance transmitter. 
		InOutActiveSound.InstanceTransmitter->OnRealizeVirtualizedActiveSound(MoveTemp(InDefaultParams));
	}
	else
	{
...
void FAudioDevice::RemoveEnvelopeFollowerDelegate(USoundSubmix* InSubmix, const FOnSubmixEnvelopeBP& OnSubmixEnvelopeBP)
{
	UE_LOG(LogAudio, Error, TEXT("Envelope following submixes only works with the audio mixer. Please run using -audiomixer or set INI file to use submix recording."));
}
...
bool FAudioDevice::RemoveVirtualLoop(FActiveSound& InActiveSound)
{
	...
	if (FAudioVirtualLoop* VirtualLoop = VirtualLoops.Find(&InActiveSound))
	{
		...
		if (bChannelEnabled)
		{
			UE_TRACE_LOG(Audio, VirtualLoopStopOrRealize, AudioChannel)
				<< VirtualLoopStopOrRealize.DeviceId(static_cast<uint32>(DeviceID))
				<< VirtualLoopStopOrRealize.Timestamp(FPlatformTime::Cycles64())
				<< VirtualLoopStopOrRealize.PlayOrder(InActiveSound.GetPlayOrder());
		}
#endif // UE_AUDIO_PROFILERTRACE_ENABLED
...
void FAudioDevice::RemoveActiveSound(FActiveSound* ActiveSound)
{
	...
	if (bChannelEnabled)
	{
		UE_TRACE_LOG(Audio, SoundStop, AudioMixerChannel)
			<< SoundStop.DeviceId(ActiveSound->AudioDevice->DeviceID)
			<< SoundStop.Timestamp(FPlatformTime::Cycles64())
			<< SoundStop.PlayOrder(ActiveSound->PlayOrder);
	}
#endif // UE_AUDIO_PROFILERTRACE_ENABLED
...
void FAudioDevice::PlaySoundAtLocation(USoundBase* Sound, UWorld* World, float VolumeMultiplier, float PitchMultiplier, float StartTime, const FVector& Location, const FRotator& Rotation, USoundAttenuation* AttenuationSettings, USoundConcurrency* Concurrency, const TArray<FAudioParameter>* Params, const AActor* OwningActor)
{
	...
	if (Sound->IsLooping() || Sound->IsPlayWhenSilent() || SoundIsAudible(Sound, World, Location, AttenuationSettingsToApply, MaxDistance, FocusFactor))
	{
		...
		if (!DisableOwnerNotify)
		{
			NotifySubsystemsSetOwner(NewActiveSound, OwningActor);
		}
...
void FAudioDevice::Precache(USoundWave* SoundWave, bool bSynchronous, bool bTrackMemory, bool bForceFullDecompression)
{
	...
	else
	{
		...
		else
		{
			...
			bDecompressToPCM = true;

			// if the decompression type is already DTYPE_RealTime from a previous call
			// the task will early exit and we won't decompress
		...
		if (SoundWave->AudioDecompressor == nullptr && (bDecompressToPCM || SoundWave->DecompressionType == DTYPE_RealTime))
		{
