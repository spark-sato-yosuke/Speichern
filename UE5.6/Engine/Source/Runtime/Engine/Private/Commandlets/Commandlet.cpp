...
void CommandletHelpers::TickEngine(UWorld* InWorld, double InDeltaTime)
{
	...
	if (InWorld && IsAllowCommandletRendering())
	{
		if (FSceneInterface* Scene = InWorld->Scene)
		{
			...
			// Submit Compute Framework work (ComputeTaskExecutionGroup::EndOfFrameUpdate)
			const ERHIFeatureLevel::Type FeatureLevel = Scene->GetFeatureLevel();
			TArray<IComputeTaskWorker*> ComputeWorkers;
			Scene->GetComputeTaskWorkers(ComputeWorkers);

			ENQUEUE_RENDER_COMMAND(ComputeTaskWorker_EndOfFrameUpdate)([FeatureLevel, ComputeWorkers](FRHICommandListImmediate& RHICmdList)
			{				
				FRDGBuilder GraphBuilder(RHICmdList, FRDGEventName(TEXT("ComputeTaskWorker_EndOfFrameUpdate")));
