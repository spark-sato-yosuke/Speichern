...
void FTextureRenderTarget2DResource::InitRHI(FRHICommandListBase& RHICmdList)
{
	...
	if( TargetSizeX > 0 && TargetSizeY > 0 )
	{
		...
		FString RHIResourceName = Owner->GetName();
		...
		FRHITextureCreateDesc Desc =
			FRHITextureCreateDesc::Create2D(*RHIResourceName)
		...
		{
			UnorderedAccessViewRHI = RHICmdList.CreateUnorderedAccessView(RenderTargetTextureRHI, FRHIViewDesc::CreateTextureUAV().SetDimensionFromTexture(RenderTargetTextureRHI));
		}
