...
FAssetCompilingManager::FAssetCompilingManager()
{
	...
	RegisterManager(&FAnimBankCompilingManager::Get());
...
void FAssetCompilingManager::MarkCompilationAsCanceled(TArrayView<UObject* const> InObjects)
{
	...
	if (InObjects.Num())
	{
		TRACE_CPUPROFILER_EVENT_SCOPE(FAssetCompilingManager::MarkCompilationAsCanceled);

		for (IAssetCompilingManager* AssetCompilingManager : AssetCompilingManagers)
		{
			AssetCompilingManager->MarkCompilationAsCanceled(InObjects);
		}
	}
#endif
}
