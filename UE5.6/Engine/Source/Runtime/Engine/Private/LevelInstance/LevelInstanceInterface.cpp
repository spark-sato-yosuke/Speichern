...
bool ILevelInstanceInterface::IsLevelInstanceHLODRelevant() const
{
	if (const ULevelInstanceComponent* LevelInstanceComponent = GetLevelInstanceComponent())
	{
		if (const AActor* Owner = LevelInstanceComponent->GetOwner())
		{
			return Owner->IsHLODRelevant();
		}
	}
	return false;
}
