...
APlayerController* FReplayHelper::CreateSpectatorController(UNetConnection* Connection)
{
	...
	if (TActorIterator<APlayerStart> It(World.Get()); It)
	{
...

		{
			...
			for (const TSharedPtr<FNetworkObjectInfo>& NetworkObjectInfo : AllObjectsSet)
			{
				if (NetworkObjectInfo.IsValid())
				{
					...
					{
						const TWeakObjectPtr<AActor>& WeakActor = NetworkObjectInfo->WeakActor;

						// If the actor isn't valid we can't safely access its properties nor serialize it.
						...
						bool bCheckpointActor = ((Actor->GetRemoteRole() != ROLE_None || Actor->GetTearOff()) && (Actor == Connection->PlayerController || Cast<APlayerController>(Actor) == nullptr));
						
						// now look for an open channel
						bCheckpointActor = bCheckpointActor && ActorChannelMap.Contains(WeakActor);

						if (!bCheckpointActor)
						{
							// has it gone dormant?
							bCheckpointActor = (Actor->NetDormancy != DORM_Initial) &&  (NetworkObjectInfo->DormantConnections.Contains(Connection) || NetworkObjectInfo->RecentlyDormantConnections.Contains(Connection));
						}
						...
						if (bCheckpointActor)
						{
							...
							checkf(Actor->GetOuter() == ActorLevel, TEXT("Outer %p %s != Level %p %s. Actor %s"), Actor->GetOuter(), ToCStr(GetNameSafe(Actor->GetOuter())), ActorLevel, ToCStr(GetNameSafe(ActorLevel)), ToCStr(GetNameSafe(Actor)));
#endif

							ActorArray.Add({ WeakActor, Actor->GetOuter() });
							NetworkObjectInfo->bDirtyForReplay = false;
						}
					}
				}
			...
			if (FExternalDataWrapper* ExternalData = ExternalDataMap.Find(Object))
			{
				if (ExternalData->NumBits > 0)
				{
					...
					if (!NetworkGUID.IsValid())
					{
						...
						NetworkGUID = Connection->Driver->GetNetGuidCache()->NetGUIDLookup.FindRef(Object);
					}
...
void FReplayHelper::CacheNetGuids(UNetConnection* Connection)
{
	if (Connection && Connection->Driver)
	{
		...
		for (auto It = Connection->Driver->GetNetGuidCache()->ObjectLookup.CreateIterator(); It; ++It)
		{
...
bool FReplayHelper::SetExternalDataForObject(UNetConnection* Connection, UObject* OwningObject, const uint8* Src, const int32 NumBits)
{
	...
	FNetworkGUID NetworkGUID = Connection->Driver->GetNetGuidCache()->NetGUIDLookup.FindRef(OwningObject);
...
void FReplayHelper::NotifyActorDestroyed(UNetConnection* Connection, AActor* Actor)
{
	...
	else
	{
		if (bDeltaCheckpoint)
		{
			FNetworkGUID NetGUID = Connection->Driver->GetNetGuidCache()->NetGUIDLookup.FindRef(Actor);
			if (NetGUID.IsValid())
			{
...
void FReplayHelper::ResetDeltaCheckpointTracking(UNetConnection* Connection)
{
	...
	if (Connection)
	{
		if (UNetDriver* Driver = Connection->GetDriver())
		{
			...
			// reset guid cache
			if (FNetGUIDCache* GuidCache = Connection->Driver->GetNetGuidCache().Get())
			{
