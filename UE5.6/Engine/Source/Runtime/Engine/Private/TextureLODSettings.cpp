...
// GlobalTextureLODBias similar to UI Texture LODBias but affects all textures types
// Intended for marketing/cinematics/screencaps, not normal cook usage
//	(eg. set to "-15" to get all possible mip levels in built textures)
// !Note that changing this in an interactive editor may not affect already loaded textures!
//	it is best to set this at startup, via command line or ini config
// (?? could use ECVF_ReadOnly)
int32 GlobalTextureLODBias = 0;
FAutoConsoleVariableRef CVarGlobalTextureLODBias(
	TEXT("r.GlobalTextureLODBias"),
	GlobalTextureLODBias,
	TEXT("Extra LOD bias to apply to all textures.  Can be negative. (default=0)"),
	ECVF_Scalability
);
...
{	
	...
	if (!FPlatformProperties::RequiresCookedData())
	{
		...
		UsedLODBias	+= LODBias;
		UsedLODBias += bVirtualTexture ? LODGroupInfo.LODBias_VT : LODGroupInfo.LODBias;

		// GlobalTextureLODBias from cvar r.GlobalTextureLODBias
		//	only applied on non-cooked platforms (will apply when cooking)
		UsedLODBias += GlobalTextureLODBias;
	}
	...
	if (LODGroup == TEXTUREGROUP_UI)
	{
		...
		// -> this looks broken, probably should be inside the !RequiresCookedData
	}
...
#if ALLOW_OTHER_PLATFORM_CONFIG
void UTextureLODSettings::SetPreviewTextureGroupRedirect(UTextureLODSettings* InPreviewTextureGroupRedirect)
{
	if (PreviewTextureGroupRedirect != InPreviewTextureGroupRedirect)
	{
		PreviewTextureGroupRedirect = InPreviewTextureGroupRedirect;

		for (FThreadSafeObjectIterator ObjIterator(UObject::StaticClass(), RF_NoFlags); ObjIterator; ++ObjIterator)
		{
			//Call UpdateResource on Textures when changing the TextureLodGroup Settings to update the CachePlatformData
			UTexture* Texture = Cast<UTexture>(*ObjIterator);
			if (Texture != nullptr)
			{
