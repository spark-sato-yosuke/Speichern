	...
	struct FDownloadContext
	{
		...
		void Start(const TSharedRef<FStreamableHandle>& OwningHandle)
		{
			...
			InstallRequest = IoStore->Install(MoveTemp(InstallArgs),
				[WeakOwningHandle = OwningHandle.ToWeakPtr()](const UE::IoStore::FOnDemandInstallResult& OnDemandInstallResult)
				{
					TSharedPtr<FStreamableHandle> OwningHandle = WeakOwningHandle.Pin();
					if (OwningHandle && OwningHandle->DownloadContext)
					{
						OwningHandle->DownloadContext->OnInstallComplete(OwningHandle.ToSharedRef(), OnDemandInstallResult);
					}
				},
		...
		void OnInstallComplete(const TSharedRef<FStreamableHandle>& OwningHandle, const UE::IoStore::FOnDemandInstallResult& OnDemandInstallResult)
		{
			// Destroy self at end of scope
			TUniquePtr<FDownloadContext> Self = MoveTemp(OwningHandle->DownloadContext);
			OwningHandle->DownloadContext = nullptr;

			if (!OnDemandInstallResult.Status.IsOk())
			{
				if (OwningHandle->WasCanceled() && OnDemandInstallResult.Status.GetErrorCode() == EIoErrorCode::Cancelled)
				{
					CompleteLoad(OwningHandle);
				}
				else
				{
					CompleteLoad(OwningHandle, UE::UnifiedError::IoStore::ConvertError(OnDemandInstallResult.Status));
				}
...
void FStreamableHandle::CompleteLoad()
{
	...
	if (Error.IsSet())
	{
		UE_LOG(LogStreamableManager, Verbose, TEXT("FStreamableHandle::CompleteLoad failed load %s, with error %s!"), *DebugName, *Error->GetErrorMessage().ToString());
	}
	if (IsActive())
	{
		bLoadCompleted = true;
...
void FStreamableHandle::AsyncLoadCallbackWrapper(const FName& PackageName, UPackage* Package, EAsyncLoadingResult::Type Result, FSoftObjectPath TargetName)
{
	...
	if (OwningManager)
	{
		...
		if (Result != EAsyncLoadingResult::Succeeded)
		{
			...
			/*UE::UnifiedError::StreamableManager::FStreamableManagerAdditionalContext MoreContext = {TargetName.ToString()};
			MaybeError->PushErrorStruct(MoveTemp(MoreContext));*/
			SetError(*MaybeError);
		}
...
void FStreamableManager::FindInMemory(FSoftObjectPath& InOutTargetName, struct FStreamable* Existing, UPackage* Package)
{
	...
	{
		Asset = StaticFindObject(UObject::StaticClass(), InOutTargetName.GetAssetPath(), /*ExactClass*/false);
	}
