...
{	
	...
	UE_LOG( LogActor, Warning, TEXT("--------------------------------------------------") );
	UE_LOG( LogActor, Warning, TEXT("Actor [%p] (%s)"), this, *GetFName().ToString() );
...
void AActor::DebugShowOneComponentHierarchy( USceneComponent* SceneComp, int32& NestLevel, bool bShowPosition )
{
	...
	if( OwnerActor )
	{
		UE_LOG(LogActor, Warning, TEXT("%sSceneComp [%p] (%s) Owned by %s %s"), *Nest, SceneComp, *SceneComp->GetFName().ToString(), *OwnerActor->GetFName().ToString(), *PosString );
	}
	else
	{
		UE_LOG(LogActor, Warning, TEXT("%sSceneComp [%p] (%s) No Owner"), *Nest, SceneComp, *SceneComp->GetFName().ToString() );
	}
	if( SceneComp->GetAttachParent())
	{
		...
		UE_LOG(LogActor, Warning, TEXT("%sAttachParent [%p] (%s) %s"), *Nest, SceneComp->GetAttachParent(), *SceneComp->GetAttachParent()->GetFName().ToString(), *PosString );
	}
	else
	{
...
void AActor::SetIsTemporarilyHiddenInEditor( bool bIsHidden )
{
	if( bHiddenEdTemporary != bIsHidden )
	{
		...
		// When updating the visibility we need to add the sync tag to the row in TEDS to ensure the visibility is synced in TEDS properly
		using namespace UE::Editor::DataStorage;
		ICoreProvider* Storage = GetMutableDataStorageFeature<ICoreProvider>(StorageFeatureName);
		ICompatibilityProvider* StorageCompatibility = GetMutableDataStorageFeature<ICompatibilityProvider>(CompatibilityFeatureName);

		if (Storage && StorageCompatibility)
		{
			RowHandle ActorRow = StorageCompatibility->FindRowWithCompatibleObject(this);

			if (Storage->IsRowAvailable(ActorRow))
			{
				Storage->AddColumn<FTypedElementSyncFromWorldTag>(ActorRow);
			}
		}
	}
}
...
UPackage* AActor::GetSceneOutlinerPackage() const
{
	return GetSceneOutlinerItemPackage();
}
PRAGMA_ENABLE_DEPRECATION_WARNINGS

UPackage* AActor::GetSceneOutlinerItemPackage() const
{
	return GetExternalPackage();
}

UPackage* AActor::GetSceneOutlinerTopParentPackage() const
{
	return GetPackage();
}
...
void AActor::SetActorLabel(const FString& NewActorLabelDirty, bool bMarkDirty)
{
	...
	// Only update the label if it has actually changed. We pass 'false' to GetActorLabel to prevent it
	//    creating a redundant name and broadcasting PostEditChangeProperty and OnActorLabelChanged unnecessarily
...
void AActor::SetInitialActorLabel(FStringView InitialLabel)
{
	if (!ensureMsgf(bHasFinishedSpawning == false, TEXT("It is invalid to call AActor::SetInitialActorLabel after it has finished spawning.")))
	{
		return;
	}

	FText OutErrorMessage;
	if (!FActorEditorUtils::ValidateActorLabel(InitialLabel, OutErrorMessage))
	{
		UE_LOG(LogActor, Warning, TEXT("SetActorLabel failed: %s"), *OutErrorMessage.ToString());
	}
	else
	{
		ActorLabel = FString(InitialLabel);
	}
}
