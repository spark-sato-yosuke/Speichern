...
{
	static const FLazyName VolumeParameterName("Volume");
	static const FLazyName PitchParameterName("Pitch");
	static const FLazyName HighpassParameterName("HPFCutoffFrequency");
	static const FLazyName LowpassParameterName("LPFCutoffFrequency");
}
...
{
	FSoundModulationDestinationSettings Settings;
	float OutValue;
	CalculateInheritedModulationSettingsAndValue(InDestination, Settings, OutValue);
...
Audio::FModulationParameter FWaveInstance::GetParameterBasedOnDestination(const EModulationDestination InDestination)
{
	switch (InDestination)
	{
	case EModulationDestination::Volume:
		if (const Audio::FModulationParameter* Parameter = Audio::GetModulationParameterPtr(AudioModulationPrivate::VolumeParameterName))
		{
			return *Parameter;
		}
		break;
	case EModulationDestination::Pitch:
		if (const Audio::FModulationParameter* Parameter = Audio::GetModulationParameterPtr(AudioModulationPrivate::PitchParameterName))
		{
			return *Parameter;
		}
		break;
	case EModulationDestination::Highpass:
		if (const Audio::FModulationParameter* Parameter = Audio::GetModulationParameterPtr(AudioModulationPrivate::HighpassParameterName))
		{
			return *Parameter;
		}
		break;
	case EModulationDestination::Lowpass:
		if (const Audio::FModulationParameter* Parameter = Audio::GetModulationParameterPtr(AudioModulationPrivate::LowpassParameterName))
		{
			return *Parameter;
		}
		break;
	...
	return Audio::GetDefaultModulationParameter();
}

void FWaveInstance::CalculateInheritedModulationSettingsAndValue(const EModulationDestination InDestination, FSoundModulationDestinationSettings& InOutSettings, float& OutValue) const
{
	const Audio::FModulationParameter& InParam = GetParameterBasedOnDestination(InDestination);
	
	const FActiveSound& InActiveSound = *ActiveSound;
	const FSoundModulationDestinationSettings& InWaveSettings = WaveData->ModulationSettings.GetSettingsForDestination(InDestination);
	EModulationRouting InWaveRouting = WaveData->ModulationSettings.GetRoutingForDestination(InDestination);

	const USoundClass* EffectiveSoundClass = InActiveSound.GetSoundClass();
	if (SoundClass)
	{
		EffectiveSoundClass = SoundClass;
	}
	
	auto MixInRoutedValue = [&InParam](float& InOutValueA, float InValueB)
	{
	...
	auto UnionSoundClassSettings = [&](FSoundModulationDestinationSettings& Settings)
		{
			if (EffectiveSoundClass)
			{
				const FSoundModulationDestinationSettings& ClassSettings = EffectiveSoundClass->Properties.ModulationSettings.GetSettingsForDestination(InDestination);
				OutValue = Settings.Value;
				MixInRoutedValue(OutValue, ClassSettings.Value);
				Settings.Modulators = Settings.Modulators.Union(ClassSettings.Modulators);
			}
		};

	switch (InActiveSound.ModulationRouting.GetRoutingForDestination(InDestination))
	{
	case EModulationRouting::Union:
	{
		FSoundModulationDestinationSettings UnionSettings = InActiveSound.ModulationRouting.GetSettingsForDestination(InDestination);
		switch (InWaveRouting)
		{
		case EModulationRouting::Union:
		{
			OutValue = UnionSettings.Value;
			MixInRoutedValue(OutValue, InWaveSettings.Value);
	...
	case EModulationRouting::Inherit:
	{
		...
		case EModulationRouting::Inherit:
		{
			if (EffectiveSoundClass)
			{
				FSoundModulationDestinationSettings InheritSettings = EffectiveSoundClass->Properties.ModulationSettings.GetSettingsForDestination(InDestination);
				InOutSettings = InheritSettings;
				OutValue = InheritSettings.Value;
			}
		}
		break;
		...
		case EModulationRouting::Override:
		{
			InOutSettings = InWaveSettings;
			OutValue = InWaveSettings.Value;
		}
	...
	case EModulationRouting::Override:
	{
		FSoundModulationDestinationSettings OverrideSettings = InActiveSound.ModulationRouting.GetSettingsForDestination(InDestination);
		InOutSettings = OverrideSettings;
		OutValue = OverrideSettings.Value;	
	}
	break;
...
void FWaveInstance::AddReferencedObjects( FReferenceCollector& Collector )
{
	...
	auto AddBusSendRef = [&Collector](FSoundSourceBusSendInfo& Info)
		{
			if (Info.SoundSourceBus)
			{
				Collector.AddReferencedObject(Info.SoundSourceBus);
			}
			if (Info.AudioBus)
			{
				Collector.AddReferencedObject(Info.AudioBus);
			}
		};

	for (int32 BusSendCount = 0; BusSendCount < (int32)EBusSendType::Count; ++BusSendCount)
	{
		Algo::ForEach(BusSends[BusSendCount], AddBusSendRef);
	}
}
...
{
	return UE_mmioFOURCC(ChunkID[0], ChunkID[1], ChunkID[2], ChunkID[3]);
}
...
bool FWaveModInfo::ReadWaveInfo( const uint8* WaveData, int32 WaveDataSize, FString* ErrorReason, bool InHeaderDataOnly, void** OutFormatHeader)
{
	...
	if (!InHeaderDataOnly && IsFormatSupported())
	{
		...
		{
			UE_LOG(LogAudio, Warning, TEXT("Wave data chunk exceeds end of wave file by %" SIZE_T_FMT " bytes, truncating"), (SampleDataEnd - WaveDataEnd));
