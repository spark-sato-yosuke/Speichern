...
namespace Audio
{
	...
	bool FSoundWavePCMWriter::BeginWriteToSoundWave(const FString& FileName, const TSampleBuffer<>& InSampleBuffer, FString InPath, TFunction<void(const USoundWave*)> OnSuccess, TObjectPtr<USoundWave> InSoundWaveTransformationTarget)
	{
		if (!IsDone())
		{
			UE_LOG(LogAudio, Error, TEXT("This instance of FSoundWavePCMWriter is already processing another write operation."));
		...
		CurrentSoundWave = NewObject<USoundWave>(CurrentPackage, *FileName, RF_Public | RF_Standalone | RF_Transactional, InSoundWaveTransformationTarget);
		CurrentSoundWave->AddToRoot();

#if WITH_EDITORONLY_DATA
		// Clears transformations in the exported soundwave as they are now baked into the sound wave cue points
		CurrentSoundWave->Transformations.Empty();
#endif

#if WITH_EDITOR
		if (InSoundWaveTransformationTarget != nullptr)
		{
			InSoundWaveTransformationTarget->OverwriteSoundWaveDataWithTransformationData();
			CurrentSoundWave->SetSoundWaveCuePoints(InSoundWaveTransformationTarget->GetSoundWaveCuePoints());
		}
#endif //WITH_EDITOR
		...
		CurrentOperation.Reset(new FAsyncSoundWavePCMWriterTask(this, ESoundWavePCMWriteTaskType::GenerateAndWriteSoundWave, [this, OnSuccess](const USoundWave* InSoundWave)
		{
			// Invalidate compressed data on new soundwave so that it gets refreshed or the soundwave is not seekable
			// and causes this error: FMixerBuffer::CreateStreamingBuffer failed to StreamCompressedInfo on SoundWave
			CurrentSoundWave->InvalidateCompressedData(true); // calls CurrentSoundWave->FreeResources(), note the game thread is the audio thread in the editor

			OnSuccess(InSoundWave);
		}));
		CurrentOperation->StartBackgroundTask();
	...
	void FSoundWavePCMWriter::Reset()
	{
		...
		if (CurrentSoundWave && CurrentSoundWave->IsValidLowLevel() && !bWasPreviouslyAddedToRoot)
		{
