...
URuntimeVirtualTexture::URuntimeVirtualTexture(const FObjectInitializer& ObjectInitializer)
	...
	, CustomMaterialData(FVector4f::Zero())
{
...
EVTProducerPriority URuntimeVirtualTexture::GetPriority() const
{
	return bUseCustomPriority ? CustomPriority : GetDefaultPriority();
}

EVTProducerPriority URuntimeVirtualTexture::GetDefaultPriority() const
{
	switch (MaterialType)
	{
	case ERuntimeVirtualTextureMaterialType::BaseColor:
	...
	case ERuntimeVirtualTextureMaterialType::BaseColor_Normal_Specular_YCoCg:
	case ERuntimeVirtualTextureMaterialType::BaseColor_Normal_Specular_Mask_YCoCg:
		return EVTProducerPriority::High;
	case ERuntimeVirtualTextureMaterialType::Mask4:
		return EVTProducerPriority::BelowNormal;
	case ERuntimeVirtualTextureMaterialType::WorldHeight:
		return EVTProducerPriority::Low;
	case ERuntimeVirtualTextureMaterialType::Displacement:
		return EVTProducerPriority::Lower;
	default:
		checkNoEntry();
	}
		...
		if(InStreamingTexture != nullptr)
		{
			...
			if(ensure(StreamingTextureData != nullptr))
			{
				...
				if(ensure(VTData != nullptr))
				{
					...
					{
						OutStreamingProducerDesc.LayerFallbackColor[LayerIndex] = VTData->LayerFallbackColors[LayerIndex];
					}
