...
static TAutoConsoleVariable<bool> CVarApplyViewOcclusionCullToShadow(
	TEXT("foliage.ApplyViewOcclusionCullToShadow"),
	true,
	TEXT("Re-use main view occlusion culling results to speed up the shadow view. Shadows for fully occluded objects may be missing."));
...
void FHierarchicalStaticMeshSceneProxy::GetDynamicMeshElements(const TArray<const FSceneView*>& Views, const FSceneViewFamily& ViewFamily, uint32 VisibilityMap, FMeshElementCollector& Collector) const
{
	...
	const bool bApplyViewOcclusionCullToShadow = CVarApplyViewOcclusionCullToShadow.GetValueOnAnyThread();
		...
		if (VisibilityMap & (1 << ViewIndex))
		{
			...
			if (ClusterTree.Num())
			{
				...
				else
				{
					...
					if (View->bIsSinglePassStereo && IStereoRendering::IsStereoEyeView(*View) && GEngine->StereoRenderingDevice.IsValid())
					{
						// TODO: Stereo culling frustum needs to use the culling origin instead of the view origin.
						InstanceParams.ViewFrustumLocal = View->GetCullingFrustum();
				...
				FBoxSphereBounds ScaledBounds = RenderData->Bounds.TransformBy(FTransform(FQuat::Identity, FVector::ZeroVector, AverageScale));
				...
				// Culling objects from the shadow view using main view occlusion culling results is technically wrong, but can give good perf improvements
				// For VSM, it's likely better to turn this off as non-visible pages aren't rendered anyway and we care more about quality
				bool bApplyOcclusionCull = !ElementParams.ShadowFrustum || bApplyViewOcclusionCullToShadow;
				if (FirstOcclusionNode >= 0 && LastOcclusionNode >= 0 && FirstOcclusionNode <= LastOcclusionNode && bApplyOcclusionCull)
				{
...
bool FHierarchicalStaticMeshSceneProxy::GetInstanceDrawDistanceMinMax(FVector2f& OutDistanceMinMax) const
{
	// HISM already does distance culling before submitting instances, so there is no need to do distance culling on GPU as well.
	// Also GPU distance culling breaks dithered fade out for HISM. This is because we apply fade out to instances only when they go 
	// beyond the culling distance, and at that point they will be GPU culled!
	// We could apply a tweak to the draw distance returned here to account for this. But there isn't really any point
	// because we know that the correct distance culling is already applied on the CPU side. So any tweaking of the value would be
	// to ensure that we don't cull. So we might as well just not do the work on the GPU, which is what happens when we return false here.
	OutDistanceMinMax = FVector2f(0, 0);
	return false;
}
...
FBoxSphereBounds UHierarchicalInstancedStaticMeshComponent::CalcBounds(const FTransform& BoundTransform) const
{
	...
	{
		FBoxSphereBounds Result = (BuiltInstanceBounds + UnbuiltInstanceBounds).TransformBy(BoundTransform);
...
int32 UHierarchicalInstancedStaticMeshComponent::AddInstance(const FTransform& InstanceTransform, bool bWorldSpace)
{
	...
	// Initialize the seed here even if the rebuild doesn't happen now to prevent races with being renamed by the time the mesh compiles
	InitializeInstancingRandomSeed();
