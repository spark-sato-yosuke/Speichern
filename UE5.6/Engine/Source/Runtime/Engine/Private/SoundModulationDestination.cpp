...
FSoundModulationDestinationSettings FSoundModulationDefaultSettings::GetSettingsForDestination(EModulationDestination Destination) const
{
	switch (Destination)
	{
	case EModulationDestination::Volume:
		return VolumeModulationDestination;
	case EModulationDestination::Pitch:
		return PitchModulationDestination;
	case EModulationDestination::Lowpass:
		return LowpassModulationDestination;
	case EModulationDestination::Highpass:
		return HighpassModulationDestination;
	default:
		break;
	}
...
EModulationRouting FSoundModulationDefaultRoutingSettings::GetRoutingForDestination(EModulationDestination Destination) const
{
	switch (Destination)
	{
		case EModulationDestination::Volume:
	...
	return EModulationRouting::Disable;
}
...
namespace Audio
{
	...
	void FModulationDestination::Init(FDeviceId InDeviceId, FName InParameterName, bool bInIsBuffered, bool bInValueNormalized)
	{
		...
		else
		{
			...
			{
				if (const FAudioDevice* AudioDevice = AudioDeviceManager->GetAudioDeviceRaw(InDeviceId))
				{
					if (AudioDevice->IsModulationPluginEnabled())
					{
						...
						UE_LOG(LogAudio, Warning, TEXT("Modulation Parameter %s not found while Initializing Modulation Destination. Using default parameter instead."), *InParameterName.ToString());
					}
				}
			}
			
			DestinationData->Parameter = Audio::GetDefaultModulationParameter();
		}
