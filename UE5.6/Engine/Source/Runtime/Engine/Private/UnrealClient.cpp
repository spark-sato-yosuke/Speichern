...
bool FRenderTarget::ReadFloat16Pixels(TArray<FFloat16Color>& OutImageData, FReadSurfaceDataFlags InFlags, FIntRect InSrcRect)
{
	...
	ENQUEUE_RENDER_COMMAND(ReadSurfaceFloatCommand)(
		[RenderTarget_RT = this, SrcRect_RT = InSrcRect, OutData_RT = &OutImageData, Flags_RT = InFlags](FRHICommandListImmediate& RHICmdList)
	{
		FRHITexture* Texture = RenderTarget_RT->GetShaderResourceTexture();
		RHICmdList.Transition(FRHITransitionInfo(Texture, ERHIAccess::SRVMask, ERHIAccess::CopySrc));
		RHICmdList.ReadSurfaceFloatData(Texture, SrcRect_RT, *OutData_RT, Flags_RT);
...
bool FRenderTarget::ReadLinearColorPixels(TArray<FLinearColor> &OutImageData, FReadSurfaceDataFlags InFlags, FIntRect InSrcRect)
{
		...
		[RenderTarget_RT = this, SrcRect_RT = InSrcRect, OutData_RT = &OutImageData, Flags_RT = InFlags](FRHICommandListImmediate& RHICmdList)
	{
		FRHITexture* Texture = RenderTarget_RT->GetShaderResourceTexture();
		RHICmdList.Transition(FRHITransitionInfo(Texture, ERHIAccess::SRVMask, ERHIAccess::CopySrc));
		RHICmdList.ReadSurfaceData(Texture, SrcRect_RT, *OutData_RT, Flags_RT);
		RHICmdList.Transition(FRHITransitionInfo(Texture, ERHIAccess::CopySrc, ERHIAccess::SRVMask));
	});
	FlushRenderingCommands();
...
int32 FStatUnitData::DrawStat(FViewport* InViewport, FCanvas* InCanvas, int32 InX, int32 InY)
{
	...
	for (uint32 GPUIndex : FRHIGPUMask::All())
	{
		...
		RawGPUFrameTime[GPUIndex] = FPlatformTime::ToMilliseconds(RHIGetGPUFrameCycles(GPUIndex));
	...
	// Draw unit.
	{
		...
		{
			const FIntPoint RenderResolution = GPixelRenderCounters.GetRenderResolution();
			...
			if (DynamicResolutionStateInfos.Status == EDynamicResolutionStatus::Enabled)
			{
				...
				DrawDefaultAvgCell(FString::Printf(TEXT("%3.1f%% (%dx%d)"), ScreenPercentage, RenderResolution.X, RenderResolution.Y), Color);
			}
			else
			{
				if (GEngine->GetDynamicResolutionState() && GEngine->GetDynamicResolutionState()->IsSupported())
				{
					DrawDefaultAvgCell(FString::Printf(TEXT("%3.1f%% (%dx%d) (Forced)"), ScreenPercentage, RenderResolution.X, RenderResolution.Y), StatMagenda);
				}
				else
				{
					DrawDefaultAvgCell(FString::Printf(TEXT("%3.1f%% (%dx%d) (Forced, Unsupported)"), ScreenPercentage, RenderResolution.X, RenderResolution.Y), StatRed);
				}
			}
		...
		{
			// Render Resolution
			const float ScreenPercentage = GPixelRenderCounters.GetResolutionFraction() * 100.0f;
			const FIntPoint RenderResolution = GPixelRenderCounters.GetRenderResolution();
			DrawTitleString(TEXT("RenderRes"), NoUnitGraphColor);
			DrawDefaultAvgCell(FString::Printf(TEXT("%3.1f%% (%dx%d)"), ScreenPercentage, RenderResolution.X, RenderResolution.Y), StatGreen);
			PushRows(/* RowsCount = */ 1);
		}
...
void FViewport::Draw( bool bShouldPresent /*= true */)
{
	...
	if(!bReentrant)
	{
		...
		else
		{
			...
			if( SizeX > 0 && SizeY > 0 )
			{
				...
				// Calculate gamethread time (excluding idle time)
				{
					...
					if (GFrameCounter != LastFrameUpdated)
					{
						...
						UE::Stats::FThreadIdleStats& GameThread = UE::Stats::FThreadIdleStats::Get();
						if (bStarted)
						{
			...
			else if (World && World->IsGameWorld() && !GUseUnifiedTimeBudgetForStreaming)
			{
				// since we aren't drawing the viewport, we still need to update streaming
				World->UpdateLevelStreaming();
			}
...
ENGINE_API bool GetViewportScreenShot(FViewport* Viewport, TArray<FColor>& Bitmap, const FIntRect& ViewRect /*= FIntRect()*/, const FReadSurfaceDataFlags& ReadSurfaceFlags /*= FReadSurfaceDataFlags()*/)
{
	...
	if (Viewport->ReadPixels(Bitmap, ReadSurfaceFlags, ViewRect))
	{
...
{
	// Read the contents of the viewport into an array.
	if (Viewport->ReadLinearColorPixels(Bitmap, ReadSurfaceFlags, ViewRect))
	{
		check(Bitmap.Num() == ViewRect.Area() || (Bitmap.Num() == Viewport->GetRenderTargetTextureSizeXY().X * Viewport->GetRenderTargetTextureSizeXY().Y));
		return true;
	}
...
PRAGMA_DISABLE_DEPRECATION_WARNINGS
bool FViewportClient::InputAxis(
		FViewport* Viewport,
		...
		bool bGamepad)
{
	// Note: Set the timestamp to 0u here to represent an "invalid" timestamp, since this older input axis
	// function did not account for timestamps. If we initalized it to the current cycle count here,
	// then deprecated code paths would always be considered "newer" then anything calling the newer API, where the timestamp
	// is calculated when the actual slate input event is constructed. 
	FInputKeyEventArgs Args(Viewport, InputDevice, Key, Delta, DeltaTime, NumSamples, 0u);

	return InputAxis(Args);
}

bool FViewportClient::InputTouch(FViewport* Viewport, int32 ControllerId, uint32 Handle, ETouchType::Type Type, const FVector2D& TouchLocation, float Force, FDateTime DeviceTimestamp, uint32 TouchpadIndex)
{
	// Get the newer FInputDeviceId from the legayc ControllerId
	FPlatformUserId UserId = PLATFORMUSERID_NONE;
	FInputDeviceId DeviceId = INPUTDEVICEID_NONE;
	IPlatformInputDeviceMapper::Get().RemapControllerIdToPlatformUserAndDevice(ControllerId, OUT UserId, OUT DeviceId);
	
	// Note: Set the timestamp to 0u here to represent an "invalid" timestamp
...
bool FViewportClient::InputGesture(FViewport* Viewport, EGestureEvent GestureType, const FVector2D& GestureDelta, bool bIsDirectionInvertedFromDevice)
{
	// Use the default platform user's primary device for any upgrade path to try and get the best behavior.
	// Legacy behavior is that all touch/gesture events go through this primary user
	const IPlatformInputDeviceMapper& Mapper = IPlatformInputDeviceMapper::Get();
	const FPlatformUserId PrimaryUser = Mapper.GetPrimaryPlatformUser();
	const FInputDeviceId DeviceId = IPlatformInputDeviceMapper::Get().GetPrimaryInputDeviceForUser(PrimaryUser);
...
bool FViewportClient::InputMotion(FViewport* Viewport, int32 ControllerId, const FVector& Tilt, const FVector& RotationRate, const FVector& Gravity, const FVector& Acceleration)
{
	FPlatformUserId UserId = PLATFORMUSERID_NONE;
	FInputDeviceId DeviceId = INPUTDEVICEID_NONE;
	IPlatformInputDeviceMapper::Get().RemapControllerIdToPlatformUserAndDevice(ControllerId, OUT UserId, OUT DeviceId);
