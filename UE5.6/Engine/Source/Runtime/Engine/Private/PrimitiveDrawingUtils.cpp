...
void BuildDiscVerts(const FVector& Center, const FVector& XAxis, const FVector& YAxis, double Radius, uint32 Sides, TArray<FDynamicMeshVertex>& OutVerts, TArray<uint32>& OutIndices)
{
	...
	FVector	LastVertex = Center + XAxis * Radius;
	...
	{
		const FVector Vertex = Center + (XAxis * FMath::Cos(AngleDelta * (SideIndex + 1)) + YAxis * FMath::Sin(AngleDelta * (SideIndex + 1))) * Radius;
		FVector Normal = Vertex - Center;
		Normal.Normalize();
	...
	//Add top triangles, in the style of a fan.
...
{
	GetDiscMesh(FMatrix::Identity, Center, XAxis, YAxis, Radius, Sides, MaterialRenderProxy, DepthPriority, ViewIndex, Collector, HitProxy);
}
...
void GetDiscMesh(const FMatrix& ToWorld, const FVector& Center, const FVector& XAxis, const FVector& YAxis, double Radius, int32 Sides, const FMaterialRenderProxy* MaterialRenderProxy, uint8 DepthPriority, int32 ViewIndex, FMeshElementCollector& Collector, HHitProxy* HitProxy)
{
	TArray<FDynamicMeshVertex> MeshVerts;
	TArray<uint32> MeshIndices;
	BuildDiscVerts(Center, XAxis, YAxis, Radius, Sides, MeshVerts, MeshIndices);

	FDynamicMeshBuilder MeshBuilder(Collector.GetFeatureLevel());
	MeshBuilder.AddVertices(MeshVerts);
	MeshBuilder.AddTriangles(MeshIndices);

	MeshBuilder.GetMesh(ToWorld, MaterialRenderProxy, DepthPriority, false, false, true, ViewIndex, Collector, HitProxy);
}
			   ...
			   uint8 DepthPriority, bool bPartial, float Angle, bool bEndCaps)
{
	...
	for (int32 OuterIndex = 0; OuterIndex < OuterSlices; OuterIndex++)
	{
		...
		for (int32 InnerIndex = 0; InnerIndex < InnerSegments; InnerIndex++)
		{
			...
			// We invert the direction before deriving tangents, to counteract winding order
			// @see DrawSphere, etc.
			Dir *= -1.0;
...
bool IsRichView(const FSceneViewFamily& ViewFamily)
{
	...
	if( !ViewFamily.EngineShowFlags.LOD ||
		...
		(!ViewFamily.EngineShowFlags.Lighting && ViewFamily.EngineShowFlags.UnlitRichView) ||
