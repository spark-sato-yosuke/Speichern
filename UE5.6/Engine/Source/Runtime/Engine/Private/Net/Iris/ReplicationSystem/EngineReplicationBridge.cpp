...
UE::Net::FNetRefHandle UEngineReplicationBridge::StartReplicatingActor(AActor* Actor, const FActorReplicationParams& ActorParams )
{
	using namespace UE::Net;
...
void UEngineReplicationBridge::StopReplicatingActor(AActor* Actor, EEndPlayReason::Type EndPlayReason)
{
	...
	const bool bIsObjectDestroyed = EndPlayReason == EEndPlayReason::Destroyed;
	...
	{
		const UObjectReplicationBridge::FDestructionParameters EndReplicationParameters
		{
	...
	EEndReplicationFlags Flags = ConvertEndPlayIntoEndReplication(EndPlayReason);
	if (Actor->bNetTemporary)
	{
...
EEndReplicationFlags UEngineReplicationBridge::ConvertEndPlayIntoEndReplication(EEndPlayReason::Type EndPlayReason) const
{
	...
	const bool bIsShuttingDown = (EndPlayReason == EEndPlayReason::EndPlayInEditor) || (EndPlayReason == EEndPlayReason::Quit) || NetDriver->GetPendingDestruction();
...
{
	if (const AActor* ReplicatedActor = Cast<AActor>(ReplicatedObject))
	{
	...
	//TODO: Ask NetObjectFactory for this value instead
	return 100.0f;
}
...
void UEngineReplicationBridge::SendErrorWithNetRefHandle(UE::Net::ENetRefHandleError ErrorType, FNetRefHandle RefHandle, uint32 ConnectionId, const TArray<FNetRefHandle>& ExtraNetRefHandle)
{
	if (NetDriver)
	{
		if (UNetConnection* ClientConnection = NetDriver->GetConnectionByHandle(UE::Net::FConnectionHandle(ConnectionId)))
		{
			...
			for(FNetRefHandle ExtraHandle : ExtraNetRefHandle)
			{
				ArrayIds.Emplace(ExtraHandle.GetId());
			}

			FNetControlMessage<NMT_IrisNetRefHandleErrorWithArray>::Send(ClientConnection, ErrorType, HandleId, ArrayIds);
		}
		else
		{
			UE_LOG(LogIrisBridge, Error, TEXT("UEngineReplicationBridge::SendErrorWithNetRefHandle could not find Connection for id:%u"), ConnectionId);
...
void UEngineReplicationBridge::AddRootObjectToLevelGroup(const UObject* RootObject, const ULevel* Level)
{
	...
	if (Level && (!Level->IsPersistentLevel() || (Level != NetDriver->GetWorld()->PersistentLevel)))
	{
		...
		if (!LevelGroup.IsValid())
		{
			...
			UE_LOG_ACTORREPLICATIONBRIDGE(Verbose, TEXT("Created new GroupIndex: %u for Level: %s"), LevelGroup.GetGroupIndex(), ToCStr(PackageName.ToString()));
		...
		// Add object to group
		UE_LOG_ACTORREPLICATIONBRIDGE(Verbose, TEXT("Added %s to GroupIndex: %u Level: %s"), *RootObjectRefHandle.ToString(), LevelGroup.GetGroupIndex(), ToCStr(PackageName.ToString()));
