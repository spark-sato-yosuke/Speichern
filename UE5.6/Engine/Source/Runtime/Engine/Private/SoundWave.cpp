...
static int32 bRetainInlinedFirstChunkCVar = 0;
FAutoConsoleVariableRef CVarRetainInlinedFirstChunk(
	TEXT("au.streamcache.RetainInlinedFirstChunk"),
...
UE_TRACE_EVENT_BEGIN(Audio, SoundWaveStart)
	...
	UE_TRACE_EVENT_FIELD(UE::Trace::WideString, SoundSuperClassName)
UE_TRACE_EVENT_END()

UE_TRACE_EVENT_BEGIN(Audio, SoundPriority)
	UE_TRACE_EVENT_FIELD(uint32, DeviceId)
	UE_TRACE_EVENT_FIELD(uint64, Timestamp)
	UE_TRACE_EVENT_FIELD(uint32, PlayOrder)
	UE_TRACE_EVENT_FIELD(uint32, ActiveSoundPlayOrder)
	UE_TRACE_EVENT_FIELD(float, Priority)
UE_TRACE_EVENT_END()

UE_TRACE_EVENT_BEGIN(Audio, SoundDistance)
	UE_TRACE_EVENT_FIELD(uint32, DeviceId)
	UE_TRACE_EVENT_FIELD(uint64, Timestamp)
	UE_TRACE_EVENT_FIELD(uint32, PlayOrder)
	UE_TRACE_EVENT_FIELD(uint32, ActiveSoundPlayOrder)
...
#endif
{
	...
	CuePointOrigin = ESoundWaveCuePointOrigin::WaveFile;
#endif
...
void USoundWave::SetSampleRate(uint32 InSampleRate, const bool bFromDecoders)
{
	if (SoundWaveDataPtr)
	{
		SoundWaveDataPtr->SetSampleRate(IntCastChecked<int32>(InSampleRate));
	}
...
void USoundWave::SetNumFrames(uint32 InNumFrames)
{
	if (SoundWaveDataPtr)
	{
		SoundWaveDataPtr->SetNumFrames(InNumFrames);	
	}
}
		...
		if (Ar.IsCooking())
		{
			...
			if (OwnerBehavior.LoadingBehavior != ESoundWaveLoadingBehavior::Uninitialized)
			{
				UE_LOG(LogAudio, VeryVerbose, TEXT("Saving Owners Loading Behavior: %s, %s"), 
					Audio::ShortEnumString(EnumToString(OwnerBehavior.LoadingBehavior)), *GetFullName());
...
{
	SoundWaveDataPtr->SetAllCuePoints(InCuePoints);
}

#if WITH_EDITOR
TArray<FSoundWaveCuePoint> USoundWave::GetSoundWaveCuePoints() const
{
	return CuePoints;
}

void USoundWave::SetSoundWaveCuePoints(const TArray<FSoundWaveCuePoint>& InCuePoints)
{
	CuePoints = InCuePoints;
	SetAllCuePoints(InCuePoints);
}
#endif //WITH_EDITOR
	...
	if (!bContainedValidData)
	{
		if (!FPlatformProperties::RequiresCookedData() && GetDerivedDataCache())
		{
			...
			if (bGetSuccessful)
			{
				...
				FAudioCookOutputs CookOutput;
				FMemoryReader Reader(OutData);
				ensure(CookOutput.Serialize(Reader));
				
				Result->Lock(LOCK_READ_WRITE);
				FMemory::Memcpy(Result->Realloc(CookOutput.EncodedData.Num()), CookOutput.EncodedData.GetData(), CookOutput.EncodedData.Num());
				Result->Unlock();
			}
		}
		else
...
void USoundWave::PostLoad()
{
	...
	if (FApp::CanEverRenderAudio())
	{
		...
		if (ShouldUseStreamCaching())
		{
			const TIndirectArray<FStreamedAudioChunk>& Chunks = SoundWaveDataPtr->RunningPlatformData.Chunks;

			// First chunk can be optionally inlined for the sake of pre-buffering.
			const bool bIsFirstChunkInlined = Chunks.IsValidIndex(1) ? Chunks[1].BulkData.IsInlined() : false;

			// Allowing retaining if we're NOT in the editor AND we've NOT disabled retaining (via a cvar).
			const bool bAllowRetaining = !GIsEditor && !DisableRetainingCVar;

			// Retaining the "inlined" first chunk (which can be any load mode) is enabled via a CVAR.
			const bool bShouldRetainInlinedFirstChunked = bIsFirstChunkInlined && !!bRetainInlinedFirstChunkCVar;

			// We retain the first chunk if we're set to retain
			// OR we've inlined the first chunk (we do when we're pre-buffering). We don't want these dumped by the stream cache if we can help it.
			const bool bShouldRetainFirstChunk = bAllowRetaining &&
				(ActualLoadingBehavior == ESoundWaveLoadingBehavior::RetainOnLoad || bShouldRetainInlinedFirstChunked);

			UE_CLOG(Audio::MatchesLogFilter(*GetName()), LogAudioTiming, Verbose, TEXT("PostLoad: Wave='%s'\tFormat='%s'\tLooping=%s\tChunk1(Retain=%s,Inlined=%s,Size=%lld),LB=%s, NumChunks=%d"), 
				*GetFName().ToString(),
				*GetRuntimeFormat().ToString(),
				ToCStr(LexToString(bLooping)),
				ToCStr(LexToString(bShouldRetainFirstChunk)),
				ToCStr(LexToString(bIsFirstChunkInlined)),
				Chunks.IsValidIndex(1) ? Chunks[1].BulkData.GetElementCount() : 0,	
				Audio::ShortEnumString(EnumToString(ActualLoadingBehavior)),
				Chunks.Num()
			);
			
			if (bShouldRetainFirstChunk)
			{
...
void USoundWave::OnCookEvent(UE::Cook::ECookEvent CookEvent, UE::Cook::FCookEventContext& CookContext)
{
	Super::OnCookEvent(CookEvent, CookContext);
	if (CookEvent == UE::Cook::ECookEvent::PlatformCookDependencies && CookContext.IsCooking())
	{
		// We might need GetOwnerLoadingBehavior cached for RecordSoundWaveLoadingBehaviorDependenciesForCook
		GetOwnerLoadingBehavior(CookContext.GetTargetPlatform());
		UE::SoundWaveLoadingUtil::Private::RecordSoundWaveLoadingBehaviorDependenciesForCook(CookContext, this);
	}
}
...
{
	TArray<FSoundWaveCuePoint> CuePointsScaled;

	switch (CuePointOrigin)
	{
	case ESoundWaveCuePointOrigin::WaveFile:
		CuePointsScaled = CuePoints;
		break;
	case ESoundWaveCuePointOrigin::MarkerTransformation:
		CuePointsScaled = GetWaveformTransformationInfo().AllCuePoints;
		break;
	default:
		CuePointsScaled = CuePoints;
		break;
...
void USoundWave::PostEditChangeProperty(FPropertyChangedEvent& PropertyChangedEvent)
{
	...
	static const FName AssetUserDataFName = GET_MEMBER_NAME_CHECKED(USoundWave, AssetUserData);
	...
	if (FProperty* PropertyThatChanged = PropertyChangedEvent.Property)
	{
		...
		else if (Name == AssetUserDataFName && !AssetUserData.IsEmpty())
		{
			// If a new Subtitle asset user datum was added, default its duration to the length of the SoundWave it was added to.
			// Look at the last item because a new user data added in the editor will always go at the end of the array- earlier data will have already had this operation applied.

			TObjectPtr<UAssetUserData>& LastModifiedUserData = AssetUserData.Last();
			if (IsValid(LastModifiedUserData))
			{
				USubtitleAssetUserData* UserDataAsSubtitle = Cast<USubtitleAssetUserData>(LastModifiedUserData.Get() );
				if (IsValid(UserDataAsSubtitle) && UserDataAsSubtitle->Duration == SubtitleDefaultDuration && UserDataAsSubtitle->Text.IsEmpty())
				{
					// If the sound is looping indefinitely, the subtitle duration will default to the length of the first loop.
					// It is still editable by the user after this default is set.
					UserDataAsSubtitle->Duration = Duration;
				}
			}
		}
...
void USoundWave::Parse(FAudioDevice* AudioDevice, const UPTRINT NodeWaveInstanceHash, FActiveSound& ActiveSound, const FSoundParseParameters& ParseParams, TArray<FWaveInstance*>& WaveInstances)
{
	...
	if (!WaveInstance)
	{
		...
		if (ModularFeatures.IsModularFeatureAvailable(ISubtitlesAndClosedCaptionsModule::GetModularFeatureName()))
		{
			const ISubtitlesAndClosedCaptionsModule& SubtitlesModule = ModularFeatures.GetModularFeature<ISubtitlesAndClosedCaptionsModule>(ISubtitlesAndClosedCaptionsModule::GetModularFeatureName());
			if (const UAssetUserData * Subtitle{ GetAssetUserDataOfClass(SubtitlesModule.GetAssetUserDataClass()) })
			{
				FSubtitlesAndClosedCaptionsDelegates::QueueSubtitle.ExecuteIfBound(FQueueSubtitleParameters{*Subtitle}, ESubtitleTiming::InternallyTimed);
			}
		}
	}
	else
	...
	// Procedural sounds do not have a duration that  makes sense to fmod with
	if (!bProcedural && IsLooping())
	{
		WaveInstance->StartTime = FMath::Fmod(ParseParams.StartTime, Duration);
	}
	else
	{
	...
	const bool bIsWavePlaying = WaveInstance->IsPlaying();
	if (bIsWavePlaying)
	{
	...
	const bool bWaveIsActive = bIsWavePlaying || !ActiveSound.bFinished;

	if (bChannelEnabled && bWaveIsActive)
	{
		if (bIsNewWave && WaveInstance->SoundClass != nullptr)
		{
			const TObjectPtr<UObject> SoundWaveObjectPtr = FSoftObjectPath(WaveInstance->WaveData->GetPathName()).ResolveObject();
			if (SoundWaveObjectPtr)
			{
				const FString SoundWaveClassName = SoundWaveObjectPtr->GetClass()->GetName();

				const TObjectPtr<UClass> SoundWaveSuperClass = SoundWaveObjectPtr->GetClass()->GetSuperClass();
				const FString SoundWaveSuperClassName = SoundWaveSuperClass ? SoundWaveSuperClass->GetName() : FString();

				UE_TRACE_LOG(Audio, SoundWaveStart, AudioMixerChannel)
					<< SoundWaveStart.DeviceId(AudioDevice->DeviceID)
					<< SoundWaveStart.Timestamp(FPlatformTime::Cycles64())
					<< SoundWaveStart.PlayOrder(WaveInstance->GetPlayOrder())
					<< SoundWaveStart.ActiveSoundPlayOrder(ActiveSound.GetPlayOrder())
					<< SoundWaveStart.Name(*SoundWaveObjectPtr->GetPathName())
					<< SoundWaveStart.SoundClassName(*SoundWaveClassName)
					<< SoundWaveStart.SoundSuperClassName(*SoundWaveSuperClassName);
			}
		}

		UE_TRACE_LOG(Audio, SoundPriority, AudioMixerChannel)
			<< SoundPriority.DeviceId(AudioDevice->DeviceID)
			<< SoundPriority.Timestamp(FPlatformTime::Cycles64())
			<< SoundPriority.PlayOrder(WaveInstance->GetPlayOrder())
			<< SoundPriority.ActiveSoundPlayOrder(ActiveSound.GetPlayOrder())
			<< SoundPriority.Priority(WaveInstance->Priority);

		UE_TRACE_LOG(Audio, SoundDistance, AudioMixerChannel)
			<< SoundDistance.DeviceId(AudioDevice->DeviceID)
			<< SoundDistance.Timestamp(FPlatformTime::Cycles64())
			<< SoundDistance.PlayOrder(WaveInstance->GetPlayOrder())
			<< SoundDistance.ActiveSoundPlayOrder(ActiveSound.GetPlayOrder())
			<< SoundDistance.Distance(WaveInstance->ListenerToSoundDistance);
	}
#endif // UE_AUDIO_PROFILERTRACE_ENABLED
...
void USoundWave::RetainCompressedAudio(bool bForceSync /*= false*/)
{
	...
	else if (bForceSync)
	{
		...
		UE_CLOG (Audio::MatchesLogFilter(*GetName()), LogAudioTiming, Verbose, TEXT("RetainCompressedAudio (Sync) on SoundWave: Wave='%s'\tFormat='%s'\tLooping=%s\tChunkSize=%u"), 
						*SoundWaveDataPtr->GetFName().ToString(),
						*SoundWaveDataPtr->GetRuntimeFormat().ToString(),
						ToCStr(LexToString(SoundWaveDataPtr->IsLooping())),
						SoundWaveDataPtr->FirstChunk.IsValid() ? SoundWaveDataPtr->FirstChunk.Num() : 0
					);
		...
		GetHandleForChunkOfAudio([WeakThis = MakeWeakObjectPtr(this)](FAudioChunkHandle&& OutHandle)
		{
			if (OutHandle.IsValid())
			{
				AsyncTask(ENamedThreads::GameThread, [WeakThis, MovedHandle = MoveTemp(OutHandle)]() mutable
				{
					...
					if (WeakThis.IsValid())
					{
						UE_CLOG (Audio::MatchesLogFilter(WeakThis->SoundWaveDataPtr->GetFName()), LogAudioTiming, Verbose, TEXT("Async SetFirstChunk on SoundWave: Wave='%s'\tFormat='%s'\tLooping=%s\tChunkSize=%u"), 
							*WeakThis->SoundWaveDataPtr->GetFName().ToString(),
							*WeakThis->SoundWaveDataPtr->GetRuntimeFormat().ToString(),
							ToCStr(LexToString(WeakThis->SoundWaveDataPtr->IsLooping())),
							MovedHandle.Num()
						);
...
#else
				if (GetNumChunks() > 1 && FApp::CanEverRenderAudio())
				{
					if (!Proxy.IsValid())
					{
						Proxy = CreateSoundWaveProxy();
...
const FWaveTransformUObjectConfiguration& USoundWave::UpdateTransformations()
{
	...
	if (TotalSamples == 0)
	{
		UE_LOG(LogAudio, Warning, TEXT("Found %" SIZE_T_FMT " total samples when updating transformations on soundwave %s"), size_t(TotalSamples), *GetNameSafe(this));
	...
	TransformationChainConfig.WaveCues = CuePoints;
	TransformationChainConfig.bIsPreviewingLoopRegion = false;
	TransformationChainConfig.bCachedSoundWaveLoopState = static_cast<bool>(bLooping);

	TSet<UClass*> TransformationClassSet;
	for (int32 i = 0; i < Transformations.Num(); ++i)
	{
		TObjectPtr<UWaveformTransformationBase>& Transformation = Transformations[i];
		...
		UClass* TransformationClass = Transformation->GetClass();

		if (TransformationClassSet.Contains(TransformationClass))
		{
			UE_LOG(LogAudio, Warning, TEXT("Can't have more than one %s on a SoundWave"), *TransformationClass->GetName());
			Transformation = nullptr;

			break;
		}

		TransformationClassSet.Add(Transformation->GetClass());
	}

	//Sort by priority to have a proper order of operations and simplify transformation audio processing
	UWaveformTransformationBase::SortTransformationsArray(Transformations);
	...
	{
		bLooping = static_cast<uint8>(TransformationChainConfig.bCachedSoundWaveLoopState);
	}

	return TransformationChainConfig;
}

void USoundWave::OverwriteSoundWaveDataWithTransformationData()
{
	for (TObjectPtr<UWaveformTransformationBase>& Transformation : Transformations)
	{
		if (Transformation)
		{
			Transformation->OverwriteSoundWaveData(*this);
		}
	}

	//Overwrite the wave data for cue points and sample loops
	...
	{
		FWaveCue WaveCue;
		WaveCue.CuePointID = CuePoint.CuePointID;
		WaveCue.Position = CuePoint.FramePosition;
		WaveCue.Label = CuePoint.Label;
		WaveCue.SampleLength = CuePoint.FrameLength;

		WaveCues.Emplace(WaveCue);
		
		if (CuePoint.IsLoopRegion())
		{
			FWaveSampleLoop WaveSampleLoop;
			WaveSampleLoop.LoopID = CuePoint.CuePointID;
			WaveSampleLoop.StartFrame = CuePoint.FramePosition;
			WaveSampleLoop.EndFrame = CuePoint.FramePosition + CuePoint.FrameLength;

			WaveSampleLoops.Emplace(WaveSampleLoop);
		}
	}

	// Some DAWs require the Cues and SampleLoops to be sorted by frame position and not CueID.
	Algo::Sort(WaveCues, [](const FWaveCue& WaveCueA, const FWaveCue& WaveCueB)
		{
			return WaveCueA.Position < WaveCueB.Position;
		});
	Algo::Sort(WaveSampleLoops, [](const FWaveSampleLoop& LoopA, const FWaveSampleLoop& LoopB)
		{
			return LoopA.StartFrame < LoopB.StartFrame;
		});

	TArray<uint8> WaveFileData;
	TFuture<FSharedBuffer> FutureBuffer = RawData.GetPayload();
	TArray<uint8> InWaveFileData;
	InWaveFileData.Append((uint8*)FutureBuffer.Get().GetData(), FutureBuffer.Get().GetSize());

	Audio::SoundFileUtils::CreateCueAndSampleChunks(InWaveFileData, WaveFileData, WaveCues, WaveSampleLoops);

	// Update the RawData with the new payload
	FSharedBuffer Buffer = FSharedBuffer::Clone(WaveFileData.GetData(), WaveFileData.Num());
	RawData.UpdatePayload(Buffer);
}

FWaveformTransformationInfo USoundWave::GetWaveformTransformationInfo() const
{
	FWaveformTransformationInfo TransformationInfo;
	for (TObjectPtr<UWaveformTransformationBase> Transformation : Transformations)
	{
		if (Transformation)
		{
			Transformation->GetTransformationInfo(TransformationInfo);
		}
	}
...
void USoundWave::SerializeCuePoints(FArchive& Ar, const bool bIsLoadingFromCookedArchive)
{
	...
	if (Ar.IsCooking())
	{
		...
		case ESoundWaveCuePointOrigin::WaveFile:
			...
			break;
		case ESoundWaveCuePointOrigin::MarkerTransformation:
			PlatformCuePoints = GetWaveformTransformationInfo().AllCuePoints;
			break;
		default:
			PlatformCuePoints = CuePoints;
			break;
	...
	: SoundWaveDataPtr(InWave->SoundWaveDataPtr)
{
	...
	checkf(!InWave->bProcedural, TEXT("FSoundWaveProxy can not  be made from a procedural sound wave."));
...
const TArray<FSoundWaveCuePoint> FSoundWaveProxy::GetCuePointsAndLoopRegions() const
{
	...
	TArray<FSoundWaveCuePoint> CuePointsAndLoopRegions;
	CuePointsAndLoopRegions.Reserve(SoundWaveDataPtr->GetLoopRegions().Num() + SoundWaveDataPtr->GetCuePoints().Num());

	// Previously loops had two cue points (one marked as a cue point, the other a loop region),
	// now they only have one that is marked as a loop region so we need to append Loop Regions.
	// Append must happen in this order to maintain the label for loops that were imported before the fix because
	// meta sounds will pick only one cue point to render if they have matching frames.
	CuePointsAndLoopRegions.Append(SoundWaveDataPtr->GetLoopRegions());
	CuePointsAndLoopRegions.Append(SoundWaveDataPtr->GetCuePoints());
	
	return CuePointsAndLoopRegions;
}
...
#if WITH_EDITORONLY_DATA
TFuture<FSharedBuffer> USoundWave::FEditorAudioBulkData::GetPayload(const EPayloadFlags Flags) const
{
	...
	FSharedBuffer BulkDataBuffer;
	if (Flags == EPayloadFlags::ReturnCopyWithTransformationData)
	{
		TObjectPtr<USoundWave> DuplicateSoundWave = DuplicateObject<USoundWave>(SoundWave, SoundWave->GetOuter());

		DuplicateSoundWave->OverwriteSoundWaveDataWithTransformationData();
		BulkDataBuffer = DuplicateSoundWave->RawData.RawData.GetPayload().Get();
	}
	else
	{
		BulkDataBuffer = RawData.GetPayload().Get();
	}
