...
: Super(ObjectInitializer)
{
	...
	SetReplicatedGravityDirection(UCharacterMovementComponent::DefaultGravityDirection);
}
...
void ACharacter::PostInitializeComponents()
{
	...
	if (IsValidChecked(this))
	{
		...
		if (GetController() == nullptr && GetNetMode() != NM_Client)
		{
...
{
	bProxyIsJumpForceApplied = bInProxyIsJumpForceApplied;
}
...
bool ACharacter::IsJumpProvidingForce() const
{
	...
	else if (GetProxyIsJumpForceApplied() && (GetLocalRole() == ROLE_SimulatedProxy))
	{
...
{
	return !IsCrouched() && CharacterMovement && CharacterMovement->CanEverCrouch() && GetRootComponent() && !GetRootComponent()->IsSimulatingPhysics();
}
...
void ACharacter::SetReplicatedServerLastTransformUpdateTimeStamp(float InReplicatedServerLastTransformUpdateTimeStamp)
{
	ReplicatedServerLastTransformUpdateTimeStamp = InReplicatedServerLastTransformUpdateTimeStamp; 
}

void ACharacter::SetReplayLastTransformUpdateTimeStamp(float InReplayLastTransformUpdateTimeStamp)
{
	ReplayLastTransformUpdateTimeStamp = InReplayLastTransformUpdateTimeStamp;
}

FBasedMovementInfo& ACharacter::GetReplicatedBasedMovement_Mutable()
{
	return ReplicatedBasedMovement;
}

void ACharacter::SetReplicatedBasedMovement(const FBasedMovementInfo& InReplicatedBasedMovement)
{
	ReplicatedBasedMovement = InReplicatedBasedMovement;
}

void ACharacter::SetReplicatedMovementMode(const uint8 InReplicatedMovementMode)
{
	ReplicatedMovementMode = InReplicatedMovementMode; 
}
...
{
	ReplicatedGravityDirection = InReplicatedGravityDirection;
}
...
void ACharacter::OnMovementModeChanged(EMovementMode PrevMovementMode, uint8 PrevCustomMode)
{
	...
	if (GetProxyIsJumpForceApplied() && CharacterMovement->IsFalling())
	{
...
{
	SavedMovementMode = GetReplicatedMovementMode();
	PreNetReceivedGravityDirection = GetReplicatedGravityDirection();
	Super::PreNetReceive();
}
...
void ACharacter::PostNetReceive()
{
	...
	{
		CharacterMovement->bNetworkGravityDirectionChanged = !PreNetReceivedGravityDirection.Equals(GetReplicatedGravityDirection());
		CharacterMovement->bNetworkMovementModeChanged |= ((SavedMovementMode != GetReplicatedMovementMode()) || (CharacterMovement->PackNetworkMovementMode() != GetReplicatedMovementMode()));
...
{	
	...
	if (RepBasedMovement.HasRelativeLocation())
	{
		...
		MovementBaseUtility::GetMovementBaseTransform(RepBasedMovement.MovementBase, RepBasedMovement.BoneName, CharacterMovement->OldBaseLocation, CharacterMovement->OldBaseQuat);
		...
		if (RepBasedMovement.HasRelativeRotation())
		{
			...
			NewRotation = (FRotationMatrix(RepBasedMovement.Rotation) * FQuatRotationMatrix(CharacterMovement->OldBaseQuat)).Rotator();
...
{
	SetReplicatedServerLastTransformUpdateTimeStamp(GetReplayLastTransformUpdateTimeStamp());
}
		...
		if( MoveIndex != INDEX_NONE )
		{
			...
			if( RestoreReplicatedMove(RootMotionRepMove) )
			{
				...
				if( FMath::Abs(DeltaPosition) > UE_KINDA_SMALL_NUMBER )
				{
					...
					const FTransform LocalRootMotionTransform = ClientMontageInstance->Montage->ExtractRootMotionFromTrackRange(ServerPosition, ClientPosition, FAnimExtractContext());
...
void ACharacter::PreReplication( IRepChangedPropertyTracker & ChangedPropertyTracker )
{
	...
	if (IsReplicatingMovement() && (CharacterMovement->CurrentRootMotion.HasActiveRootMotionSources() || IsPlayingNetworkedRootMotionMontage()))
	{
		...
		FRepRootMotionMontage& RepRootMotionRef = GetRepRootMotion_Mutable();
		RepRootMotionRef.bIsActive = true;
		// Is position stored in local space?
		RepRootMotionRef.bRelativePosition = BasedMovement.HasRelativeLocation();
		RepRootMotionRef.bRelativeRotation = BasedMovement.HasRelativeRotation();
		RepRootMotionRef.Location			= RepRootMotionRef.bRelativePosition ? BasedMovement.Location : FRepMovement::RebaseOntoZeroOrigin(GetActorLocation(), GetWorld()->OriginLocation);
		RepRootMotionRef.Rotation			= RepRootMotionRef.bRelativeRotation ? BasedMovement.Rotation : GetActorRotation();
		RepRootMotionRef.MovementBase		= BasedMovement.MovementBase;
		RepRootMotionRef.MovementBaseBoneName = BasedMovement.BoneName;
		if (RootMotionMontageInstance)
		{
			RepRootMotionRef.Animation = RootMotionMontageInstance->Montage->IsDynamicMontage() ? RootMotionMontageInstance->Montage->GetFirstAnimReference() : RootMotionMontageInstance->Montage;
			RepRootMotionRef.Position			= RootMotionMontageInstance->GetPosition();
		}
		else
		{
			RepRootMotionRef.Animation = nullptr;
		}

		RepRootMotionRef.AuthoritativeRootMotion = CharacterMovement->CurrentRootMotion;
		RepRootMotionRef.Acceleration = CharacterMovement->GetCurrentAcceleration();
		RepRootMotionRef.LinearVelocity = CharacterMovement->Velocity;

		DOREPLIFETIME_ACTIVE_OVERRIDE_FAST(ACharacter, RepRootMotion, true);
	}
	else
	{
		const bool bWasRootMotionPreviouslyActive = GetRepRootMotion().bIsActive;

		FRepRootMotionMontage& RepRootMotionRef = GetRepRootMotion_Mutable();
		RepRootMotionRef.Clear();
	...
	SetProxyIsJumpForceApplied(JumpForceTimeRemaining > 0.0f);
	SetReplicatedMovementMode(CharacterMovement->PackNetworkMovementMode());
	SetReplicatedGravityDirection(CharacterMovement->GetGravityDirection());

	if(IsReplicatingMovement())
	{
		FBasedMovementInfo& ReplicatedBasedMovementRef = GetReplicatedBasedMovement_Mutable();
		ReplicatedBasedMovementRef = BasedMovement;
		...
		if (BasedMovement.HasRelativeLocation())
		{
			...
			ReplicatedBasedMovementRef.bServerHasVelocity = !CharacterMovement->Velocity.IsZero();
			...
			{
				ReplicatedBasedMovementRef.Rotation = GetActorRotation();
			}
		}
	}
	...
	{
		SetReplicatedServerLastTransformUpdateTimeStamp(CharacterMovement->GetServerLastTransformUpdateTimeStamp());
	}
	else
	{
		SetReplicatedServerLastTransformUpdateTimeStamp(0.f);
	}
}
...
void ACharacter::PreReplicationForReplay(IRepChangedPropertyTracker& ChangedPropertyTracker)
{
	...
	if (World)
	{
		...
		if (World->IsRecordingClientReplay() && GetController() != nullptr && GetLocalRole() == ROLE_AutonomousProxy && GetNetMode() == NM_Client)
		{
			SetRemoteViewPitch(GetController()->GetControlRotation().Pitch);
		}

		SetReplayLastTransformUpdateTimeStamp(World->GetTimeSeconds());
	}
}
...
void ACharacter::FillAsyncInput(FCharacterAsyncInput& Input) const
{
	...
	Input.ControllerDesiredRotation = GetController()->GetDesiredRotation();
}
