...
TMap<TTuple<EShaderPlatform, FName>, Scalability::FQualityLevels> PreviewShaderPlatformToCachedQualityLevels;
TMap<TTuple<EShaderPlatform, FName>, Scalability::FQualityLevels> DefaultPreviewShaderPlatformToCachedQualityLevels;
...
#if WITH_EDITOR
void ApplyCachedQualityLevels()
{
	// We directly use UDeviceProfileManager::DeviceProfileManagerSingleton here so we don't unexpectedly initialize the device profile system during cooking.
	if (ScalabilityShaderPlatform != EShaderPlatform::SP_NumPlatforms && UDeviceProfileManager::DeviceProfileManagerSingleton)
	{
		const UDeviceProfile* PreviewDeviceProfile = UDeviceProfileManager::DeviceProfileManagerSingleton->GetPreviewDeviceProfile();
		if (PreviewDeviceProfile)
		{
			TTuple<EShaderPlatform, FName> PreviewShaderPlatformKey = MakeTuple(ScalabilityShaderPlatform, PreviewDeviceProfile->GetFName());

			DefaultPreviewShaderPlatformToCachedQualityLevels.FindOrAdd(PreviewShaderPlatformKey) = Scalability::GetQualityLevels();
			if (Scalability::FQualityLevels* CachedQualityLevel = PreviewShaderPlatformToCachedQualityLevels.Find(PreviewShaderPlatformKey))
			{
...
FQualityLevels GetDefaultQualityLevels()
{
	// We directly use UDeviceProfileManager::DeviceProfileManagerSingleton here so we don't unexpectedly initialize the device profile system during cooking.
	if (ScalabilityShaderPlatform != EShaderPlatform::SP_NumPlatforms && UDeviceProfileManager::DeviceProfileManagerSingleton)
	{
		...
		if (PreviewDeviceProfile)
		{
			if (Scalability::FQualityLevels* DefaultQualityLevels = DefaultPreviewShaderPlatformToCachedQualityLevels.Find(MakeTuple(ScalabilityShaderPlatform, PreviewDeviceProfile->GetFName())))
			{
				return *DefaultQualityLevels;
			}
		}
	}
	
	FQualityLevels QualityLevels;
	QualityLevels.SetFromSingleQualityLevelRelativeToMax(1);
	return QualityLevels;
}

void ResetQualityLevelsToDefault()
{
	SetQualityLevels(GetDefaultQualityLevels(), false);
}
#endif
...
void SetQualityLevels(const FQualityLevels& QualityLevels, bool bForce/* = false*/)
{
	...
	if (ScalabilityShaderPlatform != EShaderPlatform::SP_NumPlatforms && UDeviceProfileManager::DeviceProfileManagerSingleton)
	{
		...
		if (PreviewDeviceProfile)
		{
			PreviewShaderPlatformToCachedQualityLevels.FindOrAdd(MakeTuple(ScalabilityShaderPlatform, PreviewDeviceProfile->GetFName())) = ClampedLevels;
		}
	}
	bScalabilityShaderPlatformHasBeenChanged = false;
#endif
...
{
	LoadState(IniName, TEXT("ScalabilityGroups"));
}
...
{
	// Saves to the previous default
	SaveState(IniName, TEXT("ScalabilityGroups"));
}
