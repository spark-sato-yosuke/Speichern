...
void FLightMap2D::Serialize(FArchive& Ar)
{
	...
	const bool bUsingVTLightmaps = UseVirtualTextureLightmap(GMaxRHIShaderPlatform, Ar.CookingTarget());
...
FLightMapInteraction FLightMap2D::GetInteraction(ERHIFeatureLevel::Type InFeatureLevel) const
{
	...
	const bool bUseVirtualTextures = UseVirtualTextureLightmap(GetFeatureLevelShaderPlatform(InFeatureLevel));
	if (!bUseVirtualTextures)
	{
...
{
	return UseVirtualTextureLightmap(GetFeatureLevelShaderPlatform(GetFeatureLevel()));
}
