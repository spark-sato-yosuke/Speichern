...
static TAutoConsoleVariable<bool> CVarTransparentViewportClear(
	TEXT("r.TransparentViewportClear"),
	false,
	TEXT("If true, then the viewport will clear to transparent instead of black.\n")
	TEXT("This can be useful with r.PostProcessing.PropagateAlpha if the game view\n")
	TEXT("does not cover the whole window."),
	ECVF_Default);
...
static FAutoConsoleCommandWithWorld SplitScreenSwapControllersCommand
(
	TEXT("SplitScreen.RotateControllerIds"),
	TEXT("Rotates controller ids among gameplayers, useful for testing splitscreen with only one controller. Alias to 'SSSwapController'"),
	FConsoleCommandWithWorldDelegate::CreateLambda([](const UWorld* InWorld)
		{
			UGameViewportClient* Client = InWorld->GetGameViewport();
			if (!Client)
			{
				return;
			}
			Client->SSSwapControllers();
		})
);

static TAutoConsoleVariable<int32> CVarScreenshotTextureArrayMode(
	TEXT("r.Screenshot.TextureArrayMode"),
	0,
	TEXT("Choose how screenshots should be handled when the viewport is using a texture array. Collation is supported only for non-HDR, non-UI screenshots.")
	TEXT(" 0: Include only a single array slice (default, controlled by r.Screenshot.TextureArrayIndex)\n")
	TEXT(" 1: Collate all array slices horizontally (left-to-right)\n")
	TEXT(" 2: Collate all array slices vertically (top-to-bottom)\n")
	TEXT("When collating, the screenshot resolution is used for each individual array slice, so requesting a 400x400 screenshot from\n")
	TEXT("a viewport with two slices and then collating horizontally will yield a final screenshot that's 800x400."),
	ECVF_Default);

static TAutoConsoleVariable<int32> CVarScreenshotTextureArrayIndex(
	TEXT("r.Screenshot.TextureArrayIndex"),
	0,
	TEXT("Texture array slice to use for screenshots when r.Screenshot.TextureArrayMode = 0"),
	ECVF_Default);
...
bool UGameViewportClient::InputAxis(const FInputKeyEventArgs& InArgs)
{
	if (IgnoreInput())
	{
		return false;
	...
	// Handle mapping controller id and key if needed, which may need to mutate the args
	FInputKeyEventArgs MutableEventArgs = InArgs;

	FViewport* InViewport = MutableEventArgs.Viewport;
	const bool bGamepad = MutableEventArgs.IsGamepad();
	float Delta = MutableEventArgs.AmountDepressed;

	RemapControllerInput(MutableEventArgs);
	...
	if (bGamepad && InViewport)
	{
		...
		if (InViewportClient == this)
		{
			...
			if (CanRouteGamepadToSecondWindow && CanRunUnderOneProcess && InViewport->IsPlayInEditorViewport())
			{
				if (UGameViewportClient* NextViewport = GEngine->GetNextPIEViewport(this))
				{
					const bool bResult = NextViewport->InputAxis(MutableEventArgs);
					return false;
				}
			}
		}
	...
	OnInputAxisEvent.Broadcast(InViewport, MutableEventArgs.ControllerId, MutableEventArgs.Key, Delta, MutableEventArgs.DeltaTime, MutableEventArgs.NumSamples, MutableEventArgs.IsGamepad());
	...
	if (InViewport)
	{
		if( !( InViewport->IsSlateViewport() && InViewport->IsPlayInEditorViewport() ) || ViewportConsole == nullptr || !ViewportConsole->ConsoleActive() )
		{
			...
			{
				bResult = ViewportConsole->InputAxis(MutableEventArgs.InputDevice, MutableEventArgs.Key, Delta, MutableEventArgs.DeltaTime, MutableEventArgs.NumSamples, MutableEventArgs.IsGamepad());
			}
		
			// Try the override callback, this may modify event args
			if (!bResult && OnOverrideInputAxisEvent.IsBound())
			{
				bResult = OnOverrideInputAxisEvent.Execute(MutableEventArgs, Delta, MutableEventArgs.DeltaTime, MutableEventArgs.NumSamples);
			}

			if (!bResult)
			{
				ULocalPlayer* const TargetPlayer = GEngine->GetLocalPlayerFromInputDevice(this, MutableEventArgs.InputDevice);
				if (TargetPlayer && TargetPlayer->PlayerController)
				{
					bResult = TargetPlayer->PlayerController->InputKey(MutableEventArgs);
				}
			}
...
bool UGameViewportClient::InputTouch(
	...
	const uint64 Timestamp)
{
	...
	// route to subsystems that care
	bool bResult = (ViewportConsole ? ViewportConsole->InputTouch(DeviceId, Handle, Type, TouchLocation, Force, Timestamp, TouchpadIndex) : false);
	if (!bResult)
	{
		ULocalPlayer* const TargetPlayer = GEngine->GetLocalPlayerFromInputDevice(this, DeviceId);
		if (TargetPlayer && TargetPlayer->PlayerController)
		{
...
bool UGameViewportClient::InputMotion(FViewport* InViewport, const FInputDeviceId DeviceId, const FVector& Tilt, const FVector& RotationRate, const FVector& Gravity, const FVector& Acceleration, const uint64 Timestamp)
{
	...
	if (TargetPlayer && TargetPlayer->PlayerController)
	{
		bResult = TargetPlayer->PlayerController->InputMotion(DeviceId, Tilt, RotationRate, Gravity, Acceleration, Timestamp);
	}
...
void UGameViewportClient::SetDropDetail(float DeltaSeconds)
{
	if (GEngine && GetWorld())
	{
		...
		{
			FrameTime = FPlatformTime::ToSeconds(FMath::Max3<uint32>(GRenderThreadTime, GGameThreadTime, RHIGetGPUFrameCycles()));
...
void UGameViewportClient::Draw(FViewport* InViewport, FCanvas* SceneCanvas)
{
	...
	{
		const UE::StereoRenderUtils::FStereoShaderAspects Aspects(MyWorld->Scene->GetShaderPlatform());
		bRequireMultiView = Aspects.IsMobileMultiViewEnabled();
	}
		...
		if (LocalPlayer)
		{
			...
			for (int32 ViewIndex = 0; ViewIndex < NumViews; ++ViewIndex)
			{
				...
				if (View)
				{
					...
					IStreamingManager::Get().AddViewInformation(View->ViewMatrices.GetViewOrigin(), View->UnscaledViewRect.Width(), View->UnscaledViewRect.Width() * View->ViewMatrices.GetProjectionMatrix().M[0][0], StreamingScale, false, 0, nullptr, World);
	...
	bool bTransparentViewportClear = CVarTransparentViewportClear.GetValueOnAnyThread();
	if (ViewFamily.Views.Num() == 0 || TotalArea != (MaxX-MinX)*(MaxY-MinY) || bDisableWorldRendering || bStereoscopicPass || bTransparentViewportClear)
	{
		if (bDisableWorldRendering || !bStereoscopicPass || bTransparentViewportClear) // TotalArea computation does not work correctly for stereoscopic views
		{
	...
	MyWorld->FlushLineBatchers(LineBatchersToFlush);
...
bool UGameViewportClient::ProcessScreenShots(FViewport* InViewport)
{
	...
	if (GIsDumpingMovie || FScreenshotRequest::IsScreenshotRequested() || GIsHighResScreenshot)
	{
		...
		else if (InViewport->GetShaderResourceTexture() && InViewport->GetShaderResourceTexture()->GetDesc().Dimension == ETextureDimension::Texture2DArray)
		{
			// Handle Texture2DArray, we need to determine which array slices to include
			FReadSurfaceDataFlags ReadSurfaceFlags = FReadSurfaceDataFlags();
			const int32 ScreenshotTextureArrayMode = CVarScreenshotTextureArrayMode.GetValueOnAnyThread();
			const int32 ArraySize = InViewport->GetShaderResourceTexture()->GetDesc().ArraySize;

			ensureAlwaysMsgf(ScreenshotTextureArrayMode >= 0 && ScreenshotTextureArrayMode < 3, TEXT("r.Screenshot.TextureArrayMode %d is not a valid mode, defaulting to mode 0"), ScreenshotTextureArrayMode);

			// Screenshot all slices, then collate horizontally or vertically
			if (ScreenshotTextureArrayMode == 1 || ScreenshotTextureArrayMode == 2)
			{
				TArray<TArray<FColor>> BitmapArray;
				BitmapArray.SetNum(ArraySize);

				for (int CurrentArrayIndex = 0; CurrentArrayIndex < ArraySize; CurrentArrayIndex++)
				{
					ReadSurfaceFlags.SetArrayIndex(CurrentArrayIndex);
					bScreenshotSuccessful = GetViewportScreenShot(InViewport, BitmapArray[CurrentArrayIndex], FIntRect(0, 0, Size.X, Size.Y), ReadSurfaceFlags);
				...
				if (bScreenshotSuccessful)
				{
					const int32 NumPixelsPerSlice = Size.X * Size.Y;
					const int32 NumPixelsTotal = NumPixelsPerSlice * ArraySize;

					Bitmap.Reset();
					Bitmap.SetNumUninitialized(NumPixelsTotal);

					// Horizontal collation
					if (ScreenshotTextureArrayMode == 1)
					{
						for (int CurrentPixel = 0; CurrentPixel < NumPixelsTotal; CurrentPixel++)
						{
							// Complex case, need to alternate between array entries for each horizontal line
							const int32 CurrentRow = CurrentPixel / (ArraySize * Size.X);
							const int32 CurrentColumn = CurrentPixel % Size.X;
								 
							const int32 ArrayIndex = (CurrentPixel / Size.X) % ArraySize;
							const int32 PixelIndex = CurrentColumn + CurrentRow * Size.X;
							Bitmap[CurrentPixel] = BitmapArray[ArrayIndex][PixelIndex];
						}
					...
					else
					{
						for (int CurrentPixel = 0; CurrentPixel < NumPixelsTotal; CurrentPixel++)
						{
							// Simple case, just concatenate all bitmaps
							const int32 ArrayIndex = CurrentPixel / NumPixelsPerSlice;
							const int32 PixelIndex = CurrentPixel % NumPixelsPerSlice;
							Bitmap[CurrentPixel] = BitmapArray[ArrayIndex][PixelIndex];
						}
			...
			// Screenshot a specific array slice (ScreenshotTextureArrayMode == 0 or other invalid values)
			else
			{
				int32 ArrayIndex = CVarScreenshotTextureArrayIndex.GetValueOnAnyThread();
				const int32 ClampedArrayIndex = FMath::Clamp(ArrayIndex, 0, ArraySize - 1);

				if (ArrayIndex != ClampedArrayIndex)
				{
					ensureAlwaysMsgf(false, TEXT("r.Screenshot.TextureArrayIndex %d is out of range for the current viewport, which has %d texture array slices, defaulting to slice %d"), ArrayIndex, ArraySize, ClampedArrayIndex);
					ArrayIndex = ClampedArrayIndex;
				}

				ReadSurfaceFlags.SetArrayIndex(ArrayIndex);
				bScreenshotSuccessful = GetViewportScreenShot(InViewport, Bitmap, FIntRect(0, 0, Size.X, Size.Y), ReadSurfaceFlags);
			}
		}
		else
		{
...
bool UGameViewportClient::HandleViewModeCommand( const TCHAR* Cmd, FOutputDevice& Ar, UWorld* InWorld )
{
	...
	SetViewMode((EViewModeIndex)ViewModeIndex);
...
bool UGameViewportClient::HandleKillParticlesCommand( const TCHAR* Cmd, FOutputDevice& Ar )
{
	...
	if( !GIsEditor )
	{
		extern ENGINE_API bool GIsAllowingParticles;
...
void UGameViewportClient::SetViewMode(EViewModeIndex ViewMode, FOutputDevice* Output)
{
	ViewModeIndex = ViewMode;

	if (ViewModeIndex == VMI_StationaryLightOverlap)
	{
		if (Output)
		{
			Output->Logf(TEXT("This view mode is currently not supported in game."));
		}
	...
	if (FPlatformProperties::SupportsWindowedMode() == false)
	{
			...
			|| ViewModeIndex == VMI_ReflectionOverride)
		{
			if (Output)
			{
				Output->Logf(TEXT("This view mode is currently not supported on consoles."));
			}
			ViewModeIndex = VMI_Lit;
		}
	}
	...
	if (Output)
	{
		Output->Logf(TEXT("Debug viewmodes not allowed in Test or Shipping builds."));
	}
	ViewModeIndex = VMI_Lit;
#else
	...
	if ((ViewModeIndex != VMI_Lit && ViewModeIndex != VMI_ShaderComplexity) && !AllowDebugViewmodes())
	{
		if (Output)
		{
			Output->Logf(TEXT("Debug viewmodes not allowed on consoles by default.  See AllowDebugViewmodes()."));
		}
		ViewModeIndex = VMI_Lit;
	}
	...
	if (!GRHISupportsRayTracing || !GRHISupportsRayTracingShaders)
	{
		if (ViewModeIndex == VMI_PathTracing)
		{
			if (Output)
			{
				Output->Logf(TEXT("Path Tracing view mode requires ray tracing support. It is not supported on this system."));
			}
			ViewModeIndex = VMI_Lit;
		}
		...
		if (ViewModeIndex == VMI_RayTracingDebug)
		{
			if (Output)
			{
				Output->Logf(TEXT("Ray tracing view mode requires ray tracing support. It is not supported on this system."));
			}
			ViewModeIndex = VMI_Lit;
		}
	}
