...
static FAutoConsoleVariableRef CVarWithObjectValidationCheck(TEXT("net.WithObjectValidationCheck"), GbWithObjectValidationCheck, TEXT("If true, will run object property validation on property storage."));
...
namespace UE_RepLayout_Private
{
	...
	static TCopyQualifiersFromTo_T<BufferType, OutputType>*
	GetTypedProperty(const BufferType& Buffer, const CommandType& Cmd)
	{
		using ConstOrNotOutputType = TCopyQualifiersFromTo_T<BufferType, OutputType>;
	...
	static TCopyQualifiersFromTo_T<typename TRepDataBufferBase<BufferDataType, BufferUnderlyingType>::ConstOrNotVoid, OutputType>*
	GetTypedProperty(const TRepDataBufferBase<BufferDataType, BufferUnderlyingType>& Buffer, const CommandType& Cmd)
	{
		using ConstOrNotOutputType = TCopyQualifiersFromTo_T<typename TRepDataBufferBase<BufferDataType, BufferUnderlyingType>::ConstOrNotVoid, OutputType>;
	...
	const void* 			B)
{
	...
	const FObjectPropertyBase* ObjProperty = CastFieldChecked<FObjectPropertyBase>(Cmd.Property);

	// With remote object handles, use TObjectPtrs so that the objects don't have to be resolved.
	// Since that's not safe for GC'd objects, use raw pointers otherwise.
#if UE_WITH_REMOTE_OBJECT_HANDLE
	const TObjectPtr<UObject> ObjectA = ObjProperty->GetObjectPtrPropertyValue(A);
	const TObjectPtr<UObject> ObjectB = ObjProperty->GetObjectPtrPropertyValue(B);
#else
	const UObject* ObjectA = ObjProperty->GetObjectPropertyValue(A);
	const UObject* ObjectB = ObjProperty->GetObjectPropertyValue(B);
#endif
...
static FORCEINLINE void StoreProperty(const FRepLayoutCmd& Cmd, void* A, const void* B)
{
	...
	if (GbWithObjectValidationCheck)
	{
		...
		{
			if (const FObjectPropertyBase* ObjProperty = CastFieldChecked<FObjectPropertyBase>(Cmd.Property))
			{
				const UObject* AObjValue = ObjProperty->GetObjectPropertyValue(A);
				if (AObjValue && !IsValid(AObjValue))
				{
					UE_LOG(LogRepCompares, Warning, TEXT("Destination Object %s was not valid when storing for replication. StoreProperty: %s"), *GetNameSafe(AObjValue), *GetFullNameSafe(Cmd.Property));
				}

				const UObject* BObjValue = ObjProperty->GetObjectPropertyValue(B);
				if (BObjValue && !IsValid(BObjValue))
				{
					UE_LOG(LogRepCompares, Warning, TEXT("Source Object %s was not valid when storing for replication. StoreProperty: %s"), *GetNameSafe(BObjValue), *GetFullNameSafe(Cmd.Property));
				}
			}
		}
	}
	...
	UObject* OwningObject) const
{
	...
	while (true)
	{
		...
		if (Cmd.Type == ERepLayoutCmdType::DynamicArray)
		{
			...
			{
				UE_LOG(LogRep, Warning, TEXT("ReceiveProperties_BackwardsCompatible_r: Array didn't read proper number of bits. Owner: %s, Name: %s, NetFieldExportHandle: %i, Checksum: %u, BitsLeft:%" INT64_FMT), *Owner->GetName(), *NetFieldExportGroup->NetFieldExports[NetFieldExportHandle].ExportName.ToString(), NetFieldExportHandle, Checksum, TempReader.GetBitsLeft());
				return false;
			}
		}
		else
	...
	const ECreateRepLayoutFlags CreateFlags)
{
	...
	// The list of all replicated properties
	TArray<FLifetimeProperty> LifetimeProps;
	LifetimeProps.Reserve(Parents.Num());
	
	// Ask the user code for explicit settings
	UObject* Object = InObjectClass->GetDefaultObject();
	Object->GetLifetimeReplicatedProps(LifetimeProps);

	// Set default settings for properties not explicitely registered
	{
		UE::Net::FNetBitArray RegisteredProperties(InObjectClass->ClassReps.Num());
		for (const FLifetimeProperty& UserProp : LifetimeProps)
		{
			RegisteredProperties.SetBit(UserProp.RepIndex);
		}

		RegisteredProperties.ForAllUnsetBits([&LifetimeProps, InObjectClass](uint32 Index)
		{
			const uint32 PropIndex = LifetimeProps.Emplace(FLifetimeProperty(Index));

			// Check if the registration is missing if we don't automatically replicate unregistered variables
			if (!UE::Net::bAutoRegisterReplicatedProperties)
			{
				LifetimeProps[PropIndex].Condition = COND_Never;
	...
	if (UE::Net::bReplicateCustomDeltaPropertiesInRepIndexOrder && (HighestCustomDeltaRepIndex != INDEX_NONE))
	{
	...
	// Use default settings when auto-registering properties
	FLifetimeProperty DefaultLifetimeProp;

	// Setup lifetime replicated properties
	for (int32 ParentIndex=0; ParentIndex < InObjectClass->ClassReps.Num(); ++ParentIndex)
	{
		const FRepRecord& ReplicatedProp = InObjectClass->ClassReps[ParentIndex];
	
		const bool bIsValid = ensureMsgf(Parents.IsValidIndex(ParentIndex), TEXT("ClassReps RepRecord of %s point to invalid RepIndex=%i for replicated property: %s. Parents.Num() = %d"), *GetFullNameSafe(InObjectClass), ParentIndex, *GetNameSafe(ReplicatedProp.Property), Parents.Num());
		if (!bIsValid)
		{
			continue;
		}

		FRepParentCmd& RepParentCmd = Parents[ParentIndex];

		// Find a lifetime condition defined for this replicated property
		const FLifetimeProperty* UserLifetimeProp = LifetimeProps.FindByPredicate([ParentIndex](const FLifetimeProperty& Item)
		{
			return Item.RepIndex == ParentIndex;
		});

		

		// Set the correct RepIndex just in case its refereced.
		DefaultLifetimeProp.RepIndex = ParentIndex;

		// Use Default settings if the user didn't 
		const FLifetimeProperty& LifetimeProp = UserLifetimeProp ? *UserLifetimeProp : DefaultLifetimeProp;

		// Don't bother doing any setup work for COND_Never properties.
		// These are never expected to replicate.
		if (COND_Never == LifetimeProp.Condition)
		{
			continue;
		}

		// Store the condition on the parent in case we need it
		RepParentCmd.Condition = LifetimeProp.Condition;
		RepParentCmd.RepNotifyCondition = LifetimeProp.RepNotifyCondition;

		if (UFunction* RepNotifyFunc = InObjectClass->FindFunctionByName(RepParentCmd.Property->RepNotifyFunc))
		{
			RepParentCmd.RepNotifyNumParams = RepNotifyFunc->NumParms;
		}

		if (!EnumHasAnyFlags(RepParentCmd.Flags, ERepParentFlags::IsCustomDelta))
		{
			RepParentCmd.Flags |= ERepParentFlags::IsLifetime;
			if (RepParentCmd.Condition == COND_None)
			{
				RepParentCmd.Flags &= ~ERepParentFlags::IsConditional;
			}
			else if (RepParentCmd.Condition == COND_InitialOnly)
			{
				Flags |= ERepLayoutFlags::HasInitialOnlyProperties;
			}
			else if (RepParentCmd.Condition == COND_Dynamic)
			{
				Flags |= ERepLayoutFlags::HasDynamicConditionProperties;
			}

			if (EnumHasAnyFlags(RepParentCmd.Flags, ERepParentFlags::HasNetSerializeProperties | ERepParentFlags::HasObjectProperties))
			{
...
#if WITH_PUSH_MODEL
			if (bIsPushModelEnabled && LifetimeProp.bIsPushBased)
			{
		...
		else
		{
			...
			if (EnumHasAnyFlags(RepParentCmd.Flags, ERepParentFlags::IsFastArray))
			{
				int32 FastArrayItemArrayCmd = INDEX_NONE;

				for (int32 CmdIndex = RepParentCmd.CmdStart; CmdIndex < RepParentCmd.CmdEnd; ++CmdIndex)
				{
					const FRepLayoutCmd& Cmd = Cmds[CmdIndex];
					if (ERepLayoutCmdType::DynamicArray == Cmd.Type)
					{
				...
				if (!bAddedFastArray)
				{
					UE_LOG(LogRep, Warning, TEXT("FRepLayout::InitFromClass: Unable to find Fast Array Item array in Fast Array Serializer: %s"), *RepParentCmd.CachedPropertyName.ToString());
				}
			}
