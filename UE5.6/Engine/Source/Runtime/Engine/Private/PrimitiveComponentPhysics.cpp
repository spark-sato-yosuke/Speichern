...
void UPrimitiveComponent::RegisterAsFocalPointInPhysicsReplicationLOD() const
{
	if (UWorld* World = GetWorld())
	{
		if (FPhysScene* PhysScene = World->GetPhysicsScene())
		{
			if (IPhysicsReplicationLOD* PhysicsReplicationLOD = PhysScene->GetPhysicsReplicationLOD())
			{
				PhysicsReplicationLOD->RegisterFocalPoint_External(this);
			}
		}
	}
}

void UPrimitiveComponent::UnregisterAsFocalPointInPhysicsReplicationLOD() const
{
	if (UWorld* World = GetWorld())
	{
		if (FPhysScene* PhysScene = World->GetPhysicsScene())
		{
			if (IPhysicsReplicationLOD* PhysicsReplicationLOD = PhysScene->GetPhysicsReplicationLOD())
			{
				PhysicsReplicationLOD->UnregisterFocalPoint_External(this);
			}
		}
	}
}
...
void UPrimitiveComponent::SetConstraintMode(EDOFMode::Type ConstraintMode)
{
	...
	if (RootBI == NULL || !IsValidChecked(this))
	{
		return;
	}
...
void UPrimitiveComponent::SetGyroscopicTorqueEnabled(bool bEnable)
{
	FBodyInstance* BI = GetBodyInstance();
	if (BI)
	{
		BI->SetGyroscopicTorqueEnabled(bEnable);
	}
}

bool UPrimitiveComponent::GetGyroscopicTorqueEnabled() const
{
	FBodyInstance* BI = GetBodyInstance();
	if (BI)
	{
		return BI->bGyroscopicTorqueEnabled;
	}
	return false;
}
...
void UPrimitiveComponent::SyncComponentToRBPhysics()
{
	...
	if (!IsValidChecked(this) || !IsSimulatingPhysics() || !RigidBodyIsAwake())
	{
		return;
	}
...
UPrimitiveComponent * GetRootWelded(const UPrimitiveComponent* PrimComponent, FName ParentSocketName = NAME_None, FName* OutSocketName = NULL, bool bAboutToWeld = false)
{
	...
	for (; RootComponent; RootComponent = Cast<UPrimitiveComponent>(RootComponent->GetAttachParent()))
	{
		...
		if (RootBI && (!RootBI->IsValidBodyInstance() || RootBI->WeldParent != nullptr))
		{
			continue;
		}
...
bool UPrimitiveComponent::WeldToImplementation(USceneComponent * InParent, FName ParentSocketName /* = Name_None */, bool bWeldSimulatedChild /* = true */, bool bWeldToKinematicParent /* = false */)
{
	...
	if (RootComponent)
	{
		...
		if (!RootBI && RootComponent->IsA<USkeletalMeshComponent>())
		{
			USkeletalMeshComponent* RootSkeletal = Cast<USkeletalMeshComponent>(RootComponent);
		...
		if (RootBI)
		{
			...
			if ((RootComponent->IsSimulatingPhysics(BoneName) == false) && (bWeldToKinematicParent == false))
			{
