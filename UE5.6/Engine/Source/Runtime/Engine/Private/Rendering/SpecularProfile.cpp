...
public:
	...
	class FReadWriteOnArray : SHADER_PERMUTATION_BOOL("PERMUTATION_READ_WRITE_ON_ARRAY");
	using FPermutationDomain = TShaderPermutationDomain<FReadWriteOnArray>;

	BEGIN_SHADER_PARAMETER_STRUCT(FParameters, )
		SHADER_PARAMETER(uint32, SampleCount)
		SHADER_PARAMETER(FIntPoint, Resolution)
		SHADER_PARAMETER(uint32, ProfileCount)
		SHADER_PARAMETER_RDG_TEXTURE(Texture2DArray, SourceTexture)
...
IPooledRenderTarget* FSpecularProfileTextureManager::GetAtlasTexture(FRDGBuilder& GraphBuilder, EShaderPlatform ShaderPlatform)
{
	...
	if (GSpecularProfileTextureAtlas == nullptr)
	{
		...
		const bool bR11G11B10Supported = GPixelFormats[PF_FloatR11G11B10].Supported && UE::PixelFormat::HasCapabilities(PF_FloatR11G11B10, EPixelFormatCapabilities::TextureFilterable);
		const EPixelFormat Format = bR11G11B10Supported ? PF_FloatR11G11B10 : PF_FloatRGBA;

		FPooledRenderTargetDesc Desc(FPooledRenderTargetDesc::Create2DArrayDesc(FIntPoint(Resolution, Resolution), Format, FClearValueBinding::None, TexCreate_None, TexCreate_UAV | TexCreate_ShaderResource, false, LayerCount * SUBSTRATE_SPECULAR_PROFILE_ENTRY_COUNT));
		...
		if (bForceUpdate)
		{
			AddClearUAVPass(GraphBuilder, GraphBuilder.CreateUAV(SpecularProfileTexture), 0.f);
		}
		...
		if (LayerCount > 0)
		{
			// Certain platform don't support read/write on the same texture array, even if this done safely on different slices
			const bool bPlatformSupportReadWriteOnTextureArray = !IsOpenGLPlatform(ShaderPlatform);
			FRDGTextureRef SourceSpecularProfileTexture = nullptr;
			if (!bPlatformSupportReadWriteOnTextureArray)
			{
				const FIntPoint Extent = SpecularProfileTexture->Desc.Extent;
				SourceSpecularProfileTexture = GraphBuilder.CreateTexture(SpecularProfileTexture->Desc, TEXT("ConvolvedSpecularProfileTexture"));
				AddCopyTexturePass(GraphBuilder, SpecularProfileTexture, SourceSpecularProfileTexture);
			}
			...
			PassParameters->SourceTexture = SourceSpecularProfileTexture;
			PassParameters->TargetTexture = GraphBuilder.CreateUAV(SpecularProfileTexture);

			FSpecularProfileConvolveCS::FPermutationDomain PermutationVector;
			PermutationVector.Set<FSpecularProfileConvolveCS::FReadWriteOnArray>(PassParameters->SourceTexture == nullptr);
			TShaderMapRef<FSpecularProfileConvolveCS> Shader(GlobalShaderMap, PermutationVector);
			FComputeShaderUtils::AddPass(GraphBuilder, 
