...

	{
		...
		for (const FSpatialHashStreamingGridLevel& GridLevel : GridLevels)
		{
			...
			for (const FSpatialHashStreamingGridLayerCell& LayerCell : GridLevel.LayerCells)
			{
				...
				{
					if (!Cell->IsAlwaysLoaded() || !IsRunningCookCommandlet())
					{
						Cells.Add(Cell);
					}
				}
			...
			Cells.Sort([this](const UWorldPartitionRuntimeCell& A, const UWorldPartitionRuntimeCell& B)
			{
...
void FSpatialHashStreamingGrid::DrawStreamingSource3D(UWorld* World, const FSphericalSector& InShape, const FTransform& InTransform, const FColor& InColor) const
{
	...
	{
		if (ULineBatchComponent* const LineBatcher = World->GetLineBatcher(UWorld::ELineBatcherType::World))
		{
...
void ASpatialHashRuntimeGridInfo::Serialize(FArchive& Ar)
{
...
#if WITH_EDITORONLY_DATA
	if (Ar.IsLoading() && Ar.CustomVer(FFortniteMainBranchObjectVersion::GUID) < FFortniteMainBranchObjectVersion::SpatialHashRuntimeGridInfoSpriteFixup)
	{
		// Fixes ASpatialHashRuntimeGridInfo that were saved prior to the constructor using ObjectInitializer.DoNotCreateDefaultSubobject(TEXT("Sprite"))
		...
		FArchiveReplaceObjectRef<UObject>(this, { { GetSpriteComponent(), nullptr } });
	}
#endif
}
	...
	for (const FSquare2DGridHelper::FGridLevel& TempLevel : PartitionedActors.Levels)
	{
		...
		for (const auto& TempCellMapping : TempLevel.CellsMapping)
		{
			...
			for (const FSquare2DGridHelper::FGridLevel::FGridCellDataChunk& GridCellDataChunk : TempCell.GetDataChunks())
			{
				...
				const bool bIsCellSpatiallyLoaded = &TempCell != &PartitionedActors.GetAlwaysLoadedCell();
				const bool bIsCellAlwaysLoaded = !bIsCellSpatiallyLoaded && !GridCellDataChunk.HasDataLayers() && !GridCellDataChunk.GetContentBundleID().IsValid();				
				...
				if (PopulateCellActorInstances(GridCellDataChunk.GetActorSetInstances(), bIsMainWorldPartition, bIsCellAlwaysLoaded, FilteredActors))
				{
					...
					StreamingCell->SetIsSpatiallyLoaded(bIsCellSpatiallyLoaded);
					...
					const int32 DataLayersStreamingPriority = GetDataLayersStreamingPriority(DataLayers);
					CellDataSpatialHash->Priority = RuntimeGrid.Priority + DataLayersStreamingPriority;
...
FName UWorldPartitionRuntimeSpatialHash::GetDefaultGrid() const
{
	return Grids.Num() ? Grids[0].GridName : NAME_None;
}
...
ENGINE_API EWorldPartitionStreamingPerformance UWorldPartitionRuntimeSpatialHash::GetStreamingPerformanceForCell(const UWorldPartitionRuntimeCell* Cell, bool& bOutShouldBlock) const
{
	...
	if (ensure(StreamingGrid))
	{
		...
		const double OnSlowStreamingWarningRatio = GSlowStreamingRatio * GSlowStreamingWarningFactor;
		...
		if (Cell->GetBlockOnSlowLoading() && CellDataSpatialHash->bCachedWasRequestedByBlockingSource)
		{
			const double Distance = FMath::Sqrt(CellDataSpatialHash->CachedMinSquareDistanceToBlockingSource) - (double((int64)StreamingGrid->CellSize << (int64)CellDataSpatialHash->HierarchicalLevel) / 2);
			...
			else if (Ratio < GBlockOnSlowStreamingRatio)
			{
		...
		const double Distance = FMath::Sqrt(CellDataSpatialHash->CachedMinSquareDistanceToSource) - (double((int64)StreamingGrid->CellSize << (int64)CellDataSpatialHash->HierarchicalLevel) / 2);
		...
		else if (Ratio < GSlowStreamingRatio)
		{
			NonBlockingPerformance = EWorldPartitionStreamingPerformance::Critical;
		}
		else if (Ratio < OnSlowStreamingWarningRatio)
		{
...
FGuid UWorldPartitionRuntimeSpatialHash::RegisterWorldAssetStreaming(const UWorldPartition::FRegisterWorldAssetStreamingParams& InParams)
{
	...
	const FSpatialHashStreamingGrid* TargetGrid = GetStreamingGridByName(InParams.WorldAssetDesc.TargetGrid);
	if (!TargetGrid)
	{
	...
	TArray<TPair<const FSpatialHashStreamingGrid*, const TSoftObjectPtr<UWorld>>> HLODTargetGrids;
	for (const UWorldPartition::FRegisterWorldAssetStreamingParams::FWorldAssetDesc& HLODWorldAssetDesc : InParams.HLODWorldAssetDescs)
	{
		...
		{
			const FSpatialHashStreamingGrid* HLODTargetGrid = GetStreamingGridByName(HLODWorldAssetDesc.TargetGrid);
			if (!HLODTargetGrid)
			{
			...
			{
				HLODTargetGrids.Add({ HLODTargetGrid, HLODWorldAssetDesc.WorldAsset });
			}
		}
	}
	...
	auto CreateStreamingCell = [&InParams, &SourceCellGuid, StreamingObject, this](const FSpatialHashStreamingGrid* SourceGrid, const TSoftObjectPtr<UWorld>& WorldAsset, bool bIsHLODPass) -> bool
	{
		check(SourceGrid);
		...
		// Create Cell
		FName TargetGridName = SourceGrid->GridName;
	...
	if (CreateStreamingCell(TargetGrid, InParams.WorldAssetDesc.WorldAsset, false /* bisHLODPass */) == false)
	{
		return FGuid();
	}
	for (const TPair<const FSpatialHashStreamingGrid*, const TSoftObjectPtr<UWorld>>& HLODTargetGrid : HLODTargetGrids)
	{
		if (CreateStreamingCell(HLODTargetGrid.Key, HLODTargetGrid.Value, true /* bIsHLODPass */) == false)
		{
			return FGuid();
		}
	}
