...
{
	TArray<const UDataLayerInstance*> DataLayersNotLoadedInEditor;
	TArray<const UDataLayerInstance*> DataLayersLoadedInEditor;
...
void UDataLayerManager::GetUserLoadedInEditorStates(TArray<const UDataLayerInstance*>& OutDataLayersLoadedInEditor, TArray<const UDataLayerInstance*>& OutDataLayersNotLoadedInEditor) const
{
	OutDataLayersLoadedInEditor.Empty();
	OutDataLayersNotLoadedInEditor.Empty();

	const TArray<const UDataLayerInstance*>& SettingsDataLayersNotLoadedInEditor = GetDefault<UWorldPartitionEditorPerProjectUserSettings>()->GetWorldDataLayersNotLoadedInEditor(GetWorld());
	const TArray<const UDataLayerInstance*>& SettingsDataLayersLoadedInEditor = GetDefault<UWorldPartitionEditorPerProjectUserSettings>()->GetWorldDataLayersLoadedInEditor(GetWorld());
	...
	ForEachDataLayerInstance([&OutDataLayersLoadedInEditor, &OutDataLayersNotLoadedInEditor, &SettingsDataLayersNotLoadedInEditor, &SettingsDataLayersLoadedInEditor](UDataLayerInstance* DataLayerInstance)
	{
		if (DataLayerInstance->IsLoadedInEditorChangedByUserOperation())
		{
			...
			{
				OutDataLayersNotLoadedInEditor.Add(DataLayerInstance);
			}
			else if (DataLayerInstance->IsLoadedInEditor() && !DataLayerInstance->IsInitiallyLoadedInEditor())
			{
				OutDataLayersLoadedInEditor.Add(DataLayerInstance);
			}
		...
		{
			if (SettingsDataLayersNotLoadedInEditor.Contains(DataLayerInstance))
			{
				OutDataLayersNotLoadedInEditor.Add(DataLayerInstance);
			}
			else if (SettingsDataLayersLoadedInEditor.Contains(DataLayerInstance))
			{
				OutDataLayersLoadedInEditor.Add(DataLayerInstance);
			}
		}
