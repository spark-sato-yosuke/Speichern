			...
			for(TObjectIterator<UInstancedStaticMeshComponent> It; It; ++It)
			{
				...
				if(ISMComponent->ShouldCreatePhysicsState())
				{
					...
					for(const FBodyInstance* Body : Bodies)
					{
						...
						FPhysicsCommand::ExecuteRead(Body->GetPhysicsActor(), [&Body, &Shapes, &NumInstanceShapes](const FPhysicsActorHandle& Actor)
						{
							Shapes.Reset();
...
void FInstancedStaticMeshVertexFactory::InitInstancedStaticMeshVertexFactoryComponents(
...
void FInstancedStaticMeshRenderData::BindBuffersToVertexFactories(FRHICommandListBase& RHICmdList, FStaticMeshInstanceBuffer* InstanceBuffer)
{
	...
	for (int32 LODIndex = 0; LODIndex < VertexFactories.Num(); LODIndex++)
	{
		...
		FInstancedStaticMeshVertexFactory::InitInstancedStaticMeshVertexFactoryComponents(RenderData->VertexBuffers, ColorVertexBuffer, InstanceBuffer, &VertexFactory, LightMapCoordinateIndex, bRHISupportsManualVertexFetch, Data, InstanceData);
...
static uint32 GetTypeHash(const FInstancedStaticMeshVFLooseUniformShaderParameters& Params)
{
	uint32 HashParameters = 0;
	HashParameters = HashCombine(HashParameters, GetTypeHash(Params.InstancingFadeOutParams));
	HashParameters = HashCombine(HashParameters, GetTypeHash(Params.InstancingTranslatedWorldViewOriginOne));
	HashParameters = HashCombine(HashParameters, GetTypeHash(Params.InstancingTranslatedWorldViewOriginZero));
	HashParameters = HashCombine(HashParameters, GetTypeHash(Params.InstancingViewZCompareOne));
	HashParameters = HashCombine(HashParameters, GetTypeHash(Params.InstancingViewZCompareZero));
	HashParameters = HashCombine(HashParameters, GetTypeHash(Params.InstancingViewZConstant));
	return HashParameters;
}

static bool operator== (const FInstancedStaticMeshVFLooseUniformShaderParameters& A, const FInstancedStaticMeshVFLooseUniformShaderParameters& B)
{
	static_assert(sizeof(FInstancedStaticMeshVFLooseUniformShaderParameters) == 96, "FInstancedStaticMeshVFLooseUniformShaderParameters size has changed, make sure operator== is updated");
	
	return A.InstancingFadeOutParams == B.InstancingFadeOutParams
		&& A.InstancingTranslatedWorldViewOriginOne == B.InstancingTranslatedWorldViewOriginOne
		&& A.InstancingTranslatedWorldViewOriginZero == B.InstancingTranslatedWorldViewOriginZero
		&& A.InstancingViewZCompareOne == B.InstancingViewZCompareOne
		&& A.InstancingViewZCompareZero == B.InstancingViewZCompareZero
		&& A.InstancingViewZConstant == B.InstancingViewZConstant;
}

// For deduplicating FInstancedStaticMeshVFLooseUniformShaderParameters UBs
class FInstancedStaticMeshVFUniformsCache : public FRenderResource
{
	const static int32 MAX_CACHE_SIZE = 10;
	TMap<FInstancedStaticMeshVFLooseUniformShaderParameters, FInstancedStaticMeshVFLooseUniformShaderParametersRef> CachedParams;

public:
	FInstancedStaticMeshVFUniformsCache()
		: FRenderResource()
	{
	}
	...
	FInstancedStaticMeshVFLooseUniformShaderParametersRef CreateUniformBuffer(const FInstancedStaticMeshVFLooseUniformShaderParameters& Params, EUniformBufferUsage UniformBufferUsage)
	{
		LLM_SCOPE(ELLMTag::InstancedMesh);

		check(IsInRenderingThread());

		FInstancedStaticMeshVFLooseUniformShaderParametersRef* CachedUB = CachedParams.Find(Params);
		if (CachedUB)
		{
			return *CachedUB;
		}

		FInstancedStaticMeshVFLooseUniformShaderParametersRef UB = CreateUniformBufferImmediate(Params, UniformBufferUsage);
		if (CachedParams.Num() < MAX_CACHE_SIZE && UniformBufferUsage == UniformBuffer_MultiFrame)
		{
			CachedParams.Add(Params, UB);
		}
...
static TGlobalResource<FInstancedStaticMeshVFUniformsCache> GInstancedStaticMeshVFUniformsCache;
...
FInstancedStaticMeshVFLooseUniformShaderParametersRef FInstancedStaticMeshSceneProxy::CreateLooseUniformBuffer(const FSceneView* View, const FInstancingUserData* InstancingUserData, uint32 InstancedLODRange, uint32 InstancedLODIndex, EUniformBufferUsage UniformBufferUsage) const
{
...

	{
		...
		if (InstancingUserData && InstancedLODRange)
		{
			...
			FBoxSphereBounds ScaledBounds = InstancingUserData->MeshRenderData->Bounds.TransformBy(FTransform(FQuat::Identity, FVector::ZeroVector, InstancingUserData->AverageInstancesScale));
	...
	{
		return GInstancedStaticMeshVFUniformsCache.CreateUniformBuffer(LooseParameters, UniformBufferUsage);
	}
	else
	{
...
void FInstancedStaticMeshSceneProxyDesc::InitializeFromInstancedStaticMeshComponent(const UInstancedStaticMeshComponent* InComponent)
{
...
void FInstancedStaticMeshSceneProxy::SetupProxy(const FInstancedStaticMeshSceneProxyDesc& InProxyDesc)
{
	...
	bDoesMeshBatchesUseSceneInstanceCount = CVarISMFetchInstanceCountFromScene.GetValueOnAnyThread() != 0;
	...
	// TODO: when using WPO, the base class (FStaticMeshSceneProxy) currently creates DynamicRayTracingGeometries which is not actually used
	// because FInstancedStaticMeshSceneProxy creates it's own ray tracing geometries for each simulated instance... 
...
bool FInstancedStaticMeshSceneProxy::CanSetupInstancedMeshBatch(int32 LODIndex) const
{
	// TODO: If this fires somehow our LODLooseUniformBuffers created in CreateRenderThreadResources is empty, a full dmp should be investigated
	check(LODLooseUniformBuffers.Num() == InstancedRenderData.VertexFactories.Num());
...
void FInstancedStaticMeshSceneProxy::UpdateInstances_RenderThread(FRHICommandListBase& RHICmdList, const FBoxSphereBounds& InBounds, const FBoxSphereBounds& InLocalBounds)
{
	...
	return FPrimitiveSceneProxy::UpdateInstances_RenderThread(RHICmdList, InBounds, InLocalBounds);
}
...
{
	if (CanSetupInstancedMeshBatch(LODIndex) && FStaticMeshSceneProxy::GetShadowMeshElement(LODIndex, BatchIndex, InDepthPriorityGroup, OutMeshBatch, bDitheredLODTransition))
	{
...
{
	if (CanSetupInstancedMeshBatch(LODIndex) && FStaticMeshSceneProxy::GetMeshElement(LODIndex, BatchIndex, ElementIndex, InDepthPriorityGroup, bUseSelectionOutline, bAllowPreCulledIndices, OutMeshBatch))
	{
		SetupInstancedMeshBatch(LODIndex, BatchIndex, OutMeshBatch);
		return true;
	}
...
{
	if (CanSetupInstancedMeshBatch(LODIndex) && FStaticMeshSceneProxy::GetWireframeMeshElement(LODIndex, BatchIndex, WireframeRenderProxy, InDepthPriorityGroup, bAllowPreCulledIndices, OutMeshBatch))
	{
		SetupInstancedMeshBatch(LODIndex, BatchIndex, OutMeshBatch);
		return true;
	}
...
bool FInstancedStaticMeshSceneProxy::GetInstanceDrawDistanceMinMax(FVector2f& OutDistanceMinMax) const
{
	...
	{
		OutDistanceMinMax = FVector2f(float(UserData_AllInstances.MinDrawDistance), float(UserData_AllInstances.EndCullDistance));
		return true;
	}
	else
	{
...
void FInstancedStaticMeshSceneProxy::GetDynamicRayTracingInstances(FRayTracingInstanceCollector& Collector)
{
...
#if DO_CHECK
	if (!ensureMsgf(bSupportRayTracing, TEXT("If proxy doesn't support ray tracing it should've been excluded earlier.")))
	{
		return;
	}
#endif
	...
	if (bHasWorldPositionOffset)
	{
		...
		for (int32 SectionIdx = 0; SectionIdx < SectionCount; ++SectionIdx)
		{
			//#dxr_todo: so far we use the parent static mesh path to get material data
			{
				...
				if (!GetMeshElement(LODIndex, 0, SectionIdx, 0, false, false, MeshBatch))
				{
		...
		SetupRayTracingDynamicInstances(Collector.GetRHICommandList(), SimulatedInstances, LODIndex);
	...
	if (InstanceSceneDataBuffers->IsInstanceDataGPUOnly())
	{
		// TODO: Update FInstancedStaticMeshSceneProxy to make it compatible with GPU Scene when not using WPO materials, and support fallback to static geometry
		// when IsInstanceDataGPUOnly() instead of simply being hidden from ray tracing.
		return;
	}
	...
	auto GetDistanceToInstance = [&ViewPosition, InstanceSceneDataBuffers](int32 InstanceIndex, float& OutInstanceRadius, float& OutDistanceToInstanceCenter, float& OutDistanceToInstanceStart)
	{
		...
		OutDistanceToInstanceStart = FMath::Abs(OutDistanceToInstanceCenter - OutInstanceRadius);
	};
	...
	if (CVarRayTracingRenderInstancesCulling.GetValueOnRenderThread() > 0)
	{
		...
		const float UserNearCullDistance = UserData_AllInstances.MinDrawDistance * MaxDrawDistanceScale;
		const float UserFarCullDistance = UserData_AllInstances.EndCullDistance * MaxDrawDistanceScale;
		if (CullAngle < 0.0f || UserFarCullDistance > 0.0f)
		{
			//
			...
			const float BVHFarCullRadius = FMath::Min(UserFarCullDistance > 0 ? UserFarCullDistance : Inf, CVarRayTracingInstancesCullClusterRadius.GetValueOnRenderThread());
			...
			const bool ApplyGeneralCulling = BVHFarCullRadius > 0.0f;
			...
			for (uint32 InstanceIndex = 0; InstanceIndex < InstanceCount; InstanceIndex++)
			{
				...
				if (((DistanceToInstanceCenter < UserNearCullDistance) || (DistanceToInstanceCenter > BVHFarCullRadius)) && ApplyGeneralCulling)
					continue;
	...
	if (RayTracingInstanceTemplate.PrimitiveInstanceIndices.Num() > 0)
	{
		...
		for (int32 SectionIdx = 0; SectionIdx < SectionCount; ++SectionIdx)
		{
			...
			bool bResult = GetMeshElement(LODIndex, 0, SectionIdx, 0, false, false, MeshBatch);
			if (!bResult)
			{
				// Hidden material
				MeshBatch.MaterialRenderProxy = UMaterial::GetDefaultMaterial(MD_Surface)->GetRenderProxy();
...
void FInstancedStaticMeshSceneProxy::SetupRayTracingDynamicInstances(FRHICommandListBase& RHICmdList, int32 NumDynamicInstances, int32 LODIndex)
{
	...
	for (int32 Item = 0; Item < NumDynamicInstances; Item++)
	{
		...
		DynamicData.DynamicGeometry.InitResource(RHICmdList);
	}
}
...
#if WITH_EDITOR
void UInstancedStaticMeshComponent::ComputeHLODHash(FHLODHashBuilder& HashBuilder) const
{
	Super::ComputeHLODHash(HashBuilder);

	FHLODHashScope HashScope(HashBuilder, TEXT("UInstancedStaticMeshComponent"));

	for (const FInstancedStaticMeshInstanceData& InstanceData : PerInstanceSMData)
	{
		HashBuilder << FTransform(InstanceData.Transform);
	}
	HashBuilder << FHLODHashContext(GET_MEMBER_NAME_CHECKED(UInstancedStaticMeshComponent, PerInstanceSMData));

	HashBuilder << PerInstanceSMCustomData << FHLODHashContext(GET_MEMBER_NAME_CHECKED(UInstancedStaticMeshComponent, PerInstanceSMCustomData));
	HashBuilder << InstancingRandomSeed << FHLODHashContext(GET_MEMBER_NAME_CHECKED(UInstancedStaticMeshComponent, InstancingRandomSeed));
		
	for (const FInstancedStaticMeshRandomSeed& ISMCRandomSeed : AdditionalRandomSeeds)
	{
		HashBuilder << ISMCRandomSeed.StartInstanceIndex;
		HashBuilder << ISMCRandomSeed.RandomSeed;
	}
	HashBuilder << FHLODHashContext(GET_MEMBER_NAME_CHECKED(UInstancedStaticMeshComponent, AdditionalRandomSeeds));
}
#endif
...
{
	if (Index != INDEX_NONE && InstanceBodies.IsValidIndex(Index))
	{
...
FPrimitiveSceneProxy* UInstancedStaticMeshComponent::CreateSceneProxy()
{
	...
	// Verify that instance data is valid before using it.
	const bool bIsInstanceDataValid =
#if WITH_EDITOR
		bIsInstanceDataApplyCompleted && 
#endif
		// make sure we have instances - or have explicitly permitted creating empty ISMs
		(PerInstanceSMData.Num() > 0 || CVarAllowCreateEmptyISMs.GetValueOnGameThread());

	if (!bIsInstanceDataValid)
	{
		return nullptr;
	}
...
void UInstancedStaticMeshComponent::CreateAllInstanceBodies()
{
	...
	if (UBodySetup* BodySetup = GetBodySetup())
	{
		...
		{
			FBodyInstance::InitStaticBodies(MoveTemp(InstanceBodiesSanitized), MoveTemp(Transforms), BodySetup, this, GetWorld()->GetPhysicsScene());
		}
	}
	else
	{
...
{
	TRACE_CPUPROFILER_EVENT_SCOPE(UInstancedStaticMeshComponent::ClearAllInstanceBodies);
...
{
	TRACE_CPUPROFILER_EVENT_SCOPE(UInstancedStaticMeshComponent::OnDestroyPhysicsState);
...
{
	USceneComponent::OnAsyncDestroyPhysicsStateBegin_GameThread();
...
{
	TRACE_CPUPROFILER_EVENT_SCOPE(UInstancedStaticMeshComponent::OnAsyncDestroyPhysicsState);
...
uint32 FBoundsCacheElement::GetHash(const FTransform& InBoundTransform, const FBox& InInstanceBounds)
{
	// Compute a persistent hash as this value may be saved
	FVector Translation = InBoundTransform.GetTranslation();
	FQuat Rotation = InBoundTransform.GetRotation();
	FVector Scale3D = InBoundTransform.GetScale3D();

	uint64 Hash = 0;
	Hash = CityHash64WithSeed((const char*)&Translation, sizeof(Translation), Hash);
	Hash = CityHash64WithSeed((const char*)&Rotation, sizeof(Rotation), Hash);
	Hash = CityHash64WithSeed((const char*)&Scale3D, sizeof(Scale3D), Hash);
	Hash = CityHash64WithSeed((const char*)&InInstanceBounds.Min, sizeof(InInstanceBounds.Min), Hash);
	Hash = CityHash64WithSeed((const char*)&InInstanceBounds.Max, sizeof(InInstanceBounds.Max), Hash);
	return static_cast<uint32>(Hash) ^ static_cast<uint32>(Hash >> 32);
}

void UInstancedStaticMeshComponent::InvalidateCachedBounds()
{
	static_assert(UE_ARRAY_COUNT(CachedBounds) == int32(EBoundsCacheType::Count));
	for (int i = 0; i < (int)EBoundsCacheType::Count; ++i)
	{
		CachedBounds[i].Invalidate();
	}
}

FBoxSphereBounds UInstancedStaticMeshComponent::CalcBounds(const FTransform& InBoundTransform) const
{
	EBoundsCacheType Type = bIsUpdatingBounds ? EBoundsCacheType::Component : InBoundTransform.Equals(FTransform::Identity) ? EBoundsCacheType::Local : EBoundsCacheType::Other;
	FBoxSphereBounds NewBounds = GetOrCalcBounds(InBoundTransform, Type);
...
{
	GetOrCalcBounds(GetComponentTransform(), EBoundsCacheType::Navigation);
}

FBoxSphereBounds UInstancedStaticMeshComponent::GetOrCalcBounds(const FTransform& InBoundTransform, EBoundsCacheType InType) const
{
	if (GetStaticMesh() && PerInstanceSMData.Num() > 0)
	{
		const FBox InstanceBounds = (InType == EBoundsCacheType::Navigation) ? GetInstanceNavigationBounds() : GetStaticMesh()->GetBounds().GetBox();
		if (InstanceBounds.IsValid)
		{
			const UWorld* const World = GetWorld();
			const bool bIsGameWorld = World && World->IsGameWorld();
			const bool bShouldCache = InType != EBoundsCacheType::Other && (IsRegistered() || bIsGameWorld);
			if (!bShouldCache)
			{
				FBoxSphereBounds Value = CalcBoundsImpl(InBoundTransform, InstanceBounds);
				if (InType == EBoundsCacheType::Navigation)
				{
					// Store result for Navigation Cache as GetNavigationBounds() relies on it.
					CachedBounds[(int32)EBoundsCacheType::Navigation].CacheValue(Value);
				}
				return Value;
			}

			const uint32 Hash = FBoundsCacheElement::GetHash(InBoundTransform, InstanceBounds);
			int32 TypeAsInt = (int32)InType;
			FBoundsCacheElement& TargetBounds = CachedBounds[TypeAsInt];
			if (TargetBounds != Hash)
			{
				for (int32 TypeIndex = 0; TypeIndex < (int32)EBoundsCacheType::Count; ++TypeIndex)
				{
					if ((TypeIndex != TypeAsInt) && CachedBounds[TypeIndex] == Hash)
					{
						TargetBounds = CachedBounds[TypeIndex];
						return TargetBounds.GetValue();
					}
				}
				TargetBounds.CacheValue(CalcBoundsImpl(InBoundTransform, InstanceBounds), Hash);
			}
			return TargetBounds.GetValue();
		}
	}
	FBoxSphereBounds Value(InBoundTransform.GetLocation(), FVector::ZeroVector, 0.f);
	if (InType == EBoundsCacheType::Navigation)
	{
		// Store result for Navigation Cache as GetNavigationBounds() relies on it.
		CachedBounds[(int32)EBoundsCacheType::Navigation].CacheValue(Value);
...
FBoxSphereBounds UInstancedStaticMeshComponent::CalcBoundsImpl(const FTransform& BoundTransform, bool bForNavigation) const
{
	if (GetStaticMesh() && PerInstanceSMData.Num() > 0)
	{
		...
		if (InstanceBounds.IsValid)
		{
			return CalcBoundsImpl(BoundTransform, InstanceBounds);
		}
	}
	return FBoxSphereBounds(BoundTransform.GetLocation(), FVector::ZeroVector, 0.f);
}

FBoxSphereBounds UInstancedStaticMeshComponent::CalcBoundsImpl(const FTransform& BoundTransform, const FBox& InstanceBounds) const
{
	TRACE_CPUPROFILER_EVENT_SCOPE(UInstancedStaticMeshComponent::CalcBoundsImpl);
...
{
#if WITH_EDITOR
	// Skip updating bounds to avoid stall on static mesh if we're still compiling and because
	// if this is called during registration, we can end up doing a flush that will then
	// try to recreate the render state as part of the postcompilation before we are even ready to do so
	// in the registration flow, causing asserts because InstancingRandomSeed has not yet been initialized.
	if (IsCompiling())
	{
		// Bounds will be updated when we finish compiling.
		return;
	}
#endif
	FGuardValue_Bitfield(bIsUpdatingBounds, true);
	...
	if (bCanEverAffectNavigation && (IsRegistered() || IsPreRegistering()))
	{
...
void UInstancedStaticMeshComponent::PreSave(FObjectPreSaveContext ObjectSaveContext)
{
	...
	if (!IsTemplate() && IsRegistered())
	{
		GetOrCalcBounds(FTransform::Identity, EBoundsCacheType::Local);
		GetOrCalcBounds(GetComponentTransform(), EBoundsCacheType::Component);
		GetOrCalcBounds(GetComponentTransform(), EBoundsCacheType::Navigation);
	}
	else
	{
...
{
	check(PrimitiveInstanceDataManager.GetMode() != FPrimitiveInstanceDataManager::EMode::ExternalLegacyData);
	UpdateInstanceTransform(PrimitiveInstanceDataManager.IdToIndex(InstanceId), NewInstanceTransform, bWorldSpace, false, bTeleport);
}
...
bool UInstancedStaticMeshComponent::SetCustomData(int32 InstanceIndexStart, int32 InstanceIndexEnd, TConstArrayView<float> CustomDataFloats, bool bMarkRenderStateDirty)
{
	const int32 CustomDataIndexStart = InstanceIndexStart * NumCustomDataFloats;
	checkf(PerInstanceSMCustomData.Num() >= (CustomDataIndexStart + CustomDataFloats.Num()) && CustomDataFloats.GetTypeSize() == PerInstanceSMCustomData.GetTypeSize(), TEXT("CustomDataFloats and offset must fit within PerInstanceSMCustomData"));
	...
	if (!ensureMsgf(InstanceIndexStart <= InstanceIndexEnd && InstanceIndexStart >= 0 && InstanceIndexEnd < PerInstanceSMData.Num(), TEXT("InstanceIndexStart and InstanceIndexEnd need to be valid indices for PerInstanceSMData")))
	{
		return false;
	}

	if (!ensureMsgf((InstanceIndexEnd - InstanceIndexStart + 1) * NumCustomDataFloats == CustomDataFloats.Num(), TEXT("CustomDataFloats ArrayNum must match the number of Instances * NumCustomDataFloats")))
	{
		return false;
	}
	...
	FMemory::Memcpy(&PerInstanceSMCustomData[CustomDataIndexStart], CustomDataFloats.GetData(), CustomDataFloats.Num() * CustomDataFloats.GetTypeSize());

	for (int32 InstanceIndex = InstanceIndexStart; InstanceIndex <= InstanceIndexEnd; ++InstanceIndex)
	{
		PrimitiveInstanceDataManager.CustomDataChanged(InstanceIndex);
	}
...
{
	return !bDisableCollision && (IsRegistered() || (IsPreRegistering() && AllowsAsyncPhysicsStateCreation())) && !IsBeingDestroyed() && GetStaticMesh() && !GetStaticMesh()->IsCompiling() && (bAlwaysCreatePhysicsState || IsCollisionEnabled());
}
...
{
	return PrimitiveInstanceDataManager.GetProxy();
}
	...
	if (StaticMesh)
	{
		if ((FeatureLevel > ERHIFeatureLevel::ES3_1) || IsMobileDistanceFieldEnabled(GMaxRHIShaderPlatform))
		{
			...
			bNeedsCPUAccess |= PrimitiveNeedsDistanceFieldSceneData(
				...
				/* bCastsDynamicIndirectShadow */ InComponent->bCastDynamicShadow && InComponent->CastShadow && InComponent->bCastDistanceFieldIndirectShadow && InComponent->Mobility != EComponentMobility::Static && InComponent->FirstPersonPrimitiveType != EFirstPersonPrimitiveType::FirstPerson,
...
{
	TRACE_CPUPROFILER_EVENT_SCOPE(UInstancedStaticMeshComponent::OnRegister())
	Super::OnRegister();
...
void UInstancedStaticMeshComponent::PostLoad()
{
	...
	// If world is instanced, clear our cached bounds as they will likely not be valid
...
{
	TRACE_CPUPROFILER_EVENT_SCOPE(UInstancedStaticMeshComponent::PartialNavigationUpdate);
...
{
	return FInstancedStaticMeshComponentHelper::DoCustomNavigableGeometryExport(*this, GeomExport, FNavDataPerInstanceTransformDelegate::CreateUObject(this, &UInstancedStaticMeshComponent::GetNavigationPerInstanceTransforms));
}
...
{
	FInstancedStaticMeshComponentHelper::GetNavigationData(*this, Data, FNavDataPerInstanceTransformDelegate::CreateUObject(this, &UInstancedStaticMeshComponent::GetNavigationPerInstanceTransforms));
}

FBox UInstancedStaticMeshComponent::GetNavigationBounds() const
{
	const UWorld* const World = GetWorld();
	// Navigation bounds must be calculated before being requested
	constexpr bool bCheckValid = true;
	return CachedBounds[(int)EBoundsCacheType::Navigation].GetValue(bCheckValid).GetBox();
}
...
{
	return FInstancedStaticMeshComponentHelper::GetInstanceNavigationBounds(*this);
}
...
#if WITH_EDITOR
void UInstancedStaticMeshComponent::PostStaticMeshCompilation()
{
	// Multiple operations (including 'UpdateBounds') can early out when mesh is compiling so we need to 
	// override PostStaticMeshCompilation to make sure that UpdateBounds is completed before 
	// UStaticMeshComponent::PostStaticMeshCompilation calls FNavigationSubsystem::UpdateComponentData, which relies
	// on updated and cached bounds to create a valid FNavigationElement.
	...
	if(PropertyChangedEvent.Property != NULL)
	{
		if (PropertyChangedEvent.Property->GetFName() == GET_MEMBER_NAME_CHECKED(UInstancedStaticMeshComponent, PerInstanceSMData))
		{
				...
				|| PropertyChangedEvent.ChangeType == EPropertyChangeType::Duplicate)
			{
				...
				// This path may reorder the instances by insertion and this is not handled by the incremental updates in the proxy, so we force a full update.
				PrimitiveInstanceDataManager.ClearChangeTracking();
			}
...
void UInstancedStaticMeshComponent::BeginCacheForCookedPlatformData( const ITargetPlatform* TargetPlatform )
{
	...
	// Early out to avoid call to BuildComponentInstanceData if we don't need to call BeginCacheForCookedPlatformData
	bool bNeedPrimitiveInstanceDataManagerCookedData = PrimitiveInstanceDataManager.ShouldWriteCookedData(TargetPlatform, PerInstanceSMData.Num());
	...
	if (GetStaticMesh() != nullptr)
	{
		...
		if (bNeedPrimitiveInstanceDataManagerCookedData)
		{
			// Calling this function can be expensive, because of its call to GetUsedMaterials, which immediately forces completion of in-flight
			// static mesh compilation, reducing parallelism.  That's why we try to avoid the call where it's not necessary.
			//
			// TODO:  In the future, we could make it so the following function is itself more efficient.  The component data is only affected by
			// materials where displacement is used (rare), and we could skip the call to GetUsedMaterials if none of the original material references
			// in the static mesh have displacement.  This isn't completely trivial, as the static mesh currently protects access to the material
			// references, also forcing completion of in-flight compilation when accessed.  You'd need some way to bypass that protection, perhaps
			// caching the relevant information (whether displacement may be present) on load.
			BuildComponentInstanceData(GMaxRHIFeatureLevel, ComponentData);
		}
	}
...
void UInstancedStaticMeshComponent::InitializeInstancingRandomSeed()
{
	...
	{
		// During cook, we must generate a deterministic value. Subsequent save of the asset in editor can save
		// a new unique value that persists.
		if (InstancingRandomSeed == 0)
		{
			TStringBuilder<256> ResultBuilder;
			GetPathName(nullptr, ResultBuilder);
			InstancingRandomSeed = (int32)CityHash32((const char*)ResultBuilder.GetData(), ResultBuilder.Len()*sizeof(decltype(ResultBuilder)::ElementType));
			if (InstancingRandomSeed == 0)
			{
				InstancingRandomSeed = 42;
			}
		}
	}
	else
