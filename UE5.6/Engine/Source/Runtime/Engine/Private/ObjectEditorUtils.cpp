...
namespace FObjectEditorUtils
{
	...
	bool IsStructExperimentalForValkyrie(const UStruct& Struct)
	{
		if (const UValkyrieMetaData* DefaultValkyrieMetaData = GetDefault<UValkyrieMetaData>())
		{
			if (const UStruct* MostDerivedStruct = Struct.HasMetaDataHierarchical(DefaultValkyrieMetaData->DevelopmentStatusKey))
			{
				const FString DevelopmentStatus = MostDerivedStruct->GetMetaData(DefaultValkyrieMetaData->DevelopmentStatusKey);
				if (DevelopmentStatus == DefaultValkyrieMetaData->DevelopmentStatusValue_Experimental)
				{
					return true;
				}
			}
	...
	bool IsStructValkyrieDeprecatedForValkyrie(const UStruct& Struct)
	{
		if (const UValkyrieMetaData* DefaultValkyrieMetaData = GetDefault<UValkyrieMetaData>())
		{
			if (const UStruct* MostDerivedStruct = Struct.HasMetaDataHierarchical(DefaultValkyrieMetaData->DeprecationStatusKey))
			{
				const FString DeprecationStatus = MostDerivedStruct->GetMetaData(DefaultValkyrieMetaData->DeprecationStatusKey);
				if (DeprecationStatus == DefaultValkyrieMetaData->DeprecationStatusValue_Deprecated)
				{
					return true;
				}
			}
	...
	static void CopySinglePropertyRecursive(UObject* SourceObject, const void* const InSourcePtr, FProperty* InSourceProperty, void* const InTargetPtr, UObject* InDestinationObject, FProperty* InDestinationProperty)
	{
		...
		if (FStructProperty* const DestStructProperty = CastField<FStructProperty>(InDestinationProperty))
		{
			...
			// Now go through each property and make sure we take care of things like nested instanced objects
			...
			for (int32 ArrayIndex = 0; ArrayIndex < PropertyArrayDim; ArrayIndex++)
			{
				...
				// Copy complete element value
				InSourceProperty->CopyCompleteValue(TargetPtr, SourcePtr);
