...
void UCheatManager::DestroyTarget()
{
    ...
    if (TargetActor)
	{
		...
		if (Pawn != NULL)
		{
			if ((Pawn->GetController() != NULL) && (Cast<APlayerController>(Pawn->GetController()) == NULL))
			{
...
void UCheatManager::DestroyAllPawnsExceptTarget()
{
	...
	if (HitPawnTarget)
	{
		for (TActorIterator<APawn> It(GetWorld(), APawn::StaticClass()); It; ++It)
		{
			...
			{
				if ((Pawn != HitPawnTarget) && Cast<APlayerController>(Pawn->GetController()) == NULL)
				{
...
void UCheatManager::DestroyPawns(TSubclassOf<APawn> aClass)
{
	...
	{
		APawn* Pawn = *It;
		if ( Cast<APlayerController>(Pawn->GetController()) == NULL )
		{
			if ( Pawn->GetController() != NULL )
			{
				Pawn->GetController()->Destroy();
