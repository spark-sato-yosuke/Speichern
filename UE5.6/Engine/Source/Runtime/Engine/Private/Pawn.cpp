...
void APawn::PostInitializeComponents()
{
	...
	if (IsValidChecked(this))
	{
		...
		if (AutoPossessPlayer == EAutoReceiveInput::Disabled
			&& AutoPossessAI != EAutoPossessAI::Disabled && GetController() == nullptr && GetNetMode() != NM_Client
#if WITH_EDITOR
...
{
	INavMovementInterface* NavMovementInterface = FindComponentByInterface<INavMovementInterface>();
	
	//// Update Nav Agent props with collision component's setup if it's not set yet
	if (RootComponent != nullptr && NavMovementInterface != nullptr)
	{
		const FNavMovementProperties* NavMovementProperties = NavMovementInterface->GetNavMovementProperties();
		if (NavMovementProperties && NavMovementProperties->bUpdateNavAgentWithOwnersCollision != 0)
		{
			RootComponent->UpdateBounds();
			NavMovementInterface->UpdateNavAgent(*this);
		}
	}
}
...
bool APawn::ReachedDesiredRotation()
{
	...
	FRotator DesiredRotation = GetController() ? GetController()->GetDesiredRotation() : GetActorRotation();
...
void APawn::SetRemoteViewPitch(double NewRemoteViewPitch)
{
	// Compress pitch to a 16-bit short
...
UPawnNoiseEmitterComponent* APawn::GetPawnNoiseEmitterComponent() const
{
	 ...
	 if (!NoiseEmitterComponent && GetController())
	 {
		 NoiseEmitterComponent = GetController()->FindComponentByClass<UPawnNoiseEmitterComponent>();
	 }
...
{
	return GetController() ? GetController()->GetControlRotation() : FRotator::ZeroRotator;
}
...
{
	ReceiveControllerChanged(PreviousController, GetController());
	ReceiveControllerChangedDelegate.Broadcast(this, PreviousController, GetController());
	if (UGameInstance* GameInstance = GetGameInstance())
	{
		GameInstance->GetOnPawnControllerChanged().Broadcast(this, GetController());
	}
...
class UNetConnection* APawn::GetNetConnection() const
{
	...
	if (GetController())
	{
		return GetController()->GetNetConnection();
	}
...
class UPlayer* APawn::GetNetOwningPlayerAnyRole()
{
	if (GetController())
	{
		APlayerController* PC = Cast<APlayerController>(GetController());
...
{
	return GetController() != nullptr && GetController()->IsMoveInputIgnored();
}
...
{
	return GetPlayerState() ? GetPlayerState()->GetPlayerName() : Super::GetHumanReadableName();
}
...
// 	}
	...
	{
		GetPlayerState()->DisplayDebug(Canvas, DebugDisplay, YL, YPos);
	}
	...
	{
		GetController()->DisplayDebug(Canvas, DebugDisplay, YL, YPos);
	}
}
...
FRotator APawn::GetBaseAimRotation() const
{
	...
	FVector POVLoc;
	FRotator POVRot;
	if(GetController() != nullptr && !InFreeCam() )
	{
		GetController()->GetPlayerViewPoint(POVLoc, POVRot);
		return POVRot;
	}
	...
	if( FMath::IsNearlyZero(POVRot.Pitch) )
	{
		...
		{
			// Use deprecated 8-bit value serialized from old replays
			PRAGMA_DISABLE_DEPRECATION_WARNINGS
			POVRot.Pitch = RemoteViewPitch;
			PRAGMA_ENABLE_DEPRECATION_WARNINGS
			POVRot.Pitch = POVRot.Pitch * 360.0f / 255.0f;
		}
		else if (DemoNetDriver && DemoNetDriver->IsPlaying() && (DemoNetDriver->GetPlaybackEngineNetworkProtocolVersion() < FEngineNetworkCustomVersion::PawnRemoteViewPitchTo16Bit))
		{
			// Use deprecated 8-bit value serialized from old replays
			PRAGMA_DISABLE_DEPRECATION_WARNINGS
		...
		{
			POVRot.Pitch = FRotator::DecompressAxisFromShort(RemoteViewPitch16);
		}
	}
...
void APawn::OutsideWorldBounds()
{
	if ( !bProcessingOutsideWorldBounds )
	{
		...
		if (GetLocalRole() == ROLE_Authority && Cast<APlayerController>(GetController()) == nullptr)
		{
			Destroy();
		}
		else
...
{
	AController* const ThisController = GetController();
	if (ThisController != nullptr && ThisController->GetPawn() == this )
	{
		ThisController->PawnPendingDestroy(this);
...
{
	if ( (InstigatedBy != nullptr) && (InstigatedBy != GetController()) )
	{
		return InstigatedBy;
	}
...
{
	if (PlayerController == GetController() || PlayerController == nullptr)
	{
		bInputEnabled = true;
	}
	else
...
{
	CA_SUPPRESS(6011);
	if (bAlwaysRelevant || RealViewer == GetController() || IsOwnedBy(ViewTarget) || IsOwnedBy(RealViewer) || this == ViewTarget || ViewTarget == GetInstigator()
...
void APawn::GetLifetimeReplicatedProps( TArray< FLifetimeProperty > & OutLifetimeProps ) const
{
	...
	// Deprecated RemoteViewPitch, still loaded for old replays
	PRAGMA_DISABLE_DEPRECATION_WARNINGS
	DOREPLIFETIME_CONDITION( APawn, RemoteViewPitch, COND_ReplayOnly );
	PRAGMA_ENABLE_DEPRECATION_WARNINGS

	DOREPLIFETIME_CONDITION( APawn, RemoteViewPitch16, 	COND_SkipOwner );
}

void APawn::SetController(AController* InController)
{
...
{
	INavMovementInterface* NavMovementInterface = FindComponentByInterface<INavMovementInterface>();
	return NavMovementInterface ? NavMovementInterface->GetNavAgentPropertiesRef() : FNavAgentProperties::DefaultProperties;
}
