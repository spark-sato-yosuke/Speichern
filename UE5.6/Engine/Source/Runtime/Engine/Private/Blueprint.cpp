...
{
	extern KISMET_API bool bRunArchetypePostLoadEarly;
}
#endif
...
void UBlueprint::ConformNativeComponents()
{
	...
	{
		if (AActor* BlueprintCDO = Cast<AActor>(BlueprintClass->GetDefaultObject(false)))
		{
			...
			AActor* NativeCDO = CastChecked<AActor>(NativeSuperClass->GetDefaultObject(false));
...
void UBlueprint::Serialize(FArchive& Ar)
{
	...
	// cooked BP macros don't work well in cooked editor mixing with uncooked BPs, so we load pacakges with macros as uncooked
	if (!HasAnyFlags(RF_ClassDefaultObject) && Ar.IsPersistent() && BlueprintType == EBlueprintType::BPTYPE_MacroLibrary)
	{
...
UClass* UBlueprint::RegenerateClass(UClass* ClassToRegenerate, UObject* PreviousCDO)
{
	...
	if (ensure(GeneratedClassResolved->GetDefaultObject(false)))
	{
		UBlueprint::ForceLoadMembers(GeneratedClassResolved);
		if(UE::Private::bRunArchetypePostLoadEarly)
		{
			UBlueprint::ForceLoadGraph(GeneratedClassResolved->GetDefaultObject(false));
		}
		else
		{
			UBlueprint::ForceLoadMembers(GeneratedClassResolved->GetDefaultObject(false));
		}
	}
...
void UBlueprint::ForceLoadGraph(UObject* Object)
{
	// find every directly referenced object:
	TArray<UObject*> ReachableInnerObjects;
	ReachableInnerObjects.Add(Object);
	FReferenceFinder InnerCollector(ReachableInnerObjects, Object, false, true, true, true);
	InnerCollector.FindReferences(Object);

	// preload until we stabilize
	TSet<UObject*> LoadedObjects;
	TArray<UObject*> NewObjects = ReachableInnerObjects;
	while(NewObjects.Num())
	{
		LoadedObjects.Append(NewObjects);
		TArray<UObject*> ExternalObjects;
		for(UObject* InnerObject : NewObjects)
		{
			ForceLoad(InnerObject);
		}
		TArray<UObject*> ReachableObjects;
		for(UObject* InnerObject : NewObjects)
		{
			FReferenceFinder ComponentCollector(ReachableObjects, nullptr, false, true, true);
			ComponentCollector.FindReferences(InnerObject);
		}

		NewObjects.Reset();
		for(UObject* ReachableObject : ReachableObjects) //-V1078
		{
			if(LoadedObjects.Contains(ReachableObject))
			{
				continue;
			}

			if(ReachableObject->HasAnyFlags(RF_WasLoaded) && !ReachableObject->HasAnyFlags(RF_LoadCompleted))
			{
				NewObjects.Add(ReachableObject);
			}
			else
			{
				// make sure this objects loaded inner objects are also loaded - this specifically handles 
				// natively created objects like BodySetup that are not reachable by FReferenceFinder:
				TArray<UObject*> SubObjs;
				const bool bIncludeNested = true;
				GetObjectsWithOuter(ReachableObject, SubObjs, bIncludeNested, RF_Transient, EInternalObjectFlags::Garbage);
				for(UObject* SubObj : SubObjs)
				{
					if(SubObj->HasAnyFlags(RF_WasLoaded) && !SubObj->HasAnyFlags(RF_LoadCompleted))
					{
						NewObjects.Add(SubObj);
					}
				}
			}
		}
...
#if WITH_EDITOR
void UBlueprint::OnCookEvent(UE::Cook::ECookEvent CookEvent, UE::Cook::FCookEventContext& CookContext)
{
	...
	{
		static bool bIgnoreBlueprintCookDependencies = FParse::Param(FCommandLine::Get(), TEXT("IgnoreBlueprintCookDependencies"));
		if (bIgnoreBlueprintCookDependencies)
		{
			return;
		}

		CookContext.AddLoadBuildDependency(
			UE::Private::BlueprintDependencies::RecordCookDependencies(this));
	}
}
#endif
...
FPrimaryAssetId UBlueprint::GetPrimaryAssetId() const
{
	...
	// We use Generated instead of Skeleton because the CDO data is more accurate on Generated
	if (GeneratedClass && GeneratedClass->GetDefaultObject(false))
	{
...
FName UBlueprint::FindBlueprintPropertyNameFromGuid(const FGuid& PropertyGuid) const
{
	...
	for (const FBPVariableDescription& BPVarDesc : GeneratedVariables)
	{
		if (BPVarDesc.VarGuid == PropertyGuid)
		{
			return BPVarDesc.VarName;
