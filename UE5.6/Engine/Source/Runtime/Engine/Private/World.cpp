...
namespace UE::Private::World
{
	...
	void ParseAndSetClientHandshakeId(FURL& InUrl, UNetConnection* Connection)
	{
		if (const TCHAR* HandshakeIdStr = InUrl.GetOption(TEXT("HandshakeId="), nullptr))
		{
			...
			{
				UE_LOG(LogNet, Warning, TEXT("Connection %s received an invalid client handshake id."), *Connection->GetName());
			}
		}
	}
}
...
#if WITH_STATE_STREAM
	StateStreamManager = GetRendererModule().AllocateStateStream(this);
	StateStreamManager->Game_BeginTick();
#endif
		...
		if (bIsTimeLimitExceed)
		{
			if (LastElapsed > 0.0)
			{
				...
				if (DeltaMS > WarningLimit)
				{
					...
					UE_LOG(LogStreaming, Display, TEXT("UWorld::AddToWorld: %s for %s took (less than) %5.2f ms"), CurrentTask, *Level->GetOutermost()->GetName(), float(DeltaMS) );
				}
				LastElapsed = 0.0;
			}
		}
...
void UWorld::EnableActorTickAndUserCallbacks(bool bEnable)
{
	if (bEnable)
	{
		UE_LOG(LogWorld, Warning, TEXT("EnableActorTickAndUserCallbacks: Enabling actor ticking and actor user callbacks."));
	}
	else
	{
		UE_LOG(LogWorld, Warning, TEXT("EnableActorTickAndUserCallbacks: Disabling actor ticking and actor user callbacks."));
	}
...
void UWorld::AddToWorld(ULevel* Level, const FTransform& LevelTransform, bool bConsiderTimeLimit, const TOptional<const UE::FTimeout>& ExternalTimeout, FNetLevelVisibilityTransactionId TransactionId, ULevelStreaming* InOwningLevelStreaming)
{
	...
	if( bExecuteNextStep && !bIsLevelMakingVisible && !IsLevelMakingInvisible(Level) )
	{
		...
		FLevelRegistrationAccessor::CreateAsyncRegisterLevelContext(Level);
	...
	if( bExecuteNextStep && !Level->bAlreadyUpdatedComponents )
	{
		...
		FAsyncRegisterLevelContext* AsyncRegisterLevelContext = FLevelRegistrationAccessor::GetAsyncRegisterLevelContext(Level);
		auto SetLevelContextCanLaunchNewTasks = [AsyncRegisterLevelContext](bool bValue)
		{
			if (AsyncRegisterLevelContext)
			{
				AsyncRegisterLevelContext->SetCanLaunchNewTasks(bValue);
			}
		};
		...
		// Incremental update components
		{
			...
			do
			{
				...
				bHasRunningTasks = AsyncRegisterLevelContext ? AsyncRegisterLevelContext->IsRunningAsync() : false;
			...
			// Block level context to launch new tasks since we exit the IncrementalUpdateComponents scope
			SetLevelContextCanLaunchNewTasks(false);
		}

		check(!Level->bAreComponentsCurrentlyRegistered || !AsyncRegisterLevelContext || !AsyncRegisterLevelContext->HasRemainingWork());
		// Allow level context to launch new tasks
		SetLevelContextCanLaunchNewTasks(true);
	...
	if( bPerformedLastStep )
	{
		...
		FLevelRegistrationAccessor::DestroyAsyncRegisterLevelContext(Level);
	...
	if ( (!HasAnyLevelMakingVisible() || (bIsCandidateForRemoval && !IsLevelMakingVisible(Level))) && (Level->bIsVisible || Level->bIsBeingRemoved) )
	{
		...
		if (bAllowIncrementalRemoval)
		{
			...
			if (!IsLevelMakingInvisible(Level) && (MakingInvisibleLevels.Num() < FMath::Max<int32>(1, GMaximumMakingInvisibleLevels)))
			{
				...
				{
					TRACE_BEGIN_REGION(*WriteToString<256>(TEXT("RemoveFromWorld: "), Level->GetPackage()->GetName()));
...
void UWorld::UpdateLevelStreaming(const TOptional<const UE::FTimeout>& ExternalTimeout)
{
	...
	UE_CLOG(bIsReentrant, LogWorld, Fatal, TEXT("Attempted to call UWorld::UpdateLevelStreaming while already inside UWorld::UpdateLevelStreaming."));
	...
	const bool bShouldPurgeLevels = GLevelStreamingContinuouslyIncrementalGCWhileLevelsPendingPurge && CurrentNumLevelsPendingPurge >= GLevelStreamingContinuouslyIncrementalGCWhileLevelsPendingPurge;
...
void ValidateComponentsRegistration(UWorld* World)
{
	...
	if (Errors.Len())
	{
		...
		{
			UE_LOG(LogWorld, Fatal, TEXT("Found components to be incorrectly unregistered: \n%s"), Errors.GetData());
		}
		else
		{
			UE_LOG(LogWorld, Error, TEXT("Found components to be incorrectly unregistered: \n%s"), Errors.GetData());
		}
	}
...
#if WITH_STATE_STREAM
	StateStreamManager->Game_EndTick(RealTimeSeconds);
#endif
...
void UWorld::CreatePhysicsScene(const AWorldSettings* Settings)
{
	...
	TStringBuilder<128> PhysicsSceneNameBuilder;
 	PhysicsSceneNameBuilder.Appendf(TEXT("%s | World Type [%s]"), IsNetMode(NM_DedicatedServer) ? TEXT("Server Physics") : TEXT("Client Physics"), LexToString(WorldType));
 
	#if UE_WITH_REMOTE_OBJECT_HANDLE
 		PhysicsSceneNameBuilder.Appendf(TEXT(" | Remote Server ID [%u]"), UE::RemoteObject::GetGlobalServerId().GetIdNumber()); 
	#endif
		...
		switch (MessageType)
		{
			...
			case NMT_Login:
			{
				...
				if (bReceived)
				{
					...
					if (InURL.HasOption(TEXT("AutonomousAsSimulated")))
					{
						NetDriver->ReplicateAutonomousAsSimulated(true);
					}
			...
			case NMT_JoinNoPawn:
			{
				if (NetDriver->HasSupportForNoPawnConnection())
				{
					...
					if (!InURL.Valid)
					{
						UE_LOG(LogNet, Error, TEXT("NMT_JoinNoPawn: Invalid URL %s"), *Connection->RequestURL);
						Bunch.SetError();
						break;
					}
					...
					Connection->PlayerController->NetPlayerIndex = 0;
				...
				{
					FString Msg(TEXT("Server doesn't support NMT_JoinNoPawn."));
					Connection->SendCloseReason(ENetCloseResult::JoinFailure);
			...
			case NMT_JoinNoPawnSplit:
			{
				if (NetDriver->HasSupportForNoPawnConnection())
				{
					...
					if (FNetControlMessage<NMT_JoinNoPawnSplit>::Receive(Bunch, SplitRequestURL, UniqueIdRepl))
					{
						...
						if (!InURL.Valid)
						{
							UE_LOG(LogNet, Error, TEXT("NMT_JoinNoPawnSplit: Invalid URL %s"), *SplitRequestURL);
							Bunch.SetError();
							break;
						}
						...
						UChildConnection* ChildConnection = NetDriver->CreateChild(Connection);
						ChildConnection->PlayerId = UniqueIdRepl;
						
						UE::Private::World::ParseAndSetClientHandshakeId(InURL, ChildConnection);

						ChildConnection->PlayerController = GetWorld()->SpawnActor<APlayerController>(ANoPawnPlayerController::StaticClass(), FVector(), FRotator(), SpawnInfo);
						ChildConnection->PlayerController->SetRole(ROLE_Authority);
						ChildConnection->PlayerController->SetReplicates(true);
						ChildConnection->PlayerController->SetAutonomousProxy(true);
						ChildConnection->PlayerController->FinishSpawning(FTransform());
						ChildConnection->PlayerController->NetPlayerIndex = Connection->Children.Num();
						ChildConnection->PlayerController->NetConnection = ChildConnection;
						ChildConnection->PlayerController->SetClientHandshakeId(ChildConnection->GetClientHandshakeId());

						ChildConnection->OwningActor = ChildConnection->PlayerController;
				...
				{
					UE_LOG(LogNet, Log, TEXT("Server doesn't support NMT_JoinNoPawnSplit (enable with UNetDriver::SetSupportForNoPawnConnection() or net.EnableSupportForNoPawnConnection)."));
					FString Msg(TEXT("Server doesn't support NMT_JoinNoPawnSplit."));
					Connection->SendCloseReason(ENetCloseResult::JoinFailure);
					FNetControlMessage<NMT_Failure>::Send(Connection, Msg);
					Connection->FlushNet(true);
					Connection->Close(ENetCloseResult::JoinFailure);
...
void UWorld::ChangeFeatureLevel(ERHIFeatureLevel::Type InFeatureLevel, bool bShowSlowProgressDialog, bool bForceUpdate)
{
	...
	{
		UE_LOG(LogWorld, Log, TEXT("Changing Feature Level (Enum) from %s to %s%s"), *LexToString(GetFeatureLevel()), *LexToString(InFeatureLevel), bForceUpdate ? TEXT(" (forced)") : TEXT(""));
...
void UWorld::InsertPostProcessVolume(IInterface_PostProcessVolume* InVolume)
{
	...
	FPostProcessVolumeProperties TargetProperties = InVolume->GetProperties();
	int32 InsertIndex = 0;
	...
	for (; InsertIndex < NumVolumes; InsertIndex++)
	{
		...
		if (TargetProperties.Priority < CurrentProperties.Priority)
		{
			break;
		}
		else if (TargetProperties.Priority > CurrentProperties.Priority)
		{
			continue;
		}
		if (TargetProperties.bIsUnbound && !CurrentProperties.bIsUnbound)
		{
			break;
		}
