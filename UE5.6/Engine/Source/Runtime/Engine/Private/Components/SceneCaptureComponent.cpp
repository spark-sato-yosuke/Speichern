...
void USceneCaptureComponent::UpdateDeferredCaptures(FSceneInterface* Scene)
{
	...
	SceneCaptureUpdateDeferredCapturesInternal(Scene, {}, *SceneRenderBuilder);
	SceneRenderBuilder->Execute();
}

void SceneCaptureUpdateDeferredCapturesInternal(FSceneInterface* Scene, TConstArrayView<const FSceneViewFamily*> ViewFamilies, ISceneRenderBuilder& SceneRenderBuilder)
{
	...
	for (TWeakObjectPtr<USceneCaptureComponent> Component : SceneCapturesToUpdate)
	{
		if (Component.IsValid())
		{
			...
			Component->UpdateSceneCaptureContents(Scene, SceneRenderBuilder);
	...
	: Super(ObjectInitializer)
{
	...
	bInheritMainViewCameraPostProcessSettings = false;
	bIgnoreScreenPercentage = false;
	
	TileID = 0;

	// Update default post-processing values to reflect default values (previously and still) applied in code.
	...
	PostProcessSettings.LumenSurfaceCacheResolution = 0.5f;
...
void USceneCaptureComponent2D::SetCameraView(const FMinimalViewInfo& DesiredView)
{
	...
	FirstPersonFieldOfView = DesiredView.FirstPersonFOV;
	FirstPersonScale = DesiredView.FirstPersonScale;
}
...
void USceneCaptureComponent2D::GetCameraView(float DeltaTime, FMinimalViewInfo& OutMinimalViewInfo)
{
	...
	OutMinimalViewInfo.FirstPersonFOV = bEnableFirstPersonFieldOfView ? FirstPersonFieldOfView : OutMinimalViewInfo.FOV;
	OutMinimalViewInfo.FirstPersonScale = bEnableFirstPersonScale ? FirstPersonScale : 1.0f;
	OutMinimalViewInfo.bUseFirstPersonParameters = bEnableFirstPersonFieldOfView || bEnableFirstPersonScale;
...
void USceneCaptureComponent2D::CaptureScene()
{
	...
	if (World && World->Scene && IsVisible() && !IsCulledByDetailMode())
	{
		...
		UpdateSceneCaptureContents(World->Scene, *SceneRenderBuilder);
		SceneRenderBuilder->Execute();
	}
...
void USceneCaptureComponent2D::Serialize(FArchive& Ar)
{
	...
	if (Ar.CustomVer(FFortniteMainBranchObjectVersion::GUID) < FFortniteMainBranchObjectVersion::SceneCaptureDefaultSettings
		&& !GetArchetype()->IsInBlueprint())
	{
		// For backwards compatibility, we must preserve the old defaults that do not reflect code overrides in SetupViewFamilyForSceneCapture.
...
void USceneCaptureComponent2D::UpdateSceneCaptureContents(FSceneInterface* Scene, ISceneRenderBuilder& SceneRenderBuilder)
{
...
void USceneCaptureComponentCube::UpdateSceneCaptureContents(FSceneInterface* Scene, ISceneRenderBuilder& SceneRenderBuilder)
{
	Scene->UpdateSceneCaptureContents(this, SceneRenderBuilder);
}

void USceneCaptureComponentCube::Serialize(FArchive& Ar)
{
	Ar.UsingCustomVersion(FFortniteMainBranchObjectVersion::GUID);
