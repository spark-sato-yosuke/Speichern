...
void UInputComponent::RemoveAxisKeyBinding(FKey AxisKey)
{
	for (int32 AxisIdx = AxisKeyBindings.Num() - 1; AxisIdx >= 0; --AxisIdx)
	{
		const FInputAxisKeyBinding& Binding = AxisKeyBindings[AxisIdx];
		if (Binding.AxisKey == AxisKey)
		{
			AxisKeyBindings.RemoveAt(AxisIdx, EAllowShrinking::No);
		}
	}
}
