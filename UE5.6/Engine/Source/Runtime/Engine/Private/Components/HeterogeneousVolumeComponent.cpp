...
DECLARE_CYCLE_STAT(TEXT("Get Dynamic Mesh Elements"), STAT_HeterogeneousVolumeComponentGetDynamicMeshElements, STATGROUP_HeterogeneousVolumesRT);
DECLARE_CYCLE_STAT(TEXT("Post Edit Change Property"), STAT_HeterogeneousVolumeComponentPostEditChangeProperty, STATGROUP_HeterogeneousVolumesGT);
DECLARE_CYCLE_STAT(TEXT("Component Tick"), STAT_HeterogeneousVolumeComponentTick, STATGROUP_HeterogeneousVolumesGT);
	...
	FMeshElementCollector& Collector) const
{
	...
	SCOPE_CYCLE_COUNTER(STAT_HeterogeneousVolumeComponentGetDynamicMeshElements);
	if (Views.IsEmpty())
	{
		return;
	}
...
{
	CSV_SCOPED_TIMING_STAT_EXCLUSIVE(Effects);
	SCOPE_CYCLE_COUNTER(STAT_HeterogeneousVolumeComponentPostEditChangeProperty);
...
{
	CSV_SCOPED_TIMING_STAT_EXCLUSIVE(Effects);
	SCOPE_CYCLE_COUNTER(STAT_HeterogeneousVolumeComponentTick);
	...
	if ((ShouldRender() || bCastHiddenShadow) && DoesRuntimeSupportHeterogeneousVolumes(GetWorld()->Scene->GetShaderPlatform()) && MaterialInstanceDynamic)
	{
		const int32 SVTParameterIndex = 0;
