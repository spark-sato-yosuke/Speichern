...
constexpr bool UseStaticMeshStateStream = true;
#endif
...
void UStaticMeshComponent::GetLifetimeReplicatedProps(TArray< FLifetimeProperty > & OutLifetimeProps) const
{
	...
	DOREPLIFETIME_WITH_PARAMS_FAST(UStaticMeshComponent, StaticMesh, Params);
}
...
/** Call this after component duplication to perform the deep copy of the MeshPaintTexture. */
static UTexture* RecreateMeshPaintTexture(UTexture* InTexture, UPrimitiveComponent* InComponent)
{
	if (InTexture == nullptr || InComponent == nullptr)
	{
		return nullptr;
	}
	...
	FImage Image;
	if (InTexture->Source.GetMipImage(Image, 0))
	{
		NewTexture = NewObject<UMeshPaintVirtualTexture>(InComponent->GetOutermost());
		NewTexture->Source.Init(Image);
		NewTexture->OwningComponent = MakeWeakObjectPtr(InComponent);
...
{
 	if (!bDuplicateForPIE && MeshPaintTexture != nullptr)
 	{
		MeshPaintTexture = RecreateMeshPaintTexture(MeshPaintTexture, this);
 	}
...
{
	TRACE_CPUPROFILER_EVENT_SCOPE(UStaticMeshComponent::OnRegister)
	...
	// No need to call UpdateCollisionFromStaticMesh if pre-registered as it has already been called
	if (!IsPreRegistered())
	{
		UpdateCollisionFromStaticMesh();
	}
	...
	{
		bool bCanSkipPrecachePSOs = false;
		if (bPrecachedPSOs && GetWorld() && GetWorld()->IsGameWorld())
		{
			// World transform might have changes causing negative determinant which changes the culling mode
			const bool bIsLocalToWorldDeterminantNegative = GetRenderMatrix().Determinant() < 0;
			bCanSkipPrecachePSOs = !bIsLocalToWorldDeterminantNegative;
		}
		if (!bCanSkipPrecachePSOs)
		{
			PrecachePSOs();
		}
	}
...
#if WITH_STATE_STREAM_ACTOR
	if (UseStaticMeshStateStream)
	{
		FStaticMeshStaticState Ss;
		FStaticMeshDynamicState Ds;
		Ds.SetTransform(GetOrCreateTransformHandle());
		Ds.SetMesh(StaticMesh);
		Ds.SetOverrideMaterials(OverrideMaterials);
		Ds.SetOnlyOwnerSee(bOnlyOwnerSee);
		Ds.SetOwnerNoSee(bOwnerNoSee);
		if(bOnlyOwnerSee || bOwnerNoSee || bUseViewOwnerDepthPriorityGroup)
		{
			...
			{
				Owners.Add(CurrentOwner->GetUniqueID());
			}
			Ds.SetOwners(Owners);
		}
		MeshHandle = GetWorld()->GetStateStream<IStaticMeshStateStream>().Game_CreateInstance(Ss, Ds);
...
void UStaticMeshComponent::OnCreatePhysicsState()
{
	...
	// Async physics state creation is currently only done through component pre-registration.
	// No need to call FNavigationSystem::UpdateComponentData for a pre-registering component.
	// Once async physics state creation completes, component registration will call FNavigationSystem::OnComponentRegistered.
...
void UStaticMeshComponent::OnAsyncDestroyPhysicsStateEnd_GameThread()
{
...
{
	if (bIgnoreInstanceForTextureStreaming || !GetStaticMesh() || GetStaticMesh()->IsCompiling() || !(GetStaticMesh()->HasValidRenderData() || GetStaticMesh()->HasValidNaniteData()))
	{
		return;
	}
...
void UStaticMeshComponent::CollectPSOPrecacheDataImpl(const FVertexFactoryType* VFType, const FPSOPrecacheParams& BasePrecachePSOParams, GetPSOVertexElementsFn GetVertexElements,FMaterialInterfacePSOPrecacheParamsList& OutParams) const
{
	FStaticMeshComponentHelper::CollectPSOPrecacheDataImpl(*this, VFType, BasePrecachePSOParams, GetVertexElements, OutParams);
}
...
void UStaticMeshComponent::PostEditUndo()
{
	...
	{
		const int32 LODResourceIdx = GetStaticMesh()->GetRenderData()->GetCurrentFirstLODIdx(0);
		if (LODResourceIdx != INDEX_NONE)
		{
			FRenderResource* Resource = &GetStaticMesh()->GetRenderData()->LODResources[LODResourceIdx].IndexBuffer;
...
bool UStaticMeshComponent::CanEditChange(const FProperty* InProperty) const
{
	if (InProperty)
	{
		...
		{
			return Mobility != EComponentMobility::Static && CastShadow && bCastDynamicShadow && FirstPersonPrimitiveType != EFirstPersonPrimitiveType::FirstPerson;
		}

		if (PropertyName == GET_MEMBER_NAME_STRING_CHECKED(UStaticMeshComponent, DistanceFieldIndirectShadowMinVisibility))
		{
			return Mobility != EComponentMobility::Static && bCastDistanceFieldIndirectShadow && CastShadow && bCastDynamicShadow && FirstPersonPrimitiveType != EFirstPersonPrimitiveType::FirstPerson;
		}
...
bool UStaticMeshComponent::ShouldIncrementalPreRegister(UWorld* WorldContext) const
{
	// Make sure BodyInstance CollisionEnabled is updated before calling Super::ShouldIncrementalPreRegister as it calls
	// UPrimitiveComponent::ShouldCreatePhysicsState which calls BodyInstance.GetCollisionEnabled().
	const_cast<UStaticMeshComponent*>(this)->UpdateCollisionFromStaticMesh();
	return Super::ShouldIncrementalPreRegister(WorldContext);
}

bool UStaticMeshComponent::AllowsAsyncPhysicsStateCreation() const
{
	// Async physics state creation is supported for static collision or primitive component not using welding
	return !BodyInstance.bAutoWeld || !Cast<UPrimitiveComponent>(GetAttachParent()) || IsStaticPhysics();
}

bool UStaticMeshComponent::AllowsAsyncPhysicsStateDestruction() const
{
...
{
	MARK_PROPERTY_DIRTY_FROM_NAME(UStaticMeshComponent, StaticMesh, this);
	StaticMesh = NewMesh;
...
void UStaticMeshComponent::GetDefaultMaterialSlotsOverlayMaterial(TArray<TObjectPtr<UMaterialInterface>>& OutMaterialSlotOverlayMaterials) const
{
...
void UStaticMeshComponent::SetEvaluateWorldPositionOffsetInRayTracing(bool NewValue)
{
	...
	if (bHasChanged)
	{
		...
		ENQUEUE_RENDER_COMMAND(UpdateEvaluateWPORTCmd)
			([NewValue, SceneProxy = SceneProxy](FRHICommandList& RHICmdList)
				{
					if (SceneProxy->IsNaniteMesh())
					{
						static_cast<Nanite::FSceneProxy*>(SceneProxy)->SetEvaluateWorldPositionOffsetInRayTracing(RHICmdList, NewValue);
					}
					else
					{
						static_cast<FStaticMeshSceneProxy*>(SceneProxy)->SetEvaluateWorldPositionOffsetInRayTracing(RHICmdList, NewValue);
					}
				});
	}
}
...
FMaterialRelevance FStaticMeshSceneProxyDesc::GetMaterialRelevance(ERHIFeatureLevel::Type InFeatureLevel) const
{
	...
	return FMeshComponentHelper::GetMaterialRelevance(*this, InFeatureLevel);
}
...
#if WITH_EDITOR
void UStaticMeshComponent::ComputeHLODHash(FHLODHashBuilder& HashBuilder) const
{
	Super::ComputeHLODHash(HashBuilder);

	FHLODHashScope HashScope(HashBuilder, TEXT("UStaticMeshComponent"));

	HashBuilder << ForcedLodModel << FHLODHashContext(GET_MEMBER_NAME_CHECKED(UStaticMeshComponent, ForcedLodModel));
	
	int32 Width, Height;
	if (GetLightMapResolution(Width, Height))
	{
		HashBuilder << Width << Height << FHLODHashContext(TEXT("LightMapResolution"));
	}
	
	for (const FStaticMeshComponentLODInfo& LODInfo : LODData)
	{
		if (LODInfo.OverrideVertexColors)
		{
			HashBuilder.Serialize((uint8*)LODInfo.OverrideVertexColors->GetVertexData(), LODInfo.OverrideVertexColors->GetNumVertices() * LODInfo.OverrideVertexColors->GetStride());
			HashBuilder << FHLODHashContext(TEXT("OverrideVertexColors"));
		}
	}
	
	// Static mesh
	HashBuilder << GetStaticMesh().Get();
}
#endif
