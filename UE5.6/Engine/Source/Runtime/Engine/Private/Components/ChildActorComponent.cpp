...
static int32 GExperimentalAllowPerInstanceChildActorProperties = 0;
...
bool UChildActorComponent::ExperimentalAllowPerInstanceChildActorProperties()
{
	return GExperimentalAllowPerInstanceChildActorProperties != 0;
}
...
void UChildActorComponent::PostLoad()
{
	...
	if (ChildActor)
	{
		...
		{
			FActorParentComponentSetter::Set(ChildActor, this);
			ChildActor->SetFlags(RF_TextExportTransient | RF_DuplicateTransient);
		}
	}
...
bool UChildActorComponent::IsLevelBeingRemoved() const
{
...
void UChildActorComponent::OnChildActorDestroyed(AActor* Actor)
{
	...
	if (Actor && (Actor->HasAuthority() || !IsChildActorReplicated()) && !IsLevelBeingRemoved())
	{
	...
	if(ChildActorClass != nullptr)
	{
		...
		if(World != nullptr)
		{
			...
			if (bSpawn)
			{
				...
				Params.ObjectFlags |= (RF_TextExportTransient | RF_DuplicateTransient);
