...
void FTextRenderSceneProxy::GetDynamicRayTracingInstances(FRayTracingInstanceCollector& Collector)
{
	...
	if (bNeedsToUpdateRayTracingCache)
	{
		...
		for (int32 BatchIndex = 0; BatchIndex < NumBatches; BatchIndex++)
		{
			...
			MeshBatch.MaterialRenderProxy = TextBatch.Material->GetRenderProxy();
			MeshBatch.bCanApplyViewModeOverrides = !bAlwaysRenderAsText;
			MeshBatch.CastRayTracedShadow = IsShadowCast(Collector.GetReferenceView());
			MeshBatch.LODIndex = 0;
			MeshBatch.bUseWireframeSelectionColoring = IsSelected() ? 1 : 0;
			MeshBatch.SegmentIndex = BatchIndex;
