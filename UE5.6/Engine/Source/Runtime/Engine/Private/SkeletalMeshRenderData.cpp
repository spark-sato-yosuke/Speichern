...
void VerifyAllLodSkeletalMeshModelIntegrity(USkinnedAsset* Owner)
{
	...
	for (int32 LODIndex = 0; LODIndex < SkelMeshModel->LODModels.Num(); LODIndex++)
	{
		...
		int32 SectionsVertexNum = 0;
		int32 SectionsTriangleNum = 0;
		for (const FSkelMeshSection& Section : LODModel->Sections)
		{
			if (Section.NumTriangles == 0)
			{
				continue; // don't validate index/vertex buffer for empty sections
			}

			SectionsVertexNum += Section.GetNumVertices();
			SectionsTriangleNum += Section.NumTriangles;
			int32 LastSectionIndexBuffer = Section.BaseIndex + (Section.NumTriangles * 3) - 1;
			if (LODModel->IndexBuffer.IsValidIndex(LastSectionIndexBuffer))
			{
				...
				{
					UE_ASSET_LOG(LogSkeletalMesh, Error, Owner, TEXT("The source model is corrupted! Section triangle refers to a vertex not in the section. LOD %d"), LODIndex);
				}
			}
			else
			{
		...
		if (LODModel->NumVertices != SectionsVertexNum)
		{
			UE_ASSET_LOG(LogSkeletalMesh, Error, Owner, TEXT("The source model is corrupted! Total sections vertex count is different from source model vertex count. LOD %d"), LODIndex);
		}
...
void FSkeletalMeshRenderData::Cache(const ITargetPlatform* TargetPlatform, USkinnedAsset* Owner, FSkinnedAssetCompilationContext* ContextPtr)
{
...

	{
		...
		else
		{
			...
			for (int32 LODIndex = 0; LODIndex < SkelMeshModel->LODModels.Num(); LODIndex++)
			{
				Owner->BuildLODModel(*this, TargetPlatform, LODIndex);
				...
				BuildSettings.bBuildHalfEdgeBuffers = Owner->HasHalfEdgeBuffer(LODIndex); 
...
void FSkeletalMeshRenderData::InitStaticRayTracingGeometry(int32 LODIndex)
{
	...
	if (LODData.NumReferencingStaticSkeletalMeshObjects == 0)
	{
		...
		ENQUEUE_RENDER_COMMAND(InitSkeletalRenderStaticRayTracingGeometry)(UE::RenderCommandPipe::SkeletalMesh,
			...
			LODIndex = LODIndex,
			SkelMeshRenderData = this,
			&RayTracingGeometry = LODData.StaticRayTracingGeometry,
			&SourceRayTracingGeometry = LODData.SourceRayTracingGeometry,
			&bReferencedByStaticSkeletalMeshObjects_RenderThread = LODData.bReferencedByStaticSkeletalMeshObjects_RenderThread](FRHICommandList& RHICmdList)
			{
				...
				static const FName DebugName("FSkeletalMeshLODRenderData_Static");
				...
				for (const FSkelMeshRenderSection& Section : *RenderSections)
				{
					...
					Segment.MaxVertices = PositionVertexBufferPtr->GetNumVertices();
				...
				Initializer.SourceGeometry = SourceRayTracingGeometry.GetRHI();
	...
	if (LODData.NumReferencingStaticSkeletalMeshObjects > 0)
	{
		...
		if (LODData.NumReferencingStaticSkeletalMeshObjects == 0)
		{
			ENQUEUE_RENDER_COMMAND(ResetStaticRayTracingGeometryFlag)(UE::RenderCommandPipe::SkeletalMesh,
				[&bReferencedByStaticSkeletalMeshObjects_RenderThread = LODData.bReferencedByStaticSkeletalMeshObjects_RenderThread]
				{
					bReferencedByStaticSkeletalMeshObjects_RenderThread = false;
				});

			BeginReleaseResource(&LODData.StaticRayTracingGeometry, &UE::RenderCommandPipe::SkeletalMesh);
		}
	}
#endif // RHI_RAYTRACING
}
