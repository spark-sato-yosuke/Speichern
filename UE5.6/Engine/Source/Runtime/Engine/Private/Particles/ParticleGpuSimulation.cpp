...
public:
	...
	virtual void InitRHI(FRHICommandListBase& RHICmdList) override
	{
		...
		PositionTextureRHI = CreateStateTextureRHI(PaticlePositonTextureName, GetParticlePositionFormat());

		// 16-bit per channel RGBA texture for velocity.
		check(!IsValidRef(VelocityTextureRHI));
		VelocityTextureRHI = CreateStateTextureRHI(PaticleVelocityTextureName, GetParticleVelocityFormat());
	...
	FParticleSimulationResources* ParticleSimulationResources)
{
	...
	if (ParticleCount > 0 && FeatureLevel >= ERHIFeatureLevel::SM5)
	{
		...
		const FRHIBufferCreateDesc CreateDesc =
			FRHIBufferCreateDesc::CreateVertex<FVector4f>(TEXT("BoundsVertexBuffer"), GroupCount * 2)
			.AddUsage(EBufferUsageFlags::Static | EBufferUsageFlags::UnorderedAccess | EBufferUsageFlags::KeepCPUAccessible)
			.DetermineInitialState();

		const int32 BufferSize = CreateDesc.Size;

		FBufferRHIRef BoundsVertexBufferRHI = RHICmdList.CreateBuffer(CreateDesc);
...
public:
	...
	virtual void InitRHI(FRHICommandListBase& RHICmdList) override
	{
		if (AlignedTileCount > 0)
		{
			...
			const FRHIBufferCreateDesc CreateDesc =
				FRHIBufferCreateDesc::CreateVertex<FVector4f>(TEXT("FParticleTileVertexBuffer"), BufferAlignedTileCount)
				.AddUsage(EBufferUsageFlags::Static | EBufferUsageFlags::KeepCPUAccessible | EBufferUsageFlags::ShaderResource)
				.DetermineInitialState();
...
public:
	...
	virtual void InitRHI(FRHICommandListBase& RHICmdList) override
	{
		if ( RHISupportsGPUParticles() )
		{
			...
			const FRHIBufferCreateDesc CreateDesc =
				FRHIBufferCreateDesc::CreateVertex<FParticleIndex>(TEXT("FGPUParticleVertexBuffer"), Count)
				.AddUsage(EBufferUsageFlags::Static | /*EBufferUsageFlags::KeepCPUAccessible |*/ EBufferUsageFlags::ShaderResource)
				.DetermineInitialState();
...
FGPUSpriteParticleEmitterInstance(IParticleEmitterInstanceOwner& InComponent, FFXSystem* InFXSystem, FGPUSpriteEmitterInfo& InEmitterInfo)
	...
	virtual bool IsDynamicDataRequired(UParticleLODLevel* InCurrentLODLevel) override
	{
		...
		bool bCanRender = (FXSystem != NULL) && (Component.GetFXSystem() == FXSystem);
	...
	virtual FDynamicEmitterDataBase* GetDynamicData(bool bSelected, ERHIFeatureLevel::Type InFeatureLevel) override
	{
		...
		check(FXSystem);
		check(Component.GetFXSystem() == FXSystem);
		...
		const UParticleSystem* Template = Component.GetTemplate();
		...
		DynamicData->SimulationBounds = Template->bUseFixedRelativeBoundingBox ? Template->FixedRelativeBoundingBox.TransformBy(Component.GetComponentTransform()) : Component.GetBounds().GetBox();
		...
		{
			ColorScaleOverLife = EmitterInfo.DynamicColorScale.GetValue(0.0f,Component.GetDistributionData());
		}
		if( EmitterInfo.DynamicAlphaScale.IsCreated() )
		{
			ColorScaleOverLife.W = EmitterInfo.DynamicAlphaScale.GetValue(0.0f,Component.GetDistributionData());
		}

		if( EmitterInfo.DynamicColor.IsCreated() )
		{
			ColorOverLife = EmitterInfo.DynamicColor.GetValue(0.0f,Component.GetDistributionData());
		}
		if( EmitterInfo.DynamicAlpha.IsCreated() )
		{
			ColorOverLife.W = EmitterInfo.DynamicAlpha.GetValue(0.0f,Component.GetDistributionData());
		}
	...
	virtual void Init() override
	{
		...
		RandomStream.Initialize(Component.GetRandomStream().FRand());
		...
		WeakWorld = Component.GetWeakWorld();
	}
	...
	virtual void Tick(float DeltaSeconds, bool bSuppressSpawning) override
	{
		...
		if (bEnabled)
		{
			...
			if (Component.GetIsWarmingUp() &&
				Component.GetWarmupTime() - SecondsSinceCreation > EmitterInfo.MaxLifetime)
			{
			...
			if (!bPreventSpawning && bValidEmitterTime && bValidLoop)
			{
				...
				if (Component.GetIsWarmingUp())
				{
					SimulateWarmupParticles(
						...
						NewParticleCount,
						Component.GetWarmupTime() - SecondsSinceCreation );
				}
			}
...
private:
		...
		else
		{
			if (AllocatedTiles.Num())
			{
				UE_LOG(LogParticles, Warning,
					TEXT("%s|%s|0x%016p [ReleaseSimulationResources] LEAKING %d tiles FXSystem=0x%016" PTRINT_x_FMT),
					*Component.GetName(), *Component.GetTemplate()->GetName(), this, AllocatedTiles.Num(), (PTRINT)FXSystem);
			}
		}
				...
				else
				{
					...
					if (NewTile == INDEX_NONE)
					{
						...
						UE_LOG(LogParticles,Warning,
							...
							*Component.GetTemplate()->GetName(), NumNewParticles, ParticleIndex);
	...
	FSpawnInfo GetNumParticlesToSpawn(float DeltaSeconds)
	{
		...
		const float RateScale = CurrentLODLevel->SpawnModule->RateScale.GetValue(EmitterTime, Component.GetDistributionData()) * CurrentLODLevel->SpawnModule->GetGlobalRateScale();
		float SpawnRate = CurrentLODLevel->SpawnModule->Rate.GetValue(EmitterTime, Component.GetDistributionData()) * RateScale;
	...
	virtual void UpdateBoundingBox(float DeltaSeconds) override
	{
		...
		FVector Origin = Component.GetComponentToWorld().GetTranslation();
...
FParticleEmitterInstance* FFXSystem::CreateGPUSpriteEmitterInstance(IParticleEmitterInstanceOwner& InComponent, FGPUSpriteEmitterInfo& EmitterInfo )
{
	return new FGPUSpriteParticleEmitterInstance(InComponent, this, EmitterInfo );
}
...
void ParticleGPUSimulationGlobalPSOCollector(const FSceneTexturesConfig& SceneTexturesConfig, int32 GlobalPSOCollectorIndex, TArray<FPSOPrecacheData>& PSOInitializers)
{
	...
	{				
		TShaderMapRef<FParticleTileVS> VertexShader(GlobalShaderMap);
		
		FGraphicsPipelineStateInitializer GraphicsPSOInit;
		GraphicsPSOInit.DepthStencilState = TStaticDepthStencilState<false, CF_Always>::GetRHI();
		GraphicsPSOInit.RasterizerState = TStaticRasterizerState<FM_Solid, CM_None>::GetRHI();
		GraphicsPSOInit.BlendState = TStaticBlendState<>::GetRHI();				
		GraphicsPSOInit.BoundShaderState.VertexDeclarationRHI = GetParticleTileVertexDeclaration(SceneTexturesConfig.FeatureLevel);
		GraphicsPSOInit.BoundShaderState.VertexShaderRHI = VertexShader.GetVertexShader();
		...
		RenderTargetsInfo.RenderTargetFormats[0] = FParticleStateTextures::GetParticlePositionFormat();
		RenderTargetsInfo.RenderTargetFlags[0] = ETextureCreateFlags::RenderTargetable | ETextureCreateFlags::ShaderResource;
		RenderTargetsInfo.RenderTargetFormats[1] = FParticleStateTextures::GetParticleVelocityFormat();
		RenderTargetsInfo.RenderTargetFlags[1] = ETextureCreateFlags::RenderTargetable | ETextureCreateFlags::ShaderResource;
		RenderTargetsInfo.RenderTargetsEnabled = 2;
	...
	TShaderMapRef<TParticleSimulationPS<PCM_None> > NoCollisionPixelShader(GlobalShaderMap);
	AddPSOInitializer(NoCollisionPixelShader.GetPixelShader());

	if (IsParticleCollisionModeSupported(ShaderPlatform, PCM_DepthBuffer))
	{
		TShaderMapRef<TParticleSimulationPS<PCM_DepthBuffer> > DepthCollisionPixelShader(GlobalShaderMap);
		AddPSOInitializer(DepthCollisionPixelShader.GetPixelShader());
	}
	
	if (IsParticleCollisionModeSupported(ShaderPlatform, PCM_DistanceField))
	{
		TShaderMapRef<TParticleSimulationPS<PCM_DistanceField> > DistanceFieldCollisionPixelShader(GlobalShaderMap);
		AddPSOInitializer(DistanceFieldCollisionPixelShader.GetPixelShader());
	}
}

FRegisterGlobalPSOCollectorFunction RegisterParticleGPUSimulationGlobalPSOCollector(&ParticleGPUSimulationGlobalPSOCollector, TEXT("ParticleGPUSimulationGlobalPSOCollector"));
