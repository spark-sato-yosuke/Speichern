...
void FSubUVBoundingGeometryBuffer::InitRHI(FRHICommandListBase& RHICmdList)
{
	if (Vertices->Num() > 0)
	{
		...
		ShaderResourceView = RHICmdList.CreateShaderResourceView(
			VertexBufferRHI,
...
{
	return SrcFormat == TSF_G8 || SrcFormat == TSF_R32F || SrcFormat == TSF_BGRA8 || SrcFormat == TSF_RGBA16 || SrcFormat == TSF_RGBA16F || SrcFormat == TSF_RGBA32F;
}
...
bool ComputeOpacityValue(const uint8* BGRA, int32 x, int32 y, int32 TextureSizeX,  EOpacitySourceMode OpacitySourceMode, ETextureSourceFormat SrcFormat, float AlphaThresholdF, uint8 AlphaThreshold8, uint16 AlphaThreshold16)
{
	...
	{
		const float Opacity = *((const float*)(BGRA + Offset));
	...
	{
		const float* RGBAf = (const float*)( BGRA + Offset);
		...
		{
			Opacity = (RGBAf[0] + RGBAf[1] + RGBAf[2]) / 3.0f;
		}
	...
	TArray<FText>& OutErrors, TArray<FText>& OutWarnings, TArray<FText>& OutInfo)
{
	...
	if (SubUVTexture)
	{
		...
		if (!IsSupportedFormat(SourceFormat))
		{
			...
			OutErrors.Add(
				FText::Format(LOCTEXT("CutoutNotSupported", "Image source format ({0}) is not in supported format for cutouts ({1}). It will be ignored."),
