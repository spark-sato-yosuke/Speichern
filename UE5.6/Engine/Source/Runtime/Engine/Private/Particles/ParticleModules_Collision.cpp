...
void UParticleModuleCollision::Spawn(const FSpawnContext& Context)
{
	...
	SPAWN_INIT;
	{
		...
		CollisionPayload.UsedDampingFactor = (FVector3f)DampingFactor.GetValue(Owner->EmitterTime, Context.GetDistributionData());
		CollisionPayload.UsedDampingFactorRotation = (FVector3f)DampingFactorRotation.GetValue(Owner->EmitterTime, Context.GetDistributionData());
		CollisionPayload.UsedCollisions = FMath::RoundToInt(MaxCollisions.GetValue(Owner->EmitterTime, Context.GetDistributionData()));
		CollisionPayload.Delay = DelayAmount.GetValue(Owner->EmitterTime, Context.GetDistributionData());
		if (CollisionPayload.Delay > Context.SpawnTime)
		{
...
void UParticleModuleCollision::Update(const FUpdateContext& Context)
{
	SCOPE_CYCLE_COUNTER(STAT_ParticleCollisionTime);
	FParticleEmitterInstance* Owner = &Context.Owner;
	UParticleSystemComponent* Component = Owner->Component.AsComponent();
	...
	const FTransform& OwnerTM = Component->GetAsyncComponentToWorld();
	...
	const TArray<FVector>& PlayerLocations = Component->GetPlayerLocations();
	TArray<float> PlayerLODDistanceFactor = Component->GetPlayerLODDistanceFactor();	//Make a copy because we need to square it later
	...
	if (World->IsGameWorld())
	{
		...
		if (bCollideOnlyIfVisible && ((World->TimeSeconds - Component->GetLastRenderTime()) > GParticleCollisionIgnoreInvisibleTime))
		{
			...
			if (MaxCollisionDistance < UE_OLD_WORLD_MAX)	// LWC_TODO: Fix needed to handle WORLD_MAX increase. Remove?
			{
				...
				if (Owner->ActiveParticles > 7)
				{
					if (CollisionInstPayload->CurrentLODBoundsCheckCount == 0)
					{
						FBox BoundingBox;
						BoundingBox.Init();
						if (Component->Template && Component->Template->bUseFixedRelativeBoundingBox)
						{
							BoundingBox = Component->Template->FixedRelativeBoundingBox.TransformBy(OwnerTM);
						}
						else
						{
							// A frame behind, but shouldn't be an issue...
							BoundingBox = Component->GetAsyncBounds().GetBox();
						}
	...
	BEGIN_UPDATE_LOOP;
	{
		...
		Location	= Particle.Location + (FVector)Particle.Velocity * Context.DeltaTime;
		...
		if (PerformCollisionCheck(Owner, &Particle, Hit, IgnoreActor, End, OldLocation, Extent))
		{
			...
			if (bIgnoreCollision == false)
			{
				if (bDecrementMaxCount && (bOnlyVerticalNormalsDecrementCount == true))
				{
					...
					{
						//UE_LOG(LogParticles, Log, TEXT("Particle from %s had a non-vertical hit!"), *Context.GetTemplateName());
				...
				if (CollisionPayload.UsedCollisions > 0)
				{
					if (LODLevel->RequiredModule->bUseLocalSpace)
					{
						...
						if (bApplyPhysics)
						{
							...
							{
								FVector vImpulse;
								vImpulse = -(NewVelocity - OldVelocity) * ParticleMass.GetValue(Particle.RelativeTime, Context.GetDistributionData());
					...
					else
					{
						...
						if (bApplyPhysics)
						{
							...
							{
								FVector vImpulse;
								vImpulse = -(vNewVelocity - vOldVelocity) * ParticleMass.GetValue(Particle.RelativeTime, Context.GetDistributionData());
								PrimitiveComponent->AddImpulseAtLocation(vImpulse, Hit.Location, Hit.BoneName);
							}
						}
					}
	...
	FHitResult& Hit, AActor* SourceActor, const FVector& End, const FVector& Start, const FVector& Extent)
{
	UParticleSystemComponent* Component = Owner->Component.AsComponent();
	check(Component);
	return Component->ParticleLineCheck(Hit, SourceActor, End, Start, Extent, ObjectParams);
}
