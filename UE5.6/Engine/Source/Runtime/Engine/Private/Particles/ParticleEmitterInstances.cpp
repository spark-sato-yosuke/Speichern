...
void FParticleEmitterInstance::Init()
{
	...
	UpdateTransforms();	
	Location				= Component.GetComponentTransform().GetLocation();
...
void FParticleEmitterInstance::UpdateTransforms()
{
	...
	FMatrix ComponentToWorld = Component.GetComponentToWorld().ToMatrixNoScale();
			...
			if ((NewMaxActiveParticles < 0) || (NewMaxActiveParticles > GEngine->MaxParticleResizeWarn))
			{
				UE_LOG(LogParticles, Warning, TEXT("Emitter::Resize> Invalid NewMaxActive (%d) for Emitter in PSys %s"),
					NewMaxActiveParticles, 
								Component.GetTemplate() ? *(Component.GetTemplate()->GetPathName()) 
													: *(Component.GetName()));
			}
...
float FParticleEmitterInstance::Tick_EmitterTimeSetup(float DeltaTime, UParticleLODLevel* InCurrentLODLevel)
{
	...
	if (Component.GetJustRegistered())
	{
		Location	= Component.GetComponentTransform().GetLocation();
		OldLocation	= Location;
	}
...
void FParticleEmitterInstance::SetCurrentLODIndex(int32 InLODIndex, bool bInFullyProcess)
{
	if (SpriteTemplate != NULL)
	{
		...
		if ((Component.IsGameWorld() == true) && (CurrentLODLevel->bEnabled == false))
		{
...
{
	if (Component.HasWorld())
	{
		UE_LOG(LogParticles,Verbose,
			TEXT("FParticleEmitterInstance::Rewind @ %fs %s"), Component.GetWorldTimeSeconds(),
...
void FParticleEmitterInstance::UpdateBoundingBox(float DeltaTime)
{
	SCOPE_CYCLE_COUNTER(STAT_ParticleUpdateBounds);
	{
		...
		bool bUpdateBox = ((Component.GetIsWarmingUp() == false) && (Template != NULL) && (Template->bUseFixedRelativeBoundingBox == false));
			...
			if (LODIndex > 0)
			{
				if ((InterpolationMethod != PSUVIM_None) && (bHasSubUV == false))
				{
					UE_LOG(LogParticles, Warning, TEXT("Emitter w/ mismatched SubUV settings: %s"),
							Component.GetTemplate() ? *(Component.GetTemplate()->GetPathName()) : *(Component.GetPathName()));
				}
				...
				if (BurstIdx < LocalBurstFired.Fired.Num())
				{
					if (LocalBurstFired.Fired[BurstIdx] == false)
					{
						...
						if (EmitterTime >= BurstEntry->Time)
						{
							...
							float Scale = LODLevel->SpawnModule->BurstScale.GetValue(EmitterTime, Component.GetDistributionData());
...
#if !(UE_BUILD_SHIPPING || UE_BUILD_TEST)
	{
		if (Component.HasWorldSettings())
		{
			const int32 SizeScalar = sizeof(FParticleSpriteVertex);

			int32 MyIndex = -1;
			TArrayView<FParticleEmitterInstance*> EmitterInstances = Component.GetEmitterInstances();
			for (int32 CheckIdx = 0; CheckIdx < EmitterInstances.Num(); CheckIdx++)
			{
				if (EmitterInstances[CheckIdx] == this)
				{
					MyIndex = CheckIdx;
					break;
				}
			...
			FString ErrorMessage = 
				...
				Component.GetTemplate() ? *Component.GetTemplate()->GetPathName() : TEXT("No template"));
...
float FParticleEmitterInstance::Spawn(float DeltaTime)
{
	...
	if (SpriteTemplate->QualityLevelSpawnRateScale > 0.0f)
	{
		...
		{
			float RateScale = LODLevel->SpawnModule->RateScale.GetValue(EmitterTime, Component.GetDistributionData()) * LODLevel->SpawnModule->GetGlobalRateScale();
	...
	auto SpawnInternal = [&](bool bLegacySpawnBehavior)
	{
		...
		for (int32 i = 0; i < Count; i++)
		{
			...
			if (!ensure(ParticleData && ParticleIndices))
			{
				...
				if (!bErrorReported)
				{
					UE_LOG(LogParticles, Error, TEXT("Detected null particles. Template : %s, Component %s"), *GetNameSafe(Component.IsValidLowLevel() ? Component.GetTemplate() : nullptr), *Component.GetFullName());
					bErrorReported = true;
				}
				ActiveParticles = 0;
				Count = 0;
			...
			{
				UE_LOG(LogParticles, Error, TEXT("Detected corrupted particle indices. Template : %s, Component %s"), *GetNameSafe(Component.IsValidLowLevel() ? Component.GetTemplate() : nullptr), *Component.GetFullName());
			...
			for (int32 ModuleIndex = 0; ModuleIndex < LODLevel->SpawnModules.Num(); ModuleIndex++)
			{
				...
				if (SpawnModule->bEnabled)
				{
					...
					ensureMsgf(!Particle->Location.ContainsNaN(), TEXT("NaN in Particle Location. Template: %s, Component: %s"), *GetNameSafe(Component.GetTemplate()), *Component.GetPathName());
				}
			}
...
void FParticleEmitterInstance::SetupEmitterDuration()
{
	...
	for (int32 LODIndex = 0; LODIndex < SpriteTemplate->LODLevels.Num(); LODIndex++)
	{
		...
		CurrentDelay = RequiredModule->EmitterDelay + Component.GetEmitterDelay();
		if (RequiredModule->bEmitterDelayUseRange)
		{
			const float	Rand	= RandomStream.FRand();
			CurrentDelay	    = RequiredModule->EmitterDelayLow + 
				((RequiredModule->EmitterDelay - RequiredModule->EmitterDelayLow) * Rand) + Component.GetEmitterDelay();
		}
...
void FParticleEmitterInstance::ProcessParticleEvents(float DeltaTime, bool bSuppressSpawning)
{
	...
	{
		TArrayView<FParticleEventSpawnData> SpawnEvents = Component.GetSpawnEvents();
		TArrayView<FParticleEventDeathData> DeathEvents = Component.GetDeathEvents();
		TArrayView<FParticleEventCollideData> CollisionEvents = Component.GetCollisionEvents();
		TArrayView<FParticleEventBurstData> BurstEvents = Component.GetBurstEvents();
		TArrayView<FParticleEventKismetData> KismetEvents = Component.GetKismetEvents();
		...
		for (int32 EventModIndex = 0; EventModIndex < LODLevel->EventReceiverModules.Num(); EventModIndex++)
		{
			...
			if (EventRcvr->WillProcessParticleEvent(EPET_Spawn) && (SpawnEvents.Num() > 0))
			{
				for (EventIndex = 0; EventIndex < SpawnEvents.Num(); EventIndex++)
				{
					EventRcvr->ProcessParticleEvent(this, SpawnEvents[EventIndex], DeltaTime);
				}
			}

			if (EventRcvr->WillProcessParticleEvent(EPET_Death) && (DeathEvents.Num() > 0))
			{
				for (EventIndex = 0; EventIndex < DeathEvents.Num(); EventIndex++)
				{
					EventRcvr->ProcessParticleEvent(this, DeathEvents[EventIndex], DeltaTime);
				}
			}

			if (EventRcvr->WillProcessParticleEvent(EPET_Collision) && (CollisionEvents.Num() > 0))
			{
				for (EventIndex = 0; EventIndex < CollisionEvents.Num(); EventIndex++)
				{
					EventRcvr->ProcessParticleEvent(this, CollisionEvents[EventIndex], DeltaTime);
				}
			}

			if (EventRcvr->WillProcessParticleEvent(EPET_Burst) && (BurstEvents.Num() > 0))
			{
				for (EventIndex = 0; EventIndex < BurstEvents.Num(); EventIndex++)
				{
					EventRcvr->ProcessParticleEvent(this, BurstEvents[EventIndex], DeltaTime);
				}
			}

			if (EventRcvr->WillProcessParticleEvent(EPET_Blueprint) && (KismetEvents.Num() > 0))
			{
				for (EventIndex = 0; EventIndex < KismetEvents.Num(); EventIndex++)
				{
					EventRcvr->ProcessParticleEvent(this, KismetEvents[EventIndex], DeltaTime);
				}
			}
		}
	}
...
{
	UParticleLODLevel* LODLevel = SpriteTemplate->GetCurrentLODLevel(this);
	TArrayView<TObjectPtr<UMaterialInterface>> EmitterMaterials = Component.GetEmitterMaterials();

	bool bOverridden = false;
	if( LODLevel && LODLevel->RequiredModule && Component.GetTemplate() )
	{
	        TArray<FName>& NamedOverrides = LODLevel->RequiredModule->NamedMaterialOverrides;
	        const TArray<FNamedEmitterMaterial>& Slots = Component.GetTemplate()->NamedMaterialSlots;
 ...
 */
/** Constructor	*/
FParticleSpriteEmitterInstance::FParticleSpriteEmitterInstance(IParticleEmitterInstanceOwner& InComponent) :
...
{
	if (CumulativeResourceSize.GetResourceSizeMode() == EResourceSizeMode::EstimatedTotal || (Component.GetSceneProxy()))
	{
		int32 MaxActiveParticleDataSize = (ParticleData != NULL) ? (MaxActiveParticles * ParticleStride) : 0;
		int32 MaxActiveParticleIndexSize = (ParticleIndices != NULL) ? (MaxActiveParticles * sizeof(uint16)) : 0;
...
void FParticleMeshEmitterInstance::Tick_MaterialOverrides(int32 EmitterIndex)
{
	...
	if (EmitterMaterials.IsValidIndex(EmitterIndex))
	{
		if (EmitterMaterials[EmitterIndex])
		{
			...
			{
				CurMat = EmitterMaterials[EmitterIndex];
			}
		}
	}	
...
void FParticleMeshEmitterInstance::UpdateBoundingBox(float DeltaTime)
{
	...
	if (ActiveParticles > 0)
	{
		...
		if (Component.GetIsWarmingUp() == false)
		{	
			...
			{
				//UE_LOG(LogParticles, Log, TEXT("MeshEmitter with no mesh set?? - %s"), Component.GetTemplate() ? *(Component.GetTemplate()->GetPathName()) : TEXT("??????"));
