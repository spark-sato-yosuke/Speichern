...
bool FShaderCompilingManager::HandlePotentialRetry(TMap<int32, FShaderMapFinalizeResults>& CompletedShaderMaps)
{
	...
	for (TMap<int32, FShaderMapFinalizeResults>::TIterator It(CompletedShaderMaps); It; ++It)
	{
		...
		// interactive retries - prompt for global/default shaders that have errors
		if (!FApp::IsUnattended() && !Results.bAllJobsSucceeded)
		{
		...
		int32 OriginalFinishedJobCount = Results.FinishedJobs.Num();
		bool bAnyReissued = false;

		// retries based on r.DumpShaderDebugInfo=2/3
		for (int32 FinishedJobIndex = 0; FinishedJobIndex < Results.FinishedJobs.Num(); ++FinishedJobIndex)
		{
			...
			bool bReissue = false;
			Job->ForEachSingleShaderJob([&bReissue, bRetryCompile](FShaderCompileJob& SingleJob)
				{
					// this will set bReissue to true if any stage of a pipeline job failed, so the entire pipeline job will be 
					// resubmitted, but only the failed stage(s) will have debug info enabled on the re-run.
					bool bShouldRecompileForDebug = GShaderCompilingManager->ShouldRecompileToDumpShaderDebugInfo(SingleJob);
					// reissue the job if either we want to dump debug info for it, or a retry was requested above for any failed jobs
					bReissue = bShouldRecompileForDebug || (bRetryCompile && !SingleJob.bSucceeded);
					if (bShouldRecompileForDebug)
					{
			...
			if (bReissue)
			{
				bAnyReissued = true;
				ReissueJobs.Add(Job);
				// remove the job we will reissue from the finished jobs list; no point in allowing shrinking as it will grow again below when the reissued jobs finish
	...
	)
{
	...
	const bool bSubstrate = Substrate::IsSubstrateEnabled() && IsSubstrateSupportForShaderPipeline(Input);
	{
		...
		SET_SHADER_DEFINE(Input.Environment, SUBTRATE_GBUFFER_FORMAT, bSubstrate && !Substrate::IsSubstrateBlendableGBufferEnabled(Target.GetPlatform()) ? 1 : 0);
	...
	if (RHIGetBindlessSupport(ShaderPlatform) != ERHIBindlessSupport::Unsupported && !Input.Environment.CompilerFlags.Contains(CFLAG_ForceBindful))
	{
		...
		const bool bIsInlineRaytracingShader = Input.Environment.CompilerFlags.Contains(CFLAG_InlineRayTracing);
		const bool bInlineRayTracingRequiresBindless = FDataDrivenShaderPlatformInfo::GetRequiresBindlessForInlineRayTracing(EShaderPlatform(Target.Platform));
		...
		if (bIsInlineRaytracingShader && bInlineRayTracingRequiresBindless)
		{
			check(ResourcesConfig != ERHIBindlessConfiguration::Disabled);
			check(SamplersConfig != ERHIBindlessConfiguration::Disabled);
		}

		bool EnableBindlessForInlineRaytracing = (bIsInlineRaytracingShader && bInlineRayTracingRequiresBindless && ResourcesConfig != ERHIBindlessConfiguration::Disabled && SamplersConfig != ERHIBindlessConfiguration::Disabled);
		if (ResourcesConfig == ERHIBindlessConfiguration::AllShaders || (ResourcesConfig == ERHIBindlessConfiguration::RayTracingShaders && bIsRaytracingShader) || EnableBindlessForInlineRaytracing)
		{
		...
		if (SamplersConfig == ERHIBindlessConfiguration::AllShaders || (SamplersConfig == ERHIBindlessConfiguration::RayTracingShaders && bIsRaytracingShader) || EnableBindlessForInlineRaytracing)
		{
			bEnableBindlessMacro = true;
...
{
	// as there is no assets tags, there is no need to register this
