...
void ApplyFetchEnvironmentInternal(FShaderMaterialPropertyDefines& SrcDefines, const ContainerType& Container)
{
	...
	FETCH_COMPILE_INT(SUBTRATE_GBUFFER_FORMAT);
...
static void SetStandardGBufferSlots(
	...
	bool bIsSubstrateMaterialNewGBuffer)
{
	auto GetUsageIfNotSubstrate = [bIsSubstrateMaterial, bIsSubstrateMaterialNewGBuffer](bool bWritten, bool bForceUsed = false)
	{
		return GetGBufferSlotUsage(bWritten && (!bIsSubstrateMaterial || !bIsSubstrateMaterialNewGBuffer), bForceUsed && (!bIsSubstrateMaterial || !bIsSubstrateMaterialNewGBuffer));
	};
	...
	ERHIFeatureLevel::Type FEATURE_LEVEL)
{
	...
	bool bIsSubstrateNewGBuffer = Mat.SUBTRATE_GBUFFER_FORMAT==1; // Similarly to FetchFullGBufferInfo, we do not check for MATERIAL_IS_SUBSTRATE as this is decided per project.

	// Substrate doesn't use gbuffer, and thus doesn't need CustomData
	const bool bUseCustomData = !bIsSubstrateMaterial || !bIsSubstrateNewGBuffer;
	...
	if (Mat.MATERIAL_SHADINGMODEL_UNLIT)
	{
		...
		SetStandardGBufferSlots(Slots, true, false, bHasVelocity, bWritesVelocity, bHasStaticLightingWritten, bIsSubstrateMaterial, bIsSubstrateNewGBuffer);
	}
...
void FShaderCompileUtilities::ApplyDerivedDefines(FShaderCompilerEnvironment& OutEnvironment, FShaderCompilerEnvironment * SharedEnvironment, const EShaderPlatform Platform)
{
	...
	if (MaterialDefines.MATERIAL_IS_SUBSTRATE && MaterialDefines.SUBTRATE_GBUFFER_FORMAT==1 && SubstrateFirstMRT > 0)
	{
