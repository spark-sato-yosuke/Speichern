...
static TAutoConsoleVariable<bool> CVarStaticMeshRayTracingProxies(
	TEXT("r.StaticMesh.RayTracingProxies"),
	true,
	TEXT("Whether to use dedicated ray tracing LODs."),
	ECVF_ReadOnly);
...
static bool ShouldStripNaniteFallbackMesh(const ITargetPlatform* TargetPlatform, const UStaticMesh* StaticMesh)
{
	return DoesTargetPlatformSupportNanite(TargetPlatform)
		&& TargetPlatform->ShouldStripNaniteFallbackMeshes()
		&& StaticMesh->NaniteSettings.GenerateFallback != ENaniteGenerateFallback::Enabled
...
void FStaticMeshAsyncBuildWorker::DoWork()
{
	...
	{
		UE::Tasks::FCancellationTokenScope CancellationScope(PostLoadContext->CancellationToken);
		StaticMesh->ExecutePostLoadInternal(*PostLoadContext);
	}

	if (BuildContext.IsValid())
	{
		UE::Tasks::FCancellationTokenScope CancellationScope(BuildContext->CancellationToken);
		BuildContext->bHasRenderDataChanged = StaticMesh->ExecuteBuildInternal(BuildContext->BuildParameters);
	}
}

void FStaticMeshAsyncBuildWorker::MarkAsCanceled()
{
	if (FStaticMeshCompilingManager::Get().IsAsyncCompilationCancelable())
	{
	    TRACE_CPUPROFILER_EVENT_SCOPE(FStaticMeshAsyncBuildWorker::MarkAsCanceled);
    
	    if (PostLoadContext.IsValid())
	    {
		    PostLoadContext->CancellationToken.Cancel();
	    }
    
	    if (BuildContext.IsValid())
	    {
		    BuildContext->CancellationToken.Cancel();
	    }
	}
}
...
void FStaticMeshSectionAreaWeightedTriangleSamplerBuffer::InitRHI(FRHICommandListBase& RHICmdList)
{
	...
	if (Samplers && Samplers->Num() > 0)
	{
		...
		TRHIBufferInitializer<SectionTriangleInfo> Initializer = RHICmdList.CreateBufferInitializer(Desc);

		// Now compute the alias look up table for unifor; distribution for all section and all triangles
		for (uint32 SectionIndex = 0; SectionIndex < AllSectionCount; SectionIndex++)
		{
			const FStaticMeshSectionAreaWeightedTriangleSampler& TriangleSampler = (*Samplers)[SectionIndex];
			TConstArrayView<float> ProbTris = TriangleSampler.GetProb();
			TConstArrayView<int32> AliasTris = TriangleSampler.GetAlias();
			const uint32 NumTriangle = TriangleSampler.GetNumEntries();

			for (uint32 TriangleIndex = 0; TriangleIndex < NumTriangle; TriangleIndex++)
			{
				SectionTriangleInfo NewTriangleInfo = { ProbTris[TriangleIndex], (uint32)AliasTris[TriangleIndex] };
				Initializer[TriangleIndex] = NewTriangleInfo;
			}
		}
		BufferSectionTriangleRHI = Initializer.Finalize();
		BufferSectionTriangleSRV = RHICmdList.CreateShaderResourceView(
			BufferSectionTriangleRHI, 
...
bool FStaticMeshLODResources::IsLODCookedOut(const ITargetPlatform* TargetPlatform, UStaticMesh* StaticMesh, bool bIsBelowMinLOD)
{
	check(StaticMesh);
#if WITH_EDITOR
	if (TargetPlatform && ShouldStripNaniteFallbackMesh(TargetPlatform, StaticMesh))
	{
		return true;
	}
...
void FStaticMeshLODResources::SerializeBuffers(FArchive& Ar, UStaticMesh* OwnerStaticMesh, uint8 InStripFlags, FStaticMeshBuffersSize& OutBuffersSize)
{
	...
	if (bSerializeRayTracingGeometry)
	{
		...
		Ar << SerializedRayTracingGeometry->RawDataHeader;
		...
		{
			SerializedRayTracingGeometry->RawDataHeader = FRayTracingGeometryOfflineDataHeader();
			SerializedRayTracingGeometry->RawData.Discard();
		}
	}
...
void FStaticMeshVertexBuffers::InitComponentVF(FLocalVertexFactory* VertexFactory, int32 LightMapCoordinateIndex, bool bOverrideColorVertexBuffer, FLocalVertexFactory::FDataType& OutData) const
{
	PositionVertexBuffer.BindPositionVertexBuffer(VertexFactory, OutData);
	StaticMeshVertexBuffer.BindTangentVertexBuffer(VertexFactory, OutData);
	StaticMeshVertexBuffer.BindPackedTexCoordVertexBuffer(VertexFactory, OutData);
	StaticMeshVertexBuffer.BindLightMapVertexBuffer(VertexFactory, OutData, LightMapCoordinateIndex);
...
void FStaticMeshLODResources::InitResources(UStaticMesh* Parent, int32 LODIndex)
{
	...
	if(bHasWireframeIndices)
	{
		BeginInitResource(OwnerName, &AdditionalIndexBuffers->WireframeIndexBuffer);
	}
	...
	if (bHasReversedIndices)
	{
		BeginInitResource(OwnerName, &AdditionalIndexBuffers->ReversedIndexBuffer);
	}

	if (bHasDepthOnlyIndices)
	{
		BeginInitResource(OwnerName, &DepthOnlyIndexBuffer);
	}

	if (bHasReversedDepthOnlyIndices)
	{
		BeginInitResource(OwnerName, &AdditionalIndexBuffers->ReversedDepthOnlyIndexBuffer);
	}
	...
	if (Parent && Parent->bSupportGpuUniformlyDistributedSampling && Parent->bSupportUniformlyDistributedSampling && (AreaWeightedSampler.GetNumEntries() > 0))
	{
		...
		BeginInitResource(OwnerName, &AreaWeightedSectionSamplersBuffer);
	}
...
void FStaticMeshRayTracingProxyLOD::InitResources(UStaticMesh* Parent, int32 LODIndex)
{
	...
	{
		if (RayTracingGeometry->RawDataHeader.IsValid())
		{
			FRayTracingAccelerationStructureOfflineMetadata OfflineMetadata = RHIGetRayTracingGeometryOfflineMetadata(RayTracingGeometry->RawDataHeader);

			OfflineBVHOffset += OfflineMetadata.SerializedOffset;
			OfflineBVHSize = OfflineMetadata.SerializedSize;
		}
...
void FStaticMeshRayTracingProxyLOD::Serialize(FArchive& Ar, UObject* Owner, int32 Index)
{
...
#if WITH_EDITOR
	// if the ray tracing proxy is using the fallback mesh but it will be stripped from the cooked build
	// the ray tracing proxy needs to adopt the fallback mesh
	const bool bAdoptFallbackMesh = ShouldStripNaniteFallbackMesh(Ar.CookingTarget(), OwnerStaticMesh);
#else
	const bool bAdoptFallbackMesh = false;
#endif

	const bool bOwnsBuffersBackup = bOwnsBuffers;
	const bool bOwnsRayTracingGeometryBackup = bOwnsRayTracingGeometry;

	if (bAdoptFallbackMesh)
	{
		bOwnsBuffers = true;
		bOwnsRayTracingGeometry = true;
	}
	...
	// TODO: Support DDC streaming in editor
	bool bTmpBuffersInlined = !Ar.IsCooking();
	Ar << bTmpBuffersInlined;
	bBuffersInlined = bTmpBuffersInlined;

	if (bBuffersInlined)
	{
		SerializeBuffers(Ar, OwnerStaticMesh, 0);
		SerializeRayTracingGeometry(Ar, OwnerStaticMesh, 0);
	...
	else
	{
		...
		if (Ar.IsSaving())
		{
			checkf(Ar.IsCooking(), TEXT("Only expected to serialize buffers to FByteBulkData when cooking."));
...

			{
				...
				SerializeBuffers(MemWriter, OwnerStaticMesh, 0);
				SerializeRayTracingGeometry(MemWriter, OwnerStaticMesh, 0);
			}
	...
	if (bAdoptFallbackMesh)
	{
		// revert back to original state to prevent issues with ownership
		bOwnsBuffers = bOwnsBuffersBackup;
		bOwnsRayTracingGeometry = bOwnsRayTracingGeometryBackup;
	}
}
...
void FStaticMeshRayTracingProxyLOD::SerializeBuffers(FArchive& Ar, UStaticMesh* OwnerStaticMesh, uint8 InStripFlags)
{
	if (bOwnsBuffers)
	{
		const int64 BeforeOffset = Ar.Tell();
		...
		const int64 AfterOffset = Ar.Tell();

		if (Ar.IsSaving())
		{
			BuffersSize = AfterOffset - BeforeOffset;
		}
	}
}

void FStaticMeshRayTracingProxyLOD::SerializeRayTracingGeometry(FArchive& Ar, UStaticMesh* OwnerStaticMesh, uint8 InStripFlags)
{
	if (bOwnsRayTracingGeometry)
	{
		// logic similar to TArray::BulkSerialize(...)
		// but want to track specific offset of RawData to be able to stream-in specific chunks based on FRayTracingAccelerationStructureOfflineMetadata

		uint32 RawDataNum = RayTracingGeometry->RawData.Num();
		Ar << RawDataNum;

		if (Ar.IsLoading())
		{
			RayTracingGeometry->RawData.Empty(RawDataNum);
			RayTracingGeometry->RawData.AddUninitialized(RawDataNum);
		}

		const int64 BeforeOffset = Ar.Tell();

		Ar.Serialize(RayTracingGeometry->RawData.GetData(), RawDataNum * RayTracingGeometry->RawData.GetTypeSize());

		const int64 AfterOffset = Ar.Tell();

		if (Ar.IsSaving())
		{
			OfflineBVHOffset = BeforeOffset;
			OfflineBVHSize = AfterOffset - BeforeOffset;
		}
...
void FStaticMeshRayTracingProxyLOD::SerializeMetaData(FArchive& Ar)
{
	...
	if (bOwnsRayTracingGeometry)
	{
		...
		Ar << RayTracingGeometry->RawDataHeader;

		if (Ar.IsLoading() && RayTracing::ShouldForceRuntimeBLAS())
		{
			RayTracingGeometry->RawDataHeader = FRayTracingGeometryOfflineDataHeader();
		}
	}
}
...
{
	const uint32 RequestSize = BuffersSize + OfflineBVHSize;
	check(RequestSize < StreamableData.GetBulkDataSize());
	return RequestSize;
}

uint32 FStaticMeshRayTracingProxyLOD::GetRequestSizeBVH() const
{
	const uint32 RequestSize = OfflineBVHSize;
	check(RequestSize < StreamableData.GetBulkDataSize());
	return RequestSize;
}
...
{
	const uint32 RequestSize = BuffersSize;
	check(RequestSize < StreamableData.GetBulkDataSize());
	return RequestSize;
}
...
{
	return bBuffersStreamedIn;
}

bool FStaticMeshRayTracingProxyLOD::IsBVHStreamedIn() const
{
	return bRayTracingGeometryStreamedIn;
}

void FStaticMeshRayTracingProxyLOD::IssueRequest(FBulkDataBatchRequest& Request, FIoBuffer& RequestBuffer, bool bBuffersOnly)
{
	checkf(!StreamableData.IsBulkDataLoaded(), TEXT("Bulk data should not be loaded yet!"));
	...
	const bool bRequestBuffers = BuffersSize > 0;
	const bool bRequestOfflineBVH = OfflineBVHSize > 0 && !bBuffersOnly;

	uint32 BatchSize = 0;
	BatchSize += bRequestBuffers ? 1 : 0;
	BatchSize += bRequestOfflineBVH ? 1 : 0;
	...
	TRACE_IOSTORE_METADATA_SCOPE_TAG("StaticMeshRayTracingProxyLOD");
	FBulkDataBatchRequest::FScatterGatherBuilder Batch = FBulkDataBatchRequest::ScatterGather(BatchSize);
	if (bRequestBuffers)
	{
		Batch.Read(StreamableData, 0, BuffersSize);
	}
	if (bRequestOfflineBVH)
	{
		Batch.Read(StreamableData, OfflineBVHOffset, OfflineBVHSize);
	}
	Batch.Issue(RequestBuffer, AIOP_Low, [](FBulkDataRequest::EStatus)
		{
#if !UE_BUILD_SHIPPING
			// On some platforms the IO is too fast to test cancellation requests timing issues.
			if (FRenderAssetStreamingSettings::ExtraIOLatencyRayTracing > 0)
			{
				FPlatformProcess::Sleep(FRenderAssetStreamingSettings::ExtraIOLatencyRayTracing * .001f); // Slow down the streaming.
			}
#endif
		}, Request);
}

void FStaticMeshRayTracingProxyLOD::InitWithStreamedData(FRHICommandListBase& RHICmdList, FMemoryView StreamedData, bool bBuffersOnly)
{
	FMemoryReaderView Ar(StreamedData, /*bIsPersistent=*/ true);

	SerializeBuffers(Ar, /*OwnerStaticMesh*/nullptr, /*InStripFlags*/0);

	if (bOwnsRayTracingGeometry && OfflineBVHSize > 0 && !bBuffersOnly)
	{
		RayTracingGeometry->RawData.Empty(OfflineBVHSize);
		RayTracingGeometry->RawData.AddUninitialized(OfflineBVHSize);
		Ar.Serialize(RayTracingGeometry->RawData.GetData(), OfflineBVHSize);
	...
	if (bOwnsBuffers)
	{
		FStaticMeshVertexBuffers& VBs = *VertexBuffers;
		FBufferRHIRef TangentsVertexBuffer = VBs.StaticMeshVertexBuffer.CreateTangentsRHIBuffer(RHICmdList);
		FBufferRHIRef TexCoordVertexBuffer = VBs.StaticMeshVertexBuffer.CreateTexCoordRHIBuffer(RHICmdList);
		FBufferRHIRef PositionVertexBuffer = VBs.PositionVertexBuffer.CreateRHIBuffer(RHICmdList);
		FBufferRHIRef ColorVertexBuffer = VBs.ColorVertexBuffer.CreateRHIBuffer(RHICmdList);
		FBufferRHIRef IndexBufferRHI = IndexBuffer->CreateRHIBuffer(RHICmdList);
		...
		IndexBuffer->InitRHIForStreaming(IndexBufferRHI, Batcher);

		bBuffersStreamedIn = true;
	}

#if RHI_RAYTRACING
	if (bOwnsRayTracingGeometry && !bBuffersOnly)
	{
		{
			FRHIResourceReplaceBatcher Batcher(RHICmdList, 1);
			FRayTracingGeometryInitializer IntermediateInitializer = RayTracingGeometry->Initializer;
			IntermediateInitializer.Type = ERayTracingGeometryInitializerType::StreamingSource;

			if (!RayTracingGeometry->RawData.IsEmpty())
			{
				IntermediateInitializer.OfflineData = &RayTracingGeometry->RawData;
			}

			FRayTracingGeometryRHIRef IntermediateRayTracingGeometry = RHICmdList.CreateRayTracingGeometry(IntermediateInitializer);

			RayTracingGeometry->SetRequiresBuild(IntermediateInitializer.OfflineData == nullptr || IntermediateRayTracingGeometry->IsCompressed());

			RayTracingGeometry->InitRHIForStreaming(IntermediateRayTracingGeometry, Batcher);
		...
		if (!RayTracingGeometry->GetRequiresBuild())
		{
			// only need to request here if no build will be requested since build path already requests update as necessary
			GRayTracingGeometryManager->RequestUpdateCachedRenderState(RayTracingGeometry->GroupHandle);
		}

		RayTracingGeometry->RequestBuildIfNeeded(RHICmdList, ERTAccelerationStructureBuildPriority::Normal);

		bRayTracingGeometryStreamedIn = true;
	}
#endif // RHI_RAYTRACING
}
...
{
	ReleaseBVHForStreaming(Batcher);
	ReleaseBuffersForStreaming(Batcher);
}

void FStaticMeshRayTracingProxyLOD::ReleaseBVHForStreaming(FRHIResourceReplaceBatcher& Batcher)
{
#if RHI_RAYTRACING
	if (bOwnsRayTracingGeometry && bRayTracingGeometryStreamedIn)
	{
		RayTracingGeometry->ReleaseRHIForStreaming(Batcher);

		bRayTracingGeometryStreamedIn = false;
	}
#endif // RHI_RAYTRACING
}
...
{
	if (bOwnsBuffers && bBuffersStreamedIn)
	{
...
void FStaticMeshRayTracingProxy::Serialize(FArchive& Ar, UObject* Owner, FStaticMeshRenderData* RenderData, bool bCooked)
{
	...
	const bool bUsingRenderingLODsBackup = bUsingRenderingLODs;

	if (bAdoptFallbackMesh)
	{
		bUsingRenderingLODs = false;
	}
	...
	{
		// revert back to original state to prevent issues with ownership
		bUsingRenderingLODs = bUsingRenderingLODsBackup;
	}
#endif
}
...
#if WITH_EDITORONLY_DATA
	if (bHasNaniteFallbackMesh && Ar.IsCooking() && Ar.IsSaving())
	{
		bHasNaniteFallbackMesh = (Owner->NaniteSettings.GenerateFallback == ENaniteGenerateFallback::Enabled || !Ar.CookingTarget()->ShouldStripNaniteFallbackMeshes());
	}
#endif // WITH_EDITORONLY_DATA

	// pack multiple flags into a single uint8
	static const uint8 LODsShareStaticLightingFlag = 1;
	static const uint8 HasNaniteFallbackMeshFlag = 2;

	uint8 bRenderDataFlags = 0;
	bRenderDataFlags |= bLODsShareStaticLighting ? LODsShareStaticLightingFlag : 0;
	bRenderDataFlags |= bHasNaniteFallbackMesh ? HasNaniteFallbackMeshFlag : 0;

	Ar << bRenderDataFlags;

	bLODsShareStaticLighting = (bRenderDataFlags & LODsShareStaticLightingFlag) != 0;
	bHasNaniteFallbackMesh = (bRenderDataFlags & HasNaniteFallbackMeshFlag) != 0;
...
#if WITH_EDITORONLY_DATA
	// store a copy of NaniteSettings.GenerateFallback in FStaticMeshRenderData
	// to be used in HasNaniteFallbackMesh(...) since we don't have access to the owner's UStaticMesh at that point
	bForceGenerateNaniteFallbackMesh = Owner->NaniteSettings.GenerateFallback == ENaniteGenerateFallback::Enabled;
#endif
...
bool FStaticMeshRenderData::HasNaniteFallbackMesh(EShaderPlatform ShaderPlatform) const
{
	...
	return bForceGenerateNaniteFallbackMesh;
#else
...
static FString BuildStaticMeshDerivedDataKeySuffix(const ITargetPlatform* TargetPlatform, UStaticMesh* Mesh, const FStaticMeshLODGroup& LODGroup)
{
	...
	if (Mesh->IsNaniteEnabled())
	{
		...
		if (ShouldStripNaniteFallbackMesh(TargetPlatform, Mesh))
		{
			KeySuffix += TEXT("NFMSV2");
		}
	}
	...
	// Add hi-res mesh description into DDC key (only on platforms that build from it)
	if (DoesTargetPlatformSupportNanite(TargetPlatform) && !Mesh->GetHiResSourceModel().GetMeshDescriptionBulkData()->IsEmpty())
	{
	...
	if (Mesh->bSupportRayTracing && TargetPlatform->UsesRayTracing())
	{
		...
		if (CVarStaticMeshRayTracingProxies.GetValueOnAnyThread() && (Mesh->RayTracingProxySettings.bEnabled || ShouldGenerateRayTracingProxiesByDefault()))
		{
			TempBytes.Reset();
			FMemoryWriter Ar(TempBytes, /*bIsPersistent=*/ true);
			SerializeRayTracingProxySettingsForDDC(Ar, Mesh->RayTracingProxySettings);
...
void FStaticMeshRenderData::Cache(const ITargetPlatform* TargetPlatform, UStaticMesh* Owner, const FStaticMeshLODSettings& LODSettings)
{
...
#endif
		FString NewDerivedDataKey = BuildStaticMeshDerivedDataKey(KeySuffix);
		if (NumCacheAttempts == 0 || !IsRunningCookCommandlet())
		{
			DerivedDataKey = MoveTemp(NewDerivedDataKey);
		}
		else if (DerivedDataKey != NewDerivedDataKey)
		{
			UE_LOG(LogStaticMesh, Error,
				TEXT("Indeterminism in FStaticMeshRenderData::Cache call to BuildStaticMeshDerivedDataKey. Using the previously computed RenderData even though the key is now different.")
				TEXT("\n\tStaticMesh=%s, Platform=%s")
				TEXT("\n\tPreviousKey=%s")
				TEXT("\n\tCurrentKey =%s"),
				*Owner->GetPathName(), *TargetPlatform->PlatformName(),
	...
	if (CVar->GetValueOnAnyThread(true) != 0 || Owner->bGenerateMeshDistanceField)
	{
		if (LODResources.IsValidIndex(0))
		{
			...
			{
				// If base data is already built (implying it was loaded from DDC), copy that to platform data, otherwise
				// it will be copied later when the async queue finishes generating it.
				FDistanceFieldVolumeData* BaseDistanceFieldData = Owner->GetRenderData()->LODResources[0].DistanceFieldData;
				if (BaseDistanceFieldData && BaseDistanceFieldData->bAsyncBuilding == false)
				{
					*LODResources[0].DistanceFieldData = *BaseDistanceFieldData;
				}

				// For platform data, this either copies the base render data, or allocates a placeholder structure for data to be copied into
				// later when tasks generated by CacheDerivedData called above for the base render data completes.  For base render data (the
				// "if" case above), this call is deferred until after distance fields are built.
				BeginCacheMeshCardRepresentation(Owner, *this, FString());
			}
		}
		else
		{
	...
	{
		BeginCacheMeshCardRepresentation(Owner, *this, Owner->GetRenderData()->DerivedDataKey);
	}
}
#endif // #if WITH_EDITOR
...
FArchive& operator<<(FArchive& Ar, FStaticMaterial& Elem)
{
	...
	if (!Ar.IsLoading() || Ar.CustomVer(FFortniteMainBranchObjectVersion::GUID) >= FFortniteMainBranchObjectVersion::MeshMaterialSlotOverlayMaterialAdded)
	{
		Ar << Elem.OverlayMaterialInterface;
	}
	else
	{
		Elem.OverlayMaterialInterface = nullptr;
	}
	return Ar;
}
...
#endif //WITH_EDITORONLY_DATA
, OverlayMaterialInterface(InOverlayMaterialInterface)
{
...
bool UStaticMesh::HasAnyDependenciesCompiling() const
{
	...
	for (const TObjectPtr<UStaticMesh>& AssemblyRef : AssemblyReferenceCache)
	{
		if (AssemblyRef && AssemblyRef->IsCompiling())
		{
			return true;
		}
	}
...
bool UStaticMesh::HasNaniteFallbackMesh(EShaderPlatform ShaderPlatform) const
{	
	if (const FStaticMeshRenderData* SMRenderData = GetRenderData())
	{
		return SMRenderData->HasNaniteFallbackMesh(ShaderPlatform);
	}
...
void UStaticMesh::PreEditChange(FProperty* PropertyAboutToChange)
{
	...
	// Stall for compilation of this mesh and any other mesh that might be reading from it concurrently
	FStaticMeshCompilingManager::FFinishCompilationOptions FinishCompilationOptions;
	FinishCompilationOptions.bIncludeDependentMeshes = true;
	FStaticMeshCompilingManager::Get().FinishCompilation({ this }, FinishCompilationOptions);
...
bool UStaticMesh::TryCancelAsyncTasks()
{
	if (AsyncTask)
	{
		if (AsyncTask->IsIdle() || AsyncTask->IsDone() || AsyncTask->Cancel())
		{
			AsyncTask.Reset();
...
void UStaticMesh::GetAssetRegistryTags(FAssetRegistryTagsContext Context) const
{
	...
	Context.AddTag(FAssetRegistryTag(GetStaticMaterialsName(), ExportStaticMaterialsInfo(), FAssetRegistryTag::TT_Numerical));
...
void UStaticMesh::GetAssetRegistryTagMetadata(TMap<FName, FAssetRegistryTagMetadata>& OutMetadata) const
{
	...
	OutMetadata.Add("NaniteEnabled",
		FAssetRegistryTagMetadata()
			.SetDisplayName(NSLOCTEXT("UStaticMesh", "NaniteEnabled_Label", "Nanite Enabled"))
		);

	OutMetadata.Add("NaniteFallbackPercent",
		FAssetRegistryTagMetadata()
			.SetDisplayName(NSLOCTEXT("UStaticMesh", "NaniteFallbackPercent_Label", "Nanite Fallback Percent"))
		);
	
	OutMetadata.Add("NaniteTriangles",
		FAssetRegistryTagMetadata()
			.SetDisplayName(NSLOCTEXT("UStaticMesh", "NaniteTriangles_Label", "Nanite Triangles"))
		);

	OutMetadata.Add("NaniteVertices",
		FAssetRegistryTagMetadata()
			.SetDisplayName(NSLOCTEXT("UStaticMesh", "NaniteVertices_Label", "Nanite Vertices"))
		);
	
	OutMetadata.Add("Triangles",
		FAssetRegistryTagMetadata()
			.SetDisplayName(NSLOCTEXT("UStaticMesh", "Triangles_Label", "Triangles"))
		);

	OutMetadata.Add("Vertices",
		FAssetRegistryTagMetadata()
			.SetDisplayName(NSLOCTEXT("UStaticMesh", "Vertices_Label", "Vertices"))
		);

	OutMetadata.Add("UVChannels",
		FAssetRegistryTagMetadata()
			.SetDisplayName(NSLOCTEXT("UStaticMesh", "UVChannels_Label", "UV Channels"))
		);

	OutMetadata.Add("Materials",
		FAssetRegistryTagMetadata()
			.SetDisplayName(NSLOCTEXT("UStaticMesh", "Materials_Label", "Materials"))
		);
	
	OutMetadata.Add(GetStaticMaterialsName(),
		FAssetRegistryTagMetadata()
			.SetDisplayName(NSLOCTEXT("UStaticMesh", "StaticMaterials_Label", "Static Materials"))
		);

	OutMetadata.Add("ApproxSize",
		FAssetRegistryTagMetadata()
			.SetDisplayName(NSLOCTEXT("UStaticMesh", "ApproxSize_Label", "Approximate Size"))
		);

	OutMetadata.Add("CollisionPrims",
		FAssetRegistryTagMetadata()
			.SetDisplayName(NSLOCTEXT("UStaticMesh", "CollisionPrims_Label", "Collision Primitives"))
			.SetTooltip(NSLOCTEXT("UStaticMesh", "CollisionPrims_Tooltip", "The number of collision primitives in the static mesh"))
			.SetImportantValue(TEXT("0"))
		);

	OutMetadata.Add("LODs",
		FAssetRegistryTagMetadata()
			.SetDisplayName(NSLOCTEXT("UStaticMesh", "LODs_Label", "LODs"))
		);

	OutMetadata.Add("MinLOD",
		FAssetRegistryTagMetadata()
			.SetDisplayName(NSLOCTEXT("UStaticMesh", "MinLOD_Label", "Min LOD"))
		);

	OutMetadata.Add("QualityLevelMinLOD",
		FAssetRegistryTagMetadata()
			.SetDisplayName(NSLOCTEXT("UStaticMesh", "QualityLevelMinLOD_Label", "Quality Level Min LOD"))
		);

	OutMetadata.Add("SectionsWithCollision",
		FAssetRegistryTagMetadata()
			.SetDisplayName(NSLOCTEXT("UStaticMesh", "SectionsWithCollision_Label", "Sections With Collision"))
		);

	OutMetadata.Add("DefaultCollision",
		FAssetRegistryTagMetadata()
			.SetDisplayName(NSLOCTEXT("UStaticMesh", "DefaultCollision_Label", "Default Collision"))
		);

	OutMetadata.Add("CollisionComplexity",
		FAssetRegistryTagMetadata()
			.SetDisplayName(NSLOCTEXT("UStaticMesh", "CollisionComplexity_Label", "Collision Complexity"))
		);

	OutMetadata.Add("DistanceFieldSize",
		FAssetRegistryTagMetadata()
			.SetDisplayName(NSLOCTEXT("UStaticMesh", "DistanceFieldSize_Label", "Distance Field Size"))
		);

	OutMetadata.Add("EstTotalCompressedSize",
		FAssetRegistryTagMetadata()
			.SetDisplayName(NSLOCTEXT("UStaticMesh", "EstTotalCompressedSize_Label", "Estimated Total Compressed Size"))
		);

	OutMetadata.Add("EstNaniteCompressedSize",
		FAssetRegistryTagMetadata()
			.SetDisplayName(NSLOCTEXT("UStaticMesh", "EstNaniteCompressedSize_Label", "Estimated Nanite Compressed Size"))
		);

	OutMetadata.Add("HasHiResMesh",
		FAssetRegistryTagMetadata()
			.SetDisplayName(NSLOCTEXT("UStaticMesh", "HasHiResMesh_Label", "Has HiRes Mesh"))
		);

	OutMetadata.Add("PhysicsSize",
		FAssetRegistryTagMetadata()
			.SetDisplayName(NSLOCTEXT("UStaticMesh", "PhysicsSize_Label", "Physics Size"))
		);
}
#endif
...
FStaticMeshRenderData& UStaticMesh::GetPlatformStaticMeshRenderData(UStaticMesh* Mesh, const ITargetPlatform* Platform)
{
	...
	{
		if (PlatformRenderData->DerivedDataKey == PlatformDerivedDataKey)
		{
			PlatformRenderData->SupportedPlatforms.Add(Platform);
			break;
		}

		if (PlatformRenderData->SupportedPlatforms.Contains(Platform) && IsRunningCookCommandlet())
		{
			UE_LOG(LogStaticMesh, Error,
				TEXT("Indeterminism in GetPlatformStaticMeshRenderData call to BuildStaticMeshDerivedDataKey. Using the previously computed RenderData even though the key is now different.")
				TEXT("\n\tStaticMesh=%s, Platform=%s")
				TEXT("\n\tPreviousKey=%s")
				TEXT("\n\tCurrentKey =%s"),
				*Mesh->GetPathName(), *Platform->PlatformName(),
				*PlatformRenderData->DerivedDataKey, *PlatformDerivedDataKey);
			break;
		}
...
void UStaticMesh::ClearAllCachedCookedPlatformData()
{
	...
	GetRenderData()->NumCacheAttempts = 0;
}
...
bool UStaticMesh::IsCachedCookedPlatformDataLoaded(const ITargetPlatform* TargetPlatform)
{
	...
	if (bFailed)
	{
		if (PlatformRenderData.NumCacheAttempts >= 2)
		{
			UE_LOG(LogStaticMesh, Error,
				TEXT("Failed to recover Nanite streaming from DDC for '%s', even after multiple attempts. ")
				TEXT("Abandoning the attempt to build the static mesh renderdata; the mesh will be broken at runtime."), *GetPathName());
			return true;
		}
		...
		// For simplicity, just rebuild the entire RenderData, but preserve the CacheRetryData
		// TODO: Verify that move construction works for FPlatformRenderData, and use the move constructor rather than
		// calling CopyCacheRetryData twice..
		FStaticMeshRenderData CopyData;
		CopyData.CopyCacheRetryData(PlatformRenderData);

		PlatformRenderData.~FStaticMeshRenderData();
		new (&PlatformRenderData) FStaticMeshRenderData();

		PlatformRenderData.CopyCacheRetryData(CopyData);
...
#if WITH_EDITORONLY_DATA
void FStaticMeshRenderData::CopyCacheRetryData(FStaticMeshRenderData& Other)
{
	SupportedPlatforms = Other.SupportedPlatforms;
	DerivedDataKey = Other.DerivedDataKey;
	NumCacheAttempts = Other.NumCacheAttempts;
}
#endif
...
void UStaticMesh::CommitMeshDescription(int32 LodIndex, const FCommitMeshDescriptionParams& Params)
{
	...
	if (LodIndex == 0 && SourceModel.GetCachedMeshDescription() && !IsNaniteAssembly())
	{
...
bool UStaticMesh::SetUVChannel(int32 LODIndex, int32 UVChannelIndex, const TMap<FVertexInstanceID, FVector2f>& TexCoords)
{
	FMeshDescription* MeshDescription = GetMeshDescription(LODIndex);
	if (!MeshDescription)
	{
	...
	for (const FVertexInstanceID VertexInstanceID : MeshDescription->VertexInstances().GetElementIDs())
	{
		if (const FVector2f* UVCoord = TexCoords.Find(VertexInstanceID))
		{
...
int32 UStaticMesh::GetNumUVChannels(int32 LODIndex) const
{
	int32 NumUVChannels = 0;
#if WITH_EDITORONLY_DATA
	FMeshDescription* MeshDescription = GetMeshDescription(LODIndex);
...
void UStaticMesh::CacheDerivedData()
{
	...
	bool bCachedAssemblyRefs = false;
	if (!HasCachedNaniteAssemblyReferences() &&
		ensureMsgf(bIsInGameThread, TEXT("Nanite Assembly references were not pre-cached on the game thread. Build will likely fail")))
	{
		CacheNaniteAssemblyReferences();
		bCachedAssemblyRefs = true;
	}
#endif
...
#if WITH_EDITORONLY_DATA
	if (bCachedAssemblyRefs)
	{
		ClearNaniteAssemblyReferenceCache();
	}
#endif
}
...
void UStaticMesh::Serialize(FArchive& Ar)
{
	...
	if (!IsInAsyncLoadingThread())
	{
		...
		FStaticMeshCompilingManager::FFinishCompilationOptions FinishCompilationOptions;
		FinishCompilationOptions.bIncludeDependentMeshes = Ar.IsLoading(); // Wait for dependent meshes if we're about to be editing ourselves
		FStaticMeshCompilingManager::Get().FinishCompilation({this}, FinishCompilationOptions);
	}
#endif
...
#endif // WITH_EDITORONLY_DATA
	{
		TArray<FStaticMaterial>& StaticMaterialsArray = GetStaticMaterials();
		Ar << StaticMaterialsArray;
		UpdateStaticMaterialsInfoCache(StaticMaterialsArray);
	}
#if WITH_EDITORONLY_DATA
	else if (Ar.IsLoading())
	{
		...
		for (UMaterialInterface *MaterialInterface : Materials_DEPRECATED)
		{
			...
			StaticMaterialsArray.Add(FStaticMaterial(MaterialInterface, MaterialSlotName));
...
void UStaticMesh::PostLoad()
{
	...
	// Scope for the FStaticMeshPostLoadContext
	{
		TUniquePtr<FStaticMeshPostLoadContext> Context = MakeUnique<FStaticMeshPostLoadContext>();
		...
		if (FStaticMeshCompilingManager::Get().IsAsyncCompilationAllowed(this))
		{
			...
			AsyncTask = MakeUnique<FStaticMeshAsyncBuildTask>(this, MoveTemp(Context));
			...
			if (IsNaniteAssembly())
			{
				FStaticMeshCompilingManager::Get().AddStaticMeshesWithDependencies({this});
			}
			else
			{
...
void UStaticMesh::AppendToClassSchema(FAppendToClassSchemaContext& Context)
{
	Super::AppendToClassSchema(Context);
	AppendMeshCardConstantsToClassSchema(Context);
	AppendDistanceFieldConstantsToClassSchema(Context);
}
#endif
...
void UStaticMesh::BeginPostLoadInternal(FStaticMeshPostLoadContext& Context)
{
	...
	// Cache the assembly references, but don't sync on the compile, the async compiler wil schedule it appropriately
	CacheNaniteAssemblyReferences(false);
#endif //WITH_EDITORONLY_DATA
...
bool UStaticMesh::IsNaniteAssembly() const
{
#if NANITE_ASSEMBLY_DATA
	return IsNaniteEnabled() && NaniteSettings.NaniteAssemblyData.IsValid();
#else
	return false;
#endif
}

void UStaticMesh::CacheNaniteAssemblyReferences(bool bWaitForAsyncCompile)
{
#if NANITE_ASSEMBLY_DATA
	...
	// Load referenced static meshes for the Nanite assembly build
	...
	{
		UStaticMesh* Ref = (UStaticMesh*)StaticLoadAsset(UStaticMesh::StaticClass(), PartData.MeshObjectPath.GetAssetPath());
		if (Ref == nullptr)
		{
			UE_LOG(
				LogStaticMesh, Warning,
				TEXT("Failed to load static mesh %s when precaching Nanite Assembly references for %s."),
	...
	{
		for (TObjectPtr<UStaticMesh>& Ref : AssemblyReferenceCache)
		{
			if (Ref)
			{
				Ref->WaitUntilAsyncPropertyReleased(EStaticMeshAsyncProperties::All);
			}
		}
	}
#endif
...
bool UStaticMesh::HasCachedNaniteAssemblyReferences() const
{
#if NANITE_ASSEMBLY_DATA
...
void UStaticMesh::ClearNaniteAssemblyReferenceCache()
{
	check(IsInGameThread());
	AssemblyReferenceCache.Empty();
}

const TArray<TObjectPtr<UStaticMesh>>& UStaticMesh::GetCachedNaniteAssemblyReferences() const
{
...
void UStaticMesh::UpdateStaticMaterialsInfoCache(const TArray<FStaticMaterial>& InMaterials)
{
	FScopeLock Lock(&MaterialsInfoCriticalSection);
	StaticMaterialsInfoCache.Reset();
	for (int32 Index = 0; Index < InMaterials.Num(); Index++)
	{
		FName MaterialSlotName = InMaterials[Index].MaterialSlotName;
		FString MaterialObjectPath = InMaterials[Index].MaterialInterface ? InMaterials[Index].MaterialInterface->GetPathName() : FString();
		StaticMaterialsInfoCache.Add(FStaticMaterialMinimalInfo(MaterialSlotName, MaterialObjectPath));
	}
}

#define MATERIAL_SLOT_DELIMITER TEXT(">")
#define MATERIAL_SLOT_PAIR_DELIMITER TEXT(",")
const FString UStaticMesh::ExportStaticMaterialsInfo() const
{
	FScopeLock Lock(&MaterialsInfoCriticalSection);
	FString OutText;
	for (const FStaticMaterialMinimalInfo& Slot : StaticMaterialsInfoCache)
	{
		OutText += Slot.MaterialSlotName.ToString();
		OutText += MATERIAL_SLOT_PAIR_DELIMITER;
		OutText += FString::Printf(TEXT("\'%s\'"), *Slot.MaterialObjectPath);  // Add quotes so object path is recognized during asset header patching
		OutText += MATERIAL_SLOT_DELIMITER;
	}
	return OutText;
}
...
{
	TArray<FStaticMaterialMinimalInfo> OutArray;
	if (!InData.IsEmpty())
	{
		TArray<FString> Slot;
		const int32 NumPairs = InData.ParseIntoArray(Slot, MATERIAL_SLOT_DELIMITER, true);
		for (int32 Index = 0; Index < NumPairs; Index++)
		{
			TArray<FString> Pair;
			int32 NumEntries = Slot[Index].ParseIntoArray(Pair, MATERIAL_SLOT_PAIR_DELIMITER, true);
			if (NumEntries == 2)
			{
				// Remove quotes
				Pair[1].RemoveFromStart(TEXT("\'"));
				Pair[1].RemoveFromEnd(TEXT("\'"));

				OutArray.Add(FStaticMaterialMinimalInfo(FName(Pair[0]), Pair[1]));
			}
		}
	}
	return OutArray;
