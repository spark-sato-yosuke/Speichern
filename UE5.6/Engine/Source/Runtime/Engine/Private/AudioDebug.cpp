...
namespace Audio
{
	FString ExtraTracingSoundWaveFilterCVar;

#if !UE_BUILD_SHIPPING
	bool MatchesLogFilter(const TCHAR* InName)
	{
		return !ExtraTracingSoundWaveFilterCVar.IsEmpty()	// Trivially reject if we have no filter.
		 && FCString::Stristr(InName, *ExtraTracingSoundWaveFilterCVar);
	}
#endif
}

static FAutoConsoleVariableRef CVarExtraTracingSoundWaveFilter(
	TEXT("au.debug.extra_logging_filter"),
	Audio::ExtraTracingSoundWaveFilterCVar,
	TEXT("Enable extra logging for any sound wave that has this substring in its name."),
	ECVF_Default);
		...
		for (const FActiveSound* ActiveSound : ActiveSounds)
		{
			if (ActiveSound)
			{
				...
				for (const TPair<UPTRINT, FWaveInstance*>& WaveInstancePair : ActiveSound->GetWaveInstances())
				{
					...
					UE_LOG(LogAudio, Display, TEXT("   %s (%.3g) (%" SIZE_T_FMT ") - %.3g"),
			...
			for (const FAudioStats::FStatSoundInfo& StatSoundInfo : AudioStats.StatSoundInfos)
			{
				for (const FAudioStats::FStatWaveInstanceInfo& WaveInstanceInfo : StatSoundInfo.WaveInstanceInfos)
				{
					if (WaveInstanceInfo.Volume >= MinDisplayVolume)
					{
						...
						const FString Volume = FString::Printf(TEXT("%6.3f "), WaveInstanceInfo.Volume);
