...
#define UE_SET_PP(Name)\
...

	{
		...
		UE_SET_PP(DepthOfFieldAspectRatioScalar);
		UE_SET_PP(DepthOfFieldPetzvalBokeh);
		UE_SET_PP(DepthOfFieldPetzvalBokehFalloff);
		UE_SET_PP(DepthOfFieldPetzvalExclusionBoxExtents);
		UE_SET_PP(DepthOfFieldPetzvalExclusionBoxRadius);
		UE_SET_PP(DepthOfFieldBarrelRadius);
		UE_SET_PP(DepthOfFieldBarrelLength);
		if (OtherTo.bOverride_DepthOfFieldMatteBoxFlags)
		{
			for (uint32 Index = 0; Index < UE_ARRAY_COUNT(OtherTo.DepthOfFieldMatteBoxFlags); ++Index)
			{
				ThisFrom.DepthOfFieldMatteBoxFlags[Index] = OtherTo.DepthOfFieldMatteBoxFlags[Index];
			}
			bAnyOverwritten = true;
		}
		...
		UE_SET_PP(RayTracingTranslucencyMaxPrimaryHitEvents);
		UE_SET_PP(RayTracingTranslucencyMaxSecondaryHitEvents);
		UE_SET_PP(RayTracingTranslucencyUseRayTracedRefraction);
...
#define UE_LERP_PP(Name)\
...

	{
		...
		UE_LERP_PP(DepthOfFieldPetzvalBokeh);
		UE_LERP_PP(DepthOfFieldPetzvalBokehFalloff);
		UE_LERP_PP(DepthOfFieldPetzvalExclusionBoxExtents);
		UE_LERP_PP(DepthOfFieldPetzvalExclusionBoxRadius);
		UE_LERP_PP(DepthOfFieldBarrelRadius);
		UE_LERP_PP(DepthOfFieldBarrelLength);
		if (OtherTo.bOverride_DepthOfFieldMatteBoxFlags)
		{
			for (uint32 Index = 0; Index < UE_ARRAY_COUNT(OtherTo.DepthOfFieldMatteBoxFlags); ++Index)
			{
		...
		UE_LERP_PP(DepthOfFieldAspectRatioScalar);
