...
struct FHLSLMaterialTranslator::FEnvironmentDefines
{
	...
	{
		TABLE_MESH_PAINT = 1,
		TABLE_MATERIAL_CACHE = 2,
		LOCAL_TABLE0 = 4,
		LOCAL_TABLE1 = 8,
		LOCAL_ADAPTIVE_INDIRECTION = 16
	};
...
uint64 FHLSLMaterialTranslator::CompileMaterialAttributesCustomOutputs(uint64 CustomOutputNodeBitmask)
{
	...
	for (const FMaterialCustomOutputAttributeDefintion& CustomAttribute : CustomAttributes)
	{
		...
		for (int32 PreviousFrameEvaluation = 0; PreviousFrameEvaluation < NumEvaluations; PreviousFrameEvaluation++)
		{
			...
			CustomOutput(CustomAttribute.AttributeName, CustomAttribute.FunctionName, CustomAttribute.OutputIndex, CodeChunk, EMaterialCustomOutputFlags::None);
		}
...
void FHLSLMaterialTranslator::TranslateMaterial()
{
	...
	const bool bUseRootNodeToSubstrateHiddenConversion = bSubstrateEnabled && bSubstrateHiddenMaterialAssetConversion && !FrontMaterialExpr;
	if (bUseRootNodeToSubstrateHiddenConversion)
	{
	...
	else // if (bUseRootNodeToSubstrateHiddenConversion)
	{
				...
				if (BaseMaterial && BaseMaterial->bUseMaterialAttributes)
				{
					...
					return UMaterialExpressionSubstrateConvertMaterialAttributes::CompileCommon(this, 0,
						...
						*WaterScatteringCoefficients, *WaterAbsorptionCoefficients, *WaterPhaseG, *ColorScaleBehindWater,
						bHasSSS, bHasSSS ? MaterialInterface->SubsurfaceProfile : nullptr);
...
void FHLSLMaterialTranslator::GetMaterialEnvironment(EShaderPlatform InPlatform, FShaderCompilerEnvironment& OutEnvironment)
{
	...
	for (int i = 0; i < EnvironmentDefines->VirtualPageTypes.Num(); ++i)
	{
		...
		else if (EnvironmentDefines->VirtualPageTypes[i] & FEnvironmentDefines::TABLE_MATERIAL_CACHE)
		{
			OutEnvironment.SetDefine(
				*FString::Printf(TEXT("VIRTUALTEXTURE_PAGETABLE_%d"), i), 
				TEXT("Scene.MaterialCache.PageTableTexture"));

			OutEnvironment.SetDefine(
				*FString::Printf(TEXT("VIRTUALTEXTURE_PAGETABLE_UNIFORM_%d"), i), 
				TEXT("GetMaterialCacheTextureDescriptor(GetPrimitiveData(Parameters))"));
		}
		else
		{
...
int32 FHLSLMaterialTranslator::AddCodeChunkInner(EMaterialValueType Type, EDerivativeStatus DerivativeStatus, bool bInlined, const TCHAR* Format, ...)
{
	...
	return AddCodeChunkInner(Hash, FormattedCode, Type, DerivativeStatus, bInlined);
}
...
uint32 FHLSLMaterialTranslator::AcquireVTStackIndex(
	...
	bool bAdaptive, bool bGenerateFeedback, bool bMeshPaint, bool bMaterialCache)
{
	...
	for (int32 Index = VTStackHash.First(Hash); VTStackHash.IsValid(Index); Index = VTStackHash.Next(Index))
	{
		...
		if (!Stack.AreLayersFull() &&
			...
			Entry.bMaterialCache == bMaterialCache)
		{
			return Index;
		}
	}
...
)
{
	...
	const bool bVirtualTexture = bMeshPaint || bMaterialCache || TextureType == MCT_TextureVirtual;
	if (bVirtualTexture)
	{
		const EMaterialDomain Domain = Material->GetMaterialDomain();
		switch (Domain)
		{
			...
			case MD_LightFunction:
				break;
			default:
				return Errorf(TEXT("Sampling a virtual texture is not supported for the %s material domain."), *MaterialDomainString(Domain));
		}
	}
	...
	else if (bMaterialCache)
	{
		...
		TextureName = FString::Printf(
			TEXT("Scene.MaterialCache.PhysicalTexture%d, View.SharedBilinearClampedSampler"),
			UniformExpression->GetTextureUniformExpression()->GetTextureLayerIndex()
		);
	...
	if (bVirtualTexture)
	{
		...
		if (bMeshPaint)
		{
			VTStackIndex = AcquireVTStackIndex(MipValueMode, StaticAddressX, StaticAddressY, 1.0f, CoordinateIndex, MipValue0Index, MipValue1Index, INDEX_NONE, UV_Value, UV_Ddx, UV_Ddy, AdaptiveVirtualTexture, bGenerateFeedback, bMeshPaint, bMaterialCache);
			VTPageTableIndex = 0;
		...
		else if (bMaterialCache)
		{
			VTStackIndex = AcquireVTStackIndex(MipValueMode, StaticAddressX, StaticAddressY, 1.0f, CoordinateIndex, MipValue0Index, MipValue1Index, INDEX_NONE, UV_Value, UV_Ddx, UV_Ddy, AdaptiveVirtualTexture, bGenerateFeedback, bMeshPaint, bMaterialCache);
			VTPageTableIndex = 0;
			VTPackedUniformName = TEXT("Scene.MaterialCache.PackedUniform");
		}
		else 
		{
			...
			if (VTLayerIndex != INDEX_NONE)
			{
				...
				VTStackIndex = AcquireVTStackIndex(MipValueMode, StaticAddressX, StaticAddressY, 1.0f, NonLWCCoordinateIndex, MipValue0Index, MipValue1Index, TextureReferenceIndex, UV_Value, UV_Ddx, UV_Ddy, AdaptiveVirtualTexture, bGenerateFeedback, bMeshPaint, bMaterialCache);
			...
			else
			{
				...
				VTStackIndex = AcquireVTStackIndex(MipValueMode, StaticAddressX, StaticAddressY, TextureAspectRatio, NonLWCCoordinateIndex, MipValue0Index, MipValue1Index, INDEX_NONE, UV_Value, UV_Ddx, UV_Ddy, AdaptiveVirtualTexture, bGenerateFeedback, bMeshPaint, bMaterialCache);
...
int32 FHLSLMaterialTranslator::SampleMaterialCache(int32 TexCoordIndexLiteral)
{
	int32 TexCoord = TextureCoordinate(TexCoordIndexLiteral, false, false);

	int32 Layers[MaterialCacheMaxABuffers];
	for (int32 i = 0; i < MaterialCacheMaxABuffers; i++)
	{
		Layers[i] = TextureSample(
			MaterialCacheTextureDescriptor(i),
			TexCoord,
	...
	int32 Attributes = AddCodeChunk(
		MCT_MaterialAttributes,
		TEXT("(FMaterialAttributes)0")
	);

	Attributes = SetMaterialAttribute(Attributes, AddCodeChunk(MCT_Float3, TEXT("GetMaterialCacheBaseColor(%s)"),          *GetParameterCode(Layers[0])), FMaterialAttributeDefinitionMap::GetID(MP_BaseColor));
	Attributes = SetMaterialAttribute(Attributes, AddCodeChunk(MCT_Float, TEXT("GetMaterialCacheRoughness(%s)"),           *GetParameterCode(Layers[0])), FMaterialAttributeDefinitionMap::GetID(MP_Roughness));
	Attributes = SetMaterialAttribute(Attributes, AddCodeChunk(MCT_Float3, TEXT("GetMaterialCacheWorldNormal(%s)"),        *GetParameterCode(Layers[1])), FMaterialAttributeDefinitionMap::GetID(MP_Normal));
	Attributes = SetMaterialAttribute(Attributes, AddCodeChunk(MCT_Float, TEXT("GetMaterialCacheSpecular(%s)"),            *GetParameterCode(Layers[1])), FMaterialAttributeDefinitionMap::GetID(MP_Specular));
	Attributes = SetMaterialAttribute(Attributes, AddCodeChunk(MCT_Float, TEXT("GetMaterialCacheOpacity(%s)"),             *GetParameterCode(Layers[1])), FMaterialAttributeDefinitionMap::GetID(MP_Opacity));
	Attributes = SetMaterialAttribute(Attributes, AddCodeChunk(MCT_Float, TEXT("GetMaterialCacheMetallic(%s)"),            *GetParameterCode(Layers[2])), FMaterialAttributeDefinitionMap::GetID(MP_Metallic));
	//SetMaterialAttribute(Attributes, AddCodeChunk(MCT_Float3, TEXT("GetMaterialCacheWorldPositionOffset(%s)"), *GetParameterCode(Layers[2])), FMaterialAttributeDefinitionMap::GetID(MP_WorldPositionOffset));

	return Attributes;
}

int32 FHLSLMaterialTranslator::GetMaterialCacheAttribute(int32 MaterialCacheIndex, const FGuid& Attribute)
{
	return AccessMaterialAttribute(MaterialCacheIndex, Attribute);
}
...
int32 FHLSLMaterialTranslator::MaterialCacheTextureDescriptor(uint32 LayerIndex)
{
	int32 CodeIndex = AddInlinedCodeChunkZeroDeriv(MCT_TextureMaterialCache, TEXT("GetMaterialCacheTextureDescriptor(GetPrimitiveData(Parameters))"));

	FAddUniformExpressionScope Scope(this);
	return AddUniformExpression(Scope, new FMaterialUniformExpressionTexture(CodeIndex, LayerIndex, 0, SAMPLERTYPE_Color), MCT_TextureMaterialCache, TEXT(""));
}
			...
			for (const auto& It : SubstrateMaterialExpressionRegisteredOperators)
			{
				...
				case SUBSTRATE_OPERATOR_BSDF:
				{
					...
					case SUBSTRATE_BSDF_TYPE_SLAB:
					{
						...
						if (It.Has(ESubstrateBsdfFeature_ComplexSpecialMask))
						{
							// We need to check each features if really enabled for a platofrms
							bIsComplexSpecial = It.Has(ESubstrateBsdfFeature_Glint) && Substrate::IsGlintEnabled(ShaderPlatform);
						}

						SubstrateMaterialComplexity.bIsSimple = SubstrateMaterialComplexity.bIsSimple && !bMayHaveColoredWeight && !It.Has(ESubstrateBsdfFeature_ComplexMask) && !bIsComplexSpecial && !It.Has(ESubstrateBsdfFeature_SingleMask);
						SubstrateMaterialComplexity.bIsSingle = SubstrateMaterialComplexity.bIsSingle && !bMayHaveColoredWeight && !It.Has(ESubstrateBsdfFeature_ComplexMask) && !bIsComplexSpecial;
			...
			for (auto& It : SubstrateMaterialExpressionRegisteredOperators)
			{
				...
				case SUBSTRATE_OPERATOR_BSDF:
				{
					...
					case SUBSTRATE_BSDF_TYPE_SLAB:
					{
						...
						if (It.Has(ESubstrateBsdfFeature_Glint) && Substrate::IsGlintEnabled(ShaderPlatform))
						{
							SubstrateMaterialRequestedSizeByte += UintByteSize;
							SubstrateMaterialRequestedSizeByte += UintByteSize;
							SubstrateMaterialRequestedSizeByte += UintByteSize;
...
int32 FHLSLMaterialTranslator::CustomOutput(const FString& CustomOutputName, const FString& FunctionName, int32 OutputIndex, int32 OutputCode, EMaterialCustomOutputFlags Flags)
{
...
{
	return CustomOutput(Custom->GetDescription(), Custom->GetFunctionName(), OutputIndex, OutputCode, Flags);
}
		...
		for (uint32 SubstrateCompilationContextIndex = 0; SubstrateCompilationContextIndex < ESubstrateCompilationContext::SCC_MAX; ++SubstrateCompilationContextIndex)
		{
			...
			// Now write some feedback to the user, but only produce debug string if in editor
			{
				...
				for (int32 OpIt = 0; OpIt < SubstrateCtx.SubstrateMaterialExpressionRegisteredOperators.Num(); ++OpIt)
				{
					...
					EyeIrisNormalPluggedIn |= !!(BSDFOperator.BSDFFeatures & ESubstrateBsdfFeature_EyeIrisNormalPluggedIn);
					EyeIrisTangentPluggedIn |= !!(BSDFOperator.BSDFFeatures & ESubstrateBsdfFeature_EyeIrisTangentPluggedIn);
