	...
	switch(Property)
	{
		...
		case MP_ShadingModel:
			if (AllowPerPixelShadingModels(Compiler->GetShaderPlatform()) 
				|| !MaterialInterface->IsShadingModelFromMaterialExpression())
			{
				Ret = MaterialInterface->CompileProperty(Compiler, Property);
			}
			else
...
void FMaterialResource::GetShaderMapId(EShaderPlatform Platform, const ITargetPlatform* TargetPlatform, FMaterialShaderMapId& OutId) const
{
	...
	CachedData.GetExternalCodeReferencesHash(OutId.ExternalCodeReferencesHash);
		...
		if (MaterialResource && MaterialResource->GetRenderingThreadShaderMap())
		{
			...
			else if (Type == EMaterialParameterType::Scalar && NumSpecularProfileRT() > 0)
			{
				for (uint32 It=0,Count=NumSpecularProfileRT();It<Count;++It)
				{
					...
					{
						const USpecularProfile* SpecularProfile = GetSpecularProfileRT(It);
						const USpecularProfile* SpecularProfileOverride = GetSpecularProfileOverrideRT();
						OutValue = SpecularProfile::GetSpecularProfileId(SpecularProfileOverride ? SpecularProfileOverride : SpecularProfile);
						return true;
					}
				}
			}
...
void UMaterial::SetInitialValues()
{
	...
	// Shadow on translucent material
	bAllowTranslucentLocalLightShadow = false;
	TranslucentLocalLightShadowQuality = 0;
	TranslucentDirectionalLightShadowQuality = 0;
	...
	bSubstrateRoughnessTracking = true;

	bAllowDevelopmentShaderCompile = true;
	bIsMaterialEditorStatsMaterial = false;

	bRelaxRuntimeVirtualTextureRestrictions = false;
	bRelaxRuntimeVirtualTextureRestrictionsForOldContent = false;
...
#if WITH_EDITOR
void UMaterial::OnCookEvent(UE::Cook::ECookEvent CookEvent, UE::Cook::FCookEventContext& CookContext)
{
...
void UMaterial::GetUsedTextures(TArray<UTexture*>& OutTextures, EMaterialQualityLevel::Type QualityLevel, bool bAllQualityLevels, ERHIFeatureLevel::Type FeatureLevel, bool bAllFeatureLevels) const
{
	...
	if (!FPlatformProperties::IsServerOnly())
	{
		...
		for (const FMaterialResource* CurrentResource : MatchedResources)
		{
			...
			for (const FMaterialTextureCollectionParameterInfo& Parameter : CurrentResource->GetUniformTextureCollectionExpressions())
			{
				UTextureCollection* TextureCollection = nullptr;
				Parameter.GetGameThreadTextureCollectionValue(this, *CurrentResource, TextureCollection);
...
bool UMaterial::GetUsageByFlag(EMaterialUsage Usage) const
{
	...
	switch(Usage)
	{
		...
		case MATUSAGE_MaterialCache: UsageValue = bUsedWithMaterialCache; break;
...
bool UMaterial::SetTextureCollectionParameterValueEditorOnly(FName ParameterName, class UTextureCollection* InValue)
{
	FMaterialParameterMetadata Meta;
	Meta.Value = InValue;
	return SetParameterValueEditorOnly(ParameterName, Meta);
...
void UMaterial::SetUsageByFlag(EMaterialUsage Usage, bool NewValue)
{
	...
	switch(Usage)
	{
		...
		case MATUSAGE_MaterialCache:
		{
			bUsedWithMaterialCache = NewValue; break;
		}
		case MATUSAGE_StaticMesh:
		{
			bUsedWithStaticMesh = NewValue; break;
		}
		default: UE_LOG(LogMaterial, Fatal, TEXT("Unknown material usage: %u"), (int32)Usage);
	};
#if WITH_EDITOR
...
FString UMaterial::GetUsageName(EMaterialUsage Usage) const
{
	...
	switch(Usage)
	{
		...
		case MATUSAGE_MaterialCache: UsageName = TEXT("bUsedWithMaterialCache"); break;
	...
	if(!GetUsageByFlag(Usage) && !bUsedAsSpecialEngineMaterial)
	{
		...
		else
		{
			...
			if ((UsageFlagWarnings & UsageFlagBit) == 0)
			{
				...
				{
					// Temporarily Turn off the toast to avoid spam which can result in GPU crashes from too many swap chains from being created.
...
#if WITH_EDITORONLY_DATA
void UMaterial::SyncLayersRuntimeGraphCache(FMaterialLayersFunctions* OverrideLayers)
{
	if(UMaterialEditorOnlyData* EditorOnly = GetEditorOnlyData())
	{
...
#if WITH_EDITOR
void UMaterial::GetShaderTags(TArray<FName>& OutShaderTags) const
{
	if (!CachedExpressionData || !CachedExpressionData->EditorOnlyData)
	{
		return;
	}
...
EMaterialTranslateValidationFlags UMaterial::GetMaterialTranslateValidationFlags() const
{
	EMaterialTranslateValidationFlags Flags = EMaterialTranslateValidationFlags::None;
	if (!bRelaxRuntimeVirtualTextureRestrictions && !bRelaxRuntimeVirtualTextureRestrictionsForOldContent)
	{
		Flags |= EMaterialTranslateValidationFlags::Strict_RuntimeVirtualTexture;
	}
	return Flags;
}
#endif
...
FGraphEventArray UMaterial::PrecachePSOs(const FPSOPrecacheVertexFactoryDataList& VertexFactoryDataList, const FPSOPrecacheParams& InPreCacheParams, EPSOPrecachePriority Priority, TArray<FMaterialPSOPrecacheRequestID>& OutMaterialPSORequestIDs)
{
	...
	if (FApp::CanEverRender() && MaterialResources.Num() > 0 && (PipelineStateCache::IsPSOPrecachingEnabled() || IsPSOShaderPreloadingEnabled()) && UMaterialInterface::IsDefaultMaterialInitialized())
	{
...
// Substrate version
// 0: Glint conversion
static int32 GetSubstrateVersion()				{ return 0; }

#if WITH_EDITOR


bool UMaterial::UserRequestsConvertMaterialToSubstrateMaterial()
{
	bool bUpdated = false;
	UMaterialEditorOnlyData* EditorOnly = GetEditorOnlyData();
	if (Substrate::IsSubstrateEnabled() && EditorOnly && !EditorOnly->FrontMaterial.IsConnected())
	{
		const FScopedTransaction Transaction(LOCTEXT("SubstrateConversion", "Substrate Conversion"));

		GetTransientPackage()->Modify();
		EditorOnly->Modify();
		MaterialGraph->Modify();
		Modify(true);

		bUpdated = ConvertMaterialToSubstrateMaterial(true /*bAllowEmptyMaterialUpdate*/, true /*bRequestedByUser*/);
		if (bUpdated)
		{
			if (MaterialGraph)
			{
			...
			GetTransientPackage()->MarkPackageDirty();
			EditorOnly->MarkPackageDirty();
			MaterialGraph->MarkPackageDirty();
			MarkPackageDirty();
...
bool UMaterial::ConvertMaterialToSubstrateMaterial(bool bAllowEmptyMaterialUpdate, bool bRequestedByUser)
{
	/*
	...
	*/
	...
	// * If the current material has already been converted & versioned, skip the conversion. 
	...
	if ((EditorOnly->SubstrateConversionVersion == GetSubstrateConversionVersion() || EditorOnly->SubstrateConversionVersion == GetSubstrateNoConversionVersion()) && EditorOnly->SubstrateVersion == GetSubstrateVersion() && !bRequestedByUser)
	{
		return false;
	}
	else if (bRequestedByUser && EditorOnly->FrontMaterial.IsConnected())
	{
		return false; // We do not convert if the front material pin has is already connected.
	}
	else if(Substrate::IsHiddenMaterialAssetConversionEnabled() && !bRequestedByUser)
	{
		return false; // This is to work with ShouldAllowSubstrateReset. So that a material authored with IsHiddenMaterialAssetConversionEnabled using the root node inputs can then be converted to substrate.
	}

	// Store current node post from the root node.
	int32 CurrentNodePosX = EditorX;
	int32 ConstantNodePosX = EditorX;
	int32 ConstantNodePosY = EditorY;
	...
	auto BeginConstantNodeLayout = [&]()
	{
		// Leave some space for constants
		CurrentNodePosX += TranslationOffsetX;
	};
	auto LaydownConstant = [&](UMaterialExpression* Node)
	{
		// Position the constant on the constant column, each one on top of each other
		Node->MaterialExpressionEditorX = ConstantNodePosX;
		Node->MaterialExpressionEditorY = ConstantNodePosY;
		ConstantNodePosY += Node->IsA<UMaterialExpressionConstant>() ? 80 : 200;
	};

	// Recommendation: use the non indexed version whenever possible. However, indexed version are mandatory for material function which do not have explicit input.
	auto ConnectionToIndexed = [](auto& OldNodeInput, UMaterialExpression* NewNode, uint32 NewInputIndex, uint32 OperationType = SUBSTRATE_MOVE_CONNECTION)
	{
	...
	auto ConnectionTo = [](auto& OldNodeInput, FExpressionInput& NewNodeInput, uint32 OperationType = SUBSTRATE_MOVE_CONNECTION)
	{
		if (OldNodeInput.IsConnected())
		{
			NewNodeInput.Connect(OldNodeInput.OutputIndex, OldNodeInput.Expression);
			if (OperationType == SUBSTRATE_MOVE_CONNECTION)
			{
				OldNodeInput.Expression = nullptr;
			}
	...
	auto ScalarMatInputConnectionToIndexed = [&](FScalarMaterialInput& OldNodeInput, UMaterialExpression* NewNode, uint32 NewInputIndex, EMaterialProperty Property, uint32 OperationType = SUBSTRATE_MOVE_CONNECTION)
	{
		FVector4f DefaultPinValue = FMaterialAttributeDefinitionMap::GetDefaultValue(Property);

		if (OldNodeInput.IsConnected())
		{
			ConnectionToIndexed(OldNodeInput, NewNode, NewInputIndex, OperationType);
		}
		else if (OldNodeInput.UseConstant && DefaultPinValue.X != OldNodeInput.Constant)
		{
			UMaterialExpressionConstant* ExpressionConstant = NewObject<UMaterialExpressionConstant>(this);
			ExpressionConstant->Material = this;
			EditorOnly->ExpressionCollection.AddExpression(ExpressionConstant);
			ExpressionConstant->R = OldNodeInput.Constant;
			NewNode->GetInput(NewInputIndex)->Connect(0, ExpressionConstant);
			if (OperationType == SUBSTRATE_MOVE_CONNECTION)
			{
				OldNodeInput.Expression = nullptr;
			}
			LaydownConstant(ExpressionConstant);
		}
	};
	auto ScalarMatInputConnectionTo = [&](FScalarMaterialInput& OldNodeInput, FExpressionInput& NewNodeInput, EMaterialProperty Property, uint32 OperationType = SUBSTRATE_MOVE_CONNECTION)
	{
		FVector4f DefaultPinValue = FMaterialAttributeDefinitionMap::GetDefaultValue(Property);

		if (OldNodeInput.IsConnected())
		{
			ConnectionTo(OldNodeInput, NewNodeInput, OperationType);
		}
		else if (OldNodeInput.UseConstant && DefaultPinValue.X != OldNodeInput.Constant)
		{
			UMaterialExpressionConstant* ExpressionConstant = NewObject<UMaterialExpressionConstant>(this);
			...
			EditorOnly->ExpressionCollection.AddExpression(ExpressionConstant);
			ExpressionConstant->R = OldNodeInput.Constant;
			NewNodeInput.Connect(0, ExpressionConstant);
			if (OperationType == SUBSTRATE_MOVE_CONNECTION)
			{
				OldNodeInput.Expression = nullptr;
			}
	...
	auto ColorMatInputConnectionToIndexed = [&](FColorMaterialInput& OldNodeInput, UMaterialExpression* NewNode, uint32 NewInputIndex, EMaterialProperty Property, uint32 OperationType = SUBSTRATE_MOVE_CONNECTION)
	{
		FVector4f DefaultPinValue = FMaterialAttributeDefinitionMap::GetDefaultValue(Property);
		FLinearColor DefaultPinLinearColor = FLinearColor(DefaultPinValue.X, DefaultPinValue.Y, DefaultPinValue.Z);
		FLinearColor CurrentPinLinearColor = OldNodeInput.Constant;
		...
		{
			UMaterialExpressionConstant3Vector* ExpressionConstant = NewObject<UMaterialExpressionConstant3Vector>(this);
			ExpressionConstant->Material = this;
			EditorOnly->ExpressionCollection.AddExpression(ExpressionConstant);
			ExpressionConstant->Constant = CurrentPinLinearColor;
			NewNode->GetInput(NewInputIndex)->Connect(0, ExpressionConstant);
			if (OperationType == SUBSTRATE_MOVE_CONNECTION)
			{
				OldNodeInput.Expression = nullptr;
	...
	auto ColorMatInputConnectionTo = [&](FColorMaterialInput& OldNodeInput, FExpressionInput& NewNodeInput, EMaterialProperty Property, uint32 OperationType = SUBSTRATE_MOVE_CONNECTION)
	{
		FVector4f DefaultPinValue = FMaterialAttributeDefinitionMap::GetDefaultValue(Property);
		FLinearColor DefaultPinLinearColor = FLinearColor(DefaultPinValue.X, DefaultPinValue.Y, DefaultPinValue.Z);
		FLinearColor CurrentPinLinearColor = OldNodeInput.Constant;
	...
	auto Vector3MatInputConnectionToIndexed = [&](FVectorMaterialInput& OldNodeInput, UMaterialExpression* NewNode, uint32 NewInputIndex, EMaterialProperty Property, uint32 OperationType = SUBSTRATE_MOVE_CONNECTION)
	{
		FVector4f DefaultPinValue = FMaterialAttributeDefinitionMap::GetDefaultValue(Property);
		...
		else if (OldNodeInput.UseConstant && DefaultPinVec3f != CurrentPinVec3f)
		{
			...
			ExpressionConstant->Material = this;
			EditorOnly->ExpressionCollection.AddExpression(ExpressionConstant);
			ExpressionConstant->Constant = CurrentPinVec3f;
			NewNode->GetInput(NewInputIndex)->Connect(0, ExpressionConstant);
			if (OperationType == SUBSTRATE_MOVE_CONNECTION)
			{
				OldNodeInput.Expression = nullptr;
	...
	auto Vector3MatInputConnectionTo = [&](FVectorMaterialInput& OldNodeInput, FExpressionInput& NewNodeInput, EMaterialProperty Property, uint32 OperationType = SUBSTRATE_MOVE_CONNECTION)
	{
		FVector4f DefaultPinValue = FMaterialAttributeDefinitionMap::GetDefaultValue(Property);
		FVector3f DefaultPinVec3f = FVector3f(DefaultPinValue.X, DefaultPinValue.Y, DefaultPinValue.Z);
		FVector3f CurrentPinVec3f = FVector3f(OldNodeInput.Constant.X, OldNodeInput.Constant.Y, OldNodeInput.Constant.Z);
	...
	// We only convert materials to substrate when IsHiddenMaterialAssetConversionEnabled is false.
	// However, we do run the Substrate conversion/update code to make sure material with FrontMaterial plugged in are still converted.
	if (Substrate::IsSubstrateEnabled() && (!Substrate::IsHiddenMaterialAssetConversionEnabled() || bRequestedByUser))
	{
		BeginConstantNodeLayout();
		...
		bool bIsSubstrateLayersMaterial = false;
		if (Substrate::IsMaterialLayeringSupportEnabled() && bUseMaterialAttributes && EditorOnly->MaterialAttributes.Expression && !EditorOnly->FrontMaterial.IsConnected())
		{
			...
			for(UMaterialExpression* Expression : GetExpressions())
			{
				if(Expression->IsA<UMaterialExpressionMaterialAttributeLayers>())
				{
					...
					bIsSubstrateLayersMaterial = true;
					break;
				}
			}
		...
		if(!bIsSubstrateLayersMaterial)
		{
			...
			if (bUseMaterialAttributes && EditorOnly->MaterialAttributes.Expression && !EditorOnly->FrontMaterial.IsConnected() && !EditorOnly->MaterialAttributes.Expression->IsResultSubstrateMaterial(EditorOnly->MaterialAttributes.OutputIndex) && !bHasAnySubstrateNodes) // M_Rifle cause issues there
			{
				...
				ConnectionTo(EditorOnly->MaterialAttributes, ConvertAttributeNode->MaterialAttributes, SUBSTRATE_COPY_CONNECTION);
				...
				// Reconnect custom output to material attribute conversion node
				{
					...
					{
						ConnectionTo(SingleLayerWaterOutput->ScatteringCoefficients,	ConvertAttributeNode->WaterScatteringCoefficients);
						ConnectionTo(SingleLayerWaterOutput->AbsorptionCoefficients,	ConvertAttributeNode->WaterAbsorptionCoefficients);
						ConnectionTo(SingleLayerWaterOutput->PhaseG,					ConvertAttributeNode->WaterPhaseG);
						ConnectionTo(SingleLayerWaterOutput->ColorScaleBehindWater,		ConvertAttributeNode->ColorScaleBehindWater);
					}
				}
				...
				if (bRequireCustomTangent)
				{
					...
					SetAttributesNode->Inputs.Add(TangentOutput->Input);
				...
				else
				{
					...
					// Apply is thin surface if needed
					if (ShadingModel == MSM_TwoSidedFoliage)
					{
						bIsThinSurface = true;
					}
				}
			...
			else if (bUpdateEmptyMaterial)
			{
				...
				if (DefaultMF)
				{
					...
					if (MFCallNode->SetMaterialFunction(DefaultMF))
					{
						...
						ColorMatInputConnectionToIndexed(EditorOnly->BaseColor,		MFCallNode, 0, MP_BaseColor);
						ScalarMatInputConnectionToIndexed(EditorOnly->Metallic,		MFCallNode, 1, MP_Metallic);
						ScalarMatInputConnectionToIndexed(EditorOnly->Specular,		MFCallNode, 2, MP_Specular);
						ScalarMatInputConnectionToIndexed(EditorOnly->Roughness,	MFCallNode, 3, MP_Roughness);
						Vector3MatInputConnectionToIndexed(EditorOnly->Normal,		MFCallNode, 4, MP_Normal);
						ColorMatInputConnectionToIndexed(EditorOnly->EmissiveColor,	MFCallNode, 5, MP_EmissiveColor);
						ScalarMatInputConnectionToIndexed(EditorOnly->Opacity,		MFCallNode, 6, MP_Opacity);
					}
				}
				else
				{
			...
			else if (!bUseMaterialAttributes && !EditorOnly->FrontMaterial.IsConnected() && !bHasAnySubstrateNodes)
			{
				if (MaterialDomain == MD_Surface)
				{
					...
					ColorMatInputConnectionTo(EditorOnly->BaseColor,			ConvertNode->BaseColor,			MP_BaseColor);
					ScalarMatInputConnectionTo(EditorOnly->Metallic,			ConvertNode->Metallic,			MP_Metallic);
					ScalarMatInputConnectionTo(EditorOnly->Specular,			ConvertNode->Specular,			MP_Specular);
					ScalarMatInputConnectionTo(EditorOnly->Roughness,			ConvertNode->Roughness,			MP_Roughness);
					ScalarMatInputConnectionTo(EditorOnly->Anisotropy,			ConvertNode->Anisotropy,		MP_Anisotropy);
					ColorMatInputConnectionTo(EditorOnly->EmissiveColor,		ConvertNode->EmissiveColor,		MP_EmissiveColor);
					Vector3MatInputConnectionTo(EditorOnly->Normal,				ConvertNode->Normal,			MP_Normal, SUBSTRATE_COPY_CONNECTION);
					Vector3MatInputConnectionTo(EditorOnly->Tangent,			ConvertNode->Tangent,			MP_Tangent);
				...
				else if (MaterialDomain == MD_Volume)
				{
					...
					ColorMatInputConnectionTo(EditorOnly->BaseColor,		VolBSDF->Albedo,			MP_BaseColor);	
					ColorMatInputConnectionTo(EditorOnly->SubsurfaceColor,	VolBSDF->Extinction,		MP_SubsurfaceColor);
					ColorMatInputConnectionTo(EditorOnly->EmissiveColor,	VolBSDF->EmissiveColor,		MP_EmissiveColor);	
					ScalarMatInputConnectionTo(EditorOnly->AmbientOcclusion,VolBSDF->AmbientOcclusion,	MP_AmbientOcclusion);
				...
				else if (MaterialDomain == MD_LightFunction)
				{
					...
					ColorMatInputConnectionTo(EditorOnly->EmissiveColor, LightFunctionNode->Color, MP_EmissiveColor);
				...
				else if (MaterialDomain == MD_PostProcess)
				{
					...
					ColorMatInputConnectionTo(EditorOnly->EmissiveColor, PostProcNode->Color, MP_EmissiveColor);
					ScalarMatInputConnectionTo(EditorOnly->Opacity, PostProcNode->Opacity, MP_Opacity, SUBSTRATE_COPY_CONNECTION);	// We only copy, to keep Opacity on the root node in case BLEND_AlphaComposite is selected.
				...
				else if (MaterialDomain == MD_DeferredDecal)
				{
					...
					ReplaceNodeAndMoveToTheRight(ConvertToDecalNode, ConvertNode);
				...
				else if (MaterialDomain == MD_UI)
				{
					...
					ColorMatInputConnectionTo(EditorOnly->EmissiveColor,UINode->Color,	MP_EmissiveColor);
					ScalarMatInputConnectionTo(EditorOnly->Opacity,		UINode->Opacity,MP_Opacity, SUBSTRATE_COPY_CONNECTION);	// We only copy, to keep Opacity on the root node in case BLEND_AlphaComposite is selected.
		...
		if (bRelinkCustomOutputNodes)
		{
			...
			{
				FExpressionInput* TransmittanceColorInput = &ThinTranslucentOutput->TransmittanceColor;
				FExpressionInput* ThinTranslucentSurfaceCoverageInput = &ThinTranslucentOutput->SurfaceCoverage;
			...
			if (SingleLayerWaterOutput)
			{
				ConnectionTo(SingleLayerWaterOutput->ScatteringCoefficients,	ConvertNode->WaterScatteringCoefficients);
				ConnectionTo(SingleLayerWaterOutput->AbsorptionCoefficients,	ConvertNode->WaterAbsorptionCoefficients);
				ConnectionTo(SingleLayerWaterOutput->PhaseG,					ConvertNode->WaterPhaseG);
				ConnectionTo(SingleLayerWaterOutput->ColorScaleBehindWater,		ConvertNode->ColorScaleBehindWater);
			}
			if (ClearCoatBottomNormalOutput)
			{
				ConnectionTo(ClearCoatBottomNormalOutput->Input, ConvertNode->ClearCoatNormal, SUBSTRATE_COPY_CONNECTION);
			}
			if (TangentOutput)
			{
				ConnectionTo(TangentOutput->Input, ConvertNode->Tangent, SUBSTRATE_COPY_CONNECTION);
			}
		}
	}

	// Glint conversion parametrization
	if (EditorOnly->SubstrateVersion < 0)
	{
		// This is now done in ConvertSlabExpressionGlintDensity() during the material's PostLoad() function
		// This conversion change SubstrateVersion from -1 -> 0
	}
	//else if (EditorOnly->SubstrateVersion < 1)
	//{ [...] }
	...
	if (bInvalidateShader)
	{
		// 1. Store current Substrate version
		EditorOnly->SubstrateVersion = GetSubstrateVersion();

		// 2. Store current Substrate conversion version
		const bool bIsUpToDate = EditorOnly->SubstrateConversionVersion == GetSubstrateNoConversionVersion() || EditorOnly->SubstrateConversionVersion == GetSubstrateConversionVersion();
		if (!bIsUpToDate)
		{
			if (bEmptyShader) // == new shader
			{
				EditorOnly->SubstrateConversionVersion = GetSubstrateNoConversionVersion();
			}
			else // == Converted shader
			{
				EditorOnly->SubstrateConversionVersion = GetSubstrateConversionVersion();
				EditorOnly->bIsSubsutrateAutoConvertedAndNotSaved = true;
			}
		}
	}
...
static void ConvertSlabExpressionMaterialSubSurfaceType(TConstArrayView<TObjectPtr<UMaterialExpression>> InExpressions, EBlendMode InBlendMode, bool bInIsThinSurface)
{
	for (UMaterialExpression* Expression : InExpressions)
	{
		if (UMaterialExpressionSubstrateSlabBSDF* SlabExpression = Cast<UMaterialExpressionSubstrateSlabBSDF>(Expression))
		{
			PRAGMA_DISABLE_DEPRECATION_WARNINGS
			if (!IsOpaqueBlendMode(InBlendMode))
			{
				SlabExpression->SubSurfaceType = EMaterialSubSurfaceType::MSS_SimpleVolume;
			}
			else if (SlabExpression->bUseSSSDiffusion)
			{
				SlabExpression->SubSurfaceType = EMaterialSubSurfaceType::MSS_Diffusion;
			}
			else if (bInIsThinSurface)
			{
				SlabExpression->SubSurfaceType = EMaterialSubSurfaceType::MSS_TwoSidedWrap;
			}
			else
			{
				SlabExpression->SubSurfaceType = EMaterialSubSurfaceType::MSS_Wrap;
			}
		...
		else if (UMaterialExpressionMaterialFunctionCall* FunctionCall = Cast<UMaterialExpressionMaterialFunctionCall>(Expression))
		{
			if (FunctionCall->MaterialFunction)
			{
				ConvertSlabExpressionMaterialSubSurfaceType(FunctionCall->MaterialFunction->GetExpressions(), InBlendMode, bInIsThinSurface);
			}
		}
	}
}

static void ConvertSlabExpressionGlintDensity(UMaterial* InMaterial)
{
	// Convert existing Substrate material with glint. 
	// The FrontMaterial is expected to be connected. We rely on the Substrate version to make different 
	// between material already converted and not (in addition to the global object version which now insure all new material will have the 
	// correct glint density parameterization)
	UMaterialEditorOnlyData* EditorOnly = InMaterial->GetEditorOnlyData();
	if (EditorOnly && EditorOnly->SubstrateVersion < 0 && EditorOnly->FrontMaterial.IsConnected())
	{
		TArray<UMaterialExpressionSubstrateSlabBSDF*> SlabExpressions;
		InMaterial->GetAllExpressionsInMaterialAndFunctionsOfType<UMaterialExpressionSubstrateSlabBSDF>(SlabExpressions);
		for (UMaterialExpressionSubstrateSlabBSDF* SlabExpression : SlabExpressions)
		{
			if (SlabExpression->GlintValue.IsConnected())
			{
				// Gather custom output for thin translucency
				// Empty material: Create by default a slab node
				UMaterialFunction* ConversionMF = LoadObject<UMaterialFunction>(nullptr, TEXT("/Engine/Functions/Substrate/SMF_GlintConversion.SMF_GlintConversion"));
				if (ConversionMF)
				{
					ConversionMF->UpdateFromFunctionResource();
					ConversionMF->PostEditChange();
					ConversionMF->ConditionalPostLoad();

					UMaterialExpressionMaterialFunctionCall* MFCallNode = NewObject<UMaterialExpressionMaterialFunctionCall>(InMaterial);
					if (MFCallNode->SetMaterialFunction(ConversionMF))
					{
						// This is needed for input/output expressions to be set correctly, otherwise compilation will fail.
						EditorOnly->ExpressionCollection.AddExpression(MFCallNode);
						MFCallNode->MaterialExpressionEditorX = SlabExpression->MaterialExpressionEditorX - 200;
						MFCallNode->MaterialExpressionEditorY = SlabExpression->MaterialExpressionEditorY + 500;

						// Connect GlintValue graph to the MF input
						if (SlabExpression->GlintValue.IsConnected())
						{
							MFCallNode->GetInput(0/*Density*/)->Connect(SlabExpression->GlintValue.OutputIndex, SlabExpression->GlintValue.Expression);
							SlabExpression->GlintValue.Expression = nullptr;
						}

						// Connect the MF output to the GlintValue input
						SlabExpression->GlintValue.Connect(0, MFCallNode);
		...
		// Even if no glint conversion was done, we save the new Substrate's version (from -1 to 0)
		EditorOnly->SubstrateVersion = GetSubstrateVersion();
	}
}
#endif // WITH_EDITOR
...
void UMaterial::PostLoad()
{
	...
	// Can't use new material compilation validations on existing materials.
	bRelaxRuntimeVirtualTextureRestrictionsForOldContent = (UE5MainVer < FUE5MainStreamObjectVersion::RuntimeVirtualTextureMaterialValidation);
...
#if WITH_EDITOR
	// Convert UMaterialExpressionSubstrateSlabBSDF::bUseSSSDiffution into UMaterialExpressionSubstrateSlabBSDF::SubSurfaceType
	if (Substrate::IsSubstrateEnabled())
	{
		if (FortMainVer < FFortniteMainBranchObjectVersion::AddMaterialSubstrateSubsurfaceType)
		{
			ConvertSlabExpressionMaterialSubSurfaceType(GetExpressions(), BlendMode, bIsThinSurface);
		}

		if (FortMainVer < FFortniteMainBranchObjectVersion::ConvertGlintDensity)
		{
			ConvertSlabExpressionGlintDensity(this);
		}
	...
	else if (IsDeferredDecal() || IsUIMaterial() || (MaterialDomain == MD_LightFunction) || IsPostProcessMaterial())
	{
...
void UMaterial::AppendCompileStateDebugInfo(FStringBuilderBase& OutDebugInfo) const
{
	if (!CachedMaterialResourcesForCooking.IsEmpty())
	{
		...
		{
			for (const FMaterialResourceForCooking& Resource : Iter.Value())
			{
				OutDebugInfo << "Resource for platform " << LexToString(Resource.Platform) << ", IsCachingShaders=" << (Resource.Resource->IsCachingShaders() ? "true" : "false") << "\n";
				Resource.Resource->AppendCompileStateDebugInfo(OutDebugInfo);
			}
		}
	}
	else
...
bool UMaterial::IsCachedCookedPlatformDataLoaded( const ITargetPlatform* TargetPlatform ) 
{
	...
	if (UE_LOG_ACTIVE(LogMaterial, VeryVerbose))
	{
		...
		UE_LOG(LogMaterial, VeryVerbose, TEXT("Material [%s] Cook State:"), *GetName());
...
bool UMaterial::CanEditChange(const FProperty* InProperty) const
{
	if (InProperty)
	{
		...
		const bool bSubstrateEnabled = Substrate::IsSubstrateEnabled();
		const bool bSubstrateHiddenMaterialAssetConversion = bSubstrateEnabled && Substrate::IsHiddenMaterialAssetConversionEnabled() && !EditorOnly->FrontMaterial.IsConnected();

		if (PropertyName == GET_MEMBER_NAME_STRING_CHECKED(UMaterial, MaterialDomain))
		{
			// Material domain is no longer tweakable with Substrate. It is instead derived from the graph.
			return !bSubstrateEnabled || bSubstrateHiddenMaterialAssetConversion;
		}
		...
		if (PropertyName == GET_MEMBER_NAME_STRING_CHECKED(UMaterial, ShadingModel))
		{
			...
			return (!bSubstrateEnabled || bSubstrateHiddenMaterialAssetConversion) && (MaterialDomain == MD_Surface || MaterialDomain == MD_Volume);
		}
		...
		if (PropertyName == GET_MEMBER_NAME_STRING_CHECKED(UMaterial, bUsedWithStaticMesh))
		{
			// Don't bother to allow changes on DeferredDecal or Volume, they have shaders mixed into LocalVertexFactory which are needed
			return MaterialDomain == MD_Surface || MaterialDomain == MD_UI || MaterialDomain == MD_PostProcess || MaterialDomain == MD_LightFunction;
		}
		...
		if (PropertyName == GET_MEMBER_NAME_STRING_CHECKED(UMaterial, TranslucencyPass)
			...
			|| PropertyName == GET_MEMBER_NAME_STRING_CHECKED(UMaterial, bIsTranslucencyVelocityFromDepth))
		{
...
static bool ShouldAllowSubstrateReset(const UMaterialEditorOnlyData* EditorOnly)
{
	// We do reset the conversion to as "has been run" when Substrate is authored using the root node instead of FrontMaterial.
	return !Substrate::IsHiddenMaterialAssetConversionEnabled() || (EditorOnly && EditorOnly->FrontMaterial.IsConnected());
}
...
void UMaterial::PostEditChangePropertyInternal(FPropertyChangedEvent& PropertyChangedEvent, const EPostEditChangeEffectOnShaders EffectOnShaders)
{
	...
	if (Substrate::IsSubstrateEnabled() && ShouldAllowSubstrateReset(EditorOnly))
	{
	...
	if (Substrate::IsSubstrateEnabled() && EditorOnly->FrontMaterial.IsConnected())
	{
		...
		else
		{
			...
			else if (SubstrateMaterialInfo.HasOnlyShadingModel(SSM_VolumetricFogCloud))
			{
				...
				GetAllExpressionsInMaterialAndFunctionsOfType(ShadingModelExpressions);
				if (ShadingModelExpressions.Num() > 0)
				{
					// We can only keep a single shading model due to an assert in UMaterialInstance::UpdateOverridableBaseProperties which want a single shading model for non material attribute workflow.
					UMaterialExpressionSubstrateVolumetricFogCloudBSDF* MatExpr = ShadingModelExpressions[0];
	...
	ERHIFeatureLevel::Type InFeatureLevel, EMaterialQualityLevel::Type InQuality, ERHIShadingPath::Type InShadingPath, EShaderFrequency InShaderFrequency, EMaterialProperty InProperty, const bool bInRecurseIntoMaterialFunctions)
{
	...
	{
		checkf(SetMaterialAttributesExp->GetInput(0) && (SetMaterialAttributesExp->GetInputValueType(0) == MCT_MaterialAttributes), TEXT("There must always be one input at least : the material attribute pin"));
...
int32 UMaterial::CompilePropertyEx( FMaterialCompiler* Compiler, const FGuid& AttributeID )
{
	...
	switch (Property)
	{
		...
		case MP_Normal:
			// If material is not using tangent space normals, and nothing is connected to the normal pin,
			// instead of setting the normal to the constant use the vertex normal.
			if (!this->bTangentSpaceNormal && EditorOnly->Normal.UseConstant)
			{
				return Compiler->VertexNormal();
			}
			return EditorOnly->Normal.CompileWithDefault(Compiler, Property);
...
bool UMaterial::CheckInValidStateForCompilation(class FMaterialCompiler* Compiler) const
{
	...
	{
		for (const TPair<TObjectPtr<UMaterialExpression>, FString>& Error : CachedExpressionData->DuplicateParameterErrors)
		{
			Compiler->AppendExpressionError(Error.Get<0>(), *Error.Get<1>());
		}
...
{
	return bIsTranslucencyVelocityFromDepth;
}
...
{
	return bSubstrateRoughnessTracking;
}
...
bool UMaterial::IsPropertySupported(EMaterialProperty InProperty) const
{
	...
	if (Substrate::IsSubstrateEnabled() && !Substrate::IsHiddenMaterialAssetConversionEnabled())
	{
		bSupported = false;
		switch (InProperty)
		{
...
{
	const bool bSubstrateEnabled = Substrate::IsSubstrateEnabled();
	const bool bSubstrateHiddenConversion = Substrate::IsHiddenMaterialAssetConversionEnabled();
	const bool bShowAsSubstrate = bSubstrateEnabled && (!bSubstrateHiddenConversion || bFrontMaterialIsConnected);

	if (bSubstrateEnabled && bSubstrateHiddenConversion && InProperty == MP_FrontMaterial)
	{
		return true; // If hidden conversion, always enable FrontMaterial in case we want to plug a Substrate material in there.
	}
	...
	else
	{
		...
		case MP_FrontMaterial:
			{
				Active = bSubstrateHiddenConversion;
				break;
			}
		case MP_MaterialAttributes:
		default:
...
{
	bool bFrontMaterialConnected = false;
#if WITH_EDITOR
	if (!bUseMaterialAttributes)
	{
...
{
	BaseColor.Constant = FLinearColor::Gray;
	...
	EmissiveColor.Constant = FLinearColor(FMaterialAttributeDefinitionMap::GetDefaultValue(MP_EmissiveColor));
	...
	SubstrateVersion = GetSubstrateInvalidVersion();
}

void UMaterialEditorOnlyData::ResetSubstrateConversionVersion()
{
#if WITH_EDITOR
	if (ShouldAllowSubstrateReset(this))
#endif
	{
...
bool UMaterialEditorOnlyData::IsSubstrateAutoConverted() const
{
	return SubstrateConversionVersion > GetSubstrateNoConversionVersion();
}
