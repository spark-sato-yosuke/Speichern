...
protected:
	...
	UMaterialExpressionCustomOutput* TopCustomOutput = nullptr;
	...
	/** 
	  * Tracks if any virtual texture samples are used in the subgraph for the normal property.
	  * This is read later to know if the PixelNormalWS node is "connected" to virtual texture samples.
	  */
	uint32 bUsesVirtualTextureSampleForNormalProperty : 1;
	/** Flag that tracks if we are compiling a runtime virtual texture output subgraph. */
	uint32 bIsInRuntimeVirtualTextureOutput : 1;
...
public: 
	...
	uint64 CompileMaterialAttributesCustomOutputs(uint64 CustomOutputNodeBitmask);
...
protected:
	...
	// Constructs a code chunk that comes from an external HLSL function. Its return type and function implementation is data-driven and defined in BaseEditor.ini.
	int32 AddExternalCodeChunk(const FName& InExternalCodeIdentifier);
	int32 AddExternalCodeChunk(const FMaterialExternalCodeDeclaration& InExternalCode);
	...
	virtual int32 ViewProperty(EMaterialExposedViewProperty Property, bool bInvProperty) override;
	...
	*/	
	...
	virtual int32 SampleMaterialCache(int32 TexCoordIndexLiteral) override;
	virtual int32 GetMaterialCacheAttribute(int32 MaterialCacheIndex, const FGuid& Attribute) override;
	...
	virtual void FeedbackMaterialLayersInstancedGraphFromCompilation(const FMaterialLayersFunctions* InLayers) override;
	...
	virtual int32 MaterialCacheTextureDescriptor(uint32 LayerIndex) override;
	...
	int32 TransformBase(EMaterialCommonBasis SourceCoordBasis, EMaterialCommonBasis DestCoordBasis, FTransformParameters& Parameters, int32 A, int AWComponent);
	...
	int32 CalculatePeriodicWorldPositionOrigin(int TileScaleIndex);
	...
	int32 CustomOutput(const FString& CustomOutputName, const FString& FunctionName, int32 OutputIndex, int32 OutputCode, EMaterialCustomOutputFlags Flags); // Only used internally and intentionally not exposed on FMaterialCompiler.

	virtual int32 PushRuntimeVirtualTextureOutput() override;
	virtual int32 PopRuntimeVirtualTextureOutput(uint8 MaterialAttributeMask) override;
