...
bool UPackageMapClient::IsUsingRemoteObjectReferences() const
{
	if (Connection)
	{
		if (UNetDriver* Driver = Connection->Driver.Get())
		{
			return Driver->IsUsingRemoteObjectReferences();
		}
	}
	return false;
}
...
bool UPackageMapClient::SerializeNewActor(FArchive& Ar, class UActorChannel *Channel, class AActor*& Actor)
{
	...
	if (Ar.IsLoading() )
	{
		...
		UE_LOG(LogNetPackageMap, Log, TEXT("UPackageMapClient::SerializeNewActor BitPos: %" INT64_FMT), InBunch->GetPosBits() );
...
void UPackageMapClient::ExportNetGUIDHeader()
{
	...
	UE_LOG(LogNetPackageMap, Log, TEXT("	UPackageMapClient::ExportNetGUID. Bytes: %" INT64_FMT " Bits: %" INT64_FMT " ExportNetGUIDCount: %d"), CurrentExportBunch->GetNumBytes(), CurrentExportBunch->GetNumBits(), ExportNetGUIDCount);
...
void UPackageMapClient::ReceiveNetGUIDBunch( FInBunch &InBunch )
{
	...
	UE_LOG(LogNetPackageMap, Log, TEXT("UPackageMapClient::ReceiveNetGUIDBunch end. BitPos: %" INT64_FMT), InBunch.GetPosBits());
}
...
void UPackageMapClient::AppendExportBunches(TArray<FOutBunch *>& OutgoingBunches)
{
	...
	if (ExportBunches.Num() > 0)
	{
		if (UE_LOG_ACTIVE(LogNetPackageMap,Verbose))
		{
			...
			{
				UE_LOG(LogNetPackageMap, Verbose, TEXT("   BunchIndex: %d, ExportNetGUIDs: %d, NumBytes: %" INT64_FMT ", NumBits: %" INT64_FMT), It.GetIndex(), (*It)->ExportNetGUIDs.Num(), (*It)->GetNumBytes(), (*It)->GetNumBits() );
			}
		}
...
bool SplitAndAppendCustomExportsBunch(TArray<FOutBunch*>& AdditionalRequiredBunches, FOutBunch*& OriginalBunch, int32 MaxBunchSizeInBits)
{
	...
	if(OriginalBunch->GetNumBits() > MaxBunchSizeInBits)
	{
		...
		while(BitsLeft > 0)
		{
			...
			UE_LOG(LogNetPackageMap, Verbose, TEXT("	Creating partial bunch %s. bitsThisBunch: %" INT64_FMT " bitsLeft: %" INT64_FMT), *OriginalBunch->GetDebugString(), BitsThisBunch, BitsLeft);
...
FOutBunch* UPackageMapClient::CreateCustomExportsBunch(const FOutBunch& OutgoingBunch)
{
	...
	// NetToken exports
	{
		...
		for (FNetToken NetToken : NetTokensExportedInThisBunch)
		{
			...
			NetTokenResolveContext.NetTokenStore->WriteTokenData(*ExportsBunch, NetToken, this);
...
void UPackageMapClient::ReceiveCustomExportsBunch(FInBunch &InBunch)
{
	...
	while (bHasExportsToRead && !InBunch.IsError())
	{
		...
		NetTokenStore->ReadTokenData(InBunch, ImportedNetToken, *RemoteNetTokenStoreState, this);
...
void FNetGUIDCache::CleanReferences()
{
	...
	UE_LOG(LogNetPackageMap, Log, TEXT("FNetGUIDCache::CleanReferences: ObjectLookup: %i, NetGUIDLookup: %i, Mem: %" SIZE_T_FMT " kB"), ObjectLookup.Num(), NetGUIDLookup.Num(), (CountBytesAr.Mem / 1024));
}
...
PRAGMA_ENABLE_DEPRECATION_WARNINGS
	...
	{
		if (NetDriver->GetNetGuidCache().IsValid())
		{
			NetDriver->GetNetGuidCache()->AsyncPackageCallback(PackageName, Package, Result);
		}
	});
...
UObject* FNetGUIDCache::GetObjectFromNetGUID( const FNetworkGUID& NetGUID, const bool bIgnoreMustBeMapped )
{
	...
	if (CacheObjectPtr->Object.IsValid()
#if UE_WITH_REMOTE_OBJECT_HANDLE
		&& !UE::RemoteObject::Handle::IsRemote(FWeakObjectPtr(CacheObjectPtr->Object).GetRemoteId())
#endif
		)
	{
		Object = CacheObjectPtr->Object.Get();
...
{
	uint8 Flags = 0;
	Flags |= C.bExported	? (1 << 0) : 0;
	Flags |= C.bExportBlob	? (1 << 1) : 0;
	Ar << Flags;
	if (Ar.IsLoading())
	{
		C.bExported =	Flags & (1 << 0);
		C.bExportBlob = Flags & (1 << 1);
	}
	...
	if (C.bExported)
	{
		...
		if (C.bExportBlob)
		{
			SafeNetSerializeTArray_Default<4096>(Ar,C.Blob);
		}
	}
