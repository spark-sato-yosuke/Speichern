...
AActor* UGameplayStatics::GetActorOfClass(const UObject* WorldContextObject, TSubclassOf<AActor> ActorClass)
{
	...
	if (ActorClass)
	{
		if (UWorld* World = GEngine->GetWorldFromContextObject(WorldContextObject, EGetWorldErrorMode::LogAndReturnNull))
		{
			if (TActorIterator<AActor> It(World, ActorClass); It)
			{
				AActor* Actor = *It;
				return Actor;
			}
...
UParticleSystemComponent* CreateParticleSystem(UParticleSystem* EmitterTemplate, UWorld* World, AActor* Actor, bool bAutoDestroy, EPSCPoolMethod PoolingMethod)
{
	...
	if (FApp::CanEverRender() && World && !World->IsNetMode(NM_DedicatedServer))
	{
		...
		{
			PSC = NewObject<UParticleSystemComponent>((Actor ? Actor : (UObject*)World), NAME_None, RF_Transient);
...
{
	QUICK_SCOPE_CYCLE_COUNTER(UGameplayStatics_CreateSound2D);
...
{
	QUICK_SCOPE_CYCLE_COUNTER(UGameplayStatics_PlaySoundAtLocation);
...
{
	QUICK_SCOPE_CYCLE_COUNTER(UGameplayStatics_SpawnSoundAtLocation);
...
{
	QUICK_SCOPE_CYCLE_COUNTER(UGameplayStatics_SpawnSoundAttached);
