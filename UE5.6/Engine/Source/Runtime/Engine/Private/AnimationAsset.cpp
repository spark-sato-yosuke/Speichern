...
USkeletalMesh* UAnimationAsset::GetPreviewMesh(bool bFindIfNotSet)
{
	...
	if (PreviewMesh && (PreviewMesh->GetSkeleton() == nullptr || !PreviewMesh->GetSkeleton()->IsCompatibleForEditor(Skeleton)))
	{
		PreviewMesh = nullptr;
...
void UAnimationAsset::ValidateParentAsset()
{
	if (ParentAsset)
	{
		...
		{
			// parent asset changed skeleton, so we'll have to discard parent asset
