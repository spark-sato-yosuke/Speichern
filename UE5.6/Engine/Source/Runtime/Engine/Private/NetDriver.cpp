...
void UNetDriver::AddNetworkActor(AActor* Actor)
{
	...
	UE_LOG(LogNetLifecycle, Log, TEXT("Registering for replication: Actor: %s"), *GetFullNameSafe(Actor));
		...
		if (ReplicationSystem->IsServer())
		{
			...
			if (bShouldCreateDestructionInfoForInitiallyDormantActor && CanCreateDestructionInfo())
			{
				if (UEngineReplicationBridge* Bridge = ReplicationSystem->GetReplicationBridgeAs<UEngineReplicationBridge>())
				{
					...
					{
						const UReplicationBridge::FDestructionParameters Params
						{ 
...
{
	LLM_SCOPE_BYTAG(NetDriver);
	UE_LOG(LogNetLifecycle, Log, TEXT("Deregistering for replication: Actor: %s"), *GetFullNameSafe(Actor));
...
void UNetDriver::SetWorld(class UWorld* InWorld)
{
	...
	if (InWorld)
	{
		...
		if (IsUsingIrisReplication())
		{
			...
			{
				UE::Net::FReplicationSystemUtil::BeginReplicationForActorsInWorldForNetDriver(InWorld, this);
			}
		}
		else
#endif // UE_WITH_IRIS
...
void UNetDriver::SetReplicationSystem(UReplicationSystem* InReplicationSystem)
{
	...
	if (ReplicationSystem)
	{
		...
		UE::Net::FReplicationSystemUtil::BeginReplicationForActorsInWorldForNetDriver(GetWorld(), this);
	}
}
...
UE::Net::FNetIDVariant UNetDriver::CreateNetIDForObject(UObject* Object)
{
	...
	if (bIsUsingIris)
	{
		if (ReplicationSystem)
		{
			UObjectReplicationBridge* ObjectBridge = ReplicationSystem->GetReplicationBridgeAs<UObjectReplicationBridge>();
			if (ObjectBridge)
			{
