...
namespace LevelStreamingCVars
{
	...
	int32 GLevelStreamingPriorityBias = 0;
	FAutoConsoleVariableRef CVarLevelStreamingPriorityBias(
		TEXT("LevelStreaming.LevelStreamingPriorityBias"),
		GLevelStreamingPriorityBias,
		TEXT("PriorityBias added to level streaming priority when making async load requests"),
		ECVF_Default
	);
}
...
static void NetDriverRenameStreamingLevelPackageForPIE(const UWorld* World, FName UnPrefixedPackageName)
{
	...
	{
		if (Driver.NetDriver && Driver.NetDriver->GetNetGuidCache().IsValid())
		{
			for (TPair<FNetworkGUID, FNetGuidCacheObject>& GuidPair : Driver.NetDriver->GetNetGuidCache()->ObjectLookup)
			{
...
# if WITH_EDITORONLY_DATA
	...
	if (HasAnyFlags(RF_ClassDefaultObject) && !HasAnyFlags(RF_ImmutableDefaultObject) && ExactCast<ULevelStreaming>(this))
	{
...
ELevelStreamingTargetState ULevelStreaming::DetermineTargetState() const
{
	...
	case ELevelStreamingState::MakingVisible:
		...
		if (!ShouldBeVisible() && !World->IsLevelMakingVisible(LoadedLevel))
		{
...
bool ULevelStreaming::CanMakeInvisible()
{
	...
	//
	UWorld* World = GetWorld();
	if (World && World->IsGameWorld() && LoadedLevel && World->IsLevelMakingInvisible(LoadedLevel))
	{
		return true;
	}
...
bool ULevelStreaming::CanMakeVisible()
{
	...
	if (World && World->IsGameWorld() && LoadedLevel)
	{
		...
		if (World->IsLevelMakingVisible(LoadedLevel))
		{
			return true;
		}
	...
	if (World && World->IsGameWorld())
	{
		...
		if (bIsClientTransaction)
		{
			if (!bInShouldBeVisible && ShouldClientUseMakingInvisibleTransactionRequest())
			{
				...
				if (!LoadedLevel || !World->IsLevelMakingInvisible(LoadedLevel))
				{
			...
			else if (bInShouldBeVisible && ShouldClientUseMakingVisibleTransactionRequest())
			{
				...
				if (!LoadedLevel || !World->IsLevelMakingVisible(LoadedLevel))
				{
					NetVisibilityState.bHasClientPendingRequest = true;
...
void ULevelStreaming::UpdateStreamingState(bool& bOutUpdateAgain, bool& bOutRedetermineTarget, const TOptional<const UE::FTimeout>& InExternalTimeout)
{
	FScopeCycleCounterUObject ContextScope(this);
	...
	case ELevelStreamingState::MakingVisible:
		if (ensure(LoadedLevel))
		{
			...
			else
			{
				...
				if (!CanMakeVisible())
				{
					check(!World->IsLevelMakingVisible(LoadedLevel))
					break;
				}

				World->AddToWorld(LoadedLevel, LevelTransform, !bShouldBlockOnLoad, InExternalTimeout, TransactionId, this);
	...
	case ELevelStreamingState::MakingInvisible:
		if (ensure(LoadedLevel))
		{
			...
			if (!CanMakeInvisible())
			{
				check(!World->IsLevelMakingInvisible(LoadedLevel));
				break;
			}
			...
			World->RemoveFromWorld(LoadedLevel, !ShouldBlockOnUnload() && World->IsGameWorld(), InExternalTimeout, TransactionId, this);
...
ULevelStreaming* ULevelStreaming::FindStreamingLevel(const ULevel* Level)
{
	...
	if (Level && ensure(IsValid(Level)))
	{
		if (Level->OwningWorld && !Level->IsPersistentLevel())
		{
...
void ULevelStreaming::PrepareLoadedLevel(ULevel* InLevel, UPackage* InLevelPackage, int32 InPIEInstanceID)
{
	...
	{
		if (LoadedLevel && (LevelOwningWorld->IsLevelMakingVisible(LoadedLevel) || LevelOwningWorld->IsLevelMakingInvisible(LoadedLevel)))
		{
...
bool ULevelStreaming::RequestLevel(UWorld* PersistentWorld, bool bAllowLevelLoadRequests, EReqLevelBlock BlockPolicy)
{
    ...
    if (LoadedLevel && (PersistentWorld->IsLevelMakingVisible(LoadedLevel) || PersistentWorld->IsLevelMakingInvisible(LoadedLevel)))
    {
...
#endif
			if (!ensure(ULevelStreaming::ShouldReuseUnloadedButStillAroundLevels(World->PersistentLevel)))
			{
				UE_LOG(LogLevelStreaming, Log, TEXT("Trying to reuse level %s but ShouldReuseUnloadedButStillAroundLevels returns false. Trash the current package and reload"), *LevelPackage->GetName())
				FLevelStreamingGCHelper::TrashPackage(LevelPackage);
				return false;
			}
...
#endif
		LastAsyncLoadPackagePath = PackagePath;

		int32 OutAsyncLoadPriority;
		if (!FMath::AddAndCheckForOverflow(LevelStreamingCVars::GLevelStreamingPriorityBias, GetPriority(), OutAsyncLoadPriority))
		{
			OutAsyncLoadPriority = MAX_int32;
		}
		AsyncRequestIDs.Add(LoadPackageAsync(PackagePath, DesiredPackageName, FLoadPackageAsyncDelegate::CreateUObject(this, &ULevelStreaming::AsyncLevelLoadComplete), PackageFlags, PIEInstanceID, OutAsyncLoadPriority, InstancingContextPtr));
...
void ULevelStreaming::AsyncLevelLoadComplete(const FName& InPackageName, UPackage* InLoadedPackage, EAsyncLoadingResult::Type Result)
{
	...
	SetCurrentState(ELevelStreamingState::LoadedNotVisible);
	OnLoadingFinished();
	FPackagePath PackagePath = MoveTemp(LastAsyncLoadPackagePath);
	...
	if (InLoadedPackage)
	{
		...
		else
		{
			...
			{
				UE_LOG(LogLevelStreaming, Warning, TEXT("Failed to find a world in package '%s'"), *InPackageName.ToString());
				SetCurrentState(ELevelStreamingState::FailedToLoad);
	...
	else
	{
		...
		{
			UE_LOG(LogLevelStreaming, Error, TEXT("Couldn't find file for package %s."), *PackagePath.GetDebugName());
		}
		SetCurrentState(ELevelStreamingState::FailedToLoad);
 		SetShouldBeLoaded(false);
	}
...
void ULevelStreaming::SetPriorityOverride(const int32 PriorityOverride)
{
	if (!StreamingPriorityOverrideBackup.IsSet())
	{
		StreamingPriorityOverrideBackup = StreamingPriority;
	}

	SetPriority(PriorityOverride);
}
...
void ULevelStreaming::ResetPriorityOverride()
{
	if (StreamingPriorityOverrideBackup.IsSet())
	{
		SetPriority(StreamingPriorityOverrideBackup.GetValue());
		StreamingPriorityOverrideBackup.Reset();
	}
}
...
EStreamingStatus ULevelStreaming::GetLevelStreamingStatus() const
{
	...
	else if (LoadedLevel)
	{
		...
		else if ((CurrentState == ELevelStreamingState::MakingVisible) && (GetWorld()->IsLevelMakingVisible(LoadedLevel)))
		{
