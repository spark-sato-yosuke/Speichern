...
void FShadowMapPendingTexture::StartEncoding(const FStaticLightingBuildContext* LightingContext, ITextureCompressorModule* Compressor)
{
...

	{
		...
		// outer MipLock
		FTextureSource::FMipLock Mip0Lock(FTextureSource::ELockState::ReadWrite,&Texture->Source,0);
	...
	// @todo Oodle : manually calling CachePlatformData is not standard; not sure if this is a bug or was intentional for some reason
	//		more standard thing should be to just call PostEditChange() here and let UpdateResource do its thing.
