...
#if UE_WITH_IRIS
/** Controls if a replicated component should call EndReplication() immediately when updating its replication status. */
static bool GbCallEndReplicationForActorComponentWhenUpdatingReplicatedComponent = false;
static FAutoConsoleVariableRef CVarEndReplicationForActorComponentWhenUpdatingReplicatedComponent(
	TEXT("net.Iris.CallEndReplicationForActorComponentWhenUpdatingReplicatedComponent"),
	GbCallEndReplicationForActorComponentWhenUpdatingReplicatedComponent,
	TEXT("If true, replicated ActorComponentrs will call endreplication if they are no longer replicated after a call to UpdateReplicatedComponent."));
#endif
...
#ifndef CSV_RECORD_DETAILED_ACTOR_NET_STATS
#define CSV_RECORD_DETAILED_ACTOR_NET_STATS (CSV_PROFILER_STATS && !UE_BUILD_SHIPPING && UE_SERVER)
#endif // CSV_RECORD_DETAILED_ACTOR_NET_STATS

#if CSV_RECORD_DETAILED_ACTOR_NET_STATS
CSV_DEFINE_CATEGORY(Actor_FlushNetDormancy, false);
CSV_DEFINE_CATEGORY(Actor_ForceNetUpdate, false);
#endif // CSV_RECORD_DETAILED_ACTOR_NET_STATS
...
namespace ActorUtils
{
	...
	{
#if CSV_RECORD_DETAILED_ACTOR_NET_STATS
		// Which type of class name to record when the CSV categories: Actor_FlushNetDormancy or Actor_ForceNetUpdate are enabled
		...
		int32 CsvActorClassVerbosity = 0;
		FAutoConsoleVariableRef CVarCsvActorClassVerbosity(
			TEXT("net.ForceFlush.CsvActorClassVerbosity")
			, CsvActorClassVerbosity
			, TEXT("Which Actor type of class name to record when the CSV categories: Actor_FlushNetDormancy or Actor_ForceNetUpdate are enabled"
					" 0: Record the Parent native class name of the given Actor."
					" 1: Record the TopMost non-native class name of the given Actor."
					" 2: Record the Actor class name.")
			, ECVF_Cheat
...
#if CSV_RECORD_DETAILED_ACTOR_NET_STATS
	static void CsvRecordClassName(UClass* Class, int32 CsvCategoryIndex)
	{
		...
		if (!CsvProfiler->IsCategoryEnabled(CsvCategoryIndex))
		{
			return;
		}

		switch (ActorUtils::CVars::CsvActorClassVerbosity)
		{
			case 0: // Record the Parent native class name of the given Actor
			{
				Class = GetParentNativeClass(Class);
				break;
			}
			case 1: // Record the TopMost non-native class name of the given Actor
			{
				constexpr bool bAllowReturnNativeClass = true;
				Class = GetTopMostNonNativeClass(Class, bAllowReturnNativeClass);
				break;
			}
			case 2: // Record the Actor class name (reuse Class variable)
			{
				break;
			}
		}
		...
		if (ensure(Class))
		{
			CsvProfiler->RecordCustomStat(*GetNameSafe(Class), CsvCategoryIndex, 1, ECsvCustomStatOp::Accumulate);
		}
	}
#endif // CSV_RECORD_DETAILED_ACTOR_NET_STATS
...
void AActor::InitializeDefaults()
{
	...
	PrimaryActorTick.bRunTransactionally = true;
	...
	bHasPreRegisteredAllComponents = false;
...
#if UE_WITH_REMOTE_OBJECT_HANDLE
void FActorTickFunction::AddStructReferencedObjects(FReferenceCollector& Collector)
{
...
#if UE_WITH_REMOTE_OBJECT_HANDLE
void FActorComponentTickFunction::AddStructReferencedObjects(FReferenceCollector& Collector)
{
	Collector.AddReferencedObject(Target);
}
#endif
...
{
#if UE_WITH_REMOTE_OBJECT_HANDLE
	// Remote actors have already gone through the pcoesses of removing from world and by repeating it we'd migrate them back
	if (!HasAnyInternalFlags(EInternalObjectFlags::Remote))
	{
...
void AActor::Serialize(FArchive& Ar)
{
	...
	{
		uint8 MigrationFlags = 0;

		// TODO: This a workaround for not currently supporting native delegates in object migration
		// we are treating LifeSpan as a property of Actor rather than solving TimerManager's delegates generically.
		// Calling SetLifeSpan here is also not ideal because it's virtual which means it can do "anything" while in Serialize.
		// We could fix it with FUObjectAnnotationSparse
		float CurrentLifespan = 0.0f; 
		if (Ar.IsSaving())
		{
			CurrentLifespan = GetLifeSpan();
			SetLifeSpan(0.0f);

			MigrationFlags |= (static_cast<uint8>(ActorHasBegunPlay) & 3) << 0;
			MigrationFlags |= (bHasFinishedSpawning & 1) << 2;
			MigrationFlags |= (bActorInitialized & 1) << 3;
			MigrationFlags |= (bTickFunctionsRegistered & 1) << 4;
		}
		...
		if (Ar.IsLoading())
		{
			// When migrating actors are in a state after BeginPlay has been called but we need to call SetLifeSpan to initialize local timer
			SetLifeSpan(CurrentLifespan);

			ActorHasBegunPlay = static_cast<EActorBeginPlayState>(MigrationFlags & 3);
			bHasFinishedSpawning = (MigrationFlags >> 2) & 0x1;
			bActorInitialized = (MigrationFlags >> 3) & 0x1;
			bTickFunctionsRegistered = (MigrationFlags >> 4) & 0x1;
		}
	}
#endif // UE_WITH_REMOTE_OBJECT_HANDLE
}
...
void AActor::PostMigrate(const FUObjectMigrationContext& MigrationContext)
{
	// We want to be able to store "annotations" (metadata) about this Actor while we loan it out, so that
	// when we have the loan returned to us, we can check what has changed...
	struct FActorLoanedState
	{
		bool bActorWasDestroyed = false;
		bool bTickFunctionsRegistered = false;
		bool operator==(const FActorLoanedState&) const = default;
		bool IsDefault() const
		{
			static const FActorLoanedState DefaultLoanedState;
			return *this == DefaultLoanedState;
		}
	};
	static FUObjectAnnotationSparse<FActorLoanedState,/*bAutoRemove=*/true> RemoteObjects_ActorLoanedStates;

	Super::PostMigrate(MigrationContext);

	// We are using this as a proxy for having ownership until we sort out those mechanics
	const FRemoteServerId LocalServerId = UE::RemoteObject::GetGlobalServerId();
	const bool bHasOwnership = UE::RemoteObject::Handle::IsOwned(this);

	UWorld* const World = GetWorld();
	checkf(World, TEXT("Migrated Actor %s (%s) doesn't have a World"), *GetPathName(), *FRemoteObjectId(this).ToString());
	...
	if (MigrationContext.MigrationSide == EObjectMigrationSide::Send)
	{
		...
		UE_LOG(LogActor, Verbose, TEXT("%hs: Send %s (LocalSrvId: %s. DstSrvId: %s. HasOwnership: %d. MigrationType: %s)"), __func__, *GetName(), *LocalServerId.ToString(), *MigrationContext.RemoteServerId.ToString(), bHasOwnership, ToString(MigrationSendType));
		ensureMsgf(MigrationContext.RemoteServerId != LocalServerId, TEXT("%hs: Sending Object %s from %s to %s but destination is the same as the Local Server Id."), __func__, *GetName(), *LocalServerId.ToString(), *MigrationContext.RemoteServerId.ToString());

		const bool bKeepOwnership = (MigrationSendType == EObjectMigrationSendType::Loan);
		if (bKeepOwnership)
		{
			RemoteObjects_ActorLoanedStates.AddAnnotation(this, { .bActorWasDestroyed = (bool)bActorIsBeingDestroyed, .bTickFunctionsRegistered = (bool)bTickFunctionsRegistered });
		}

		if (!bKeepOwnership || !bHasOwnership)
		{
			// Remove the actor from the world on this server as it is about to be migrated to another server.
			// Migrated actors are unique to the server instance that currently owns their memory and are not replicated to other servers.
			if (World)
			{
				// Notify replication systems that this actor has been destroyed.
#if UE_WITH_IRIS
				EndReplication(EEndPlayReason::Destroyed);
#endif // UE_WITH_IRIS

				UE::Net::Private::FNetPropertyConditionManager::Get().NotifyObjectDestroyed(this);
				...
				if (FWorldContext* Context = GEngine->GetWorldContextFromWorld(World))
				{
					for (FNamedNetDriver& Driver : Context->ActiveNetDrivers)
					{
						...
						{
							Driver.NetDriver->NotifyActorDestroyed(this);
						}
					}
				}

				// Remove the actor from the actor list.
				World->RemoveActor(this, /*bShouldModifyLevel*/ false);
			}

			// Disabling Ticks prevents them from happening *this frame* while unregistering them stops them from happening *next frame*
			PrimaryActorTick.SetTickFunctionEnable(false);
			ForEachComponent(true, [](UActorComponent* ActorComponent)
			{
				ActorComponent->PrimaryComponentTick.SetTickFunctionEnable(false);
			});
			...
			RegisterAllActorTickFunctions(/*bRegisterTickFunctions*/ false, /*bIncludeComponents*/ true);
		}
	}
	else if (MigrationContext.MigrationSide == EObjectMigrationSide::Receive)
	{
		EObjectMigrationRecvType MigrationRecvType = MigrationContext.GetObjectMigrationRecvType(this);
		UE_LOG(LogActor, Verbose, TEXT("%hs: Recv %s (LocalSrvId: %s. FromSrvId: %s. HasOwnership: %d. MigrationType: %s)"), __func__, *GetName(), *LocalServerId.ToString(), *MigrationContext.RemoteServerId.ToString(), bHasOwnership, ToString(MigrationRecvType));

		// Build up the list of Components we have again (even if we previously owned this object, it could have been mutated)
		ResetOwnedComponents();

		// We can get rid of this check further down the road...
		// Set the actor's world transform if it has a native rootcomponent.
		const USceneComponent* SceneRootComponent = FixupNativeActorComponents(this);
		ensureMsgf(SceneRootComponent == RootComponent, TEXT("%hs: We were expecting RootComponent (%s) to be equal to FixupNativeActorComponents (%s). Transform fix-up code would be required to handle this case."), __func__, *GetNameSafe(RootComponent), *GetNameSafe(SceneRootComponent));

		// Add the actor that was migrated from another server to this server's world
		if (!bActorIsBeingDestroyed)
		{
			constexpr bool bOnlyAddIfUnique = true;
			GetLevel()->TryAddActorToList(this, bOnlyAddIfUnique);
		}

		// If we're being passed ownership, register us with the global systems
		if (MigrationRecvType == EObjectMigrationRecvType::AssignedOwnership)
		{
			ensureMsgf(!bActorIsBeingDestroyed, TEXT("We do not want to be assigned ownership of Actors that have already been destroyed. We do not yet know what to do with it."));

			#if UE_WITH_IRIS
			BeginReplication();
			#endif // UE_WITH_IRIS

			// Each component will be in charge of registering itself in its own PostMigrate, but we must register
			// the Actor (which we'll do be saying it's currently the opposite of the deserialized value, then flipping it)
			constexpr bool bRegisterComponents = false;
			bTickFunctionsRegistered = !bTickFunctionsRegistered;
			RegisterAllActorTickFunctions(!bTickFunctionsRegistered, bRegisterComponents);

			World->AddNetworkActor(this);
		}
		else if (MigrationRecvType == EObjectMigrationRecvType::ReturnedLoan)
		{
			FActorLoanedState LoanedState = RemoteObjects_ActorLoanedStates.GetAndRemoveAnnotation(this);
			if (bActorIsBeingDestroyed)
			{
				// If we weren't destroyed when we loaned it out, we should destroy it now that we've received it back...
				if (!LoanedState.bActorWasDestroyed)
				{
					ensureMsgf(IsPendingKillPending(), TEXT("%hs: We thought %s was destroyed while on loan, but our received state does not match a destroyed Actor"), __func__, *GetName());
					UE_LOG(LogActor, Warning, TEXT("%hs: Destroying %s back on owning server (it was destroyed while loaned-out)"), __func__, *GetName());

					// Destroy it again on the owning server, as it needs to be deregistered from our systems
					bActorIsBeingDestroyed = false;
					ClearGarbage();
					World->DestroyActor(this);
				}
			}
			else if (bTickFunctionsRegistered != LoanedState.bTickFunctionsRegistered)
			{
				// Remote server changed the tick registration
				constexpr bool bRegisterComponents = false;
				bTickFunctionsRegistered = LoanedState.bTickFunctionsRegistered;
				RegisterAllActorTickFunctions(!bTickFunctionsRegistered, bRegisterComponents);
			}

		}
		else if (MigrationRecvType == EObjectMigrationRecvType::Borrowed)
		{
			// Make sure that all tick functions have been deregistered if we don't have ownership.
			bool bTickFunctionsRegisteredOnOwningServer = bTickFunctionsRegistered;
			RegisterAllActorTickFunctions(false, false);
			bTickFunctionsRegistered = bTickFunctionsRegisteredOnOwningServer;

			// We will always add ourselves to the list when borrowed, even if destroyed
			// The reason is that we never want to GC it, and always want to return it during a sync point
			if (bActorIsBeingDestroyed)
			{
				constexpr bool bOnlyAddIfUnique = true;
				GetLevel()->TryAddActorToList(this, bOnlyAddIfUnique);
...
bool AActor::IsMigrationRoot() const
{
	return true;
}
...
void AActor::ProcessEvent(UFunction* Function, void* Parameters)
{
	...
	const bool bAllowScriptExecution = GAllowActorScriptExecutionInEditor || 
		Function->GetBoolMetaData(CallInEditorMeta) ||
		Function->HasAnyFunctionFlags(FUNC_UbergraphFunction); // uber graph frame is only used internally, e.g. by timer handlers
	#else
	const bool bAllowScriptExecution = GAllowActorScriptExecutionInEditor;
	#endif
	UWorld* MyWorld = GetWorld();
...
void AActor::GetStreamingBounds(FBox& OutRuntimeBounds, FBox& OutEditorBounds) const
{
	...
	if (!ForEachStreamingRelevantComponent(this, false, [&OutRuntimeBounds](const UActorComponent* Component, const FBox& StreamingBound)
	{
		OutRuntimeBounds += StreamingBound;
	}))
	{
		const FVector ActorLocation = GetActorLocation();
		OutRuntimeBounds = FBox(ActorLocation, ActorLocation);
	}
...
void AActor::PreSaveFromRoot(FObjectPreSaveRootContext ObjectSaveContext)
{
	...
	{
		if (UPackage* ActorExternalPackage = GetExternalPackage())
		{
			DetachExternalPackage();

			// Process objects found in the actor package
			...
			for (UObject* Object : Objects)
			{
				...
				{
					// Move objects in the destination actor package
					Object->Rename(nullptr, ActorPackage);

					// Keep track of which package to reattach later
					ObjectsDetachedFromExternalPackage.Add(Object);
				}
			}
		}
	}
...
void AActor::PostSaveFromRoot(FObjectPostSaveRootContext ObjectSaveContext)
{
	...
	if (ObjectSaveContext.IsCooking())
	{
		...
		if (UPackage* ActorExternalPackage = GetExternalPackage())
		{
			...
			for (const TObjectPtr<UObject>& ObjectToReattach : ObjectsDetachedFromExternalPackage)
			{
				ObjectToReattach->Rename(nullptr, ActorExternalPackage);
			}
		}

		ObjectsDetachedFromExternalPackage.Empty();
	}
#endif
}
...
bool AActor::Modify( bool bAlwaysMarkDirty/*=true*/ )
{
	...
	if (UChildActorComponent::ExperimentalAllowPerInstanceChildActorProperties())
	{
...
void AActor::ClearComponentOverlaps()
{
	...
	if (!GetWorld()->IsActorTickAndUserCallbacksEnabled())
	{
		return;
	}
#endif
...
{
#if CSV_RECORD_DETAILED_ACTOR_NET_STATS
	ActorUtils::CsvRecordClassName(GetClass(), CSV_CATEGORY_INDEX(Actor_ForceNetUpdate));
#endif // CSV_RECORD_DETAILED_ACTOR_NET_STATS
...
#if CSV_RECORD_DETAILED_ACTOR_NET_STATS
	ActorUtils::CsvRecordClassName(GetClass(), CSV_CATEGORY_INDEX(Actor_FlushNetDormancy));
#endif // CSV_RECORD_DETAILED_ACTOR_NET_STATS
...
{
	return Instigator ? Instigator->GetController() : nullptr;
}
...
void AActor::UpdateReplicatedComponent(UActorComponent* Component)
{
	...
	if (HasAuthority())
	{
		...
		{
			if (GbCallEndReplicationForActorComponentWhenUpdatingReplicatedComponent)
			{
				Component->EndReplication();
			}
			else
			{
				UE::Net::FReplicationSystemUtil::SetNetConditionForActorComponent(Component, COND_Never);
			}
		}
	}
#endif // UE_WITH_IRIS
	...
	for (FReplicatedComponentInfo& RepComponentInfo : ReplicatedComponentsInfo)
	{
		...
		if (HasAuthority())
		{
			...
			if (RepComponentInfo.Component && !RepComponentInfo.Component->GetIsReplicated())
			{
				...
				{
					UE::Net::FReplicationSystemUtil::SetNetConditionForActorComponent(RepComponentInfo.Component, COND_Never);
				}
			}
		}
#endif
...
static void DispatchOnComponentsCreated(AActor* NewActor)
{
	...
	LLM_SCOPE_DYNAMIC_STAT_OBJECTPATH(NewActor->GetPackage(), ELLMTagSet::Assets);
	LLM_SCOPE_DYNAMIC_STAT_OBJECTPATH(NewActor->GetClass(), ELLMTagSet::AssetClasses);
	UE_TRACE_METADATA_SCOPE_ASSET(NewActor, NewActor->GetClass());
...
void AActor::DispatchBeginPlay(bool bFromLevelStreaming)
{
	...
	if (World)
	{
		SCOPE_CYCLE_UOBJECT(BeginPlay, this);
		LLM_SCOPE_DYNAMIC_STAT_OBJECTPATH(GetPackage(), ELLMTagSet::Assets);
		LLM_SCOPE_DYNAMIC_STAT_OBJECTPATH(GetClass(), ELLMTagSet::AssetClasses);
		UE_TRACE_METADATA_SCOPE_ASSET(this, GetClass());
...
#if UE_SUPPORT_FOR_ACTOR_TICK_DISABLE
		if (World->IsActorTickAndUserCallbacksEnabled())
#endif
		{
			BeginPlay();
		}
...
int32 AActor::GetFunctionCallspace( UFunction* Function, FFrame* Stack )
{
	...
	else if (UE::Net::Private::FCoreNetContext::Get()->GetCurrentRemoteFunctionMode() == UE::Net::Private::ERemoteFunctionMode::Receiving)
	{
...
{
	if (!UChildActorComponent::ExperimentalAllowPerInstanceChildActorProperties() && IsChildActor())
	{
...
void AActor::RegisterAllComponents()
{
	...
	ensureMsgf(HasPreRegisteredAllComponents(), TEXT("Actor %s PreRegisterAllComponents implementation seems be to missing a call to AActor::PreRegisterAllComponents."), *this->GetFullName());
...
static USceneComponent* GetNonPreRegisteredParent(UActorComponent* Component)
{
	USceneComponent* ParentComponent = nullptr;
	USceneComponent* SceneComponent = Cast<USceneComponent>(Component);
	...
	while (SceneComponent && 
		   SceneComponent->GetAttachParent() && 
		   SceneComponent->GetAttachParent()->GetOwner() == Component->GetOwner() && 
		   !SceneComponent->GetAttachParent()->IsPreRegistering() &&
		   !SceneComponent->GetAttachParent()->IsPreRegistered())
	{
		SceneComponent = SceneComponent->GetAttachParent();
		if (SceneComponent->bAutoRegister && IsValidChecked(SceneComponent))
		{
			// We found non-preregistered parent that should be preregistered
...
void AActor::IncrementalPreUnregisterComponents()
{
	if (!SupportsIncrementalPreUnregisterComponents())
	{
		return;
	}

	TRACE_CPUPROFILER_EVENT_SCOPE(AActor::IncrementalPreUnregisterComponents);

	TInlineComponentArray<UActorComponent*> Components;
	for (UActorComponent* Component : GetComponents())
	{
		if (Component->ShouldIncrementalPreUnregister())
		{
			Components.Add(Component);
		}
	}
	...
	for (UActorComponent* Component : Components)
	{
		Component->PreUnregisterComponentFromWorld();
	}
}

void AActor::IncrementalPreRegisterComponents()
{
	if (!SupportsIncrementalPreRegisterComponents())
	{
		return;
	}

	TRACE_CPUPROFILER_EVENT_SCOPE(AActor::IncrementalPreRegisterComponents);

	// First call PreRegisterAllComponents (IncrementalPreRegisterComponents includes PreRegisterAllComponents)
#if WITH_EDITOR
	// In PIE, WorldSettings is spawned when world partition cell level is created, AActor::PostSpawnInitialize will 
	// have already called RegisterAllComponents (which calls PreRegisterAllComponents)
	if (!HasPreRegisteredAllComponents())
#endif
	{
		PreRegisterAllComponents();
	}
	...
	{
#if WITH_EDITOR
		// Ignore registered components (can happen if construction script adds component(s), which happens before this)
		if (!Component->IsRegistered())
#endif
		{
			if (Component->ShouldIncrementalPreRegister(World))
			{
				Components.Add(Component);
			}
		}
	...
	// No component found to pre-register
	if (Components.IsEmpty())
	{
		return;
	}

	// Pre-register RootComponent first so all other children components can reliably use it (i.e., call GetLocation) when they pre-register
	if (RootComponent && RootComponent->bAutoRegister && !RootComponent->IsPreRegistered())
	{
#if WITH_EDITOR
		// Ignore registered components (can happen if construction script adds component(s), which happens before this)
...
#endif 
		{
			check(!RootComponent->IsPreRegistering());
			RootComponent->PreRegisterComponentWithWorld(World);
		}
	}

	TSet<UActorComponent*> PreRegisteredParents;
	for (int32 CompIdx = 0; CompIdx < Components.Num(); CompIdx++)
	{
		UActorComponent* Component = Components[CompIdx];
		if (!Component->IsPreRegistered() && !Component->IsPreRegistering() && Component->bAutoRegister && IsValidChecked(Component))
		{
			// Ensure that all parents are pre-registered first
			USceneComponent* NonPreRegisteredParentComponent = UE::Private::GetNonPreRegisteredParent(Component);
			if (NonPreRegisteredParentComponent)
			{
				bool bParentAlreadyHandled = false;
				PreRegisteredParents.Add(NonPreRegisteredParentComponent, &bParentAlreadyHandled);
				if (bParentAlreadyHandled)
				{
					UE_LOG(LogActor, Error, TEXT("AActor::IncrementalPreRegisterComponents parent component '%s' cannot be pre-registered in actor '%s'"), *GetPathNameSafe(NonPreRegisteredParentComponent), *GetPathName());
					break;
				}

				// Register parent first, then return to this component on a next iteration
				Component = NonPreRegisteredParentComponent;
				CompIdx--;
			}
			Component->PreRegisterComponentWithWorld(World);
		}
	}
}
...
bool AActor::IncrementalRegisterComponents(int32 NumComponentsToRegister, FRegisterComponentContext* Context)
{
	...
	const UE::FTimeout* Timeout = Context ? &Context->GetIncrementalUpdateTimeout() : nullptr;
	...
	for (int32 CompIdx = 0; CompIdx < ComponentCount; CompIdx++)
	{
		...
		if (!Component->IsRegistered() && Component->bAutoRegister && IsValidChecked(Component))
		{
			...
			USceneComponent* UnregisteredParentComponent = UE::Private::GetUnregisteredParent(Component);
		...
		if ((CompIdx + 1 < ComponentCount) && (NumRegisteredComponentsThisRun >= NumComponentsToRegister))
		{
			NumRegisteredComponentsThisRun = 0;
			if (Timeout == nullptr)
			{
				// If there is no time limit, stop at NumComponentsToRegister
				break;
			}
			else if (Timeout->IsExpired())
			{
				// If there is a time limit, only stop if we're out of time
				break;
			}
		}
	}
	...
	if (ComponentCount == NumTotalRegisteredComponents)
	{
#if PERF_TRACK_DETAILED_ASYNC_STATS
...
{
#if UE_WITH_REMOTE_OBJECT_HANDLE
	// World Timer Manager is not yet transactional, so let's make this call transactional to make up for it
	static FName TransactionalWorkName(TEXT("AActor::LifeSpanExpired"));
	UE::RemoteExecutor::ExecuteTransactional(TransactionalWorkName, [this] { Destroy(); });
#else
	Destroy();
#endif
}
...
bool AActor::IsHLODRelevant() const
{
	...
	{
#if WITH_EDITOR
		// Treat actors in LI as HLOD relevant for the sake of visualisation modes
		if (!IsInLevelInstance())
		{
			return false;
		}
		else if (ULevelInstanceSubsystem* LevelInstanceSubsystem = UWorld::GetSubsystem<ULevelInstanceSubsystem>(GetWorld()))
		{
			if (const ILevelInstanceInterface* LevelInstance = LevelInstanceSubsystem->GetOwningLevelInstance(GetLevel()))
			{
				if (!LevelInstance->IsLevelInstanceHLODRelevant())
				{
					return false;
				}
			}
