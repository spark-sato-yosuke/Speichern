...
// Enable/Disable optimizing curve tables by removing redundant keys from rows
...
namespace
{
	...
	public:
		static FTransactionallySafeCriticalSection CriticalSection;
	};

	TMap<UCurveTable*, int32> FScopedCurveTableChange::ScopeCount;
	FTransactionallySafeCriticalSection FScopedCurveTableChange::CriticalSection;

#define CURVETABLE_CHANGE_SCOPE()	FScopedCurveTableChange ActiveScope(this);
}

FTransactionallySafeCriticalSection& UCurveTable::GetCurveTableChangeCriticalSection()
{
...
void UCurveTable::EmptyTable()
{
	...
	if (RowMap.Num() > 0)
	{
		...
		if (HasAnyFlags(RF_LoadCompleted) || !HasAnyFlags(RF_WasLoaded))
		{
			...
			// Ignore this for objects that are still loading as nothing can reference them yet.
...
void CopySharedInterpolationMode<FSimpleCurve>(const FSimpleCurve* SourceCurve, FSimpleCurve* TargetCurve)
{
	...
	{
		TargetCurve->SetKeyInterpMode(SourceCurve->GetKeyInterpMode());
	}
}
...
void CopyRowsToTable(const TMap<FName, CurveType*>& SourceRows, TMap<FName, FRealCurve*>& RowMap, TArray<FString>& OutProblems)
{
	for (const TPair<FName, CurveType*>& CurveRow : SourceRows)
	{
		...
		const TArray<CurveKeyType>& CurveKeys = InCurve->GetConstRefOfKeys();
		NewCurve->SetKeys(CurveKeys);
		
		// Simple curves need to setup their InterpMode
		CopySharedInterpolationMode(InCurve, NewCurve);
