...
public:
	...
	*/
	 ...
	 * @param Component RVT component containing the runtime virtual texture to invalidate
	 * @param WorldBounds World-space bounds of the area of the runtime virtual texture to invalidate
	 * @param InvalidatePriority Ensures the corresponding pages get processed in priority. This allows increased responsiveness when 
	...
	UE_DEPRECATED(5.6, "Use the alternate prototype with the Priority param")
	...
	UE_EXPERIMENTAL(5.6, "Experimental API, scene writing delegate may be internalized in the future.")
	virtual void UpdatePrimitiveInstancesFromCompute(FPrimitiveSceneDesc* Primitive, FGPUSceneWriteDelegate&& DataWriterGPU) = 0;

	UE_DEPRECATED(5.6, "StaticMeshBounds is no longer a parameter of this interface, please update FPrimitiveSceneDesc version.")
	...
	*/
	...
	UE_DEPRECATED(5.6, "FRayTracingDynamicGeometryCollection has been removed - all raytracing dynamic geometries are collected via the FRayTracingInstanceCollector during GetDynamicRayTracingInstances on the proxy.")
	virtual class FRayTracingDynamicGeometryCollection* GetRayTracingDynamicGeometryCollection() { return nullptr; }
	UE_DEPRECATED(5.6, "FRayTracingSkinnedGeometryUpdateQueue has been removed - all raytracing dynamic geometries are collected via the FRayTracingInstanceCollector during GetDynamicRayTracingInstances on the proxy.")
	...
	struct FCustomRenderPassRendererInput
	{
		...
		{
			EngineShowFlags.DisableFeaturesForUnlit();
		}
