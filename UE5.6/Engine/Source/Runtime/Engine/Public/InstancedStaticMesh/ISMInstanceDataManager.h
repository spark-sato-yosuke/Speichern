...
public:
	...
	const TSharedPtr<FISMCInstanceDataSceneProxy, ESPMode::ThreadSafe>& GetProxy() const { return Proxy; }
...
#if WITH_EDITOR
	// Early out test to save performance if no features are enabled that require BeginCacheForCookedPlatformData to be called
