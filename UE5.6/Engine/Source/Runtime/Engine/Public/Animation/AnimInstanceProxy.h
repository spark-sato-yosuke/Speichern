...
public:
	...
	UE_DEPRECATED(5.6, "Please use RegisterWatchedPose that takes a pose context")
	...
	// Record pose for node of ID LinkID if it is currently being watched
	ENGINE_API void RegisterWatchedPose(const FPoseContext& Pose, int32 LinkID);
	ENGINE_API void RegisterWatchedPose(const FComponentSpacePoseContext& Pose, int32 LinkID);
#endif
	...
	void AnimDrawDebugCapsule(const FVector& Center, float HalfHeight, float Radius, const FRotator& Rotation, const FColor& Color, bool bPersistentLines = false, float LifeTime = -1.f, float Thickness = 0.f, ESceneDepthPriorityGroup DepthPriority = SDPG_World) {}
