...
public:
	...
	/** Custom new/delete with recycling */
	ENGINE_API void* operator new(size_t Size);
	...
	*/
	virtual void InitRHI(FRHICommandListBase& RHICmdList) override
	{
		...
		const FRHIBufferCreateDesc CreateDesc =
			FRHIBufferCreateDesc::CreateVertex(TEXT("FNullDynamicParameterVertexBuffer"), sizeof(FParticleVertexDynamicParameter))
			.AddUsage(EBufferUsageFlags::Static)
			.SetInitialState(ERHIAccess::VertexOrIndexBuffer)
			.SetInitActionInitializer();

		FParticleVertexDynamicParameter Value;
		Value.DynamicValue[0] = Value.DynamicValue[1] = Value.DynamicValue[2] = Value.DynamicValue[3] = 1.0f;

		FRHIBufferInitializer Initializer = RHICmdList.CreateBufferInitializer(CreateDesc);
		Initializer.WriteData(&Value, sizeof(Value));
