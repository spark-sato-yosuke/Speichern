...
public:
	...
	ENGINE_API virtual bool GetActorIsHLODRelevant() const override;
	ENGINE_API virtual const TArray<FGuid>& GetReferences() const override;
	ENGINE_API virtual const FDataLayerInstanceNames& GetDataLayerInstanceNames() const override;
	ENGINE_API virtual bool IsEditorOnlyReference(const FGuid& ReferenceGuid) const override;
	ENGINE_API virtual FString ToString(FWorldPartitionActorDesc::EToStringMode Mode) const override;
	...
	ENGINE_API void SetRuntimeGrid(FName InRuntimeGrid);
	...
	FTransform GetEditorOnlyParentTransform() const { return EditorOnlyParentTransform.IsSet() ? EditorOnlyParentTransform.GetValue() : FTransform::Identity; }

	ENGINE_API void SetForcedNoHLODLayer();
	ENGINE_API void SetRuntimeHLODLayer(const FSoftObjectPath& InHLODLayer);
	ENGINE_API void SetForcedHLODIrrelevant();
...
private:
	...
	TOptional<FName> ResolvedRuntimeGrid;
...
public:
	...
	FWorldPartitionActorDescView* Emplace(const FGuid& InActorGuid, const FWorldPartitionActorDescView& InActorDescView) { return nullptr; }

	UE_DEPRECATED(5.4, "Use FWorldPartitionActorDescInstance version instead")
	class FWorldPartitionActorDescView* Emplace(const FWorldPartitionActorDesc* InActorDesc) { return nullptr; }
