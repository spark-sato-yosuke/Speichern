...
class ILevelInstanceInterface
{
	...
	bool IsInAnyEditMode() const { return IsEditing() || IsEditingPropertyOverrides(); }
	...
	// Whether the level instance is HLOD relevant. If not, all it's actors will be excluded from HLOD.
	ENGINE_API virtual bool IsLevelInstanceHLODRelevant() const;
#endif
