...
public:
	...
	void CacheDerivedData(
		...
		FSourceMeshDataForDerivedDataTask&& OptionalSourceMeshData = FSourceMeshDataForDerivedDataTask());

	UE_DEPRECATED(5.6, "Provide OptionalSourceMeshData by value (ideally using MoveTemp(...) to avoid copying the data)")
	void CacheDerivedData(
		const FString& InDDCKey,
	...
	{
		CacheDerivedData(InDDCKey, Mesh, GenerateSource, MaxLumenMeshCards, bGenerateDistanceFieldAsIfTwoSided, OptionalSourceMeshData ? *OptionalSourceMeshData : FSourceMeshDataForDerivedDataTask());
	}
...
private:
	...
	friend class FDistanceFieldAsyncQueue;
	...
	/** Task is being run immediately from an existing background worker (typically FAsyncDistanceFieldTask). */
	void RunImmediateTask(FAsyncCardRepresentationTask* Task);
	...
	/** Mark compilation of the provided objects as canceled. */
...
#if WITH_EDITORONLY_DATA
/**
 * IncrementalCook: Any UClass that uses data created by BeginCacheMeshCardRepresentation must add the version
 * numbers for that calculation to their ClassSchema, to invalidate their cooked data when those constants change.
 * This function handles that appending.
 */
extern ENGINE_API void AppendMeshCardConstantsToClassSchema(FAppendToClassSchemaContext& Context);

extern ENGINE_API FString BuildCardRepresentationDerivedDataKey(const FString& InMeshKey);

extern ENGINE_API void BeginCacheMeshCardRepresentation(
...
UE_DEPRECATED(5.6, "Provide OptionalSourceMeshData by value (ideally using MoveTemp(...) to avoid copying the data)")
inline void BeginCacheMeshCardRepresentation(
	const ITargetPlatform* TargetPlatform,
	UStaticMesh* StaticMeshAsset,
	class FStaticMeshRenderData& RenderData,
	const FString& DistanceFieldKey,
	FSourceMeshDataForDerivedDataTask* OptionalSourceMeshData)
{
	BeginCacheMeshCardRepresentation(StaticMeshAsset, RenderData, DistanceFieldKey, OptionalSourceMeshData ? *OptionalSourceMeshData : FSourceMeshDataForDerivedDataTask());
}
#endif
