...
/**
* Represents the evaluation of a material parameter directly to a byte offset in the preshader uniform buffer.
* A UniformParameterEvaluation avoids the overhead of executing an entire preshader when all it does is 
* evaluate a parameter with a single instruction.
*/
class FMaterialUniformParameterEvaluation
{
	DECLARE_TYPE_LAYOUT(FMaterialUniformParameterEvaluation, NonVirtual);
public:
	LAYOUT_FIELD(uint16, ParameterIndex);	/** Index of the parameter for UniformExpressionSet::GetNumericParameter() */
	LAYOUT_FIELD(uint16, BufferOffset); 	/** Offset in the preshader uniform buffer (in bytes) where result will be written */
};
...
inline constexpr uint32 NumMaterialTextureParameterTypes = (uint32)EMaterialTextureParameterType::Count;
...
public:
	...
	const FMaterialTextureCollectionParameterInfo& GetTextureCollectionParameter(int32 Index) const { return UniformTextureCollectionParameters[Index]; }
	...
	int32 GetNumTextureCollections() const { return UniformTextureCollectionParameters.Num(); }
	...
	/// Adds a new direct parameter evaluation for parameter with specified index (returned by FindOrAddNumericParameter)
	/// BufferOffset is the location in the uniform buffer in which to evaluate the parameter (in multiple of float components).
	...
	uint32 AllocateFromUniformBuffer(uint32 SizeInBytes);
...
public:
	...
	bool NeedsGBuffer() const
	{
		return
			...
			IsSceneTextureUsed(PPI_IsFirstPerson);
	}
...
struct FMaterialShaderParametersBase
{
	...
	union
	{
		// Room for up to 16 flags in previous padding space
	...
	union
	{
		// These flags are full, add more flags above
		uint64 PackedFlags;
		struct
		{
...
private:
	...
	friend inline FCbWriter& operator<<(FCbWriter& Writer, const FMaterialShaderParametersBase& Value)
	{
		Value.Save(Writer);
		return Writer;
	}
	friend ENGINE_API bool LoadFromCompactBinary(FCbFieldView Field, FMaterialShaderParametersBase& OutValue);
#endif
};
...
public:
#if WITH_EDITOR
	void Record(FMaterialKeyGeneratorContext& Context);
#endif
...
public:
	...
	/** A hash of all external code delcarations referenced in the shader map. */
	FSHAHash ExternalCodeReferencesHash;
...
public:
	...
	ENGINE_API virtual void GetShaderList(TMap<FHashedName, TShaderRef<FShader>>& OutShaders) const override;
	...
	ENGINE_API virtual void GetShaderPipelineList(TArray<FShaderPipelineRef>& OutShaderPipelines) const override;
	...
	*/
	...
	void AppendCompileStateDebugInfo(FStringBuilderBase& OutDebugInfo) const;
#endif
...
#if WITH_EDITOR && ALLOW_SHADERMAP_DEBUG_DATA
	// InFlightJobs is only populated during cooking as its current use is to track cook hangs.
	// if we need to generalize this later threadsafety will need to be added to make this work with ODSC
	// where the same shadermap can be accessed simultaneously both on GT and RT
	mutable TMap<uint32, TPair<FString, TSharedPtr<FShaderCompileJobStatus>>> InFlightJobs;
#endif
...
enum class EMaterialTranslateValidationFlags : uint32
{
	None = 0,
	Strict_RuntimeVirtualTexture = 1 << 0,
};
ENUM_CLASS_FLAGS(EMaterialTranslateValidationFlags)
...
public:
	...
	// Expressions are different (e.g. ResolvedView.PrevWorldViewOrigin) when using previous frame's values, value if from FHLSLMaterialTranslator::bCompilingPreviousFrame
...
UE_DEPRECATED(5.8, "Should call UMaterialExpression::IsAllowedIn on the Class' DefaultObject")
extern ENGINE_API bool IsAllowedExpressionType(const UClass* Class, const bool bMaterialFunction);
...
public:	
	...
	virtual bool IsTranslucencyVelocityFromDepth() const{ return false; }
	...
	virtual bool IsUsedWithHeterogeneousVolumes() const { return false; }
	virtual bool SupportsMaterialCache() const { return false; }
	...
	virtual bool IsUsedWithStaticMesh() const { return true; }
	virtual bool IsFullyRough() const { return false; }
	virtual bool GetForceCompatibleWithLightFunctionAtlas() const { return false; }
	...
	virtual bool IsDisplacementFadeEnabled() const { return false; }
	virtual FDisplacementFadeRange GetDisplacementFadeRange() const { return FDisplacementFadeRange(); }
	...
	virtual bool HasSubstrateRoughnessTracking() const { return true; }
	virtual bool AllowTranslucentLocalLightShadow() const { return false;  }
	virtual float GetTranslucentLocalLightShadowQuality() const { return 0.f;  }
	virtual float GetTranslucentDirectionalLightShadowQuality() const { return 0.f;  }
	...
	virtual EMaterialTranslateValidationFlags GetMaterialTranslateValidationFlags() const { return EMaterialTranslateValidationFlags::None; }
	...
	*/
	...
	inline bool IsGameThreadShaderMapComplete() const
	{
		...
		return bGameThreadShaderMapIsComplete.load(std::memory_order_relaxed);
	}

	inline bool IsRenderingThreadShaderMapComplete() const
	{
		checkSlow(IsInParallelRenderingThread());
		return bRenderingThreadShaderMapIsComplete.load(std::memory_order_relaxed);
	}
	...
	*/
	...
	virtual const FMaterialLayersFunctions* GetMaterialLayers() const { return nullptr; }
...
private:
	...
	// These are touched by different threads and need to be outside the bitfield above.
	std::atomic<bool> bGameThreadShaderMapIsComplete{ false };
	std::atomic<bool> bRenderingThreadShaderMapIsComplete{ false };
...
public:
	...
	ENGINE_API virtual bool IsUsedWithStaticMesh() const override;
	ENGINE_API virtual bool SupportsMaterialCache() const override;
	ENGINE_API virtual bool IsFullyRough() const override;
	ENGINE_API virtual bool GetForceCompatibleWithLightFunctionAtlas() const override;
	...
	ENGINE_API virtual bool AllowTranslucentLocalLightShadow() const override;
	ENGINE_API virtual float GetTranslucentLocalLightShadowQuality() const override;
	ENGINE_API virtual float GetTranslucentDirectionalLightShadowQuality() const override;
...
#if WITH_EDITOR
	ENGINE_API void GetShaderTags(TArray<FName>& OutShaderTags) const override;
	ENGINE_API EMaterialTranslateValidationFlags GetMaterialTranslateValidationFlags() const override;
	ENGINE_API virtual void NotifyCompilationFinished() override;
	/** Allows to associate the shader resources with the asset for load order. */
	ENGINE_API virtual FName GetAssetPath() const override;
	UE_DEPRECATED(5.5, "ShouldInlineShaderCode is no longer used when packaging shader maps. If you rely on this for some reason please reach out to the UE rendering team.")
	virtual bool ShouldInlineShaderCode() const override { return false; }
	...
	ENGINE_API virtual void FeedbackMaterialLayersInstancedGraphFromCompilation(const FMaterialLayersFunctions* InLayers) override;
	ENGINE_API virtual const FMaterialLayersFunctions* GetMaterialLayers() const override;
protected:
