...
public:
	...
	*/
	...
	virtual const FSoundWaveProxyPtr& GetStreamingSoundWave() const { return StreamingSoundWave; }
...
private:
	/** Attempts a seek to the frame by consuming decoded PCM data. Returns true
	 * if future seeks should no longer be attempted. */
	bool SeekToDecodedPCMFrame(uint32 InTargetAudioFrame);
};
