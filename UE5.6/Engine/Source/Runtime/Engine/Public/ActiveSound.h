...
private:
	...
	EVirtualizationMode VirtualizationModeOverride;

	uint8 bEnableVirtualizationModeOverride: 1;
...
public:
	...
	*/
	...
	/** Creates a copy of the modulation configuration for all WaveInstances on this ActiveSound.
	 * Each element in the array corresponds to a Wave Instance's modulation settings, factoring in the routing settings on the wave instances. */
...
private:
	...
	/** if OcclusionCheckInterval >= 0.0, periodically checks if the sound has become (un)occluded during playback
	 * When the interval is < 0.0, perform the initial occlusion check, but do not retest occlusion.
	 * @param ListenerLocation - location of the closest listener to the sound
	 * @param SoundLocation - location of the sound
	 * @param AttenuationSettingsPtr - Attenuation settings used (subject to various overrides)
	 */
