...
struct FCSPose
{
	...
	*/
	FORCEINLINE_DEBUGGABLE void LocalBlendCSBoneTransforms(TConstArrayView<FBoneTransform> BoneTransforms, float Alpha);
	 ...
	 */
	FORCEINLINE_DEBUGGABLE static void ConvertComponentPosesToLocalPoses(FCSPose<PoseType>&& InPose, PoseType& OutPose);
	 ...
	 * If a part of chain hasn't been converted, it will trigger ensure. 
	 */
	FORCEINLINE_DEBUGGABLE static void ConvertComponentPosesToLocalPoses(const FCSPose<PoseType>& InPose, PoseType& OutPose);
	 ...
	 */
	FORCEINLINE_DEBUGGABLE static void ConvertComponentPosesToLocalPosesSafe(FCSPose<PoseType>& InPose, PoseType& OutPose);
protected:
	PoseType Pose;
...
void FCSPose<PoseType>::CalculateComponentSpaceTransform(BoneIndexType BoneIndex)
{
	...
	do
	{
		...
		check(!Bone.ContainsNaN());
		check(!ParentBone.ContainsNaN());
