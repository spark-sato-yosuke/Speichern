...
struct IStreamingManager
{
	 ...
	 * @param InWorld				The world this view is located in
	 */
...
struct IRenderAssetStreamingManager : public IStreamingManager
{
	...
	**/
	...
	constexpr static auto ShouldChooseDefault = [](const UPrimitiveComponent*) { return true; };
	...
	virtual void GetAssetComponents(
		...
		TFunction<bool(const UPrimitiveComponent*)> ShouldChoose = ShouldChooseDefault) = 0;
...
struct IAudioStreamingManager : public IStreamingManager
{
	 ...
	 */
	constexpr static auto LoadCompletedDefault = [](EAudioChunkLoadResult) {};
	...
	/**
	 * In the event that the decoder playing back the chunks is in a starvation state due
	 * to an outstanding chunk load, handle that here.
	 */
...
struct FDummyAudioStreamingManager final : public IAudioStreamingManager
{
	...
	virtual bool RequestChunk(const FSoundWaveProxyPtr& SoundWave, uint32 ChunkIndex, TFunction<void(EAudioChunkLoadResult)> OnLoadCompleted = LoadCompletedDefault, ENamedThreads::Type ThreadToCallOnLoadCompletedOn = ENamedThreads::AnyThread, bool bForImmediatePlayback = false) override { return false; }
