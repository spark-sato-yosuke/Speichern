...
struct FQuantizedSignedDistanceFieldShadowSample
{
	...
	float GetFilterableComponent(int32 Index) const
	{
		if (Index == 0)
		{
			return Distance * (1.f / 255.0f);
		}
		else
		{
			checkSlow(Index == 1);
			return PenumbraSize * (1.f / 255.0f);
		}
	}
	
	// this is not a correct U8 quantizer (should round, not trunc); see below for correct version
	...
	#if 0
	// correct quantizer; needs to be tested to make sure other biases aren't compensating for the error
	void SetFilterableComponent(float InComponent, int32 Index)
	{
		uint8 Quantized = FColor::QuantizeUNormFloatTo8(InComponent);
