...
public:
	inline FPrimitiveUniformShaderParametersBuilder& Defaults()
	{
		...
		Parameters.NaniteAssemblyTransformOffset		= INDEX_NONE;
		...
		Parameters.MaterialCacheTextureDescriptor		= FUintVector2(0, 0);
...
#define PRIMITIVE_UNIFORM_BUILDER_FLAG_METHOD(INPUT_TYPE, VARIABLE_NAME) \
	...
	PRIMITIVE_UNIFORM_BUILDER_METHOD(uint32,			NaniteAssemblyTransformOffset);
	...
	PRIMITIVE_UNIFORM_BUILDER_METHOD(FUintVector2,		MaterialCacheTextureDescriptor);
...
#undef PRIMITIVE_UNIFORM_BUILDER_METHOD
	...
	inline FPrimitiveUniformShaderParametersBuilder& CustomPrimitiveData(const FCustomPrimitiveData* InCustomPrimitiveData)
	{
		...
		if (InCustomPrimitiveData && !InCustomPrimitiveData->Data.IsEmpty())
		{
	...
	inline const FPrimitiveUniformShaderParameters& Build()
	{
		...
		Parameters.Flags |= bIsFirstPerson ? PRIMITIVE_SCENE_DATA_FLAG_IS_FIRST_PERSON : 0u;
