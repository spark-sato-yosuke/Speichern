...
// Deprecated. Text, log with output to regular unreal logs too.  Regular log will always happen even if the Visual Logger is disabled by via compiler switch.  See also UE_CVLOG_ALWAYS_UELOG below
#define UE_VLOG_ALWAYS_UELOG(LogOwner, CategoryName, Verbosity, Format, ...) UE_DEPRECATED_MACRO(5.6, "Use UE_VLOG_UELOG instead, it now behaves like the 'always' version.") { UE_VLOG_UELOG(LogOwner, CategoryName, Verbosity, Format, ##__VA_ARGS__); }
...
// Definition is the same with/without ENABLE_VISUAL_LOG, because it's dependent on UE_VLOG_UELOG which handles the difference.
...
class FVisualLogger : public FOutputDevice
{
	...
	UE_DEPRECATED(5.6, "Use ArrowLineLogfImpl")
	...
	static ENGINE_API void ArrowLineLogfImpl(const UObject* LogOwner, const FName& CategoryName, ELogVerbosity::Type Verbosity, const FVector& Start, const FVector& End, const FColor& Color, const uint16 Mag, const TCHAR* Fmt, ...);
	UE_DEPRECATED(5.6, "Use DiscLogfImpl")
	...
	static ENGINE_API void DiscLogfImpl(const UObject* LogOwner, const FName& CategoryName, ELogVerbosity::Type Verbosity, const FVector& Center, const FVector& UpAxis, const float Radius, const FColor& Color, const uint16 Thickness, bool bWireframe, const TCHAR* Fmt, ...);
	static ENGINE_API void CoordinateSystemLogfImpl(const UObject* LogOwner, const FName& CategoryName, ELogVerbosity::Type Verbosity, const FVector& AxisLoc, const FRotator& AxisRot, const float Scale, const FColor& Color, const uint16 Thickness, const TCHAR* Fmt, ...);
...
public:
	...
	// Arrow
	template <typename FmtType, typename... Types>
	UE_DEPRECATED(5.6, "Use ArrowLineLogf")
	static void ArrowLogf(const UObject* LogOwner, const FLogCategoryBase& Category, ELogVerbosity::Type Verbosity, const FVector& Start, const FVector& End, const FColor& Color, const FmtType& Fmt, Types... Args)
	{
		...
		ArrowLineLogfImpl(LogOwner, Category.GetCategoryName(), Verbosity, Start, End, Color, 8, (const TCHAR*)Fmt, Args...);
	}
	template <typename FmtType, typename... Types>
	UE_DEPRECATED(5.6, "Use ArrowLineLogf")
	static void ArrowLogf(const UObject* LogOwner, const FName& CategoryName, ELogVerbosity::Type Verbosity, const FVector& Start, const FVector& End, const FColor& Color, const FmtType& Fmt, Types... Args)
	{
		...
		ArrowLineLogfImpl(LogOwner, CategoryName, Verbosity, Start, End, Color, 8, (const TCHAR*)Fmt, Args...);
	}
	...
	// Circle log
	template <typename FmtType, typename... Types>
	UE_DEPRECATED(5.6, "Use DiscLogf")
	static void CircleLogf(const UObject* LogOwner, const FLogCategoryBase& Category, ELogVerbosity::Type Verbosity, const FVector& Center, const FVector& UpAxis, const float Radius, const FColor& Color, const uint16 Thickness, const FmtType& Fmt, Types... Args)
	{
		...
		DiscLogfImpl(LogOwner, Category.GetCategoryName(), Verbosity, Center, UpAxis, Radius, Color, Thickness, /*bWireframe = */false, (const TCHAR*)Fmt, Args...);
	}
	template <typename FmtType, typename... Types>
	UE_DEPRECATED(5.6, "Use DiscLogf")
	static void CircleLogf(const UObject* LogOwner, const FName& CategoryName, ELogVerbosity::Type Verbosity, const FVector& Center, const FVector& UpAxis, const float Radius, const FColor& Color, const uint16 Thickness, const FmtType& Fmt, Types... Args)
	{
		...
		DiscLogfImpl(LogOwner, CategoryName, Verbosity, Center, UpAxis, Radius, Color, Thickness, /*bWireframe = */false, (const TCHAR*)Fmt, Args...);
	}
	...
	// Cone log
	template <typename FmtType, typename... Types>
	UE_DEPRECATED(5.4, "Use ConeLogf")
	...
	{ 
		...
		{
			UE::TWriteScopeLock Lock(Logger.RedirectRWLock);
	...
	{ 
		FVisualLogger& Logger = FVisualLogger::Get();
		UE::TReadScopeLock Lock(Logger.RedirectRWLock);
	 ...
	 *	@note this function can return null */
	UE_DEPRECATED(5.6, "This method will no longer be exposed, use ExecuteOnLastEntryForObject instead.")
	 ...
	 */
	ENGINE_API bool ExecuteOnLastEntryForObject(const UObject* Object, TFunctionRef<void(FVisualLogEntry&)> Function);
	...
	struct FVisualLoggerObjectEntryMap : TMap<FObjectKey, FVisualLogEntry>
	{
		...
		UE_MT_DECLARE_RW_RECURSIVE_ACCESS_DETECTOR(EntriesAccessDetector);
	};
...
private:
	...
	ENGINE_API void CalculateEntryAllowLogging(FVisualLogEntry* CurrentEntry, const UObject* LogOwner, const UObject* Object) const;
