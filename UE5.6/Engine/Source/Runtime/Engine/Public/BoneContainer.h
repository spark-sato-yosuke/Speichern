...
public:
	...
	*/
	...
	void FillWithCompactRefPose(ArrayType& OutTransforms) const
	{
		...
		const TArray<FTransform>& RefPoseTransforms = RefPoseOverride.IsValid() ? RefPoseOverride->RefBonePoses : RefSkeleton->GetRefBonePose();
	...
	{
		const TArray<FTransform>& RefPoseTransforms = RefPoseOverride.IsValid() ? RefPoseOverride->RefBonePoses : RefSkeleton->GetRefBonePose();
		return RefPoseTransforms[BoneIndicesArray[BoneIndex.GetInt()]];
	}
