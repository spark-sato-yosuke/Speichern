...
static inline constexpr auto DefaultPartitionActorCreated = [](APartitionActor*) {};
...
{
	ENGINE_API FActorPartitionGetParams(const TSubclassOf<APartitionActor>& InActorClass, bool bInCreate, ULevel* InLevelHint, const FVector& InLocationHint, uint32 InGridSize = 0, const FGuid& InGuidHint = FGuid(), bool bInBoundsSearch = true, TFunctionRef<void(APartitionActor*)> InActorCreated = DefaultPartitionActorCreated);
...
public:
	 ...
	 */
	ENGINE_API APartitionActor* GetActor(const TSubclassOf<APartitionActor>& InActorClass, const FCellCoord& InCellCoords, bool bInCreate, const FGuid& InGuid = FGuid(), uint32 InGridSize = 0, bool bInBoundsSearch = true, TFunctionRef<void(APartitionActor*)> InActorCreated = DefaultPartitionActorCreated);
	...
	/**
	 * Updates the partition actor data layers to match the provided ones.
	 * The partition actor name and its external package will change accordingly.
	 */
	ENGINE_API bool MoveActorToDataLayers(APartitionActor* InActor, const TArray<UDataLayerInstance*>& InDataLayerInstances);
#endif
...
#if WITH_EDITOR
	APartitionActor* GetActor(const FActorPartitionIdentifier& InActorPartitionId, const FCellCoord& InCellCoords, bool bInCreate, uint32 InGridSize, bool bInBoundsSearch, TFunctionRef<void(APartitionActor*)> InActorCreated = DefaultPartitionActorCreated);
