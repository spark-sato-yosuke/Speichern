...
private:
	...
	*/
	...
	/**
	 * Sets the current connection to Connection and serializes the property.
	 * The CriticalSection lock must be held while calling.
	 *
	 * @param	NameTableIndex	Name of the property being replicated
	 * @param	NumBits			Number of bits used to replicate this property
	 */
	void SerializeReplicateProperty( uint32 NameTableIndex, uint16 NumBits, UNetConnection* Connection );
	
	/**
	 * Sets the current connection to Connection and serializes the property header.
	 * The CriticalSection lock must be held while calling.
	 *
	 * @param	NameTableIndex	Name of the property being replicated
	 * @param	NumBits			Number of bits used in the header
	 */
	void SerializeWritePropertyHeader( uint32 NameTableIndex, uint16 NumBits, UNetConnection* Connection );

	/**
	 * Sets the current connection to Connection and serializes the property handle.
	 * The CriticalSection lock must be held while calling.
	 *
	 * @param NumBits Number of bits in the property handle
	 */
	void SerializeWritePropertyHandle( uint16 NumBits, UNetConnection* Connection );

	/**
	 * Sets the current connection to Connection and serializes the start of actor content block headers
	 *
	 * @param NameTableIndex the name of the object being replicated (might be a subobject of the actor)
	 * @param NumBits the number of bits in the content block header
	 */
	void SerializeBeginContentBlock( uint32 NameTableIndex, uint16 NumBits, UNetConnection* Connection );

	/**
	 * Sets the current connection to Connection and serializes the end of actor content block headers
	 *
	 * @param NameTableIndex the name of the object being replicated (might be a subobject of the actor)
	 * @param NumBits the number of bits in the content block header
	 */
	void SerializeEndContentBlock( uint32 NameTableIndex, uint16 NumBits, UNetConnection* Connection );
...
public:
	...
	*/
	...
	void TrackCompareProperties(const UObject* Object, uint32 Cycles, TBitArray<>& PropertiesCompared, TBitArray<>& PropertiesThatChanged, const TArray<T>& PropertyNameContainers, ProjectionType PropertyNameProjection)
	{
		...
		{
			UE::TScopeLock ScopeLock(CriticalSection);
