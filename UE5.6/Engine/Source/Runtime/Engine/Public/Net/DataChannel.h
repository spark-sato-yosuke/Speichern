...
public:
	...
	template<int32 Index, typename... ParamTypes> friend class UE::Net::TNetControlMessageImpl;
private:
...
public:
	...
	{
		FNetControlMessageInfo::SetName(Index, InName);
		return 0;
	}
	...
	static void Send(UNetConnection* Conn, ParamTypes&... Params)
	{
		...
		if (Conn->Channels[0] != nullptr && !Conn->Channels[0]->Closing)
		{
...
DEFINE_CONTROL_CHANNEL_MESSAGE(Hello, 0, uint8, uint32, FString, EEngineNetworkRuntimeFeatures); // initial client connection message
...
DEFINE_CONTROL_CHANNEL_MESSAGE(Upgrade, 2, uint32, EEngineNetworkRuntimeFeatures); // server tells client their version is incompatible
...
DEFINE_CONTROL_CHANNEL_MESSAGE(BeaconAssignGUID, 27, UE::Net::FNetIDVariant); // client assigns NetGUID from server to beacon actor, sends NetGUIDAck
...
DEFINE_CONTROL_CHANNEL_MESSAGE(IrisNetRefHandleError, 30, UE::Net::ENetRefHandleError, uint64); // a specific handle caused an error and we want the remote connection to log all information it has on it
DEFINE_CONTROL_CHANNEL_MESSAGE(IrisNetRefHandleErrorWithArray, 33, UE::Net::ENetRefHandleError, uint64, TArray<uint64>); // a specific handle caused 

// Called in place of Join or JoinSplit during connection handshake, and will created a connection that has the following properties:
//	* Won't spawn an instance of APawn.
//	* Won't call AGameModeBase::Login().
//	* Spawns ANoPawnPlayerController instead of using the class in AGameMode::PlayerControllerClass.
//	* Network relevency will be based on the location of ANoPawnPlayerController.
//
// These messages are only intended to be used when called from one game server to another game server. For this reason it is disabled
// by default and must be explicitly enabled on the receiving server using the cvar net.EnableSupportForNoPawnConnection or calling
// UNetDriver::SetSupportForNoPawnConnection() directly.
