...
public:
	...
	/**
	 * The platform(s) that are using using this RenderData. There may be more than one if multiple
	 * platforms share the same key.
	 */
	TSet<const ITargetPlatform*> SupportedPlatforms;
	/** The number of times Cache has been called. Used to log errors when we are calling it repeatedly. */
	...
	/** Derived from NaniteSettings to be used in HasNaniteFallbackMesh(...) in builds with editor-only data. */
	...
	/**
	 * Copies data from an earlier FStaticMeshRenderData for the same platform which should persist across
	 * retries; the persistent data is used to track repeated failures and error exit.
	 */
