...
class UMaterialInstanceDynamic : public UMaterialInstance
{
	...
	UFUNCTION(BlueprintCallable, Category="Rendering|Material")
	ENGINE_API void SetTextureCollectionParameterValue(FName ParameterName, UTextureCollection* Value);

	/** Set an MID texture collection parameter value using MPI (to allow access to layer parameters) */
	UFUNCTION(BlueprintCallable, Category = "Rendering|Material")
	ENGINE_API void SetTextureCollectionParameterValueByInfo(const FMaterialParameterInfo& ParameterInfo, UTextureCollection* Value);

	/** Get the current MID texture collection parameter value, using MPI (to allow access to layer parameters) */
	UFUNCTION(BlueprintCallable, meta = (DisplayName = "Get Texture Collection Parameter Value By Info", ScriptName = "GetTextureCollectionParameterValueByInfo"), Category = "Rendering|Material")
	ENGINE_API UTextureCollection* K2_GetTextureCollectionParameterValueByInfo(const FMaterialParameterInfo& ParameterInfo);

	/** Get the current MID texture collection parameter value */
	UFUNCTION(BlueprintCallable, meta=(DisplayName = "Get Texture Collection Parameter Value", ScriptName = "GetTextureCollectionParameterValue"), Category="Rendering|Material")
	ENGINE_API UTextureCollection* K2_GetTextureCollectionParameterValue(FName ParameterName);
