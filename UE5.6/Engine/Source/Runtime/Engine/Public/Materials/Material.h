...
public:
	...
	/** Store the version of the Substrate code used by this material (default, no version =-1). */
	UPROPERTY()
	int32 SubstrateVersion;

	/** Whether the material has been auto - converted at load time, and hasn't been saved. */
	UPROPERTY()
	bool bIsSubsutrateAutoConvertedAndNotSaved = false;
	...
	/** Return true if the Substrate's auto-conversion applied to this material */
	ENGINE_API bool IsSubstrateAutoConverted() const;
};
...
public:
	...
	UPROPERTY(EditAnywhere, Category=Nanite, meta = (Keywords = "Displacement"))
	 ...
	 */
	UPROPERTY(EditAnywhere, BlueprintReadOnly, meta = (DisplayName = "Roughness Tracking"), Category = Substrate)
	...
	*/
	...
	UPROPERTY(EditAnywhere, BlueprintReadOnly, Category = Usage)
	uint8 bUsedWithMaterialCache : 1;
	...
	/**
	 * Indicates that the material and its instances can be used on StaticMeshes, which is true by default on all Materials
	...
	UPROPERTY(EditAnywhere, Category = Translucency, AdvancedDisplay, meta = (DisplayName = "Allow Local Light Shadow"))
	uint8 bAllowTranslucentLocalLightShadow : 1;

	/** Set local light shadow quality received by translucent material. */
	UPROPERTY(EditAnywhere, Category = Translucency, AdvancedDisplay, meta = (DisplayName = "Local Light Shadow Quality", UIMin = "0", UIMax = "1", ClampMin = "0", ClampMax = "1", SliderExponent = 1.0))
	float TranslucentLocalLightShadowQuality = 0;

	/** Set directional light shadow quality received by translucent material. */
	UPROPERTY(EditAnywhere, Category = Translucency, AdvancedDisplay, meta = (DisplayName = "Directional Light Shadow Quality", UIMin = "0", UIMax = "1", ClampMin = "0", ClampMax = "1", SliderExponent = 1.0))
	...
	UE_DEPRECATED(5.8, "Experimental bEnableExecWire is no longer supported.")
	UPROPERTY(EditAnywhere, Category = Material)
	...
	/** 
	 * When true, translucent materials that write velocity will calculate it only based on the written depth and camera movement. 
	 ...
	 */
	UPROPERTY(EditAnywhere, Category = Translucency, meta = (DisplayName = "Velocity From Depth Only", editcondition = "bOutputTranslucentVelocity"))
	...
	*/
	...
	/** Controls whether refraction takes into account the material surface coverage, or not. */
	UPROPERTY(EditAnywhere, Category=PixelDepthOffset)
...
public:
	...
	ENGINE_API virtual bool IsCompatibleWithLumenCardSharing() const override;
	...
	ENGINE_API virtual void SyncLayersRuntimeGraphCache(FMaterialLayersFunctions* OverrideLayers) override;
#endif
	...
	*/
	...
	ENGINE_API bool SetTextureCollectionParameterValueEditorOnly(FName ParameterName, class UTextureCollection* InValue);
...
private:
	...
	bool ConvertMaterialToSubstrateMaterial(bool bAllowEmptyMaterialUpdate, bool bRequestedByUser = false);
...
#if WITH_EDITOR
	ENGINE_API bool UserRequestsConvertMaterialToSubstrateMaterial();
#endif
...
public:
	...
	ENGINE_API virtual void AppendCompileStateDebugInfo(FStringBuilderBase& OutDebugInfo) const override;
#endif // WITH_EDITOR
...
#if WITH_EDITOR
	/** Get collection of shader tags from this material's expressions. */
	ENGINE_API void GetShaderTags(TArray<FName>& OutShaderTags) const;
	/** Get user flags on the material that influence translation validation errors. */
	ENGINE_API EMaterialTranslateValidationFlags GetMaterialTranslateValidationFlags() const;
#endif
...
private:
	...
	// UPROPERTY()
	// TObjectPtr<class UMaterialExpression> ExpressionExecBegin_DEPRECATED; // maxtodo
	
	// UPROPERTY()
	// TObjectPtr<class UMaterialExpression> ExpressionExecEnd_DEPRECATED; // // maxtodo
