...
struct FEngineShowFlags
{
	...
	static bool FindShowFlagDisplayName(const FString& InName, FText& OutText)
	{
		...
		if (FindCustomShowFlagDisplayName(InName, OutText))
		{
			return true;
		}
	...
	// Features that make sense to disable together for unlit rendering (custom render passes or unlit scene captures)
	void DisableFeaturesForUnlit(bool bAllowAtmosphere = false)
	{
		SetDynamicShadows(false);		// Not supported for CRPs (shadows aren't used if lighting is disabled)
		SetLighting(false);				// Not supported for CRPs
		SetPostProcessing(false);		// Not supported for CRPs (post processing is skipped for unlit scene captures, via bResolveScene being set to false)
		SetRefraction(false);			// Not supported for CRPs (enables separate translucency, which requires post processing to run to composite it back in)

		SetLightFunctions(false);		// Needless performance cost (updates light function atlas, which is unused if lighting is disabled)
		SetUnlitRichView(false);		// Needless performance cost (expensive dynamic rendering code path, related to main view debug modes active when lighting is disabled)
		SetUnlitViewmode(false);		// Development shader feature that writes BaseColor to the emissive channel when lighting is disabled (typically only on non-shipping PC builds)

		if (!bAllowAtmosphere)
		{
			SetAtmosphere(false);		// Technically works with unlit, but costs performance, and is assumed to be undesirable by default
		}
	}
...
private:
	...
	void Init(EShowFlagInitMode InitMode)
	{
		...
		SetVisualizeLocalFogVolumes(false);
		...
		SetMaterialNormal(true);
		SetMaterialAmbientOcclusion(true);
		...
		SetVisualizeVirtualTexture(false);
		...
		SetCompositeDebugPrimitives(true);
#else
		SetCompositeDebugPrimitives(false);
#endif
...
struct TCustomShowFlag
{
	...
	bool SetEnabled(FEngineShowFlags& ShowFlags, bool bSet)
	{
#if UE_BUILD_SHIPPING
		if constexpr (ShippingValue == EShowFlagShippingValue::ForceDisabled ||
			ShippingValue == EShowFlagShippingValue::ForceEnabled)
		{
			return false;
		}
		else
#endif
		{
			...
			{
				ShowFlags.SetSingleFlag((uint32)FlagIndex + FEngineShowFlags::SF_FirstCustom, bSet);
				return true;
			}
		}
	}
