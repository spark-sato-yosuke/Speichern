...
namespace UE::Net::Private
{
	...
	public:
		...
		: StartCycle(bEnabled ? FPlatformTime::Cycles64() : uint64(0)) 
		{
		}
...
struct FNetTrace
{
	...
	static void OnTraceStarted(FTraceAuxiliary::EConnectionType TraceType, const FString& TraceDestination);

	static void OnTraceStopped(FTraceAuxiliary::EConnectionType TraceType, const FString& TraceDestination);

	DECLARE_TS_MULTICAST_DELEGATE(FOnResetPersistentNetDebugNames);
	static NETCORE_API FOnResetPersistentNetDebugNames OnResetPersistentNetDebugNames;

	DECLARE_TS_MULTICAST_DELEGATE(FOnNetTraceStarted);
	static NETCORE_API FOnNetTraceStarted OnNetTraceStarted;
	...
	NETCORE_API static void FoldTraceCollector(FNetTraceCollector* DstCollector, const FNetTraceCollector* SrcCollector, uint32 Offset, uint32 LevelOffset = 0U);
	...
	NETCORE_API static void TracePacketStatsCounter(uint32 GameInstanceId, uint32 ConnectionId, const UE::Net::FNetDebugName* CounterName, uint32 StatValue);
	...
	NETCORE_API static void TraceFrameStatsCounter(uint32 GameInstanceId, const UE::Net::FNetDebugName* CounterNameId,  uint32 StatValue);
	...
	NETCORE_API static void TraceObjectPolled(uint32 GameInstanceId, uint64 NetObjectId, uint64 Cycles, bool bIsDirty);

	NETCORE_API static void TraceObjectQuantized(uint32 GameInstanceId, uint64 NetObjectId, uint64 Cycles);

	NETCORE_API static void TraceObjectWritten(uint32 GameInstanceId, uint64 NetObjectId, uint64 Cycles);

	NETCORE_API static void TraceSubObject(uint32 GameInstanceId, uint64 ParentNetObjectId, uint64 SubObjectNetObjectId);
...
public:
	...
	inline void AddReferencedObjectId(uint64 ObjectId) { ReferencedObjectIds.Add(ObjectId); }
...
private:
	...
	// Object Ids referenced by events
	TArray<uint64, TInlineAllocator<InlinedObjectIdCount>> ReferencedObjectIds;
};
...
struct FNetTraceObjectTimer
{
	FNetTraceObjectTimer(const UE::Net::Private::FNetTimer& InTimer, uint32 InGameInstanceId, uint64 InNetObjectId)
	: Timer(InTimer)
	, NetObjectId(InNetObjectId)
	, GameInstanceId(InGameInstanceId)
	{}

	const UE::Net::Private::FNetTimer& Timer;
	uint64 NetObjectId;
	uint32 GameInstanceId;
};

struct FNetTraceObjectPollTimer : public FNetTraceObjectTimer
{
	FNetTraceObjectPollTimer(const UE::Net::Private::FNetTimer& InTimer, uint32 InGameInstanceId, uint64 InNetObjectId) 
	: FNetTraceObjectTimer(InTimer, InGameInstanceId, InNetObjectId)
	, bIsWaste(false) {}

	~FNetTraceObjectPollTimer() 
	{ 
		if (GameInstanceId != -1)
		{
			FNetTrace::TraceObjectPolled(GameInstanceId, NetObjectId, Timer.GetCyclesSinceStart(), bIsWaste);
		}
	}
...
struct FNetTraceObjectQuantizeTimer : public FNetTraceObjectTimer
{
	FNetTraceObjectQuantizeTimer(const UE::Net::Private::FNetTimer& InTimer, uint32 InGameInstanceId, uint64 InNetObjectId) 
	: FNetTraceObjectTimer(InTimer, InGameInstanceId, InNetObjectId)
	{}

	~FNetTraceObjectQuantizeTimer() 
	{ 
		if (GameInstanceId != -1)
		{
			FNetTrace::TraceObjectQuantized(GameInstanceId, NetObjectId, Timer.GetCyclesSinceStart());
		}
	}
};

struct FNetTraceObjectWriteTimer : public FNetTraceObjectTimer
{
	FNetTraceObjectWriteTimer(const UE::Net::Private::FNetTimer& InTimer, uint32 InGameInstanceId, uint64 InNetObjectId) 
	: FNetTraceObjectTimer(InTimer, InGameInstanceId, InNetObjectId)
	{}

	~FNetTraceObjectWriteTimer()
	{
		if (GameInstanceId != -1)
		{
			FNetTrace::TraceObjectWritten(GameInstanceId, NetObjectId, Timer.GetCyclesSinceStart());
		}
	}
};
	...
	, EventIndex(FNetTrace::InvalidEventIndex)
{
	if (Collector)
	{
		...
		if (InObjectId != 0)
		{
			Collector->AddReferencedObjectId(InObjectId);
		}
	}
}
...
void FNetTraceEventScope<T>::SetEventObjectId(uint64 ObjectId)
{
	if (EventIndex != FNetTrace::InvalidEventIndex)
	{
		...
		Collector->AddReferencedObjectId(ObjectId);
	}
}
...
#define UE_NET_TRACE_INTERNAL_SCOPE(Name, Stream, Collector, Verbosity) \
	auto PREPROCESSOR_JOIN(NetTraceNameFunc_, __LINE__) = []() { static const UE::Net::FNetDebugName* Name = UE::Net::CreatePersistentNetDebugName(TEXT(#Name)); return FNetTrace::TraceName(Name); }; \
...
#define UE_NET_TRACE_INTERNAL_NAMED_SCOPE(ScopeName, EventName, Stream, Collector, Verbosity) \
	auto PREPROCESSOR_JOIN(NetTraceNameFunc_, __LINE__) = []() { static const UE::Net::FNetDebugName* Name = UE::Net::CreatePersistentNetDebugName(TEXT(#EventName)); return FNetTrace::TraceName(Name); }; \
...
#define UE_NET_TRACE_INTERNAL_NAMED_DYNAMIC_NAME_SCOPE(ScopeName, EventName, Stream, Collector, Verbosity) \
	auto PREPROCESSOR_JOIN(NetTraceDynamicNameFunc_, __LINE__) = [&]() { UE::Net::FNetDebugNameId NameId = FNetTrace::TraceName(EventName); return NameId; }; \
...
#define UE_NET_TRACE_INTERNAL(Name, Collector, StartPos, EndPos, Verbosity) \
	do { if (FNetTrace::GetCollectorAtVerbosity(Collector, Verbosity)) { \
			static const UE::Net::FNetDebugName* PREPROCESSOR_JOIN(NetTraceNameId_, __LINE__) = UE::Net::CreatePersistentNetDebugName(TEXT(#Name)); \
			FNetTrace::TracePacketContentEvent(*Collector, FNetTrace::TraceName(PREPROCESSOR_JOIN(NetTraceNameId_, __LINE__)), StartPos, EndPos, Verbosity); \
	} } while (0)
...
#define UE_NET_TRACE_INTERNAL_PACKET_STATSCOUNTER(GameInstanceId, ConnectionId, Name, StatValue, Verbosity) \
	do { if (FNetTrace::GetNetTraceVerbosityEnabled(Verbosity)) { \
			static const UE::Net::FNetDebugName* PREPROCESSOR_JOIN(NetTraceNameId_, __LINE__) = UE::Net::CreatePersistentNetDebugName(TEXT(#Name)); \
			FNetTrace::TracePacketStatsCounter(GameInstanceId, ConnectionId, FNetTrace::TraceName(PREPROCESSOR_JOIN(NetTraceNameId_, __LINE__)), StatValue); \
	} } while (0)

#define UE_NET_TRACE_INTERNAL_FRAME_STATSCOUNTER(GameInstanceId, Name, StatValue, Verbosity) \
	do { if (FNetTrace::GetNetTraceVerbosityEnabled(Verbosity)) { \
			static const UE::Net::FNetDebugName* PREPROCESSOR_JOIN(NetTraceNameId_, __LINE__) = UE::Net::CreatePersistentNetDebugName(TEXT(#Name)); \
			FNetTrace::TraceFrameStatsCounter(GameInstanceId, FNetTrace::TraceName(PREPROCESSOR_JOIN(NetTraceNameId_, __LINE__)), StatValue); \
	} } while (0)
...
#define UE_NET_TRACE_INTERNAL_UPDATE_INSTANCE(...) FNetTrace::TraceInstanceUpdated(__VA_ARGS__)
	...
	#define UE_NET_TRACE_INTERNAL_POLL_OBJECT_SCOPE(Handle, Timer) FNetTraceObjectPollTimer __NetTracePollTimer(Timer, (GNetTraceRuntimeVerbosity >= ENetTraceVerbosity::VeryVerbose && Handle.IsValid()) ? Handle.GetReplicationSystemId() : -1, Handle.GetId());
#define UE_NET_TRACE_INTERNAL_POLL_OBJECT_IS_WASTE __NetTracePollTimer.bIsWaste = true;
#define UE_NET_TRACE_INTERNAL_QUANTIZE_OBJECT_SCOPE(Handle, Timer) FNetTraceObjectQuantizeTimer PREPROCESSOR_JOIN(__NetTraceQuantizeTimer, __LINE__)(Timer, (GNetTraceRuntimeVerbosity >= ENetTraceVerbosity::VeryVerbose && Handle.IsValid()) ? Handle.GetReplicationSystemId() : -1, Handle.GetId());
#define UE_NET_TRACE_INTERNAL_WRITE_OBJECT_SCOPE(Handle, Timer) FNetTraceObjectWriteTimer PREPROCESSOR_JOIN(__NetTraceWriteTimer, __LINE__)(Timer, (GNetTraceRuntimeVerbosity >= ENetTraceVerbosity::VeryVerbose && Handle.IsValid()) ? Handle.GetReplicationSystemId() : -1, Handle.GetId());	
	
#define UE_NET_TRACE_INTERNAL_SUBOBJECT(ParentHandle, SubObjectHandle) FNetTrace::TraceSubObject(ParentHandle.GetReplicationSystemId(), GetObjectIdForNetTrace(ParentHandle), GetObjectIdForNetTrace(SubObjectHandle))
