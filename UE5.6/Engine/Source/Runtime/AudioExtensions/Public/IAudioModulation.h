...
namespace Audio
{
	...
	AUDIOEXTENSIONS_API const FModulationParameter* GetModulationParameterPtr(FName InName);
	AUDIOEXTENSIONS_API const FModulationParameter& GetDefaultModulationParameter();
	
	UE_DEPRECATED(5.6, "Please use the version of GetModulationParameter which returns a pointer instead of a reference, and check that it is valid in your code.")
