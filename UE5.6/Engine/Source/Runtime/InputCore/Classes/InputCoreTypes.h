...
struct EKeys
{
	...
	// Map the two provided keys to the X and Y axes of the paired key
	...
	{
		explicit FPairedKeyDetails(TSharedPtr<FKeyDetails> XDetails, TSharedPtr<FKeyDetails> YDetails)
			: XKeyDetails(XDetails)
			, YKeyDetails(YDetails)
		{ }
...
private:
	...
	/**
	 * A map of Paired Key's (keys which are made up of a "pair" of other FKeys, such as Mouse2D
	 * being made up of MouseX and MouseY) to the keys which make up their pair.
	 */
	static INPUTCORE_API TMap<FKey, FPairedKeyDetails> PairedInputKeysLookup;
