...
{
	if (const FString& DeprecatedEngineVersion = Plugin.GetDeprecatedEngineVersion(); !DeprecatedEngineVersion.IsEmpty())
	{
		UE_LOG(LogPluginManager, Display, TEXT("The Plugin %s has been marked deprecated for %s and will be removed soon. Please update your dependencies."), *Plugin.GetName(), *DeprecatedEngineVersion);
	}
}
...
namespace DiscoveredPluginMapUtils
{
	...
	static TSharedRef<FPlugin>& FindPluginInMap_Checked(FDiscoveredPluginMap& InMap, const FDiscoveredPluginMap::KeyType& Name)
	{
		...
		checkf(PluginPtr, TEXT("Cannot find plugin %s"), *Name);
		return *PluginPtr;
	}
	...
	if(PluginsToConfigure.Num() > 0)
	{
		...
		// Check which plugins have been enabled or excluded via the command line
		{
			...
			auto ParsePluginsList = [this](const TCHAR* InListKey) -> TArray<FString>
			{
				...
				TArray<FString> PluginListFilenames;
				PluginsList.RemoveAll([&PluginListFilenames](const FString& ExceptPluginName)
				{
					if (ExceptPluginName.EndsWith(TEXT(".txt")))
					{
						PluginListFilenames.Add(ExceptPluginName);
						return true;
					}
					return false;
				});

				for (const FString& PluginListFilename : PluginListFilenames) //-V1078
				{
					FFileHelper::LoadFileToStringWithLineVisitor(*PluginListFilename, [&PluginsList](FStringView Line)
					{
						PluginsList.Add(FString(Line));
					});
				}
...

		{
			...
			// Merged modular builds act as if they were monolithic as far as plugin discovery is concerned.
			...
			for (TPair<FString, FPlugin*>& Pair : EnabledPlugins)
			{
				...
				// We do not run this process while using merged modular build, we'll keep acting as if the build was monolithic.
		...
		// Initially this will be correct. PluginsEnabledForTarget will be updated with new mounts and this will stop including compiled plugins.
		PluginsEnabledAtStartup.Append(PluginsEnabledForTarget);
		...
		// even if we had plugins in the Config already, we need to process Mod plugins
		{
			...
			PluginNames.Reserve(PluginsArray.Num());
			...
			for (TSharedRef<FPlugin> PluginPtr : PluginsArray)
			{
				...
				UE::PluginManager::Private::CheckForDeprecatedPlugin(Plugin);
			...
			// gang up dynamic layer adds as much as possible to reduce the amount of "CalcDiff" calls etc 
			FConfigCacheIni::AddMultiplePluginsToAllBranches(PluginNames);

			// walk over each plugin, modify existing ini's, mount the content, etc
			ParallelFor(PluginsArray.Num(), [&PluginsArray, &ConfigCS, &PluginPakCS, &ConfigFilesPluginsCannotOverride, &ChangeTracker, this](int32 Index)
			{
	...
	FConfigurePluginResultInfo& OutResultInfo)
{
	...
	for (int32 Idx = 0; Idx < NewPluginQueue.Num(); Idx++)
	{
...
#if !UE_BUILD_SHIPPING
		if (Reference.bOptional && !AllowedOptionalDependencies.IsEmpty() && !AllowedOptionalDependencies.Contains(Reference.Name))
		{
			UE_LOG(LogPluginManager, Verbose, TEXT("Ignored optional reference to '%s' plugin; plugin was not built by target."), *Reference.Name);
			continue;
		}
#endif
		...
		for (const FPluginReferenceDescriptor& NextReference : Plugin.Descriptor.Plugins)
		{
			...
			{
				UE_LOG(LogPluginManager, Verbose, TEXT("Ignored optional reference to '%s' plugin from '%s' plugin; plugin was not built by target."), *NextReference.Name, *Plugin.GetName());
				continue;
			}
#endif
...
bool FPluginManager::LoadModulesForEnabledPlugins( const ELoadingPhase::Type LoadingPhase )
{
	...
	else
	{
		...
		// This is avoided while using merged modular build, as we don't want modules to be loaded except when we're certain to need them.
...
TArray<TSharedRef<IPlugin>> FPluginManager::GetEnabledPluginsWithVerse() const
{
	...
	for (const FDiscoveredPluginMap::ElementType& PluginPair : AllPlugins)
	{
		const TSharedRef<FPlugin>& PluginRef = DiscoveredPluginMapUtils::ResolvePluginFromMapVal(PluginPair.Value);
		const FPlugin& Plugin = *PluginRef;
		if (Plugin.IsEnabled() && (Plugin.CanContainVerse() || (Plugin.CanContainContent() && Plugin.GetDescriptor().bEnableVerseAssetReflection)) && Plugin.IsMounted())
		{
			Plugins.Add(PluginRef);
		}
	}
...
{
	ensureMsgf(!PluginsEnabledAtStartup.IsEmpty(), TEXT("ConfigureEnabledPlugins hasn't been called yet."));
	return PluginsEnabledAtStartup;
}
...
bool FPluginManager::MountExplicitlyLoadedPluginLocalizationData(const FString& PluginName, TUniqueFunction<void(bool bLoadedLocalization, const FString& PluginName)>&& Notification)
{
	TSharedPtr<FPlugin> Plugin = FindPluginInstance(PluginName);
	...
	if (Notification)
	{
		FTextLocalizationManager::Get().HandleLocalizationTargetsMounted(AdditionalLocResPaths, [PluginName, Notification = MoveTemp(Notification)]()
		{
			Notification(true, PluginName);
		});
	}
	else
	{
...
bool FPluginManager::UnmountExplicitlyLoadedPluginLocalizationData(const FString& PluginName, TUniqueFunction<void(bool bUnloadedLocalization, const FString& PluginName)>&& Notification)
{
	TSharedPtr<FPlugin> Plugin = FindPluginInstance(PluginName);
	...
	if (Notification)
	{
		FTextLocalizationManager::Get().HandleLocalizationTargetsUnmounted(AdditionalLocResPaths, [PluginName, Notification = MoveTemp(Notification)]()
		{
			Notification(true, PluginName);
		});
	}
...
void FPluginManager::EnumerateVersePathMountPoints(const UE::Core::FVersePath& VersePath, TFunctionRef<bool(FStringView /*MountedAssetPath*/, FStringView /*LeafPath*/)> Callback)
{
	...
	const FStringView VersePathView = VersePath.AsStringView();
	...
	for (const TPair<FString, TArray<TSharedRef<FPlugin>>>& NamePluginPair : AllPlugins)
	{
		...
		if (Plugin->IsMounted() && !Plugin->GetVersePath().IsEmpty())
		{
			UE::Core::FVersePath PluginVersePath;
			if (UE::Core::FVersePath::TryMake(PluginVersePath, Plugin->GetVersePath()))
			{
				FStringView LeafPath;
				if (PluginVersePath.IsBaseOf(VersePath, &LeafPath))
				{
					if (!Callback(Plugin->GetMountedAssetPath(), LeafPath))
					{
						break;
					}
				}
