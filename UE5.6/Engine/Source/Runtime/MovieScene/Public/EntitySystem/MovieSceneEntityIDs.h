 ...
 */
static constexpr int32 MaximumNumComponentsSupported = 384;
...
struct FComponentTypeIDFilter
{
	...
	FComponentTypeIDFilter(FComponentTypeID InComponentType, bool bInPassFlag)
		: Value(InComponentType ? uint16(InComponentType.BitIndex()) : INVALID)
		, PassFlag(bInPassFlag)
	{
	}
	...
	FComponentTypeID GetComponentType() const
	{
		...
		return FComponentTypeID::FromBitIndex(Value);
	}

private:
	static const uint16 INVALID = uint16(-1) >> 1u;
