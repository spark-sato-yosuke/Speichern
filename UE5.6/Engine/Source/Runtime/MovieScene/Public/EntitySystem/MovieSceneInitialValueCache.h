...
public:
	 ...
	 *
	 * @param PropertyComponents    The property component definitions for the storage including its traits
	...
	{
		return GetStorage<PropertyTraits>(PropertyComponents.InitialValueType);
	}
	...
	{
		return FindStorage<PropertyTraits>(PropertyComponents.InitialValue);
	}
...
struct TPropertyValueStorage : IPropertyValueStorage
{
	...
	{
		FKeyType Key{ FObjectKey(BoundObject), FPropertyKey(TInPlaceType<FName>(), PropertyPath)};
