...
UMovieSceneSequencePlayer::UMovieSceneSequencePlayer(const FObjectInitializer& Init)
	...
	, bIsTimeControllerReadyToPlay(true)
...
void UMovieSceneSequencePlayer::PlayInternal()
{
	...
	if (!IsPlaying() && Sequence && CanPlay() && bIsTimeControllerReadyToPlay)
	{
		...
		{
			bIsTimeControllerReadyToPlay = false;
		}
	}
}
...
void UMovieSceneSequencePlayer::StopInternal(FFrameTime TimeToResetTo)
{
	...
	// so bIsTimeControllerReadyToPlay may be false. Here we reset that back to its
...
void UMovieSceneSequencePlayer::Update(const float DeltaSeconds)
{
	...
	if (!bIsTimeControllerReadyToPlay && TimeController->IsReadyToPlay())
	{
		StartTimeControllerAndBroadcastPlayState();
		bIsTimeControllerReadyToPlay = true;
	}

	if (bIsTimeControllerReadyToPlay && IsPlaying())
	{
