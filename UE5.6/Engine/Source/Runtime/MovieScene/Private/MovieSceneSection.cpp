...
FOnMovieSceneSectionAddedToTrack UMovieSceneSection::OnAddedToTrackEvent;
FOnMovieSceneSectionChanged UMovieSceneSection::OnRemovedFromTrackEvent;
#endif
...
FOnMovieSceneSectionAddedToTrack& UMovieSceneSection::GetOnSectionAddedToTrack()
{
	return OnAddedToTrackEvent;
}

FOnMovieSceneSectionChanged& UMovieSceneSection::GetOnSectionRemovedFromTrack()
{
	return OnRemovedFromTrackEvent;
}
...
#if WITH_EDITORONLY_DATA
	OnAddedToTrackEvent.Broadcast(Track, this);
#endif
}
...
#if WITH_EDITORONLY_DATA
	OnRemovedFromTrackEvent.Broadcast(this);
#endif
}
...
void UMovieSceneSection::OnDecorationAdded(UObject* NewDecoration)
{
	...
	IMovieSceneLifetimeDecoration* Lifetime = Cast<IMovieSceneLifetimeDecoration>(NewDecoration);

	if (MovieScene && Lifetime)
	{
		Lifetime->OnReconstruct(MovieScene);
	}
	if (IMovieSceneSectionDecoration* DecorationInterface = Cast<IMovieSceneSectionDecoration>(NewDecoration))
	{
		DecorationInterface->OnDecorationAdded(this);
	}

	EventHandlers.Trigger(&UE::MovieScene::ISectionEventHandler::OnDecorationAdded, NewDecoration);
}

void UMovieSceneSection::OnDecorationRemoved(UObject* Decoration)
{
	if (IMovieSceneSectionDecoration* DecorationInterface = Cast<IMovieSceneSectionDecoration>(Decoration))
	{
		DecorationInterface->OnDecorationRemoved();
	}

	UMovieScene* MovieScene = GetTypedOuter<UMovieScene>();
	IMovieSceneLifetimeDecoration* Lifetime = Cast<IMovieSceneLifetimeDecoration>(Decoration);

	if (MovieScene && Lifetime)
	{
		Lifetime->OnDestroy(MovieScene);
	}
	EventHandlers.Trigger(&UE::MovieScene::ISectionEventHandler::OnDecorationRemoved, Decoration);
}
...
void UMovieSceneSection::PostRename(UObject* OldOuter, const FName OldName)
{
	...
	UMovieScene* NewMovieScene = GetTypedOuter<UMovieScene>();
	if (NewMovieScene && NewMovieScene != OldOuter->GetTypedOuter<UMovieScene>())
	{
		for (UObject* Decoration : GetDecorations())
		{
			// If this is being renamed into a movie scene structure, call reconstruct on its decorations
			if (IMovieSceneLifetimeDecoration* DecorationInterface = Cast<IMovieSceneLifetimeDecoration>(Decoration))
			{
				DecorationInterface->OnReconstruct(NewMovieScene);
			}
		}
	}
...
void UMovieSceneSection::PostPaste()
{
	...
	for (UObject* Decoration : GetDecorations())
	{
		Decoration->ClearFlags(RF_Transient);
	}
}
