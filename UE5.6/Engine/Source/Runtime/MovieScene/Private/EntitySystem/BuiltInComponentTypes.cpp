...
FBuiltInComponentTypes::FBuiltInComponentTypes()
{
	...
	ComponentRegistry->NewComponentType(&InitialValueIndex,     TEXT("Initial Value Index"), EComponentTypeFlags::MigrateToOutput);
	...
	Tags.AdditiveAnimation       = ComponentRegistry->NewTag(TEXT("Is Additive Animation"));
	...
	Tags.HasAssignedInitialValue = ComponentRegistry->NewTag(TEXT("Has Assigned Initial Value"), EComponentTypeFlags::MigrateToOutput);
	...
	Tags.AlwaysCacheInitialValue = ComponentRegistry->NewTag(TEXT("Always Cache Initial Value"), EComponentTypeFlags::CopyToOutput);
	Tags.OldStyleSpawnable       = ComponentRegistry->NewTag(TEXT("Old Style Spawnable"));
	Tags.ExternalBlending        = ComponentRegistry->NewTag(TEXT("External Blending"));
	...
	ComponentRegistry->Factories.DefineChildComponent(Tags.AdditiveAnimation, Tags.AdditiveAnimation);
