...
private:
    ...
    TSRef<CExpressionBase> DesugarDefinition(const Vst::Definition& DefinitionVst)
    {
        ...
        if (!Name.IsNull() && !ValueDomain)
        {
            ...
            return AddMapping(DefinitionVst, TSRef<CExprMakeNamed>::New(Name, Move(Value)));
        }
    ...
    TSRef<CExpressionBase> DesugarInterpolatedString(const Vst::InterpolatedString& InterpolatedStringNode)
    {
        ...
        const CSymbol ConcatenateSymbol = _Symbols.AddChecked("Concatenate");
        ...
        {
            TSRef<CExpressionBase> ConcatenateArgs = TSRef<CExprMakeTuple>::New(Move(DesugaredChildren));
            TSRef<CExprInvocation> ConcatenateInvocation = TSRef<CExprInvocation>::New(
                CExprInvocation::EBracketingStyle::Parentheses,
                TSRef<CExprIdentifierUnresolved>::New(ConcatenateSymbol),
                ConcatenateArgs);
            return AddMapping(InterpolatedStringNode, Move(ConcatenateInvocation));
        }
        else
        {
