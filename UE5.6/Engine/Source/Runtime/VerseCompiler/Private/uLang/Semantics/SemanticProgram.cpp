...
const CTypeBase* CSemanticProgram::InstantiateTaskType(const CTypeBase* TypeArgument)
{
    ...
    bool bConstrained = SemanticTypeUtils::Constrain(&GetOrCreateTypeType(&_anyType, TypeArgument, ERequiresCastable::No), &InstTaskType->GetParamsType());
...
CTypeType& CSemanticProgram::GetOrCreateTypeType(const CTypeBase* NegativeType, const CTypeBase* PositiveType, const ERequiresCastable RequiresCastable)
{
    ...
    CTypeType* TypeType = _TypeTypes.Find({NegativeType, PositiveType, RequiresCastable });
    if (!TypeType)
    {
        TURef<CTypeType> NewTypeType = TURef<CTypeType>::New(*this, NegativeType, PositiveType, RequiresCastable);
...
CTypeType& CSemanticProgram::GetOrCreateCastableSubtypeType(const CTypeBase* NegativeType)
{
    return GetOrCreateTypeType(&_falseType, NegativeType, ERequiresCastable::Yes);
}
    ...
    const CTypeType* Type)
{
    ...
    TSRef<CTypeVariable> TypeVariable = Function->CreateTypeVariable(
        TypeName,
        Type);
    TypeVariable->_NegativeType = Type;
    ...
    NegativeTypeVariable->_NegativeType = &NegativeTypeVariableType;
...
void CSemanticProgram::PopulateCoreAPI()
{
    ...
    _typeType = &GetOrCreateTypeType(&_falseType, &_anyType, ERequiresCastable::No);
    ...
    auto ComparableOp = [this, LhsName, RhsName, ConvergesDecidesEffectSet, &VerseModuleBuiltInPart](CSymbol FunctionName)
    {
        ...
        TSRef<CTypeVariable> Type = NewFunction->CreateTypeVariable(
            ...
            &GetOrCreateTypeType(&_falseType, &_comparableType, ERequiresCastable::No));
        Type->_NegativeType = &GetOrCreateTypeType(&_falseType, &_comparableType, ERequiresCastable::No);
        const CFunctionType& FunctionType = GetOrCreateFunctionType(
...

    {
        ...
        TSRef<CTypeVariable> ElementType = _ArrayAddOp->CreateTypeVariable(
            _Symbols->AddChecked("t"),
            _typeType);
        ElementType->_NegativeType = _typeType;
    ...
    CTypeType& ComparableSubtypeType = GetOrCreateTypeType(&_falseType, &_comparableType, ERequiresCastable::No);
...

    {
        ...
        const CFunctionType& FunctionType = GetOrCreateFunctionType(
            ...
            GetOrCreateTypeType(&MapType, &NegativeMapType, ERequiresCastable::No),
            ConvergesEffectSet,
    ...
    //
    {
        const CTypeType& PersistableSubtypeType = GetOrCreateTypeType(&_falseType, &_persistableType, ERequiresCastable::No);
        ...
        ValType->_NegativeType = &PersistableSubtypeType;
        const CFunctionType& FunctionType = GetOrCreateFunctionType(
            *ValType,
            *ValType,
