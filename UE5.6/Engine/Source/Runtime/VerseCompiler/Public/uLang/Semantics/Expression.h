...
public:
    ...
    virtual CUTF8String GetErrorDesc() const override { return _bRequiresCastable ? "castable_subtype(..)" : "subtype(..)"; }
    virtual bool operator==(const CExpressionBase& Other) const override { return false; }
    
    UE_API const CTypeType& GetSubtypeType() const;

    bool _bRequiresCastable = false;
};
...
{
public:
    explicit CExprMakeNamed(CSymbol Name)
        : _Name(Name)
    {
    }

    CExprMakeNamed(CSymbol Name, TSPtr<CExpressionBase>&& Argument)
        : _Name(Name)
