...
#undef VISIT_VSTNODE
    ...
    struct Identifier : public CAtom
    {
        ...
        Identifier(const CUTF8StringView& InName, const SLocus& Whence) : CAtom(InName, StaticType, Whence) {}
    ...
    struct Interpolant : public Node
    {
        static const Vst::NodeType StaticType = NodeType::Interpolant;

        Interpolant(const SLocus& Whence)
            : Node(StaticType, Whence)
        {}
    };

    struct InterpolatedString : public Node
    {
        static const Vst::NodeType StaticType = NodeType::InterpolatedString;

        InterpolatedString(const SLocus& Whence)
            : Node(StaticType, Whence)
        {}
    };
