...
void FCEFBrowserHandler::OnAcceleratedPaint(CefRefPtr<CefBrowser> Browser,
	...
	void* SharedHandle)
#else
	const CefAcceleratedPaintInfo& SharedHandle)
#endif
{
	TSharedPtr<FCEFWebBrowserWindow> BrowserWindow = BrowserWindowPtr.Pin();
...
CefResourceRequestHandler::ReturnValue FCEFBrowserHandler::OnBeforeResourceLoad(
	CefRefPtr<CefBrowser> Browser,
	CefRefPtr<CefFrame> Frame,
...
void FCEFBrowserHandler::OnRenderProcessTerminated(CefRefPtr<CefBrowser> Browser,
#if CEF_VERSION_MAJOR < 128
...
{
	if (IWebBrowserModule::Get().GetSingleton()->IsShuttingDown())
	{
		UE_LOG(LogWebBrowser, Warning, TEXT("Received a process message while the module is shutting down, ignoring."));
		return false;
	}
	...
	CefRefPtr<CefFileDialogCallback> Callback)
{
	...
	if (BrowserWindow.IsValid())
	{
		Retval = BrowserWindow->OnFileDialog(Mode, Title, DefaultFilePath, AcceptFilters,
#if CEF_VERSION_MAJOR < 128
