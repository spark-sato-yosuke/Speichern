...
void SAndroidWebBrowserWidget::Construct(const FArguments& Args)
{
	...
	JavaWebBrowser = MakeShared<FJavaAndroidWebBrowser, ESPMode::ThreadSafe>(false, FAndroidMisc::ShouldUseVulkan(), viewportSize.X, viewportSize.Y,
		reinterpret_cast<jlong>(this), !(UE_BUILD_SHIPPING || UE_BUILD_TEST), Args._UseTransparency, bEnableDomStorage, bShouldUseBitmapRender, Args._UserAgentApplication, bEnableFloatingCloseButton);
...
void SAndroidWebBrowserWidget::ShowFloatingCloseButton(bool bShow, bool bDraggable)
{
	JavaWebBrowser->ShowFloatingCloseButton(bShow, bDraggable);
}
...
void SAndroidWebBrowserWidget::FloatingCloseButtonPressed()
{
	if (WebBrowserWindowPtr.IsValid())
	{
		TSharedPtr<FAndroidWebBrowserWindow> BrowserWindow = WebBrowserWindowPtr.Pin();
		if (BrowserWindow.IsValid())
		{
			FGraphEventRef OnRequestClose = FFunctionGraphTask::CreateAndDispatchWhenReady([&]()
				{
					BrowserWindow->FloatingCloseButtonPressed();
				}, TStatId(), NULL, ENamedThreads::GameThread);
			FTaskGraphInterface::Get().WaitUntilTaskCompletes(OnRequestClose);
		}
	}
}
...
JNI_METHOD void Java_com_epicgames_unreal_WebViewControl_FloatingCloseButtonPressed(JNIEnv* JEnv, jobject Client)
{
	TSharedPtr<SAndroidWebBrowserWidget> Widget = SAndroidWebBrowserWidget::GetWidgetPtr(JEnv, Client);
	if (Widget.IsValid())
	{
		Widget->FloatingCloseButtonPressed();
	}
}
