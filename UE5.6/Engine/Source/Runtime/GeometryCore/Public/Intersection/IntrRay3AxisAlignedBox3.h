...
public:
	 ...
	 * @return true if ray intersects box
	 */
	inline static bool FindIntersection(const TRay<RealType>& Ray, const TAxisAlignedBox3<RealType>& Box, RealType& RayParamOut)
	{
		return FindIntersection(Ray.Origin, Ray.Direction, Box, RayParamOut);
	}
	/**
	 * Find intersection of ray with AABB and returns ray T-value of intersection point (or TNumericLimits::Max() on miss)
	 * @param Origin query ray origin
	 * @param Direction query ray direction
	 * @param Box query box
	 * @param RayParamOut ray intersect T-value, or TNumericLimits::Max()
	 * @return true if ray intersects box
	 */
	static bool FindIntersection(const TVector<RealType>& Origin, const TVector<RealType>& Direction, const TAxisAlignedBox3<RealType>& Box, RealType& RayParamOut)
	{
		...
		DoClipping(RayParam0, RayParam1, Origin, Direction, Box,
