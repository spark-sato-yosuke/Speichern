...
void FDynamicMeshEditor::DisconnectTriangles(const TArray<int>& Triangles, bool bPreventBowties)
{
	...
	if (bPreventBowties)
	{
		// reusable arrays for SplitBowtiesInternal
		TArray<int> BowtieTriangles, BowtieContiguousGroupLengths;
		TArray<bool> BowtieGroupIsLoop;
		for (int VID : OldVertsThatSplit)
		{
			SplitBowtiesInternal(VID, BowtieTriangles, BowtieContiguousGroupLengths, BowtieGroupIsLoop, nullptr);
		}
...
bool FDynamicMeshEditor::DisconnectTrianglesAlongEdges(const TSet<int32>& EdgeIDs, TArray<int32>* AddedVertexIDs)
{
	...
	// reusable arrays for bowtie and edge disconnecting logic below
	TArray<int> BowtieTriangles, BowtieContiguousGroupLengths;
	TArray<bool> BowtieGroupIsLoop;
	TArray<int32> VertTIDsToUseForSplit[2];
	for (int32 EID : EdgeIDs)
	{
		...
		if (Mesh->IsBoundaryEdge(EID))
		{
			...
			for (int i = 0; i < 2; ++i)
			{
				SplitBowtiesInternal(EdgeVIDs[i], BowtieTriangles, BowtieContiguousGroupLengths, BowtieGroupIsLoop, AddedVertexIDs);
			}
			continue;
		}
		...
		if (!VertNeedsSplitting[0] && !VertNeedsSplitting[1])
		{
			...
			{
				SplitBowtiesInternal(PotentialBowtieVID, BowtieTriangles, BowtieContiguousGroupLengths, BowtieGroupIsLoop, AddedVertexIDs);
			}
		}
		else
		{
...
{
	TArray<int> Triangles, ContiguousGroupLengths;
	...
	for (int VertexID = 0, OriginalMaxID = Mesh->MaxVertexID(); VertexID < OriginalMaxID; VertexID++)
	{
		...
		SplitBowtiesInternal(VertexID, Triangles, ContiguousGroupLengths, GroupIsLoop, &ResultOut.NewVertices);
...
void FDynamicMeshEditor::SplitBowtiesInternal(int VertexID, TArray<int>& TrianglesOut, TArray<int>& ContiguousGroupLengths, TArray<bool>& GroupIsLoop, TArray<int32>* NewVertices)
{
	DynamicMeshInfo::FVertexSplitInfo SplitInfo;
...
void FDynamicMeshEditor::SplitBowtiesAtTriangles(const TArray<int32>& TriangleIDs, FDynamicMeshEditResult& ResultOut)
{
	...
	for (int32 Tid : TriangleIDs)
	{
		...
		for (int i = 0; i < 3; ++i)
		{
			...
			{
				SplitBowtiesInternal(Vid, Triangles, ContiguousGroupLengths, GroupIsLoop, &ResultOut.NewVertices);
			}
		}
	}
}
...
{
	// TODO: add a 'fast path' w/ the new Mesh->AppendWithOffsets, and build IndexMapsOut from the offsets
	// (this could always be used w/ identical results if both meshes are compact, and perhaps via a flag in the non-compact case)
	...
	if (AppendMesh->HasAttributes() && Mesh->HasAttributes())
	{
		...
		if (Mesh->Attributes()->NumSculptLayers() > 0 && AppendMesh->Attributes()->NumSculptLayers() > 0)
		{
			Mesh->Attributes()->GetSculptLayers()->CopyThroughMapping(*AppendMesh->Attributes()->GetSculptLayers(), IndexMapsOut);
		}
	}
...
static void AppendVertexAttributes(const FDynamicMesh3* FromMesh, FDynamicMesh3* ToMesh, FMeshIndexMappings& IndexMaps)
{
	...
	if (FromMesh->Attributes()->NumSculptLayers() > 0 && ToMesh->Attributes()->NumSculptLayers() > 0)
	{
		ToMesh->Attributes()->GetSculptLayers()->CopyThroughMapping(*FromMesh->Attributes()->GetSculptLayers(), IndexMaps);
	}
}
...
bool FDynamicMeshEditor::SplitMesh(const FDynamicMesh3* SourceMesh, TArray<FDynamicMesh3>& SplitMeshes, TFunctionRef<int(int)> TriIDToMeshID, int DeleteMeshID, TArray<int32>* MeshIDsOut, bool bSortByID)
{
	using namespace UE::DynamicMeshEditorInternals;
	...
	if (bSortByID)
	{
		TArray<int32> SortedMeshIDs;
		MeshIDToIndex.GenerateKeyArray(SortedMeshIDs);
		SortedMeshIDs.Sort();
		for (int32 Idx = 0; Idx < SortedMeshIDs.Num(); ++Idx)
		{
			MeshIDToIndex[SortedMeshIDs[Idx]] = Idx;
		}
	}

	if (MeshIDsOut)
	{
		MeshIDsOut->SetNum(NumMeshes);
		for (TPair<int32, int32> MeshIDIndexPair : MeshIDToIndex)
		{
			(*MeshIDsOut)[MeshIDIndexPair.Value] = MeshIDIndexPair.Key;
		}
	}
