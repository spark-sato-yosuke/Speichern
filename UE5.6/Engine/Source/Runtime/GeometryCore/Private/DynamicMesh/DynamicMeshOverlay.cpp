...
template<typename RealType, int ElementSize>
bool TDynamicMeshOverlay<RealType, ElementSize>::IsSeamIntersectionVertex(int32 VertexID) const
{
	int32 SeamCount = 0;
	for (int32 EdgeID : ParentMesh->VtxEdgesItr(VertexID))
	{
		SeamCount += int32(IsSeamEdge(EdgeID));
	}
...
int TDynamicMeshOverlay<RealType, ElementSize>::GetElementIDAtVertex(int TriangleID, int VertexID) const
{
	...
	// Check first element ID to see if triangle is unset (Note: triangles must be fully set or fully unset, so checking first element should be enough)
...
void TDynamicMeshOverlay<RealType, ElementSize>::OnFlipEdge(const FDynamicMesh3::FEdgeFlipInfo& FlipInfo)
{
	...
	int32 NumSet = (int32)bT0Set + (int32)bT1Set;
	if (NumSet == 0)
	{
	...
	{
		ensure(false); // flipping across a set/unset boundary is not allowed?
	...
	bool bHasSharedUVEdge = (Triangle0[idx_base_a1] == Triangle1[idx_base_a2]) && (Triangle0[idx_base_b1] == Triangle1[idx_base_b2]);
	...
	{
		// ideally we wouldn't flip a seam edge, but if we have done so, may need to clean up a now-unreferenced element
		// (note it's too late to prevent the flip here)
		...
		{
			ElementsRefCounts.Decrement(A2);
			ParentVertices[A2] = FDynamicMesh3::InvalidID;
		}
		...
		{
			ElementsRefCounts.Decrement(B2);
			ParentVertices[B2] = FDynamicMesh3::InvalidID;
		}
	}
}
