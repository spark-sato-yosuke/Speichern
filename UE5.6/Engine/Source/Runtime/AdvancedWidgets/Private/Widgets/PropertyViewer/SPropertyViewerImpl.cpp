...
void FTreeNode::BuildChildNodesRecursive(IFieldIterator& FieldIterator, IFieldExpander& FieldExpander, bool bSortChildNode, int32 RecursiveCount, TArray<FNodeReference>& OutTickReference)
{
	...
	const UStruct* CurrentFieldOwner = nullptr;
	FName CurrentFieldName = NAME_None;
	if (bIsStructProperty || bIsObjectProperty)
	{
		check(!PropertyName.IsNone());
		CurrentFieldName = PropertyName;
		CurrentFieldOwner = FieldOwner.Get();
	...
	else if (const TSharedPtr<FContainer> ContainerPin = Container.Pin())
	{
		...
		CurrentFieldName = FName(ContainerPin->GetDisplayName().Get(FText::GetEmpty()).ToString());
	}

	if (ChildStructType)
	{
		for (const FFieldVariant& FieldIt : FieldIterator.GetFields(ChildStructType, CurrentFieldName, CurrentFieldOwner))
		{
