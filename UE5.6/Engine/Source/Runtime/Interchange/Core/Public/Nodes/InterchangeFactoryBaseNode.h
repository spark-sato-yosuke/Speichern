...
enum class EReimportStrategyFlags : uint8
{
	...
	ApplyEditorChangedProperties UMETA(DisplayName = "Preserve Editor Changed Properties")
};
...
public:
	...
	static INTERCHANGECORE_API FString BuildTranslatedNodeUid(const FString& InFactoryNodeUid);
	...
	/**
	 * Adds apply and fill delegates for attributes again that were using custom delegates.
	 */
	...
	/**
	 * Removes all custom attribute delegates that are currently added.
	 */
	 ...
	 */
	UFUNCTION(BlueprintCallable, Category = "Interchange | Node")
	virtual bool IsRuntimeImportAllowed() const
	{
		return true;
	}
...
protected:
	 ...
	 * Adds apply and fill delegates for attributes again that were using custom delegates.
	 */
	INTERCHANGECORE_API virtual void OnRestoreAllCustomAttributeDelegates();
...
protected:
	...
	UPROPERTY(Transient)
	TSet<FString> AttributesAppliedThroughDelegatesKeySet;
