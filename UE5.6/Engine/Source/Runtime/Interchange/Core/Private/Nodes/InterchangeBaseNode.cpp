...
namespace UE
{
	namespace Interchange
	{
		...
		const FAttributeKey& FBaseNodeStaticData::NamespaceKey()
		{
			static FAttributeKey AttributeKey(TEXT("__Namespace_"));
			return AttributeKey;
		}
		...
		const FAttributeKey& FBaseNodeStaticData::DesiredChildIndexKey()
		{
			static FAttributeKey AttributeKey(TEXT("__DESIRED_CHILD_INDEX_"));
			return AttributeKey;
		}
...
bool UInterchangeBaseNode::GetNamespace(FString& OutNamespace) const
{
	if (!Attributes->ContainAttribute(UE::Interchange::FBaseNodeStaticData::NamespaceKey()))
	{
		return false;
	}
	UE::Interchange::FAttributeStorage::TAttributeHandle<FString> Handle = Attributes->GetAttributeHandle<FString>(UE::Interchange::FBaseNodeStaticData::NamespaceKey());
	if (Handle.IsValid())
	{
		UE::Interchange::EAttributeStorageResult Result = Handle.Get(OutNamespace);
		return IsAttributeStorageResultSuccess(Result);
	}
	return false;
}

bool UInterchangeBaseNode::SetNamespace(const FString& InNamespace)
{
	if(InNamespace.IsEmpty())
	{
		if(Attributes->ContainAttribute(UE::Interchange::FBaseNodeStaticData::NamespaceKey()))
		{
			Attributes->UnregisterAttribute(UE::Interchange::FBaseNodeStaticData::NamespaceKey());
		}
		return !Attributes->ContainAttribute(UE::Interchange::FBaseNodeStaticData::NamespaceKey());
	}
	UE::Interchange::EAttributeStorageResult Result = Attributes->RegisterAttribute(UE::Interchange::FBaseNodeStaticData::NamespaceKey(), InNamespace);
	if (IsAttributeStorageResultSuccess(Result))
	{
		UE::Interchange::FAttributeStorage::TAttributeHandle<FString> Handle = Attributes->GetAttributeHandle<FString>(UE::Interchange::FBaseNodeStaticData::NamespaceKey());
		return Handle.IsValid();
...
int32 UInterchangeBaseNode::GetDesiredChildIndex() const
{
	if (!Attributes->ContainAttribute(UE::Interchange::FBaseNodeStaticData::DesiredChildIndexKey()))
	{
		return INDEX_NONE;
	}

	int32 ChildIndex;
	UE::Interchange::FAttributeStorage::TAttributeHandle<int32> Handle = Attributes->GetAttributeHandle<int32>(UE::Interchange::FBaseNodeStaticData::DesiredChildIndexKey());
	if (Handle.IsValid())
	{
		Handle.Get(ChildIndex);
		return ChildIndex;
	}
	return INDEX_NONE;
}

bool UInterchangeBaseNode::SetDesiredChildIndex(const int32& ChildIndex)
{
	UE::Interchange::EAttributeStorageResult Result = Attributes->RegisterAttribute(UE::Interchange::FBaseNodeStaticData::DesiredChildIndexKey(), ChildIndex);
	if (IsAttributeStorageResultSuccess(Result))
	{
		UE::Interchange::FAttributeStorage::TAttributeHandle<int32> Handle = Attributes->GetAttributeHandle<int32>(UE::Interchange::FBaseNodeStaticData::DesiredChildIndexKey());
		return Handle.IsValid();
...
void UInterchangeBaseNode::CopyStorageAttributes(const UInterchangeBaseNode* SourceNode, UInterchangeBaseNode* DestinationNode, const TArray<UE::Interchange::FAttributeKey>& AttributeKeys)
{
	UE::Interchange::FAttributeStorage::CopyStorageAttributes(*(SourceNode->Attributes), *(DestinationNode->Attributes), AttributeKeys);
}

void UInterchangeBaseNode::CopyStorageAttributes(const UInterchangeBaseNode* SourceNode, UE::Interchange::FAttributeStorage& DestinationStorage, const TArray<UE::Interchange::FAttributeKey>& AttributeKeys)
{
	UE::Interchange::FAttributeStorage::CopyStorageAttributes(*(SourceNode->Attributes), DestinationStorage, AttributeKeys);
}

void UInterchangeBaseNode::CopyStorageAttributes(const UE::Interchange::FAttributeStorage& SourceStorage, UInterchangeBaseNode* DestinationNode, const TArray<UE::Interchange::FAttributeKey>& AttributeKeys)
{
	UE::Interchange::FAttributeStorage::CopyStorageAttributes(SourceStorage, *(DestinationNode->Attributes), AttributeKeys);
}

void UInterchangeBaseNode::CopyStorageAttributes(const UInterchangeBaseNode* SourceNode, UE::Interchange::FAttributeStorage& DestinationStorage, const TArray<TPair<UE::Interchange::FAttributeKey, UE::Interchange::FAttributeKey>>& SrcDestAttributeKeys)
{
	UE::Interchange::FAttributeStorage::CopyStorageAttributes(*(SourceNode->Attributes), DestinationStorage, SrcDestAttributeKeys);
}
