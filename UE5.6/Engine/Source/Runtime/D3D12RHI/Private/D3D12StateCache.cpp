...
void FD3D12StateCache::ApplyState(ERHIPipeline HardwarePipe, ED3D12PipelineType PipelineType)
{
...
#if PLATFORM_SUPPORTS_BINDLESS_RENDERING
	{
		...
		bool bRequireExplicitDescriptorCacheSet = bBindlessResources && !bHaveResourceHeap;
		if (bRequireExplicitDescriptorCacheSet)
		{
			// Samplers should be bindless as well
			check(bBindlessSamplers);
			// Should have any SRVs or UAVs
			check(!bHasTableResources);
			...
			TransientDescriptorCache.Init(0, 0, 0, ERHIBindlessConfiguration::RayTracingShaders);

			CmdContext.SetExplicitDescriptorCache(TransientDescriptorCache);
