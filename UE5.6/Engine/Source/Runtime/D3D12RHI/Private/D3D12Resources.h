...
private:
	...
	D3D12_RESOURCE_STATES InitialResourceState{ D3D12_RESOURCE_STATE_TBD };
...
public:
	...
	D3D12_RESOURCE_STATES GetInitialResourceState() const { return InitialResourceState; }
	...
	struct FD3D12ResourceTypeHelper
	{
		...
		const D3D12_RESOURCE_STATES GetOptimalInitialState(ERHIAccess InResourceState, bool bAccurateWriteableStates) const
		{
			...
			{
				return GetD3D12ResourceState(InResourceState, ED3D12QueueType::Direct, nullptr);
			}
			else
			{
				if (bSRVOnly)
...
private:
	void InitalizeResourceState(D3D12_RESOURCE_STATES InInitialState, ED3D12ResourceStateMode InResourceStateMode, D3D12_RESOURCE_STATES InDefaultState)
	{
		...
		if (bRequiresResourceStateTracking)
		{
			...
			InitialResourceState = InInitialState;
		}
	}
...
public:
	...
	bool OnAllocationMoved(FD3D12ContextArray const& Contexts, FRHIPoolAllocationData* InNewData, D3D12_RESOURCE_STATES& OutCreateState);
	void UnlockPoolData();
