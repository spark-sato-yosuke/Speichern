...
bool GLowLevelCacheKeepPrecachedPSOs = false;
static FAutoConsoleVariableRef CVarPSOPrecacheKeepLowLevel(
	TEXT("D3D12.PSOPrecache.KeepLowLevel"),
	GLowLevelCacheKeepPrecachedPSOs,
	TEXT("Keep in memory the d3d12 PSO blob for precached PSOs. Consumes more memory but reduces stalls.\n"),
	ECVF_ReadOnly
);

bool GLowLevelCacheKeepUsedPSOs = false;
static FAutoConsoleVariableRef CVarLowLevelCacheKeepUsedPSOs(
	TEXT("D3D12.PSO.KeepUsedPSOsInLowLevelCache"),
	GLowLevelCacheKeepUsedPSOs,
	TEXT("Keep in memory the d3d12 PSO blob for used PSOs. Consumes more memory but reduces potential stalls.\n"),
	ECVF_ReadOnly
);
...
bool FD3D12PipelineState::UsePSORefCounting()
{
	...
	return false;
#else
	bool bShouldRefCountForPSOPrecaching = PipelineStateCache::IsPSOPrecachingEnabled() && !GLowLevelCacheKeepPrecachedPSOs;
	bool bShouldRefCountForUsedPSOs = !GLowLevelCacheKeepUsedPSOs;
	return bShouldRefCountForPSOPrecaching || bShouldRefCountForUsedPSOs;
#endif
}
...
FD3D12ComputePipelineState::FD3D12ComputePipelineState(const FComputePipelineStateInitializer& InInitializer, const FD3D12RootSignature* InRootSignature, FD3D12PipelineState* InPipelineState)
	: FRHIComputePipelineState(InInitializer.ComputeShader)
	, FD3D12PipelineStateCommonData(InRootSignature, InPipelineState)
{
	FD3D12ComputeShader* D3D12ComputeShader = static_cast<FD3D12ComputeShader*>(InInitializer.ComputeShader);
	bShaderNeedsGlobalConstantBuffer = D3D12ComputeShader && D3D12ComputeShader->UsesGlobalUniformBuffer();
	bPSOPrecache = InInitializer.bPSOPrecache;
}
...
FD3D12ComputePipelineState::~FD3D12ComputePipelineState()
{
	...
	if (FD3D12PipelineState::UsePSORefCounting() && PipelineState != nullptr)
	{
		uint32 RefCount = PipelineState->Release();
		check(RefCount > 0);
		bool bRemovePrecachedPSO = bPSOPrecache && !GLowLevelCacheKeepPrecachedPSOs;
		bool bRemoveUsedPSO = !bPSOPrecache && !GLowLevelCacheKeepUsedPSOs;
		if (RefCount == 1 && (bRemoveUsedPSO || bRemovePrecachedPSO))
		{
			FD3D12DynamicRHI* D3D12RHI = FD3D12DynamicRHI::GetD3DRHI();
			FD3D12PipelineStateCache& PSOCache = D3D12RHI->GetAdapter().GetPSOCache();
			// NB: it's possible that this remove will not do anything because another thread might have requested the same PSO since we entered the if
			PSOCache.RemoveFromLowLevelCache(PipelineState, static_cast<FD3D12ComputeShader*>(GetComputeShader()), RootSignature);
		}
	}
}
...
void FD3D12PipelineStateCacheBase::RemoveFromLowLevelCache(FD3D12PipelineState* PipelineState, const FGraphicsPipelineStateInitializer& PipelineStateInitializer, const FD3D12RootSignature* RootSignature)
{
	...
	else
	{
		...
		PipelineState->AddRef().CheckAtLeast(2);
	}
}

void FD3D12PipelineStateCacheBase::RemoveFromLowLevelCache(FD3D12PipelineState* PipelineState, const FD3D12ComputeShader* ComputeShader, const FD3D12RootSignature* RootSignature)
{
	if (!FD3D12PipelineState::UsePSORefCounting())
	{
		checkNoEntry();
	...
	if (PipelineState->Release() == 0)
	{
		FD3D12ComputePipelineStateDesc LowLevelDesc = GetComputePipelineStateDesc(ComputeShader, RootSignature);
		LowLevelDesc.Desc.NodeMask = FRHIGPUMask::All().GetNative();
		LowLevelDesc.CombinedHash = FD3D12PipelineStateCacheBase::HashPSODesc(LowLevelDesc);
		int32 ElementsRemoved = ComputePipelineStateCache.Remove(LowLevelDesc);
		ensure(ElementsRemoved == 1);
	}
	else
	{
...
void FD3D12PipelineStateCacheBase::AddToLowLevelCache(const FD3D12ComputePipelineStateDesc& Desc, FD3D12PipelineState** OutPipelineState, const FPostCreateComputeCallback& PostCreateCallback)
{
	...
	// This avoids having multiple threads try to create the same PSO.
	{
		...
		*OutPipelineState = NewPipelineState;
		...
		// This AddRef is for the FD3D12ComputePipelineState requesting it.
		if (FD3D12PipelineState::UsePSORefCounting())
		{
			(*OutPipelineState)->AddRef();
		}
...
FD3D12ComputePipelineState* FD3D12PipelineStateCacheBase::FindInLoadedCache(const FComputePipelineStateInitializer& Initializer, const FD3D12RootSignature* RootSignature, FD3D12ComputePipelineStateDesc& OutLowLevelDesc)
{
	// TODO: For now PSOs will be created on every node of the LDA chain.
