	...
	{
		{
			...
			VERIFYD3D12RESULT(Adapter->CreateBuffer(HeapProps, GetGPUMask(), InitConfig.InitialResourceState, ED3D12ResourceStateMode::SingleState, InitConfig.InitialResourceState, PoolSize, BackingResource.GetInitReference(), TEXT("Resource PoolAllocator Underlying Buffer"), InitConfig.ResourceFlags));
#if UE_MEMORY_TRACE_ENABLED
...
bool FD3D12PoolAllocator::HandleDefragRequest(FRHIContextArray const& Contexts, FRHIPoolAllocationData* InSourceBlock, FRHIPoolAllocationData& InTmpTargetBlock)
{
	...
	if (CurrentResource->RequiresResourceStateTracking())
	{
		...
		ERHIAccess SourceResourceAccess = ERHIAccess::Unknown;
		FD3D12Texture* CurrentResourceRHITexture = nullptr;
		if (Owner->Owner)
		{
			if (CurrentResource->GetDesc().Dimension == D3D12_RESOURCE_DIMENSION_BUFFER)
			{
				FD3D12Buffer* CurrentResourceRHIBuffer = (FD3D12Buffer*)Owner->Owner;
				SourceResourceAccess = CurrentResourceRHIBuffer->GetTrackedAccess_Unsafe();
			}
			else
			{
				CurrentResourceRHITexture = (FD3D12Texture*)Owner->Owner;
				SourceResourceAccess = CurrentResourceRHITexture->GetTrackedAccess_Unsafe();
			}
		}

		SourceResourceState = SourceResourceAccess == ERHIAccess::Unknown ? CurrentResource->GetInitialResourceState() : GetD3D12ResourceState(SourceResourceAccess, ED3D12QueueType::Direct, CurrentResourceRHITexture);

		if (SourceResourceState == D3D12_RESOURCE_STATE_TBD)
		{
			SourceResourceState = CurrentResource->GetReadableState();
		}
	}
	else
	{
		//If the resource is a single state type and doesn't need state tracking set the state as TBD so the scoped transition in FlushPendingCopyOps can revert back to the default state
		SourceResourceState = D3D12_RESOURCE_STATE_TBD;
	}
	...
	D3D12_RESOURCE_STATES DestCreateState = D3D12_RESOURCE_STATE_TBD;
	Owner->OnAllocationMoved(Contexts, InSourceBlock, DestCreateState);

	if (Owner->GetResource()->RequiresResourceStateTracking())
	{
		if (DestCreateState == D3D12_RESOURCE_STATE_TBD)
		{
	...
	{
		//If the resource is a single state type and doesn't need state tracking set the state as TBD so the scoped transition in FlushPendingCopyOps can revert back to the default state
		DestCreateState = D3D12_RESOURCE_STATE_TBD;
	}
...
void FD3D12PoolAllocator::FlushPendingCopyOps(FD3D12CommandContext& InCommandContext)
{
	...
	for (FD3D12VRAMCopyOperation& CopyOperation : PendingCopyOps)
	{
...
#endif // D3D12_RHI_RAYTRACING
		{
			...
			{
				FScopedResourceBarrier ScopedResourceBarrierSrc(InCommandContext, CopyOperation.SourceResource, nullptr, D3D12_RESOURCE_STATE_COPY_SOURCE, D3D12_RESOURCE_BARRIER_ALL_SUBRESOURCES, CopyOperation.SourceResourceState);
				FScopedResourceBarrier ScopedResourceBarrierDst(InCommandContext, CopyOperation.DestResource, nullptr, D3D12_RESOURCE_STATE_COPY_DEST, D3D12_RESOURCE_BARRIER_ALL_SUBRESOURCES, CopyOperation.DestResourceState);
				...
				{
					InCommandContext.CopyBufferRegionChecked(
						CopyOperation.DestResource->GetResource(), CopyOperation.DestResource->GetName(),
						CopyOperation.DestOffset,
						CopyOperation.SourceResource->GetResource(), CopyOperation.SourceResource->GetName(),
			...
			// if the resource need tracking we are setting the state of the destination equal to the original source state
			if (CopyOperation.SourceResource->RequiresResourceStateTracking())
			{
				...
				check(CopyOperation.SourceResourceState != D3D12_RESOURCE_STATE_TBD);

				if (CopyOperation.SourceResourceState != CopyOperation.DestResourceState)
				{
					InCommandContext.AddTransitionBarrier(CopyOperation.DestResource, CopyOperation.DestResourceState, CopyOperation.SourceResourceState, D3D12_RESOURCE_BARRIER_ALL_SUBRESOURCES);
				}
			}
		}
