...
void UInterchangeGenericAssetsPipeline::FilterPropertiesFromTranslatedData(UInterchangeBaseNodeContainer* InBaseNodeContainer)
{
	...
	if (CommonMeshesProperties && CommonSkeletalMeshesAndAnimationsProperties && MeshPipeline && AnimationPipeline)
	{
		...
		if (!CommonSkeletalMeshesAndAnimationsProperties->bImportOnlyAnimations)
		{
			...
			PipelineMeshesUtilities->GetAllGeometryCacheInstance(GeometryCaches);
			if (GeometryCaches.Num() == 0)
			{
				PipelineMeshesUtilities->GetAllGeometryCacheGeometry(GeometryCaches);
			}
		...
		bool bHideGeometryCaches = GeometryCaches.Num() == 0;
		...
		if (bHideGeometryCaches)
		{
			HideFullCategory(UInterchangeGenericMeshPipeline::GetPipelineCategory(UGeometryCache::StaticClass()), MeshPipeline);
		}
...
bool UInterchangeGenericAssetsPipeline::IsPropertyChangeNeedRefresh(const FPropertyChangedEvent& PropertyChangedEvent) const
{
	static const TSet<FName> NeedRefreshProperties =
	{
		...
		GET_MEMBER_NAME_CHECKED(UInterchangeGenericAssetsPipeline, bSceneNameSubFolder),
		GET_MEMBER_NAME_CHECKED(UInterchangeGenericAssetsPipeline, bAssetTypeSubFolders),
...
void UInterchangeGenericAssetsPipeline::CreateSkeletonConflict(USkeleton* SpecifiedSkeleton, USkeletalMesh* SkeletalMesh, UInterchangeBaseNodeContainer* TransientBaseNodeContainer)
{
	...
	constexpr int32 LodIndex = 0;
	{
		...
		if (!SpecifiedSkeleton && !(SkeletalMesh && SkeletalMesh->GetSkeleton()))
		{
			return;
		}
		...
		if (UE::Interchange::Private::FSkeletonHelper::IsCompatibleSkeleton(SpecifiedSkeleton ? SpecifiedSkeleton : SkeletalMesh->GetSkeleton()
			, RootJointNodeId
			, TransientBaseNodeContainer
			, bConvertStaticToSkeletalActive
			, bCheckForIdenticalSkeleton
					...
					if (Skeleton)
					{
						...
						TArray<FName> MemoryDependencies;
						{
							...
							do
							{
								...
								{
									UClass* ObjectClass = FoundReferencerObject->GetClass();
									if (FoundReferencerObject->GetOutermost() != GetTransientPackage()
										&& !ObjectClass->IsChildOf(UInterchangePipelineBase::StaticClass())
										&& !ObjectClass->IsChildOf(UInterchangeBlueprintPipelineBase::StaticClass())
										&& !ObjectClass->IsChildOf(UInterchangePythonPipelineBase::StaticClass()))
									{
...
void UInterchangeGenericAssetsPipeline::ExecutePipeline(UInterchangeBaseNodeContainer* InBaseNodeContainer, const TArray<UInterchangeSourceData*>& InSourceDatas, const FString& ContentBasePath)
{
	...
	// Setup the source node sub paths, which will be used here but also by all other pipelines that run after us and create asset factory nodes
	UInterchangeSourceNode* SourceNode = UInterchangeSourceNode::FindOrCreateUniqueInstance(InBaseNodeContainer);

	SourceNode->SetCustomReimportStrategyFlags(int8(ReimportStrategy));

	if (bAssetTypeSubFolders)
	{
		SourceNode->SetCustomUseAssetTypeSubPathSuffix(bAssetTypeSubFolders);
	}
	if (bSceneNameSubFolder)
	{
		FString SceneNameFolderPrefix = FPaths::GetBaseFilename(InSourceDatas[0]->GetFilename());

		UInterchangeManager::GetInterchangeManager().SanitizeNameInline(
			SceneNameFolderPrefix,
			ESanitizeNameTypeFlags::ObjectName | ESanitizeNameTypeFlags::ObjectPath | ESanitizeNameTypeFlags::LongPackage
		);

		SourceNode->SetCustomSubPathPrefix(SceneNameFolderPrefix);
	}

	InBaseNodeContainer->IterateNodesOfType<UInterchangeFactoryBaseNode>(
		[SourceNode, ReimportStrategyClosure = ReimportStrategy](const FString& NodeUid, UInterchangeFactoryBaseNode* FactoryNode)
		{
			// This obviously only covers the factory nodes that were created so far. Any pipeline after this will need
			// to call this themselves if they create additional asset factory nodes
...
void UInterchangeGenericAssetsPipeline::AddMetaData(UObject* CreatedAsset, const UInterchangeBaseNode* Node)
{
	...
	const bool bAssetIsActor = CreatedAsset->IsA<AActor>();
#if WITH_EDITORONLY_DATA
	FMetaData& MetaData = CreatedAsset->GetPackage()->GetMetaData();
#endif
