	...
	if (DecalActor)
	{
		if (const UInterchangeDecalActorFactoryNode* DecalActorFactoryNode = Cast<UInterchangeDecalActorFactoryNode>(&FactoryNode))
		{
			...
			if (DecalActorFactoryNode->GetCustomDecalMaterialPathName(DecalMaterialPath))
			{
				...
				{
					const FText Message = FText::Format(NSLOCTEXT("DecalActorImport", "NoMaterial", "No valid decal material found. Make sure that the DecalMaterialPath is valid: {0}"), FText::FromString(DecalMaterialPath));
#if WITH_EDITOR
