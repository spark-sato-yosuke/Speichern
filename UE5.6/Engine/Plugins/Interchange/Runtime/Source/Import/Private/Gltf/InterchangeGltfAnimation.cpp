...
namespace UE::Interchange::Gltf::Private
{
	...
	*/
	...
	bool GetTransformAnimationPayloadData(const FString& PayLoadKey, const GLTF::FAsset& GltfAsset, FAnimationPayloadData& OutPayloadData)
	{
		...
		for (const int32 ChannelIndex : ChannelIndices)
		{
			...
			case GLTF::FAnimation::EPath::Rotation:
			{
				...
				ConvertCurve([&HeadingHelper](const float* QuatData)
					{
	...
	void GetNonJointTransforms(const GLTF::FAsset& GltfAsset, int32 CurrentNodeIndex, FTransform& Transform)
	{
		if (GltfAsset.Nodes.IsValidIndex(CurrentNodeIndex))
		{
			if (GltfAsset.Nodes[CurrentNodeIndex].Type != GLTF::FNode::EType::Joint)
			{
				Transform = GltfAsset.Nodes[CurrentNodeIndex].Transform * Transform;

				GetNonJointTransforms(GltfAsset, GltfAsset.Nodes[CurrentNodeIndex].ParentIndex, Transform);
			}
		}
	}
	...
	bool GetBakedAnimationTransformPayloadData(const FString& PayLoadKey, const GLTF::FAsset& GltfAsset, FAnimationPayloadData& PayloadData)
	{
		...
		bool bNonJointTransformNeedsApplication = false;
		FTransform NonJointTransformToApply;
		...
		for (const int32& ChannelIndex : ChannelIndices)
		{
			...
			if (!BaseTransformsAcquired)
			{
				...
				if (Channel.Target.Node.Type == GLTF::FNode::EType::Joint &&
					Channel.Target.Node.ParentIndex != Channel.Target.Node.ParentJointIndex)
				{
					GetNonJointTransforms(GltfAsset, Channel.Target.Node.ParentIndex, NonJointTransformToApply);
					bNonJointTransformNeedsApplication = true;
				}
			}
		...
		if (bNonJointTransformNeedsApplication)
		{
			FVector3d NonJointTranslation = NonJointTransformToApply.GetTranslation();
			FQuat4d NonJointRotation = NonJointTransformToApply.GetRotation();
			FVector3d NonJointScale = NonJointTransformToApply.GetScale3D();

			for (size_t Index = 0; Index < BakeKeyCount; Index++)
			{
				PayloadData.Transforms.Add(FTransform(NonJointRotation * FQuat4d(RotationData[Index]), NonJointTranslation + FVector3d(TranslationData[Index]), NonJointScale * FVector3d(ScaleData[Index])));
			}
		}
		else
		{
	...
	namespace AnimationHelpers
	{
		struct FAnimationHandler
		{
			...
			const TArray<GLTF::FSkinInfo>& GLTFSkins;
			...
			FAnimationHandler(UInterchangeBaseNodeContainer& InNodeContainer, 
				...
				const TArray<GLTF::FSkinInfo>& InGLTFSkins,
			...
			UInterchangeSkeletalAnimationTrackNode* AcquireTrackNode(const FString& SkeletonNodeUid, FAnimationTimes& AnimationTimes)
			{
				...
				else
				{
					...
					NodeContainer.SetupNode(TrackNode, TrackNodeUid, GLTFAnimation.Name, EInterchangeNodeContainerType::TranslatedAsset);
			...
			void ProcessRigidAnimation(const TMap<int32, TSet<int32>>& RigidAnimation)
			{
				...
				NodeContainer.SetupNode(TrackSetNode, AnimTrackSetNodeUid, GLTFAnimation.Name, EInterchangeNodeContainerType::TranslatedAsset);
			...
			void Process()
			{
				...
				TMap<int32, TMap<int32, TSet<int32>>> MorphTargetAnimations;		// (UEAnimatedNodeIndex, (AnimatedNodeIndex, [Channels]) //Channel is a Target.Path==Weight channel.
				...
				for (int32 ChannelIndex = 0; ChannelIndex < GLTFAnimation.Channels.Num(); ++ChannelIndex)
				{
					...
					{
						int32 UEAnimatedNodeIndex = AnimatedNodeIndex;
						if (AnimatedNode.Type == GLTF::FNode::EType::MeshSkinned)
						{
							const GLTF::FSkinInfo& Skin = GLTFSkins[AnimatedNode.Skindex];
							if (GLTFSkins.IsValidIndex(AnimatedNode.Skindex)
								&& GLTFSkins[AnimatedNode.Skindex].Joints.Num()
								&& GLTFNodes.IsValidIndex(GLTFSkins[AnimatedNode.Skindex].Joints[0]))
							{
								UEAnimatedNodeIndex = GLTFNodes[Skin.Joints[0]].RootJointIndex;
							}
						}
						else if (AnimatedNode.Type == GLTF::FNode::EType::Joint)
						{
							UEAnimatedNodeIndex = AnimatedNode.RootJointIndex;
						}

						TMap<int32, TSet<int32>>& AnimatedNodeIndicesToChannelIndices = MorphTargetAnimations.FindOrAdd(UEAnimatedNodeIndex);
		...
		const TMap<const GLTF::FNode*, FString>& GLTFNodeToInterchangeUidMap)
	{
		...
		{
			FAnimationHandler AnimationHadler(NodeContainer, AnimationIndex, Animations[AnimationIndex], GLTFNodes, GLTFSkins, GLTFNodeToInterchangeUidMap);
