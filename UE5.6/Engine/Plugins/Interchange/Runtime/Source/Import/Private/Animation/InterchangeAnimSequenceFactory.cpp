			...
			for (const FString& NonAnimatedSkeletonNodeUID : NonAnimatedSkeletonNodes)
			{
				if (const UInterchangeSceneNode* SkeletonNode = Cast<UInterchangeSceneNode>(NodeContainer->GetNode(NonAnimatedSkeletonNodeUID)))
				{
						...
						)
					{
						...
						// On the unusal edge use case where Non-Joint Node(s) are inserted into a Skeleton hierarchy, bake said transforms onto the Joint.
						if (!SkeletonNode->GetUniqueID().Equals(SkeletonRootUid))
						{
							TArray<FTransform> NonJointTransforms; //Child to Parent
							const UInterchangeSceneNode* ParentNode = Cast<UInterchangeSceneNode>(NodeContainer->GetNode(SkeletonNode->GetParentUid()));
							while (ParentNode && !ParentNode->IsSpecializedTypeContains(UE::Interchange::FSceneNodeStaticData::GetJointSpecializeTypeString()))
							{
								FTransform NonJointLocalTransform;
								if (ParentNode->GetCustomLocalTransform(NonJointLocalTransform))
								{
									NonJointTransforms.Add(NonJointLocalTransform);
								}

								ParentNode = Cast<UInterchangeSceneNode>(NodeContainer->GetNode(ParentNode->GetParentUid()));
							}
							
							if (NonJointTransforms.Num() > 0)
							{
								FTransform NonJointTransformToApply;
								for (int32 NonJointCounter = NonJointTransforms.Num() - 1; NonJointCounter >= 0; NonJointCounter--)
								{
									NonJointTransformToApply *= NonJointTransforms[NonJointCounter];
								}

								LocalTransform = NonJointTransformToApply * LocalTransform;
							}
						}
