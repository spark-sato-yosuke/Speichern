		...
		const TArray<float>& MorphTargetWeights)
	{
		...
		if (Primitive.HasNormals())
		{
			...
			if (TotalMatrixForNormal != FMatrix::Identity)
			{
				...
				{
					FVector TransformedPosition = TotalMatrixForNormal.TransformVector(FVector(Normal)).GetSafeNormal();
					Normal = static_cast<FVector3f>(TransformedPosition);
				}
			}
		...
		if (!bIgnoreTangents && Primitive.HasTangents())
		{
			...
			if (TotalMatrixForNormal != FMatrix::Identity)
			{
				for (FVector4f& TangentBinormal : Tangents)
				{
					const float Binormal = TangentBinormal.W * BinormalMultipler;
					FVector4 TransformedTangentBinormal = TotalMatrixForNormal.TransformVector(FVector(TangentBinormal)).GetSafeNormal();
					TransformedTangentBinormal.W = Binormal;
					TangentBinormal = FVector4f(TransformedTangentBinormal);
				}
			}
