...
namespace GLTF
{
		...
		for (size_t SkinIndex = 0; SkinIndex < Asset->Skins.Num(); SkinIndex++)
		{
				...
				Skin.InverseBindMatrices.IsValid())
			{
				for (size_t JointCounter = 0; JointCounter < Skin.Joints.Num(); JointCounter++)
				{
					...
					if (CurrentNode.ParentJointIndex != INDEX_NONE &&
						Asset->Nodes.IsValidIndex(CurrentNode.ParentJointIndex) &&
						(Asset->Nodes[CurrentNode.ParentJointIndex].SkinIndexToGlobalInverseBindMatrix.Contains(SkinIndex) || Asset->Nodes[CurrentNode.ParentJointIndex].SkinIndexToGlobalInverseBindMatrix.Num() > 0)
						)
					{
						FNode& ParentNode = Asset->Nodes[CurrentNode.ParentJointIndex];
	...
	void FFileReader::BuildParentIndices(int32 ParentNodeIndex, int32 ParentJointIndex, int32 CurrentNodeIndex) const
	{
		...
		Node.ParentJointIndex = ParentJointIndex;

		for (const int32 ChildNodeIndex : Node.Children)
		{
			BuildParentIndices(CurrentNodeIndex, Node.Type == GLTF::FNode::EType::Joint ? CurrentNodeIndex : ParentJointIndex, ChildNodeIndex);
		}
	}
			...
			for (size_t SkinIndex = 0; SkinIndex < Asset->Skins.Num(); SkinIndex++)
			{
				...
				for (TPair<int32, FRootJoints> Group : ParentToRootJointIndices)
				{
					...
					if (Asset->Nodes.IsValidIndex(Group.Key))
					{
						...
						Node.ParentJointIndex = OriginalNode.ParentJointIndex;
