...
bool FFbxMesh::GetGlobalJointBindPoseTransform(FFbxParser* Parser, FbxScene* SDKScene, FbxNode* Joint, FFbxJointMeshBindPoseGenerator& FbxJointMeshBindPoseGenerator,
	...
	bool& bBadBindPoseMessageDisplay, const FbxAMatrix& JointOrientationMatrix)
{
	...
	bool bRetrieveBindPoseResult = RetrievePoseFromBindPose();
	// get bind pose
	if (!bRetrieveBindPoseResult)
	{
		...
		bRetrieveBindPoseResult = RetrievePoseFromBindPose();
	}

	if (bRetrieveBindPoseResult)
	{
		GlobalBindPoseJointMatrix = GlobalBindPoseJointMatrix * JointOrientationMatrix;
	}

	return bRetrieveBindPoseResult;
}
	...
	for (int32 GeometryIndex = 0; GeometryIndex < GeometryCount; ++GeometryIndex)
	{
		...
		for (int32 GeometryDeformerIndex = 0; GeometryDeformerIndex < GeometryDeformerCount; ++GeometryDeformerIndex)
		{
			...
			for (int32 ClusterIndex = 0; ClusterIndex < ClusterCount; ClusterIndex++)
			{
				...
				GlobalBindPoseJointMatrixPerGeo = GlobalBindPoseJointMatrixPerGeo * Parser.JointOrientationMatrix;
				...
				GlobalBindPoseReferenceMatrix = GlobalBindPoseReferenceMatrix * Parser.JointOrientationMatrix;
