...
const FName TG_Category::Arrays = "Arrays";
...
FTG_Signature::FInit UTG_Expression::GetSignatureInitArgsFromClass() const
{
	...
	for (TFieldIterator<FProperty> Prop(GetClass()); Prop; ++Prop) {
		...
		// Add NotConnectable for certain types:
		{
			...
			bIsEnum = (EnumProperty != nullptr);
			FNameProperty* NameProperty = CastField<FNameProperty>(*Prop);
			bIsNotConnectable |= (NameProperty != nullptr);
		}
		uint8 Flags = bIsNotConnectable ? static_cast<uint8>(ETG_Access::NotConnectableFlag) : 0;
		Flags |= bIsEnum ? AccessToUint8(EnumFlag) : 0;

		if (PropertyTGType.Compare(TEXT("TG_InputParam")) == 0)
		{
			// Add the new argument to the signature
			SignatureInit.Arguments.Add(FTG_Argument{ PropName, PropTypeName, { ETG_Access::InParam, Flags} });
		}																								   
		else if (PropertyTGType.Compare(TEXT("TG_OutputParam")) == 0)									   
		{																									   
			// Add the new argument to the signature													   
			SignatureInit.Arguments.Add(FTG_Argument{ PropName, PropTypeName, { ETG_Access::OutParam, Flags } });
		}																								   
		else if (PropertyTGType.Compare(TEXT("TG_Input")) == 0)										   
		{													   
			// Add the new argument to the signature													   
			SignatureInit.Arguments.Add(FTG_Argument{ PropName, PropTypeName, { ETG_Access::In, Flags } });
		}																								   
		else if (PropertyTGType.Compare(TEXT("TG_Output")) == 0)										   
		{																	   
			// Add the new argument to the signature													   
			SignatureInit.Arguments.Add(FTG_Argument{ PropName, PropTypeName, { ETG_Access::Out, Flags } });
		}
		else if (PropertyTGType.Compare(TEXT("TG_Setting")) == 0)
		{
			// Add the new argument to the signature as a Setting													   
			SignatureInit.Arguments.Add(FTG_Argument{ PropName, PropTypeName, { ETG_Access::InSetting, Flags } });
		}
		else if (PropertyTGType.Compare(TEXT("TG_Private")) == 0)
		{
			// Add the new argument to the signature as a Private													   
			SignatureInit.Arguments.Add(FTG_Argument{ PropName, PropTypeName, { ETG_Access::Private, Flags } });
		}
		else
		{
...
bool UTG_Expression::CanEditChange(const FProperty* InProperty) const
{
	...
	if (bIsEditable && InProperty != nullptr)
	{
		...
		if (PropertyName == GET_MEMBER_NAME_CHECKED(UTG_Expression, BaseOutputSettings))
		{
			bIsEditable = ShouldShowSettings();
		}
	}
...
void UTG_Expression::SetupAndEvaluate(FTG_EvaluationContext* InContext)
{
	...
	{
		if (OutputVar.Value.Argument.IsVariant())
		{
			FTG_Variant& Variant = OutputVar.Value.Var->GetAs<FTG_Variant>();
			if (Variant.IsTexture())
			{
				FTG_Texture& Texture = Variant.GetTexture();
				Texture.Descriptor = BaseOutputSettings;
			}
		}
		else if (OutputVar.Value.Argument.IsTexture())
		{
			FTG_Texture& Texture = OutputVar.Value.Var->GetAs<FTG_Texture>();
			Texture.Descriptor = BaseOutputSettings;
		}
...
FTG_Variant::EType UTG_Expression::EvalExpressionCommonInputVariantType() const
{
	FTG_Variant::EType Type = FTG_Variant::EType::Invalid;
	const UTG_Node* ParentNode = GetParentNode();
	for (auto Pin : ParentNode->Pins)
	{
		if (Pin->IsInput() && Pin->IsArgVariant())
		{
			if (Pin->IsConnected())
			{
				...
				UTG_Pin* OtherPin = ParentNode->GetGraph()->GetPin(Pin->GetVarId());
...
FTG_Variant::EType UTG_Expression::EvalExpressionCommonOutputVariantType() const
{
	return GetCommonInputVariantType();
}