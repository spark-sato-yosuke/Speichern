...
FTG_Texture& FTG_Variant::GetTexture() 
{
	check(IsTexture());
	return Data.Get<FTG_Texture>();
}
...
void FTG_VariantArray::SetNum(int Count)
{
	int CurrentCount = Array.Num();
	Array.SetNum(Count);

	/// Fill the new items with black
	for (int Index = CurrentCount; Index < Count; Index++)
	{
		Array[Index] = FTG_Texture::GetBlack();
	}
}

void FTG_VariantArray::Set(int Index, const FTG_Variant& Var)
{
	check(Index >= 0 && Index < Array.Num());
	Array[Index] = Var;
}

void FTG_VariantArray::Set(int Index, const FTG_Texture& Tex)
{
	check(Index >= 0 && Index < Array.Num());
	Array[Index].EditTexture() = Tex;
}

void FTG_VariantArray::CopyFrom(const FTG_VariantArray& RHS)
{
	Array = RHS.GetArray();
}

void FTG_VariantArray::SetViewIndex(int Index)
{
	check(Index >= 0 && Index < Array.Num());
	ViewIndex = Index;
}
