...
enum class ETG_Access : uint8
{
	...
	EnumFlag = 0x40,				// the bit mask for telling whether an argument is an enum or not
};
...
struct TEXTUREGRAPH_API FTG_ArgumentType
{
	...
	bool			MatchAccessFilter(uint8 InFilter) const { return ((AccessToUint8(AccessBitMask) & Flags) & (InFilter == 0 ? AccessToUint8(AccessBitMask) : InFilter));  }
	...
	bool			IsEnum() const { return (Flags & AccessToUint8(EnumFlag)) != 0; }
...
public:
	...
	bool				IsArray() const; // FTG_TextureArray or ...
