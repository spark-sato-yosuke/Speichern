...
void FTG_Editor::RegisterTabSpawners(const TSharedRef<class FTabManager>& InTabManager)
{
	...
	InTabManager->RegisterTabSpawner(FTG_EditorTabs::PreviewSceneSettingsTabId, FOnSpawnTab::CreateSP(this, &FTG_Editor::SpawnTab_PreviewSceneSettings))
	...
	InTabManager->RegisterTabSpawner(FTG_EditorTabs::PreviewSettingsTabId, FOnSpawnTab::CreateSP(this, &FTG_Editor::SpawnTab_PreviewSettings))
...
void FTG_Editor::InitEditor(const EToolkitMode::Type Mode, const TSharedPtr< class IToolkitHost >& InitToolkitHost, UTextureGraph* InTextureGraph)
{
	...
	EditedTextureGraph->OnBatchQueued.BindRaw(this, &FTG_Editor::OnBatchQueued);
	...
	PreviewSettingsView->OnFinishedChangingProperties().AddSP(this, &FTG_Editor::OnFinishedChangingSettings);
...
FTG_Editor::~FTG_Editor()
{
	...
	EditedTextureGraph->OnBatchQueued.Unbind();
...
void FTG_Editor::OnBatchQueued(UMixInterface* TextureGraph, const FInvalidationDetails* Details)
{
	if (TextureGraph != nullptr)
	{
...
TSharedRef<class SGraphEditor> FTG_Editor::CreateGraphEditorWidget()
{
	...
	InEvents.OnCreateActionMenuAtLocation = SGraphEditor::FOnCreateActionMenuAtLocation::CreateSP(this, &FTG_Editor::OnCreateGraphActionMenu);
...
FActionMenuContent FTG_Editor::OnCreateGraphActionMenu(UEdGraph* InGraph, const FVector2f& InNodePosition, const TArray<UEdGraphPin*>& InDraggedPins, bool bAutoExpand, SGraphEditor::FActionMenuClosed InOnMenuClosed)
{
...
TSharedRef<SDockTab> FTG_Editor::SpawnTab_PreviewSceneSettings(const FSpawnTabArgs& Args)
{
...
TSharedRef<SDockTab> FTG_Editor::SpawnTab_PreviewSettings(const FSpawnTabArgs& Args)
{
	...
	GetPreviewSettingsView()->SetObject(EditedTextureGraph->GetSettings(), true);
...
void FTG_Editor::OnSelectedNodesChanged(const TSet<class UObject*>& NewSelection)
{
	...
	for (TSet<class UObject*>::TConstIterator SetIt(NewSelection); SetIt; ++SetIt)
	{
		if (UTG_EdGraphNode* GraphNode = Cast<UTG_EdGraphNode>(*SetIt))
		{
			...
			// calling this here to set EditCondition metadata properly for Details panel
			// this is driven from the CanEditChange() method of the expression which can be customized as required
			GraphNode->UpdatePinsEditCondition();
		}  
	}
...
bool FTG_Editor::UpdateOriginalTextureGraph()
{
	...
	{
		// Cache any metadata
		const TMap<FName, FString>* MetaData = FMetaData::GetMapForObject(OriginalTextureGraph);
		...
		if (MetaData)
		{
			FMetaData& PackageMetaData = OriginalTextureGraph->GetPackage()->GetMetaData();
