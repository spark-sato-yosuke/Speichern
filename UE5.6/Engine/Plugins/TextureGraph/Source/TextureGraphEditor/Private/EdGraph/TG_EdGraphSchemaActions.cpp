...
UEdGraphNode* FTG_EdGraphSchemaAction_NewNode::PerformAction(UEdGraph* ParentGraph, UEdGraphPin* FromPin, const FVector2f& Location, bool bSelectNewNode)
{
	return CreateExpressionNode(ParentGraph, TG_ExpressionClass.Get(), FromPin, Location, bSelectNewNode);
}

UEdGraphNode* FTG_EdGraphSchemaAction_NewNode::CreateExpressionNode(UEdGraph* ParentGraph, const UClass* ExpressionClass, UEdGraphPin* FromPin, const UE::Slate::FDeprecateVector2DParameter& Location, bool bSelectNewNode)
{
	...
	{	
		UTG_EdGraphNode* NewNode = TG_EdGraph->AddModelNode(NewExpressionNode, true, FDeprecateSlateVector2D(Location));
...
UEdGraphNode* FTG_EdGraphSchemaAction_NewNode::CreateExpressionNode(UEdGraph* ParentGraph, UTG_Expression* Expression, UEdGraphPin* FromPin,  const UE::Slate::FDeprecateVector2DParameter& Location, bool bSelectNewNode)
{
	const FScopedTransaction Transaction(NSLOCTEXT("UnrealEd", "TSEditorNewNode", "TS Editor: New Node"));
