...
FTransform UPaperTerrainComponent::GetTransformAtDistance(float InDistance) const
{
	...
	const float Param = AssociatedSpline->GetInputKeyValueAtDistanceAlongSpline(InDistance);
	const FVector Position3D = AssociatedSpline->GetLocationAtSplineInputKey(Param, ESplineCoordinateSpace::Local);

	const FVector Tangent = AssociatedSpline->GetTangentAtSplineInputKey(Param, ESplineCoordinateSpace::Local).GetSafeNormal();
	const FVector NormalEst = StaticCastPtr<const UPaperTerrainSplineComponent, UPaperTerrainSplineComponent>(AssociatedSpline)->GetSplinePointsPosition().EvalSecondDerivative(Param, FVector(0.0f, 1.0f, 0.0f)).GetSafeNormal();
...
void UPaperTerrainComponent::ConstrainSplinePointsToXZ()
{
	if (AssociatedSpline != nullptr)
	{
		...
		const int32 NumPoints = AssociatedSpline->GetNumberOfSplinePoints();
		for (int PointIndex = 0; PointIndex < NumPoints; ++PointIndex)
		{
			const FSplinePoint Point = AssociatedSpline->GetSplinePointAt(PointIndex, ESplineCoordinateSpace::Local);
			
			if (Point.ArriveTangent.Y != 0 || Point.LeaveTangent.Y != 0 || Point.Position.Y != 0)
			{
				FVector NewArriveTangent = Point.ArriveTangent; NewArriveTangent.Y = 0;
				FVector NewLeaveTangent = Point.LeaveTangent; NewLeaveTangent.Y = 0;
				AssociatedSpline->SetTangentsAtSplinePoint(PointIndex, NewArriveTangent, NewLeaveTangent, ESplineCoordinateSpace::Local, false);
				
				FVector NewPosition = Point.Position; NewPosition.Y = 0;
				AssociatedSpline->SetLocationAtSplinePoint(PointIndex, NewPosition, ESplineCoordinateSpace::Local, false);
	...
	if ((AssociatedSpline != nullptr) && (TerrainMaterial != nullptr))
	{
		...
		if (bClosedSpline && bFilledSpline)
		{
			...
			TArray<float> SplineEdgeOffsetAmounts;
			{
				...
				while (CurrentTime < SplineLength)
				{
					// note [nickolas.drake]: SplineLength is not measured in time, I am adapting this code in spite of the fact that it seems wrong at a glance.
					const float Param = AssociatedSpline->GetInputKeyValueAtDistanceAlongSpline(CurrentTime);
					const FVector Position3D = AssociatedSpline->GetLocationAtSplineInputKey(Param, ESplineCoordinateSpace::Local);
