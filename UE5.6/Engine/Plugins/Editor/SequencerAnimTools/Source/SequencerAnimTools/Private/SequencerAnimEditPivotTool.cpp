...
void USequencerPivotTool::Setup()
{
	...
	GizmoData.InstanceIdentifier = TEXT("PivotToolGizmoIdentifier");
	FSequencerAnimToolHelpers::CreateGizmo(GizmoData, TransformGizmo, TRSGizmo);
	//turn default widget(transform gizmo) off when entering this mode if not off
...
FVector2f USequencerPivotTool::LastPivotOverlayLocation = FVector2f::ZeroVector;
...
static FVector2f GetActiveViewportSize(TSharedPtr<IAssetViewport>& ActiveViewport)
{
...
void USequencerPivotTool::CreateAndShowPivotOverlay()
{
	if (FLevelEditorModule* LevelEditorModule = FModuleManager::GetModulePtr<FLevelEditorModule>(TEXT("LevelEditor")))
	{
		...
		if (ActiveLevelViewport.IsValid())
		{
			FVector2f NewWidgetLocation = LastPivotOverlayLocation;

			if (NewWidgetLocation.IsZero())
			{
				const FVector2f ActiveViewportSize = GetActiveViewportSize(ActiveLevelViewport);
				NewWidgetLocation.X = ActiveViewportSize.X / 2.0f;
				NewWidgetLocation.Y = 50.0f;			

			}
			UpdatePivotOverlayLocation(FVector2D(NewWidgetLocation), ActiveLevelViewport);
...
{
	const FVector2f ActiveViewportSize = GetActiveViewportSize(ActiveLevelViewport);
	FVector2f ScreenPos = UE::Slate::CastToVector2f(InLocation);
