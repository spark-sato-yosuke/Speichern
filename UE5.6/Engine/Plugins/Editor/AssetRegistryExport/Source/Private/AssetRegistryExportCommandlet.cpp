...
int32 UAssetRegistryExportCommandlet::Main(const FString& CmdLineParams)
{
		...
		FParse::Value(*CmdLineParams, TEXT("Input="), AssetRegistryFileName) == false)
	{
		...
		UE_LOG(LogAssetRegistryExport, Display, TEXT("    -FilterToClass=<classname>          Only add the table for the given class (and global assets)"));
		UE_LOG(LogAssetRegistryExport, Display, TEXT("                                        Example: -FilterToClass=\"/Script/Engine.Texture2D\""));
		return 1;
	}
	...
	FString FilterToClass;
	FParse::Value(*CmdLineParams, TEXT("FilterToClass="), FilterToClass);
	if (FilterToClass.Len())
	{
		UE_LOG(LogAssetRegistryExport, Display, TEXT("Got option: FilterToClass=%s"), *FilterToClass);
	}
	...
	AssetRegistry.EnumerateAllAssets(TSet<FName>(), [&](const FAssetData& AssetData)
	{
		...
		if (FilterToClass.Len() &&
			FilterToClass != AssetClassPathString)
		{
			return true;
		}

		FClassInfo& ClassInfo = ClassInfos.FindOrAdd(AssetData.AssetClassPath);
		ClassInfo.Name = MoveTemp(AssetClassPathString);
		{
	...
	for (TPair<FTopLevelAssetPath, FClassInfo>& Pair : ClassInfos)
	{
		...
		else
		{
			...
			for (FName TagName : Class.Tags)
			{
				...
				CreateTable += TEXT(", \"") + SanitizedTagName + TEXT("\"");
				InsertStatement += TEXT(", \"") + SanitizedTagName + TEXT("\"");
	...
	if (bCSV)
	{
		...
		TUtf8StringBuilder<128> Header;
		Header << "Name,Class,Path,Stage_ChunkCompressedSize,Stage_ChunkSize,Stage_ChunkOptionalSize,Stage_ChunkStreamingSize\r\n";
		CSVWriter.Serialize(Header.GetData(), Header.Len());
	}
	else
	{
		// Create catchall table and insert statements
		if (OutputDatabase.Execute(TEXT("CREATE TABLE IF NOT EXISTS Assets(Id INTEGER PRIMARY KEY, Name, Class, Path TEXT UNIQUE, Stage_ChunkCompressedSize INTEGER, Stage_ChunkSize INTEGER, Stage_ChunkOptionalSize INTEGER, Stage_ChunkStreamingSize INTEGER);")) == false)
		{
		...
		GlobalInsertStatement.Emplace(OutputDatabase, TEXT("INSERT INTO Assets(Name, Class, Path, Stage_ChunkCompressedSize, Stage_ChunkSize, Stage_ChunkOptionalSize, Stage_ChunkStreamingSize) Values(?1, ?2, ?3, ?4, ?5, ?6, ?7);"));
	...
	AssetRegistry.EnumerateAllAssets(TSet<FName>(), [&](const FAssetData& AssetData)
	{
		...
		if (bCSV)
		{
			// Add to the 'Assets' table.
			{
				...
				TUtf8StringBuilder<128> Line;
				Line << AssetData.AssetName.ToString() << TEXTVIEW(",");
				Line << AssetData.AssetClassPath.ToString() << TEXTVIEW(",");
				Line << AssetObjectPath << TEXTVIEW(",");
				...
				FString Size;
				AssetData.GetTagValue(UE::AssetRegistry::Stage_ChunkSizeFName, Size);
				FString StreamingSize;
				AssetData.GetTagValue(UE::AssetRegistry::Stage_ChunkStreamingSizeFName, StreamingSize);
				FString OptionalSize;
				AssetData.GetTagValue(UE::AssetRegistry::Stage_ChunkOptionalSizeFName, OptionalSize);
				
				Line << CompressedSize << TEXTVIEW(",");
				Line << Size << TEXTVIEW(",");
				Line << OptionalSize << TEXTVIEW(",");
				Line << StreamingSize;
			...
			FClassInfo* ClassPtr = ClassInfos.Find(AssetData.AssetClassPath);
			if (ClassPtr)
			{
				FClassInfo& Class = *ClassPtr;
		...
		else
		{
			// Add to the 'Assets' table.
			{
				...
				FString Size;
				if (AssetData.GetTagValue(UE::AssetRegistry::Stage_ChunkSizeFName, Size))
				{
					GlobalInsertStatement.GetPtrOrNull()->SetBindingValueByIndex(5, Size);
				}

				FString StreamingSize;
				if (AssetData.GetTagValue(UE::AssetRegistry::Stage_ChunkStreamingSizeFName, StreamingSize))
				{
					GlobalInsertStatement.GetPtrOrNull()->SetBindingValueByIndex(6, StreamingSize);
				}
				FString OptionalSize;
				if (AssetData.GetTagValue(UE::AssetRegistry::Stage_ChunkOptionalSizeFName, OptionalSize))
				{
					GlobalInsertStatement.GetPtrOrNull()->SetBindingValueByIndex(7, OptionalSize);
				}
