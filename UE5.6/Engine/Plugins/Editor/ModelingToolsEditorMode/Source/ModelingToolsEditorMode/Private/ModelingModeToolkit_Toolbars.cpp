...
void MakeSubMenu_SelectionSupport(FModelingToolsEditorModeToolkit* Toolkit, FMenuBuilder& MenuBuilder)
{
	...
	const FUIAction EnableSelectionsToggle(
		...
		{
			Settings->SetMeshSelectionsEnabled(!Settings->GetMeshSelectionsEnabled());
			Settings->SaveConfig();
		...
		FIsActionChecked::CreateLambda([Settings]()
		{
			return Settings->GetMeshSelectionsEnabled();
		}));
...
void MakeSubMenu_Selection_DragMode(FMenuBuilder& MenuBuilder)
{
	const FModelingToolsManagerCommands& Commands = FModelingToolsManagerCommands::Get();

	MenuBuilder.AddMenuEntry(
		Commands.SelectionDragMode_None,
		NAME_None,
	...
	MenuBuilder.AddMenuEntry(
		Commands.SelectionDragMode_Path,
		NAME_None,
...
void MakeSubMenu_Selection_MeshType(FMenuBuilder& MenuBuilder)
{
	const FModelingToolsManagerCommands& Commands = FModelingToolsManagerCommands::Get();

	MenuBuilder.AddMenuEntry(
		Commands.SelectionMeshTypes_Volumes,
		NAME_None,
	...
	MenuBuilder.AddMenuEntry(
		Commands.SelectionMeshTypes_StaticMeshes,
		NAME_None,
...
void MakeSubMenu_Selection_LocalFrameMode(FMenuBuilder& MenuBuilder)
{
	const FModelingToolsManagerCommands& Commands = FModelingToolsManagerCommands::Get();
	MenuBuilder.AddMenuEntry(
		Commands.SelectionLocalFrameMode_Geometry,
		NAME_None,
	...
	MenuBuilder.AddMenuEntry(
		Commands.SelectionLocalFrameMode_Object,
		NAME_None,
...
void MakeSubMenu_Selection_SelectionFilters(FMenuBuilder& MenuBuilder)
{
	const FModelingToolsManagerCommands& Commands = FModelingToolsManagerCommands::Get();
	MenuBuilder.AddMenuEntry(
		Commands.SelectionHitBackFaces,
		NAME_None,
		LOCTEXT("Selection_SelectionFilters_HitBackFaces", "Hit Back Faces"), 
		LOCTEXT("Selection_SelectionFilters_HitBackFaces_Tooltip", "Hit triangles that are facing away from the camera"),
		FSlateIcon(), NAME_None);
}

TSharedRef<SWidget> MakeMenu_SelectionConfigSettings(const FModelingToolsEditorModeToolkit* Toolkit)
{
	...
	MakeSubMenu_Selection_DragMode(MenuBuilder);
	MenuBuilder.EndSection();

	MenuBuilder.BeginSection("Section_MeshTypes", LOCTEXT("Section_MeshTypes", "Selectable Mesh Types"));
	MakeSubMenu_Selection_MeshType(MenuBuilder);
	MenuBuilder.EndSection();

	MenuBuilder.BeginSection("Section_LocalFrameMode", LOCTEXT("Section_LocalFrameMode", "Local Frame Mode"));
	MakeSubMenu_Selection_LocalFrameMode(MenuBuilder);
	MenuBuilder.EndSection();

	MenuBuilder.BeginSection("Section_SelectionFilters", LOCTEXT("Section_SelectionFilters", "Selection Filters"));
	MakeSubMenu_Selection_SelectionFilters(MenuBuilder);
	MenuBuilder.EndSection();
...
void FModelingToolsEditorModeToolkit::MakeSelectionPaletteOverlayWidget()
{
	...
	SelectionPaletteOverlayWidget->SetVisibility( TAttribute<EVisibility>::CreateLambda([this]()
	{
		...
		{
			// only show old MES Toolbar (on left side) when exclusively showing the Old Toolbar (& MES is enabled)
			// TODO: when new viewport toolbar is finalized remove the ShowNewViewportToolbars condition
			return ModelingMode->GetMeshElementSelectionSystemEnabled() && !UE::UnrealEd::ShowNewViewportToolbars() ? EVisibility::SelfHitTestInvisible : EVisibility::Collapsed;
		}
