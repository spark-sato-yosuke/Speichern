...
void FWaveformEditorWaveWriter::ExportTransformedWaveform()
{
	...
	TFunction<void(const USoundWave*)> OnSoundWaveWritten = [AssetName, AssetPath](const USoundWave* ResultingWave) {
		...
		if(ResultingWave->GetPackage())
		{
			...
			FEditorFileUtils::EPromptReturnCode Result = FEditorFileUtils::PromptForCheckoutAndSave(PackagesToSave, false /*bCheckDirty*/, true /*bPromptToSave*/);

			// If the user cancels the checkout and save, delete the new generated soundwave
			if (Result == FEditorFileUtils::PR_Cancelled)
			{
				UEditorAssetLibrary::DeleteAsset(ResultingWave->GetPathName());
			}
		}
	};
	
	//Duplicate the SourceSoundWave so we do not overwrite the original file with the transformations on export
	TObjectPtr<USoundWave> ExportSoundWave = DuplicateObject<USoundWave>(SourceSoundWave, SourceSoundWave->GetOuter());

	if (!WaveWriter->BeginWriteToSoundWave(AssetName, BufferToWrite, AssetPath, OnSoundWaveWritten, ExportSoundWave))
	{
