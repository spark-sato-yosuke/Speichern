...
bool PrivatizeItems(IAssetTools* InAssetTools, IAssetRegistry* InAssetRegistry, const UContentBrowserDataSource* InOwnerDataSource, TArrayView<const FContentBrowserItemData> InItems, const EAssetAccessSpecifier InAssetAccessSpecifier)
{
	TArray<TSharedRef<const FContentBrowserAssetFileItemDataPayload>, TInlineAllocator<16>> AssetPayloads;
	...
	return ObjectTools::PrivatizeAssets(AssetsToPrivatize, /*bShowConfirmation*/true, InAssetAccessSpecifier) > 0;
}

bool DeleteAssetFolderItems(TArrayView<const TSharedRef<const FContentBrowserAssetFolderItemDataPayload>> InFolderPayloads)
{
	TArray<FContentBrowserItemPath> ItemPathsToDelete;
	ItemPathsToDelete.Reserve(InFolderPayloads.Num());

	for (const TSharedRef<const FContentBrowserAssetFolderItemDataPayload>& FolderPayload : InFolderPayloads)
	{
		ItemPathsToDelete.Add(FContentBrowserItemPath(FolderPayload->GetInternalPath(), EContentBrowserPathType::Internal));
	}

	FContentBrowserModule& ContentBrowserModule = FModuleManager::Get().GetModuleChecked<FContentBrowserModule>(TEXT("ContentBrowser"));
	ContentBrowserModule.GetOnDeleteFolders().Broadcast(ItemPathsToDelete);

	TArray<FString> FoldersToDelete;
	Algo::Transform(ItemPathsToDelete, FoldersToDelete, [](FContentBrowserItemPath ItemPath) { return ItemPath.GetInternalPathString(); });
...
{
	constexpr bool bCheckUniqueName = true;
	return CanRenameItem(InAssetTools, InOwnerDataSource, InItem, bCheckUniqueName, InNewName, OutErrorMsg);
}

bool CanRenameItem(IAssetTools* InAssetTools, const UContentBrowserDataSource* InOwnerDataSource, const FContentBrowserItemData& InItem, const bool bCheckUniqueName, const FString* InNewName, FText* OutErrorMsg)
{
	if (TSharedPtr<const FContentBrowserAssetFolderItemDataPayload> FolderPayload = GetAssetFolderItemPayload(InOwnerDataSource, InItem))
	{
		return CanRenameAssetFolderItem(InAssetTools, *FolderPayload, bCheckUniqueName, InNewName, OutErrorMsg);
	}

	if (TSharedPtr<const FContentBrowserAssetFileItemDataPayload> AssetPayload = GetAssetFileItemPayload(InOwnerDataSource, InItem))
	{
		return CanRenameAssetFileItem(InAssetTools, *AssetPayload, bCheckUniqueName, InNewName, InItem.IsTemporary(), OutErrorMsg);
	}
...
{
	constexpr bool bCheckUniqueName = true;
	return CanRenameAssetFolderItem(InAssetTools, InFolderPayload, bCheckUniqueName, InNewName, OutErrorMsg);
}

bool CanRenameAssetFolderItem(IAssetTools* InAssetTools, const FContentBrowserAssetFolderItemDataPayload& InFolderPayload, const bool bCheckUniqueName, const FString* InNewName, FText* OutErrorMsg)
{
	if (!CanModifyAssetFolderItem(InAssetTools, InFolderPayload, OutErrorMsg))
	{
		return false;
	...
	if (InNewName)
	{
		...
		const bool bAllowExistingFolder = !bCheckUniqueName;
		if (!AssetViewUtils::IsValidFolderPathForCreate(FolderPath, *InNewName, ValidationErrorMsg, bAllowExistingFolder))
		{
...
{
	constexpr bool bCheckUniqueName = true;
	return CanRenameAssetFileItem(InAssetTools, InAssetPayload, bCheckUniqueName, InNewName, InIsTempoarary, OutErrorMsg);
}

bool CanRenameAssetFileItem(IAssetTools* InAssetTools, const FContentBrowserAssetFileItemDataPayload& InAssetPayload, const bool bCheckUniqueName, const FString* InNewName, const bool InIsTempoarary, FText* OutErrorMsg)
{
	if (!CanModifyAssetFileItem(InAssetTools, InAssetPayload, OutErrorMsg))
	{
		return false;
	...
	if (InNewName && (InIsTempoarary || InAssetPayload.GetAssetData().AssetName != FName(**InNewName))) // Deliberately ignore case here to allow case-only renames of existing assets
	{
		...
		FText ValidationErrorMsg;
		const bool bAllowExistingAsset = !bCheckUniqueName;
		if (!AssetViewUtils::IsValidObjectPathForCreate(ObjectPath, ValidationErrorMsg, bAllowExistingAsset))
		{
...
{
	constexpr bool bCheckUniqueName = false;

	if (TSharedPtr<const FContentBrowserAssetFolderItemDataPayload> FolderPayload = GetAssetFolderItemPayload(InOwnerDataSource, InItem))
	{
		if (CanRenameAssetFolderItem(InAssetTools, *FolderPayload, bCheckUniqueName, &InNewName, nullptr))
		{
	...
	if (TSharedPtr<const FContentBrowserAssetFileItemDataPayload> AssetPayload = GetAssetFileItemPayload(InOwnerDataSource, InItem))
	{
		if (CanRenameAssetFileItem(InAssetTools, *AssetPayload, bCheckUniqueName, &InNewName, InItem.IsTemporary(), nullptr))
		{
...
bool RenameAssetFolderItem(IAssetRegistry* InAssetRegistry, const FContentBrowserAssetFolderItemDataPayload& InFolderPayload, const FString& InNewName)
{
	...
	{
		InAssetRegistry->AddPath(NewPath);
		if (AssetViewUtils::RenameFolder(NewPath, OldPath))
		{
			return true;
		}
	}
...
void PopulateAssetFileContextMenu(UContentBrowserDataSource* InOwnerDataSource, UToolMenu* InMenu, FAssetFileContextMenu& InAssetFileContextMenu)
{
	...
	const UContentBrowserAssetContextMenuContext* AssetContextMenu = InMenu->FindContext<UContentBrowserAssetContextMenuContext>();
	checkf(AssetContextMenu, TEXT("Required context UContentBrowserAssetContextMenuContext was missing!"));
	...
	InAssetFileContextMenu.MakeContextMenu(
		InMenu,
		AssetContextMenu->SelectedAssets,
