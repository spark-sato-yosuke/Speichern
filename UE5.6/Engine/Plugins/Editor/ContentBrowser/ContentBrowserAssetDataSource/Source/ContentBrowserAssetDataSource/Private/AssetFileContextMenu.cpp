...
void FAssetFileContextMenu::MakeAssetActionsSubMenu(UToolMenu* Menu)
{
	...
	// ADVANCED ACTIONS
	{
		...
		// Property Matrix
		bool bCanUsePropertyMatrix = bCanBeModified && FModuleManager::LoadModuleChecked<FPropertyEditorModule>( "PropertyEditor" ).GetCanUsePropertyMatrix();
		...
		// Chunk actions
		if (bCanBeModified && GetDefault<UEditorExperimentalSettings>()->bContextMenuChunkAssignments)
		{
			Section.AddMenuEntry(
				"AssignAssetChunk",
		...
		if (FPackageName::DoesPackageExist(SelectedAssets[0].PackageName.ToString(), &PackageFilename))
		{
			FToolMenuSection& Section = Menu->AddSection("LocalizationCache", LOCTEXT("LocalizationCacheHeading", "Localization Cache"));
			{
				...
				Section.AddMenuEntry(
					...
					FSlateIcon(),
					FUIAction(FExecuteAction::CreateSP(this, &FAssetFileContextMenu::ExecuteShowLocalizationCache, SelectedAssets[0], PackageFilename))
					);
			}
		}
	}
...
void FAssetFileContextMenu::ExecuteShowAssetMetaData()
{
	for (const FAssetData& AssetData : SelectedAssets)
	{
		...
		if (Asset)
		{
			TMap<FName, FString>* TagValues = FMetaData::GetMapForObject(Asset);
			if (TagValues)
			{
...
void FAssetFileContextMenu::ExecuteShowLocalizationCache(const FAssetData InAsset, const FString InPackageFilename)
{
	TStringBuilder<2048> LocalizationCacheStr;
	auto AppendLocalizationCache = [&LocalizationCacheStr](const FString& CachedLocalizationId, const TArray<FGatherableTextData>& GatherableTextDataArray)
	{
		// Convert the gathered text array into a readable format
		LocalizationCacheStr.Appendf(TEXT("Package: %s"), *CachedLocalizationId);
		for (const FGatherableTextData& GatherableTextData : GatherableTextDataArray)
		{
			if (LocalizationCacheStr.Len() > 0)
			{
				LocalizationCacheStr += TEXTVIEW("\n\n");
			}

			TStringBuilder<512> KeysStr;
			TStringBuilder<512> EditorOnlyKeysStr;
			for (const FTextSourceSiteContext& TextSourceSiteContext : GatherableTextData.SourceSiteContexts)
			{
				TStringBuilder<512>* KeysStrPtr = TextSourceSiteContext.IsEditorOnly ? &EditorOnlyKeysStr : &KeysStr;
				if (KeysStrPtr->Len() > 0)
				{
					*KeysStrPtr += TEXTVIEW(", ");
				}
			...
			LocalizationCacheStr.Appendf(TEXT("Namespace: %s\n"), *GatherableTextData.NamespaceName);
			if (KeysStr.Len() > 0)
			{
				LocalizationCacheStr.Appendf(TEXT("Keys: %s\n"), *KeysStr);
			}
			if (EditorOnlyKeysStr.Len() > 0)
			{
				LocalizationCacheStr.Appendf(TEXT("Keys (Editor-Only): %s\n"), *EditorOnlyKeysStr);
			}
			LocalizationCacheStr.Appendf(TEXT("Source: %s"), *GatherableTextData.SourceData.SourceString);
		}
	};
	...
	// Read the localization data from the cache in the package header
	{
		...
		LocalizationCacheStr += TEXTVIEW("== On-Disk Cache ==\n\n");
		AppendLocalizationCache(CachedLocalizationId, GatherableTextDataArray);
	}

	// If the asset is loaded, also show the "live" version of the cache (for debugging cache invalidation issues)
	{
		LocalizationCacheStr += TEXTVIEW("\n\n\n\n== In-Memory Cache ==\n\n");

		if (UObject* Asset = InAsset.FastGetAsset())
		{
			const FString CachedLocalizationId = TextNamespaceUtil::GetPackageNamespace(Asset);
			TArray<FGatherableTextData> GatherableTextDataArray;

			EPropertyLocalizationGathererResultFlags GatherableTextResultFlags = EPropertyLocalizationGathererResultFlags::Empty;
			FPropertyLocalizationDataGatherer _(GatherableTextDataArray, Asset->GetPackage(), GatherableTextResultFlags);

			AppendLocalizationCache(CachedLocalizationId, GatherableTextDataArray);
		}
		else
		{
			LocalizationCacheStr += TEXTVIEW("<Asset Not Loaded>");
		}
	}
