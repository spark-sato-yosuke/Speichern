	...
	while (StopTaskCounter.GetValue() == 0)
	{
		FScanPath LocalScanPath;
		{
			...
			if (PendingScanPaths.Num() > 0)
			{
				...
				PendingScanPaths.RemoveAt(0, EAllowShrinking::No);
			}
		}
...
bool UContentBrowserFileDataSource::CreateFolder(const FName InPath, const TSharedPtr<IContentBrowserHideFolderIfEmptyFilter>& HideFolderIfEmptyFilter, FContentBrowserItemDataTemporaryContext& OutPendingItem)
{
	FName InternalPath;
	if (!TryConvertVirtualPathToInternal(InPath, InternalPath))
	{
	...
	OutPendingItem = FContentBrowserItemDataTemporaryContext(
		...
		FContentBrowserItemDataTemporaryContext::FOnValidateItem::CreateLambda([DataSource = TWeakObjectPtr<UContentBrowserFileDataSource>(this), HideFolderIfEmptyFilter](const FContentBrowserItemData& InItem, const FString& InProposedName, FText* OutErrorMsg)
			{
				if (UContentBrowserFileDataSource* DataSourcePtr = DataSource.Get())
				{
...
bool UContentBrowserFileDataSource::IsFolderVisible(const FName InPath, const EContentBrowserIsFolderVisibleFlags InFlags, const FContentBrowserFolderContentsFilter& ContentsFilter)
{
	if (!EnumHasAllFlags(ContentsFilter.ItemCategoryFilter, EContentBrowserItemCategoryFilter::IncludeMisc))
	{
		return false;
	}
	...
	// unless a custom empty folder filter is provided, in which case we use that
	if (EmptyFolders.ContainsByHash(InternalPathHash, InternalPathStrView))
	{
		if (ContentsFilter.HideFolderIfEmptyFilter)
		{
			return !HideFolderIfEmpty(*ContentsFilter.HideFolderIfEmptyFilter, ConvertedPath, InternalPathStrView);
		}
		return false;
	}
	return true;
...
bool UContentBrowserFileDataSource::CanRenameItem(const FContentBrowserItemData& InItem, const FString* InNewName, const IContentBrowserHideFolderIfEmptyFilter* HideFolderIfEmptyFilter, FText* OutErrorMsg)
{
	...
	if (InNewName != nullptr && HideFolderIfEmptyFilter != nullptr)
	{
		if (TSharedPtr<const FContentBrowserFolderItemDataPayload> FolderPayload = GetFolderItemPayload(InItem))
		{
			FString NewInternalPath = FPaths::GetPath(FolderPayload->GetInternalPath().ToString()) / *InNewName;
			const uint32 NewInternalPathHash = GetTypeHash(NewInternalPath);

			if (!AlwaysVisibleFolders.ContainsByHash(NewInternalPathHash, NewInternalPath) &&
				EmptyFolders.ContainsByHash(NewInternalPathHash, NewInternalPath) &&
				HideFolderIfEmpty(*HideFolderIfEmptyFilter, FName(NewInternalPath), NewInternalPath))
			{
	...
	return ContentBrowserFileData::CanRenameItem(this, InItem, bCheckUniqueName, InNewName, OutErrorMsg);
}
...
bool UContentBrowserFileDataSource::CanCopyItem(const FContentBrowserItemData& InItem, const FName InDestPath, FText* OutErrorMsg)
{
	...
	{
		ContentBrowserFileData::SetOptionalErrorMessage(OutErrorMsg, LOCTEXT("CopyError_ItemInsideItself", "Can't copy folder inside itself"));
		return false;
	}
	return true;
}
...
bool UContentBrowserFileDataSource::CanMoveItem(const FContentBrowserItemData& InItem, const FName InDestPath, FText* OutErrorMsg)
{
	...
	if (InDestPath == VirtualPath)
	{
		ContentBrowserFileData::SetOptionalErrorMessage(OutErrorMsg, LOCTEXT("MoveError_ItemInsideItself", "Can't move Item inside itself"));
		return false;
	}
	...
	{
		ContentBrowserFileData::SetOptionalErrorMessage(OutErrorMsg, LOCTEXT("MoveError_ItemInsideSamePath", "Can't move Item inside the same location"));
		return false;
	}
...
bool UContentBrowserFileDataSource::HideFolderIfEmpty(const IContentBrowserHideFolderIfEmptyFilter& HideFolderIfEmptyFilter, FName Path, FStringView PathString) const
{
	check(EmptyFolders.ContainsByHash(GetTypeHash(PathString), PathString));
	...
	if (const FDiscoveredItem* DiscoveredItem = DiscoveredItems.Find(Path))
	{
		checkf(DiscoveredItem->Type == FDiscoveredItem::EType::Directory, TEXT("Empty folders should not contain files."));

		for (const FName ChildItem : DiscoveredItem->ChildItems)
		{
			FNameBuilder ChildPathBuilder(ChildItem);
			if (!HideFolderIfEmpty(HideFolderIfEmptyFilter, ChildItem, ChildPathBuilder))
			{
