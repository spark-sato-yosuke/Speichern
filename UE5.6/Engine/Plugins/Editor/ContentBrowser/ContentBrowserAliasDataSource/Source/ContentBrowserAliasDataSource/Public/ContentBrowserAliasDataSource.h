...
public:
	...
	virtual bool IsFolderVisible(const FName Path, const EContentBrowserIsFolderVisibleFlags Flags, const FContentBrowserFolderContentsFilter& ContentsFilter) override;
	...
	bool HasAliasesForPath(const FSoftObjectPath& InInternalPath) const;
	...
	void SetFilterShouldMatchCollectionContent(bool bInFilterShouldMatchCollectionContent);
...
private:
	...
	struct FAliasData
	{
			...
			: AssetData(InAssetData), PackagePath(InPackagePath), AliasDisplayName(InDisplayName), bIsFromMetaData(bInIsFromMetaData)
		{
			...
			ObjectPath = FSoftObjectPath(FTopLevelAssetPath(PackageName, FName(AssetNameBuilder.ToView())));
		}
