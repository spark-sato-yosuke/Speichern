...
void FCurveEditorMultiScaleTool::OnToolOptionsUpdated(const FPropertyChangedEvent& InPropertyChangedEvent)
{
	...
	TangentFlattener.Reset();
}

FText FCurveEditorMultiScaleTool::GetLabel() const
{
	return FCurveEditorToolCommands::Get().ActivateMultiScaleTool->GetLabel();
}

FText FCurveEditorMultiScaleTool::GetDescription() const
{
	return FCurveEditorToolCommands::Get().ActivateMultiScaleTool->GetDescription();
}

FSlateIcon FCurveEditorMultiScaleTool::GetIcon() const
{
	return FCurveEditorToolCommands::Get().ActivateMultiScaleTool->GetIcon();
}
...
void FCurveEditorMultiScaleTool::OnDragStart()
{
	...
	TangentFlattener.Emplace(*CurveEditor);
...
void FCurveEditorMultiScaleTool::ScaleUnique(const FVector2D& InChangeAmount, const bool bInAffectsX, const bool bInAffectsY)
{
	TSharedPtr<FCurveEditor> CurveEditor = WeakCurveEditor.Pin();
	if (!CurveEditor.IsValid())
	{
	...
	// This recalculates the user specified tangents based on the distance moved. We'll flatten the curves depending on the resulting selection's height.
	if (bInAffectsY)
	{
		const bool bIsMirroring = PercentChanged.Y < 0.0;
		TangentFlattener->ComputeMirroringParallel(*CurveEditor, bIsMirroring);
	}
}
