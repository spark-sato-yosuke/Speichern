...
bool FObjectMixerOutlinerMode::GetIsObjectOfFilteredClass(const UObject* Object) const
{
	if (IsValid(Object))
	{
		for (UClass* Class : GetListModelPtr().Pin()->ObjectClassesToFilterCache)
		{
			if (Object->IsA(Class))
			{
				return true;
...
void FObjectMixerOutlinerMode::OnActorLabelChanged(AActor* ChangedActor)
{
	...
	// Check if this actor is one of the registered types
	if (GetIsObjectOfFilteredClass(ChangedActor))
	{
	...
	else if (const UObjectMixerObjectFilter* Filter = GetListModelPtr().Pin()->GetMainObjectFilterInstance())
	{
		// This actor is likely an associated actor of one or more registered type actors, so we need to force an update for each parent/associated pair
		const TSet<TWeakObjectPtr<AActor>> ApplicableActorsCopy = ApplicableActors; // This set may change while we're iterating, so make a copy
		for (TWeakObjectPtr<AActor> ApplicableActor : ApplicableActorsCopy)
		{
			if (!ApplicableActor.IsValid())
			{
				continue;
			}

			if (Filter->IsActorAssociated(ApplicableActor.Get(), ChangedActor))
			{
				if (FSceneOutlinerTreeItemPtr Item = CreateItemFor<FObjectMixerEditorListRowActor>(
					FObjectMixerEditorListRowActor(ChangedActor, GetSceneOutliner(), FText::GetEmpty(), ApplicableActor.Get()), true))
				{
					SceneOutliner->OnItemLabelChanged(Item);
				}
			}
...
TSharedPtr<SWidget> FObjectMixerOutlinerMode::BuildContextMenu()
{
	...
	if (const UObjectMixerObjectFilter* Filter = GetListModelPtr().Pin()->GetMainObjectFilterInstance())
	{
		Filter->OnContextMenuContextCreated(Context);
	}
...
void FObjectMixerOutlinerMode::OnItemAdded(FSceneOutlinerTreeItemPtr Item)
{
	if (const FActorTreeItem* ActorItem = Item->CastTo<FActorTreeItem>())
	{
		if (!Item->Flags.bIsFilteredOut)
		{
			...
			{
				if (const FObjectMixerEditorListRowActor* ObjectMixerItem = Item->CastTo<FObjectMixerEditorListRowActor>())
				{
					if (ObjectMixerItem->OverrideParent)
					{
						// If there's an OverrideParent, this actor may appear multiple times. We disambiguate the selection
						// based on a GUID that combines the parent and actor itself, but the general editor doesn't. To avoid
						// selecting the wrong one, skip selecting this new item if it wasn't previously selected.
						if (!SelectedIDs.Contains(ObjectMixerItem->GetID()))
						{
							return;
						}
					}
...
FSceneOutlinerDragValidationInfo FObjectMixerOutlinerMode::ValidateDrop(const ISceneOutlinerTreeItem& DropTarget, const FSceneOutlinerDragDropPayload& Payload) const
{
	if (const UObjectMixerObjectFilter* Filter = GetListModelPtr().Pin()->GetMainObjectFilterInstance())
	{
		if (Filter->HasCustomDropHandling(DropTarget))
		{
			return Filter->ValidateDrop(DropTarget, Payload);
		}
	}
...
void FObjectMixerOutlinerMode::OnDrop(ISceneOutlinerTreeItem& DropTarget, const FSceneOutlinerDragDropPayload& Payload, const FSceneOutlinerDragValidationInfo& ValidationInfo) const
{
	if (const UObjectMixerObjectFilter* Filter = GetListModelPtr().Pin()->GetMainObjectFilterInstance())
	{
		if (Filter->HasCustomDropHandling(DropTarget))
		{
			Filter->OnDrop(DropTarget, Payload, ValidationInfo);
			return;
		}
	}
