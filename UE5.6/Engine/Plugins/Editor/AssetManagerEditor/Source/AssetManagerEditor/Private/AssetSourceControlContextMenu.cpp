...
private:
	...
	void FillDiffAgainstSubMenu(UToolMenu* Menu);
...
bool FAssetSourceControlContextMenuState::AddSourceControlMenuOptions(FToolMenuSection& InSection)
{
	...
	else if (SelectedAssets.Num() == 1 && !bContainsFolders)
	{
		InSection.AddSubMenu(
			"DiffAgainst",
			LOCTEXT("DiffAgainst", "Diff Against..."),
			LOCTEXT("DiffAgainstTooltip", "Pick another asset of the same type to compare."),
			FNewToolMenuDelegate::CreateSP(this, &FAssetSourceControlContextMenuState::FillDiffAgainstSubMenu),
			false,
			FSlateIcon(FRevisionControlStyleManager::GetStyleSetName(), "RevisionControl.Actions.Diff")
		);
	}
...
void FAssetSourceControlContextMenuState::FillDiffAgainstSubMenu(UToolMenu* Menu)
{
	...
	AssetPickerConfig.Filter.ClassPaths.Add(SelectedAssets[0].GetClass()->GetClassPathName());
	AssetPickerConfig.OnAssetSelected = FOnAssetSelected::CreateLambda([this](const FAssetData& PickedAsset)
		{
			if (SelectedAssets.Num() == 1)
			{
				UObject* FirstObjectSelected = SelectedAssets[0].GetAsset();
				UObject* SecondObjectSelected = PickedAsset.GetAsset();

				if ((FirstObjectSelected != NULL) && (SecondObjectSelected != NULL))
				{
					if (FirstObjectSelected == SecondObjectSelected)
					{
						FMessageDialog::Open(EAppMsgType::Ok, LOCTEXT("SourceControlDiffSameAssets", "Can't diff an asset against itself."));
					}
					else
					{
						...
						// Need to dismiss context menu first or the diff window will try to attach to it
	...
	FToolMenuSection& Section = Menu->AddSection("AssetDiffAgainstPicker", LOCTEXT("AssetDiffAgainstPickerHeading", "Diff Against..."));

	Section.AddEntry(FToolMenuEntry::InitWidget("DiffAgainst", AssetPicker, {}));
}
