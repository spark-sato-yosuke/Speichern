...
public:
	...
	virtual bool WriteCollection(ICollectionContainer& CollectionContainer, FName CollectionName, ECollectionShareType::Type ShareType, const TArray<FName>& PackageNames, bool bShowFeedback) override;
...
private:
	...
	bool CreateOrEmptyCollection(ICollectionContainer& CollectionContainer, FName CollectionName, ECollectionShareType::Type ShareType, FText& OutError);
...
void FAssetManagerEditorModule::OpenReferenceViewerTab(const TArray<FAssetIdentifier>& InAssetIdentifiers, const FReferenceViewerParams& ReferenceViewerParams)
{
	...
	if (!ReferenceViewerDefaultTab.IsValid())
	{
		if (!AssetsHashToTabID.IsEmpty())
		{
			// Let's get the last opened tab, so we use it as reference for where new tabs should go
			TArray<FName> TabNames;
			AssetsHashToTabID.GenerateValueArray(TabNames);
			TabNames.Sort(
			...
			ReferenceViewerDefaultTab = ReferenceViewerTabs.FindRef(TabNames.Last()).Pin();
		}
		else
		{
			// There's no Reference Viewer Tab, let's retrieve the default one
	...
	if (!CurrentAssetTab)
	{
		...
		// Get reference Tab name, used to insert the new tab at its location
		const FName PlacementRefTabName = ReferenceViewerDefaultTab
											? ReferenceViewerDefaultTab->GetLayoutIdentifier().TabType
		...
		FGlobalTabmanager::Get()->InsertNewDocumentTab(
			PlacementRefTabName,
			TabID,
			FTabManager::FLastMajorOrNomadTab(PlacementRefTabName),
...
{
	UAssetManager::Get().UpdateManagementDatabase(EUpdateManagementDatabaseFlags::BuildChunkMap | EUpdateManagementDatabaseFlags::ForceRefresh);
...
void FAssetManagerEditorModule::CopyPackagePaths(const TArray<FAssetData> SelectedPackages)
{
	...
	FString ClipboardText = FString::JoinBy(SortedItems, LINE_TERMINATOR, [](const FAssetData& Item)
		{
			FString ItemFilename = FPackageName::LongPackageNameToFilename(Item.PackageName.ToString(), FPackageName::GetAssetPackageExtension());
...
bool FAssetManagerEditorModule::CreateOrEmptyCollection(ICollectionContainer& CollectionContainer, FName CollectionName, ECollectionShareType::Type ShareType, FText& OutError)
{
	if (CollectionContainer.CollectionExists(CollectionName, ShareType))
	{
		return CollectionContainer.EmptyCollection(CollectionName, ShareType, &OutError);
	}
	else if (CollectionContainer.CreateCollection(CollectionName, ShareType, ECollectionStorageMode::Static, &OutError))
	{
		return true;
	}
...
{
	return WriteCollection(*FCollectionManagerModule::GetModule().Get().GetProjectCollectionContainer(), CollectionName, ShareType, PackageNames, bShowFeedback);
}

bool FAssetManagerEditorModule::WriteCollection(ICollectionContainer& CollectionContainer, FName CollectionName, ECollectionShareType::Type ShareType, const TArray<FName>& PackageNames, bool bShowFeedback)
{
	...
	else if (CreateOrEmptyCollection(CollectionContainer, CollectionName, ShareType, Error))
	{
		if (CollectionContainer.AddToCollection(
				CollectionName, ShareType, ObjectPathsToAddToCollection.Array(), nullptr, &Error))
		{
