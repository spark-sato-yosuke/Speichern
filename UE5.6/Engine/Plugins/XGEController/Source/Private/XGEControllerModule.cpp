...
#ifdef _M_ARM64EC
	#define XGE_CONTROL_WORKER_FILENAME	TEXT("XGEControlWorkerarm64ec.exe")
#elif PLATFORM_CPU_ARM_FAMILY
	#define XGE_CONTROL_WORKER_FILENAME	TEXT("XGEControlWorkerarm64.exe")
#else
...
{
	return FPaths::EngineDir() / TEXT("Binaries/Win64/") / XGE_CONTROL_WORKER_FILENAME;
}
...
namespace XGEControllerVariables
{
	...
	FAutoConsoleVariableRef CVarXGEControllerAvoidUsingLocalMachine(
		...
		FConsoleVariableDelegate::CreateLambda(
			[](IConsoleVariable* /*CVar*/) -> void
			{
				// If XGE has changed its support for local workers, notify shader compiling manager in case local shader compiling thread must be spun up.
				if (GShaderCompilingManager)
				{
					GShaderCompilingManager->OnDistributedShaderCompilingChanged();
				}
			}
		),
...
bool FXGEControllerModule::SupportsLocalWorkers()
{
	// XGE supports local workers if it's not explicitly disabled
	const bool bAvoidLocalWorkers = XGEController::AvoidUsingLocalMachine();
	return !bAvoidLocalWorkers;
}
...
void FXGEControllerModule::ReadBackThreadProc()
{
	while (!bShutdown && !bRestartWorker)
	{
		...
		else
		{
			...
			Task->Finalize(CompletedTaskResponse.ReturnCode);
			delete Task;
		}
