...
{
	TSharedPtr<SLayeredImage> ViewOptionsImage;
	
	TSharedRef<SWidget> Result = SNew(SVerticalBox)
		...
		[
			...
			[
				...
				.ButtonContent()
				[
					SAssignNew(ViewOptionsImage, SLayeredImage)
					.Image(FAppStyle::Get().GetBrush("DetailsView.ViewOptions"))
					.ColorAndOpacity(FSlateColor::UseForeground())
				]
			]
	...
	ViewOptionsImage->AddLayer(TAttribute<const FSlateBrush*>(this, &SNiagaraStack::GetViewOptionsBadgeIcon));
...
TSharedRef<SWidget> SNiagaraStack::GetViewOptionsMenu() const
{
	...
	MenuBuilder.AddMenuEntry(
		LOCTEXT("ShowOnlyModifiedLabel", "Show Only Modified Properties"),
		LOCTEXT("ShowOnlyModifiedToolTip", "Displays only properties that have been changes from their default."),
		FSlateIcon(),
		FUIAction(
			FExecuteAction::CreateLambda([this]() { StackViewModel->SetShowOnlyModified(!StackViewModel->GetShowOnlyModified()); }),
			FCanExecuteAction(),
			FGetActionCheckState::CreateLambda([this]() { return StackViewModel->GetShowOnlyModified() ? ECheckBoxState::Checked : ECheckBoxState::Unchecked; })),
	...
	MenuBuilder.AddMenuEntry(
		...
		LOCTEXT("ShowIssuesToolTip", "Hides all modules except those that have unresolved issues. This action is only enabled when system and emitter nodes are selected in the graph, as it could potentially hide any selected modules."),
		FSlateIcon(),
		FUIAction(
			...
			FCanExecuteAction::CreateLambda([this]()
			{
				...
				{
					if (UNiagaraSystemSelectionViewModel* SelectionViewModel = RootEntry->GetSystemViewModel()->GetSelectionViewModel())
					{
						// only allow the user to toggle this option when a system or emitter node is selected
						TArray<UNiagaraStackEntry*> SelectedEntries;
						SelectionViewModel->GetSelectedEntries(SelectedEntries);
						for (UNiagaraStackEntry* SelectedEntry : SelectedEntries)
						{
							if (!SelectedEntry->IsA<UNiagaraStackRoot>())
							{
								return false;
							}
						}
...
SNiagaraStack::FRowWidgets SNiagaraStack::ConstructNameAndValueWidgetsForItem(UNiagaraStackEntry* Item, TSharedRef<SNiagaraStackTableRow> Container)
{
	...
	else if (Item->IsA<UNiagaraStackPropertyRow>())
	{
		...
		{
			TSharedPtr<const FNiagaraStackObjectPropertyCustomization> Customization = PropertyRow->GetCustomization();
			if(Customization)
			{
				TOptional<TSharedPtr<SWidget>> CustomizedNameWidget = Customization->GenerateNameWidget(PropertyRow);

				if(CustomizedNameWidget.IsSet())
				{
					PropertyRowWidgets.NameWidget = CustomizedNameWidget.GetValue();
				}
			}
...
const FSlateBrush* SNiagaraStack::GetViewOptionsBadgeIcon() const
{
	// Badge the icon if any view option that narrows down the results is checked
	bool bHasBadge = (StackViewModel->GetShowOnlyModified() || StackViewModel->GetShowOnlyIssues()
		|| StackViewModel->GetShowAllAdvanced() || StackViewModel->GetShowLinkedInputs()
		|| StackViewModel->GetShowOutputs());
