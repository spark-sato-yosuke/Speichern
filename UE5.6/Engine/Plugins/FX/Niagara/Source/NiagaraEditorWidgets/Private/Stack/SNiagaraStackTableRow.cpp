...
FReply SNiagaraStackTableRow::OnMouseButtonUp(const FGeometry& MyGeometry, const FPointerEvent& MouseEvent)
{
	if (MouseEvent.GetEffectingButton() == EKeys::RightMouseButton)
	{
		...
		FNiagaraStackEditorWidgetsUtilities::AddStackNoteContextMenuActions(MenuBuilder, *StackEntry);
		
		if(StackEntry->GetEmitterViewModel().IsValid() && StackEntry->GetEmitterViewModel())
		{			
			if(StackEntry->SupportsSummaryView())
			{
				FHierarchyElementIdentity Identity = StackEntry->DetermineSummaryIdentity();
				if(Identity.IsValid())
				{
...
void SNiagaraStackTableRow::ToggleShowInSummaryView() const
{
	if(StackEntry->GetEmitterViewModel().IsValid())
	{
		...
		if(Identity.IsValid())
		{
			...
			{
				StackEntry->GetEmitterViewModel()->GetSummaryHierarchyViewModel()->DeleteElementWithIdentity(Identity);
			}
			else
			{
				UClass* HierarchyClass = DetermineHierarchyClassForSummaryView();
				check(HierarchyClass);
				StackEntry->GetEmitterViewModel()->GetSummaryHierarchyViewModel()->AddElementUnderRoot(HierarchyClass, Identity);
			}
		}
	}
}
...
void SNiagaraStackTableRow::NavigateToSummaryView() const
{
	...
	StackEntry->GetEmitterViewModel()->GetSummaryHierarchyViewModel()->NavigateToElementInHierarchy(StackEntry->DetermineSummaryIdentity());
}

TSubclassOf<UHierarchyElement> SNiagaraStackTableRow::DetermineHierarchyClassForSummaryView() const
{
