		...
		[
			...
			[
				...
				[
	                ...
	                {
	                	return FNiagaraEditorUtilities::GetWeightForItem(Item, INiagaraStackItemGroupAddAction::FavoriteActionsProfile, FilterTerms);
	                })
					...
					.OnItemRowHoverEvent(this, &SNiagaraStackItemGroupAddMenu::OnItemRowHoverEvent)
...
TArray<TSharedPtr<FNiagaraMenuAction_Generic>> SNiagaraStackItemGroupAddMenu::CollectActions()
{
	...
	// We translate group add actions into the generic menu actions here
	
	TArray<TSharedRef<INiagaraStackItemGroupAddAction>> AddActions;
	AddUtilities->GenerateAddActions(AddActions, AddOptions);

	// For each group action, we parse its data and match data formats to the generic action
	...
	{		
		TSharedPtr<FNiagaraMenuAction_Generic> NewNodeAction(
            ...
            AddAction->GetDisplayName(), AddAction->GetCategories(), AddAction->GetFavoritesData(), AddAction->GetDescription(), AddAction->GetKeywords()));
...
void SNiagaraStackItemGroupAddMenu::OnItemRowHoverEvent(const TSharedPtr<FNiagaraMenuAction_Generic>& Item, bool bHover) const
{
	Item->bIsHovered = bHover;
}
	...
	const TSharedPtr<FNiagaraMenuAction_Generic>& Item)
{
	...
	{
		FNiagaraFavoriteActionsProfile& ActionsProfile = UNiagaraFavoriteActionsConfig::Get()->GetActionsProfile(INiagaraStackItemGroupAddAction::FavoriteActionsProfile);
	...
	const TSharedPtr<FNiagaraMenuAction_Generic>& Item)
{
	...
	ActionData.FavoriteActionsProfileName = INiagaraStackItemGroupAddAction::FavoriteActionsProfile;
