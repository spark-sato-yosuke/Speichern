...
public:
	...
	void GetFilteredChildrenOfType(TArray<T*>& OutFilteredChildrenOfType, bool bRecursive = false) const
	{
		...
		{
			T* FilteredChildOfType = Cast<T>(FilteredChild);
			if (FilteredChildOfType != nullptr)
			{
				OutFilteredChildrenOfType.Add(FilteredChildOfType);
			}
		}
	...
	/** Same as GetSystemViewModel but can return a nullptr. */
	NIAGARAEDITOR_API TSharedPtr<FNiagaraSystemViewModel> GetSystemViewModelPtr() const;
	...
	NIAGARAEDITOR_API virtual bool SupportsStackNotes() const { return false; }
	...
	NIAGARAEDITOR_API void AddStackNote();
	NIAGARAEDITOR_API void ToggleStackNoteInlineDisplay();
	NIAGARAEDITOR_API void CopyStackNote();
	NIAGARAEDITOR_API void PasteStackNote();
	
	virtual bool SupportsSummaryView() const { return false; }
	NIAGARAEDITOR_API virtual struct FHierarchyElementIdentity DetermineSummaryIdentity() const;
...
protected:
	...
	NIAGARAEDITOR_API void SetIndentLevelOverride(int32 IndentLevelOverride);
