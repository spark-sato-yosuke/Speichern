...
namespace FNiagaraEditorUtilities
{
	...
	*/
	...
	NIAGARAEDITOR_API int32 GetWeightForItem(const TSharedPtr<FNiagaraMenuAction_Generic>& Item, FName FavoriteActionsProfile, const TArray<FString>& FilterTerms);
	...
	NIAGARAEDITOR_API void GetAvailableParameterDefinitions(TArray<UNiagaraParameterDefinitions*>& OutParameterDefinitions);
	...
	NIAGARAEDITOR_API EAxisList::Type VectorComponentToAxis(int32 NumComponents, int32 ComponentIndex);
	...
	{		
		namespace Scripts
		{
			TSharedRef<SWidget> GenerateRowContentForScriptParameterHierarchyEditor(TSharedRef<FHierarchyElementViewModel> HierarchyItem);
		}
	}
}
