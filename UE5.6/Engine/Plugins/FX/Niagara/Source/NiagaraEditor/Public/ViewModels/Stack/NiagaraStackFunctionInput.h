...
public:
	...
	struct NIAGARAEDITOR_API FNiagaraAvailableParameterInfo
	{
		FNiagaraAvailableParameterInfo(FNiagaraVariableBase InVariableBase);
		FNiagaraAvailableParameterInfo(FNiagaraVariableBase InVariableBase, UNiagaraScript* InConversionScript);

		/** The variable this info struct is about. */
		FNiagaraVariableBase Variable;
		/** If this is set, the implication is that the variable can only be applied to this stack function input through conversion. */
		UNiagaraScript* ConversionScript = nullptr;
		/** If this variable is defined in a parameter definition asset, the data is included here for further processing. */
		UNiagaraScriptVariable* MatchingParameterDefinition = nullptr;

		/** We use the variable as identifier so we can use array operations such as Contains or Find based on just the variable. */
		bool operator==(const FNiagaraAvailableParameterInfo& Other) const
		{
...
public:
	...
	NIAGARAEDITOR_API const FNiagaraVariableBase& GetLinkedParameterValue() const;

	/** Sets the value of this input to a linked parameter handle. */
	NIAGARAEDITOR_API void SetLinkedParameterValue(const FNiagaraVariableBase& InParameter);

	/** Sets the value of this input to a linked parameter handle using a conversion script. */
	NIAGARAEDITOR_API void SetLinkedParameterValueViaConversionScript(const FNiagaraVariableBase& InParameter, UNiagaraScript& ConversionScript);
	...
	struct NIAGARAEDITOR_API FGetAvailableParameterArgs
	{
		bool bIncludeConversionScripts = true;
		bool bIncludeParameterDefinitions = false;
	};
	
	NIAGARAEDITOR_API void GetAvailableParameters(TSet<FNiagaraAvailableParameterInfo>& OutAvailableParameters, FGetAvailableParameterArgs Arguments) const;
	...
	NIAGARAEDITOR_API bool HasAnyResettableChildrenInputs() const;
...
private:
	...
	NIAGARAEDITOR_API void GetDefaultLinkedParameterOrLinkedFunctionFromDefaultPin(UEdGraphPin* DefaultPin, UNiagaraStackFunctionInput::FInputValues& InInputValues) const;
...
FORCEINLINE uint32 GetTypeHash(const UNiagaraStackFunctionInput::FNiagaraAvailableParameterInfo& ParameterInfo)
{
	return GetTypeHash(ParameterInfo.Variable);
}
