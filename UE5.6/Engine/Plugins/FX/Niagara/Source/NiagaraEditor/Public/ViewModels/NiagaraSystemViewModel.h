...
struct FNiagaraSystemViewModelOptions
{
	...
	/**
	Should we compile the system for edit or not, this is only relevant if it's allowed.
	For example when bIsForDataProcessingOnly is true we won't allow edit mode.
	*/
	bool bCompileForEdit = true;
};

struct FNiagaraStackModuleData
{
	TWeakObjectPtr<UNiagaraNodeFunctionCall> WeakModuleNode;
...
public:
	...
	/** Update the compile for edit mode. */
	void SetCompileForEdit(bool bNewState);
	/** Are we running in compile for edit mode or not. */
	bool IsCompileForEdit() const { return bIsCompileForEdit; }
	...
	/** Returns what level of performance stats we have available in the editor. */
	NIAGARAEDITOR_API ENiagaraPerformanceStateScopeMode GetPerformanceStateScopeMode() const;

	/** Are the performance stats enabled? */
	bool IsPerformanceStatsEnabled() const { return bPerformanceStatsEnabled; }
	/** Set if performance stats should be enabled or not */
	void SetPerformanceStatsEnabled(bool bEnabled) { bPerformanceStatsEnabled = bEnabled; }
	...
	void SetSystenIsolationEnabled(bool bIsolationEnabled);

	/** Enable the supplied emitters. */
	NIAGARAEDITOR_API void EnableEmitters(TArray<FGuid> EmitterHandlesIdsToEnable);
...
private:
	...
	/** What mode compile for edit is currently in. */
	bool bIsCompileForEdit;
	...
	/** If performance stats should be enable in the editor or not. */
