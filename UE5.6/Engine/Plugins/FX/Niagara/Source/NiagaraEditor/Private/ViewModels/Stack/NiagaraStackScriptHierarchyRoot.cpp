...
const TArray<UHierarchySection*>& UNiagaraStackScriptHierarchyRoot::GetSections() const
{
...
const UHierarchySection* UNiagaraStackScriptHierarchyRoot::GetActiveSection() const
{
	return ActiveSection.IsValid() ? ActiveSection.Get() : nullptr;
}

void UNiagaraStackScriptHierarchyRoot::SetActiveSection(const UHierarchySection* InActiveSection)
{
...
void UNiagaraStackScriptHierarchyRoot::RefreshChildrenInternal(const TArray<UNiagaraStackEntry*>& CurrentChildren, TArray<UNiagaraStackEntry*>& NewChildren, TArray<FStackIssue>& NewIssues)
{
	...
	for(const UHierarchyElement* ChildHierarchyItem : GetScriptParameterHierarchyRoot()->GetChildren())
	{
		...
		if(const UHierarchyCategory* HierarchyCategory = Cast<UHierarchyCategory>(ChildHierarchyItem))
		{
...
{
	UHierarchySection* const* FoundSection = GetSections().FindByPredicate([SectionName](UHierarchySection* SectionCandidate)
	{
...
bool UNiagaraStackScriptHierarchyRoot::FilterByActiveSection(const UNiagaraStackEntry& Child) const
{
	...
	{
		if(StackHierarchyCategory->GetHierarchyCategory()->GetSection() == ActiveSection)
		{
			return true;
		}
	}
