...
void UNiagaraStackSummaryViewCollection::Initialize(FRequiredEntryData InRequiredEntryData,FVersionedNiagaraEmitterWeakPtr InEmitter, FString InOwningStackItemEditorDataKey)
{
	...
	AddChildFilter(FOnFilterChild::CreateUObject(this, &UNiagaraStackSummaryViewCollection::FilterOnlyModified));
}
...
{	
	...
	const UHierarchyRoot* Root = ViewModel->GetEditorData().GetSummaryRoot();
	TSharedPtr<FHierarchyRootViewModel> RootViewModel = ViewModel->GetSummaryHierarchyViewModel()->GetHierarchyRootViewModel();
	...
	RootViewModel->GetChildrenViewModelsForType<UHierarchyElement, FHierarchyElementViewModel>(ChildrenViewModels, false);

	TArray<TSharedPtr<FHierarchyElementViewModel>> AllChildrenViewModels;
	RootViewModel->GetChildrenViewModelsForType<UHierarchyElement, FHierarchyElementViewModel>(AllChildrenViewModels, true);
...
{
	const TArray<UHierarchySection*>& HierarchySections = GetHierarchySections();

	TMap<const UHierarchySection*, TArray<UHierarchyCategory*>> SectionToCategoryMap;

	TSharedPtr<FNiagaraEmitterViewModel> ViewModel = GetEmitterViewModel();
	const UHierarchyRoot* Root = ViewModel->GetEditorData().GetSummaryRoot();
	
	TArray<UHierarchyCategory*> HierarchyCategories;
	Root->GetChildrenOfType<UHierarchyCategory>(HierarchyCategories, false);
	
	for(UHierarchyCategory* TopLevelHierarchyCategories : HierarchyCategories)
	{
		SectionToCategoryMap.FindOrAdd(TopLevelHierarchyCategories->GetSection()).Add(TopLevelHierarchyCategories);
	}
	
	for(const UHierarchySection* HierarchySection : HierarchySections)
	{
		...
		{
			for(UHierarchyCategory* Category : SectionToCategoryMap[HierarchySection])
			{
...
const TArray<UHierarchySection*>& UNiagaraStackSummaryViewCollection::GetHierarchySections() const
{
	if (GetEmitterViewModel().IsValid())
	{
	...
	static TArray<UHierarchySection*> Dummy;
	return Dummy;
}
...
bool UNiagaraStackSummaryViewCollection::FilterOnlyModified(const UNiagaraStackEntry& Child) const
{
