...
const FName INiagaraStackItemGroupAddAction::FavoriteActionsProfile = FName("StackGroupAdd");
...
public:
	static TSharedRef<FScriptGroupAddAction> CreateAssetModuleAction(FAssetData AssetData)
	{
		...
		FavoritesActionData.ActionIdentifier.Names.Add(FName(AssetData.GetObjectPathString()));
		...
		return MakeShareable(new FScriptGroupAddAction(DisplayName, {Category.ToString()}, Description, Keywords, FavoritesActionData, bIsInLibrary, FNiagaraVariable(), false, AssetData, nullptr, false, false, SourceData));
	}
	...
	static TSharedRef<FScriptGroupAddAction> CreateModuleActionFromScratchPadScript(UNiagaraScript* ModuleScript)
	{
		...
		FNiagaraFavoritesActionData FavoritesActionData;
		FavoritesActionData.ActionIdentifier.Names.Add(FName(GetPathNameSafe(ModuleScript)));
		...
		return MakeShareable(new FScriptGroupAddAction(DisplayName, {Category.ToString()}, Description, Keywords, FavoritesActionData, bIsInLibrary, FNiagaraVariable(), false, FAssetData(), ModuleScript, false, false, SourceData));
	}
	...
	static TSharedRef<FScriptGroupAddAction> CreateExistingParameterModuleAction(FNiagaraVariable ParameterVariable)
	{
		...
		return MakeShareable(new FScriptGroupAddAction(DisplayName, {Category.ToString()}, Description, FText(), {}, true, ParameterVariable, false, FAssetData(), nullptr, false, false));
	}
	...
	static TSharedRef<FScriptGroupAddAction> CreateNewParameterModuleAction(FName NewParameterNamespace, FNiagaraTypeDefinition NewParameterType)
	{
		...
		return MakeShareable(new FScriptGroupAddAction( DisplayName, {Category.ToString()}, Description, FText(), {}, true, NewParameter, true, FAssetData(), nullptr, false, false));
	}
	...
	static TSharedRef<FScriptGroupAddAction> CreateNewScratchModuleAction()
	{
		...
		return MakeShareable(new FScriptGroupAddAction( DisplayName, {}, Description, FText(), FavoritesActionData, true, FNiagaraVariable(), false, FAssetData(), nullptr, true, false));
	}
	...
	static TSharedRef<FScriptGroupAddAction> CreateNewSetSpecificModuleAction()
	{
		...
		return MakeShareable(new FScriptGroupAddAction( DisplayName, {}, Description, FText(), FavoritesActionData, true, FNiagaraVariable(), false, FAssetData(), nullptr, false, true));
	}

	static TSharedRef<FScriptGroupAddAction> CreateWizardAction(const FModuleWizardGenerator::FAction& Action)
	{
		FNiagaraFavoritesActionData FavoritesActionData;
		FavoritesActionData.ActionIdentifier.Names.Add(Action.WizardModel->GetIdentifier());
		FavoritesActionData.bFavoriteByDefault = Action.bSuggestedAction;
		
		TSharedRef<FScriptGroupAddAction> AddAction = MakeShareable(new FScriptGroupAddAction( Action.DisplayName, {}, Action.Description, Action.Keywords, FavoritesActionData, true, FNiagaraVariable(), false, FAssetData(), nullptr, false, true));
	...
	virtual TOptional<FNiagaraFavoritesActionData> GetFavoritesData() const override
	{
...
private:
	FScriptGroupAddAction(FText InDisplayName, TArray<FString> InCategories, FText InDescription, FText InKeywords, TOptional<FNiagaraFavoritesActionData> InFavoritesData, bool bInIsInLibrary,
	...
	FText Description;
	FText Keywords;
	TOptional<FNiagaraFavoritesActionData> FavoritesData;
	bool bIsInLibrary;
...
void RenameInputsFromClipboard(TMap<FName, FName> OldModuleOutputNameToNewModuleOutputNameMap, UObject* InOuter, const TArray<const UNiagaraClipboardFunctionInput*>& InSourceInputs, TArray<const UNiagaraClipboardFunctionInput*>& OutRenamedInputs)
{
	for (const UNiagaraClipboardFunctionInput* SourceInput : InSourceInputs)
	{
		TOptional<bool> bEditConditionValue = SourceInput->bHasEditCondition 
			...
			: TOptional<bool>();
		if (SourceInput->ValueMode == ENiagaraClipboardFunctionInputValueMode::Linked && OldModuleOutputNameToNewModuleOutputNameMap.Contains(SourceInput->Linked.GetName()))
		{
			FName RenamedLinkedValue = OldModuleOutputNameToNewModuleOutputNameMap[SourceInput->Linked.GetName()];
			FNiagaraVariableBase RenamedLinkedParameter = FNiagaraVariableBase(SourceInput->Linked.GetType(), RenamedLinkedValue);
			OutRenamedInputs.Add(UNiagaraClipboardFunctionInput::CreateLinkedValue(InOuter, SourceInput->InputName, SourceInput->InputType, bEditConditionValue, RenamedLinkedParameter));
		}
