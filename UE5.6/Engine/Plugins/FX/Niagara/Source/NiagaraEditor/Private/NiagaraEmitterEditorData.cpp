...
UNiagaraEmitterEditorData::UNiagaraEmitterEditorData(const FObjectInitializer& ObjectInitializer)
{
	...
	SummaryViewRoot = CreateDefaultSubobject<UHierarchyRoot>(TEXT("SummaryViewRoot"));
...
void UNiagaraEmitterEditorData::PostLoad_TransferSummaryDataToNewFormat()
{
	...
	auto GenerateStableIdentityForCategories = [](UHierarchyCategory* Category) -> TArray<FName>
	{
		TArray<FName> CategoryNameChain;
		
		const UHierarchyCategory* OuterCategory = Category;

		CategoryNameChain.Add(Category->GetCategoryName());

		for (; OuterCategory != nullptr; OuterCategory = OuterCategory->GetTypedOuter<UHierarchyCategory>() )
		{
	...
	TMap<FName, UHierarchySection*> Sections;
	...
	if(SummarySections_DEPRECATED.Num() > 0)
	{
		for(FNiagaraStackSection& StackSection : SummarySections_DEPRECATED)
		{
			...
			UHierarchySection* Section = SummaryViewRoot->AddSection(SectionDisplayName);
			...
			for(const FText& CategoryIdentity : StackSection.Categories)
			{
				...
				UHierarchyCategory* HierarchyCategory = SummaryViewRoot->AddChild<UHierarchyCategory>();
	...
	if(SummaryViewFunctionInputMetadata_DEPRECATED.Num() > 0)
	{
		...
		TMap<UHierarchyElement*, int32> SummarySortOrder;
		// the original metadata defined in the script asset for a given input.
		TMap<FHierarchyElementIdentity, FNiagaraVariableMetaData> AssetVariableMetadata;
		...
		for(auto It = SummaryViewFunctionInputMetadata_DEPRECATED.CreateConstIterator(); It; ++It)
		{
			...
			FHierarchyElementIdentity ParentInputIdentity;
			TArray<FHierarchyElementIdentity> ChildInputIdentities;
			...
			{
				Category = SummaryViewRoot->AddChild<UHierarchyCategory>();
			...
			if(ParentInputIdentity.Guids.Num() == 2)
			{
				...
				for(const FHierarchyElementIdentity& ChildInputIdentity : ChildInputIdentities)
				{
		...
		for(UHierarchyCategory* Category : TopLevelCategories)
		{
			...
			if(Category->GetChildren().Num() > 0)
			{
				...
				UHierarchyElement* MinimumItem = Category->GetChildren()[0];
		...
		auto SortCategoriesPredicate = ([&](UHierarchyElement& ItemA, UHierarchyElement& ItemB)
		{
		...
		{
			FHierarchyElementIdentity StableIdentity;
...
void UNiagaraEmitterEditorData::PostLoad()
{
	...
	{
		SummaryViewRoot = NewObject<UHierarchyRoot>(this, TEXT("SummaryViewRoot"), RF_Transactional);
	}
...
const TArray<UHierarchySection*>& UNiagaraEmitterEditorData::GetSummarySections() const
{
