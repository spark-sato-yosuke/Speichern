...
DECLARE_CYCLE_STAT(TEXT("Niagara - StackGraphUtilities - GetStackFunctionInputs"), STAT_NiagaraEditor_StackGraphUtilities_GetStackFunctionInputs, STATGROUP_NiagaraEditor);
DECLARE_CYCLE_STAT(TEXT("Niagara - StackGraphUtilities - GetStackFunctionStaticSwitchPins"), STAT_NiagaraEditor_StackGraphUtilities_GetStackFunctionStaticSwitchPins, STATGROUP_NiagaraEditor);
...
{
	TRACE_CPUPROFILER_EVENT_SCOPE_ON_CHANNEL(FNiagaraStackGraphUtilities::BuildParameterMapHistoryWithStackContextResolution, NiagaraChannel);
...
void FNiagaraStackGraphUtilities::GetStackFunctionInputsLegacy(const UNiagaraNodeFunctionCall& FunctionCallNode, TArray<FNiagaraVariable>& OutInputVariables, ENiagaraGetStackFunctionInputPinsOptions Options /*= ENiagaraGetStackFunctionInputPinsOptions::AllInputs*/, bool bIgnoreDisabled /*= false*/)
{
	FCompileConstantResolver EmptyResolver;
	GetStackFunctionInputsLegacy(FunctionCallNode, OutInputVariables, EmptyResolver, Options, bIgnoreDisabled);
}

namespace UE::Niagara::TraversalCache::StackGraphUtilities
{
	void ConstantResolverToTopLevelScriptContext(const FCompileConstantResolver& ConstantResolver, const UNiagaraNodeFunctionCall& FunctionCallNode, FTopLevelScriptStaticContext& OutTopLevelScriptContext)
	{
		const UNiagaraSystem* System = nullptr;
		const FVersionedNiagaraEmitterData* EmitterData = nullptr;
		const UNiagaraScript* Script = nullptr;

		// Get the system.
		if (ConstantResolver.GetSystem() != nullptr)
		{
			System = ConstantResolver.GetSystem();
		}
		else if (ConstantResolver.GetEmitter() != nullptr)
		{
			System = ConstantResolver.GetEmitter()->GetTypedOuter<UNiagaraSystem>();
		}
		else
		{
			System = FunctionCallNode.GetTypedOuter<UNiagaraSystem>();
		}
		...
		// Get the emitter if available
		bool bIsEmitterOrParticleScript = UNiagaraScript::IsEmitterScript(ConstantResolver.GetUsage()) || UNiagaraScript::IsParticleScript(ConstantResolver.GetUsage());
		if (bIsEmitterOrParticleScript)
		{
			if (ConstantResolver.GetEmitter() != nullptr)
			{
				EmitterData = ConstantResolver.GetEmitter()->GetLatestEmitterData();
			}
			else
			{
				const UNiagaraEmitter* Emitter = FunctionCallNode.GetTypedOuter<UNiagaraEmitter>();
				EmitterData = Emitter->GetLatestEmitterData();
			}
		}

		if (bIsEmitterOrParticleScript && EmitterData == nullptr)
		{
			return;
		}

		// Get the script.
		if (UNiagaraScript::IsSystemScript(ConstantResolver.GetUsage()))
		{
			Script = ConstantResolver.GetUsage() == ENiagaraScriptUsage::SystemSpawnScript
				? System->GetSystemSpawnScript()
				: System->GetSystemUpdateScript();
		}
		else if(bIsEmitterOrParticleScript)
		{
			const UNiagaraNodeOutput* OutputNode = FNiagaraStackGraphUtilities::GetEmitterOutputNodeForStackNode(FunctionCallNode);
			Script = OutputNode != nullptr ? EmitterData->GetScript(OutputNode->GetUsage(), OutputNode->GetUsageId()) : nullptr;
		}
		...
		FStaticContextBuilder::CreateTopLevelScriptContext(*System, EmitterData, *Script, OutTopLevelScriptContext);
	}

	EStackFunctionReadFilterFlags InputOptionsToReadFilterFlags(FNiagaraStackGraphUtilities::ENiagaraGetStackFunctionInputPinsOptions Options, bool bIgnoreDisabled)
	{
		bool bInputsOnly = Options == FNiagaraStackGraphUtilities::ENiagaraGetStackFunctionInputPinsOptions::ModuleInputsOnly;
		return 
			(bInputsOnly ? EStackFunctionReadFilterFlags::InputsOnly : EStackFunctionReadFilterFlags::None) |
			(bIgnoreDisabled ? EStackFunctionReadFilterFlags::EnabledOnly : EStackFunctionReadFilterFlags::None);
	}
}
...
{
	SCOPE_CYCLE_COUNTER(STAT_NiagaraEditor_StackGraphUtilities_GetStackFunctionInputs);
	if (GbEnableTraversalCache == 0)
	{
		GetStackFunctionInputsLegacy(FunctionCallNode, OutInputVariables, OutHiddenVariables, ConstantResolver, Options, bIgnoreDisabled);
	}
	else
	{
		using namespace UE::Niagara::TraversalCache;
		FTopLevelScriptStaticContext ScriptContext;
		StackGraphUtilities::ConstantResolverToTopLevelScriptContext(ConstantResolver, FunctionCallNode, ScriptContext);
		EStackFunctionReadFilterFlags FilterFlags = StackGraphUtilities::InputOptionsToReadFilterFlags(Options, bIgnoreDisabled);
		FTraversalCache::GetStackFunctionReads(ScriptContext, FunctionCallNode, OutInputVariables, OutHiddenVariables, FilterFlags);
	}
}
...
void FNiagaraStackGraphUtilities::GetStackFunctionInputs(const UNiagaraNodeFunctionCall& FunctionCallNode, TArray<FNiagaraVariable>& OutInputVariables, FCompileConstantResolver ConstantResolver, ENiagaraGetStackFunctionInputPinsOptions Options, bool bIgnoreDisabled)
{
	...
	if (GbEnableTraversalCache == 0)
	{
		GetStackFunctionInputsLegacy(FunctionCallNode, OutInputVariables, ConstantResolver, Options, bIgnoreDisabled);
	}
	else
	{
		using namespace UE::Niagara::TraversalCache;
		...
		StackGraphUtilities::ConstantResolverToTopLevelScriptContext(ConstantResolver, FunctionCallNode, ScriptContext);
		EStackFunctionReadFilterFlags FilterFlags = StackGraphUtilities::InputOptionsToReadFilterFlags(Options, bIgnoreDisabled);
		FTraversalCache::GetStackFunctionReads(ScriptContext, FunctionCallNode, OutInputVariables, HiddenVariables, FilterFlags);
	}
}
...
void FNiagaraStackGraphUtilities::GetStackFunctionInputs(
	...
	ENiagaraGetStackFunctionInputPinsOptions Options, bool bIgnoreDisabled)
{
	SCOPE_CYCLE_COUNTER(STAT_NiagaraEditor_StackGraphUtilities_GetStackFunctionInputs);
	if (GbEnableTraversalCache == 0)
	{
		GetStackFunctionInputsLegacy(FunctionCallNode, OutInputVariables, Options, bIgnoreDisabled);
	}
	else
	{
		using namespace UE::Niagara::TraversalCache;
...
void FNiagaraStackGraphUtilities::GatherInputRelationsForStack(FInputDataCollection& State, TSharedRef<FNiagaraEmitterViewModel> EmitterViewModel)
{
	...
	UHierarchyRoot* SummaryRoot = EmitterViewModel->GetEditorData().GetSummaryRoot();
...
void FNiagaraStackGraphUtilities::GetStackFunctionStaticSwitchPinsLegacy(const UNiagaraNodeFunctionCall& FunctionCallNode, TArray<UEdGraphPin*>& OutInputPins, TSet<UEdGraphPin*>& OutHiddenPins,
	FCompileConstantResolver& ConstantResolver)
{
	...
	TSharedPtr<FNiagaraGraphCachedDataBase, ESPMode::ThreadSafe> CachedTraversalData;
	FString EmitterName;
	FNiagaraAliasContext::ERapidIterationParameterMode Mode = FNiagaraAliasContext::ERapidIterationParameterMode::StaticVariables;
...
{
	SCOPE_CYCLE_COUNTER(STAT_NiagaraEditor_StackGraphUtilities_GetStackFunctionStaticSwitchPins);
	if (GbEnableTraversalCache == 0)
	{
		GetStackFunctionStaticSwitchPinsLegacy(FunctionCallNode, OutInputPins, OutHiddenPins, ConstantResolver);
	}
	else
	{
		using namespace UE::Niagara::TraversalCache;
		FTopLevelScriptStaticContext ScriptContext;
		StackGraphUtilities::ConstantResolverToTopLevelScriptContext(ConstantResolver, FunctionCallNode, ScriptContext);
		FTraversalCache::GetStackFunctionStaticInputPins(ScriptContext, FunctionCallNode, OutInputPins, OutHiddenPins);
	}
}
...
void FNiagaraStackGraphUtilities::GetParametersForContext(UEdGraph* InGraph, UNiagaraSystem& System, TSet<FNiagaraVariableBase>& OutParameters)
{
	if (UNiagaraGraph* NiagaraGraph = Cast<UNiagaraGraph>(InGraph))
	{
		for (const TPair<FNiagaraVariable, FNiagaraGraphParameterReferenceCollection>& ReferencePair : NiagaraGraph->GetParameterReferenceMap())
		{
			OutParameters.Add(ReferencePair.Key);
		}
	}
...
void FNiagaraStackGraphUtilities::SetLinkedParameterValueForFunctionInput(UEdGraphPin& OverridePin, const FNiagaraVariableBase& LinkedParameter, const TSet<FNiagaraVariableBase>& KnownParameters, ENiagaraDefaultMode DesiredDefaultMode, const FGuid& NewNodePersistentId)
{
	...
	FNiagaraVariableBase ParameterToRead = LinkedParameter;
	if (Settings->bEnforceStrictStackTypes == false && (InputType == FNiagaraTypeDefinition::GetVec3Def() || InputType == FNiagaraTypeDefinition::GetPositionDef()))
	{
		...
		FNiagaraVariable ParameterAlternative(AlternateType, LinkedParameter.GetName());
	...
	else
	{
		...
		FNiagaraVariable StaticParameter(StaticInputType, LinkedParameter.GetName());
	...
	UEdGraphPin* GetOutputPin = GetNode->RequestNewTypedPin(EGPD_Output, ParameterToRead.GetType(), ParameterToRead.GetName());
...
void FNiagaraStackGraphUtilities::SetDataInterfaceValueForFunctionInput(UEdGraphPin& OverridePin, UClass* DataObjectType, FString InputNodeInputName, UNiagaraDataInterface*& OutDataObject, const FGuid& NewNodePersistentId)
{
	...
	const EObjectFlags DataInterfaceObjectFlags = UNiagaraDataInterface::BuildObjectFlagsForOwner(InputNode, RF_Transactional);
	OutDataObject = NewObject<UNiagaraDataInterface>(InputNode, DataObjectType, *ObjectTools::SanitizeObjectName(InputNodeInputName), DataInterfaceObjectFlags);
...
bool FNiagaraStackGraphUtilities::FindScriptModulesInStack(FAssetData ModuleScriptAsset, UNiagaraNodeOutput& TargetOutputNode, TArray<UNiagaraNodeFunctionCall*> OutFunctionCalls)
{
	...
	FName ModuleScriptAssetSoftObjectPathName = *ModuleScriptAsset.GetSoftObjectPath().ToString();
	for (UEdGraphNode* Node : Nodes)
	{
		if (UNiagaraNodeFunctionCall* FunctionCallNode = Cast<UNiagaraNodeFunctionCall>(Node))
		{
			if (FunctionCallNode->FunctionScriptAssetObjectPath == ModuleScriptAssetSoftObjectPathName ||
...
void FNiagaraStackGraphUtilities::GatherRenamedStackFunctionOutputVariableNames(FVersionedNiagaraEmitter Emitter, UNiagaraNodeFunctionCall& FunctionCallNode, const FString& OldFunctionName, const FString& NewFunctionName, TMap<FName, FName>& OutOldToNewNameMap)
{
	...
	FCompileConstantResolver ConstantResolver = OutputNode != nullptr 
		? FCompileConstantResolver(Emitter, OutputNode->GetUsage(), FunctionCallNode.DebugState)
...
void FNiagaraStackGraphUtilities::SynchronizeReferencingMapPinsWithFunctionCall(UNiagaraNodeFunctionCall& InFunctionCallNode)
{
	...
	if (OverridePins.Num() > 0)
	{
		...
		if (bModifiedGraph)
		{
			ModifiedGraphs.Add(InFunctionCallNode.GetNiagaraGraph());
		}
	}

	// Update linked module outputs or module attributes referenced by downstream parameter map gets.
	auto SynchronizeForStackLinkedModuleParameterPins = [&NiagaraSchema, &InFunctionCallNode, &AssetVariableIdToVariableMap, &ModifiedGraphs](const TArray<UEdGraphPin*>& StackLinkedModuleParameterPins)
	{
		...
		for (UEdGraphPin* StackLinkedModuleParameterPin : StackLinkedModuleParameterPins)
		{
			...
			if (TargetFunctionCallNode != nullptr)
			{
				...
				if (bModifiedGraph)
				{
					ModifiedGraphs.Add(TargetFunctionCallNode->GetNiagaraGraph());
				}
			}
		}
	};
	...
	for (UNiagaraGraph* ModifiedGraph : ModifiedGraphs)
	{
		ModifiedGraph->MarkGraphRequiresSynchronization(__FUNCTION__);
	}
}
...
{
	FNiagaraParameterHandle LinkedHandle = FNiagaraParameterHandle(LinkedParameter.GetName());
	if (LinkedHandle.GetHandleParts().Num() >= 3 && (
			LinkedHandle.IsOutputHandle() ||
			LinkedHandle.IsStackContextHandle() ||
			LinkedHandle.IsSystemHandle() ||
			LinkedHandle.IsEmitterHandle() ||
			LinkedHandle.IsParticleAttributeHandle()))
	{
		...
		if (SourceFunctionCallNodePtr != nullptr)
		{
			...
			if (CalledGraph != nullptr)
			{
				FNiagaraVariable LinkedModuleVariable = FNiagaraUtilities::ResolveAliases(LinkedParameter, FNiagaraAliasContext()
...
int32 FNiagaraStackGraphUtilities::DependencyUtilities::FindBestIndexForModuleInStack(UNiagaraNodeFunctionCall& ModuleNode, const UNiagaraNodeOutput& TargetOutputNode)
{
	...
	auto GetStackModuleDataRequiredDependenciesBeingProvided =
		[&NewModuleScriptProvidedDependencies](const FNiagaraStackModuleData& StackModuleData)->const TArray<FNiagaraModuleDependency> /*OutDependencies*/
		{
			TArray<FNiagaraModuleDependency> OutDependencies;
			if (UNiagaraNodeFunctionCall* ModuleNode = StackModuleData.WeakModuleNode.Get())
			{
				for (FNiagaraModuleDependency& RequiredDependency : ModuleNode->GetScriptData()->RequiredDependencies)
				{
