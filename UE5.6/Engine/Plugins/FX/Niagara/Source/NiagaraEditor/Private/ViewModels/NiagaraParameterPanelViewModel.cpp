...
{
	const TArray<UNiagaraScriptVariable*> ScriptVariables = GetEditableScriptVariablesWithName(ItemToCopy.GetVariable().GetName());
	if (ScriptVariables.Num() > 0 && ScriptVariables[0] != nullptr)
	{
		UNiagaraClipboardContent* ClipboardContent = UNiagaraClipboardContent::Create();
		ClipboardContent->ScriptVariables.Add({*ScriptVariables[0]});
...
void FNiagaraSystemToolkitParameterPanelViewModel::Cleanup()
{
	...
	{
		if(SystemGraphSelectionViewModelWeak.IsValid())
		{
...
void FNiagaraSystemToolkitParameterPanelViewModel::Init(const FSystemToolkitUIContext& InUIContext)
{
	...
	if (SystemViewModel->GetEditMode() == ENiagaraSystemViewModelEditMode::SystemAsset)
	{
		if(ensureMsgf(SystemGraphSelectionViewModelWeak.IsValid(), TEXT("SystemGraphSelectionViewModel was null for System edit mode!")))
		{
			SystemGraphSelectionViewModelWeak.Pin()->GetOnSelectedEmitterScriptGraphsRefreshedDelegate().AddSP(this, &FNiagaraSystemToolkitParameterPanelViewModel::RefreshNextTick);
		}
		
		SystemViewModel->GetSelectionViewModel()->OnSystemIsSelectedChanged().AddSP(this, &FNiagaraSystemToolkitParameterPanelViewModel::Refresh);
	}
...
bool FNiagaraSystemToolkitParameterPanelViewModel::IsVariableSelected(const FNiagaraVariableBase& InVar) const
{
...
TArray<FNiagaraParameterPanelItem> FNiagaraSystemToolkitParameterPanelViewModel::GetViewedParameterItems() const
{
	...
	if(SystemViewModel->GetSelectionViewModel()->GetSystemIsSelected() == false && SystemViewModel->GetSelectionViewModel()->GetSelectedEmitterHandleIds().Num() == 0)
	{	
		GraphsToCheck = AllGraphs;
	}
	else
	{
		GraphsToCheck = EditableGraphs;
	}
	
	// Collect all metadata to be packaged with the FNiagaraParameterPanelItems.
	for (const UNiagaraGraph* GraphToCheck : GraphsToCheck)
	{
		ParameterToScriptVariableMap.Append(GraphToCheck->GetAllMetaData());
	}
	...
	if (bForceSystem)
	{
		...
		// -For each applicable graph perform a parameter map history traversal and record each unique visited parameter.
		for (const UNiagaraGraph* GraphToCheck : GraphsToCheck)
		{
			TArray<UNiagaraNodeOutput*> OutputNodes;
			GraphToCheck->GetNodesOfClass<UNiagaraNodeOutput>(OutputNodes);
			for (UNiagaraNodeOutput* OutputNode : OutputNodes)
			{
				...
				FVersionedNiagaraEmitter GraphOwningEmitter = GraphToCheck->GetOwningEmitter();
				...
				*Builder.ConstantResolver = ConstantResolver;
				...
				auto AddReadReference = [GraphToCheck](const FNiagaraParameterMapHistory::FReadHistory& History, FNiagaraParameterPanelItem& Item)
				{
					FNiagaraParameterReferencePath Source;
					Source.SourceGraph = GraphToCheck;
					...
					if (Source.ModuleName == NAME_None && History.ReadPin.Pin->LinkedTo.Num() > 0)
					{
						...
						// find the name of the module this belongs to when used in a dynamic input
						TArray<UEdGraphNode*> NodesToCheck;
						NodesToCheck.Add(History.ReadPin.Pin->LinkedTo[0]->GetOwningNode());
						int32 NodeIndex = 0;
						while (NodesToCheck.IsValidIndex(NodeIndex))
						{
							if (UNiagaraNode* Node = Cast<UNiagaraNode>(NodesToCheck[NodeIndex]))
							{
								if (UNiagaraNodeFunctionCall* FunctionNode = Cast<UNiagaraNodeFunctionCall>(Node))
								{
									const bool bIsModuleFunctionScript = FunctionNode->FunctionScript && FunctionNode->FunctionScript->IsModuleScript();
									if (FunctionNode->IsNodeEnabled() && bIsModuleFunctionScript)
									{
										// Found the module
										FString CurrentName = Source.ModuleName.ToString();
										if (!CurrentName.StartsWith(FunctionNode->GetFunctionName()))
										{
											Source.ModuleName = FName("[" + FunctionNode->GetFunctionName() + "] " + CurrentName);
										}
										break;
									}
								}
								if (UEdGraphPin* OutputPin = FNiagaraStackGraphUtilities::GetParameterMapOutputPin(*Node))
								{
				...
				auto AddWriteReference = [GraphToCheck](const FNiagaraParameterMapHistory::FModuleScopedPin& History, FNiagaraParameterPanelItem& Item)
				{
