...
const UNiagaraStackModuleItem::FCollectedUsageData& UNiagaraStackModuleItem::GetCollectedUsageData() const
{
	if (CachedCollectedUsageData.IsSet() == false)
	{
		...
		if(InputRoot)
		{
			if(InputRoot->GetCollectedUsageData().bHasReferencedParameterRead)
			{
				CachedCollectedUsageData.GetValue().bHasReferencedParameterRead = true;
			}
			if (InputRoot->GetCollectedUsageData().bHasReferencedParameterWrite)
			{
				CachedCollectedUsageData.GetValue().bHasReferencedParameterWrite = true;
			}
		}
...
namespace NiagaraStackModuleItemIssues {
		...
		FNiagaraModuleDependency& RequiredDependency)
	{
		...
		for (int32 StackModuleDataIndex = StartIndex; StackModuleDataIndex != EndIndex; StackModuleDataIndex += NextIndexOffset)
		{
			...
			UNiagaraNodeFunctionCall* ModuleNode = CurrentStackModuleData.WeakModuleNode.Get();
			if (ModuleNode && ModuleNode->GetScriptData() != nullptr &&
				ModuleNode->GetScriptData()->RequiredDependencies.ContainsByPredicate(
	...
	void GetCompatibleOutputNodeAndIndex(TSharedRef<FNiagaraSystemViewModel> DependentSystemViewModel, FGuid EmitterHandleId, UNiagaraScript& DependencyProviderScript, const FNiagaraStackModuleData& LastDependentModuleData, FNiagaraModuleDependency RequiredDependency, UNiagaraNodeOutput*& OutTargetOutputNode, TOptional<int32>& OutTargetIndex)
	{
		...
		if (UNiagaraScript::ContainsEquivilentUsage(CompatibleUsages, LastDependentModuleData.Usage))
		{
			...
			if ( UNiagaraNodeFunctionCall* ModuleNode = LastDependentModuleData.WeakModuleNode.Get() )
			{
				OutTargetOutputNode = FNiagaraStackGraphUtilities::GetEmitterOutputNodeForStackNode(*ModuleNode);
		...
		else
		{
			...
			// We always use INDEX_NONE as target index (add to end), because PreDependency and PostDependency across stages doesn't mean much
			// If a module in ParticleSpawn requires a module in ParticleUpdate, only other ParticleUpdate scripts are relevant for its placement
		...
		FAssetData DependencyProviderModuleAsset)
	{
		...
		int32 DependentModuleIndex = StackModuleData.IndexOfByPredicate([DependentModuleNode](const FNiagaraStackModuleData& StackModuleDataItem) { return StackModuleDataItem.WeakModuleNode.Get() == DependentModuleNode; });
		...
		TArray<UNiagaraStackEntry::FStackIssueFix>& OutFixes)
	{
		for (int32 WrongOrderDependencyProviderIndex : WrongOrderDependencyProviderIndices)
		{
			...
			{
				UNiagaraNodeFunctionCall* ModuleNode = CurrentStackModuleData.WeakModuleNode.Get();
				if (ensure(ModuleNode))
				{
					FText FixDescription = FText::Format(LOCTEXT("MoveDependentFixDescriptionFormat", "Move module {0} {1} {2}"), FText::FromString(DependentModuleNode.GetFunctionName()), LocationText, FText::FromString(ModuleNode->GetFunctionName()));
		...
		for (int32 DisabledDependencyProviderIndex : DisabledDependencyProviderIndices)
		{
			...
			if (ensure(ModuleNode))
			{
				FText FixDescription = FText::Format(LOCTEXT("EnableDependencyProviderFixDescriptionFormat", "Enable module {0} which provides the dependency."),
					FText::FromString(ModuleNode->GetFunctionName()));
		...
		TArray<UNiagaraStackEntry::FStackIssue>& NewIssues)
	{
		...
		int32 ModuleIndex = SourceStackModuleData.IndexOfByPredicate([&SourceModuleNode](const FNiagaraStackModuleData& ModuleData) { return ModuleData.WeakModuleNode.Get() == &SourceModuleNode; });
			...
			*SourceSystemViewModel->GetSystem().GetPathName(), *SourceModuleNode.GetFunctionName(), *SourceModuleNode.GetName()))
		{
			for (const FNiagaraModuleDependency& SourceRequiredDependency : SourceModuleNode.GetScriptData()->RequiredDependencies)
			{
				...
				for (int32 DependencyProviderIndex : DependencyProviderIndices)
				{
					...
					UNiagaraNodeFunctionCall* ModuleNode = DependencyProviderData.WeakModuleNode.Get();
					bool bCorrectOrder =
					...
					bool bEnabled = ModuleNode && ModuleNode->GetDesiredEnabledState() == ENodeEnabledState::Enabled;
...
bool UNiagaraStackModuleItem::SupportsDebugDraw() const
{
	return GetModuleNode().ContainsDebugSwitch();
}

TOptional<FText> UNiagaraStackModuleItem::GetDebugDrawTooltip() const
{
	if (FVersionedNiagaraScriptData* ScriptData = GetModuleNode().GetScriptData())
	{
		if (ScriptData->DebugDrawMessage.IsEmpty() == false)
		{
			return ScriptData->DebugDrawMessage;
		}
	}
	return {};
}
...
FHierarchyElementIdentity UNiagaraStackModuleItem::DetermineSummaryIdentity() const
{
