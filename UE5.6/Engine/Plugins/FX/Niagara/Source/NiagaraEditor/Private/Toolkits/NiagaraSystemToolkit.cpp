...
void FNiagaraSystemToolkit::InitializeWithSystem(const EToolkitMode::Type Mode, const TSharedPtr< class IToolkitHost >& InitToolkitHost, UNiagaraSystem& InSystem)
{
	...
	SystemOptions.bCompileForEdit = IsStatPerformanceChecked() ? IsProfileInEditMode() : true;
...
void FNiagaraSystemToolkit::SetupCommands()
{
	...
	GetToolkitCommands()->MapAction(
		FNiagaraEditorCommands::Get().ToggleProfileInEditMode,
		FExecuteAction::CreateSP(this, &FNiagaraSystemToolkit::ToggleProfileInEditMode),
		FCanExecuteAction(),
		FIsActionChecked::CreateSP(this, &FNiagaraSystemToolkit::IsProfileInEditMode));
	GetToolkitCommands()->MapAction(
	...
	GetToolkitCommands()->MapAction(
		FNiagaraEditorCommands::Get().EnableSelectedEmitters,
		FExecuteAction::CreateLambda([this]()
			{
				SystemViewModel->EnableSelectedEmitters();
			}),
		FCanExecuteAction());
...
TSharedRef<SWidget> FNiagaraSystemToolkit::GenerateStatConfigMenuContent(TSharedRef<FUICommandList> InCommandList)
{
	...
	MenuBuilder.AddMenuEntry(FNiagaraEditorCommands::Get().ToggleProfileInEditMode);
	MenuBuilder.AddMenuSeparator();
...
{
#if STATS
	return SystemViewModel->GetSystem().GetPerformanceStateScopeMode() != ENiagaraPerformanceStateScopeMode::None;
#else
	return false;
#endif
}
...
{
	const bool IsEnabled = IsStatPerformanceChecked();
	if (SystemViewModel.IsValid())
	{
		SystemViewModel->SetPerformanceStatsEnabled(!IsEnabled);

		const bool bCompileForEdit = !IsEnabled ? bProfileInEditMode : true;
		SystemViewModel->SetCompileForEdit(bCompileForEdit);
	}
...
{
	return SystemViewModel.IsValid() ? SystemViewModel->IsPerformanceStatsEnabled() : false;
}
...
void FNiagaraSystemToolkit::ToggleProfileInEditMode()
{
	bProfileInEditMode = !bProfileInEditMode;

	if (SystemViewModel.IsValid() && IsStatPerformanceChecked())
	{
		SystemViewModel->SetCompileForEdit(bProfileInEditMode);
	}
}

bool FNiagaraSystemToolkit::IsProfileInEditMode() const
{
	return bProfileInEditMode;
}
