...
void SNiagaraSystemViewportToolBar::ExtendOptionsMenu(FMenuBuilder& OptionsMenuBuilder) const
{
	...
	if (TSharedPtr<SNiagaraSystemViewport> NiagaraSystemViewport = WeakViewport.Pin())
	{
		OptionsMenuBuilder.AddWidget(UE::NiagaraSystemEditor::CreateMotionMenuWidget(NiagaraSystemViewport.ToSharedRef()), FText());
	}
