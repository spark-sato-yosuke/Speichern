...
TSharedPtr<FNiagaraSystemViewModel> UNiagaraStackEntry::GetSystemViewModelPtr() const
{
...
void UNiagaraStackEntry::RefreshChildren()
{
	...
	{		
		if(StackNote == nullptr)
		{
			...
			StackNote->IndentLevel = GetIndentLevel();
...
void UNiagaraStackEntry::AddStackNote()
{
	if(SupportsStackNotes())
	{
		if(HasStackNoteData() == false)
		{
			FScopedTransaction Transaction(LOCTEXT("NewStackNoteAdded", "Added Stack Note"));
			GetStackEditorData().Modify();
		
			FNiagaraStackNoteData NewStackNote;
			NewStackNote.MessageHeader = LOCTEXT("DefaultStackNoteHeader", "Note");
			NewStackNote.Message = LOCTEXT("DefaultStackNoteMessage", "Text");
			NewStackNote.Color = GetDefault<UNiagaraEditorSettings>()->GetDefaultNoteColor();

			GetStackEditorData().AddOrReplaceStackNote(GetStackEditorDataKey(), NewStackNote);
			RefreshChildren();
		
			if(UNiagaraStackNote* Note = GetStackNote())
			{
...
void UNiagaraStackEntry::ToggleStackNoteInlineDisplay()
{
	if(GetStackNote() != nullptr)
	{
		GetStackNote()->ToggleInlineDisplay();
	}
}

void UNiagaraStackEntry::CopyStackNote()
{
	FNiagaraStackClipboardUtilities::CopyNote(this);
}

void UNiagaraStackEntry::PasteStackNote()
{
	if(SupportsStackNotes())
	{
...
FHierarchyElementIdentity UNiagaraStackEntry::DetermineSummaryIdentity() const
{
	return FHierarchyElementIdentity();
}
...
bool UNiagaraStackEntry::IsInSummaryView() const
{
	if(GetEmitterViewModel().IsValid())
	{
		...
		{
			FHierarchyElementIdentity Identity = DetermineSummaryIdentity();
			if(Identity.IsValid())
			{
