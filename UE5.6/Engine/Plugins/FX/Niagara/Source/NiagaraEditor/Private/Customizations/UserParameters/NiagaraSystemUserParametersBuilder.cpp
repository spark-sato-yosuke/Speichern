...
#include "INiagaraVerseReflectionUtilities.h"
...
FNiagaraSystemUserParameterBuilder::FNiagaraSystemUserParameterBuilder(TSharedPtr<FNiagaraSystemViewModel> InSystemViewModel, FName InCustomBuilderRowName)
{
	...
	if (INiagaraModule* NiagaraModule = FModuleManager::Get().LoadModulePtr<INiagaraModule>(TEXT("Niagara")))
	{
		VerseReflectionUtilities = NiagaraModule->GetVerseReflectionUtilities();
	}
}
...
void FNiagaraSystemUserParameterBuilder::GenerateChildContent(IDetailChildrenBuilder& ChildrenBuilder)
{
	...
	if (System.IsValid())
	{
		if(bDelegatesInitialized == false)
		{
			...
			SystemViewModel.Pin()->GetUserParametersHierarchyViewModel()->OnHierarchyChanged().Add(UDataHierarchyViewModelBase::FOnHierarchyChanged::FDelegate::CreateSP(this, &FNiagaraSystemUserParameterBuilder::Rebuild));
			SystemViewModel.Pin()->GetUserParametersHierarchyViewModel()->OnHierarchyPropertiesChanged().RemoveAll(this);
			SystemViewModel.Pin()->GetUserParametersHierarchyViewModel()->OnHierarchyPropertiesChanged().Add(UDataHierarchyViewModelBase::FOnHierarchyPropertiesChanged::FDelegate::CreateSP(this, &FNiagaraSystemUserParameterBuilder::Rebuild));
		...
		UHierarchyRoot* Root = Cast<UNiagaraSystemEditorData>(System->GetEditorData())->UserParameterHierarchy;
...
void FNiagaraSystemUserParameterBuilder::DeleteParameter(FNiagaraVariable UserParameter)
{
...
{
	TSharedPtr<SWidget> VerseWidget = SNullWidget::NullWidget;
	if (VerseReflectionUtilities)
	{
		VerseWidget = SNew(SImage)
			.ToolTipText(TAttribute<FText>::CreateSP(this, &FNiagaraSystemUserParameterBuilder::GetParameterAccessLevelTooltip, UserParameter))
			.Image(TAttribute<const FSlateBrush*>::CreateSP(this, &FNiagaraSystemUserParameterBuilder::GetParameterAccessLevelBrush, UserParameter))
	...
	TSharedRef<SNiagaraParameterNameTextBlock> ParameterName = SNew(SNiagaraParameterNameTextBlock)
		...
		.OnVerifyTextChanged_Lambda([this, UserParameter](const FText& InNewName, FText& OutErrorMessage)
		{
			...
			// Check to see if it already exists
			{
				...
				{
					if (ExistingUserParameter.GetName().ToString().Equals(NewNameString, ESearchCase::IgnoreCase))
					{
		...
		.OnTextCommitted_Lambda([this, UserParameter](const FText& InText, ETextCommit::Type Type) { RenameParameter(UserParameter, FName(InText.ToString())); })
		.DecoratorLocation(ENiagaraParameterDecoratorLocation::BeforeName)
		.Decorator()
		[
			VerseWidget.ToSharedRef()
		];
...
FString FNiagaraSystemUserParameterBuilder::GetObjectAssetPathForUserParameter(FNiagaraVariable UserParameter) const
{
	...
	if (TObjectPtr<UObject> Object = Variant.GetUObject())
	{
...
bool FNiagaraSystemUserParameterBuilder::IsParameterExposedToVerse(FNiagaraVariable UserParameter) const
{
	return GetParameterAccessLevel(UserParameter) != ENiagaraParameterAccessLevel::Private;
}

ENiagaraParameterAccessLevel FNiagaraSystemUserParameterBuilder::GetParameterAccessLevel(FNiagaraVariable UserParameter) const
{
	TOptional<ENiagaraParameterAccessLevel> AccessValue;
	if ( VerseReflectionUtilities )
	{
		UserParameter.RemoveRootNamespace(FNiagaraConstants::UserNamespaceString);
		if (!VerseReflectionUtilities->IsValidVariable(UserParameter).IsSet())
		{
			return ENiagaraParameterAccessLevel::Public;
		}
	}
	return AccessValue.Get(ENiagaraParameterAccessLevel::Private);
}

const FSlateBrush* FNiagaraSystemUserParameterBuilder::GetParameterAccessLevelBrush(FNiagaraVariable UserParameter) const
{
	switch (GetParameterAccessLevel(UserParameter))
	{
		case ENiagaraParameterAccessLevel::Public:
			return FNiagaraEditorStyle::Get().GetBrush("NiagaraEditor.Verse.Public");

		default:
		case ENiagaraParameterAccessLevel::Private:
			return FNiagaraEditorStyle::Get().GetBrush("NiagaraEditor.Verse.Private");
	}
}

FText FNiagaraSystemUserParameterBuilder::GetParameterAccessLevelTooltip(FNiagaraVariable UserParameter) const
{
	if (VerseReflectionUtilities)
	{
		TOptional<FText> ErrorText = VerseReflectionUtilities->IsValidVariable(UserParameter);
		if (ErrorText.IsSet())
		{
	...
	const UEnum* Enum = StaticEnum<ENiagaraParameterAccessLevel>();
	return Enum ? Enum->GetToolTipTextByIndex((int32)GetParameterAccessLevel(UserParameter)) : FText();
}

bool FNiagaraSystemUserParameterBuilder::CanExposeParameterToVerse(FNiagaraVariable UserParameter) const
{
	return VerseReflectionUtilities ? !VerseReflectionUtilities->IsValidVariable(UserParameter).IsSet() : false;
}
