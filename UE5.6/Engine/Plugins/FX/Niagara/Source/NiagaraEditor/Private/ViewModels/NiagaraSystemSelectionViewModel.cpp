...
{
	bool bWasSystemSelected = bSystemIsSelected;
	bSystemIsSelected = false;
	...
	bool bSystemSelectionChanged = bWasSystemSelected != bSystemIsSelected;
	...
	if(bSystemSelectionChanged)
	{
		OnSystemIsSelectedChangedDelegate.Broadcast();
	}
}
