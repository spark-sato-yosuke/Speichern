...
bool FNiagaraBakerViewModel::PreviewLoopedOutput() const
{
	UNiagaraBakerSettings* BakerSettings = GetBakerSettings();
	return BakerSettings ? BakerSettings->bPreviewLoopedOutput : false;
}

void FNiagaraBakerViewModel::TogglePreviewLoopedOutput()
{
	if (UNiagaraBakerSettings* BakerSettings = GetBakerSettings())
	{
		const FScopedTransaction Transaction(LOCTEXT("SetRenderComponentOnly", "Set Render Component Only"));
		BakerSettings->Modify();
		BakerSettings->bPreviewLoopedOutput = !BakerSettings->bPreviewLoopedOutput;
	}
}
