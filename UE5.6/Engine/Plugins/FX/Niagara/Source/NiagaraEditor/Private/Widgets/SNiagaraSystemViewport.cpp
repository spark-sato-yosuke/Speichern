...
void FNiagaraSystemViewportClient::Draw(FViewport* InViewport, FCanvas* Canvas)
{
	...
	const bool bDrawBounds = NiagaraViewport.IsValid() && NiagaraViewport->GetDrawElement(SNiagaraSystemViewport::EDrawElements::Bounds);
	if (bDrawBounds)
	{
	...
	if (ParticleSystem && Component)
	{
		...
		// Draw Streaming Bounds if enabled
		if (bDrawBounds)
		{
			const FBox StreamingBounds = ParticleSystem->GetInitialStreamingBounds();
			if (StreamingBounds.IsValid)
			{
				DrawDebugBox(
					World,
					StreamingBounds.GetCenter(),
					StreamingBounds.GetExtent(),
					FQuat::Identity,
					FColor::Green
				);
			}
	...
	if (NiagaraViewport.IsValid() )
	{
		...
		if (NiagaraViewport->GetDrawElement(SNiagaraSystemViewport::EDrawElements::OriginAxis) && Component)
		{
			UWorld* World = Component->GetWorld();
			...
			LineBatcher->DrawDirectionalArrow(ArrowMatrix, FColorList::Red, 50.f, 4.0f, SDPG_World);

			ArrowMatrix = FMatrix(YAxis, ZAxis, XAxis, FVector::ZeroVector);
			LineBatcher->DrawDirectionalArrow(ArrowMatrix, FColorList::Green, 50.f, 4.0f, SDPG_World);

			ArrowMatrix = FMatrix(ZAxis, XAxis, YAxis, FVector::ZeroVector);
			LineBatcher->DrawDirectionalArrow(ArrowMatrix, FColorList::Blue, 50.f, 4.0f, SDPG_World);
		}
	}
...
void SNiagaraSystemViewport::BindCommands()
{
	...
	CommandList->MapAction(
		...
		{
			Viewport->ToggleDrawElement(EDrawElements::OriginAxis);
			Viewport->RefreshViewport();
		}),
		FCanExecuteAction(),
		FIsActionChecked::CreateLambda([Viewport = this]() -> bool { return Viewport->GetDrawElement(EDrawElements::OriginAxis); })
	);
...
TSharedPtr<SWidget> SNiagaraSystemViewport::BuildViewportToolbar()
{
	...
	if (!UToolMenus::Get()->IsMenuRegistered(ViewportToolbarName))
	{
		...
		// Add the right-aligned part of the viewport toolbar.
		{
			...
			RightSection.AddEntry(UE::NiagaraSystemEditor::CreateShowSubmenu());
			RightSection.AddEntry(UE::UnrealEd::CreatePerformanceAndScalabilitySubmenu());

			// Add Preview Scene Submenu
			{
				const FName AssetViewerProfileMenuName = "NiagaraSystemEditor.ViewportToolbar.AssetViewerProfile";
				...
				UE::NiagaraSystemEditor::ExtendPreviewSceneSettingsSubmenu(AssetViewerProfileMenuName);
	...
	FToolMenuContext ViewportToolbarContext;
	{
		...
		// Add the UnrealEd viewport toolbar context.
		{
			...
			ContextObject->PreviewSettingsTabId = FNiagaraSystemToolkitModeBase::PreviewSettingsTabId;
...
TSharedPtr<IPreviewProfileController> SNiagaraSystemViewport::CreatePreviewProfileController()
{
...
void FNiagaraBaselineViewportClient::Tick(float DeltaSeconds)
{
	...
	if (UWorld* World = PreviewScene->GetWorld())
	{
		...
		{
			for (TActorIterator<ANiagaraPerfBaselineActor> It(World); It; ++It)
			{
				if (ANiagaraPerfBaselineActor* BaselineActor = *It)
				{
