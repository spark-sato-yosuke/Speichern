...
void FNiagaraUserParameterNodeBuilder::GenerateUserParameterRows(IDetailChildrenBuilder& ChildrenBuilder, UHierarchyRoot& UserParameterHierarchyRoot)
{
	...
	UHierarchyRoot* Root = &UserParameterHierarchyRoot;
	...
	if(SectionCategoryMap.Num() > 0)
	{
					...
					[
						...
						[
							...
							[
								...
								.Text_UObject(Section, &UHierarchySection::GetSectionNameAsText)
								.ToolTipText_UObject(Section, &UHierarchySection::GetTooltip)
							]
						]
					];
				}
		...
		if(SectionsBox->GetChildren()->Num() > 0)
		{
			...
			FDetailWidgetRow& Row = ChildrenBuilder.AddCustomRow(FText::FromString("User Parameter Sections"));
			Row.RowTag(FName("User Parameter Sections"));
	...
	{		
		if(const UHierarchyCategory* HierarchyCategory = Cast<UHierarchyCategory>(Child))
		{
			...
			{
				GenerateCategory(ChildrenBuilder, *HierarchyCategory);
			}
		}
...
void FNiagaraUserParameterNodeBuilder::GenerateCategory(IDetailChildrenBuilder& ChildrenBuilder, const UHierarchyCategory& HierarchyCategory)
{
...
FNiagaraUserParameterCategoryBuilder::FNiagaraUserParameterCategoryBuilder(const UHierarchyCategory& InNiagaraHierarchyCategory, FName InCustomBuilderRowName, FNiagaraUserParameterNodeBuilder& InNodeBuilder) : NodeBuilder(InNodeBuilder)
{
...
void FNiagaraUserParameterCategoryBuilder::GenerateChildContent(IDetailChildrenBuilder& ChildrenBuilder)
{
	...
	TArray<const UHierarchyCategory*> ContainedCategories;

	for(const UHierarchyElement* Child : HierarchyCategory->GetChildren())
	{
	...
	for(UHierarchyElement* Child : HierarchyCategory->GetChildren())
	{
		if(UHierarchyCategory* Category = Cast<UHierarchyCategory>(Child))
		{
			ContainedCategories.Add(Category);
		}
	}

	for(const UHierarchyCategory* Category : ContainedCategories)
	{
		if(Category->DoesOneChildExist<UNiagaraHierarchyUserParameter>(true))
		{
			NodeBuilder.GenerateCategory(ChildrenBuilder, *Category);
		}
	}
