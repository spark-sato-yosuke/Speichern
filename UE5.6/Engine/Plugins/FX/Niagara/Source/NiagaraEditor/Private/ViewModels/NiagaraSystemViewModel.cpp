...
void FNiagaraSystemViewModel::Initialize(UNiagaraSystem& InSystem, FNiagaraSystemViewModelOptions InOptions)
{
	...
	SystemScriptViewModel->Initialize(this->AsShared());
	...
	bSupportCompileForEdit = !bIsForDataProcessingOnly && (EditMode == ENiagaraSystemViewModelEditMode::SystemAsset || EditMode == ENiagaraSystemViewModelEditMode::EmitterAsset);
	bIsCompileForEdit = false;
	if (bSupportCompileForEdit)
	{
		bIsCompileForEdit = InOptions.bCompileForEdit;
		System->SetCompileForEdit(bIsCompileForEdit);
	}
...
void FNiagaraSystemViewModel::Cleanup()
{
	...
	if (System)
	{
		if (bSupportCompileForEdit && bIsCompileForEdit)
		{
...
void FNiagaraSystemViewModel::SetCompileForEdit(bool bNewState)
{
	if (bSupportCompileForEdit && bIsCompileForEdit != bNewState)
	{
		check(System->GetCompileForEdit() == bIsCompileForEdit);
		bIsCompileForEdit = bNewState;

		System->SetCompileForEdit(bNewState);
		System->InvalidateActiveCompiles();
		System->RequestCompile(false);
	}
}
...
void FNiagaraSystemViewModel::IsolateEmitters(TArray<FGuid> EmitterHandlesIdsToIsolate)
{
	...
	SetSystenIsolationEnabled(bAnyEmitterIsolated);
}
...
void FNiagaraSystemViewModel::SetSystenIsolationEnabled(bool bIsolationEnabled)
{
	UNiagaraSystem* NiagaraSystem = &GetSystem();
	NiagaraSystem->SetIsolateEnabled(bIsolationEnabled);

	for (TObjectIterator<UNiagaraComponent> It; It; ++It)
	{
		UNiagaraComponent* Comp = *It;
		if (::IsValid(Comp) && Comp->IsRegistered() && Comp->GetAsset() == NiagaraSystem)
		{
...
void FNiagaraSystemViewModel::EnableEmitters(TArray<FGuid> EmitterHandlesIdsToEnable)
{
	FScopedTransaction Transaction(LOCTEXT("EnableSelectedEmitters", "Enable selected emitters"));
	GetSystem().Modify();

	bool bAnyEmittersEnabled = false;

	for (TSharedRef<FNiagaraEmitterHandleViewModel> EmitterHandle : EmitterHandleViewModels)
	{
		if (EmitterHandlesIdsToEnable.Contains(EmitterHandle->GetId()))
		{
			const bool bEnabled = EmitterHandle->GetEmitterHandle()->SetIsEnabled(true, GetSystem(), false);
			if (bEnabled)
			{
				EmitterHandlePropertyChanged(EmitterHandle->GetId());
			}
			bAnyEmittersEnabled = bAnyEmittersEnabled || bEnabled;
		}
	}

	if (bAnyEmittersEnabled)
	{
...
void FNiagaraSystemViewModel::EnableSelectedEmitters()
{
	const TArray<FGuid> EmitterHandles = GetSelectionViewModel()->GetSelectedEmitterHandleIds();
	EnableEmitters(EmitterHandles);
}
...
void FNiagaraSystemViewModel::UpdateCompiledDataInterfaces(UNiagaraDataInterface* ChangedDataInterface)
{
	...
	else
	{
		...
		const FNiagaraVariableBase* FoundExposedDataInterface = GetSystem().GetExposedParameters().FindVariableFromDataInterface(ChangedDataInterface);
	...
	if (bUpdatingSequencerFromEmitterDataChange == false && GIsTransacting == false && NiagaraSequence)
	{
		...
		for (UMovieSceneTrack* Track : NiagaraSequence->GetMovieScene()->GetTracks())
		{
			...
			if (EmitterTrack->GetEmitterHandleViewModel().IsValid())
			{
				...
				EmitterTrack->GetEmitterHandleViewModel()->GetEmitterViewModel()->GetEditorData().Modify();
				EmitterTrack->GetEmitterHandleViewModel()->GetEmitterViewModel()->GetEditorData().SetPlaybackRange(GetEditorData().GetPlaybackRange());
...
ENiagaraPerformanceStateScopeMode FNiagaraSystemViewModel::GetPerformanceStateScopeMode() const
{
	return System->GetPerformanceStateScopeMode();
}
