...
void FNiagaraComponentUserParametersNodeBuilder::RegisterRebuildOnHierarchyChanged()
{
	if(Component.IsValid())
	{
		if(UNiagaraSystem* Asset = Component->GetAsset())
		{
			...
			{
				SystemViewModel->GetUserParametersHierarchyViewModel()->OnHierarchyChanged().RemoveAll(this);
				SystemViewModel->GetUserParametersHierarchyViewModel()->OnHierarchyChanged().Add(UDataHierarchyViewModelBase::FOnHierarchyChanged::FDelegate::CreateSP(this, &FNiagaraComponentUserParametersNodeBuilder::Rebuild));
				SystemViewModel->GetUserParametersHierarchyViewModel()->OnHierarchyPropertiesChanged().RemoveAll(this);
				SystemViewModel->GetUserParametersHierarchyViewModel()->OnHierarchyPropertiesChanged().Add(UDataHierarchyViewModelBase::FOnHierarchyPropertiesChanged::FDelegate::CreateSP(this, &FNiagaraComponentUserParametersNodeBuilder::Rebuild));
			}
		}
	}
}
