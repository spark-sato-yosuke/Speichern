...
void FNiagaraEditorUtilities::InitializeParameterInputNode(UNiagaraNodeInput& InputNode, const FNiagaraTypeDefinition& Type, const UNiagaraGraph* InGraph, FName InputName)
{
	...
	else if(Type.IsDataInterface())
	{
		...
		InputNode.SetDataInterface(NewObject<UNiagaraDataInterface>(&InputNode, Type.GetClass(), NAME_None, UNiagaraDataInterface::BuildObjectFlagsForOwner(&InputNode, RF_Transactional)));
	}
...
void FNiagaraEditorUtilities::GetFilteredScriptAssets(FGetFilteredScriptAssetsOptions InFilter, TArray<FAssetData>& OutFilteredScriptAssets)
{
	...
	{
		if(AssetToolsModule.Get().IsAssetVisible(FilteredScriptAssets[i], true) == false)
		{
			continue;
		}
...
int32 FNiagaraEditorUtilities::GetWeightForItem(const TSharedPtr<FNiagaraMenuAction_Generic>& InCurrentAction, FName FavoriteActionsProfile, const TArray<FString>& InFilterTerms)
{
	// The overall 'weight'
	int32 TotalWeight = 0;
	...
	// favorite actions get favored massively
	if(InCurrentAction->FavoritesActionData.IsSet())
	{
		FNiagaraFavoritesActionData ActionData = InCurrentAction->FavoritesActionData.GetValue();
		if(UNiagaraFavoriteActionsConfig::Get()->GetActionsProfile(FavoriteActionsProfile).IsFavorite(ActionData))
		{
...
const FGuid FNiagaraEditorUtilities::AddEmitterToSystem(UNiagaraSystem& InSystem, UNiagaraEmitter& InEmitterToAdd, FGuid EmitterVersion, bool bCreateCopy)
{
	...
	{
		// FNiagaraUtilities::GetUniqueName will produce a unique emitter name (i.e. Empty -> Empty001 -> Empty002, etc)
		// but it will not produce a unique number so if that UObject is already taken by another object type we need to find a unique number
		// The number is not important to the stack / emitter name as it's not displayed but is important to avoid colliding between UNiagaraEmitter and UNiagaraStatelessEmitter types
		FName EmitterName = FNiagaraUtilities::GetUniqueName(InEmitterToAdd.GetFName(), EmitterHandleNames);
		while (FindObjectFast<UObject>(&InSystem, EmitterName))
		{
			EmitterName.SetNumber(EmitterName.GetNumber() + 1);
		}

		InSystem.Modify();
		EmitterHandle = InSystem.AddEmitterHandle(InEmitterToAdd, EmitterName, EmitterVersion);
	}
	else if (bCreateCopy)
	{
...
{
	const TArray<TWeakObjectPtr<UNiagaraParameterDefinitions>>& CachedParameterDefinitionAssets = FNiagaraEditorModule::Get().GetCachedParameterDefinitionsAssets();

	for (const TWeakObjectPtr<UNiagaraParameterDefinitions>& CachedParameterDefinitionAsset : CachedParameterDefinitionAssets)
	{
		if (UNiagaraParameterDefinitions* ParameterDefinition = CachedParameterDefinitionAsset.Get())
		{
			OutParameterDefinitions.Add(ParameterDefinition);
		}
	}
}
...
TSharedRef<SWidget> FNiagaraParameterUtilities::GetParameterWidget(FNiagaraVariable Variable, bool bAddTypeIcon, bool bShowValue)
{
	if(Variable.IsValid() == false)
	{
		return SNew(STextBlock)
			.Text(LOCTEXT("InvalidParameterWidgetName", "Invalid Parameter"))
			.ToolTipText(LOCTEXT("InvalidParameterWidgetTooltip", "This parameter is invalid. If there is a source, check where it might be coming from."));
	}
...
TSharedRef<SWidget> FNiagaraEditorUtilities::HierarchyEditor::Scripts::GenerateRowContentForScriptParameterHierarchyEditor(TSharedRef<FHierarchyElementViewModel> HierarchyItem)
{
	...
	else if(const UNiagaraHierarchyScriptParameter* ScriptParameter = Cast<UNiagaraHierarchyScriptParameter>(HierarchyItem->GetData()))
	{
		...
		TSharedRef<SWidget> ParameterWidget = FNiagaraParameterUtilities::GetParameterWidget(ScriptParameter->GetVariable().Get(FNiagaraVariable()), ScriptParameter->GetScriptVariable()->Metadata, WidgetOptions);
	...
	[
		...
		[
			...
			[
				...
				.ColorAndOpacity(StackNote.GetColor())
			]
		]
		+ SHorizontalBox::Slot()
		.Padding(2.f)
...
{
	FNiagaraAliasContext RenameContext(FNiagaraAliasContext::ERapidIterationParameterMode::StaticVariables);
