...
void UNiagaraStackObject::Initialize(FRequiredEntryData InRequiredEntryData, UObject* InObject, bool bInIsTopLevelObject, bool bInHideTopLevelCategories, FString InOwnerStackItemEditorDataKey, UNiagaraNode* InOwningNiagaraNode)
{
	...
	AddChildFilter(FOnFilterChild::CreateUObject(this, &UNiagaraStackObject::FilterOnlyModified));
}
...
void UNiagaraStackObject::NotifyPostChange(const FPropertyChangedEvent& PropertyChangedEvent, FProperty* PropertyThatChanged)
{
	...
	{
		UNiagaraDataInterface* DataInterfaceObject = Cast<UNiagaraDataInterface>(WeakObject.Get());
		if (DataInterfaceObject != nullptr)
		{
			// First we tell the Details Panel to refresh its error UI
			{
...
FHierarchyElementIdentity UNiagaraStackObject::DetermineSummaryIdentity() const
{
...
// 				}
	...
	if(PropertyRowGenerator.IsValid())
	{
		...
		TSharedPtr<const FNiagaraStackObjectPropertyCustomization> Customization = NiagaraEditorModule->GetStackObjectCustomizationRegistry().GetCustomizationForStackObject(*this);
		for (TSharedRef<IDetailTreeNode> RootTreeNode : RootTreeNodes)
		{
			if (RootTreeNode->GetNodeType() == EDetailNodeType::Advanced)
			{
			...
			if (ChildRow == nullptr)
			{
				...
				if(Customization)
				{
					ChildRow->SetPropertyCustomization(Customization);
				}
			}
...
bool UNiagaraStackObject::FilterOnlyModified(const UNiagaraStackEntry& Child) const
{
