...
{
	FNiagaraVariableBase LinkedParameterValue;
	FGuid LinkedFunctionNodeId;

	bool operator==(const FNiagaraStackLinkedValueData& Other) const
	{
		return LinkedParameterValue == Other.LinkedParameterValue && LinkedFunctionNodeId == Other.LinkedFunctionNodeId;
	}
};
...
struct FNiagaraEmitterDiffResults
{
	...
	TArray<UHierarchyElement*> AddedSummaryEntriesInOther;	
	TArray<UHierarchySection*> AddedSummarySectionsInOther;
...
public:
	...
	bool DoesSummaryItemExistInBase(const FVersionedNiagaraEmitter& Emitter, FHierarchyElementIdentity Identity);
