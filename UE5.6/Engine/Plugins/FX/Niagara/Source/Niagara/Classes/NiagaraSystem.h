...
enum class ENiagaraPerformanceStateScopeMode : int
{
	/** No stats exist and we should not show the option. */
	None,
	/** Only basic (i.e. whole script) stats exist. */
	Basic,
	/** Detailed (i.e. per module) stats exist. */
	Detailed,
};
...
{
	/**
	When a simulation moves greater or equal to the number of tiles specified in fx.LWCTileRecache the simulation will be reset.
	This means all particles are cleared and we start the system from time 0 again.
	*/
	ResetSimulation,
	/**
	When a simulation moves greater or equal to the number of tiles specified in fx.LWCTileRecache we will trigger a rebase.
	A rebase will move all Position type attributes by the difference in tiles.
	This is not guaranteed to cover everything, such as data inteface, matrix attributes, etc, and is therefore could result in simulation errors.
	*/
	Rebase,
	/**
	This option is similar to Rebase with the exception that when we cross fx.LWCTileForceReset number of tiles we will reset the simulation.
	This can provide some additional safety if you were to cross a large number of tiles and move back the same number of tiles,
	as in situations like the floating point numbers may have lost significant precision resulting in an invalid simulation.
	*/
	RebaseOrResetSimulation,
};
...
#if WITH_EDITOR
	NIAGARA_API virtual void OnCookEvent(UE::Cook::ECookEvent CookEvent, UE::Cook::FCookEventContext& CookContext) override;
...
public:
	...
	/** Blocks until all active compile jobs have finished but does not flush any requests that were deferred from loading. */
	NIAGARA_API void WaitForCompilationComplete_SkipPendingOnDemand(bool bIncludingGPUShaders = false, bool bShowProgress = true);
	...
	ENiagaraLwcTileUpdateMode GetLargeWorldCoordinateTileUpdateMode() const;
	...
	/** Returns information on the availability of performance stat scopes */
	NIAGARA_API ENiagaraPerformanceStateScopeMode GetPerformanceStateScopeMode() const;

	/** Do we have any pending compilation requests or not */
	bool NeedsRequestCompile() const { return RequestCompileStatus != ERequestCompileStatus::None; }
...
protected:
	...
	/** Used to track the status of compilation request, when using on demand they will initially be in RequestPendingOnDemand. */
	enum class ERequestCompileStatus
	{
		None,
		RequestPending,
		RequestPendingOnDemand,
		RequestPendingForcedRetry,
	};
	ERequestCompileStatus RequestCompileStatus = ERequestCompileStatus::None;
...
private:
	...
	*/
	...
	UPROPERTY(EditAnywhere, Category = Rendering, AdvancedDisplay, meta = (EditCondition = "bSupportLargeWorldCoordinates", EditConditionHides, DisplayAfter=bSupportLargeWorldCoordinates))
...
public:
	...
	FBox GetInitialStreamingBounds() const { return bUseInitialStreamingBounds ? InitialStreamingBounds : FBox(); }
	...
	FNiagaraPlatformSet& GetScalabilityPlatformSet() { return Platforms; }
	const FNiagaraPlatformSet& GetScalabilityPlatformSet() const { return Platforms; }
	...
	const FNiagaraSystemStateData& GetSystemStateData() const;
...
protected:
	...
	// If true then we need to manually update the asset registry after the system is fully loaded
	mutable bool bUpdateTagsAfterLoad = false;
#endif
	...
	UPROPERTY(EditAnywhere, Category = "System", meta = (InlineEditConditionToggle, SkipSystemResetOnChange = "true"))
	bool bUseInitialStreamingBounds = false;
	...
	*/
	UPROPERTY(EditAnywhere, Category = "System", meta = (SkipSystemResetOnChange = "true", EditCondition = "bUseInitialStreamingBounds"))
	FBox InitialStreamingBounds = FBox(FVector(-100), FVector(100));
...
protected:
	...
	UPROPERTY()
	FInstancedStruct SystemStateDataStruct;
