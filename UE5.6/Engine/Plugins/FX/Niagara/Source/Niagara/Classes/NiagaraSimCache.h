...
struct FNiagaraSimCacheDataBuffersLayout
{
	...
	struct FVariableCopyContext
	{
		...
		FTransform				RebaseATransform;
		FTransform				RebaseBTransform;
...
public:
	...
	/** Returns true if we have any rebasing enabled. */
	bool IsRebaseEnabled() const { return CreateParameters.bAllowRebasing; }
	...
	const UNiagaraSimCacheDebugData* GetDebugData() const { return DebugData; }

#if WITH_EDITORONLY_DATA
	/** Remove the debug data from the cache */
	NIAGARA_API void RemoveDebugData();
#endif
