...
public:
	...
	[[nodiscard]] NIAGARA_API TObjectPtr<UObject> FindBinding(const FNiagaraVariable& InVariable) const;
	[[nodiscard]] NIAGARA_API UNiagaraDataInterface* FindDataInterface(const FNiagaraVariable& InVariable) const;

	[[nodiscard]] FNiagaraEmitterID GetEmitterID() const{ return FNiagaraEmitterID(EmitterIndex); }
