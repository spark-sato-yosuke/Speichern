...
public:
	...
	virtual void InitRHI(FRHICommandListBase& RHICmdList) override
	{
		...
		{
			FRHIBufferInitializer Initializer = RHICmdList.CreateBufferInitializer(CreateDesc.SetInitActionInitializer());
			Initializer.WriteData(&DefaultValue, sizeof(DefaultValue));
...
bool FNiagaraRenderer::ViewFamilySupportLowLatencyTranslucency(const FSceneViewFamily& ViewFamily)
{
	return
		...
		// Some of the other view modes short circuit rendering so the data may not be ready in time resulting in corruption / GPU hangs
		ViewFamily.ViewMode == VMI_Lit &&
		// when underwater, single layer water will render translucency before opaque
		!ViewFamily.Views.ContainsByPredicate([](const FSceneView* SceneView) { return SceneView->IsUnderwater(); });
}
