...
struct FNiagaraSimCacheHelper
{
	...
	void BuildCacheLayout(const FNiagaraSimCacheCreateParameters& CreateParameters, FNiagaraSimCacheDataBuffersLayout& CacheLayout, const FNiagaraDataSetCompiledData& CompiledData, FName LayoutName, TArray<FName> InRebaseVariableNames, TArray<FName> InInterpVariableNames, TConstArrayView<FName> ExplicitCaptureAttributes) const
	{
		...
		bool bHasPositionAttribute = false;
		...
		for (int32 i = 0; i < CompiledData.Variables.Num(); ++i)
		{
			...
			if (CreateParameters.AttributeCaptureMode == ENiagaraSimCacheAttributeCaptureMode::All || ExplicitCaptureAttributes.Contains(DataSetVariable.GetName()))
			{
				...
				bHasPositionAttribute |= DataSetVariable.GetType() == FNiagaraTypeDefinition::GetPositionDef();
			}
		}

		// We need to preserve the velocity attribute if we want to use velocity based extrapolation of positions
		// Note:  Only preserve the velocity attribute if it will be used (i.e. we have a position attribute we need to extrapolate)
		const FNiagaraVariableBase VelocityVariable(FNiagaraTypeDefinition::GetVec3Def(), "Velocity");
		if (CreateParameters.bAllowVelocityExtrapolation && bHasPositionAttribute)
		{
	...
	static bool BuildCacheReadMappings(FNiagaraSimCacheDataBuffersLayout& CacheLayout, const FNiagaraDataSetCompiledData& CompiledData)
	{
		...
		for (const FNiagaraSimCacheVariable& SourceVariable : CacheLayout.Variables)
		{
			...
			{
				CacheBufferReadInfo.ComponentMappingsToDataBuffer[FloatOffset++] = DestVariableLayout ? IntCastChecked<uint16>(DestVariableLayout->GetFloatComponentStart() + i) : uint16(INDEX_NONE);
			}

			for (int32 i = 0; i < SourceVariable.HalfCount; ++i)
			{
				CacheBufferReadInfo.ComponentMappingsToDataBuffer[HalfOffset++] = DestVariableLayout ? IntCastChecked<uint16>(DestVariableLayout->GetHalfComponentStart() + i) : uint16(INDEX_NONE);
			}

			for (int32 i = 0; i < SourceVariable.Int32Count; ++i)
			{
				CacheBufferReadInfo.ComponentMappingsToDataBuffer[Int32Offset++] = DestVariableLayout ? IntCastChecked<uint16>(DestVariableLayout->GetInt32ComponentStart() + i) : uint16(INDEX_NONE);
			}
		}
	...
	static void ReadCustomBuffers(uint32 NumInstances, float FrameFraction, float FrameDeltaSeconds, float SimDeltaSeconds, const FTransform& RebaseATransform, const FTransform& RebaseBTransform, uint16 ComponentVelocity, const TConstArrayView<FNiagaraSimCacheDataBuffersLayout::FVariableCopyMapping>& VariableCopyMappingsToDataBuffer, const FNiagaraSimCacheDataBuffers& CacheBufferA, const FNiagaraSimCacheDataBuffers& CacheBufferB, uint8* DestBuffer, uint32 DestStride)
	{
		...
		VariableCopyDataContext.RebaseATransform	= RebaseATransform;
		VariableCopyDataContext.RebaseBTransform	= RebaseBTransform;
	...
	void ReadDataBuffer(float FrameFraction, float FrameDeltaSeconds, float SimDeltaSeconds, const FTransform& RebaseATransform, const FTransform& RebaseBTransform, const FNiagaraSimCacheDataBuffersLayout& CacheLayout, const FNiagaraSimCacheDataBuffers& CacheBufferA, const FNiagaraSimCacheDataBuffers& CacheBufferB, FNiagaraDataSet& DataSet)
	{
		...
		if (CacheBufferA.NumInstances > 0 )
		{
			...
			ReadCustomBuffers(NumInstances, FrameFraction, FrameDeltaSeconds, SimDeltaSeconds, RebaseATransform, RebaseBTransform, CacheLayout.ComponentVelocity, CacheLayout.CacheBufferReadInfo.VariableCopyMappingsToDataBuffer, CacheBufferA, CacheBufferB, DataBuffer.GetComponentPtrFloat(0), DataBuffer.GetFloatStride());
		}
	...
	void ReadDataBufferGPU(float InFrameFraction, float InFrameDeltaSeconds, float InSimDeltaSeconds, const FTransform& InRebaseATransform, const FTransform& InRebaseBTransform, FNiagaraEmitterInstance& EmitterInstance, const FNiagaraSimCacheDataBuffersLayout& InCacheLayout, const FNiagaraSimCacheDataBuffers& InCacheBufferA, const FNiagaraSimCacheDataBuffers& InCacheBufferB, FNiagaraDataSet& InDataSet, std::atomic<int32>& InPendingCommandsCounter)
	{
		...
		++GPUExecContext->ParticleCountReadFence;
		GPUExecContext->bResetPending_GT = false;

		ENQUEUE_RENDER_COMMAND(NiagaraSimCacheGpuReadFrame)(
			[DispathInterface, GPUExecContext, ParticleCountFence=GPUExecContext->ParticleCountReadFence, FrameFraction=InFrameFraction, FrameDeltaSeconds=InFrameDeltaSeconds, SimDeltaSeconds=InSimDeltaSeconds, RebaseATransform=InRebaseATransform, RebaseBTransform=InRebaseBTransform, CacheLayout=&InCacheLayout, CacheBufferA=&InCacheBufferA, CacheBufferB=&InCacheBufferB, DataSet=&InDataSet, PendingCommandsCounter=&InPendingCommandsCounter](FRHICommandListImmediate& RHICmdList)
			{
				const uint32 NumInstances = FMath::Min(CacheBufferA->NumInstances, DataSet->GetMaxAllocationCount());
				...
				FNiagaraDataBuffer& DataBuffer = DataSet->GetCurrentDataChecked();
				{
					...
					DataBuffer.SetGPUInstanceCountBufferOffset(GPUExecContext->CountOffset_RT);

					GPUExecContext->CurrentNumInstances_RT = NumInstances;
					GPUExecContext->ParticleCountWriteFence = ParticleCountFence;
				...
				if (NumInstances > 0)
				{
					...
					if ( CacheLayout->FloatCount > 0 )
					{
						...
						ReadCustomBuffers(NumInstances, FrameFraction, FrameDeltaSeconds, SimDeltaSeconds, RebaseATransform, RebaseBTransform, CacheLayout->ComponentVelocity, CacheLayout->CacheBufferReadInfo_RT.VariableCopyMappingsToDataBuffer, *CacheBufferA, *CacheBufferB, RWBufferMemory, DataBuffer.GetFloatStride());
	...
	static void ExtrapolatePositions(const FNiagaraSimCacheDataBuffersLayout::FVariableCopyContext& CopyDataContext)
	{
		...
		for (uint32 i = 0; i < CopyDataContext.NumInstances; ++i)
		{
			...
			const FVector3f RebasedPosition = bWithRebase ? FVector3f(CopyDataContext.RebaseATransform.TransformPosition(FVector(ExtrapolatedPosition))) : ExtrapolatedPosition;
	...
	static void InterpPositions(const FNiagaraSimCacheDataBuffersLayout::FVariableCopyContext& CopyDataContext)
	{
		...
		if (bWithPrevious)
		{
			...
			for (uint32 iInstanceA = 0; iInstanceA < CopyDataContext.NumInstances; ++iInstanceA)
			{
				...
				FVector3f CurrPosA = SrcAPositions.Get(iInstanceA);
				FVector3f CurrPosB = iInstanceB == INDEX_NONE ? CurrPosA : SrcBPositions.Get(iInstanceB);
				FVector3f PrevPosA = SrcAPrevPositions.Get(iInstanceA);
				if ( bWithRebase )
				{
					CurrPosA = FVector3f(CopyDataContext.RebaseATransform.TransformPosition(FVector(CurrPosA)));
					CurrPosB = FVector3f(CopyDataContext.RebaseBTransform.TransformPosition(FVector(CurrPosB)));
					PrevPosA = FVector3f(CopyDataContext.RebaseATransform.TransformPosition(FVector(PrevPosA)));
				}
				...
				DstPositions.Set(iInstanceA, CurrPos);
				DstPrevPositions.Set(iInstanceA, PrevPos);
			}
		}
		else
		{
	...
	static void RebasePositions(const FNiagaraSimCacheDataBuffersLayout::FVariableCopyContext& CopyDataContext)
	{
		...
		for (uint32 i = 0; i < CopyDataContext.NumInstances; ++i)
		{
			...
			const FVector3f RebasedPosition = FVector3f(CopyDataContext.RebaseATransform.TransformPosition(FVector(CachePosition)));
			DstPositions.Set(i, RebasedPosition);
		}
	}
	...
	static void InterpQuaternions(const FNiagaraSimCacheDataBuffersLayout::FVariableCopyContext& CopyDataContext)
	{
		...
		const FQuat4f RebaseAQuat(CopyDataContext.RebaseATransform.GetRotation());
		const FQuat4f RebaseBQuat(CopyDataContext.RebaseBTransform.GetRotation());
		if (bWithPrevious)
		{
			...
			{
				const uint32 iInstanceB = CopyDataContext.InterpMappings[iInstanceA];
				FQuat4f CurrQuatA = SrcAQuats.Get(iInstanceA);
				FQuat4f CurrQuatB = iInstanceB == INDEX_NONE ? CurrQuatA : SrcBQuats.Get(iInstanceB);
				FQuat4f PrevQuatA = SrcAPrevQuats.Get(iInstanceA);
				if (bWithRebase)
				{
					CurrQuatA = CurrQuatA * RebaseAQuat;
					CurrQuatB = CurrQuatB * RebaseBQuat;
					PrevQuatA = PrevQuatA * RebaseAQuat;
				}
				...
				DstQuats.Set(iInstanceA, CurrQuat);
				DstPrevQuats.Set(iInstanceA, PrevQuat);
			}
		}
		else
		{
	...
	static void RebaseQuaternions(const FNiagaraSimCacheDataBuffersLayout::FVariableCopyContext& CopyDataContext)
	{
		...
		const FQuat4f RebaseQuat(CopyDataContext.RebaseATransform.GetRotation());
		for (uint32 i = 0; i < CopyDataContext.NumInstances; ++i)
		{
	...
	static void RebaseMatrices(const FNiagaraSimCacheDataBuffersLayout::FVariableCopyContext& CopyDataContext)
	{
		...
		const FMatrix44d RebaseMatrix = CopyDataContext.RebaseATransform.ToMatrixWithScale();
		for (uint32 i = 0; i < CopyDataContext.NumInstances; ++i)
		{
