...
namespace NDIStaticMeshLocal
{
	...
	struct FInstanceData_GameThread
	{
		...
		TWeakObjectPtr<UObject> ComponentWeakPtr;
		...
		void UpdateTransforms(UObject* Component, FNiagaraSystemInstance* SystemInstance)
		{
			...
			if (SceneComponentUtils->GetStaticMeshTransforms(Component, ComponentTransform, ISMTransforms))
			{
		...
		void UpdateTransformsResetPrevious(UObject* Component, FNiagaraSystemInstance* SystemInstance)
		{
			UpdateTransforms(Component, SystemInstance);
			PrevTransform = Transform;
			PrevTransformInverseTransposed = TransformInverseTransposed;
			PrevRotation = Rotation;
		}

		void UpdateTransformsCopyPrevious(UObject* Component, FNiagaraSystemInstance* SystemInstance)
		{
			PrevTransform = Transform;
			PrevTransformInverseTransposed = TransformInverseTransposed;
			PrevRotation = Rotation;
		...
		bool Init(UNiagaraDataInterfaceStaticMesh* Interface, FNiagaraSystemInstance* SystemInstance)
		{
			...
			UStaticMesh* StaticMesh = Interface->ResolveStaticMesh(Component, SystemInstance, MeshParameterBinding.GetValue());
			ComponentWeakPtr = Component;

			// Gather attached information
			InstanceIndex = Interface->InstanceIndex;
			UpdateTransformsResetPrevious(Component, SystemInstance);

			if (bUsePhysicsVelocity)
			{
				PhysicsVelocity = SceneComponentUtils->GetPhysicsLinearVelocity(Component);
			}
			...
			const EShaderPlatform ShaderPlatform = GShaderPlatformForFeatureLevel[SystemInstance->GetFeatureLevel()];
			...
			else if (StaticMesh->HasValidNaniteData() && !StaticMesh->HasNaniteFallbackMesh(ShaderPlatform))
			{
				UE_LOG(LogNiagara, Warning, TEXT("NiagaraStaticMeshDataInterface does not have a fallback mesh. Enable Nanite fallback mesh to fix this issue. System: %s, Mesh: %s"), *GetFullNameSafe(SystemInstance->GetSystem()), *GetFullNameSafe(StaticMesh));
				StaticMesh = nullptr;
			}
			...
			if (StaticMesh != nullptr)
			{
				...
				if (NumMeshSockets > 0)
				{
					...
					{
						UE_LOG(LogNiagara, Warning, TEXT("Static Mesh Data Interface '%s' requires more sockets '%d' than we currently support '%d' Mesh '%s' Component '%s'"), *GetFullNameSafe(Interface), NumMeshSockets, TNumericLimits<uint16>::Max(), *GetFullNameSafe(StaticMesh), *GetFullNameSafe(Component));
						return false;
					}
					...
					if (Interface->FilteredSockets.Num() > 0)
					{
						...
						for (int32 i = 0; i < Interface->FilteredSockets.Num(); ++i)
						{
							...
							{
								UE_LOG(LogNiagara, Warning, TEXT("Static Mesh Data Interface '%s' could not find socket '%s' Mesh '%s' Component '%s'"), *GetFullNameSafe(Interface), *FilteredSocketName.ToString(), *GetFullNameSafe(StaticMesh), *GetFullNameSafe(Component));
								continue;
							}
							++NumFilteredSockets;
		...
		bool Tick(UNiagaraDataInterfaceStaticMesh* Interface, FNiagaraSystemInstance* SystemInstance, float InDeltaSeconds)
		{
			...
			UObject* Component = ComponentWeakPtr.Get();
			if (bCaptureTransformsPerFrame)
			{
				UpdateTransformsCopyPrevious(Component, SystemInstance);
			}
		...
		bool ResetRequired(UNiagaraDataInterfaceStaticMesh* Interface, FNiagaraSystemInstance* Instance)
		{
			...
			{
				ResolvedMesh = Interface->ResolveStaticMesh(ResolvedComponent, Instance, MeshParameterBinding.GetValue());
			}
		...
		const FStaticMeshLODResources* GetCurrentLODWithVertexColorOverrides(FColorVertexBuffer*& OutVertexColorOverrides)
		{
			...
			if (bMeshValid && StaticMesh)
			{
				...
				if (CachedLODIdx >= CurrentFirstLOD && StaticMesh->GetRenderData()->LODResources.IsValidIndex(CachedLODIdx))
				{
					OutVertexColorOverrides = SceneComponentUtils->GetStaticMeshOverrideColors(ComponentWeakPtr.Get(), CachedLODIdx);
...
void UNiagaraDataInterfaceStaticMesh::ProvidePerInstanceDataForRenderThread(void* InDataFromGT, void* InInstanceData, const FNiagaraSystemInstanceID& SystemInstance)
{
	...
	DataFromGT->DistanceFieldPrimitiveId = InstanceData->SceneComponentUtils->GetPrimitiveSceneId(InstanceData->ComponentWeakPtr.Get());
}
...
void UNiagaraDataInterfaceStaticMesh::DrawDebugHud(FNDIDrawDebugHudContext& DebugHudContext) const
{
	...
	UObject* Component = InstanceData_GT->ComponentWeakPtr.Get();
	UStaticMesh* SkeletalMesh = InstanceData_GT->StaticMeshWeakPtr.Get();
	DebugHudContext.GetOutputString().Appendf(TEXT("StaticMesh(%s) StaticMeshComp(%s)"), *GetNameSafe(SkeletalMesh), *GetNameSafe(Component));
}
#endif
...
UStaticMesh* UNiagaraDataInterfaceStaticMesh::ResolveStaticMesh(UObject*& OutComponent, FNiagaraSystemInstance* SystemInstance, UObject* ParameterBindingValue) const
{
	...
	if (bTrySource && SourceComponent.IsValid())
	{
		OutComponent = SourceComponent.Get();
		OutMesh = SourceComponent->GetStaticMesh();
	}
	else if (bTrySource && SoftSourceActor.Get())
	{
		PRAGMA_DISABLE_DEPRECATION_WARNINGS
		const INiagaraSceneComponentUtils& SceneComponentUtils = FNiagaraSystemInstance::GetSceneComponentUtils(SystemInstance);
		PRAGMA_ENABLE_DEPRECATION_WARNINGS

		SceneComponentUtils.ResolveStaticMesh(SoftSourceActor.Get(), false, OutComponent, OutMesh);
	}
	else
	{
		PRAGMA_DISABLE_DEPRECATION_WARNINGS
		...
		{
			SceneComponentUtils.ResolveStaticMesh(ParameterBindingValue, true, OutComponent, OutMesh);
		}

		// Try attached parent, only if we did not already find a valid component / mesh
		if (bTryAttachParent && SystemInstance && !OutComponent && !OutMesh)
		{
			SceneComponentUtils.ResolveStaticMesh(true, OutComponent, OutMesh);
		}
	}
