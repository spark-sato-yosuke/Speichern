...
UNiagaraEmitter* UNiagaraEmitter::DuplicateWithoutMerging(UObject* InOuter)
{
	UNiagaraEmitter* Duplicate;
	{
		...
		// Reset the linker to the last version to make sure PostLoad on the duplicated object is not doing any data conversion, squashing new properties with not properly initialized _DEPRECATED members.
		ResetLoaders(InOuter->GetPackage());
		FObjectDuplicationParameters DuplicationParameters(this, InOuter);
		DuplicationParameters.FlagMask = GetFlags() & ~(RF_Standalone | RF_Public);
		Duplicate = Cast<UNiagaraEmitter>(StaticDuplicateObjectEx(DuplicationParameters));
	}
	return Duplicate;
}
#endif
...
void UNiagaraEmitter::PostLoad()
{
	...
	for (FVersionedNiagaraEmitterData& Data : VersionData)
	{
		...
		if (UE5MainVer < FUE5MainStreamObjectVersion::FixGpuAlwaysRunningUpdateScriptNoneInterpolated)
		{
			...
			{
				Data.bGpuAlwaysRunParticleUpdateScript = Data.UsesInterpolatedSpawning();
			}
			else
			{
...
void FVersionedNiagaraEmitterData::PostLoad(UNiagaraEmitter& Emitter, int32 NiagaraVer)
{
	...
	if (NiagaraVer < FNiagaraCustomVersion::AddedNewInterpolatedSpawnMode && bInterpolatedSpawning_DEPRECATED)
	{
		InterpolatedSpawnMode = ENiagaraInterpolatedSpawnMode::Interpolation;
	}
#endif
	...
	if (SpawnScriptProps.Script)
	{
		...
		if (UsesInterpolatedSpawning() != bActualInterpolatedSpawning)
		{
			InterpolatedSpawnMode = ENiagaraInterpolatedSpawnMode::NoInterpolation;
...
void UNiagaraEmitter::PostEditChangeVersionedProperty(FPropertyChangedEvent& PropertyChangedEvent, const FGuid& Version)
{
	...
	if (PropertyName == GET_MEMBER_NAME_CHECKED(FVersionedNiagaraEmitterData, InterpolatedSpawnMode))
	{
		ENiagaraInterpolatedSpawnMode ScriptInterpolationMode = EmitterData->SpawnScriptProps.Script->GetComputedVMCompilationId().InterpolatedSpawnMode;
		ENiagaraInterpolatedSpawnMode EmitterInterpolationMode = EmitterData->InterpolatedSpawnMode;
		if (EmitterInterpolationMode != ScriptInterpolationMode)
		{
			//Recompile spawn script if we've altered the interpolated spawn property.
			EmitterData->SpawnScriptProps.Script->SetUsage(EmitterData->UsesInterpolatedSpawning() ? ENiagaraScriptUsage::ParticleSpawnScriptInterpolated : ENiagaraScriptUsage::ParticleSpawnScript);
			UE_LOG(LogNiagara, Log, TEXT("Updating script usage: Script->IsInterpolatdSpawn %d Emitter->bInterpolatedSpawning %d"), (int32)EmitterData->SpawnScriptProps.Script->IsInterpolatedParticleSpawnScript(), EmitterData->UsesInterpolatedSpawning());
...
void UNiagaraEmitter::InvalidateCachedTraversalData(const FGuid& EmitterVersion)
{
	const FVersionedNiagaraEmitterData* EmitterData = GetEmitterData(EmitterVersion);
	if (EmitterData->CachedTraversalData.IsValid() && EmitterData->CachedTraversalData->IsValidForEmitter(EmitterData))
	{
		EmitterData->CachedTraversalData.Reset();
	}
}
...
UNiagaraScript* FVersionedNiagaraEmitterData::GetScript(ENiagaraScriptUsage Usage, FGuid UsageId) const
{
	TArray<UNiagaraScript*> Scripts;
...
void FVersionedNiagaraEmitterData::CacheFromCompiledData(const FNiagaraDataSetCompiledData* CompiledData, const UNiagaraEmitter& Emitter)
{
	...
	if (CompiledData != nullptr)
	{
		...
		const uint64 MaxBufferElements = (GNiagaraEmitterMaxGPUBufferElements > 0) ? uint64(GNiagaraEmitterMaxGPUBufferElements) : MaxViewDimensionForTypedBuffer;
		const uint64 ClampedMaxBufferElements = (int32)FMath::Min<int64>(MaxBufferElements, TNumericLimits<int32>::Max());
		...
		MaxAllocationCount = (uint32)FMath::Min(ClampedMaxBufferElements / MaxGPUBufferComponents, (uint64)UINT_MAX);
		...
		if (SimTarget == ENiagaraSimTarget::GPUComputeSim)
		{
			...
			if (MaxAllocationCount < NiagaraComputeMaxThreadGroupSize)
			{
				...
				check(MaxAllocationCount * MaxGPUBufferComponents <= MaxViewDimensionForTypedBuffer);
			}
...
bool FVersionedNiagaraEmitterData::UsesInterpolatedSpawning() const
{
	return InterpolatedSpawnMode != ENiagaraInterpolatedSpawnMode::NoInterpolation;
}
...
void UNiagaraEmitter::CheckVersionDataAvailable()
{
	...
	Data.bInterpolatedSpawning_DEPRECATED = bInterpolatedSpawning_DEPRECATED;
	Data.InterpolatedSpawnMode = bInterpolatedSpawning_DEPRECATED ? ENiagaraInterpolatedSpawnMode::Interpolation : ENiagaraInterpolatedSpawnMode::NoInterpolation;
...
void UNiagaraEmitter::ResolveScalabilitySettings()
{
	for (FVersionedNiagaraEmitterData& EmitterData : VersionData)
	{
		...
		// Clear out any allocation estimates as changing scalability could change the max spawn count
		if (EmitterData.RuntimeEstimation.AllocationEstimate > 0)
		{
			EmitterData.ClearRuntimeAllocationEstimate(INDEX_NONE);
		}
	}
}
