...
void FNiagaraSystemRenderData::GenerateSetDynamicDataCommands(FSetDynamicDataCommandList& Commands, const FNiagaraSceneProxy& SceneProxy, const FNiagaraSystemInstance* SystemInstance, TConstArrayView<FMaterialOverride> MaterialOverrides)
{
	...
	for (int32 i = 0; i < SystemInstance->GetEmitters().Num(); i++)
	{
		...
		EmitterInst->ForEachEnabledRenderer(
			...
			{
			#if WITH_EDITOR
				if ( RendererIndex >= NumEmitterRenderers )
				{
					++RendererIndex;
					return;
				}
	...
	if (ensure(RendererIndex == NumEmitterRenderers) == false)
	{
		...
		if ( RendererIndex > NumEmitterRenderers )
		{
			Commands.SetNum(NumEmitterRenderers);
		}
	}
#endif
}
...
#if WITH_EDITOR
	// In editor builds we may have cases where modifications to the enabled state is inside undo / redo and we have no pre/post calls to execute an update context.
	// We can handle these invalid cases by clearing the renderers and logging an ensure, when we have a solid repro we can investigate fixing, or we can separate out editor vs runtime data.
	if (!ensureMsgf(EmitterRenderers_GT.Num() == 0|| (EmitterRenderers_GT.Num() == RendererDrawOrder.Num()), TEXT("EmitterRenderers Num %d does not match System DrawOrder %d"), EmitterRenderers_GT.Num(), RendererDrawOrder.Num()))
	{
		EmitterRenderers_GT.Empty();
	}
#else
	// In cooked builds the enabled state will not be mutated so any failures here are fatals
