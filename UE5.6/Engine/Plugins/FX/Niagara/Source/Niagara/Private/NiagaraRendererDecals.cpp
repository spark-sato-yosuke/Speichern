...
#define NIAGARARENDERERDECALS_DEBUG_ENABLED !UE_BUILD_SHIPPING
	...
	{
		if (World == nullptr || AllUpdateParams.Num() == 0)
		{
			return;
		}
		...
		AsyncTask(
			...
			[WeakWorld=MakeWeakObjectPtr(World), AllUpdateParams_GT=TArray<FDeferredDecalUpdateParams>(AllUpdateParams), WorldTime]
			{
				...
				for (const FDeferredDecalUpdateParams& UpdateParams : AllUpdateParams_GT)
				{
					...
					const FMatrix DecalMatrix = UpdateParams.Transform.ToMatrixWithScale();
					const FBox BoundsBox(-FVector(1), FVector(1));
					const FColor DecalColor = UpdateParams.DecalColor.ToFColor(false);
					LineBatcher->DrawBox(BoundsBox, DecalMatrix, DecalColor, 0, 0.0f);

					const FVector StartLocation = UpdateParams.Transform.TransformPosition(FVector(-1.0f, 0.0f, 0.0f));
					const FVector MidLocation = UpdateParams.Transform.TransformPosition(FVector(0.0f, 0.0f, 0.0f));
					const float ArrowSize = FMath::Min(UpdateParams.Transform.GetScale3D().Y, UpdateParams.Transform.GetScale3D().Z) * 0.5f;
					
					LineBatcher->DrawDirectionalArrow(StartLocation, MidLocation, ArrowSize * ArrowSize, FLinearColor::Red, 0.0f, 0, 1.0f);
				}
			}
		);
	}
