...
#define LOCTEXT_NAMESPACE "NiagaraSimCache"
...
bool UNiagaraSimCache::ReadFrame(int32 FrameIndex, float FrameFraction, FNiagaraSystemInstance* SystemInstance) const
{
	...
	FTransform RebaseATransform = FTransform::Identity;
	FTransform RebaseBTransform = FTransform::Identity;
	if ( USceneComponent* AttachComponent = SystemInstance->GetAttachComponent() )
	{
		// Note: We remove the scaling as how that is applied to uncached emitters can not be represented by applying a transform.
		//       If we desire to control this an option can be exposed on the cache.
		FTransform LocalToWorld = AttachComponent->GetComponentToWorld();
		LocalToWorld.AddToTranslation(FVector(SystemInstance->GetLWCTile()) * -FLargeWorldRenderScalar::GetTileSize());
		LocalToWorld.RemoveScaling();

		RebaseATransform = CacheFrame.LocalToWorld;
		RebaseATransform.RemoveScaling();
		RebaseBTransform = CacheFrames[NextFrameIndex].LocalToWorld;
		RebaseBTransform.RemoveScaling();

		RebaseATransform = RebaseATransform.Inverse() * LocalToWorld;
		RebaseBTransform = RebaseBTransform.Inverse() * LocalToWorld;
	}
	...
	Helper.ReadDataBuffer(FrameFraction, FrameDeltaSeconds, SimDeltaSeconds, RebaseATransform, RebaseBTransform, CacheLayout.SystemLayout, CacheFrame.SystemData.SystemDataBuffers, CacheFrame.SystemData.SystemDataBuffers, Helper.GetSystemSimulationDataSet());
	...
	for (int32 i=0; i < NumEmitters; ++i)
	{
		...
		if (CacheLayout.EmitterLayouts[i].SimTarget == ENiagaraSimTarget::GPUComputeSim)
		{
			Helper.ReadDataBufferGPU(FrameFraction, FrameDeltaSeconds, SimDeltaSeconds, RebaseATransform, RebaseBTransform, EmitterInstance, CacheLayout.EmitterLayouts[i], CacheEmitterFrame.ParticleDataBuffers, CacheEmitterFrameB.ParticleDataBuffers, EmitterInstance.GetParticleData(), PendingCommandsInFlight);
		}
		else
		{
			Helper.ReadDataBuffer(FrameFraction, FrameDeltaSeconds, SimDeltaSeconds, RebaseATransform, RebaseBTransform, CacheLayout.EmitterLayouts[i], CacheEmitterFrame.ParticleDataBuffers, CacheEmitterFrameB.ParticleDataBuffers, EmitterInstance.GetParticleData());
		}
	}
	...
	{
		TSet<FNiagaraVariableBase> VisitedDataInterfaces;
		VisitedDataInterfaces.Reserve(DataInterfaceStorage.Num());

		FNiagaraSimCacheDataInterfaceReadContext DIReadContext;
		DIReadContext.SystemInstance		= Helper.SystemInstance;
		DIReadContext.FrameIndexA			= FrameIndex;
		DIReadContext.FrameIndexB			= NextFrameIndex;
		DIReadContext.FrameInterp			= FrameFraction;
		DIReadContext.RebaseTransformA		= RebaseATransform;
		DIReadContext.RebaseTransformB		= RebaseBTransform;
		DIReadContext.LWCTileA				= CacheFrames[FrameIndex].LWCTile;
		DIReadContext.LWCTileB				= CacheFrames[NextFrameIndex].LWCTile;
			...
			[&](const FNiagaraVariableBase& Variable, UNiagaraDataInterface* DataInterface)
			{
				if (UObject* StorageObject = DataInterfaceStorage.FindRef(Variable))
				{
					if (INiagaraSimCacheCustomStorageInterface* SimCacheCustomStorageInterface = Cast<INiagaraSimCacheCustomStorageInterface>(DataInterface))
					{
						...
						DIReadContext.StorageObject				= StorageObject;
						DIReadContext.OptionalPerInstanceData	= Helper.SystemInstance->FindDataInterfaceInstanceData(DataInterface);

						// Determine the interpolation state for the data interface
						if (CreateParameters.bAllowInterpolation)
						{
							if (CreateParameters.InterpolationIncludeAttributes.Contains(Variable.GetName()))
							{
								DIReadContext.InterpolateData = true;
							}
							else if (CreateParameters.InterpolationExcludeAttributes.Contains(Variable.GetName()))
							{
								DIReadContext.InterpolateData = false;
							}
							else
							{
								DIReadContext.InterpolateData.Reset();
							}
						}
						else
						{
						...
						// Determine the rebasing state for the data interface
						if (CreateParameters.bAllowRebasing)
						{
							if (CreateParameters.RebaseIncludeAttributes.Contains(Variable.GetName()))
							{
								DIReadContext.RebaseData = true;
							}
							else if (CreateParameters.RebaseExcludeAttributes.Contains(Variable.GetName()))
							{
								DIReadContext.RebaseData = false;
							}
							else
							{
								DIReadContext.RebaseData.Reset();
							}
						}
						else
						{
						...
						bDataInterfacesSucess &= SimCacheCustomStorageInterface->SimCacheReadFrame(DIReadContext);
					}
				}
				return true;
			},
...
#if WITH_EDITORONLY_DATA
void UNiagaraSimCache::RemoveDebugData()
{
	...
	const FScopedTransaction Transaction(LOCTEXT("RemoveDebugData", "Sim Cache Remove Debug Data"));
