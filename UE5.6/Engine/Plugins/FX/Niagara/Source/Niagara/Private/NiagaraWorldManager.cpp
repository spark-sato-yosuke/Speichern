...
bool GNiagaraWorldManagerUseTickPasses = false;
static FAutoConsoleVariableRef CVarNiagaraWorldManagerUseTickPasses(
	TEXT("fx.Niagara.WorldManager.UseTickPasses"),
	GNiagaraWorldManagerUseTickPasses,
	TEXT("When enabled we will use the tick passes to avoid blocking overlapping sims at tick boundaries."),
	ECVF_Default
);
...
FAutoConsoleCommandWithWorldAndArgs DumpNiagaraWorldManagerCommand(
	...
	FConsoleCommandWithWorldAndArgsDelegate::CreateLambda(
		[](const TArray<FString>& Args, UWorld* World)
		{
			...
			{
				const bool bDetailed = Args.Num() > 0 ? FCString::Atoi(*Args[0]) != 0 : false;

				WorldManager->DumpDetails(*GLog, bDetailed);
			}
		}
	)
);
...
namespace NiagaraWorldManagerInternal
{
	...
	static bool GTickParameterCollectionsOncePerFrame = true;
	static FAutoConsoleVariableRef CVarTickParameterCollectionsOncePerFrame(
		TEXT("fx.Niagara.WorldManager.TickParameterCollectionsOncePerFrame"),
		GTickParameterCollectionsOncePerFrame,
		TEXT("When true parameter collection updates will occur once per frame in the first tick group.\n")
		TEXT("When false updates will occur every tick group.\n")
		TEXT("Updating constantly can cost performance but can fix issues with NPC values updated then used from a system in a later tick group, this would be a frame out without updateing each tick group."),
...
{
	check(Owner);
	Owner->Tick(TickPass, TickGroup, DeltaTime, MyCompletionGraphEvent);
}
...
FNiagaraWorldManager::FNiagaraWorldManager()
	...
	, bUseTickPasses(GNiagaraWorldManagerUseTickPasses)
	, bAppHasFocus(true)
...
void FNiagaraWorldManager::Init(UWorld* InWorld)
{
	...
	{
		for (int32 iTickPass=0; iTickPass < int32(ENiagaraWorldManagerTickPass::Num); ++iTickPass )
		{
			const ENiagaraWorldManagerTickPass TickPass = ENiagaraWorldManagerTickPass(iTickPass);

			FNiagaraWorldManagerTickFunction& TickFunc = TickFunctions[TickGroup][iTickPass];
			TickFunc.TickGroup						= ETickingGroup(NiagaraFirstTickGroup + TickGroup);
			TickFunc.EndTickGroup					= TickPass == ENiagaraWorldManagerTickPass::Overlapping ? TG_LastDemotable : (ETickingGroup)TickFunc.TickGroup;
			TickFunc.bCanEverTick					= true;
			TickFunc.bStartWithTickEnabled			= TickPass == ENiagaraWorldManagerTickPass::Overlapping;
			...
			TickFunc.Owner							= this;
			TickFunc.TickPass						= TickPass;
			TickFunc.SetTickFunctionEnable(TickFunc.bStartWithTickEnabled);
			TickFunc.RegisterTickFunction(InWorld->PersistentLevel);
			if (iTickPass > 0)
			{
				TickFunc.AddPrerequisite(ComponentPool, TickFunctions[TickGroup][iTickPass - 1]);
			}
		}
	}
...
FNiagaraSystemSimulationRef FNiagaraWorldManager::GetSystemSimulation(ETickingGroup TickGroup, UNiagaraSystem* System)
{
	...
	const bool bCanOverlapTickGroups = System->AsyncWorkCanOverlapTickGroups();
	const ENiagaraWorldManagerTickPass TickPass = bCanOverlapTickGroups || bUseTickPasses == false ? ENiagaraWorldManagerTickPass::Overlapping : ENiagaraWorldManagerTickPass::NoneOverlapping;
	FNiagaraSystemSimulationRef* SimPtr = SystemSimulations[ActualTickGroup][int(TickPass)].Find(System);
	if (SimPtr != nullptr)
	{
		return *SimPtr;
	}
	...
	SystemSimulations[ActualTickGroup][int(TickPass)].Add(System, Sim);
	Sim->Init(System, World, false, TickGroup);

	// Old path which will block simulation at tick boundaries
	if (bUseTickPasses == false)
	{
		...
		if(!bCanOverlapTickGroups)
		{
			TickFunctions[ActualTickGroup][int(TickPass)].EndTickGroup = (ETickingGroup)ActualTickGroup;
		}
	}
	// New path which will only block none overlapping simulations are tick boundaries
	else
	{
		// Ensure tick function is running
		if (TickFunctions[ActualTickGroup][int(TickPass)].IsTickFunctionEnabled() == false )
		{
			TickFunctions[ActualTickGroup][int(TickPass)].SetTickFunctionEnable(true);
		}
	}
	...
	{
		SystemPostChangeDelegateHandles.Emplace(System, System->OnSystemPostEditChange().AddRaw(this, &FNiagaraWorldManager::OnSystemPostChange));
	}
#endif
...
void FNiagaraWorldManager::DestroySystemSimulation(UNiagaraSystem* System)
{
	...
	{
		for (int iTickPass=0; iTickPass < int(ENiagaraWorldManagerTickPass::Num); ++iTickPass)
		{
			FNiagaraSystemSimulationRef* Simulation = SystemSimulations[TG][iTickPass].Find(System);
			if (Simulation != nullptr)
			{
				...
				(*Simulation)->Destroy();
				SystemSimulations[TG][iTickPass].Remove(System);
			}
		}
	}
	if (ComponentPool)
...
void FNiagaraWorldManager::OnWorldCleanup(bool bSessionEnded, bool bCleanupResources)
{
	...
	for (int TG = 0; TG < NiagaraNumTickGroups; ++TG)
	{
		for (int iTickPass = 0; iTickPass < int(ENiagaraWorldManagerTickPass::Num); ++iTickPass)
		{
			for (TPair<UNiagaraSystem*, FNiagaraSystemSimulationRef>& SimPair : SystemSimulations[TG][iTickPass])
			{
				SimPair.Value->Destroy();
			}
			SystemSimulations[TG][iTickPass].Empty();
		}
	}
...
{
	// Post garbage collection we must mark parameter stores with UObjects dirty to ensure any objects which are nullptr are updated in the various parameter stores.
	// We must also mark the component render data as dirty to ensure any material references in renderers are updated as they could also have become nullptr.
	for (TObjectIterator<UNiagaraComponent> ComponentIt; ComponentIt; ++ComponentIt)
	{
		...
		if (ComponentIt->GetOverrideParameters().GetUObjects().Num() > 0)
		{
			ComponentIt->MarkRenderStateDirty();
		}
	}
}
...
void FNiagaraWorldManager::PostActorTick(float DeltaSeconds)
{
	...
	{
		// Tick Parameter Collections to ensure they are up to date
		if (NiagaraWorldManagerInternal::GTickParameterCollectionsOncePerFrame == false)
		{
...
void FNiagaraWorldManager::Tick(ENiagaraWorldManagerTickPass TickPass, ETickingGroup TickGroup, float DeltaSeconds, const FGraphEventRef& MyCompletionGraphEvent)
{
	...
	// Determine what work needs to run
	const int ActualTickGroup = NiagaraWorldManagerInternal::GetNiagaraTickGroup(TickGroup);
	const FNiagaraWorldManagerTickFunction& NonOverlappingTickFunction = TickFunctions[ActualTickGroup][int(ENiagaraWorldManagerTickPass::NoneOverlapping)];
	const bool bIsFirstPass = TickPass == ENiagaraWorldManagerTickPass::First || NonOverlappingTickFunction.IsTickFunctionEnabled() == false;
	const bool bIsLastPass = TickPass == ENiagaraWorldManagerTickPass::Last || NonOverlappingTickFunction.IsTickFunctionEnabled() == false;

	// Execute work at the start of each tick group
	if (bIsFirstPass)
	{
		ExecutePreTickWork(TickGroup, DeltaSeconds);
	}
	...
	// Tick generated data (First tick pass only)
	if (bIsFirstPass)
	{
	...
	// Execute simulations for this tick group
	ExecuteSimulations(TickPass, TickGroup, DeltaSeconds, MyCompletionGraphEvent);

	// Execute work at the end of each tick group
	if (bIsLastPass)
	{
		ExecutePostTickWork(TickGroup, DeltaSeconds);
	}
}

void FNiagaraWorldManager::ExecutePreTickWork(ETickingGroup TickGroup, float DeltaSeconds)
{
...
void FNiagaraWorldManager::ExecuteSimulations(ENiagaraWorldManagerTickPass TickPass, ETickingGroup TickGroup, float DeltaSeconds, const FGraphEventRef& MyCompletionGraphEvent)
{
	...
	bool bHasSimulationsInTickPass = false;
	bool bCanOverlapTickGroups = true;
	for (auto SimIt=SystemSimulations[ActualTickGroup][int(TickPass)].CreateIterator(); SimIt; ++SimIt)
	{
		FNiagaraSystemSimulation* Sim = &SimIt.Value().Get();

		if (!SimIt.Value().IsUnique() && Sim->IsValid())
		{
			bHasSimulationsInTickPass = true;
			Sim->Tick_GameThread(DeltaSeconds, MyCompletionGraphEvent);
			bCanOverlapTickGroups &= Sim->GetSystem()->AsyncWorkCanOverlapTickGroups();
		}
		else
		{
			Sim->Destroy();
	...
	// New path that uses different tick passes for overlapping / non overlapping work
	if (bUseTickPasses)
	{
		// Should we disable the none overlapping tick function from running?
		if (TickPass == ENiagaraWorldManagerTickPass::NoneOverlapping && !bHasSimulationsInTickPass)
		{
			FNiagaraWorldManagerTickFunction& NonOverlappingTickFunction = TickFunctions[ActualTickGroup][int(ENiagaraWorldManagerTickPass::NoneOverlapping)];
			NonOverlappingTickFunction.SetTickFunctionEnable(false);
		}
	}
	// Old tick path which modifies the end tick group which can cause stalls
	else
	{
		...
		if ( bCanOverlapTickGroups )
		{
			...
			TickFunctions[ActualTickGroup][int(TickPass)].EndTickGroup = TG_LastDemotable;
		}
		else
		{
			TickFunctions[ActualTickGroup][int(TickPass)].EndTickGroup = (ETickingGroup)ActualTickGroup;
...
void FNiagaraWorldManager::ExecutePostTickWork(ETickingGroup TickGroup, float DeltaSeconds)
{
...
void FNiagaraWorldManager::DumpDetails(FOutputDevice& Ar, bool bDetailed)
{
	Ar.Logf(TEXT("=== FNiagaraWorldManager Dumping Information"));

	const UEnum* TickingGroupEnum = StaticEnum<ETickingGroup>();
	for ( int TG=0; TG < NiagaraNumTickGroups; ++TG )
	{
		TStringBuilder<8> TickStateString;
		int32 TotalSimulations = 0;
		for (int iTickPass=0; iTickPass < int(ENiagaraWorldManagerTickPass::Num); ++iTickPass)
		{
			TickStateString.Append(TickFunctions[TG][iTickPass].IsTickFunctionEnabled() ? TEXT("X") : TEXT("."));
			TotalSimulations += SystemSimulations[TG][iTickPass].Num();
		}

		Ar.Logf(
			TEXT("%-20s | TickState(%s) | %d Simulations"),
			*TickingGroupEnum->GetNameStringByIndex(TG + NiagaraFirstTickGroup),
			TickStateString.ToString(),
			TotalSimulations
		);
		...
		if (bDetailed)
		{
			for (int iTickPass = 0; iTickPass < int(ENiagaraWorldManagerTickPass::Num); ++iTickPass)
			{
				for (TPair<UNiagaraSystem*, FNiagaraSystemSimulationRef>& SystemSim : SystemSimulations[TG][iTickPass])
				{
...
#if DO_CHECK
	TGuardValue<bool> GuardScalabilityManagersLocked(bScalabilityManagersLocked, true);
#endif
...
void FNiagaraWorldManager::RegisterWithScalabilityManager(UNiagaraComponent* Component, UNiagaraEffectType* EffectType)
{
	...
	if ( EffectType )
	{
		...
		{
		#if DO_CHECK
			ensureMsgf(bScalabilityManagersLocked == false, TEXT("ScalabilityManagers modified while iterating, by Component (%s) System (%s)"), *GetNameSafe(Component), *GetNameSafe(Component ? Component->GetAsset() : nullptr));
		#endif
...
{
	if (GNigaraAllowPrimedPools && HasActiveWorld() && World->IsGameWorld() && ComponentPool)
	{
...
#if WITH_NIAGARA_DEBUGGER
void FNiagaraWorldManager::AddDebugHUDMessage(FName Key, const FNiagaraDebugMessage& Message)
{
	if(NiagaraDebugHud)
	{
		NiagaraDebugHud->AddMessage(Key, Message);
	}
}
#endif
...
void FNiagaraWorldManager::SetTickGroupPriority(const int32 NiagaraTickGroupIndex, const bool bHighPriority)
{
	...
	for ( int iTickPass=0; iTickPass < int(ENiagaraWorldManagerTickPass::Num); ++iTickPass)
	{
		TickFunctions[NiagaraTickGroupIndex][iTickPass].SetPriorityIncludingPrerequisites(bHighPriority);
	}
}
