	  ...
	  , FeatureLevel(GMaxRHIFeatureLevel)
{
	...
	if (UNiagaraComponent* NiagaraComponent = Cast<UNiagaraComponent>(InAttachComponent))
	{
		PRAGMA_DISABLE_DEPRECATION_WARNINGS
		SceneComponentUtils.Reset(NiagaraComponent->CreateSceneComponentUtils());
...
void FNiagaraSystemInstance::UpdateLWCTile(const FVector3f& InLWCTile, FNiagaraDataSet& EmitterDataSet)
{
	...
	{
		case ENiagaraLwcTileUpdateMode::ResetSimulation:
			bResetSimulation = true;
			break;

		case ENiagaraLwcTileUpdateMode::RebaseOrResetSimulation:
			bResetSimulation = TileDifference.GetAbsMax() >= GetDefault<UNiagaraSettings>()->LargeWorldCoordinateMaxTilesBeforeReset;
			break;
	}

	// Should we reset the system simulation
	if ( bResetSimulation )
	{
		if (TickCount > 1)
		{
			UE_LOG(LogNiagara, Warning, TEXT("NiagaraComponent(%s - %s) required LWC tile recache and was reset."), *GetFullNameSafe(GetAttachComponent()), *GetFullNameSafe(System));
		}
		Reset(FNiagaraSystemInstance::EResetMode::ResetAll);
		return;
	}

	// Rebase internal variables
	const FVector LWCTileShift = FVector(TileDifference) * UE_LWC_RENDER_TILE_SIZE;
	const FVector3f TileShift(LWCTileShift);

	WorldTransform.AddToTranslation(LWCTileShift);
	if (PreviousLocation.IsSet())
	{
		PreviousLocation = PreviousLocation.GetValue() + LWCTileShift;
	}

	for (FNiagaraOwnerParameters& Parameters : OwnerParameters)
	{
		Parameters.EngineLocalToWorld			= Parameters.EngineLocalToWorld.ConcatTranslation(TileShift);
		Parameters.EngineWorldToLocal			= Parameters.EngineLocalToWorld.Inverse();
		Parameters.EngineLocalToWorldTransposed	= Parameters.EngineLocalToWorld.GetTransposed();
		Parameters.EngineWorldToLocalTransposed	= Parameters.EngineWorldToLocal.GetTransposed();
		Parameters.EngineLocalToWorldNoScale	= Parameters.EngineLocalToWorldNoScale.ConcatTranslation(TileShift);
		Parameters.EngineWorldToLocalNoScale	= Parameters.EngineLocalToWorldNoScale.Inverse();
		Parameters.EnginePosition				+= TileShift;
		Parameters.EngineLWCTile				= InLWCTile;

	}

	GatheredInstanceParameters.ComponentTrans.AddToTranslation(LWCTileShift);

	// Rebase data sets
	NiagaraLWCHelper::RebaseDataSet(TileShift, EmitterDataSet, SystemInstanceIndex);
	NiagaraLWCHelper::RebaseEmitters(TileShift, Emitters, ComputeDispatchInterface);

	//-TODO: Rebase data interface data

	// Set the new tile and invalidate the render state to ensure the proxy is updated with the latest tile
	LWCTile = InLWCTile;
...
void FNiagaraSystemInstance::MultiTick(int32 TicksToProcess, float DeltaSecondsPerTick, float MaxSimTime, const FGraphEventRef& MyCompletionGraphEvent)
{
	SCOPE_CYCLE_COUNTER(STAT_NiagaraOverview_GT);
	SCOPE_CYCLE_COUNTER(STAT_NiagaraSystemInst_ComponentTickGT);
	CSV_SCOPED_TIMING_STAT_EXCLUSIVE(Effects);
	...
	TSharedPtr<FNiagaraSystemSimulation> SystemSim = GetSystemSimulation();
	check(SystemSim.IsValid());
	check(IsInGameThread());
	check(bSolo);

	SystemSim->MultiTick_GameThread(TicksToProcess, DeltaSecondsPerTick, MaxSimTime, MyCompletionGraphEvent);
}
...
void FNiagaraSystemInstance::SimCacheTick_GameThread(UNiagaraSimCache* SimCache, float DesiredAge, float DeltaSeconds, const FGraphEventRef& MyCompletionGraphEvent)
{
	...
	TOptional<FTransform> OwnerTransform;
	if (USceneComponent* TransformComponent = AttachComponent.Get())
	{
		OwnerTransform = AttachComponent->GetComponentToWorld();
		OwnerTransform->AddToTranslation(FVector(LWCTile) * -FLargeWorldRenderScalar::GetTileSize());
	}

	// Early out if our age and desired age are virtually equal
	if (FMath::IsNearlyEqual(Age, DesiredAge))
	{
		// If rebasing is enabled and the transform has moved then force an update otherwise local space emitters won't move which differs from uncached.
		// World space emitters will also move on transform change, which doesn't match uncached but is matches how it would change on next tick
		// Local space emitter do not need to update since they will automatically move.
		// Note: We could expose this as an option when using a sim cache if we desire more control
		if (!SimCache->IsRebaseEnabled() || !OwnerTransform.IsSet() || WorldTransform.Equals(*OwnerTransform))
		{
			return;
		}
	}
	...
	{
		WorldTransform = OwnerTransform.GetValue();
	}
...
const INiagaraSceneComponentUtils& FNiagaraSystemInstance::GetSceneComponentUtils(FNiagaraSystemInstance* Instance)
{
	static FNiagaraActorSceneComponentUtils GNullUtils(nullptr);
	return Instance && Instance->SceneComponentUtils.IsValid() ? *Instance->SceneComponentUtils.Get() : GNullUtils;
}
