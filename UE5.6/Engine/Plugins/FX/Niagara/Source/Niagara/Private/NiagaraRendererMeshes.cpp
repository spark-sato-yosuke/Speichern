...
DECLARE_CYCLE_STAT_WITH_FLAGS(TEXT("Generate Mesh Vertex Data [GT]"), STAT_NiagaraGenMeshVertexData, STATGROUP_Niagara, EStatFlags::Verbose);
...
static bool GbEnableNiagaraMeshRendering = true;
static FAutoConsoleVariableRef CVarEnableNiagaraMeshRendering(
	...
	TEXT("If false Niagara Mesh Renderers are disabled."),
	ECVF_Default
);
...
void FNiagaraRendererMeshes::Initialize(const UNiagaraRendererProperties* InProps, const FNiagaraEmitterInstance* Emitter, const FNiagaraSystemInstanceController& InController)
{
	...
	const INiagaraRenderableMeshArrayInterface* MeshArrayInterface = Properties->GetMeshArrayInterface(Emitter);

	MeshUsedMax = 0;

	FNiagaraSystemInstance* SystemInstance = Emitter ? Emitter->GetParentSystemInstance() : nullptr;

	uint32 SourceMeshIndex = 0;
	MeshArrayInterface->ForEachMesh(
		SystemInstance,
		[this](int32 NumMeshes)
		{
			Meshes.Empty(NumMeshes);
		},
		[this, &SourceMeshIndex, Emitter, Properties](const FNiagaraMeshRendererMeshProperties& MeshProperties)
		{
			...
			if (RenderableMesh != nullptr)
			{
				...
				for (int32 LOD=LODRange.X; LOD < LODRange.Y; ++LOD)
				{
					...
					else if (MeshProperties.LODMode == ENiagaraMeshLODMode::ByComponentBounds || MeshProperties.LODMode == ENiagaraMeshLODMode::ComponentOrigin || MeshProperties.LODMode == ENiagaraMeshLODMode::PerParticle)
					{
			...
			// Increment the source mesh index
			++SourceMeshIndex;
		}
	);
}
...
void FNiagaraRendererMeshes::PrepareParticleMeshRenderData(FRHICommandListBase& RHICmdList, FParticleMeshRenderData& ParticleMeshRenderData, const FSceneViewFamily& ViewFamily, FMeshElementCollector& Collector, FNiagaraDynamicDataBase* InDynamicData, const FNiagaraSceneProxy* SceneProxy, bool bRayTracing, ENiagaraGpuComputeTickStage::Type GpuReadyTickStage) const
{
	...
	ParticleMeshRenderData.bUseGPUScene = UseGPUScene(SceneProxy->GetScene().GetShaderPlatform(), FeatureLevel);
#else
	...
	if (SourceMode == ENiagaraRendererSourceDataMode::Particles)
	{
		...
		{
			if (!ensureMsgf(!ParticleMeshRenderData.bNeedsCull || ParticleMeshRenderData.bSortCullOnGpu, TEXT("Culling is requested on GPU but sorting is disabled, this will result in incorrect rendering. Asset(%s)."), *SceneProxy->GetResourceName().ToString()))
			{
	...
	if (bEnableFrustumCulling)
	{
		...
		else
		{
			if (bIsInstancedStereo)
			{
				...
				OutSortInfo.CullPlanes = View.GetCullingFrustum().Planes;
			}
			else
			{
...
) const
{
	...
	if (bWriteOnGpu)
	{
		...
		GPUSceneRes.DynamicPrimitiveData.DataWriterGPU = FGPUSceneWriteDelegate::CreateLambda(		
			ParticleMeshRenderData.Collector,
...
void FNiagaraRendererMeshes::GetDynamicMeshElements(const TArray<const FSceneView*>& Views, const FSceneViewFamily& ViewFamily, uint32 VisibilityMap, FMeshElementCollector& Collector, const FNiagaraSceneProxy* SceneProxy) const
{
	...
	if (ParticleMeshRenderData.SourceParticleData == nullptr || GbEnableNiagaraMeshRendering == false)
	{
		return;
	}
	...
	for (int32 ViewIndex = 0; ViewIndex < NumViews; ViewIndex++)
	{
		if (VisibilityMap & (1 << ViewIndex))
		{
			...
			for (int32 MeshIndex = 0; MeshIndex < Meshes.Num(); ++MeshIndex)
			{
				...
				else if (MeshData.LODMode == ENiagaraMeshLODMode::ComponentOrigin)
				{
					const FBox MeshLocalBounds = ScaleFBox(MeshData.RenderableMesh->GetLocalBounds(), MeshData.Scale, MeshBoundsScale);
					const FBox MeshWorldBounds = MeshLocalBounds.TransformBy(SceneProxy->GetLocalToWorld());
					const int32 LODLevel = MeshData.RenderableMesh->ComputeLOD(MeshWorldBounds.GetCenter(), float(MeshWorldBounds.GetExtent().Size()), *View, MeshData.LODDistanceFactor);
					MeshData.RenderableMesh->GetLODModelData(LODModel, LODLevel);
				}
				else
				{
...
void FNiagaraRendererMeshes::GetDynamicRayTracingInstances(FRayTracingInstanceCollector& Collector, const FNiagaraSceneProxy* SceneProxy)
{
	...
	for (int32 MeshIndex = 0; MeshIndex < Meshes.Num(); ++MeshIndex)
	{
		...
		VertexFactory.EnablePrimitiveIDElement(ParticleMeshRenderData.bUseGPUScene);
		VertexFactory.InitResource(RHICmdList);
		MeshData.RenderableMesh->SetupRayTraceVertexFactory(RHICmdList, VertexFactory, LODModel);
...
bool FNiagaraRendererMeshes::IsMaterialValid(const UMaterialInterface* Mat)const
{
	...
	if (Mat)
	{
		bIsMaterialValid = Mat->CheckMaterialUsage_Concurrent(MATUSAGE_NiagaraMeshParticles);
	}
