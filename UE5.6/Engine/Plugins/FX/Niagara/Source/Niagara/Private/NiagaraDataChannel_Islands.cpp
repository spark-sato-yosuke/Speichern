...
void UNiagaraDataChannelHandler_Islands::EndFrame(float DeltaTime, FNiagaraWorldManager* OwningWorld)
{
	...
	for (auto It = ActiveIslands.CreateIterator(); It; ++It)
	{
		...
		if (Island.IsBeingUsed() == false && Island.GetData()->HasData() == false)
		{
...
FNDCIsland* UNiagaraDataChannelHandler_Islands::FindOrCreateIsland(const FNiagaraDataChannelSearchParameters& SearchParams, ENiagaraResourceAccess AccessType)
{
	...
	if(IslandChannel->GetMode() == ENiagraDataChannel_IslandMode::AlignedStatic)
	{
		//for static islands we can just find the first one that contains our point.
		for (int32 i : ActiveIslands)
		{
			FNDCIsland& Island = IslandPool[i];
	...
	else if(IslandChannel->GetMode() == ENiagraDataChannel_IslandMode::Dynamic)
	{
		//for dynamic islands we find the first island that can grow to accommodate this point.
		for (int32 i : ActiveIslands)
		{
			FNDCIsland& Island = IslandPool[i];
...
FNDCIsland::~FNDCIsland()
{
	for (UNiagaraComponent* Comp : SpawnedComponents)
	{
		if (IsValid(Comp))
		{
			Comp->ReleaseToPool();
...
{
	return SpawnedComponents.Contains(Component);
}

bool FNDCIsland::IsBeingUsed()const
{
	bool bSystemsComplete = SpawnedComponents.ContainsByPredicate([](const TObjectPtr<UNiagaraComponent>& Comp) { return !IsValid(Comp) || Comp->IsComplete(); });

	return Data.IsValid() && (!Data.IsUnique() || !bSystemsComplete);
}
...
bool FNDCIsland::TryGrow(FVector Point, FVector PerElementExtents, FVector MaxIslandExtents)
{
	...
	FBox HandlerSystemBounds(-Bounds.BoxExtent, Bounds.BoxExtent);
	for (auto It = SpawnedComponents.CreateIterator(); It; ++It)
	{
