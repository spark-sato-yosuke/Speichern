...
void FNiagaraDebugHud::DrawOverview(class FNiagaraWorldManager* WorldManager, FCanvas* DrawCanvas)
{
	...
	if (Settings.bOverviewEnabled)
	{
		...
		else if (Settings.OverviewMode == ENiagaraDebugHUDOverviewMode::Performance && StatsListener)
		{
			...
			OverviewColumns.Emplace(TEXT("Render Thread Avg:"), FormatPerfValue(GlobalPerfStats.Avg.Time_RT), GlobalDataStringSize, FormatPerfString(TEXT("RT Avg")), ColumnOffset, Font, SystemStringSize,
				[&DetailColor, &DetailHighlightColor, &fAdvanceHeight](FCanvas* Canvas, UFont* Font, float X, float Y, FOverviewColumn& Col, const FSystemDebugInfo& SystemInfo)
				{
					FLinearColor RowBGColor = SystemInfo.UniqueColor;
					RowBGColor.A = Settings.SystemColorTableOpacity;
			...
			OverviewColumns.Emplace(TEXT("Gpu Avg:"), FormatPerfValue(GlobalPerfStats.Avg.Time_GPU), GlobalDataStringSize, FormatPerfString(TEXT("Gpu Avg")), ColumnOffset, Font, SystemStringSize,
				[&DetailColor, &DetailHighlightColor, &fAdvanceHeight](FCanvas* Canvas, UFont* Font, float X, float Y, FOverviewColumn& Col, const FSystemDebugInfo& SystemInfo)
				{
					FLinearColor RowBGColor = SystemInfo.UniqueColor;
					RowBGColor.A = Settings.SystemColorTableOpacity;
	...
	for (TWeakObjectPtr<UFXSystemComponent> WeakComponent : InWorldComponents)
	{
		...
		if (!FMath::IsNearlyZero(ScreenLocation.Z))
		{
			...
			if ( Settings.SystemDebugVerbosity != ENiagaraDebugHudVerbosity::None)
			{
				...
				if (OwnerActor && Settings.SystemDebugVerbosity == ENiagaraDebugHudVerbosity::Verbose)
				{
					StringBuilder.Appendf(TEXT(" OwnerClass(%s)"), *OwnerActor->GetClass()->GetName());
				}
				StringBuilder.Append(TEXT("\n"));
				...
				if (Settings.SystemDebugVerbosity == ENiagaraDebugHudVerbosity::Verbose)
				{
					...
					if (bIsActive)
					{
						...
						if ( NiagaraSystem->bSupportLargeWorldCoordinates )
						{
							const FVector3f LWCTile = SystemInstance->GetLWCTile();
							if (!LWCTile.Equals(FVector3f::ZeroVector) )
							{
								StringBuilder.Appendf(TEXT("LWCTile - %.1f, %.1f, %.1f"), LWCTile.X, LWCTile.Y, LWCTile.Z);
							}
						}
					}

					const int64 TotalBytes = SystemInstanceController->GetTotalBytesUsed();
