...
NIAGARA_API UObject* UNiagaraDataInterfaceRenderTarget2D::SimCacheBeginWrite(UObject* InSimCache, FNiagaraSystemInstance* NiagaraSystemInstance, const void* OptionalPerInstanceData, FNiagaraSimCacheFeedbackContext& FeedbackContext) const
{
	...
	UNDIRenderTargetSimCacheData* SimCacheData = nullptr;
	SimCacheData = NewObject<UNDIRenderTargetSimCacheData>(InSimCache);
	...
	if (InstanceData_GT->TargetTexture)
	{
		...
		(
			[RT_Proxy, RT_InstanceID = SystemInstance->GetId(), RT_TargetTexture, RT_TextureData = &TextureData, RT_VolumeResolution = InstanceData_GT->Size](FRHICommandListImmediate& RHICmdList)
			{
				...
				{
					RHICmdList.Transition(FRHITransitionInfo(InstanceData_RT->TextureRHI, ERHIAccess::SRVMask, ERHIAccess::CopySrc));
					...
					RHICmdList.Transition(FRHITransitionInfo(InstanceData_RT->TextureRHI, ERHIAccess::CopySrc, ERHIAccess::SRVMask));
				}
			}
		);
		...
		{
			UNDIRenderTargetSimCacheData* SimCacheData = CastChecked<UNDIRenderTargetSimCacheData>(StorageObject);
			SimCacheData->SetPixelData(
				FrameIndex,
				FIntVector(InstanceData_GT->Size.X, InstanceData_GT->Size.Y, 1),
				GetPixelFormatFromRenderTargetFormat(InstanceData_GT->Format),
				TextureData
			);
		}
	}
...
NIAGARA_API bool UNiagaraDataInterfaceRenderTarget2D::SimCacheReadFrame(const FNiagaraSimCacheDataInterfaceReadContext& ReadContext)
{
	UNDIRenderTargetSimCacheData* SimCacheData = ReadContext.GetStorageObject<UNDIRenderTargetSimCacheData>();

	FRenderTarget2DRWInstanceData_GameThread* InstanceData_GT = ReadContext.GetPerInstanceData<FRenderTarget2DRWInstanceData_GameThread>();

	const int FrameIndex = ReadContext.GetFrameInterp() >= 0.5f ? ReadContext.GetFrameIndexB() : ReadContext.GetFrameIndexA();
	if (!SimCacheData->IsValidFrame(FrameIndex))
	{
		return false;
	}

	{
		const FIntVector TextureSize = SimCacheData->GetTextureSize(FrameIndex);
		const EPixelFormat TextureFormat = SimCacheData->GetTextureFormat(FrameIndex);

		InstanceData_GT->Size = FIntPoint(TextureSize.X, TextureSize.Y);
		InstanceData_GT->Format = NiagaraDataInterfaceRenderTargetCommon::GetRenderTargetFormatFromPixelFormat(TextureFormat);
	...
	if (InstanceData_GT->TargetTexture && SimCacheData->HasPixelData(FrameIndex))
	{
		FNiagaraDataInterfaceProxyRenderTarget2DProxy* RT_Proxy = GetProxyAs<FNiagaraDataInterfaceProxyRenderTarget2DProxy>();
		FTextureRenderTargetResource* RT_TargetTexture = InstanceData_GT->TargetTexture->GameThread_GetRenderTargetResource();
		ENQUEUE_RENDER_COMMAND(NDIRenderTargetUpdate)
			(
				[RT_Proxy, RT_InstanceID=SystemInstance->GetId(), RT_TargetTexture, RT_SimCacheData=SimCacheData, RT_FrameIndex=FrameIndex](FRHICommandListImmediate& RHICmdList)
				{
					if (FRenderTarget2DRWInstanceData_RenderThread* InstanceData_RT = RT_Proxy->SystemInstancesToProxyData_RT.Find(RT_InstanceID))
					{
						...
						RT_SimCacheData->GetPixelData(RT_FrameIndex, OutBuffer, Stride, 0);
...
bool UNiagaraDataInterfaceRenderTarget2D::SimCacheCompareFrame(const UObject* LhsStorageObject, const UObject* RhsStorageObject, int FrameIndex, TOptional<float> Tolerance, FString& OutErrors) const
{
	const UNDIRenderTargetSimCacheData* LhsSimCacheData = CastChecked<UNDIRenderTargetSimCacheData>(LhsStorageObject);
	const UNDIRenderTargetSimCacheData* RhsSimCacheData = CastChecked<UNDIRenderTargetSimCacheData>(RhsStorageObject);
	return LhsSimCacheData->CompareFrame(RhsSimCacheData, FrameIndex, Tolerance, OutErrors);
}
...
{
	const UNDIRenderTargetSimCacheData* SimCacheData = CastChecked<UNDIRenderTargetSimCacheData>(StorageObject);
	return SimCacheData->FrameToJson(FrameIndex, TargetFolder, FilenamePrefix);
}
