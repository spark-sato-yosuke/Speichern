...
namespace NDISimCacheReaderLocal
{
	...
	static const FName GetNumInstances("GetNumInstances");
...
void UNiagaraDataInterfaceSimCacheReader::GetFunctionsInternal(TArray<FNiagaraFunctionSignature>& OutFunctions) const
{
	...
	{
		FNiagaraFunctionSignature& Sig = OutFunctions.Add_GetRef(DefaultSignature);
		Sig.Name = GetNumInstances;
		Sig.Inputs.Emplace(FNiagaraTypeDefinition::GetIntDef(), TEXT("FrameIndex"));
		Sig.Outputs.Emplace(FNiagaraTypeDefinition::GetIntDef(), TEXT("InstanceCount"));
		Sig.SetDescription(LOCTEXT("GetNumInstancesDesc", "Returns the number of instances for the provided frame."));
	}
...
void UNiagaraDataInterfaceSimCacheReader::GetVMExternalFunction(const FVMExternalFunctionBindingInfo& BindingInfo, void* PerInstanceData, FVMExternalFunction& OutFunc)
{
	...
	else if (BindingInfo.Name == GetEmitterIndex)
	{
		OutFunc = FVMExternalFunction::CreateUObject(this, &UNiagaraDataInterfaceSimCacheReader::VMGetEmitterIndex);
	}
	else if (BindingInfo.Name == GetNumInstances)
	{
		OutFunc = FVMExternalFunction::CreateUObject(this, &UNiagaraDataInterfaceSimCacheReader::VMGetNumInstances);
	}
...
void UNiagaraDataInterfaceSimCacheReader::VMGetEmitterIndex(FVectorVMExternalFunctionContext& Context)
{
	using namespace NDISimCacheReaderLocal;

	VectorVM::FUserPtrHandler<FInstanceData_GameThread> InstanceData_GT(Context);
	FNDIOutputParam<int32> OutEmitterIndex(Context);

	for (int32 i = 0; i < Context.GetNumInstances(); ++i)
	{
		OutEmitterIndex.SetAndAdvance(InstanceData_GT->EmitterIndex);
	}
}

void UNiagaraDataInterfaceSimCacheReader::VMGetNumInstances(FVectorVMExternalFunctionContext& Context)
{
	using namespace NDISimCacheReaderLocal;

	VectorVM::FUserPtrHandler<FInstanceData_GameThread> InstanceData_GT(Context);
	FNDIInputParam<int32>	InFrameIndex(Context);
	FNDIOutputParam<int32>	OutNumInstances(Context);

	UNiagaraSimCache* InstanceSimCache = InstanceData_GT->WeakSimCache.Get();
	int32 LastFrameIndex = INDEX_NONE;
	int32 LastNumInstances = 0;
	...
	for (int32 i = 0; i < Context.GetNumInstances(); ++i)
	{
		...
		if (FrameIndex != LastFrameIndex)
		{
			...
			{
				FNiagaraSimCacheAttributeReaderHelper AttributeReader;
				AttributeReader.Initialize(InstanceData_GT->WeakSimCache.Get(), InstanceData_GT->EmitterIndex, 0, FrameIndex);
				LastNumInstances = AttributeReader.IsValid() ? AttributeReader.GetNumInstances() : 0;
			}
			else
			{
				LastNumInstances = 0;
		...
		OutNumInstances.SetAndAdvance(LastNumInstances);
	}
}
