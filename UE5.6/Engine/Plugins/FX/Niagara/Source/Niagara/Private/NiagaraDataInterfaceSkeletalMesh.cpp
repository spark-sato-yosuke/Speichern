...
void FSkeletalMeshSkinningData::RegisterUser(FSkeletalMeshSkinningDataUsage Usage, bool bNeedsDataImmediately)
{
	...
	if (bNeedsDataImmediately)
	{
		...
		const int32 NumExpectedTransforms = SkelMesh ? SkelMesh->GetRefBasesInvMatrix().Num() : 0;
		if (CurrBoneRefToLocals().Num() != NumExpectedTransforms || CurrComponentTransforms().Num() != NumExpectedTransforms)
		{
...
void FSkeletalMeshGpuSpawnStaticBuffers::InitRHI(FRHICommandListBase& RHICmdList)
{
	...
	if (bUseGpuUniformlyDistributedSampling)
	{
		...
		const uint32 SizeByte = NDISkelMeshLocal::GetProbAliasDWORDSize(TriangleCount) * sizeof(uint32);

		const FRHIBufferCreateDesc CreateDesc =
			FRHIBufferCreateDesc::CreateVertex(TEXT("FSkeletalMeshGpuSpawnStaticBuffers"), SizeByte)
		...
		NDISkelMeshLocal::PackProbAlias(Initializer.GetWritableData(), triangleSampler);

		BufferTriangleUniformSamplerProbAliasRHI = Initializer.Finalize();
		BufferTriangleUniformSamplerProbAliasSRV = RHICmdList.CreateShaderResourceView(
	...
	if (NumSamplingRegionTriangles > 0)
	{
		...
		{
			const FRHIBufferCreateDesc CreateDesc =
				FRHIBufferCreateDesc::CreateVertex(TEXT("SampleRegionsProbAliasBuffer"), SampleRegionsProbAlias.Num() * SampleRegionsProbAlias.GetTypeSize())
				.AddUsage(EBufferUsageFlags::Static | EBufferUsageFlags::ShaderResource)
				.SetInitActionResourceArray(&SampleRegionsProbAlias)
				.DetermineInitialState();

			SampleRegionsProbAliasBuffer = RHICmdList.CreateBuffer(CreateDesc);
			SampleRegionsProbAliasSRV = RHICmdList.CreateShaderResourceView(
		...
		{
			const FRHIBufferCreateDesc CreateDesc =
				FRHIBufferCreateDesc::CreateVertex(TEXT("SampleRegionsTriangleIndicesBuffer"), SampleRegionsTriangleIndicies.Num() * SampleRegionsTriangleIndicies.GetTypeSize())
				.AddUsage(EBufferUsageFlags::Static | EBufferUsageFlags::ShaderResource)
				.SetInitActionResourceArray(&SampleRegionsTriangleIndicies)
				.DetermineInitialState();

			SampleRegionsTriangleIndicesBuffer = RHICmdList.CreateBuffer(CreateDesc);
			SampleRegionsTriangleIndicesSRV = RHICmdList.CreateShaderResourceView(
		...
		{
			const FRHIBufferCreateDesc CreateDesc =
				FRHIBufferCreateDesc::CreateVertex(TEXT("SampleRegionsVerticesBuffer"), SampleRegionsVertices.Num() * SampleRegionsVertices.GetTypeSize())
				.AddUsage(EBufferUsageFlags::Static | EBufferUsageFlags::ShaderResource)
				.SetInitActionResourceArray(&SampleRegionsVertices)
				.DetermineInitialState();

			SampleRegionsVerticesBuffer = RHICmdList.CreateBuffer(CreateDesc);
			SampleRegionsVerticesSRV = RHICmdList.CreateShaderResourceView(
	...
	{
		const FRHIBufferCreateDesc CreateDesc =
			FRHIBufferCreateDesc::CreateVertex(TEXT("FSkeletalMeshGpuSpawnStaticBuffers"), VertexCount * sizeof(uint32))
			.AddUsage(EBufferUsageFlags::Static | EBufferUsageFlags::ShaderResource)
			.SetInitialState(ERHIAccess::VertexOrIndexBuffer | ERHIAccess::SRVMask)
			.SetInitActionInitializer();
		...
		for (uint32 s = 0; s < SectionCount; ++s)
		{
			...
			{
				Initializer[SectionBaseVertexIndex + SectionVertex] = AccumulatedMatrixOffset;
			}
			AccumulatedMatrixOffset += Section.BoneMap.Num();
		}

		BufferTriangleMatricesOffsetRHI = Initializer.Finalize();
		BufferTriangleMatricesOffsetSRV = RHICmdList.CreateShaderResourceView(
	...
	{
		const FRHIBufferCreateDesc CreateDesc =
			FRHIBufferCreateDesc::CreateVertex(TEXT("FilteredAndUnfilteredBonesBuffer"), FilteredAndUnfilteredBonesArray.Num() * FilteredAndUnfilteredBonesArray.GetTypeSize())
			.AddUsage(EBufferUsageFlags::Static | EBufferUsageFlags::ShaderResource)
			.SetInitActionResourceArray(&FilteredAndUnfilteredBonesArray)
			.DetermineInitialState();

		FilteredAndUnfilteredBonesBuffer = RHICmdList.CreateBuffer(CreateDesc);
		FilteredAndUnfilteredBonesSRV = RHICmdList.CreateShaderResourceView(
			FilteredAndUnfilteredBonesBuffer,
			FRHIViewDesc::CreateBufferSRV()
			.SetType(FRHIViewDesc::EBufferType::Typed)
			.SetFormat(PF_R16_UINT)
		);
	}
	else
	{
...
void FSkeletalMeshGpuDynamicBufferProxy::InitRHI(FRHICommandListBase& RHICmdList)
{
	...
	{
		const FRHIBufferCreateDesc SectionBufferDesc =
			FRHIBufferCreateDesc::CreateVertex<FVector4f>(TEXT("SkeletalMeshGpuDynamicBuffer"), 3 * SectionBoneCount)
			.AddUsage(EBufferUsageFlags::ShaderResource | EBufferUsageFlags::Dynamic)
			.DetermineInitialState();

		Buffer.SectionBuffer = RHICmdList.CreateBuffer(SectionBufferDesc);
		Buffer.SectionSRV = RHICmdList.CreateShaderResourceView(
		...
		const FRHIBufferCreateDesc SamplingBufferDesc =
			FRHIBufferCreateDesc::CreateVertex<FVector4f>(TEXT("SkeletalMeshGpuDynamicBuffer"), 3 * (SamplingBoneCount + SamplingSocketCount))
			.AddUsage(EBufferUsageFlags::ShaderResource | EBufferUsageFlags::Dynamic)
			.DetermineInitialState();

		Buffer.SamplingBuffer = RHICmdList.CreateBuffer(SamplingBufferDesc);
		Buffer.SamplingSRV = RHICmdList.CreateShaderResourceView(
...
bool FNDISkeletalMesh_InstanceData::Init(UNiagaraDataInterfaceSkeletalMesh* Interface, FNiagaraSystemInstance* SystemInstance)
{
	...
	if (Mesh)
	{
		...
		if ( Mesh != nullptr )
		{
			...
			PreSkinnedLocalBoundsExtents = FVector3f(LocalBounds.BoxExtent) * 2.0f;
		}
	}
	...
	if (InstanceData && InstanceData->StaticBuffers)
	{
		...
		if (InstanceData->bReadDeformedGeometry)
		{
			...
			if (SkelMeshObject)
			{
				...
				bool bCacheValid = SkelMeshObject->GetCachedGeometry(Context.GetGraphBuilder(), CacheGeometry);
