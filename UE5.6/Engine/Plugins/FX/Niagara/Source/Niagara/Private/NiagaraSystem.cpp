...
namespace NiagaraSystemPrivate
{
	...
	bool GMarkPackageDirtyWhenNotSynchronized = false;
	static FAutoConsoleVariableRef CVarMarkPackageDirtyWhenNotSynchronized(
		TEXT("fx.Niagara.MarkPackageDirtyWhenNotSynchronized"),
		GMarkPackageDirtyWhenNotSynchronized,
		TEXT("When running a commandlet will mark the package dirty if the system or emitter scripts are not synchronized."),
		ECVF_Default
	);
#endif
	...
	void RegisterCookDependencies(UE::Cook::FCookEventContext& CookContext, bool bIncludeGpuTarget);
...
#if WITH_EDITORONLY_DATA
static int GNiagaraOnDemandCompileEnabled = 2;
static FAutoConsoleVariableRef CVarNiagaraOnDemandCompileEnabled(
	...
	TEXT("Compiles Niagara Systems on demand rather than on post load.  1 - Will defer when in editor; 2 - Will defer in editor & game"),
	ECVF_Default
);
...
#if WITH_EDITOR
void UNiagaraSystem::OnCookEvent(UE::Cook::ECookEvent CookEvent, UE::Cook::FCookEventContext& CookContext)
{
	...
	if (CookEvent == UE::Cook::ECookEvent::PlatformCookDependencies && CookContext.IsCooking())
	{
		...
		NiagaraSystemPrivate::RegisterCookDependencies(CookContext, bHasAnyGPUEmitters);
	}
}
...
ENiagaraLwcTileUpdateMode UNiagaraSystem::GetLargeWorldCoordinateTileUpdateMode() const
{
	return LargeWorldCoordinateTileUpdateMode.Get(GetDefault<UNiagaraSettings>()->LargeWorldCoordinateTileUpdateMode);
}
...
void UNiagaraSystem::UpdateSystemAfterLoad()
{
	...
	RequestCompileStatus = ERequestCompileStatus::None;
	if (GetOutermost()->HasAnyPackageFlags(EPackageFlags::PKG_Cooked) == false)
	{
		...
		{
			RequestCompileStatus = ERequestCompileStatus::RequestPendingOnDemand;

			if (NiagaraSystemPrivate::GMarkPackageDirtyWhenNotSynchronized && IsRunningCommandlet())
			{
			...
			const bool CompileOnLoad = !GNiagaraOnDemandCompileEnabled || (!GIsEditor && GNiagaraOnDemandCompileEnabled == 1);
	...
	if (bUpdateTagsAfterLoad)
	{
		if (IAssetRegistry* AssetRegistry = IAssetRegistry::Get())
		{
			AssetRegistry->AssetUpdateTags(this, EAssetRegistryTagsCaller::FullUpdate);
		}
	}
#endif
}
...
ENiagaraPerformanceStateScopeMode UNiagaraSystem::GetPerformanceStateScopeMode() const
{
	return ENiagaraPerformanceStateScopeMode::Basic;
}
...
{
	if (!bFullyLoaded && !HasAnyFlags(RF_ClassDefaultObject))
	{
		// We're currently not fully loaded and can't do so in the context of asset tags, as it could cause a deadlock with emitter merging.
		// The system will update the asset registry manually once fully loaded. The CDO is an exception to this, because otherwise the content browser can't display any of the tags.
#if WITH_EDITORONLY_DATA
		bUpdateTagsAfterLoad = true;
#endif
...
FNiagaraEmitterHandle UNiagaraSystem::DuplicateEmitterHandle(const FNiagaraEmitterHandle& EmitterHandleToDuplicate, FName EmitterName)
{
	...
	if (DuplicateEmitter)
	{
		EmitterHandles.Emplace(*DuplicateEmitter, EmitterHandleToDuplicate.GetInstance().Version);
	}
	else
	{
...
{
	for (auto It=EmitterHandles.CreateIterator(); It; ++It)
	{
		const FNiagaraEmitterHandle& EmitterHandle = *It;
		if (!HandlesToRemove.Contains(EmitterHandle.GetId()))
		{
			continue;
		}
		RemoveSystemParametersForEmitter(EmitterHandle);
		It.RemoveCurrent();
	}
...
const FNiagaraSystemStateData& UNiagaraSystem::GetSystemStateData() const
{
	static const FNiagaraSystemStateData DefaultSystemStateData;
	const FNiagaraSystemStateData* SystemStateData = SystemStateDataStruct.GetPtr<const FNiagaraSystemStateData>();
	return SystemStateData ? *SystemStateData : DefaultSystemStateData;
}

const TCHAR* UNiagaraSystem::GetSystemStateModeString() const
{
	const FNiagaraSystemStateData& SystemStateData = GetSystemStateData();
...
void UNiagaraSystem::SetCompileForEdit(bool bNewCompileForEdit)
{
	if (bNewCompileForEdit != bCompileForEdit)
	{
		...
		RequestCompileStatus = ERequestCompileStatus::RequestPending;
	}
}
...
void UNiagaraSystem::WaitForCompilationComplete_SkipPendingOnDemand(bool bIncludingGPUShaders, bool bShowProgress)
{
	if (RequestCompileStatus != ERequestCompileStatus::RequestPendingOnDemand)
	{
		WaitForCompilationComplete(bIncludingGPUShaders, bShowProgress);
	}
}
...
{
	if (NeedsRequestCompile() && bFlushRequestCompile)
	{
		RequestCompile(false);
	}
...
void UNiagaraSystem::EvaluateCompileResultDependencies() const
{
	...
	for (const FNiagaraEmitterHandle& Handle : EmitterHandles)
	{
		if (Handle.GetIsEnabled() && Handle.GetEmitterData())
		{
			...
			if (EmitterData->UsesInterpolatedSpawning())
			{
...
bool UNiagaraSystem::QueryCompileComplete(bool bWait)
{
	...
	{
		ForceGraphToRecompileOnNextCheck();
		RequestCompileStatus = ERequestCompileStatus::RequestPendingForcedRetry;
		return true;
	}
...
bool UNiagaraSystem::RequestCompile(bool bForce, FNiagaraSystemUpdateContext* OptionalUpdateContext, const ITargetPlatform* TargetPlatform)
{
	...
	if (RequestCompileStatus == ERequestCompileStatus::RequestPendingForcedRetry)
	{
		bForce = true;
	}
...
void UNiagaraSystem::ResolveRequiresScripts()
{
	...
	if (bAllowSystemStateFastPath)
	{
		...
		TOptional<FNiagaraSystemStateData> SystemStateData = EditorOnlyDataUtilities.GetSystemStateData(*this);
		if (SystemStateData.IsSet())
		{
			SystemStateDataStruct = FInstancedStruct::Make(SystemStateData.GetValue());
		}
	}
#endif //WITH_EDITORONLY_DATA
}
