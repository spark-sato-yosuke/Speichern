...
class UNiagaraMeshRendererProperties : public UNiagaraRendererProperties, public INiagaraRenderableMeshArrayInterface
{
public:
	GENERATED_BODY()
	...
	//INiagaraRenderableMeshArrayInterface BEGIN
	...
	//INiagaraRenderableMeshArrayInterface END

	const INiagaraRenderableMeshArrayInterface* GetMeshArrayInterface(const FNiagaraEmitterInstance* EmitterInstance) const;
...
#if WITH_EDITORONLY_DATA
	/**
	When disabled the mesh batches do not contribute to the hit proxy, i.e. they are not part of editor selection.
	*/
	UPROPERTY(EditAnywhere, Category = "Rendering")
	uint8 bIncludeInHitProxy : 1 = 1;
#endif
