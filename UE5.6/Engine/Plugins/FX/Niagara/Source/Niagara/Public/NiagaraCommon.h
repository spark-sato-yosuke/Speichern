...
#define NIAGARA_SYSTEM_CAPTURE (!UE_BUILD_SHIPPING || WITH_EDITORONLY_DATA)

#define NIAGARA_VERBOSE_LOGGING_ENABLED (!NO_LOGGING && !UE_BUILD_SHIPPING)
...
struct FNiagaraSystemUpdateContext
{
	...
	template <typename AddableType> // Must be handled by Add function
	FNiagaraSystemUpdateContext(AddableType&& Addable, bool bReInit,
		bool bInDestroyOnAdd = false, bool bInOnlyActive = false, bool bInDestroySystemSim = true)
		: bDestroyOnAdd(bInDestroyOnAdd), bOnlyActive(bInOnlyActive), bDestroySystemSim(bInDestroySystemSim)
	{
		Add(Forward<AddableType>(Addable), bReInit);
	}
...
private:
	...
	void AddComponentsWithMatchingSystem(bool bReinit, SystemMatchesFunc&& SystemMatches);
...
struct FNiagaraAliasContext
{
	...
	enum class ERapidIterationParameterMode
	{
		...
		None,
		/** Special case of evaluating static variables which can include variables from both system scripts and particle/emitter scripts */
		StaticVariables,
	};
	...
	NIAGARA_API FNiagaraAliasContext& ChangeRapidIterationParameterMode(ERapidIterationParameterMode InMode);
...
#else
	constexpr bool LogVerboseWarnings() { return false; }
#endif
