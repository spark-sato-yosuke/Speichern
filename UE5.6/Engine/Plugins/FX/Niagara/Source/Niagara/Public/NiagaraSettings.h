...
{
	Original = 0	UMETA(DisplayName = "Serial Compilation (Deprecated)"),
	AsyncTasks = 1	UMETA(DisplayName = "Async System Compilation"),
...
class UNiagaraSettings : public UDeveloperSettings
{
	...
	/** Controls how we respond to a system instance crossing large world coordinate tiles. Can be overriden on individual assets. */
	UPROPERTY(config, EditAnywhere, Category = Niagara, meta = (EditCondition = "bSystemsSupportLargeWorldCoordinates", EditConditionHides))
	ENiagaraLwcTileUpdateMode LargeWorldCoordinateTileUpdateMode = ENiagaraLwcTileUpdateMode::ResetSimulation;

	/** The maximum tiles we can cross before forcing a simulation to reset, if the tile update mode is set to RebaseOrResetSimulation. */
	UPROPERTY(config, EditAnywhere, Category = Niagara, meta = (EditCondition = "bSystemsSupportLargeWorldCoordinates", EditConditionHides))
	uint32 LargeWorldCoordinateMaxTilesBeforeReset = 4;
	...
	ENiagaraCompilationMode CompilationMode = ENiagaraCompilationMode::AsyncTasks;
#endif
...
#if WITH_EDITORONLY_DATA
	UPROPERTY(config, EditAnywhere, Category = Niagara)
	bool bGenerateMetaDataOnCompile = false;
#endif // WITH_EDITORONLY_DATA
