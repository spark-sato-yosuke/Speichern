...
struct FNiagaraEmitterID
{
	...
	explicit FNiagaraEmitterID(int32 InID) : ID(InID) {}

	UPROPERTY(EditAnywhere, Category = Emitter)
	int32 ID = INDEX_NONE;

	bool operator==(const FNiagaraEmitterID OtherID) const { return ID == OtherID.ID; }
	bool operator!=(const FNiagaraEmitterID OtherID) const { return ID != OtherID.ID; }
};
...
public:
	...
	*/
	...
	static const FNiagaraTypeDefinition& GetVector2DDef() { return Vector2DDef; }
	static const FNiagaraTypeDefinition& GetVectorDef() { return VectorDef; }
	static const FNiagaraTypeDefinition& GetVector4Def() { return Vector4Def; }
	static const FNiagaraTypeDefinition& GetQuatDef() { return QuatDef; }
	static const FNiagaraTypeDefinition& GetDoubleDef() { return DoubleDef; }

	static NIAGARA_API void InitLWCTypes();

	//Utility functions for accessing typedefs for specific C++ types.
	...
	//Special case for any enum types to access the correct TypeDefs.
	template <typename T, std::enable_if_t<std::is_enum_v<T>, int> = 0>
	static FNiagaraTypeDefinition GetTypeDef() ;
	template <typename T, std::enable_if_t<std::is_enum_v<T>, int> = 0>
	static FNiagaraTypeDefinition GetSimTypeDef() ;
...
private:
	...
	NIAGARA_API static FNiagaraTypeDefinition Vector2DDef;
	NIAGARA_API static FNiagaraTypeDefinition VectorDef;
	NIAGARA_API static FNiagaraTypeDefinition Vector4Def;
	NIAGARA_API static FNiagaraTypeDefinition QuatDef;
	NIAGARA_API static FNiagaraTypeDefinition DoubleDef;
...
public:
...
#endif
	{
		...
		GeneratePathNameHash();
	}

	FORCEINLINE FNiagaraTypeDefinition(UClass *ClassDef)
		: ClassStructOrEnum(ClassDef), UnderlyingType(UT_Class)
#if WITH_EDITORONLY_DATA
		, Struct_DEPRECATED(nullptr), Enum_DEPRECATED(nullptr)
#endif
	{
	...
	FORCEINLINE FNiagaraTypeDefinition(UEnum *EnumDef)
		: ClassStructOrEnum(EnumDef), UnderlyingType(UT_Enum)
#if WITH_EDITORONLY_DATA
		, Struct_DEPRECATED(nullptr), Enum_DEPRECATED(nullptr)
#endif
	{
	...
	FORCEINLINE FNiagaraTypeDefinition(UScriptStruct* StructDef, EAllowUnfriendlyStruct AllowUnfriendlyStruct)
		: ClassStructOrEnum(StructDef), UnderlyingType(UT_Struct)
#if WITH_EDITORONLY_DATA
		, Struct_DEPRECATED(nullptr), Enum_DEPRECATED(nullptr)
#endif
	{
	...
	FORCEINLINE FNiagaraTypeDefinition()
		: ClassStructOrEnum(nullptr), UnderlyingType(UT_None)
#if WITH_EDITORONLY_DATA
		, Struct_DEPRECATED(nullptr), Enum_DEPRECATED(nullptr)
#endif
	{}
	...
	*/
	...
	NIAGARA_API bool ImportTextItem(const TCHAR*& Buffer, int32 PortFlags, UObject* Parent, FOutputDevice* ErrorText);

private:
	// for allowing access to the cached RegisteredTypeDefIndex
	friend class FNiagaraTypeRegistry;

	UPROPERTY(EditAnywhere, Category = Type)
	uint8 Flags = TF_None;

	mutable int16 Size = INDEX_NONE;
	mutable int16 Alignment = INDEX_NONE;

	mutable int32 RegisteredTypeDefIndex = INDEX_NONE;

	// the type hash of the full path name for the underlying type.  This is helpful when identifying the type
	// especially in scenarios where plugins contain the structs/classes and we want to re-use slots in the registry
	// even if the objects are recreated (with different pointers/object indices)
	uint32 UnderlyingTypePathNameHash = INDEX_NONE;

	void InvalidateCachedIndex(bool bInvalidate = true);
...
private:
	...
	void NIAGARA_API GeneratePathNameHash();
...
//////////////////////////////////////////////////////////////////////////
//Utilities for getting the correct LWC Niagara Type Def.

template <typename T, std::enable_if_t<!std::is_enum_v<T>, int>>
FNiagaraTypeDefinition FNiagaraTypeHelper::GetTypeDef() { return FNiagaraTypeDefinition(StaticStruct<T>()); }

template <typename T, std::enable_if_t<!std::is_enum_v<T>, int>>
FNiagaraTypeDefinition FNiagaraTypeHelper::GetSimTypeDef() { return FNiagaraTypeDefinition(StaticStruct<T>()); }

//Special case for any enum types to access the correct TypeDefs.
template <typename T, std::enable_if_t<std::is_enum_v<T>, int>>
FNiagaraTypeDefinition FNiagaraTypeHelper::GetTypeDef()
{
	return FNiagaraTypeDefinition(StaticEnum<T>());
}

template <typename T, std::enable_if_t<std::is_enum_v<T>, int>>
FNiagaraTypeDefinition FNiagaraTypeHelper::GetSimTypeDef()
{
	return FNiagaraTypeDefinition(StaticEnum<T>());
}

template<> inline FNiagaraTypeDefinition FNiagaraTypeHelper::GetTypeDef<FNiagaraBool>() { return FNiagaraTypeDefinition::GetBoolDef(); }
template<> inline FNiagaraTypeDefinition FNiagaraTypeHelper::GetTypeDef<bool>() { return FNiagaraTypeDefinition::GetBoolDef(); }
template<> inline FNiagaraTypeDefinition FNiagaraTypeHelper::GetTypeDef<int32>() { return FNiagaraTypeDefinition::GetIntDef(); }
template<> inline FNiagaraTypeDefinition FNiagaraTypeHelper::GetTypeDef<float>() { return FNiagaraTypeHelper::GetDoubleDef(); }
template<> inline FNiagaraTypeDefinition FNiagaraTypeHelper::GetTypeDef<double>() { return FNiagaraTypeHelper::GetDoubleDef(); }
template<> inline FNiagaraTypeDefinition FNiagaraTypeHelper::GetTypeDef<FVector2D>() { return FNiagaraTypeHelper::GetVector2DDef(); }
template<> inline FNiagaraTypeDefinition FNiagaraTypeHelper::GetTypeDef<FVector>() { return FNiagaraTypeHelper::GetVectorDef(); }
template<> inline FNiagaraTypeDefinition FNiagaraTypeHelper::GetTypeDef<FVector4>() { return FNiagaraTypeHelper::GetVector4Def(); }
template<> inline FNiagaraTypeDefinition FNiagaraTypeHelper::GetTypeDef<FQuat>() { return FNiagaraTypeHelper::GetQuatDef(); }
template<> inline FNiagaraTypeDefinition FNiagaraTypeHelper::GetTypeDef<FNiagaraPosition>() { return FNiagaraTypeDefinition::GetPositionDef(); }
template<> inline FNiagaraTypeDefinition FNiagaraTypeHelper::GetTypeDef<FLinearColor>() { return FNiagaraTypeDefinition::GetColorDef(); }
template<> inline FNiagaraTypeDefinition FNiagaraTypeHelper::GetTypeDef<FNiagaraID>() { return FNiagaraTypeDefinition::GetIDDef(); }

//Utilities for getting the correct Niagara Sim TypeDef used internally to Niagara.
template<> inline FNiagaraTypeDefinition FNiagaraTypeHelper::GetSimTypeDef<FNiagaraBool>() { return FNiagaraTypeDefinition::GetBoolDef(); }
template<> inline FNiagaraTypeDefinition FNiagaraTypeHelper::GetSimTypeDef<bool>() { return FNiagaraTypeDefinition::GetBoolDef(); }
template<> inline FNiagaraTypeDefinition FNiagaraTypeHelper::GetSimTypeDef<int32>() { return FNiagaraTypeDefinition::GetIntDef(); }
template<> inline FNiagaraTypeDefinition FNiagaraTypeHelper::GetSimTypeDef<float>() { return FNiagaraTypeDefinition::GetFloatDef(); }
template<> inline FNiagaraTypeDefinition FNiagaraTypeHelper::GetSimTypeDef<double>() { return FNiagaraTypeDefinition::GetFloatDef(); }
template<> inline FNiagaraTypeDefinition FNiagaraTypeHelper::GetSimTypeDef<FVector2D>() { return FNiagaraTypeDefinition::GetVec2Def(); }
template<> inline FNiagaraTypeDefinition FNiagaraTypeHelper::GetSimTypeDef<FVector>() { return FNiagaraTypeDefinition::GetVec3Def(); }
template<> inline FNiagaraTypeDefinition FNiagaraTypeHelper::GetSimTypeDef<FVector4>() { return FNiagaraTypeDefinition::GetVec4Def(); }
template<> inline FNiagaraTypeDefinition FNiagaraTypeHelper::GetSimTypeDef<FQuat>() { return FNiagaraTypeDefinition::GetQuatDef(); }
template<> inline FNiagaraTypeDefinition FNiagaraTypeHelper::GetSimTypeDef<FNiagaraPosition>() { return FNiagaraTypeDefinition::GetVec3Def(); }
template<> inline FNiagaraTypeDefinition FNiagaraTypeHelper::GetSimTypeDef<FLinearColor>() { return FNiagaraTypeDefinition::GetColorDef(); }
template<> inline FNiagaraTypeDefinition FNiagaraTypeHelper::GetSimTypeDef<FNiagaraID>() { return FNiagaraTypeDefinition::GetIDDef(); }

//Utils for converting to Niagara Sim or LWC Types inside template code.
template<typename T, typename Enable = void> 
struct TNiagaraTypeHelper{ typedef T TSimType; typedef T TLWCType; };

template<> struct TNiagaraTypeHelper<bool> { typedef FNiagaraBool TSimType; typedef FNiagaraBool TLWCType; };
template<> struct TNiagaraTypeHelper<float> { typedef float TSimType; typedef double TLWCType; };
template<> struct TNiagaraTypeHelper<double> { typedef float TSimType; typedef double TLWCType; };
template<> struct TNiagaraTypeHelper<FVector2D> { typedef FVector2f TSimType; typedef FVector2D TLWCType; };
template<> struct TNiagaraTypeHelper<FVector> { typedef FVector3f TSimType; typedef FVector TLWCType; };
template<> struct TNiagaraTypeHelper<FVector4> { typedef FVector4f TSimType; typedef FVector4 TLWCType; };
template<> struct TNiagaraTypeHelper<FQuat> { typedef FQuat4f TSimType; typedef FQuat TLWCType; };

template<> struct TNiagaraTypeHelper<FVector2f> { typedef FVector2f TSimType; typedef FVector2D TLWCType; };
template<> struct TNiagaraTypeHelper<FVector3f> { typedef FVector3f TSimType; typedef FVector TLWCType; };
template<> struct TNiagaraTypeHelper<FVector4f> { typedef FVector4f TSimType; typedef FVector4 TLWCType; };
template<> struct TNiagaraTypeHelper<FQuat4f> { typedef FQuat4f TSimType; typedef FQuat TLWCType; };

template<> struct TNiagaraTypeHelper<FNiagaraPosition> { typedef FVector3f TSimType; typedef FVector TLWCType; };

//Always treat enums as int32s internally to Niagara for both Sim and Game.
template<typename T> 
struct TNiagaraTypeHelper<T, std::enable_if_t<std::is_enum_v<T>>> 
{
	typedef int32 TSimType; typedef int32 TLWCType; 
};