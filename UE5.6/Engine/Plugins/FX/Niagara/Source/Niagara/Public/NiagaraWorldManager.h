...
public:
	...
	void Tick(ENiagaraWorldManagerTickPass TickPass, ETickingGroup TickGroup, float DeltaSeconds, const FGraphEventRef& MyCompletionGraphEvent);
private:
	void ExecutePreTickWork(ETickingGroup TickGroup, float DeltaSeconds);
	void ExecuteSimulations(ENiagaraWorldManagerTickPass TickPass, ETickingGroup TickGroup, float DeltaSeconds, const FGraphEventRef& MyCompletionGraphEvent);
	void ExecutePostTickWork(ETickingGroup TickGroup, float DeltaSeconds);
...
public:
	...
	void DumpDetails(FOutputDevice& Ar, bool bDetailed);
...
public:
	...
	NIAGARA_API void AddDebugHUDMessage(FName Key, const FNiagaraDebugMessage& Message);
#endif
	...
	*/
	...
	bool UseTickPasses() const { return bUseTickPasses; }
...
private:
	...
	TMap<UNiagaraSystem*, FNiagaraSystemSimulationRef> SystemSimulations[NiagaraNumTickGroups][int(ENiagaraWorldManagerTickPass::Num)];
...
#if DO_CHECK
	bool bScalabilityManagersLocked = false;
#endif
