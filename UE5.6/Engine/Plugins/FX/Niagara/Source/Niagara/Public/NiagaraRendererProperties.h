...
#define WITH_NIAGARA_RENDERER_DEBUGDRAW	WITH_EDITORONLY_DATA
...
public:
	...
	*/
	...
	NIAGARA_API void CreateDefaultRendererWidget(TArray<TSharedPtr<SWidget>>& OutWidgets) const;
	NIAGARA_API void CreateRendererWidgetsForAssets(TConstArrayView<UObject*> InAssets, TSharedPtr<FAssetThumbnailPool> InThumbnailPool, TArray<TSharedPtr<SWidget>>& OutWidgets) const;
	NIAGARA_API void CreateRendererWidgetsForAssets(TConstArrayView<UMaterialInterface*> InMaterials, TSharedPtr<FAssetThumbnailPool> InThumbnailPool, TArray<TSharedPtr<SWidget>>& OutWidgets) const;

	NIAGARA_API virtual void GetRendererWidgets(const FNiagaraEmitterInstance* InEmitter, TArray<TSharedPtr<SWidget>>& OutWidgets, TSharedPtr<FAssetThumbnailPool> InThumbnailPool) const;
	...
	void GetMaterialUsageFeedback(EMaterialUsage Usage, TConstArrayView<EMaterialDomain> InvalidMaterialDomains, TArray<FNiagaraRendererFeedback>& OutFeedback) const;
	void GetMaterialUsageFeedback(EMaterialUsage Usage, TArray<FNiagaraRendererFeedback>& OutFeedback) const;
	...
	NIAGARA_API virtual bool SupportsDebugDraw() const;
	NIAGARA_API virtual TOptional<FText> GetDebugDrawTooltip() const;
	NIAGARA_API virtual bool IsDebugDrawEnabled() const;
	NIAGARA_API virtual void SetDebugDrawEnabled(bool bInEnabled);
#endif
