...
public:
	...
	/** Processes multiple ticks at once on the game thread. */
	void MultiTick_GameThread(int32 TicksToProcess, float DeltaSecondsPerTick, float MaxSimTime, const FGraphEventRef& MyCompletionGraphEvent);
...
protected:
	...
	void AddSystemToTickBatch(FNiagaraSystemSimulationTickContext& Context, FNiagaraSystemInstance* Instance, bool bIsLastInstance);
	void FlushTickBatch(FNiagaraSystemSimulationTickContext& Context, bool bIsLastInstance);
