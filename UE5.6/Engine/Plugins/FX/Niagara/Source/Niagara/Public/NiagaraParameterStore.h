...
public:
	...
	*/
	...
	FORCEINLINE const TArray<TObjectPtr<UObject>>& GetUObjects()const { return UObjects; }
	...
	FORCEINLINE const uint8* GetParameterData(int32 Offset, int32 SizeInBytes) const
	{
	...
	FORCEINLINE const uint8* GetParameterData(int32 Offset, const FNiagaraTypeDefinition& TypeDef) const
	{
	...
	FORCEINLINE uint8* GetMutableParameterData(int32 Offset, const FNiagaraTypeDefinition& TypeDef)
	{
	...
	FORCEINLINE_DEBUGGABLE const uint8* GetParameterData(const FNiagaraVariableWithOffset& Parameter) const
	{
	...
	FORCEINLINE TObjectPtr<UObject> GetUObject(int32 Offset)const
	{
	...
	FORCEINLINE TObjectPtr<UObject> GetUObject(const FNiagaraVariable& Parameter)const
	{
		int32 Offset = IndexOf(Parameter);
	...
	FORCEINLINE_DEBUGGABLE void SetUObject(TObjectPtr<UObject> InObject, int32 Offset)
	{
		UObjects[Offset] = InObject;
		OnUObjectChange();
	}

	FORCEINLINE_DEBUGGABLE void SetUObject(TObjectPtr<UObject> InObject, const FNiagaraVariable& Parameter)
	{
		int32 Offset = IndexOf(Parameter);
		if (Offset != INDEX_NONE)
		{
...
struct FNiagaraParameterDirectBinding<UObject*>
{
	...
	FORCEINLINE TObjectPtr<UObject> GetValue() const
	{
		if (UObjectOffset != INDEX_NONE)
		{
			checkSlow(BoundVariable.GetType().IsUObject());
