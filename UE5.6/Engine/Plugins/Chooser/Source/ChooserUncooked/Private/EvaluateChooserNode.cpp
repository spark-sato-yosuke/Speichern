...
void UK2Node_EvaluateChooser2::ChooserChanged()
{
	if (Chooser != CurrentCallbackChooser)
	{
		...
		if (Chooser)
		{
			...
			{
				bShowModeSetting = true;
			}
			else
			{
				// NoPrimaryResult tables only set the output to their first result anyway
				Mode = EEvaluateChooserMode::FirstResult;
			}
		}
...
void UK2Node_EvaluateChooser2::AllocateDefaultPins()
{
	...
	if (Chooser)
	{
		// We show input struct contexts pins as "references", because the supplied struct might get modified if it's an In/Out param.
		// We also show output struct contexts pins as references, to show they are there just for convenience and actually pointing to the very same
		// struct that the one on the input side.
			...
			if (ContextDataEntry.IsValid())
			{
				...
				else if (EntryType == FContextObjectTypeStruct::StaticStruct())
				{
					...
					if (StructContext.Direction == EContextObjectDirection::Read || StructContext.Direction == EContextObjectDirection::ReadWrite)
					{
						...
						{
							CreatePin(EGPD_Input, UEdGraphSchema_K2::PC_Struct, StructContext.Struct, StructContext.Struct.GetFName(), StructContextPinCreationParams);
						}
					}
		...
		if (Chooser->ResultType == EObjectChooserResultType::NoPrimaryResult)
		{
			// No need to keep existing result pin if this chooser doesn't work with primary results
			if (ResultPin)
			{
				RemovePin(ResultPin);
			}
		...
		else
		{
			...
			if (ResultPin)
			{
				ResultPin->PinType.PinCategory = bReturnSoftObjectReference ? UEdGraphSchema_K2::PC_SoftObject
												: bResultIsClass ? UEdGraphSchema_K2::PC_Class : UEdGraphSchema_K2::PC_Object;
			...
			else
			{
				UEdGraphNode::FCreatePinParams PinParams;
				PinParams.ContainerType = (Mode == EEvaluateChooserMode::AllResults) ? EPinContainerType::Array : EPinContainerType::None;
				PinParams.ValueTerminalType.TerminalCategory = bReturnSoftObjectReference ? UEdGraphSchema_K2::PC_SoftObject
												: bResultIsClass ? UEdGraphSchema_K2::PC_Class : UEdGraphSchema_K2::PC_Object;
...
void UK2Node_EvaluateChooser2::PostEditChangeProperty(struct FPropertyChangedEvent& PropertyChangedEvent)
{
	...
	if (PropertyChangedEvent.Property->GetName() == "Mode" || PropertyChangedEvent.Property->GetName() == "bReturnSoftObjectReference")
	{
		ReconstructNode();
	}
	Super::PostEditChangeProperty(PropertyChangedEvent);
...
void UK2Node_EvaluateChooser2::ExpandNode(class FKismetCompilerContext& CompilerContext, UEdGraph* SourceGraph)
{
	...
	check(ResultPin || (Chooser && Chooser->ResultType == EObjectChooserResultType::NoPrimaryResult));
	...
	if (ExecInput->HasAnyConnections())
	{
		...
		if (bReturnSoftObjectReference)
		{
			if (Mode == EEvaluateChooserMode::AllResults)
			{
				CallFunction->SetFromFunction(UChooserFunctionLibrary::StaticClass()->FindFunctionByName(GET_MEMBER_NAME_CHECKED(UChooserFunctionLibrary, EvaluateObjectChooserBaseMultiSoft)));
			}
			else
			{
				CallFunction->SetFromFunction(UChooserFunctionLibrary::StaticClass()->FindFunctionByName(GET_MEMBER_NAME_CHECKED(UChooserFunctionLibrary, EvaluateObjectChooserBaseSoft)));
			}
		}
		else
		{
