...
FString USocialParty::GetInviteMetadata(const FString& ExistingMetadata)
{
...
{
	const FString InviteMetadata = GetInviteMetadata(MetaData);	
	...
	if (CanInviteResult == ESocialPartyInviteFailureReason::Success)
	{
			...
			!IsInviteRateLimited(UserToInvite, ESocialSubsystem::Platform))
		{
			...
			OnInviteSentInternal(ESocialSubsystem::Platform, UserToInvite, bSentPlatformInvite, FailureReason, InviteMethod, InviteMetadata);
		...
		if ((!bSentInvite || bMustSendPrimaryInvite) && PerformingPlayer && UserPrimaryId.IsValid() && !IsInviteRateLimited(UserToInvite, ESocialSubsystem::Primary))
		{
			...
			const FPartyInvitationRecipient Recipient(*UserPrimaryId, InviteMetadata);
			const IOnlinePartyPtr PartyInterface = Online::GetPartyInterfaceChecked(GetWorld());
			...
			OnInviteSentInternal(ESocialSubsystem::Primary, UserToInvite, bSentPrimaryInvite, FailureReason, InviteMethod, InviteMetadata);
			bSentInvite |= bSentPrimaryInvite;
		}
	}
	else
	{
		OnInviteSentInternal(ESocialSubsystem::MAX, UserToInvite, false, CanInviteResult, InviteMethod, InviteMetadata);
	}
	return bSentInvite;
}
...
bool USocialParty::AllowJoinInProgressToMember()
{
	return false;
}
...
void USocialParty::OnInviteSentInternal(ESocialSubsystem SubsystemType, const USocialUser& InvitedUser, bool bWasSuccessful, const ESocialPartyInviteFailureReason FailureReason, const ESocialPartyInviteMethod InviteMethod, const FString& MetaData)
{
	...
	// Call the deprecated methods after the current OnInviteSentInternal
PRAGMA_DISABLE_DEPRECATION_WARNINGS
	OnInviteSentInternal(SubsystemType, InvitedUser, bWasSuccessful, FailureReason, InviteMethod);
...
{
	QUICK_SCOPE_CYCLE_COUNTER(STAT_USocialParty_HandlePartyMemberExited);
	if (PartyId == GetPartyId())
	{
		if (TObjectPtr<UPartyMember>* FoundPartyMember = PartyMembersById.Find(MemberId.AsShared()))
		{
			...
			else
			{
				...
				{
					QUICK_SCOPE_CYCLE_COUNTER(STAT_USocialParty_HandlePartyMemberExited_Delegates);
...
{
	QUICK_SCOPE_CYCLE_COUNTER(STAT_SocialParty_ConnectToReservationBeacon);

	const bool bJIPRequiresLeaderApproval = !AllowJoinInProgressToMember();

	APartyBeaconClient* LocalReservationBeaconClient = ReservationBeaconClient.Get();
	if ((IsLocalPlayerPartyLeader() || !bJIPRequiresLeaderApproval) && !LocalReservationBeaconClient)
	{
					...
					if (ensure(SessionInterface->GetResolvedConnectString(PartyGameSessionName, URL, NAME_BeaconPort)))
					{
						...
						if (LocalReservationBeaconClient)
						{
							...
							if (InitializeBeaconEncryptionData(*ReservationBeaconClient, Session->GetSessionIdStr()))
							{
								...
								const FUniqueNetIdRepl ReservationLeaderId = AllowJoinInProgressToMember() ? OwningLocalUserId : GetPartyLeader()->GetPrimaryNetId();
								bStartedConnection = ReservationBeaconClient->RequestReservationUpdate(URL, Session->GetSessionIdStr(), ReservationLeaderId, ReservationAsArray, NextApproval.bIsPlayerRemoval);
							}
						}
					}
				}
...
void USocialParty::PumpApprovalQueue()
{
	...
	if (PendingApprovals.Peek(NextApproval))
	{
		...
		if (ensure(LocalReservationBeaconClient))
		{
			...
			const FUniqueNetIdRepl ReservationLeaderId = AllowJoinInProgressToMember() ? OwningLocalUserId : GetPartyLeader()->GetPrimaryNetId();
			LocalReservationBeaconClient->RequestReservationUpdate(ReservationLeaderId, PlayersToAdd);
		}
		else
		{
...
{
	// When a join-in-progress attempt is already in progress, cancel the previous attempt before creating a new one.
	if (IsJoinInProgeressRequestActive())
	{
		CancelJoinInProgressRequest();
	}
...
void USocialParty::CancelJoinInProgressRequest()
{
	if (IsJoinInProgeressRequestActive())
	{
		UE_LOG(LogParty, Log, TEXT("RequestJoinInProgress: Canceling join-in-progress request."));

		FPartyMemberJoinInProgressRequest EmptyRequest;
		for (UPartyMember* RequestingMember : GetLocalPartyMembersForJoinInProgress())
		{
			if (RequestingMember && ensure(RequestingMember->IsLocalPlayer()))
			{
				RequestingMember->GetMutableRepData().SetJoinInProgressDataRequest(EmptyRequest);
			}
		}
...
bool USocialParty::IsJoinInProgeressRequestActive() const
{
	return RequestJoinInProgressComplete.IsSet();
}
...
{
	if (Request.Time == 0 || !(AllowJoinInProgressToMember() || IsLocalPlayerPartyLeader()))
	{
