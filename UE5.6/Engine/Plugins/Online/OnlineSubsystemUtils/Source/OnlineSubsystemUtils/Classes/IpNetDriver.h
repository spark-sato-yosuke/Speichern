...
public:
	...
	/** Base constructor */
	UE_API UIpNetDriver(const FObjectInitializer& ObjectInitializer);
...
private:
	 ...
	 */
	UE_API UNetConnection* ProcessConnectionlessPacket(FReceivedPacketView& PacketRef, const FPacketBufferView& WorkingBuffer);

	/** Clears all references to sockets held by this net driver. */
	UE_API void ClearSockets();
...
public:
	 ...
	 */
	UE_API virtual FSocket* GetSocket();
	...
	*/
	UE_API void SetSocketAndLocalAddress(const TSharedPtr<FSocket>& SharedSocket);
	 ...
	 */
	UE_API virtual int GetClientPort();
	 ...
	 */
	UE_API UE::Net::ERecreateSocketResult RecreateSocket(int32 OverridePort = INDEX_NONE);
...
protected:
	 ...
	 */
	UE_API virtual FUniqueSocket CreateSocketForProtocol(const FName& ProtocolType);
	 ...
	 */
	UE_API virtual FUniqueSocket CreateAndBindSocket(TSharedRef<FInternetAddr> BindAddr, int32 Port, bool bReuseAddressAndPort, int32 DesiredRecvSize, int32 DesiredSendSize, FString& Error);
...
public:
	...
	UE_API bool HandlePauseReceiveCommand(const TCHAR* Cmd, FOutputDevice& Ar, UWorld* InWorld);
	UE_API bool HandleRecreateSocketCommand(const TCHAR* Cmd, FOutputDevice& Ar, UWorld* InWorld);
	...
	*/
	UE_API void TestSuddenPortChange(uint32 NumConnections);
#endif


	/** @return IP connection to server */
	UE_API class UIpConnection* GetServerConnection();
...
private:
	 ...
	 */
	UE_API void TrackAndLogNewIP(const FInternetAddr& InAddr);
	 ...
	 */
	UE_API void RemoveFromNewIPTracking(const FInternetAddr& InAddr);

	/**
	 * Resets new pre-connection IP tracking and timers.
	 */
	UE_API void ResetNewIPTracking();
	 ...
	 */
	UE_API void TickNewIPTracking(float DeltaTime);
	 ...
	 */
	UE_API void OnRecreateSocketComplete();
	 ...
	 */
	UE_API void SetSocket_Internal(const TSharedPtr<FSocket>& InSocket, UE::Net::Private::FSetSocketComplete InSetCallback=nullptr);
...
public:
	...
	static UE_API FOnNetworkProcessingCausingSlowFrame OnNetworkProcessingCausingSlowFrame;
