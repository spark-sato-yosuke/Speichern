...
void AOnlineBeacon::CleanupNetDriver()
{
	if (NetDriver)
	{
		...
		{
			UE_LOG(LogBeacon, Log, TEXT("OnlineBeacon %s requesting net driver destruction: %s"), *GetName(), *NetDriver->GetDescription());
			NetDriver->RequestNetDriverDestruction();
		}
		else
		{
			UE_LOG(LogBeacon, Log, TEXT("OnlineBeacon %s immediately destroying netdriver: %s"),*GetName(), *NetDriver->GetDescription());
