...
/** Created when a new local user starts a login process, keeps the state for the login session and ongoing operations, and gets destroyed when login fails or the user logs out */
...
// ~IOnlineUser Interface
	...
	using FResolveUniqueNetIdsCallback = TFunction<void(TMap<EOS_ProductUserId, FUniqueNetIdEOSRef> ResolvedUniqueNetIds, const FOnlineError& Error)>;
	using FResolveEpicAccountIdsCallback = TFunction<void(bool bWasSuccessful, TMap<EOS_EpicAccountId, FUniqueNetIdEOSRef> ResolvedUniqueNetIds, const FOnlineError& Error)>;
	using FResolveUniqueNetIdCallback = TFunction<void(FUniqueNetIdEOSRef ResolvedUniqueNetId, const FOnlineError& Error)>;

	bool GetEpicAccountIdFromProductUserId(int32 LocalUserNum, const EOS_ProductUserId& ProductUserId, EOS_EpicAccountId& OutEpicAccountId, FOnlineError& Error) const;
...
private:
	...
	FLocalUserEOS& AddLocalUser(int32 LocalUserNum);
	FLocalUserEOS& UpdateLocalUser(int32 LocalUserNum, EOS_EpicAccountId EpicAccountId, EOS_ProductUserId UserId);
