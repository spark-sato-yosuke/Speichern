...
bool FOnlineSubsystemEOS::PlatformCreate()
{
	...
	PlatformOptions.RTCOptions = EOSSettings.bUseEOSRTC ? &RtcOptions : nullptr;
#endif
...
bool FOnlineSubsystemEOS::Init()
{
	...
	PlayerSanctionHandle = EOS_Platform_GetSanctionsInterface(*EOSPlatformHandle);
	if (PlayerSanctionHandle == nullptr)
	{
		UE_LOG_ONLINE(Error, TEXT("FOnlineSubsystemEOS: failed to init EOS platform, couldn't get player sanction handle"));
		return false;
	}
	PlayerReportHandle = EOS_Platform_GetReportsInterface(*EOSPlatformHandle);
	if (PlayerReportHandle == nullptr)
	{
		UE_LOG_ONLINE(Error, TEXT("FOnlineSubsystemEOS: failed to init EOS platform, couldn't get player report handle"));
		return false;
	}
	...
	UserManager = MakeShared<FUserManagerEOS>(this);
	UserManager->Init();
	SessionInterfacePtr = MakeShared<FOnlineSessionEOS>(this);
...
IOnlinePlayerSanctionEOSPtr FOnlineSubsystemEOS::GetPlayerSanctionEOSInterface() const
{
	return PlayerSanctionEOSPtr;
}

IOnlinePlayerReportEOSPtr FOnlineSubsystemEOS::GetPlayerReportEOSInterface() const
{
	return PlayerReportInterfacePtr;
}
...
void FOnlineSubsystemEOS::QueryUniqueNetId(int32 LocalUserNum, const EOS_ProductUserId& ProductUserId, const FOnQueryUniqueNetIdComplete & Callback)
{
	if (UserManager)
	{
		FUserManagerEOS::FResolveUniqueNetIdCallback ResolveUniqueNetIdCallback = [Callback](FUniqueNetIdEOSRef ResolvedUniqueNetId, const FOnlineError& Error)
		{
			Callback.ExecuteIfBound(ResolvedUniqueNetId, Error);
		};

		UserManager->ResolveUniqueNetId(LocalUserNum, ProductUserId, ResolveUniqueNetIdCallback);
	}
	else
	{
		UE_LOG_ONLINE(Warning, TEXT("Unable to query UniqueNetId for ProductUserId. UserManager not valid."));

		ExecuteNextTick([Callback]()
		{
			Callback.ExecuteIfBound(FUniqueNetIdEOS::EmptyId(), FOnlineError(EOnlineErrorResult::MissingInterface));
		});
	}
}
