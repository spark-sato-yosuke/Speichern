...
FOnlineSessionIdRegistryEOSGS::FOnlineSessionIdRegistryEOSGS(EOnlineServices ServicesType)
	: FOnlineSessionIdRegistryLAN(ServicesType)
{
}

FOnlineSessionIdRegistryEOSGS& FOnlineSessionIdRegistryEOSGS::GetRegistered(EOnlineServices Services)
{
	IOnlineSessionIdRegistry* Registry = FOnlineIdRegistryRegistry::Get().GetSessionIdRegistry(Services);
	check(Registry);
	return *static_cast<FOnlineSessionIdRegistryEOSGS*>(Registry);
}

/** FOnlineSessionInviteIdRegistryEOSGS */

FOnlineSessionInviteIdRegistryEOSGS::FOnlineSessionInviteIdRegistryEOSGS(EOnlineServices ServicesType)
	: FOnlineSessionInviteIdStringRegistry(ServicesType)
{
}

FOnlineSessionInviteIdRegistryEOSGS& FOnlineSessionInviteIdRegistryEOSGS::GetRegistered(EOnlineServices Services)
{
	IOnlineSessionInviteIdRegistry* Registry = FOnlineIdRegistryRegistry::Get().GetSessionInviteIdRegistry(Services);
	check(Registry);
	return *static_cast<FOnlineSessionInviteIdRegistryEOSGS*>(Registry);
}

/** FSessionEOSGS */

FSessionEOSGS::FSessionEOSGS(const FSessionsEOSGS& SessionsInstance, const TSharedPtr<FSessionDetailsHandleEOSGS>& InSessionDetailsHandle)
	: SessionDetailsHandle(InSessionDetailsHandle)
{
	...
	{
		SessionInfo.SessionId = SessionsInstance.CreateSessionId(UTF8_TO_TCHAR(SessionDetailsInfo->SessionId));
...
FSessionsEOSGS::FSessionsEOSGS(FOnlineServicesEpicCommon& InServices)
	: Super(InServices)
{
}
...
void FSessionsEOSGS::Initialize()
{
	...
	SessionsHandle = EOS_Platform_GetSessionsInterface(*GetServices<FOnlineServicesEpicCommon>().GetEOSPlatformHandle());
...
void FSessionsEOSGS::SetSessionSearchSessionId(FSessionSearchHandleEOSGS& SessionSearchHandle, const FOnlineSessionId& SessionId)
{
	...
	const auto SessionIdUtf8 = StringCast<UTF8CHAR>(*ToString(SessionId));
	Options.SessionId = (const char*)SessionIdUtf8.Get();
...
FOnlineSessionId FSessionsEOSGS::CreateSessionId(const FString& SessionId) const
{
	return FOnlineSessionIdRegistryEOSGS::GetRegistered(Services.GetServicesProvider()).BasicRegistry.FindOrAddHandle(SessionId);
}

FSessionInviteId FSessionsEOSGS::CreateSessionInviteId(const FString& SessionInviteId) const
{
	return FOnlineSessionInviteIdRegistryEOSGS::GetRegistered(Services.GetServicesProvider()).BasicRegistry.FindOrAddHandle(SessionInviteId);
}
...
TFuture<TOnlineResult<FJoinSession>> FSessionsEOSGS::JoinSessionImpl(const FJoinSession::Params& Params)
{
	...
	if (GetSessionByIdResult.IsError())
	{
		...
		if (FOnlineSessionIdRegistryEOSGS::GetRegistered(Services.GetServicesProvider()).IsSessionIdExpired(Params.SessionId))
		{
	...
	if (GetIdsResult.IsOk())
	{
			...
			.Next([this, WeakOp = Op->AsWeak(), Params = MoveTemp(Params)](TArray<FAccountId> ResolvedAccountIds) mutable
		{
			if (TOnlineAsyncOpPtr<FBuildSessionFromDetailsHandle> StrongOp = WeakOp.Pin())
			{
				...
				StrongOp->SetResult(FBuildSessionFromDetailsHandle::Result{ StrongOp->GetParams().LocalAccountId, MakeShared<FSessionEOSGS>(*this, Params.SessionDetailsHandleEOSGS) });
			}
		});
	}
	else
...
TFuture<TOnlineResult<FRejectSessionInvite>> FSessionsEOSGS::RejectSessionInviteImpl(const FRejectSessionInvite::Params& Params)
{
	...
	const FString& InviteIdStr = FOnlineSessionInviteIdRegistryEOSGS::GetRegistered(Services.GetServicesProvider()).BasicRegistry.FindIdValue(Params.SessionInviteId);
...
FAccountId FSessionsEOSGS::FindAccountId(const EOS_ProductUserId ProductUserId)
{
