...
namespace UE::Online {
	...
	FAchievementsEOSGS::FAchievementsEOSGS(FOnlineServicesEpicCommon& InServices)
	: Super(InServices)
{
}
...
void FAchievementsEOSGS::Initialize()
{
	...
	AchievementsHandle = EOS_Platform_GetAchievementsInterface(*GetServices<FOnlineServicesEpicCommon>().GetEOSPlatformHandle());
	check(AchievementsHandle);

	OnAchievementsUnlocked = EOS_RegisterComponentEventHandler(
		this,
		AchievementsHandle,
		2,
		&EOS_Achievements_AddNotifyAchievementsUnlockedV2,
		&EOS_Achievements_RemoveNotifyAchievementsUnlocked,
		&FAchievementsEOSGS::HandleAchievementsUnlocked);
...
void FAchievementsEOSGS::Shutdown()
{
	...
	OnAchievementsUnlocked = nullptr;
}
...
void FAchievementsEOSGS::HandleAchievementsUnlocked(const EOS_Achievements_OnAchievementsUnlockedCallbackV2Info* Data)
{
	...
	if (LocalAccountId.IsValid())
	{
		...
		UE_LOG(LogOnlineServices, Verbose, TEXT("%hs User=%s AchievId=%s"), __FUNCTION__, *ToLogString(LocalAccountId), *AchievementId);

		if (FAchievementStateMap* LocalUserAchievementStates = AchievementStates.Find(LocalAccountId))
		{
		...
		OnAchievementStateUpdatedEvent.Broadcast(EventParams);
	}
}

FAccountId FAchievementsEOSGS::FindAccountId(const EOS_ProductUserId ProductUserId)
{
