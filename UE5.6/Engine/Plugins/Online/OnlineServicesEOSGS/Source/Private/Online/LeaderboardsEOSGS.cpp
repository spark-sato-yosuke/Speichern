...
FLeaderboardsEOSGS::FLeaderboardsEOSGS(FOnlineServicesEpicCommon& InServices)
	: Super(InServices)
{
}
...
void FLeaderboardsEOSGS::Initialize()
{
	...
	LeaderboardsHandle = EOS_Platform_GetLeaderboardsInterface(*GetServices<FOnlineServicesEpicCommon>().GetEOSPlatformHandle());
...
void FLeaderboardsEOSGS::ReadEntriesInRange(uint32 StartIndex, uint32 EndIndex, TArray<FLeaderboardEntry>& OutEntries)
{
	for (uint32 i = StartIndex; i <= EndIndex; ++i)
	{
		...
		if (CopyResult == EOS_EResult::EOS_Success)
		{
			...
			LeaderboardEntry.AccountId = FindAccountIdChecked(Services.GetServicesProvider(), LeaderboardRecord->UserId);
...
TOnlineAsyncOpHandle<FReadEntriesAroundRank> FLeaderboardsEOSGS::ReadEntriesAroundRank(FReadEntriesAroundRank::Params&& Params)
{
	...
	if (!Op->IsReady())
	{
		...
		.Then([this](TOnlineAsyncOp<FReadEntriesAroundRank>& InAsyncOp, const EOS_Leaderboards_OnQueryLeaderboardRanksCompleteCallbackInfo* Data)
		{
			...
			ReadEntriesInRange(StartIndex, EndIndex, Result.Entries);
			InAsyncOp.SetResult(MoveTemp(Result));
		})
		.Enqueue(GetSerialQueue());
	}
