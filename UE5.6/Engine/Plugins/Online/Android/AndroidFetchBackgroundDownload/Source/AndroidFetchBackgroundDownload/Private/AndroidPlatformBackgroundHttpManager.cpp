...
const FString FAndroidNativeDownloadWorkerParameterKeys::NOTIFICATION_CONTENT_AIRPLANE_MODE_TEXT_KEY = TEXT("NotificationContentAirmodeModeText");
const FString FAndroidNativeDownloadWorkerParameterKeys::NOTIFICATION_CONTENT_DATA_SAVER_ENABLED_TEXT_KEY = TEXT("NotificationContentDataSaverEnabledText");
...
{	
	...
	//Update cellular preference in case if download was allowed to go over cellular from notification.
	CellularPreference = FPlatformMisc::GetCellularPreference();
...
void FAndroidPlatformBackgroundHttpManager::SetCellularPreference(int32 Value)
{
	CellularPreference = Value;
...
void FAndroidPlatformBackgroundHttpManager::ActivatePendingRequests()
{
	...
	if (bHasPendingRequests || bForceActiveRequestsToRequeue)
	{
		...
		// UEDownloadWorker.java actively reads and _removes_ this setting, so setting it every time we schedule a request.
		FPlatformMisc::SetCellularPreference(CellularPreference);
		...
		{				
			...
			if (ensureAlwaysMsgf((bDidWriteSucceed), TEXT("Failed to create download description manifest for the WorkManager UEDownloadWorker. Can not schedule download work without the file as nothing will be downloaded!")))
			{
				...
				WorkParams.AddDataToWorkerParameters(FAndroidNativeDownloadWorkerParameterKeys::NOTIFICATION_CONTENT_AIRPLANE_MODE_TEXT_KEY, AndroidBackgroundHTTPManagerDefaultLocalizedText.DefaultNotificationText_AirplaneMode.GetText());
				WorkParams.AddDataToWorkerParameters(FAndroidNativeDownloadWorkerParameterKeys::NOTIFICATION_CONTENT_DATA_SAVER_ENABLED_TEXT_KEY, AndroidBackgroundHTTPManagerDefaultLocalizedText.DefaultNotificationText_DataSaverEnabled.GetText());
...
const FString FAndroidPlatformBackgroundHttpManager::GetFullFileNameForDownloadDescriptionList() const
{
	...
	FString FileName = FString::Printf(TEXT("%s%s%s"), *BASE_FILE_NAME, *FGuid::NewGuid().ToString(), *FILE_EXTENSION);
...
{
	const FString* SettingStringValue = FPlatformMisc::GetConfigRulesVariable(AndroidBackgroundDownloadConfigRulesSettingKey);
...
FAndroidPlatformBackgroundHttpManager::FAndroidBackgroundHTTPManagerDefaultLocalizedText::FAndroidBackgroundHTTPManagerDefaultLocalizedText::FAndroidBackgroundHTTPManagerDefaultLocalizedText()
	...
	, DefaultNotificationText_AirplaneMode()
	, DefaultNotificationText_DataSaverEnabled()
...
void FAndroidPlatformBackgroundHttpManager::FAndroidBackgroundHTTPManagerDefaultLocalizedText::InitFromIniSettings(const FString& ConfigFileName)
{
	if (ensureAlwaysMsgf(GConfig, TEXT("Invalid GConfig, can not load default localization strings for this manager!")))
	{
		...
		if (ensureAlwaysMsgf(Config, TEXT("Unable to find .ini file for %s. Can not load localized text for this manager!"), *ConfigFileName))
		{
			...
			//DefaultNotificationText_AirplaneMode
			{
				TArray<FString> AirplaneModeTextStrings;
				Config->GetArray(*TextConfigSection, TEXT("DefaultNotificationText_AirplaneMode"), AirplaneModeTextStrings);
				ParsePolyglotTextItem(DefaultNotificationText_AirplaneMode, TEXT("Notification.AirplaneModeText"), AirplaneModeTextStrings);
			}

			//DefaultNotificationText_DataSaverEnabled
			{
				TArray<FString> DataSaverEnabledTextStrings;
				Config->GetArray(*TextConfigSection, TEXT("DefaultNotificationText_DataSaverEnabled"), DataSaverEnabledTextStrings);
				ParsePolyglotTextItem(DefaultNotificationText_DataSaverEnabled, TEXT("Notification.DataSaverEnabledText"), DataSaverEnabledTextStrings);
			}
