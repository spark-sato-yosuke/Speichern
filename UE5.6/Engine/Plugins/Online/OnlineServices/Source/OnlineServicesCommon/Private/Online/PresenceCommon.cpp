...
{
public:
	FBatchQueryPresenceHelper(TOnlineAsyncOpPtr<FBatchQueryPresence> InOp, TSharedPtr<IPresence> InPresence)
		: Op(InOp)
		, Presence(InPresence)
		, ErrorState(Errors::Success())
	{
		NumPresencesRemaining = InOp->GetParams().TargetAccountIds.Num();
	}

	TFuture<FBatchQueryPresenceHelperResult> GetPromise()
	{
		TOnlineAsyncOpPtr<FBatchQueryPresence> OpPtr = Op.Pin();
		if (OpPtr.IsValid())
		{
			for(FAccountId TargetAccountId : OpPtr->GetParams().TargetAccountIds)
			{
				FQueryPresence::Params Params;
				Params.LocalAccountId = OpPtr->GetParams().LocalAccountId;
				Params.TargetAccountId = TargetAccountId;
				Params.bListenToChanges = OpPtr->GetParams().bListenToChanges;
	...
	FBatchQueryPresence::Result Result;
	// Need to use weak ptr here otherwise there will be circular reference with Op->Steps->Lambda->Helper->Op
	TWeakPtr<TOnlineAsyncOp<FBatchQueryPresence>> Op;
...
TOnlineAsyncOpHandle<FBatchQueryPresence> FPresenceCommon::BatchQueryPresence(FBatchQueryPresence::Params&& Params)
{
	...
	TSharedRef<FBatchQueryPresenceHelper> Helper = MakeShared<FBatchQueryPresenceHelper>(Op.ToSharedPtr(), this->AsShared());
