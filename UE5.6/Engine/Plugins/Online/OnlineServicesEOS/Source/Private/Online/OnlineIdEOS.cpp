...
FAccountId FOnlineAccountIdRegistryEOS::FindOrAddAccountId(const EOS_ProductUserId ProductUserId)
{
	checkNoEntry();

	UE_LOG(LogTemp, Warning, TEXT("This method is not implemented in FOnlineAccountIdRegistryEOS, use FindOrAddAccountId(const EOS_EpicAccountId InEpicAccountId, const EOS_ProductUserId InProductUserId) instead."));
...
FAccountId FOnlineAccountIdRegistryEOS::FindAccountId(const EOS_ProductUserId ProductUserId) const
{
	...
	{
		const UE::TReadScopeLock ReadLock(Lock);
...
FAccountId FOnlineAccountIdRegistryEOS::FindOrAddAccountId(const EOS_EpicAccountId InEpicAccountId, const EOS_ProductUserId InProductUserId)
{
	...
	if (ensure(bInEpicAccountIdValid || bInProductUserIdValid))
	{
		...
		if(!Result.IsValid())
		{
			...
			const UE::TWriteScopeLock WriteLock(Lock);
			Result = FindExisting();
...
{
	EOnlineServices ServicesType = AccountId.GetOnlineServicesType();

	if (ServicesType == EOnlineServices::EpicAccount)
	{
		IOnlineAccountIdRegistry* Registry = FOnlineIdRegistryRegistry::Get().GetAccountIdRegistry(ServicesType);
		check(Registry);

		IOnlineAccountIdRegistryEpicAccount* EpicAccountRegistry = static_cast<IOnlineAccountIdRegistryEpicAccount*>(Registry);
		check(EpicAccountRegistry);

		return EpicAccountRegistry->GetEpicAccountId(AccountId);
	}
	else if (ServicesType == EOnlineServices::Epic)
	{
...
{
	return FindAccountId(EOnlineServices::Epic, EpicAccountId);
}

FAccountId FindAccountId(EOnlineServices Services, const EOS_EpicAccountId EpicAccountId)
{
	if (Services == EOnlineServices::EpicAccount)
	{
		...
		return EpicAccountRegistry->GetAccountId(EpicAccountId);
	}
	else if (Services == EOnlineServices::Epic)
	{
...
{
	return FindAccountIdChecked(EOnlineServices::Epic, EpicAccountId);
}

FAccountId FindAccountIdChecked(EOnlineServices Services, const EOS_EpicAccountId EpicAccountId)
{
	FAccountId Result = FindAccountId(Services, EpicAccountId);
	check(Result.IsValid());
	return Result;
}
