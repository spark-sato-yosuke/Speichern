...
FSocialEOS::FSocialEOS(FOnlineServicesEpicCommon& InServices)
...
void FSocialEOS::Initialize()
{
	...
	FriendsHandle = EOS_Platform_GetFriendsInterface(*GetServices<FOnlineServicesEpicCommon>().GetEOSPlatformHandle());
	...
	OnFriendsUpdate = EOS_RegisterComponentEventHandler(
		this,
		FriendsHandle,
		1,
		&EOS_Friends_AddNotifyFriendsUpdate,
		&EOS_Friends_RemoveNotifyFriendsUpdate,
		&FSocialEOS::HandleFriendsUpdate);
...
void FSocialEOS::HandleFriendsUpdate(const EOS_Friends_OnFriendsUpdateInfo* Data)
{		
	...
	{
		UE_LOG(LogOnlineServices, Log, TEXT("%hs LocalUserId=%s not found."), __FUNCTION__, *LexToString(Data->LocalUserId));
		return;
	}

	Services.Get<FAuthEOS>()->ResolveAccountId(LocalAccountId, Data->TargetUserId)
...
FAccountId FSocialEOS::FindAccountId(const EOS_EpicAccountId EpicAccountId)
{
