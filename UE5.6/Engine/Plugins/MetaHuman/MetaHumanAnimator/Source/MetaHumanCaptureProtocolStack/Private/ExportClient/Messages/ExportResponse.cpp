...
#define CPS_CHECK_RESULT_RET_ERROR(Result) if (Result.IsError()) { return Result.ClaimError(); }
...
TProtocolResult<FExportResponse> FExportResponse::Deserialize(ITcpSocketReader& InReader)
{
	...
	Response.Status = ToStatus(StatusResult.ClaimResult()[0]);
	...
	if (Response.Status == EStatus::Success)
	{
		...
		TArray<uint8> LengthData = LengthResult.ClaimResult();
...
TProtocolResult<TStaticArray<uint8, 16>> FExportResponse::DeserializeHash(ITcpSocketReader& InReader)
{
	...
	TArray<uint8> HashData = HashResult.ClaimResult();
