...
{
	class SAttributesTreeView : public SCompoundWidget
	{
	public:
		SLATE_BEGIN_ARGS(SAttributesTreeView) { }
		SLATE_END_ARGS()
	...
	class FNamedElementDragDropOp : public FDecoratedDragDropOp
	{
	public:
		DRAG_DROP_OPERATOR_TYPE(FNamedElementDragDropOp, FDecoratedDragDropOp)

		static TSharedRef<FNamedElementDragDropOp> New(const FName InNamedAttribute);

		TSharedPtr<SWidget> GetDefaultDecorator() const override;

		FName NamedAttribute;
	};
}