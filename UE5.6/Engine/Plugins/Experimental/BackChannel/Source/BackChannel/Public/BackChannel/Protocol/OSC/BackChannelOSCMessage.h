 ...
 *	being returned.
 */
class FBackChannelOSCMessage : public FBackChannelOSCPacket, public IBackChannelPacket
{
public:

	UE_API FBackChannelOSCMessage(OSCPacketMode InMode);

	UE_API FBackChannelOSCMessage(const TCHAR* Address);

	UE_API virtual ~FBackChannelOSCMessage();

	/* Move constructor */
	UE_API FBackChannelOSCMessage(FBackChannelOSCMessage&& RHS);

	/* Move operator */
	UE_API FBackChannelOSCMessage& operator=(FBackChannelOSCMessage&& RHS);

	/* Return our type */
	virtual OSCPacketType GetType() const override { return OSCPacketType::Message; }

	/* Return our size (plus any necessary padding) */
	UE_API virtual int32 GetSize() const override;

	/* Returns a buffer with the contents of this message. Data is padded per OSC requirements */
	UE_API virtual TArray<uint8> WriteToBuffer() const override;
	
	/* Writes this message into the provided buffer at an offset of Buffer.Num() */
	UE_API virtual void WriteToBuffer(TArray<uint8>& Buffer) const override;
	...
	*/
	...
	UE_API virtual int Read(const TCHAR* InName, FString& OutValue) override;

	//! Raw data blobs

	/* Read a blob of data from our arguments */
	UE_API virtual int Read(const TCHAR* InName, void* InBlob, int32 MaxBlobSize, int32& OutBlobSize) override;	
	...
	*	for the size, then allocated and reads a blob of sizeof(t) * NumItems
	...
	static UE_API TSharedPtr<FBackChannelOSCMessage> CreateFromBuffer(const void* Data, int32 DataLength);
...
protected:
	...
	UE_API void GetComponentSizes(int32& OutAddressSize, int32& OutTagSize, int32& OutBufferSize) const;

	UE_API int Serialize(const TCHAR Code, void* InData, int32 InSize);

	UE_API TCHAR ReadTag(const TCHAR ExpectedTag, bool SuppressWarning=false);
	UE_API int ReadData(void* InData, int32 InSize);
	UE_API int ReadTagAndData(const TCHAR ExpectedTag, void* InData, int32 InSize);

	UE_API int WriteTag(const TCHAR Code);
	UE_API int WriteTagAndData(const TCHAR Code, const void* InData, int32 InSize);
	UE_API int WriteData(const void* InData, int32 InSize);
...
protected:
	...
	static UE_API bool bIsLegacyConnection;
};
