 ...
 */
class FStateGraphNode : public TSharedFromThis<FStateGraphNode>, public FNoncopyable
{
public:

	UE_API FStateGraphNode(FName InName);

	UE_API virtual ~FStateGraphNode();
	...
	const FString& GetLogName() const
	{
		return LogName;
	}
	...
	static UE_API const TCHAR* GetStatusName(EStatus Status);
	...
	UE_API void SetTimeout(double InTimeout);
...
private:
	...
	UE_API void SetStatus(EStatus NewStatus);
 ...
 */
class FStateGraphNodeFunction : public FStateGraphNode
{
public:

	UE_API FStateGraphNodeFunction(FName InName, const FStateGraphNodeFunctionStart& InStartFunction);
	UE_API virtual bool CheckDependencies() const override;
 ...
 */
class FStateGraph : public TSharedFromThis<FStateGraph>, public FNoncopyable
{
public:

	UE_API FStateGraph(FName InName, const FString& InContextName = FString());

	UE_API virtual ~FStateGraph();
	...
	UE_API bool AddDependencies(FName NodeName, const TArrayView<const FName> Dependencies);
	...
	UE_API void LogDebugInfo(ELogVerbosity::Type Verbosity);
...
protected:
	...
	UE_API void UpdateConfig();

	/** Callback to update state graph and node configuration during hotfixes. */
	UE_API void OnConfigSectionsChanged(const FString& IniFilename, const TSet<FString>& SectionNames);
...
TSharedPtr<NodeType, ESPMode::ThreadSafe> FStateGraphNode::Next(ArgsTypes&&... Args) const
{
	...
	if (!StateGraphPtr)
	{
		UE_LOG(LogStateGraph, Warning, TEXT("[%s] Node attempted to be created with invalid state graph"), *LogName);
