...
void UFractureEditorMode::Render(const FSceneView* View, FViewport* Viewport, FPrimitiveDrawInterface* PDI)
{
	...
	RenderSelectionAsBoxes(View, Viewport, PDI);

	// if highlight is not selected disable them
	// technically we should prevent the highlight to be set originally but too many calling site, will need some refactor
	if (const UFractureSettings* FractureSettings = GetDefault<UFractureSettings>())
	{
		if (FractureSettings->SelectionDisplayMode != EFractureSelectionDisplayMode::Highlight)
		{
			for (TWeakObjectPtr<UGeometryCollectionComponent> SelectedCompWeakPtr : SelectedGeometryComponents)
			{
				if (UGeometryCollectionComponent* SelectedComp = SelectedCompWeakPtr.Get())
				{
					FScopedColorEdit EditBoneColor = SelectedComp->EditBoneSelection();
...
void UFractureEditorMode::RenderSelectionAsBoxes(const FSceneView* View, FViewport* Viewport, FPrimitiveDrawInterface* PDI)
{
	// this is a uggestion has nanite based geometry collection may want to override it 
	bool bShouldDisplayBounds = false;
	if (const UFractureSettings* FractureSettings = GetDefault<UFractureSettings>())
	{
		bShouldDisplayBounds = (FractureSettings->SelectionDisplayMode == EFractureSelectionDisplayMode::BoundingBox);
	}
	...
	for (TWeakObjectPtr<UGeometryCollectionComponent> SelectedCompWeakPtr : SelectedGeometryComponents)
	{
		if (UGeometryCollectionComponent* SelectedComp = SelectedCompWeakPtr.Get())
		{
			TSharedPtr<const FGeometryCollection> Collection = (SelectedComp->GetRestCollection()) ? SelectedComp->GetRestCollection()->GetGeometryCollection() : nullptr;
			if (!Collection)
			{
				continue;
			}

			const bool bUsesNanite = SelectedComp->GetSceneProxy() && SelectedComp->GetSceneProxy()->IsNaniteMesh();
			if (!bShouldDisplayBounds && !bUsesNanite)
			{
				continue;
			}

			const GeometryCollection::Facades::FCollectionTransformSelectionFacade TransformFacade(*Collection);
			const GeometryCollection::Facades::FBoundsFacade BoundsFacade(*Collection);
			if (!TransformFacade.IsValid() || !BoundsFacade.IsValid())
			{
				continue;
			}

			const FTransform& WorldTransform = SelectedComp->GetComponentToWorld();
			const TArray<FTransform3f>& ComponentSpaceTransforms = SelectedComp->GetComponentSpaceTransforms3f();

			const int32 NumTransforms = Collection->NumElements(FGeometryCollection::TransformGroup);
			const TManagedArray<FBox>& GeometryBounds = BoundsFacade.GetBoundingBoxes();
			const TManagedArray<int32>& TransformToGeometry = BoundsFacade.GetTransformToGeometryIndex();

			for (int32 SelectedBone : SelectedComp->GetSelectedBones())
			{
				if (SelectedBone < 0 || SelectedBone >= NumTransforms)
				{
					continue;
				}
				...
				// only leaf bones have geometry bounds we need to aggregate them 
				TArray<int32> LeafBoneIndices;
				TransformFacade.ConvertSelectionToRigidNodes(SelectedBone, LeafBoneIndices);
				for (const int32 LeafBoneIndex : LeafBoneIndices)
				{
					const int32 GeometryIndex = TransformToGeometry[LeafBoneIndex];
					if (GeometryBounds.IsValidIndex(GeometryIndex) && ComponentSpaceTransforms.IsValidIndex(LeafBoneIndex))
					{
						BoneExtent += GeometryBounds[GeometryIndex].TransformBy(FTransform(ComponentSpaceTransforms[LeafBoneIndex]));
					}
				}

				if (!BoneExtent.IsValid)
				{
					continue;
				}

				const FVector BoxCenter = BoneExtent.GetCenter();// +FVector(Offset);
				const FVector BoxHalfSize = BoneExtent.GetExtent();
				const FVector Corners[] =
				{
					WorldTransform.TransformPosition(BoxCenter + FVector(-BoxHalfSize.X, -BoxHalfSize.Y, -BoxHalfSize.Z)),
					WorldTransform.TransformPosition(BoxCenter + FVector(+BoxHalfSize.X, -BoxHalfSize.Y, -BoxHalfSize.Z)),
					WorldTransform.TransformPosition(BoxCenter + FVector(-BoxHalfSize.X, +BoxHalfSize.Y, -BoxHalfSize.Z)),
					WorldTransform.TransformPosition(BoxCenter + FVector(+BoxHalfSize.X, +BoxHalfSize.Y, -BoxHalfSize.Z)),
					WorldTransform.TransformPosition(BoxCenter + FVector(-BoxHalfSize.X, -BoxHalfSize.Y, +BoxHalfSize.Z)),
				...
				// bottom 
				PDI->DrawLine(Corners[0], Corners[1], FLinearColor::Yellow, 1.f, 0.0f);
				PDI->DrawLine(Corners[1], Corners[3], FLinearColor::Yellow, 1.f, 0.0f);
				PDI->DrawLine(Corners[3], Corners[2], FLinearColor::Yellow, 1.f, 0.0f);
				PDI->DrawLine(Corners[2], Corners[0], FLinearColor::Yellow, 1.f, 0.0f);

				// top 
				PDI->DrawLine(Corners[4], Corners[5], FLinearColor::Yellow, 1.f, 0.0f);
				PDI->DrawLine(Corners[5], Corners[7], FLinearColor::Yellow, 1.f, 0.0f);
				PDI->DrawLine(Corners[7], Corners[6], FLinearColor::Yellow, 1.f, 0.0f);
				PDI->DrawLine(Corners[6], Corners[4], FLinearColor::Yellow, 1.f, 0.0f);

				// vertical edges
				PDI->DrawLine(Corners[0], Corners[4], FLinearColor::Yellow, 1.f, 0.0f);
				PDI->DrawLine(Corners[1], Corners[5], FLinearColor::Yellow, 1.f, 0.0f);
				PDI->DrawLine(Corners[2], Corners[6], FLinearColor::Yellow, 1.f, 0.0f);
				PDI->DrawLine(Corners[3], Corners[7], FLinearColor::Yellow, 1.f, 0.0f);
			}
		}
	}
}
