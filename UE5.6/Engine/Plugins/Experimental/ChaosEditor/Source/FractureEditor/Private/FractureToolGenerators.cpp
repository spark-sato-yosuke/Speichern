...
DECLARE_DELEGATE_OneParam(FOnPathChosen, const UFractureToolGenerateAsset::FCreationParameters& Params);
...
public:
	...
	SLATE_ARGUMENT(bool, bUseRootProxies)
	SLATE_ARGUMENT(UDataflow*, DataflowAsset)
	SLATE_ARGUMENT(UPhysicalMaterial*, PhysicalMaterial)
...
private:
	...
	FString GetPhysicalMaterialPath() const
	{
		if (PhysicalMaterial)
		{
	...
	void OnPhysicalMaterialChanged(const FAssetData& InAssetData)
	{
		if (UPhysicalMaterial* NewPhysicalMaterial = Cast<UPhysicalMaterial>(InAssetData.GetAsset()))
		{
...
private:
	...
	/** Whether to use the meshes used to create the geometry collection as root proxies */
	bool bUseRootProxies = true;

	/** Dataflow asset to assign */
	TObjectPtr<UDataflow> DataflowAsset = nullptr;

	/** Physical Material to assign */
...
void SCreateGeometryCollectionFromObject::Construct(const FArguments& InArgs, TSharedPtr<SWindow> InParentWindow)
{
	...
	bUseRootProxies = InArgs._bUseRootProxies;
	DataflowAsset = InArgs._DataflowAsset;
	PhysicalMaterial = InArgs._PhysicalMaterial;
		...
		[
			...
			[
				...
				[
					...
					[
						SNew(STextBlock)
						.Text(LOCTEXT("UseRootProxiesLabel", "Use Root Proxies"))
						.ToolTipText(LOCTEXT("UseRootProxiesToolTip", "If checked, the meshes used to create the geometry collection will be used as root proxies for rendering the unbroken state."))
					]
					...
					[
						...
						.IsChecked_Lambda([this]()->ECheckBoxState
							{
								return bUseRootProxies ? ECheckBoxState::Checked : ECheckBoxState::Unchecked;
							})
						.OnCheckStateChanged_Lambda([this](ECheckBoxState NewState)
						{
							if (NewState == ECheckBoxState::Undetermined)
							...
							bUseRootProxies = NewState == ECheckBoxState::Checked;
						})
						.ToolTipText(LOCTEXT("UseRootProxiesToolTip", "If checked, the meshes used to create the geometry collection will be used as root proxies for rendering the unbroken state."))
					]
				]
				...
				[
					...
					[
						SNew(STextBlock)
						.Text(LOCTEXT("PhysicalMaterialAssetLabel", "PhysicalMaterial"))
						.ToolTipText(LOCTEXT("PhysicalMaterialAssetToolTip", "Optionally assign a Physical material to the new Geometry Collection."))
					]
					...
					[
						SNew(SObjectPropertyEntryBox)
						.ObjectPath(this, &SCreateGeometryCollectionFromObject::GetPhysicalMaterialPath)
						.OnObjectChanged(this, &SCreateGeometryCollectionFromObject::OnPhysicalMaterialChanged)
...
FReply SCreateGeometryCollectionFromObject::OnCreateAssetFromActorClicked()
{
	...
	UFractureToolGenerateAsset::FCreationParameters Params
	{
		.AssetPath = AssetPath / FileNameWidget->GetText().ToString(),
		.bFromToMeshTool = bFromToMeshTool,
		.bSplitIslands = bSplitIslands,
		.bUseRootProxies = bUseRootProxies,
		.DataflowAsset = DataflowAsset,
		.PhysicalMaterial = PhysicalMaterial,
	};
	OnCreateAssetAction.ExecuteIfBound(Params);
...
void UFractureToolGenerateAsset::OpenGenerateAssetDialog(TArray<AActor*>& Actors)
{
	...
	PickAssetPathWindow->SetContent(
		...
		.PhysicalMaterial(LastPhysicalMaterial)
		.bReimportToMeshOutput(bLastFromToMesh)
		.bSplitIslands(bLastSplitComponents)
		.bUseRootProxies(bLastUseRootProxies)
...
void UFractureToolGenerateAsset::OnGenerateAssetPathChosen(const UFractureToolGenerateAsset::FCreationParameters& Params, TArray<AActor*> Actors)
{	
	//Record the path and other settings
	int32 LastSlash = INDEX_NONE;
	if (Params.AssetPath.FindLastChar('/', LastSlash))
	{
		AssetPath = Params.AssetPath.Left(LastSlash);
	}
	bLastFromToMesh = Params.bFromToMeshTool;
	bLastSplitComponents = Params.bSplitIslands;
	bLastUseRootProxies= Params.bUseRootProxies;
	LastDataflowAsset = Params.DataflowAsset;
	LastPhysicalMaterial = Params.PhysicalMaterial;
	...
	if (Actors.Num() > 0)
	{
		...
		GeometryCollectionActor = ConvertActorsToGeometryCollection(Params, false/*bAddInternalMaterials*/, Actors);
...
AGeometryCollectionActor* UFractureToolGenerateAsset::ConvertActorsToGeometryCollection(const FCreationParameters& Params, bool bAddInternalMaterials, TArray<AActor*>& Actors)
{
	...
	AGeometryCollectionActor* NewActor = CreateNewGeometryActor(Params.AssetPath, FTransform(), true);
	...
	for (AActor* Actor : Actors)
	{
		...
		for (int32 ii = 0, ni = StaticMeshComponents.Num(); ii < ni; ++ii)
		{
			...
			if (StaticMeshComponent != nullptr)
			{
				...
				bool bSetInternalFromMaterialIndex = Params.bFromToMeshTool;
				
				FracturedGeometryCollection->GeometrySource.Emplace(SourceSoftObjectPath, ComponentTransform, SourceMaterials, Params.bSplitIslands, bSetInternalFromMaterialIndex);
				
				FGeometryCollectionEngineConversion::AppendStaticMesh(ComponentStaticMesh, SourceMaterials, ComponentTransform, FracturedGeometryCollection, false/*bReindexMaterials*/, bAddInternalMaterials, Params.bSplitIslands, bSetInternalFromMaterialIndex);
			}
		}
	...
	if (Params.PhysicalMaterial)
	{
		FracturedGeometryCollection->PhysicsMaterial = Params.PhysicalMaterial;
	}

	if (Params.bUseRootProxies)
	{
		FracturedGeometryCollection->CustomRendererType = UGeometryCollectionRootProxyRenderer::StaticClass();
		FracturedGeometryCollection->SetRootProxiesFromGeometrySources();
	}

	FracturedGeometryCollection->InitializeMaterials(bAddInternalMaterials);

	if (Params.DataflowAsset)
	{
		FracturedGeometryCollection->DataflowInstance.SetDataflowAsset(Params.DataflowAsset);
	}
...
class AGeometryCollectionActor* UFractureToolGenerateAsset::CreateNewGeometryActor(const FString& InAssetPath, const FTransform& Transform, bool AddMaterials /*= false*/)
{
	...
	if (InGeometryCollection->PhysicsMaterial == nullptr)
	{
