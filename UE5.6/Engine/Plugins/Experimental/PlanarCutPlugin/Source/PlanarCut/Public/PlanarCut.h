...
struct FInternalSurfaceMaterials
{
	 ...
	 */
	UE_API void SetUVScaleFromCollection(const GeometryCollection::Facades::FCollectionMeshFacade& CollectionMesh, int32 GeometryIdx = -1);

	
	UE_API int32 GetDefaultMaterialIDForGeometry(const FGeometryCollection& Collection, int32 GeometryIdx = -1) const;
};
...
struct FPlanarCells
{
	...
	UE_API FPlanarCells(const FPlane& Plane);
	UE_API FPlanarCells(const TArrayView<const FVector> Sites, FVoronoiDiagram &Voronoi);
	UE_API FPlanarCells(const TArrayView<const FBox> Boxes, bool bResolveAdjacencies = false);
