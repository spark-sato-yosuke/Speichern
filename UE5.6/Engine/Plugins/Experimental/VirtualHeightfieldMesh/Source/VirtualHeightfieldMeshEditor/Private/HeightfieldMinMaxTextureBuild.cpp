...
namespace VirtualHeightfieldMesh
{
	...
	bool BuildMinMaxHeightTexture(UVirtualHeightfieldMeshComponent* InComponent)
	{
		...
		URuntimeVirtualTexture* VirtualTexture = InComponent->GetVirtualTexture();
		const int32 RuntimeVirtualTextureId = VirtualTexture->GetUniqueID();
		...
		for (int32 TileY = 0; TileY < NumTilesY && !Task.ShouldCancel(); TileY++)
		{
			for (int32 TileX = 0; TileX < NumTilesX; TileX++)
			{
					...
					MaxLevel, MipLevel = 0](FRHICommandListImmediate& RHICmdList)
				{
					...
					Desc.NumPageDescs = 1;
					Desc.Targets[0].PooledRenderTarget = RenderTileResources.GetTileRenderTarget();
					Desc.Targets[1].PooledRenderTarget = nullptr;
					Desc.Targets[2].PooledRenderTarget = nullptr;
					Desc.PageDescs[0].DestRect[0] = FIntRect(0, 0, TileSize, TileSize);
					Desc.PageDescs[0].DestRect[1] = FIntRect();
					Desc.PageDescs[0].DestRect[2] = FIntRect();
