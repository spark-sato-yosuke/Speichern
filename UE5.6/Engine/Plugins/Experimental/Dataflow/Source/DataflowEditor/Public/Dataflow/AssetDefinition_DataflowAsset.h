...
struct DATAFLOWEDITOR_API FDataflowConnectionData
{
	...
	void Set(const FDataflowOutput& Output, const FDataflowInput& Input);

	static FString GetNode(const FString InConnection);
	static FString GetProperty(const FString InConnection);
	static void GetNodePropertyAndType(const FString InConnection, FString& OutNode, FString& OutProperty, FString& OutType);
};
