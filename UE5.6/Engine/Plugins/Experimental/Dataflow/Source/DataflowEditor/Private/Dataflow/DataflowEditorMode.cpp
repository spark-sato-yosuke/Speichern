		...
		FExecuteAction::CreateWeakLambda(ToolsContext, [this, ToolsContext, ToolIdentifier, Builder]()
		{
			...
			if (const IDataflowEditorToolBuilder* const DataflowToolBuilder = Cast<IDataflowEditorToolBuilder>(Builder))
			{
				...
				if (SupportedModes.Num() > 0 && !SupportedModes.Contains(GetConstructionViewMode()))
				{
					...
					if (const USelection* const SelectedComponents = FDataflowPreviewSceneBase::GetSelectedComponents(ConstructionScene))
					{
					...
					if (bHadSingleSelectedMesh)
					{
						...
						{
							if (USelection* const SelectedComponents = FDataflowPreviewSceneBase::GetSelectedComponents(ConstructionScene))
							{
...
const TArray<FName>& UDataflowEditorMode::GetToolCategories()
{
	...
	if(!LocalEditor)
	{
		...
		{
			LocalEditor = ConstructionScene->GetDataflowEditor();
		}
...
{
	if (ConstructionScene && ConstructionScene->GetDataflowModeManager())
	{
		if (const UTypedElementSelectionSet* const TypedElementSelectionSet = ConstructionScene->GetDataflowModeManager()->GetEditorSelectionSet())
		{
...
void UDataflowEditorMode::RefocusSimulationViewportClient()
{
	...
	if (PinnedVC.IsValid() && SimulationScene)
	{
		// This will happen in FocusViewportOnBox anyways; do it now to get a consistent end result
		PinnedVC->ToggleOrbitCamera(false);

		const FBox SceneBounds = SimulationScene->GetBoundingBox();
...
void UDataflowEditorMode::ModeTick(float DeltaTime)
{
	...
	checkf(ConstructionScene, TEXT("Null ConstructionScene in DataflowEditorMode"));
	if (const UDataflowBaseContent* const EditorContent = ConstructionScene->GetEditorContent())
	{
