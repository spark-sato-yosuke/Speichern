...
struct FObservedComponent
{
	...
	*/
	...
	/** Gets the chaos cache from the observed component */
	UChaosCache* GetChaosCache() const {return Cache;}
...
public:
	...
	*/
	...
	CHAOSCACHING_API void SetEditorIconVisibility(bool bVisibility);
#endif
	void SetRestartSimulation(bool InbRestartSimulation) { bRestartSimulation = InbRestartSimulation; };
	void SetRestartTimeRange(float InRestartTimeStart, float InRestartTimeEnd) {
		RestartTimeStart = InRestartTimeStart; RestartTimeEnd = InRestartTimeEnd;
	};
	void SetIsSimulating(bool InbIsSimulating) { bIsSimulating = InbIsSimulating; };
	/** sets state for all observed components at the restart time. */
	CHAOSCACHING_API void ReadRestartData();
...
private:
	...
	*/
	...
	bool bRestartSimulation = false; // if restart for a specified time range
	float RestartTimeStart = TNumericLimits<float>::Max(); // if bRestartSimulation, use RestartTimeStart as an offset when saving, and clear cache data in [RestartTimeStart, RestartTimeEnd] 
	float RestartTimeEnd = -TNumericLimits<float>::Max();
