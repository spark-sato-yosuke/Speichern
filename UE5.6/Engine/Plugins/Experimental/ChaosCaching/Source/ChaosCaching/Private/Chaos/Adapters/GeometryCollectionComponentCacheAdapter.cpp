...
namespace Chaos
{
															...
															TArray<TPBDRigidParticleHandle<Chaos::FReal, 3>*>& OutUpdatedRigids) const
	{
		...
		if(EnableEvents)
		{
			...
			{
				if( const FEnableStateEvent* Event = Handle.GetConst<FEnableStateEvent>())
				{
		...
		if ((Proxy->GetSimParameters().bGenerateBreakingData || Proxy->GetSimParameters().bGenerateGlobalBreakingData) && BreakingEvents)
		{
			...
			{
				if (const FBreakingEvent* Event = Handle.GetConst<FBreakingEvent>())
				{
					if (Chaos::FPBDRigidClusteredParticleHandle* Particle = Proxy->GetParticle_Internal(Event->Index))
					{
						...
						CachedBreak.Orientation = ComponentToWorld.TransformRotation(Event->Orientation);
		...
		if (Proxy->GetSimParameters().bGenerateTrailingData && TrailingEvents)
		{
			...
			{
				if (const FTrailingEvent* Event = Handle.GetConst<FTrailingEvent>())
				{
					if (Chaos::FPBDRigidClusteredParticleHandle* Particle = Proxy->GetParticle_Internal(Event->Index))
					{
						if (Particle->ObjectState() != EObjectStateType::Kinematic)
						...
						CachedTrail.Orientation = ComponentToWorld.TransformRotation(Event->Orientation);
		...
		if ((Proxy->GetSimParameters().bGenerateCollisionData || Proxy->GetSimParameters().bGenerateGlobalCollisionData) && CollisionEvents)
		{
			...
			{
				if (const FCollisionEvent* Event = Handle.GetConst<FCollisionEvent>())
				{
