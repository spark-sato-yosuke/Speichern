...
TSharedRef<SWidget> FDaySequenceEditorModule::CreateDaySequenceViewportToolbarExtension(FWeakObjectPtr ExtensionContext)
{
	...
	{
		if (TSharedPtr<SViewportToolBar> Toolbar = ExtensionContextObject->ViewportToolBar.Pin())
		{
			return SNew(SEditorViewportToolbarMenu)
				.ParentToolBar(Toolbar)
				.Label(LOCTEXT("DaySequenceMenuLabel", "Time of Day"))
				.LabelIcon(FDaySequenceEditorStyle::Get()->GetBrush("DaySequenceEditor.ViewportToolBar"))
				.OnGetMenuContent_Lambda([this]()
				{
					return CreateTimeOfDayWidget();
				})
				.Visibility_Lambda([this]()
				{
					return IsMenuVisible() && UE::UnrealEd::ShowOldViewportToolbars() ? EVisibility::Visible : EVisibility::Collapsed;
				});
		}
	}
