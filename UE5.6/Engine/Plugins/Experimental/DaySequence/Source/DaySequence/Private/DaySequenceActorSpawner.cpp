...
namespace DaySequenceActorSpawnerLocals
{
...
TSharedRef<IMovieSceneObjectSpawner> FDaySequenceActorSpawner::CreateObjectSpawner()
{
	return MakeShareable(new FDaySequenceActorSpawner);
}

UClass* FDaySequenceActorSpawner::GetSupportedTemplateType() const
{
	return AActor::StaticClass();
}

UObject* FDaySequenceActorSpawner::SpawnObject(FMovieSceneSpawnable& Spawnable, FMovieSceneSequenceIDRef TemplateID, TSharedRef<const UE::MovieScene::FSharedPlaybackState> SharedPlaybackState)
{
	...
	UWorld* WorldContext = PlaybackContext->GetWorld();
	...
	if (DesiredLevelName != NAME_None)
	{
		...
		{
			ULevelStreaming* LevelStreaming = DaySequenceActorSpawnerLocals::GetLevelStreaming(DesiredLevelName, WorldContext);
...
void FDaySequenceActorSpawner::DestroySpawnedObject(UObject& Object)
{
