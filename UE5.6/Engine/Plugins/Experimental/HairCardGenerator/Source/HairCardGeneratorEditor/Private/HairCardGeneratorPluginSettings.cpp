...
void UHairCardGeneratorPluginSettings::MakeSingleFilterGroupSettings()
{
	FilterGroupGenerationSettings.Reset();
	
	for(FName& CardGroup : CardGroupIds)
	{
		FilterGroupGenerationSettings.Add(NewObject<UHairCardGeneratorGroupSettings>(this));

		FilterGroupGenerationSettings.Last()->SetSource(GroomAsset, LODIndex);
		FilterGroupGenerationSettings.Last()->Reset(UHairCardGeneratorGroupSettings::StaticClass()->GetDefaultObject<UHairCardGeneratorGroupSettings>());

		FilterGroupGenerationSettings.Last()->ResetFilterDefault({CardGroup});
		FilterGroupGenerationSettings.Last()->UpdateStrandFilter();
	}

	ResetNumFilterGroups(CardGroupIds.Num());
	PostResetUpdates();
}

void UHairCardGeneratorPluginSettings::BuildFilterGroupSettings(const TArray<TArray<FName>>& FilterCardGroups, const TArray<FName>& FilterGroupNames)
{
	if(FilterGroupNames.Num() == FilterCardGroups.Num())
	{
		FilterGroupGenerationSettings.Reset();
		int32 FilterIndex = 0;
		for(const TArray<FName>& CardGroups : FilterCardGroups)
		{
			TArray<FName> LocalGroups;
			for(const FName& CardGroup : CardGroups)
			{
				if(CardGroupIds.Contains(CardGroup))
				{
					LocalGroups.Add(CardGroup);
				}
			}
			FilterGroupGenerationSettings.Add(NewObject<UHairCardGeneratorGroupSettings>(this));
			...
			{
				FilterGroupGenerationSettings.Last()->ResetFilterDefault(LocalGroups);
			}
			else
			{
				FilterGroupGenerationSettings.Last()->ResetFilterDefault(CardGroupIds);
			}
			FilterGroupGenerationSettings.Last()->UpdateStrandFilter();
			FilterGroupGenerationSettings.Last()->SetFilterName(FilterGroupNames[FilterIndex]);
			++FilterIndex;
		}

		ResetNumFilterGroups(FilterGroupGenerationSettings.Num());
		PostResetUpdates();
	}
}
