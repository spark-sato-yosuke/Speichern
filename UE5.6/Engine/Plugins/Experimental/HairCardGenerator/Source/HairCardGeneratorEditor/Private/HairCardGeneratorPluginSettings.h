...
class HAIRCARDGENERATOREDITOR_API UHairCardGeneratorPluginSettings : public UHairCardGenerationSettings
{
	GENERATED_BODY()
...
public:
	...
	/** Make filter settings for each groups */
	void MakeSingleFilterGroupSettings();

	/** Build the filter group settings according to a list of valid card groups and names */
	void BuildFilterGroupSettings(const TArray<TArray<FName>>& FilterCardGroups, const TArray<FName>& FilterGroupNames);
	...
	UGroomAsset* GetGroomAsset() const {return GroomAsset;}
...
public:
	...
	/** Get the filter name */
	const FName& GetFilterName() const {return FilterName;}

	/** Set the filter name with a given identifier */
	void SetFilterName(const FName& InFilterName) {FilterName = InFilterName;}
