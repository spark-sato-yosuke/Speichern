...
namespace HairCardGeneratorEditor_Impl
{
	...
	static bool LoadGroomData(UGroomAsset* GroomAsset, const bool SaveCached = false);
	...
	static bool GenerateCardsForCardGroup(const TObjectPtr<const UHairCardGeneratorPluginSettings> GenerationSettings, const int32 FilterIndex, const uint8 GenFlags);
	static bool GenerateCardsGeometry(TObjectPtr<const UHairCardGeneratorPluginSettings> Settings, int index, TArray<TArray<FVector3f>>& ClumpsGeometry);
...
/* FHairCardGeneratorUtils Definition
 *****************************************************************************/

bool FHairCardGeneratorUtils::BuildGenerationSettings(const bool bQuerySettings, UGroomAsset* NewGroomAsset, FHairGroupsCardsSourceDescription& CardsDesc,
	TObjectPtr<UHairCardGeneratorPluginSettings>& GenerationSettings, uint8& AllGenFlags, uint8& PipelineGenFlags)
{
	// Find the HairCardGenController in Python (or wherever it may be implemented -- could be moved to C++/Blueprints/etc.)
	...
	UE_CLOG(CardsDesc.GenerationSettings && !UHairCardGeneratorPluginSettings::IsCompatibleSettings(CardsDesc.GenerationSettings), LogHairCardGenerator, Warning, TEXT("Old hair-card generation settings (from a different generator) will be discarded when you run this generator."));

	GenerationSettings = CardGenController->GetGroomSettings(NewGroomAsset, CardsDesc.LODIndex);
	...
	AllGenFlags = GenerationSettings->GetAllPipelineGeneratedDifferences();
	...
	PipelineGenFlags = GenerationSettings->GetPipelineGeneratedDifferences();
...
bool FHairCardGeneratorUtils::LoadGenerationSettings(const TObjectPtr<UHairCardGeneratorPluginSettings>& GenerationSettings)
{
	// Find the HairCardGenController in Python (or wherever it may be implemented -- could be moved to C++/Blueprints/etc.)
	TObjectPtr<UHairCardGenControllerBase> CardGenController = FHairCardGenController::GetHairCardGenerator();
	if ( !CardGenController )
...
bool FHairCardGeneratorUtils::GenerateCardsClumps(const TObjectPtr<const UHairCardGeneratorPluginSettings>& GenerationSettings, const int32 FilterIndex, const uint8 GenFlags, TArray<int32>& StrandsClumps, int32& NumClumps)
{
	TObjectPtr<UHairCardGenControllerBase> CardGenController = FHairCardGenController::GetHairCardGenerator();
	if ( !CardGenController )
	{
	...
	if ( UHairCardGeneratorPluginSettings::CheckGenerationFlags(GenFlags, EHairCardGenerationPipeline::StrandClustering) && !GenerationSettings->bReduceCardsFromPreviousLOD )
	{
		StrandsClumps = CardGenController->GenerateClumps(FilterIndex);
		if (StrandsClumps.IsEmpty())
		{
			UE_LOG(LogHairCardGenerator, Error, TEXT("Strand clustering failed for settings group %d. Check the log for details."), FilterIndex);
			return false;
		}
		else  
		{
			NumClumps = StrandsClumps.Last();
		}
	}
	return true;
}

bool FHairCardGeneratorUtils::GenerateCardsGeometry(const TObjectPtr<const UHairCardGeneratorPluginSettings>& GenerationSettings, const int32 FilterIndex, const uint8 GenFlags, TArray<TArray<FVector3f>>& ClumpsGeometry)
{
	TObjectPtr<UHairCardGenControllerBase> CardGenController = FHairCardGenController::GetHairCardGenerator();
	if ( !CardGenController )
	{
	...
	{
		if ( !HairCardGeneratorEditor_Impl::GenerateCardsGeometry(GenerationSettings, FilterIndex,ClumpsGeometry) )
		{
			UE_LOG(LogHairCardGenerator, Error, TEXT("Cards geometry failed for settings group %d. Check the log for details."), FilterIndex)
			return false;
		}
	}
	return true;
...
bool FHairCardGeneratorUtils::GenerateCardsTexturesClusters(const TObjectPtr<const UHairCardGeneratorPluginSettings>& GenerationSettings, const int32 FilterIndex, const uint8 GenFlags,
	TArray<int32>& CardsTextures, int32& NumTextures)
{
	TObjectPtr<UHairCardGenControllerBase> CardGenController = FHairCardGenController::GetHairCardGenerator();
	if ( !CardGenController )
	{
	...
	if ( UHairCardGeneratorPluginSettings::CheckGenerationFlags(GenFlags, EHairCardGenerationPipeline::TextureClustering) && !GenerationSettings->bReduceCardsFromPreviousLOD )
	{
		CardsTextures = CardGenController->ClusterTextures(FilterIndex);
		if (CardsTextures.IsEmpty())
		{
			UE_LOG(LogHairCardGenerator, Error, TEXT("Texture clustering failed for settings group %d. Check the log for details."), FilterIndex)
			return false;
		}
		else
		{
			NumTextures = CardsTextures.Last();
		}
	}
	return true;
}

bool FHairCardGeneratorUtils::GenerateTexturesLayoutAndAtlases(const TObjectPtr<const UHairCardGeneratorPluginSettings>& GenerationSettings, const uint8 GenFlags, TArray<float>& VertexUVs)
{
	TObjectPtr<UHairCardGenControllerBase> CardGenController = FHairCardGenController::GetHairCardGenerator();
	if ( !CardGenController )
	{
	...
	if ( UHairCardGeneratorPluginSettings::CheckGenerationFlags(GenFlags, EHairCardGenerationPipeline::TextureLayout) && !GenerationSettings->bReduceCardsFromPreviousLOD )
	{
		VertexUVs = CardGenController->GenerateTextureLayout();
	...
	if ( UHairCardGeneratorPluginSettings::CheckGenerationFlags(GenFlags, EHairCardGenerationPipeline::TextureRendering) && !GenerationSettings->bReduceCardsFromPreviousLOD )
	{
...
bool FHairCardGeneratorUtils::RunCardsGeneration(const TObjectPtr<UHairCardGeneratorPluginSettings>& GenerationSettings, const uint8 PipelineFlags, const FGeneratorFunction& PipelineFunction, const bool bCheckFlags)
{
	// Run card group generation in loop
	bool bAllGenerated = true;
	for ( int32 FilterIndex = 0; FilterIndex < GenerationSettings->GetFilterGroupSettings().Num(); ++FilterIndex )
	{
		const uint8 GroupGenFlags = bCheckFlags ? (PipelineFlags | GenerationSettings->GetPipelineFilterGroupDifferences(FilterIndex)) : static_cast<uint8>(EHairCardGenerationPipeline::All);
		...
		{
			UE_LOG(LogHairCardGenerator, Display, TEXT("Setttings group %d already generated. Skipping."), FilterIndex);
			continue;
		}

		// TODO: On errors this will force full regeneration (because we clear the group gen object)
		GenerationSettings->ClearPipelineGeneratedFilterGroup(FilterIndex);
		bAllGenerated &= PipelineFunction(GenerationSettings, FilterIndex, GroupGenFlags);
		...
		GenerationSettings->WritePipelineGeneratedFilterGroup(FilterIndex);
	}
...
static bool GenerateHairCardGenGroomData(const int PointsPerStrand, const FHairDescription& InGroomData, FHairCardGen_GroomData& Out, const FHairCardGeneratorUtils::FBuilderFunction& BuilderFunction)
{
	...
	TArray<TArray<FVector>> StrandsPositions;
	BuilderFunction(StrandsPositions);
	...
	for ( const FHairStrandAttribsRefProxy& StrandAttribs : TraverseStrands )
	{
		...
		const bool bIsVertexValid = StrandsPositions.IsValidIndex(StrandIndex) && (StrandAttribs.NumVerts() <= StrandsPositions[StrandIndex].Num());
		for (int StrandVertIndex = 0; StrandVertIndex < StrandAttribs.NumVerts(); ++StrandVertIndex)
		{
			VertexPositions[StrandVertIndex] = bIsVertexValid ? StrandsPositions[StrandIndex][StrandVertIndex] : FVector(StrandAttribs.GetVertexPosition(StrandVertIndex));
...
bool FHairCardGeneratorUtils::LoadGroomStrands(const UGroomAsset* GroomAsset, const FBuilderFunction& BuilderFunction)
{
	TObjectPtr<UHairCardGenControllerBase> CardGenController = FHairCardGenController::GetHairCardGenerator();
	if ( !CardGenController )
	{
	...
	FHairCardGen_GroomData CardGenData;
	GenerateHairCardGenGroomData(CardGenController->GetPointsPerCurve(), GroomAsset->GetHairDescription(), CardGenData, BuilderFunction);

	FString CachedGroomsPath = FPaths::ProjectIntermediateDir() / TEXT("GroomHairCardGen") / TEXT("CachedGrooms");
	return CardGenController->LoadGroomData(CardGenData, GroomAsset->GetName(), CachedGroomsPath);
}

bool FHairCardGeneratorUtils::BuildCardsAssets(UGroomAsset* NewGroomAsset, FHairGroupsCardsSourceDescription& CardsDesc,
	const TObjectPtr<UHairCardGeneratorPluginSettings>& GenerationSettings, const uint8 GenFlags)
{
	...
	if ( GenerationSettings->CheckGenerationFlags(GenFlags, EHairCardGenerationPipeline::ImportTextures) )
	{
	...
	const int32 NumPhysGroups = NewGroomAsset->GetHairDescriptionGroups().HairGroups.Num();

	// Generate static mesh from cards and add to destination package
	if ( GenerationSettings->CheckGenerationFlags(GenFlags, EHairCardGenerationPipeline::GenerateMesh) )
	{
...
static bool HairCardGeneratorEditor_Impl::LoadGroomData(UGroomAsset* NewGroomAsset, const bool SaveCached)
{
...
{
	return GenerateHairCardGenGroomData(PointsPerStrand, InGroomData, Out, [](TArray<TArray<FVector>>&){});
}

static bool HairCardGeneratorEditor_Impl::GenerateCardsGeometry(TObjectPtr<const UHairCardGeneratorPluginSettings> Settings, int index, TArray<TArray<FVector3f>>& ClumpsGeometry)
{
	TObjectPtr<UHairCardGenControllerBase> CardGenController = FHairCardGenController::GetHairCardGenerator();
	if ( !CardGenController )
	{
	...
	SlowTask.EnterProgressFrame(9);
	TArray<float> ClumpsVertices = CardGenController->GenerateCardsGeometry();
	if (ClumpsVertices.IsEmpty())
	{
		UE_LOG(LogHairCardGenerator, Error, TEXT("Card geometry generation failed for settings group %d. Check the log for details."), index);
		return false;
	}
	
	// Fill the vertices coming from the optimizer
	ClumpsGeometry.Reset();
	ClumpsGeometry.AddDefaulted();
	
	int32 VertexIndex = 0;
	while(VertexIndex < ClumpsVertices.Num())
	{
		if(ClumpsVertices[VertexIndex] == -1.0)
		{
			if(VertexIndex != (ClumpsVertices.Num()-1))
			{
				ClumpsGeometry.AddDefaulted();
			}
			VertexIndex += 1;
		...
		{
			ClumpsGeometry.Last().Add(FVector3f(ClumpsVertices[VertexIndex], ClumpsVertices[VertexIndex+1], ClumpsVertices[VertexIndex+2]));
			VertexIndex += 3;
		}
	}
...
static bool HairCardGeneratorEditor_Impl::GenerateCardsForCardGroup(const TObjectPtr<const UHairCardGeneratorPluginSettings> GenerationSettings, const int32 FilterIndex, const uint8 GenFlags)
{
	FScopedSlowTask SlowTask(10, FText::Format(LOCTEXT("GeneratingHairCards.GenerateGroup", "Generating Hair Cards for settings group {0}..."), FilterIndex));
	SlowTask.MakeDialog(/*bShowCancelButton =*/true);

	SlowTask.EnterProgressFrame(1);
	TArray<int32> StrandsClumps;
	int32 NumClumps = 0;
	if(!FHairCardGeneratorUtils::GenerateCardsClumps(GenerationSettings, FilterIndex, GenFlags, StrandsClumps, NumClumps))
	{
		SlowTask.EnterProgressFrame(9);
		return false;
	}
	...
	if(!FHairCardGeneratorUtils::GenerateCardsGeometry(GenerationSettings, FilterIndex, GenFlags, ClumpsGeometry))
	{
	...
	int32 NumTextures= 0;
	if(!FHairCardGeneratorUtils::GenerateCardsTexturesClusters(GenerationSettings, FilterIndex, GenFlags, CardsTextures, NumTextures))
	{
		return false;
	}
...
bool FHairCardGeneratorEditorModule::GenerateHairCardsForLOD(UGroomAsset* NewGroomAsset, FHairGroupsCardsSourceDescription& CardsDesc)
{
	...
	uint8 AllGenFlags = 0, PipelineGenFlags = 0;
	if(!FHairCardGeneratorUtils::BuildGenerationSettings(true, NewGroomAsset, CardsDesc, GenerationSettings, AllGenFlags, PipelineGenFlags))
	{
		return false;
	}
	...
	if(!FHairCardGeneratorUtils::RunCardsGeneration(GenerationSettings, PipelineGenFlags,
		[](const TObjectPtr<const UHairCardGeneratorPluginSettings>& GenerationSettings, const int32 FilterIndex, const uint8 GenFlags)
		{
			return HairCardGeneratorEditor_Impl::GenerateCardsForCardGroup(GenerationSettings, FilterIndex, GenFlags);
		}))
	{
		return false;
	}
	...
	if(!FHairCardGeneratorUtils::GenerateTexturesLayoutAndAtlases(GenerationSettings, AllGenFlags, VertexUVs))
	{
		return false;
	}

	if(!FHairCardGeneratorUtils::BuildCardsAssets(NewGroomAsset, CardsDesc, GenerationSettings, AllGenFlags))
	{
		return false;
	}
	return true;
