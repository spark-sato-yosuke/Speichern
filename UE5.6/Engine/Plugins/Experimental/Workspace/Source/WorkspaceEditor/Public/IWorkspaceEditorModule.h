...
struct WORKSPACEEDITOR_API FWorkspaceDocument
{
	FWorkspaceOutlinerItemExport Export;
	UObject* Object = nullptr;
	
	FWorkspaceDocument() = default;
	FWorkspaceDocument(const FWorkspaceOutlinerItemExport& InExport, UObject* InObject);
	...
	UObject* GetObject() const { check(Object); return Object; }
	template<class ObjectClass>
	ObjectClass* GetTypedObject() const { return CastChecked<ObjectClass>(Object); }
};

// Context passed to workspace editor delegates
struct WORKSPACEEDITOR_API FWorkspaceEditorContext
{
	FWorkspaceEditorContext(const TSharedRef<IWorkspaceEditor>& InWorkspaceEditor, const FWorkspaceDocument& InDocument);
	FWorkspaceEditorContext(const TSharedRef<IWorkspaceEditor>& InWorkspaceEditor, UObject* InObject, const FWorkspaceOutlinerItemExport& InExport);
