...
struct TTransformArrayAoS
{
	...
	void SetNum(int32 NumTransforms, EAllowShrinking AllowShrinking = EAllowShrinking::Default)
	{
	...
	void SetNumUninitialized(int32 NumTransforms, EAllowShrinking AllowShrinking = EAllowShrinking::Default)
	{
	...
	// Sets the whole array to 0xCDCDCDCD to aid debugging
	void DebugFill()
	{
		FMemory::Memset(Transforms.GetData(), 0xCD, sizeof(FTransform) * Transforms.Num());
	}
...
{
	// We add a bit of padding at the end of our buffer to allow 32-byte wide load/stores
	// without the risk of triggering access exceptions
	static constexpr int32 PaddingSize = sizeof(double);
	static constexpr int32 TransformSize = sizeof(FVector) + sizeof(FQuat) + sizeof(FVector);
	...
	{
		const int32 Padding = Slack != 0 ? PaddingSize : 0;
		AllocatedMemory.Empty(Slack + Padding);
		UpdateViews(AllocatedMemory.GetData(), 0);
	}

	inline void Reset(int32 NumTransforms)
	{
		const int32 Padding = NumTransforms != 0 ? PaddingSize : 0;
		AllocatedMemory.Reset(NumTransforms * TransformSize + Padding);
	}

	void SetNum(int32 NumTransforms, EAllowShrinking AllowShrinking = EAllowShrinking::Default)
	{
		const int32 Padding = NumTransforms != 0 ? PaddingSize : 0;
		AllocatedMemory.SetNum(NumTransforms * TransformSize + Padding, AllowShrinking);
	...
	{
		const int32 Padding = NumTransforms != 0 ? PaddingSize : 0;
		AllocatedMemory.SetNumUninitialized(NumTransforms * TransformSize + Padding, AllowShrinking);
	...
	inline void CopyTransforms(const FTransformArraySoAConstView& Other, int32 Index, int32 NumTransforms)
	{
		SCOPE_CYCLE_COUNTER(STAT_AnimNext_CopyTransforms_SoA);
	...
	void DebugFill()
	{
		FMemory::Memset(AllocatedMemory.GetData(), 0xCD, AllocatedMemory.Num());
	}
