...
struct FEffector
{
	...
	void UpdateFromInputs(const FBone& InSolverRoot, const bool bAllowStretch);
	float CalculateDistanceToChainRoot() const;
	float CalculateDistanceToSolveRootAlongBones() const;
...
public:
	...
	PBIK_API void Solve(const FPBIKSolverSettings& InSolverSettings);
...
private:
	...
	void SolveConstraints(const FPBIKSolverSettings& InSolverSettings);
