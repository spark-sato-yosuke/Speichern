...
namespace
{
	...
	public:
		...
		{
			IndexBufferRHI = UE::RHIResourceUtils::CreateIndexBufferFromArray(RHICmdList, TEXT("FImagePlateIndexBuffer"), EBufferUsageFlags::Static | EBufferUsageFlags::ShaderResource, MakeConstArrayView(Indices));
		}
	};
...
UImagePlateComponent::UImagePlateComponent(const FObjectInitializer& Init)
	: Super(Init)
{
	static ConstructorHelpers::FObjectFinder<UMaterialInterface> DefaultImagePlateMaterial(TEXT("/ImagePlate/DefaultImagePlateMaterial"));
	Plate.Material = DefaultImagePlateMaterial.Object;
...
void UImagePlateComponent::SetImagePlate(FImagePlateParameters NewPlate)
{
	...
	// Interpret a null material to mean the default material
	if (!Plate.Material)
	{
		Plate.Material = GetDefault<UImagePlateComponent>()->Plate.Material;
	}
