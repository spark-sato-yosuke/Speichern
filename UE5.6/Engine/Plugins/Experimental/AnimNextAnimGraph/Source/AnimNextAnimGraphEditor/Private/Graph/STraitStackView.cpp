											...
											if (URigVMNode* ModelNode = EdGraphNode->GetModelNode())
											{
												if (UAnimNextTraitStackUnitNode* UnitNode = Cast<UAnimNextTraitStackUnitNode>(ModelNode))
												{
													...
													if (UScriptStruct* Struct = UnitNode->GetScriptStruct())
													{
														...
														{
															if(Controller->SetNodeTitle(ModelNode, InNewText.ToString(), true, false, true))
															{
																if (UnitNode->IsExposedToManifest()) // if the node is exposed to manifest, we have to force a recompile to update asset tags
																{
																	if (UAnimNextRigVMAssetEditorData* EditorData = EdGraphNode->GetGraph()->GetTypedOuter<UAnimNextRigVMAssetEditorData>())
																	{
																		UE::AnimNext::UncookedOnly::FAnimGraphUtils::RequestVMAutoRecompile(EditorData);
																	}
																}
															}
														}
