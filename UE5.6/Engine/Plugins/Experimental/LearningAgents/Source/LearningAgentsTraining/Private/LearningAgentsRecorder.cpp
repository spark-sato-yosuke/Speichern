...
void ULearningAgentsRecorder::AddExperience()
{
	...
	for (const int32 AgentId : Manager->GetAllAgentSet())
	{
		...
		RecordBuffers[AgentId].Push(Interactor->GetObservationVectorsArrayView()[AgentId], Interactor->GetActionVectorsArrayView()[AgentId]);
	}
}
...
void ULearningAgentsRecorder::SaveSchemaConfig()
{
	TSharedRef<FJsonObject> ConfigObject = MakeShared<FJsonObject>();
	ConfigObject->SetObjectField(TEXT("ObservationSchema"),
		UE::Learning::Trainer::ConvertObservationSchemaToJSON(Interactor->GetObservationSchema()->ObservationSchema, Interactor->GetObservationSchemaElement().SchemaElement));
	ConfigObject->SetObjectField(TEXT("ActionSchema"),
		UE::Learning::Trainer::ConvertActionSchemaToJSON(Interactor->GetActionSchema()->ActionSchema, Interactor->GetActionSchemaElement().SchemaElement));
	...
	FFileHelper::SaveStringToFile(ConfigString, *(RecordingDirectory / "schema-config.json"));
}
