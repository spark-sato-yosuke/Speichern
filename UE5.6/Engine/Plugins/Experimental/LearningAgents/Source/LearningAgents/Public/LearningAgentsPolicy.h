...
public:
	...
	/** If true, policy can be evaluated on multiple threads to speed up large agent counts. */
	UPROPERTY(EditAnywhere, Category = "LearningAgents")
	bool bUseParallelEvaluation = true;
};
...
public:
	...
	/**
	 * Decodes and samples action vectors using the Decoder network. This should be called after both MakeActionModifiers and EvaluatePolicy, but 
	 * before PerformActions.
	 * 
