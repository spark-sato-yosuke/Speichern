	...
	const int32 Seed)
{
	...
	if (!EncoderNetwork->NeuralNetworkData || bReinitializeEncoderNetwork)
	{
		...
		check(EncoderInputSize == ObservationVectorSize);
		check(EncoderOutputSize == ObservationEncodedVectorSize);
	...
	if (!PolicyNetwork->NeuralNetworkData || bReinitializePolicyNetwork)
	{
		...
		// Scale down memory cell initial weights to improve stability at beginning of training
		UE::NNE::RuntimeBasic::FModelBuilder::FLinearLayerSettings MemoryCellWeightSettings;
		MemoryCellWeightSettings.WeightInitializationSettings.Scale = 0.1f;
		...
		Builder.WriteFileDataAndReset(FileData, PolicyInputSize, PolicyOutputSize,
			Builder.MakeMemoryBackbone(
				...
				Builder.MakeMLP(
					...
					true),
				Builder.MakeMemoryCellLayer(
					PolicyHiddenLayerSize,
					PolicyHiddenLayerSize,
					MemoryStateSize,
					MemoryCellWeightSettings),
				Builder.MakeSequence({
					...
					Builder.MakeDenormalize(
						ActionEncodedVectorSize,
						Builder.MakeValuesZero(ActionEncodedVectorSize),
						Builder.MakeValuesConstant(ActionEncodedVectorSize, PolicyInitialEncodedActionScale))
					})
			));

		check(PolicyInputSize == ObservationEncodedVectorSize + MemoryStateSize);
		check(PolicyOutputSize == ActionEncodedVectorSize + MemoryStateSize);
	...
	if (!DecoderNetwork->NeuralNetworkData || bReinitializeDecoderNetwork)
	{
		...
		check(DecoderInputSize == ActionEncodedVectorSize);
		check(DecoderOutputSize == ActionDistributionVectorSize);
	...
	InferenceSettings.bParallelEvaluation = PolicySettings.bUseParallelEvaluation;
...
{
	TRACE_CPUPROFILER_EVENT_SCOPE(ULearningAgentsPolicy::EncodeObservations);
	...
	if (EncoderObject->GetNeuralNetwork()->GetInputSize() != Interactor->GetObservationVectorSize())
	{
		UE_LOG(LogLearning, Error, TEXT("%s: Encoder Network Input size doesn't match. Network input size is %i but Encoder expects %i."), *GetName(),
			EncoderObject->GetNeuralNetwork()->GetInputSize(), Interactor->GetObservationVectorSize());
		return;
	}
...
{
	TRACE_CPUPROFILER_EVENT_SCOPE(ULearningAgentsPolicy::EvaluatePolicy);
...
{
	TRACE_CPUPROFILER_EVENT_SCOPE(ULearningAgentsPolicy::DecodeAndSampleActions);
	...
	// Check which agents have had their encoded action vector set and action modifiers made
	...
	for (const int32 AgentId : Manager->GetAllAgentSet())
	{
		...
		if (Interactor->GetActionModifierIteration(AgentId) == 0)
		{
			UE_LOG(LogLearning, Warning, TEXT("%s: Agent with id %i does not have an action modifier made so actions will not be decoded for it. Was MakeActionModifiers run without error?"), *GetName(), AgentId);
			continue;
		}
	...
	for (const int32 AgentId : ValidAgentSet)
	{
		UE::Learning::Action::SampleVectorFromDistributionVector(
			...
			Interactor->GetActionModifierVectorsArrayView()[AgentId],
			Interactor->GetActionSchema()->ActionSchema,
			Interactor->GetActionSchemaElement().SchemaElement,
			ActionNoiseScale);
	}
...
{
	TRACE_CPUPROFILER_EVENT_SCOPE(ULearningAgentsPolicy::RunInference);
