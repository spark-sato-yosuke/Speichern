...
namespace UE::Learning::Agents::Observation::Private
{
		...
		const Learning::Observation::FObjectElement ObjectElement)
	{
		...
		case Learning::Observation::EType::DiscreteExclusive:
		{
			...
			{
				UE_LOG(LogLearning, Error, TEXT("ValidateObservationObjectMatchesSchema: Observation '%s' index out of range for Schema. Expected '<%i', got '%i'."),
					*ObservationSchemaElementTag.ToString(),
					SchemaElementSize,
					ObjectElementIndex);
				return false;
		...
		case Learning::Observation::EType::DiscreteInclusive:
		{
			...
			{
				UE_LOG(LogLearning, Error, TEXT("ValidateObservationObjectMatchesSchema: Observation '%s' too many indices provided. Expected at most '%i', got '%i'."),
					*ObservationSchemaElementTag.ToString(),
					SchemaElementSize,
		...
		const FString& Prefix)
	{
		...
		case UE::Learning::Observation::EType::DiscreteExclusive:
		{
			const UE::Learning::Observation::FObjectDiscreteExclusiveParameters Parameters = Object.GetDiscreteExclusive(ObjectElement);

			UE_LOG(LogLearning, Display, TEXT("%s%s \"%s\" (%s) %i"), *Indentation, *Prefix, *Tag.ToString(), GetObservationTypeString(Type), Parameters.DiscreteIndex);
			return;
		}

		case UE::Learning::Observation::EType::DiscreteInclusive:
		{
			const UE::Learning::Observation::FObjectDiscreteInclusiveParameters Parameters = Object.GetDiscreteInclusive(ObjectElement);

			UE_LOG(LogLearning, Display, TEXT("%s%s \"%s\" (%s) %s"), *Indentation, *Prefix, *Tag.ToString(), GetObservationTypeString(Type), *UE::Learning::Array::FormatInt32(Parameters.DiscreteIndices));
			return;
		}

		case UE::Learning::Observation::EType::NamedDiscreteExclusive:
		{
			const UE::Learning::Observation::FObjectNamedDiscreteExclusiveParameters Parameters = Object.GetNamedDiscreteExclusive(ObjectElement);

			UE_LOG(LogLearning, Display, TEXT("%s%s \"%s\" (%s) %s"), *Indentation, *Prefix, *Tag.ToString(), GetObservationTypeString(Type), *Parameters.ElementName.ToString());
			return;
		}

		case UE::Learning::Observation::EType::NamedDiscreteInclusive:
		{
			const UE::Learning::Observation::FObjectNamedDiscreteInclusiveParameters Parameters = Object.GetNamedDiscreteInclusive(ObjectElement);

			UE_LOG(LogLearning, Display, TEXT("%s%s \"%s\" (%s) %s"), *Indentation, *Prefix, *Tag.ToString(), GetObservationTypeString(Type), *UE::Learning::Agents::Observation::Private::FormatNames(Parameters.ElementNames));
			return;
		}
...
FLearningAgentsObservationSchemaElement ULearningAgentsObservations::SpecifyExclusiveDiscreteObservation(ULearningAgentsObservationSchema* Schema, const int32 Size, const FName Tag)
{
	if (!Schema)
	{
		UE_LOG(LogLearning, Error, TEXT("SpecifyExclusiveDiscreteObservation: Schema is nullptr."));
		return FLearningAgentsObservationSchemaElement();
	}

	if (Size < 1)
	{
		UE_LOG(LogLearning, Error, TEXT("SpecifyExclusiveDiscreteObservation: Invalid Exclusive Discrete Observation Size '%i'."), Size);
		return FLearningAgentsObservationSchemaElement();
	}

	return { Schema->ObservationSchema.CreateDiscreteExclusive({ Size }, Tag) };
}
...
FLearningAgentsObservationSchemaElement ULearningAgentsObservations::SpecifyInclusiveDiscreteObservation(ULearningAgentsObservationSchema* Schema, const int32 Size, const FName Tag)
{
	if (!Schema)
	{
		UE_LOG(LogLearning, Error, TEXT("SpecifyInclusiveDiscreteObservation: Schema is nullptr."));
		return FLearningAgentsObservationSchemaElement();
	}

	if (Size < 0)
	{
		UE_LOG(LogLearning, Error, TEXT("SpecifyInclusiveDiscreteObservation: Invalid Inclusive Discrete Observation Size '%i'."), Size);
		return FLearningAgentsObservationSchemaElement();
	}

	if (Size == 0)
	{
		UE_LOG(LogLearning, Warning, TEXT("SpecifyInclusiveDiscreteObservation: Specifying zero-sized Inclusive Discrete Observation."));
	}

	return { Schema->ObservationSchema.CreateDiscreteInclusive({ Size }, Tag) };
}

FLearningAgentsObservationSchemaElement ULearningAgentsObservations::SpecifyNamedExclusiveDiscreteObservation(ULearningAgentsObservationSchema* Schema, const TArray<FName>& ElementNames, const FName Tag)
{
	return SpecifyNamedExclusiveDiscreteObservationFromArrayView(Schema, ElementNames, Tag);
}

FLearningAgentsObservationSchemaElement ULearningAgentsObservations::SpecifyNamedExclusiveDiscreteObservationFromArrayView(ULearningAgentsObservationSchema* Schema, const TArrayView<const FName> ElementNames, const FName Tag)
{
	if (!Schema)
	{
		UE_LOG(LogLearning, Error, TEXT("SpecifyNamedExclusiveDiscreteObservationFromArrayView: Schema is nullptr."));
		return FLearningAgentsObservationSchemaElement();
	}

	if (ElementNames.Num() < 1)
	{
		UE_LOG(LogLearning, Error, TEXT("SpecifyNamedExclusiveDiscreteObservationFromArrayView: Invalid Named Exclusive Discrete Observation Size '%i'."), ElementNames.Num());
		return FLearningAgentsObservationSchemaElement();
	}

	return { Schema->ObservationSchema.CreateNamedDiscreteExclusive({ ElementNames }, Tag) };
}

FLearningAgentsObservationSchemaElement ULearningAgentsObservations::SpecifyNamedInclusiveDiscreteObservation(ULearningAgentsObservationSchema* Schema, const TArray<FName>& ElementNames, const FName Tag)
{
	return SpecifyNamedInclusiveDiscreteObservationFromArrayView(Schema, ElementNames, Tag);
}

FLearningAgentsObservationSchemaElement ULearningAgentsObservations::SpecifyNamedInclusiveDiscreteObservationFromArrayView(ULearningAgentsObservationSchema* Schema, const TArrayView<const FName> ElementNames, const FName Tag)
{
	if (!Schema)
	{
		UE_LOG(LogLearning, Error, TEXT("SpecifyNamedInclusiveDiscreteObservationFromArrayView: Schema is nullptr."));
		return FLearningAgentsObservationSchemaElement();
	}

	if (ElementNames.Num() == 0)
	{
		UE_LOG(LogLearning, Warning, TEXT("SpecifyNamedInclusiveDiscreteObservationFromArrayView: Specifying zero-sized Named Inclusive Discrete Observation."));
	}

	return { Schema->ObservationSchema.CreateNamedDiscreteInclusive({ ElementNames }, Tag) };
}
	...
	const FLinearColor VisualLoggerColor)
{
	if (!Object)
	{
		UE_LOG(LogLearning, Error, TEXT("MakeExclusiveDiscreteObservation: Object is nullptr."));
		return FLearningAgentsObservationObjectElement();
	}
	...
	return { Object->ObservationObject.CreateDiscreteExclusive({ DiscreteIndex }, Tag) };
}
...
FLearningAgentsObservationObjectElement ULearningAgentsObservations::MakeInclusiveDiscreteObservation(
	...
	const FLinearColor VisualLoggerColor)
{
	return MakeInclusiveDiscreteObservationFromArrayView(Object, DiscreteIndices, Tag, bVisualLoggerEnabled, VisualLoggerListener, VisualLoggerAgentId, VisualLoggerLocation, VisualLoggerColor);
}
	...
	const FLinearColor VisualLoggerColor)
{
	...
	if (bVisualLoggerEnabled && VisualLoggerListener)
	{
		...
		UE_LEARNING_AGENTS_VLOG_STRING(VisualLoggerObject, LogLearning, Display, VisualLoggerLocation,
			...
			*Tag.ToString(),
			VisualLoggerAgentId,
			*UE::Learning::Array::FormatInt32(DiscreteIndices));
	}
#endif

	return { Object->ObservationObject.CreateDiscreteInclusive({ DiscreteIndices }, Tag) };
}

FLearningAgentsObservationObjectElement ULearningAgentsObservations::MakeNamedExclusiveDiscreteObservation(
	ULearningAgentsObservationObject* Object,
	...
	const FLinearColor VisualLoggerColor)
{
	if (!Object)
	{
		UE_LOG(LogLearning, Error, TEXT("MakeNamedExclusiveDiscreteObservation: Object is nullptr."));
		return FLearningAgentsObservationObjectElement();
	}
	...
	return { Object->ObservationObject.CreateNamedDiscreteExclusive({ ElementName }, Tag) };
}

FLearningAgentsObservationObjectElement ULearningAgentsObservations::MakeNamedInclusiveDiscreteObservation(
	ULearningAgentsObservationObject* Object,
	...
	const FLinearColor VisualLoggerColor)
{
	return MakeNamedInclusiveDiscreteObservationFromArrayView(Object, ElementNames, Tag, bVisualLoggerEnabled, VisualLoggerListener, VisualLoggerAgentId, VisualLoggerLocation, VisualLoggerColor);
}

FLearningAgentsObservationObjectElement ULearningAgentsObservations::MakeNamedInclusiveDiscreteObservationFromArrayView(
	ULearningAgentsObservationObject* Object,
	...
	const FLinearColor VisualLoggerColor)
{
	if (!Object)
	{
		UE_LOG(LogLearning, Error, TEXT("MakeNamedInclusiveDiscreteObservationFromArrayView: Object is nullptr."));
		return FLearningAgentsObservationObjectElement();
	}

	if (UE::Learning::Agents::Observation::Private::ContainsDuplicates(ElementNames))
	{
		UE_LOG(LogLearning, Error, TEXT("MakeNamedInclusiveDiscreteObservationFromArrayView: Names contain duplicates."));
		return FLearningAgentsObservationObjectElement();
	}
	...
	if (bVisualLoggerEnabled && VisualLoggerListener)
	{
		...
		UE_LEARNING_AGENTS_VLOG_STRING(VisualLoggerObject, LogLearning, Display, VisualLoggerLocation,
			...
			*Tag.ToString(),
			VisualLoggerAgentId,
			*UE::Learning::Agents::Observation::Private::FormatNames(ElementNames, 256));
	}
#endif

	return { Object->ObservationObject.CreateNamedDiscreteInclusive({ ElementNames }, Tag) };
}
...
bool ULearningAgentsObservations::GetExclusiveDiscreteObservation(int32& OutIndex, const ULearningAgentsObservationObject* Object, const FLearningAgentsObservationObjectElement Element, const FName Tag)
{
	if (!Object)
	{
		UE_LOG(LogLearning, Error, TEXT("GetExclusiveDiscreteObservation: Object is nullptr."));
		OutIndex = 0;
		return false;
	}
	...
	if (!Object->ObservationObject.IsValid(Element.ObjectElement))
	{
		UE_LOG(LogLearning, Error, TEXT("GetExclusiveDiscreteObservation: Invalid Observation Object."));
		OutIndex = 0;
		return false;
	}
	...
	if (Object->ObservationObject.GetTag(Element.ObjectElement) != Tag)
	{
		UE_LOG(LogLearning, Warning, TEXT("GetExclusiveDiscreteObservation: Observation tag does not match. Observation is '%s' but asked for '%s'."), *Object->ObservationObject.GetTag(Element.ObjectElement).ToString(), *Tag.ToString());
	}

	if (Object->ObservationObject.GetType(Element.ObjectElement) != UE::Learning::Observation::EType::DiscreteExclusive)
	{
		UE_LOG(LogLearning, Error, TEXT("GetExclusiveDiscreteObservation: Observation '%s' type does not match. Observation is '%s' but asked for '%s'."),
			*Object->ObservationObject.GetTag(Element.ObjectElement).ToString(),
			UE::Learning::Agents::Observation::Private::GetObservationTypeString(Object->ObservationObject.GetType(Element.ObjectElement)),
			UE::Learning::Agents::Observation::Private::GetObservationTypeString(UE::Learning::Observation::EType::DiscreteExclusive));
		OutIndex = 0;
		return false;
	}

	OutIndex = Object->ObservationObject.GetDiscreteExclusive(Element.ObjectElement).DiscreteIndex;
	return true;
}
...
bool ULearningAgentsObservations::GetInclusiveDiscreteObservationNum(int32& OutNum, const ULearningAgentsObservationObject* Object, const FLearningAgentsObservationObjectElement Element, const FName Tag)
{
	if (!Object)
	{
		UE_LOG(LogLearning, Error, TEXT("GetInclusiveDiscreteObservationNum: Object is nullptr."));
		OutNum = 0;
		return false;
	}
	...
	if (!Object->ObservationObject.IsValid(Element.ObjectElement))
	{
		UE_LOG(LogLearning, Error, TEXT("GetInclusiveDiscreteObservationNum: Invalid Observation Object."));
		OutNum = 0;
		return false;
	}
	...
	if (Object->ObservationObject.GetTag(Element.ObjectElement) != Tag)
	{
		UE_LOG(LogLearning, Warning, TEXT("GetInclusiveDiscreteObservationNum: Observation tag does not match. Observation is '%s' but asked for '%s'."), *Object->ObservationObject.GetTag(Element.ObjectElement).ToString(), *Tag.ToString());
	}

	if (Object->ObservationObject.GetType(Element.ObjectElement) != UE::Learning::Observation::EType::DiscreteInclusive)
	{
		UE_LOG(LogLearning, Error, TEXT("GetInclusiveDiscreteObservationNum: Observation '%s' type does not match. Observation is '%s' but asked for '%s'."),
			*Object->ObservationObject.GetTag(Element.ObjectElement).ToString(),
			UE::Learning::Agents::Observation::Private::GetObservationTypeString(Object->ObservationObject.GetType(Element.ObjectElement)),
			UE::Learning::Agents::Observation::Private::GetObservationTypeString(UE::Learning::Observation::EType::DiscreteInclusive));
		OutNum = 0;
		return false;
	}

	OutNum = Object->ObservationObject.GetDiscreteInclusive(Element.ObjectElement).DiscreteIndices.Num();
	return true;
}
...
bool ULearningAgentsObservations::GetInclusiveDiscreteObservationToArrayView(TArrayView<int32> OutIndices, const ULearningAgentsObservationObject* Object, const FLearningAgentsObservationObjectElement Element, const FName Tag)
{
	...
	if (!Object->ObservationObject.IsValid(Element.ObjectElement))
	{
		UE_LOG(LogLearning, Error, TEXT("GetInclusiveDiscreteObservationToArrayView: Invalid Observation Object."));
		UE::Learning::Array::Zero<1, int32>(OutIndices);
		return false;
	}
	...
	if (Object->ObservationObject.GetTag(Element.ObjectElement) != Tag)
	{
		UE_LOG(LogLearning, Warning, TEXT("GetInclusiveDiscreteObservationToArrayView: Observation tag does not match. Observation is '%s' but asked for '%s'."), *Object->ObservationObject.GetTag(Element.ObjectElement).ToString(), *Tag.ToString());
	}

	if (Object->ObservationObject.GetType(Element.ObjectElement) != UE::Learning::Observation::EType::DiscreteInclusive)
	{
		UE_LOG(LogLearning, Error, TEXT("GetInclusiveDiscreteObservationToArrayView: Observation '%s' type does not match. Observation is '%s' but asked for '%s'."),
			*Object->ObservationObject.GetTag(Element.ObjectElement).ToString(),
			UE::Learning::Agents::Observation::Private::GetObservationTypeString(Object->ObservationObject.GetType(Element.ObjectElement)),
			UE::Learning::Agents::Observation::Private::GetObservationTypeString(UE::Learning::Observation::EType::DiscreteInclusive));
		UE::Learning::Array::Zero<1, int32>(OutIndices);
	...
	const TArrayView<const int32> Indices = Object->ObservationObject.GetDiscreteInclusive(Element.ObjectElement).DiscreteIndices;

	if (Indices.Num() != OutIndices.Num())
	{
		UE_LOG(LogLearning, Error, TEXT("GetInclusiveDiscreteObservationToArrayView: Observation '%s' size does not match. Observation is '%i' values but asked for '%i'."),
			*Object->ObservationObject.GetTag(Element.ObjectElement).ToString(),
...
bool ULearningAgentsObservations::GetNamedExclusiveDiscreteObservation(FName& OutName, const ULearningAgentsObservationObject* Object, const FLearningAgentsObservationObjectElement Element, const FName Tag)
{
	if (!Object)
	{
		UE_LOG(LogLearning, Error, TEXT("GetNamedExclusiveDiscreteObservation: Object is nullptr."));
		OutName = NAME_None;
		return false;
	}
	...
	if (!Object->ObservationObject.IsValid(Element.ObjectElement))
	{
		UE_LOG(LogLearning, Error, TEXT("GetNamedExclusiveDiscreteObservation: Invalid Observation Object."));
		OutName = NAME_None;
		return false;
	}
	...
	if (Object->ObservationObject.GetTag(Element.ObjectElement) != Tag)
	{
		UE_LOG(LogLearning, Warning, TEXT("GetNamedExclusiveDiscreteObservation: Observation tag does not match. Observation is '%s' but asked for '%s'."), *Object->ObservationObject.GetTag(Element.ObjectElement).ToString(), *Tag.ToString());
	}

	if (Object->ObservationObject.GetType(Element.ObjectElement) != UE::Learning::Observation::EType::NamedDiscreteExclusive)
	{
		UE_LOG(LogLearning, Error, TEXT("GetNamedExclusiveDiscreteObservation: Observation '%s' type does not match. Observation is '%s' but asked for '%s'."),
			*Object->ObservationObject.GetTag(Element.ObjectElement).ToString(),
			UE::Learning::Agents::Observation::Private::GetObservationTypeString(Object->ObservationObject.GetType(Element.ObjectElement)),
			UE::Learning::Agents::Observation::Private::GetObservationTypeString(UE::Learning::Observation::EType::NamedDiscreteExclusive));
		OutName = NAME_None;
		return false;
	}

	OutName = Object->ObservationObject.GetNamedDiscreteExclusive(Element.ObjectElement).ElementName;
	return true;
}

bool ULearningAgentsObservations::GetNamedInclusiveDiscreteObservationNum(int32& OutNum, const ULearningAgentsObservationObject* Object, const FLearningAgentsObservationObjectElement Element, const FName Tag)
{
	if (!Object)
	{
		UE_LOG(LogLearning, Error, TEXT("GetNamedInclusiveDiscreteObservationNum: Object is nullptr."));
		OutNum = 0;
		return false;
	}
	...
	if (!Object->ObservationObject.IsValid(Element.ObjectElement))
	{
		UE_LOG(LogLearning, Error, TEXT("GetNamedInclusiveDiscreteObservationNum: Invalid Observation Object."));
		OutNum = 0;
		return false;
	}
	...
	if (Object->ObservationObject.GetTag(Element.ObjectElement) != Tag)
	{
		UE_LOG(LogLearning, Warning, TEXT("GetNamedInclusiveDiscreteObservationNum: Observation tag does not match. Observation is '%s' but asked for '%s'."), *Object->ObservationObject.GetTag(Element.ObjectElement).ToString(), *Tag.ToString());
	}

	if (Object->ObservationObject.GetType(Element.ObjectElement) != UE::Learning::Observation::EType::NamedDiscreteInclusive)
	{
		UE_LOG(LogLearning, Error, TEXT("GetNamedInclusiveDiscreteObservationNum: Observation '%s' type does not match. Observation is '%s' but asked for '%s'."),
			*Object->ObservationObject.GetTag(Element.ObjectElement).ToString(),
			UE::Learning::Agents::Observation::Private::GetObservationTypeString(Object->ObservationObject.GetType(Element.ObjectElement)),
			UE::Learning::Agents::Observation::Private::GetObservationTypeString(UE::Learning::Observation::EType::NamedDiscreteInclusive));
		OutNum = 0;
		return false;
	}

	OutNum = Object->ObservationObject.GetNamedDiscreteInclusive(Element.ObjectElement).ElementNames.Num();
	return true;
}

bool ULearningAgentsObservations::GetNamedInclusiveDiscreteObservation(TArray<FName>& OutNames, const ULearningAgentsObservationObject* Object, const FLearningAgentsObservationObjectElement Element, const FName Tag)
{
	int32 OutNamesNum;
	if (!GetNamedInclusiveDiscreteObservationNum(OutNamesNum, Object, Element, Tag))
	{
		OutNames.Empty();
		return false;
	}

	OutNames.SetNumUninitialized(OutNamesNum);
	return GetNamedInclusiveDiscreteObservationToArrayView(OutNames, Object, Element, Tag);
}

bool ULearningAgentsObservations::GetNamedInclusiveDiscreteObservationToArrayView(TArrayView<FName> OutNames, const ULearningAgentsObservationObject* Object, const FLearningAgentsObservationObjectElement Element, const FName Tag)
{
	if (!Object)
	{
		UE_LOG(LogLearning, Error, TEXT("GetNamedInclusiveDiscreteObservationToArrayView: Object is nullptr."));
	...
	if (!Object->ObservationObject.IsValid(Element.ObjectElement))
	{
		UE_LOG(LogLearning, Error, TEXT("GetNamedInclusiveDiscreteObservationToArrayView: Invalid Observation Object."));
		UE::Learning::Array::Set<1, FName>(OutNames, NAME_None);
		return false;
	}
	...
	if (Object->ObservationObject.GetTag(Element.ObjectElement) != Tag)
	{
		UE_LOG(LogLearning, Warning, TEXT("GetNamedInclusiveDiscreteObservationToArrayView: Observation tag does not match. Observation is '%s' but asked for '%s'."), *Object->ObservationObject.GetTag(Element.ObjectElement).ToString(), *Tag.ToString());
	}

	if (Object->ObservationObject.GetType(Element.ObjectElement) != UE::Learning::Observation::EType::NamedDiscreteInclusive)
	{
		UE_LOG(LogLearning, Error, TEXT("GetNamedInclusiveDiscreteObservationToArrayView: Observation '%s' type does not match. Observation is '%s' but asked for '%s'."),
			*Object->ObservationObject.GetTag(Element.ObjectElement).ToString(),
			UE::Learning::Agents::Observation::Private::GetObservationTypeString(Object->ObservationObject.GetType(Element.ObjectElement)),
			UE::Learning::Agents::Observation::Private::GetObservationTypeString(UE::Learning::Observation::EType::NamedDiscreteInclusive));
		UE::Learning::Array::Set<1, FName>(OutNames, NAME_None);
	...
	const TArrayView<const FName> Names = Object->ObservationObject.GetNamedDiscreteInclusive(Element.ObjectElement).ElementNames;

	if (Names.Num() != OutNames.Num())
	{
		UE_LOG(LogLearning, Error, TEXT("GetNamedInclusiveDiscreteObservationToArrayView: Observation '%s' size does not match. Observation is '%i' values but asked for '%i'."),
			*Object->ObservationObject.GetTag(Element.ObjectElement).ToString(),
