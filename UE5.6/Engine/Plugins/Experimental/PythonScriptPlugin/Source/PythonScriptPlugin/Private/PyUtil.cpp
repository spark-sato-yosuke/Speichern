...
const TArray<FString>& GetSitePackageSubdirs()
{
	const static TArray<FString> CheckSitePackageDirs = {
		FPaths::Combine(TEXT("Lib"), FPlatformMisc::GetUBTPlatform(), PyUtil::GetArchString(), TEXT("site-packages")),
...
void CollectGarbage(int32 Generations)
{
	...
	else
	{
		...
		if (FPyObjectPtr PyGCModule = FPyObjectPtr::StealReference(PyImport_ImportModule("gc")))
		{
			PyObject* PyGCDict = PyModule_GetDict(PyGCModule);

			if (PyObject* PyCollectFunc = PyDict_GetItemString(PyGCDict, "collect"))
			{
				FPyObjectPtr PyCollectResult = FPyObjectPtr::StealReference(PyObject_CallFunction(PyCollectFunc, PyCStrCast("i"), Generations));
				// PyCollectResult is unused, but the FPyObjectPtr handles its ref-count
			}
		}
	}
}
