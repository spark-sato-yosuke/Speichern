...
public:
    ...
    UFUNCTION(BlueprintCallable, Category=Mover)
    static UE_API bool IsValidLandingSpot(const FMovingComponentSet& MovingComps, const FVector& Location, const FHitResult& Hit, float FloorSweepDistance, float MaxWalkSlopeCosine, UPARAM(Ref) FFloorCheckResult& OutFloorResult);
...
public:	
	 ...
	 */
	UFUNCTION(BlueprintCallable, Category = Mover)
	static UE_API float TestFallingMoveAlongHitSurface(const FMovingComponentSet& MovingComps, const FVector& OriginalMoveDelta, const FVector& LocationAtHit, const FQuat& TargetRotation, bool bHandleImpact, float FloorSweepDistance, float MaxWalkSlopeCosine, UPARAM(Ref) FHitResult& InOutHit, UPARAM(Ref) FFloorCheckResult& OutFloorResult, UPARAM(Ref) FMovementRecord& InOutMoveRecord);
