...
{
public:
	explicit FText3DGlyphLoader(const FT_GlyphSlot InGlyph);

	TText3DGlyphContourNodeShared GetContourList() const;
...
private:
	...
	TText3DGlyphContourNodeShared Root;
	int32 EndIndex;
	TDoubleLinkedList<FVector2f> ProcessedContour;
	FVector2D FirstPosition;
	TSharedPtr<struct FText3DGlyphArrangement> Arrangement;
	 ...
	 * @param NodeB
	 */
	void Insert(const TText3DGlyphContourNodeShared NodeA, const TText3DGlyphContourNodeShared NodeB) const;
	/**
	 ...
	 */
	void FixParityAndDivide(const TText3DGlyphContourNodeShared Node, const bool bClockwiseIn);
	 ...
	 */
	void DetachFinishedContour(const int32 RepeatedJunctionIndex, const TArray<FVector2f>& InitialVertices, const TText3DGlyphContourNodeShared RootForDetaching);
	 ...
	 */
	void RemoveUnneededNodes(const TText3DGlyphContourNodeShared Node) const;
	/**
	 ...
	 */
	void MergeRootForDetaching(const TText3DGlyphContourNodeShared RemovedNode, const TText3DGlyphContourNodeShared RootForDetaching, const TText3DGlyphContourNodeShared RemovedNodeParent) const;
	...
	int32 GetOtherEdgeVertex(const int32 EID, const int32 VID) const;
