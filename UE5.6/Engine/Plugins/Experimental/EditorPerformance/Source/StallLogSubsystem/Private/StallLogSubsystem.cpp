...
namespace
{
	...
	class SStallLogStatusBarWidget : public SCompoundWidget
	{
		SLATE_BEGIN_ARGS(SStallLogStatusBarWidget) {}
			...
			SLATE_ARGUMENT(FGetSlateBrush, GetFilterBadgeNotificationIcon)
		SLATE_END_ARGS()
	...
	private:
		...
		const FSlateBrush*			GetBadgeNotificationIcon() const;

		FGetSlateBrush GetFilterBadgeIconDelegate;
		FGetSlateBrush GetFilterBadgeNotificationIconDelegate;
	};
	...
	const FSlateBrush* SStallLogStatusBarWidget::GetBadgeNotificationIcon() const
	{
		if (GetFilterBadgeNotificationIconDelegate.IsBound())
		{
			return GetFilterBadgeNotificationIconDelegate.Execute();
		}
		return nullptr;
	}
	...
	void SStallLogStatusBarWidget::Construct(const FArguments& InArgs)
	{
		...
		GetFilterBadgeNotificationIconDelegate = InArgs._GetFilterBadgeNotificationIcon;
		this->ChildSlot
		[
			SNew(SButton)
			...
			[
				...
				[
					...
					[
						SNew(SImage)
							.Image_Lambda([this] { return GetBadgeNotificationIcon();  })
					]
				]
			]
			.OnClicked_Lambda([]()
...
void FStallLogHistory::AddStallToLog(FStallLogItemPtr LogItemToAdd)
{
	...
	// If the tab is open, we want to automatically say we viewed any new stalls
	if (!FGlobalTabmanager::Get()->FindExistingLiveTab(FTabId(StallLogTabName)).IsValid())
	{
...
void UStallLogSubsystem::Initialize(FSubsystemCollectionBase& Collection)
{
	...
	{
		const FSlateIcon StallLogIcon(FAppStyle::GetAppStyleSetName(), "EditorPerformance.Stall");
	...
	FToolMenuSection& StallDetectorSection = Menu->AddSection("StallLog", FText::GetEmpty());
	...
	auto CreateStallLogWidget = [this]()
	{
		return SNew(SStallLogStatusBarWidget)
			...
			.GetFilterBadgeNotificationIcon(FGetSlateBrush::CreateUObject(this, &UStallLogSubsystem::GetStatusBarBadgeNotificationIcon));
	};
...
{
	return FAppStyle::GetBrush(TEXT("EditorPerformance.Stall"));
}

const FSlateBrush* UStallLogSubsystem::GetStatusBarBadgeNotificationIcon() const
{
	// If the StallLog has any unviewed stalls, display the notification overlay
	if (!StallLogHistory->GetHasViewedStalls())
	{
		return FAppStyle::GetBrush(TEXT("Icons.BadgeModified"));
	}
	return nullptr;
}
