...
)
{
	...
	{
		UE_LOG(LogApproximateActors, Log, TEXT("very large voxel size %d clamped to %d"), VoxelDimTarget, Options.ClampVoxelDimension);
	...
	// if solid mesh has no triangles, something has gone wrong
	if (SolidMesh.TriangleCount() == 0)
	{
		UE_LOG(LogApproximateActors, Warning, TEXT("Solidify mesh has no triangles - unable to generate mesh"));
	...
	FDynamicMesh3* DebugMesh)
{
	...
	MeshAssetOptions.bDeferPostEditChange = true; // Defer static mesh build to control options
	...
	BuildParameters.bIgnoreBoundsDiff = Options.bGenerateNaniteEnabledMesh; 

	MeshAssetOutputs.StaticMesh->Build(BuildParameters);
