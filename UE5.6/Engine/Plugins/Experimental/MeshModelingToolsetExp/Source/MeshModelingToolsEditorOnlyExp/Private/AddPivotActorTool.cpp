...
void UAddPivotActorToolBuilder::InitializeNewTool(UMultiSelectionMeshEditingTool* NewTool, const FToolBuilderState& SceneState) const
{
	const FToolTargetTypeRequirements& Requirements = GetTargetRequirements();
	TArray<TObjectPtr<UToolTarget>> Targets;
	if (SceneState.SelectedComponents.Num() > 0)
	{
		Targets = SceneState.TargetManager->BuildAllSelectedTargetable(SceneState, Requirements);
	}
	else
	{
		// When dealing with actor selection, only consider root components.
		SceneState.TargetManager->EnumerateSelectedAndTargetableComponents(SceneState, Requirements, [this, &SceneState, &Targets, &Requirements](UActorComponent* Component)
		{
			if (AActor* OwnerActor = Component->GetOwner())
			{
				if (OwnerActor->GetRootComponent() == Component)
				{
					Targets.Add(SceneState.TargetManager->BuildTarget(Component, Requirements));
				}
			}
		});
	}
