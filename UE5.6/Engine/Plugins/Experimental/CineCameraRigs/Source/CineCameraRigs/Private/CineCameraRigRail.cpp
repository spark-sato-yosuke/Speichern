...
void ACineCameraRigRail::DriveByParam(float DeltaTime)
{
	...
	const float Increment = PlayForward() ? AdjustedDeltaTime : -AdjustedDeltaTime;
	...
	// Reverse direction in Bounce mode when Param goes outside of the range 
	if (LoopMode == ECineCameraRigRailLoopMode::Bounce)
	{
		if (Param < StartPositionValue() || Param > LastPositionValue())
		{
			bBounceForward = !bBounceForward;
		}
	}

	// Mod or Clamp the Param variable so that it's within the range
	if (LoopMode == ECineCameraRigRailLoopMode::Loop)
	{
...
void ACineCameraRigRail::DriveBySpeed(float DeltaTime)
{
	...
	// Reverse direction in Bounce mode when CurrenTime goes outside of the range
	if (LoopMode == ECineCameraRigRailLoopMode::Bounce)
	{
		if (CurrentTime < 0.0f || CurrentTime > TotalTime)
		{
			bBounceForward = !bBounceForward;
		}
	}

	// Mod or Clamp the CurrentTime variable so that it's within the range
	CurrentTime = LoopMode == ECineCameraRigRailLoopMode::Loop ? FMath::Fmod(CurrentTime + TotalTime, TotalTime) : FMath::Clamp(CurrentTime, 0.0f, TotalTime);
...
bool ACineCameraRigRail::PlayForward() const
{
	if (LoopMode == ECineCameraRigRailLoopMode::Bounce)
	{
		return bBounceForward;
	}
	return !bReverse;
}
