...
public:
	UE_API UTargetingSelectionTask_AOE(const FObjectInitializer& ObjectInitializer);
	...
	UE_API void DebugDrawBoundingVolume(const FTargetingRequestHandle& TargetingHandle, const FColor& Color, const FOverlapDatum* OverlapDatum = nullptr) const;

	UE_API void SetShapeType(ETargetingAOEShape InShapeType);
	UE_API void SetHalfExtent(FVector InHalfExtent);
	UE_API void SetRadius(FScalableFloat InRadius);
	UE_API void SetHalfHeight(FScalableFloat InHalfHeight);
...
protected:
	...
	UE_API FVector GetSourceLocation(const FTargetingRequestHandle& TargetingHandle) const;

	/** Native Event to get a source location offset for the AOE */
	UFUNCTION(BlueprintNativeEvent, Category = "Target AOE Selection")
	UE_API FVector GetSourceOffset(const FTargetingRequestHandle& TargetingHandle) const;

	/** Native event to get the source rotation for the AOE  */
	UFUNCTION(BlueprintNativeEvent, Category = "Target AOE Selection")
	UE_API FQuat GetSourceRotation(const FTargetingRequestHandle& TargetingHandle) const;
