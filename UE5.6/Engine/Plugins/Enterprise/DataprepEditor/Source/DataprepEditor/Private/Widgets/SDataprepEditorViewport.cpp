...
void SDataprepEditorViewport::ClearScene()
{
	...
	{
		ensure(PreviewMeshComponents.IsEmpty() && MeshComponentsMapping.IsEmpty() && MeshComponentsReverseMapping.IsEmpty() && SelectedPreviewComponents.IsEmpty());
		return;
	}

	UWorld* PreviewSceneWorld = PreviewScene->GetWorld();

	TFunction<void(AActor*)> DeleteActor = [PreviewSceneWorld](AActor* ActorToDelete)
		{
			if (IsValid(ActorToDelete))
			{
				PreviewSceneWorld->EditorDestroyActor(ActorToDelete, true);
				...
				ActorToDelete->UObject::Rename(nullptr, GetTransientPackage(), REN_DontCreateRedirectors);
			}
		};

	DeleteActor(StaticActor.Get());
	DeleteActor(MovableActor.Get());
			...
			SlowTask.EnterProgressFrame( 90.0f, LOCTEXT("UpdateMeshes_Components", "Adding meshes to viewport ...") );
			{
				...
				for( UStaticMeshComponent* SceneMeshComponent : SceneMeshComponents )
				{
					if(SceneMeshComponent != nullptr)
					{
						...
						AActor* PreviewActor = Mobility == EComponentMobility::Movable && MovableActor.IsValid() ? MovableActor.Get() : StaticActor.Get();
	...
	if(DataprepEditorViewport != nullptr)
	{
		if(HitProxy != nullptr)
		{
			if( HitProxy->IsA( HActor::StaticGetType() ) )
			{
				...
				if( UStaticMeshComponent* Component = const_cast<UStaticMeshComponent*>(Cast<UStaticMeshComponent>( ((HActor*)HitProxy)->PrimComponent )))
				{
...
AActor* SDataprepEditorViewport::CreateActor(USceneComponent* ParentComponent, const TCHAR* ActorName)
{
	UWorld* PreviewSceneWorld = PreviewScene->GetWorld();
...
FPrimitiveSceneProxy* UCustomStaticMeshComponent::CreateSceneProxy()
{
	...
	if (Mesh->HasValidNaniteData() && Mesh->HasNaniteFallbackMesh(ShaderPlatform))
	{
		UE_LOG(LogStaticMesh, Warning, TEXT("Unable to create a proxy for StaticMeshComponent [%s] because it doesn't have a fallback mesh."), *GetFullName());
		return nullptr;
	}

	const FStaticMeshLODResourcesArray& LODResources = Mesh->GetRenderData()->LODResources;
	if (LODResources.Num() == 0	|| LODResources[FMath::Clamp<int32>(Mesh->GetDefaultMinLOD(), 0, LODResources.Num()-1)].VertexBuffers.StaticMeshVertexBuffer.GetNumVertices() == 0)
	{
