	...
	void WriteSnapshotData(UObject* Object, FSnapshotBuffer& OutSerializedData)
	{
		...
		// Sort array of sub-objects based on their inter-dependency
		{
			...
			for(UObject* SubObject : SubObjectsArray)
			{
				...
				if (!Analyzer.DependentObjects.IsEmpty())
				{
					TSet<UObject*>& SubObjectEntry = SubObjectDependencyGraph.FindOrAdd(SubObject);
					for (UObject* DependentObject : Analyzer.DependentObjects)
					{
						// Skip cyclical dependency
						if (!SubObjectDependencyGraph.Contains(DependentObject))
						{
							SubObjectEntry.Add(DependentObject);
						}
					}
				}
			}
			...
			while(Count != SubObjectsArray.Num())
			{
				for(auto& Entry : SubObjectDependencyGraph)
				{
					if(Entry.Value.Num() == 0)
					{
						...
						bool bNoEndlessLoop = SubObjectDependencyGraph.IsEmpty();
						for(auto& SubEntry : SubObjectDependencyGraph)
						{
							...
							bNoEndlessLoop |= SubEntry.Value.Num() == 0;
						}

						ensure(bNoEndlessLoop);
		...
		for (int32 Index = 0; Index < SubObjectsArray.Num(); ++Index)
		{
