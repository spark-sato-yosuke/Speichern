...
bool FDatasmithCADTranslator::LoadScene(TSharedRef<IDatasmithScene> DatasmithScene)
{
	...
	{
		// todo: copied from FDatasmithDispatcher::LogMessages
		for (const TPair<uint8, FString>& Message : FileReader.GetCADFileData().GetMessages())
		{
			switch (Message.Key)
			{
			case ELogVerbosity::Fatal:
				UE_LOG(LogCADTranslator, Fatal, TEXT("%s"), *Message.Value);
				break;
			case ELogVerbosity::Error:
				UE_LOG(LogCADTranslator, Error, TEXT("%s"), *Message.Value);
				break;
			case ELogVerbosity::Warning:
				UE_LOG(LogCADTranslator, Warning, TEXT("%s"), *Message.Value);
				break;
			case ELogVerbosity::Display:
				UE_LOG(LogCADTranslator, Display, TEXT("%s"), *Message.Value);
				break;
			case ELogVerbosity::Log:
				UE_LOG(LogCADTranslator, Log, TEXT("%s"), *Message.Value);
				break;
			case ELogVerbosity::Verbose:
				UE_LOG(LogCADTranslator, Verbose, TEXT("%s"), *Message.Value);
				break;
			case ELogVerbosity::VeryVerbose:
			default:
				UE_LOG(LogCADTranslator, VeryVerbose, TEXT("%s"), *Message.Value);
				break;
			}
		}
