...
const TArray<FName> UEdGraphSchema_CustomizableObject::SupportedMacroPinTypes = { PC_Object, PC_Component, PC_Material, PC_Modifier, PC_Mesh, PC_Image,
PC_PassThroughImage, PC_Projector, PC_GroupProjector, PC_Color, PC_Float, PC_Enum, PC_Stack, PC_MaterialAsset, PC_PoseAsset, PC_Transform, PC_String };
...
void UEdGraphSchema_CustomizableObject::GetGraphContextActions(FGraphContextMenuBuilder& ContextMenuBuilder) const
{
	...
	
	{
		UCustomizableObjectNode* ExperimentalTemplateNodes[] =
		{
			...
			ContextMenuBuilder.CreateTemplateNode<UCustomizableObjectNodeMeshParameter>(),
		};
...
FLinearColor UEdGraphSchema_CustomizableObject::GetPinTypeColor(const FName& TypeString)
{
	...
	{
		return FLinearColor(0.004f, 0.42f, 0.384f, 1.000000f); // Light green
	}
	...
	{
		return FLinearColor(FColorList::Aquamarine);
	}
	else if (TypeString == PC_GroupProjector)
	{
		return FLinearColor(FColorList::DarkTan);
	}
	...
	else if (TypeString == PC_Image || TypeString == PC_PassThroughImage)
	{
		return FLinearColor(0.75294f, 0.08098f, 0.08098f, 1.000000f); // Soft Red
	}
	...
	{
		return FLinearColor(FColorList::LightGrey); 
	}
	...
	{
		return FLinearColor(FColorList::DarkOrchid); 
	}
	...
	{
		return FLinearColor(FColorList::Orange); 
	}
	else if (TypeString == PC_String)
	{
		return FLinearColor(0.700000f, 0.010000f, 0.660000f, 1.000000f); // Dark Pink
	}
...
{
	UObject* Object = MutablePrivate::LoadObject(InAsset);
...
void UEdGraphSchema_CustomizableObject::DroppedAssetsOnGraph(const TArray<FAssetData>& Assets, const FVector2f& GraphPosition, UEdGraph* Graph) const
{
	...
	for (FAssetData Asset : Assets)
	{
		...
		UObject* Object = MutablePrivate::LoadObject(Asset);
...
FName UEdGraphSchema_CustomizableObject::GetPinCategoryName(const FName& PinCategory)
{
	...
	else if (PinCategory == UEdGraphSchema_CustomizableObject::PC_String)
	{
		return TEXT("String");
	}
	else
