...
void UCustomizableObjectNodeCopyMaterial::AllocateDefaultPins(UCustomizableObjectNodeRemapPins* RemapPins)
{
	...
	Pin = CustomCreatePin(EGPD_Input, Schema->PC_Material, FName(CustomizableObjectNodeCopyMaterial::MeshSectionPinName));
	Pin->PinFriendlyName = LOCTEXT("BaseMeshSectionPin", "Base Mesh Section");;
	Pin->bDefaultValueIsIgnored = true;

	// Output pins
	Pin = CustomCreatePin(EGPD_Output, Schema->PC_Material, FName(CustomizableObjectNodeCopyMaterial::OutputPinName));
