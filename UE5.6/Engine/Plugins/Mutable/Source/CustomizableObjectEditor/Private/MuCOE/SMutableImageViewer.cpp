...
void SMutableImageViewer::Tick(const FGeometry& AllottedGeometry, const double InCurrentTime, const float InDeltaTime)
{
	...
	if (bIsPendingUpdate)
	{
		...
		if (MutableImage && MutableImage->GetSizeX()>0 && MutableImage->GetSizeY()>0)
		{
			...
			TSharedPtr<const mu::FImage> ImageToConvert = MutableImage;

			// \TODO: This test is a bit weak
			if (MutableImage->GetFormat() >= mu::EImageFormat::ASTC_4x4_RGB_LDR
				&&
				MutableImage->GetFormat() <= mu::EImageFormat::ASTC_10x10_RG_LDR)
			{
...
void SMutableImageViewer::SetImage(const TSharedPtr<const mu::FImage>& InMutableImage, int32 CurrentLOD)
{
	MutableImage = InMutableImage;
	CurrentVisibleLOD = CurrentLOD;
	bIsPendingUpdate = true;

	constexpr bool bDebugDump = false;
	if (bDebugDump && InMutableImage)
	{
		static int32 SImageCount = 0;
		FString ImagePath = FPaths::ConvertRelativePathToFull(FPaths::Combine(FPaths::ProjectSavedDir(), FString::Printf( TEXT("MutableDebug_%d.astc"), SImageCount)));
		++SImageCount;

		DebugImageDump( InMutableImage.Get(), ImagePath);
	}
}
