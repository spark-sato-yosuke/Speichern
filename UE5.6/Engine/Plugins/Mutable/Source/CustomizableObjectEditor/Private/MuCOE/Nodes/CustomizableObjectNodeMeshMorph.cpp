...
bool UCustomizableObjectNodeMeshMorph::CreateStaticStringPinWidget() const
{
	return false;
}
...
void UCustomizableObjectNodeMeshMorph::BackwardsCompatibleFixup(int32 CustomizableObjectCustomVersion)
{
	...
	if (CustomizableObjectCustomVersion == FCustomizableObjectCustomVersion::EnableMutableMacrosNewVersion)
	{
		...
		{
			FString PinName = TEXT("Morph Target Name");
			MorphTargetNamePinRef = CustomCreatePin(EGPD_Input, UEdGraphSchema_CustomizableObject::PC_String, FName(*PinName));
		}
	}
}
