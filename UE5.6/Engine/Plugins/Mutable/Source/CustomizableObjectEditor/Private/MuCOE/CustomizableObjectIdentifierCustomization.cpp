...
void FCustomizableObjectIdentifierCustomization::CustomizeHeader(TSharedRef<class IPropertyHandle> StructPropertyHandle, class FDetailWidgetRow& HeaderRow, IPropertyTypeCustomizationUtils& StructCustomizationUtils)
{
	...
	if (Guid.IsValid())
	{
		...
		{
			UCustomizableObject* CustomizableObject = Cast<UCustomizableObject>(MutablePrivate::LoadObject(*Itr));

			if (CustomizableObject)
			{
				UCustomizableObjectNodeObject* RootNode = GetRootNode(CustomizableObject);

				if (RootNode && RootNode->Identifier == Guid)
				{
...
{
	SelectedCustomizableObject = Cast<UCustomizableObject>(MutablePrivate::LoadObject(InAssetData));

	if (SelectedCustomizableObject)
	{
		if (UCustomizableObjectNodeObject* RootNode = GetRootNode(SelectedCustomizableObject))
		{
