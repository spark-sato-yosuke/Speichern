...
namespace mu
{
	...
	//!
	//---------------------------------------------------------------------------------------------
	inline void ImageMakeGrowMap(FImage* ResultImage, const FImage* MaskImage, int32 InBorder)
	{
		...
		TSharedPtr<const FImage> ThisMaskCopy;
		const FImage* ThisMask = MaskImage;
		for (int32 B = 0; B < InBorder; ++B)
		{
			TSharedPtr<FImage> NextMask = ThisMask->Clone();
			...
			ThisMaskCopy = NextMask;
			ThisMask = ThisMaskCopy.Get();
		}
