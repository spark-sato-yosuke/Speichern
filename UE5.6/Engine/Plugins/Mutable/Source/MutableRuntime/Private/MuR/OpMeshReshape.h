...
namespace mu
{
	...
	inline void ApplyToVertices(FMesh* Mesh, TArrayView<const FReshapeVertexBindingData> BindingData, const FShapeMeshDescriptorApply& Shape, bool bSkipNormalReshape)
	{
		...
		const UntypedMeshBufferIterator PositionIter(Mesh->GetVertexBuffers(), EMeshBufferSemantic::Position);
		...
		for (int32 VertexIndex = 0; VertexIndex < MeshVertexCount; ++VertexIndex)
		{
			...
			if (bModified)
			{
                ...
                FVector3f PositionDisplacement;
                if ((OldPosition - Binding.AttachmentPoint).IsNearlyZero())
                {
				    PositionDisplacement = (NewPosition - OldPosition) * Binding.Weight;
				}
                else
                {
				    PositionDisplacement = (NewPosition - Binding.AttachmentPoint) * Binding.Weight;
                }
