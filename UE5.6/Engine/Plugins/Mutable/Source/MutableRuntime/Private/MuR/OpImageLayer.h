...
namespace mu
{
	...
	*/
    ...
    inline void BufferLayerColour(FImage* ResultImage, const FImage* BaseImage, FVector4f Color)
	{
	...
	template<
		...
		bool CLAMP>
    inline void BufferLayerColourFromAlpha(FImage* ResultImage, const FImage* BaseImage, FVector4f Color)
	{
        check(ResultImage->GetFormat() == BaseImage->GetFormat());
        check(ResultImage->GetSizeX() == BaseImage->GetSizeX());
        check(ResultImage->GetSizeY() == BaseImage->GetSizeY());
    ...
    inline void BufferLayerColourFormat(FImage* DestImage, const FImage* BaseImage, const FImage* MaskImage, FVector4f Col, uint32 BaseChannelOffset, uint8 ColorChannelOffset, bool bOnlyFirstLOD)
	{
        ...
        if (bIsMaskUncompressed)
        {
			...
			](int32 BatchId)
			{
				...
				if (!BaseBatchView.GetData() || !MaskBuf || !DestBatchView.GetData())
				{
					return;
				}
	...
	*/
	template<
		...
		uint32 (*BLEND_FUNC)(uint32,uint32),
		bool CLAMP>
    inline void BufferLayerColour(FImage* ResultImage, const FImage* BaseImage, const FImage* MaskImage, FVector4f Col)
	{
	...
	template< 
		uint32 (*BLEND_FUNC_MASKED)(uint32, uint32, uint32),
		uint32 (*BLEND_FUNC)(uint32, uint32)>
	inline void BufferLayerColour(FImage* DestImage, const FImage* BaseImage, const FImage* MaskImage, FVector4f Col)
	{
	...
	*/
	template< 
		...
		// Number of total channels in the base image
		int32 BASE_CHANNEL_STRIDE>
	inline void BufferLayerColourFormatInPlace(FImage* BaseImage, FVector4f Color, uint32 BaseChannelOffset, uint8 ColorChannelOffset, bool bOnlyFirstLOD)
	{
	...
	inline void BufferLayerColourInPlace(FImage* BaseImage, FVector4f Col, bool bOnlyOneMip, uint32 BaseOffset, uint8 ColOffset)
	{
		EImageFormat BaseFormat = BaseImage->GetFormat();
	...
	template< 
		uint32 (*BLEND_FUNC)(uint32, uint32), 
		uint32 CHANNEL_COUNT>
	inline void BufferLayerColourInPlace(FImage* BaseImage, FVector4f Color, bool bOnlyOneMip, uint32 BaseOffset, uint8 ColorOffset)
	{
		bool bIsClampNeeded = IsAnyComponentLargerThan1(Color);
		if (bIsClampNeeded)
		{
	...
	template< 
		...
		bool CLAMP, 
		int32 CHANNEL_COUNT>
	inline void BufferLayerColourInPlace(FImage* BaseImage, const FImage* MaskImage, FVector4f Color, bool bOnlyOneMip, uint32 BaseOffset, uint8 ColorOffset)
	{
		check(BaseImage->GetSizeX() == MaskImage->GetSizeX());
		check(BaseImage->GetSizeY() == MaskImage->GetSizeY());
	...
	*/
	...
	inline void BufferLayerFormat(FImage* DestImage, const FImage* BaseImage, const FImage* BlendedImage, bool bOnlyFirstLOD)
	{
		check(BaseImage->GetSizeX() == BlendedImage->GetSizeX());
		check(BaseImage->GetSizeY() == BlendedImage->GetSizeY());
	...
	*/
	template< uint32 (*BLEND_FUNC)(uint32, uint32), bool CLAMP >
	inline void BufferLayer(FImage* ResultImage, const FImage* BaseImage, const FImage* BlendedImage, bool bApplyToAlpha, bool bOnlyOneMip, bool bUseBlendSourceFromBlendAlpha)
	{
		check(ResultImage->GetFormat() == BaseImage->GetFormat());
		check(ResultImage->GetSizeX() == BaseImage->GetSizeX());
		check(ResultImage->GetSizeY() == BaseImage->GetSizeY());
	...
	inline void BufferLayerInPlace(FImage* BaseImage, const FImage* BlendedImage, bool bOnlyOneMip, uint32 BaseOffset, uint32 BlendedOffset)
	{
		check(BaseImage->GetSizeX() == BlendedImage->GetSizeX());
		check(BaseImage->GetSizeY() == BlendedImage->GetSizeY());
		...
		bool bOnlyFirstLOD)
	{
		...
		] (int32 BatchId)
		{
			...
			if (BlendImage->GetFormat() == EImageFormat::RGBA_UByte)
			{
	...
	template<uint32(*BLEND_FUNC_MASKED)(uint32, uint32, uint32),
			 uint32(*BLEND_FUNC)(uint32, uint32),
			 bool CLAMP>
    inline void BufferLayer(FImage* DestImage,
							const FImage* BaseImage,
							const FImage* MaskImage,
                            const FImage* BlendImage,
                            bool bApplyToAlpha,
	...
	template<uint32 (*BLEND_FUNC)(uint32,uint32),
			 bool CLAMP>
    inline void BufferLayerStrideNoAlpha(FImage* DestImage, int32 DestOffset, int32 Stride, const FImage* MaskImage, const FImage* BlendImage/*, int32 LODCount*/)
	{
	...
	*/
	...
	template< 
		uint32 (*BLEND_FUNC)(uint32, uint32), 
		uint32 NC>
	inline void BufferLayerCombineColour(FImage* ResultImage, const FImage* BaseImage, FVector4f Color, bool bOnlyFirstLOD = false)
	{
		static_assert(NC > 0 && NC <= 4);
	...
	template< 
		uint32 (*BLEND_FUNC)(uint32, uint32), 
		uint32 NC>
	inline void BufferLayerCombine(FImage* ResultImage, const FImage* BaseImage, const FImage* BlendImage, bool bOnlyFirstLOD)
	{
		static_assert(NC > 0 && NC <= 4);
	...
	template< 
		uint32 (*BLEND_FUNC_MASKED)(uint32, uint32, uint32), 
		uint32 NC>
	inline void BufferLayerCombine(FImage* ResultImage, const FImage* BaseImage, const FImage* MaskImage, const FImage* BlendImage, bool bOnlyFirstLOD)
	{
		static_assert(NC > 0 && NC <= 4);
	...
	template< 
		uint32 (*BLEND_FUNC_MASKED)(uint32, uint32, uint32), 
		uint32 NC>
	inline void BufferLayerCombineColour(FImage* ResultImage, const FImage* BaseImage, const FImage* MaskImage, FVector4f Color)
	{
		static_assert(NC > 0 && NC <= 4);
	...
	template<uint32 (*BLEND_FUNC)(uint32, uint32)>
	inline void ImageLayerCombine(FImage* ResultImage, const FImage* BaseImage, const FImage* BlendedImage, bool bOnlyFirstLOD)
	{
		check(ResultImage->GetFormat() == BaseImage->GetFormat());
		check(ResultImage->GetSizeX() == BaseImage->GetSizeX());
		check(ResultImage->GetSizeY() == BaseImage->GetSizeY());
	...
	template< 
		uint32 (*BLEND_FUNC)(uint32, uint32), 
		uint32 (*BLEND_FUNC_MASKED)(uint32, uint32, uint32)>
	inline void ImageLayerCombine(FImage* ResultImage, const FImage* BaseImage, const FImage* MaskImage, const FImage* BlendedImage, bool bOnlyFirstLOD)
	{
		check(ResultImage->GetFormat() == BaseImage->GetFormat());
		check(ResultImage->GetSizeX() == BaseImage->GetSizeX());
		check(ResultImage->GetSizeY() == BaseImage->GetSizeY());
	...
	template<uint32 (*BLEND_FUNC)(uint32, uint32)>
	inline void ImageLayerCombineColour(FImage* ResultImage, const FImage* BaseImage, FVector4f Color)
	{
		check(ResultImage->GetFormat() == BaseImage->GetFormat());
		check(ResultImage->GetSizeX() == BaseImage->GetSizeX());
		check(ResultImage->GetSizeY() == BaseImage->GetSizeY());
	...
	template< 
		uint32 (*BLEND_FUNC)(uint32, uint32),
		uint32 (*BLEND_FUNC_MASKED)(uint32, uint32, uint32)>
	inline void ImageLayerCombineColour(FImage* ResultImage, const FImage* BaseImage, const FImage* MaskImage, FVector4f Color)
	{
		check(ResultImage->GetFormat() == BaseImage->GetFormat());
		check(ResultImage->GetSizeX() == BaseImage->GetSizeX());
		check(ResultImage->GetSizeY() == BaseImage->GetSizeY());
	...
	inline void BufferLayerCombineFunctor(FImage* DestImage, const FImage* BaseImage, const FImage* BlendImage, ImageCombineFn&& ImageCombine)
	{
	...
	inline void ImageLayerCombineFunctor(FImage* ResultImage, const FImage* BaseImage, const FImage* BlendedImage, ImageCombineFn&& ImageCombine)
	{
		check(ResultImage->GetFormat() == BaseImage->GetFormat());
		check(ResultImage->GetSizeX() == BaseImage->GetSizeX());
		check(ResultImage->GetSizeY() == BaseImage->GetSizeY());
	...
	inline void ImageBlendOnBaseNoAlpha(FImage* BaseImage,
		const FImage* MaskImage,
		const FImage* BlendedImage,
		const box<FIntVector2>& Rect)
	{
