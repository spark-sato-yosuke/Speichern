...
EDataValidationResult UAssetValidator_CustomizableObjects::IsCustomizableObjectValid(UCustomizableObject* InCustomizableObject, TArray<FText>& OutValidationErrors, TArray<FText>& OutValidationWarnings)
{
	...
	UCustomizableObject* RootObject = ICustomizableObjectEditorModule::GetChecked().GetRootObject(InCustomizableObject);
	check (RootObject);
	...
	for	(const FCompilationOptions& Options : CompilationOptionsToTest)
	{
		...
		TSharedRef<FCompilationRequest> CompileRequest = MakeShared<FCompilationRequest>(*RootObject);
		CompileRequest->Options = Options;
		CompileRequest->bAsync = false;
		...
		const TArray<FText>& CompilationErrors = CompileRequest->Errors;
		const TArray<FText>& CompilationWarnings = CompileRequest->Warnings;
