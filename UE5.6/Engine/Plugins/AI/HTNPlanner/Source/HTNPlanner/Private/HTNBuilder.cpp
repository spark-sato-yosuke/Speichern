...
bool FHTNBuilder_Domain::Compile()
{
	...
	constexpr std::size_t MemoryAlignment = FMath::Max(FMath::Max3(alignof(FHTNPrimitiveTask), alignof(FHTNEffect), alignof(FHTNMethod)),
		FMath::Max(alignof(FHTNPolicy::FTaskID), alignof(FHTNCondition)));

	uint8* CompiledData = static_cast<uint8*>(FMemory::Malloc(MemoryRequired, MemoryAlignment));
