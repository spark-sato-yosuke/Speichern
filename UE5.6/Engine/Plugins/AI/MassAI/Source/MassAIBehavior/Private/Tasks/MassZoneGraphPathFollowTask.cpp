...
void FMassZoneGraphPathFollowTask::GetDependencies(UE::MassBehavior::FStateTreeDependencyBuilder& Builder) const
{
	...
	Builder.AddReadOnly(AgentRadiusHandle);
...
EStateTreeRunStatus FMassZoneGraphPathFollowTask::EnterState(FStateTreeExecutionContext& Context, const FStateTreeTransitionResult& Transition) const
{
	...
	{
		// @todo: Verbosity of this message was downgraded from 'Error' to 'Log' until we have time to investigate.
		...
		MASSBEHAVIOR_LOG(Log, TEXT("Target is not on current lane, target lane is %s expected %s."), *TargetLocation->LaneHandle.ToString(), *LaneLocation.LaneHandle.ToString());
		return EStateTreeRunStatus::Failed;
	}
