...
void FMassZoneGraphFindEscapeTarget::GetDependencies(UE::MassBehavior::FStateTreeDependencyBuilder& Builder) const
{
	Builder.AddReadOnly(LocationHandle);
	Builder.AddReadOnly(ZoneGraphSubsystemHandle);
	Builder.AddReadOnly(ZoneGraphAnnotationSubsystemHandle);
}
...
EStateTreeRunStatus FMassZoneGraphFindEscapeTarget::EnterState(FStateTreeExecutionContext& Context, const FStateTreeTransitionResult& Transition) const
{
	...
	{
		// @todo: Verbosity of this message was downgraded from 'Warning' to 'Log' until we have time to investigate.
		// MASSBEHAVIOR_LOG now outputs to the debug log in addition to the visual logger and some sample
		// projects (e.g., CitySample) might spam.
		MASSBEHAVIOR_LOG(Log, TEXT("Invalid flee exit lane."));
		return EStateTreeRunStatus::Failed;
	}
