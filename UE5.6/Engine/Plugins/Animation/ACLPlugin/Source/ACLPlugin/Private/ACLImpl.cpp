	...
	bool bBuildAdditiveBase, ACLPhantomTrackMode PhantomTrackMode)
{
	...
	const FVector3f UEDefaultScale(bIsAdditive ? 0.0f : 1.0f);
	...
	for (int32 BoneIndex = 0; BoneIndex < NumBones; ++BoneIndex)
	{
		...
		else
		{
			...
			{
				BindTransform = rtm::qvv_set(UEQuatToACL(UEBone.Orientation), UEVector3ToACL(UEBone.Position), UEVector3ToACL(UEBone.Scale));
			}
