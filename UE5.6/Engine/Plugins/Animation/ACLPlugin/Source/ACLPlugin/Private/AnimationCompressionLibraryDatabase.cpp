...
void UAnimationCompressionLibraryDatabase::BuildDatabase(TArray<uint8>& OutCompressedBytes, TArray<uint64>& OutAnimSequenceMappings, TArray<uint8>& OutBulkData, bool bStripLowestTier)
{
	...
	const uint32 LibraryHash = GetTypeHash(this);
	...
	{
		AnimSeq->RequestResidency(CurrentPlatform, LibraryHash);
	}

	UE::Anim::IAnimSequenceCompilingManager::FinishCompilation(AnimSequences);

	auto ReleaseResidency = [this, CurrentPlatform, &LibraryHash]()
	{
		for (UAnimSequence* AnimSeq : AnimSequences)
		{
			AnimSeq->ReleaseResidency(CurrentPlatform, LibraryHash);
		}
	};	
	...
	for (UAnimSequence* AnimSeq : AnimSequences)
	{
		...
		check(CompressedAnimSequence.Get().IsValid(AnimSeq));

		UAnimBoneCompressionCodec_ACLDatabase* DatabaseCodec = Cast<UAnimBoneCompressionCodec_ACLDatabase>(CompressedAnimSequence.Get().BoneCompressionCodec);
		...
		const FACLDatabaseCompressedAnimData& AnimData = static_cast<const FACLDatabaseCompressedAnimData&>(*CompressedAnimSequence.Get().CompressedDataStructure);
		if (!AnimData.IsValid())
		{
	...
	for (int32 MappingIndex = 0; MappingIndex < NumSequences; ++MappingIndex)
	{
		...
		UAnimSequence::FScopedCompressedAnimSequence CompressedAnimSequence = const_cast<const UAnimSequence*>(AnimSeq)->GetCompressedData();
		const acl::compressed_tracks* CompressedTracks = ACLDBCompressedTracks[MappingIndex];
		FACLDatabaseCompressedAnimData& AnimData = static_cast<FACLDatabaseCompressedAnimData&>(*CompressedAnimSequence.Get().CompressedDataStructure);
...
void UAnimationCompressionLibraryDatabase::BeginDestroy()
{
	...
	{
		FTSTicker::GetCoreTicker().RemoveTicker(FidelityUpdateTickerHandle);
...
uint32 UAnimationCompressionLibraryDatabase::SetVisualFidelityImpl(ACLVisualFidelity VisualFidelity, ACLVisualFidelityChangeResult* OutResult)
{
	...
	if (bIsFirstRequest)
	{
		...
		FidelityUpdateTickerHandle = FTSTicker::GetCoreTicker().AddTicker(TEXT("ACLDBStreamOut"), 0.0F, UpdateVisualFidelity);
	}
