					...
					.ToolTipText_Lambda([]() 
						{
							...
							const int32 Index = Enum->GetIndexByValue(static_cast<int64>(GetDefault<ULiveLinkHubSettings>()->AutoConnectClients));
...
private:
	...
	{
		GetMutableDefault<ULiveLinkHubSettings>()->AutoConnectClients = AutoConnectMode;
		GetMutableDefault<ULiveLinkHubSettings>()->TryUpdateDefaultConfigFile();
	}
	...
	TSharedRef<SWidget> CreateAutoConnectModeSwitcher()
	{
		TSharedRef<SWidgetSwitcher> Switcher =
			...
			.WidgetIndex_Lambda([]()
			{
				return StaticEnum<ELiveLinkHubAutoConnectMode>()->GetIndexByValue(static_cast<int64>(GetDefault<ULiveLinkHubSettings>()->AutoConnectClients));
			});
