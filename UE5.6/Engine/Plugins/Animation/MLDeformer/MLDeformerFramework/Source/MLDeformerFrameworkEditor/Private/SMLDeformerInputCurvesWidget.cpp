...
namespace UE::MLDeformer
{
	...
	void SMLDeformerInputCurveListWidget::RefreshElements(const TArray<FName>& CurveNames, USkeletalMesh* SkeletalMesh)
	{
		Elements.Reset();

		const FLinearColor ErrorColor = FMLDeformerEditorStyle::Get().GetColor("MLDeformer.InputsWidget.ErrorColor");

		// If we have no skeletal mesh, just add everything as flat list as we don't have hierarchy data.
		...
		else
		{
			...
			if (SkeletalMesh)
			{
				...
				if (SkelMeshCurveData)
				{
					...
					AssetCurveNames.Append(SkelMeshCurveNames);
				}
			}
	...
	void SMLDeformerInputCurvesWidget::OnAddInputCurves()
	{
		...
		if (SkelMesh)
		{
			...
			if (!PickerCurveNames.IsEmpty())
			{
				...
				FScopedTransaction Transaction(TEXT("SMLDeformerInputCurvesWidget"), LOCTEXT("AddInputCurvesText", "Add Input Curves"), Model);
				Model->Modify();
				...
				for (const FName CurveName : PickerCurveNames)
				{
					...
					TArray<FMLDeformerCurveReference>& ModelCurveIncludeList = Model->GetCurveIncludeList();
	...
	{
		UMLDeformerModel* Model = EditorModel->GetModel();
		FScopedTransaction Transaction(LOCTEXT("ClearInputCurvesText", "Clear Input Curves"));
		Model->Modify();
	...
	void SMLDeformerInputCurvesWidget::OnDeleteInputCurves()
	{
		...
		UMLDeformerModel* Model = EditorModel->GetModel();
		FScopedTransaction Transaction(LOCTEXT("RemoveInputCurvesText", "Remove Input Curves"));
		Model->Modify();
	...
	{
		UMLDeformerModel* Model = EditorModel->GetModel();
		FScopedTransaction Transaction(LOCTEXT("AddAnimatedInputCurvesText", "Add Animated Input Curves"));
		Model->Modify();
	...
	void SMLDeformerInputCurvesWidget::RefreshList(bool bBroadCastPropertyChanged)
	{
		...
		ListWidget->RefreshElements(CurveNames, Model->GetSkeletalMesh());
