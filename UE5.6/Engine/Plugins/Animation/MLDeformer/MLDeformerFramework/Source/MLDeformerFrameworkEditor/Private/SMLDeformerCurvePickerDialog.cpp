...
namespace UE::MLDeformer
{
	...
	void SMLDeformerCurvePickerDialog::RefreshListElements() const 
	{
		CurveListView->Clear();
		...
		if (SkelMeshCurveData)
		{
			...
			CurveNames.Append(SkelMeshCurveNames);
		}
		...
		for (const FName CurveName : CurveNames)
		{
			...
			bool bIncludeCurve = (FilterText.IsEmpty() || CurveName.ToString().Contains(FilterText));
			if (!IncludeList.IsEmpty())
			{
				bIncludeCurve &= IncludeList.Contains(CurveName);
			}

			if (bIncludeCurve)
			{
				TSharedPtr<FMLDeformerCurvePickerElement> Element = MakeShared<FMLDeformerCurvePickerElement>(CurveName, FSlateColor::UseForeground());
				Element->TextColor = (HighlightCurveNames.Contains(CurveName)) ? HighlightColor : FSlateColor::UseForeground();
