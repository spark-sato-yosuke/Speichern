...
{
	static TAutoConsoleVariable<bool> CVarMLDeformerDebugDrawInputPoses(
		TEXT("MLDeformer.DebugDrawInputPoses"),
		false,
		TEXT("Draw the input pose for each active ML Deformer.")
	);
}
...
void UMLDeformerModelInstance::DrawDebugInputPoses()
{
	...
	{
		const FTransform& ComponentTransform = FinalSkelMeshComponent->GetComponentTransform();
		const TArray<FTransform>& LeaderComponentSpaceTransforms = LeaderPoseComponent->GetComponentSpaceTransforms();
		if (!LeaderComponentSpaceTransforms.IsEmpty())
		{
			...
			for (int32 Index = 0; Index < NumTrainingBones; ++Index)
			{
				...
				if (LeaderComponentSpaceTransforms.IsValidIndex(ComponentBoneIndex))
				{
					const FTransform& ComponentSpaceBoneTransform = LeaderComponentSpaceTransforms[ComponentBoneIndex];
					const int32 ParentIndex = RefSkel.GetParentIndex(ComponentBoneIndex);
					const FTransform WorldTransform = ComponentSpaceBoneTransform * ComponentTransform;
					if (LeaderComponentSpaceTransforms.IsValidIndex(ParentIndex))
					{
						const FTransform WorldParentTransform = LeaderComponentSpaceTransforms[ParentIndex] * ComponentTransform;
						DrawDebugLine(World, WorldTransform.GetTranslation(), WorldParentTransform.GetTranslation(), FColor::Yellow, false, -1.0f, 255, 0.0f);
						DrawDebugPoint(World, WorldTransform.GetTranslation(), 4.0f, FColor::Yellow, false, -1.0f, 255);
						DrawDebugCoordinateSystem(World, WorldTransform.GetTranslation(), WorldTransform.GetRotation().Rotator(), 7.5f, false, -1.0f, 255, 0.5f);						
					}
					else
					{
						DrawDebugPoint(World, WorldTransform.GetTranslation(), 4.0f, FColor::Yellow, false, -1.0f, 255);
	...
	else // No leader pose is used.
	{
		...
		{
			const FTransform& ComponentTransform = FinalSkelMeshComponent->GetComponentTransform();
			const TArray<FTransform>& ComponentSpaceTransforms = FinalSkelMeshComponent->GetComponentSpaceTransforms();
			if (!ComponentSpaceTransforms.IsEmpty())
			{
				const USkinnedAsset* SkinnedAsset = FinalSkelMeshComponent->GetSkinnedAsset();
				const FReferenceSkeleton& RefSkel = SkinnedAsset->GetRefSkeleton();
				const int32 NumTrainingBones = AssetBonesToSkelMeshMappings.Num();
				for (int32 Index = 0; Index < NumTrainingBones; ++Index)
				{
					const int32 ComponentBoneIndex = AssetBonesToSkelMeshMappings[Index];
					if (ComponentSpaceTransforms.IsValidIndex(ComponentBoneIndex))
					{
						const FTransform& ComponentSpaceBoneTransform = ComponentSpaceTransforms[ComponentBoneIndex];
						const FTransform WorldTransform = ComponentSpaceBoneTransform * ComponentTransform;
						const int32 ParentIndex = RefSkel.GetParentIndex(ComponentBoneIndex);
						if (ComponentSpaceTransforms.IsValidIndex(ParentIndex))
						{
							const FTransform WorldParentTransform = ComponentSpaceTransforms[ParentIndex] * ComponentTransform;
							DrawDebugLine(World, WorldTransform.GetTranslation(), WorldParentTransform.GetTranslation(), FColor::Yellow, false, -1.0f, 255, 0.0f);
							DrawDebugPoint(World, WorldTransform.GetTranslation(), 4.0f, FColor::Yellow, false, -1.0f, 255);
							DrawDebugCoordinateSystem(World, WorldTransform.GetTranslation(), WorldTransform.GetRotation().Rotator(), 7.5f, false, -1.0f, 255, 0.5f);
						}
						...
						{
							DrawDebugPoint(World, WorldTransform.GetTranslation(), 4.0f, FColor::Yellow, false, -1.0f, 255);
							DrawDebugCoordinateSystem(World, WorldTransform.GetTranslation(), WorldTransform.GetRotation().Rotator(), 7.5f, false, -1.0f, 255, 1.0f);						
						}
					}
				}
			}
...
{
	bIsCompatible = SkeletalMeshComponent && SkeletalMeshComponent->GetSkeletalMeshAsset() && CheckCompatibility(SkeletalMeshComponent.Get(), true).IsEmpty();
}
...
FString UMLDeformerModelInstance::CheckCompatibility(USkeletalMeshComponent* InSkelMeshComponent, bool bLogIssues)
{
	...
	if (SkelMesh && !InputInfo->IsCompatible(SkelMesh) && Model->GetDeformerAsset() && Model->IsTrained())
	{
...
void UMLDeformerModelInstance::UpdateBoneTransforms()
{
	...
	else // No leader pose is used.
	{
		if (FinalSkelMeshComponent == SkeletalMeshComponent)
		{
			const TArray<FTransform>& ComponentTransforms = FinalSkelMeshComponent->GetComponentSpaceTransforms();
			if (!ComponentTransforms.IsEmpty())
			{
				const USkinnedAsset* SkinnedAsset = FinalSkelMeshComponent->GetSkinnedAsset();
				const FReferenceSkeleton& RefSkel = SkinnedAsset->GetRefSkeleton();
				const int32 NumTrainingBones = AssetBonesToSkelMeshMappings.Num();
				...
				{
					const int32 ComponentBoneIndex = AssetBonesToSkelMeshMappings[Index];
					if (ComponentTransforms.IsValidIndex(ComponentBoneIndex))
					{
						const FTransform& ComponentSpaceTransform = ComponentTransforms[ComponentBoneIndex];
						const int32 ParentIndex = RefSkel.GetParentIndex(ComponentBoneIndex);
						if (ComponentTransforms.IsValidIndex(ParentIndex))
						{
							TrainingBoneTransforms[Index] = ComponentSpaceTransform.GetRelativeTransform(ComponentTransforms[ParentIndex]);
							TrainingBoneTransforms[Index].NormalizeRotation();
						}
						else
						{
...
void UMLDeformerModelInstance::Tick(float DeltaTime, float ModelWeight)
{
	...
	if (ModelWeight > 0.0001f && IsCompatible() && HasValidTransforms() && SetupInputs())
	{
		...
		Execute(ModelWeight);
		const bool bDrawDebugInputPoses = UE::MLDeformer::CVarMLDeformerDebugDrawInputPoses->GetBool();
		if (bDrawDebugInputPoses)
		{
			DrawDebugInputPoses();
		}
