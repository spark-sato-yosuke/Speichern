...
void UNeuralMorphModelInstance::Execute(float ModelWeight)
{
	...
	if (!NetworkInstance || MorphNetwork->IsEmpty())
	{
	...
	const int32 NumMorphWeights = WeightData->Weights.Num();

	// Set the weights to zero if we don't have enough weights.
	if (NumMorphWeights < NumNetworkWeights + 1)	// Plus 1 because of the extra means morph target.
	{
		WeightData->ZeroWeights();
		return;
	}
