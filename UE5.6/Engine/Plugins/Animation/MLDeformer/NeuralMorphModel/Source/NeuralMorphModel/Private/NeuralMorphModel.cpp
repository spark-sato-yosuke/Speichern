...
{
	return NewObject<UNeuralMorphInputInfo>(this, NAME_None, RF_Transactional);
}
...
void UNeuralMorphModel::PostLoad()
{
	Super::PostLoad();
	UpdateMissingGroupNames();
	check(SupportsGlobalModeOnly() ? Mode == ENeuralMorphMode::Global : true);
}
...
void UNeuralMorphModel::GetAssetRegistryTags(FAssetRegistryTagsContext Context) const
{
	...
	Context.AddTag(FAssetRegistryTag("MLDeformer.NeuralMorphModel.UseDualQuaternions", SkinningMode == EMLDeformerSkinningMode::DualQuaternion ? TEXT("True") : TEXT("False"), FAssetRegistryTag::TT_Alphabetical));
}
