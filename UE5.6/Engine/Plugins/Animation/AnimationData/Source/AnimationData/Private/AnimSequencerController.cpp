...
void UAnimSequencerController::ResizeNumberOfFrames(FFrameNumber NewLength, FFrameNumber T0, FFrameNumber T1, bool bShouldTransact)
{
	...
	if (UMovieScene* MovieScene = Model->GetMovieScene())
	{
...
void UAnimSequencerController::SetMovieSceneRange(FFrameNumber InFrameNumber) const
{
	...
	{
		Section->SectionRange = DataRange;
	}
}
...
bool UAnimSequencerController::SetBoneTrackKeys(FName BoneName, const TArray<FVector>& PositionalKeys, const TArray<FQuat>& RotationalKeys, const TArray<FVector>& ScalingKeys, bool bShouldTransact /*= true*/)
{
	...
	FloatPositionalKeys.Reserve(PositionalKeys.Num());
	FloatRotationalKeys.Reserve(RotationalKeys.Num());
	FloatScalingKeys.Reserve(ScalingKeys.Num());
...
void UAnimSequencerController::UpdateWithSkeleton(USkeleton* TargetSkeleton, bool bShouldTransact)
{
	if (ModelInterface->HasBeenPopulated())
	{
		...
		{
			// Work out if any work has to be performed.
			TArray<FName> BonesToBeRemoved;
			TArray<FName> CurvesToBeRemoved;
			...
			if(TargetSkeleton)
			{
			    ...
			    if (UMovieSceneControlRigParameterSection* Section = Model->GetFKControlRigSection())
			    {
				    ...
				    for (const FTransformParameterNameAndCurves& BoneCurve : BoneTrackCurves)
				    {
					    ...
					    if (ReferenceSkeleton.FindBoneIndex(ExpectedBoneName) == INDEX_NONE)
					    {
						    // Bone no longer exists on skeleton
						    BonesToBeRemoved.Add(ExpectedBoneName);
					    }
				    }
				    ...
				    const TArray<FScalarParameterNameAndCurve>& SectionCurves = Section->GetScalarParameterNamesAndCurves();
				    for (const FScalarParameterNameAndCurve& SectionCurve : SectionCurves)
				    {
					    const FName ExpectedCurveName = UFKControlRig::GetControlTargetName(SectionCurve.ParameterName, ERigElementType::Curve);
					    if (!SkeletonCurveNames.Contains(ExpectedCurveName))
					    {
						    // Curve no longer exists on skeleton
						    CurvesToBeRemoved.Add(ExpectedCurveName);
					    }
				    }
			    }
			}

			if (BonesToBeRemoved.Num() || CurvesToBeRemoved.Num())
			{
	...
	{		
		...
		if (UFKControlRig* FKRig = Cast<UFKControlRig>(ControlRig))
		{
			if(URigHierarchy* Hierarchy = Model->GetControlRigHierarchy())
			{
				...
				const bool bIsSorted = Algo::IsSorted(TimeValues);
				if (bContainsBone && bContainsBoneControl && bContainsParameter && bIsSorted)
				{
					FTransformParameterNameAndCurves* ParameterCurvePair = Section->GetTransformParameterNamesAndCurves().FindByPredicate([BoneNameControlKey](const FTransformParameterNameAndCurves& Parameter)
					{
						return Parameter.ParameterName == BoneNameControlKey.Name;
					...
					TArray<FMovieSceneFloatValue> EulerX;
					TArray<FMovieSceneFloatValue> EulerY;
					TArray<FMovieSceneFloatValue> EulerZ;
					TArray<FMovieSceneFloatValue> ScaleX;
					TArray<FMovieSceneFloatValue> ScaleY;
					TArray<FMovieSceneFloatValue> ScaleZ;
					InTimes.Reserve(NumKeys);
					LocationX.Reserve(NumKeys);
					LocationY.Reserve(NumKeys);
					LocationZ.Reserve(NumKeys);
					EulerX.Reserve(NumKeys);
					EulerY.Reserve(NumKeys);
					EulerZ.Reserve(NumKeys);
					ScaleX.Reserve(NumKeys);
					ScaleY.Reserve(NumKeys);
					ScaleZ.Reserve(NumKeys);
					...
					for (int32 KeyIndex = 0; KeyIndex < NumKeys; ++KeyIndex)
					{
						...
						FMovieSceneFloatValue LocationValueX(Location.X);
						FMovieSceneFloatValue LocationValueY(Location.Y);
						FMovieSceneFloatValue LocationValueZ(Location.Z);
						LocationValueX.InterpMode = ERichCurveInterpMode::RCIM_Linear;
						LocationValueY.InterpMode = ERichCurveInterpMode::RCIM_Linear;
						LocationValueZ.InterpMode = ERichCurveInterpMode::RCIM_Linear;
						FMovieSceneFloatValue EulerValueX(EulerAngles.X);
						FMovieSceneFloatValue EulerValueY(EulerAngles.Y);
						FMovieSceneFloatValue EulerValueZ(EulerAngles.Z);
						EulerValueX.InterpMode = ERichCurveInterpMode::RCIM_Linear;
						EulerValueY.InterpMode = ERichCurveInterpMode::RCIM_Linear;
						EulerValueZ.InterpMode = ERichCurveInterpMode::RCIM_Linear;
						FMovieSceneFloatValue ScaleValueX(Scale.X);
						FMovieSceneFloatValue ScaleValueY(Scale.Y);
						FMovieSceneFloatValue ScaleValueZ(Scale.Z);
						ScaleValueX.InterpMode = ERichCurveInterpMode::RCIM_Linear;
						ScaleValueY.InterpMode = ERichCurveInterpMode::RCIM_Linear;
						ScaleValueZ.InterpMode = ERichCurveInterpMode::RCIM_Linear;
						InTimes.Add(FrameNumber);
						LocationX.Add(LocationValueX);
						LocationY.Add(LocationValueY);
						LocationZ.Add(LocationValueZ);
						EulerX.Add(EulerValueX);
						EulerY.Add(EulerValueY);
						EulerZ.Add(EulerValueZ);
						ScaleX.Add(ScaleValueX);
						ScaleY.Add(ScaleValueY);
						ScaleZ.Add(ScaleValueZ);
					}

					ParameterCurvePair->Translation[0].UpdateOrAddKeys(InTimes, LocationX);
					ParameterCurvePair->Translation[1].UpdateOrAddKeys(InTimes, LocationY);
					ParameterCurvePair->Translation[2].UpdateOrAddKeys(InTimes, LocationZ);
					ParameterCurvePair->Rotation[0].UpdateOrAddKeys(InTimes, EulerX);
					ParameterCurvePair->Rotation[1].UpdateOrAddKeys(InTimes, EulerY);
					ParameterCurvePair->Rotation[2].UpdateOrAddKeys(InTimes, EulerZ);
					ParameterCurvePair->Scale[0].UpdateOrAddKeys(InTimes, ScaleX);
					ParameterCurvePair->Scale[1].UpdateOrAddKeys(InTimes, ScaleY);
					ParameterCurvePair->Scale[2].UpdateOrAddKeys(InTimes, ScaleZ);
					ParameterCurvePair->Translation[0].ClearDefault();
					ParameterCurvePair->Translation[1].ClearDefault();
					ParameterCurvePair->Translation[2].ClearDefault();
					ParameterCurvePair->Rotation[0].ClearDefault();
					ParameterCurvePair->Rotation[1].ClearDefault();
					ParameterCurvePair->Rotation[2].ClearDefault();
					ParameterCurvePair->Scale[0].ClearDefault();
					ParameterCurvePair->Scale[1].ClearDefault();
					ParameterCurvePair->Scale[2].ClearDefault();
					return true;
				}
				else
				{
					...
					if (!bIsSorted)
					{
						ReportWarning(LOCTEXT("TimeValuesNotSorted", "Given Time Values are not sorted"));
					}
				}
			}
			else
		...
		if (UFKControlRig* FKRig = Cast<UFKControlRig>(ControlRig))
		{
			if(const URigHierarchy* Hierarchy = Model->GetControlRigHierarchy())
			{
				...
				if (bContainsCurve && bContainsCurveControl)
				{
					...
					{
						const FFrameRate MaxChannelRate = FFrameRate(240000, 1);
						ParameterCurvePair->ParameterCurve.RemapTimes(UE::MovieScene::FFrameRateRetiming{ CurveRate, MaxChannelRate });
