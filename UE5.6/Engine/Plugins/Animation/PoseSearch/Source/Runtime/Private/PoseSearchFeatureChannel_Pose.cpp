...
bool UPoseSearchFeatureChannel_Pose::Finalize(UPoseSearchSchema* Schema)
{
	...
	for (int32 ChannelBoneIdx = 0; ChannelBoneIdx != SampledBones.Num(); ++ChannelBoneIdx)
	{
		...
		if (EnumHasAnyFlags(SampledBone.Flags, EPoseSearchBoneFlags::Rotation))
		{
			...
			HeadingX->PermutationTimeType = PermutationTimeType;
			...
			HeadingY->PermutationTimeType = PermutationTimeType;
		...
		if (EnumHasAnyFlags(SampledBone.Flags, EPoseSearchBoneFlags::Velocity))
		{
			...
			Velocity->PermutationTimeType = PermutationTimeType;
		...
		if (EnumHasAnyFlags(SampledBone.Flags, EPoseSearchBoneFlags::Phase))
		{
			...
			if (PermutationTimeType != EPermutationTimeType::UseSampleTime)
			{
				UE_LOG(LogPoseSearch, Error, TEXT("UPoseSearchFeatureChannel_Pose::Finalize - unsupported PermutationTimeType %d for EPoseSearchBoneFlags::Phase in Schema '%s'"), (int32)PermutationTimeType, *Schema->GetName());
				return false;
			}
