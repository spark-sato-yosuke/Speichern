...
struct FKDTree
{
	...
	struct FKNNMaxHeapResultSet
	{
		struct FResult
		{
			...
			FORCEINLINE bool operator()(const FResult& A, const FResult& B) const
			{
		...
		inline FKNNMaxHeapResultSet(TArrayView<FResult> Results, float InitWorstDist = UE_BIG_NUMBER)
		: MaxHeap(Results)
		, MaxHeapNum(0)
		, WorstDist(InitWorstDist)
		{
		}
		...
		inline bool addPoint(float dist, AccessorType index)
		{
			if (full())
			{
				...
				{
					MaxHeap[0].Distance = dist;
					MaxHeap[0].Index = index;

					AlgoImpl::HeapSiftDown(MaxHeap.GetData(), 0, MaxHeapNum, FIdentityFunctor(), FResult());
					WorstDist = MaxHeap[0].Distance;
				}
			}
			else
			{
				MaxHeap[MaxHeapNum] = { dist, index };
				// calling HeapSiftUp every addPoint seems to be faster than calling HeapifyInternal the first time MaxHeap is full
				// @todo: do more profiling! (comment out AlgoImpl::HeapSiftUp and uncomment below AlgoImpl::HeapifyInternal)
				AlgoImpl::HeapSiftUp(MaxHeap.GetData(), 0, MaxHeapNum, FIdentityFunctor(), FResult());
				++MaxHeapNum;

				if (full())
				{
					//AlgoImpl::HeapifyInternal(MaxHeap.GetData(), MaxHeapNum, FIdentityFunctor(), FResult());
					WorstDist = MaxHeap[0].Distance;
				}
			}
	...
	protected:
		TArrayView<FResult> MaxHeap;
		int32 MaxHeapNum;
		float WorstDist;
	};

	// Experimental, this feature might be removed without warning, not for production use
	struct FFilteredKNNMaxHeapResultSet : public FKNNMaxHeapResultSet
	{
		inline FFilteredKNNMaxHeapResultSet(TArrayView<FResult> Results, TConstArrayView<AccessorType> InExcludeFromSearchIndexes)
		: FKNNMaxHeapResultSet(Results)
		, ExcludeFromSearchIndexes(InExcludeFromSearchIndexes)
		{
			check(Algo::IsSorted(InExcludeFromSearchIndexes));
		}
		...
		inline bool addPoint(float dist, AccessorType index)
		{
			if (Algo::BinarySearch(ExcludeFromSearchIndexes, index) == INDEX_NONE)
			{
				FKNNMaxHeapResultSet::addPoint(dist, index);
			}
			return true;
		}
	...
	// Experimental, this feature might be removed without warning, not for production use
	struct FRadiusMaxHeapResultSet : public FKNNMaxHeapResultSet
	{
		inline FRadiusMaxHeapResultSet(TArrayView<FResult> Results, float Radius)
		: FKNNMaxHeapResultSet(Results, Radius)
		{
		}
	...
	FKDTree(FKDTree&& Other);
	
	~FKDTree();

	FKDTree& operator=(const FKDTree& Other);
	FKDTree& operator=(FKDTree&& Other);
	...
	// Experimental, this feature might be removed without warning, not for production use
	int32 FindNeighbors(FKNNMaxHeapResultSet& Result, TConstArrayView<float> Query) const;
	// Experimental, this feature might be removed without warning, not for production use
	int32 FindNeighbors(FFilteredKNNMaxHeapResultSet& Result, TConstArrayView<float> Query) const;
	// Experimental, this feature might be removed without warning, not for production use
	int32 FindNeighbors(FRadiusMaxHeapResultSet& Result, TConstArrayView<float> Query) const;
