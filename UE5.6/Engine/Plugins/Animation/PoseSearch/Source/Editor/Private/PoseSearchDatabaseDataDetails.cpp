...
{
public:
	FChannelItem(const UPoseSearchFeatureChannel* Channel, int32 ChannelComponentIdx, const FSearchIndex& SearchIndex)
	...
	, Weight(ComputeWeight(Channel, ChannelComponentIdx, SearchIndex))
	{
	}
...
private:
	...
	static float ComputeWeight(const UPoseSearchFeatureChannel* Channel, int32 ChannelComponentIdx, const FSearchIndex& SearchIndex)
	{
		// Using double precision for accumulation of component weights
		float Weight = 0.0f;

		const int32 DataOffset = ComputeDataOffset(Channel, ChannelComponentIdx);
		const int32 Cardinality = ComputeCardinality(Channel, ChannelComponentIdx);

		for (int32 Component = 0; Component != Cardinality; ++Component)
		{
			// Weights are pre-multiplied by inverse variance in PreprocessSearchIndexWeights, so we undo that
			// here to reconstitute the original weights
			const float WeightSqrt = SearchIndex.WeightsSqrt[DataOffset + Component];
			const float Deviation = SearchIndex.DeviationEditorOnly[DataOffset + Component];
			Weight += FMath::Square(WeightSqrt * Deviation);
		}
...
public:
	...
	virtual TSharedRef<SWidget> GenerateWidgetForColumn(const FName& ColumnName) override
	{
		...
		if (ColumnName == FName("Weight"))
		{
			...
			return SNew(STextBlock)
				.Text(FText::FromString(FString::Printf(TEXT("%.3f"), ChannelItem->GetWeight())));
		}
		...
		if (ChannelItem->GetDataOffset() == INDEX_NONE)
		{
										...
										for (const TArray<FDatabasePreviewActor>& PreviewActorGroup : ViewModel->GetPreviewActors())
										{
											for (const FDatabasePreviewActor& PreviewActor : PreviewActorGroup)
											{
												if (*GetNameSafe(PreviewActor.GetActor()) == ColumnName)
												{
													...
													{
														FString SourceAssetIndexString;
														SourceAssetIndexString.AppendInt(Assets[AssetIdx].GetSourceAssetIdx());
														return FText::FromString(SourceAssetIndexString);
													}
												}
											}
										}
			...
			if (ChannelItem->GetCompactLabel() == "BlendParams")
			{
				return SNew(STextBlock)
					.Margin(FMargin(1.f, 1.f, 1.f, 1.f))
					.Text_Lambda([this, ColumnName]() -> FText
										...
										for (const TArray<FDatabasePreviewActor>& PreviewActorGroup : ViewModel->GetPreviewActors())
										{
											for (const FDatabasePreviewActor& PreviewActor : PreviewActorGroup)
											{
												if (*GetNameSafe(PreviewActor.GetActor()) == ColumnName)
												{
													...
													if (Assets.IsValidIndex(AssetIdx))
													{
														const FVector BlendParameters = Assets[AssetIdx].GetBlendParameters();
														FString BlendParamsString = FString::Printf(TEXT("%f, %f"), BlendParameters.X, BlendParameters.Y);
														return FText::FromString(BlendParamsString);
													}
												}
											}
										}
								...
								for (const TArray<FDatabasePreviewActor>& PreviewActorGroup : ViewModel->GetPreviewActors())
								{
									for (const FDatabasePreviewActor& PreviewActor : PreviewActorGroup)
									{
										if (*GetNameSafe(PreviewActor.GetActor()) == ColumnName)
										{
											...
											const TConstArrayView<float> PoseValues = PoseSearchDatabase->GetSearchIndex().GetPoseValuesSafe(PoseIdx, BufferUsedForReconstruction);
...
void SDatabaseDataDetails::Reconstruct(int32 MaxPreviewActors)
{
	...
	RebuildChannelItemsTreeRecursively(ChannelItems, PoseSearchDatabase->Schema->GetChannels(), PoseSearchDatabase->GetSearchIndex());
	...
	HeaderRow->AddColumn(
		SHeaderRow::Column(TEXT("ChannelName"))
			...
			.DefaultTooltip(LOCTEXT("ChannelName_ToolTip", "Channel Name"))
			.FillWidth(0.2f));

	HeaderRow->AddColumn(
		SHeaderRow::Column(TEXT("DataOffset"))
		.DefaultLabel(LOCTEXT("DataOffset_Header", "Data Offset"))
		.DefaultTooltip(LOCTEXT("DataOffset_ToolTip", "Offset from the beginning of the features data"))
		.FillWidth(0.1f));

	HeaderRow->AddColumn(
		SHeaderRow::Column(TEXT("Weight"))
		.DefaultLabel(LOCTEXT("Weight_Header", "Weight"))
		.DefaultTooltip(LOCTEXT("Weight_ToolTip", "Normalized weight of feature's contribution to pose comparisons.\nWeights are set on channels in the schema, and channels distribute weights over their features.\nWeights of all individual features are then normalized together so they sum to one."))
		.FillWidth(0.1f));
	...
	if (ViewModel->ShouldDrawQueryVector())
	{
		HeaderRow->AddColumn(
			...
			.DefaultTooltip(LOCTEXT("Query_ToolTip", "Query Values")));
	}
...
void SDatabaseDataDetails::RebuildChannelItemsTreeRecursively(TArray<FChannelItemPtr>& ChannelItems, TConstArrayView<TObjectPtr<UPoseSearchFeatureChannel>> Channels, const FSearchIndex& SearchIndex)
{
	for (const TObjectPtr<UPoseSearchFeatureChannel>& ChannelPtr : Channels)
	{
		if (ChannelPtr)
		{
			TSharedRef<FChannelItem> ChannelItem = MakeShareable(new FChannelItem(ChannelPtr, INDEX_NONE, SearchIndex));
			...
			if (ChannelPtr->GetSubChannels().IsEmpty())
			{
				...
				{
					TSharedRef<FChannelItem> SubChannelItem = MakeShareable(new FChannelItem(ChannelPtr, ChannelComponentIdx, SearchIndex));
					ChannelItem->GetChannelItems().Add(SubChannelItem);
				}
			}
			else
			{
				RebuildChannelItemsTreeRecursively(ChannelItem->GetChannelItems(), ChannelPtr->GetSubChannels(), SearchIndex);
			}
		}
	}
}
...
void SDatabaseDataDetails::RebuildChannelItemsStats(TArray<FChannelItemPtr>& ChannelItems)
{
	...
	ChannelItem->GetChannelItems().Add(MakeShareable(new FChannelItem("SDatabaseDataDetailsPoseIndex", "BlendParams")));
