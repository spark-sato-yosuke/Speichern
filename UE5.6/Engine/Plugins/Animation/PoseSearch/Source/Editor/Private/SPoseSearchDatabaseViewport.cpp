...
namespace UE::PoseSearch
{
	...
	void SDatabaseViewport::OnToggleDisplayEventData()
	{
		AssetEditorPtr.Pin()->GetViewModelSharedPtr().ToSharedRef()->ToggleDisplayEventData();
		Invalidate();
	}

	bool SDatabaseViewport::IsDisplayEventDataChecked() const
	{
		return AssetEditorPtr.Pin()->GetViewModelSharedPtr().ToSharedRef()->IsDisplayEventDataChecked();
	}
	...
	void SDatabaseViewport::BindCommands()
	{
		...
		CommandList->MapAction(
			Commands.ShowDisplayEventData,
			FExecuteAction::CreateSP(this, &SDatabaseViewport::OnToggleDisplayEventData),
			FCanExecuteAction(),
			FIsActionChecked::CreateSP(this, &SDatabaseViewport::IsDisplayEventDataChecked));
	}
