...
void UEncodeRootBoneModifier::OnApply_Implementation(UAnimSequence* Animation)
{
	...
	FScopedSlowTask SlowTask(static_cast<float>(NumAnimKeys));
	SlowTask.MakeDialog();
	for (int32 AnimKey = 0; AnimKey < NumAnimKeys; AnimKey++)
	{
		...
		Animation->GetController().UpdateBoneTrackKeys(Animation->GetSkeleton()->GetReferenceSkeleton().GetBoneName(0), KeyRangeToSet, { RootTransformNew.GetLocation() }, { RootTransformNew.GetRotation() }, { RootTransformNew.GetScale3D() }, bShouldTransact);
		...
		for (const FName& ChildBoneName : RootBoneChildBones)
		{
			...
			Animation->GetController().UpdateBoneTrackKeys(ChildBoneName, KeyRangeToSet, { ChildBoneNewTransform.GetLocation() }, { ChildBoneNewTransform.GetRotation() }, { ChildBoneNewTransform.GetScale3D() }, bShouldTransact);
		}
	}
