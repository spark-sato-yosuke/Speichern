...
void UZeroOutRootBoneModifier::OnApply_Implementation(UAnimSequence* Animation)
{
	...
	FScopedSlowTask SlowTask(static_cast<float>(NumKeys));
	SlowTask.MakeDialog();
	...
	for (int32 AnimKey = 0; AnimKey < NumKeys; AnimKey++)
	{
		...
		Controller.UpdateBoneTrackKeys(RootBoneName, KeyRangeToSet, { NewRootTransform.GetLocation() }, { NewRootTransform.GetRotation() }, { NewRootTransform.GetScale3D() }, bShouldTransact);
	}
