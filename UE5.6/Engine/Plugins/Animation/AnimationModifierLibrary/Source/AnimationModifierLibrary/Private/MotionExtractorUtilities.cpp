...
namespace UE::Anim::MotionExtractorUtility
{
	...
	TArray<FVector2D> GetRangesFromRootMotionByPredicate(const UAnimSequence* Animation, Predicate Pred, float SampleRate = 120.0f)
	{
		...
		int32 NumSteps = static_cast<int32>(AnimLength / SampleInterval);
		...
		{
			const double Time = static_cast<double>(Step) * SampleInterval;
			constexpr bool bAllowLooping = false;
			const FAnimExtractContext Context(Time, true, FDeltaTimeRecord(SampleInterval), bAllowLooping);
			const FTransform RootMotion = Animation->ExtractRootMotion(Context);
	...
	EMotionExtractor_Axis Axis)
{
	...
	case EMotionExtractor_MotionType::TranslationSpeed:
	{
		...
		{
			const double Delta = UMotionExtractorUtilityLibrary::CalculateMagnitude((BoneTransform.GetTranslation() - LastBoneTransform.GetTranslation()), Axis);
...
TArray<FVector2D> UMotionExtractorUtilityLibrary::GetStoppedRangesFromRootMotion(const UAnimSequence* AnimSequence, float StopSpeedThreshold, float SampleRate)
{
	auto Criterion = [StopSpeedThreshold](const FTransform& InRootMotionDelta, float DeltaTime)
	{
		...
		const double RootMotionSpeed = RootMotionTranslation.Size() / DeltaTime;
...
double UMotionExtractorUtilityLibrary::CalculateMagnitude(const FVector& Vector, EMotionExtractor_Axis Axis)
{
	switch (Axis)
	{
	...
	case EMotionExtractor_Axis::XY:		return FMath::Sqrt(Vector.X * Vector.X + Vector.Y * Vector.Y);
	case EMotionExtractor_Axis::XZ:		return FMath::Sqrt(Vector.X * Vector.X + Vector.Z * Vector.Z);
	case EMotionExtractor_Axis::YZ:		return FMath::Sqrt(Vector.Y * Vector.Y + Vector.Z * Vector.Z);
	case EMotionExtractor_Axis::XYZ:	return FMath::Sqrt(Vector.X * Vector.X + Vector.Y * Vector.Y + Vector.Z * Vector.Z);
	default: check(false); break;
	}
