...
FLiveLinkSubject::FLiveLinkSubject(TSharedPtr<FLiveLinkTimedDataInput> InTimedDataGroup)
	...
	, State(ELiveLinkSubjectState::Unknown)
{
...
void FLiveLinkSubject::Initialize(FLiveLinkSubjectKey InSubjectKey, TSubclassOf<ULiveLinkRole> InRole, ILiveLinkClient* InLiveLinkClient)
{
	...
	State = ELiveLinkSubjectState::Disconnected;
...
void FLiveLinkSubject::Update()
{
	...
	if (bPaused)
	{
		// No need to process if it's paused, just keep the same frame snapshot.
		if (HasValidFrameSnapshot())
		{
			FrameSnapshot.FrameData.GetBaseData()->WorldTime = FPlatformTime::Seconds();
		}
		return;
	}
...
void FLiveLinkSubject::AddFrameData(FLiveLinkFrameDataStruct&& InFrameData)
{
	...
	if (FrameIndex >= 0)
	{
		...
		{
			PreProcessor->PreProcessFrame(GetStaticData(), InFrameData);
		}
...
bool FLiveLinkSubject::IsPaused() const
{
	return bPaused;
}

void FLiveLinkSubject::PauseSubject()
{
	bPaused = true;
}

void FLiveLinkSubject::UnpauseSubject()
{
	bPaused = false;
}

void FLiveLinkSubject::PreprocessFrame(const FLiveLinkStaticDataStruct& InStaticData, FLiveLinkFrameDataStruct& InOutFrameData)
{
	for (ULiveLinkFramePreProcessor::FWorkerSharedPtr PreProcessor : FramePreProcessors)
	{
		PreProcessor->PreProcessFrame(InStaticData, InOutFrameData);
	}
}
...
void FLiveLinkSubject::UpdateState()
{
	const ELiveLinkSubjectState PreviousState = State;

	const bool bHasValidFrame = (FApp::GetCurrentTime() - GetLastPushTime() < GetDefault<ULiveLinkSettings>()->GetTimeWithoutFrameToBeConsiderAsInvalid());
	State = (bHasValidFrame && HasValidFrameSnapshot()) ? ELiveLinkSubjectState::Connected : ELiveLinkSubjectState::Unresponsive;

	if (State != PreviousState)
	{
		OnStateChangedDelegate.ExecuteIfBound(State);
	}
}
			...
			if (NewRemapper.IsValid())
			{
				...
				if (bRecreateRemapper)
				{
					...
					if (SubjectSetting->Remapper->IsValidRemapper())
					{
						...
						if (NewRemapper->IsRemapperCompatible(StaticData))
						{
							if (HasValidFrameSnapshot())
							{
						...
						{
							UE_LOG(LogLiveLink, Warning, TEXT("Remapper is not compatible with this static data."));
						}
					}
					else
					{
...
{
	using enum ELiveLinkSubjectState;
	switch (State)
	{
		case Connected: return ETimedDataInputState::Connected;
		case Unresponsive: return ETimedDataInputState::Unresponsive;
		case Disconnected: return ETimedDataInputState::Disconnected;
		default:
			ensureMsgf(false, TEXT("Unhandled ELiveLinkSubjectState::%d"), State);
