...
UObject* UDNAAssetImportFactory::FactoryCreateFile(UClass* InClass, UObject* InParent, FName InName, EObjectFlags Flags, const FString& Filename, const TCHAR* Parms, FFeedbackContext* Warn, bool& bOutOperationCanceled)
{
	...
	// If the user provided a SkeletalMesh through the AssetImportTask Options 
	// refrain from searching for the Skeletal Mesh in the asset package

	if (!IsValid(ImportUI->SkeletalMesh)) {
	...
	bool bIsAutomated = IsAutomatedImport() || IsAutomatedReimport();
