...
void SControlRigAnimAttributeView::Construct(const FArguments& InArgs, TSharedRef<IControlRigBaseEditor> InControlRigEditor)
{
	...
	const FName& InEventName) const
{
	if (IsValid(InControlRig) && ensure(InControlRig == ControlRigBeingDebuggedPtr))
	{
		if (InEventName == FRigUnit_BeginExecution::EventName)
		{
			...
			{
				const UE::Anim::FMeshAttributeContainer& InputSnapshot = InControlRig->InputAnimAttributeSnapshot;
				const UE::Anim::FMeshAttributeContainer& OutputSnapshot = InControlRig->OutputAnimAttributeSnapshot;
