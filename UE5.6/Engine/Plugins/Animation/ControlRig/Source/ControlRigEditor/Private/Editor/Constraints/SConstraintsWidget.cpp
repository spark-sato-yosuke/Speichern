...
namespace UE::Private
{
	...
	{
		const TWeakPtr<ISequencer> WeakSequencer = AnimationEditMode::GetSequencer();
	...
	void GetConstrainable(AActor* InActor, FConstrainable& InOutConstrainable)
	{
		InOutConstrainable = FConstrainable();
		...
		InOutConstrainable.Actor = InActor;
		InOutConstrainable.Object = InActor;
		...
		// if no component socket available then return the actor
		...
		auto GetSocketChooserWidget = [&InOutConstrainable](USceneComponent* Component)
		{
			return SNew(SSocketChooserPopup)
			.SceneComponent( Component )
			.OnSocketChosen_Lambda([&InOutConstrainable, Component](FName InSocketName)
			{
				InOutConstrainable.Object = Component;
				InOutConstrainable.Socket = InSocketName;
			});
		};

		// decal the menu 
		const FVector2D SummonLocation = FSlateApplication::Get().GetCursorPos();
	
		// if one component with sockets then constrain the component selecting the socket
		if (NumComponentsWithSockets == 1)
		{
			return CreateMenu( GetSocketChooserWidget(ComponentsWithSockets[0]), FSlateApplication::Get().GetCursorPos());
		}
	...
	bool CreateConstraint(const TArray<AActor*>& InChildren, const FConstrainable& InParent, const ETransformConstraintType InConstraintType, const bool bUseDefault)
	{
		UObject* Parent = InParent.Object.Get();
		UWorld* World = GetCurrentWorld();
	
		if (!Parent || !IsValid(World))
		{
			return false;
		}
		...
		bool bCreated = false;
		for (AActor* Child: InChildren)
		{
			if (Child != InParent.Object)
			{
				FScopedTransaction Transaction(LOCTEXT("CreateConstraintKey", "Create Constraint Key"));
				UTickableTransformConstraint* Constraint =
					UE::TransformConstraintUtil::CreateAndAddFromObjects(World, Parent, InParent.Socket, Child, NAME_None, InConstraintType, true, bUseDefault);
	...
	bool IsActorConstrainable(const AActor* InActor)
	{
		if (!InActor)
		{
			return false;
		...
		const ETransformConstraintType& InType)
{
	...
	OnGetParent = InArgs._OnGetParent;
...
FReply SConstraintMenuEntry::OnMouseButtonDown(const FGeometry& MyGeometry, const FPointerEvent& MouseEvent)
{
	if (MouseEvent.GetEffectingButton() == EKeys::LeftMouseButton)
	{
		...
		return CreateFromSelectionOrPicker(bUseMutableDefault);
	}
...
FReply SConstraintMenuEntry::CreateFromSelectionOrPicker(const bool bUseDefault) const
{
	struct FCreationArgs
	{
		...
		FConstrainable Parent;
	};

	FCreationArgs Args;
	Args.CreationDelegate = OnConstraintCreated;
	Args.ConstraintType = ConstraintType;
	Args.Parent = OnGetParent.IsBound() ? OnGetParent.Execute() : FConstrainable();
		
	// hide the creation menu
	FSlateApplication::Get().DismissAllMenus();
	
	// use the selected parent if any
	if (Args.Parent.Object.IsValid())
	{
		const TArray<AActor*> Children = UE::Private::GetCurrentSelection();
		const bool bCreated = UE::Private::CreateConstraint(Children, Args.Parent, Args.ConstraintType, bUseDefault);
		if (!bCreated)
		{
			return FReply::Unhandled();
		}
		...
		// update list
		if (Args.CreationDelegate.IsBound())
		{
			Args.CreationDelegate.Execute();
		}
	...
	// pick the parent
	return CreateSelectionPicker(bUseDefault);
}
...
FReply SConstraintMenuEntry::CreateSelectionPicker(const bool bUseDefault) const
{
	...
	ActorPickerMode.BeginActorPickingMode(
		...
		{
			OnParentPicked(InActor, Args.CreationDelegate, Args.ConstraintType, Args.bUseDefault);
		}) );
	
	return FReply::Handled();
}

void SConstraintMenuEntry::OnParentPicked(
	AActor* InParent,
	const FOnConstraintCreated& InCreationDelegate,
	...
	const bool bUseDefault)
{
	...
	// create constraints
	auto CreateConstraint = [InCreationDelegate, InConstraintType, bUseDefault](
		const TArray<AActor*>& Selection, UObject* InParent, const FName& InSocketName)
	{
		FConstrainable Parent;
		Parent.Object = InParent;
		Parent.Socket = InSocketName;

		const bool bCreated = UE::Private::CreateConstraint(Selection, Parent, InConstraintType, bUseDefault);
...
int32 FBaseConstraintListWidget::RefreshConstraintList()
{
	...
	if (ShowConstraints == EShowConstraints::ShowSelected)
	{
		...
		else
		{
			...
			{
				UE::TransformConstraintUtil::GetParentConstraints(World, Actor, Constraints);
			}
...
void SConstraintsEditionWidget::ResetParent()
{
	ConstrainableParent = FConstrainable();
}

const FConstrainable& SConstraintsEditionWidget::GetParent() const
{
	return ConstrainableParent;
}

bool SConstraintsEditionWidget::IsParentValid() const
{
	return ConstrainableParent.Actor.IsValid();
}

void SConstraintsEditionWidget::UpdateParent(AActor* InActor)
{
	UE::Private::GetConstrainable(InActor, ConstrainableParent);
}
...
void SConstraintsEditionWidget::Construct(const FArguments& InArgs)
{
	...
	auto GetParentWidget = [this]()
	{
		...
		[
			...
			[
				...
				.Text(LOCTEXT("PreSelectedParent_Label", "Pre Selected Parent"))
			]
		]
		...
		[
			...
			{
				const bool bHasAnyNotConstrainable = InAssets.ContainsByPredicate([](const FAssetData& Data)
				{
					return !IsActorConstrainable(Cast<AActor>(Data.GetAsset()));	
				});
				return !bHasAnyNotConstrainable;
			})
			.OnAssetsDropped_Lambda( [this](const FDragDropEvent&, TArrayView<FAssetData> InAssets)
			{
				UpdateParent(InAssets.IsEmpty() ? nullptr : Cast<AActor>(InAssets[0].GetAsset()));
			})
			[
				SNew(SHorizontalBox)
											
				// actor chooser widget
				+ SHorizontalBox::Slot()
				.Padding(FMargin(4.0f,2.0f))
				.HAlign(HAlign_Right)
				[
					GetActorChooserWidget().ToSharedRef()
				]
		...
		[
			...
			[
				...
				.ToolTipText(LOCTEXT("ResetParent_Tooltip", "Reset parent."))
				[
					SNew(SButton)
					...
					{
						return IsParentValid() ? EVisibility::Visible : EVisibility::Hidden;
					})
					.ContentPadding(0.0f)
					.IsFocusable(false)
					[ 
	...
	auto GetConstraintListWidget = [this]()
	{
	...
	auto GetBakingWidget = [this]()
	{
		...
		[
			...
			.ToolTipText(LOCTEXT("BakeConstraintButtonToolTip", "Bake selected constraints"))
		];
	};
	...
	[
		...
		[
			...
			[
				...
				// constraints list widget
				+ SVerticalBox::Slot()
				.AutoHeight()
				.Padding(FMargin(0.0f, 0.f))
				[
					GetConstraintListWidget()
				]
...
TSharedRef<SWidget> SConstraintsEditionWidget::OnGetParentMenuContent()
{
	static constexpr bool bAllowClear = true, bAllowPicking = false, bUseSelected = true, bShowTransient = true;

	FOnShouldFilterActor FilterActorDelegate = FOnShouldFilterActor::CreateLambda([](const AActor* InActor)
	{
		return UE::Private::IsActorConstrainable(InActor);
	});
	
	FOnActorSelected SelectedActorDelegate = FOnActorSelected::CreateLambda([this](AActor* InActor)
	{
		UpdateParent(InActor);
	});

	FSimpleDelegate OnCloseDelegate = FSimpleDelegate::CreateLambda([this]
	{
		GetActorChooserWidget()->SetIsOpen(false);
	});

	FSimpleDelegate UseSelectedDelegate = FSimpleDelegate::CreateLambda([]{ });
	
	return PropertyCustomizationHelpers::MakeActorPickerWithMenu(
		ConstrainableParent.Actor.Get(),
		bAllowClear,
		bAllowPicking,
		FilterActorDelegate,
		SelectedActorDelegate,
		OnCloseDelegate,
		UseSelectedDelegate,
		bUseSelected,
		bShowTransient);
...
void SConstraintsEditionWidget::OnParentMenuOpenChanged(bool bOpened)
{
	if (!bOpened)
	{
		GetActorChooserWidget()->SetMenuContent(SNullWidget::NullWidget);
	}
}

TSharedPtr<SComboButton> SConstraintsEditionWidget::GetActorChooserWidget()
{
	if (!ActorSelectionButtons.IsValid())
	{
		auto GetParentLabel = [this]()
		{
			FString Label;
			if (ConstrainableParent.Actor.IsValid())
			{
				Label = ConstrainableParent.Actor->GetActorLabel();
				if (ConstrainableParent.Socket != NAME_None)
				{
					Label = Label + ":" + ConstrainableParent.Socket.ToString();
				}
			}
			return Label;
		};
		
		SAssignNew(ActorSelectionButtons, SComboButton)
		.ToolTipText_Lambda([this, GetParentLabel]()
		{
			if (ConstrainableParent.Actor.IsValid())
			{
				const FString Tooltip = FString::Printf(TEXT("'%s' will be used as the constraint's parent."), *GetParentLabel());
				return FText::FromString(Tooltip);
			}
			
			return LOCTEXT("SelectParent_Tooltip", "Select a parent actor.");
		})
		.OnGetMenuContent(this, &SConstraintsEditionWidget::OnGetParentMenuContent)
		.OnMenuOpenChanged(this, &SConstraintsEditionWidget::OnParentMenuOpenChanged)
		.ButtonContent()
		[
			SNew(SHorizontalBox)
			...
			[
				...
				SNew(STextBlock)
				.Font(FAppStyle::Get().GetFontStyle("PropertyWindow.NormalFont"))
				.Text_Lambda([this, GetParentLabel]()
				{
					if (ConstrainableParent.Actor.IsValid())
					{
						return FText::FromString(GetParentLabel()); 
					}
					return LOCTEXT("SelectConstraintParent", "Select Parent");
				})
			]
		];
	}
	
	return ActorSelectionButtons;
}

TSharedRef<SWidget> SConstraintsEditionWidget::GetUseSelectedActorWidget()
{
	static constexpr bool bIsActor = true, bEnabled = true;
	...
	[
		PropertyCustomizationHelpers::MakeUseSelectedButton(
			...
			{
				const TArray<AActor*> Actors = UE::Private::GetCurrentSelection();
				UpdateParent(Actors.IsEmpty() ? nullptr : Actors[0]);
			}),
			LOCTEXT("SetFromSelection", "Set from current selection."),
			bEnabled, bIsActor)
	];
}
...
void SConstraintsEditionWidget::MoveItemUp(const TSharedPtr<FEditableConstraintItem>& Item)
{
	...
	{
		UWorld* World = UE::Private::GetCurrentWorld();
		if (!IsValid(World))
		{
			return;
		}
...
void SConstraintsEditionWidget::UpdateSequencer()
{
	if (!WeakSequencer.IsValid())
	{
		WeakSequencer = UE::Private::GetSequencerChecked();
	}
}
...
void SConstraintBakeWidget::Construct(const FArguments& InArgs)
{
	...
	DetailsView->GetDetailsView()->RegisterInstancedCustomPropertyTypeLayout("FrameNumber",
		FOnGetPropertyTypeCustomizationInstance::CreateSP(Sequencer.ToSharedRef(), &ISequencer::MakeFrameNumberDetailsCustomization));
