		...
		if(const UModularRig* ModularRig = Cast<UModularRig>(ControlRigModel->ControlRigBeingDebuggedPtr.Get()))
		{
			if (const URigHierarchy* Hierarchy = ModularRig->GetHierarchy())
			{
				...
				for(const FRigElementKey& Connector : Connectors)
				{
					...
					const FText ConnectorShortestPath = Hierarchy->GetDisplayNameForUI(Connector, EElementNameDisplayMode::ForceLong);
...
void FControlRigSchematicModel::SetEditor(const TSharedRef<IControlRigBaseEditor>& InEditor)
{
...
void FControlRigSchematicModel::UpdateElementKeyLinks()
{
	...
	const TSharedPtr<IControlRigBaseEditor> Editor = ControlRigEditor.Pin();
	if(!Editor.IsValid())
	{
		return;
	}
				...
				if (const URigHierarchy* Hierarchy = ControlRigBeingDebuggedPtr->GetHierarchy())
				{
					...
					for(const FRigElementKey& ConnectorKey : Connectors)
					{
						if(const FSchematicGraphNode* ConnectorNode = FindElementKeyNode(ConnectorKey))
						{
							...
							if(NameString.StartsWith(InModule->GetElementPrefix(), ESearchCase::IgnoreCase))
							{
								const FString LocalName = NameString.Mid(InModule->GetElementPrefix().Len());
								if(!LocalName.Contains(FRigHierarchyModulePath::ModuleNameSuffix))
								{
									UpdateConnector(ConnectorKey);
								}
							}
...
FVector2d FControlRigSchematicModel::GetPositionForNode(const FSchematicGraphNode* InNode) const
{
	if(const FControlRigSchematicRigElementKeyNode* Node = Cast<FControlRigSchematicRigElementKeyNode>(InNode))
	{
		if(!InNode->HasParentNode())
		{
			...
			if(Key.IsValid())
			{
				if(const TSharedPtr<IControlRigBaseEditor> Editor = ControlRigEditor.Pin())
				{
					if(Editor.IsValid())
					{
	...
	if(ControlRigBlueprint.IsValid())
	{
		if(const FControlRigSchematicRigElementKeyNode* ElementKeyNode = Cast<FControlRigSchematicRigElementKeyNode>(InNode))
		{
			...
			if(!Connectors.IsEmpty())
			{
				...
				for(const FRigElementKey Connector : Connectors)
				{
					FText Label = ModularRig->GetHierarchy()->GetDisplayNameForUI(Connector, EElementNameDisplayMode::ForceShort);
...
TArray<FRigElementKey> FControlRigSchematicModel::GetElementKeysFromDragDropEvent(const FDragDropOperation& InDragDropOperation, const UControlRig* InControlRig)
{
	...
	if(InDragDropOperation.IsOfType<FRigElementHierarchyDragDropOp>())
	{
		...
		const TArray<FRigHierarchyKey>& HierarchyKeys = RigDragDropOp->GetElements();
		TArray<FRigElementKey> Elements;
		Elements.Reserve(HierarchyKeys.Num());
		for(const FRigHierarchyKey& HierarchyKey : HierarchyKeys)
		{
			if(HierarchyKey.IsElement())
			{
				Elements.Add(HierarchyKey.GetElement());
			}
		}
		return Elements;
	}
	...
	if(InDragDropOperation.IsOfType<FModularRigModuleDragDropOp>())
	{
		...
		const TArray<FName> ModuleNames = ModuleDropOp->GetModules();
		...
		for (const FName& ModulePathOrConnectorName : ModuleNames)
		{
			...
			if(const UModularRig* ModularRig = Cast<UModularRig>(InControlRig))
			{
				if(const FRigModuleInstance* ModuleInstance = ModularRig->FindModule(ModulePathOrConnectorName))
				{
					for(const FRigConnectorElement* Connector : Connectors)
					{
						...
						if(ModuleInstance->Name == ConnectorModuleName)
						{
							if(Connector->IsPrimary())
							{
								DraggedKeys.Add(Connector->GetKey());
			...
			if(UControlRigBlueprint* AssetBlueprint = Cast<UControlRigBlueprint>(AssetData.GetAsset()))
			{
				FFunctionGraphTask::CreateAndDispatchWhenReady([this, AssetBlueprint, TargetKeys]()
				{
					if (UModularRigController* Controller = ControlRigBlueprint->GetModularRigController())
					{
						...
						{
							const FName DesiredModuleName = Controller->GetSafeNewName(FRigName(AssetBlueprint->RigModuleSettings.Identifier.Name));
							const FName ModuleName = Controller->AddModule(DesiredModuleName, AssetBlueprint->GetControlRigClass(), NAME_None);
			...
			if (UModularRigController* Controller = ControlRigBlueprint->GetModularRigController())
			{
				...
				for (const FName& ModulePathOrConnectorName : ModuleNames)
				{
					...
					if(const FRigModuleReference* Module = Controller->FindModule(ModulePathOrConnectorName))
					{
						...
						{
							const FName ConnectorModuleName = Hierarchy->GetModuleFName(Connector->GetKey());
							if(Module->Name == ConnectorModuleName)
							{
								if(Connector->IsPrimary())
								{
...
void FControlRigSchematicModel::OnShowCandidatesForConnector(const FRigElementKey& InConnectorKey)
{
	...
	const FName ModuleName = Hierarchy->GetModuleFName(InConnectorKey);
	const FRigModuleInstance* ModuleInstance = ModularRig->FindModule(ModuleName);
	if (!ModuleInstance)
	{
		return;
	}
...
void FControlRigSchematicModel::OnHierarchyModified(ERigHierarchyNotification InNotif, URigHierarchy* InHierarchy, const FRigNotificationSubject& InSubject)
{
