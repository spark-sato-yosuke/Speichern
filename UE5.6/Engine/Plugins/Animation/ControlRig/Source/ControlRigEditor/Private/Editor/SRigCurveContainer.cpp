...
void SRigCurveContainer::Construct(const FArguments& InArgs, TSharedRef<IControlRigBaseEditor> InControlRigEditor)
{
...
void SRigCurveContainer::OnEditorClose(IControlRigBaseEditor* InEditor, UControlRigBlueprint* InBlueprint)
{
	if (InBlueprint)
	{
...
void SRigCurveContainer::OnHierarchyModified(ERigHierarchyNotification InNotif, URigHierarchy* InHierarchy, const FRigNotificationSubject& InSubject)
{
	if (bIsChangingRigHierarchy)
	{
		return;
...
URigHierarchy* SRigCurveContainer::GetInstanceHierarchy() const
{
	if (ControlRigEditor.IsValid())
	{
		UControlRig* ControlRig = ControlRigEditor.Pin()->GetControlRig();
		if (ControlRig)
		{
