...
static FKeyHandle SequencerOpenSpaceSwitchDialog(UControlRig* ControlRig, TArray<FRigElementKey> SelectedControls,ISequencer* Sequencer, FMovieSceneControlRigSpaceChannel* Channel, UMovieSceneSection* SectionToKey, FFrameNumber Time)
{
	...
	.OnSpaceListChanged_Lambda([SelectedControls, ControlRig](URigHierarchy* InHierarchy, const FRigElementKey& InControlKey, const TArray<FRigElementKeyWithLabel>& InSpaceList)
	{
	...
	FFrameNumber TimeOfDeletion)
{
	...
	{															//if that's the case we assume the user may not want to compensate
		return;
	}
	FMovieSceneControlRigSpaceBaseKey ExistingValue, PreviousValue;
	using namespace UE::MovieScene;
	...
	bool bCompPreviousTick)
{
	...
	if (!ControlRig || ControlRig->IsEvaluating())
	{
		// avoid reentrant evaluation: space switching evaluates the rig at T-1, but this must not happen if it's already being evaluated.
		return;
	}
