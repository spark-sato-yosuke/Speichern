...
TAutoConsoleVariable<bool> CVarControlRigDisableTickDependencyCheck(
	TEXT("ControlRig.DisableTickDependencyCheck"),
	false,
	TEXT("Disables tick dependency checking, which prevents cycles. Use carefully.\n")
	);
...
FTransactionallySafeCriticalSection gPendingSkeletalMeshesLock;
...
void UControlRigComponent::BeginDestroy()
{
	...
	UE::TScopeLock Lock(gPendingSkeletalMeshesLock);
...
void UControlRigComponent::Initialize()
{
...
#if WITH_EDITOR
	TickDependencyErrorMessages.Reset();
	...
	for (const FControlRigComponentMappedElement& MappedElement : MappedElements)
	{
		if (MappedElement.SceneComponent)
		{
...
#if WITH_EDITOR
			FString TickDependencyErrorMessage;
			FString* TickDependencyErrorMessagePtr = &TickDependencyErrorMessage;
#else
			static FString* TickDependencyErrorMessagePtr = nullptr;
#endif
			if (MappedElement.Direction == EControlRigComponentMapDirection::Output)
			{
				if(AllowsTickDependency(MappedElement.SceneComponent, this, TickDependencyErrorMessagePtr))
				{
			...
			{
				if(AllowsTickDependency(this, MappedElement.SceneComponent, TickDependencyErrorMessagePtr))
				{
...
#if WITH_EDITOR
			if(!TickDependencyErrorMessage.IsEmpty())
			{
				// only throw the error message if the user has picked a particular component to set up the dependency for
				if(!MappedElement.ComponentReference.ComponentProperty.IsNone() || MappedElement.ComponentReference.OtherActor.IsValid())
				{
					if(!TickDependencyErrorMessages.Contains(TickDependencyErrorMessage))
					{
						UE_LOG(LogControlRig, Warning, TEXT("%s"), *TickDependencyErrorMessage);
						TickDependencyErrorMessages.Add(TickDependencyErrorMessage);
					}
				}
			}
#endif
...
bool UControlRigComponent::HasTickDependency(const UObject* InDependent, const UObject* InDependency, FString* OutErrorMessage)
{
	TSet<const UObject*> Visited;
	return HasTickDependency_Impl(InDependent, InDependency, Visited, OutErrorMessage);
}

bool UControlRigComponent::HasTickDependency_Impl(const UObject* InDependent, const UObject* InDependency, TSet<const UObject*>& InOutVisited, FString* OutErrorMessage)
{
	...
	if(InOutVisited.Contains(InDependent))
	{
		return false;
	}
	InOutVisited.Add(InDependent);
	
	if(const USceneComponent* DependencyComponent = Cast<USceneComponent>(InDependency))
	{
		if(const USceneComponent* DependentComponent = Cast<USceneComponent>(InDependent))
		{
			if(const AActor* DependencyActor = DependencyComponent->GetOwner())
			{
				if(const AActor* DependentActor = DependentComponent->GetOwner())
				{
					if(const USceneComponent* DependentParentComponent = DependentComponent->GetAttachParent())
					{
						if(HasTickDependency_Impl(DependentParentComponent, InDependency, InOutVisited, OutErrorMessage))
						{
#if WITH_EDITOR
							if(DependencyActor == DependentActor)
							{
								if(DependentParentComponent == InDependency)
								{
									if(OutErrorMessage)
									{
										...
										static constexpr TCHAR Format[] = TEXT("The dependency '%s' is a parent component of dependent '%s'.");
										*OutErrorMessage += FString::Printf(Format, *InDependency->GetPathName(), *InDependent->GetPathName());
									}
								}
							}
#endif
		...
		else if(const AActor* DependencyActor = Cast<AActor>(InDependency))
		{
			if(DependencyActor->GetComponents().Contains(DependentComponent))
			{
#if WITH_EDITOR
				if(OutErrorMessage)
				{
					...
					static constexpr TCHAR Format[] = TEXT("The actor '%s' owns dependent component '%s'.");
					*OutErrorMessage += FString::Printf(Format, *DependencyActor->GetPathName(), *InDependent->GetPathName());
				}
#endif
				return true;
			}
	...
	// iterate all existing tick dependency components
	if(const UActorComponent* DependentActorComponent = Cast<UActorComponent>(InDependent))
	{
		const TArray<struct FTickPrerequisite>& Prerequisites = DependentActorComponent->PrimaryComponentTick.GetPrerequisites();
		for(const struct FTickPrerequisite& Prerequisite : Prerequisites)
		{
			if(Prerequisite.PrerequisiteObject.IsValid())
			{
				if(const UObject* TickDependency = Prerequisite.PrerequisiteObject.Get())
				{
					if(TickDependency == InDependency)
					{
#if WITH_EDITOR
						if(OutErrorMessage)
						{
							...
							static constexpr TCHAR Format[] = TEXT("The dependency '%s' is a prerequisite of dependent'%s'.");
							*OutErrorMessage += FString::Printf(Format, *InDependency->GetPathName(), *InDependent->GetPathName());
						}
#endif
						return true;
					...
					if(HasTickDependency_Impl(TickDependency, InDependency, InOutVisited, OutErrorMessage))
					{
#if WITH_EDITOR
						if(OutErrorMessage)
						{
							...
							static constexpr TCHAR Format[] = TEXT("The dependency '%s' is a indirect prerequisite of dependent '%s'.");
							*OutErrorMessage += FString::Printf(Format, *InDependency->GetPathName(), *InDependent->GetPathName());
						}
#endif
						return true;
...
bool UControlRigComponent::AllowsTickDependency(const UObject* InDependent, const UObject* InDependency, FString* OutErrorMessage)
{
	if (CVarControlRigDisableTickDependencyCheck.GetValueOnAnyThread())
	{
		return true;
	}
	...
	if(InDependent == InDependency)
	{
		...
		if(OutErrorMessage)
		{
			...
			static constexpr TCHAR Format[] = TEXT("The dependency and dependent '%s' are identical.");
			*OutErrorMessage += FString::Printf(Format, *InDependent->GetPathName());
		}
#endif
		return false;
	}
	
	// don't allow to depend on an inner
	if(InDependency->IsInOuter(InDependent))
	{
#if WITH_EDITOR
		if(OutErrorMessage)
		{
			...
			static constexpr TCHAR Format[] = TEXT("The dependency '%s' is within the dependent '%s'.");
			*OutErrorMessage += FString::Printf(Format, *InDependency->GetPathName(), *InDependent->GetPathName());
		}
#endif
		return false;
	...
	if(HasTickDependency(InDependent, InDependency, OutErrorMessage))
	{
#if WITH_EDITOR
		if(OutErrorMessage)
		{
			...
			static constexpr TCHAR Format[] = TEXT("The dependent '%s' already depends on dependency '%s'.");
			*OutErrorMessage += FString::Printf(Format, *InDependent->GetPathName(), *InDependency->GetPathName());
		}
#endif
		return false;
	}
	
	if(HasTickDependency(InDependency, InDependent, OutErrorMessage))
	{
#if WITH_EDITOR
		if(OutErrorMessage)
		{
			...
			static constexpr TCHAR Format[] = TEXT("The dependency '%s' depends on dependent '%s'. This would cause a cycle.");
			*OutErrorMessage += FString::Printf(Format, *InDependency->GetPathName(), *InDependent->GetPathName());
		}
#endif
		return false;
