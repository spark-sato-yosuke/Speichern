...
FName UFKControlRig::GetControlName(const FName& InName, const ERigElementType& InType)
{
	if (InName != NAME_None)
	{
		...
		{
			using NameToControlMappingCache = TStaticArray<TMap<FName, FName>, 2>;
			UE_AUTORTFM_DECLARE_THREAD_LOCAL_VAR(NameToControlMappingCache, NameToControlMappings);
...
FName UFKControlRig::GetControlTargetName(const FName& InName, const ERigElementType& InType)
{
	if (InName != NAME_None)
	{
		...
		using NameToTargetMappingCache = TStaticArray<TMap<FName, FName>, 2>;
		UE_AUTORTFM_DECLARE_THREAD_LOCAL_VAR(NameToTargetMappingCache, NameToTargetMappings);
		...
		else
		{
			...
			FString ControlTargetString = FString::ConstructFromPtrSize(*ScratchString, StartPostFix != INDEX_NONE ? StartPostFix : ScratchString.Len());

			// Force lower-case names
			ControlTargetString.ToLowerInline();
...
bool UFKControlRig::Execute_Internal(const FName& InEventName)
{
	...
	if(URigHierarchy* Hierarchy = GetHierarchy())
	{
		...
		Hierarchy->bRecordTransformsAtRuntime = false;
	}
#endif	
