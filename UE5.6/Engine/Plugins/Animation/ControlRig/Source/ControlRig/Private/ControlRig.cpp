...
UControlRig::UControlRig(const FObjectInitializer& ObjectInitializer)
	...
	, bEvaluationTriggeredFromInteraction(false)
...
#if WITH_EDITOR
	if (!HasAnyFlags(RF_ClassDefaultObject) && !IsEngineExitRequested())
	{
		if(const UClass* Class = GetClass())
		{
			if(IsValid(Class))
			{
				if(UControlRig* CDO = Class->GetDefaultObject<UControlRig>())
				{
...
int32 UControlRig::NumOverrideAssets() const
{
	return OverrideAssets.Num();
}

const UControlRigOverrideAsset* UControlRig::GetLastOverrideAsset() const
{
	return OverrideAssets.IsEmpty() ? nullptr : OverrideAssets.Last();
}

UControlRigOverrideAsset* UControlRig::GetLastOverrideAsset()
{
	return OverrideAssets.IsEmpty() ? nullptr : OverrideAssets.Last();
}

const UControlRigOverrideAsset* UControlRig::GetOverrideAsset(int32 InIndex) const
{
	return OverrideAssets.IsValidIndex(InIndex) ? OverrideAssets[InIndex] : nullptr;
}

UControlRigOverrideAsset* UControlRig::GetOverrideAsset(int32 InIndex)
{
	return OverrideAssets.IsValidIndex(InIndex) ? OverrideAssets[InIndex] : nullptr;
}

int32 UControlRig::LinkOverrideAsset(UControlRigOverrideAsset* InAsset)
{
	if (!InAsset)
	{
		return INDEX_NONE;
	...
	if (OverrideAssets.Contains(InAsset))
	{
		return INDEX_NONE;
	}
	
	const int32 Index = OverrideAssets.Add(InAsset);
	InAsset->OnChanged().AddUObject(this, &UControlRig::HandleOverrideChanged);
	OverrideAssetsChangedDelegate.Broadcast(this);
	RequestConstruction();
	return Index;
}

bool UControlRig::UnlinkOverrideAsset(UControlRigOverrideAsset* InAsset)
{
	const int32 NumRemoved = OverrideAssets.Remove(const_cast<UControlRigOverrideAsset*>(InAsset));
	if (NumRemoved == 0)
	{
		return false;
	}
	...
	if (InAsset)
	{
		InAsset->OnChanged().RemoveAll(this);
	}
...
bool UControlRig::UnlinkAllOverrideAssets()
{
	if (OverrideAssets.IsEmpty())
	{
		return false;
	}

	for (const TObjectPtr<UControlRigOverrideAsset>& OverrideAsset : OverrideAssets)
	{
		OverrideAsset->OnChanged().RemoveAll(this);
	}
...
bool UControlRig::IsLinkedToOverrideAsset(const UControlRigOverrideAsset* InOverrideAsset) const
{
	return OverrideAssets.Contains(InOverrideAsset);
}

void UControlRig::ApplyElementOverrides()
{
	...
	TSet<FRigControlElement*> ControlsChanged;
	
	for (const UControlRigOverrideAsset* OverrideAsset : OverrideAssets)
	{
		if (!OverrideAsset)
		{
			continue;
		...
		for (const FControlRigOverrideValue& Override : OverrideAsset->Overrides)
		{
			const FRigElementKey Key(Override.GetSubjectKey(), ERigElementType::Control);
			...
			const FProperty* ShapeTransformProperty =
				FRigControlSettings::StaticStruct()->FindPropertyByName(GET_MEMBER_NAME_CHECKED(FRigControlSettings, ShapeTransform));
			const bool bIsTransformShapeOverride = Override.ContainsProperty(ShapeTransformProperty);
			if (bIsTransformShapeOverride)
			{
				ControlElement->Settings.ShapeTransform = Hierarchy->GetControlShapeTransform(ControlElement, ERigTransformType::InitialLocal);
			}

			// in the future let's avoid this check know on a container level
			// if this container represents changes to the control
			if (!Override.CopyToSubject(ControlElement, FRigControlElement::StaticStruct()))
			{
				continue;
			}
			ControlsChanged.Add(ControlElement);

			if (bIsTransformShapeOverride)
			{
				Hierarchy->SetControlShapeTransformByIndex(ControlElement->GetIndex(), ControlElement->Settings.ShapeTransform, true);
				Hierarchy->SetControlShapeTransformByIndex(ControlElement->GetIndex(), ControlElement->Settings.ShapeTransform, false);
			}
		}
	}
}

void UControlRig::HandleOverrideChanged(const UControlRigOverrideAsset* InOverrideAsset)
{
	if (!bSuspendOverrideAssetChangedDelegate&& InOverrideAsset && OverrideAssets.Contains(InOverrideAsset))
	{
		RequestConstruction();
	}
}
	...
	if (bIsAdditive)
	{
		...
		for (TPair<FRigElementKey, FRigSwitchParentInfo>& Pair : SwitchParentValues)
		{
			if (FRigBaseElement* Element = Hierarchy->Find(Pair.Key))
			{
				...
				{
#if WITH_EDITOR
					FRigVMDependenciesProvider DependencyProvider(Hierarchy, GetVM());
					Hierarchy->SwitchToParent(Element, NewParent, Pair.Value.bInitial, Pair.Value.bAffectChildren, DependencyProvider, nullptr);
#else
...
void UControlRig::InitializeFromCDO()
{
	...
	if (!HasAnyFlags(RF_ClassDefaultObject))
	{
		if (!IsRigModuleInstance())
		{
			...
			// Danny TODO this could copy the physics solvers from the CDO, but they are transient
			// objects that only exist through construction, so I think we just need to start afresh
			// each time. 
			PhysicsSimulations.Empty();
			...
			Hierarchy->Notify(ERigHierarchyNotification::HierarchyReset, {});
...
UControlRig::FAnimAttributeContainerPtrScope::FAnimAttributeContainerPtrScope(UControlRig* InControlRig,
	UE::Anim::FMeshAttributeContainer& InExternalContainer)
{
...
bool UControlRig::AllConnectorsAreResolved(FString* OutFailureReason, FRigElementKey* OutConnector) const
{
	if(const URigHierarchy* Hierarchy = GetHierarchy())
	{
		...
		TMap<FRigElementKey, FRigElementKeyRedirector::FKeyArray> ConnectionMap;
		for(const FRigModuleConnector& Connector : Connectors)
		{
			...
			if(const FRigElementKeyRedirector::FCachedKeyArray* Cache = ElementKeyRedirector.Find(ConnectorKey))
			{
				for(const FCachedRigElement& CachedRigElement : *Cache)
				{
					...
					else
					{
						if(OutFailureReason)
						{
							...
							*OutFailureReason = FString::Printf(Format, *GetRigModulePrefix(), *Connector.Name, *CachedRigElement.GetKey().ToString());
						}
						if(OutConnector)
						{
				...
				ConnectionMap.Add(ConnectorKey, TargetKeys);
			}
			else
			{
				if(OutFailureReason)
...
bool UControlRig::Execute(const FName& InEventName)
{
	...
	{
		// only reset the draw interface if we are not running the rig during interaction with simulation present. 
		if(InEventName != FRigUnit_InteractionExecution::EventName || !bEvaluationTriggeredFromInteraction || !ContainsSimulation())
		{
			DrawInterface.Reset();
		}
	}
	...
	if (bIsConstructionEvent)
	{
		...
		// remember the previous selection (copy here since it may change during the code path below)
		const TArray<FRigHierarchyKey> PreviousSelection = Hierarchy->GetSelectedHierarchyKeys();
		...
		const bool bConstructionModeEnabled = IsConstructionModeEnabled();
		{
			...
			TGuardValue<bool> DisableSelectionNotifications(Hierarchy->GetController(true)->bSuspendSelectionNotifications, true);
			{
				...
				if(!bPostConstructionEventInQueue)
				{
					...
					EmitPostConstructionEventFinished();
				}
			...
			Hierarchy->GetController()->SetHierarchySelection(PreviousSelection);
		...
		const TArray<FRigHierarchyKey>& CurrentSelection = Hierarchy->GetSelectedHierarchyKeys();
		for(const FRigHierarchyKey& SelectedHierarchyKey : CurrentSelection)
		{
			if(!PreviousSelection.Contains(SelectedHierarchyKey))
			{
				if(SelectedHierarchyKey.IsElement())
				{
					if(const FRigBaseElement* SelectedElement = Hierarchy->Find(SelectedHierarchyKey.GetElement()))
					{
						Hierarchy->Notify(ERigHierarchyNotification::ElementSelected, SelectedElement);
					}
				}
				if(SelectedHierarchyKey.IsComponent())
				{
					if(const FRigBaseComponent* SelectedComponent = Hierarchy->FindComponent(SelectedHierarchyKey.GetComponent()))
					{
						Hierarchy->Notify(ERigHierarchyNotification::ComponentSelected, SelectedComponent);
					}
				}
			}
		}
		for(const FRigHierarchyKey& PreviouslySelectedKey : PreviousSelection)
		{
			if(!CurrentSelection.Contains(PreviouslySelectedKey))
			{
				if(PreviouslySelectedKey.IsElement())
				{
					if(const FRigBaseElement* SelectedElement = Hierarchy->Find(PreviouslySelectedKey.GetElement()))
					{
						Hierarchy->Notify(ERigHierarchyNotification::ElementDeselected, SelectedElement);
					}
				}
				if(PreviouslySelectedKey.IsComponent())
				{
					if(const FRigBaseComponent* SelectedComponent = Hierarchy->FindComponent(PreviouslySelectedKey.GetComponent()))
					{
						Hierarchy->Notify(ERigHierarchyNotification::ComponentDeselected, SelectedComponent);
					}
				}
			}
		}
...
{
	RequestRunOnceEvent(FRigUnit_PrepareForExecution::EventName);
	RequestRunOnceEvent(FRigUnit_PostPrepareForExecution::EventName);
}
...
void UControlRig::AdaptEventQueueForEvaluate(TArray<FName>& InOutEventQueueToRun)
{
	if(IsReplayEnabled())
	{
		...
		// find all events at this particular frame
		const int32 NumTimes = Replay->InputTracks.GetNumTimes();

		int32 TimeIndex = GetReplayTimeIndex();
		if(TimeIndex >= NumTimes || TimeIndex < 0)
		{
			TimeIndex = 0;
			SetReplayTimeIndex(TimeIndex);
		}
		const double TimeAtIndex = Replay->InputTracks.GetAbsoluteTime(TimeIndex);

		InOutEventQueueToRun.Reset();
		while(TimeIndex < NumTimes)
		{
			const FName EventFromReplay = Replay->InputTracks.GetRigVMEvent(TimeIndex);
			InOutEventQueueToRun.Add(EventFromReplay);

			// also queue up the next event to run immediately if it was recorded at the same time
			if(TimeIndex != NumTimes - 1)
			{
				if(FMath::IsNearlyEqual(TimeAtIndex, Replay->InputTracks.GetAbsoluteTime(++TimeIndex)))
				{
					continue;
				}
			}
	...
	if(InteractionType != (uint8)EControlRigInteractionType::None)
	{
		...
		if(ContainsSimulation() && bEvaluationTriggeredFromInteraction)
		{
			InOutEventQueueToRun.Remove(FRigUnit_BeginExecution::EventName);
			InOutEventQueueToRun.Remove(FRigUnit_PreBeginExecution::EventName);
			InOutEventQueueToRun.Remove(FRigUnit_PostBeginExecution::EventName);
		}
	}
}
...
{
	DECLARE_SCOPE_HIERARCHICAL_COUNTER_FUNC()
	UE_RIGVM_ARCHIVETRACE_SCOPE(Ar, FString::Printf(TEXT("UControlRig(%s)"), *GetName()));
	...
	// We are no longer storing the physics solver descriptions as part of the rig, and the struct
	// definitions have been removed. In addition, the array would have been empty when serialized
	// with the affected version, so we just need to absorb the data into a temporary array.
	if (Ar.CustomVer(FControlRigObjectVersion::GUID) >= FControlRigObjectVersion::ControlRigStoresPhysicsSolvers)
	{
		TArray<FRigPhysicsSolverDescription> TempSolvers;
		Ar << TempSolvers;
		UE_RIGVM_ARCHIVETRACE_ENTRY(Ar, TEXT("PhysicsSolvers"));
	}
	else
	{
		PhysicsSimulations.Reset();
	}
...
const FString& UControlRig::GetRigModulePrefix() const
{
	if(IsRigModule())
	{
		if(IsRigModuleInstance() || GetRigModuleSettings().Identifier.Name.IsEmpty())
		{
			RigModulePrefix = GetFName().ToString() + FRigHierarchyModulePath::ModuleNameSuffix;
		}
		else
		{
			RigModulePrefix = GetRigModuleSettings().Identifier.Name + FRigHierarchyModulePath::ModuleNameSuffix;
		}
...
FRigControlElement* UControlRig::FindControl(const FName& InControlName) const
{
	...
	// look up the control based on the stored name. 
	const FRigElementKey ControlKey(InControlName, ERigElementType::Control);
	if(FRigControlElement* Control = DynamicHierarchy->Find<FRigControlElement>(ControlKey))
	{
		return Control;
	}
	...
	const FRigControlModifiedContext& Context, bool bSetupUndo, bool bPrintPythonCommnds, bool bFixEulerFlips)
{
	...
	if (bIsAdditive)
	{
		...
		ControlValues.Add(ControlElement->GetKey(), Info);
		if (Context.SetKey == EControlRigSetKey::Always && IsInGameThread())
		{
			// Force evaluation to set keys in game thread
			Evaluate_AnyThread();
		}
	}
	else
...
void UControlRig::SwitchToParent(const FRigElementKey& InElementKey, const FRigElementKey& InNewParentKey, bool bInitial, bool bAffectChildren)
{
	...
	{
#if WITH_EDITOR
		FRigVMDependenciesProvider DependenciesProvider(DynamicHierarchy, GetVM());
		DynamicHierarchy->SwitchToParent(InElementKey, InNewParentKey, bInitial, bAffectChildren, DependenciesProvider, nullptr);
#else
...
void UControlRig::SelectControl(const FName& InControlName, bool bSelect, bool bSetupUndo)
{
	if(DynamicHierarchy)
	{
		if(URigHierarchyController* Controller = DynamicHierarchy->GetController(true))
		{
			Controller->SelectElement(FRigElementKey(InControlName, ERigElementType::Control), bSelect, false, bSetupUndo);
		}
	}
}
...
bool UControlRig::ClearControlSelection(bool bSetupUndo)
{
	if(DynamicHierarchy)
	{
		if(URigHierarchyController* Controller = DynamicHierarchy->GetController(true))
		{
			return Controller->ClearSelection(bSetupUndo);
		}
	}
	return false;
}
...
void UControlRig::EmitPostConstructionEventFinished()
{
...
void UControlRig::SetBoneInitialTransformsFromSkeletalMeshComponent(USkeletalMeshComponent* InSkelMeshComp, bool bUseAnimInstance)
{
	...
	else if(InSkelMeshComp->GetSkeletalMeshAsset())
	{
	...
	const FName& InControlName) const
{
	...
	auto IsConstrainable = [ControlElement]()
	{
		if (!ControlElement)
		{
			return false;
	...
	if (ControlElement->GetFName() != InControlName)
	{
		CtrlHandle->ControlName = ControlElement->GetFName();
		
#if WITH_EDITOR
		const FRigElementKey ControlKey(InControlName, ERigElementType::Control);
		if (DynamicHierarchy->ReceivedNameSpaceBasedKeys.Contains(ControlKey))
		{
			UE_LOG(
				LogControlRig,
				Warning,
				TEXT("%s: control '%s' has been accessed using a namespace based key ('%s') when creating the control handle. Please consider updating your code."),
				*GetName(),
				*ControlElement->GetName(),
				*InControlName.ToString()
			);	
		}
#endif
	}
...
FRigPhysicsSimulationBase* UControlRig::GetPhysicsSimulation(const FRigComponentKey& InComponentKey)
{
	TSharedPtr<FRigPhysicsSimulationBase>* PhysicsSimulation = PhysicsSimulations.Find(InComponentKey);
	return PhysicsSimulation ? PhysicsSimulation->Get() : nullptr;
}

const FRigPhysicsSimulationBase* UControlRig::GetPhysicsSimulation(const FRigComponentKey& InComponentKey) const
{
	const TSharedPtr<FRigPhysicsSimulationBase>* PhysicsSimulation = PhysicsSimulations.Find(InComponentKey);
	return PhysicsSimulation ? PhysicsSimulation->Get() : nullptr;
}

bool UControlRig::RegisterPhysicsSimulation(TSharedPtr<FRigPhysicsSimulationBase> InPhysicsSimulation, const FRigComponentKey& InComponentKey)
{
	if(!InPhysicsSimulation.IsValid())
	{
		return false;
	}
	PhysicsSimulations.FindOrAdd(InComponentKey, InPhysicsSimulation);
	InPhysicsSimulation->PhysicsSolverComponentKey = InComponentKey;
	return true;
}
...
int32 UControlRig::GetReplayTimeIndex() const
{
	return ReplayTimeIndex;
}

void UControlRig::SetReplayTimeIndex(int32 InReplayTimeIndex)
{
	ReplayTimeIndex = InReplayTimeIndex;
}

void UControlRig::DisableReplay()
{
	Replay = nullptr;
}

bool UControlRig::IsReplayEnabled() const
{
	return GetReplayTimeIndex() != INDEX_NONE && Replay != nullptr;
}

void UControlRig::SetReplay(UControlRigReplay* InReplay)
{
	if(InReplay == nullptr)
	{
		DisableReplay();
...
bool UControlRig::ContainsSimulation() const
{
	return !PhysicsSimulations.IsEmpty();
}
