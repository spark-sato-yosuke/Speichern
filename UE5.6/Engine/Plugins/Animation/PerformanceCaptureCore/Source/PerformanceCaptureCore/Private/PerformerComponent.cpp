...
bool UPerformerComponent::SetControlledMesh(USkeletalMeshComponent* SkeletalMesh)
{
	if (SkeletalMesh)
	{
		if (SkeletalMesh->GetOwner() == this->GetOwner()) //Check the given skeletal mesh component belongs to the same actor as this performer component and is valid
		{
			ControlledSkeletalMesh.OverrideComponent = SkeletalMesh;
			bIsDirty=true; // Mark bIsDirty true so that next frame InitiateAnimation is called.
			return true;
		}
	}
