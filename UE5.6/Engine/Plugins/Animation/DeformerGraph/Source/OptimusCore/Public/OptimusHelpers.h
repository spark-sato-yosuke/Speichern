...
namespace Optimus
{
	...
	const TCHAR* GetKernelInternalNamespaceName();

	Expression::FParseResult ParseExecutionDomainExpression(
		const FString& InExpression,
		TWeakObjectPtr<const UOptimusComponentSource> InComponentSource
		);

	TVariant<bool, FText> IsExecutionDomainUnifiedDispatchOnly(
		const FString& InExpression,
		TWeakObjectPtr<const UOptimusComponentSource> InComponentSource
		);
	
	bool EvaluateExecutionDomainExpressionParseResult(
