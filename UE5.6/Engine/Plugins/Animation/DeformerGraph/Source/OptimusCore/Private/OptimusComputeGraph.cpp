...
#include "DataInterfaces/OptimusDataInterfaceHalfEdge.h"
...
void UOptimusComputeGraph::OnKernelCompilationComplete(int32 InKernelIndex, FComputeKernelCompileResults const& InCompileResults)
{
	...
	if (KernelToNode.IsValidIndex(InKernelIndex))
	{
		...
		if (UOptimusNode* Node = const_cast<UOptimusNode*>(KernelToNode[InKernelIndex].LoadSynchronous()))
		{
...
bool UOptimusComputeGraph::HasHalfEdgeDataInterface() const
{
	for (UComputeDataInterface* DataInterface : DataInterfaces)
	{
		if (DataInterface->IsA<UOptimusHalfEdgeDataInterface>())
		{
			return true;
		}
	}
