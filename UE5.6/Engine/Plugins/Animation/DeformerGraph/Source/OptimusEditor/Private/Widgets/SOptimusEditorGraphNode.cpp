...
public:	
		...
		TSharedPtr<ITableRow> InOptionalOwnerRow)
	{
		...
		if (bIsLeaf && bIsInput && bShowValueWidget)
		{
...
void SOptimusEditorGraphNode::UpdateGraphNode()
{
	if (GraphNode)
	{
		// Assign a tool tip before SGraphNode::UpdateGraphNode because
		// by default SGraphNode adds the raw name of the graph node to
		// the tool tip if the EdGraph is transient, which we don't want
		FText ToolTipText = GraphNode->GetTooltipText();
		TSharedRef<SToolTip> ToolTip = IDocumentation::Get()->CreateToolTip( ToolTipText, NULL, GraphNode->GetDocumentationLink(), GraphNode->GetDocumentationExcerptName() );
		SetToolTip(ToolTip);
	}
...
{
	const bool bShowValueWidget =
		InModelPin->GetDataDomain().IsSingleton() &&
		InModelPin->GetPropertyFromPin() != nullptr &&
		!InModelPin->GetDataType()->IsArrayType();
	...
	RowWidget->SetContent(
		SNew(SOptimusEditorGraphPinWidget, PinWidget.ToSharedRef(), bShowValueWidget, RowWidget)
		.PinLabel(this, &SOptimusEditorGraphNode::GetPinLabel, WeakPin)
	);
