	...
	) const
{
		...
		if (const UOptimusEditorGraph* Graph = Cast<UOptimusEditorGraph>(InGraph))
		{
			if (UOptimusNodeGraph* ModelGraph = Graph->GetModelGraph())
			{
				if (UOptimusDeformer* Deformer = Cast<UOptimusDeformer>(ModelGraph->GetCollectionRoot()))
				{
					...
					{
						FOptimusFunctionNodeGraphHeaderWithGuid Header = FunctionNodeGraph->GetHeaderWithGuid();
						...
						Action->AssetPath = Deformer;
						Action->FunctionGraphGuid = Header.FunctionGraphGuid;
	...
	// Public functions
	{
		...
		auto AddPublicFunctionAction = [&IoActionBuilder](const FAssetData& AssetData, const FOptimusFunctionNodeGraphHeaderWithGuid& Header)
		{
			...
			Action->AssetPath = AssetData.GetSoftObjectPath();
			Action->FunctionGraphGuid = Header.FunctionGraphGuid;
		...
		{
			bool bPublicFunctionImported = false;
			// New public function exports
			{
				FString PublicFunctionString = AssetData.GetTagValueRef<FString>(UOptimusDeformer::PublicFunctionsWithGuidAssetTagName);

				if (!PublicFunctionString.IsEmpty())
				{
					bPublicFunctionImported = true;
					FOptimusFunctionNodeGraphHeaderWithGuidArray PublicFunctionHeaderArray;

					FOptimusFunctionNodeGraphHeaderWithGuidArray::StaticStruct()->ImportText(*PublicFunctionString, &PublicFunctionHeaderArray, nullptr, PPF_None, nullptr, {});

					for (const FOptimusFunctionNodeGraphHeaderWithGuid& Header : PublicFunctionHeaderArray.Headers)
					{
						AddPublicFunctionAction(AssetData, Header);	
					}
				}	
			}
			
			// Old public function exports
			if (!bPublicFunctionImported)
			{
				...
				if (!PublicFunctionString.IsEmpty())
				{
					...
					for (const FOptimusFunctionNodeGraphHeader& Header : PublicFunctionHeaderArray.Headers)
					{
						...
						// Unfortunately we need to do a load here because we need to grab the latest Guid from the function graph
						if (UOptimusFunctionNodeGraph* FunctionNodeGraph = Header.GraphPath.LoadSynchronous())
						{
							FOptimusFunctionNodeGraphHeaderWithGuid HeaderWithGuid = FunctionNodeGraph->GetHeaderWithGuid();
							AddPublicFunctionAction(AssetData, HeaderWithGuid);
						}
					}
				}	
			}
	...
	// Comment nodes
	{
		TSharedPtr< FOptimusGraphSchemaAction_NewCommentNode> Action(
			new FOptimusGraphSchemaAction_NewCommentNode(
				{},
				FText::FromName(TEXT("Comment")),
				/* Tooltip */{}, 0, /* Keywords */{}
		));
...
{
	UOptimusEditorGraph* Graph = Cast<UOptimusEditorGraph>(InNode->GetGraph());

	return Graph->GetModelGraph()->RemoveNode(Graph->FindModelNodeFromGraphNode(InNode));
}
