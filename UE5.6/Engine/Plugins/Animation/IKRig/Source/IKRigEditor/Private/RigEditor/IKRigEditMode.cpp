...
void FIKRigEditMode::RenderBones(FPrimitiveDrawInterface* PDI)
{
	...
	// is the ik rig available?
	const FIKRigProcessor* ProcessorPtr = Controller->GetIKRigProcessor();
	if (!ProcessorPtr)
	{
		return;
	}

	// is the ik rig initialized?
	const FIKRigProcessor& CurrentProcessor = *ProcessorPtr;
...
void FIKRigEditMode::GetBoneColors(
	...
	const FIKRigProcessor& Processor,
	...
	TArray<FLinearColor>& OutBoneColors) const
{
	...
	switch (Controller->GetLastSelectedType())
	{
		...
		case EIKRigSelectionType::SolverStack:
		{
			...
			const UIKRigController* AssetController = Controller->AssetController;
			if(const FIKRigSolverBase* SelectedSolver = AssetController->GetSolverAtIndex(SelectedSolvers[0].Get()->IndexInStack))
			{
