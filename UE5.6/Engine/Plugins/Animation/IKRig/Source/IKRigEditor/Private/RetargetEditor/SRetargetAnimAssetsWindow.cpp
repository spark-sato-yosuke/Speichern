...
void SRetargetPoseViewport::Construct(const FArguments& InArgs)
{
	...
	FIKRetargetProcessor* Processor = TargetAnimInstance->GetRetargetProcessor();
	if (ensure(Processor))
	{
		Processor->Log.SetLogTarget(BatchContext->IKRetargetAsset);
	}
...
void SRetargetPoseViewport::SetSkeletalMesh(USkeletalMesh* InSkeletalMesh, ERetargetSourceOrTarget SourceOrTarget)
{
	...
	const double Offset = SourceComponent->Bounds.GetBox().Max.X + FMath::Abs(TargetComponent->Bounds.GetBox().Min.X);
...
void FProceduralRetargetAssets::AutoGenerateIKRetargetAsset()
{
	...
	FScopedReinitializeIKRetargeter Reinitialize(RetargetController, ERetargetRefreshMode::ProcessorAndOpStack);
	...
	// NOTE: this must be done BEFORE generating the retarget pose since it relies on the first op chain mapping
	RetargetController->RemoveAllOps();
	RetargetController->AddDefaultOps();

	// use only exact chain mapping in the auto-retarget (default is fuzzy which can map toes to fingers and other chaos)
	FIKRetargetFKChainsOp* FKChainOp = RetargetController->GetFirstRetargetOpOfType<FIKRetargetFKChainsOp>();
	constexpr bool bForceRemap = true;
	FKChainOp->ChainMapping.AutoMapChains(EAutoMapChainType::Exact, bForceRemap);
	...
	FIKRetargetRunIKRigOp* RunIKOp = RetargetController->GetFirstRetargetOpOfType<FIKRetargetRunIKRigOp>();
	RunIKOp->SetEnabled(false);
	
	// setup the root motion generator (but only if the skeleton has a root bone separate from the pelvis)
	FIKRetargetRootMotionOp* RootOp = RetargetController->GetFirstRetargetOpOfType<FIKRetargetRootMotionOp>();
	...
	if (TargetSkeletonRoot != TargetPelvis)
	{
		...
		{
			RootOp->Settings.RootMotionSource = ERootMotionSource::GenerateFromTargetPelvis;
			RootOp->Settings.RootHeightSource = ERootMotionHeightSource::SnapToGround;
			RootOp->Settings.bRotateWithPelvis = true;
			RootOp->Settings.bMaintainOffsetFromPelvis = true;
		}
	}
	else
	{
		// the pelvis IS the root, so we let the Pelvis op handle "root" motion
		RootOp->SetEnabled(false);
	}
	
	// setup a "Pin Bone Op" to pin IK bones to the new target locations
	RetargetController->AddRetargetOp(FIKRetargetPinBoneOp::StaticStruct());
	FIKRetargetPinBoneOp* PinBoneOp = RetargetController->GetFirstRetargetOpOfType<FIKRetargetPinBoneOp>();
	PinBoneOp->Settings.TranslationMode = EPinBoneTranslationMode::CopyGlobalPosition;
	PinBoneOp->Settings.RotationMode = EPinBoneRotationMode::CopyGlobalRotation;
	...
	{
		FPinBoneData PinBoneData;
		PinBoneData.BoneToCopyFrom.BoneName = BonesToPin.BoneToPinTo;
		PinBoneData.BoneToCopyTo.BoneName = BonesToPin.BoneToPin;
		PinBoneOp->Settings.BonesToPin.Add(PinBoneData);
	}
}
...
SRetargetAnimAssetsWindow::SRetargetAnimAssetsWindow()
{
	...
	// register log name
	Log.SetLogTarget(ProceduralAssets.Retargeter);
}
