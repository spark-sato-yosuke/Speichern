...
void FIKRigTreeElement::GetStruct(const UScriptStruct*& OutType, uint8*& OutMemory) const
{
	...
	case IKRigTreeElementType::SOLVERGOAL:
		if (const FIKRigSolverBase* SolverWithEffector = Controller->AssetController->GetSolverAtIndex(EffectorIndex))
		{
			return nullptr; // TODO SolverWithEffector->GetGoalSettings(EffectorGoalName);
		}
	default:
		return nullptr;
	}*/
...
TSharedRef<SWidget> SIKRigSkeletonRow::GenerateWidgetForColumn(const FName& ColumnName)
{
	...
	if (ColumnName == IKRigChainColumnName)
	{
		...
		const FSlateFontInfo Font = ChainName != "(Pelvis)" ? FAppStyle::Get().GetFontStyle("NormalFont") : FAppStyle::Get().GetFontStyle("BoldFont");
...
void SIKRigHierarchy::Construct(const FArguments& InArgs, TSharedRef<FIKRigEditorController> InEditorController)
{
    ...
    [
		...
		[
			SNew(SHorizontalBox)
			.Visibility_Lambda([this](){return IsSkeletonLoaded() ? EVisibility::Visible : EVisibility::Collapsed; })
		...
		[
			SNew(SHorizontalBox)
			.Visibility_Lambda([this](){return IsSkeletonLoaded() ? EVisibility::Collapsed : EVisibility::Visible; })
			...
			[
				...
				.OnClicked(FOnClicked::CreateSP(this, &SIKRigHierarchy::OnImportSkeletonClicked))
				.Text(LOCTEXT("ImportSkeletonLabel", "Import Hierarchy"))
				.IsEnabled_Lambda([this](){ return !IsSkeletonLoaded(); })
			]
		]
...
bool SIKRigHierarchy::IsElementConnectedToSolver(TWeakPtr<FIKRigTreeElement> InTreeElement, int32 SolverIndex)
{
	...
	const FIKRigSolverBase* Solver = AssetController->GetSolverAtIndex(SolverIndex);
	...
	if (TreeElement->ElementType == IKRigTreeElementType::BONE_SETTINGS)
	{
		...
		return Solver->HasSettingsOnBone(TreeElement->BoneSettingBoneName);
	}
...
bool SIKRigHierarchy::IsSkeletonLoaded() const
{
	const UIKRigController* AssetController = EditorController.Pin()->AssetController;
	return AssetController->GetAsset()->GetPreviewMesh() != nullptr;
}

FReply SIKRigHierarchy::OnImportSkeletonClicked() const
{
	TSharedPtr<FStructOnScope> StructToDisplay = MakeShareable(new FStructOnScope(FIKRigHierarchyImportSettings::StaticStruct(), (uint8*)&ImportSettings));
	...
	DialogArguments.OnOkPressed_Lambda([this] ()
	{
		...
		Controller->AssetController->SetSkeletalMesh(ImportSettings.Mesh);
	});

	constexpr bool bAsModalDialog = false;
	SGenericDialogWidget::OpenDialog(LOCTEXT("IKRigHierarchyImport", "Import Hierarchy"), KismetInspector, DialogArguments, bAsModalDialog);
...
void SIKRigHierarchy::BindCommands()
{
	...
	CommandList->MapAction(Commands.SetStartBoneOnSolver,
	...
	CommandList->MapAction(Commands.SetPelvis,
		FExecuteAction::CreateSP(this, &SIKRigHierarchy::HandleSetRetargetRoot),
		FCanExecuteAction::CreateSP(this, &SIKRigHierarchy::CanSetPelvis));

	CommandList->MapAction(Commands.ClearPelvis,
		FExecuteAction::CreateSP(this, &SIKRigHierarchy::HandleClearPelvis),
		FCanExecuteAction::CreateSP(this, &SIKRigHierarchy::CanClearPelvis));
...
void SIKRigHierarchy::FillContextMenu(FMenuBuilder& MenuBuilder)
{
	...
	MenuBuilder.AddMenuEntry(Actions.SetStartBoneOnSolver);
	...
	MenuBuilder.AddMenuEntry(Actions.SetPelvis);
	MenuBuilder.AddMenuEntry(Actions.ClearPelvis);
...
void SIKRigHierarchy::HandleSetRootBoneOnSolvers()
{
	...
	for (const TSharedPtr<FSolverStackElement>& Solver : SelectedSolvers)
	{
		AssetController->SetStartBone(RootBoneName, Solver->IndexInStack);
...
bool SIKRigHierarchy::CanSetRootBoneOnSolvers()
{
	...
	for (const TSharedPtr<FSolverStackElement>& Solver : SelectedSolvers)
	{
		if (AssetController->GetSolverAtIndex(Solver->IndexInStack)->UsesStartBone())
		{
			return true;
		}
	}
...
bool SIKRigHierarchy::CanSetEndBoneOnSolvers() const
{
	...
	for (const TSharedPtr<FSolverStackElement>& Solver : SelectedSolvers)
	{
		if (AssetController->GetSolverAtIndex(Solver->IndexInStack)->UsesEndBone())
		{
			return true;
		}
	}
...
void SIKRigHierarchy::HandleSetRetargetRoot()
{
	...
	// set the first selected bone as the pelvis
	Controller->AssetController->SetRetargetRoot(SelectedBones[0]->BoneName);
}

bool SIKRigHierarchy::CanSetPelvis()
{
	TArray<TSharedPtr<FIKRigTreeElement>> SelectedBones;
	GetSelectedBones(SelectedBones);
	return !SelectedBones.IsEmpty();
...
void SIKRigHierarchy::HandleClearPelvis()
{
	const TSharedPtr<FIKRigEditorController> Controller = EditorController.Pin();
	if (!Controller.IsValid())
	{
...
bool SIKRigHierarchy::CanClearPelvis()
{
	const TSharedPtr<FIKRigEditorController> Controller = EditorController.Pin();
	if (!Controller.IsValid())
	{
...
void SIKRigHierarchy::RefreshTreeView(bool IsInitialSetup /*=false*/)
{
	...
	// get all solvers
	const TArray<FIKRigSolverBase*> Solvers = AssetController->GetSolverArray();
	...
	for (int32 BoneIndex=0; BoneIndex<Skeleton.BoneNames.Num(); ++BoneIndex)
	{
		...
		{
			if (!Solvers[SolverIndex]->HasSettingsOnBone(BoneName))
			{
				continue;
			}
		...
		for (const UIKRigEffectorGoal* Goal : Goals)
		{
			...
			for (int32 SolverIndex=0; SolverIndex<Solvers.Num(); ++SolverIndex)
			{
				if (!AssetController->DoesSolverHaveCustomGoalSettings(Goal->GoalName, SolverIndex))
				{
					continue;
				}
