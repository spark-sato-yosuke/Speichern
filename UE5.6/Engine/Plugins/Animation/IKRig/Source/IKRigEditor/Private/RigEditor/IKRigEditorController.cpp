...
void FIKRigEditorController::ShowDetailsForBoneSettings(const FName& BoneName, int32 SolverIndex) const
{
	...
	ShowStructViewerInDetailsView(StructViewer);
}
...
void FIKRigEditorController::ShowDetailsForGoalSettings(const FName GoalName, const int32 SolverIndex) const
{
	...
	FIKRigStructToView StructToView;
	StructToView.Type = InstancedStruct->GetPtr<FIKRigSolverBase>()->GetGoalSettingsType();
	StructToView.MemoryProvider = MemoryProvider;
	StructToView.Owner = AssetController->GetAsset();
...
void FIKRigEditorController::ShowDetailsForElements(const TArray<TSharedPtr<FIKRigTreeElement>>& InItems) const
{
	...
	// edit the last selected element
...
void FIKRigEditorController::ShowStructViewerInDetailsView(UIKRigStructViewer* StructViewer) const
{
