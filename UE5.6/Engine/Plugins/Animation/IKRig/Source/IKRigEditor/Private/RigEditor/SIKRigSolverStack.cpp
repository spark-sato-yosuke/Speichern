...
bool SIKRigSolverStackItem::GetWarningMessage(FText& Message) const
{
	...
	if (const FIKRigSolverBase* Solver = GetSolver(bFromAsset))
	{
...
const FIKRigSolverBase* SIKRigSolverStackItem::GetSolver(const bool bFromAsset) const
{
	...
	else
	{
		...
		if (FIKRigProcessor* Processor = EditorController->GetIKRigProcessor())
		{
			return Processor->GetSolver(SolverIndex);
		}
	}
...
void SIKRigSolverStack::BuildAddNewMenu(FMenuBuilder& MenuBuilder)
{
	...
	for (const FIKRigSolverMetaData& SolverMetaData : AllSolversMetaData)
	{
		FUIAction Action = FUIAction( FExecuteAction::CreateSP(this, &SIKRigSolverStack::AddNewSolver, SolverMetaData.ScriptStruct));
		MenuBuilder.AddMenuEntry(SolverMetaData.NiceName, FText::GetEmpty(), FSlateIcon(), Action);
	}
...
void SIKRigSolverStack::CacheSolverMetaData()
{
	AllSolversMetaData.Reset();

	// instantiate all types of solvers to cache their meta data
	const UStruct* BaseStruct = FIKRigSolverBase::StaticStruct();
	for (TObjectIterator<UStruct> StructIt; StructIt; ++StructIt)
	{
		...
		FInstancedStruct TempInstancedStruct = FInstancedStruct(ScriptStruct);
		const FIKRigSolverBase& TempSolver = TempInstancedStruct.Get<FIKRigSolverBase>();

		// store meta data for this type
		FIKRigSolverMetaData SolverMetaData;
		SolverMetaData.NiceName = TempSolver.GetNiceName();
		SolverMetaData.ScriptStruct = ScriptStruct;
		AllSolversMetaData.Add(SolverMetaData);
	}
}

void SIKRigSolverStack::AddNewSolver(UScriptStruct* SolverType)
{
	const TSharedPtr<FIKRigEditorController> Controller = EditorController.Pin();
	if (!Controller.IsValid())
	{
	...
	// add the solver
	const int32 NewSolverIndex = AssetController->AddSolver(SolverType);
	// update stack view
	RefreshStackView();
...
void SIKRigSolverStack::RefreshStackView()
{
	...
	{
		const FIKRigSolverBase* Solver = AssetController->GetSolverAtIndex(i);
	...
	TSharedPtr<FSolverStackElement> TargetItem)
{
	...
	if (bWasMoved)
	{
		...
		Controller->ShowDetailsForSolver(TargetIndex);
		ListView->SetSelection(ListViewItems[TargetIndex]);
	}
