...
bool FUdpMessageTransport::StartTransport(IMessageTransportHandler& Handler)
{
...
#endif
	{
		...
		if (CurrentSettings->MaxConcurrentDeserializationTasks > 1)
		{
			UE_LOG(LogUdpMessaging, Display, TEXT("Using asynchronous task graph for message deserialization."));
			DeserializationTasks = MakeUnique<UE::Tasks::FTaskConcurrencyLimiter>(CurrentSettings->MaxConcurrentDeserializationTasks);
		}
...
void FUdpMessageTransport::StopTransport()
{
	...
	// Clear out any deserialize tasks before resetting the transport handler. They depend on the handler to dispatch
	// deserialized messages.
	//
	if (DeserializationTasks)
	{
		DeserializationTasks->Wait();
		DeserializationTasks.Reset();		
	}
...
void FUdpMessageTransport::HandleProcessorMessageReassembled(FUdpReassembledMessageSharedPtr ReassembledMessagePtr, const TSharedPtr<IMessageAttachment, ESPMode::ThreadSafe>& Attachment, const FGuid& NodeId)
{
	...
	auto DeserializeReassembledMessage = [DeserializedMessage = MoveTemp(DeserializedMessage), TransportHandler = this->TransportHandler, ReassembledMessagePtr = MoveTemp(ReassembledMessagePtr), NodeId](uint32 Slot)
	{
		const FUdpReassembledMessage& ReassembledMessage = *ReassembledMessagePtr.Get();
	...
	if (DeserializationTasks)
	{
		// The user has asked for async deserialization.  Dispatch the task.
		// 
		DeserializationTasks->Push(UE_SOURCE_LOCATION, MoveTemp(DeserializeReassembledMessage));
	}
	else
	{
		// Otherwise we should run on this thread. 
		DeserializeReassembledMessage(0);
	}
}
