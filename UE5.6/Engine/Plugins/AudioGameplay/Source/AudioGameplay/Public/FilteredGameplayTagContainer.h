...
*/
class FFilteredGameplayTagContainer
{
public:
	...
	*/
	UE_API bool AddTagFiltered(const FilteredGPTContainer::FGPTagQueryPair& InPair);
	...
	* @return True if tag was added
	*/
	UE_API bool AddTagFiltered(const FGameplayTag& InTag, const FGameplayTagQuery& InQuery = FGameplayTagQuery());
	...
	*/
	UE_API bool RemoveTagFiltered(const FGameplayTag& InTag);
...
private:
	...
	UE_API bool AddTagToContainers(const FilteredGPTContainer::FGPTagQueryPair& InPair);
	UE_API bool RemoveTagFromContainers(const FGameplayTag& Tag);
	UE_API void FilterTags();
	UE_API void CheckContainersConsistency();
