...
Microsoft::WRL::ComPtr<IUnknown> GetAdapterNpu(bool bVerbose)
{
	...
	// New path from Windows 11, version 24H2 on
	{
...
#else
		const GUID DxGUIDs[] = { { 0xb71b0d41, 0x1088, 0x422f, { 0xa2, 0x7c, 0x2, 0x50, 0xb7, 0xd3, 0xa9, 0x88 } } }; // Value of DXCORE_ADAPTER_ATTRIBUTE_D3D12_GENERIC_ML
#endif
		...
		{
			Hr = AdapterList->GetAdapter(static_cast<uint32_t>(i), IID_PPV_ARGS(&Adapter));
			if (FAILED(Hr))
			{
		...
		if (!Adapter)
		{
			UE_LOG(LogNNERuntimeORT, Log, TEXT("No NPU adapter found with attribute DXCORE_ADAPTER_ATTRIBUTE_D3D12_GENERIC_ML (Windows 11 Version 24H2 or newer)!"));
		}
		else
		{
			if (bVerbose)
