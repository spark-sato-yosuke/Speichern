...
bool FRHIReservedResourceTests::Test_ReservedResource_CreateBuffer(FRHICommandListImmediate& RHICmdList)
{
...

	{
		const FRHIBufferCreateDesc CreateDesc =
			FRHIBufferCreateDesc::CreateVertex(TEXT("TestSmallReservedVertexBuffer"), 32768)
			.AddUsage(EBufferUsageFlags::ReservedResource)
	...
	{
		const FRHIBufferCreateDesc CreateDesc =
			FRHIBufferCreateDesc::Create(TEXT("TestSmallReservedUAV"), 32768, 4, EBufferUsageFlags::UnorderedAccess | EBufferUsageFlags::ShaderResource)
			.AddUsage(EBufferUsageFlags::ReservedResource)
			.SetInitialState(ERHIAccess::UAVGraphics);
		FBufferRHIRef Buffer = RHICmdList.CreateBuffer(CreateDesc);
	}

	{
		const FRHIBufferCreateDesc CreateDesc =
			FRHIBufferCreateDesc::CreateVertex(TEXT("TestReservedVertexBuffer"), 32 * 1024 * 1024)
			.AddUsage(EBufferUsageFlags::ReservedResource)
			.SetInitialState(ERHIAccess::CopyDest);
		FBufferRHIRef Buffer = RHICmdList.CreateBuffer(CreateDesc);
	}
	...
	{
		const FRHIBufferCreateDesc CreateDesc =
			FRHIBufferCreateDesc::Create(TEXT("TestReservedAccelerationStructureBuffer"), 32 * 1024 * 1024, 4, EBufferUsageFlags::AccelerationStructure)
			.SetUsage(EBufferUsageFlags::ReservedResource)
	...
	{
		const FRHIBufferCreateDesc CreateDesc =
			FRHIBufferCreateDesc::Create(TEXT("TestReservedRayTracingScratchBuffer"), 32 * 1024 * 1024, 4, EBufferUsageFlags::RayTracingScratch)
			.AddUsage(EBufferUsageFlags::ReservedResource)
...
bool FRHIReservedResourceTests::Test_ReservedResource_CommitBuffer(FRHICommandListImmediate& RHICmdList)
{
	...
	const FRHIBufferCreateDesc CreateDesc =
		FRHIBufferCreateDesc::Create(TEXT("TestReservedBufferExplicitCommit"), BufferSizeInBytes, 4, EBufferUsageFlags::ReservedResource | EBufferUsageFlags::UnorderedAccess | EBufferUsageFlags::ShaderResource | EBufferUsageFlags::SourceCopy)
		.SetInitialState(ERHIAccess::UAVCompute);
