...
FInterchangeTestFunctionResult UStaticMeshImportTestFunctions::CheckRenderVertexIndexNormal(UStaticMesh* Mesh, int32 LodIndex, int32 VertexIndex, const FVector4f& ExpectedVertexNormal)
{
	...
	else if (Mesh->GetRenderData() && Mesh->GetRenderData()->LODResources.IsValidIndex(LodIndex))
	{
		...
		else
		{
			...
			const FVector4f VertexNormal = RoundVector4ToDecimalPlaces(Mesh->GetRenderData()->LODResources[LodIndex].VertexBuffers.StaticMeshVertexBuffer.VertexTangentZ(VertexIndex));
			const FVector4f ExpectedVertexNormalRounded = RoundVector4ToDecimalPlaces(ExpectedVertexNormal);
			if (!VertexNormal.Equals(ExpectedVertexNormalRounded, UE_KINDA_SMALL_NUMBER))
			{
...
FInterchangeTestFunctionResult UStaticMeshImportTestFunctions::CheckVertexIndexPosition(UStaticMesh* Mesh, int32 LodIndex, int32 VertexIndex, const FVector& ExpectedVertexPosition)
{
	...
	else
	{
		...
		{
			using namespace InterchangeTestsMathUtilities;
			const FVector VertexPosition = RoundVectorToDecimalPlaces(FVector(Attributes.GetVertexPositions()[VertexIndex]));
...
FInterchangeTestFunctionResult UStaticMeshImportTestFunctions::CheckSocketLocation(UStaticMesh* Mesh, int32 SocketIndex, const FVector& ExpectedSocketLocation)
{
	...
	if (Mesh->Sockets.IsValidIndex(SocketIndex))
	{
		using namespace InterchangeTestsMathUtilities;

		const FVector SocketLocation = RoundVectorToDecimalPlaces(Mesh->Sockets[SocketIndex]->RelativeLocation);
		const FVector ExpectedSocketLocationRounded = RoundVectorToDecimalPlaces(ExpectedSocketLocation);
		if (!SocketLocation.Equals(ExpectedSocketLocationRounded, UE_DOUBLE_KINDA_SMALL_NUMBER))
		{
	...
	{
		Result.AddError(FString::Printf(TEXT("The imported mesh doesn't contain socket with index %d."), SocketIndex));
	}
	...
	if (bCheckVertexPositionsEqual)
	{
		for (int32 LodIndex = 0; LodIndex < NumLods; LodIndex++)
		{
			...
			else
			{
				...
				{
					using namespace InterchangeTestsMathUtilities;
					const FVector3f V0 = RoundVectorToDecimalPlaces(VertexPositions[VertexIndex]);
					const FVector3f V1 = RoundVectorToDecimalPlaces(GroundTruthVertexPositions[VertexIndex]);
	...
	if (bCheckNormalsEqual)
	{
		for (int32 LodIndex = 0; LodIndex < NumLods; LodIndex++)
		{
			...
			else
			{
				...
				{
					using namespace InterchangeTestsMathUtilities;
					const FVector3f N0 = RoundVectorToDecimalPlaces(VertexInstanceNormals[NormalIndex]);
					const FVector3f N1 = RoundVectorToDecimalPlaces(GroundTruthVertexInstanceNormals[NormalIndex]);
...
FInterchangeTestFunctionResult UStaticMeshImportTestFunctions::CheckBoundingBoxSizeGreaterThan(UStaticMesh* Mesh, const FVector& GreaterThanSize)
{
	using namespace InterchangeTestsMathUtilities;
	FBox BBox = Mesh->GetBoundingBox();
	const FVector Size = RoundVectorToDecimalPlaces(BBox.GetSize());
	const FVector GreaterThanSizeRounded = RoundVectorToDecimalPlaces(GreaterThanSize);
	
	FInterchangeTestFunctionResult Result;

	auto CompareDimension = [](FInterchangeTestFunctionResult& Result, double AxisSize, double ExpectedAxisSize, FString AxisName)
	{
		if (AxisSize <= ExpectedAxisSize)
		{
			Result.AddError(FString::Printf(TEXT("BoundingBoxSize.%s[%lf] is not greater than expected[%lf]."), *AxisName, AxisSize, ExpectedAxisSize));
		}
	};
	
	CompareDimension(Result, Size.X, GreaterThanSizeRounded.X, "X");
	CompareDimension(Result, Size.Y, GreaterThanSizeRounded.Y, "Y");
	CompareDimension(Result, Size.Z, GreaterThanSizeRounded.Z, "Z");

	return Result;
}

FInterchangeTestFunctionResult UStaticMeshImportTestFunctions::CheckBoundingBoxSizeLessThan(UStaticMesh* Mesh, const FVector& LessThanSize)
{
	using namespace InterchangeTestsMathUtilities;
	FBox BBox = Mesh->GetBoundingBox();
	const FVector Size = RoundVectorToDecimalPlaces(BBox.GetSize());
	const FVector LessThanSizeRounded = RoundVectorToDecimalPlaces(LessThanSize);
	...
	auto CompareDimension = [](FInterchangeTestFunctionResult& Result, double AxisSize, double ExpectedAxisSize, FString AxisName)
	{
		if (AxisSize >= ExpectedAxisSize)
		{
			Result.AddError(FString::Printf(TEXT("BoundingBoxSize.%s[%lf] is not less than expected[%lf]."), *AxisName, AxisSize, ExpectedAxisSize));
		}
	};

	CompareDimension(Result, Size.X, LessThanSizeRounded.X, "X");
	CompareDimension(Result, Size.Y, LessThanSizeRounded.Y, "Y");
	CompareDimension(Result, Size.Z, LessThanSizeRounded.Z, "Z");

	return Result;
}

FInterchangeTestFunctionResult UStaticMeshImportTestFunctions::CheckBoundingBoxSize(UStaticMesh* Mesh, const FVector& ExpectedSize)
{
	using namespace InterchangeTestsMathUtilities;
	FBox BBox = Mesh->GetBoundingBox();
	const FVector Size = RoundVectorToDecimalPlaces(BBox.GetSize());
	const FVector ExpectedSizeRounded = RoundVectorToDecimalPlaces(ExpectedSize);
	...
	auto CompareDimension = [](FInterchangeTestFunctionResult& Result, double AxisSize, double ExpectedAxisSize, FString AxisName)
	{
		...
		{
			Result.AddError(FString::Printf(TEXT("BoundingBoxSize.%s[%lf] is not equal to expected[%lf]."), *AxisName, AxisSize, ExpectedAxisSize));
		}
	};

	CompareDimension(Result, Size.X, ExpectedSizeRounded.X, "X");
	CompareDimension(Result, Size.Y, ExpectedSizeRounded.Y, "Y");
	CompareDimension(Result, Size.Z, ExpectedSizeRounded.Z, "Z");
