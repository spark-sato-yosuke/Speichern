...
CHAOSCLOTHASSETENGINE_API void FChaosClothSimulationModel::RemapBoneIndices(const TArray<int32>& BoneMap)
{
	// Default to the root bone index whenever an invalid index is provided to avoid any issues further on
	constexpr int32 RootBoneIndex = 0;

	// Remap the reference bone if needed
	ReferenceBoneIndex = ensure(BoneMap.IsValidIndex(ReferenceBoneIndex)) ? BoneMap[ReferenceBoneIndex] : RootBoneIndex;

	// UsedBoneIndices is used to reindex from the subset index to the actual skeleton bone index, so needs updating
	for (int32& UsedBoneIndex : UsedBoneIndices)
	{
		UsedBoneIndex = ensure(BoneMap.IsValidIndex(UsedBoneIndex)) ? BoneMap[UsedBoneIndex] : RootBoneIndex;
	}
}
