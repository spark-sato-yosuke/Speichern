...
void UChaosClothAsset::FBuilder::BuildLod(FSkeletalMeshLODModel& LODModel, const UChaosClothAsset& ClothAsset, int32 LodIndex, const ITargetPlatform* TargetPlatform)
{
	...
	FScopedSlowTask SlowTask(0.0f, LOCTEXT("ClothAssetBuildLOD", "Building Cloth Asset LOD sections..."));
	...
	for (int32 SectionIndex = 0; SectionIndex < NumSections; ++SectionIndex)
	{
		...
		UE::Tasks::FTask PendingTask = UE::Tasks::Launch(UE_SOURCE_LOCATION, 
			...
			[SectionIndex, &Section, NumFaces, NumIndices, &LODModelData, &ActiveBoneIndices, 
			// Const data that should not be modified or protected data by mutex.
			&ClothFacade, MaterialOffset, &ClothAsset, &RecomputeTangent, &MeshUtilities, bIsValidSourceMesh, &SourceMesh, &SourceMeshMutex, TargetPlatform, LodIndex, MaxDistances]() 
		{
			FCollectionClothRenderPatternConstFacade RenderPatternFacade = ClothFacade.GetRenderPattern(SectionIndex);
			...
			for (int32 VertexIndex = 0; VertexIndex < NumVertices; ++VertexIndex)
			{
				...
				LODModelData.MeshToImportVertexMap.Add(VertexIndex + PatternVertexOffset);
				LODModelData.MaxImportVertex = VertexIndex + PatternVertexOffset;
			...
			if (bIsValidSourceMesh)
			{
				...
				else
				{
					...
					const TArrayView<uint32> SectionIndexBuffer(LODModelData.IndexBuffer.GetData(), NumIndices);
	...
	if (const TSharedPtr<const FChaosClothSimulationModel> ClothSimulationModel = ClothAsset.GetClothSimulationModel())
	{
		...
		{
			ActiveBoneIndicesArray[0].Append(ClothSimulationModel->ClothSimulationLodModels[LodIndex].RequiredExtraBoneIndices);

			LODModel.RequiredBones = ActiveBoneIndicesArray[0].Array();
