...
struct FChaosClothSimulationModel
{
	...
	CHAOSCLOTHASSETENGINE_API FChaosClothSimulationModel(const TArray<TSharedRef<const FManagedArrayCollection>>& ClothCollections, const FReferenceSkeleton& ReferenceSkeleton, TArray<FChaosClothAssetLodTransitionDataCache>* InOutTransitionCache = nullptr);
	...
	CHAOSCLOTHASSETENGINE_API void GetResourceSizeEx(FResourceSizeEx& CumulativeResourceSize);

	/** Remap used bones to a new skeleton. */
	CHAOSCLOTHASSETENGINE_API void RemapBoneIndices(const TArray<int32>& BoneMap);
};
