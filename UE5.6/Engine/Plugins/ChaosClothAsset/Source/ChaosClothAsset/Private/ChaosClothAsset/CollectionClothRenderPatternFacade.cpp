...
namespace UE::Chaos::ClothAsset
{
	...
	TConstArrayView<float> FCollectionClothRenderPatternConstFacade::GetRenderCustomResizingBlend() const
	{
		return ClothCollection->GetElements(
			ClothCollection->GetRenderCustomResizingBlend(),
			ClothCollection->GetRenderVerticesStart(),
			ClothCollection->GetRenderVerticesEnd(),
			GetElementIndex());
	}
	...
	void FCollectionClothRenderPatternFacade::Initialize(const FCollectionClothRenderPatternConstFacade& Other, int32 SimVertex3DOffset)
	{
		...
		if (GetClothCollection()->IsValid(EClothCollectionExtendedSchemas::RenderDeformer) && Other.ClothCollection->IsValid(EClothCollectionExtendedSchemas::RenderDeformer))
		{
		...
		if (GetClothCollection()->IsValid(EClothCollectionExtendedSchemas::Resizing) && Other.ClothCollection->IsValid(EClothCollectionExtendedSchemas::Resizing))
		{
			FClothCollection::CopyArrayViewData(GetRenderCustomResizingBlend(), Other.GetRenderCustomResizingBlend());
		}
	...
	void FCollectionClothRenderPatternFacade::SetRenderDeformerNumInfluences(int32 NumInfluences)
	{
		...
		if (!GetClothCollection()->IsValid(EClothCollectionExtendedSchemas::RenderDeformer))
		{
			if (NumInfluences == 0)
			{
				return;  // Not having the schema is the same as having zero influences
			}
			GetClothCollection()->DefineSchema(EClothCollectionExtendedSchemas::RenderDeformer);
		}
	...
	TArrayView<float> FCollectionClothRenderPatternFacade::GetRenderCustomResizingBlend()
	{
		return GetClothCollection()->GetElements(
			GetClothCollection()->GetRenderCustomResizingBlend(),
			GetClothCollection()->GetRenderVerticesStart(),
			GetClothCollection()->GetRenderVerticesEnd(),
			GetElementIndex());
	}
