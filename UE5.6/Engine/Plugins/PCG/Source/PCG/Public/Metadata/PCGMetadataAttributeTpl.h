...
public:
	...
	FPCGMetadataAttribute(UPCGMetadata* InMetadata, FName InName, const FPCGMetadataAttributeBase* InParent, const T& InDefaultValue, bool bInAllowsInterpolation)
		: FPCGMetadataAttribute(PCGMetadataHelpers::GetDefaultMetadataDomain(InMetadata), InName, InParent, InDefaultValue, bAllowsInterpolation)
	{}
	...
	virtual void FlattenAndCompress(const TArrayView<const PCGMetadataEntryKey>& InEntryKeysToKeep) override
	{
	...
	{
		return static_cast<FPCGMetadataAttribute*>(TypedCopy(NewName, PCGMetadataHelpers::GetDefaultMetadataDomain(InMetadata), bKeepParent, bCopyEntries, bCopyValues));
	}
	...
	UE_DEPRECATED(5.6, "Use the version with FPCGMetadataDomain")
	virtual FPCGMetadataAttribute* Copy(FName NewName, UPCGMetadata* InMetadata, bool bKeepParent, bool bCopyEntries = true, bool bCopyValues = true) const override
	{
		return static_cast<FPCGMetadataAttribute*>(Copy(NewName, PCGMetadataHelpers::GetDefaultMetadataDomain(InMetadata), bKeepParent, bCopyEntries, bCopyValues));
	}
	...
	template<typename U>
	UE_DEPRECATED(5.6, "Do not use anymore, use CopyToAnotherType")
	...
	{
		return CopyInternal<U>(NewName, PCGMetadataHelpers::GetDefaultMetadataDomain(InMetadata), bKeepParent, bCopyEntries, bCopyValues);
	}
...
protected:
	...
	{
		IT Value = PCG::Private::MetadataTraits<IT>::WeightedSum(
				GetValueFromItemKey(ItemKey),
				static_cast<const FPCGMetadataAttribute<T>*>(InAttribute)->GetValueFromItemKey(InEntryKey),
				Weight);

		if constexpr (PCG::Private::MetadataTraits<IT>::InterpolationNeedsNormalization)
		{
			static_assert(PCG::Private::MetadataTraits<IT>::CanNormalize);

			// We need to normalize the resulting value
			PCG::Private::MetadataTraits<IT>::Normalize(Value);
		}
