...
protected:
	...
	{
		UTEST_EQUAL("Only one output exists", OutputData.GetAllSpatialInputs().Num(), 1);
...
bool FPCGSwitchTest_Enum::RunTest(const FString& Parameters)
{
	...
	for (uint8 i = 0; i < NumEnums - 1; ++i)
	{
		...
		{
			UTEST_TRUE("Hidden enum selection output is empty", OutputData.GetAllSpatialInputs().IsEmpty());
		}
		else
		{
