...
TArray<FPCGGraphTask> FPCGGraphCompiler::CompileGraph(UPCGGraph* InGraph, FPCGTaskId& NextId, FPCGStackContext& InOutStackContext)
{
	...
	while (NodeQueue.Num() > 0)
	{
		...
		const bool bIsRecursiveGraph = Subgraph && (InGraph == Subgraph || Subgraph->Contains(InGraph));
		bool bIsDynamic = (!SubgraphSettings || SubgraphSettings->IsDynamicGraph());
		...
		if(Subgraph && !bIsDynamic && bIsNotDisabled && !bIsRecursiveGraph)
		{
		...
		else
		{
			...
			for (const UPCGPin* InputPin : Node->InputPins)
			{
				// Cull dependency only data if the pin is marked as such.
				const bool bIsDependencyOnly = InputPin->Properties.Usage == EPCGPinUsage::DependencyOnly;
				for (const UPCGEdge* InboundEdge : InputPin->Edges)
				{
					if (!InboundEdge->IsValid())
					{
					...
					check(InboundEdge->OutputPin == InputPin);
					if (FPCGTaskId* InboundId = IdMapping.Find(InboundEdge->InputPin->Node))
					{
						Task.Inputs.Emplace(*InboundId, InboundEdge->InputPin->Properties, InputPin->Properties, !bIsDependencyOnly);
					}
					else
					{
	...
	bool bIsCooking)
{
	...
	for (FPCGTaskId TaskId = 0; TaskId < NumCompiledTasksBefore; ++TaskId)
	{
		...
		for (int32 InputIndex = 0; InputIndex < InOutCompiledTasks[TaskId].Inputs.Num(); ++InputIndex)
		{
			const FPCGGraphTaskInput TaskInput = InOutCompiledTasks[TaskId].Inputs[InputIndex];
			...
			if (InputGraphGenerationGrid != EPCGHiGenGrid::Uninitialized && InputGraphGenerationGrid > GraphGenerationGrid)
			{
				...
				InOutCompiledTasks[TaskId].Inputs[InputIndex].TaskId = LinkTask.NodeId;

				// Set the task input's UpstreamPin so that it has the correct type.
				InOutCompiledTasks[TaskId].Inputs[InputIndex].UpstreamPin = UpstreamPinProps;
...
void FPCGGraphCompiler::CullTasks(TArray<FPCGGraphTask>& InOutCompiledTasks, bool bAddPassthroughWires, TFunctionRef<bool(const FPCGGraphTask&)> CullTask, TArray<FPCGTaskId>* OutOptionalOldIdToNewIdMap)
{
	...
	OldIdToNewId.SetNumUninitialized(InOutCompiledTasks.Num());
	...
	{
		OldIdToNewId[TaskIndex] = CullTask(InOutCompiledTasks[TaskIndex]) ? InvalidPCGTaskId : 0;
	}
	...
	if (bAddPassthroughWires)
	{
		for (int TaskIndex = 1; TaskIndex < InOutCompiledTasks.Num(); ++TaskIndex)
		{
			...
			while (InputIndex < Task.Inputs.Num())
			{
				...
				// Is node culled?
				if (OldIdToNewId[InputTaskId] == InvalidPCGTaskId)
				{
	...
	while (ReadIndex < InOutCompiledTasks.Num())
	{
		...
		if (OldIdToNewId[ReadIndex] != InvalidPCGTaskId)
		{
			...
			OldIdToNewId[ReadIndex] = WriteIndex;
			++WriteIndex;
		}
	...
	for (FPCGGraphTask& Task : InOutCompiledTasks)
	{
		...
		{
			const FPCGTaskId InputTaskId = Task.Inputs[InputIndex].TaskId;
			const FPCGTaskId Remap = OldIdToNewId[InputTaskId];
		...
		{
			const FPCGTaskId RemappedParentId = OldIdToNewId[Task.ParentId];
	...
	// Return the mapping if caller requested it.
	if (OutOptionalOldIdToNewIdMap)
	{
		*OutOptionalOldIdToNewIdMap = MoveTemp(OldIdToNewId);
	}
}
...
void FPCGGraphCompiler::AddReferencedObjects(FReferenceCollector& Collector)
{
	...
	for (TPair<UPCGGraph*, TMap<uint32, TArray<TObjectPtr<UPCGComputeGraph>>>>& GraphToGridSizes : Cache.TopGraphToComputeGraphMap)
	{
		for (TPair<uint32, TArray<TObjectPtr<UPCGComputeGraph>>>& GridSizeToComputeGraphs : GraphToGridSizes.Value)
		{
...
void FPCGGraphCompiler::CompileTopGraph(UPCGGraph* InGraph, uint32 GenerationGridSize)
{
	...
	for (int TaskIndex = 0; TaskIndex < CompiledTasks.Num(); ++TaskIndex)
	{
		for (FPCGGraphTaskInput& TaskInput : CompiledTasks[TaskIndex].Inputs)
		{
			...
			if (!InputOrigin.TaskInput || !InputOrigin.TaskInput->bProvideData)
			{
				continue;
			}
			...
			{
				// @todo_pcg: This should not have that big of a cost, as most InputTasks do not already have an element when compiling.
				// The virtual call will then not be often called. However, it would be cleaner to have a boolean state on the TaskInput 
				// that is set to true when we know we cross boundaries.
				// By definition, if we cross a boundary, then an input will be used multiple times.
				const FPCGGraphTask* InputTask = CompiledTasks.IsValidIndex(TaskInput.TaskId) ? &CompiledTasks[TaskInput.TaskId] : nullptr;
				const bool bCrossBoundaries = InputTask && InputTask->Element && InputTask->Element->IsGridLinkage();
				TaskInput.bIsUsedMultipleTimes = bCrossBoundaries;
	...
	for (int TaskIndex = TaskNum - 1; TaskIndex >= 0; --TaskIndex)
	{
		...
		if (!TasksWithSuccessors.Contains(Task.NodeId))
		{
			...
			FPCGGraphTaskInput& TaskInput = PostExecuteTask.Inputs.Emplace_GetRef(Task.NodeId, /*InUpstreamPin=*/FPCGGraphTaskInput::NoPin, /*InDownstreamPin=*/FPCGGraphTaskInput::NoPin, bProvideData);
