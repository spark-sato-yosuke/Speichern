...
private:
	...
	void ClearCacheInternal(int32 InMaxEntries, bool bClearMemory);
	void AddToCacheInternal(const FPCGCacheEntryKey& InKey, const FPCGDataCollection& InCollection, bool bAddToMemory);
	void RemoveFromCacheInternal(const FPCGCacheEntryKey& InKey);
	void ValidateElementToCacheEntryKeys() const;
	...
	TMap<const IPCGElement*, TSet<FPCGCacheEntryKey>> ElementToCacheEntryKeys;
