...
TArray<FName> UPCGSettingsWithDynamicInputs::GetNodeDefinedPinLabels() const
{
	const TArray<FPCGPinProperties> PinProperties = InputPinProperties();

	TArray<FName> Labels;
	Labels.Reserve(PinProperties.Num());
	Algo::Transform(PinProperties, Labels, [](const FPCGPinProperties& Properties) { return Properties.Label; });
