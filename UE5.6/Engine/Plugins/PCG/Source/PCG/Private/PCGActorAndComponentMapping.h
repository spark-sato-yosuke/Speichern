...
private:
	...
	/** Used to track where a change comes from. */
	enum class EChangedObjectReason
	{
		ActorAdded,
		ActorChanged,
		ActorDeleted,
...
#endif
	bool RegisterOrUpdatePCGComponent(UPCGComponent* InComponent, bool bDoActorMapping, bool bForce);
	...
	void OnActorChanged_Internal(AActor* InActor, UObject* InOriginatingChangeObject, const EChangedObjectReason InChangeReason);
	void OnActorChanged_Recursive(AActor* InActor, UObject* InOriginatingChangeObject, const EChangedObjectReason InChangeReason);
	...
	*/
	void OnObjectChanged(UObject* InObject, const EChangedObjectReason InChangeReason, const FActorPreviousData* InPreviousData = nullptr, const UObject* InOriginatingChangeObject = nullptr);
...
private:
	...
	mutable FCriticalSection DelayedComponentsRegistrationLock;
	TSet<UPCGComponent*> DelayedComponentsToUnregister;
	TSet<UPCGComponent*> DelayedComponentsToRegister;
	...
	TMap<TObjectKey<AActor>, TTuple<TObjectKey<UObject>, EChangedObjectReason>> DelayedChangedActors;
