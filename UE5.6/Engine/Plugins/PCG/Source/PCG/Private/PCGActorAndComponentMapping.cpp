...
bool FPCGActorAndComponentMapping::IsActorTracked(const AActor* InActor) const
{
	...
	FSoftObjectPath ActorPath(InActor);
	auto Matching = [InActor, &ActorPath, &EmptySet](const TPair<FPCGSelectionKey, TSet<UPCGComponent*>>& It) -> bool
	{
		return It.Key.IsMatching(InActor, ActorPath, EmptySet, It.Value, nullptr);
	};
...
void FPCGActorAndComponentMapping::OnObjectChanged(UObject* InObject, const EChangedObjectReason InChangeReason, const FActorPreviousData* InPreviousData, const UObject* InOriginatingChangeObject)
{
	...
	FSoftObjectPath InOriginatingChangeObjectPath(InOriginatingChangeObject);
	auto Gather = [InObject, &ObjectPath, &RemovedTags, &MatchedKeys, InOriginatingChangeObject, &InOriginatingChangeObjectPath](const TMap<FPCGSelectionKey, TSet<UPCGComponent*>>& InMap, TSet<UPCGComponent*>& OutSet)
	{
		for (const auto& It : InMap)
		{
			if (It.Key.IsMatching(InObject, ObjectPath, RemovedTags, It.Value, &OutSet) || It.Key.IsMatching(InOriginatingChangeObject, InOriginatingChangeObjectPath, RemovedTags, It.Value, &OutSet))
			{
	...
	TArray<const UObject*, TInlineAllocator<2>> ChangedObjects = {InObject};
	...
	//   * It is generating and one of the objects that changed is managed by it (safe-guard, would probably induce an infinite loop).
	//   * One of the objects that changed is ignored by its original component.
	auto ShouldDiscardComponent = [&ChangedObjects, bNoRefreshOwner](const UPCGComponent* InComponent)
	{
		if (!InComponent
			|| ChangedObjects.Contains(InComponent)
			|| (bNoRefreshOwner && ChangedObjects.Contains(InComponent->GetOwner()))
			|| (InComponent->IsGenerating() && InComponent->IsAnyObjectManagedByResource(ChangedObjects)))
		{
			return true;
		}
	...
	if (!CulledTrackedComponents.IsEmpty())
	{
		...
		auto UpdateNonPartitioned = [this, &DirtyComponents, InObject, CulledTrackedComponents, &RemovedTags, DirtyFlag, InOriginatingChangeObject, bNoRefreshOwner, &MatchedKeys, &ShouldDiscardComponent](const FPCGComponentRef& ComponentRef) -> void
		{
		...
		auto UpdatePartitioned = [this, &DirtyComponents, InObject, CulledTrackedComponents, &CurrentActorBoundsPtr, &RemovedTags, DirtyFlag, InOriginatingChangeObject, bNoRefreshOwner, &MatchedKeys, &ShouldDiscardComponent](const FPCGComponentRef& ComponentRef)  -> void
		{
		...
		if (InPreviousData)
		{
			...
			{
				TRACE_CPUPROFILER_EVENT_SCOPE(FPCGActorAndComponentMapping::OnObjectChanged::SecondUpdateHasMoved);
...
void FPCGActorAndComponentMapping::OnPCGGraphStartsGenerating(UPCGComponent* InComponent)
{
	...
	for (TPair<FPCGSelectionKey, TSet<UPCGComponent*>>& It : CulledTrackedKeysToComponentsMap)
	{
		...
		It.Key.IsMatching(Actor, ActorPath, RemovedTags, It.Value, &TempTrackedComponents);
	...
	for (TPair<FPCGSelectionKey, TSet<UPCGComponent*>>& It : AlwaysTrackedKeysToComponentsMap)
	{
		...
		It.Key.IsMatching(Actor, ActorPath, RemovedTags, It.Value, &TrackedComponents);
	}
