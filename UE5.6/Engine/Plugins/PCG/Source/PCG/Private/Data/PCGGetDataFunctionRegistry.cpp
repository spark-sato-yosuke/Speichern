...
int FPCGGetDataFunctionRegistry::DefaultDataFromActor(FPCGContext* InContext, const FPCGGetDataFunctionRegistryParams& InParams, AActor* Actor, FPCGGetDataFunctionRegistryOutput& Output) const
{
	...
	{
		UPCGBasePointData* Data = FPCGContext::NewPointData_AnyThread(InContext);
...
int FPCGGetDataFunctionRegistry::DefaultDataFromComponent(FPCGContext* InContext, const FPCGGetDataFunctionRegistryParams& InParams, UActorComponent* InComponent, FPCGGetDataFunctionRegistryOutput& Output) const
{
	...
	// RuntimeVirtualTexture component
	...
	if (!TaggedData.Data && !!(InDataFilter & EPCGDataType::VirtualTexture))
	{
		if (URuntimeVirtualTextureComponent* VirtualTextureComponent = Cast<URuntimeVirtualTextureComponent>(InComponent))
		{
			UPCGVirtualTextureData* VirtualTextureData = FPCGContext::NewObject_AnyThread<UPCGVirtualTextureData>(InContext);
			VirtualTextureData->Initialize(VirtualTextureComponent);
			TaggedData.Data = VirtualTextureData;

			if (const URuntimeVirtualTexture* VirtualTexture = VirtualTextureData->GetRuntimeVirtualTexture())
			{
				FString VirtualTextureName;
				VirtualTexture->GetName(VirtualTextureName);

				const UEnum* MaterialTypeEnum = StaticEnum<ERuntimeVirtualTextureMaterialType>();
				check(MaterialTypeEnum);

				const FString MaterialTypeString = MaterialTypeEnum->GetNameStringByValue(static_cast<int64>(VirtualTexture->GetMaterialType()));

				TaggedData.Tags.Add(PCGComputeHelpers::GetPrefixedDataLabel(VirtualTextureName));
				TaggedData.Tags.Add(PCGComputeHelpers::GetPrefixedDataLabel(MaterialTypeString));
			}
