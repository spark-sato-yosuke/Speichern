...
UPCGGetPropertyFromObjectPathSettings::UPCGGetPropertyFromObjectPathSettings()
{
...
void UPCGGetPropertyFromObjectPathSettings::PostLoad()
{
	Super::PostLoad();
...
bool FPCGGetPropertyFromObjectPathElement::ExecuteInternal(FPCGContext* Context) const
{
	...
	for (const TTuple<FSoftObjectPath, int32, int32>& SoftPathAndIndex : ThisContext->PathsToObjectsAndDataIndex)
	{
		...
		PCGPropertyHelpers::FExtractorParameters Parameters(Object, Object->GetClass(), Settings->PropertyName.ToString(), OutputAttributeName, Settings->bForceObjectAndStructExtraction, /*bPropertyNeedsToBeVisible=*/true);
