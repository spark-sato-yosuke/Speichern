...
UPCGGetActorPropertySettings::UPCGGetActorPropertySettings(const FObjectInitializer& ObjectInitializer)
{
	...
	// as we have no guarantee on the order components are found. Also the new default is @Source for the output attribute
...
FString UPCGGetActorPropertySettings::GetAdditionalTitleInformation() const
{
	...
	return Suffix.IsEmpty()
		? FString::Printf(TEXT("%s, %s"), *ActorSelector.GetTaskName().ToString(), *PropertyName.ToString())
		: FString::Printf(TEXT("%s: %s, %s"), *ActorSelector.GetTaskName().ToString(), *Suffix.ToString(), *PropertyName.ToString());
#else
...
void FPCGGetActorPropertyElement::GetDependenciesCrc(const FPCGGetDependenciesCrcParams& InParams, FPCGCrc& OutCrc) const
{
	FPCGCrc Crc;
	IPCGElement::GetDependenciesCrc(InParams, Crc);

	// If we track self or original, we are dependent on the actor data
	if (const UPCGGetActorPropertySettings* Settings = Cast<const UPCGGetActorPropertySettings>(InParams.Settings))
	{
		...
		if (PCGComponent)
		{
			...
			// If it is depending on Self, we need to CRC the tags (as they will be passed to the output data)
			// and also the owner path.
			if (bDependsOnSelfOrHierarchy && ComponentToCheck)
			{
				if (const AActor* Owner = ComponentToCheck->GetOwner())
				{
					Crc.Combine(UPCGActorHelpers::ComputeHashFromActorTagsAndReference(Owner, /*bIncludeTags=*/true, /*bIncludeActorReference=*/true));
				}
			}
		}
	}
...
bool FPCGGetActorPropertyElement::ExecuteInternal(FPCGContext* InContext) const
{
	...
	if (Settings->ActorSelector.ActorFilter == EPCGActorFilter::FromInput)
	{
		...
		{
			// Skip tracking for references that weren't successfully resolved
	...
	for (AActor* FoundActor : FoundActors)
	{
		...
		{
			const FName OutputAttributeName = Settings->OutputAttributeName.GetName();

			PCGPropertyHelpers::FExtractorParameters Parameters(ObjectToInspect, ObjectToInspect->GetClass(), Settings->PropertyName.ToString(), OutputAttributeName, Settings->bForceObjectAndStructExtraction, /*bPropertyNeedsToBeVisible=*/true);
