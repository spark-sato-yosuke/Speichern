...
bool FPCGSplineToSegmentElement::ExecuteInternal(FPCGContext* InContext) const
{
	...
	for (const FPCGTaggedData& Input : Inputs)
	{
		...
		OutputPointData->SetNumPoints(NumSegments);
		OutputPointData->AllocateProperties(EPCGPointNativeProperties::Transform | EPCGPointNativeProperties::Seed | EPCGPointNativeProperties::MetadataEntry | EPCGPointNativeProperties::BoundsMin | EPCGPointNativeProperties::BoundsMax);
		...
		for (int32 Index = 0; Index < NumSegments; ++Index)
		{
			...
			if (bHasMetadata)
			{
				OutputPointData->Metadata->InitializeOnSet(OutRanges.MetadataEntryRange[Index]);
			}
			...
			OutRanges.TransformRange[Index].SetLocation(ControlPoint + Extents);
			OutRanges.TransformRange[Index].SetRotation(FRotationMatrix::MakeFromXZ(Extents, ControlPointZUp).ToQuat());
			OutRanges.BoundsMinRange[Index] = FVector(-ExtentsLength, 0.0, 0.0);
			OutRanges.BoundsMaxRange[Index] = FVector(ExtentsLength, 1.0, 1.0);

			OutRanges.SeedRange[Index] = PCGHelpers::ComputeSeedFromPosition(OutRanges.TransformRange[Index].GetLocation());
			...
			auto SetMetadata = [&](const int32 Index, const FVector& Tangent, const double Angle)
			{
				...
				const PCGMetadataEntryKey PreviousPointKey = OutRanges.MetadataEntryRange[PreviousIndex];
				const PCGMetadataEntryKey CurrentPointKey = OutRanges.MetadataEntryRange[Index];
