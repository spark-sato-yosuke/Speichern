...
namespace PCGMetadataVectorSettings
{
	...
	inline InType ApplyVectorOperation(const InType& Value1, const InType& Value2, double& DoubleValue, EPCGMetadataVectorOperation Operation)
	{
		...
		{
			InType Res = Value1;
			PCG::Private::MetadataTraits<InType>::Normalize(Res);
			return Res;
		}
...
{
	return FPCGPreConfiguredSettingsInfo::PopulateFromEnum<EPCGMetadataVectorOperation>({ EPCGMetadataVectorOperation::VectorOp, EPCGMetadataVectorOperation::TransformOp });
}
#endif // WITH_EDITOR
...
bool UPCGMetadataVectorSettings::IsPinDefaultValueEnabled(const FName PinLabel) const
{
	// @todo_pcg: Remove this when transforms are enabled.
	if (PCGMetadataVectorSettings::IsTransformOp(Operation) && PinLabel == GetInputPinLabel(1))
	{
		return false;
	}

	return Super::IsPinDefaultValueEnabled(PinLabel);
}
