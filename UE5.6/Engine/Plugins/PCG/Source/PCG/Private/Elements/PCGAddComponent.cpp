...
#if WITH_EDITOR
void UPCGAddComponentSettings::OnObjectsReplaced(const TMap<UObject*, UObject*>& InOldToNewInstances)
{
	...
	if (UObject* NewObject = InOldToNewInstances.FindRef(TemplateComponent))
	{
		TemplateComponent = Cast<UActorComponent>(NewObject);
		DirtyCache();
...
void UPCGAddComponentSettings::SetupOnBlueprintChangedEvent()
{
	...
	{
		FCoreUObjectDelegates::OnObjectsReplaced.AddUObject(this, &UPCGAddComponentSettings::OnObjectsReplaced);
	}
}
...
bool FPCGAddComponentElement::ExecuteInternal(FPCGContext* InContext) const
{
	...
	if (Inputs.Num() == 0 || !SourceComponent)
	{
		return true;
	}
	...
	while (Context->CurrentInputIndex < Inputs.Num())
	{
		...
		else
		{
			...
			TargetActorAccessor = MakeUnique<FPCGConstantValueAccessor<FSoftObjectPath>>(FSoftObjectPath(SourceComponent->GetOwner()));
	...
	IPCGAttributeAccessorKeys* OutputComponentKeys) const
{
	...
	for (int32 i = 0; i < NumberOfIterations; ++i)
	{
		...
		for (int32 j = 0; j < TransformsView.Num(); ++j)
		{
			...
			AActor* TargetActor = TSoftObjectPtr<AActor>(TargetActorsView[j]).Get();
			...
			ComponentClass = ComponentClassPtr.Get();

			if (!ComponentClass)
			{
				TSoftObjectPtr<UActorComponent> ComponentObjectPtr(ClassOrTemplatePath);
				ComponentTemplate = ComponentObjectPtr.Get();
			...
			if (TargetActor && ComponentClass)
			{
				...
				{
					if (!SceneComponent->AttachToComponent(TargetActor->GetRootComponent(), FAttachmentTransformRules(EAttachmentRule::KeepRelative, EAttachmentRule::KeepWorld, EAttachmentRule::KeepWorld, false)))
					{
