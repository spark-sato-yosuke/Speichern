...
{
	if (DataTable.IsNull() || IsPropertyOverriddenByPin(GET_MEMBER_NAME_CHECKED(UPCGLoadDataTableSettings, DataTable)))
	{
		return;
	}
...
bool FPCGLoadDataTableElement::PrepareLoad(FPCGExternalDataContext* Context) const
{
	...
	if (Settings->OutputType == EPCGExclusiveDataType::Point)
	{
		...
		// Set num points before creating accessors
		PointData->SetNumPoints(DataTable->GetRowMap().Num());
...
bool FPCGLoadDataTableElement::ExecuteLoad(FPCGExternalDataContext* Context) const
{
	...
	if (UPCGParamData* ParamData = Cast<UPCGParamData>(Data))
	{
