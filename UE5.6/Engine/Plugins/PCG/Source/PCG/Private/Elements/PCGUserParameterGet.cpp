...
void UPCGUserParameterGetSettings::ApplyPreconfiguredSettings(const FPCGPreConfiguredSettingsInfo& PreconfigureInfo)
{
	...
	UPCGGraph* Graph = Node ? Node->GetGraph() : nullptr;
	if (const FInstancedPropertyBag* PropertyBag = Graph ? Graph->GetUserParametersStruct() : nullptr)
	{
		...
		check(PropertyBag->IsPropertyNameValid(PropertyName));
		...
		const FPropertyBagPropertyDesc* Desc = PropertyBag->FindPropertyDescByName(PropertyName);
		check(Desc);
