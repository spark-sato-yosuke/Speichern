...
static constexpr TBinarySignature<T> BinaryCompare[] =
{
	...
	[](const T& LHS, const T& RHS) { return PCG::Private::MetadataTraits<T>::Equal(LHS, RHS); }, // Adapter for 'Equal', as it changed from binary to ternary with tolerance
...
bool FPCGSelectGrammarElement::ExecuteInternal(FPCGContext* InContext) const
{
	...
	{
		const UPCGBasePointData* InputPointData = Cast<UPCGBasePointData>(PointInput.Data);
		...
		UPCGBasePointData* OutputPointData = CastChecked<UPCGBasePointData>(OutputData);
