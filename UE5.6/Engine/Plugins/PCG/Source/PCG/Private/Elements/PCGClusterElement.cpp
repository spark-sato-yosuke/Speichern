...
namespace PCGClusterElement
{
	namespace Constants
	{
		...
		const FName ClusterIDAttributeDefaultName = TEXT("ClusterID");
		const FName FinalCentroidElementCountAttributeDefaultName = TEXT("ClusterElementCount");
	}
	...
	namespace Algorithm
	{
		...
		bool KMeans(FPCGContext* Context, FClusteringData& ClusteringData, const int MaxIterations)
		{
			...
			const TConstPCGValueRange<FTransform> TransformRange = ClusteringData.OutputData->GetConstTransformValueRange();
			...
			while (ClusteringData.Iteration < MaxIterations)
			{
				...
				auto ProcessPoints = [&TransformRange, &Centroids, &bConverged = ClusteringData.bConverged](const int32& PointIndex, int32& InOutCentroidIndex)
				{
					...
					double MinDistanceSquared = FVector::DistSquared(TransformRange[PointIndex].GetLocation(), Centroids[InOutCentroidIndex]);
					for (CentroidIndex = 0; CentroidIndex < Centroids.Num(); ++CentroidIndex)
					{
						...
						const double DistanceSquared = FVector::DistSquared(TransformRange[PointIndex].GetLocation(), Centroids[CentroidIndex]);
				...
				auto ProcessCentroids = [TransformRange, ClusterAssignments](const int32 CentroidIndex, FVector& OutResult)
				{
					...
					for (int PointIndex = 0; PointIndex < TransformRange.Num(); ++PointIndex)
					{
						if (ClusterAssignments[PointIndex] == CentroidIndex)
						{
							CentroidSum += TransformRange[PointIndex].GetLocation();
							++Count;
						}
					}
...
UPCGClusterSettings::UPCGClusterSettings(const FObjectInitializer& ObjectInitializer)
	: Super(ObjectInitializer)
{
	ClusterAttribute.SetAttributeName(PCGClusterElement::Constants::ClusterIDAttributeDefaultName);
	FinalCentroidElementCountAttribute.SetAttributeName(PCGClusterElement::Constants::FinalCentroidElementCountAttributeDefaultName);
}
...
bool FPCGClusterElement::PrepareDataInternal(FPCGContext* InContext) const
{
	...
	Context->InitializePerExecutionState([&CentroidInputs, Settings](const ContextType* Context, ExecStateType& OutState)
	{
		...
		else
		{
			...
			const UPCGBasePointData* CentroidPoints = Cast<UPCGBasePointData>(CentroidInputs[0].Data);
			if (!CentroidPoints || CentroidPoints->IsEmpty())
			{
				return EPCGTimeSliceInitResult::NoOperation;
			}

			OutState.InitialCentroids.Reserve(CentroidPoints->GetNumPoints());
			const TConstPCGValueRange<FTransform> TransformRange = CentroidPoints->GetConstTransformValueRange();
			for (const FTransform& Transform : TransformRange)
			{
				OutState.InitialCentroids.Emplace(Transform.GetLocation());
			}
		}
	...
	auto InitializeIteration = [&PointInputs, Settings, Context](IterStateType& OutState, const ExecStateType& ExecState, const uint32 IterationIndex)
	{
		...
		const UPCGBasePointData* InputPointData = Cast<UPCGBasePointData>(PointInputs[IterationIndex].Data);
		...
		if (!OutPointData->HasSpatialDataParent())
		{
			UPCGBasePointData::SetPoints(InputPointData, OutPointData, {}, /*bCopyAll=*/true);
		}

		UPCGMetadata* OutPointMetadata = OutPointData->MutableMetadata();
		check(OutPointMetadata);

		const FPCGAttributePropertySelector& Selector = Settings->ClusterAttribute;
		if (Selector.IsBasicAttribute())
		{
			PCGMetadataElementCommon::ClearOrCreateAttribute(OutPointMetadata, Selector, 0, Context);
		}
...
PRAGMA_ENABLE_DEPRECATION_WARNINGS
		...
		auto InitializeCentroids = [Settings, NumClusters, NumPoints, &OutState, &ExecState]
		{
			...
			{
				const TConstPCGValueRange<FTransform> TransformRange = OutState.OutputData->GetConstTransformValueRange();
				...
				{
					OutState.Centroids.Emplace(TransformRange[StartingIndices[i]].GetLocation());
				}
			}
			else
			{
		...
		switch (Settings->Algorithm)
		{
			...
			case EPCGClusterAlgorithm::EM:
			{
				...
				for (const FTransform& Transform : TransformRange)
				{
					MaxSquaredLength = FMath::Max(MaxSquaredLength, Transform.GetLocation().SquaredLength());
				}
				...
				OutState.EMData.Probabilities.Initialize(OutState.OutputData->GetNumPoints(), NumClusters);

				OutState.EMData.NormalizedPointLocations.Reserve(OutState.OutputData->GetNumPoints());
				Algo::Transform(TransformRange, OutState.EMData.NormalizedPointLocations, [ScalingFactor = OutState.EMData.ScalingFactor](const FTransform& Transform)
				{
					return Transform.GetLocation() * ScalingFactor;
				});
...
bool FPCGClusterElement::ExecuteInternal(FPCGContext* InContext) const
{
	...
	return ExecuteSlice(TimeSlicedContext, [Settings](ContextType* Context, const ExecStateType& ExecState, IterStateType& ClusteringData, const uint32 IterIndex)
	{
		...
		if (Settings->bOutputFinalCentroids)
		{
			...
			UPCGBasePointData* OutputCentroidPointData = FPCGContext::NewPointData_AnyThread(Context);
			OutputCentroidPointData->SetNumPoints(OutCentroids.Num());
			OutputCentroidPointData->SetDensity(1.0f);
			OutputCentroidPointData->AllocateProperties(EPCGPointNativeProperties::Transform | EPCGPointNativeProperties::Seed);

			FPCGPointValueRanges OutRanges(OutputCentroidPointData, /*bAllocate=*/false);
			for (int32 i = 0; i < OutCentroids.Num(); ++i)
			{
				OutRanges.TransformRange[i] = FTransform(OutCentroids[i]);
				OutRanges.SeedRange[i] = PCGHelpers::ComputeSeedFromPosition(OutCentroids[i]);
			}
			...
			if (Settings->bOutputFinalCentroidElementCount)
			{
				check(OutputCentroidPointData->MutableMetadata());

				const FPCGAttributePropertyOutputNoSourceSelector& Selector = Settings->FinalCentroidElementCountAttribute;
				if (Selector.IsBasicAttribute())
				{
					PCGMetadataElementCommon::ClearOrCreateAttribute(OutputCentroidPointData->MutableMetadata(), Selector, 0, Context);
				}

				const TUniquePtr<IPCGAttributeAccessor> Accessor = PCGAttributeAccessorHelpers::CreateAccessor(OutputCentroidPointData, Selector);
				const TUniquePtr<IPCGAttributeAccessorKeys> Keys = PCGAttributeAccessorHelpers::CreateKeys(OutputCentroidPointData, Selector);
				if (!Accessor || !Keys)
				{
					PCGLog::Metadata::LogFailToCreateAccessorError(Selector, Context);
					return true;
				...
				TArray<int32, TInlineAllocator<16>> CentroidElementCount;
				CentroidElementCount.SetNumZeroed(OutCentroids.Num());
				// Tabulate the count of each ClusterID
				for (int32 PointIndex = 0; PointIndex < ClusteringData.PointToClusterAssignments.Num(); ++PointIndex)
				{
					++CentroidElementCount[ClusteringData.PointToClusterAssignments[PointIndex]];
				}

				Accessor->SetRange(MakeConstArrayView(CentroidElementCount), 0, *Keys);
			}
		}
