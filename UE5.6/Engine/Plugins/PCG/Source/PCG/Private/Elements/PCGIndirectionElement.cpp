...
void FPCGIndirectionContext::SetupInnerContext()
{
	...
	InnerContext = InnerElement->Initialize(FPCGInitializeElementParams(&InnerInput, ExecutionSource.Get(), nullptr));
	check(InnerContext);
	InitializeGraphExecutor(InnerContext);
	InnerContext->InitializeSettings();

	// Unclear whether we should give those new values
	InnerContext->TaskId = TaskId;
	InnerContext->CompiledTaskId = CompiledTaskId;
	InnerContext->DependenciesCrc = DependenciesCrc;

	InnerContext->AsyncState = AsyncState;
}
...
void FPCGIndirectionContext::AddExtraStructReferencedObjects(FReferenceCollector& Collector)
{
	...
	if (InnerContext)
	{
		InnerContext->AddExtraStructReferencedObjects(Collector);
	}
}
...
bool FPCGIndirectionElement::PrepareDataInternal(FPCGContext* InContext) const
{
	...
	if(UPCGSettings* InnerSettings = Context->InnerSettings)
	{
		...
		if (!Context->InnerElement->ConvertInputsIfNeeded(Context))
		{
			return false;
		}

		Context->SetupInnerContext();
