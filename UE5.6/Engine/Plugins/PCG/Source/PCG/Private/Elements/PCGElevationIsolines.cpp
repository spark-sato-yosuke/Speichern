...
TArray<FPCGPinProperties> UPCGElevationIsolinesSettings::InputPinProperties() const
{
	...
	Properties.Emplace(TEXT("BoundingShape"), EPCGDataType::Spatial);
...
bool FPCGElevationIsolinesElement::ExecuteInternal(FPCGContext* InContext) const
{
	...
	if (!BoundingShape && InContext->ExecutionSource.IsValid())
	{
		check(!bOutUnionDataCreated);
		BoundingShape = Cast<UPCGSpatialData>(InContext->ExecutionSource->GetExecutionState().GetSelfData());
	}
		...
		for (double Elevation = Settings->ElevationStart; Elevation < Settings->ElevationEnd; Elevation += Settings->ElevationIncrement)
		{
			for (const PCGSpatialAlgo::FPCGMarchingSquareResult& Result : PCGSpatialAlgo::MarchingSquares(CellXCount, CellYCount, Elevation, GetElevation, /*bUseLinearInterpolation=*/true))
			{
				...
				else
				{
					...
					OutPointData->SetNumPoints(FinalTransforms.Num());
					OutPointData->SetBoundsMin(CellBounds.Min);
					OutPointData->SetBoundsMax(CellBounds.Max);
					OutPointData->AllocateProperties(EPCGPointNativeProperties::Transform);
					
					TPCGValueRange<FTransform> TransformRange = OutPointData->GetTransformValueRange(/*bAllocate=*/false);
					for (int32 Index = 0; Index < FinalTransforms.Num(); ++Index)
					{
						TransformRange[Index] = FinalTransforms[Index];
					}
