...
namespace PCGSubgraphHelpersExtra
{
	...
	*/
	...
	void FCollapsingInformation::CopyNodes(const TArray<UObject*>& ExtraNodesToCollapse)
	{
		...
		TArray<FPCGGraphCommentNodeData> NewCommentNodes;
		
		for (const UObject* ExtraNode : ExtraNodesToCollapse)
		{
			// Special treatment of comment nodes
			if (const UEdGraphNode_Comment* CommentNode = Cast<const UEdGraphNode_Comment>(ExtraNode))
			{
				NewCommentNodes.Emplace_GetRef().InitializeFromCommentNode(*CommentNode);
			}
			else
			{
		...
		NewGraph->SetCommentNodes(std::move(NewCommentNodes));
#endif ///WITH_EDITOR
	}
	...
	void FCollapsingInformation::DeleteNodes(const TArray<UObject*>& ExtraNodesToCollapse)
	{
		...
		for (const UObject* ExtraNode : ExtraNodesToCollapse)
		{
			...
			if (const UEdGraphNode_Comment* CommentNode = Cast<const UEdGraphNode_Comment>(ExtraNode))
			{
				OriginalGraph->RemoveCommentNode(CommentNode->NodeGuid);
			}
			else
			{
