...
namespace PCGSettingsHelpers
{
	...
	static thread_local TMap<const UPCGPin*, EPCGDataType, TInlineSetAllocator<32>> VisitedPinsMap;
	...
	EPCGDataType FPinTypeScopeHelper::GetCurrentPinType(const UPCGPin* InPin)
	{
		...
		if (EPCGDataType* ExistingResult = bVisitedPinsMapExists ? VisitedPinsMap.Find(InPin) : nullptr)
		{
			return *ExistingResult;
		}
		else if(InPin)
		{
			EPCGDataType PinResult = InPin->GetCurrentTypes();
