...
namespace PCGGraphParametersHelpersPrivate
{
	...
	static const FName NewGraphParameterDefaultName(LOCTEXT("NewGraphParameterDefaultName", "NewProperty").ToString());
	static constexpr uint64 MaxIncrementalNameIterations = 100;
...
{
	bool GenerateUniqueName(const UPCGGraphInterface* InGraph, FName& InOutName)
	{
		if (!InGraph)
		{
			return false;
		...
		{
			InOutName = PCGGraphParametersHelpersPrivate::NewGraphParameterDefaultName;
		}

		const FInstancedPropertyBag* UserParametersStruct = InGraph->GetUserParametersStruct();
		check(UserParametersStruct);

		// Validate that the property name doesn't exist... if it does, increment until it doesn't
		uint64 NameIteration = 1;
		while (UserParametersStruct->FindPropertyDescByName(InOutName))
		{
			if (++NameIteration > PCGGraphParametersHelpersPrivate::MaxIncrementalNameIterations)
			{
				return false;
			}

			InOutName.SetNumber(NameIteration);
		}

		return true;
	}
} // PCGGraphParameter::Helpers
