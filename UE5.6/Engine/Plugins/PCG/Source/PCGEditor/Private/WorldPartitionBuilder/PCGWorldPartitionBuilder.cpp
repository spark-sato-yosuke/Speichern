...
struct FPCGWorldPartitionBuilder
{
	...
	static TArray<UPackage*> GetDirtyPackages(const TSet<FString>& AllowedAssetPackages);
	...
	/** Returns when GraphExecutor is done running */
	static void WaitOnGraphExecutor(UWorld* InWorld);

	/** Allows to temporarly disable the clearing of results */
	static void SetDisableClearResults(UPCGSubsystem* InSubsystem, bool bInDisableClearResults);
...
/** Disable clearing of results while in this scope */
struct FPCGDisableClearResults
{
	FPCGDisableClearResults(UWorld* InWorld)
	{
		Subsystem = UWorld::GetSubsystem<UPCGSubsystem>(InWorld);

		if(Subsystem)
		{
			FPCGWorldPartitionBuilder::SetDisableClearResults(Subsystem, true);
		}
	}

	virtual ~FPCGDisableClearResults()
	{
		if (Subsystem)
		{
			FPCGWorldPartitionBuilder::SetDisableClearResults(Subsystem, false);
		}
	}
...
bool FPCGWorldPartitionBuilderArgs::InitializeFrom(const FPCGWorldPartitionCommandlineArgs& CommandlineArgs, FPCGWorldPartitionBuilderArgs& OutArgs)
{
	...
	if (SettingsPath.IsValid() || SettingsPath.IsPending())
	{
		...
		{
			UE_LOG(LogPCGWorldPartitionBuilder, Error, TEXT("Failed to load PCGBuilderSettings '%s'"), *SettingsPath.ToString());
			return false;
		}
	}
	...
	// Log DataLayers
	TArray<FString> DataLayersToString;
	Algo::Transform(BuilderArgs.IncludedDataLayers, DataLayersToString, [](const TSoftObjectPtr<UDataLayerAsset>& DataLayerAsset) { return DataLayerAsset.ToString(); });

	UE_LOG(LogPCGWorldPartitionBuilder, Display, TEXT("IncludedDataLayers '%s' (%s)"),
		DataLayersToString.IsEmpty() ? TEXT("None") : *FString::Join(DataLayersToString, TEXT(",")),
		FromDefault);

	DataLayersToString.Empty();
	Algo::Transform(BuilderArgs.ExcludedDataLayers, DataLayersToString, [](const TSoftObjectPtr<UDataLayerAsset>& DataLayerAsset) { return DataLayerAsset.ToString(); });

	UE_LOG(LogPCGWorldPartitionBuilder, Display, TEXT("ExcludedDataLayers '%s' (%s)"),
		DataLayersToString.IsEmpty() ? TEXT("None") : *FString::Join(DataLayersToString, TEXT(",")),
		FromDefault);
...
void UPCGWorldPartitionBuilderHelper::AddAssetToSubmitAllowList(const TSoftObjectPtr<UObject>& InPath)
{
	for (TObjectIterator<UPCGWorldPartitionBuilder> It; It; ++It)
	{
		It->AddAssetToSubmitAllowList(InPath);
	}
}

void UPCGWorldPartitionBuilder::AddAssetToSubmitAllowList(const TSoftObjectPtr<UObject>& InPath)
{
	UE_LOG(LogPCGWorldPartitionBuilder, Display, TEXT("Added Asset to Submit Allow List: '%s'"), *InPath.ToString());
	AllowedAssetPackages.Add(InPath.GetLongPackageName());
}
...
bool UPCGWorldPartitionBuilder::PreRun(UWorld* World, FPackageSourceControlHelper& PackageHelper)
{
	...
	if (!FPCGWorldPartitionBuilderArgs::InitializeFrom(CommandlineArgs, Args))
	{
		return false;
	}

	// Reset Allow list
	AllowedAssetPackages.Empty();
	...
	bIncludeInitiallyActivatedRuntimeDataLayers = Args.bLoadActivatedRuntimeDataLayers;

	IncludedDataLayers.Append(Args.IncludedDataLayers);
	ExcludedDataLayers.Append(Args.ExcludedDataLayers);
...
bool UPCGWorldPartitionBuilder::RunEntireWorld(UWorld* World, FPackageSourceControlHelper& PackageHelper)
{
	...
	{
		// Wait for other executions that might have spawned from generating the components
		FPCGWorldPartitionBuilder::WaitOnGraphExecutor(World);
		
		DirtyPackages = FPCGWorldPartitionBuilder::GetDirtyPackages(AllowedAssetPackages);
	}
...
bool UPCGWorldPartitionBuilder::RunPerCell(UWorld* InWorld, const FCellInfo& InCellInfo, FPackageSourceControlHelper& PackageHelper)
{
	...
	if (LoaderAdapter)
	{
		// Update Mappings for new loaded PAs from Expanded Bounds
		bCellHasChanges |= FPCGWorldPartitionBuilder::UpdatePartitionedActors(InWorld, PartitionedComponents, CellDeletedPackageNames);
	}
	...
	{
		// Wait for other executions that might have spawned from generating the components
		FPCGWorldPartitionBuilder::WaitOnGraphExecutor(InWorld);
...
{
	TArray<UPackage*> DirtyPackages = FPCGWorldPartitionBuilder::GetDirtyPackages(AllowedAssetPackages);
	TArray<UPackage*> PackagesToDelete;
	...
	bool& bOutGenerationErrors)
{
	...
	{
		// While we process a batch of inter-dependent components (InTaskDependencies) we want to prevent the Subsystem's GraphExecutor from clearing its outputs.
		// This is specially important when InArgs.bOneComponentAtATime is true which waits for a component generation to end before scheduling another one
		// which will tick the world and the Subsystem's GraphExecutor will become idle and clear it's outputs.
		// If that following component generation is dependant on the previous one through its InTaskDependencies then it will never be allowed to run as the outputs will no longer contain
		// the previous component's results.
		// Important: the list of component created sorted by dependency so that we can rely on the order of scheduling here
		const FPCGDisableClearResults DisableClearResults(InWorld);
...
void FPCGWorldPartitionBuilder::WaitOnGraphExecutor(UWorld* InWorld)
{
	UPCGSubsystem* PCGSubsystem = UWorld::GetSubsystem<UPCGSubsystem>(InWorld);
	check(PCGSubsystem);

	while (PCGSubsystem->IsAnyGraphCurrentlyExecuting())
	{
		FWorldPartitionHelpers::FakeEngineTick(InWorld);
	}
...
void FPCGWorldPartitionBuilder::SetDisableClearResults(UPCGSubsystem* InSubsystem, bool bInDisableClearResults)
{
	if (InSubsystem)
	{
		InSubsystem->SetDisableClearResults(bInDisableClearResults);
	}
}

TArray<UPackage*> FPCGWorldPartitionBuilder::GetDirtyPackages(const TSet<FString>& AllowedAssetPackages)
{
	TArray<UPackage*> DirtyPackages;
	FEditorFileUtils::GetDirtyWorldPackages(DirtyPackages);

	if (!AllowedAssetPackages.IsEmpty())
	{
		TArray<UPackage*> DirtyAssetPackages;
		FEditorFileUtils::GetDirtyContentPackages(DirtyAssetPackages);

		for (UPackage* DirtyAssetPackage : DirtyAssetPackages)
		{
			if (AllowedAssetPackages.Contains(DirtyAssetPackage->GetName()))
			{
				DirtyPackages.Add(DirtyAssetPackage);
			}
		}
	}
