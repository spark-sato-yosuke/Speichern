...
struct FPCGWorldPartitionBuilderArgs
{
	...
	static bool InitializeFrom(const FPCGWorldPartitionCommandlineArgs& CommandlineArgs, FPCGWorldPartitionBuilderArgs& OutArgs);
};

UCLASS()
class UPCGWorldPartitionBuilderHelper : public UBlueprintFunctionLibrary
{
	GENERATED_BODY()
...
public:
	...
	void AddAssetToSubmitAllowList(const TSoftObjectPtr<UObject>& SoftObjectPath);
...
private:
	...
	/** Non Actor assets that are allowed to be submitted by the builder */
	TSet<FString> AllowedAssetPackages;
