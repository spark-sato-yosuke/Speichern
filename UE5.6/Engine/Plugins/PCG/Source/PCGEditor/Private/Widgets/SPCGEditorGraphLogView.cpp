...
FReply SPCGEditorGraphLogView::Clear()
{
	...
	if (IPCGGraphExecutionSource* ExecutionSource = GetExecutionSource())
	{
		ExecutionSource->GetExecutionState().GetExtraCapture().ResetCapturedMessages();
		Refresh();
	}
...
void SPCGEditorGraphLogView::CreateAndAddItem(const UPCGEditorGraphNode* InPCGEditorNode, const UPCGNode* InPCGNode, const FName& InName)
{
	...
	IPCGGraphExecutionSource* ExecutionSource = GetExecutionSource();

	if (!ExecutionSource || !InPCGEditorNode || !InPCGNode)
	{
		return;
	}

	const TArray<PCGUtils::FCapturedMessage>* CapturedMessagesPtr = ExecutionSource->GetExecutionState().GetExtraCapture().GetCapturedMessages().Find(InPCGNode);
...
IPCGGraphExecutionSource* SPCGEditorGraphLogView::GetExecutionSource() const
{
