...
void FOpenColorIOTransformResource::SetupResource(ERHIFeatureLevel::Type InFeatureLevel, const FString& InShaderCodeHash, const FString& InShadercode, const FString& InRawConfigHash, const FString& InFriendlyName, const FName& InAssetPath, FShaderParametersMetadata* InShaderParameterMetadata)
{
	// Take ownership of the parameter metadata
	ShaderParametersMetadata = TSharedPtr<FShaderParametersMetadata>(InShaderParameterMetadata);

	// When this happens we assume that shader was cooked and we don't need to do anything else.
	...
	if (!ShaderParametersMetadata.IsValid())
	{
		UE_LOG(LogOpenColorIO, Error, TEXT("OpenColorIO shader transform resource is expecting a valid shader parameter metadata for compilation."));
	}
...
TSharedPtr<const FShaderParametersMetadata, ESPMode::ThreadSafe> FOpenColorIOTransformResource::GetShaderParamMetadata() const
{
