...
struct FWaveTableSettings
{
...
#if WITH_EDITOR
	UE_API void VersionPCMData();
#endif // WITH_EDITOR

	// Returns the respective offset & number of samples that
	// characterize the source data to apply light edits to.
	UE_API void GetEditSourceBounds(int32& OutSourceTopOffset, int32& OutSourceNumSamples) const;
	...
	UE_API TArrayView<const float> GetEditSourceView() const;
