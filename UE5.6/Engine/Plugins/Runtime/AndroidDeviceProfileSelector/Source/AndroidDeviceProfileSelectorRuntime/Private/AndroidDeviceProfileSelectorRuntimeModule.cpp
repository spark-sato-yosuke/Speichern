...

		{
			...
			if (bForceEmulatorProfileSelectionInNonShippingBuilds && DeviceMake == TEXT("Google"))
			{
				if (DeviceModel == TEXT("HPE device"))
				{
					...
					UE_LOG(LogAndroid, Log, TEXT("Forced Profile: [Android_PC_Emulator]"));
					return ProfileName;
				}
				
				if (AndroidParams.FindChecked(FAndroidProfileSelectorSourceProperties::SRC_GPUFamily).StartsWith(TEXT("Android Emulator")))
				{
					ProfileName = TEXT("Android_Emulator");
					UE_LOG(LogAndroid, Log, TEXT("Forced Profile: [Android_Emulator]"));
					return ProfileName;
				}
			}
		}
