...
{
	FlushDeferredRegistryLoad(false);
...
void UDataRegistrySubsystem::LoadAllRegistries()
{
	...
	OnPreLoadAllDataRegistries().Broadcast();
...
FPreLoadAllDataRegistriesCallback& UDataRegistrySubsystem::OnPreLoadAllDataRegistries()
{
	return OnPreLoadAllDataRegistriesCallback;
}
...
void UDataRegistrySubsystem::ApplyPreregisterMap(UDataRegistry* Registry)
{
	...
	if (FoundPreregister)
	{
		...
		for (int32 i = 0; i < FoundPreregister->Num(); i++)
		{
			...
			{
				UE_LOG(LogDataRegistry, Verbose, TEXT("ApplyPreregisterMap did not register %s with %s, this asset was already registered with the data registry."), *AssetPath.ToString(), *Registry->GetRegistryType().ToString());
			}
		}
	}
...
#if WITH_EDITOR
	if (GIsEditor && !IsRunningCommandlet() && GetDefault<UDataRegistrySettings>()->bDelayLoadingDataRegistriesUntilPIE)
	{
		bReadyForInitialization = false;
		bLoadAllRegistriesOnNextPIE = true;
	}
#endif
	if (bReadyForInitialization)
	{
...
{
	FlushDeferredRegistryLoad(true);
...
void UDataRegistrySubsystem::FlushDeferredRegistryLoad(bool bExpected) const
{
#if WITH_EDITOR
	if (bLoadAllRegistriesOnNextPIE)
	{
		// Const cast because the flush can be lazy on first access.
		UDataRegistrySubsystem* MutableThis = const_cast<UDataRegistrySubsystem*>(this);
		if (!bExpected)
		{
			UE_LOG(LogDataRegistry, Warning, TEXT("Accessed data registry map before PIE. Loading on demand."));
		}
		
		MutableThis->bReadyForInitialization = true;
		MutableThis->bLoadAllRegistriesOnNextPIE = false;
		MutableThis->LoadAllRegistries();
	}
#endif // WITH_EDITOR
}
