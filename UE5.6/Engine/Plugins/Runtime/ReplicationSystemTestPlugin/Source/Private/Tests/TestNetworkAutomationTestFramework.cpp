...
UE_NET_TEST(NetworkAutomationTest, TestPushModelEnabled)
{
#if !WITH_PUSH_MODEL
	UE_NET_FAIL("Push model support not compiled in.");
#endif

	IConsoleVariable* PushModelCVar = IConsoleManager::Get().FindConsoleVariable(TEXT("Net.IsPushModelEnabled"));
	UE_NET_ASSERT_NE(PushModelCVar, nullptr);
	UE_NET_ASSERT_TRUE(PushModelCVar->GetBool());
}

UE_NET_TEST(NetworkAutomationTest, TestRemoteObjectHandleEnabled)
{
#if !UE_WITH_REMOTE_OBJECT_HANDLE
	UE_NET_FAIL("Remote object handle support not compiled in.");
#endif
}
