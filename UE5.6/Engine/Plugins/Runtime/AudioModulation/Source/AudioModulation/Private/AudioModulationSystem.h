...
UE_TRACE_EVENT_BEGIN_EXTERN(Audio, ModulatingSourceDeactivate)
...
namespace AudioModulation
{
	...
	public:
		 ...
		 */
		void UpdateMix(const TArray<FSoundControlBusMixStage>& InStages, USoundControlBusMix& InOutMix, bool bInUpdateObject = false, float InFadeTime = -1.0f, double Duration = -1.0, bool bRetriggerOnActivation = false);
	...
	private:
		...
		void RegisterModulator(Audio::FModulatorHandleId InHandleId, TModSettings&& InModSettings, TMapType& OutProxyMap, TMap<THandleType, FModulatorHandleSet>& OutModMap, TInitFunction InInitFunction = [](THandleType){ })
		{
