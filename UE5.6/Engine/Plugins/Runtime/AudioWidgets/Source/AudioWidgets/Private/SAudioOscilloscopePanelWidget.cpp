...
{
	// Calculate number of frames, ensuring to span at least two frames so that FFixedSampledSequenceGridData can successfully calculate its grid metrics:
	const uint32 TotalFrames = FMath::Max(DataView.SampleData.Num() / DataView.NumDimensions, 2U);
	SequenceGridData = MakeShared<FFixedSampledSequenceGridData>(TotalFrames, DataView.SampleRate, RulerStyle.TicksTextFont, RulerStyle.DesiredWidth);
}
