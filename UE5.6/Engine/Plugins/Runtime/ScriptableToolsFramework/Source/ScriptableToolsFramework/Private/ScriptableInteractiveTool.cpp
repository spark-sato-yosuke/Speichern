...
{
	if (UScriptableInteractiveTool* OwningTool = ParentTool.Get())
	{
...
UWorld* UScriptableInteractiveToolPropertySet::GetWorld() const
{
	UWorld* Result = nullptr;
	if (UScriptableInteractiveTool* OwningTool = ParentTool.Get())
	{
		Result = OwningTool->GetWorld();
	}
	return Result;
}
	...
	const FToolEnumPropertyModifiedDelegate& OnModified)
{
	...
	} else if (FByteProperty* ByteProperty = UELocal::FindValidPropertyByTypeAndName<FByteProperty>(
		PropertySet, PropertyName, TEXT("WatchEnumProperty")))
	{
		int32 ArrayIndex = WatchEnumPropertyDelegates.Num();
		WatchEnumPropertyDelegates.Add(OnModified);
		...
		PropertySet->WatchProperty<int>(
			[ByteProperty, PropertySet]()->int64 {
				const void* ValuePtr = ByteProperty->ContainerPtrToValuePtr<void>(PropertySet);
				int64 Value = ByteProperty->GetSignedIntPropertyValue(ValuePtr);
				return Value;
			},
			[this, PropertySet, PropertyName, ArrayIndex](int64 NewValue) {
				WatchEnumPropertyDelegates[ArrayIndex].ExecuteIfBound(PropertySet, PropertyName, (uint8)NewValue);
	...
	EToolsFrameworkOutcomePins& Outcome)
{
	...
	ConfigureGizmoTranslation(EScriptableToolGizmoTranslation::FreeTranslate, ETransformGizmoSubElements::FreeTranslate);
	...
	ConfigureGizmoRotation(EScriptableToolGizmoRotation::FreeRotate, ETransformGizmoSubElements::FreeRotate);
	...
	ConfigureGizmoScale(EScriptableToolGizmoScale::ScaleUniform, ETransformGizmoSubElements::ScaleUniform);
	...
	EToolsFrameworkOutcomePins& Outcome)
{
	...
	if ((*Found)->ActiveTarget)
	{
		(*Found)->ActiveTarget->OnTransformChanged.RemoveAll(this);
		(*Found)->ActiveTarget->OnBeginTransformEdit.RemoveAll(this);
		(*Found)->ActiveTarget->OnEndTransformEdit.RemoveAll(this);
		(*Found)->ActiveTarget->OnTransformChangedUndoRedo.RemoveAll(this);
	}
