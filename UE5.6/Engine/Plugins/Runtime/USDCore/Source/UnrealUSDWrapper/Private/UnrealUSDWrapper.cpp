...
namespace UnrealIdentifiers
{
	...
	const TfToken LodSubtreeAPI("LodSubtreeAPI");
	const TfToken UnrealLodSubtreeLevels = TfToken("unreal:lodSubtree:levels");
	...
	const TfToken Xform = TfToken("Xform");
...
FName UnrealIdentifiers::SensorHorizontalOffsetPropertyName = GET_MEMBER_NAME_CHECKED(UCineCameraComponent, Filmback.SensorHorizontalOffset);
FName UnrealIdentifiers::SensorVerticalOffsetPropertyName = GET_MEMBER_NAME_CHECKED(UCineCameraComponent, Filmback.SensorVerticalOffset);
FName UnrealIdentifiers::ExposureCompensationPropertyName = GET_MEMBER_NAME_CHECKED(UCineCameraComponent, PostProcessSettings.AutoExposureBias);
FName UnrealIdentifiers::ProjectionModePropertyName = GET_MEMBER_NAME_CHECKED(UCineCameraComponent, ProjectionMode);
FName UnrealIdentifiers::OrthoFarClipPlanePropertyName = GET_MEMBER_NAME_CHECKED(UCineCameraComponent, OrthoFarClipPlane);
FName UnrealIdentifiers::OrthoNearClipPlanePropertyName = GET_MEMBER_NAME_CHECKED(UCineCameraComponent, OrthoNearClipPlane);
FName UnrealIdentifiers::CustomNearClipppingPlanePropertyName = GET_MEMBER_NAME_CHECKED(UCineCameraComponent, CustomNearClippingPlane);
...
#if USE_USD_SDK
FString UnrealIdentifiers::MaterialAllPurpose = UTF8_TO_TCHAR(pxr::UsdShadeTokens->allPurpose.GetString().c_str());
FString UnrealIdentifiers::MaterialPreviewPurpose = UTF8_TO_TCHAR(pxr::UsdShadeTokens->preview.GetString().c_str());
FString UnrealIdentifiers::MaterialFullPurpose = UTF8_TO_TCHAR(pxr::UsdShadeTokens->full.GetString().c_str());
FString UnrealIdentifiers::PrimvarsDisplayColor = UTF8_TO_TCHAR(pxr::UsdGeomTokens->primvarsDisplayColor.GetString().c_str());
FString UnrealIdentifiers::PrimvarsDisplayOpacity = UTF8_TO_TCHAR(pxr::UsdGeomTokens->primvarsDisplayOpacity.GetString().c_str());
FString UnrealIdentifiers::DoubleSided = UTF8_TO_TCHAR(pxr::UsdGeomTokens->doubleSided.GetString().c_str());
FString UnrealIdentifiers::ModelDrawMode = UTF8_TO_TCHAR(pxr::UsdGeomTokens->modelDrawMode.GetString().c_str());
FString UnrealIdentifiers::ModelApplyDrawMode = UTF8_TO_TCHAR(pxr::UsdGeomTokens->modelApplyDrawMode.GetString().c_str());
FString UnrealIdentifiers::UsdNamespaceDelimiter = UTF8_TO_TCHAR(pxr::SdfPathTokens->namespaceDelimiter.GetString().c_str());
#else
...
const FName UnrealIdentifiers::UniversalRenderContext = UTF8_TO_TCHAR(pxr::UsdShadeTokens->universalRenderContext.GetString().c_str());
#else
...
const FName UnrealIdentifiers::PointInstancerTag = TEXT("PointInstancer");
...
TArray<FString> UnrealUSDWrapper::RegisterPlugins(const FString& PathToPlugInfo) TSAN_SAFE
{
	return RegisterPlugins(TArray<FString>{PathToPlugInfo});
}

TArray<FString> UnrealUSDWrapper::RegisterPlugins(const TArray<FString>& PathsToPlugInfo) TSAN_SAFE
{
...
TArray<FString> UnrealUSDWrapper::GetAllSupportedFileFormats() TSAN_SAFE
{
	UE::UnrealUSDWrapper::Private::EnsureModuleIsLoaded();
	...
	for (const std::string& Ext : Extensions)
	{
		...
		Result.Emplace(UTF8_TO_TCHAR(Ext.c_str()));
	}
...
TArray<FString> UnrealUSDWrapper::GetNativeFileFormats() TSAN_SAFE
{
	UE::UnrealUSDWrapper::Private::EnsureModuleIsLoaded();
	...
	{
		Result.Emplace(UTF8_TO_TCHAR(FileExtension.c_str()));
	}
#endif	  // #if USE_USD_SDK
...
void UnrealUSDWrapper::GetNativeFileFormats(TArray<FString>& OutTextFormats, TArray<FString>& OutPossiblyBinaryFormats) TSAN_SAFE
{
	UE::UnrealUSDWrapper::Private::EnsureModuleIsLoaded();
	...
	{
		OutTextFormats.Emplace(UTF8_TO_TCHAR(Extension.c_str()));
	}
	...
	{
		OutPossiblyBinaryFormats.Emplace(UTF8_TO_TCHAR(Extension.c_str()));
	}
#endif	  // #if USE_USD_SDK
}
...
namespace UE::UnrealUSDWrapper::Private
{
	...
	)
	{
		...
		if (PopulationMask)
		{
			...
			{
				Mask.Add(pxr::SdfPath{TCHAR_TO_UTF8(*AllowedPrimPath)});
			}
		}
		...
		pxr::SdfLayerRefPtr RootLayer = pxr::SdfLayer::FindOrOpen(TCHAR_TO_UTF8(*RootIdentifierStr));
		pxr::SdfLayerRefPtr SessionLayer = pxr::SdfLayer::FindOrOpen(TCHAR_TO_UTF8(*SessionIdentifierStr));
		if (RootLayer)
		{
			if (PopulationMask)
			{
...
UE::FUsdStage UnrealUSDWrapper::NewStage(const TCHAR* FilePath)
{
	...
	UE::FUsdStage UsdStage(pxr::UsdStage::CreateNew(TCHAR_TO_UTF8(FilePath)));
	if (UsdStage)
	{
...
UE::FUsdStage UnrealUSDWrapper::GetClipboardStage()
{
	...
	if (!ClipboardStage)
	{
		...
		ClipboardStage = UE::FUsdStage{pxr::UsdStage::CreateInMemory(TCHAR_TO_UTF8(TEXT("unreal_clipboard_layer")))};
...
{
public:
	virtual void StartupModule() override TSAN_SAFE
	{
#if USE_USD_SDK
		LLM_SCOPE_BYTAG(Usd);
