...
namespace UE
{
	...
	bool FUsdPrim::GetMetadata(const TCHAR* Key, UE::FVtValue& Value) const
	{
#if USE_USD_SDK
		return Impl->PxrUsdPrim.Get().GetMetadata(pxr::TfToken{TCHAR_TO_UTF8(Key)}, &Value.GetUsdValue());
#else
		return false;
#endif	  // #if USE_USD_SDK
	}

	bool FUsdPrim::HasMetadata(const TCHAR* Key) const
	{
#if USE_USD_SDK
		return Impl->PxrUsdPrim.Get().HasMetadata(pxr::TfToken{TCHAR_TO_UTF8(Key)});
#else
		return false;
#endif	  // #if USE_USD_SDK
	}

	bool FUsdPrim::SetMetadata(const TCHAR* Key, const UE::FVtValue& Value) const
	{
#if USE_USD_SDK
		return Impl->PxrUsdPrim.Get().SetMetadata(pxr::TfToken{TCHAR_TO_UTF8(Key)}, Value.GetUsdValue());
#else
		return false;
#endif	  // #if USE_USD_SDK
	}

	bool FUsdPrim::ClearMetadata(const TCHAR* Key) const
	{
#if USE_USD_SDK
		return Impl->PxrUsdPrim.Get().ClearMetadata(pxr::TfToken{TCHAR_TO_UTF8(Key)});
#else
		return false;
#endif	  // #if USE_USD_SDK
	}
	...
	TArray<FName> FUsdPrim::GetAppliedSchemas() const
	{
		...
		{
			AppliedSchemas.Add(UTF8_TO_TCHAR(UsdSchema.GetString().c_str()));
		}
#endif	  // #if USE_USD_SDK
	...
	bool FUsdPrim::IsA(FName SchemaIdentifier) const
	{
		...
		const pxr::TfToken UsdSchemaIdentifier(TCHAR_TO_UTF8(*SchemaIdentifier.ToString()));
	...
	bool FUsdPrim::HasAPI(FName SchemaIdentifier, FName InstanceName) const
	{
		...
		const pxr::TfToken UsdSchemaIdentifier(TCHAR_TO_UTF8(*SchemaIdentifier.ToString()));
		const pxr::TfToken UsdInstanceName(TCHAR_TO_UTF8(*InstanceName.ToString()));
	...
	bool FUsdPrim::CanApplyAPI(FName SchemaIdentifier, FString* OutWhyNot) const
	{
		...
		{
			*OutWhyNot = UTF8_TO_TCHAR(WhyNot.c_str());
		}
#endif	  // #if USE_USD_SDK
	...
	{
#if USE_USD_SDK
		return FName(UTF8_TO_TCHAR(Impl->PxrUsdPrim.Get().GetName().GetString().c_str()));
#else
		return FName();
#endif	  // #if USE_USD_SDK
	}
	...
	{
#if USE_USD_SDK
		return FName(UTF8_TO_TCHAR(Impl->PxrUsdPrim.Get().GetTypeName().GetString().c_str()));
#else
		return FName();
#endif	  // #if USE_USD_SDK
	}
	...
	bool FUsdPrim::SetTypeName(FName TypeName) const
	{
		...
		return Impl->PxrUsdPrim.Get().SetTypeName(pxr::TfToken(TCHAR_TO_UTF8(*TypeName.ToString())));
#else
		return false;
#endif	  // #if USE_USD_SDK
	}
	...
	FUsdVariantSet FUsdPrim::GetVariantSet(const FString& VariantSetName) const
	{
		...
		const std::string UsdVariantSetName(TCHAR_TO_UTF8(*VariantSetName));
	...
	FUsdAttribute FUsdPrim::GetAttribute(const TCHAR* AttrName) const
	{
		...
		return FUsdAttribute(Impl->PxrUsdPrim.Get().GetAttribute(pxr::TfToken(TCHAR_TO_UTF8(AttrName))));
#else
	...
	{
#if USE_USD_SDK
		return Impl->PxrUsdPrim.Get().HasAttribute(pxr::TfToken(TCHAR_TO_UTF8(AttrName)));
#else
		return false;
#endif	  // #if USE_USD_SDK
	}
	...
	FUsdAttribute FUsdPrim::CreateAttribute(const TCHAR* AttrName, FName TypeName) const
	{
		...
		return FUsdAttribute(Impl->PxrUsdPrim.Get().CreateAttribute(
			pxr::TfToken(TCHAR_TO_UTF8(AttrName)),
			pxr::SdfSchema::GetInstance().FindType(TCHAR_TO_UTF8(*TypeName.ToString()))
		));
#else
		return FUsdAttribute{};
#endif	  // #if USE_USD_SDK
	...
	FUsdRelationship FUsdPrim::CreateRelationship(const TCHAR* RelName, bool bCustom) const
	{
		...
		return FUsdRelationship(Impl->PxrUsdPrim.Get().CreateRelationship(pxr::TfToken(TCHAR_TO_UTF8(RelName)), bCustom));
#else
	...
	{
#if USE_USD_SDK
		return FUsdRelationship(Impl->PxrUsdPrim.Get().GetRelationship(pxr::TfToken(TCHAR_TO_UTF8(RelationshipName))));
#else
		return FUsdRelationship();
#endif	  // #if USE_USD_SDK
	}
	...
	{
#if USE_USD_SDK
		return Impl->PxrUsdPrim.Get().HasRelationship(pxr::TfToken(TCHAR_TO_UTF8(RelName)));
#else
		return false;
#endif	  // #if USE_USD_SDK
	}
...
#if USE_USD_SDK
		FScopedUsdAllocs UsdAllocs;
		return Impl->PxrUsdPrim.Get().RemoveProperty(pxr::TfToken(TCHAR_TO_UTF8(*PropName.ToString())));
#else
		return false;
#endif	  // #if USE_USD_SDK
	}
