...
namespace UsdUtils
{
	...
	USDUTILITIES_API double GetUsdStageMetersPerUnit(const pxr::UsdStageRefPtr& Stage);
	USDUTILITIES_API void SetUsdStageMetersPerUnit(const pxr::UsdStageRefPtr& Stage, double MetersPerUnit);

	USDUTILITIES_API int32 GetUsdStageNumFrames(const pxr::UsdStageRefPtr& Stage);

	/**
	 * Simple API to interact with the UsdUtils' singleton StageCache from modules not setup to interact with the USD SDK directly
	 * like Interchange's Import module.
	 *
	 * Note: We do this instead of having a UsdStageCache wrapper type because when dealing with stage caches we are realistically going
	 * to be using the UsdUtils' singleton instance. The wrapper types all *own* the USD version of their objects, and so having the
	 * FUsdStageCache wrapper *refer* to an external, existing instance of the USD version of itself instead (the singleton instance)
	 * would likely be very misleading and confusing.
	 */
	USDUTILITIES_API int64 GetUsdUtilsStageCacheStageId(const pxr::UsdStageRefPtr& Stage);
	USDUTILITIES_API UE::FUsdStage FindUsdUtilsStageCacheStageId(int64 Id);
	USDUTILITIES_API int64 InsertStageIntoUsdUtilsStageCache(const pxr::UsdStageRefPtr& Stage);
	USDUTILITIES_API bool RemoveStageFromUsdUtilsStageCache(int64 StageId);
	 ...
	 * "Comp2" attached to "Comp1". If Comp2 is provided to this function, it will return "/<RootPrimName>/Actor/Child/Comp1/Comp2".
	 *
	 ...
	 */
	USDUTILITIES_API FString GetPrimPathForObject(
	...
	/**
	 * Calls GetUVSetPrimvars() followed by AssemblePrimvarsIntoPrimvarToUVIndexMap().
	 * This exists so that it can be called without having to manipulate the USD types (e.g. for Interchange)
	 */
	USDUTILITIES_API TMap<FString, int32> GetPrimvarToUVIndexMap(const pxr::UsdPrim& UsdPrim, int32 MaxNumPrimvars = USD_PREVIEW_SURFACE_MAX_UV_SETS);
	...
	/** Returns true if this prim has any authored opinion for prim kind */
	USDUTILITIES_API bool HasAuthoredKind(const pxr::UsdPrim& Prim);
	...
	/** Copy-pasted from OpenVDBImportOptions.h because that is editor-only, and we want to avoid spreading more #if WITH_EDITORs around */
	...
	/**
	 * Collects an FVolumePrimInfo for each SparseVolumeTexture that should be generated for VolumePrim.
	 * The map keys are FilePathHashes, because some of these SparseVolumeTextures may consume multiple .vdb files (e.g .animations)
	 */
	USDUTILITIES_API TMap<FString, FVolumePrimInfo> GetVolumeInfoByFilePathHash(const pxr::UsdPrim& VolumePrim);

	/**
	 * Extracts from the volume prim a desired mapping between Unreal material parameter and the volume field name
	 * (note the corresponding VolumeFieldNames field within FVolumePrimInfo)
	 */
	USDUTILITIES_API TMultiMap<FString, FString> GetVolumeMaterialParameterToFieldNameMap(const pxr::UsdPrim& VolumePrim);

	/** Gets the names of all SparseVolumeTexture material parameters within the provided material */
	USDUTILITIES_API TArray<FString> GetSparseVolumeTextureParameterNames(const UMaterial* Material);
	...
	/**
	 * Adds a reference on Prim to the layer at AbsoluteFilePath, optionally specifying a target prim inside that layer.
	 * @param Prim - Referencer prim
	 * @param AbsoluteFilePath - Path to the layer to reference (can be empty for internal references). e.g. "C:\MyFolder\Cube.usda"
	...
	/**
	 * Adds a payload on Prim pointing at the layer at AbsoluteFilePath, optionally specifying a target prim inside that layer.
	 * @param Prim - Prim to receive the payload
	 * @param AbsoluteFilePath - Path to the layer to use as payload (can be empty for internal references). e.g. "C:\MyFolder\Cube.usda"
	 * @param TargetPrimPath - Path to a specific prim to use as payload within the target layer. e.g. "/SomeOtherPrim"
	 ...
	 * @param ReferencerSchemaHandling - How to behave when Prim has a different type than the prim used by the payload.
	 *                                   Passing an unset TOptional will check the project settings for the value instead.
	 *                                   The default value is MatchReferencedType for backwards compatibility.
	 *                                   Note: If this is set to ShowDialog, the call may create an Editor transaction.
	 */
	...
	/** Parses and stores primary Stage metadata key-info pairs in the MetadataMap */
	USDUTILITIES_API void ReadStageMetaData(UE::FUsdStage Stage, TMap<FString, FString>& OutMetaDataMap);
}	 // namespace UsdUtils
