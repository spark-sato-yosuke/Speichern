...
namespace UsdUtils
{
	 ...
	 */
	USDUTILITIES_API ECanInsertSublayerResult CanInsertSubLayer(const pxr::SdfLayerRefPtr& ParentLayer, const TCHAR* SubLayerIdentifier);
	 ...
	 * It will only do this if the file is actually within the project's directory (or within its folder tree). Otherwise it will return an absolute
	 * path
	 */
	USDUTILITIES_API FString MakePathRelativeToProjectDir(const FString& Path);

	/**
	 * If InFilePathIntoUSDZArchive is a path to something inside an USDZ file, like "C:/MyFiles/scene.usdz[0/texture.png]",
	 * this will put "C:/MyFiles/scene.usdz" on OutUSDZFilePath, and "0/texture.png" on OutInnerAssetPath and return true.
	 * Returns false when it can't split the provided path in that way.
	 */
	USDUTILITIES_API bool SplitUSDZAssetPath(const FString& InFilePathIntoUSDZArchive, FString& OutUSDZFilePath, FString& OutInnerAssetPath);

	/**
	 * If InUSDZFilePath points at an actual USDZ file, this will decompress the archive and put its contents inside InOutputDir,
	 * returning true if succeeding. In that case, if OutDecompressedRootLayer is not nullptr, this function will also set it to
	 * contain the path to the root layer of the decompressed USDZ archive
	 */
	USDUTILITIES_API bool DecompressUSDZFile(const FString& InUSDZFilePath, const FString& InOutputDir, FString* OutDecompressedRootLayer = nullptr);

	/** Creates a new layer with a default prim */
	USDUTILITIES_API TUsdStore<pxr::SdfLayerRefPtr> CreateNewLayer(
	...
	USDUTILITIES_API UE::FSdfLayer FindLayerForAttributes(
		const TArray<UE::FUsdAttribute>& Attributes,
		double TimeCode,
		bool bIncludeSessionLayers = true
	);
	...
	/** Loads and returns the session sublayer that is used for storing persistent UE state, which can be saved to disk (e.g. metadata for whether an
	 * attribute is muted or not) */
	USDUTILITIES_API UE::FSdfLayer GetUEPersistentStateSublayer(const UE::FUsdStage& Stage, bool bCreateIfNeeded = true);

	/** Loads and returns the anonymous session sublayer that is used for storing transient UE session state, and won't be saved to disk (e.g. the
	 * opinion that actually mutes the attribute) */
	USDUTILITIES_API UE::FSdfLayer GetUESessionStateSublayer(const UE::FUsdStage& Stage, bool bCreateIfNeeded = true);

	/** Uses FindOrOpen to return the layer with the given identifier if possible. If the identifier is for an anonymous layer, it will search via
