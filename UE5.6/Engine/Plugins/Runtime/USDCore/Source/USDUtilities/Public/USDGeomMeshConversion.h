...
namespace UsdToUnreal
{
	...
	{
		/** This enum must match EInterchangeUsdPrimvar in InterchangeUsdDefinitions.h*/
		enum EImportPrimvar
		{
		...
		// List of the primvars that should be stored on the MeshDescription
		TSet<FString> PrimvarNames;
		...
		// How to store the primvars in the MeshDescription
		EImportPrimvar ImportPrimvars = EImportPrimvar::Standard;
	};
	 ...
	 * If SkinningQuery and SkeletonQuery are provided we parse the Skeletal Attributes and fill the FMeshDescription accordingly.
	 *
	 ...
	 * @param SkinningQuery - For computing Skinned Points, if the target and source data set is for a SkeletalMesh.
	 * @param SkeletonQuery - For computing Skinned Points, if the target and source data set is for a SkeletalMesh. (primarily for
	 * MeshToSkeletonRestPose calculations)
	...
	USDUTILITIES_API bool ConvertGeomMesh(
		...
		const pxr::UsdSkelSkinningQuery* SkinningQuery = nullptr,
		const pxr::UsdSkelSkeletonQuery* SkeletonQuery = nullptr
	);
	USDUTILITIES_API bool ConvertGeomMesh(
	 ...
	 * @param CommonOptions - Shared options used for the conversion
	 * @param bSkipRootPrimTransform - Whether to bake in the transform of SubtreeRootPrim into the mesh
	 * @param bSkipRootPrimVisiblity - Whether to bake in the visibility of SubtreeRootPrim into the mesh
	 * @return Whether the conversion was successful or not.
	 */
	USDUTILITIES_API bool ConvertGeomMeshSubtree(
		const pxr::UsdPrim& SubtreeRootPrim,
		FMeshDescription& InOutMeshDescription,
		UsdUtils::FUsdPrimMaterialAssignmentInfo& InOutMaterialAssignments,
		const FUsdMeshConversionOptions& CommonOptions = FUsdMeshConversionOptions::DefaultOptions,
	...
	UE_DEPRECATED(5.6, "Use ConvertGeomMeshSubtree instead.")
...
namespace UnrealToUsd
{
	 ...
	 */
	USDUTILITIES_API bool CreateSkeletalAnimationToMeshBaker(
		UE::FUsdPrim& MeshPrim,
		USkeletalMeshComponent& Component,
		struct FComponentBaker& OutBaker
	);
}
#endif	  // USE_USD_SDK
...
namespace UsdUtils
{
	...
	/** Returns whether this UsdPrim has the UsdPhysicsCollisionAPI schema and has enabled collisions */
	USDUTILITIES_API bool IsCollisionEnabledForPrim(const pxr::UsdPrim& UsdPrim);

	/** Returns whether this UsdPrim is a custom collision mesh */
	USDUTILITIES_API bool IsCollisionMesh(const pxr::UsdPrim& UsdPrim);

	/** Returns which collision type should be generated for the provided prim, according to the applied physics schemas */
	USDUTILITIES_API EUsdCollisionType GetCollisionApproximationType(const pxr::UsdPrim& UsdPrim);
	...
	USDUTILITIES_API void GetAnimatedMeshTimeCodes(const UE::FUsdStage& Stage, const FString& PrimPath, int32& OutStartFrame, int32& OutEndFrame);
	...
	USDUTILITIES_API EMeshTopologyVariance GetMeshTopologyVariance(const UE::FUsdPrim& UsdPrim);
