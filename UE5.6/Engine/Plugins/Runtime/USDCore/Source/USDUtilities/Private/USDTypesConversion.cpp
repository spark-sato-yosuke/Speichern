...
namespace UnrealToUsd
{
	...
	{
		return MakeUsdStore<pxr::SdfPath>(TCHAR_TO_UTF8(InString));
	}

	TUsdStore<std::string> ConvertName(const FName& InName)
	{
		return MakeUsdStore<std::string>(TCHAR_TO_UTF8(*InName.ToString()));
	}

	TUsdStore<pxr::TfToken> ConvertToken(const TCHAR* InString)
	{
		return MakeUsdStore<pxr::TfToken>(TCHAR_TO_UTF8(InString));
	}
