...
namespace UE::USDValueConversion::Private
{
	...
	// Use a control character here because USD shouldn't normally let this be used for actual SdfPaths
	const static FString ReferencePayloadDelimiter = TEXT("\x1c");

#if USE_USD_SDK
	// Functions instead of just variables to make sure we don't run into any bizarre issues with these static variables being
	// initialized before the TfTypes are registered (not sure if that is possible, but we don't need to find out)
	template<typename T>
	const FString& GetSdfTypeName()
	{
		const static FString TypeName = *UsdToUnreal::ConvertString(pxr::TfType::Find<T>().GetTypeName());
		return TypeName;
	}

	enum EListOpListIndex
	{
		Explicit = 0,
		Added,
		Deleted,
	...
	static_assert((int)EListOpListIndex::Explicit == (int)pxr::SdfListOpType::SdfListOpTypeExplicit);
	static_assert((int)EListOpListIndex::Added == (int)pxr::SdfListOpType::SdfListOpTypeAdded);
	static_assert((int)EListOpListIndex::Deleted == (int)pxr::SdfListOpType::SdfListOpTypeDeleted);
	static_assert((int)EListOpListIndex::Ordered == (int)pxr::SdfListOpType::SdfListOpTypeOrdered);
	static_assert((int)EListOpListIndex::Prepended == (int)pxr::SdfListOpType::SdfListOpTypePrepended);
	static_assert((int)EListOpListIndex::Appended == (int)pxr::SdfListOpType::SdfListOpTypeAppended);
#endif	  // USE_USD_SDK
}	 // namespace UE::USDValueConversion::Private
...
namespace UsdToUnrealImpl
{
	...
	template<typename UEType, typename USDType, typename ConverterFunc>
	void ConvertListOpValue(const pxr::VtValue& InValue, UsdUtils::FConvertedVtValue& OutValue, ConverterFunc Converter)
	{
		using namespace pxr;
		...
		OutValue.Entries.SetNum(EListOpListIndex::Num);

		const SdfListOp<USDType>& Val = InValue.UncheckedGet<SdfListOp<USDType>>();

		// Explicit ops only have explicit items
		if (Val.IsExplicit())
		{
			TArray<FConvertedVtValueComponent>& ExplicitItems = OutValue.Entries[EListOpListIndex::Explicit];

			for (const USDType& Item : Val.GetExplicitItems())
			{
				ExplicitItems.Add(UsdUtils::FConvertedVtValueComponent(TInPlaceType<UEType>(), Converter(Item)));
			}
		}
		// Non explicit ops may have items in all other lists
		else
		{
			TArray<FConvertedVtValueComponent>& AddedItems = OutValue.Entries[EListOpListIndex::Added];
			TArray<FConvertedVtValueComponent>& PrependedItems = OutValue.Entries[EListOpListIndex::Prepended];
			TArray<FConvertedVtValueComponent>& AppendedItems = OutValue.Entries[EListOpListIndex::Appended];
			TArray<FConvertedVtValueComponent>& DeletedItems = OutValue.Entries[EListOpListIndex::Deleted];
			TArray<FConvertedVtValueComponent>& OrderedItems = OutValue.Entries[EListOpListIndex::Ordered];

			for (const USDType& Item : Val.GetAddedItems())
			{
				FString Converted = Converter(Item);
				AddedItems.Add(UsdUtils::FConvertedVtValueComponent(TInPlaceType<UEType>(), Converter(Item)));
			}
			for (const USDType& Item : Val.GetPrependedItems())
			{
				FString Converted = Converter(Item);
				PrependedItems.Add(UsdUtils::FConvertedVtValueComponent(TInPlaceType<UEType>(), Converter(Item)));
			}
			for (const USDType& Item : Val.GetAppendedItems())
			{
				FString Converted = Converter(Item);
				AppendedItems.Add(UsdUtils::FConvertedVtValueComponent(TInPlaceType<UEType>(), Converter(Item)));
			}
			for (const USDType& Item : Val.GetDeletedItems())
			{
				FString Converted = Converter(Item);
				DeletedItems.Add(UsdUtils::FConvertedVtValueComponent(TInPlaceType<UEType>(), Converter(Item)));
			}
			for (const USDType& Item : Val.GetOrderedItems())
			{
				FString Converted = Converter(Item);
				OrderedItems.Add(UsdUtils::FConvertedVtValueComponent(TInPlaceType<UEType>(), Converter(Item)));
			}
		}
	}

	template<typename SdfRefOrPayload>
	FString StringifySdfReferenceOrPayload(const SdfRefOrPayload& Reference)
	{
		using namespace UE::USDValueConversion::Private;

		FString AssetPath = UsdToUnreal::ConvertString(Reference.GetAssetPath());
		FString PrimPath = UsdToUnreal::ConvertPath(Reference.GetPrimPath());
		double Offset = Reference.GetLayerOffset().GetOffset();
		double Scale = Reference.GetLayerOffset().GetScale();

		// TODO: Apparently there's a full CustomData VtDictionary inside the SdfReference that we should probably also stringify.
		// This doesn't seem to be used that often (and we never author it anyway), so we'll skip it for now

		return FString::Printf(
			TEXT("%s%s%s%s%f%s%f"),
			*AssetPath,
			*ReferencePayloadDelimiter,
			*PrimPath,
			*ReferencePayloadDelimiter,
			Offset,
			*ReferencePayloadDelimiter,
...
#define CHECK_TYPE(T) UnderlyingType.IsA<T>() || UnderlyingType.IsA<VtArray<T>>()
		...
		{
			OutValue.SourceType = EUsdBasicDataTypes::String;
			OutValue.ExtraTypeInfo = GetSdfTypeName<SdfListOp<SdfPath>>();

			ConvertListOpValue<FString, SdfPath, FString(const pxr::SdfPath&)>(	   //
				UsdValue,
				OutValue,
				UsdToUnreal::ConvertPath
			);
		}
		else if (UnderlyingType.IsA<SdfListOp<std::string>>())
		{
			OutValue.SourceType = EUsdBasicDataTypes::String;
			OutValue.ExtraTypeInfo = GetSdfTypeName<SdfListOp<std::string>>();

			ConvertListOpValue<FString, std::string, FString(const std::string&)>(	  //
				UsdValue,
				OutValue,
				UsdToUnreal::ConvertString
			);
		}
		else if (UnderlyingType.IsA<SdfListOp<SdfReference>>())
		{
			OutValue.SourceType = EUsdBasicDataTypes::String;
			OutValue.ExtraTypeInfo = GetSdfTypeName<SdfListOp<SdfReference>>();

			ConvertListOpValue<FString, SdfReference>(	  //
				UsdValue,
				OutValue,
				StringifySdfReferenceOrPayload<SdfReference>
			);
		}
		else if (UnderlyingType.IsA<SdfListOp<SdfPayload>>())
		{
			OutValue.SourceType = EUsdBasicDataTypes::String;
			OutValue.ExtraTypeInfo = GetSdfTypeName<SdfListOp<SdfPayload>>();

			ConvertListOpValue<FString, SdfPayload>(	  //
				UsdValue,
				OutValue,
				StringifySdfReferenceOrPayload<SdfPayload>
			);
		}
		// Type most commonly used for the value of the apiSchemas prim metadata
		else if (UnderlyingType.IsA<SdfListOp<TfToken>>())
		{
			OutValue.SourceType = EUsdBasicDataTypes::Token;
			OutValue.ExtraTypeInfo = GetSdfTypeName<SdfListOp<TfToken>>();

			ConvertListOpValue<FString, TfToken, FString(const pxr::TfToken&)>(	   //
				UsdValue,
				OutValue,
				UsdToUnreal::ConvertToken
			);
		}
...
namespace UnrealToUsdImpl
{
	...
	template<typename UEElementType, typename USDElementType, typename ConverterFunc>
	void ConvertListOpValue(const UsdUtils::FConvertedVtValue& InValue, pxr::VtValue& OutValue, ConverterFunc Converter)
	{
		using namespace pxr;
		using namespace UsdUtils;
		using namespace UE::USDValueConversion::Private;

		if (InValue.Entries.Num() == EListOpListIndex::Num)
		{
			SdfListOp<USDElementType> NewList;

			bool bAddedItems = false;
			for (int32 ListIndex = 0; ListIndex < EListOpListIndex::Num; ++ListIndex)
			{
				const TArray<FConvertedVtValueComponent>& UEItems = InValue.Entries[ListIndex];

				std::vector<USDElementType> UsdItems;
				UsdItems.reserve(UEItems.Num());

				for (const FConvertedVtValueComponent& UEItem : UEItems)
				{
					if (const UEElementType* Value = UEItem.TryGet<UEElementType>())
					{
						UsdItems.push_back(Converter(*Value));
					}
				}

				// It's very important that we set ExplicitItems first, and that we only set if they have at
				// least one item in the list. This because SdfListOp has essentially two "modes":
				// Explicit and not explicit. Any time we set explicit items and it wasn't in "explicit mode" yet,
				// it switches to explicit mode and dumps all previous items. Analogously, if we set any of the
				// non-explicit items and it was previously in "explicit mode", it dumps everything again and
				// switches to "non-explicit mode". So starting with explicit and only calling SetItems when we
				// have something prevents us from dumping items when we didn't mean to
				if (UsdItems.size() > 0)
				{
					NewList.SetItems(UsdItems, (SdfListOpType)ListIndex);
					bAddedItems = true;
				}
			}
	...
	template<typename SdfRefOrPayload>
	SdfRefOrPayload UnstringifySdfReferenceOrPayload(const FString& String)
	{
		using namespace UE::USDValueConversion::Private;

		TArray<FString> Elements;
		const bool bCullEmpty = false;
		int32 NumElements = String.ParseIntoArray(Elements, *ReferencePayloadDelimiter, bCullEmpty);

		SdfRefOrPayload Result;
		if (NumElements == 4)
		{
			if (Elements[0].Len() > 0)
			{
				Result.SetAssetPath(UnrealToUsd::ConvertString(*Elements[0]).Get());
			}

			if (Elements[1].Len() > 0)
			{
				Result.SetPrimPath(UnrealToUsd::ConvertPath(*Elements[1]).Get());
			}

			pxr::SdfLayerOffset LayerOffset;
			{
				double Offset = 0.0f;
				if (LexTryParseString(Offset, *Elements[2]))
				{
					LayerOffset.SetOffset(Offset);
				}

				double Scale = 1.0f;
				if (LexTryParseString(Scale, *Elements[3]))
				{
					LayerOffset.SetScale(Scale);
				}
			}
			Result.SetLayerOffset(LayerOffset);
		}
		return Result;
	}
}	 // namespace UnrealToUsdImpl
	...
	bool ConvertValue(const UsdUtils::FConvertedVtValue& InValue, UE::FVtValue& OutValue)
	{
		...
		switch (InValue.SourceType)
		{
			...
			case EUsdBasicDataTypes::String:
				if (InValue.ExtraTypeInfo == GetSdfTypeName<SdfListOp<std::string>>())
				{
					ConvertListOpValue<FString, std::string>(
						InValue,
						UsdValue,
						[](const FString& Item) -> std::string
						{
							return UnrealToUsd::ConvertString(*Item).Get();
						}
					);
				}
				else if (InValue.ExtraTypeInfo == GetSdfTypeName<SdfListOp<SdfPath>>())
				{
					ConvertListOpValue<FString, SdfPath>(
						InValue,
						UsdValue,
						[](const FString& Item) -> SdfPath
						{
							return UnrealToUsd::ConvertPath(*Item).Get();
						}
					);
				}
				else if (InValue.ExtraTypeInfo == GetSdfTypeName<SdfListOp<SdfReference>>())
				{
					ConvertListOpValue<FString, SdfReference>(	  //
						InValue,
						UsdValue,
						UnstringifySdfReferenceOrPayload<SdfReference>
					);
				}
				else if (InValue.ExtraTypeInfo == GetSdfTypeName<SdfListOp<SdfPayload>>())
				{
					ConvertListOpValue<FString, SdfPayload>(	//
						InValue,
						UsdValue,
						UnstringifySdfReferenceOrPayload<SdfPayload>
					);
				}
				else
				{
			...
			case EUsdBasicDataTypes::Token:
				if (InValue.ExtraTypeInfo == GetSdfTypeName<SdfListOp<TfToken>>())
				{
					ConvertListOpValue<FString, TfToken>(
						InValue,
						UsdValue,
						[](const FString& Item) -> TfToken
						{
							return UnrealToUsd::ConvertToken(*Item).Get();
						}
					);
				}
				else
...
namespace UsdUtils
{
	...
	FString StringifyTokenList(const pxr::VtValue& Value)
	{
		...
		TUsdStore<std::vector<pxr::TfToken>> TokenListItems = TokenList.GetAppliedItems();
		for (const pxr::TfToken& Token : TokenListItems.Get())
		{
			const std::string& String = Token.GetString();
...
namespace UsdUtils
{
	...
	bool Unstringify(const FString& String, const FString& TypeName, pxr::VtValue& Output)
	{
		...
		if (const TFunction<bool(const FString&, pxr::VtValue&)>* FoundUnstringifier = Unstringifiers.Find(TypeName))
		{
			...
			{
				USD_LOG_ERROR(TEXT("Found unstringifier for typeName '%s', but failed to unstringify string '%s'"), *TypeName, *String);
			}
		}
		else
		{
			USD_LOG_ERROR(TEXT("Failed to find unstringifier for typeName '%s'"), *TypeName);
		}
	...
	bool UnstringifyAsListOpTokens(const FString& String, TArray<FString>& OutValue)
	{
		...
		{
			std::vector<pxr::TfToken> ItemVector = InnerValue.GetAppliedItems();
