...
FMeshDiffusionIntegrator::FMeshDiffusionIntegrator(const FDynamicMesh3& DynamicMesh, const ELaplacianWeightScheme Scheme, FProgressCancel* Progress)
{
	Id = 0;
	bIsSymmetric = false;
	MinDiagonalValue = 0;
...
void FMeshDiffusionIntegrator::Initialize(const FDynamicMesh3& DynamicMesh, const ELaplacianWeightScheme Scheme)
{
	...
	FPlatformMisc::LowLevelOutputDebugString(*Foo);
#endif
