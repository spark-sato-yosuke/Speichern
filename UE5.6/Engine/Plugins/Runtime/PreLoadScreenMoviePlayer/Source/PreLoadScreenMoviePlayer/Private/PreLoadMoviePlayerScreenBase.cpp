...
{
	if (bInitialized || !MovieStreamer.IsValid())
	{
		UE_LOG(LogPreloadMoviePlayer, Log, TEXT("Init failed for PreLoadMoviePlayerScreenBase"));
		return;
	}

	InitSettingsFromConfig(GGameIni);
	if (MovieAttributes.MoviePaths.IsEmpty())
	{
		UE_LOG(LogPreloadMoviePlayer, Log, TEXT("Init failed for PreLoadMoviePlayerScreenBase, no MoviePaths defined."));
		return;
	}
	...
	MovieStreamer->Init(MovieAttributes.MoviePaths, ((EMoviePlaybackType)((int)MovieAttributes.PlaybackType)));
	...
	UE_LOG(LogPreloadMoviePlayer, Log, TEXT("Init completed for PreLoadMoviePlayerScreenBase"));
}
...
void FPreLoadMoviePlayerScreenBase::CleanUp()
{
    ...
    MovieStreamer.Reset();
	MoviePlayerContents.Reset();
}

void FPreLoadMoviePlayerScreenBase::Tick(float DeltaTime)
{
	// Make sure if focus is lost for some reason it is restored to allow skipping
	if (MoviePlayerContents.IsValid() && !MoviePlayerContents->HasAnyUserFocus())
	{
		FSlateApplication::Get().SetAllUserFocus(MoviePlayerContents);
	}
