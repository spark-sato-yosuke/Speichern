...
UObject* UMidiFileFactory::FactoryCreateFile(UClass* InClass, UObject* InParent, FName InName, EObjectFlags Flags, const FString& Filename, const TCHAR* Parms, FFeedbackContext* Warn, bool& bOutOperationCanceled)
{
	...
	UMidiFile::ELoadResult Result = MidiFileAsset->LoadStdMidiFile(Filename, Harmonix::Midi::Constants::GTicksPerQuarterNoteInt);
	if (Result != UMidiFile::ELoadResult::Success)
	{
		return nullptr;
	}
...
bool UMidiFileFactory::CanReimport(UObject* Obj, TArray<FString>& OutFilenames)
{
	if (UMidiFile* AsMidiFile = Cast<UMidiFile>(Obj))
	{
		...
		{
			UE_LOG(LogHarmonixMidiEditor, Warning, TEXT("%s: Failed to reimport with new file. Invalid extension: %s"), *AsMidiFile->GetPathName(), *FPaths::GetExtension(PreferredReimportPath));
			return false;
		}
...
EReimportResult::Type UMidiFileFactory::Reimport(UObject* Obj)
{
	...
	UObject* ImportedObject = ImportObject(AsMidiFile->GetClass(), AsMidiFile->GetOuter(), *AsMidiFile->GetName(), RF_Public | RF_Standalone, ReimportPath, nullptr, OutCanceled);
	if (ImportedObject && !OutCanceled)
	{
		// the imported object should be the existing midi file
		ensure(ImportedObject == AsMidiFile);

		CheckImportedFilesForValidLengths();
		
		if (AsMidiFile->GetOuter())
		{
			AsMidiFile->GetOuter()->MarkPackageDirty();
		}
		else
		{
			AsMidiFile->MarkPackageDirty();
		}
	}
	else if (OutCanceled)
	{
		UE_LOG(LogHarmonixMidiEditor, Warning, TEXT("import canceled"));
		return EReimportResult::Cancelled;
	}
	else
	{
		UE_LOG(LogHarmonixMidiEditor, Warning, TEXT("import failed"));
		return EReimportResult::Failed;
	}
