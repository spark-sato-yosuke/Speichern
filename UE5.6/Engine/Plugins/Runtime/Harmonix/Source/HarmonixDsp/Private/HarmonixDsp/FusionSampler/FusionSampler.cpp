...
FFusionSampler::FFusionSampler(float InSampleRateHz, bool InUsePitchShifters)
	: RampCallRateHz(1)
	, Speed(1.0f)
	...
	, VoicePool(nullptr)
	, bUsePitchShifters(InUsePitchShifters)
	...
	, KeyzoneSelectMode(EKeyzoneSelectMode::Layers)
{
	...
	if (bUsePitchShifters)
	{
		IStretcherAndPitchShifterFactory::AddClient(this, InSampleRateHz);
	}
}
...
bool FFusionSampler::TryKeyOnZone(FMidiVoiceId InVoiceId, uint8 InTriggeredNote, uint8 InTransposedNote, uint8 InVelocity, int32 InEventTick, int32 InTriggerTick, float InOffsetMs, const FKeyzoneSettings* InKeyzone)
{
	...
	IStretcherAndPitchShifterFactory* PitchShifterFactory = nullptr;
	if (bUsePitchShifters)
	{
		if (InKeyzone->TimeStretchConfig.bMaintainTime)
		{
			PitchShifterFactory = IStretcherAndPitchShifterFactory::FindBestFactory(InKeyzone->TimeStretchConfig);
		}
	}
	...
	FFusionVoice* Voice = VoicePool->GetFreeVoice(this, InVoiceId, InKeyzone, [this](FFusionVoice* Voice) { return RelinquishVoice(Voice); }, PitchShifterFactory);
	if (Voice == nullptr)
	{
