...
void UFusionPatch::PostEditChangeChainProperty(FPropertyChangedChainEvent& PropertyChangedChainEvent)
{
	...
	const FProperty* ChildChanged = PropertyChangedChainEvent.PropertyChain.GetHead()->GetNextNode()->GetValue();
	if (ChildChanged && ChildChanged->GetFName() == GET_MEMBER_NAME_CHECKED(FFusionPatchData, Settings))
	{
		UE_LOG(LogFusionPatch, Log, TEXT("Fusion patch settings changed"));
		bSettingsChanged = true;
	}

	const FFusionPatchSettings& DefaultSettings = FusionPatchData.Settings;
	for (FKeyzoneSettings& Keyzone : FusionPatchData.Keyzones)
	{
		...
		// update keyzone ADSR default settings for non-overridden keyzones
		// maintains continuity when deciding to override ADSR settings later.
		// only update the keyzones if the default settings have changed
		if (bSettingsChanged && !Keyzone.bOverrideAdsr)
		{
			Keyzone.Adsr = DefaultSettings.Adsr[0];
		}
	}
...
void UFusionPatch::PostLoad()
{
	...
	// apply the default settings to each keyzone ADSR on post load
	// only do this for keyzones that are _NOT_ currently overridden
	// that way, when overriding a keyzone, it'll initialize with the current patch
	const FFusionPatchSettings& DefaultSettings = FusionPatchData.Settings;
	for (FKeyzoneSettings& Keyzone : FusionPatchData.Keyzones)
	{
		if (!Keyzone.bOverrideAdsr)
		{
			Keyzone.Adsr = DefaultSettings.Adsr[0];
		}
	}
