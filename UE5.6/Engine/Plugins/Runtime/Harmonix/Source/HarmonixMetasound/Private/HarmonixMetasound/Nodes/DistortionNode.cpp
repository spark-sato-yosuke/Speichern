...
#define METASOUND_GET_PARAM_NAME_WITH_INDEX_AND_METADATA_ADVANCED_DISPLAY(NAME, INDEX) METASOUND_GET_PARAM_NAME_WITH_INDEX(NAME, INDEX), FDataVertexMetadata { NAME##Tooltip, FText::Format(NAME##DisplayName, INDEX), true }
		...
		static TUniquePtr<IOperator> CreateOperator(const FBuildOperatorParams& InParams, FBuildResults& OutResults)
		{
			...
			FAudioBufferReadRef InAudio = InputData.GetOrCreateDefaultDataReadReference<Metasound::FAudioBuffer>(
			...
			FDistortionTypeReadRef InDistortionType = InputData.GetOrCreateDefaultDataReadReference<FEnumDistortionType>(
				METASOUND_GET_PARAM_NAME(Inputs::DistortionType), InParams.OperatorSettings);
			...
			{			
				return FFilterSettings(
					...
					InputData.GetOrCreateDefaultDataReadReference<FEnumHmxBiquadFilterType>(METASOUND_GET_PARAM_NAME_WITH_INDEX(Inputs::FilterType, Index), InParams.OperatorSettings),
					...
					InputData.GetOrCreateDefaultDataReadReference<FEnumDistortionFilterPasses>(METASOUND_GET_PARAM_NAME_WITH_INDEX(Inputs::Passes, Index), InParams.OperatorSettings)
				);
			};
	...
	using FDistortionNode = Metasound::TNodeFacade<FDistortionOperator>;
