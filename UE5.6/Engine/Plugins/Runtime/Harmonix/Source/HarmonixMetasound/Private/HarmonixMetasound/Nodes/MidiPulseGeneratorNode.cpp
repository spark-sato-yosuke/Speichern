...
namespace HarmonixMetasound::Nodes::MidiPulseGeneratorNode
{
	...
	public:
		...
		static TUniquePtr<IOperator> CreateOperator(const Metasound::FBuildOperatorParams& InParams, Metasound::FBuildResults& OutResults)
		{
			FInputs Inputs
			{
				InParams.InputData.GetOrCreateDefaultDataReadReference<FMidiClock>(Inputs::MidiClockName, InParams.OperatorSettings),
		...
		void Execute()
		{
			...
			PulseGenerator.Process(*Inputs.Clock, *Outputs.MidiStream);
		}
	private:
	...
	using FMidiPulseGeneratorNode = Metasound::TNodeFacade<FMidiPulseGeneratorOperator>;
