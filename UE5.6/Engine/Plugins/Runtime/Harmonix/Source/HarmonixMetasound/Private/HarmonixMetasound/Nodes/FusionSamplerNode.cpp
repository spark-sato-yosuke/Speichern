...
namespace HarmonixMetasound
{
	...
	{
		METASOUND_PARAM(MidiTrackNumber, "Track Number", "Track number (1 based). 0 will play all tracks (track-omni).")
		...
		METASOUND_PARAM(UsePitchShifters, "Use Pitch Shifters", "Set whether to use pitch shifters. If Disabled, pitch shifting will be disabled, and voices that are set to \"Maintain Time\" won't play.")
	...
	FAutoConsoleVariableRef CVarMetaSoundProfileAllEnabled(
		TEXT("au.Fusion.DisableThreadedRender"),
	...
	*********************************************************************************************************/
	...
	public:
		struct FConstructionArgs
		{
			...
			FConstructionArgs(const FBuildOperatorParams& InParams, EAudioBufferChannelLayout OutputChannelLayout)
				...
				, InMidiStream(InParams.InputData.GetOrCreateDefaultDataReadReference<FMidiStream>(METASOUND_GET_PARAM_NAME(CommonPinNames::Inputs::MidiStream), InParams.OperatorSettings))
				, InTrackNumber(InParams.InputData.GetOrCreateDefaultDataReadReference<int32>(METASOUND_GET_PARAM_NAME(FusionSamplerNodePinNames::MidiTrackNumber), InParams.OperatorSettings))
				...
				, InPatch(InParams.InputData.GetOrCreateDefaultDataReadReference<FFusionPatchAsset>(METASOUND_GET_PARAM_NAME(CommonPinNames::Inputs::SynthPatch), InParams.OperatorSettings))
			    ...
			    , InUsePitchShifters(InParams.InputData.GetOrCreateDefaultValue<bool>(METASOUND_GET_PARAM_NAME(FusionSamplerNodePinNames::UsePitchShifters), InParams.OperatorSettings))
		...
		static const FInputVertexInterface& GetInputVertexInterface()
		{
			...
			static const FInputVertexInterface InputInterface = FInputVertexInterface(
				...
				TInputDataVertex<int32>(METASOUND_GET_PARAM_NAME_AND_METADATA(FusionSamplerNodePinNames::MidiTrackNumber), 1),
				...
				TInputConstructorVertex<bool>(METASOUND_GET_PARAM_NAME_AND_METADATA_ADVANCED(FusionSamplerNodePinNames::UsePitchShifters), false)
			);
		...
		FFusionSamplerOperatorBase(const FConstructionArgs& Args)
			: FFusionSampler(Args.Params.OperatorSettings.GetSampleRate(), Args.InUsePitchShifters)
			...
			, UsePitchShifters(Args.InUsePitchShifters)
		...
		virtual void BindInputs(FInputVertexInterfaceData& InVertexData) override
		{
			...
			InVertexData.BindReadVertex(METASOUND_GET_PARAM_NAME(FusionSamplerNodePinNames::MidiTrackNumber), TrackNumberInPin);
			...
			InVertexData.SetValue(METASOUND_GET_PARAM_NAME(FusionSamplerNodePinNames::UsePitchShifters), UsePitchShifters);
			Init();
		}
	...
	private:
		...
		TSharedPtr<const FMidiClock, ESPMode::NotThreadSafe> MidiClock;
	...
	*********************************************************************************************************/
	...
	public:
		...
		static TUniquePtr<IOperator> CreateOperator(const FBuildOperatorParams& InParams, FBuildResults& OutResults)
		{
			...
			const TNodeFacade<FFusionSamplerOperator<NUM_CHANNELS>>& PlayerNode = static_cast<const TNodeFacade<FFusionSamplerOperator<NUM_CHANNELS>>&>(InParams.Node);
	...
	*********************************************************************************************************/
	...
	using FFusionSamplerNodeMono   = TNodeFacade<FFusionSamplerOperator<1>>;
	using FFusionSamplerNodeStereo = TNodeFacade<FFusionSamplerOperator<2>>;
	...
	*********************************************************************************************************/
	void FFusionSamplerOperatorBase::Execute()
	{
		...
		MidiClock = MidiStreamInPin->GetClock();
		...
		while (FramesRequired > 0)
		{
			while (MidiEventIterator != MidiEvents.end())
			{
				if ((*MidiEventIterator).BlockSampleFrameIndex <= CurrentBlockFrameIndex)
				{
					...
					if (MidiMessage.IsStd() && (CurrentTrackNumber == 0 || (*MidiEventIterator).TrackIndex == CurrentTrackNumber))
					{
						// If using -1 for the track number to play all tracks, There's currently an issue
						// With voice IDs that playing the same note and channel from the same source but with different tracks
						// Will create the same ID. This means that notes with the same pitch and data will have the same ID,
						// So they won't play as separate fusion voices.
						HandleMidiMessage(
