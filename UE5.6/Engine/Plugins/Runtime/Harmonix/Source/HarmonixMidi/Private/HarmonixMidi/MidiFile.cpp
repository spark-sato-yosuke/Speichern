...
#define LOCTEXT_NAMESPACE "Harmonix_MidiFile"
...
UMidiFile::ELoadResult UMidiFile::LoadStdMidiFile(
	...
	bool EnsureNotFailed)
{
	...
	const ELoadResult Result = LoadStdMidiFile(Archive, Filename, DesiredTicksPerQuarterNote, InTextEncoding, EnsureNotFailed);
	...
	bool EnsureNotFailed)
{
	...
	return LoadStdMidiFile(BufferArchive, Filename, DesiredTicksPerQuarterNote, InTextEncoding, EnsureNotFailed);
}
...
void UMidiFile::CreateFrameBasedMusicMap(UFrameBasedMusicMap* Map) const
{
	return TheMidiData.SongMaps.FillInFrameBasedMusicMap(Map);
}

FMarkerProviderResults UMidiFile::GatherMarkers(const UFrameBasedMusicMap* Map) const
{
	FMarkerProviderResults Results;

	if (TheMidiData.Tracks.Num() > 0)
	{
		const FMidiTrack& ConductorTrack = TheMidiData.Tracks[0];
		const FMidiEventList& Events = ConductorTrack.GetEvents();
		for (const FMidiEvent& Event : Events)
		{
			const FMidiMsg& Msg = Event.GetMsg();
			if (Msg.IsText() && (Msg.GetTextType() == Harmonix::Midi::Constants::GMeta_CuePoint || Msg.GetTextType() == Harmonix::Midi::Constants::GMeta_Marker))
			{
				if (Results.Channels.IsEmpty())
				{
					Results.Channels.Emplace(TEXT("Conductor"));
				}
				float Ms = TheMidiData.SongMaps.TickToMs(static_cast<float>(Event.GetTick()));
				FFrameNumber Location = Map->FrameResolution.AsFrameNumber(static_cast<double>(Ms) / 1000.0);;
				Results.Channels[0].Markers.Emplace(ConductorTrack.GetTextForMsg(Msg), Location);
			}
		}
	}
	return Results;
...
EDataValidationResult UMidiFile::IsDataValid(FDataValidationContext& Context) const
{
	if (TheMidiData.IsEmpty())
	{
		Context.AddError(LOCTEXT("MidiFile_DatIsEmpty", "MidiFile data is empty"));
...
#undef LOCTEXT_NAMESPACE // Harmonix_MidiFile