...
UDynamicMesh* UGeometryScriptLibrary_MeshNormalsFunctions::FlipTriangleSelectionNormals(
	UDynamicMesh* TargetMesh,
	FGeometryScriptMeshSelection Selection, 
	bool bFlipTriangleOrientation,
	bool bFlipNormalDirection, 
	UGeometryScriptDebug* Debug)
{
	if (TargetMesh == nullptr)
	{
		UE::Geometry::AppendError(Debug, EGeometryScriptErrorType::InvalidInputs, LOCTEXT("FlipTriangleSelectionNormals_InvalidInput", "FlipTriangleSelectionNormals: TargetMesh is Null"));
		return TargetMesh;
	}

	// Return early if no flipping was requested
	if (!bFlipTriangleOrientation && !bFlipNormalDirection)
	{
		return TargetMesh;
	}

	TargetMesh->EditMesh([&Selection, bFlipTriangleOrientation, bFlipNormalDirection](FDynamicMesh3& EditMesh)
	{
		TSet<int32> FlippedVIDs, FlippedEIDs;
		FDynamicMeshNormalOverlay* Normals = EditMesh.HasAttributes() ? EditMesh.Attributes()->PrimaryNormals() : nullptr;


		Selection.ProcessByTriangleID(EditMesh, 
		[&EditMesh, &FlippedVIDs, &FlippedEIDs, Normals, bFlipTriangleOrientation, bFlipNormalDirection](int32 TriangleID)
		{
			if (bFlipTriangleOrientation)
			{
				EditMesh.ReverseTriOrientation(TriangleID);
			}

			if (bFlipNormalDirection)
			{
				if (EditMesh.HasVertexNormals())
				{
					...
					{
						int32 VertexID = Tri[SubIdx];
						bool bWasInSet;
						FlippedVIDs.Add(VertexID, &bWasInSet);
						if (!bWasInSet)
						{
							EditMesh.SetVertexNormal(VertexID, -EditMesh.GetVertexNormal(VertexID));
						}
					}
				}
				if (Normals)
				{
					FIndex3i TriEls;
					if (Normals->GetTriangleIfValid(TriangleID, TriEls))
					{
						for (int32 SubIdx = 0; SubIdx < 3; ++SubIdx)
						{
							int32 ElementID = TriEls[SubIdx];
							bool bWasInSet;
							FlippedEIDs.Add(ElementID, &bWasInSet);
							if (!bWasInSet)
							{
								Normals->SetElement(ElementID, -Normals->GetElement(ElementID));
							}
						}
					}
				}
			}
		}, false /*bProcessAllTrisIfSelectionEmpty*/);
	}, EDynamicMeshChangeType::GeneralEdit, EDynamicMeshAttributeChangeFlags::Unknown, false);
