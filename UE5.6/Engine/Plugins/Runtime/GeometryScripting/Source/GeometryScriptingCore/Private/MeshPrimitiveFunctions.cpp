...
namespace UE::Private::AppendMeshHelper
{
		...
		UGeometryScriptDebug* Debug)
	{
		...
		if (!Box.IsValid)
		{
			UE::Geometry::AppendError(Debug, EGeometryScriptErrorType::InvalidInputs, LOCTEXT("PrimitiveFunctions_AppendBoundingBox_InvalidBox", "AppendBoundingBox: Bounding Box is empty, and cannot be appended."));
			return TargetMesh;
		}
