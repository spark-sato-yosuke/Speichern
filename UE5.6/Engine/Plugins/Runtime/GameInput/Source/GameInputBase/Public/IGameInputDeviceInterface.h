...
{
public:
	GAMEINPUTBASE_API explicit IGameInputDeviceInterface(const TSharedRef<FGenericApplicationMessageHandler>& InMessageHandler, struct IGameInput* InGameInput);
	GAMEINPUTBASE_API virtual ~IGameInputDeviceInterface() override;
	...
	*/
	GAMEINPUTBASE_API void Initialize();

protected:

	//~ Begin IInputDevice interface
	GAMEINPUTBASE_API virtual void Tick(float DeltaTime) override;
	GAMEINPUTBASE_API virtual void SendControllerEvents() override;
	GAMEINPUTBASE_API virtual void SetMessageHandler(const TSharedRef<FGenericApplicationMessageHandler>& InMessageHandler) override;
	GAMEINPUTBASE_API virtual bool Exec(UWorld* InWorld, const TCHAR* Cmd, FOutputDevice& Ar) override;
	GAMEINPUTBASE_API virtual void SetChannelValue(int32 ControllerId, FForceFeedbackChannelType ChannelType, float Value) override;
	GAMEINPUTBASE_API virtual void SetChannelValues(int32 ControllerId, const FForceFeedbackValues& values) override;
	GAMEINPUTBASE_API virtual bool IsGamepadAttached() const override;
	//~End IInputDevice interface

#if GAME_INPUT_SUPPORT

	/** Returns a mask of the currently supported Game Input Kind that we want to get the reading of. */
	GAMEINPUTBASE_API virtual GameInputKind GetCurrentGameInputKindSupport() const;

	/** Binds a listener for device status changes from game input to this class */
	GAMEINPUTBASE_API virtual bool BindDeviceStatusCallbacks();
	
	/**
	 * A callback for when an input device connection change has been detected.
	 */
	GAMEINPUTBASE_API virtual void OnDeviceConnectionChanged(GameInputCallbackToken CallbackToken, IGameInputDevice* Device, uint64 Timestamp, GameInputDeviceStatus CurrentStatus, GameInputDeviceStatus PreviousStatus);
	...
	GAMEINPUTBASE_API void ProcessDeferredDeviceConnectionChanges();
	...
	GAMEINPUTBASE_API void OnAppConstrained();
	GAMEINPUTBASE_API void OnAppUnconstrained();
	
	/** Called on the first update after the was un-constrained. */
	GAMEINPUTBASE_API void DetermineStateAfterFirstUnconstrainedUpdate();

	/** Returns a matching FGameInputDeviceContainer if one exists for the given GameInput device. Nullptr if none exist. */
	GAMEINPUTBASE_API FGameInputDeviceContainer* GetDeviceData(IGameInputDevice* InDevice);
	...
	*/
	GAMEINPUTBASE_API FGameInputDeviceContainer* GetOrCreateDeviceData(IGameInputDevice* InDevice);
	...
	*/
	GAMEINPUTBASE_API virtual void EnumerateCurrentlyConnectedDeviceTypes();
