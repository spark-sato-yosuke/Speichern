...
public:
	...
	{
		BindingReference.SetMemberName(NewName);
	}
#endif
...
private:
	...
	/** True if this property path is for a UI component. This affects how some fields in the path are filtered out. */
	UPROPERTY(VisibleAnywhere, Category = "MVVM")
...
public:
	 ...
	 */
	MODELVIEWVIEWMODELBLUEPRINT_API TArray<UE::MVVM::FMVVMConstFieldVariant> GetCompleteFields(const UBlueprint* SelfContext, bool bIsForDisplay = false) const;
	...
	void SetIsComponent(bool InIsComponent)
	{
		bIsComponent = InIsComponent;
	}
	...
	/**
	 * Handle when a field that is part of the path is renamed
	 * Returns true if the renamed field was part of the path
	 */
	bool OnFieldRenamed(const UBlueprint* InContext, UClass* FieldClass, FName OldObjectName, FName NewObjectName);
	...
	bool operator==(const FMVVMBlueprintPropertyPath& Other) const
	{
		return WidgetName == Other.WidgetName
			...
			&& Source == Other.Source
			&& Paths == Other.Paths
			&& bIsComponent == Other.bIsComponent;
	}
...
private:
	...
	TArray<UE::MVVM::FMVVMConstFieldVariant> GetComponentFields(const UClass* InSelfContext) const;
};
