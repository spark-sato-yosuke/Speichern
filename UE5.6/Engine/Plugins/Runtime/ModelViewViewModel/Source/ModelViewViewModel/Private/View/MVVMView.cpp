...
bool UMVVMView::SetSourceInternal(FMVVMViewClass_SourceKey ClassSourceKey, TScriptInterface<INotifyFieldValueChanged> NewValue, bool bForDynamicSource)
{
	...
	if (PreviousValue != NewValue.GetObject())
	{
		...
		{
			// if we had bindings init, then the vm gets set to null, then the vm gets set to a valid ptr, we want to update the binding values as it gets set
			// the only issue is that UninitializeSourceBindings expect the source to be non null
			// if the previous source was null, we don't need to do the work anyway
			if (ViewSource.Source != nullptr)
			{
