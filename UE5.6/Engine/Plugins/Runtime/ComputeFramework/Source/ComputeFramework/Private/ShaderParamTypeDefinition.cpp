...
static FRWLock GloballyKnownValueTypesLock;
...
FShaderValueTypeHandle FShaderValueType::GetOrCreate(FShaderValueType&& InValueType)
{
	...
	{
		FReadScopeLock Lock(GloballyKnownValueTypesLock);
	
		if (FShaderValueTypeHandle* FoundHandle = GloballyKnownValueTypes.Find(Handle))
		{
			return *FoundHandle;
		}
	}

	FWriteScopeLock Lock(GloballyKnownValueTypesLock);
...
FArchive& operator<<(FArchive& InArchive, FShaderValueTypeHandle& InHandle)
{
	...
	else if (InHandle.ValueTypePtr)
	{
		ValueTypePtr = const_cast<FShaderValueType*>(InHandle.ValueTypePtr);
	}
	else
	{
		ValueTypeTemp.Type = EShaderFundamentalType::None;
		ValueTypePtr = &ValueTypeTemp;
	}
