...
#if WITH_EDITOR
		return
			GComputeFrameworkDeferredCompilation != 0 &&
			!(IsRunningCommandlet() && IsAllowCommandletRendering()) &&
			!GIsAutomationTesting; // Making sure shader is ready before running the test, this helps avoid sync compilation during SendAllEndOfFrameUpdates, which is not allowed
#else
		return false;
#endif
	}
