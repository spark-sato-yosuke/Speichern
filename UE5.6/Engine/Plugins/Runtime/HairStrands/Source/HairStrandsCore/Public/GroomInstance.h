...
{
	UE_API virtual ~FHairGroupInstance();
	...
	struct FDebug
	{
		...
		FTransform				RigidRestLocalToWorld = FTransform::Identity;
		FTransform				RigidCurrentLocalToWorld = FTransform::Identity;
		FTransform				SkinningRestLocalToWorld = FTransform::Identity;
	...
	/** Get the rest local to world transform according to the internal binding type */
	FORCEINLINE const FTransform& GetRestLocalToWorld() const
	{
		return (BindingType == EHairBindingType::Skinning) ? Debug.SkinningRestLocalToWorld: 
															 Debug.RigidRestLocalToWorld;
	}
	...
	UE_API FHairStrandsVertexFactoryUniformShaderParameters GetHairStandsUniformShaderParameters(EGroomViewMode ViewMode) const;
};
