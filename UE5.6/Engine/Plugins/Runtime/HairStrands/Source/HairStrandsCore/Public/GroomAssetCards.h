...
struct FHairGroupCardsTextures
{
	...
	UE_API void SetLayout(EHairTextureLayout InLayout);
	UE_API void SetTexture(int32 SlotID, UTexture2D* Texture);
...
class UHairCardGenerationSettings : public UObject
{
	GENERATED_BODY()
public:
	UE_API virtual void BuildDDCKey(FArchive& Ar) PURE_VIRTUAL(BuildDDCKey,);
};
...
struct FHairGroupsCardsSourceDescription
{
	...
	UE_API FHairGroupsCardsSourceDescription();
	...
	UE_API bool operator==(const FHairGroupsCardsSourceDescription& A) const;

	UE_API UStaticMesh* GetMesh() const;
