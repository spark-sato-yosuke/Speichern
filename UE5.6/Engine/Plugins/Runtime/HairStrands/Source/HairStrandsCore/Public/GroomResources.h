...
struct FHairStrandsRestResource : public FHairCommonResource
{
	...
	uint32 GetResourcesSize() const
	{
		...
		Total += GetBufferTotalNumBytes(CurveMappingBuffer);
		Total += GetBufferTotalNumBytes(PointMappingBuffer);
		return Total;
	}
	...
	/* Strand hair per-curve source mapping buffer */
	FRDGExternalBuffer CurveMappingBuffer;

	/* Strand hair per-point source mapping buffer */
	FRDGExternalBuffer PointMappingBuffer;
	...
	inline uint32 GetSourcePointCount() const { return BulkData.GetNumSourcePoints(); }
	inline uint32 GetSourceCurveCount() const { return BulkData.GetNumSourceCurves(); }

	uint32 CachedTangentPointCount = 0;

	/** List of external buffers */
	TMap<FString,FRDGExternalBuffer> ExternalBuffers;
};
...
struct FHairStrandsDeformedResource : public FHairCommonResource
{
	...
	inline const FRDGExternalBuffer& GetPositionOffsetBuffer(EFrameType T) const { return DeformedOffsetBuffer[GetIndex(T)]; }
...
{
	/** Build the hair strands resource */
	FHairStrandsLODRestRootResource(TArray<FHairStrandsRootBulkData>& InBulkDataArray, int32 InBulkDataIndex, EHairStrandsResourcesType CurveType, uint32 InMeshLODIndex, const FHairResourceName& ResourceName, const FName& OwnerName);
	...
	virtual bool InternalGetOrRequestData(uint32 InRequestedCurveCount, uint32 InRequestedPointCount) override;
	virtual void InternalResetLoadedSize() override;
	virtual FHairStrandsBulkCommon* InternalGetBulkData() override { return &GetBulkData(); }
	...
	// Accessors
	uint32 GetRootCount()const { return GetBulkData().GetRootCount(); }

	/* Accessor to deference bulk data*/
	FORCEINLINE bool IsBulkDataValid() const { return BulkDataArray.IsValidIndex(BulkDataIndex); }

	FORCEINLINE const FHairStrandsRootBulkData& GetBulkData() const
	{
		check(BulkDataArray.IsValidIndex(BulkDataIndex));
		return BulkDataArray[BulkDataIndex];
	}

	FORCEINLINE FHairStrandsRootBulkData& GetBulkData()
	{
		check(BulkDataArray.IsValidIndex(BulkDataIndex));
		return BulkDataArray[BulkDataIndex];
	}
	...
	const bool IsDataValid() const { return GetBulkData().Header.RootCount > 0 && GetBulkData().Header.LODIndex >= 0; }
	...
	TArray<FHairStrandsRootBulkData>& BulkDataArray;
	int32 BulkDataIndex = -1;
...
struct FHairStrandsRestRootResource
{
	...
	{ 
		// Find the first valid root count and return that
		for (const FHairStrandsLODRestRootResource* LOD : LODs)
		{
			if (LOD)
			{
				const uint32 RootCount = LOD->GetRootCount();
				if (RootCount > 0)
				{
					return RootCount;
				}
...
struct FHairCardsRestResource : public FHairCommonResource
{
	...
	FHairCardsVertexBuffer VertexColorsBuffer;
