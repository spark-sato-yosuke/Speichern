...
struct FHairGeometryCache
{
	...
	struct FData
	{
		...
		TRefCountPtr<FSkeletalMeshLODRenderData> LODData; // Ref. count to ensure the data are not streamed out while being used
		...
		if (bDebugEnable)
		{
			...
			for (FDebugData& DebugData : DebugDatas)
			{
				if (DebugData.Proxy == InProxy && DebugData.LODIndex == InGeom.LODIndex)
				{
					...
					DebugData.SectionBits = TBitArray<>::BitwiseOR(DebugData.SectionBits, ToBitArray(InGeom.Sections), EBitwiseOperatorFlags::MaxSize);
					return;
				}
			}
	...
	uint32& OutTotalSectionCount)
{
	...
	const TArray<uint32>& UniqueSections = RootResource->GetLOD(LODIndex)->GetBulkData().Header.UniqueSectionIndices;
	...
	FTransform& OutMeshLODLocalToWorld)
{
	...
	if (const FPrimitiveSceneInfo* PrimitiveSceneInfo = GetMeshSceneInfo(Scene, Instance))
	{
		...
		{
			// For now, binding on Nanite mesh are not supported
			if (PrimitiveSceneInfo->Proxy && !PrimitiveSceneInfo->Proxy->IsNaniteMesh())
			{
		...
		{
			OutMeshLODLocalToWorld = FTransform(PrimitiveSceneInfo->Proxy->GetLocalToWorld());
		}
	}
	...
	if (const FPrimitiveSceneInfo* PrimitiveSceneInfo = GetMeshSceneInfo(Scene, Instance))
	{
		if (Instance->Debug.GroomBindingType == EGroomBindingMeshType::SkeletalMesh)
		{
			if (const FSkeletalMeshSceneProxy* SceneProxy = static_cast<const FSkeletalMeshSceneProxy*>(PrimitiveSceneInfo->Proxy))
			{
				...
				SceneProxy->GetCachedGeometry(GraphBuilder, Out);
		...
		// 1. Guides
		{
			...
			{	
				const bool bNeedSurfaceUpdate = 
					...
					Instance->HairGroupPublicData->bIsSimulationCacheEnable);
...
struct FGuideInstanceData
{
	...
	bool NeedsMeshUpdate(bool bCheckMeshLODIndex=true) const
	{
		return 
			...
			(bGlobalDeformationEnable || bSimulationEnable || bDeformationEnable || ActiveGroomCacheType==EGroomCacheType::Guides);
	}
};

/** Gather all the instance Data */
FORCEINLINE void GatherGuideInstanceData(const FHairStrandsInstances& Instances, FHairTransientResources& TransientResources, TArray<FGuideInstanceData>& GuideInstanceDatas, bool& bHasAnySimCacheInstances)
{
	// Gather all instances which require guides or RBF deformations
	bHasAnySimCacheInstances = false;
	GuideInstanceDatas.Reserve(Instances.Num());
	for (FHairStrandsInstance* AbstractInstance : Instances)
	{
		FHairGroupInstance* Instance = static_cast<FHairGroupInstance*>(AbstractInstance);
		if (Instance->GeometryType == EHairGeometryType::NoneGeometry)
		...
		FGuideInstanceData& InstanceData = GuideInstanceDatas.AddDefaulted_GetRef();
	...
	FGlobalShaderMap* ShaderMap)
{
	...
	TArray<FGuideInstanceData> InstanceDatas;
	GatherGuideInstanceData(Instances, TransientResources, InstanceDatas, bHasAnySimCacheInstances);
	...
	for (FGuideInstanceData& InstanceData : InstanceDatas)
	{
	...
	for (FGuideInstanceData& InstanceData : InstanceDatas)
	{
		const bool bRequiresCaching = bHasAnySimCacheInstances && (InstanceData.ActiveGroomCacheType == EGroomCacheType::Guides);
		if (InstanceData.NeedsMeshUpdate() && !bRequiresCaching)
		{
	...
	{
		const bool bRequiresCaching = bHasAnySimCacheInstances && (InstanceData.ActiveGroomCacheType == EGroomCacheType::Guides);
		if (InstanceData.MeshLODIndex >= 0 && (InstanceData.BindingType != EHairBindingType::Skinning) && !bRequiresCaching && (InstanceData.bSimulationEnable || InstanceData.bDeformationEnable))
		{		
			AddHairStrandUpdatePositionOffsetPass(
				GraphBuilder,
				ShaderMap,
...
static void RunHairStrandsDeformation_Guide(
	FRDGBuilder& GraphBuilder,
	FSceneInterface* Scene,
	const FSceneView* View,
	const uint32 ViewUniqueID,
	...
	FGlobalShaderMap* ShaderMap)
{
	...
	// Apply deformation from graph deformer
	for (FGuideInstanceData& InstanceData : InstanceDatas)
	{
		if(InstanceData.Instance->Guides.bIsDeformationEnable && InstanceData.Instance->Guides.DeformedResource && InstanceData.Instance->Guides.DeformedResource->DeformerBuffer.Buffer)
		{
			FRDGBufferRef DstBuffer = Register(GraphBuilder, InstanceData.Instance->Guides.DeformedResource->GetBuffer(FHairStrandsDeformedResource::Current), ERDGImportedBufferFlags::None).Buffer;
			FRDGBufferRef SrcBuffer = Register(GraphBuilder, InstanceData.Instance->Guides.DeformedResource->DeformerBuffer, ERDGImportedBufferFlags::None).Buffer;
			
			AddCopyBufferPass(GraphBuilder, DstBuffer, SrcBuffer);
		}
	}
		...
		for (FGuideInstanceData& InstanceData : InstanceDatas)
		{
			if (InstanceData.ActiveGroomCacheType == EGroomCacheType::Guides)
			{
				...
				AddGroomCacheUpdatePass(
					...
					InterpolationFactor,
					1.f /*MaxHairRadius*/,
					InstanceData.Instance->Guides.RestResource->BulkData.HasFlags(FHairStrandsBulkData::EDataFlags::DataFlags_HasExtraEndingPoint),
					CacheResources0,
					CacheResources1,
					RegisterAsSRV(GraphBuilder, InstanceData.Instance->Guides.RestResource->PositionBuffer),
	...
	FGlobalShaderMap* ShaderMap)
{
	...
	for (FHairStrandsInstance* AbstractInstance : Instances)
	{
		...
		InstanceData.bSupportVoxelization		= Instance->Strands.Modifier.bSupportVoxelization && (bCastShadow || Instance->Debug.Proxy->AffectsIndirectLightingWhileHidden()); // Lumen also uses hair voxelization
		...
		for (FInstanceData& InstanceData : InstanceDatas)
		{
			if (InstanceData.ActiveGroomCacheType == EGroomCacheType::Strands)
			{
				...
				AddGroomCacheUpdatePass(
					...
					InstanceData.Instance->Strands.RestResource->BulkData.HasFlags(FHairStrandsBulkData::EDataFlags::DataFlags_HasExtraEndingPoint),
					CacheResources0,
					CacheResources1,
					RegisterAsSRV(GraphBuilder, InstanceData.Instance->Strands.RestResource->PositionBuffer),
					RegisterAsSRV(GraphBuilder, InstanceData.Instance->Strands.RestResource->CurveBuffer),
	...
	FGlobalShaderMap* ShaderMap)
{
	...
	ERHIPipeline CardPositionExternalAccessPipeline = ERHIPipeline::Graphics;
#if RHI_RAYTRACING
	// Also add async compute as queue which can access these external hair resources
	if (GSupportsEfficientAsyncCompute && GRHIGlobals.RayTracing.SupportsAsyncRayTraceDispatch)
	{
		EnumAddFlags(CardPositionExternalAccessPipeline, ERHIPipeline::AsyncCompute);
	}
#endif
	FRDGExternalAccessQueue ExternalAccessQueue;
	...
	for (uint32 InstanceIndex : CardInstances)
	{
		...
		{		
			...
			{
				ExternalAccessQueue.Add(Register(GraphBuilder, LOD.DeformedResource->GetBuffer(FHairCardsDeformedResource::Current), ERDGImportedBufferFlags::None).Buffer, ERHIAccess::SRVMask, CardPositionExternalAccessPipeline);
	...
	const bool bIsReflectionCapture = View && View->bIsReflectionCapture;

	// Cards only - Build/update RT geometry
	#if RHI_RAYTRACING
	if (IsRayTracingEnabled() && !bIsReflectionCapture)
	{
		for (uint32 InstanceIndex : CardInstances)
		{
			FInstanceData& InstanceData = InstanceDatas[InstanceIndex];
...
static FHairLOD ComputeHairLODIndex(const FHairGroupInstance* Instance, const TArray<const FSceneView*>& Views)
{
	...
	if (Instance->Strands.ClusterResource && Instance->Strands.RestResource && Instance->bSupportStreaming)
	{
	...
	float PrevMeshLODIndex)
{
	...
	const bool bDoesBindingSupportMeshLOD = MeshLODIndex >= Instance->HairGroupPublicData->BindingMinMeshLOD;
	if (!bIsVisible || !bDoesBindingSupportMeshLOD)
	{
		GeometryType = EHairGeometryType::NoneGeometry;
	}
	...
	if (!bIsLODDataReady && (!bIsLODValid || bHasMeshLODChanged || !bDoesBindingSupportMeshLOD))
	{
		return false;
	}
	...
	FGlobalShaderMap* ShaderMap)
{
	...
	{
		// Sync the skeletal mesh update tasks involved with LOD selection so that GetLOD returns a valid result.
		FSkeletalMeshUpdater::WaitForStage(GraphBuilder, ESkeletalMeshUpdateStage::Filter);
	}
	...
	for (FInstanceData& InstanceData : InstanceDatas)
	{
		...
		// Cull instance if it is not visible in any View
		{
			const FPrimitiveSceneProxy* Proxy = InstanceData.Instance->Debug.Proxy;
			check(Proxy != nullptr);
			bool bVisibleInView = false;
			for (const FSceneView* View : Views)
			{
				if (Proxy->IsShown(View) || Proxy->IsShadowCast(View))
				{
					bVisibleInView = true;
					break;
				}
			}
			if (!bVisibleInView && !Proxy->AffectsIndirectLightingWhileHidden())
			{
				InstanceData.Instance->GeometryType = EHairGeometryType::NoneGeometry;
				InstanceData.Instance->BindingType = EHairBindingType::NoneBinding;
			}
...
static void RunHairStrandsProcess(FRDGBuilder& GraphBuilder, FGlobalShaderMap* ShaderMap, const struct FShaderPrintData* ShaderPrintData, const FSceneView* SceneView)
{
#if WITH_EDITOR
	...
	{
		RunHairStrandsPositionQueries(GraphBuilder, ShaderMap, ShaderPrintData, SceneView);
	}
#endif
	...
	FHairStrandsBookmarkParameters& Parameters)
{
	...
	if (IsInstanceFrustumCullingEnable())
	{
		...
		{
			Instances = &Parameters.CullingResults.VisibleInAnyViews_CardsOrMeshes_Primary;
		}
		else if (Bookmark == EHairStrandsBookmark::ProcessCardsAndMeshesInterpolation_ShadowView)
		{
			Instances = &Parameters.CullingResults.VisibleInAnyViews_CardsOrMeshes_Shadow;
		}
		else if (Bookmark == EHairStrandsBookmark::ProcessStrandsInterpolation)
		{
			Instances = &Parameters.CullingResults.VisibleInAnyViews_Strands;
		}
		else
		{
	...
	if (Bookmark == EHairStrandsBookmark::ProcessTasks)
	{
		...
		{
			check(GraphBuilder);
			RunHairStrandsProcess(*GraphBuilder, Parameters.ShaderMap, Parameters.ShaderPrintData, Parameters.View);
		}
	}
	...
	else if (Bookmark == EHairStrandsBookmark::ProcessGuideDeformation)
	{
		RunHairStrandsDeformation_Guide(
			*GraphBuilder,
			Parameters.Scene,
			Parameters.View,
			Parameters.ViewUniqueID,
	...
	else if (Bookmark == EHairStrandsBookmark::ProcessDebug)
	{
		...
		if (IsInstanceFrustumCullingEnable())
		{
			DebugInstances.Append(Parameters.CullingResults.VisibleInAnyViews_Strands);
			DebugInstances.Append(Parameters.CullingResults.VisibleInAnyViews_CardsOrMeshes_Primary);
			DebugInstances.Append(Parameters.CullingResults.VisibleInAnyViews_CardsOrMeshes_Shadow);
