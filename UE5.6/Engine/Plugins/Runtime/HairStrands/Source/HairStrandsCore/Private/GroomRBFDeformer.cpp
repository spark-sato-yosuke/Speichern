...
void UpdateMeshSamples(
	...
	const TArray<FVector3f>& DeformedSamplePositionsBuffer,
...
)
{
	...
	const bool bIsRBFLocalSpaceEnabled = !RestSamplePositionsBuffer.IsEmpty() && !DeformedSamplePositionsBuffer.IsEmpty() && UE::Groom::IsRBFLocalSpaceEnabled();
	const FVector4f RestSampleOffset = bIsRBFLocalSpaceEnabled ? RestSamplePositionsBuffer[0] : FVector4f(0, 0, 0, 0);
	const FVector3f DeformedSampleOffset = bIsRBFLocalSpaceEnabled ? DeformedSamplePositionsBuffer[0] : FVector3f(0, 0, 0);
	...
	for (uint32 SampleIndex = 0; SampleIndex < EntryCount; ++SampleIndex)
	{
		...
		{
			SampleDeformation += InterpolationWeightsBuffer[WeightsOffset] * ((DeformedSamplePositionsBuffer[i]- DeformedSampleOffset) - (RestSamplePositionsBuffer[i]-RestSampleOffset));
		}
...
{
	const bool bIsRBFLocalSpaceEnabled = !RestSamplePositionsBuffer.IsEmpty() && !DeformedSamplePositionsBuffer.IsEmpty() && UE::Groom::IsRBFLocalSpaceEnabled();
	const FVector3f RestSampleOffset = bIsRBFLocalSpaceEnabled ? FVector3f(RestSamplePositionsBuffer[0]) : FVector3f(0, 0, 0);
	const FVector3f DeformedSampleOffset = bIsRBFLocalSpaceEnabled ? DeformedSamplePositionsBuffer[0] : FVector3f(0, 0, 0);

	FVector3f ControlPoint = RestControlPoint-RestSampleOffset;
	...
	ControlPoint += MeshSampleWeightsBuffer[SampleCount + 1] * (RestControlPoint.X - RestSampleOffset.X);
	ControlPoint += MeshSampleWeightsBuffer[SampleCount + 2] * (RestControlPoint.Y - RestSampleOffset.Y);
	ControlPoint += MeshSampleWeightsBuffer[SampleCount + 3] * (RestControlPoint.Z - RestSampleOffset.Z);
	return ControlPoint + DeformedSampleOffset;
}
...
)
{
	...
	ParallelFor(VertexCount, [&](uint32 VertexIndex)
	{
		...
		const FVector3f DisplacedPosition = DisplacePosition(ControlPoint, SampleCount, RestSamplePositionsBuffer, DeformedSamplePositionsBuffer, MeshSampleWeightsBuffer);
	...
	const FHairStrandsRootData& RestLODData)
{
	...
	DeformStrands(
		...
		SampleRestPositionsBuffer,
		SampleDeformedPositionsBuffer,
	...
	const FHairStrandsRootData& RestLODData)
{
	...
	for (uint32 MeshLODIt = 0; MeshLODIt < MeshLODCount; ++MeshLODIt)
	{
		...
		ParallelFor(VertexCount, [&](uint32 VertexIndex)
		{
			...
			Point = DisplacePosition(Point, MaxSampleCount, SampleRestPositionsBuffer, SampleDeformedPositionsBuffer, MeshSampleWeightsBuffer);
		});
...
void FGroomRBFDeformer::GetRBFDeformedGroomAsset(const UGroomAsset* InGroomAsset, const UGroomBindingAsset* BindingAsset, FTextureSource* MaskTextureSource, const float MaskScale, UGroomAsset* OutGroomAsset, const ITargetPlatform* TargetPlatform)
{
	...
	if (InGroomAsset && BindingAsset && BindingAsset->GetTargetSkeletalMesh() && BindingAsset->GetSourceSkeletalMesh())
	{
		...
		const int32 NumGroups = InGroomAsset->GetHairGroupsPlatformData().Num();
		...
		FGroomBindingBuilder::FInput Input;
		{
			...
			Input.bForceUseRunningPlatform = true;
		}
		...
		{
			const uint32 GroupIndex = Group.Info.GroupIndex;
			const FHairStrandsRawDatas& OriginalGuides = Group.Guides;
			...
			// Sanity check
			check(RootDatas[GroupIndex].MeshPositions.IsValidIndex(MeshLODIndex));
			check(RootDatas[GroupIndex].SimRootDatas.IsValidIndex(MeshLODIndex));
			check(RootDatas[GroupIndex].RenRootDatas.IsValidIndex(MeshLODIndex));
		...
		for (FHairGroupsCardsSourceDescription& Desc : OutGroomAsset->GetHairGroupsCards())
		{
			if (UStaticMesh* Mesh = Desc.ImportedMesh)
			{
				if (Desc.GroupIndex >= 0)
				{
					...
					check(RootDatas[Desc.GroupIndex].SimRootDatas.IsValidIndex(MeshLODIndex));
