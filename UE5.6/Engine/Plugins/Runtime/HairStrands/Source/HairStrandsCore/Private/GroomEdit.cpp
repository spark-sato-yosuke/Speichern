...
static void HairStrandsDataToEditableHairGuides(const FHairStrandsDatas& In, TArray<FEditableHairGuide>& Out)
{
	...
	const bool bHasValidRootUVs = In.StrandsCurves.CurvesRootUV.Num() > 0;
	...
	for (uint32 StrandsIt = 0; StrandsIt < StrandsCount; ++StrandsIt)
	{
		...
		Out[StrandsIt].RootUV = bHasValidRootUVs ? In.StrandsCurves.CurvesRootUV[StrandsIt] : FVector2f(0.0f,0.0f);

		Out[StrandsIt].bHasGuideID = bHasValidStrandsIDs;
		Out[StrandsIt].bHasRootUV = bHasValidRootUVs;
...
static void HairStrandsDataToEditableHairStrands(const FHairStrandsDatas& In, TArray<FEditableHairStrand>& Out)
{
	...
	{			
		...
		Out[StrandsIt].bHasStrandID = bHasValidStrandsIDs;
		Out[StrandsIt].bHasClumpID = bHasValidClumpIDs;
		Out[StrandsIt].bHasClosestGuide = bHasValidClosestGuide;
		Out[StrandsIt].bHasRootUV = bHasValidRootUVs;

		const uint32 PointCount  = In.StrandsCurves.CurvesCount[StrandsIt];
		const uint32 PointOffset = In.StrandsCurves.CurvesOffset[StrandsIt];
		const uint32 MaxPoint = PointOffset + PointCount - 1;

		const bool bHasValidColor = (In.StrandsPoints.PointsBaseColor.IsValidIndex(MaxPoint));
		const bool bHasValidRoughness = (In.StrandsPoints.PointsRoughness.IsValidIndex(MaxPoint));
		const bool bHasValidAO = (In.StrandsPoints.PointsAO.IsValidIndex(MaxPoint));
		...
		{				
			...
			Out[StrandsIt].ControlPoints[PointIt].bHasColor = bHasValidColor;
			Out[StrandsIt].ControlPoints[PointIt].bHasRoughness = bHasValidRoughness;
			Out[StrandsIt].ControlPoints[PointIt].bHasAO = bHasValidAO;
			
			if(bHasValidColor)
			{
				Out[StrandsIt].ControlPoints[PointIt].BaseColor	= In.StrandsPoints.PointsBaseColor[EffectiveIndex];
			}
			if(bHasValidRoughness)
			{
...
void ConvertFromGroomAsset(UGroomAsset* InGroom, FEditableGroom* OutGroom, const bool bAllowCurveReordering, const bool bApplyDecimation, const bool bAllowAddEndControlPoint)
{
#if WITH_EDITORONLY_DATA
	check(InGroom);
	check(OutGroom);
	...
	FGroomBuilder::BuildHairDescriptionGroups(In.GetHairDescription(), HairDescriptionGroups, bAllowAddEndControlPoint);
	...
	for (uint32 GroupIndex = 0; GroupIndex < GroupCount; ++GroupIndex)
	{
		...
		FGroomBuilder::BuildData(HairDescriptionGroups.HairGroups[GroupIndex], In.GetHairGroupsInterpolation()[GroupIndex], GroupInfo, StrandsData, GuidesData, bAllowCurveReordering, bApplyDecimation);

		Out.Groups[GroupIndex].GroupIndex = GroupIndex;
...
void ConvertToGroomAsset(UGroomAsset* OutGroom, const FEditableGroom* InGroom, uint32 Operations)
{
	...
	OutGroom->CommitHairDescription(MoveTemp(HairDescription), UGroomAsset::EHairDescriptionType::Source);
