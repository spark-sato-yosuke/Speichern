...
static bool InternalCreateCardsGuides(
	FHairCardsGeometry& InCards,
	FHairStrandsRawDatas& OutGuides,
...
void InternalCreateCardsInterpolation(
	...
	const FHairStrandsRawDatas& InGuides,
	...
	FHairCardsInterpolationBulkData& OutInterpolationBulkData)
{
	...
	const TVertexInstanceAttributesRef<const FVector4f> VertexColors			= MeshDescription->VertexInstanceAttributes().GetAttributesRef<FVector4f>(MeshAttribute::VertexInstance::Color);

	const bool bHasVertexColor = VertexColors.GetNumElements() > 0;
	if (bHasVertexColor)
	{
		Out.Cards.VertexColors.SetNum(VertexCount);
	}
	for (const FVertexID VertexId : MeshDescription->Vertices().GetElementIDs())
	{
		...
		if (bHasVertexColor)
		{
			Out.Cards.VertexColors[VertexIndex] = FLinearColor(VertexColors[VertexInstanceId0]);
		}

		Out.Cards.BoundingBox += Out.Cards.Positions[VertexIndex];
	}
	OutBulk.Header.BoundingBox = ToFBox3d(Out.Cards.BoundingBox);
	OutBulk.Header.bVertexColor = bHasVertexColor;
	...
	OutBulk.VertexColors.Empty();
	if (bHasVertexColor)
	{
		OutBulk.VertexColors.SetNum(PointCount);
	}
	for (uint32 PointIt = 0; PointIt < PointCount; ++PointIt)
	{
		...
		if (bHasVertexColor)
		{
			OutBulk.VertexColors[PointIt] = Out.Cards.VertexColors[PointIt].ToFColor(true/*sRGB*/).Bits;
		}
	}
	...
	FHairStrandsRawDatas OutRawGuides;
	TArray<float> CardLengths;
	CardLengths.Reserve(CardsCount);
	bool bSuccess = InternalCreateCardsGuides(Out.Cards, OutRawGuides, CardLengths);
	if (bSuccess)
	{
		FHairCardsInterpolationDatas InterpolationData;
		InternalCreateCardsInterpolation(Out.Cards, OutRawGuides, InterpolationData);
	...
	// Build the FHairStrandsData from the FHairStrandsRawData
	PRAGMA_DISABLE_DEPRECATION_WARNINGS
	FGroomBuilder::BuildData(OutRawGuides, OutGuides);
	...
	FHairCardsInterpolationBulkData& OutInterpolationBulkData)
{
	...
	for (const FVertexID VertexId : MeshDescription->Vertices().GetElementIDs())
	{
		...
		if (bHasVertexColor)
		{
			OutBulk.VertexColors[VertexIndex]= FLinearColor(VertexColors[VertexInstanceId0]).ToFColor(true/*sRGB*/).Bits;
		}

		BoundingBox += OutBulk.Positions[VertexIndex];
	}
	OutBulk.Header.BoundingBox = ToFBox3d(BoundingBox);
	OutBulk.Header.bVertexColor = bHasVertexColor;
	...
	FHairMeshesBulkData& OutBulk)
{
	...
	OutBulk.Header.BoundingBox = ToFBox3d(Out.Meshes.BoundingBox);
	OutBulk.Header.bVertexColor = false;
	...
	FHairMeshesBulkData& OutBulk)
{
	...
	const bool bHasVertexColor = LODData.VertexBuffers.ColorVertexBuffer.GetNumVertices() > 0;
	...
	Out.Meshes.VertexColors.Empty();
	Out.Meshes.Indices.SetNum(IndexCount);
	if (bHasVertexColor)
	{
		Out.Meshes.VertexColors.SetNum(VertexCount);
		LODData.VertexBuffers.ColorVertexBuffer.GetVertexColors(Out.Meshes.VertexColors);
	}
	...
	for (uint32 PointIt = 0; PointIt < PointCount; ++PointIt)
	{
		...
		if (bHasVertexColor)
		{
			OutBulk.VertexColors[PointIt] = Out.Meshes.VertexColors[PointIt].Bits;
		}
	}
