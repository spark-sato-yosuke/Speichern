...
static int32 GHairStrandsReleaseResourceBeforeBuildingData = 1;
static FAutoConsoleVariableRef CVarHairStrandsReleaseResourceBeforeBuildingData(TEXT("r.HairStrands.ReleaseResourceBeforeBuildingData"), GHairStrandsReleaseResourceBeforeBuildingData, TEXT("Experimental settings for releasing resources prior to building data. This avoids issue when reloading groom."));
	...
	const bool bBuildSourceMapping)
{
	...
	if (bIsValid)
	{
		...
		FGroomBuilder::BuildData(HairGroup, InHairGroupsInterpolation[GroupIndex], OutHairGroupsInfo[GroupIndex], StrandsData, GuidesData, true/*bAllowCurveReordering*/, true/*bApplyDecimation*/, bBuildSourceMapping);
...
void UGroomAsset::Serialize(FArchive& Ar)
{
	...
	if ((bIsCookedOrCooking && Ar.IsSaving()) || (Ar.IsCountingMemory() && Ar.IsFilterEditorOnly()))
	{
		...
		StrippedDataflowAsset = DataflowSettings.GetDataflowAsset();
		DataflowSettings.SetDataflowAsset(nullptr);
	...
	if (Ar.CustomVer(FPhysicsObjectVersion::GUID) >= FPhysicsObjectVersion::GroomWithDescription)
	{
		...
		{
#if WITH_EDITORONLY_DATA
			DataflowSettings.SetDataflowAsset(StrippedDataflowAsset);
#endif
		}
	}
	else
...
static bool IsStrandsInterpolationAttributes(const FName PropertyName)
{
	return
		...
		// Add dependency on simulation, on deformer, and on per LOD-simulation/global-interpolation to strip-out interpolation data if there are not needed
		...
		|| PropertyName == GET_MEMBER_NAME_CHECKED(FHairSolverSettings, bEnableDeformation)
...
static bool IsCardsAttributes(const FName PropertyName)
{
	...
	|| PropertyName == GET_MEMBER_NAME_CHECKED(FHairGroupsCardsSourceDescription, GuideType)
	|| PropertyName == GET_MEMBER_NAME_CHECKED(FHairGroupsCardsSourceDescription, ImportedMesh)
//	|| PropertyName == GET_MEMBER_NAME_CHECKED(FHairGroupsCardsSourceDescription, Textures)
	|| PropertyName == GET_MEMBER_NAME_CHECKED(FHairGroupsCardsSourceDescription, GroupIndex)
	|| PropertyName == GET_MEMBER_NAME_CHECKED(FHairGroupsCardsSourceDescription, LODIndex);
}

static bool IsMeshesAttributes(const FName PropertyName)
{
	return PropertyName == UGroomAsset::GetHairGroupsMeshesMemberName()
	|| PropertyName == GET_MEMBER_NAME_CHECKED(FHairGroupsMeshesSourceDescription, ImportedMesh)
//	|| PropertyName == GET_MEMBER_NAME_CHECKED(FHairGroupsMeshesSourceDescription, Textures)
	|| PropertyName == GET_MEMBER_NAME_CHECKED(FHairGroupsMeshesSourceDescription, GroupIndex)
	|| PropertyName == GET_MEMBER_NAME_CHECKED(FHairGroupsMeshesSourceDescription, LODIndex);
}
...
void UGroomAsset::PostEditChangeProperty(FPropertyChangedEvent& PropertyChangedEvent)
{
	...
	else if(PropertyName == GET_MEMBER_NAME_CHECKED(FHairSolverSettings, NiagaraSolver))
	{
		UpdatePhysicsSystems();
	}
...
static void InternalSerializeStrand(FArchive& Ar, UObject* Owner, FHairGroupPlatformData::FStrands& StrandData, bool bHeader, bool bData, bool* bOutHasDataInCache)
{
	...
	if (!Ar.IsCooking() || !StrandData.bIsCookedOut)
	{
		...
		if (IsRenderCurveEnabled())
		{
			if (bHeader){ StrandData.CurveResourceData.Serialize(Ar, Owner); }
		}
...
void UGroomAsset::SetNumGroup(uint32 InGroupCount, bool bResetGroupData, bool bResetOtherData)
{
	...
	if (InGroupCount != GetHairGroupsLOD().Num())
	{
		...
		for (FHairGroupsLOD& GroupLOD : GetHairGroupsLOD())
		{
			...
			{
				GroupLOD = FHairGroupsLOD::GetDefault();
			}
		}
	}
...
void UGroomAsset::ClearNumGroup(uint32 InGroupCount)
{
	ReleaseResource();

	const FHairGroupsLOD Default = FHairGroupsLOD::GetDefault();
	...
	GetHairGroupsRendering().Init(FHairGroupsRendering(), InGroupCount);
	GetHairGroupsInterpolation().Init(FHairGroupsInterpolation(), InGroupCount);	
	GetHairGroupsLOD().Init(Default, InGroupCount);
	GetHairGroupsInfo().Init(FHairGroupInfoWithVisibility(), InGroupCount);
	GetEffectiveLODBias().Init(0, InGroupCount);
	// The following groups remain unchanged:
	// * HairGroupsCards
	// * GetHairGroupsMeshes()
	// * GetHairGroupsMaterials()
	#if WITH_EDITORONLY_DATA
	StrandsDerivedDataKey.Init(FString(), InGroupCount);
	CardsDerivedDataKey.Init(FString(), InGroupCount);
	MeshesDerivedDataKey.Init(FString(), InGroupCount);
	#endif
}
...
namespace GroomDerivedDataCacheUtils
{
	...
	void SerializeHairSettingsForDDC(FArchive& Ar, uint32 GroupIndex, FHairGroupsInterpolation& InterpolationSettings, FHairGroupsLOD& LODSettings, FHairGroupsPhysics& PhysicsSettings, bool bRequireInterpolationData)
	{
		...
		PhysicsSettings.BuildDDCKey(Ar);
	}
	...
	FString BuildStrandsDerivedDataKeySuffix(uint32 GroupIndex, const FHairGroupsInterpolation& InterpolationSettings, const FHairGroupsLOD& LODSettings, const FHairGroupsPhysics& PhysicsSettings, bool bNeedInterpolationData, const FHairDescriptionBulkData* HairBulkDescription)
	{
		...
		GroomDerivedDataCacheUtils::SerializeHairSettingsForDDC(Ar, GroupIndex, const_cast<FHairGroupsInterpolation&>(InterpolationSettings), const_cast<FHairGroupsLOD&>(LODSettings), const_cast<FHairGroupsPhysics&>(PhysicsSettings), bNeedInterpolationData);
...
FString UGroomAsset::GetDerivedDataKeyForStrands(uint32 GroupIndex)
{
	...
	const FHairGroupsPhysics& PhysicsSettings = GetHairGroupsPhysics()[GroupIndex];
	...
	const FString KeySuffix = GroomDerivedDataCacheUtils::BuildStrandsDerivedDataKeySuffix(GroupIndex, InterpolationSettings, LODSettings, PhysicsSettings, bNeedInterpolationData, HairDescriptionBulkData[HairDescriptionType].Get());
	const FString DerivedDataKey = GroomDerivedDataCacheUtils::BuildGroomDerivedDataKey(KeySuffix);
...
const FHairDescriptionGroups& UGroomAsset::GetHairDescriptionGroups()
{
	...
	
	{
		...
		if (!CachedHairDescriptionGroups[HairDescriptionType])
		{
			...
			FGroomBuilder::BuildHairDescriptionGroups(*CachedHairDescription[HairDescriptionType], *HairDescriptionGroups, true /*bAllowAddEndControlPoint*/);
...
bool UGroomAsset::CacheDerivedDatas()
{
	...
	// (Temporary) Workaround when reloading groom asset several time, which can cause resources to be still alive while the data are being updated 
	// by CacheDerivedData. The BulkData referenced by the resources becomes no longer valid, causing use-after-free crash.
	const bool bReleaseResourceBeforeCacheDerivedData = bIsGameThread && GHairStrandsReleaseResourceBeforeBuildingData > 0;
	if (bReleaseResourceBeforeCacheDerivedData)
	{
		const bool bFlushCommands = bIsInitialized;
		ReleaseResource();
		if (bFlushCommands)
		{
...
bool UGroomAsset::CacheStrandsData(uint32 GroupIndex, FString& OutDerivedDataKey)
{
	...
	if (!bSuccess)
	{
		...
		bSuccess = BuildHairGroup_Strands(
			...
			IsDeformationEnable(GroupIndex));
	...
	FHairCardsInterpolationBulkData& OutInterpolationBulkData)
{
	...
	FHairStrandsDatas TempHairGuidesData;
	...
	for (uint32 LODIt = 0; LODIt < LODCount; ++LODIt)
	{
		if (FHairGroupsCardsSourceDescription* Desc = GetSourceDescription(GetHairGroupsCards(), GroupIndex, LODIt))
		{
			...
			if (bSuccess)
			{
				...
				// 1. Build bulk data for the cards guides
...
{
	return 
		(GetGuideType(GroupIndex) == EGroomGuideType::Rigged) ||
		(GetHairGroupsPhysics().IsValidIndex(GroupIndex) &&	GetHairGroupsPhysics()[GroupIndex].SolverSettings.bEnableDeformation);
}

EGroomGuideType UGroomAsset::GetGuideType(int32 GroupIndex) const
{
	return GetHairGroupsInterpolation().IsValidIndex(GroupIndex) ? GetHairGroupsInterpolation()[GroupIndex].InterpolationSettings.GuideType : EGroomGuideType::Generated;
}
...
GROOMASSET_DEFINE_MEMBER_NAME(DataflowSettings)
...
float UGroomAsset::GetAutoLODBias(int32 InGroupIndex) const
{
	float GroupAutoLODBias = 0;
	if (GetHairGroupsLOD().IsValidIndex(InGroupIndex))
	{
		GroupAutoLODBias = GetHairGroupsLOD()[InGroupIndex].AutoLODBias;
	}
	return FMath::Clamp(AutoLODBias + GroupAutoLODBias, -1.f, 1.f);
}

TObjectPtr<UDataflowBaseContent> UGroomAsset::CreateDataflowContent()
{
	TObjectPtr<UDataflowGroomContent> GroomContent = NewObject<UDataflowGroomContent>(this, UDataflowGroomContent::StaticClass());
	GroomContent->SetIsSaved(false);

	GroomContent->SetDataflowOwner(this);
	GroomContent->SetTerminalAsset(this);

	WriteDataflowContent(GroomContent);
	
	return GroomContent;
}

void UGroomAsset::WriteDataflowContent(const TObjectPtr<UDataflowBaseContent>& DataflowContent) const
{
	if(const TObjectPtr<UDataflowGroomContent> GroomContent = Cast<UDataflowGroomContent>(DataflowContent))
	{
		GroomContent->SetDataflowAsset(DataflowSettings.GetDataflowAsset());
		GroomContent->SetDataflowTerminal(DataflowSettings.GetDataflowTerminal());

#if WITH_EDITORONLY_DATA
		GroomContent->SetAnimationAsset(DataflowSettings.GetPreviewAnimationAsset());
		GroomContent->SetBindingAsset(DataflowSettings.GetPreviewBindingAsset());
#endif
	}
}

void UGroomAsset::ReadDataflowContent(const TObjectPtr<UDataflowBaseContent>& DataflowContent)
{
	if(const TObjectPtr<UDataflowGroomContent> GroomContent = Cast<UDataflowGroomContent>(DataflowContent))
	{
#if WITH_EDITORONLY_DATA
		DataflowSettings.SetPreviewAnimationAsset( GroomContent->GetAnimationAsset());
		DataflowSettings.SetPreviewBindingAsset(GroomContent->GetBindingAsset());
#endif
	}
}

void UGroomAsset::ValidateSimulationCache()
{
	Modify();
	SetEnableSimulationCache(true);
