...
#define GROOMBINDING_SKELMESH_VALIDATION 0
...
// Return a ref counted hair group instance. Will be removed/modified when we will switch to use PrimitiveId instead of components
...
{
	const FHairStrandsSceneProxy* GroomProxy = GroomComponent ? static_cast<FHairStrandsSceneProxy*>(GroomComponent->GetSceneProxy()) : nullptr;
	if (GroomProxy && GroomProxy->HairGroupInstances.IsValidIndex(GroupIndex))
	{
		return GroomProxy->HairGroupInstances[GroupIndex];
	}
	return nullptr;
}

// @todo: deprecate this function in 5.6 to use GetRefCountedHairGroupInstance
FHairGroupInstance* GetHairGroupInstance(UGroomComponent* GroomComponent, int32 GroupIndex)
{
	return GetRefCountedHairGroupInstance(GroomComponent, GroupIndex).GetReference();
}
...
bool UGroomComponent::IsDeformationEnable(int32 GroupIndex) const
{
	// To enable deformation groom asset should allow it and we must have a mesh deformer locally or being register to a solver component
	return GroomAsset->IsDeformationEnable(GroupIndex) && (GroomSolver || MeshDeformer || GroomAsset->GetGuideType(GroupIndex) == EGroomGuideType::Rigged);
}
	...
	if (GroupIndex < NiagaraComponents.Num())
	{
		...
		if (GroomAsset && (GroupIndex < GroomAsset->GetHairGroupsPhysics().Num()) && (LODIndex < GroomAsset->GetHairGroupsLOD()[GroupIndex].LODs.Num()) && IsSimulationEnable(GroupIndex, LODIndex))
		{
			...
			if (GetWorld() && GetWorld()->bIsWorldInitialized)
			{
				...
				NiagaraComponent->SetVisibility(SimulationSettings.SimulationSetup.bDebugSimulation || GroomAsset->GetHairGroupsPhysics()[GroupIndex].SolverSettings.bForceVisible, false);
...
{
	SetGroomAsset(GroomAsset, InBinding);
}
...
void UGroomComponent::CheckHairStrandsUsage(int32 ElementIndex) const
{
	...
	const bool bValidUsage = OverrideMaterial && OverrideMaterial->CheckMaterialUsage_Concurrent(MATUSAGE_HairStrands);
	if(!OverrideMaterial || !bValidUsage)
	{		
		UE_CLOG(OverrideMaterial && !bValidUsage, LogHairStrands, Warning, TEXT("Material with missing usage flag was applied to hair strands %s"), *OverrideMaterial->GetPathName());
	}
}
...
void UGroomComponent::InitResources(bool bIsBindingReloading)
{
	...
	for (int32 GroupIt = 0, GroupCount = GroomAsset->GetHairGroupsPlatformData().Num(); GroupIt < GroupCount; ++GroupIt)
	{
		...
		bHasNeedDeformation[GroupIt] = IsDeformationEnable(GroupIt);
	...
	for (int32 GroupIt = 0, GroupCount = GroomAsset->GetHairGroupsPlatformData().Num(); GroupIt < GroupCount; ++GroupIt)
	{
		...
		HairGroupInstance->Debug.SkinningRestLocalToWorld		= HairGroupInstance->Debug.SkinningCurrentLocalToWorld;
		...
		HairGroupInstance->Debug.RigidRestLocalToWorld			= HairGroupInstance->Debug.RigidCurrentLocalToWorld;
		...
		bool bNeedStrandsData = false;
		{
			...
			HairGroupInstance->HairGroupPublicData->bIsDeformationEnable = IsDeformationEnable(GroupIt);
			...
			HairGroupInstance->HairGroupPublicData->AutoLODBias = GroomAsset->GetAutoLODBias(GroupIt);

			HairGroupInstance->HairGroupPublicData->BindingMinMeshLOD = INDEX_NONE;
			if (LocalBindingAsset)
			{
				const TArray<UGroomBindingAsset::FHairGroupPlatformData>& BindingPlatformData = LocalBindingAsset->GetHairGroupsPlatformData();
				if (BindingPlatformData.IsValidIndex(GroupIt))
				{
					HairGroupInstance->HairGroupPublicData->BindingMinMeshLOD = BindingPlatformData[GroupIt].TargetMeshMinLOD;
				}
			}
		}
		...
		if (bNeedGuides)
		{
			if (LocalBindingAsset)
			{
				...
				#if GROOMBINDING_SKELMESH_VALIDATION
				if (USkeletalMeshComponent* SkeletalMeshComponent = Cast<USkeletalMeshComponent>(RegisteredMeshComponent))
				{
			...
			HairGroupInstance->Guides.bIsDeformationEnable = IsDeformationEnable(GroupIt);
		...
		if (bNeedStrandsData && GroupData.Strands.HasValidData())
		{
			...
			#if RHI_RAYTRACING
			HairGroupInstance->Strands.ViewRayTracingMask = EHairViewRayTracingMask::None;
			if (IsHairRayTracingEnabled() && bVisibleInRayTracing)
			{
				// If the project has path tracing, we may need raytracing resources
				static const auto CVarLocalVariablePathTracing = IConsoleManager::Get().FindTConsoleVariableDataInt(TEXT("r.PathTracing"));
				const int PathTracingEnabled = CVarLocalVariablePathTracing ? CVarLocalVariablePathTracing->GetValueOnAnyThread() : 0;
				if (PathTracingEnabled)
				{
					// Always visible for path tracing
					HairGroupInstance->Strands.ViewRayTracingMask |= EHairViewRayTracingMask::PathTracing;
				}
				// Visible to ray tracing if the user has opted in
				if (HairGroupInstance->Strands.Modifier.bUseHairRaytracingGeometry)
				{
					HairGroupInstance->Strands.ViewRayTracingMask |= EHairViewRayTracingMask::RayTracing;
				}
				if (HairGroupInstance->Strands.ViewRayTracingMask != EHairViewRayTracingMask::None)
				{
...
void UGroomComponent::PostEditChangeProperty(FPropertyChangedEvent& PropertyChangedEvent)
{
	...
	if (GroomAsset && !bAssetChanged)
	{
		...
		{
			const FHairGroupDesc GroupDesc = GetGroomGroupsDesc(GroomAsset, this, Instance->Debug.GroupIndex);
			const bool bWasUsingRayTracingGeometry = EnumHasAnyFlags(Instance->Strands.ViewRayTracingMask, EHairViewRayTracingMask::RayTracing);
			const bool bRecreate = bWasUsingRayTracingGeometry != GroupDesc.bUseHairRaytracingGeometry;
			if (bRecreate)
			{
	...
	if (GroomCacheType == EGroomCacheType::Guides && GroomAsset)
	{
		for (int32 Index = 0; Index < GroomAsset->GetNumHairGroups(); ++Index)
		{
			...
			{
				GroomAsset->ValidateSimulationCache();
				break;
			}
		}
	}
...
bool UGroomComponent::CanEditChange(const FProperty* InProperty) const
{
	if (InProperty)
	{
		...
		if (PropertyName == GET_MEMBER_NAME_STRING_CHECKED(UGroomComponent, MeshDeformer))
		{
			if(GroomAsset)
			{
				for(int32 GroupIndex = 0, NumGroups = GroomAsset->GetNumHairGroups(); GroupIndex < NumGroups; ++GroupIndex)
				{
