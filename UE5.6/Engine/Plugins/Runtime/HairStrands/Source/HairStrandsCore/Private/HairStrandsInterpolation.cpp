...
static int32 GVarHairStrands_ScaleHairRadiusByTransform = 1;
static FAutoConsoleVariableRef CVarHairStrands_ScaleHairRadiusByTransform(TEXT("r.HairStrands.Strands.ScaleHairRadiusByTransform"), GVarHairStrands_ScaleHairRadiusByTransform, TEXT("Scale hair strands radius by the instance's transform to make hair radius consistent with the instance scale."));

static int32 GVarHairStrands_AddDeformerStrandsDisplacement = 0;
static FAutoConsoleVariableRef CVarHairStrands_AddDeformerStrandsDisplacement(TEXT("r.HairStrands.Strands.AddDeformerStrandsDisplacement"), GVarHairStrands_AddDeformerStrandsDisplacement, TEXT("Add the deformer displacement to the current position. If set to 0 the deformer position will overwrite the deformed one"));
	...
	FRDGBufferUAVRef OutBuffer)
{
	...
	// Retrive the cache point count from the SRV.
	const uint32 CachePointCount = CacheResources0.PositionBuffer->GetParent() ? CacheResources0.PositionBuffer->GetParent()->Desc.NumElements / 3u : PointCount;
	...
	Parameters->CachePointCount = CachePointCount;
	...
	Parameters->bHasAddedControlPoint = bHasAddedControlPoint ? 1u : 0u;
...
class FDeformGuideCS : public FGlobalShader
{
	...
	BEGIN_SHADER_PARAMETER_STRUCT(FParameters, )
		...
		SHADER_PARAMETER(uint32, RBFLocalSpace)
		...
		else
		{
			...
			if (bSupportDynamicMesh)
			{
				...
				else
				{
					...
					Parameters->RBFLocalSpace = UE::Groom::IsRBFLocalSpaceEnabled();
;
				}
			}
		}
...
class FHairInterpolationCS : public FGlobalShader
{
	...
	class FDeformer : SHADER_PERMUTATION_INT("PERMUTATION_DEFORMER", 3);
	...
	const FHairStrandsLODDeformedRootResource::EFrameType DeformedFrame)
{
	...
	// Guides
	// Guide interpolation with dynamic mesh geometry is now support with guide cache. It implicitly assumes that the guide-cache follows correctly the binding surface
	bool bSupportGlobalInterpolation = false;
	if (bSupportDynamicMesh && (Instance->Guides.bIsSimulationEnable || Instance->Guides.bHasGlobalInterpolation || Instance->Guides.bIsDeformationEnable || Instance->Guides.bIsSimulationCacheEnable))
	{
	...
	// Deformer type : 0 no deformer, 1 add deformer displacement, 2 overwrite deformed position with the deformer one
	const int32 DeformerType = !bSupportDeformer ? 0 : GVarHairStrands_AddDeformerStrandsDisplacement ? 1 : 2;
	...
	PermutationVector.Set<FHairInterpolationCS::FDeformer>(DeformerType);
...
void AddHairCardsDeformationPass(
	...
	const int32 MeshLODIndex,
	ERHIPipeline CardPositionExternalAccessPipeline)
{
	...
	GraphBuilder.UseExternalAccessMode(CardsDeformedPositionBuffer_Curr.Buffer, ERHIAccess::SRVMask, CardPositionExternalAccessPipeline);
...
FHairGroupPublicData::FVertexFactoryInput InternalComputeHairStrandsVertexInputData(FRDGBuilder* GraphBuilder, const FHairGroupInstance* Instance, EGroomViewMode ViewMode)
{
	...
	// Scale hair radius based on the max. scaling factor from the instance's transform
	float InstanceScale = 1.f;
	if (GVarHairStrands_ScaleHairRadiusByTransform > 0)
	{
		const FVector3d S = Instance->LocalToWorld.GetScale3D();
		InstanceScale = FMath::Max3(S.X, S.Y, S.Z);
	}
	...
	OutVFInput.Strands.Common.Radius *= InstanceScale;
	...
	OutVFInput.Strands.Common.Flags |= (Instance->Debug.Proxy && Instance->Debug.Proxy->Holdout()) ? HAIR_FLAGS_HOLDOUT : 0u;
	OutVFInput.Strands.Common.Flags |= (Instance->Debug.Proxy && Instance->Debug.Proxy->CastsContactShadow()) ? HAIR_FLAGS_CAST_CONTACT_SHADOW : 0u;
	...
	OutVFInput.Strands.Common.bSingleGuide = (Instance->Strands.InterpolationResource && (Instance->Strands.InterpolationResource->BulkData.Header.Flags & FHairStrandsInterpolationBulkData::DataFlags_HasSingleGuideData) != 0) ? 1 : 0;
	OutVFInput.Strands.Common.bCurveRemapping = (Instance->Strands.RestResource && (Instance->Strands.RestResource->BulkData.Header.Flags & FHairStrandsBulkData::DataFlags_HasCurveMapping) != 0) ? 1 : 0;
	OutVFInput.Strands.Common.bPointRemapping = (Instance->Strands.RestResource && (Instance->Strands.RestResource->BulkData.Header.Flags & FHairStrandsBulkData::DataFlags_HasPointMapping) != 0) ? 1 : 0;
	OutVFInput.Strands.Common.SourcePointCount = bUseGuideAttributeOffsets ? Instance->Guides.RestResource->GetSourcePointCount() : Instance->Strands.RestResource->GetSourcePointCount();
	OutVFInput.Strands.Common.SourceCurveCount = bUseGuideAttributeOffsets ? Instance->Guides.RestResource->GetSourceCurveCount() : Instance->Strands.RestResource->GetSourceCurveCount();
