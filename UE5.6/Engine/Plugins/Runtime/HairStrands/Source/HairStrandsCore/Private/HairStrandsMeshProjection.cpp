...
{	
	...
	check(RestLODData.GetBulkData().Header.LODIndex == MeshLODIndex);
	const uint32 UniqueTriangleCount = RestLODData.GetBulkData().Header.UniqueTriangleCount;
	const uint32 TotalMeshSectionCount = RestLODData.GetBulkData().Header.MeshSectionCount;
	...
	FHairStrandsDeformedRootResource* DeformedRootResources)
{
	...
	Parameters->DeformedSamplePositionsBuffer	= RegisterAsSRV(GraphBuilder, DeformedLODData.GetDeformedSamplePositionsBuffer(FHairStrandsLODDeformedRootResource::Current));
	...
	FHairStrandsDeformedRootResource* DeformedResources)
{
	...
	const uint32 TotalSectionCount = RestLODData.GetBulkData().Header.MeshSectionCount;
	...
	*PassParameters = CommonParameters;
	...
	FComputeShaderUtils::AddPass(
		GraphBuilder,
		RDG_EVENT_NAME("HairStrands::InitMeshSamples"),
		ComputeShader,
		PassParameters,
		DispatchGroupCount);
}
	...
	FRDGTextureRef OutTexture)
{
	...
	Parameters->MaxUniqueTriangleIndex = LODData.GetBulkData().Header.UniqueTriangleCount;
