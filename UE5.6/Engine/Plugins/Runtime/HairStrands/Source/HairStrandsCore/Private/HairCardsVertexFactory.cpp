	...
	bool bSupportsManualVertexFetch)
{
	...
	if (GeometryType == EHairGeometryType::Cards)
	{
		...
		const bool bHasVertexColor = LOD.RestResource->BulkData.Header.bVertexColor;

		// Cards atlas UV are inverted so fetching needs to be inverted on the y-axis
		UniformParameters.Flags = 
			(LOD.RestResource->bInvertUV ? uint32(EHairCardsFactoryFlags::InvertedUV) : 0u) |
			(bHasVertexColor ? uint32(EHairCardsFactoryFlags::VertexColor) : 0u);
		...
		if (bHasVertexColor)
		{
			UniformParameters.VertexColorsBuffer = LOD.RestResource->VertexColorsBuffer.ShaderResourceViewRHI.GetReference();
		}
		else
		{
			UniformParameters.VertexColorsBuffer = UniformParameters.NormalsBuffer; // Reuse normal buffer as a dummy input for material buffer, since it not used for meshes (material data is fetch through textures)
		}
	...
	else if (GeometryType == EHairGeometryType::Meshes)
	{
		...
		UniformParameters.Flags = (bHasVertexColor ? uint32(EHairCardsFactoryFlags::VertexColor) : 0u);
		UniformParameters.AttributeTextureIndex = 0; // TODO pack attribute texture/channel description. Currently use hardcoded layout in shader attribute code.
		UniformParameters.AttributeChannelIndex = 0; // TODO pack attribute texture/channel description. Currently use hardcoded layout in shader attribute code.
		UniformParameters.MaxVertexCount = LOD.RestResource->GetVertexCount();
	...
	if (!bSupportsManualVertexFetch)
	{
		...
		UniformParameters.VertexColorsBuffer     = GNullVertexBuffer.VertexBufferSRV;
	}
...
void FHairCardsVertexFactory::GetPSOPrecacheVertexFetchElements(EVertexInputStreamType VertexInputStreamType, FVertexDeclarationElementList& Elements)
{
	...
	if (!bManualFetch)
	{
		...
		Elements.Add(FVertexElement(3+NumStreams, 0, FHairCardsMaterialFormat::VertexElementType, 4, FHairCardsMaterialFormat::SizeInByte, false)); // Material
		Elements.Add(FVertexElement(3+NumStreams, 0, FHairCardsMaterialFormat::VertexElementType, 5, FHairCardsMaterialFormat::SizeInByte, false)); // VertexColor
	}
}
...
void FHairCardsVertexFactory::InitResources(FRHICommandListBase& RHICmdList)
{
	...
	if (!bManualFetch)
	{
		if (Data.GeometryType == EHairGeometryType::Cards)
		{
			...
			if (bHasVertexColor)
			{
				Elements.Add(AccessStreamComponent(FVertexStreamComponent(&LOD.RestResource->VertexColorsBuffer,	0, 0,									FHairCardsMaterialFormat::SizeInByte,												FHairCardsMaterialFormat::VertexElementType,	EVertexStreamUsage::Default), 6));
			}
			else
			{
				Elements.Add(AccessStreamComponent(FVertexStreamComponent(&LOD.RestResource->NormalsBuffer,			0, 0,									FHairCardsMaterialFormat::SizeInByte,												FHairCardsMaterialFormat::VertexElementType,	EVertexStreamUsage::Default), 6));
			}
			...
			[RestResource, bHasVertexColor](FRHICommandListImmediate& RHICmdImmediateList)
			{
				...
				if (bHasVertexColor)
				{
					RHICmdImmediateList.Transition(FRHITransitionInfo(RestResource->VertexColorsBuffer.VertexBufferRHI, ERHIAccess::Unknown, ERHIAccess::VertexOrIndexBuffer));
				}
			});
		}
		else if (Data.GeometryType == EHairGeometryType::Meshes)
		{
			...
			Elements.Add(AccessStreamComponent(FVertexStreamComponent(&LOD.RestResource->NormalsBuffer,				0, FHairCardsNormalFormat::SizeInByte,	FHairCardsNormalFormat::SizeInByte * FHairCardsNormalFormat::ComponentCount,		FHairCardsNormalFormat::VertexElementType,		EVertexStreamUsage::Default), 2));
			Elements.Add(AccessStreamComponent(FVertexStreamComponent(&LOD.RestResource->UVsBuffer,					0, 0,									FHairCardsUVFormat::SizeInByte,														FHairCardsUVFormat::VertexElementType,			EVertexStreamUsage::Default), 3));
			Elements.Add(AccessStreamComponent(FVertexStreamComponent(&LOD.RestResource->NormalsBuffer,				0, 0,									FHairCardsMaterialFormat::SizeInByte,												FHairCardsMaterialFormat::VertexElementType,	EVertexStreamUsage::Default), 4)); // Dummy
			if (bHasVertexColor)
			{
				Elements.Add(AccessStreamComponent(FVertexStreamComponent(&LOD.RestResource->VertexColorsBuffer,	0, 0,									FHairCardsMaterialFormat::SizeInByte,												FHairCardsMaterialFormat::VertexElementType,	EVertexStreamUsage::Default), 6));
			}
