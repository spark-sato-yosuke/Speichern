...
namespace Metasound
{
	namespace Editor
	{
		namespace DrawingPolicyPrivate
		{
			...
			{
				const FVector2f& TL = InDrawConnectionData.Bounds.Min;
				const FVector2f& BR = InDrawConnectionData.Bounds.Max;
				const FVector2f TR = FVector2f(InDrawConnectionData.Bounds.Max.X, InDrawConnectionData.Bounds.Min.Y);
				const FVector2f BL = FVector2f(InDrawConnectionData.Bounds.Min.X, InDrawConnectionData.Bounds.Max.Y);

				auto DrawSpaceLine = [&](const FVector2f& Point1, const FVector2f& Point2, const FLinearColor& InColor)
				{
					const FVector2f FakeTangent = (Point2 - Point1).GetSafeNormal();
		...
		FDrawConnectionData::FDrawConnectionData(const UE::Slate::FDeprecateVector2DParameter& InStart, const UE::Slate::FDeprecateVector2DParameter& InEnd, const FEditor& Editor, const UE::Slate::FDeprecateVector2DParameter& InSplineTangent, const FConnectionParams& InParams, const UGraphEditorSettings& InSettings, const UE::Slate::FDeprecateVector2DParameter& InMousePosition)
			: P0(InStart)
			, P0Tangent(InParams.StartDirection == EGPD_Output ? UE::Slate::CastToVector2f(InSplineTangent) : UE::Slate::CastToVector2f(-InSplineTangent))
			, P1(InEnd)
			, P1Tangent(InParams.EndDirection == EGPD_Input ? UE::Slate::CastToVector2f(InSplineTangent) : UE::Slate::CastToVector2f(-InSplineTangent))
			, Params(InParams)
		{
			...
			Bounds = FBox2f(ForceInit);

			Bounds += P0;
			Bounds += P0 + MaximumTangentContribution * P0Tangent;
			Bounds += P1;
			Bounds += P1 - MaximumTangentContribution * P1Tangent;
			...
			ClosestPoint = FVector2f(ForceInit);
			if (InSettings.bTreatSplinesLikePins)
			{
				if (bCloseToSpline)
				{
					...
					for (float TestAlpha = 0.0f; TestAlpha < 1.0f; TestAlpha += StepInterval)
					{
						...
						const FVector2f ClosestPointToSegment = FMath::ClosestPointOnSegment2D(InMousePosition, Point1, Point2);
		...
		void FGraphConnectionDrawingPolicy::DrawConnection(int32 LayerId, const FVector2f& Start, const FVector2f& End, const FConnectionParams& Params)
		{
		...
		void FGraphConnectionDrawingPolicy::DrawConnection_MidpointImage(int32 InLayerId, const FInterpCurve<float>& InSplineReparamTable, float InSplineLength, const FDrawConnectionData& InData)
		{
			...
			const FVector2f Midpoint = FMath::CubicInterp(InData.P0, InData.P0Tangent, InData.P1, InData.P1Tangent, MidpointAlpha);

			// Approximate the slope at the midpoint (to orient the midpoint image to the spline)
			const FVector2f MidpointPlusE = FMath::CubicInterp(InData.P0, InData.P0Tangent, InData.P1, InData.P1Tangent, MidpointAlpha + KINDA_SMALL_NUMBER);
			const FVector2f MidpointMinusE = FMath::CubicInterp(InData.P0, InData.P0Tangent, InData.P1, InData.P1Tangent, MidpointAlpha - KINDA_SMALL_NUMBER);
		...
		void FGraphConnectionDrawingPolicy::DrawConnectionSignal(const FDrawConnectionSignalData& InParams)
		{
			...
			const FVector2f BubbleSize = BubbleImage->ImageSize * ZoomFactor * 0.2f * InParams.ConnectionData.Params.WireThickness;
			...
			for (int32 i = 0; i < NumBubbles; ++i)
			{
				...
				if (Distance < InParams.SplineLength)
				{
					...
					FVector2f BubblePos = FMath::CubicInterp(InParams.ConnectionData.P0, InParams.ConnectionData.P0Tangent, InParams.ConnectionData.P1, InParams.ConnectionData.P1Tangent, Alpha);
		...
		void FGraphConnectionDrawingPolicy::DrawConnectionSignal_Envelope(const FDrawConnectionSignalData& InParams, FName InAnalyzerName)
		{
			...
			FVector2f LastPointPos = FVector2f::Zero();
			FVector2f LastTangent = FVector2f::Zero();

			auto AddPoint = [&](const float EnvMagnitude, const float Distance)
			{
				const float Alpha = InParams.SplineReparamTable.Eval(Distance, 0.f);
				FVector2f PointPos = FMath::CubicInterp(InParams.ConnectionData.P0, InParams.ConnectionData.P0Tangent, InParams.ConnectionData.P1, InParams.ConnectionData.P1Tangent, Alpha);

				LastTangent = FVector2f(PointPos.X - LastPointPos.X, PointPos.Y - LastPointPos.Y);
				const FVector2f Normal = FVector2f(LastTangent.Y, -1.0f * LastTangent.X).GetSafeNormal();
