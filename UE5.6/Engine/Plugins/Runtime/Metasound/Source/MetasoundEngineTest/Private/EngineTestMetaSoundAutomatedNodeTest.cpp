...
namespace Metasound::EngineTest{
	...
	FString GetPrettyName(const Frontend::FNodeClassRegistryKey& InRegistryKey)
	{
	...
	TUniquePtr<INode> CreateNodeFromRegistry(const Frontend::FNodeRegistryKey& InNodeRegistryKey)
	{
		...
		switch (NodeClass.Metadata.GetType())
		{
			...
			case EMetasoundFrontendClassType::Graph:
			{
				...
				NodeRegistry->FindDefaultVertexInterface(InNodeRegistryKey, VertexInterface);
				Node = NodeRegistry->CreateNode(InNodeRegistryKey, FNodeData{ TestNodeName, TestNodeID , VertexInterface });
			}
			break;
			...
			case EMetasoundFrontendClassType::Input:
			{
				...
				TSharedPtr<const FNodeClassMetadata> InputClassMetadata = DataTypeRegistry.GetInputClassMetadata(DataTypeName);
				checkf(InputClassMetadata, TEXT("Input class metadata could not be found from data type registry"));

				FVertexInterface InputNodeInterface = InputClassMetadata->DefaultInterface;
				// Vertex names must be set for input nodes
				InputNodeInterface.GetInputInterface().At(0).VertexName = TestVertexName;
				InputNodeInterface.GetOutputInterface().At(0).VertexName = TestVertexName;
				InputNodeInterface.GetInputInterface().At(0).SetDefaultLiteral(NodeClass.GetDefaultInterface().Inputs[0].FindConstDefaultChecked(Frontend::DefaultPageID).ToLiteral(DataTypeName));

				Node = DataTypeRegistry.CreateInputNode(DataTypeName, FNodeData{ TestNodeName, TestNodeID, InputNodeInterface });
			}
			break;
			...
			{
				FName DataTypeName = NodeClass.Metadata.GetClassName().Name;
				TSharedPtr<const FNodeClassMetadata> VariableMetadata = DataTypeRegistry.GetVariableClassMetadata(DataTypeName);
				checkf(VariableMetadata, TEXT("Variable class metadata could not be found from data type registry"));
				Node = DataTypeRegistry.CreateVariableNode(DataTypeName, DataTypeRegistry.CreateDefaultLiteral(DataTypeName),
					FNodeData{ TestNodeName, TestNodeID, VariableMetadata->DefaultInterface });
			}
			break;
...
PRAGMA_ENABLE_DEPRECATION_WARNINGS
			...
			{
				FName DataTypeName = NodeClass.Metadata.GetClassName().Name;
				TSharedPtr<const FNodeClassMetadata> OutputClassMetadata = DataTypeRegistry.GetOutputClassMetadata(DataTypeName);
				checkf(OutputClassMetadata, TEXT("Output class metadata could not be found from data type registry"));

				FVertexInterface OutputNodeInterface = OutputClassMetadata->DefaultInterface;
				// Vertex names must be set for output nodes
				OutputNodeInterface.GetInputInterface().At(0).VertexName = TestVertexName;
				OutputNodeInterface.GetOutputInterface().At(0).VertexName = TestVertexName;
				Node = DataTypeRegistry.CreateOutputNode(DataTypeName, FNodeData{ TestNodeName, TestNodeID, OutputNodeInterface });
			}
			break;
	...
	void GetAllRegisteredNodes(TArray<FString>& OutBeautifiedNames, TArray <FString>& OutTestCommands, bool bInIncludeDeprecatedNodes)
	{
		...
		for (const FMetasoundFrontendClass& NodeClass : AllClasses)
		{
			...
			else if (!bInIncludeDeprecatedNodes && NodeClass.Metadata.GetIsDeprecated())
			{
				// Optionally skip deprecated classes. 
				continue;
			}
	...
	void GetAllRegisteredNativeNodes(TArray<FString>& OutBeautifiedNames, TArray <FString>& OutTestCommands, bool bInIncludeDeprecatedNodes)
	{
		using namespace Metasound;
		...
		for (const FMetasoundFrontendClass& NodeClass : AllClasses)
		{
			...
			// Exclude asset-defined node classes
			if (AssetManager && AssetManager->ContainsKey(NodeRegistryKey))
			{
				continue;
			}
		...
		for (const FInputBinding& Binding : OutVertexData)
		{
			...
			if (const Frontend::IDataTypeRegistryEntry* VariableEntry = DataTypeRegistry.FindDataTypeRegistryEntry(InputVertex.DataTypeName))
			{
				if (VariableEntry->GetDataTypeInfo().bIsVariable)
				{
					...
					{
						TSharedPtr<const FNodeClassMetadata> VariableNodeMetadata = DataTypeEntry->GetVariableClassMetadata();
						if (!VariableNodeMetadata)
						{
							continue;
						}

						// Create a variable node for the underlying data type
						TUniquePtr<INode> VariableNode = DataTypeEntry->CreateVariableNode(
							DataTypeRegistry.CreateDefaultLiteral(UnderlyingDataTypeName),
							FNodeData{"VariableNodeName", FGuid::NewGuid(), VariableNodeMetadata->DefaultInterface});
