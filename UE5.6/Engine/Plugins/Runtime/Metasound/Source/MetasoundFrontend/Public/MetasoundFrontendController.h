	...
	namespace Frontend
	{
		...
		class IDocumentAccessor
		{
			protected:
				...
				static UE_API FDocumentAccess GetSharedAccess(IDocumentAccessor& InDocumentAccessor);

				/** Returns the shared access from an IDocumentAccessor. */
				static UE_API FConstDocumentAccess GetSharedAccess(const IDocumentAccessor& InDocumentAccessor);
		};
		
		/* An IOutputController provides methods for querying and manipulating a metasound output vertex. */
		class IOutputController : public TSharedFromThis<IOutputController>, public IDocumentAccessor
		{
		public:
			static UE_API FOutputHandle GetInvalidHandle();
		...
		class IVariableController : public TSharedFromThis<IVariableController>, public IDocumentAccessor
		{
		public:
			static UE_API FVariableHandle GetInvalidHandle();
		...
		class IInputController : public TSharedFromThis<IInputController>, public IDocumentAccessor
		{
		public:
			static UE_API FInputHandle GetInvalidHandle();
		...
		class INodeController : public TSharedFromThis<INodeController>, public IDocumentAccessor
		{

		public:
			static UE_API FNodeHandle GetInvalidHandle();
			...
			/**
			  ...
			  */
			virtual bool DiffAgainstRegistryInterface(FClassInterfaceUpdates& OutInterfaceUpdates, bool bInUseHighestMinorVersion, bool bInForceRegenerateClassInterfaceOverride=true) const = 0;
			...
			virtual void SetNodeStyle(const FMetasoundFrontendNodeStyle& InStyle) = 0;
	   		...
	   		virtual void SetNodeConfiguration(TInstancedStruct<FMetaSoundFrontendNodeConfiguration> InConfig) = 0;
	   		virtual TConstStructView<FMetaSoundFrontendNodeConfiguration> GetNodeConfiguration() const = 0;
		...
		class IGraphController : public TSharedFromThis<IGraphController>, public IDocumentAccessor
		{
			public:
			static UE_API FGraphHandle GetInvalidHandle();
			...
			// Sets graph style.
			virtual void SetGraphStyle(FMetasoundFrontendGraphStyle Style) = 0;

			// Sets the input style for the graph.
			virtual void SetInputStyle(FMetasoundFrontendInterfaceStyle Style) = 0;

			// Sets the output style for the graph.
			virtual void SetOutputStyle(FMetasoundFrontendInterfaceStyle Style) = 0;
		...
		class IDocumentController : public TSharedFromThis<IDocumentController>, public IDocumentAccessor
		{
		public:
			static UE_API FDocumentHandle GetInvalidHandle();

			/** Create a document from FMetasoundFrontendDocument description pointer. */
			static UE_API FDocumentHandle CreateDocumentHandle(FDocumentAccessPtr InDocument);
			static UE_API FDocumentHandle CreateDocumentHandle(FMetasoundFrontendDocument& InDocument);

			/** Create a document from FMetasoundFrontendDocument description pointer. */
			static UE_API FConstDocumentHandle CreateDocumentHandle(FConstDocumentAccessPtr InDocument);
			static UE_API FConstDocumentHandle CreateDocumentHandle(const FMetasoundFrontendDocument& InDocument);
