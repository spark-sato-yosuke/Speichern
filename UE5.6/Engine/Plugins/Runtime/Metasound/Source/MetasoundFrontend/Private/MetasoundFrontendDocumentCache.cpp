...
namespace Metasound::Frontend
{
		...
		: Parent(ParentCache)
	{
		...
		const TArray<FMetasoundFrontendClassOutput>& Outputs = GraphClass.GetDefaultInterface().Outputs;
	...
	const FMetasoundFrontendClassInput* FDocumentGraphInterfaceCache::FindInput(FName InputName) const
	{
		if (const int32* Index = InputNameToIndex.Find(InputName))
		{
			...
			return &Doc.RootGraph.GetDefaultInterface().Inputs[*Index];
		}
	...
	const FMetasoundFrontendClassOutput* FDocumentGraphInterfaceCache::FindOutput(FName OutputName) const
	{
		...
		{
			const FMetasoundFrontendDocument& Doc = Parent->GetDocument();
			return &Doc.RootGraph.GetDefaultInterface().Outputs[*Index];
		}
...
#if WITH_EDITOR
		OutDelegates.OnInputDataTypeChanged.AddSP(this, &FDocumentGraphInterfaceCache::OnInputDataTypeChanged);
		OutDelegates.OnOutputDataTypeChanged.AddSP(this, &FDocumentGraphInterfaceCache::OnOutputDataTypeChanged);
#endif // WITH_EDITOR
	}
	...
	void FDocumentGraphInterfaceCache::OnInputAdded(int32 NewIndex)
	{
		...
		const FMetasoundFrontendClassInput& Input = GraphClass.GetDefaultInterface().Inputs[NewIndex];
...
#if WITH_EDITOR
	void FDocumentGraphInterfaceCache::OnInputDataTypeChanged(int32 Index)
	{
		++TransactionCount;
	}
#endif // WITH_EDITOR
	...
	void FDocumentGraphInterfaceCache::OnOutputAdded(int32 NewIndex)
	{
		const FMetasoundFrontendDocument& Document = Parent->GetDocument();
		const FMetasoundFrontendGraphClass& GraphClass = Document.RootGraph;
		const FMetasoundFrontendClassOutput& Output = GraphClass.GetDefaultInterface().Outputs[NewIndex];
...
#if WITH_EDITOR
	void FDocumentGraphInterfaceCache::OnOutputDataTypeChanged(int32 Index)
	{
		++TransactionCount;
	}
#endif // WITH_EDITOR
	...
	void FDocumentGraphInterfaceCache::OnRemovingInput(int32 IndexBeingRemoved)
	{
		const FMetasoundFrontendDocument& Document = Parent->GetDocument();
		const FMetasoundFrontendGraphClass& GraphClass = Document.RootGraph;
		const FMetasoundFrontendClassInput& Input = GraphClass.GetDefaultInterface().Inputs[IndexBeingRemoved];
	...
	void FDocumentGraphInterfaceCache::OnRemovingOutput(int32 IndexBeingRemoved)
	{
		const FMetasoundFrontendDocument& Document = Parent->GetDocument();
		const FMetasoundFrontendGraphClass& GraphClass = Document.RootGraph;
		const FMetasoundFrontendClassOutput& Output = GraphClass.GetDefaultInterface().Outputs[IndexBeingRemoved];
	...
	FDocumentGraphNodeCache::FDocumentGraphNodeCache(TSharedRef<const FDocumentCache> ParentCache, const FGuid& InPageID)
		: Parent(ParentCache)
		, PageID(InPageID)
	{
		const FMetasoundFrontendGraph& Graph = Parent->GetDocument().RootGraph.FindConstGraphChecked(InPageID);
	...
	TArray<const FMetasoundFrontendVertex*> FDocumentGraphNodeCache::FindReroutedInputVertices(const FGuid& InNodeID, const FGuid& InVertexID, TArray<const FMetasoundFrontendNode*>* ConnectedNodes, bool* bOutIsRerouted) const
	{
		...
		if (const FMetasoundFrontendNode* Node = FindNode(InNodeID))
		{
			...
			if (Class->Metadata.GetClassName() == FRerouteNodeTemplate::ClassName)
			{
				...
				{
					const FMetasoundFrontendEdge& EdgeConnectedToOutput = Doc.RootGraph.FindConstGraphChecked(PageID).Edges[Index];
	...
	const FMetasoundFrontendVertex* FDocumentGraphNodeCache::FindReroutedOutputVertex(const FGuid& InNodeID, const FGuid& InVertexID, const FMetasoundFrontendNode** ConnectedNode, bool* bOutIsRerouted) const
	{
		if (const FMetasoundFrontendNode* Node = FindNode(InNodeID))
		{
			...
			if (Class->Metadata.GetClassName() == FRerouteNodeTemplate::ClassName)
			{
				...
				{
					const FMetasoundFrontendEdge& EdgeConnectedToInput = Doc.RootGraph.FindConstGraphChecked(PageID).Edges[*ConnectedEdgeIndex];
	...
	void FDocumentGraphNodeCache::OnRemoveSwappingNode(int32 SwapIndex, int32 LastIndex)
	{
		...
		const FMetasoundFrontendDocument& Document = Parent->GetDocument();
		const TArray<FMetasoundFrontendNode>& Nodes = Document.RootGraph.FindConstGraphChecked(PageID).Nodes;
	...
	FDocumentGraphEdgeCache::FDocumentGraphEdgeCache(TSharedRef<const FDocumentCache> ParentCache, const FGuid& InPageID)
		: Parent(ParentCache)
		, PageID(InPageID)
	{
		const TArray<FMetasoundFrontendEdge>& Edges = Parent->GetDocument().RootGraph.FindConstGraphChecked(InPageID).Edges;
	...
	{
		const FMetasoundFrontendDocument& Document = Parent->GetDocument();
		const TArray<FMetasoundFrontendEdge>& Edges = Document.RootGraph.FindConstGraphChecked(PageID).Edges;
