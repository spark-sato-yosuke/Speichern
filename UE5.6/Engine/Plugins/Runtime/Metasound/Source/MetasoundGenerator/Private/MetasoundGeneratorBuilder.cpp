...
namespace Metasound
{
	LLM_DEFINE_TAG(Audio_MetaSound_Generator_Build);
	...
	namespace GeneratorBuilder
	{
		...
		FInputVertexInterfaceData BuildGraphOperatorInputs(const FOperatorSettings& InOperatorSettings, FMetasoundGeneratorInitParams& InInitParams)
		{
			...
			for (const VertexDataPrivate::FInputBinding& Binding : InputData)
			{
		...
		FOperatorAndInputs BuildGraphOperator(const FOperatorSettings& InOperatorSettings, FMetasoundGeneratorInitParams& InInitParams, FBuildResults& OutBuildResults)
		{
			...
			LLM_SCOPE_BYTAG(Audio_MetaSound_Generator_Build);

			// asset-level trace asset path
			METASOUND_DEBUG_DECLARE_SCOPE;
			METASOUND_DEBUG_SET_ACTIVE_ASSET_SCOPE(
				  FName(InInitParams.MetaSoundName)
				, InInitParams.ClassName
				, InInitParams.AssetPath
			);
		...
		MetasoundGeneratorPrivate::FMetasoundGeneratorData BuildGeneratorData(const FOperatorSettings& InOperatorSettings, const FMetasoundGeneratorInitParams& InInitParams, FOperatorAndInputs&& InGraphOperatorAndInputs, TUniquePtr<Frontend::FGraphAnalyzer> InAnalyzer)
		{
			...
			// Get inputs
			FTriggerWriteRef PlayTrigger = VertexData.GetInputs().GetOrCreateDefaultDataWriteReference<FTrigger>(SourceInterface::Inputs::OnPlay, InOperatorSettings);
			...
			if (const FAnyDataReference* FinishTriggerRef = VertexData.GetOutputs().FindDataReference(SourceOneShotInterface::Outputs::OnFinished))
			{
 
				FinishTrigger = FinishTriggerRef->GetDataReadReference<FTrigger>();
			}
		...
		void ResetGraphOperatorInputs(const FOperatorSettings& InOperatorSettings, TArray<FAudioParameter> InParameterOverrides, FInputVertexInterfaceData& InOutInterface)
		{
			...
			for (VertexDataPrivate::FInputBinding& Binding : InOutInterface)
			{
