...
PRAGMA_ENABLE_DEPRECATION_WARNINGS
	...
	int32 FMetasoundGenerator::OnGenerateAudio(float* OutAudio, int32 NumSamplesRemaining)
	{
		...
		while (NumSamplesRemaining > 0 && !bIsFinishTriggered)
		{
			if (GraphRenderCost)
			{
			...
			// Create a scoped timed section for the bulk of the processing...
			{
				...
				{
					UE_LOG(LogMetasoundGenerator, Warning, TEXT("Terminating OnGenerateAudio() loop for MetaSound %s -- FillWithBuffer() returned %i"), *MetasoundName, ThisLoopNumSamplesWritten);
					break;
				}
			}
