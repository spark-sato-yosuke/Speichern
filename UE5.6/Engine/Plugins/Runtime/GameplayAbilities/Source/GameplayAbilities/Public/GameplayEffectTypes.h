...
private:
	...
	UE_API bool UpdateTagMap_Internal(const FGameplayTag& Tag, int32 CountDelta);
	...
	UE_API bool UpdateTagMapDeferredParentRemoval_Internal(const FGameplayTag& Tag, int32 CountDelta, TArray<FDeferredTagChangeDelegate>& DeferredTagChangeDelegates);

	/** Internal helper function to adjust the explicit tag list & corresponding map. */
	UE_API bool UpdateExplicitTags(const FGameplayTag& Tag, int32 CountDelta, bool bDeferParentTagsOnRemove);

	/** Internal helper function to collect the delegates that need to be called when Tag has its count changed by CountDelta. */
	UE_API bool GatherTagChangeDelegates(const FGameplayTag& Tag, int32 CountDelta, TArray<FDeferredTagChangeDelegate>& TagChangeDelegates);
};
