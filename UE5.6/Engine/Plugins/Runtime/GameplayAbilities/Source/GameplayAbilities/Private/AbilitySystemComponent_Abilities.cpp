...
static TAutoConsoleVariable<bool> CVarReplicateCancelDuringOnRemove(TEXT("AbilitySystem.ReplicateCancelOnRemove"), false, TEXT("When an ability is removed and not yet ended, assume it's cancelled and replicate to ensure the other side receives that before instance destruction. If false, you may miss EndAbility if the Actor is destroyed."));
	...
	for (auto Instance : Instances)
	{
		if (Instance)
		{
			if (Instance->IsActive())
			{
				...
				if (CVarReplicateCancelDuringOnRemove.GetValueOnGameThread())
				{
