...
#include "Editor/Kismet/RigVMBlueprintCompilationManager.h"
...
#define LOCTEXT_NAMESPACE "RigVMBlueprintCompilationManager"
...
namespace UE::RigVM::RigVMBlueprintCompilationManager::Private
{
	namespace ConsoleVariables
	{
		...
		/**
		 * Flag to disable skipping of reinstancing logic when a class is not observabling changed
		 ...
		 */
		static bool bAllowFastReinstancing = true;
...
struct FRigVMBlueprintCompilationManagerImpl : public FGCObject
{
	FRigVMBlueprintCompilationManagerImpl();
	virtual ~FRigVMBlueprintCompilationManagerImpl();

	// FGCObject:
	virtual void AddReferencedObjects(FReferenceCollector& Collector);
	virtual FString GetReferencerName() const override;

	void RegisterCompilerExtension(TSubclassOf<UBlueprint> BlueprintType, URigVMBlueprintCompilerExtension* Extension);

	void QueueForCompilation(const FRigVMBPCompileRequest& CompileJob);
	void CompileSynchronouslyImpl(const FRigVMBPCompileRequest& Request);
	...
	static void QueueOutOfDateDependencies( const TArray<FRigVMBPCompileRequest>& QueuedRequests, TArray<UBlueprint*>& OutBlueprintsToRecompile, TArray<FCompilerData>& CurrentlyCompilingBPs);
	...
	TMap<TObjectPtr<UClass>, TArray<TObjectPtr<URigVMBlueprintCompilerExtension>>> CompilerExtensions;

	// Queued requests to be processed in the next FlushCompilationQueueImpl call:
	TArray<FRigVMBPCompileRequest> QueuedRequests;
...
{
	FRigVMBlueprintCompilationManagerImpl::ReparentHierarchies(OldToNewMap, EReparentClassOptions::ReplaceReferencesToOldClasses);
}

FRigVMBlueprintCompilationManagerImpl::FRigVMBlueprintCompilationManagerImpl()
{
...
void FRigVMBlueprintCompilationManagerImpl::AddReferencedObjects(FReferenceCollector& Collector)
{
	...
	for (FRigVMBPCompileRequest& Job : QueuedRequests)
	{
...
FString FRigVMBlueprintCompilationManagerImpl::GetReferencerName() const
{
	return TEXT("FRigVMBlueprintCompilationManagerImpl");
}

void FRigVMBlueprintCompilationManagerImpl::RegisterCompilerExtension(TSubclassOf<UBlueprint> BlueprintType, URigVMBlueprintCompilerExtension* Extension)
{
	CompilerExtensions.FindOrAdd(BlueprintType).Emplace(Extension);
}

void FRigVMBlueprintCompilationManagerImpl::QueueForCompilation(const FRigVMBPCompileRequest& CompileJob)
{
#if WITH_EDITOR
...
void FRigVMBlueprintCompilationManagerImpl::CompileSynchronouslyImpl(const FRigVMBPCompileRequest& Request)
{
	...
	const bool bFindAndReplaceCDOReferences	= (Request.CompileOptions & EBlueprintCompileOptions::IncludeCDOInReferenceReplacement	) != EBlueprintCompileOptions::None ||
		...
		(UE::RigVM::RigVMBlueprintCompilationManager::Private::ConsoleVariables::bAllowFastReinstancing);
	...
	{
		FRigVMFindInBlueprintSearchManager::Get().AddOrUpdateBlueprintSearchMetadata(Request.BPToCompile);
	}
...
struct FCompilerData
{
		...
		bool bBytecodeOnly)
	{
		...
		if(InResultsLogOverride == nullptr)
		{
			...
			ResultsLog->BeginEvent(TEXT("RigVMBlueprintCompilationManager Compile"));
...
void FRigVMBlueprintCompilationManagerImpl::FlushCompilationQueueImpl(bool bSuppressBroadcastCompiled, TArray<UBlueprint*>* BlueprintsCompiled, TArray<UBlueprint*>* BlueprintsCompiledOrSkeletonCompiled, FUObjectSerializeContext* InLoadContext, TMap<UClass*, TMap<UObject*, UObject*>>* OldToNewTemplates /* = nullptr*/)
{
	...
	{ // begin TimeCompiling scope
		...
		for (const FRigVMBPCompileRequest& CompileJob : QueuedRequests)
		{
		...
		{
			FRigVMBPCompileRequest& QueuedJob = QueuedRequests[I];
		...
		*/
		...
		{
			using namespace UE::RigVM::RigVMBlueprintCompilationManager;
		...
		for (FCompilerData& CompilerData : CurrentlyCompilingBPs)
		{
			...
			if (CompilerData.ShouldUpdateBlueprintSearchMetadata())
			{
				...
				if (GEditor && GIsEditor)
				{
					...
					if (!BP->bIsRegeneratingOnLoad)
					{
						FRigVMFindInBlueprintSearchManager::Get().AddOrUpdateBlueprintSearchMetadata(BP);
					}
				}
			}
	...
	UE_LOGFMT(LogRigVMDeveloper, Display, "Finished compiling {0} Blueprint(s):", CurrentlyCompilingBPs.Num());
	for (const FCompilerData& CompilerData : CurrentlyCompilingBPs)
	{
		UE_LOGFMT(LogRigVMDeveloper, Display, "\t{0}", CompilerData.BP->GetFullName());
	}
	UE_LOGFMT(LogRigVMDeveloper, Display, "\tTime Compiling: {0} ms. Time Reinstancing: {1} ms.", TimeCompiling * 1000.0, TimeReinstancing * 1000.0);

	VerifyNoQueuedRequests(CurrentlyCompilingBPs);
}

void FRigVMBlueprintCompilationManagerImpl::FixupDelegateProperties(const TArray<FCompilerData>& InCurrentlyCompilingBPs)
{
...
void FRigVMBlueprintCompilationManagerImpl::ProcessExtensions(const TArray<FCompilerData>& InCurrentlyCompilingBPs)
{
	...
	{
		UBlueprint* BP = CompilerData.BP;
		FRigVMBlueprintCompiledData CompiledData; // populate only if we find an extension:
		...
		if(CompilerData.ShouldCompileClassLayout())
		{
			...
			while(Iter != UBlueprint::StaticClass()->GetSuperClass())
			{
				...
				if(Extensions)
				{
					...
					for(URigVMBlueprintCompilerExtension* Extension : *Extensions)
					{
...
void FRigVMBlueprintCompilationManagerImpl::FlushReinstancingQueueImpl(bool bFindAndReplaceCDOReferences, TMap<UClass*, TMap<UObject*, UObject*>>* OldToNewTemplates /* = nullptr*/)
{
	...
	UE_LOGFMT(LogRigVMDeveloper, Display, "Finished reinstancing for {0} class(es):", NumClassesToReinstance);
	UE_LOGFMT(LogRigVMDeveloper, Display, "\tTime Reinstancing: {0} ms.", TimeReinstancing * 1000.0);
}

bool FRigVMBlueprintCompilationManagerImpl::HasBlueprintsToCompile() const
{
	return QueuedRequests.Num() != 0;
}

bool FRigVMBlueprintCompilationManagerImpl::IsGeneratedClassLayoutReady() const
{
	return bGeneratedClassLayoutReady;
}

void FRigVMBlueprintCompilationManagerImpl::GetDefaultValue(const UClass* ForClass, const FProperty* Property, FString& OutDefaultValueAsString) const
{
...
void FRigVMBlueprintCompilationManagerImpl::VerifyNoQueuedRequests(const TArray<FCompilerData>& CurrentlyCompilingBPs)
{
	if (QueuedRequests.Num() != 0)
	{
		FString QueuedBlueprints = TEXT("");
		for (const FRigVMBPCompileRequest& Request: QueuedRequests)
		{
...
void FRigVMBlueprintCompilationManagerImpl::SaveConstructionHashes(const TArray<FCompilerData>& CurrentlyCompilingBPs)
{
	if( !UE::RigVM::RigVMBlueprintCompilationManager::Private::ConsoleVariables::bAllowFastReinstancing )
	{
		return;
	}
...
void FRigVMBlueprintCompilationManagerImpl::ReparentHierarchies(const TMap<UClass*, UClass*>& OldToNewClasses, EReparentClassOptions Options)
{
...
void FRigVMBlueprintCompilationManagerImpl::BuildDSOMap(UObject* OldObject, UObject* NewObject, const TMap<UClass*, UClass*>& InOldToNewClassMap, TMap<UObject*, UObject*>& OutOldToNewDSO)
{
...
void FRigVMBlueprintCompilationManagerImpl::ReinstanceBatch(TArray<FReinstancingJob>& Reinstancers, TMap< UClass*, UClass* >& InOutOldToNewClassMap, FUObjectSerializeContext* InLoadContext, TMap<UClass*, TMap<UObject*, UObject*>>* OldToNewTemplates /* = nullptr*/)
{
	TGuardValue<std::atomic<bool>, bool> ReinstancingGuard(GIsReinstancing, true);
	...
	{
		UObject* OriginalCDO = Class->GetDefaultObject(false);
	...
	for (const FReinstancingJob* ReinstancingJobPtr : ReinstancersPtr)
	{
		...
		if (UBlueprintGeneratedClass* BPGClass = Cast<UBlueprintGeneratedClass>(ReinstancingJob.OldToNew.Value))
		{
			...
			if (ReinstancingJob.Reinstancer.IsValid())
			{
				...
				UObject* NewCDO = CurrentBP->GeneratedClass->GetDefaultObject(false);
				FLinkerLoad* CurrentLinker = CurrentBP->GetLinker();
				if (CurrentLinker)
				{
		...
		if (OldClass)
		{
			...
			Algo::TopologicalSort(ReinstancingJob.OldArchetypeObjects, [&ArchetypeObjects](const FReinstancingJob::FArchetypeInfo& OldArchetypeInfo)
			{
				...
				if (OldArchetypeInfo.ArchetypeTemplate && !OldArchetypeInfo.ArchetypeTemplate->HasAnyFlags(RF_ClassDefaultObject))
				{
					...
					{
						UE_LOG(LogRigVMDeveloper, Warning, TEXT("Expecting the template object (%s) of archetype (%s) to already be in the list of archetypes"), *GetNameSafe(OldArchetypeInfo.ArchetypeTemplate), *GetNameSafe(OldArchetypeInfo.Archetype));
					}
				}
...
*/
UClass* FRigVMBlueprintCompilationManagerImpl::FastGenerateSkeletonClass(UBlueprint* BP, FKismetCompilerContext& CompilerContext, bool bIsSkeletonOnly, TArray<FSkeletonFixupData>& OutSkeletonFixupData)
{
...
bool FRigVMBlueprintCompilationManagerImpl::IsQueuedForCompilation(UBlueprint* BP)
{
	return BP->bQueuedForCompilation;
}

void FRigVMBlueprintCompilationManagerImpl::ConformToParentAndInterfaces(UBlueprint* BP)
{
...
void FRigVMBlueprintCompilationManagerImpl::RelinkSkeleton(UClass* SkeletonToRelink)
{
...
void FRigVMBlueprintCompilationManagerImpl::GatherOutOfDateDependenciesRecursive(TObjectPtr<UBlueprint> Gather, TSet<TObjectPtr<UBlueprint>>& OutOfDateDeps)
{
...
void FRigVMBlueprintCompilationManagerImpl::QueueOutOfDateDependencies( const TArray<FRigVMBPCompileRequest>& QueuedRequests, TArray<UBlueprint*>& OutBlueprintsToRecompile, TArray<FCompilerData>& CurrentlyCompilingBPs)
{
	...
	Algo::TransformIf(QueuedRequests, RootCompilationRequests,
		[](const FRigVMBPCompileRequest& CompileRequest) -> bool
		{
			return
		...
		[](const FRigVMBPCompileRequest& CompileRequest)
		{
...
FRigVMBlueprintCompilationManagerImpl::FFixupBytecodeReferences::FFixupBytecodeReferences(UObject* InObject)
{
...
FArchive& FRigVMBlueprintCompilationManagerImpl::FFixupBytecodeReferences::operator<<( UObject*& Obj )
{
	if (Obj != NULL)
	{
...
FArchive& FRigVMBlueprintCompilationManagerImpl::FFixupBytecodeReferences::operator<<(FField*& Field)
{
	if (Field != nullptr)
	{
...
FRigVMBlueprintCompilationManagerImpl* BPCMImpl = nullptr;
...
void FlushCompilationQueueImplWrapper(FUObjectSerializeContext* InLoadContext)
{
	FRigVMBlueprintCompilationManager::FlushCompilationQueue(InLoadContext);
}
...
// ***************************************************************
// FRigVMBPCompileRequest
// ***************************************************************

FRigVMBPCompileRequest::FRigVMBPCompileRequest(UBlueprint* InBPToCompile, EBlueprintCompileOptions InCompileOptions, FCompilerResultsLog* InClientResultsLog): BPToCompile(InBPToCompile)
...
// ***************************************************************
void FRigVMBlueprintCompilationManager::Initialize()
{
	if(!BPCMImpl)
	{
		BPCMImpl = new FRigVMBlueprintCompilationManagerImpl();
	}
}

void FRigVMBlueprintCompilationManager::Shutdown()
{
	delete BPCMImpl;
	BPCMImpl = nullptr;
}
...
// Forward to impl:
void FRigVMBlueprintCompilationManager::FlushCompilationQueue(FUObjectSerializeContext* InLoadContext)
{
	if(BPCMImpl)
	{
...
void FRigVMBlueprintCompilationManager::FlushCompilationQueueAndReinstance()
{
	if(BPCMImpl)
	{
...
void FRigVMBlueprintCompilationManager::CompileSynchronously(const FRigVMBPCompileRequest& Request)
{
	if(BPCMImpl)
	{
...
void FRigVMBlueprintCompilationManager::NotifyBlueprintLoaded(UBlueprint* BPLoaded)
{
	...
	BPCMImpl->QueueForCompilation(FRigVMBPCompileRequest(BPLoaded, EBlueprintCompileOptions::IsRegeneratingOnLoad, nullptr));
}

void FRigVMBlueprintCompilationManager::QueueForCompilation(UBlueprint* BPLoaded)
{
	BPCMImpl->QueueForCompilation(FRigVMBPCompileRequest(BPLoaded, EBlueprintCompileOptions::None, nullptr));
}

bool FRigVMBlueprintCompilationManager::IsGeneratedClassLayoutReady()
{
	if(!BPCMImpl)
	{
...
bool FRigVMBlueprintCompilationManager::GetDefaultValue(const UClass* ForClass, const FProperty* Property, FString& OutDefaultValueAsString)
{
	if(!BPCMImpl)
	{
...
void FRigVMBlueprintCompilationManager::ReparentHierarchies(const TMap<UClass*, UClass*>& OldClassToNewClass)
{
	FRigVMBlueprintCompilationManagerImpl::ReparentHierarchies(OldClassToNewClass, EReparentClassOptions::None);
}

void FRigVMBlueprintCompilationManager::RegisterCompilerExtension(TSubclassOf<UBlueprint> BlueprintType, URigVMBlueprintCompilerExtension* Extension)
{
	Initialize();
