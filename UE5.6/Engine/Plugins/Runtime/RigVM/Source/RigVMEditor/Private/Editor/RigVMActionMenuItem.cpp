...
#define LOCTEXT_NAMESPACE "RigVMActionMenuItem"
...
//------------------------------------------------------------------------------
FRigVMActionMenuItem::FRigVMActionMenuItem(UBlueprintNodeSpawner const* NodeSpawner, FBlueprintActionUiSpec const& UiSpec, IBlueprintNodeBinder::FBindingSet const& InBindings, FText InNodeCategory, int32 InGrouping)
...
//------------------------------------------------------------------------------
UEdGraphNode* FRigVMActionMenuItem::PerformAction(UEdGraph* ParentGraph, UEdGraphPin* FromPin, const FVector2f& Location, bool bSelectNewNode/* = true*/)
{
	...
	if (FromPin != nullptr)
	{
		...
		if (FromPin->Direction == EGPD_Input)
		{
			...
			const float FromNodeX = static_cast<float>(FromNode->NodePosX);

			constexpr float MinNodeDistance = 60.0f; // min distance between spawned nodes (to keep them from overlapping)
	...
	do
	{
		...
		bool bNewNode = false;
		LastSpawnedNode = InvokeAction(Action, ParentGraph, FDeprecateSlateVector2D(ModifiedLocation), BindingsSubset, /*out*/ bNewNode);
...
//------------------------------------------------------------------------------
UEdGraphNode* FRigVMActionMenuItem::PerformAction(UEdGraph* ParentGraph, TArray<UEdGraphPin*>& FromPins, const FVector2f& Location, bool bSelectNewNode/* = true*/)
{
...
//------------------------------------------------------------------------------
void FRigVMActionMenuItem::AddReferencedObjects(FReferenceCollector& Collector)
{
...
//------------------------------------------------------------------------------
UBlueprintNodeSpawner const* FRigVMActionMenuItem::GetRawAction() const
{
	return Action;
}

FSlateBrush const* FRigVMActionMenuItem::GetMenuIcon(FSlateColor& ColorOut)
{
	ColorOut = IconTint;
	return IconBrush;
}
