...
#include "RigVMBlueprintAssetHandler.h"
...
DEFINE_LOG_CATEGORY(LogRigVMFindInBlueprint);
CSV_DEFINE_CATEGORY(RigVMFindInBlueprint, false);

#define LOCTEXT_NAMESPACE "RigVMFindInBlueprintManager"

FRigVMFindInBlueprintSearchManager* FRigVMFindInBlueprintSearchManager::Instance = NULL;

const FText FRigVMFindInBlueprintSearchTags::FiB_Properties = LOCTEXT("Properties", "Properties");

const FText FRigVMFindInBlueprintSearchTags::FiB_Components = LOCTEXT("Components", "Components");
const FText FRigVMFindInBlueprintSearchTags::FiB_IsSCSComponent = LOCTEXT("IsSCSComponent", "IsSCSComponent");

const FText FRigVMFindInBlueprintSearchTags::FiB_Nodes = LOCTEXT("Nodes", "Nodes");

const FText FRigVMFindInBlueprintSearchTags::FiB_SchemaName = LOCTEXT("SchemaName", "SchemaName");

const FText FRigVMFindInBlueprintSearchTags::FiB_UberGraphs = LOCTEXT("Uber", "Uber");
const FText FRigVMFindInBlueprintSearchTags::FiB_Functions = LOCTEXT("Functions", "Functions");
const FText FRigVMFindInBlueprintSearchTags::FiB_Macros = LOCTEXT("Macros", "Macros");
const FText FRigVMFindInBlueprintSearchTags::FiB_SubGraphs = LOCTEXT("Sub", "Sub");
const FText FRigVMFindInBlueprintSearchTags::FiB_ExtensionGraphs = LOCTEXT("ExtGraphs", "ExtGraphs");
const FText FRigVMFindInBlueprintSearchTags::FiB_Extensions = LOCTEXT("Extensions", "Extensions");

const FText FRigVMFindInBlueprintSearchTags::FiB_Name = LOCTEXT("Name", "Name");
const FText FRigVMFindInBlueprintSearchTags::FiB_NativeName = LOCTEXT("NativeName", "Native Name");
const FText FRigVMFindInBlueprintSearchTags::FiB_ClassName = LOCTEXT("ClassName", "ClassName");
const FText FRigVMFindInBlueprintSearchTags::FiB_NodeGuid = LOCTEXT("NodeGuid", "NodeGuid");
const FText FRigVMFindInBlueprintSearchTags::FiB_Tooltip = LOCTEXT("Tooltip", "Tooltip");
const FText FRigVMFindInBlueprintSearchTags::FiB_DefaultValue = LOCTEXT("DefaultValue", "DefaultValue");
const FText FRigVMFindInBlueprintSearchTags::FiB_Description = LOCTEXT("Description", "Description");
const FText FRigVMFindInBlueprintSearchTags::FiB_Comment = LOCTEXT("Comment", "Comment");
const FText FRigVMFindInBlueprintSearchTags::FiB_Path = LOCTEXT("Path", "Path");
const FText FRigVMFindInBlueprintSearchTags::FiB_ParentClass = LOCTEXT("ParentClass", "ParentClass");
const FText FRigVMFindInBlueprintSearchTags::FiB_Interfaces = LOCTEXT("Interfaces", "Interfaces");
const FText FRigVMFindInBlueprintSearchTags::FiB_FuncOriginClass = LOCTEXT("FuncOriginClass", "FuncOriginClass");

const FText FRigVMFindInBlueprintSearchTags::FiB_Pins = LOCTEXT("Pins", "Pins");
const FText FRigVMFindInBlueprintSearchTags::FiB_PinCategory = LOCTEXT("PinCategory", "PinCategory");
const FText FRigVMFindInBlueprintSearchTags::FiB_PinSubCategory = LOCTEXT("SubCategory", "SubCategory");
const FText FRigVMFindInBlueprintSearchTags::FiB_ObjectClass = LOCTEXT("ObjectClass", "ObjectClass");
const FText FRigVMFindInBlueprintSearchTags::FiB_IsArray = LOCTEXT("IsArray", "IsArray");
const FText FRigVMFindInBlueprintSearchTags::FiB_IsReference = LOCTEXT("IsReference", "IsReference");
const FText FRigVMFindInBlueprintSearchTags::FiB_Glyph = LOCTEXT("Glyph", "Glyph");
const FText FRigVMFindInBlueprintSearchTags::FiB_GlyphStyleSet = LOCTEXT("GlyphStyleSet", "GlyphStyleSet");
const FText FRigVMFindInBlueprintSearchTags::FiB_GlyphColor = LOCTEXT("GlyphColor", "GlyphColor");

const FText FRigVMFindInBlueprintSearchTags::FiBMetaDataTag = LOCTEXT("FiBMetaDataTag", "!!FiBMD");

const FString FRigVMFiBMD::FiBSearchableMD = TEXT("BlueprintSearchable");
const FString FRigVMFiBMD::FiBSearchableShallowMD = TEXT("BlueprintSearchableShallow");
const FString FRigVMFiBMD::FiBSearchableExplicitMD = TEXT("BlueprintSearchableExplicit");
const FString FRigVMFiBMD::FiBSearchableHiddenExplicitMD = TEXT("BlueprintSearchableHiddenExplicit");
const FString FRigVMFiBMD::FiBSearchableFormatVersionMD = TEXT("BlueprintSearchableFormatVersion");

/** Attempts to initiate a rebuild of a random subset of the index cache each tick. */
static TAutoConsoleVariable<bool> CVarRandomSearchIndexCacheRebuildEachTick(
	TEXT("bp.RandomSearchIndexCacheRebuildEachTick"),
	false,
	TEXT("Attempts to initiate a random index cache update each tick (for debugging purposes)"),
	ECVF_Default
);
...
////////////////////////////////////
// FRigVMSearchDataVersionInfo
FRigVMSearchDataVersionInfo FRigVMSearchDataVersionInfo::Current =
{
	/** FiBDataVersion */
	ERigVMFiBVersion::RIGVM_FIB_VER_LATEST,
	/** EditorObjectVersion */
...
FRigVMStreamSearch::FRigVMStreamSearch(const FString& InSearchValue, const FRigVMStreamSearchOptions& InSearchOptions)
	...
	, StopTaskCounter(0)
{
	...
	Thread = TUniquePtr<FRunnableThread>(FRunnableThread::Create( this, *FString::Printf(TEXT("FRigVMStreamSearch_%d"), SearchId), 0, TPri_BelowNormal ));
}

bool FRigVMStreamSearch::Init()
{
	return true;
}

uint32 FRigVMStreamSearch::Run()
{
	const double StartTime = FPlatformTime::Seconds();
	CSV_EVENT(RigVMFindInBlueprint, TEXT("FRigVMStreamSearch_%d START"), SearchId);

	FRigVMFindInBlueprintSearchManager::Get().BeginSearchQuery(this);

	TFunction<void(const FRigVMSearchResult&)> OnResultReady = [this](const FRigVMSearchResult& Result) {
	...
	FRigVMSearchData QueryResult;
	while (FRigVMFindInBlueprintSearchManager::Get().ContinueSearchQuery(this, QueryResult))
	{
		if (QueryResult.ImaginaryBlueprint.IsValid())
		{
			...
			if (SearchOptions.ImaginaryDataFilter != ERigVMSearchQueryFilter::RigVMAllFilter)
			{
	...
	FRigVMFindInBlueprintSearchManager::Get().EnsureSearchQueryEnds(this);

	bThreadCompleted = true;

	CSV_EVENT(RigVMFindInBlueprint, TEXT("FRigVMStreamSearch_%d END"), SearchId);
	UE_LOG(LogRigVMFindInBlueprint, Log, TEXT("Search completed in %0.2f seconds."), FPlatformTime::Seconds() - StartTime);

	return 0;
}

void FRigVMStreamSearch::Stop()
{
	StopTaskCounter.Increment();
}

void FRigVMStreamSearch::Exit()
{

}

void FRigVMStreamSearch::EnsureCompletion()
{
	{
...
bool FRigVMStreamSearch::IsComplete() const
{
	return bThreadCompleted;
}

bool FRigVMStreamSearch::WasStopped() const
{
	return StopTaskCounter.GetValue() > 0;
}

void FRigVMStreamSearch::GetFilteredItems(TArray<FRigVMSearchResult>& OutItemsFound)
{
	FScopeLock ScopeLock(&SearchCriticalSection);
...
float FRigVMStreamSearch::GetPercentComplete() const
{
	return FRigVMFindInBlueprintSearchManager::Get().GetPercentComplete(this);
}

void FRigVMStreamSearch::GetFilteredImaginaryResults(TArray<FRigVMImaginaryFiBDataSharedPtr>& OutFilteredImaginaryResults)
{
...
namespace FiBSerializationHelpers
{
	...
	*/
	...
	bool ValidateSearchDataVersionInfo(const FRigVMSearchDataVersionInfo& InVersionInfo)
	{
		return InVersionInfo.FiBDataVersion != ERigVMFiBVersion::RIGVM_FIB_VER_NONE && InVersionInfo.EditorObjectVersion >= 0;
	}

	/** Helper function to validate and/or deserialize version info if necessary */
	bool ValidateSearchDataVersionInfo(const FString& InAssetPath, const FString& InFiBData, FRigVMSearchDataVersionInfo& InOutVersionInfo)
	{
		CSV_SCOPED_TIMING_STAT(RigVMFindInBlueprint, ValidateSearchDataVersionInfo);

		if (InOutVersionInfo.FiBDataVersion == ERigVMFiBVersion::RIGVM_FIB_VER_NONE)
		{
...
namespace BlueprintSearchMetaDataHelpers
{
	...
	struct FSearchableProperty
	{
		...
		ERigVMFiBVersion MinDataFormatVersion;
	...
	class TRigVMFindInBlueprintJsonStringWriter : public TJsonStringWriter<PrintPolicy>
	{
	public:
		typedef PrintPolicy InnerPrintPolicy;

		TRigVMFindInBlueprintJsonStringWriter(FString* const InOutString, int32 InFormatVersion)
	...
	/** Json Writer used for serializing FText's in the correct format for Find-in-Blueprints */
	class FRigVMFindInBlueprintJsonWriter : public TRigVMFindInBlueprintJsonStringWriter<TCondensedJsonPrintPolicy<TCHAR>>
	{
	public:
		FRigVMFindInBlueprintJsonWriter(FString* const InOutString, int32 InFormatVersion)
			:TRigVMFindInBlueprintJsonStringWriter<TCondensedJsonPrintPolicy<TCHAR>>(InOutString, InFormatVersion)
		...
		virtual bool Close() override
			...
			bool bResult = TRigVMFindInBlueprintJsonStringWriter<TCondensedJsonPrintPolicy<TCHAR>>::Close();
	...
	protected:
		...
		virtual void WriteTextValue(const FText& Text) override
		{
			...
			{
				TRigVMFindInBlueprintJsonStringWriter<TCondensedJsonPrintPolicy<TCHAR>>::WriteStringValue(FString::FromInt(*TableLookupValuePtr));
			}
			else
			{
				...
				TRigVMFindInBlueprintJsonStringWriter<TCondensedJsonPrintPolicy<TCHAR>>::WriteStringValue(FString::FromInt(TableLookupValue));
			}
		}
	...
	bool CheckIfJsonValueIsSearchable( TSharedPtr< FJsonValue > InJsonValue )
	{
		...
		else if(InJsonValue->Type == EJson::Object)
		{
			...
			for(TMap<FString, TSharedPtr<FJsonValue>>::TIterator Iter = JsonObject->Values.CreateIterator(); Iter; ++Iter)
			{
				...
				{
					bValidPropertyValue = true;
				}
			}
			
		}

		return bValidPropertyValue;
	}
	 ...
	 */
	template<class PrintPolicy>
	void SavePinTypeToJson(const TSharedRef<TRigVMFindInBlueprintJsonStringWriter<PrintPolicy>>& InWriter, const FEdGraphPinType& InPinType)
	{
		// Only save strings that are not empty

		if(!InPinType.PinCategory.IsNone())
		{
			InWriter->WriteValue(FRigVMFindInBlueprintSearchTags::FiB_PinCategory, InPinType.PinCategory.ToString());
		}

		if(!InPinType.PinSubCategory.IsNone())
		{
			InWriter->WriteValue(FRigVMFindInBlueprintSearchTags::FiB_PinSubCategory, InPinType.PinSubCategory.ToString());
		}

		if(InPinType.PinSubCategoryObject.IsValid())
		{
			// Write the full path because this can be an ambiguous blueprint class name
			InWriter->WriteValue(FRigVMFindInBlueprintSearchTags::FiB_ObjectClass, FText::FromString(InPinType.PinSubCategoryObject->GetPathName()));
		}
		InWriter->WriteValue(FRigVMFindInBlueprintSearchTags::FiB_IsArray, InPinType.IsArray());
		InWriter->WriteValue(FRigVMFindInBlueprintSearchTags::FiB_IsReference, InPinType.bIsReference);
	}
	 ...
	 */
	template<class PrintPolicy>
	void SaveVariableDescriptionToJson(const TSharedRef<TRigVMFindInBlueprintJsonStringWriter<PrintPolicy>>& InWriter, const UBlueprint* InBlueprint, const FBPVariableDescription& InVariableDescription)
	{
		FEdGraphPinType VariableType = InVariableDescription.VarType;

		InWriter->WriteObjectStart();

		InWriter->WriteValue(FRigVMFindInBlueprintSearchTags::FiB_Name, InVariableDescription.FriendlyName);
		...
		InWriter->WriteValue(FRigVMFindInBlueprintSearchTags::FiB_Tooltip, TooltipResult);
		...
		if(VariableProperty)
		{
			...
			if(BlueprintSearchMetaDataHelpers::CheckIfJsonValueIsSearchable(JsonValue))
			{
				...
				FJsonSerializer::Serialize<TCHAR, PrintPolicy>(JsonValue, FRigVMFindInBlueprintSearchTags::FiB_DefaultValue.ToString(), InWriter, false );
			}
		}
	 ...
	 */
	template<class PrintPolicy>
	void GatherSearchableProperties(const TSharedRef<TRigVMFindInBlueprintJsonStringWriter<PrintPolicy>>& InWriter, const void* InValue, UStruct* InStruct, EGatherSearchableType InSearchableType = SEARCHABLE_AS_DESIRED);
	 ...
	 */
	template<class PrintPolicy>
	void GatherSearchablesFromProperty(const TSharedRef<TRigVMFindInBlueprintJsonStringWriter<PrintPolicy>>& InWriter, FProperty* InProperty, const void* InValue, UStruct* InStruct)
	{
		...
		{
			if (!InProperty->HasMetaData(*FRigVMFiBMD::FiBSearchableMD) || InProperty->GetBoolMetaData(*FRigVMFiBMD::FiBSearchableMD))
			{
		...
		else if (FObjectProperty* ObjectProperty = CastField<FObjectProperty>(InProperty))
		{
			...
			if (SubObject)
			{
				...
				if (InProperty->HasMetaData(*FRigVMFiBMD::FiBSearchableMD))
				{
					// Check if that metadata informs us that the property should not be searchable
					bool bSearchable = InProperty->GetBoolMetaData(*FRigVMFiBMD::FiBSearchableMD);
					if (bSearchable)
					{
	...
	template<class PrintPolicy>
	void GatherSearchableProperties(const TSharedRef<TRigVMFindInBlueprintJsonStringWriter<PrintPolicy>>& InWriter, const void* InValue, UStruct* InStruct, EGatherSearchableType InSearchableType)
	{
		if (InValue)
		{
			...
			for (FSearchableProperty& SearchableProperty : *SearchablePropertyData)
			{
				...
				if (bShouldGatherSearchableProperty && InWriter->GetFormatVersion() >= SearchableProperty.MinDataFormatVersion)
				{
					...
					{
						InWriter->WriteObjectStart(FRigVMFindInBlueprintSearchTags::FiBMetaDataTag);
						{
							if (Property->GetBoolMetaData(*FRigVMFiBMD::FiBSearchableHiddenExplicitMD))
							{
								InWriter->WriteValue(FText::FromString(FRigVMFiBMD::FiBSearchableHiddenExplicitMD), true);
							}
							else if (Property->GetBoolMetaData(*FRigVMFiBMD::FiBSearchableExplicitMD))
							{
								InWriter->WriteValue(FText::FromString(FRigVMFiBMD::FiBSearchableExplicitMD), true);
							}
						}
						InWriter->WriteObjectEnd();
	...
	void CacheSearchableProperties(TMap<UStruct*, TArray<FSearchableProperty>>& InOutCachePropertyMapping, const void* InValue, UStruct* InStruct, EGatherSearchableType InSearchableType)
	{
		if (InValue)
		{
			...
			for (TFieldIterator<FProperty> PropIt(InStruct); PropIt; ++PropIt)
			{
				...
				bool bIsSearchableMD = Property->GetBoolMetaData(*FRigVMFiBMD::FiBSearchableMD);
				bool bIsShallowSearchableMD = Property->GetBoolMetaData(*FRigVMFiBMD::FiBSearchableShallowMD);
				// It only is truly marked as not searchable if it has the metadata set to false, if the metadata is missing then we assume the searchable type that is passed in unless SEARCHABLE_AS_DESIRED
				bool bIsMarkedNotSearchableMD = Property->HasMetaData(*FRigVMFiBMD::FiBSearchableMD) && !bIsSearchableMD;

				// Searchable properties default to the latest format version, unless otherwise specified in the property metadata.
				ERigVMFiBVersion MinDataFormatVersion = ERigVMFiBVersion::RIGVM_FIB_VER_LATEST;
				if (Property->HasMetaData(*FRigVMFiBMD::FiBSearchableFormatVersionMD))
				{
					const FString VersionString = Property->GetMetaData(*FRigVMFiBMD::FiBSearchableFormatVersionMD);
					if (!VersionString.IsEmpty())
					{
						const UEnum* FiBVersionEnum = StaticEnum<ERigVMFiBVersion>();
						const int64 VersionValue = FiBVersionEnum->GetValueByNameString(VersionString);
						if (VersionValue != INDEX_NONE)
						{
							MinDataFormatVersion = static_cast<ERigVMFiBVersion>(VersionValue);
						}
					}
				}
	 ...
	 */
	template<class PrintPolicy>
	void GatherNodesFromGraph(const TSharedRef<TRigVMFindInBlueprintJsonStringWriter<PrintPolicy>>& InWriter, const UEdGraph* InGraph)
	{
		// Collect all macro graphs
		InWriter->WriteArrayStart(FRigVMFindInBlueprintSearchTags::FiB_Nodes);
		{
				...
				{
					{
						...
						if (URigVMEdGraphNode* RigVMEdGraphNode = Cast<URigVMEdGraphNode>(Node))
						{
							...
							TArray<struct UBlueprintExtension::FSearchTagDataPair> Tags;
							RigVMEdGraphNode->AddRigVMSearchMetaDataInfo(Tags);

							// Go through the node metadata tags and put them into the Json object.
							for (const UBlueprintExtension::FSearchTagDataPair& SearchData : Tags)
							{
...

					{
						...
						InWriter->WriteArrayStart(FRigVMFindInBlueprintSearchTags::FiB_Pins);
							...
							if (Pin->bHidden == false || Pin->PinName == UEdGraphSchema_K2::PN_Self)
							{
								InWriter->WriteObjectStart();
								{
									if (URigVMEdGraphNode* RigVMEdGraphNode = Cast<URigVMEdGraphNode>(Node))
									{
										// Retrieve the search metadata from the node, some node types may have extra metadata to be searchable.
										TArray<struct UBlueprintExtension::FSearchTagDataPair> Tags;
										RigVMEdGraphNode->AddRigVMPinSearchMetaDataInfo(Pin, Tags);
	 ...
	 */
	template<class PrintPolicy>
	void GatherGraphSearchData(const TSharedRef<TRigVMFindInBlueprintJsonStringWriter<PrintPolicy>>& InWriter, const UBlueprint* InBlueprint, const TArray< UEdGraph* >& InGraphArray, FText InTitle, TArray< UEdGraph* >* InOutSubGraphs)
	{
		if(InGraphArray.Num() > 0)
		{
			...
			InWriter->WriteArrayStart(InTitle);
			{
				for(const UEdGraph* Graph : InGraphArray)
				{
					...
					InWriter->WriteValue(FRigVMFindInBlueprintSearchTags::FiB_Name, DisplayInfo.PlainName);

					FText GraphDescription = FBlueprintEditorUtils::GetGraphDescription(Graph);
					if(!GraphDescription.IsEmpty())
					{
						InWriter->WriteValue(FRigVMFindInBlueprintSearchTags::FiB_Description, GraphDescription);
					}
					...
					InWriter->WriteArrayStart(FRigVMFindInBlueprintSearchTags::FiB_Properties);
					{
	...
	template<class PrintPolicy>
	void GatherExtensionsSearchData_Recursive(const TSharedRef<TRigVMFindInBlueprintJsonStringWriter<PrintPolicy>>& InWriter, const UBlueprintExtension::FSearchData& SearchData)
	{
		...
		{
			InWriter->WriteArrayStart(ArrayData->Identifier.IsEmpty() ? FRigVMFindInBlueprintSearchTags::FiB_Extensions : ArrayData->Identifier);
	...
	template<class PrintPolicy>
	void GatherExtensionsSearchData(const TSharedRef<TRigVMFindInBlueprintJsonStringWriter<PrintPolicy>>& InWriter, const UBlueprint* InBlueprint)
	{
		if (InBlueprint->GetExtensions().Num() > 0)
		{
			...
			for (UBlueprintExtension* Extension : InBlueprint->GetExtensions())
			{
				if (Extension)
				{
					...
					{
						InWriter->WriteArrayStart(FRigVMFindInBlueprintSearchTags::FiB_Extensions);
						bExtensionArrayStarted = true;
					}

					InWriter->WriteObjectStart();
					InWriter->WriteValue(FRigVMFindInBlueprintSearchTags::FiB_Name, FText::FromString(Extension->GetName()));
					InWriter->WriteValue(FRigVMFindInBlueprintSearchTags::FiB_ClassName, FText::FromString(Extension->GetClass()->GetName()));
	...
	template<class PrintPolicy>
	void GatherBlueprintSearchMetadata(const TSharedRef<TRigVMFindInBlueprintJsonStringWriter<PrintPolicy>>& InWriter, const UBlueprint* Blueprint)
	{
		...
		GatherGraphSearchData(InWriter, Blueprint, Blueprint->UbergraphPages, FRigVMFindInBlueprintSearchTags::FiB_UberGraphs, &SubGraphs);
		...
		for (UBlueprintExtension* Extension : Blueprint->GetExtensions())
		{
			if (Extension)
			{
				...
				GatherGraphSearchData(InWriter, Blueprint, ExtensionGraphs, FRigVMFindInBlueprintSearchTags::FiB_ExtensionGraphs, &SubGraphs);
			}
		}
		...
		// We have interface graphs and function graphs to put into the Functions category. We cannot do them separately, so we must compile the full list
		{
			...
			if (InWriter->GetFormatVersion() >= ERigVMFiBVersion::RIGVM_FIB_VER_INTERFACE_GRAPHS)
			{
			...
			GatherGraphSearchData(InWriter, Blueprint, CompleteGraphList, FRigVMFindInBlueprintSearchTags::FiB_Functions, &SubGraphs);
		}

		// Gather Macros
		GatherGraphSearchData(InWriter, Blueprint, Blueprint->MacroGraphs, FRigVMFindInBlueprintSearchTags::FiB_Macros, &SubGraphs);
		...
		GatherGraphSearchData(InWriter, Blueprint, SubGraphs, FRigVMFindInBlueprintSearchTags::FiB_SubGraphs, nullptr);
		...
		if (Blueprint->SimpleConstructionScript)
		{
			...
			InWriter->WriteArrayStart(FRigVMFindInBlueprintSearchTags::FiB_Components);
			for (TFieldIterator<FProperty> PropertyIt(Blueprint->SkeletonGeneratedClass, EFieldIteratorFlags::ExcludeSuper); PropertyIt; ++PropertyIt)
			{
				...
				if (bComponentProperty && FBlueprintEditorUtils::FindSCS_Node(Blueprint, PropName))
				{
					...
					if (UEdGraphSchema_K2::StaticClass()->GetDefaultObject<UEdGraphSchema_K2>()->ConvertPropertyToPinType(Property, PropertyPinType))
					{
						InWriter->WriteObjectStart();
						{
							InWriter->WriteValue(FRigVMFindInBlueprintSearchTags::FiB_Name, FText::FromName(PropName));
							InWriter->WriteValue(FRigVMFindInBlueprintSearchTags::FiB_IsSCSComponent, true);
	...
	{
		CSV_SCOPED_TIMING_STAT(RigVMFindInBlueprint, GatherBlueprintSearchMetadata);
...
public:
	...
	virtual uint32 Run() override
	{
		while (Controller->IsWorkPending())
		{
			...
			{
				CSV_CUSTOM_STAT(RigVMFindInBlueprint, IndexedAssetCountThisFrame, 1, ECsvCustomStatOp::Accumulate);

				FSoftObjectPath AssetPath = AssetPathsToIndex[ArrayIdx];
				FRigVMFindInBlueprintSearchManager& FindManager = FRigVMFindInBlueprintSearchManager::Get();
				FRigVMSearchData SearchData = FindManager.GetSearchDataForAssetPath(AssetPath);
					...
					else
					{
						if (bEnableFullIndexingPass)
						{
							...
							if (FindManager.ProcessEncodedValueForUnloadedBlueprint(SearchData))
							{
								...
								TArray<FRigVMImaginaryFiBDataSharedPtr> IndexNodes = { SearchData.ImaginaryBlueprint };
								while (IndexNodes.Num() > 0)
								{
									...
									TArray<FRigVMImaginaryFiBDataSharedPtr> ChildNodes;
									for (FRigVMImaginaryFiBDataSharedPtr& NodePtr : IndexNodes)
									{
							...
							SearchData.StateFlags |= ERigVMSearchDataStateFlags::IsIndexed;
...
public:
	...
	struct FRigVMCacheParams
	{
		/** Cache type */
		ERigVMFiBCacheOpType OpType;

		/** Control flags */
		ERigVMFiBCacheOpFlags OpFlags;
		...
		FRigVMCacheParams()
			:OpFlags(ERigVMFiBCacheOpFlags::None)
			,AsyncTaskBatchSize(0)
		{
		}
	};

	FCacheAllBlueprintsTickableObject(const TSet<FSoftObjectPath>& InAssets, const FRigVMCacheParams& InParams)
		...
		, bIsCancelled(false)
	{
		...
		if (EnumHasAnyFlags(CacheParams.OpFlags, ERigVMFiBCacheOpFlags::ShowProgress)
			&& !EnumHasAnyFlags(CacheParams.OpFlags, ERigVMFiBCacheOpFlags::HideNotifications))
		{
			...
			if (EnumHasAnyFlags(CacheParams.OpFlags, ERigVMFiBCacheOpFlags::AllowUserCancel))
			{
	...
	ERigVMFiBCacheOpType GetCurrentCacheOpType() const
	{
		return CacheParams.OpType;
	}

	/** Returns the current cache operation control flags */
	ERigVMFiBCacheOpFlags GetCurrentCacheOpFlags() const
	{
	...
	void OnCancelCaching(bool bIsImmediate)
	{
		if (!bIsCancelled)
		{
			...
			if (bIsImmediate)
			{
				...
				FRigVMFindInBlueprintSearchManager::Get().FinishedCachingBlueprints(CacheParams.OpType, CacheParams.OpFlags, TickCacheIndex, FailedToCacheList);
			}
			else
			{
				bIsCancelled = true;
	...
	virtual bool IsWorkPending() const override
	{
		...
		return StartIndex < UncachedAssets.Num() || !AssetsPendingAsyncIndexing.IsEmpty() || bHasAssetsPendingGatherWork || bIsGatheringSearchMetadata;
	}

	virtual bool ShouldFullyIndexAssets() const override
	{
		return !EnumHasAnyFlags(CacheParams.OpFlags, ERigVMFiBCacheOpFlags::ExecuteGatherPhaseOnly);
	}

	virtual bool ShouldEnableMultiprocessing() const override
	{
		if (CacheParams.OpType == ERigVMFiBCacheOpType::CachePendingAssets)
		{
			...
			return FSlateThrottleManager::Get().IsAllowingExpensiveTasks()
				&& (FRigVMFindInBlueprintSearchManager::Get().IsGlobalFindResultsOpen() || FRigVMFindInBlueprintSearchManager::Get().IsAsyncSearchQueryInProgress())
				&& !FRigVMFindInBlueprintSearchManager::Get().IsAssetDiscoveryInProgress()
				&& !EnumHasAnyFlags(CacheParams.OpFlags, ERigVMFiBCacheOpFlags::ExecuteOnSingleThread);
		}
	...
	void Start()
	{
		...
		{
			bIsStarted = true;
			FRigVMFindInBlueprintSearchManager::Get().StartedCachingBlueprints(CacheParams.OpType, CacheParams.OpFlags);

			if (CacheParams.OpType == ERigVMFiBCacheOpType::CachePendingAssets && !EnumHasAnyFlags(CacheParams.OpFlags, ERigVMFiBCacheOpFlags::ExecuteOnMainThread))
			{
		...
		else
		{
				...
				if (AsyncIndexBuilderTask.IsValid())
				{
					...
					if (!AssetsPendingGatherQueue.IsEmpty())
					{
						...
						if (AssetsPendingGatherQueue.Dequeue(AssetPath))
						{
							...
							FRigVMSearchData SearchData = FRigVMFindInBlueprintSearchManager::Get().GetSearchDataForAssetPath(AssetPath);
							if (SearchData.IsValid() && !SearchData.IsMarkedForDeletion())
							{
								if (UBlueprint* Blueprint = SearchData.Blueprint.Get())
								{
									...
									SearchData.Value = GatherBlueprintSearchMetadata<FRigVMFindInBlueprintJsonWriter>(Blueprint, SearchData.VersionInfo.FiBDataVersion);
								...
								FRigVMFindInBlueprintSearchManager::Get().ApplySearchDataToDatabase(MoveTemp(SearchData));
							}
			...
			else
			{
				...
				if (AssetData.IsValid())
				{
					...
					const bool bCheckOutAndSave = EnumHasAnyFlags(CacheParams.OpFlags, ERigVMFiBCacheOpFlags::CheckOutAndSave);
...
FRigVMFindInBlueprintSearchManager& FRigVMFindInBlueprintSearchManager::Get()
{
	if (Instance == NULL)
	{
		Instance = new FRigVMFindInBlueprintSearchManager();
		Instance->Initialize();
	}
...
FRigVMFindInBlueprintSearchManager::FRigVMFindInBlueprintSearchManager()
...
void FRigVMFindInBlueprintSearchManager::Initialize()
{
	// Init configuration
	...
	{
		FCsvProfiler::Get()->EnableCategoryByString(TEXT("RigVMFindInBlueprint"));
		if (!FCsvProfiler::Get()->IsCapturing())
		{
			const FString CaptureFolder = FPaths::ProfilingDir() + TEXT("CSV/RigVMFindInBlueprint");
	...
	if (!GIsSavingPackage || (GIsSavingPackage && FModuleManager::Get().IsModuleLoaded(TEXT("AssetRegistry"))))
	{
		...
		if (AssetRegistry)
		{
			AssetRegistry->OnFilesLoaded().AddRaw(this, &FRigVMFindInBlueprintSearchManager::OnAssetRegistryFilesLoaded);
		}
	}
	else
	{
	...
	FKismetEditorUtilities::OnBlueprintUnloaded.AddRaw(this, &FRigVMFindInBlueprintSearchManager::OnBlueprintUnloaded);

	FCoreUObjectDelegates::GetPreGarbageCollectDelegate().AddRaw(this, &FRigVMFindInBlueprintSearchManager::PauseFindInBlueprintSearch);
	FCoreUObjectDelegates::GetPostGarbageCollect().AddRaw(this, &FRigVMFindInBlueprintSearchManager::UnpauseFindInBlueprintSearch);
	FCoreUObjectDelegates::OnAssetLoaded.AddRaw(this, &FRigVMFindInBlueprintSearchManager::OnAssetLoaded);
	
	// Register to be notified of reloads
	FCoreUObjectDelegates::ReloadCompleteDelegate.AddRaw(this, &FRigVMFindInBlueprintSearchManager::OnReloadComplete);
...
void FRigVMFindInBlueprintSearchManager::OnAssetAdded(const FAssetData& InAssetData)
{
	...
	const IRigVMBlueprintAssetHandler* Handler = AssetClass ? FRigVMBlueprintAssetHandler::Get().FindHandler(AssetClass) : nullptr;
...
void FRigVMFindInBlueprintSearchManager::AddUnloadedBlueprintSearchMetadata(const FAssetData& InAssetData)
{
	...
	if (Result.IsSet())
	{
		if (bDisableImmediateAssetDiscovery)
		{
			...
			ExtractUnloadedFiBData(InAssetData, nullptr, FBlueprintTags::FindInBlueprintsData, ERigVMFiBVersion::RIGVM_FIB_VER_NONE);
		}
		else
		{
			// Extract it now
			...
			{
				ExtractUnloadedFiBData(InAssetData, &FiBVersionedSearchData, NAME_None, ERigVMFiBVersion::RIGVM_FIB_VER_NONE);
			}
		}
	}
	else
	{
		...
		if (ResultLegacy.IsSet())
		{
			if (bDisableImmediateAssetDiscovery)
			{
				ExtractUnloadedFiBData(InAssetData, nullptr, FBlueprintTags::UnversionedFindInBlueprintsData, ERigVMFiBVersion::RIGVM_FIB_VER_BASE);
			}
			else
			{
				FString FiBUnversionedSearchData = ResultLegacy.GetValue();
				ExtractUnloadedFiBData(InAssetData, &FiBUnversionedSearchData, NAME_None, ERigVMFiBVersion::RIGVM_FIB_VER_BASE);
			}
		}
...
bool FRigVMFindInBlueprintSearchManager::ProcessEncodedValueForUnloadedBlueprint(FRigVMSearchData& SearchData)
{
	...
	if (!SearchData.AssetKeyForValue.IsNone() && SearchData.Value.IsEmpty())
	{
		...
		if (TempEncodedString.Len() == 0)
		{
			...
			UE_LOG(LogRigVMFindInBlueprint, Warning, TEXT("%s AssetData loaded with %s has an invalid FiB tag!"), *AssetData.GetSoftObjectPath().ToString(), *AssetPath);
	...
	if (FiBSerializationHelpers::ValidateSearchDataVersionInfo(AssetPath, EncodedString, SearchData.VersionInfo))
	{
		...
		SearchData.ImaginaryBlueprint = MakeShareable(new FRigVMImaginaryBlueprint(FPaths::GetBaseFilename(AssetPath), AssetPath, SearchData.ParentClass, SearchData.Interfaces, EncodedString, SearchData.VersionInfo));
...
void FRigVMFindInBlueprintSearchManager::ExtractUnloadedFiBData(const FAssetData& InAssetData, FString* InFiBData, FName InKeyForFiBData, ERigVMFiBVersion InFiBDataVersion)
{
	CSV_SCOPED_TIMING_STAT(RigVMFindInBlueprint, ExtractUnloadedFiBData);
	CSV_CUSTOM_STAT(RigVMFindInBlueprint, ExtractUnloadedCountThisFrame, 1, ECsvCustomStatOp::Accumulate);
	...
	FRigVMSearchData SearchData = GetSearchDataForAssetPath(InAssetData.GetSoftObjectPath());
	if (SearchData.IsValid() && !SearchData.IsMarkedForDeletion())
	{
		return;
	}

	FRigVMSearchData NewSearchData;
	...
	else
	{
		...
		NewSearchData.StateFlags |= ERigVMSearchDataStateFlags::IsIndexed;
	}
...
FRigVMSearchData FRigVMFindInBlueprintSearchManager::GetSearchDataForIndex(int32 CacheIndex)
{
	...
	return FRigVMSearchData();
}

FRigVMSearchData FRigVMFindInBlueprintSearchManager::GetSearchDataForAssetPath(const FSoftObjectPath& InAssetPath)
{
	FScopeLock ScopeLock(&SafeModifyCacheCriticalSection);
...
void FRigVMFindInBlueprintSearchManager::ApplySearchDataToDatabase(FRigVMSearchData InSearchData, bool bAllowNewEntry)
{
	FScopeLock ScopeLock(&SafeModifyCacheCriticalSection);
...
void FRigVMFindInBlueprintSearchManager::RemoveBlueprintByPath(const FSoftObjectPath& InPath)
{
	FScopeLock ScopeLock(&SafeModifyCacheCriticalSection);
	...
	if(SearchIdx)
	{
		...
		SearchArray[*SearchIdx].StateFlags |= ERigVMSearchDataStateFlags::WasRemoved;
	}
}
void FRigVMFindInBlueprintSearchManager::OnAssetRemoved(const struct FAssetData& InAssetData)
{
...
void FRigVMFindInBlueprintSearchManager::OnAssetRenamed(const struct FAssetData& InAssetData, const FString& InOldName)
{
...
void FRigVMFindInBlueprintSearchManager::OnAssetRegistryFilesLoaded()
{
	CSV_EVENT(RigVMFindInBlueprint, TEXT("OnAssetRegistryFilesLoaded"));
	...
	if (!IsCacheInProgress() && PendingAssets.Num() == 0)
	{
		...
		for (TWeakPtr<SRigVMFindInBlueprints> FindResultsPtr : GlobalFindResults)
		{
			if (FindResultsPtr.IsValid())
			{
				FindResultsPtr.Pin()->OnCacheComplete(ERigVMFiBCacheOpType::CachePendingAssets, ERigVMFiBCacheOpFlags::None);
			}
		}
	}
}

void FRigVMFindInBlueprintSearchManager::OnAssetLoaded(UObject* InAsset)
{
	const IRigVMBlueprintAssetHandler* Handler = FRigVMBlueprintAssetHandler::Get().FindHandler(InAsset->GetClass());
	...
	if (BlueprintObject)
	{
		...
		FRigVMSearchData SearchData = GetSearchDataForAssetPath(AssetPath);
...
void FRigVMFindInBlueprintSearchManager::OnBlueprintUnloaded(UBlueprint* InBlueprint)
{
	if (InBlueprint)
	{
...
void FRigVMFindInBlueprintSearchManager::OnReloadComplete(EReloadCompleteReason Reason)
{
	CachedAssetClasses.Reset();
}

void FRigVMFindInBlueprintSearchManager::AddOrUpdateBlueprintSearchMetadata(UBlueprint* InBlueprint, ERigVMAddOrUpdateBlueprintSearchMetadataFlags InFlags/* = ERigVMAddOrUpdateBlueprintSearchMetadataFlags::None*/, ERigVMFiBVersion InVersion/* = ERigVMFiBVersion::FIB_VER_LATEST*/)
{
	CSV_SCOPED_TIMING_STAT(RigVMFindInBlueprint, AddOrUpdateBlueprintSearchMetadata);
	CSV_CUSTOM_STAT(RigVMFindInBlueprint, AddOrUpdateCountThisFrame, 1, ECsvCustomStatOp::Accumulate);
	...
	// Control flags
	const bool bForceRecache = EnumHasAllFlags(InFlags, ERigVMAddOrUpdateBlueprintSearchMetadataFlags::ForceRecache);
	const bool bClearCachedValue = EnumHasAllFlags(InFlags, ERigVMAddOrUpdateBlueprintSearchMetadataFlags::ClearCachedValue);
	...
	if (SearchData.IsValid())
	{
		...
		SearchData.StateFlags &= ~(ERigVMSearchDataStateFlags::IsIndexed | ERigVMSearchDataStateFlags::WasRemoved);
	}
	else
	{
		SearchData = FRigVMSearchData();
	...
	if (SearchData.IsValid())
	{
		...
		SearchData.VersionInfo = FRigVMSearchDataVersionInfo::Current;
		if (InVersion != ERigVMFiBVersion::RIGVM_FIB_VER_NONE)
		{
		...
		if (bForceRecache || bClearCachedValue || IsUnindexedCacheInProgress() || bDisableDeferredIndexing)
		{
			...
			if (!bClearCachedValue && InBlueprint->SkeletonGeneratedClass != nullptr)
			{
				...
				SearchData.Value = GatherBlueprintSearchMetadata<FRigVMFindInBlueprintJsonWriter>(InBlueprint, SearchData.VersionInfo.FiBDataVersion);
			}
...
FRigVMFindInBlueprintSearchManager::FActiveSearchQueryPtr FRigVMFindInBlueprintSearchManager::FindSearchQuery(const class FRigVMStreamSearch* InSearchOriginator) const
{
...
FRigVMSearchData FRigVMFindInBlueprintSearchManager::GetNextSearchDataForQuery(const FRigVMStreamSearch* SearchOriginator, FActiveSearchQueryPtr SearchQuery, bool bCheckDeferredList)
{
	...
	FRigVMSearchData SearchData = GetSearchDataForIndex(SearchQuery->NextIndex);
		...
		while (SearchQuery->DeferredAssetPaths.Dequeue(AssetPath))
		{
			...
			if (SearchData.IsValid() && !SearchData.IsMarkedForDeletion())
			{
				...
				if (!SearchData.IsIndexingCompleted())
				{
					...
					{
						UE_LOG(LogRigVMFindInBlueprint, Warning, TEXT("%s has been unexpectedly excluded from indexing and will not be included in the search."), *AssetPath.ToString());
					}
...
void FRigVMFindInBlueprintSearchManager::BlockSearchQueryIfPaused()
{
...
void FRigVMFindInBlueprintSearchManager::BeginSearchQuery(const FRigVMStreamSearch* InSearchOriginator)
{
...
bool FRigVMFindInBlueprintSearchManager::ContinueSearchQuery(const FRigVMStreamSearch* InSearchOriginator, FRigVMSearchData& OutSearchData)
{
	...
	FRigVMSearchData SearchData = GetNextSearchDataForQuery(InSearchOriginator, SearchQuery, bCheckDeferredList);
	if (SearchData.IsValid())
	{
...
void FRigVMFindInBlueprintSearchManager::EnsureSearchQueryEnds(const class FRigVMStreamSearch* InSearchOriginator)
{
	// Must lock this behind a critical section to ensure that no other thread is accessing it at the same time
	FScopeLock ScopeLock(&SafeQueryModifyCriticalSection);
...
float FRigVMFindInBlueprintSearchManager::GetPercentComplete(const FRigVMStreamSearch* InSearchOriginator) const
{
	FScopeLock ScopeLock(&SafeQueryModifyCriticalSection);
	FActiveSearchQueryPtr SearchQuery = FindSearchQuery(InSearchOriginator);
...
FRigVMSearchData FRigVMFindInBlueprintSearchManager::QuerySingleBlueprint(UBlueprint* InBlueprint, bool bInRebuildSearchData)
{
	...
	if (!bIsDiffingBlueprint)
	{
		if (bInRebuildSearchData)
		{
			...
			AddOrUpdateBlueprintSearchMetadata(InBlueprint, ERigVMAddOrUpdateBlueprintSearchMetadataFlags::ForceRecache);
		}
		...
		FRigVMSearchData SearchData = GetSearchDataForAssetPath(Key);
		if (SearchData.IsValid())
		{
			return SearchData;
		}
...
void FRigVMFindInBlueprintSearchManager::PauseFindInBlueprintSearch()
{
...
void FRigVMFindInBlueprintSearchManager::UnpauseFindInBlueprintSearch()
{
...
void FRigVMFindInBlueprintSearchManager::CleanCache()
{
	...
	for( auto It = ActiveSearchQueries.CreateIterator() ; It ; ++It )
	{
	 	...
	 	check(ActiveSearch);
	 	{
			...
			if (SearchQuery.IsValid())
			{
				...
				FRigVMSearchData SearchData = GetNextSearchDataForQuery(ActiveSearch, SearchQuery, bCheckDeferredList);
				if (SearchData.IsValid())
				{
	...
	TArray<FRigVMSearchData> NewSearchArray;
	...
	for(const TPair<FSoftObjectPath, int32>& SearchValuePair : SearchMap)
	{
		...
		FRigVMSearchData& SearchData = SearchArray[SearchValuePair.Value];
		if (!SearchData.IsValid()
...
void FRigVMFindInBlueprintSearchManager::BuildCache()
{
	TRACE_CPUPROFILER_EVENT_SCOPE(FRigVMFindInBlueprintSearchManager::BuildCache);
	...
	for (FTopLevelAssetPath ClassPathName : FRigVMBlueprintAssetHandler::Get().GetRegisteredClassNames())
	{
	...
	AssetRegistry->OnAssetAdded().AddRaw(this, &FRigVMFindInBlueprintSearchManager::OnAssetAdded);
	AssetRegistry->OnAssetRemoved().AddRaw(this, &FRigVMFindInBlueprintSearchManager::OnAssetRemoved);
	AssetRegistry->OnAssetRenamed().AddRaw(this, &FRigVMFindInBlueprintSearchManager::OnAssetRenamed);
}

void FRigVMFindInBlueprintSearchManager::DumpCache(FArchive& Ar)
{
	FScopeLock ScopeLock(&SafeModifyCacheCriticalSection);

	for (const FRigVMSearchData& SearchData : SearchArray)
	{
...
void FRigVMFindInBlueprintSearchManager::DoRandomIndexCacheRebuild()
{
	if (AssetRegistryModule && bHasFirstSearchOccurred && !IsCacheInProgress())
	{
		FARFilter ARFilter;
		...
		const int32 MinSize = FMath::Min(1000, AssetList.Num());
		const int32 MaxSize = FMath::Min(10000, AssetList.Num());
		const int32 SubsetSize = MinSize + (FMath::Rand() % (MaxSize - MinSize + 1));
		const int32 StartingIndex = FMath::Rand() % (AssetList.Num() - SubsetSize + 1);
		for (int32 i = StartingIndex; i < StartingIndex + SubsetSize; ++i)
		{
			RemoveBlueprintByPath(AssetList[i].ToSoftObjectPath());

			if (AssetList[i].IsAssetLoaded())
			{
				UBlueprint* Blueprint = CastChecked<UBlueprint>(AssetList[i].GetAsset());
				AddOrUpdateBlueprintSearchMetadata(Blueprint);
			}
			else
			{
				AddUnloadedBlueprintSearchMetadata(AssetList[i]);
			}
		}
	}
}

FText FRigVMFindInBlueprintSearchManager::ConvertHexStringToFText(FString InHexString)
{
	TArray<uint8> SerializedData;
...
FString FRigVMFindInBlueprintSearchManager::ConvertFTextToHexString(FText InValue)
{
	TArray<uint8> SerializedData;
	FMemoryWriter Ar(SerializedData);
...
FString FRigVMFindInBlueprintSearchManager::GenerateSearchIndexForDebugging(UBlueprint* InBlueprint)
{
	using namespace BlueprintSearchMetaDataHelpers;
	return GatherBlueprintSearchMetadata<TRigVMFindInBlueprintJsonStringWriter<TPrettyJsonPrintPolicy<TCHAR>>>(InBlueprint, ERigVMFiBVersion::RIGVM_FIB_VER_LATEST);
}

void FRigVMFindInBlueprintSearchManager::OnCacheAllUnindexedAssets(bool bInSourceControlActive, bool bInCheckoutAndSave)
{
...
void FRigVMFindInBlueprintSearchManager::CacheAllAssets(TWeakPtr< SRigVMFindInBlueprints > InSourceWidget, const FRigVMFindInBlueprintCachingOptions& InOptions)
{
	// Do not start another caching process if one is in progress
	if(!IsCacheInProgress())
	{
		FCacheAllBlueprintsTickableObject::FRigVMCacheParams CacheParams;
		...
		if (CacheParams.OpType == ERigVMFiBCacheOpType::CachePendingAssets)
		{
			...
			{
				CacheParams.OpFlags |= ERigVMFiBCacheOpFlags::ShowProgress;
			}

			// Keep popup notifications hidden during this operation
			CacheParams.OpFlags |= ERigVMFiBCacheOpFlags::HideNotifications;
			...
			bool bIsCachingDiscoveredAssets = EnumHasAnyFlags(InOptions.OpFlags, ERigVMFiBCacheOpFlags::IsCachingDiscoveredAssets);
			...
			if (bIsCachingDiscoveredAssets)
			{
				CacheParams.OpFlags |= ERigVMFiBCacheOpFlags::IsCachingDiscoveredAssets;
				...
				if (bIsAssetDiscoveryInProgress)
				{
					...
					CacheParams.OpFlags |= ERigVMFiBCacheOpFlags::HideProgressBars;
					CacheParams.OpFlags |= ERigVMFiBCacheOpFlags::KeepProgressVisibleOnCompletion;
				}
			}

			if (bDisableThreadedIndexing)
			{
				CacheParams.OpFlags |= ERigVMFiBCacheOpFlags::ExecuteOnMainThread;
			}
//			else if (bEnableCSVStatsProfiling)
//			{
//				CacheParams.OpFlags |= ERigVMFiBCacheOpFlags::ExecuteOnSingleThread;
//			}
			...
			else
			{
				...
				for (const FSoftObjectPath& AssetPath : PendingAssets)
				{
					...
					{
						const FRigVMSearchData& SearchData = SearchArray[*IndexPtr];
				...
				if (AssetsToPartiallyCache.Num() > 0)
				{
					...
					CacheParams.OpFlags |= ERigVMFiBCacheOpFlags::ExecuteGatherPhaseOnly;
		...
		else
		{
			...
			for (FRigVMSearchData SearchData : SearchArray)
			{
			...
			const bool bCanCheckoutResaveAll = GetDefault<UBlueprintEditorSettings>()->AllowIndexAllBlueprints == (EFiBIndexAllPermission)ERigVMFiBIndexAllPermission::CheckoutAndResave;
			...
			if (ReturnValue != EAppReturnType::Cancel)
			{
				...
				CacheParams.OpFlags = ERigVMFiBCacheOpFlags::ShowProgress | ERigVMFiBCacheOpFlags::AllowUserCancel | ERigVMFiBCacheOpFlags::AllowUserCloseProgress;
				if (bCheckOutAndSave)
				{
					CacheParams.OpFlags |= ERigVMFiBCacheOpFlags::CheckOutAndSave;
				}
				CacheParams.OnFinished = InOptions.OnFinished;
				...
				if (!bIsSourceControlEnabled && bCheckOutAndSave)
				{
					...
					ISourceControlModule::Get().ShowLoginDialog(FSourceControlLoginClosed::CreateRaw(this, &FRigVMFindInBlueprintSearchManager::OnCacheAllUnindexedAssets, bCheckOutAndSave), ELoginWindowMode::Modeless, EOnLoginWindowStartup::PreserveProvider);
				}
				else
				{
...
void FRigVMFindInBlueprintSearchManager::ExportOutdatedAssetList()
{
	// Construct path for output text file
	const FString FileLocation = FPaths::ConvertRelativePathToFull(FPaths::ProjectSavedDir());
	const FString FullPath = FString::Printf(TEXT("%s/RigVMFindInBlueprints_OutdatedAssetList.txt"), *FileLocation);
	...
	if (FArchive* Ar = IFileManager::Get().CreateFileWriter(*FullPath))
	{
		...
		for (FRigVMSearchData SearchData : SearchArray)
		{
			if ((SearchData.Value.Len() != 0 || SearchData.ImaginaryBlueprint.IsValid()) && SearchData.VersionInfo.FiBDataVersion < ERigVMFiBVersion::RIGVM_FIB_VER_LATEST)
			{
		...
		UE_LOG(LogRigVMFindInBlueprint, Log, TEXT("%s"), *ExportConfirmationText.ToString());
	...
	else
	{
		...
		UE_LOG(LogRigVMFindInBlueprint, Log, TEXT("%s"), *ExportFailureText.ToString());
		FMessageDialog::Open(EAppMsgType::Ok, ExportFailureText);
	}

}

void FRigVMFindInBlueprintSearchManager::CancelCacheAll(SRigVMFindInBlueprints* InFindInBlueprintWidget)
{
...
int32 FRigVMFindInBlueprintSearchManager::GetCurrentCacheIndex() const
{
...
FSoftObjectPath FRigVMFindInBlueprintSearchManager::GetCurrentCacheBlueprintPath() const
{
...
float FRigVMFindInBlueprintSearchManager::GetCacheProgress() const
{
...
int32 FRigVMFindInBlueprintSearchManager::GetNumberUnindexedAssets() const
{
	return UnindexedAssets.Num();
}

int32 FRigVMFindInBlueprintSearchManager::GetNumberUncachedAssets() const
{
	if (CachingObject.IsValid())
	{
...
void FRigVMFindInBlueprintSearchManager::StartedCachingBlueprints(ERigVMFiBCacheOpType InCacheOpType, ERigVMFiBCacheOpFlags InCacheOpFlags)
{
...
void FRigVMFindInBlueprintSearchManager::FinishedCachingBlueprints(ERigVMFiBCacheOpType InCacheOpType, ERigVMFiBCacheOpFlags InCacheOpFlags, int32 InNumberCached, TSet<FSoftObjectPath>& InFailedToCacheList)
{
	// Update the list of cache failures
	FailedToCachePaths = InFailedToCacheList;

	bool bContinueCachingPendingAssets = false;
	if (InCacheOpType == ERigVMFiBCacheOpType::CachePendingAssets)
	{
		// In the discovery stage, check to see if discovery has ended and we have no additional assets to process. 
		const bool bIsCachingDiscoveredAssets = EnumHasAnyFlags(InCacheOpFlags, ERigVMFiBCacheOpFlags::IsCachingDiscoveredAssets);
		...
		else if (!IsAssetDiscoveryInProgress())
		{
			...
			InCacheOpFlags &= ~(ERigVMFiBCacheOpFlags::HideProgressBars | ERigVMFiBCacheOpFlags::IsCachingDiscoveredAssets | ERigVMFiBCacheOpFlags::KeepProgressVisibleOnCompletion);
		}
	}
	...
	{
		FRigVMFindInBlueprintCachingOptions CacheOptions;
...
bool FRigVMFindInBlueprintSearchManager::IsCacheInProgress() const
{
	return CachingObject.IsValid();
}

bool FRigVMFindInBlueprintSearchManager::IsUnindexedCacheInProgress() const
{
	return IsCacheInProgress() && CachingObject->GetCurrentCacheOpType() == ERigVMFiBCacheOpType::CacheUnindexedAssets;
}

bool FRigVMFindInBlueprintSearchManager::IsAssetDiscoveryInProgress() const
{
	return GIsRunning && AssetRegistryModule && AssetRegistryModule->Get().IsLoadingAssets();
}

bool FRigVMFindInBlueprintSearchManager::IsAsyncSearchQueryInProgress() const
{
...
TSharedPtr< FJsonObject > FRigVMFindInBlueprintSearchManager::ConvertJsonStringToObject(FRigVMSearchDataVersionInfo InVersionInfo, const FString& InJsonString, TMap<int32, FText>& OutFTextLookupTable)
{
	...
	if (InVersionInfo.FiBDataVersion > ERigVMFiBVersion::RIGVM_FIB_VER_BASE)
	{
...
void FRigVMFindInBlueprintSearchManager::GlobalFindResultsClosed(const TSharedRef<SRigVMFindInBlueprints>& FindResults)
{
	CSV_EVENT(RigVMFindInBlueprint, TEXT("GlobalFindResultsClosed: %s"), *FindResults->GetHostTabId().ToString());
...
FText FRigVMFindInBlueprintSearchManager::GetGlobalFindResultsTabLabel(int32 TabIdx)
{
...
TSharedRef<SDockTab> FRigVMFindInBlueprintSearchManager::SpawnGlobalFindResultsTab(const FSpawnTabArgs& SpawnTabArgs, int32 TabIdx)
{
	CSV_EVENT(RigVMFindInBlueprint, TEXT("SpawnGlobalFindResultsTab: %d"), TabIdx);

	TAttribute<FText> Label = TAttribute<FText>::Create(TAttribute<FText>::FGetter::CreateRaw(this, &FRigVMFindInBlueprintSearchManager::GetGlobalFindResultsTabLabel, TabIdx));
	...
	TSharedRef<SRigVMFindInBlueprints> FindResults = SNew(SRigVMFindInBlueprints)
...
TSharedPtr<SRigVMFindInBlueprints> FRigVMFindInBlueprintSearchManager::OpenGlobalFindResultsTab()
{
	TSet<FName> OpenGlobalTabIDs;

	for (TWeakPtr<SRigVMFindInBlueprints> FindResultsPtr : GlobalFindResults)
	{
		TSharedPtr<SRigVMFindInBlueprints> FindResults = FindResultsPtr.Pin();
	...
	for (int32 Idx = 0; Idx < UE_ARRAY_COUNT(GlobalFindResultsTabIDs); ++Idx)
	{
		...
		if (!OpenGlobalTabIDs.Contains(GlobalTabId))
		{
			...
			if (NewTab.IsValid())
			{
				TSharedRef<SRigVMFindInBlueprints> NewFindTab = StaticCastSharedRef<SRigVMFindInBlueprints>(NewTab->GetContent());
				return NewFindTab;
			}
			else
			{
	...
	return TSharedPtr<SRigVMFindInBlueprints>();
}

TSharedPtr<SRigVMFindInBlueprints> FRigVMFindInBlueprintSearchManager::GetGlobalFindResults()
{
	TSharedPtr<SRigVMFindInBlueprints> FindResultsToUse;
...
void FRigVMFindInBlueprintSearchManager::EnableGlobalFindResults(bool bEnable)
{
	...
	if (bEnable)
	{
		...
		for (int32 TabIdx = 0; TabIdx < UE_ARRAY_COUNT(GlobalFindResultsTabIDs); TabIdx++)
		{
			...
			if (!GlobalTabManager->HasTabSpawner(TabID))
			{
				...
				GlobalTabManager->RegisterNomadTabSpawner(TabID, FOnSpawnTab::CreateRaw(this, &FRigVMFindInBlueprintSearchManager::SpawnGlobalFindResultsTab, TabIdx))
	...
	else
	{
		...
		TSet<TSharedPtr<SRigVMFindInBlueprints>> FindResultsToClose;
		...
		for (TSharedPtr<SRigVMFindInBlueprints> FindResults : FindResultsToClose)
		{
...
void FRigVMFindInBlueprintSearchManager::CloseOrphanedGlobalFindResultsTabs(TSharedPtr<class FTabManager> TabManager)
{
...
void FRigVMFindInBlueprintSearchManager::Tick(float DeltaTime)
{
	CSV_SCOPED_TIMING_STAT(RigVMFindInBlueprint, Tick);
	...
	else if (PendingAssets.Num() > 0)
	{
		...
		CachingOptions.OpType = ERigVMFiBCacheOpType::CachePendingAssets;
		CacheAllAssets(nullptr, CachingOptions);
	}
	else if (CVarRandomSearchIndexCacheRebuildEachTick.GetValueOnGameThread())
	{
		DoRandomIndexCacheRebuild();
	}
}

bool FRigVMFindInBlueprintSearchManager::IsTickable() const
{
	...
	return IsCacheInProgress() || bNeedsFirstIndex || (bHasPendingAssets && (IsGlobalFindResultsOpen() || IsAsyncSearchQueryInProgress())) || CVarRandomSearchIndexCacheRebuildEachTick.GetValueOnGameThread();
}

TStatId FRigVMFindInBlueprintSearchManager::GetStatId() const
{
	RETURN_QUICK_DECLARE_CYCLE_STAT(FRigVMFindInBlueprintSearchManager, STATGROUP_Tickables);
}
...
class FFiBDumpIndexCacheToFileExecHelper : public FSelfRegisteringExec
{
	virtual bool Exec_Editor(class UWorld* InWorld, const TCHAR* Cmd, FOutputDevice& Ar)
	{
		if (FParse::Command(&Cmd, TEXT("DUMPFIBINDEXCACHE")))
		{
			...
			if (DumpFile)
			{
				FRigVMFindInBlueprintSearchManager::Get().DumpCache(*DumpFile);

				DumpFile->Close();
				delete DumpFile;

				UE_LOG(LogRigVMFindInBlueprint, Log, TEXT("Wrote full index cache to %s"), *FullPath);
			}
			return true;
		}
		return false;
