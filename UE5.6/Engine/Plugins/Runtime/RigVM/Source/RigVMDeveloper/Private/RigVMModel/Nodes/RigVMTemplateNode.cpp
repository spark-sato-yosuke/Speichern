...
FName URigVMTemplateNode::GetDisplayNameForPin(const URigVMPin* InPin) const
{
#if WITH_EDITOR
	if(const FRigVMTemplate* Template = GetTemplate())
	{
		...
		if(InPin->IsRootPin())
		{
			if(InPin->IsExecuteContext())
			{
				...
				if(Template->NumExecuteArguments(DispatchContext) == 1)
				{
					return FRigVMStruct::ExecuteName;
				}
			}
		
			const FName RootPinName = InPin->GetFName();
