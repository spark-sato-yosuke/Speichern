...
{
	if(!CachedEventName.IsBound())
	{
		CachedEventName.Bind(this);
	}
	if(CachedEventName.IsValid())
	{
		return CachedEventName.GetValue();
	}
	...
	if (StructOnScope.IsValid())
	{
		...
		CachedEventName = StructMemory->GetEventName();
	}
	return CachedEventName.Get(Super::GetEventName());
}
...
FText URigVMUnitNode::GetToolTipTextForPin(const URigVMPin* InPin) const
{
	if(UScriptStruct* Struct = GetScriptStruct())
	{
		...
		for (int32 PartIndex = 1; PartIndex < Parts.Num(); PartIndex++)
		{
			...
			if (PartIndex == Parts.Num() - 1)
			{
				const FText ToolTipBody = Property->GetToolTipText();
				return GetTypedToolTipText(InPin, ToolTipBody);
			}
...
FName URigVMUnitNode::GetDisplayNameForPin(const URigVMPin* InPin) const
{
		...
		if(InPin->IsExecuteContext())
		{
			...
			for (TFieldIterator<FProperty> It(ScriptStruct); It; ++It)
			{
				if(const FStructProperty* StructProperty = CastField<FStructProperty>(*It))
				{
					...
					{
						NumExecuteContextPins++;
					}
				}
			}
			if(NumExecuteContextPins == 1)
			{
				return FRigVMStruct::ExecuteName;
			}
		}
		
		const FName StructMemberDisplayName = GetDisplayNameForStructMember(ScriptStruct, InPin->GetSegmentPath(true));
		if((StructMemberDisplayName == FRigVMStruct::ExecuteContextName) ||
			(InPin->GetFName() == FRigVMStruct::ExecuteContextName && StructMemberDisplayName.IsNone()))
		{
			return FRigVMStruct::ExecuteName;
		}
		return StructMemberDisplayName;
	}
