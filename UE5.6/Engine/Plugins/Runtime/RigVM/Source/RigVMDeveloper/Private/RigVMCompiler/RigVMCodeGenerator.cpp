...
FString FRigVMCodeGenerator::DumpInstructions(const FRigVMExtendedExecuteContext& Context, int32 InInstructionGroup, bool bLog)
{
	...
	if(Group.ChildGroups.IsEmpty())
	{
		...
		for(const FRigVMBranchInfo& BranchInfo : ByteCode.BranchInfos)
		{
			...
			if(BranchInfo.FirstInstruction < Group.First ||
				BranchInfo.LastInstruction > Group.Last)
			{
				continue;
			}

			InstructionIndices.RemoveAll([BranchInfo](int32 Index) {
				return FMath::IsWithinInclusive<int32>(Index, BranchInfo.FirstInstruction, BranchInfo.LastInstruction);
			});
			...
			Lines.Add(DumpInstructions(Context, TEXT("\t\t"), BranchInfo.FirstInstruction, BranchInfo.LastInstruction, Group, false));
		...
		switch(Instruction.OpCode)
		{
			...
			case ERigVMOpCode::JumpToBranch:
			{
				...
				for(int32 BranchIndex = Op.FirstBranchInfoIndex; BranchIndex < Branches.Num(); BranchIndex++)
				{
					...
					Lines.Add(Prefix + Format(RigVM_JumpToBranchFormat, *GetOperandName(Context, Op.Arg, false), Branches[BranchIndex].Label.ToString(), Branch.FirstInstruction));
				}
				break;
			}
...
void FRigVMCodeGenerator::ParseInclude(UStruct* InDependency, const FName& InMethodName)
{
	...
	if (InDependency == URigVM::StaticClass() ||
		...
		InDependency->IsChildOf(FRigVMExecutePin::StaticStruct()) ||
			...
			switch(Instruction.OpCode)
			{
				...
				case ERigVMOpCode::JumpToBranch:
				{
					...
					for(int32 BranchIndex = Op.FirstBranchInfoIndex; BranchIndex < Branches.Num(); BranchIndex++)
					{
						...
						{
							Group.RequiredLabels.AddUnique(Branches[BranchIndex].FirstInstruction);
						}
					}
					break;
				}
