...
struct FRigVMBranchInfo
{
	...
	{
		return RigVMStringUtils::JoinStrings(InArrayName, InElementName, TEXT("_"));
	}
...
public:
	...
	RIGVM_API ERigVMExecuteResult Execute(FRigVMExtendedExecuteContext& Context);
	RIGVM_API ERigVMExecuteResult ExecuteIfRequired(FRigVMExtendedExecuteContext& Context, uint32 InSliceHash);
...
public:
	...
	RIGVM_API const uint8* GetData(const FRigVMExecuteContext& Context) const;
...
public:
	...
	RIGVM_API FRigVMPropertyDescription(const FProperty* InProperty, const FString& InDefaultValue, const FName& InName = NAME_None);

	// Constructor from complete data
	RIGVM_API FRigVMPropertyDescription(const FName& InName, const FString& InCPPType, UObject* InCPPTypeObject, const FString& InDefaultValue);

	// Returns a sanitized, valid name to use for a new property 
	static RIGVM_API FName SanitizeName(const FName& InName);

	// Sanitizes a name using a string ref, creating a valid name to use for a new property 
	static RIGVM_API void SanitizeName(FString& InString);

	// Sanitize the name of this description in line
	RIGVM_API void SanitizeName();
...
public:
	...
	// UClass overrides
	RIGVM_API void PurgeClass(bool bRecompilingOnLoad) override;
	RIGVM_API void Link(FArchive& Ar, bool bRelinkExistingProperties) override;
	RIGVM_API void Serialize(FArchive& Ar) override;
	...
	static RIGVM_API const FString& GetClassName(ERigVMMemoryType InMemoryType);

	// Returns an existing class for a memory type within the package (or nullptr) 
	static RIGVM_API URigVMMemoryStorageGeneratorClass* GetStorageClass(UObject* InOuter, ERigVMMemoryType InMemoryType);
	 ...
	 */
	static RIGVM_API URigVMMemoryStorageGeneratorClass* CreateStorageClass(
		UObject* InOuter,
	 ...
	 * @param InMemoryType The memory type to use for this class (affects class name)
	 */
	static RIGVM_API bool RemoveStorageClass(
		UObject* InOuter,
		ERigVMMemoryType InMemoryType);
	...
	RIGVM_API uint32 GetMemoryHash() const;
	...
	RIGVM_API FRigVMMemoryStatistics GetStatistics() const;

protected:

	// Adds a single property to a class.
	static RIGVM_API FProperty* AddProperty(URigVMMemoryStorageGeneratorClass* InClass, const FRigVMPropertyDescription& InProperty, FField::FLinkedListBuilder& PropertyListBuilder);

public:

	RIGVM_API void RefreshLinkedProperties();
	RIGVM_API void RefreshPropertyPaths();
...
public:
	...
	RIGVM_API const TArray<const FProperty*>& GetProperties() const;

	// Returns the property paths provided by this instance
	RIGVM_API const TArray<FRigVMPropertyPath>& GetPropertyPaths() const;

	// Returns the index of a property given the property itself
	RIGVM_API int32 GetPropertyIndex(const FProperty* InProperty) const;

	// Returns the index of a property given its name
	RIGVM_API int32 GetPropertyIndexByName(const FName& InName) const;
	...
	RIGVM_API FProperty* FindPropertyByName(const FName& InName) const;

	// Creates and returns a new operand for a property (and optionally a property path)
	RIGVM_API FRigVMOperand GetOperand(int32 InPropertyIndex, int32 InPropertyPathIndex = INDEX_NONE) const;

	// Creates and returns a new operand for a property (and optionally a property path)
	RIGVM_API FRigVMOperand GetOperandByName(const FName& InName, int32 InPropertyPathIndex = INDEX_NONE) const;
	...
	RIGVM_API FString GetDataAsString(int32 InPropertyIndex, int32 PortFlags = PPF_None);
	...
	RIGVM_API FString GetDataAsString(const FRigVMOperand& InOperand, int32 PortFlags = PPF_None);

	// Returns the exported text for a given property index
	RIGVM_API FString GetDataAsStringSafe(int32 InPropertyIndex, int32 PortFlags = PPF_None);
	...
	// Returns the exported text for a given operand
	RIGVM_API FString GetDataAsStringSafe(const FRigVMOperand& InOperand, int32 PortFlags = PPF_None);

	// Sets the content of a property by index given an exported string. Returns true if succeeded
	RIGVM_API bool SetDataFromString(int32 InPropertyIndex, const FString& InValue);
	...
	RIGVM_API FRigVMMemoryHandle GetHandle(int32 InPropertyIndex, const FRigVMPropertyPath* InPropertyPath = nullptr);
	 ...
	 */ 
	static RIGVM_API bool CopyProperty(
	...
	RIGVM_API void CopyFrom(URigVMMemoryStorage* InSourceMemory);

private:
	
	static RIGVM_API const TArray<const FProperty*> EmptyProperties;
	static RIGVM_API const TArray<FRigVMPropertyPath> EmptyPropertyPaths;
};
