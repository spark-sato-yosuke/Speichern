...
public:
	...
	UE_API virtual ~URigVMNativized();

	// URigVM interface
	UE_API virtual void Serialize(FArchive& Ar) override;
	virtual void PostLoad() override { return; }
	UE_API virtual void Reset(bool IsIgnoringArchetypeRef = false) override;
...
#if WITH_EDITOR
	UE_API void SetByteCode(const FRigVMByteCode& InByteCode);
#endif
...
protected:
	...
	UE_API const FRigVMMemoryHandle& GetLazyMemoryHandle(int32 InIndex, uint8* InMemory, const FProperty* InProperty, TFunction<ERigVMExecuteResult()> InLambda);
	...
	UE_API void AllocateLazyMemoryHandles(int32 InCount);
