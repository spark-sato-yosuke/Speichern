...
FRigVMTemplateTypeMap FRigVMStructUpgradeInfo::GetTypeMapFromStruct(UScriptStruct* InScriptStruct)
{
	...
	for (TFieldIterator<FProperty> It(InScriptStruct); It; ++It)
	{
		...
		const TRigVMTypeIndex TypeIndex = FRigVMTemplateArgument::Make(*It).GetTypeIndex(0);
	...
	if (const FProperty* Property = InStruct->FindPropertyByName(PinName))
	{
		if (Property->GetCPPType() != ExpectedType)
		{
			if(ExpectedType == FRigVMExecuteContext::StaticStruct()->GetStructCPPName() ||
				ExpectedType == FRigVMExecutePin::StaticStruct()->GetStructCPPName())
			{
...
FString FRigVMStruct::ExportToFullyQualifiedText(const FProperty* InMemberProperty, const uint8* InMemberMemoryPtr, bool bUseQuotes)
{
	...
	else if (const FArrayProperty* ArrayProperty = CastField<FArrayProperty>(InMemberProperty))
	{
		...
		DefaultValue = RigVMStringUtils::JoinStrings(ElementValues, TEXT(","), TEXT("("), TEXT(")"));
	}
	else
	{
		// Note we pass PPF_Delimited here to correctly escape strings
		InMemberProperty->ExportTextItem_Direct(DefaultValue, InMemberMemoryPtr, nullptr, nullptr, PPF_Delimited);

		if (CastField<FNameProperty>(InMemberProperty) != nullptr ||
			CastField<FStrProperty>(InMemberProperty) != nullptr ||
			CastField<FTextProperty>(InMemberProperty) != nullptr)
		{
			if(bUseQuotes)
			{
				...
				else if (DefaultValue[0] != TCHAR('\"') && DefaultValue[DefaultValue.Len() - 1] != TCHAR('\"'))
				{
...
FString FRigVMStruct::ExportToFullyQualifiedText(const UScriptStruct* InStruct, const uint8* InStructMemoryPtr, bool bUseQuotes)
{
	...
	// If a struct has a custom text import/export, we must use its ExportText call rather than going per-property
	if((InStruct->StructFlags & (STRUCT_ImportTextItemNative | STRUCT_ExportTextItemNative)) != 0)
	{
		FInstancedStruct Defaults;
		Defaults.InitializeAs(InStruct);
		FString Value;
		InStruct->ExportText(Value, InStructMemoryPtr, Defaults.GetMemory(), nullptr, PPF_None, nullptr);
		return Value;
	}
	else
	{
		...
		return RigVMStringUtils::JoinStrings(FieldValues, TEXT(","), TEXT("("), TEXT(")"));
	}
}
...
TMap<FName, FString> FRigVMStruct::GetDefaultValues(UScriptStruct* InScriptStruct) const
{
	...
	// If a struct has a custom text import/export, we cannot extract its property values individually
	if((InScriptStruct->StructFlags & (STRUCT_ImportTextItemNative | STRUCT_ExportTextItemNative)) == 0)
	{
		for (TFieldIterator<FProperty> It(InScriptStruct); It; ++It)
		{
			if(It->HasAnyPropertyFlags(CPF_Transient))
