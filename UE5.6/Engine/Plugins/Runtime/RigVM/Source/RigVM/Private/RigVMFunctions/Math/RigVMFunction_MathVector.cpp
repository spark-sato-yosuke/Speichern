...
{
	Result.X = FMath::Min<double>(A.X, B.X);
	Result.Y = FMath::Min<double>(A.Y, B.Y);
	Result.Z = FMath::Min<double>(A.Z, B.Z);
}

FRigVMFunction_MathVectorMax_Execute()
{
	Result.X = FMath::Max<double>(A.X, B.X);
	Result.Y = FMath::Max<double>(A.Y, B.Y);
	Result.Z = FMath::Max<double>(A.Z, B.Z);
}
...
{
	Result.X = FMath::Clamp<double>(Value.X, Minimum.X, Maximum.X);
	Result.Y = FMath::Clamp<double>(Value.Y, Minimum.Y, Maximum.Y);
	Result.Z = FMath::Clamp<double>(Value.Z, Minimum.Z, Maximum.Z);
}
...
{
	FVector Ratio = FVector::ZeroVector;
	...
	if (bClamp)
	{
		Ratio.X = FMath::Clamp<double>(Ratio.X, 0, 1);
		Ratio.Y = FMath::Clamp<double>(Ratio.Y, 0, 1);
		Ratio.Z = FMath::Clamp<double>(Ratio.Z, 0, 1);
	}
...
{
	Result = static_cast<float>(Value.SizeSquared());
}

FRigVMFunction_MathVectorLength_Execute()
{
	Result = static_cast<float>(Value.Size());
}

FRigVMFunction_MathVectorDistance_Execute()
{
	Result = static_cast<float>(FVector::Distance(A, B));
}
...
FRigVMFunction_MathVectorAngle_Execute()
{
	...
	Result = static_cast<float>(FRigVMMathLibrary::FindQuatBetweenVectors(A, B).GetAngle());
}
...
void FRigVMFunction_DrawPlane(FRigVMDrawInterface* DrawInterface, EAxis::Type Axis, float Minimum, float& CurrentMaximum, FTransform& CurrentSpace, FVector Result)
{
	...
	const double PlaneScale = FMath::Max<double>(10., FVector::Distance(CurrentSpace.GetTranslation(), PointOnPlane)) * 0.525;
...
FRigVMFunction_MathIntersectPlane_Execute()
{
	...
	Distance = static_cast<float>((Start - Result).Size());
}
...
FRigVMFunction_MathDistanceToPlane_Execute()
{
	...
	{ 
		...
		SignedDistance = static_cast<float>(FVector::PointPlaneDist(Point, PlanePoint, UnitVectorNormal));
	}
	else
	{
