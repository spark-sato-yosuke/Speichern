...
namespace MobileFSRPrivate
{
	bool IsPostProcessingWithAlphaChannelSupported()
	{
		...
		return CVarPropagateAlpha ? CVarPropagateAlpha->GetBool() : false;
	}
}
...
class FMobileFSR_UsePassthroughAlpha : SHADER_PERMUTATION_BOOL("USE_PASSTHROUGH_ALPHA");
...
public:
	...
	using FPermutationDomain = TShaderPermutationDomain<FMobileFSR_UseFP16Dim, FMobileFSR_UsePassthroughAlpha>;
...
{
	using namespace MobileFSRPrivate;
	...
	if (!bOutputSupportsUAV || GMobileFSRDisableCompute)
	{
		...
		PSPermutationVector.Set<FMobileFSR_UsePassthroughAlpha>(IsPostProcessingWithAlphaChannelSupported());
	...
	else
	{
		...
		CSPermutationVector.Set<FMobileFSR_UsePassthroughAlpha>(IsPostProcessingWithAlphaChannelSupported());
...
public:
	...
	using FPermutationDomain = TShaderPermutationDomain<FMobileFSR_UseFP16Dim, FMobileFSR_UsePassthroughAlpha, FMobileRCAS_DenoiseDim>;
