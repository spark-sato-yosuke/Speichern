...
void UMassLODDistanceCollectorProcessor::ConfigureQueries(const TSharedRef<FMassEntityManager>& EntityManager)
{
...
{
	{
		TRACE_CPUPROFILER_EVENT_SCOPE(LODDistanceCollector_Close);
		{
			...
			EntityQuery_RelevantRangeAndOnLOD.ForEachEntityChunk(Context, [this](FMassExecutionContext& Context) { CollectLODForChunk<bLocalViewersOnly>(Context); });
		}
		{
			TRACE_CPUPROFILER_EVENT_SCOPE(LODDistanceCollector_Close_OnLODOnly);
			EntityQuery_RelevantRangeOnly.ForEachEntityChunk(Context, [this](FMassExecutionContext& Context) { CollectLODForChunk<bLocalViewersOnly>(Context); });
		}
		{
			TRACE_CPUPROFILER_EVENT_SCOPE(LODDistanceCollector_Close_OnLODOnly);
...

	{
		...
		EntityQuery_NotRelevantRangeAndOffLOD.ForEachEntityChunk(Context, [this](FMassExecutionContext& Context) { CollectLODForChunk<bLocalViewersOnly>(Context); });
	}
}
