...
void UMassActorSubsystem::Initialize(FSubsystemCollectionBase& Collection)
{
	...
	OverrideSubsystemTraits<UMassActorSubsystem>(Collection);
}
...
FMassEntityHandle FMassActorManager::GetEntityHandleFromActor(const TObjectKey<const AActor> Actor)
{
	...
	checkCode
	(
		const TObjectKey<const AActor> ActorKey(GetActorFromHandle(*Entity, FMassActorFragment::EActorAccess::IncludeUnreachable));
		checkf(ActorKey == Actor || ActorKey == TObjectKey<const AActor>()
			, TEXT("Actor associated to the Entity handle should either match or not be set (FMassActorFragment/UMassAgentComponent not used)"));
	);
