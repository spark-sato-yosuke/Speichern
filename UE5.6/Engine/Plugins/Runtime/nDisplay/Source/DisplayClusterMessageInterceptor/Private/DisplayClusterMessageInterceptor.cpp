...
void FDisplayClusterMessageInterceptor::HandleClusterNodeFailure(const FString& NodeId)
{
	TArray<TSharedPtr<IMessageContext, ESPMode::ThreadSafe>> ContextToForward;

	{
		...
		const uint32 NodesInCluster = ClusterManager->GetNodesAmount();

		// Will contain the message IDs that we can forward to process and forget about
		TSet<FString> ContextsToClear;

		// Remove this node ID from all the message contexts
		for (TPair<FString, FContextSync>& MessageContext : ContextMap)
		{
			MessageContext.Value.NodesReceived.Remove(NodeId);

			// If this node ID was the only one missing, then forward this message to process
			const uint32 NodesReceivedNum = static_cast<uint32>(MessageContext.Value.NodesReceived.Num());
			if (NodesReceivedNum >= NodesInCluster)
			{
				UE_LOG(LogDisplayClusterInterception, VeryVerbose, TEXT("Fowarding message for message id %s on frame %d"), *MessageContext.Key, GFrameCounter);
				ContextToForward.Add(MessageContext.Value.ContextPtr);
				ContextsToClear.Add(MessageContext.Key);
			}
		}

		// Now remove the contexts we don't need anymore
		for (const FString& MessageId : ContextsToClear)
		{
			ContextMap.Remove(MessageId);
		}
	}

	// Forward all selected messages to process
	if (InterceptedBus)
	{
		for (const auto& Context : ContextToForward)
		{
