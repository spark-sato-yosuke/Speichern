...
bool FDisplayClusterProjectionMPCDIPolicy::ShouldSupportICVFX(const IDisplayClusterViewport* InViewport) const
{
	check(IsInGameThread());
	check(InViewport);
...
void FDisplayClusterProjectionMPCDIPolicy::UpdateProxyData(IDisplayClusterViewport* InViewport)
{
	...
	ENQUEUE_RENDER_COMMAND(DisplayClusterProjectionMPCDIPolicy_UpdateProxyData)(
		[
			...
			WarpBlendInterfacePtr = WarpBlendInterface,
			WarPolicyInterfacePtr = WarpPolicyInterface,
			Contexts = WarpBlendContexts
		](FRHICommandListImmediate& RHICmdList)
	{
		This->WarpBlendInterface_Proxy = WarpBlendInterfacePtr;
		This->WarpPolicyInterface_Proxy = WarPolicyInterfacePtr;
		This->WarpBlendContexts_Proxy = Contexts;
	});
}
...
bool FDisplayClusterProjectionMPCDIPolicy::IsWarpBlendSupported(const IDisplayClusterViewport* InViewport) const
{
	return true;
}

bool FDisplayClusterProjectionMPCDIPolicy::IsWarpBlendSupported_RenderThread(const IDisplayClusterViewportProxy* InViewportProxy) const
{
	return true;
}
...
void FDisplayClusterProjectionMPCDIPolicy::ApplyWarpBlend_RenderThread(FRHICommandListImmediate& RHICmdList, const IDisplayClusterViewportProxy* InViewportProxy)
{
	...
	if (EnumHasAllFlags(SettingsICVFX.RuntimeFlags, EDisplayClusterViewportRuntimeICVFXFlags::Target))
	{
		...
		for (int32 ContextNum = 0; ContextNum < InputTextures.Num(); ContextNum++)
		{
			...
			// Convert Lightcards gamma from linear to the Outer viewport gamma
			FDisplayClusterColorEncoding InputEncoding = InViewportProxy->GetResourceColorEncoding_RenderThread(EDisplayClusterViewportResourceType::InputShaderResource);
			if (InputEncoding == EDisplayClusterColorEncoding::Linear)
			{
				ShaderICVFX.LightCardGamma = 1.0f;
			}
			else
			{
				ShaderICVFX.LightCardGamma = 2.2f;
			}
