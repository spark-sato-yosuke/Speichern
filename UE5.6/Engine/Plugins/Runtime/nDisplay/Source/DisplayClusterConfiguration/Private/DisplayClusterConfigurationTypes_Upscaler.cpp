	...
	if (const UEnum* EnumClass = StaticEnum<EDisplayClusterConfigurationUpscalingMethod>())
	{
		for (int32 EnumElementIndex = 0; EnumElementIndex < EnumClass->NumEnums(); ++EnumElementIndex)
		{
				...
				)
			{
				...
				// Migrate any data from the default upscaler settings that isn't explicitly overridden in the parameter overrides
				if (InDefaultUpscalerSettings)
				{
					OutUpscalerSettings.CustomUpscalerSettings.MigrateToNewBagInstanceWithOverrides(InDefaultUpscalerSettings->EditingData, ParameterOverrideGuids);
				}
