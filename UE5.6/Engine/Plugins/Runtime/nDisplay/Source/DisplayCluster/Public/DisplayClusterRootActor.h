...
public:
	...
	*/
	...
	/**
	* Blueprint setter for the PreviewEnableHoldoutComposite property. Makes sure preview pipeline is updated properly.
	*
	* @param bNewbPreviewEnableHoldoutComposite - Desired new property value.
	*/
	UFUNCTION(BlueprintSetter)
	void SetPreviewEnableHoldoutComposite(const bool bNewPreviewEnableHoldoutComposite);
...
private:
	UPROPERTY(EditAnywhere, Transient, Category = Viewports, meta = (PropertyPath = "CurrentConfigData.StageSettings.OuterViewportUpscalerSettings", DisplayName = "Default Upscaler Settings"))
	FDisplayClusterEditorPropertyReference OuterViewportUpscalerSettingsRef;

	UPROPERTY(EditInstanceOnly, Transient, Category = Viewports, meta = (DisplayName = "Per-Viewport Upscaling Setting Overrides", PropertyPath = "CurrentConfigData.Cluster.Nodes.Viewports.RenderSettings.UpscalerSettings", ToolTip = "Adjust upscaler settings for an individual viewport."))
	FDisplayClusterEditorPropertyReference ViewportUpscalerSettingsRef;
	...
	UPROPERTY(EditInstanceOnly, Transient, Category = Viewports, meta = (DisplayName = "Content Hidden Per-Viewport", PropertyPath = "CurrentConfigData.Cluster.Nodes.Viewports.RenderSettings.HiddenContent", ToolTip = "Hide actors from a specific viewport."))
	FDisplayClusterEditorPropertyReference PerOuterHideListRef;
	
	UPROPERTY(EditAnywhere, Transient, Category = "In Camera VFX", meta = (PropertyPath = "CurrentConfigData.StageSettings.GlobalInnerFrustumUpscalerSettings", DisplayName = "Default Upscaler Settings"))
	FDisplayClusterEditorPropertyReference GlobalInnerFrustumUpscalerSettingsRef;
...
public:
	...
	/**
	* Render a preview with postprocessing in the scene using the HoldoutComposite plugin.
	* 
	* The `Preview`>`Enable Post Process` option must be set
	* 
	* These project settings also need to be set:
	* 1. "Alpha Output"
	* 2. "Support Primitive Alpha Holdout (Deferred)"
	*/
	UPROPERTY(EditAnywhere, BlueprintReadWrite, Category = "Preview", meta = (DisplayName = "Enable Preview Holdout", EditCondition = "bPreviewEnablePostProcess"), BlueprintSetter = SetPreviewEnableHoldoutComposite)
	bool bPreviewEnableHoldoutComposite = true;

	/** Enable TemporalAA/TSR in preview.
	* Increases memory consumption. This will impact editor performance. 
	*/
	UPROPERTY(EditAnywhere, BlueprintReadWrite, Category = "Preview", meta = (DisplayName = "Allow TemporalAA/TSR in Preview"))
	bool bPreviewEnableTSR = false;

	/** Enable OCIO on preview. */
	UPROPERTY(EditAnywhere, BlueprintReadWrite, Category = "Preview", meta = (DisplayName = "Enable OCIO on Preview"))
	bool bPreviewEnableOCIO = false;
