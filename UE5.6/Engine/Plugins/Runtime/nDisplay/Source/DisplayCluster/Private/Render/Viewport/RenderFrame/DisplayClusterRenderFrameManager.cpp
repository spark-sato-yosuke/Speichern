...
bool FDisplayClusterRenderFrameManager::BuildSimpleFrame(FViewport* InViewport, const TArray<TSharedPtr<FDisplayClusterViewport, ESPMode::ThreadSafe>>& InViewports, FDisplayClusterRenderFrame& OutRenderFrame)
{
	for (const TSharedPtr<FDisplayClusterViewport, ESPMode::ThreadSafe>& ViewportIt : InViewports)
	{
		...
		{
			// Exclude all ViewExtensions. They are added later, in FDisplayClusterViewportManager::PostConfigureViewFamily()
			ViewportIt->GetRenderSettingsImpl().UpscalerSettings.bIsActive = false;
			...
			for (const FDisplayClusterViewport_Context& ContextIt : ViewportIt->GetContexts())
			{
				...
				FDisplayClusterRenderFrameTargetViewFamily FrameViewFamily;
				{
					...
					FrameViewFamily.UpscalerSettings = ViewportIt->GetRenderSettings().UpscalerSettings;

					// We need to determine if we can use upscalers in the future when AntiAliasing methods are configured in SceneViews.
					FrameViewFamily.ViewExtensions = ViewportIt->GatherActiveExtensions(ContextIt.ContextNum, InViewport);
				}

				FDisplayClusterRenderFrameTarget FrameRenderTarget;
				{
					FrameRenderTarget.Viewport = InViewport;
			...
			// After the GatherActiveExtensions() enable upscalers ViewExtensions back.
			ViewportIt->GetRenderSettingsImpl().UpscalerSettings.bIsActive = true;
		}
	}
