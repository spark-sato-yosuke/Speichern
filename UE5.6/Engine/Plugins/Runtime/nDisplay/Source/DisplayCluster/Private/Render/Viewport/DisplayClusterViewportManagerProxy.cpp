...
void FDisplayClusterViewportManagerProxy::ImplRenderFrame_GameThread(FViewport* InViewport)
{
		...
		[InViewportManagerProxy = SharedThis(this), OutputViewport = InViewport](FRHICommandListImmediate& RHICmdList)
	{
		...
		// Another capture+playback round for media passthrough
		IDisplayCluster::Get().GetCallbacks().OnDisplayClusterPassthroughMediaCapture_RenderThread().Broadcast(RHICmdList, ViewportManagerProxy);
		IDisplayCluster::Get().GetCallbacks().OnDisplayClusterPassthroughMediaInput_RenderThread().Broadcast(RHICmdList, ViewportManagerProxy);
...
bool FDisplayClusterViewportManagerProxy::ImplResolveFrameTargetToBackBuffer_RenderThread(FRDGBuilder& GraphBuilder, const uint32 InContextNum, const int32 DestArrayIndex, FRDGTextureRef DestTexture, FVector2f WindowSize) const
{
	check(IsInRenderingThread());
	...
	// Output texture must supports quadbuffer stereo.
	if (DestArrayIndex > 0)
	{
		// Check if the backbuffer texture has a second slice for stereo.
		const bool bStereoTexture = DestTexture->Desc.Dimension == ETextureDimension::Texture2DArray && DestTexture->Desc.ArraySize > 1;
		if (!bStereoTexture)
		{
			return false;
		}
	...
	if (GetFrameTargets_RenderThread(FrameResources, TargetOffsets) && FrameResources.IsValidIndex(InContextNum))
	{
		...
		if (FRHITexture* FrameTexture = FrameResources[InContextNum])
		{
			...
			if (!FDisplayClusterViewportHelpers::GetValidResourceRectsForResolve(FrameTexture->GetDesc().Extent, DestTexture->Desc.Extent, SrcRect, DestRect))
			{
			...
			FRDGTextureRef SrcTexture = RegisterExternalTexture(GraphBuilder, FrameTexture->GetTexture2D(), TEXT("DisplayClusterStereoTexture"));

			AddCopyTexturePass(GraphBuilder, SrcTexture, DestTexture, CopyInfo);
...
bool FDisplayClusterViewportManagerProxy::ResolveFrameTargetToBackBuffer_RenderThread(FRHICommandListImmediate& RHICmdList, const uint32 InContextNum, const int32 DestArrayIndex, FRHITexture* DestTexture, FVector2D WindowSize) const
{
	...
	{
		// Check if the backbuffer texture has a second slice for stereo.
		const bool bStereoTexture = DestTexture->GetDesc().Dimension == ETextureDimension::Texture2DArray && DestTexture->GetDesc().ArraySize > 1;
		if (!bStereoTexture)
		{
			return false;
		}
	...
	if (GetFrameTargets_RenderThread(FrameResources, TargetOffsets) && FrameResources.IsValidIndex(InContextNum))
	{
		...
		if (FRHITexture* FrameTexture = FrameResources[InContextNum])
		{
			...
			// Check if resources with the specified regions can be resolved.
			if (!FDisplayClusterViewportHelpers::GetValidResourceRectsForResolve(FrameTexture->GetDesc().Extent, DestTexture->GetDesc().Extent, SrcRect, DestRect))
			{
				// The SrcRect or DestRect is invalid.
				return false;
			}
