...
public:
	...
	* [RDG version]
	* 
	* @param GraphBuilder   - Render Graph interface
	* @param InContextNum   - renderframe source context num
	* @param DestArrayIndex - dest array index on backbuffer
	* @param DestTexture    - dest RDG texture
	* @param WindowSize     - dest backbuffer window size
	*
	* @return - true if success
	*/
	bool ImplResolveFrameTargetToBackBuffer_RenderThread(FRDGBuilder& GraphBuilder, const uint32 InContextNum, const int32 DestArrayIndex, FRDGTextureRef DestTexture, FVector2f WindowSize) const;
