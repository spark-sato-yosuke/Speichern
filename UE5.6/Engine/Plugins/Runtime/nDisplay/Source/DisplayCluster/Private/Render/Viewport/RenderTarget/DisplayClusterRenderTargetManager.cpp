...
{
	/** Return custom flags for capture mode. */
	static EDisplayClusterViewportResourceSettingsFlags ImplGetCustomFlags(EDisplayClusterViewportCaptureMode CaptureMode)
	{
		switch (CaptureMode)
		{
		...
		case EDisplayClusterViewportCaptureMode::Lightcard:
			...
			// LightCards renders in linear color space
			return EDisplayClusterViewportResourceSettingsFlags::LinearColorSpace;

		default:
			break;
		}

		return EDisplayClusterViewportResourceSettingsFlags::None;
	}

	/** Return texture format for capture mode. */
...
bool FDisplayClusterRenderTargetManager::AllocateRenderFrameResources(FViewport* InViewport, const TArray<TSharedPtr<FDisplayClusterViewport, ESPMode::ThreadSafe>>& InViewports, FDisplayClusterRenderFrame& InOutRenderFrame)
{
	...
	if(ResourcesPool->BeginReallocateResources(InViewport, Configuration->GetRenderFrameSettings()))
	{
		...
		for (FDisplayClusterRenderFrameTarget& FrameRenderTargetIt : InOutRenderFrame.RenderTargets)
		{
			...
			{
				const EDisplayClusterViewportResourceSettingsFlags CustomFlags = ImplGetCustomFlags(FrameRenderTargetIt.CaptureMode);
				const EPixelFormat CustomFormat = ImplGetCustomFormat(FrameRenderTargetIt.CaptureMode);

				// reallocate
				TSharedPtr<FDisplayClusterViewportResource, ESPMode::ThreadSafe> NewResource = ResourcesPool->AllocateResource(EmptyString, FrameRenderTargetIt.RenderTargetSize, CustomFormat, EDisplayClusterViewportResourceSettingsFlags::RenderTarget | CustomFlags);
		...
		for (const TSharedPtr<FDisplayClusterViewport, ESPMode::ThreadSafe>& ViewportIt : InViewports)
		{
			...
			{
				const EDisplayClusterViewportCaptureMode CaptureMode = ViewportIt->GetRenderSettings().CaptureMode;

				const EDisplayClusterViewportResourceSettingsFlags CustomFlags = ImplGetCustomFlags(CaptureMode);
				const EPixelFormat CustomFormat = ImplGetCustomFormat(CaptureMode);
				...
				for (const FDisplayClusterViewport_Context& ContextIt : ViewportIt->GetContexts())
				{
					...
					{
						TSharedPtr<FDisplayClusterViewportResource, ESPMode::ThreadSafe>& ReallocResource = ViewportIt->GetViewportResourcesImpl(EDisplayClusterViewportResource::OutputPreviewTargetableResources)[ContextNum];
						if (!ReallocResource.IsValid() || !EnumHasAnyFlags(ReallocResource->GetResourceState(), EDisplayClusterViewportResourceState::DisableReallocate))
						{
							ReallocResource = ResourcesPool->AllocateResource(ViewportIt->GetId(), ContextSize, CustomFormat, EDisplayClusterViewportResourceSettingsFlags::PreviewTargetableTexture | CustomFlags);
					...
					{
						TSharedPtr<FDisplayClusterViewportResource, ESPMode::ThreadSafe>& ReallocResource = ViewportIt->GetViewportResourcesImpl(EDisplayClusterViewportResource::InputShaderResources)[ContextNum];
						if (!ReallocResource.IsValid() || !EnumHasAnyFlags(ReallocResource->GetResourceState(), EDisplayClusterViewportResourceState::DisableReallocate))
						{
							ReallocResource = ResourcesPool->AllocateResource(EmptyString, ContextSize, CustomFormat, EDisplayClusterViewportResourceSettingsFlags::ResolveTargetableTexture | CustomFlags);
					...
					{
						TSharedPtr<FDisplayClusterViewportResource, ESPMode::ThreadSafe>& ReallocResource = ViewportIt->GetViewportResourcesImpl(EDisplayClusterViewportResource::AdditionalTargetableResources)[ContextNum];
						if (!ReallocResource.IsValid() || !EnumHasAnyFlags(ReallocResource->GetResourceState(), EDisplayClusterViewportResourceState::DisableReallocate))
						{
							ReallocResource = ResourcesPool->AllocateResource(EmptyString, ContextSize, CustomFormat, EDisplayClusterViewportResourceSettingsFlags::RenderTargetableTexture | CustomFlags);
					...
					{
						TSharedPtr<FDisplayClusterViewportResource, ESPMode::ThreadSafe>& ReallocResource = ViewportIt->GetViewportResourcesImpl(EDisplayClusterViewportResource::MipsShaderResources)[ContextNum];
						if (!ReallocResource.IsValid() || !EnumHasAnyFlags(ReallocResource->GetResourceState(), EDisplayClusterViewportResourceState::DisableReallocate))
						{
							ReallocResource = ResourcesPool->AllocateResource(EmptyString, ContextSize, CustomFormat, EDisplayClusterViewportResourceSettingsFlags::ResolveTargetableTexture | CustomFlags, ContextIt.NumMips);
