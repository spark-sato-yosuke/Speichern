...
void IDisplayClusterViewportManager::SetupSceneView(const EDisplayClusterViewportCaptureMode InMode, FSceneViewFamily& InOutViewFamily, FSceneView& InOutView)
{
	if (InMode == EDisplayClusterViewportCaptureMode::Lightcard)
	{
		// The UVLC texture is shared and can be used for any kind of viewport (with or without OCIO).
		// Because of UVLC texture sharing, always render any kind of LightCard viewports as for OCIO.
		// This will result in the same rendering result, and as a result the LC and UVLC colors will match.
...
void FDisplayClusterViewportManager::Release()
{
	check(IsInGameThread());

	// Calls OnSceneEnd() for all subsystems. They will release their own resources.
	HandleEndScene();

	// Frees GPU memory as fast as possible.
	ReleaseTextures();

	// This is a flag to check in the destructor that Release() has been called before.
	bReleaseFunctionCalledBeforeDestructor = true;
}

FDisplayClusterViewportManager::~FDisplayClusterViewportManager()
{
	if (!bReleaseFunctionCalledBeforeDestructor)
	{
		// This is the wrong way to call the destructor.
		// Lets show a message in the log
		UE_LOG(LogDisplayClusterViewport, Error,
			TEXT("~FDisplayClusterViewportManager() is called at an inappropriate time. The Release() function must be called before this."));
	}
...
bool FDisplayClusterViewportManager::ShouldUseOutputFrameTargetableResources() const
{
	check(IsInGameThread());

	for (const TSharedPtr<FDisplayClusterViewport, ESPMode::ThreadSafe>& Viewport : ImplGetCurrentRenderFrameViewports())
	{
		if (Viewport.IsValid() && Viewport->IsResourceUsed(EDisplayClusterViewportResource::OutputFrameTargetableResources))
		{
			return true;
		}
	}
...
bool FDisplayClusterViewportManager::ShouldUseAdditionalFrameTargetableResource() const
{
	...
	for (const TSharedPtr<FDisplayClusterViewport, ESPMode::ThreadSafe>& Viewport : ImplGetCurrentRenderFrameViewports())
	{
		if (Viewport.IsValid() && Viewport->IsResourceUsed(EDisplayClusterViewportResource::AdditionalFrameTargetableResources))
		{
			return true;
		}
	}
	...
	const bool bInAdditionalViewFamily) const
{
	...
	case EDisplayClusterViewportCaptureMode::Lightcard:
		...
		// Disable postprocess for LightCard & CK
		InEngineShowFlags.PostProcessing = 0;
	...
	FSceneViewFamilyContext& ViewFamily)
{
	...
	if (!ShouldRenderFinalColor() || InFrameTarget.CaptureMode == EDisplayClusterViewportCaptureMode::Lightcard)
	{
...
{
	// Configure upscaler
	const FName CustomUpscalerName = FDisplayClusterUpscaler::PostConfigureViewFamily(InFrameViewFamily.UpscalerSettings, InFrameViewFamily.CustomBufferRatio, InDPIScale, ViewFamily, Views);

	// Add ViewExtensions from the Custom Upscaler to this view family
	if (!CustomUpscalerName.IsNone() && InFrameViewFamily.UpscalerSettings.CustomUpscalerName == CustomUpscalerName)
	{
		// Iterate over all viewports, and collect all used ViewExtensions
		for (const FDisplayClusterRenderFrameTargetView& FrameTargetViewIt : InFrameViewFamily.Views)
		{
			if (FrameTargetViewIt.Viewport.IsValid())
			{
				TArray<FSceneViewExtensionRef> ViewExtensions = FrameTargetViewIt.Viewport->ToSharedRef()->GatherActiveExtensions(FrameTargetViewIt.ContextNum, InFrameTarget.Viewport);

				// Lookup for the new ViewExtensions, and register it for that ViewFamily
				for (FSceneViewExtensionRef& ViewExtensionIt : ViewExtensions)
				{
					if (!ViewFamily.ViewExtensions.Contains(ViewExtensionIt))
					{
						ViewFamily.ViewExtensions.Add(ViewExtensionIt);
						ViewExtensionIt->SetupViewFamily(ViewFamily);
					}
				}
			}
		}
...
void FDisplayClusterViewportManager::SetCurrentStereoViewIndexForViewPointExtension(const int32 InStereoViewIndex) const
{
	if (ViewportManagerViewPointExtension.IsValid())
	{
...
FSceneView* FDisplayClusterViewportManager::CalcSceneView(ULocalPlayer* LocalPlayer, FSceneViewFamily* ViewFamily, FVector& OutViewLocation, FRotator& OutViewRotation, FViewport* InViewport, FViewElementDrawer* InViewDrawer, int32 InStereoViewIndex)
{
	...
	SetCurrentStereoViewIndexForViewPointExtension(InStereoViewIndex);
	...
	SetCurrentStereoViewIndexForViewPointExtension(INDEX_NONE);
