...
struct FActorRepList : FNoncopyable
{
	/** List Header */
	int32 RefCount;
	int32 Max;
...
struct FActorRepListRefView
{
	...
	bool RemoveFast(const FActorRepListType& ElementToRemove, EAllowShrinking AllowShrink = EAllowShrinking::Default)
	{
	...
	UE_API bool VerifyContents_Slow() const;
	...
	UE_API FString BuildDebugString() const;
...
PRAGMA_DISABLE_DEPRECATION_WARNINGS
	...
	UE_API FORCEINLINE const TArray<FActorRepListConstView>& GetLists(EActorRepListTypeFlags ListFlags) const;
...
{
	UE_API void AddActor(const FNewReplicatedActorInfo& ActorInfo);
	...
	*/
	UE_API bool RemoveActor(const FNewReplicatedActorInfo& ActorInfo, bool bWarnIfNotFound, UObject* Outer=nullptr);
	...
	* @param Outer Optional pointer to the owner of the list if you want to output the owner name in any log messages
	*/
	UE_API bool RemoveActorFast(const FNewReplicatedActorInfo& ActorInfo, UObject* Outer=nullptr);
	...
	*/
	UE_API bool RemoveActorFromLevelFast(AActor* Actor, FName LevelName);

	UE_API void Reset();

	/** Add to the Params' gather list the list of actors for levels that are visible to that connection */
	UE_API void Gather(const FConnectionGatherActorListParameters& Params) const;

	/** Add to the passed gather list the list of actors that are visible to the connection */
	UE_API void Gather(const UNetReplicationGraphConnection& ConnectionManager, FGatheredReplicationActorLists& OutGatheredList) const;

	/** Add to the passed gather list all list of actors held */
	UE_API void AppendAllLists(FGatheredReplicationActorLists& OutGatheredList) const;

	/** Is this actor held in one of the actor lists */
	UE_API bool Contains(const FNewReplicatedActorInfo& ActorInfo) const;

	UE_API void DeepCopyFrom(const FStreamingLevelActorListCollection& Source);
	UE_API void GetAll_Debug(TArray<FActorRepListType>& OutArray) const;
	UE_API void Log(FReplicationGraphDebugInfo& DebugInfo) const;
...
{
	UE_API void AddNetworkActor(AActor* NetActor);
	UE_API bool RemoveNetworkActor(const FNewReplicatedActorInfo& NetActor);
	...
	UE_API bool RemoveNetworkActorOrdered(AActor* NetActor);

	UE_API void Reset();

	UE_API bool Contains(AActor* NetActor) const;
	...
	UE_API void GetAllActors(TArray<AActor*>& OutAllActors) const;

	UE_API void CountBytes(FArchive& Ar) const;

	/** Update actor to new level */
	UE_API void UpdateActorLevel(AActor* NetActor, FName PreviousLevelName);
...
public:
	...
	UE_API void VisitRepList(const UReplicationGraphNode* NodeToVisit, const FActorRepListRefView& RepList);

	/** Collect stats on a collection of FActorRepLists */
	UE_API void VisitStreamingLevelCollection(const UReplicationGraphNode* NodeToVisit, const struct FStreamingLevelActorListCollection& StreamingLevelList);

	/** Collect stats for FActorRepLists not held by a node */
	UE_API void VisitExplicitStreamingLevelList(FName ListOwnerName, FName StreamLevelName, const FActorRepListRefView& RepList);

	/** Prevents the node from being visited twice in case it's shared multiple times */
	UE_API void FlagNodeVisited(const UReplicationGraphNode* NodeToVisit);

	/** Print the statistics previously collected */
	UE_API void PrintCollectedData(FOutputDevice& Ar);

private:

	UE_API bool WasNodeVisited(const UReplicationGraphNode* NodeToVisit);
