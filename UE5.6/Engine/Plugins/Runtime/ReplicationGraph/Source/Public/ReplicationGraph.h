...
#define UE_API REPLICATIONGRAPH_API
...
public:
	...
	UE_API virtual void NotifyAddNetworkActor(const FNewReplicatedActorInfo& Actor ) PURE_VIRTUAL(UReplicationGraphNode::NotifyAddNetworkActor, );
	
	/** Called when a networked actor is being destroyed or no longer wants to replicate */
	UE_API virtual bool NotifyRemoveNetworkActor(const FNewReplicatedActorInfo& Actor, bool bWarnIfNotFound=true) PURE_VIRTUAL(UReplicationGraphNode::NotifyRemoveNetworkActor, return false; );
	 ...
	 */
	UE_API virtual bool NotifyActorRenamed(const FRenamedReplicatedActorInfo& Actor, bool bWarnIfNotFound=true) PURE_VIRTUAL(UReplicationGraphNode::NotifyActorRenamed, return false; );
	...
	UE_API virtual void NotifyResetAllNetworkActors();
	
	/** Mark the node and all its children PendingKill */
	UE_API virtual void TearDown();
	
	UE_API virtual void GatherActorListsForConnection(const FConnectionGatherActorListParameters& Params) PURE_VIRTUAL(UReplicationGraphNode::GatherActorListsForConnection, );
	...
	UE_API virtual void GetAllActorsInNode_Debugging(TArray<FActorRepListType>& OutArray) const;

	/** Debugging only function to verify referenced actors (if any) are still valid. */
	UE_API void VerifyActorReferences();
	...
	UE_API virtual void LogNode(FReplicationGraphDebugInfo& DebugInfo, const FString& NodeName) const;

	virtual FString GetDebugString() const { return GetName(); }

	UE_API void DoCollectActorRepListStats(struct FActorRepListStatCollector& StatsCollector) const;
	...
	UE_API bool RemoveChildNode(UReplicationGraphNode* OutChildNode, UReplicationGraphNode::NodeOrdering NodeOrder=UReplicationGraphNode::NodeOrdering::IgnoreOrdering);

	/** Remove all null and about to be destroyed nodes from our list */
	UE_API void CleanChildNodes(UReplicationGraphNode::NodeOrdering NodeOrder);
...
protected:
	...
	UE_API bool VerifyActorReference(const FActorRepListType& Actor) const;
...
public:
	...
	UE_API virtual void NotifyAddNetworkActor(const FNewReplicatedActorInfo& ActorInfo) override;
	
	UE_API virtual bool NotifyRemoveNetworkActor(const FNewReplicatedActorInfo& ActorInfo, bool bWarnIfNotFound=true) override;
	
	UE_API virtual bool NotifyActorRenamed(const FRenamedReplicatedActorInfo& ActorInfo, bool bWarnIfNotFound=true) override;

	UE_API virtual void NotifyResetAllNetworkActors() override;
	
	UE_API virtual void GatherActorListsForConnection(const FConnectionGatherActorListParameters& Params) override;

	UE_API virtual void LogNode(FReplicationGraphDebugInfo& DebugInfo, const FString& NodeName) const override;

	UE_API virtual void GetAllActorsInNode_Debugging(TArray<FActorRepListType>& OutArray) const override;

	UE_API virtual void VerifyActorReferencesInternal() override;

	UE_API virtual void TearDown() override;

	/** Removes the actor very quickly but breaks the list order */
	UE_API bool RemoveNetworkActorFast(const FNewReplicatedActorInfo& ActorInfo);
	...
	UE_API void DeepCopyActorListsFrom(const UReplicationGraphNode_ActorList* Source);
...
protected:
	...
	UE_API void LogActorList(FReplicationGraphDebugInfo& DebugInfo) const;

	UE_API virtual void OnCollectActorRepListStats(struct FActorRepListStatCollector& StatsCollector) const override;
...
public:
	...
	static UE_API FSettings DefaultSettings;
	...
	UE_API void SetNonStreamingCollectionSize(const int32 NewSize);

protected:

	UE_API void CheckRebalance();
...
public:
	...
	UE_API UReplicationGraphNode_DynamicSpatialFrequency();
	...
	static UE_API FSettings DefaultSettings;	// Default settings used by all instance
...
protected:
	...
	UE_API virtual void GatherActors(const FActorRepListRefView& RepList, FGlobalActorReplicationInfoMap& GlobalMap, FPerConnectionActorInfoMap& ConnectionMap, const FConnectionGatherActorListParameters& Params, UNetConnection* NetConnection);
	UE_API virtual void GatherActors_DistanceOnly(const FActorRepListRefView& RepList, FGlobalActorReplicationInfoMap& GlobalMap, FPerConnectionActorInfoMap& ConnectionMap, const FConnectionGatherActorListParameters& Params);
	...
	UE_API void CalcFrequencyForActor(AActor* Actor, UReplicationGraph* RepGraph, UNetConnection* NetConnection, FGlobalActorReplicationInfo& GlobalInfo, FPerConnectionActorInfoMap& ConnectionMap, FSettings& MySettings, const FNetViewerArray& Viewers, const uint32 FrameNum, int32 ExistingItemIndex);

	UE_API void CalcFrequencyForActor(AActor* Actor, UReplicationGraph* RepGraph, const UNetReplicationGraphConnection& RepGraphConnection, FGlobalActorReplicationInfo& GlobalInfo, FPerConnectionActorInfoMap& ConnectionMap, FSettings& MySettings, const FNetViewerArray& Viewers, const uint32 FrameNum, int32 ExistingItemIndex);
};
...
public:
	...
	*/
	static UE_API void SetNumFramesUntilObsolete(uint32 NumFrames);
...
public:
	...
	UE_API virtual void GatherActorListsForConnection(const FConnectionGatherActorListParameters& Params) override;
	UE_API virtual bool NotifyRemoveNetworkActor(const FNewReplicatedActorInfo& ActorInfo, bool WarnIfNotFound) override;
	UE_API virtual bool NotifyActorRenamed(const FRenamedReplicatedActorInfo& ActorInfo, bool bWarnIfNotFound=true) override;
	UE_API virtual void NotifyResetAllNetworkActors() override;
	UE_API virtual void VerifyActorReferencesInternal() override;

	UE_API void NotifyActorDormancyFlush(FActorRepListType Actor);

	UE_API void OnClientVisibleLevelNameAdd(FName LevelName, UWorld* World);

	UE_API bool IsNodeObsolete(uint32 CurrentFrame) const;

private:

	static UE_API uint32 NumFramesUntilObsolete;
...
private:
	...
	UE_API void ConditionalGatherDormantActorsForConnection(FActorRepListRefView& ConnectionRepList, const FConnectionGatherActorListParameters& Params, FActorRepListRefView* RemovedList);
...
public:
	...
	static UE_API FVector::FReal MaxZForConnection; 
...
public:
	...
	UE_API void AddDormantActor(const FNewReplicatedActorInfo& ActorInfo, FGlobalActorReplicationInfo& GlobalInfo);
	UE_API void RemoveDormantActor(const FNewReplicatedActorInfo& ActorInfo, FGlobalActorReplicationInfo& ActorRepInfo);
	UE_API void RenameDormantActor(const FRenamedReplicatedActorInfo& ActorInfo);

	UE_API virtual void GatherActorListsForConnection(const FConnectionGatherActorListParameters& Params) override;

	UE_API void OnActorDormancyFlush(FActorRepListType Actor, FGlobalActorReplicationInfo& GlobalInfo);

	UE_API void ConditionalGatherDormantDynamicActors(FActorRepListRefView& RepList, const FConnectionGatherActorListParameters& Params, FActorRepListRefView* RemovedList, bool bEnforceReplistUniqueness = false, FActorRepListRefView* RemoveFromList = nullptr);

	UE_API UReplicationGraphNode_ConnectionDormancyNode* GetExistingConnectionNode(const FConnectionGatherActorListParameters& Params);

	UE_API UReplicationGraphNode_ConnectionDormancyNode* GetConnectionNode(const FConnectionGatherActorListParameters& Params);

private:

	UE_API UReplicationGraphNode_ConnectionDormancyNode* CreateConnectionNode(const FConnectionGatherActorListParameters& Params);
	 ...
	 */
	UE_API void CallFunctionOnValidConnectionNodes(FConnectionDormancyNodeFunction Function);
...
public:
	...
	UE_API void AddStaticActor(const FNewReplicatedActorInfo& ActorInfo, FGlobalActorReplicationInfo& GlobalRepInfo, bool bParentNodeHandlesDormancyChange);
	UE_API void AddDynamicActor(const FNewReplicatedActorInfo& ActorInfo);

	UE_API void RemoveStaticActor(const FNewReplicatedActorInfo& ActorInfo, FGlobalActorReplicationInfo& ActorRepInfo, bool bWasAddedAsDormantActor);
	UE_API void RemoveDynamicActor(const FNewReplicatedActorInfo& ActorInfo);

	UE_API void RenameStaticActor(const FRenamedReplicatedActorInfo& ActorInfo, bool bWasAddedAsDormantActor);
	UE_API void RenameDynamicActor(const FRenamedReplicatedActorInfo& ActorInfo);
	...
	UE_API UReplicationGraphNode_DormancyNode* GetDormancyNode(bool bInCreateIfMissing = true);
...
private:
	...
	UE_API UReplicationGraphNode* GetDynamicNode();

	UE_API void OnActorDormancyFlush(FActorRepListType Actor, FGlobalActorReplicationInfo& GlobalInfo, UReplicationGraphNode_DormancyNode* DormancyNode );

	UE_API void ConditionalCopyDormantActors(FActorRepListRefView& FromList, UReplicationGraphNode_DormancyNode* ToNode);	
	UE_API void OnStaticActorNetDormancyChange(FActorRepListType Actor, FGlobalActorReplicationInfo& GlobalInfo, ENetDormancy NewVlue, ENetDormancy OldValue);
};
...
public:
	...
	UE_API UReplicationGraphNode_GridSpatialization2D();

	UE_API virtual void Serialize(FArchive& Ar) override;

	UE_API virtual void NotifyAddNetworkActor(const FNewReplicatedActorInfo& Actor) override;	
	UE_API virtual bool NotifyRemoveNetworkActor(const FNewReplicatedActorInfo& ActorInfo, bool bWarnIfNotFound=true) override;
	UE_API virtual bool NotifyActorRenamed(const FRenamedReplicatedActorInfo& Actor, bool bWarnIfNotFound=true) override;
	UE_API virtual void NotifyResetAllNetworkActors() override;
	UE_API virtual void PrepareForReplication() override;
	UE_API virtual void GatherActorListsForConnection(const FConnectionGatherActorListParameters& Params) override;
	UE_API virtual void LogNode(FReplicationGraphDebugInfo& DebugInfo, const FString& NodeName) const override;
	UE_API virtual void VerifyActorReferencesInternal() override;
	...
	UE_API void AddActor_Dormancy(const FNewReplicatedActorInfo& ActorInfo, FGlobalActorReplicationInfo& ActorRepInfo);

	UE_API void RemoveActor_Static(const FNewReplicatedActorInfo& ActorInfo);
	void RemoveActor_Dynamic(const FNewReplicatedActorInfo& ActorInfo) { RemoveActorInternal_Dynamic(ActorInfo); }
	UE_API void RemoveActor_Dormancy(const FNewReplicatedActorInfo& ActorInfo);

	UE_API void RenameActor_Static(const FRenamedReplicatedActorInfo& ActorInfo);
	UE_API void RenameActor_Dynamic(const FRenamedReplicatedActorInfo& ActorInfo);
	UE_API void RenameActor_Dormancy(const FRenamedReplicatedActorInfo& ActorInfo);
	...
	UE_API virtual void NotifyActorCullDistChange(AActor* Actor, FGlobalActorReplicationInfo& GlobalInfo, float OldDist);
	...
	*/
	UE_API void SetBiasAndGridBounds(const FBox& GridBox);
...
protected:
	...
	UE_API virtual void AddActorInternal_Dynamic(const FNewReplicatedActorInfo& ActorInfo);
	UE_API virtual void AddActorInternal_Static(const FNewReplicatedActorInfo& ActorInfo, FGlobalActorReplicationInfo& ActorRepInfo, bool IsDormancyDriven); // Can differ the actual call to implementation if actor is not ready
	UE_API virtual void AddActorInternal_Static_Implementation(const FNewReplicatedActorInfo& ActorInfo, FGlobalActorReplicationInfo& ActorRepInfo, bool IsDormancyDriven);

	UE_API virtual void RemoveActorInternal_Dynamic(const FNewReplicatedActorInfo& Actor);
	UE_API virtual void RemoveActorInternal_Static(const FNewReplicatedActorInfo& Actor, FGlobalActorReplicationInfo& ActorRepInfo, bool WasAddedAsDormantActor);

private:

	UE_API bool WillActorLocationGrowSpatialBounds(const FVector& Location) const;

	/** Called when an actor is out of spatial bounds */
	UE_API void HandleActorOutOfSpatialBounds(AActor* Actor, const FVector& Location3D, const bool bStaticActor);
	...
	UE_API void OnNetDormancyChange(FActorRepListType Actor, FGlobalActorReplicationInfo& GlobalInfo, ENetDormancy NewVlue, ENetDormancy OldValue);
	
	UE_API void PutStaticActorIntoCell(const FNewReplicatedActorInfo& ActorInfo, FGlobalActorReplicationInfo& ActorRepInfo, bool bDormancyDriven);
	...
	UE_API void GetGridNodesForActor(FActorRepListType Actor, const FGlobalActorReplicationInfo& ActorRepInfo, TArray<UReplicationGraphNode_GridCell*>& OutNodes);
	UE_API void GetGridNodesForActor(FActorRepListType Actor, const UReplicationGraphNode_GridSpatialization2D::FActorCellInfo& CellInfo, TArray<UReplicationGraphNode_GridCell*>& OutNodes);

	UE_API FActorCellInfo GetCellInfoForActor(FActorRepListType Actor, const FVector& Location3D, float CullDistance);
...
public:
	...
	UE_API UReplicationGraphNode_AlwaysRelevant();
	...
	UE_API void AddAlwaysRelevantClass(UClass* Class);
...
protected:
	...
	UE_API void AddCachedRelevantActor(const FConnectionGatherActorListParameters& Params, AActor* NewActor, TWeakObjectPtr<AActor>& LastActor);
	UE_API void UpdateCachedRelevantActor(const FConnectionGatherActorListParameters& Params, AActor* NewActor, TWeakObjectPtr<AActor>& LastActor);
...
public:
	...
	UE_API void NotifyTearOffActor(AActor* Actor, uint32 FrameNum);
...
public:
	...
	UE_API virtual void SetRepDriverWorld(UWorld* InWorld) override;
	UE_API virtual void InitForNetDriver(UNetDriver* InNetDriver) override;
	UE_API virtual void InitializeActorsInWorld(UWorld* InWorld) override;
	virtual void ResetGameWorldState() override { }
	UE_API virtual void TearDown() override;

	/** Called by the NetDriver when the client connection is ready/added to the NetDriver's client connection list */
	UE_API virtual void AddClientConnection(UNetConnection* NetConnection) override;
	UE_API virtual void RemoveClientConnection(UNetConnection* NetConnection) override;
	UE_API virtual void AddNetworkActor(AActor* Actor) override;
	UE_API virtual void RemoveNetworkActor(AActor* Actor) override;
	UE_API virtual void ForceNetUpdate(AActor* Actor) override;
	UE_API virtual void FlushNetDormancy(AActor* Actor, bool bWasDormInitial) override;
	UE_API virtual void NotifyActorTearOff(AActor* Actor) override;
	UE_API virtual void NotifyActorFullyDormantForConnection(AActor* Actor, UNetConnection* Connection) override;
	UE_API virtual void NotifyActorDormancyChange(AActor* Actor, ENetDormancy OldDormancyState) override;
	virtual void NotifyDestructionInfoCreated(AActor* Actor, FActorDestructionInfo& DestructionInfo) override {}
	UE_API virtual void NotifyActorRenamed(AActor* Actor, UObject* PreviousOuter, FName PreviousName) override;
	UE_API virtual void SetRoleSwapOnReplicate(AActor* Actor, bool bSwapRoles) override;
	UE_API virtual bool ProcessRemoteFunction(class AActor* Actor, UFunction* Function, void* Parameters, FOutParmRec* OutParms, FFrame* Stack, class UObject* SubObject) override;
	UE_API virtual int32 ServerReplicateActors(float DeltaSeconds) override;
	UE_API virtual void PostTickDispatch() override;
	//~ End UReplicationDriver Interface

	UE_API virtual void RouteAddNetworkActorToNodes(const FNewReplicatedActorInfo& ActorInfo, FGlobalActorReplicationInfo& GlobalInfo);

	UE_API virtual void RouteRemoveNetworkActorToNodes(const FNewReplicatedActorInfo& ActorInfo);

	UE_API bool IsConnectionReady(UNetConnection* Connection);

	UE_API void SetActorDiscoveryBudget(int32 ActorDiscoveryBudgetInKBytesPerSec);

	/** Sets the global and connection-specific cull distance setting of this actor */
	UE_API void SetAllCullDistanceSettingsForActor(const FActorRepListType& Actor, float CullDistanceSquared);
	...
	UE_API void AddGlobalGraphNode(UReplicationGraphNode* GraphNode);
	UE_API void RemoveGlobalGraphNode(UReplicationGraphNode* GraphNode);
	
	/** Associate a node to a specific connection. When this connection replicates, this GraphNode will get a chance to add. */
	UE_API void AddConnectionGraphNode(UReplicationGraphNode* GraphNode, UNetReplicationGraphConnection* ConnectionManager);
	...
	UE_API void RemoveConnectionGraphNode(UReplicationGraphNode* GraphNode, UNetReplicationGraphConnection* ConnectionManager);
	...
	UE_API virtual void LogGraph(FReplicationGraphDebugInfo& DebugInfo) const;
	UE_API virtual void LogGlobalGraphNodes(FReplicationGraphDebugInfo& DebugInfo) const;
	UE_API virtual void LogConnectionGraphNodes(FReplicationGraphDebugInfo& DebugInfo) const;

	UE_API virtual void CollectRepListStats(struct FActorRepListStatCollector& StatCollector) const;
	...
	UE_API virtual class AReplicationGraphDebugActor* CreateDebugActor() const;
#endif
	...
	UE_API void DebugPrintCullDistances(UNetReplicationGraphConnection* SpecificConnection=nullptr) const;
	...
	static UE_API TFunction<void(int32)> OnListRequestExceedsPooledSize;

	// --------------------------------------------------------------

	UE_API virtual void ReplicateActorsForConnection(UNetConnection* NetConnection, FPerConnectionActorInfoMap& ConnectionActorInfoMap, UNetReplicationGraphConnection* ConnectionManager, const uint32 FrameNum);

	UE_API int64 ReplicateSingleActor(const FActorRepListType& Actor, FConnectionReplicationActorInfo& ActorInfo, FGlobalActorReplicationInfo& GlobalActorInfo, FPerConnectionActorInfoMap& ConnectionActorInfoMap, UNetReplicationGraphConnection& ConnectionManager, const uint32 FrameNum);
	UE_API int64 ReplicateSingleActor_FastShared(AActor* Actor, FConnectionReplicationActorInfo& ConnectionData, FGlobalActorReplicationInfo& GlobalActorInfo, UNetReplicationGraphConnection& ConnectionManager, const uint32 FrameNum);

	UE_API void UpdateActorChannelCloseFrameNum(AActor* Actor, FConnectionReplicationActorInfo& ConnectionData, const FGlobalActorReplicationInfo& GlobalData, const uint32 FrameNum, UNetConnection* NetConnection) const;

	UE_API void NotifyConnectionSaturated(class UNetReplicationGraphConnection& Connection);

	UE_API void NotifyConnectionFastPathSaturated();

	UE_API void SetActorDestructionInfoToIgnoreDistanceCulling(AActor* DestroyedActor);
	...
	UE_API UNetReplicationGraphConnection* FindConnectionManager(UNetConnection* NetConnection) const;
...
protected:
	...
	UE_API virtual void InitNode(UReplicationGraphNode* Node);

	/** Override this function to initialize the per-class data for replication */
	UE_API virtual void InitGlobalActorClassSettings();

	/** Override this function to init/configure your project's Global Graph */
	UE_API virtual void InitGlobalGraphNodes();
	...
	UE_API virtual void InitConnectionGraphNodes(UNetReplicationGraphConnection* ConnectionManager);

	UE_API UNetReplicationGraphConnection* FindOrAddConnectionManager(UNetConnection* NetConnection);
	...
	UE_API void HandleStarvedActorList(const UNetReplicationGraphConnection& RepGraphConnection, const FPrioritizedRepList& List, int32 StartIdx, FPerConnectionActorInfoMap& ConnectionActorInfoMap, uint32 FrameNum);

	/** Routes a rename/outer change to every global node. Subclasses will want a more direct routing function where possible. */
	UE_API virtual void RouteRenameNetworkActorToNodes(const FRenamedReplicatedActorInfo& ActorInfo);
...
protected:
	...
	UE_API void ReplicateActorListsForConnections_Default(UNetReplicationGraphConnection* ConnectionManager, FGatheredReplicationActorLists& GatheredReplicationListsForConnection, FNetViewerArray& Viewers);

	/** "FastShared" Replication Path */
	UE_API void ReplicateActorListsForConnections_FastShared(UNetReplicationGraphConnection* ConnectionManager, FGatheredReplicationActorLists& GatheredReplicationListsForConnection, FNetViewerArray& Viewers);
	...
	UE_API virtual void PostServerReplicateStats(const FFrameReplicationStats& Stats);

private:

	UE_API UNetReplicationGraphConnection* FixGraphConnectionList(TArray<UNetReplicationGraphConnection*>& OutList, int32& ConnectionId, UNetConnection* RemovedNetConnection);
...
private:
	...
	UE_API UNetReplicationGraphConnection* CreateClientConnectionManagerInternal(UNetConnection* Connection);
...
public:
	...
	UE_API UNetReplicationGraphConnection();
	...
	UE_API virtual void NotifyClientVisibleLevelNamesAdd(FName LevelName, UWorld* StreamingWorld) override;
	...
	UE_API virtual void NotifyActorChannelAdded(AActor* Actor, class UActorChannel* Channel) override;

	UE_API virtual void NotifyActorChannelRemoved(AActor* Actor) override;

	UE_API virtual void NotifyActorChannelCleanedUp(UActorChannel* Channel) override;

	UE_API virtual void NotifyAddDestructionInfo(FActorDestructionInfo* DestructInfo) override;

	UE_API virtual void NotifyRemoveDestructionInfo(FActorDestructionInfo* DestructInfo) override;

	UE_API virtual void NotifyAddDormantDestructionInfo(AActor* Actor) override;

	UE_API virtual void NotifyResetDestructionInfo() override;
	...
	UE_API virtual void GetClientVisibleLevelNames(TSet<FName>& OutLevelNames) const;
	...
	UE_API FActorRepListRefView& GetPrevDormantActorListForNode(const UReplicationGraphNode* GridNode);

	UE_API void RemoveActorFromAllPrevDormantActorLists(AActor* InActor);
	...
	UE_API TArray<FVisibleCellInfo>& GetVisibleCellsForNode(const UReplicationGraphNode* GridNode);

	UE_API void CleanupNodeCaches(const UReplicationGraphNode* Node);

	virtual void NotifyDSFNodeSaturated(const UReplicationGraphNode_DynamicSpatialFrequency* Node) {}

	UE_API virtual void TrackReplicationForAnalytics(bool bFastPathSaturated);

	UE_API virtual void ResetSaturationAnalytics();
...
private:
	...
	UE_API void InitForGraph(UReplicationGraph* Graph);

	/** Called after InitForGraph is called to associate this connection manager with a net connection */
	UE_API void InitForConnection(UNetConnection* Connection);

	/** Adds a node to this connection manager */
	UE_API void AddConnectionGraphNode(UReplicationGraphNode* Node);

	/** Remove a node to this connection manager */
	UE_API void RemoveConnectionGraphNode(UReplicationGraphNode* Node);

	UE_API bool PrepareForReplication();

	UE_API int64 ReplicateDestructionInfos(const FRepGraphDestructionViewerInfoArray& DestructionViewersInfo, const FReplicationGraphDestructionSettings& DestructionSettings);
	
	UE_API int64 ReplicateDormantDestructionInfos();

	/** Update the last location for viewers on a connection */
	UE_API void UpdateGatherLocationsForConnection(const FNetViewerArray& ConnectionViewers, const FReplicationGraphDestructionSettings& DestructionSettings);

    /** Update the location info of a specific viewer */
	UE_API void OnUpdateViewerLocation(FLastLocationGatherInfo* LocationInfo, const FNetViewer& Viewer, const FReplicationGraphDestructionSettings& DestructionSettings);

	UE_API void SetActorNotDormantOnConnection(AActor* InActor);

	UE_API void BuildVisibleLevels();
...
public:
	...
	UE_API void ServerStartDebugging();

	UFUNCTION(Server, Reliable)
	UE_API void ServerStopDebugging();

	UFUNCTION(Server, Reliable)
	UE_API void ServerCellInfo();

	UFUNCTION(Server, Reliable)
	UE_API void ServerPrintAllActorInfo(const FString& Str);

	UFUNCTION(Server, Reliable)
	UE_API void ServerSetCullDistanceForClass(UClass* Class, float CullDistance);

	UFUNCTION(Server, Reliable)
	UE_API void ServerSetPeriodFrameForClass(UClass* Class, int32 PeriodFrame);

	UFUNCTION(Server, Reliable)
	UE_API void ServerSetConditionalActorBreakpoint(AActor* Actor);

	UFUNCTION(Server, Reliable)
	UE_API void ServerPrintCullDistances();

	UFUNCTION(Client, Reliable)
	UE_API void ClientCellInfo(FVector CellLocation, FVector CellExtent, const TArray<AActor*>& Actors);

	UE_API void PrintCullDistances();

	UE_API void PrintAllActorInfo(FString MatchString);
