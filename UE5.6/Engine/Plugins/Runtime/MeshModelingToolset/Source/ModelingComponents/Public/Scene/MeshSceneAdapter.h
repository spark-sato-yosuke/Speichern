...
public:
	 ...
	 */
	UE_API void AddActors(const TArray<AActor*>& ActorsSetIn);
	 ...
	 */
	UE_API void AddComponents(const TArray<UActorComponent*>& ComponentSetIn);
	 ...
	 */
	UE_API void GenerateBaseClosingMesh(double BaseHeight = 1.0, double ExtrudeHeight = 0.0);

	/**
	 * Build the Mesh Scene. This must be called after the functions above, and before any queries below can be run.
	 */
	UE_API void Build(const FMeshSceneAdapterBuildOptions& BuildOptions);
	 ...
	 */
	UE_API virtual void BuildSpatialEvaluationCache();
	 ...
	 */
	UE_API void FastUpdateTransforms(bool bRebuildSpatialCache);
	...
	UE_API virtual FAxisAlignedBox3d GetBoundingBox();
	...
	/** @return bounding box for the Actor set */
	UE_API virtual void GetGeometryStatistics(FStatistics& StatsOut);

	/** @return a set of points on the surface of the meshes, can be used to initialize the MarchingCubes mesher */
	UE_API virtual void CollectMeshSeedPoints(TArray<FVector3d>& PointsOut);
	 ...
	 */
	UE_API virtual double FastWindingNumber(const FVector3d& WorldPoint, bool bFastEarlyOutIfPossible = true);
	 ...
	 */
	UE_API virtual bool FindNearestRayIntersection(const FRay3d& WorldRay, FMeshSceneRayHit& HitResultOut);

	/** Append all instance triangles to a single mesh. May be **very** large. */
	UE_API virtual void GetAccumulatedMesh(FDynamicMesh3& AccumMesh);

	/** Get all world-space bounding boxes for all Scene Meshes */
	UE_API virtual void GetMeshBoundingBoxes(TArray<FAxisAlignedBox3d>& Bounds);
	...
	UE_API virtual FAxisAlignedBox3d GetMeshBoundingBox(UActorComponent* Component, int32 ComponentIndex = -1);
	 ...
	 */
	UE_API virtual void ParallelMeshVertexEnumeration(
	 ...
	 */
	UE_API virtual void ProcessActorChildMeshes(TFunctionRef<void(const FActorAdapter* ActorAdapter, const FActorChildMesh* ChildMesh)> ProcessFunc);
...
protected:
	...
	UE_API void InitializeSpatialWrappers(const TArray<FActorAdapter*>& NewItemsToProcess);

	bool bSceneIsAllSolids = false;

	UE_API void UpdateActorBounds(FActorAdapter& Actor);

	UE_API void Build_FullDecompose(const FMeshSceneAdapterBuildOptions& BuildOptions);
