...
struct FRenderCaptureTypeFlags
{
	...
	static UE_API FRenderCaptureTypeFlags All(bool bCombinedMRS = false);

	/** @return FRenderCaptureTypeFlags with all types disabled/false */
	static UE_API FRenderCaptureTypeFlags None();

	/** @return FRenderCaptureTypeFlags with only BaseColor enabled/true */
	static UE_API FRenderCaptureTypeFlags BaseColor();

	/** @return FRenderCaptureTypeFlags with only WorldNormal enabled/true */
	static UE_API FRenderCaptureTypeFlags WorldNormal();

	/** @return FRenderCaptureTypeFlags with a single capture type enabled/true */
	static UE_API FRenderCaptureTypeFlags Single(ERenderCaptureType CaptureType);

	/** Set the indicated CaptureType to enabled */
	UE_API void SetEnabled(ERenderCaptureType CaptureType, bool bEnabled);

	/** @return mutable flag corresponding to the given CaptureType */
	UE_API bool& operator[](ERenderCaptureType CaptureType);

	/** @return constant flag corresponding to the given CaptureType */
	UE_API const bool& operator[](ERenderCaptureType CaptureType) const;

	/** @return true if the flags for this object match the Other object and false otherwise */
	UE_API bool operator==(const FRenderCaptureTypeFlags& Other) const;
	UE_API bool operator!=(const FRenderCaptureTypeFlags& Other) const;
};

template <typename T>
struct TRenderCaptureTypeData
{
...
UE_API PRAGMA_ENABLE_DEPRECATION_WARNINGS
	 ...
	 */
	UE_API void SetVisibleActors(const TArray<AActor*>& Actors);

	/**
	 *  Set the set of Components in the target World that should be included in the Rendering.
	 */
	UE_API void SetVisibleComponents(const TArray<UActorComponent*>& Components);

	/**
	 *  Set the set of Actors and Components in the target World that should be included in the Rendering.
	 */
	UE_API void SetVisibleActorsAndComponents(const TArray<AActor*>& VisibleActors, const TArray<UActorComponent*>& VisibleComponents);
	 ...
	 */
	UE_API FSphere ComputeContainingRenderSphere(float HorzFOVDegrees, float SafetyBoundsScale = 1.25f) const;

	/** Set desired pixel dimensions of the rendered target image */
	UE_API void SetDimensions(const FImageDimensions& Dimensions);
	 ...
	 */
	UE_API bool CaptureFromPosition(
	 ...
	 */
	UE_API void SetEnableWriteDebugImage(bool bEnable, int32 ImageCounter = -1, FString FolderName = FString());
...
protected:
	...
	UE_API UTextureRenderTarget2D* GetRenderTexture(bool bLinear);
	UE_API UTextureRenderTarget2D* GetDepthRenderTexture();
	...
	UE_API bool CaptureEmissiveFromPosition(
	...
	UE_API bool CaptureMRSFromPosition(
		const FFrame3d& Frame,
		double HorzFOVDegrees,
		double NearPlaneDist,
		FImageAdapter& ResultImageOut,
	...
	UE_API bool CaptureDeviceDepthFromPosition(
		const FFrame3d& Frame,
		double HorzFOVDegrees,
		double NearPlaneDist,
		FImageAdapter& ResultImageOut,
	 ...
	 */
	UE_API bool CaptureBufferVisualizationFromPosition(
	...
	UE_API void WriteDebugImage(const FImageAdapter& ResultImageOut, const FString& ImageTypeName);
};
