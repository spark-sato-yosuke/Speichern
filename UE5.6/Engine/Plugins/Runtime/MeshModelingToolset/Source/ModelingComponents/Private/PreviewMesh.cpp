...
void UPreviewMesh::SetOverrideWireframeRenderMaterial(UMaterialInterface* Material)
{
	check(DynamicMeshComponent);
	return DynamicMeshComponent->SetOverrideWireframeRenderMaterial(Material);
}

void UPreviewMesh::ClearOverrideWireframeRenderMaterial()
{
	check(DynamicMeshComponent);
	return DynamicMeshComponent->ClearOverrideWireframeRenderMaterial();
}

void UPreviewMesh::SetOverrideSecondaryWireframeRenderMaterial(UMaterialInterface* Material)
{
	check(DynamicMeshComponent);
	return DynamicMeshComponent->SetOverrideSecondaryWireframeRenderMaterial(Material);
}

void UPreviewMesh::ClearOverrideSecondaryWireframeRenderMaterial()
{
	check(DynamicMeshComponent);
	return DynamicMeshComponent->ClearOverrideSecondaryWireframeRenderMaterial();
}
...
bool UPreviewMesh::TestRayIntersection(const FRay3d& WorldRay, TFunction<bool(int32 Tid)> TriangleFilterF)
{
	if (IsVisible() && TemporaryParentActor != nullptr && bBuildSpatialDataStructure)
	{
		...
		QueryOptions.TriangleFilterF = TriangleFilterF;

		int HitTriID = MeshAABBTree.FindNearestHitTriangle(LocalRay, QueryOptions);
...
bool UPreviewMesh::FindRayIntersection(const FRay3d& WorldRay, FHitResult& HitOut, TFunction<bool(int32)>TriangleFilterF)
{
	if (IsVisible() && TemporaryParentActor != nullptr && bBuildSpatialDataStructure)
	{
