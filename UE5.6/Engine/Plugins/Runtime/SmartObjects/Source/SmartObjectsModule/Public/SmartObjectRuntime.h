...
public:
	...
	const FOnSmartObjectEvent& GetEventDelegate() const
	{
		return OnEvent;
	}
	 ...
	 * @param ReasonTag Valid Tag to specify the reason for changing the enabled state of the object. Method will ensure if not valid (i.e. None).
	 * @param bEnabled Flag indicating if the object should be enabled or not.
	 */
	 ...
	 */
	UE_API AActor* GetOwnerActor(ETrySpawnActorIfDehydrated TrySpawnActorIfDehydrated = ETrySpawnActorIfDehydrated::No) const;
	 ...
	 */
	UE_API USmartObjectComponent* GetOwnerComponent(ETrySpawnActorIfDehydrated TrySpawnActorIfDehydrated = ETrySpawnActorIfDehydrated::No) const;
...
private:
	...
	void SetRegisteredHandle(const FSmartObjectHandle Value)
	{
...
public:
	...
	const T* GetStateDataPtr() const
	{
		...
		for (FConstStructView Data : Slot->GetStateData())
		{
			if (const T* Item = Data.GetPtr<const T>())
			{
				return Item;
			}
		}
...
protected:
	...
	FConstSmartObjectSlotView(const FSmartObjectSlotHandle InSlotHandle, const FSmartObjectRuntime& InRuntime, const FSmartObjectRuntimeSlot& InSlot)
	...
	FSmartObjectSlotHandle SlotHandle;
	const FSmartObjectRuntime* Runtime;
...
struct FSmartObjectSlotView : public FConstSmartObjectSlotView
{
	...
	/**
	 * Returns a reference to the mutable slot state data of the specified type.
	 * Method will fail a check if the slot doesn't have the given type.
	 */
	template<typename T>
	...
	/**
	 * Returns a pointer to the mutable slot state data of the specified type.
	 * Method will return null if the slot doesn't have the given type.
	 */
	template<typename T>
