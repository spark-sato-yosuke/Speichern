...
struct FMapPlayerKeyArgs final
{
	...
	FMapPlayerKeyArgs();	
	~FMapPlayerKeyArgs() = default;
	FMapPlayerKeyArgs(const FMapPlayerKeyArgs&) = default;
	FMapPlayerKeyArgs(FMapPlayerKeyArgs&&) = default;
	FMapPlayerKeyArgs& operator=(const FMapPlayerKeyArgs&) = default;
	FMapPlayerKeyArgs& operator=(FMapPlayerKeyArgs&&) = default;
	...
	UE_DEPRECATED(5.6, "Use ProfileIdString instead.")	
	UPROPERTY(meta=(DeprecatedProperty, DeprecationMessage="Use ProfileIdString instead."))
...
public:
	 ...
	 */
	UE_API const FKey& GetCurrentKey() const;

	/** Returns the default key that this mapping is to */
	UE_API const FKey& GetDefaultKey() const;
	 ...
	 */
	UE_API const FName GetMappingName() const;
	...
	UE_API void SetCurrentKey(const FKey& NewKey);
	...
	UE_API void SetHardwareDeviceId(const FHardwareDeviceIdentifier& InDeviceId);
	 ...
	 */
	UE_API void UpdateDefaultKeyFromActionKeyMapping(const FEnhancedActionKeyMapping& OriginalMapping);
	 ...
	 */
	UE_API void UpdateMetadataFromActionKeyMapping(const FEnhancedActionKeyMapping& OriginalMapping);
	
	ENHANCEDINPUT_API friend uint32 GetTypeHash(const FPlayerKeyMapping& InMapping);
	UE_API bool operator==(const FPlayerKeyMapping& Other) const;
	UE_API bool operator!=(const FPlayerKeyMapping& Other) const;
	UE_API FPlayerKeyMapping& operator=(FPlayerKeyMapping& Other);
	UE_API FPlayerKeyMapping& operator=(const FPlayerKeyMapping& Other);
	UE_API FPlayerKeyMapping(const FPlayerKeyMapping& Other);

	/** Returns true if this mapping has been modified since it was registered from an IMC */
	UE_API const bool IsDirty() const;
...
protected:
	...
	UE_DEPRECATED(5.6, "This property is deprecated, use AssociatedInputActionSoft instead.")
	UPROPERTY(Transient, VisibleAnywhere, BlueprintReadOnly, Category="Enhanced Input|User Settings", meta=(DeprecatedProperty, DeprecationMessage = "This property is deprecated, use AssociatedInputActionSoft instead."))
	TObjectPtr<const UInputAction> AssociatedInputAction; 

	/** The input action associated with this player key mapping */
	UPROPERTY(Transient, VisibleAnywhere, BlueprintReadOnly, Category="Enhanced Input|User Settings")
	TSoftObjectPtr<const UInputAction> AssociatedInputActionSoft;
};
...
struct FPlayerMappableKeyQueryOptions final
{
	...
	UE_API FPlayerMappableKeyQueryOptions();
...
public:
     ...
     */
    UE_API virtual void Serialize(FArchive& Ar) override final;
    //~ End UObject Interface

	UFUNCTION(BlueprintCallable, Category="Enhanced Input|User Settings")
	UE_API virtual void SetDisplayName(const FText& NewDisplayName);

	UE_DEPRECATED(5.6, "Use GetProfileIdString instead")
	UE_API const FGameplayTag& GetProfileIdentifer() const;

	UFUNCTION(BlueprintCallable, BlueprintPure, Category="Enhanced Input|User Settings")
	UE_API const FString& GetProfileIdString() const;

	/** Get the localized display name for this profile */
	UFUNCTION(BlueprintCallable, BlueprintPure, Category="Enhanced Input|User Settings")
	UE_API const FText& GetProfileDisplayName() const;
	 ...
	 */
	UFUNCTION(BlueprintCallable, BlueprintPure, Category="Enhanced Input|User Settings")
	UE_API const TMap<FName, FKeyMappingRow>& GetPlayerMappingRows() const;

	/** Resets every player key mapping to this mapping back to it's default value */
	UFUNCTION(BlueprintCallable, Category="Enhanced Input|User Settings")
	UE_API void ResetMappingToDefault(UPARAM(Meta=(GetOptions="EnhancedInput.PlayerMappableKeySettings.GetKnownMappingNames")) const FName InMappingName);
	
	/** Get all the key mappings associated with the given mapping name on this profile */
	UE_API FKeyMappingRow* FindKeyMappingRowMutable(const FName InMappingName);

	/** Get all the key mappings associated with the given mapping name on this profile */
	UE_API const FKeyMappingRow* FindKeyMappingRow(const FName InMappingName) const;

	/** A helper function to print out all the current profile settings to the log. */
	UFUNCTION(BlueprintCallable, Category="Enhanced Input|User Settings")
	UE_API virtual void DumpProfileToLog() const;
	...
	UE_API virtual int32 GetMappedKeysInRow(UPARAM(Meta=(GetOptions="EnhancedInput.PlayerMappableKeySettings.GetKnownMappingNames")) const FName MappingName, /*OUT*/ TArray<FKey>& OutKeys) const;
	 ...
	 */
	UE_API virtual int32 GetPlayerMappedKeysForRebuildControlMappings(const FEnhancedActionKeyMapping& DefaultMapping, /*OUT*/ TArray<FKey>& OutKeys) const;
	...
	*/
	...
	UE_API virtual int32 QueryPlayerMappedKeys(const FPlayerMappableKeyQueryOptions& Options, /*OUT*/ TArray<FKey>& OutKeys) const;
	 ...
	 */
	UFUNCTION(BlueprintCallable, Category="Enhanced Input|User Settings", meta = (ReturnDisplayName = "Number of mappings"))
	UE_API virtual int32 GetMappingNamesForKey(const FKey& InKey, /*OUT*/ TArray<FName>& OutMappingNames) const;

	/** Returns a pointer to the player key mapping that fits with the given arguments. Returns null if none exist. */
	UE_API virtual FPlayerKeyMapping* FindKeyMapping(const FMapPlayerKeyArgs& InArgs) const;
	...
	UE_API void K2_FindKeyMapping(FPlayerKeyMapping& OutKeyMapping, const FMapPlayerKeyArgs& InArgs) const;

	/**
	 * Resets all the key mappings in this profile to their default value from their Input Mapping Context.
	 */
	UFUNCTION(BlueprintCallable, Category="Enhanced Input|User Settings")
	UE_API virtual void ResetToDefault();
	
	/**
	 * Returns true if the given player key mapping passes the query filter provided. 
	 */
	UFUNCTION(BlueprintCallable, Category="Enhanced Input|User Settings")
	UE_API virtual bool DoesMappingPassQueryOptions(const FPlayerKeyMapping& PlayerMapping, const FPlayerMappableKeyQueryOptions& Options) const;
...
protected:
	...
	*  Make any changes to the given Enhanced Player input object that may be necessary for
	 ...
	 */
	UE_API virtual void EquipProfile();
	 ...
	 * This function will only ever be called by UEnhancedInputUserSettings::SetKeyProfile
	 */
	UE_API virtual void UnEquipProfile();
	
	/** The ID of this profile. This can be used by each Key Mapping to filter down which profile is required for it be equipped. */
	UE_DEPRECATED(5.6, "Use ProfileIdentifierString instead")	
	UPROPERTY(SaveGame, meta=(DeprecatedProperty, DeprecationMessage = "Use ProfileIdentifierString"))
...
struct FPlayerMappableKeyProfileCreationArgs final
{
	...
	~FPlayerMappableKeyProfileCreationArgs() = default;
	FPlayerMappableKeyProfileCreationArgs(const FPlayerMappableKeyProfileCreationArgs&) = default;
	FPlayerMappableKeyProfileCreationArgs(FPlayerMappableKeyProfileCreationArgs&&) = default;
	FPlayerMappableKeyProfileCreationArgs& operator=(const FPlayerMappableKeyProfileCreationArgs&) = default;
	FPlayerMappableKeyProfileCreationArgs& operator=(FPlayerMappableKeyProfileCreationArgs&&) = default;
	PRAGMA_ENABLE_DEPRECATION_WARNINGS
	...
	UE_DEPRECATED(5.6, "Use ProfileStringIdentifier instead.")
	UPROPERTY(meta=(DeprecatedProperty, DeprecationMessage="Use ProfileStringIdentifier instead."))
	FGameplayTag ProfileIdentifier;

	/** The unique identifier that this profile should have */
	UPROPERTY(BlueprintReadWrite, EditAnywhere, Category="Enhanced Input|User Settings")
	FString ProfileStringIdentifier;
...
public:
	...
	static UE_API UEnhancedInputUserSettings* LoadOrCreateSettings(ULocalPlayer* LP);
	UE_API virtual void Initialize(ULocalPlayer* LP);
	 ...
	 */
	UFUNCTION(BlueprintCallable, Category="Enhanced Input|User Settings")
	UE_API virtual void ApplySettings();
	 ...
	 */
	UFUNCTION(BlueprintCallable, Category="Enhanced Input|User Settings")
	UE_API virtual void SaveSettings();
	 ...
	 */
	UFUNCTION(BlueprintCallable, Category="Enhanced Input|User Settings")
	UE_API virtual void AsyncSaveSettings();

private:
	
	/**
	 * Called when a save file is loaded from disk but has a different class then the one specified in the developer settings.
	 * 
	 * @param OldSettingsObject Pointer to the old user settings object
	 */
	UE_API void HandleSettingsClassChangedOnLoad(UEnhancedInputUserSettings* OldSettingsObject);
	
protected:
	
	/**
	 * Override this function to be notified of when the user settings class has changed.
	 * 
	 * @param OldSettingsObject Pointer to the old user settings object
	 */
	UE_API virtual void HandleSettingsClassChangedOnLoad_Impl(UEnhancedInputUserSettings* OldSettingsObject);

	/**
	* Returns the save file name to use when saving the settings file for the given local player
	*/
	static UE_API FString GetSaveFilename(const ULocalPlayer* LP);
	
	UE_API virtual void OnAsyncSaveComplete(const FString& SlotName, const int32 UserIndex, bool bSuccess);
	 ...
	 */
	UE_API virtual void OnKeyMappingUpdated(FPlayerKeyMapping* ChangedMapping, const FMapPlayerKeyArgs& InArgs, const bool bIsBeingUnmapped);
	 ...
	 */
	UE_API virtual void OnKeyMappingRegistered(FPlayerKeyMapping& RegisteredMapping, const FEnhancedActionKeyMapping& SourceMapping);

public:
	
	UE_API ULocalPlayer* GetLocalPlayer() const;
...
public:
	...
	UE_API virtual void MapPlayerKey(const FMapPlayerKeyArgs& InArgs, FGameplayTagContainer& FailureReason);

	/** 
	* Unmaps a single player mapping that matches the given Mapping name, slot, and hardware device.
	*/
	UFUNCTION(BlueprintCallable, Category="Enhanced Input|User Settings", meta = (AutoCreateRefTerm = "FailureReason"))
	UE_API virtual void UnMapPlayerKey(const FMapPlayerKeyArgs& InArgs, FGameplayTagContainer& FailureReason);
	...
	*/
	UFUNCTION(BlueprintCallable, Category="Enhanced Input|User Settings", meta = (AutoCreateRefTerm = "FailureReason"))
	UE_API virtual void ResetAllPlayerKeysInRow(const FMapPlayerKeyArgs& InArgs, FGameplayTagContainer& FailureReason);
	 ...
	 * @param FailureReason	Populated with failure reasons if the operation fails.
	 */
	UE_DEPRECATED(5.6, "Use the version which takes an FString instead.")
	UFUNCTION(BlueprintCallable, Category="Enhanced Input|User Settings", meta = (AutoCreateRefTerm = "FailureReason"))
	UE_API virtual void ResetKeyProfileToDefault(const FGameplayTag& ProfileId, FGameplayTagContainer& FailureReason) final;
	 ...
	 */
	UFUNCTION(BlueprintCallable, Category="Enhanced Input|User Settings", meta = (AutoCreateRefTerm = "FailureReason"))
	UE_API virtual void ResetKeyProfileIdToDefault(const FString& ProfileId, FGameplayTagContainer& FailureReason);

	/** Returns a set of all player key mappings for the given mapping name. */
	UFUNCTION(BlueprintCallable, Category="Enhanced Input|User Settings")
	UE_API virtual const TSet<FPlayerKeyMapping>& FindMappingsInRow(UPARAM(Meta=(GetOptions="EnhancedInput.PlayerMappableKeySettings.GetKnownMappingNames")) const FName MappingName) const;

	/** Returns the current player key mapping for the given row name in the given slot */
	UE_API virtual const FPlayerKeyMapping* FindCurrentMappingForSlot(const FName MappingName, const EPlayerMappableKeySlot InSlot) const;

	/** Returns the Input Action associated with the given player mapping name */
	UE_API const UInputAction* FindInputActionForMapping(const FName MappingName) const;
	 ...
	 */
	UE_DEPRECATED(5.6, "Use the version that takes an FString instead.")
	UE_API virtual bool SetKeyProfile(const FGameplayTag& InProfileId) final;

	/**
	 * Changes the currently active key profile to the one with the given name. Returns true if the operation was successful.
	 */
	UFUNCTION(BlueprintCallable, Category="Enhanced Input|User Settings", meta=(ReturnDisplayName = "Was Successful"))
	UE_API virtual bool SetActiveKeyProfile(const FString& InProfileId);
	
	/** Gets the currently selected key profile */
	UE_DEPRECATED(5.6, "Use GetActiveKeyProfileId instead ")
	UE_API virtual const FGameplayTag& GetCurrentKeyProfileIdentifier() const final;

	/** Gets the currently selected key profile */
	UFUNCTION(BlueprintCallable, BlueprintPure, Category="Enhanced Input|User Settings")
	UE_API virtual const FString& GetActiveKeyProfileId() const;

	/** Get the current key profile that the user has set */
	UE_DEPRECATED(5.6, "Use GetActiveKeyProfile instead.")
	UE_API UEnhancedPlayerMappableKeyProfile* GetCurrentKeyProfile() const;

	/** Get the currently active key profile that the user has set */
	UFUNCTION(BlueprintCallable, BlueprintPure, Category="Enhanced Input|User Settings")
	UE_API UEnhancedPlayerMappableKeyProfile* GetActiveKeyProfile() const;

	/** Get the current key profile that the user has set */
	template<class T> UE_DEPRECATED(5.6, "Use GetActiveKeyProfileAs instead.")
	inline T* GetCurrentKeyProfileAs() const
	{
		...
		return Cast<T>(GetActiveKeyProfile());
	}

	/** Get the current key profile that the user has set, casted to type T */
	template<class T>
	inline T* GetActiveKeyProfileAs() const
	{
		static_assert(TIsDerivedFrom<T, UEnhancedPlayerMappableKeyProfile>::IsDerived, "T must be a UEnhancedPlayerMappableKeyProfile-based type!");
		return Cast<T>(GetActiveKeyProfile());
	}
	...
	UE_API virtual UEnhancedPlayerMappableKeyProfile* CreateNewKeyProfile(const FPlayerMappableKeyProfileCreationArgs& InArgs);

	/** Returns the key profile with the given name if one exists. Null if one doesn't exist */
	UE_DEPRECATED(5.6, "Use the version which takes an FString instead")
	UE_API virtual UEnhancedPlayerMappableKeyProfile* GetKeyProfileWithIdentifier(const FGameplayTag& ProfileId) const final;

	UFUNCTION(BlueprintCallable, BlueprintPure, Category="Enhanced Input|User Settings")
	UE_API virtual UEnhancedPlayerMappableKeyProfile* GetKeyProfileWithId(const FString& ProfileId) const;
	...
	{
		static_assert(TIsDerivedFrom<T, UEnhancedPlayerMappableKeyProfile>::IsDerived, "T must be a UEnhancedPlayerMappableKeyProfile-based type!");
		return Cast<T>(GetKeyProfileWithId(ProfileId.ToString()));
	}

	/** Returns the key profile with the given name if one exists. Null if one doesn't exist */
	template<class T>
	inline T* GetKeyProfileWithIdAs(const FString& ProfileId) const
	{
		static_assert(TIsDerivedFrom<T, UEnhancedPlayerMappableKeyProfile>::IsDerived, "T must be a UEnhancedPlayerMappableKeyProfile-based type!");
		return Cast<T>(GetKeyProfileWithId(ProfileId));
	}
	 ...
	 */
	UFUNCTION(BlueprintCallable, Category="Enhanced Input|User Settings")
	UE_API virtual bool RegisterInputMappingContext(const UInputMappingContext* IMC);

	/** Registers multiple mapping contexts with the settings */
	UFUNCTION(BlueprintCallable, Category="Enhanced Input|User Settings")
	UE_API bool RegisterInputMappingContexts(const TSet<UInputMappingContext*>& MappingContexts);

	/** Removes this mapping context from the registered mapping contexts */
	UFUNCTION(BlueprintCallable, Category="Enhanced Input|User Settings")
	UE_API virtual bool UnregisterInputMappingContext(const UInputMappingContext* IMC);

	/** Removes multiple mapping contexts from the registered mapping contexts */
	UFUNCTION(BlueprintCallable, Category="Enhanced Input|User Settings")
	UE_API bool UnregisterInputMappingContexts(const TSet<UInputMappingContext*>& MappingContexts);

	/** Gets all the currently registered mapping contexts with the settings */
	UE_API const TSet<TObjectPtr<const UInputMappingContext>>& GetRegisteredInputMappingContexts() const;

	/** Returns true if this mapping context is currently registered with the settings */
	UFUNCTION(BlueprintCallable, BlueprintPure, Category="Enhanced Input|User Settings")
	UE_API bool IsMappingContextRegistered(const UInputMappingContext* IMC) const;
	
	/** Draw debug information on the screen for these user settings. */
	UE_API void ShowDebugInfo(UCanvas* Canvas) const;
...
protected:
	 ...
	 */
	UE_API virtual bool RegisterKeyMappingsToProfile(UEnhancedPlayerMappableKeyProfile& Profile, const UInputMappingContext* IMC);

	UE_DEPRECATED(5.6, "Use DetermineHardwareDeviceForActionMapping which takes in a mapping context instead")
	virtual FHardwareDeviceIdentifier DetermineHardwareDeviceForActionMapping(const FEnhancedActionKeyMapping& ActionMapping) const final
	{
		return DetermineHardwareDeviceForActionMapping(ActionMapping, nullptr);
	}
	 ...
	 */
	UE_API virtual FHardwareDeviceIdentifier DetermineHardwareDeviceForActionMapping(const FEnhancedActionKeyMapping& ActionMapping, const UInputMappingContext* IMC = nullptr) const;
	...
	*/
	UE_API virtual void ShowDebugInfoInternal(UCanvas* Canvas) const;

	/** The current key profile that is equipped by the user. */
	UE_DEPRECATED(5.6, "Use the CurrentProfileIdentifierString instead.")
	UPROPERTY(SaveGame, meta=(DeprecatedProperty))
	 ...
	 */
	UPROPERTY(SaveGame)
	FString CurrentProfileIdentifierString;
