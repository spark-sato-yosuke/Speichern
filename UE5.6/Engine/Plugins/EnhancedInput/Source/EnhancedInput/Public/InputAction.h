...
public:
	...
	static UE_API TSet<const UInputAction*> ActionsWithModifiedValueTypes;
	static UE_API TSet<const UInputAction*> ActionsWithModifiedTriggers;
	
	/**
	 * Returns a bitmask of supported trigger events that is built from each UInputTrigger on this Action.
	 */
	UE_API ETriggerEventsSupported GetSupportedTriggerEvents() const;
...
public:
	...
	UE_API FInputActionInstance(const UInputAction* InSourceAction);
	...
	const TArray<TObjectPtr<UInputTrigger>>& GetTriggers() const { return Triggers; }
	const TArray<TObjectPtr<UInputModifier>>& GetModifiers() const { return Modifiers; }

	// The source action that this instance is created from
	TObjectPtr<const UInputAction> GetSourceAction() const { return SourceAction; }
