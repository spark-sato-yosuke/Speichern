...
void UEnhancedInputLocalPlayerSubsystem::Deinitialize()
{
	Super::Deinitialize();
...
void UEnhancedInputLocalPlayerSubsystem::PlayerControllerChanged(APlayerController* NewPlayerController)
{
	...
	{
		// Add any default input mapping contexts that we may need
    	for (const FDefaultContextSetting& DefaultMapping : GetDefault<UEnhancedInputDeveloperSettings>()->DefaultMappingContexts)
    	{
    		if (const UInputMappingContext* IMC = DefaultMapping.InputMappingContext.LoadSynchronous())
    		{
    			if (DefaultMapping.bAddImmediately)
    			{
    				AddMappingContext(IMC, DefaultMapping.Priority);
    			}
    			
    			if (DefaultMapping.bRegisterWithUserSettings && UserSettings)
    			{
...
void UEnhancedInputLocalPlayerSubsystem::InitalizeUserSettings()
{
	...
	// We can't load the user settings without a valid player input object because we get the save file name from it
	if (!GetPlayerInput())
	{
		UE_LOG(LogEnhancedInput, Warning, TEXT("UEnhancedInputLocalPlayerSubsystem for Local Player '%s' does not have a valid PlayerInput object. Failed to load user settings."), *GetNameSafe(LP));
		return;
	}
...
bool UEnhancedInputWorldSubsystem::InputKey(const FInputKeyEventArgs& Params)
{
	if (PlayerInput)
	{
...
void UEnhancedInputWorldSubsystem::AddDefaultMappingContexts()
{
	...
	{
		for (const FDefaultContextSetting& ContextSetting : GetDefault<UEnhancedInputDeveloperSettings>()->DefaultWorldSubsystemMappingContexts)
		{
