...
{
	static bool bAlwaysGetRealValueFromActionInstanceData = 1;
	static FAutoConsoleVariableRef CVarAlwaysGetRealValueFromActionInstanceData(TEXT("EnhancedInput.bAlwaysGetRealValueFromActionInstanceData"),
		bAlwaysGetRealValueFromActionInstanceData,
		TEXT("If true, then input action instances will always return their true value instead of being conditoinal on the trigger state. Note: This CVar will eventually be removed"));
}
...
FInputActionValue FInputActionInstance::GetValue() const
{
	if (UE::EnhancedInput::bAlwaysGetRealValueFromActionInstanceData)
	{
		return Value;
	}
	
	// The legacy behavior only returned the true value when in the "Triggered" state
	// which would result in bool (digital) input actions being false on the "started" phase,
	// which is confusing. 
	return
		TriggerEvent == ETriggerEvent::Triggered ?
		Value :
		FInputActionValue(Value.GetValueType(), FInputActionValue::Axis3D::ZeroVector);
 } 
