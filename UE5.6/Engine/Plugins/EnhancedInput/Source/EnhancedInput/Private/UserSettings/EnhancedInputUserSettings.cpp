...
PRAGMA_ENABLE_DEPRECATION_WARNINGS
	, AssociatedInputActionSoft(nullptr)
{
}
...
PRAGMA_DISABLE_DEPRECATION_WARNINGS
	, AssociatedInputAction(nullptr)
PRAGMA_ENABLE_DEPRECATION_WARNINGS
	, AssociatedInputActionSoft(OriginalMapping.Action)
{
}

FPlayerKeyMapping::FPlayerKeyMapping(const FPlayerKeyMapping& Other)
{
	...
	AssociatedInputActionSoft = Other.AssociatedInputActionSoft;
}
	...
	return
		FString::Printf(TEXT("Mapping Name: '%s'  Slot: '%s'  Default Key: '%s'  Player Mapped Key: '%s'  HardwareDevice:  '%s'   AssociatedInputAction: '%s'"),
			...
			*HardwareDeviceId.ToString(),
			AssociatedInputActionSoft.IsValid() ? *AssociatedInputActionSoft.Get()->GetFName().ToString() : TEXT("NULL INPUT ACTION"));
}
...
{
	return AssociatedInputActionSoft.Get();
}
...
FPlayerKeyMapping& FPlayerKeyMapping::operator=(FPlayerKeyMapping& Other)
{
	MappingName = Other.MappingName;
	DisplayName = Other.DisplayName;
	DisplayCategory = Other.DisplayCategory;
...
FPlayerKeyMapping& FPlayerKeyMapping::operator=(const FPlayerKeyMapping& Other)
{
	MappingName = Other.MappingName;
	DisplayName = Other.DisplayName;
	DisplayCategory = Other.DisplayCategory;
...
void UEnhancedPlayerMappableKeyProfile::ResetToDefault()
{
	...
	UE_LOG(LogEnhancedInput, Verbose, TEXT("Reset Player Mappable Key Profile '%s' to default values"), *ProfileIdentifierString);
}

void UEnhancedPlayerMappableKeyProfile::EquipProfile()
{
	UE_LOG(LogEnhancedInput, Verbose, TEXT("Equipping Player Mappable Key Profile '%s'"), *ProfileIdentifierString);
}

void UEnhancedPlayerMappableKeyProfile::UnEquipProfile()
{
	UE_LOG(LogEnhancedInput, Verbose, TEXT("Unequipping Player Mappable Key Profile '%s'"), *ProfileIdentifierString);
}
...
const FString& UEnhancedPlayerMappableKeyProfile::GetProfileIdString() const
{
	return ProfileIdentifierString;
}
...
int32 UEnhancedPlayerMappableKeyProfile::GetMappedKeysInRow(const FName MappingName, TArray<FKey>& OutKeys) const
{
	...
	{
		UE_LOG(LogEnhancedInput, Verbose, TEXT("Player Mappable Key Profile '%s' doesn't have any mappings for '%s'"), *ProfileIdentifierString, *MappingName.ToString());
	}
	return OutKeys.Num();
}
...
int32 UEnhancedPlayerMappableKeyProfile::QueryPlayerMappedKeys(const FPlayerMappableKeyQueryOptions& Options, TArray<FKey>& OutKeys) const
{
	...
	{
		UE_LOG(LogEnhancedInput, Verbose, TEXT("Player Mappable Key Profile '%s' doesn't have any mappings for '%s'"), *ProfileIdentifierString, *Options.MappingName.ToString());
	}
...
UEnhancedInputUserSettings* UEnhancedInputUserSettings::LoadOrCreateSettings(ULocalPlayer* LocalPlayer)
{
	...
	const FString& SaveFileName = GetSaveFilename(LocalPlayer);
	
	// If the save game exists, load it.
	if (UGameplayStatics::DoesSaveGameExist(SaveFileName, LocalPlayer->GetLocalPlayerIndex()))
	{
		USaveGame* Slot = UGameplayStatics::LoadGameFromSlot(SaveFileName, LocalPlayer->GetLocalPlayerIndex());
	...
	// If the class has changed, then we should call into a customizable upgrade path notifying you of the change.
	// This can happen if you change the class in the editor and there is still an old save file on disk,
	// or if you change the class of your user settings object and would to keep some of the old data.
	else if (Settings->GetClass() != SettingsClass)
	{
		UE_LOG(
			LogEnhancedInput,
			Log,
			TEXT("[%hs] Enhanced Input User Settings class has been changed from '%s' to '%s'. A new settings save object will be created (your saved settings will be reset). Override HandleSettingsClassChangedOnLoad_Impl to retain data."),
			__func__, *GetNameSafe(Settings->GetClass()), *GetNameSafe(SettingsClass))

		// Create a new settings object here with the new class which it has been changed to
		UEnhancedInputUserSettings* NewSettingsObject = Cast<UEnhancedInputUserSettings>(UGameplayStatics::CreateSaveGameObject(SettingsClass));

		// Allow you to upgrade the old settings if you ever change the class of your user settings type
		NewSettingsObject->HandleSettingsClassChangedOnLoad(Settings);

		// Store our new settings object as the return value
...
void UEnhancedInputUserSettings::SaveSettings()
{
	...
	{
		UGameplayStatics::SaveGameToSlot(this, GetSaveFilename(OwningPlayer), OwningPlayer->GetLocalPlayerIndex());
...
void UEnhancedInputUserSettings::AsyncSaveSettings()
{
	if (const ULocalPlayer* OwningPlayer = GetLocalPlayer())
	{
		...
		UGameplayStatics::AsyncSaveGameToSlot(this, GetSaveFilename(OwningPlayer), OwningPlayer->GetLocalPlayerIndex(), SavedDelegate);
	}
	else
	{
...
void UEnhancedInputUserSettings::HandleSettingsClassChangedOnLoad(UEnhancedInputUserSettings* OldSettingsObject)
{
	// This function is called when the user settings class has changed
	check(OldSettingsObject);
	
	// This should only ever be called for settings objects of two different classes
	check(OldSettingsObject->GetClass() != GetClass());

	// Call the overridable virtual function to allow games to customize this behavior
	HandleSettingsClassChangedOnLoad_Impl(OldSettingsObject);
}

void UEnhancedInputUserSettings::HandleSettingsClassChangedOnLoad_Impl(UEnhancedInputUserSettings* OldSettingsObject)
{
	// Copy over any values of properties from the old settings object to this new one for initialization.
	// This will keep things like the key mappings, profile id, and other UPROPERTY's that are the same across
	// the two objects.
	PRAGMA_DISABLE_DEPRECATION_WARNINGS
	CurrentProfileIdentifier = OldSettingsObject->CurrentProfileIdentifier;
	SavedKeyProfiles = OldSettingsObject->SavedKeyProfiles;
	PRAGMA_ENABLE_DEPRECATION_WARNINGS
	
	CurrentProfileIdentifierString = OldSettingsObject->CurrentProfileIdentifierString;
	SavedKeyProfilesMap = OldSettingsObject->SavedKeyProfilesMap;

	// Note: Calling any Serialize functions, or something like CopyPropertiesForUnrelatedObjects, will cause
	// errors to be logged because there will be a size mismatch between the two types, as they are different classes.
	// This is why we naively assign the data we would like to keep above instead of calling a more complex serialization function.
}
...
{
	/**
	* The save version for player mappable keys serialization in Enhanced Input. 
	* When adding a new version, make sure to set "Current"! 
	*/
	struct FPlayerMappableSaveVersion
	{
		/**
		 * The version number which this feature shipped with. Needed for loading previous versions of the save files
		 */
		static constexpr int32 Base = 1;

		/**
		 * Save version for when we converted UEnhancedPlayerMappableKeyProfile to be identified with an FString instead
		 * of with a FGameplayTags
		 */
		static constexpr int32 ConvertProfileIdFromTagToString = 2;

		/**
		 * The current save version of this feature. In the future, make sure to increment this if a new version is added.
		 */
		static constexpr int32 Current = ConvertProfileIdFromTagToString;
	};	

	/**
	* Data saved which represents a single dirty key mapping.
	*/
	struct FKeyMappingSaveData
	{
		...
		/**
		* The name of the key mapping which is dirty.
		*/
		FName ActionName = NAME_None;

		/**
		* The currently set key that should be used and was set by the player.
		*/
		FName CurrentKeyName = NAME_None;

		/**
		* Specific hardware which this mapping should be applied to if not invalid.
		*/
		FHardwareDeviceIdentifier HardwareDeviceId = FHardwareDeviceIdentifier::Invalid;

		/**
		* Which slot this mapping is for. 
		*/
	...
	{
		// Support for loading in legacy files
		friend FArchive& operator<<(FArchive& Ar, FMappableKeysHeader& Header)
		{
			// We should only ever be going down this code path when loading an old version. 
			// To save new data, we use the "Serialize" function below which takes a version number. 
			check(Ar.IsLoading());
			
			// HEY! Don't add or modify anything that is being serialized here. Use the new Serialize function which takes a version instead.
			FGameplayTag ProfileIdentifier;
			Ar << ProfileIdentifier;
			Ar << Header.ClassPath;
			Ar << Header.ObjectPath;
			Ar << Header.DirtyMappings;
			// HEY! Don't add or modify anything that is being serialized here.

			// Set the new FString profile ID based on the legacy gameplay tag.
			Header.ProfileIdentifierString = ProfileIdentifier.ToString();
			
			return Ar;
		}
		
		/**
		* Serializes the given Header while considering the given version.
		*/
		static FArchive& Serialize(FArchive& Ar, FMappableKeysHeader& Header, const int32 Version)
		{			
			Ar << Header.ClassPath;
			Ar << Header.ObjectPath;
			Ar << Header.DirtyMappings;
			Ar << Header.ProfileIdentifierString;

			// Note: When adding new properties to be serialized in the future, make sure to respect the version number! 
			// i.e.
			// 
			// if (Version >= FPlayerMappableSaveVersion::NewVersionName)
			// { 
			//		Ar << Header.NewProperty; 
			// }
			
			return Ar;
		}
		
		FString ProfileIdentifierString;
		FString ClassPath;
		FString ObjectPath;
...
void UEnhancedInputUserSettings::Serialize(FArchive& Ar)
{
	...
	int32 SaveVersion = UE::EnhancedInput::FPlayerMappableSaveVersion::Current;
	Ar << SaveVersion;
	
	// Detect a mis-match of byte streams, i.e. file corruption
	// The version should be less than or equal to the current version. It can be "less than" if loading an 
	// older save file version.
	ensure(SaveVersion <= UE::EnhancedInput::FPlayerMappableSaveVersion::Current);
	...
	if (Ar.IsSaving())
	{
		...
		for (TPair<FString, UEnhancedPlayerMappableKeyProfile*> ProfilePair : SavedKeyProfilesMap)
		{
			UE::EnhancedInput::FMappableKeysHeader Header =
				{
					.ProfileIdentifierString = ProfilePair.Key,
					.ClassPath = ProfilePair.Value->GetClass()->GetPathName(),
					.ObjectPath = ProfilePair.Value->GetPathName(Outer),
					.DirtyMappings = {}
				};

			// Serialize only dirty mappings here, not every mapping in the key profile.
	...
	// We need to support this legacy loading path from before we serialized out the headers with consideration to the version.
	if (Ar.IsLoading() && SaveVersion < UE::EnhancedInput::FPlayerMappableSaveVersion::ConvertProfileIdFromTagToString)
	{
		Ar << (Headers);
	}
	else
	{
		// Serialize out all the headers, need to track the number of them so that we can read them back in
		int32 NumberHeaders = Headers.Num();
		Ar << NumberHeaders;

		// When loading, make sure that there are some values in the header array that we can populate when serializing
		if (Ar.IsLoading())
		{
			Headers.AddZeroed(NumberHeaders);
		}
		
		for (int32 i = 0; i < NumberHeaders; ++i)
		{	
			UE::EnhancedInput::FMappableKeysHeader::Serialize(Ar, Headers[i], SaveVersion);
		}
	}
	
	// Now we can actually re-populate our runtime data with the info from the headers that we just loaded in
	if (Ar.IsLoading())
	{
		for (const UE::EnhancedInput::FMappableKeysHeader& Header : Headers)
		{
			if (const UClass* FoundClass = FindObject<UClass>(nullptr, *Header.ClassPath))
			{
				...
				NewProfile->ProfileIdentifierString = Header.ProfileIdentifierString;

				// Add the new profile to the settings object. This will replace the old profile if one existed
				SavedKeyProfilesMap.Add(Header.ProfileIdentifierString, NewProfile);				
				
				// Add any player saved keys to this profile!
				for (const UE::EnhancedInput::FKeyMappingSaveData& SavedKeyData : Header.DirtyMappings)
				{
	...
	// This is the final piece of serializing the key profiles. It's a sanity and safety check.
	...
	for (const TPair<FString, TObjectPtr<UEnhancedPlayerMappableKeyProfile>>& ProfilePair : SavedKeyProfilesMap)
	{
...
void UEnhancedInputUserSettings::MapPlayerKey(const FMapPlayerKeyArgs& InArgs, FGameplayTagContainer& FailureReason)
{
	...
	UEnhancedPlayerMappableKeyProfile* KeyProfile = !InArgs.ProfileIdString.IsEmpty() ? GetKeyProfileWithId(InArgs.ProfileIdString) : GetActiveKeyProfile();
	if (!KeyProfile)
	{
	...
	else if (FKeyMappingRow* MappingRow = KeyProfile->FindKeyMappingRowMutable(InArgs.MappingName))
	{
		...
		if (InArgs.bCreateMatchingSlotIfNeeded && NumMappings > 0)
		{
			...
			PlayerMappingData.AssociatedInputActionSoft = ExistingMapping->AssociatedInputActionSoft;
...
{
	return ResetKeyProfileIdToDefault(ProfileId.ToString(), FailureReason);
}
PRAGMA_ENABLE_DEPRECATION_WARNINGS

void UEnhancedInputUserSettings::ResetKeyProfileIdToDefault(const FString& ProfileId, FGameplayTagContainer& FailureReason)
{
	if (UEnhancedPlayerMappableKeyProfile* KeyProfile = GetKeyProfileWithId(ProfileId))
	{
...
{
	if (UEnhancedPlayerMappableKeyProfile* KeyProfile = GetActiveKeyProfile())
	{
...
{
	return SetActiveKeyProfile(InProfileId.ToString());
}
PRAGMA_ENABLE_DEPRECATION_WARNINGS

bool UEnhancedInputUserSettings::SetActiveKeyProfile(const FString& InProfileId)
{
	const FString OriginalProfileId = GetActiveKeyProfileId();
	UEnhancedPlayerMappableKeyProfile* OriginalProfile = GetActiveKeyProfile();

	// If this key profile is already equipped, then there is nothing to do
	if (InProfileId.Equals(OriginalProfileId))
	{
		UE_LOG(LogEnhancedInput, Log, TEXT("Key profile '%s' is already currently equipped. Nothing will happen."), *InProfileId);
		return false;
	}
	
	if (const TObjectPtr<UEnhancedPlayerMappableKeyProfile>* NewProfile = SavedKeyProfilesMap.Find(InProfileId))
	{
		...
		CurrentProfileIdentifierString = InProfileId;
	...
	{
		UE_LOG(LogEnhancedInput, Error, TEXT("No profile with name '%s' exists! Did you call CreateNewKeyProfile at any point?"), *InProfileId);
		return false;
	}

	UE_LOG(LogEnhancedInput, Verbose, TEXT("Successfully changed Key Profile from '%s' to '%s'"), *OriginalProfileId, *CurrentProfileIdentifierString);
	return true;
}
...
const FString& UEnhancedInputUserSettings::GetActiveKeyProfileId() const
{
	return CurrentProfileIdentifierString;
}

PRAGMA_DISABLE_DEPRECATION_WARNINGS
UEnhancedPlayerMappableKeyProfile* UEnhancedInputUserSettings::GetCurrentKeyProfile() const
{
	return GetActiveKeyProfile();	
}
PRAGMA_ENABLE_DEPRECATION_WARNINGS

UEnhancedPlayerMappableKeyProfile* UEnhancedInputUserSettings::GetActiveKeyProfile() const
{
	return GetKeyProfileWithId(GetActiveKeyProfileId());
}
...
const TMap<FString, TObjectPtr<UEnhancedPlayerMappableKeyProfile>>& UEnhancedInputUserSettings::GetAllAvailableKeyProfiles() const
{
	return SavedKeyProfilesMap;
}
...
{
	return GetKeyProfileWithId(ProfileId.ToString());
}
PRAGMA_ENABLE_DEPRECATION_WARNINGS

UEnhancedPlayerMappableKeyProfile* UEnhancedInputUserSettings::GetKeyProfileWithId(const FString& ProfileId) const
{
	if (const TObjectPtr<UEnhancedPlayerMappableKeyProfile>* ExistingProfile = SavedKeyProfilesMap.Find(ProfileId))
	{		
...
FHardwareDeviceIdentifier UEnhancedInputUserSettings::DetermineHardwareDeviceForActionMapping(const FEnhancedActionKeyMapping& ActionMapping, const UInputMappingContext* IMC) const
{
...
void UEnhancedInputUserSettings::ShowDebugInfoInternal(UCanvas* Canvas) const
{
	...
	DisplayDebugManager.DrawString(FString::Printf(TEXT("\t There are '%d' saved key profiles"), SavedKeyProfilesMap.Num()));

	// currently active profile
	if (UEnhancedPlayerMappableKeyProfile* CurrentProfile = GetActiveKeyProfile())
	{
		DisplayDebugManager.DrawString(FString::Printf(TEXT("\t Currently Active Profile: %s (%s)"), *CurrentProfile->GetProfileIdString(), *GetNameSafe(CurrentProfile->GetClass())));
	}
	else
	{
		DisplayDebugManager.SetDrawColor(UE::EnhancedInput::DebugColors::ErrorColor);
