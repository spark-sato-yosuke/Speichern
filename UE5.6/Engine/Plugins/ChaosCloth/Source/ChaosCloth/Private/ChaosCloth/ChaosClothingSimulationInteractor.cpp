...
{
	ConfigCommands.Add(FChaosClothingInteractorConfigCommand::CreateLambda([bEnableLinearVelocityClamping, MaxLinearVelocity, bEnableLinearAccelerationClamping, MaxLinearAcceleration, bEnableAngularVelocityClamping, MaxAngularVelocity, bEnableAngularAccelerationClamping, MaxAngularAcceleration](Chaos::FClothingSimulationConfig* Config, int32 LODIndex)
		{
			static const FVector3f DefaultLinearClamp = { TNumericLimits<float>::Max(), TNumericLimits<float>::Max(), TNumericLimits<float>::Max() };
			Config->GetProperties(LODIndex).SetValue(TEXT("MaxLinearVelocity"), bEnableLinearVelocityClamping ? FVector3f(MaxLinearVelocity) : DefaultLinearClamp);
			Config->GetProperties(LODIndex).SetValue(TEXT("MaxLinearAcceleration"), bEnableLinearAccelerationClamping ? FVector3f(MaxLinearAcceleration) : DefaultLinearClamp);
			Config->GetProperties(LODIndex).SetValue(TEXT("MaxAngularVelocity"), bEnableAngularVelocityClamping ? MaxAngularVelocity : TNumericLimits<float>::Max());
			Config->GetProperties(LODIndex).SetValue(TEXT("MaxAngularAcceleration"), bEnableAngularAccelerationClamping ? MaxAngularAcceleration : TNumericLimits<float>::Max());
		}));
}
