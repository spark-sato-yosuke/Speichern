...
// Temporary hack to return to previous broken acceleration clamping behavior.
static bool bClothSolverBrokenAccelerationClampingToZero = false;
FAutoConsoleVariableRef CVarClothSolverBrokenAccelerationClampingHack(TEXT("p.ChaosCloth.Solver.BrokenAccelerationClampingHack"), bClothSolverBrokenAccelerationClampingToZero, TEXT("Return to previous broken behavior where acceleration clamping would clamp to zero velocity. This cvar will be removed."));

static bool bClothSolverEnableResetIfExtremeEdgeDeformation = true;
FAutoConsoleVariableRef CVarClothSolverEnableResetIfExtremeEdgeDeformation(TEXT("p.ChaosCloth.Solver.EnableResetIfExtremeEdgeDeformation"), bClothSolverEnableResetIfExtremeEdgeDeformation, TEXT("Set to false to disable reset to animation positions if there's any extreme edge deformation."));
...
namespace ClothingSimulationSolverDefault
{
	...
	static const bool bUseSelfCollisionSubstepsForSkinnedTriangleMeshes = true;
	...
	, bEnableSolver(true)
{
	...
	if (!bUseLegacySolver)
	{
			...
			[this](Softs::FSolverParticlesRange& ParticlesInput, const Softs::FSolverReal Dt, const Softs::FSolverReal LocalTime)
		{
			...
			const Softs::FSolverVec3* const AnimV = ParticlesInput.GetConstArrayView(AnimationVelocities).GetData();
			for (int32 Index = 0; Index < ParticlesInput.GetRangeSize(); ++Index)
			{
				if (PAndInvM[Index].InvM == (FSolverReal)0.)
				{
					...
					// This could be set per frame since we currently use linear interpolation, but it's cleaner to set here.
					V[Index] = AnimV[Index];
				}
			}
		});
			...
			[this](Softs::FSolverCollisionParticlesRange& ParticlesInput, const Softs::FSolverReal Dt, const Softs::FSolverReal LocalTime)
		{
			...
			const int32 CollisionRangeId = ParticlesInput.GetRangeId();
			if (FClothComplexColliders* const Colliders = GetComplexColliders(CollisionRangeId))
			{
				Colliders->KinematicUpdate(ParticlesInput, Dt, Alpha);
			}
		});
	}
	else
	{
			...
			[this](Softs::FSolverParticles& ParticlesInput, const Softs::FSolverReal Dt, const Softs::FSolverReal LocalTime, const int32 Index)
		{
			...
			// This could be set per frame since we currently use linear interpolation, but it's cleaner to set here.
			ParticlesInput.V(Index) = AnimationVelocities[Index];
		});
...
int32 FClothingSimulationSolver::AddCollisionParticles(int32 NumCollisionParticles, uint32 GroupId, int32 RecycledCollisionRangeId)
{
	...
	if (Evolution)
	{
		if (Evolution->IsValidCollisionParticleRange(RecycledCollisionRangeId))
		{
			...
			ComplexColliders.Remove(RecycledCollisionRangeId);
			const TArray<int32>& GroupSoftBodies = Evolution->GetGroupSoftBodies(GroupId);
			for (int32 SoftBodyId : GroupSoftBodies)
			{
				if (TUniquePtr<FClothConstraints>* ClothConstraints = ClothsConstraints.Find(SoftBodyId))
				{
					check(ClothConstraints);
					(*ClothConstraints)->OnCollisionRangeRemoved(RecycledCollisionRangeId);
				}
			}
		}
		if (NumCollisionParticles)
		{
			...
			// Add an empty ComplexColliders
			ComplexColliders.Emplace(CollisionRangeId, MakeUnique<FClothComplexColliders>(Evolution.Get(), CollisionRangeId));
		}
	}
	else
	{
	...
	if (CollisionRangeId != INDEX_NONE)
	{
...
void FClothingSimulationSolver::SetReferenceVelocityScale(uint32 GroupId,
	...
	TVec3<FReal>& InOutAppliedReferenceVelocity, // Old reference velocity is passed in. New reference velocity is returned.
	TVec3<FReal>& InOutAppliedReferenceAngularVelocity, // Old reference velocity is passed in. New reference velocity is returned.
...
)
{
	...
	const FVec3 OldAppliedReferenceVelocity(InOutAppliedReferenceVelocity); // In world space
	const FVec3 OldAppliedReferenceAngularVelocity(InOutAppliedReferenceAngularVelocity); // In world space
	...
	{	
		...
		if (MaxLinearAcceleration != TVec3<FRealSingle>(TNumericLimits<FRealSingle>::Max()))
		{
			...
			{
			case EChaosSoftsSimulationSpace::WorldSpace:
				OldScaledVelocity = OldAppliedReferenceVelocity;
				break;
			case EChaosSoftsSimulationSpace::ComponentSpace:
				OldScaledVelocity = LocalSpaceRotation.UnrotateVector(OldAppliedReferenceVelocity);
				break;
			case EChaosSoftsSimulationSpace::ReferenceBoneSpace:
			default:
				OldScaledVelocity = (OldReferenceSpaceRotationInverse * OldAppliedReferenceVelocity);
				break;
			}
		...
		// Temporary cvar to return to previous broken behavior where previous reference velocity wasn't stored.
		if (!bClothSolverBrokenAccelerationClampingToZero)
		{
			// Only update old reference velocity when ticking.
			switch (VelocityScaleSpace)
			{
			case EChaosSoftsSimulationSpace::WorldSpace:
				InOutAppliedReferenceVelocity = AppliedLinearVelocity;
				break;
			case EChaosSoftsSimulationSpace::ComponentSpace:
				InOutAppliedReferenceVelocity = LocalSpaceRotation.RotateVector(AppliedLinearVelocity);
				break;
			case EChaosSoftsSimulationSpace::ReferenceBoneSpace:
			default:
				InOutAppliedReferenceVelocity = OldReferenceSpaceTransform.GetRotation().RotateVector(AppliedLinearVelocity);
				break;
			}
		}
		...
		// Apply angular acceleration clamping
		if (MaxAngularAcceleration != TNumericLimits<FRealSingle>::Max() && MaxAngularAcceleration >= 0.f)
		{
			const FVec3 OldScaledAngularVelocity = OldAppliedReferenceAngularVelocity;
		...
		// Apply angular velocity clamping
		...
		// Only update old reference velocity when ticking.
		InOutAppliedReferenceAngularVelocity = AppliedAngularVelocity;
...
const Softs::FVelocityAndPressureField& FClothingSimulationSolver::GetWindVelocityAndPressureField(uint32 GroupId) const
{
	if (Evolution)
	{
		...
		if (auto It = ParticleRanges.CreateConstIterator(); It)
		{
			const int32 ParticleRangeId = *It;
...
void FClothingSimulationSolver::ApplyPreSimulationTransforms()
{
	...
	if (Evolution)
	{
		const TArray<uint32> ActiveGroups = Evolution->GetActiveGroupsArray();
		...
		for (const int32 SoftBodyId : ActiveSoftBodies)
		{
			...
			for (const int32 CollisionRangeId : ActiveCollisionRanges)
			{
				...
				if (FClothComplexColliders* const Colliders = GetComplexColliders(CollisionRangeId))
				{
					Colliders->ApplyPreSimulationTransforms(GroupSpaceTransform, DeltaLocalSpaceLocation, CollisionParticles.GetConstArrayView(OldCollisionTransforms),
						CollisionParticles.GetConstArrayView(CollisionTransforms), DeltaTime);
				}
			}
...
void FClothingSimulationSolver::PreSubstep( const Softs::FSolverReal InterpolationAlpha, bool bDetectSelfCollisions)
{
	...
	if (Evolution)
	{
		...
		PhysicsParallelFor(ActiveGroups.Num(), [this, &ActiveGroups, InterpolationAlpha, bDetectSelfCollisions](int32 ActiveGroupIndex)
		{
			...
			bool bDoTriangleMeshUpdate = false;
			...
			for (const int32 SoftBodyId : ActiveSoftBodies)
			{
				...
				FClothConstraints& Constraints = GetClothConstraints(SoftBodyId);

				Constraints.SetSkipSelfCollisionInit(!bDetectSelfCollisions);
				if (const TSharedPtr<Softs::FPBDSkinnedTriangleMeshCollisionConstraints>& TriMeshConstraint = Constraints.GetSkinnedTriangleCollisionsConstraint())
				{
					bDoTriangleMeshUpdate |= TriMeshConstraint->IsEnabled() && (!TriMeshConstraint->GetUseSelfCollisionSubstepsForSkinnedTriangleMeshes() || bDetectSelfCollisions);
				}
#if INTEL_ISPC
			...
			for (const int32 CollisionRangeId : ActiveCollisionRanges)
			{
				if (FClothComplexColliders* const Colliders = GetComplexColliders(CollisionRangeId))
				{
					Colliders->SetSkipSkinnedTriangleMeshKinematicUpdate(!bDoTriangleMeshUpdate);
				}
			}
...
void FClothingSimulationSolver::UpdateSolverField()
{
	if ((Evolution || PBDEvolution) && !PerSolverField.IsEmpty())
	{
		...
		if (Evolution)
		{
			...
			int32 SampleIndex = 0;
			for (const uint32 GroupId : Evolution->GetActiveGroupsArray())
			{
...
void FClothingSimulationSolver::Update(Softs::FSolverReal InDeltaTime)
{
	...
	// Update Cloths and cloth colliders
	{
		...
		for (TPair<int32, TUniquePtr<FClothComplexColliders>>& Collider : ComplexColliders)
		{
			if (Collider.Value)
			{
				Collider.Value->SwapBuffersForFrameFlip();
			}
		}
...
void FClothingSimulationSolver::ExtremeDeformationReset()
{
	if (bClothSolverEnableResetIfExtremeEdgeDeformation)
	{
		if (Evolution)
		{
			const TArray<uint32> ActiveGroups = Evolution->GetActiveGroupsArray();
			PhysicsParallelFor(ActiveGroups.Num(), [this, &ActiveGroups](int32 ActiveGroupIndex)
				{
					const uint32 GroupId = ActiveGroups[ActiveGroupIndex];
					const TSet<int32>& ActiveSoftBodies = Evolution->GetGroupActiveSoftBodies(GroupId);
					for (const int32 SoftBodyId : ActiveSoftBodies)
					{
						if (const Softs::FPBDExtremeDeformationConstraints* const ExtremeDeformationConstraints = GetClothConstraints(SoftBodyId).GetExtremeDeformationConstraints().Get())
						{
							if (ExtremeDeformationConstraints->IsExtremelyDeformed(GetParticleXsView(SoftBodyId)))
							{
								ResetStartPose(SoftBodyId, Evolution->GetSoftBodyParticleNum(SoftBodyId));
							}
						}
					}
				});
		...
		else
		{
			const TPBDActiveView<Softs::FSolverParticles>& ParticlesActiveView = PBDEvolution->ParticlesActiveView();
			ParticlesActiveView.RangeFor(
				[this](Softs::FSolverParticles& Particles, int32 Offset, int32 Range)
			{
				if (const Softs::FPBDExtremeDeformationConstraints* const ExtremeDeformationConstraints = GetClothConstraints(Offset).GetExtremeDeformationConstraints().Get())
				{
					if (ExtremeDeformationConstraints->IsExtremelyDeformed(GetParticleXs()))
					{
						ResetStartPose(Offset, PBDEvolution->GetParticleRangeSize(Offset));
					}
				}
			}, /*bForceSingleThreaded =*/ !bClothSolverParallelClothPostUpdate);
		}
...
FBoxSphereBounds FClothingSimulationSolver::CalculateBounds() const
{
	...
	if (Evolution)
	{
		...
		const TArray<uint32> ActiveGroupsArray = Evolution->GetActiveGroupsArray();
		for (const uint32 GroupId : ActiveGroupsArray)
		{
			for (const int32 SoftBodyId : Evolution->GetGroupActiveSoftBodies(GroupId))
			{
				const Softs::FSolverParticlesRange& Particles = Evolution->GetSoftBodyParticles(SoftBodyId);
