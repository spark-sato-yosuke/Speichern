...
public:
	...
	UFUNCTION(BlueprintCallable, Category = ClothingSimulation, Meta = (Keywords = "Chaos Max Linear Angular Velocity Acceleration Clamp"))
	CHAOSCLOTH_API void SetVelocityClamps(bool bEnableLinearVelocityClamping = false, FVector MaxLinearVelocity = FVector(1000., 1000., 1000.),
		bool bEnableLinearAccelerationClamping = false, FVector MaxLinearAcceleration = FVector(60000., 60000., 60000.),
		bool bEnableAngularVelocityClamping = false, float MaxAngularVelocity = 200.f,
		bool bEnableAngularAccelerationClamping = false, float MaxAngularAcceleration = 12000.f);
