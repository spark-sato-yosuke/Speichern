...
namespace Chaos
{
	...
	public:
		...
		const FClothComplexColliders* GetComplexColliders(int32 CollisionRangeId) const 
		{ 
			if (const TUniquePtr<FClothComplexColliders>* const Found = ComplexColliders.Find(CollisionRangeId))
			{
				return Found->Get();
			}
			return nullptr;
		}
		FClothComplexColliders* GetComplexColliders(int32 CollisionRangeId) 
		{
			if (TUniquePtr<FClothComplexColliders> const* Found = ComplexColliders.Find(CollisionRangeId))
			{
				return Found->Get();
			}
			return nullptr;
	...
	private:
		...
		// Extended collision data to support complex colliders (FEvolution only)
		TMap<int32, TUniquePtr<FClothComplexColliders>> ComplexColliders;
