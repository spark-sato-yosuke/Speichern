...
void SFilmOverlayOptions::Construct(const FArguments& InArgs, TSharedPtr<SFilmOverlay> InFilmOverlay)
{
	...
	auto& ExistingPrimaryOverlays = UFilmOverlayToolkit::GetPrimaryFilmOverlays();
	
#define PREPARE_PRIMARY_OVERLAY(Name, Construct) { TSharedPtr<IFilmOverlay> Overlay = ExistingPrimaryOverlays.FindRef(Name); \
	if (!Overlay) { UFilmOverlayToolkit::RegisterPrimaryFilmOverlay(Name, Construct); }}
...
#undef PREPARE_PRIMARY_OVERLAY
	...
	auto& ExistingToggleableOverplays = UFilmOverlayToolkit::GetToggleableFilmOverlays();

#define PREPARE_TOGGLEABLE_OVERLAY(Name, Construct) { TSharedPtr<IFilmOverlay> Overlay = ExistingToggleableOverplays.FindRef(Name); \
	if (!Overlay) { UFilmOverlayToolkit::RegisterToggleableFilmOverlay(Name, Construct); }}
...
const FSlateBrush* SFilmOverlayOptions::GetCurrentThumbnail() const
{
	...
	{
		return UFilmOverlayToolkit::GetPrimaryFilmOverlays()[PrimaryOverlay].Get()->GetThumbnail();
	}
