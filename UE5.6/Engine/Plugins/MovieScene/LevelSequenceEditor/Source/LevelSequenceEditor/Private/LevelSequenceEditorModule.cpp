...
protected:
	...
	void RegisterSequenceCustomizations()
	{
		...
		SequencerModule.GetSequencerCustomizationManager()->RegisterInstancedSequencerCustomization(ULevelSequence::StaticClass(),
				FOnGetSequencerCustomizationInstance::CreateLambda([this]()
				{
					UE::Sequencer::FLevelSequenceCustomization* Customization = new UE::Sequencer::FLevelSequenceCustomization();
					TArray<FOnGetSequencerCustomizationInstance> CustomizationInstances;
					AdditionalLevelSequenceEditorCustomizations.GenerateValueArray(CustomizationInstances);
					for (const FOnGetSequencerCustomizationInstance& CustomizationInstance : CustomizationInstances)
					{
						Customization->AddCustomization(TUniquePtr<ISequencerCustomization>(CustomizationInstance.Execute()));
					}
...
protected:
	...
	FGuid RegisterAdditionalLevelSequenceEditorCustomization(FOnGetSequencerCustomizationInstance OnGetInstance) override
	{
		FGuid Handle = FGuid::NewGuid();
		AdditionalLevelSequenceEditorCustomizations.Add(Handle, OnGetInstance);
		
		return Handle;
	}
	
	void UnregisterAdditionalLevelSequenceEditorCustomization(FGuid InHandle) override
	{
		AdditionalLevelSequenceEditorCustomizations.Remove(InHandle);
	}
...
private:
	...
	TMap<FGuid, FOnGetSequencerCustomizationInstance> AdditionalLevelSequenceEditorCustomizations;
};
