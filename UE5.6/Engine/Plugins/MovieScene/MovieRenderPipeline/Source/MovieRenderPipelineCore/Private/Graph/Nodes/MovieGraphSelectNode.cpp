...
TArray<UMovieGraphPin*> UMovieGraphSelectNode::EvaluatePinsToFollow(FMovieGraphEvaluationContext& InContext) const
{
	...
	if (UMovieGraphPin* SelectPin = GetInputPin(UE::MovieGraph::SelectNode::SelectedOption, EMovieGraphPinQueryRequirement::BuiltIn))
	{
		constexpr bool bFollowRerouteConnections = true;
		if (UMovieGraphPin* OtherPin = SelectPin->GetFirstConnectedPin(bFollowRerouteConnections))
		{
			FString ResolvedValue;
			bGotValueFromConnection = UE::MovieGraph::ResolveConnectedPinValue(SelectPin, {OtherPin}, InContext, ResolvedValue);

			if (bGotValueFromConnection)
			{
				ResolvedSelectedOption = NewObject<UMovieGraphValueContainer>();
				ResolvedSelectedOption->SetValueType(OtherPin->Properties.Type);
				ResolvedSelectedOption->SetValueTypeObject(OtherPin->Properties.TypeObject);
				ResolvedSelectedOption->SetValueSerializedString(ResolvedValue);
			}
		}
	}
