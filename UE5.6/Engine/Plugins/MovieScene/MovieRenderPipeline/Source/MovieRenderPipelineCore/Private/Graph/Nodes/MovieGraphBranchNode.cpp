...
TArray<UMovieGraphPin*> UMovieGraphBranchNode::EvaluatePinsToFollow(FMovieGraphEvaluationContext& InContext) const
{
	...
	if (ConditionalPin->Edges.Num() > 0)
	{
		UMovieGraphEdge* Edge = ConditionalPin->Edges[0];
		constexpr bool bFollowRerouteConnections = true;
		OtherPin = Edge->GetOtherPin(ConditionalPin, bFollowRerouteConnections);
	}
	...
	UE::MovieGraph::ResolveConnectedPinValue(ConditionalPin, {OtherPin}, InContext, ResolvedValue);
