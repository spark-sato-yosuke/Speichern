				...
				for (int32 Index = 0; Index < LeafNode->MovieScene->GetPossessableCount(); Index++)
				{
					...
					if (ValidBinding.IsValid())
					{
						...
						while (ParentGuid.IsValid())
						{
							...
							{
								// If the content is set up wrong and doesn't have a valid parent, we need to break out of the loop
								UE_LOG(LogMovieRenderPipeline, Error, TEXT("A camera binding was found while calculating multi-camera support, but the binding had an invalid parent (ParentGuid %s), skipping..."), *ParentGuid.ToString());
								break;
							}
						}
...
{
	return GetOverscannedResolution(InPrimaryConfig, InPipelineExecutorShot, DefaultOverscan);
}

FIntPoint UMoviePipelineBlueprintLibrary::GetDesiredOutputResolution(const UMoviePipelinePrimaryConfig* InPrimaryConfig)
{
	if (InPrimaryConfig)
	{
		const UMoviePipelineOutputSetting* OutputSetting = InPrimaryConfig->FindSetting<UMoviePipelineOutputSetting>();
		check(OutputSetting);
		return OutputSetting->OutputResolution;
	}

	return FIntPoint();
}

FIntPoint UMoviePipelineBlueprintLibrary::GetOverscannedResolution(const UMoviePipelinePrimaryConfig* InPrimaryConfig, const UMoviePipelineExecutorShot* InPipelineExecutorShot, float InDefaultOverscan)
{
	if (InPrimaryConfig && InPipelineExecutorShot)
	{
		const FIntPoint DesiredOutputResolution = GetDesiredOutputResolution(InPrimaryConfig);
		
		const UMoviePipelineCameraSetting* CameraSettings = Cast<const UMoviePipelineCameraSetting>(FindOrGetDefaultSettingForShot(UMoviePipelineCameraSetting::StaticClass(), InPrimaryConfig, InPipelineExecutorShot));
		check(CameraSettings);
		const float Overscan = CameraSettings->bOverrideCameraOverscan ? CameraSettings->OverscanPercentage : InDefaultOverscan;
		return UE::MoviePipeline::ScaleResolutionByOverscan(Overscan, DesiredOutputResolution);
	}

	return FIntPoint();
}

FIntPoint UMoviePipelineBlueprintLibrary::GetBackbufferResolution(const UMoviePipelinePrimaryConfig* InPrimaryConfig, const UMoviePipelineExecutorShot* InPipelineExecutorShot, float InDefaultOverscan)
{
	if (InPrimaryConfig && InPipelineExecutorShot)
	{
		const FIntPoint OverscannedResolution = GetOverscannedResolution(InPrimaryConfig, InPipelineExecutorShot, InDefaultOverscan);

		const UMoviePipelineHighResSetting* HighResSettings = Cast<const UMoviePipelineHighResSetting>(FindOrGetDefaultSettingForShot(UMoviePipelineHighResSetting::StaticClass(), InPrimaryConfig, InPipelineExecutorShot));
		check(HighResSettings);
		
		FIntPoint TileResolution = FIntPoint(
			FMath::CeilToInt((float)OverscannedResolution.X / (float)HighResSettings->TileCount),
			FMath::CeilToInt((float)OverscannedResolution.Y / (float)HighResSettings->TileCount));

		return HighResSettings->CalculatePaddedBackbufferSize(TileResolution);
	}

	return FIntPoint();
}

FIntRect UMoviePipelineBlueprintLibrary::GetOverscanCropRectangle(const UMoviePipelinePrimaryConfig* InPrimaryConfig, const UMoviePipelineExecutorShot* InPipelineExecutorShot, float DefaultOverscan)
{
	if (InPrimaryConfig && InPipelineExecutorShot)
	{
		const FIntPoint DesiredOutputResolution = GetDesiredOutputResolution(InPrimaryConfig);
		const FIntPoint OverscannedResolution = GetOverscannedResolution(InPrimaryConfig, InPipelineExecutorShot, DefaultOverscan);
