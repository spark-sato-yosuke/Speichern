...
{
	bool bIsAlreadyExposedProperty = ExposedPropertyInfo.ContainsByPredicate([PropertyName](const FMovieGraphPropertyInfo& ExposedProperty)
	{
		return ExposedProperty.Name == PropertyName;
	});
	
	// Both Promote and Unpromote will call Modify() and UpdatePins() on their own.
	if(bIsAlreadyExposedProperty)
	{
		UnpromotePropertyFromPin(PropertyName);
	}
	else
	{
		PromotePropertyToPin(PropertyName);
	}
}

void UMovieGraphNode::PromotePropertyToPin(const FName& PropertyName)
{
	Modify();
	...
	{
		ExposedPropertyInfo.AddUnique(MoveTemp(OverrideablePropertyInfo));
	}
	else
	{
...
void UMovieGraphNode::UnpromotePropertyFromPin(const FName& PropertyName)
{
	Modify();
	
	bool bIsExistingProperty = ExposedPropertyInfo.ContainsByPredicate([PropertyName](const FMovieGraphPropertyInfo& ExposedProperty)
	{
		return ExposedProperty.Name == PropertyName;
	});
	
	if(bIsExistingProperty)
	{
		// Remove the requested pin from our ExposedPropertyInfo array. We use ExposedPropertyInfo here (and not GetOverrideablePropertyInfo)
		// because this reflects the actual currently exposed pins, even if they're no longer something you could override. This allows you to
		// unpromote an old property.
		ExposedPropertyInfo.RemoveAll([&PropertyName](const FMovieGraphPropertyInfo& ExposedInfo)
		{
			return ExposedInfo.Name == PropertyName;
		});
	}
	else
	{
...
TArray<UMovieGraphPin*> UMovieGraphNode::EvaluatePinsToFollow(FMovieGraphEvaluationContext& InContext) const
{
	...
	// If this is a data (non-branch) connection, just return the first connected pin (following any reroute nodes).
	if (!InContext.PinBeingFollowed->Properties.bIsBranch)
	{
		const TObjectPtr<UMovieGraphNode> OwningNode = InContext.PinBeingFollowed->Node;
		...
		// the upstream connected data pin is. If this pin is on a reroute node, keep following the connection.
		if (InContext.PinBeingFollowed->IsInputPin() || (OwningNode && OwningNode->IsA<UMovieGraphRerouteNode>()))
		{
			constexpr bool bFollowRerouteConnections = true;
			if (UMovieGraphPin* ConnectedInputPin = InContext.PinBeingFollowed->GetFirstConnectedPin(bFollowRerouteConnections))
			{
