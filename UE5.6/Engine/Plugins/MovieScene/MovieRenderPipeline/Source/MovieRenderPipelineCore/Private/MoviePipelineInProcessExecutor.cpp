...
void UMoviePipelineInProcessExecutor::OnMapLoadFinished(UWorld* NewWorld)
{
	...
	// Force the pipeline to the default graph pipeline if we're using a graph and the pipeline isn't graph-compatible
	if (CurrentJob->IsUsingGraphConfiguration() && !MoviePipelineClass->IsChildOf(UMovieGraphPipeline::StaticClass()))
	{
