...
void UMovieGraphCommandLineEncoderNode::StartEncodingProcess(TArray<FMovieGraphRenderOutputData>& InGeneratedData, const bool bInIsShotEncode)
{
	...
	// Early out if there isn't any data to feed to the encoder. Could happen if it's running on a branch with no nodes that generate output (eg, JPG).
	if (InGeneratedData.IsEmpty())
	{
		UE_LOG(LogMovieRenderPipelineIO, Error, TEXT("The Command Line Encoder cannot run because no input files were generated for it to use."));
		CachedPipeline->RequestShutdown(true);
