...
public:
	...
	*/
	...
	const TMap<TWeakObjectPtr<UObject>, TArray<MoviePipeline::FClothSimSettingsCache>>& GetClothSimCache() const { return ClothSimCache; }

	/** Gets cached data for cloth simulations, needed to save/restore cloth sub-stepping. */
	TMap<TWeakObjectPtr<UObject>, TArray<MoviePipeline::FClothSimSettingsCache>>& GetClothSimCache() { return ClothSimCache; }
...
protected:
	 ...
	 * OnReceiveImageData() will be called on each of these nodes, and in turn they will typically call AddOutputFuture() on the pipeline to
	 * provide a future that communicates the status of their write task.
	 */
	virtual void ProcessOutstandingFinishedFrames();

	/**
	 * Processes any futures that have completed by updating GeneratedOutputData. Futures are processed sequentially in the order that they're
	 * generated. If the future next-in-line to be processed has not completed, this will early exit, and must be called again to continue
	 * processing futures. Optionally, all futures can be processed by setting "bStallAndProcessAllFutures" to true; this will still process
	 * all futures sequentially, but waits for the current one to finish before moving on to the next (and so on).
	 */
	virtual void ProcessOutstandingFutures(const bool bStallAndProcessAllFutures = false);
