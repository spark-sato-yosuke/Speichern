...
public:
	...
	virtual void ValidateStateImpl() override
	{
		...
		if (TileCount > 1)
		{
			ValidationResults.Add(NSLOCTEXT("MovieRenderPipeline", "HighRes_UnsupportedFeatures", "Tiling does not support all rendering features (bloom, some screen-space effects). Additionally, TAA is not supported and will be forced off when rendering. Use Spatial/Temporal sampling instead. Tiling does not support reference motion blur with the Path Tracer."));
...
public:
	...
	*/
	...
	/*
	* If true, allocate a unique history for each tile. This is needed to make some render features work, including auto-exposure and Lumen,
	* but should be disabled when dealing with extremely large resolutions as you will spend all of your GPU memory on history buffers.
	*/
	UPROPERTY(EditAnywhere, BlueprintReadWrite, Category = "Render Settings")
	bool bAllocateHistoryPerTile;

	/*
	* If true, persistent GPU data per high resolution tile is mirrored to system memory, allowing higher resolutions, but increasing render time.
	*/
	UPROPERTY(EditAnywhere, BlueprintReadWrite, Category = "Render Settings")
