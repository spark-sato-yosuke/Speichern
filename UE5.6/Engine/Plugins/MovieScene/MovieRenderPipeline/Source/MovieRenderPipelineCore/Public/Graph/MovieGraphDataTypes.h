...
public:
	...
	struct FSharedData
	{
		FSharedData()
			...
			, bHasTimeDilationBeenUsed(false)
		{}
		...
		*/
		...
		/**
		* If true, a frame rendered for this job used time dilation, and all subsequent frame numbers must be their relative versions
		* to avoid any frame overwriting an already produced one.
		*/
		bool bHasTimeDilationBeenUsed;
	};
...
struct FMovieGraphRenderPassLayerData
{
	...
	// The index of this layer in the list of layers being rendered (0 being the first layer)
	int32 LayerIndex = 0;

	// The number of cameras that are being rendered.
	uint32 NumCameras = 1;
...
namespace UE::MovieGraph
{
	...
	struct FMovieGraphOutputFutureData
	{
		...
		/** The "frame templated" filepath. This groups together a set of output frames by frame number, eg "Seq.Shot.{frame_placeholder}.exr". */
		FString FrameTemplatedFilePath;

		/** Which render pass is this for? */
		FMovieGraphRenderDataIdentifier DataIdentifier;

		/** The node class that this file originated from. */
		UClass* OriginNodeClass;

		/** The index of the render layer this future belongs to. Index 0 is the first render layer rendered, and so on. */
		int32 RenderLayerIndex;
	};
	...
	*/
	...
	struct FMovieGraphSampleState : IImagePixelDataPayload, public TSharedFromThis<FMovieGraphSampleState>
	{
		...
		/** The backbuffer resolution of this render resource (ie: the resolution the sample was rendered at). Includes overscan and tiling */
		FIntPoint BackbufferResolution;

		/** The size of a tile without padding (ie: OverscannedResolution/TileCount) */
		FIntPoint UnpaddedTileSize;
		...
		*/
		...
		/**
		* When bWriteSampleToDisk is true, if this string is specified then the single-sample debug writing will use this filename for the format
		* token instead of the default one. Useful for render passes with complex output cases (like Panoramics) that need pass-specific names.
		*/
		...
		*/
		...
		/** The index of the render layer this sample belongs to. Index 0 is the first render layer rendered, and so on. */
		int32 RenderLayerIndex;
