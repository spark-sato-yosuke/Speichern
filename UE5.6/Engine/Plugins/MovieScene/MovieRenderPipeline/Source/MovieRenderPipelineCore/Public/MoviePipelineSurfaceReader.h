...
public:
	...
	/** Updates the stored last used frame to be the current frame */
	void UpdateLastUsedFrame();

	/** Returns true if the amount of frames between the current frame and the last used frame is greater than a specific threshold */
	bool IsStale() const;
...
private:
	...
	/** The last frame this surface was used. Surfaces that haven't been used for a certain number of frames are cleaned up */
