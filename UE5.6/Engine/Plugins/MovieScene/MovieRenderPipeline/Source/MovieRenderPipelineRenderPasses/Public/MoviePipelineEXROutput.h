...
namespace UE
{
	...
	{
		/** CVar indicating if the EXR file writer is allowed to inflate layers of a lower resolution */
		extern TAutoConsoleVariable<bool> CVarMoviePipelinePadLayersForMultiPartEXR;
...
public:
	...
	/** Crop rectangle used to set the displayWindow for EXR output. Used to crop out overscan */
	FIntRect CropRectangle;
	
	/** Color space chromaticity metadata. */
	TArray<FVector2d> ColorSpaceChromaticities;

	/** Indicates if a multi-part file should be used, which allows each channel to have its own header and properties */
	bool bMultipart;
	
	/** Indicates that any layers that do not match the base resolution of the EXR file should be inflated with empty pixels to that resolution */
	bool bPadToDataWindowSize;
	...
	FEXRImageWriteTask()
		...
		, CropRectangle(FIntRect())
		, bMultipart(true)
		, bPadToDataWindowSize(true)
	{}
...
private:
	/** EXR file parts and their metadata (scanline count, bytes written) */
	struct FEXRPart
	{
		Imf::Header Header;
	...
	/** Generates EXR file parts (header, framebuffer, etc) for the specified layers at the specified resolution */
	bool GenerateFilePartsForResolution(const FIntPoint& Resolution, const TArray<FImagePixelData*>& InLayers, const FString& InPartName, FEXRPart& OutPart, TArray<TUniquePtr<FImagePixelData>>& OutQuantizedLayers);
...
public:
	...
	*/
	 ...
	 * Multi-part exr files are a feature of EXR 2.0 and may not be supported by all software. If the console variable 'MoviePipeline.PadLayersForMultiPartEXR' is enabled,
