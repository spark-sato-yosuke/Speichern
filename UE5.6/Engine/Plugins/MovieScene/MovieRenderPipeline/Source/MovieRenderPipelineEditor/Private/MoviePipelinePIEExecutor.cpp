...
void UMoviePipelinePIEExecutor::SetAllowUsingUnsavedLevels(const bool bShouldAllow)
{
	UE_LOG(LogMovieRenderPipeline, Log, TEXT("Updating the PIE Executor to allow using unsaved levels."));
	
	bAllowUnsavedLevels = bShouldAllow;
}
...
void UMoviePipelinePIEExecutor::Start(const UMoviePipelineExecutorJob* InJob)
{
	...
	// Note that opting-in to allowing unsaved levels is permitted (Quick Render is a good example of when this is OK).
	const bool bAllMapsValid = UMoviePipelineEditorBlueprintLibrary::IsMapValidForRemoteRender(Queue->GetJobs());
	if (!bAllMapsValid && !bAllowUnsavedLevels)
	{
		if(!IsRenderingOffscreen())
		{
	...
	if (ExecutorSettings->bResizePIEWindowToOutputResolution)
	{
		...
		{
			// Like the GameMode below, this isn't a perfect evaluation, and is further complicated by things like Adapt Output Resolution and Overscan
			// but it should work in the basic case.
			...
			if (UMovieGraphEvaluatedConfig* EvaluatedGraph = GraphConfig->CreateFlattenedGraph(TraversalContext, OutTraversalError))
			{
				WindowSize = UMovieGraphBlueprintLibrary::GetOverscannedResolution(EvaluatedGraph);
			}
			else
			{
				UE_LOG(LogMovieRenderPipeline, Warning, TEXT("Failed to evaluate graph to determine output resolution. Traversal error: %s"), *OutTraversalError);
			}
		}
		else
		{
...
void UMoviePipelinePIEExecutor::OnPIEStartupFinished(bool)
{
	...
	// Ensure the pipeline class we're using is compatible with the job's configuration type
	const UMoviePipelineExecutorJob* CurrentJob = Queue->GetJobs()[CurrentPipelineIndex];
	const bool bIsUsingGraph = CurrentJob->IsUsingGraphConfiguration();
	if (bIsUsingGraph && !PipelineClass->IsChildOf(UMovieGraphPipeline::StaticClass()))
	{
		UE_LOG(LogMovieRenderPipeline, Log, TEXT("Using the default MovieGraphPipeline; the pipeline specified in Project Settings (%s) is not compatible with graphs."), *PipelineClass->GetName());
		PipelineClass = UMovieGraphPipeline::StaticClass();
	}
	else if (!bIsUsingGraph && !PipelineClass->IsChildOf(UMoviePipeline::StaticClass()))
	{
		UE_LOG(LogMovieRenderPipeline, Log, TEXT("Using the default MoviePipeline; the pipeline specified in Project Settings (%s) is not compatible with normal (non-graph) presets."), *PipelineClass->GetName());
		PipelineClass = UMoviePipeline::StaticClass();
	}
...
void UMoviePipelinePIEExecutor::OnPIEEnded(bool)
{
	...
	// Cache off the output data so we can use it in DelayedFinishNotification, at which point ActiveMoviePipeline is null.	
	...
	else if (UMovieGraphPipeline* GraphPipeline = Cast<UMovieGraphPipeline>(ActiveMoviePipeline))
	{
		// We only set GraphData for the graph pipeline
		CachedOutputDataParams.GraphData = GraphPipeline->GetGeneratedOutputData();
	}
