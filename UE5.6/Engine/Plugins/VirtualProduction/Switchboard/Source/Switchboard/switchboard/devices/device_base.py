...
from switchboard.config import CONFIG, BoolSetting, AddressSetting, Setting
...
class DeviceQtHandler(QtCore.QObject):
    ...
    signal_device_built_changelist_changed = QtCore.Signal(object)
...
class Device(QtCore.QObject):
    ...
    def init(self, widget_class, icons):
        ...
        self.widget = widget_class(self.name, self.device_hash, self.address, icons)

        self.widget.signal_device_name_changed.connect(self.on_device_name_changed)
        self.widget.signal_address_changed.connect(self.on_address_changed)

        # weakref to void an unintended strong reference that can prevent garbage collection

        self_weak_ref = weakref.ref(self)

        def on_address_changed_weak(old_address, new_address):
            self_instance = self_weak_ref()
            if self_instance is not None and self_instance.widget is not None:
                self_instance.widget.on_address_changed(new_address)

        self.setting_address.signal_setting_changed.connect(on_address_changed_weak)

        self.disable_override_style_on_device_only_settings()
    ...
    def disable_override_style_on_device_only_settings(self):
        ''' If a given setting does not exist in the plugin settings, then there is
        no point in supporting override highlights. If we don't do this, we may get
        override highlights on device instance properties that don't have a plugin
        value.
        '''
        for setting in self.device_settings():
            if setting.attr_name not in self.plugin_settings():
                setting.allow_override_style = False
