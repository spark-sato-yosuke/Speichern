...
from switchboard.devices.device_widget_base import DeviceAutoJoinMUServerUI
...
class DeviceListWidget(QtWidgets.QListWidget):
    ...
    def connect_to_device_autojoin(self, device, category):
        ''' Connects to the device autojoin setting with a weak reference to avoid strong reference cycles. '''
        self_weak_ref = weakref.ref(self)

        def child_device_changed_weak():
            self_instance = self_weak_ref()
            if self_instance is not None:
                self_instance.update_header_autojoin_state(category)

        if device.autojoin_mu_server:
            device.autojoin_mu_server.signal_setting_changed.connect(child_device_changed_weak)
            self.update_header_autojoin_state(category)
