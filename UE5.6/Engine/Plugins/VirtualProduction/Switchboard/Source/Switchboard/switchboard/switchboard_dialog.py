...
from switchboard import switchboard_logging
...
from switchboard.ui.switchboard_ui import Ui_switchboard_window

import switchboard.debug_utils as debug_utils
...
class SwitchboardDialog(QtCore.QObject):
    ...
    def __init__(self, script_manager):
        ...
        self.window = Ui_switchboard_window()
        ...
        # Debugging menu item
        if debug_utils.ADD_MENU:
            self.debug_menu = self.window.menu_bar.addMenu("&Debug")
            debug_utils.register_menu_actions(self.debug_menu)
    ...
    def _on_menu_load_config_about_to_show(self):
        ...
        menu_hierarchy = {
            config.CONFIG_MGR.config_dir: self.window.menu_load_config
        }

        def _get_menu_for_path(path):
            # Safe guard to make sure we don't accidentally traverse up past
            # the root configs path.
            if (path != config.CONFIG_MGR.config_dir and
                    config.CONFIG_MGR.config_dir not in path.parents):
                return None
        ...
        config_paths = config.CONFIG_MGR.list_config_paths()
    ...
    def _on_open_external_config(self):
        ...
        config_path, _ = QtWidgets.QFileDialog.getOpenFileName(
            self.window,
            'Select config file', str(config.CONFIG_MGR.config_dir),
    ...
    def refresh_window_title(self):
        ...
        buildver_data = ugs_utils.get_sb_buildver()
        buildver = [
            (f"{buildver_data.get('MajorVersion', '?')}"
             f".{buildver_data.get('MinorVersion', '?')}"
             f".{buildver_data.get('PatchVersion', '?')}"),
            str(buildver_data.get('Changelist', '(unknown CL)')),
            str(buildver_data.get('BranchName', '(unknown branch)'))
        ]
    ...
    def menu_save_config_as(self):
        ...
        new_config_path, _ = QtWidgets.QFileDialog.getSaveFileName(
            self.window,
            'Select config file', str(config.CONFIG_MGR.config_dir),
            f'Config files (*{config.CONFIG_SUFFIX})'
        )
    ...
    def menu_delete_config(self):
        ...
        rel_config_path = config.CONFIG_MGR.get_relative_config_path(SETTINGS.CONFIG)
    ...
    def _sync_build_all(self, *, do_sync: bool, do_build: bool):
        ...
        for device in self.device_list_widget.devices():
            ...
            engine_workspace = CONFIG.SOURCE_CONTROL_WORKSPACE.get_value(device.name).casefold()
            project_workspace = CONFIG.PROJECT_WORKSPACE.get_value(device.name).casefold()

            workspaces = [ws for ws in [engine_workspace, project_workspace] if ws]

            if len(workspaces) == 0:
                LOGGER.info(f'Skipping {actions_str} for {device.name} because it has empty workspaces')
                continue
    ...
    def device_added(self, device):
        ...
        device.device_qt_handler.signal_device_built_changelist_changed.connect(self.device_built_changelist_changed, QtCore.Qt.QueuedConnection)
    ...
    def _set_project_changelist(self, value):
        ...
        if self.window.project_cl_combo_box.currentText() != self._project_changelist:
            self.window.project_cl_combo_box.setCurrentText(self._project_changelist)
    ...
    def _set_engine_changelist(self, changelist_value):
        ...
        if self.window.engine_cl_combo_box.currentText() != self._engine_changelist:
            self.window.engine_cl_combo_box.setCurrentText(self._engine_changelist)
    ...
    def device_widget_sync(self, device_widget):
        ...
        workspace = CONFIG.SOURCE_CONTROL_WORKSPACE.get_value(device.name)
        project_workspace = CONFIG.PROJECT_WORKSPACE.get_value(device.name)

        # If the project workspace is separate and there is a target project_cl, then we need to issue separate
        # sync commands for engine and project.
        if project_cl and project_workspace and (project_workspace != workspace):
            puuid = device.sync(engine_cl=engine_cl, project_cl=None)
            device.sync(engine_cl=None, project_cl=project_cl, puuid_dependency=puuid)
        else:
            # Specify engine and project together because they are in the same workspace
    ...
    @QtCore.Slot(object)
    def device_built_changelist_changed(self, device):
        device_widget = self.device_list_widget.device_widget_by_hash(device.device_hash)
        device_widget.update_build_info(device)
    ...
    def logger_level_comboBox_currentTextChanged(self):
        ...
        if value == 'Message':
            switchboard_logging.QT_HANDLER.setLevel(switchboard_logging.SBLevels.MESSAGE)
        elif value == 'OSC':
            switchboard_logging.QT_HANDLER.setLevel(switchboard_logging.SBLevels.OSC)
        elif value == 'Debug':
            switchboard_logging.QT_HANDLER.setLevel(logging.DEBUG)
        else:
            switchboard_logging.QT_HANDLER.setLevel(logging.INFO)
    ...
    def p4_refresh_project_cl(self):
        ...
        project_client = CONFIG.PROJECT_WORKSPACE.get_value()
        ...
        if use_ugs:
            ...
            changelists = ugs_utils.latest_chagelists(Path(CONFIG.UPROJECT_PATH.get_value()), client=client)
            if not changelists:
    ...
    def refresh_levels(self, levels: Optional[List[str]] = None):
        ...
        # Disable CONFIG saving because _update_level_list will trigger saves when updating
        # the levels combobox even when the level selection hasn't change
        CONFIG.push_saving_allowed(False)

        # try-finally ensures that pop_saving_allowed happens in the case of exceptionsS
        try:
        ...
        # if the current level changed, save the config
