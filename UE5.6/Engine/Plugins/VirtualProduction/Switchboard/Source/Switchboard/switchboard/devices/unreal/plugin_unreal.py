...
from switchboard.config import (
    CONFIG, DEFAULT_MAP_TEXT, ENABLE_UGS_SUPPORT, SETTINGS, BoolSetting, DirectoryPathSetting,
...
class ProgramStartQueue:
    ...
    def __init__(self):
        ...
        self.rd_puuid_by_name: dict[str, uuid.UUID] = dict()
    ...
    def puuid_from_name(self, name: str) -> uuid.UUID:
        '''
...
class SyncCategoryOption:
    ...
    def __init__(self, catid: Union[str, uuid.UUID], name: str):
        if isinstance(catid, str):
            catid = uuid.UUID(catid)
...
class DeviceUnreal(Device):
    ...
    csettings: dict[str, Setting] = {
        ...
        'retrieve_logs': BoolSetting(
            ...
            value=True,
            tool_tip='When checked, retrieves the logs and traces after Unreal Engine terminates.',
            category="UE Settings",
        ),
        'livelink_preset': LiveLinkPresetSetting(
            ...
            value='',
            tool_tip='Adds the selected LiveLink preset to the command line',
            category="Tools Settings",
        ),
        ...
        'included_sync_categories': MultiOptionSetting(
            ...
            lambda val_list: [(str(x.catid), x.name) for x in val_list]
        ...
        'force_holdout_if_ndisplay': BoolSetting(
            attr_name='force_holdout_if_ndisplay',
            nice_name='Force nDisplay Editor Holdout',
            value=True,
            tool_tip=(
                'When checked, force-enables Primitive Alpha Holdout on Editor launches when there are nDisplay devices in '
                'this Switchboard configuration \n'),
            category="UE Settings",
        ),
        'editor_target_override': StringSetting(
            attr_name='editor_target_override',
            nice_name='Editor Target Name Override',
            value='',
            placeholder_text='$(ProjectName)Editor',
            tool_tip=(
                "The name of your project's editor target. Typically $(ProjectName)Editor (which is inferred "
                "automatically). This override only needs to be set if deviating from the standard convention."),
            category='UE Settings',
        )
    }
    ...
    def __init__(self, name, address, **kwargs):
        ...
        osc_port_setting.signal_setting_changed.connect(
            lambda: weak_self() and weak_self()._check_recreate_osc_client()
        )
        ...
        # The CLs returned via p4 cstat query, or optimistically
        # updated with the "inflight" values upon successful sync
        # (Defined in the base class, documented here for context)
        #self._project_changelist: Optional[int] = None
        #self._engine_changelist: Optional[int] = None

        # The CLs for which outstanding p4 syncs have been queued
        self.inflight_project_cl = None
        self.inflight_engine_cl = None
        self.inflight_sync_hash: Optional[str] = None

        # The fields read from Build.version JSON
        # (Written during sync by UGS/sbl_helper.py)
        self._synced_changelist: Optional[int] = None
        self._synced_compatible_cl: Optional[int] = None

        # The fields read from UnrealEditor.version JSON
        # (Written by UBT following a successful build of ANY project)
        self._built_eng_changelist: Optional[int] = None
        self._built_eng_compatible_cl: Optional[int] = None

        # The fields read from (Project)Editor.target JSON
        # (Written by UBT following a successful build of the active project)
        self._built_proj_changelist: Optional[int] = None
        self._built_proj_compatible_cl: Optional[int] = None
    ...
    def init(self, widget_class, icons):
        ...
        self.exclude_from_build.signal_setting_changed.connect(
            lambda _, new_value: weak_self() and weak_self().on_setting_exclude_from_build_changed(new_value)
        )
        ...
        self.exclude_from_insights.signal_setting_changed.connect(
            lambda _, new_value: weak_self() and weak_self().on_setting_exclude_from_insights_changed(new_value)
        )
    ...
    def on_engine_sync_method_changed(self, _, engine_sync_method):
        if engine_sync_method == EngineSyncMethod.Build_Engine.value:
            ...
            self.widget.update_build_info(self)
    ...
    def _request_roles_file(self):
        ...
        if not roles_filename:
            LOGGER.warning(f'{self.name}: Roles filename is unset; not requesting')
            return

        uproject_path: str = CONFIG.UPROJECT_PATH.get_value(self.name).replace('"', '')
        uproject_dir = os.path.dirname(uproject_path)
        roles_file_path = os.path.join(uproject_dir, "Config", "Tags", roles_filename)
        _, msg = message_protocol.create_copy_file_from_listener_message(roles_file_path)
        self.unreal_client.send_message(msg)

    def _request_version_files(self):
        '''
        Requests the Build.version, UnrealEditor.version and $(Proj)Editor.target files.
        On success, _on_receive_version_file is called with the file content.
        '''
        engine_dir = CONFIG.ENGINE_DIR.get_value(self.name)
        uproj_path = CONFIG.UPROJECT_PATH.get_value(self.name)
        proj_dir = os.path.dirname(uproj_path)
        platform_dir = self.platform_binary_directory

        proj_name, _ = os.path.splitext(os.path.basename(uproj_path))
        editor_target = DeviceUnreal.csettings['editor_target_override'].get_value()
        if not editor_target:
            editor_target = f'{proj_name}Editor'

        build_version_path = os.path.join(engine_dir, 'Build', 'Build.version')
        _, msg = message_protocol.create_copy_file_from_listener_message(build_version_path)
        self.unreal_client.send_message(msg)

        editor_version_path = os.path.join(engine_dir, 'Binaries', platform_dir,
                                           'UnrealEditor.version')
        _, msg = message_protocol.create_copy_file_from_listener_message(editor_version_path)
        self.unreal_client.send_message(msg)

        proj_target_path = os.path.join(proj_dir, 'Binaries', platform_dir,
                                        f'{editor_target}.target')
        _, msg = message_protocol.create_copy_file_from_listener_message(proj_target_path)
        self.unreal_client.send_message(msg)
    ...
    def _request_project_changelist_number(self):
        ...
        project_workspace = CONFIG.PROJECT_WORKSPACE.get_value(self.name)

        if project_workspace:
            client_name = project_workspace
    ...
    def sync(
            self,
            engine_cl: Optional[int],
            project_cl: Optional[int],
        ...
        # If only we're syncing the project and the project has a specified workspace, use the project workspace
        # instead.
        if not engine_cl and project_cl and project_workspace:
            workspace = project_workspace

        program_name = UnrealJobs.Sync.value

        # If there are no task dependencies, check if it is already on its way:
        if not puuid_dependency:
            try:
        ...
        if engine_cl:
            sync_args.append(f'--engine-dir="{engine_dir}"')

        sync_args = ' '.join(sync_args)
        ...
        if ENABLE_UGS_SUPPORT:
            ...
            if sync_precompiled_bins:
                ...
                # If we're syncing 'Precompiled Binaries' one of those binaries may be the SwitchboardListener
                # executable which means (on Windows atleast) we need to move the executable to make way for the new one
    ...
    def build(self):
        ...
        # This should only be possible when invoked on individual devices
        # manually; SwitchboardDialog._sync_build_all already only invokes
        # this method for a single device per workspace.
        for device in self.devices_sharing_workspace():
            if device.status >= DeviceStatus.BUILDING:
                LOGGER.warning(f'{self.name}: Unable to build, because '
                               f'another device ({device.name}) using the '
                               'same workspace is already running/building.')
                return
        ...
        # We've picked one device to act as build representative for our workspace;
        # however, we want to let the others weigh in, especially derived classes.
        for device in self.devices_sharing_workspace(only_connected=False):
            puuid_dependency = device._queue_all_builds(self, puuid_dependency)

    def _queue_all_builds(
        self,
        requesting_device: DeviceUnreal,
        puuid_dependency: uuid.UUID | None = None,
    ) -> uuid.UUID | None:
        '''
        Customization point for derived classes to customize build actions.

        The passed-in `requesting_device` may be a different device, sharing our workspace, to use as a proxy.

        Adding "redundant" actions should be safe; `_queue_build` (and subsequently the various `_build_*` wrapper
        methods) will early out when attempting to queue a second build with the same name.
        '''
        ...
        if (self.is_designated_local_builder() and
            ...
            if CONFIG.MUSERVER_AUTO_BUILD.get_value():
                ...
                puuid_dependency = requesting_device._build_mu_server(puuid_dependency)
                puuid_dependency = requesting_device._build_mu_slate_server(puuid_dependency)
            ...
            #     puuid_dependency = requesting_device._build_listener(puuid_dependency)

        puuid_dependency = requesting_device._build_shadercompileworker(puuid_dependency)
        puuid_dependency = requesting_device._build_project(puuid_dependency)

        return puuid_dependency
    ...
    def _build_project(self, puuid_dependency: Optional[uuid.UUID] = None) -> uuid.UUID | None:
        ubt_args = (
        ...
        return self._queue_build('project', ubt_args, puuid_dependency)

    def _build_mu_server(self, puuid_dependency: Optional[uuid.UUID] = None) -> uuid.UUID | None:
        ubt_args = f'UnrealMultiUserServer {self.target_platform} Development -Progress'
        return self._queue_build('mu_server', ubt_args, puuid_dependency)

    def _build_mu_slate_server(self, puuid_dependency: Optional[uuid.UUID] = None) -> uuid.UUID | None:
        ubt_args = f'UnrealMultiUserSlateServer {self.target_platform} Development -Progress'
        return self._queue_build('mu_slate_server', ubt_args, puuid_dependency)

    def _build_listener(self, puuid_dependency: Optional[uuid.UUID] = None) -> uuid.UUID | None:
        ubt_args = f'SwitchboardListener {self.target_platform} Development -Progress'
        return self._queue_build('listener', ubt_args, puuid_dependency)

    def _build_shadercompileworker(self, puuid_dependency: Optional[uuid.UUID] = None) -> uuid.UUID | None:
        ubt_args = f'ShaderCompileWorker {self.target_platform} Development -Progress'
        return self._queue_build('shadercw', ubt_args, puuid_dependency)

    def _queue_build(
        self,
        program_name_suffix: str,
        ubt_args: str,
        puuid_dependency: Optional[uuid.UUID] = None,
    ) -> uuid.UUID | None:
        '''
        Adds a UBT invocation to the queue as "build_{program_name_suffix}",
        but only if an entry with that name does not already exist.

        If no new build was queued, returns the `puuid_dependency` which was
        passed in, in order to make chaining calls more convenient.
        '''
        ...
        try:
            existing_puuid = self.program_start_queue.puuid_from_name(program_name)
            LOGGER.debug(f'{self.name}: Not queuing {program_name}; exists as {existing_puuid}')
            return puuid_dependency
        except KeyError:
            pass
    ...
    def conform_dpcvars(self, cvars: list[str]) -> list[str]:
        ''' Conforms input cvar to expected format for command line (key=value)
        It will allow the user to separate key/value using space(s), like one would in Unreal's command input field.
        '''
        ocvars = []

        disallowed_chars = [
            ',',  # Comma is used as a dpcvar delimiter.
        ]
        ...
        for cvar in cvars:
            ...
            # Split by '=' (allow surround by spaces) or 1+ spaces
            match = re.split(r'\s*=\s*|\s{1,}', cvar, maxsplit=1)

            if len(match) != 2:
                LOGGER.warning(f"Ignored cvar '{cvar}' because it could not be parsed")
                continue
            ...
            if not key or not value:
                LOGGER.warning(f"Ignored cvar '{cvar}' because it has an empty key or value")
                continue

            cvar = f"{key}={value}"

            # Check for disallowed characters
            if (any(char in cvar for char in disallowed_chars)):
                LOGGER.warning(f"Ignored cvar '{cvar}' because it contained one or more invalid characters ({disallowed_chars})")
                continue
    ...
    def generate_unreal_command_line_args(self, map_name):
        ...
        # Holdout composite on Editors if this is a Switchboard config with nDisplay devices.
        if DeviceUnreal.csettings["force_holdout_if_ndisplay"].get_value(self.name):
            if any(device.device_type == "nDisplay" for device in self.active_unreal_devices):
                dp_cvars.extend([
                    'r.PostProcessing.PropagateAlpha=1',
                    'r.Deferred.SupportPrimitiveAlphaHoldout=1'
                ])

        # Add user set dp cvars, overriding any of the forced ones.
        user_dp_cvars = self.conform_dpcvars(DeviceUnreal.csettings["dp_cvars"].get_value(self.name))
    ...
    def do_program_running_update(self, prog):
        if prog.name == UnrealJobs.Unreal.value:
            ...
            for device in self.devices_sharing_workspace():
                ...
                # Reset the progress.
                device.device_qt_handler.signal_device_sync_update.emit(device, "")
        ...
        *,
        ...
        if program_name == UnrealJobs.Unreal.value and not len(remaining_homonyms):
            ...
            if CONFIG.ENGINE_SYNC_METHOD.get_value() == EngineSyncMethod.Build_Engine.value:
                self._request_version_files()
                if self.inflight_engine_cl:
                    self.engine_changelist = self.inflight_engine_cl
                    self.inflight_engine_cl = None

            # Update CL with the one in flight
            if self.inflight_project_cl:
            ...
            remaining_build_names = list(filter(lambda name: name.startswith(UnrealJobs.BuildPrefix.value),
                                                self.program_start_queue.rd_puuid_by_name.keys()))

            if not remaining_build_names:
                for device in self.devices_sharing_workspace():
                    device.status = DeviceStatus.CLOSED
    ...
    def on_file_received(self, source_path, content):
        if source_path.endswith('.version') or source_path.endswith('.target'):
            self._on_receive_version_file(source_path, content)
            return

        roles_filename = DeviceUnreal.csettings["roles_filename"].get_value(self.name)
        if roles_filename and source_path.endswith(roles_filename):
    ...
    def _on_receive_version_file(self, source_path: str, content):
        '''
        Handles .version/.target files (see _request_version_files)
        '''
        decoded_content = base64.b64decode(content).decode()
        ...
        compatible_cl = data.get('CompatibleChangelist')

        for device in self.devices_sharing_workspace():
            if source_path.endswith('Build.version'):
                device._synced_changelist = changelist
                device._synced_compatible_cl = compatible_cl

                # Warn the user if the remote workspace points to a different branch
                sb_buildver = ugs_utils.get_sb_buildver()
                local_branch = sb_buildver.get('BranchName')
                remote_branch = data.get('BranchName')
                if local_branch is None or local_branch != remote_branch:
                    LOGGER.warn(f'{self.name}: Workspace BranchName differs! ({remote_branch})')

            elif source_path.endswith('UnrealEditor.version'):
                device._built_eng_changelist = changelist
                device._built_eng_compatible_cl = compatible_cl
            elif source_path.endswith('.target'):
                device._built_proj_changelist = changelist
                device._built_proj_compatible_cl = compatible_cl

        self.device_qt_handler.signal_device_built_changelist_changed.emit(self)
    ...
    def on_file_receive_failed(self, source_path, error):
        ...
        LOGGER.error(f"{self.name}: Error retrieving file {source_path}: {error}")
    ...
    def devices_sharing_workspace(
        ...
        def get_ws(device):
            return (CONFIG.SOURCE_CONTROL_WORKSPACE.get_value(device.name) or '').casefold()

        def get_prj_ws(device):
            return (CONFIG.PROJECT_WORKSPACE.get_value(device.name) or '').casefold()

        self_ws = get_ws(self)
        self_prj_ws = get_prj_ws(self)

        def predicate(device: DeviceUnreal):
            if self_ws or self_prj_ws:
                same_ws = (get_ws(device) == self_ws) if self_ws else False
                same_prj_ws = (get_prj_ws(device) == self_prj_ws) if self_prj_ws else False
                return same_ws or same_prj_ws
            else:
    ...
    def fetch_file(self, remote_path, dest_filename = ""):
        """
        Copies a remote file to the local machine. Optionally renames
        the file if a new name is provided.
        """
        ...
        dest_path = f'rsync://{dest_endpoint}/{dest_module}/{dest_filename}'
...
class DeviceWidgetUnreal(DeviceWidget):
    ...
    def _add_control_buttons(self):
        ...
        self.engine_changelist_label.setTextInteractionFlags(QtCore.Qt.TextSelectableByMouse)
        ...
        self.project_changelist_label.setTextInteractionFlags(QtCore.Qt.TextSelectableByMouse)
        ...
        self.open_button = self.add_control_button(
            icon_size=CONTROL_BUTTON_ICON_SIZE,
            tool_tip=f'Start {self.app_name}',
            hover_focus=False,
            name='open')
    ...
    def update_status(self, status, previous_status):
        ...
        if self.open_button.isChecked():
            if self.open_button.isEnabled():
                self.open_button.setToolTip(f'Stop {self.app_name}')
            else:
                self.open_button.setToolTip(f'Stopping {self.app_name}...')
        else:
            self.open_button.setToolTip(f'Start {self.app_name}')
    ...
    def update_engine_changelist(
        ...
        synced_cl = device.engine_changelist

        self.engine_changelist_label.setText(f'E: {synced_cl}')
        ...
        is_synched = required_cl is None or required_cl == synced_cl
        self._set_engine_changelist_is_synched(is_synched)
        self.update_build_info(device)
    ...
    def update_build_info(
        ...
        if CONFIG.ENGINE_SYNC_METHOD.get_value() == EngineSyncMethod.Build_Engine.value:
            eng_differs = device._built_eng_changelist != device._synced_changelist
            eng_code_differs = device._built_eng_compatible_cl != device._synced_compatible_cl
            if (eng_differs or eng_code_differs):
                self._needs_rebuild = True
                details += (f"\nBuilt engine CL: {device._built_eng_changelist}"
                            f" (Code: {device._built_eng_compatible_cl})")

            proj_differs = device._built_proj_changelist != device._synced_changelist
            proj_code_differs = device._built_proj_compatible_cl != device._synced_compatible_cl
            if (proj_differs or proj_code_differs):
                self._needs_rebuild = True
                details += (f"\nBuilt project CL: {device._built_proj_changelist}"
                            f" (Code: {device._built_proj_compatible_cl})")

        desired_tooltip = 'Build changelist'

        if self._needs_rebuild:
            desired_tooltip += "\n\nBuild required.\n\n"
        else:
            desired_tooltip += " (not required - built CL matches synced CL)\n\n"

        desired_tooltip += (f"Synced CL: {device._synced_changelist} "
                            f"(Code: {device._synced_compatible_cl})")

        desired_tooltip += details
    ...
    def populate_context_menu(self, cmenu: QtWidgets.QMenu):
        ...
        if self.connect_button.isChecked():
            ...
            fill_ddc_menu.addAction("Current Level", lambda: self.signal_device_widget_fill_ddc.emit(self, True))
            fill_ddc_menu.addAction("All Levels", lambda: self.signal_device_widget_fill_ddc.emit(self, False))
