...
class AddnDisplayDialog(AddDeviceDialog):
    def __init__(self, existing_devices, parent=None):
        ...
        for device in existing_devices:
            if device.device_type == "nDisplay" or str(device.device_type).startswith("nDisplay"):
...
class DevicenDisplay(DeviceUnreal):
    ...
    csettings = {
        ...
        'overwrite_remote_ndc_asset': BoolSetting(
            attr_name="overwrite_remote_ndc_asset",
            nice_name="Overwrite nDisplay UAsset",
            tool_tip="Overwrite the remote nDisplay .uasset from the local one when launching, if it exists",
            value=False,
    ...
    def create_monitor_if_necessary(cls):
        ...
        '''
        if not DevicenDisplay.ndisplay_monitor:
            DevicenDisplay.ndisplay_monitor = nDisplayMonitor(None)

        return DevicenDisplay.ndisplay_monitor
    ...
    def generate_unreal_command_line(self, map_name=""):
        ...
        # Add -multiprocess to all except the first co-hosted node, sorted by name.
        # Used to avoid race conditions when writing or deleting certain files on disk, such as
        # shader caches.

        shared_ip_connected_nodes = [
            node for node in self.get_connected_devices()
            if ip_address(node.address) == ip_address(self.address)
        ]

        # Launch command is also generated when devices are not yet connected, so empty shared_ip_connected_nodes
        # is a valid situation when the cluster is not being actually launched.
        if len(shared_ip_connected_nodes):
            first_cohosted_node = sorted(shared_ip_connected_nodes, key=lambda node: node.name)[0]

            if self != first_cohosted_node:
        ...
        # PSO Eviction time.
        #
        # PSO eviction may cause PSO rebuilding and may induce hitches.
        # Currently, only non-editor builds support pre-caching, so for now a way to avoid
        # these hitches is by lengthening the eviction time from the default 60 seconds.
        dp_cvars.append(f'r.pso.evictiontime={60*60}')

        # Add user set dp cvars, overriding any of the forced ones.
        user_dp_cvars = self.conform_dpcvars(self.csettings['ndisplay_dp_cvars'].get_value(self.name))
    ...
    def apply_local_overrides_to_config(cls, cfg_content, cfg_ext):
        ...
        devices = [
            dev for dev in cls.active_unreal_devices
            if dev.device_type == 'nDisplay' or str(dev.device_type).startswith('nDisplay')]
    ...
    def update_settings_controlled_by_config(self, cfg_file):
        ...
        try:
            ...
            LOGGER.error(f"Error parsing config file {cfg_file}: {e}")
            return
    ...
    def launch(self, map_name):
        ...
        overwrite_ndc = DevicenDisplay.csettings['overwrite_remote_ndc_asset'].get_value(self.name)

        # When desired, send .uasset but only if not launching packaged
        if overwrite_ndc and self.bp_object_path and not self.is_packaged_game():
