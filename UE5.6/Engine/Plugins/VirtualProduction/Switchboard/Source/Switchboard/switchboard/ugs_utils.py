...
_CACHED_SB_BUILDVER: dict[str, int | str] | None = None

def get_sb_buildver(*, force_refresh=False):
    ''' Return the JSON dict of Build.version in the Switchboard source tree. '''
    global _CACHED_SB_BUILDVER

    if _CACHED_SB_BUILDVER is None or force_refresh:
        # The objective is to query the UE version associated with Switchboard,
        # which is not guaranteed to be the same as the engine_dir setting.
        ...
        sbmod_dir = pathlib.Path(os.path.dirname(sys.modules['switchboard'].__file__))

        for parent in sbmod_dir.parents:
            if parent.name == 'Engine':
                buildver_path = parent / 'Build' / 'Build.version'
                try:
                    with open(buildver_path, 'r', encoding='utf-8') as f:
                        _CACHED_SB_BUILDVER = json.load(f)
                except Exception as e:
                    logging.warning(f'Error while reading {buildver_path}: {e}', exc_info=True)

                break

    if _CACHED_SB_BUILDVER is None:
        _CACHED_SB_BUILDVER = {}

    return _CACHED_SB_BUILDVER
