...
class UassetParser(object):
    ...
    def _readUassetSummary(self):
        ...
        if s.LegacyFileVersion not in [-7,-8,-9]:
        ...
        KNOWN_SUPPORTED_UE5VER = 1017
        if s.FileVersionUE5 > KNOWN_SUPPORTED_UE5VER:
            LOGGER.warning(f"{self.f.name}: ObjectUE5Version {s.FileVersionUE5} too new; "
                           f"newest known supported version {KNOWN_SUPPORTED_UE5VER}")
            LOGGER.warning(f"{self.f.name}: Parsing will attempt to continue, but there "
                           "may be errors reading the file")

        if s.FileVersionUE5 >= EUnrealEngineObjectUE5Version.PACKAGE_SAVED_HASH:
            s.SavedHash = struct.unpack('<20c', self.f.read(20))
        ...
        if s.FileVersionUE5 >= EUnrealEngineObjectUE5Version.ADD_SOFTOBJECTPATH_LIST:
        ...
        s.GatherableTextDataCount, s.GatherableTextDataOffset, \
        s.ExportCount, s.ExportOffset, s.ImportCount, s.ImportOffset \
            = struct.unpack('<6i', self.f.read(6*4))

        if s.FileVersionUE5 >= EUnrealEngineObjectUE5Version.VERSE_CELLS:
            s.CellExportCount, s.CellExportOffset, s.CellImportCount, s.CellImportOffset \
                = struct.unpack('<4i', self.f.read(4*4))

        if s.FileVersionUE5 >= EUnrealEngineObjectUE5Version.METADATA_SERIALIZATION_OFFSET:
            s.MetaDataOffset = self.readInt32()

        s.DependsOffset, s.SoftPackageReferencesCount, s.SoftPackageReferencesOffset, \
        s.SearchableNamesOffset, s.ThumbnailTableOffset \
            = struct.unpack('<5i', self.f.read(5*4))

        if s.FileVersionUE5 < EUnrealEngineObjectUE5Version.PACKAGE_SAVED_HASH:
            s.Guid = struct.unpack('<16s', self.f.read(16))

        s.PersistentGuid = struct.unpack('<16s', self.f.read(16))
