	...
	switch (InputIndex)
	{
		case InputPreviousLayer:
		{
			...
			if (UDMMaterialLayerObject* PreviousLayer = Layer->GetPreviousLayer(StageProperty, EDMMaterialLayerStage::Base))
			{
				...
				OutExpressions.Add(PreviousLayerExpressions.Last());
				return 0;
			}
		...
		case InputBaseStage:
		{
			...
			const TArray<UMaterialExpression*>& BaseStageExpressions = InBuildState->GetStageExpressions(BaseStage);
			OutExpressions.Add(BaseStageExpressions.Last());
...
void UDMMaterialStageThroughputLayerBlend::PostEditUndo()
{
	Super::PostEditUndo();
	...
	UMaterialExpression*& OutMaterialExpression, int32& OutputIndex)
{
	...
	UDMMaterialStage* StageActual = InStage;
	UDMMaterialStageSource* PreviewSource = nullptr;

	if (UDMMaterialStageInputExpression* InputExpression = Cast<UDMMaterialStageInputExpression>(GetInputMask()))
	{
		...
		{
			if (TextureSample->GetUseBaseTexture())
			{
				if (const UDMMaterialStageExpressionTextureSample* BaseTextureSample = TextureSample->GetBaseTextureSample())
				{
					UDMMaterialStage* BaseStage = BaseTextureSample->GetStage();

					if (UDMMaterialSubStage* SubStage = Cast<UDMMaterialSubStage>(BaseStage))
					{
						BaseStage = SubStage->GetParentMostStage();
					}
					...
					if (BaseStage)
					{
						...
						{
							if (UDMMaterialStageInput* BaseInputExpression = Blend->GetInputB())
							{
								StageActual = BaseStage;
								PreviewSource = BaseInputExpression;
							}
						}
					}
				}
	...
	UDMMaterialLayerObject* LayerActual = StageActual->GetLayer();
	check(LayerActual);

	UDMMaterialSlot* Slot = LayerActual->GetSlot();
	check(Slot);
	...
	BuildState->SetPreviewObject(StageActual);

	if (!PreviewSource)
	{
		PreviewSource = GetInputMask();
	}
...
void UDMMaterialStageThroughputLayerBlend::PostEditChangeProperty(FPropertyChangedEvent& InPropertyChangedEvent)
{
	Super::PostEditChangeProperty(InPropertyChangedEvent);

	static const FName MaskChannelOverrideName = GET_MEMBER_NAME_CHECKED(UDMMaterialStageThroughputLayerBlend, MaskChannelOverride);

	if (InPropertyChangedEvent.GetPropertyName() == MaskChannelOverrideName)
	{
		PushMaskChannelOverride();
	}
}
...
{
	bool bIsSharedBaseTextureSample = false;
		...
		if (UDMMaterialStageExpressionTextureSample* TextureSample = Cast<UDMMaterialStageExpressionTextureSample>(InputExpression->GetMaterialStageExpression()))
		{
			if (TextureSample->GetUseBaseTexture())
			{
				if (const UDMMaterialStageExpressionTextureSample* BaseTextureSample = TextureSample->GetBaseTextureSample())
				{
					...
					OutExpressions = InBuildState->GetStageSourceExpressions(BaseTextureSample);

					switch (MaskChannelOverride)
					{
						default:
						case EAvaColorChannel::Red:
							OutChannel.OutputIndex = 1;
							break;

						case EAvaColorChannel::Green:
							OutChannel.OutputIndex = 2;
							break;

						case EAvaColorChannel::Blue:
							OutChannel.OutputIndex = 3;
							break;

						case EAvaColorChannel::Alpha:
							OutChannel.OutputIndex = 4;
							break;
					}

					bIsSharedBaseTextureSample = true;
				}
			}
		}
	}

	if (!bIsSharedBaseTextureSample)
	{
