...
public:
	...
	UFUNCTION(BlueprintPure, Category = "VirtualCamera")
	bool GetRecordAsCineCamera() const { return bRecordAsCineCamera; }
	UFUNCTION(BlueprintCallable, Category = "VirtualCamera")
	void SetRecordAsCineCamera(bool bValue) { bRecordAsCineCamera = bValue; }
...
private:
	...
	/** Affects recordings in Take Recorder. If true, the owning actor should be recorded as a spawnable ACineCameraActor. This allows you to disable the Virtual Camera plugin but still use the sequences taken by it. */
	UPROPERTY(EditAnywhere, BlueprintGetter = "GetDisableComponentWhenSpawnedBySequencer", BlueprintSetter = "SetDisableComponentWhenSpawnedBySequencer", Category = "VirtualCamera", meta = (AllowPrivateAccess = "true", DisplayPriority = "5"))
	bool bRecordAsCineCamera = true;
