...
void FMediaOutputSynchronizationPolicyRivermaxHandler::Synchronize()
{
	...
	if (UE::RivermaxSync::GbTriggerRandomDesync)
	{
		...
		TSharedPtr<FRivermaxVideoOutputOptions> VideoOptions = OutputOptions.GetStreamOptions<FRivermaxVideoOutputOptions>(ERivermaxStreamType::VIDEO_2110_20_STREAM);
		...
		if (RmaxCapture->GetState() == EMediaCaptureState::Capturing)
		{
			...
			if (TSharedPtr<IRivermaxManager> RivermaxMgr = IRivermaxCoreModule::Get().GetRivermaxManager())
			{
				...
				TSharedPtr<FRivermaxVideoOutputOptions> VideoOptions =
					Options.GetStreamOptions<FRivermaxVideoOutputOptions>(ERivermaxStreamType::VIDEO_2110_20_STREAM);
