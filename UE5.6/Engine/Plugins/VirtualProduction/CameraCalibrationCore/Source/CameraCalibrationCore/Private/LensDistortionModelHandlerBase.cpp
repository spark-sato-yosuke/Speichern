...
{
	/* Distorted UV position in the view space:
					   ^ View space's Y
					   |
			  0        1        2
			  ...
			  6        5        4
	*/
	const TArray<FVector2D> DistortedUVs =
	{
		FVector2D(0.0f, 0.0f),
		FVector2D(0.5f, 0.0f),
		FVector2D(1.0f, 0.0f),
	...
	OverscanFactors.Reserve(DistortedUVs.Num());
	for (const FVector2D& DistortedUV : DistortedUVs)
	{
		const FVector2D UndistortedUV = ComputeUndistortedUV(DistortedUV);
		const float OverscanX = (DistortedUV.X != 0.5f) ? (UndistortedUV.X - 0.5f) / (DistortedUV.X - 0.5f) : 1.0f;
		const float OverscanY = (DistortedUV.Y != 0.5f) ? (UndistortedUV.Y - 0.5f) / (DistortedUV.Y - 0.5f) : 1.0f;
...
float ULensDistortionModelHandlerBase::ComputeInverseOverscanFactor() const
{
