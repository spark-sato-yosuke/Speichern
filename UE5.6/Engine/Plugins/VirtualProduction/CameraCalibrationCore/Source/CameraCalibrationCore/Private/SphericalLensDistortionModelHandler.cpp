...
FVector2D USphericalLensDistortionModelHandler::ComputeDistortedUV(const FVector2D& InScreenUV) const
{
	...
	FVector2D NormalizedDistanceFromImageCenter = (InScreenUV - FVector2D(0.5f, 0.5f)) / CurrentState.FocalLengthInfo.FxFy;
	...
	NormalizedDistanceFromImageCenter = NormalizedDistanceFromImageCenter * RadialDistortion + TangentialDistortion;

	const FVector2D DistortedUV = (NormalizedDistanceFromImageCenter * CurrentState.FocalLengthInfo.FxFy) + CurrentState.ImageCenter.PrincipalPoint;
	return DistortedUV;
}

FVector2D USphericalLensDistortionModelHandler::ComputeUndistortedUV(const FVector2D& InScreenUV) const
{
	// These distances cannot be zero in real-life. If they are, the current distortion state must be bad
	if ((CurrentState.FocalLengthInfo.FxFy.X == 0.0f) || (CurrentState.FocalLengthInfo.FxFy.Y == 0.0f))
	{
	...
	FVector2D NormalizedDistanceFromImageCenter = (InScreenUV - CurrentState.ImageCenter.PrincipalPoint) / CurrentState.FocalLengthInfo.FxFy;
