...
{
	/*
	 * Causes the action to not execute if this is true. The widget (from the SP Lambda) is destroyed
	 * prior to the action executing. Using FSlateApplication to dismiss the menu manually.
	 */
	constexpr bool bInShouldCloseWindowAfterMenuSelection = false;
				...
				FNewMenuDelegate::CreateSPLambda(this, [this, ThemeIdx](FMenuBuilder& SubMenuBuilder)
					{
						...
						if (ColorThemes.IsValidIndex(ThemeIdx) && ColorThemes[ThemeIdx].GetColors().Num() > 0)
						{
