...
bool SAvaLevelViewportGuide::SetOffset(float InOffset)
{
	const FAvaLevelViewportGuideFrameAndClient FrameAndClient(ViewportFrameWeak);
	...
	{
		case Orient_Horizontal:
			InOffset = FMath::Clamp(
				InOffset,
				MinimumViewportEdgeDistance,
				ViewportSize.Y - MinimumViewportEdgeDistance
			);

			NewOffsetFraction = InOffset / ViewportSize.Y;
			break;
		...
		case Orient_Vertical:
			...
			NewOffsetFraction = InOffset / ViewportSize.X;
			break;
...
void SAvaLevelViewportGuide::SetState(EAvaViewportGuideState InNewState)
{
	Info.State = InNewState;

	TSharedPtr<SAvaLevelViewportFrame> ViewportFrame = GetViewportFrame();
