...
public:
	...
	const FAvaInteractiveToolsToolParameters* GetActiveToolParameters() const;
	...
	virtual void OnRegisterTool(IAvalancheInteractiveToolsModule* InAITModule) {}
...
protected:
	...
	virtual bool CanActivate(const FString& InToolIdentifier, bool bInReactivate) const;
	virtual void Activate(const FString& InToolIdentifier, bool bInReactivate);
	...
	AActor* SpawnActor(TSubclassOf<AActor> InActorClass, bool bInPreview, FString* InActorLabelOverride = nullptr);
	...
	virtual AActor* SpawnActor(TSubclassOf<AActor> InActorClass, EAvaViewportStatus InViewportStatus, 
		const FVector2f& InViewportPosition, bool bInPreview, FString* InActorLabelOverride = nullptr);
