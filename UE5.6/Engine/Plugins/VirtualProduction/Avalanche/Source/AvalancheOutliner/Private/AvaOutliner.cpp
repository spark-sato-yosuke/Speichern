...
void FAvaOutliner::Refresh()
{
	...
	if (UWorld* const World = GetWorld())
	{
		// Ensure the latest world has its delegates bound
	...
	, TOptional<EItemDropZone> InRelativeDropZone)
{
	...
	const EItemDropZone RelativeDropZone = InRelativeDropZone.Get(EItemDropZone::BelowItem);
...
void FAvaOutliner::GroupSelection(AActor* InGroupingActor, const TOptional<FAttachmentTransformRules>& InTransformRules)
{
	...
	if (bIsGroupingItemNew)
	{
		...
		AddGroupingItemParams.Items = { GroupingItem };
	...
	AddChildItemParams.Items = SelectedItems;
...
void FAvaOutliner::SortItems(TArray<FAvaOutlinerItemPtr>& OutOutlinerItems)
{
	OutOutlinerItems.Sort([](const FAvaOutlinerItemPtr& ItemA, const FAvaOutlinerItemPtr& ItemB)
	{
		return UE::AvaOutliner::CompareOutlinerItemOrder(ItemA, ItemB);
	});
}
...
void FAvaOutliner::OnActorSpawned(AActor* InActor)
{
	...
	if (!bSpawnNotifyIgnored && IsValid(InActor) && InActor->GetWorld() == GetWorld())
	{
		...
		{
			FAvaOutlinerAddItemParams Params;
			Params.Items            = { FindOrAdd<FAvaOutlinerActor>(InActor) };
...
void FAvaOutliner::BindWorldDelegates(UWorld* InWorld)
{
	if (BoundWorldWeak == InWorld)
	{
		return;
	}

	UnbindWorldDelegates();
	BoundWorldWeak = InWorld;

	if (InWorld)
	{
		InWorld->OnLevelsChanged().AddSP(this, &FAvaOutliner::RequestRefresh);
	}
}

void FAvaOutliner::UnbindWorldDelegates()
{
	if (UWorld* const BoundWorld = BoundWorldWeak.Get())
	{
		BoundWorld->OnLevelsChanged().RemoveAll(this);
	}
}
