...
bool FAvaOutlinerActor::CanAddChild(const FAvaOutlinerItemPtr& InChild) const
{
	// Super checks InChild.IsValid
	return Super::CanAddChild(InChild) && InChild->IsA<FAvaOutlinerActor>();
}

void FAvaOutlinerActor::OnChildrenAdded(TConstArrayView<FAvaOutlinerItemPtr> InAddedChildren, const FAvaOutlinerAddItemParams& InAddItemParams)
{
	Super::OnChildrenAdded(InAddedChildren, InAddItemParams);

	for (const FAvaOutlinerItemPtr& Item : InAddedChildren)
	{
		FAvaOutlinerActor* ActorItem = Item->CastTo<FAvaOutlinerActor>();
		if (!ActorItem)
		{
			continue;
		}
