...
void UAvaMask2DBaseModifier::SavePreState()
{
	...
	NewActorDatas.Reserve(ActorData.Num());

	ForEachActor<AActor>(
		[this, &NewActorDatas](AActor* InActor)
		{
			SaveActorPreState(InActor, NewActorDatas.Emplace(InActor, ActorData.FindOrAdd(InActor)));
			return true;
		}
	...
	const TSharedPtr<IAvaMaskMaterialCollectionHandle> MaterialCollectionHandle =
		UE::Ava::Internal::FindOrAddHandleByLambda(
			...
			, [this, InActor, &InActorData]()
			{
				...
				for (const TPair<FAvaMask2DComponentMaterialPath, TObjectPtr<UMaterialInterface>>& OriginalMaterial : InActorData.OriginalMaterials)
				{
					Handle->SetMaterial(OriginalMaterial.Key.Component, OriginalMaterial.Key.SlotIdx, OriginalMaterial.Value);
				}

				InActorData.OriginalMaterials.Empty();

				Handle->ForEachMaterial([&InActorData](const FSoftComponentReference& InComponent, const int32 InIdx, UMaterialInterface* InMaterial)
				{
					InActorData.OriginalMaterials.Add(FAvaMask2DComponentMaterialPath(InComponent, InIdx), InMaterial);
					return true;
				});
...
void UAvaMask2DBaseModifier::OnRenderStateUpdated(AActor* InActor, UActorComponent* InComponent)
{
	...
	if (!(ActorModified == InActor || InActor->IsAttachedTo(ActorModified)))
	{
		return;
	}
	...
	if (const FAvaMask2DActorData* ActorMaskData = ActorData.Find(InActor))
	{
		if (ActorMaskData->CanvasTextureWeak.IsValid())
		{
			return;
		}
	}
