...
FAvaBroadcastOutputChannel::FAvaBroadcastOutputChannel(FAvaBroadcastProfile* InProfile)
	: FAvaBroadcastOutputChannel(
		InProfile ? InProfile->GetName() : NAME_None,
		InProfile ? &InProfile->GetBroadcast() : &UAvaBroadcast::Get())
{
	
}

FAvaBroadcastOutputChannel::FAvaBroadcastOutputChannel(FName InProfileName, UAvaBroadcast* InBroadcast)
...
FAvaBroadcastProfile& FAvaBroadcastOutputChannel::GetProfile() const
{
	...
	ensure(ParentBroadcastWeak.IsValid()); 
	UAvaBroadcast& Broadcast = ParentBroadcastWeak.IsValid() ? *ParentBroadcastWeak.Get() : UAvaBroadcast::Get();
	FAvaBroadcastProfile& Profile = Broadcast.GetProfile(ProfileName);
	return Profile.IsValidProfile() ? Profile : Broadcast.GetCurrentProfile();
}
...
EAvaBroadcastOutputState FAvaBroadcastOutputChannel::GetMediaOutputState(const UMediaOutput* InMediaOutput) const
{
	...
	// Check if this output has reported errors.	
	return GetLocalSeverityForOutput(InMediaOutput) == EAvaBroadcastIssueSeverity::Errors ?
		EAvaBroadcastOutputState::Error : EAvaBroadcastOutputState::Idle;
}
...
bool FAvaBroadcastOutputChannel::StartChannelBroadcast()
{
	...
	if (Module.IsPlaybackClientStarted())
	{
		...
		{
			Module.GetPlaybackClient().RequestBroadcast(ProfileName.ToString(), GetChannelName(), RemoteMediaOutputs, EAvaBroadcastAction::Start);
		}
	}
	...
	for (UMediaOutput* MediaOutput : MediaOutputs)
	{
		...
		if (!IsValid(MediaCapture))
		{
			FString& Message = EmplaceMessageForOutput(MediaOutput, EAvaBroadcastIssueSeverity::Errors);
			
			// Try to get a failure reason.
			FString FailureReason;
			if (!MediaOutput->Validate(FailureReason))
			{
				Message = FString::Printf(TEXT("Couldn't create media capture. %s"), *FailureReason);
			}
			else
			{
				Message = FString::Printf(TEXT("Couldn't create media capture."));
			}

			UE_LOG(LogAvaOutputChannel, Error, TEXT("Output Channel (%s) has a Media Output (%s): %s"),
				*ChannelName, *MediaOutput->GetName(), *Message);
		...
		{
			FString& Message = EmplaceMessageForOutput(MediaOutput, EAvaBroadcastIssueSeverity::Errors);
			Message = TEXT("Failed to initialize. See logs for more info.");
		}
	}
...
void FAvaBroadcastOutputChannel::StopChannelBroadcast()
{
	...
	if (Module.IsPlaybackClientStarted())
	{
		...
		if (!RemoteMediaOutputs.IsEmpty())
		{
			Module.GetPlaybackClient().RequestBroadcast(ProfileName.ToString(), GetChannelName(), RemoteMediaOutputs, EAvaBroadcastAction::Stop);
		}
	}
...
int32 FAvaBroadcastOutputChannel::RemoveMediaOutput(UMediaOutput* InMediaOutput)
{
	...
	if (IndexToRemove != INDEX_NONE)
	{
		...
		LocalMediaOutputStatuses.Remove(InMediaOutput);
		OnChannelChanged.Broadcast(*this, EAvaBroadcastChannelChange::MediaOutputs);
		UpdateChannelResources(/*bInIsProfileActive*/true);
		RefreshState();
	}
...
void FAvaBroadcastOutputChannel::OnMediaOutputModified(UMediaOutput* InMediaOutput)
{
	...
	if (IsMediaOutputRemote(InMediaOutput) && AvaMediaModule.IsPlaybackClientStarted())
	{
		...
		PlaybackClient.RequestBroadcast(ProfileName.ToString(), GetChannelName(), GetRemoteMediaOutputs(), EAvaBroadcastAction::UpdateConfig);
	}
	else
	{
		UpdateChannelResources(/*bInIsProfileActive*/true);
...
void FAvaBroadcastOutputChannel::SetProfileName(FName InProfileName)
{
	ProfileName = InProfileName;	
}
...
{
	PostLoadMediaOutputs(bInIsProfileActive, InProfile ? InProfile->GetName() : NAME_None,
		InProfile ? &InProfile->GetBroadcast() : &UAvaBroadcast::Get());
}

void FAvaBroadcastOutputChannel::PostLoadMediaOutputs(bool bInIsProfileActive, FName InProfileName, UAvaBroadcast* InBroadcast)
{
	ProfileName = InProfileName;
	ParentBroadcastWeak = InBroadcast;
