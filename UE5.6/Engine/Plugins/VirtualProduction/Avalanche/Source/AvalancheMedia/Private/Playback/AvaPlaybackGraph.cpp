...
namespace UE::AvaPlaybackGraph::Private
{
	const FString& GetLoadOptions(UAvaPlaybackNode* InPlaybackNode)
	{
		if (UAvaPlaybackNodePlayer* PlayerNode = Cast<UAvaPlaybackNodePlayer>(InPlaybackNode))
		{
			return PlayerNode->GetLoadOptions();
		}
...
void UAvaPlaybackGraph::LoadInstances()
{
	...
	{
		// Load the assets
		for (const FAvaPlaybackPlayerParameters& Player : Playback.Value.Players)
		{
			FindOrLoadPlayable(Player.Asset, ResolveChannelName(Playback.Key), UE::AvaPlaybackGraph::Private::GetLoadOptions(Player.Node.Get()));
		}
	}
}
...
void UAvaPlaybackGraph::Stop(EAvaPlaybackStopOptions InStopOptions)
{
	...
	{
		bIsShuttingDown = PlaybackManager->IsShuttingDown();
	}
	...
	for (const TPair<FName, FAvaPlaybackPlayableGroup>& ChannelPlayableGroup : ChannelPlayableGroups)
	{
		...
		for (UAvaPlayable* Playable : Playables)
		{
			// EndPlayWorld is no longer issued here (unless shutting down) as some of the engine systems don't seem
			...
			Playable->EndPlay(EAvaPlayableEndPlayOptions::None); //bIsShuttingDown ? EAvaPlayableEndPlayOptions::ConditionalEndPlayWorld : EAvaPlayableEndPlayOptions::None);
...
UAvaPlayable* UAvaPlaybackGraph::FindOrLoadPlayable(const FAvaSoftAssetPtr& InSourceAsset, const FName& InChannelName, const FString& InLoadOptions)
{
	...
	if (NewPlayable)
	{
		...
		NewPlayable->LoadAsset(InSourceAsset, false, InLoadOptions);
	}
	return NewPlayable;
}
...
TArray<UAvaGameInstance*> UAvaPlaybackGraph::GetActiveGameInstances() const
{
	...
	if (IsPlaying())
	{
		...
		for (const TPair<FName, FAvaPlaybackChannelParameters>& Playback : PlaybackSettings)
		{
			...
			for (const FAvaPlaybackPlayerParameters& Player : Playback.Value.Players)
			{
				const UAvaPlayable* Playable = FindPlayable(Player.Asset.ToSoftObjectPath(), ChannelName);
...
void UAvaPlaybackGraph::LoadAsset(const FAvaSoftAssetPtr& InSourceAsset, const FName& InChannelName, const FString& InLoadOptions)
{
	FindOrLoadPlayable(InSourceAsset, InChannelName, InLoadOptions);
}
...
void UAvaPlaybackGraph::Tick(float DeltaTime)
{
	...
	for (const TPair<FName, FAvaPlaybackChannelParameters>& Playback : PlaybackSettings)
	{
		...
		for (const FAvaPlaybackPlayerParameters& Player : Playback.Value.Players)
		{
			if (IsPreviewOnly())
			{
				RefreshPreview(Player.Asset, UE::AvaPlaybackGraph::Private::GetLoadOptions(Player.Node.Get()), Channel, ChannelName);
			}
			else if (Channel.IsValidChannel())
			{
				RefreshPlayback(Player.Asset, UE::AvaPlaybackGraph::Private::GetLoadOptions(Player.Node.Get()), Channel, ChannelName);
			}
		}
		...
		{
			for (const FAvaPlaybackPlayerParameters& PreviousPlayer : PreviousParameters->Players)
			{
				// We still don't support multi-instance within a playback graph. The asset remains the key.
				auto IsSameAsset = [&PreviousPlayer](const FAvaPlaybackPlayerParameters& InPlayer)
				{
					return PreviousPlayer.Asset == InPlayer.Asset;
				};
				
				if (!Playback.Value.Players.FindByPredicate(IsSameAsset))
				{
					StopPlayable(PreviousPlayer.Asset.ToSoftObjectPath(), ChannelName);
				}
			}
		}
	}
...
void UAvaPlaybackGraph::PushRemoteControlValues(const FSoftObjectPath& InSourceAssetPath, const FString& InChannelName, const TSharedRef<FAvaPlayableRemoteControlValues>& InRemoteControlValues, EAvaPlayableRCUpdateFlags InFlags)
{
	const FDateTime Timeout = FDateTime::UtcNow() + FTimespan::FromSeconds(10);
	PendingRemoteControlCommands.Add({{Timeout, InSourceAssetPath, InChannelName, FName(InChannelName)}, InRemoteControlValues, InFlags});
}
...
void UAvaPlaybackGraph::ExecutePendingRemoteControlCommands()
{
	...
	for (const FRemoteControlCommand& RemoteControlCommand : PendingRemoteControlCommands)
	{
		...
		const EAvaPlayableCommandResult Result = Playable->UpdateRemoteControlCommand(RemoteControlCommand.Values, RemoteControlCommand.Flags);
		if (Result == EAvaPlayableCommandResult::KeepPending)
		{
...
bool UAvaPlaybackGraph::RefreshPlayback(const FAvaSoftAssetPtr& InSourceAsset, const FString& InLoadOptions, FAvaBroadcastOutputChannel& InChannel, const FName& InChannelName)
{
	...
	if (UAvaPlayable* const Playable = FindOrLoadPlayable(InSourceAsset, InChannelName, InLoadOptions))
	{
...
bool UAvaPlaybackGraph::RefreshPreview(const FAvaSoftAssetPtr& InSourceAsset, const FString& InLoadOptions, FAvaBroadcastOutputChannel& InChannel, const FName& InChannelName)
{
	// If the preview channel has been setup, go through the normal channel refresh.
	if (InChannel.IsValidChannel())
	{
		return RefreshPlayback(InSourceAsset, InLoadOptions, InChannel, InChannelName);
	}
	...
	UAvaPlayable* const Playable = FindOrLoadPlayable(InSourceAsset, InChannelName, InLoadOptions);
