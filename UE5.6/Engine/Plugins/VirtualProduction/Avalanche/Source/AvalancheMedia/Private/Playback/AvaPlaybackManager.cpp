...
TSharedPtr<FAvaPlaybackInstance> FAvaPlaybackManager::LoadPlaybackInstance(const FSoftObjectPath& InAssetPath, const FString& InChannelName, const FString& InLoadOptions)
{
	TSharedPtr<FAvaPlaybackInstance> PlaybackInstance;
	
	if (UAvaPlaybackGraph* Playback = LoadPlaybackObject(InAssetPath, InChannelName, InLoadOptions))
	{
...
TSharedPtr<FAvaPlaybackInstance> FAvaPlaybackManager::AcquireOrLoadPlaybackInstance(const FSoftObjectPath& InAssetPath, const FString& InChannelName, const FString& InLoadOptions)
{
	TSharedPtr<FAvaPlaybackInstance> PlaybackInstance = AcquirePlaybackInstance(InAssetPath, InChannelName);
	if (!PlaybackInstance)
	{
		PlaybackInstance = LoadPlaybackInstance(InAssetPath, InChannelName, InLoadOptions);
	}
	return PlaybackInstance;
}
...
UAvaPlaybackGraph* FAvaPlaybackManager::LoadPlaybackObject(const FSoftObjectPath& InAssetPath, const FString& InChannelName, const FString& InLoadOptions) const
{
	...
	{
		UAvaPlaybackGraph* const AvaPlayback = BuildPlaybackFromWorld(TSoftObjectPtr<UWorld>(InAssetPath), InChannelName, InLoadOptions);
		check(AvaPlayback);
		return AvaPlayback;
	}
	...
	// Warning: this is not an instance, but the asset itself. Will not work for multi-instancing (TL).
	...
	{
		UAvaPlaybackGraph* const AvaPlayback = BuildPlaybackFromWorld(World, InChannelName, InLoadOptions);
		check(AvaPlayback);
		return AvaPlayback;
	}
...
UAvaPlaybackGraph* FAvaPlaybackManager::BuildPlaybackFromWorld(const TSoftObjectPtr<UWorld>& InWorld, const FString& InChannelName, const FString& InLoadOptions) const
{
	using namespace UE::AvaPlaybackManager::Private;
	...
	PlayerNode->SetLoadOptions(InLoadOptions);
...
bool FAvaPlaybackManager::PushRemoteControlCommand(const FGuid& InInstanceId, const FSoftObjectPath& InSourcePath, const FString& InChannelName, const TSharedRef<FAvaPlayableRemoteControlValues>& InRemoteControlValues, EAvaPlayableRCUpdateFlags InFlags)
{
	...
	if (InInstanceId.IsValid())
	{
		...
		if (const TSharedPtr<FAvaPlaybackInstance> PlaybackInstance = FindPlaybackInstance(InInstanceId, InSourcePath, InChannelName))
		{
			PlaybackInstance->GetPlayback()->PushRemoteControlValues(InSourcePath, InChannelName, InRemoteControlValues, InFlags);
	...
	{
		// If we had a playback asset (not from the rundown path), check if it is present in one of the playback instances.
		ForAllPlaybackInstances([InSourcePath, InChannelName, InRemoteControlValues, &bRemoteControlValuesPushed, InFlags](FAvaPlaybackInstance& InPlaybackInstance)
		{
			if (InPlaybackInstance.GetPlayback()->HasPlayerNodeForSourceAsset(InSourcePath))
			{
				InPlaybackInstance.GetPlayback()->PushRemoteControlValues(InSourcePath, InChannelName, InRemoteControlValues, InFlags);
				bRemoteControlValuesPushed = true;
			}
		});
	}
	...
	{
		FString CommandBufferKey = MakeCommandBufferKey(InInstanceId, InSourcePath, InChannelName);
		GetOrCreatePlaybackCommandBuffers(MoveTemp(CommandBufferKey)).RemoteControlCommands.Add({InRemoteControlValues, InFlags});
	}
...
void FAvaPlaybackManager::ApplyPendingCommands(UAvaPlaybackGraph* InPlaybackObject, const FGuid& InInstanceId, const FSoftObjectPath& InSourcePath, const FString& InChannelName)
{
	...
	if (const FPlaybackObjectCommandBuffers* Commands = GetPlaybackCommandBuffers(CommandBufferKey))
	{
		...
		{
			InPlaybackObject->PushRemoteControlValues(InSourcePath, InChannelName, RCCommand.Values, RCCommand.UpdateFlags);
		}
