...
{
	/** Returns true if the binding is valid and access to read the property. */
	AVALANCHEMEDIA_API bool HasReadAccess(const TSharedPtr<const FRemoteControlEntity>& InRemoteControlEntity, FString& OutErrorText);

	/** Returns true if the binding is valid and access to write the property. */
	AVALANCHEMEDIA_API bool HasWriteAccess(const TSharedPtr<const FRemoteControlEntity>& InRemoteControlEntity, FString& OutErrorText);
	...
	AVALANCHEMEDIA_API EAvaPlayableRemoteControlResult SetValueOfController(URCVirtualPropertyBase* InController, const FString& InValue, bool bInBehaviorsEnabled);
