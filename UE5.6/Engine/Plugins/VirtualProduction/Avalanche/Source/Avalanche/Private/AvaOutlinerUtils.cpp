...
bool FAvaOutlinerUtils::EditorOutlinerItemsToActors(const TArray<FAvaOutlinerItemPtr>& InItems, TArray<AActor*>& OutActors)
{
	...
	for (int32 i = 0; i < ItemCount; ++i)
	{
		...
		// Root of the tree is level
		const FAvaOutlinerLevel* const LevelItem = InItems[i]->CastTo<FAvaOutlinerLevel>();
		if (LevelItem)
		{
			return EditorOutlinerItemsToActors(LevelItem->GetChildren(), OutActors);
		}
