...
void UTakeRecorder::SetCountdown(float InSeconds)
{
	CountdownSeconds = InSeconds;
}
...
{
	SCOPED_TAKERECORDER_TRACE(UTakeRecorder_Initialize);
	...
	if (MetaData == nullptr)
	{
		if (OutError)
		{
			*OutError = LOCTEXT("NoMetaDataError", "No Meta Data specified.");
		}
		return false;
	}
	...
	UTakeMetaData::SetMostRecentMetaData(MetaData);
	...
	{
		// The editor subsystem is normally responsible broadcasting their delegates, but they don't have a reliable way of binding to a
		// pre-initialize event.
		TakeRecorderSubsystem->GetOnRecordingPreInitializedEvent().Broadcast(this);
	...
	{
		if (!CreateDestinationAsset(*FinalParameters.Project.GetTakeAssetPath(), LevelSequenceBase, Sources, MetaData, OutError, FinalParameters.bOpenSequencer))
		{
			return false;
		}
	}
	...
	{
		if (!SetupDestinationAsset(FinalParameters, LevelSequenceBase, Sources, MetaData, OutError, FinalParameters.bOpenSequencer))
		{
			return false;
		}
	}
...
bool UTakeRecorder::CreateDestinationAsset(const TCHAR* AssetPathFormat, ULevelSequence* LevelSequenceBase, UTakeRecorderSources* Sources, UTakeMetaData* MetaData, FText* OutError, bool bOpenSequencer)
{
	check(LevelSequenceBase && Sources && MetaData);

	FString   PackageName;

	if (!MetaData->TryGenerateRootAssetPath(AssetPathFormat, PackageName))
	{
		return false;
	}
	...
	bool bSuccess = true;
	if (bOpenSequencer && GIsEditor)
	{
		WeakSequencer = TakesUtils::OpenSequencer(SequenceAsset, OutError);
		bSuccess = WeakSequencer.IsValid();
	}
	return bSuccess;
}

bool UTakeRecorder::SetupDestinationAsset(const FTakeRecorderParameters& InParameters, ULevelSequence* LevelSequenceBase, UTakeRecorderSources* Sources, UTakeMetaData* MetaData, FText* OutError, bool bOpenSequencer)
{
	check(LevelSequenceBase && Sources && MetaData);
...
{
	SCOPED_TAKERECORDER_TRACE(UTakeRecorder_InternalTick);
	...
	OnTickRecordingEvent.Broadcast(this, RecordTime);
...
{
	SCOPED_TAKERECORDER_TRACE(UTakeRecorder_PreRecord);
	...
	{
		FString AssetName;
		if (AssetMetaData->TryGenerateRootAssetPath(Parameters.Project.GetTakeAssetPath(), AssetName))
		{
	...
	TakeRecorderSourcesSettings.SubSequenceDirectory = Parameters.Project.SubSequenceDirectory;

	TSharedPtr<ISequencer> Sequencer = WeakSequencer.Pin();
	if (Sequencer)
	{
		TakeRecorderSourcesSettings.TransformOrigin = MovieSceneToolHelpers::GetTransformOriginForFocusedSequence(Sequencer);
	}
...
{
	SCOPED_TAKERECORDER_TRACE(UTakeRecorder_Start);
	using namespace UE::MovieScene;
	...
	if (!Parameters.bDisableRecordingAndSave)
	{
		SCOPED_TAKERECORDER_TRACE(UTakeRecorder_StartRecordingSources);
...
{
	SCOPED_TAKERECORDER_TRACE(UTakeRecorder_StopInternal);
	...
	if (bDidEverStartRecording)
	{
		...
		if (bRecordingFinished)
		{
			...
			// We have recorded this sequence, mark it.
			UMovieSceneShotMetaData* MovieSceneMetaData = SequenceAsset->FindOrAddMetaData<UMovieSceneShotMetaData>();
			MovieSceneMetaData->SetIsRecorded(true);
	...
	{
		UTakeMetaData::SetMostRecentMetaData(nullptr);
...
FOnTickRecording& UTakeRecorder::OnTickRecording()
{
	return OnTickRecordingEvent;
}
