...
struct FTakeRecorderProjectParameters
{
	...
	UPROPERTY(config, EditAnywhere, BlueprintReadWrite, Category = "Take Recorder", meta = (ContentDir, NamingTokens))
	 ...
	 */
	UPROPERTY(config, EditAnywhere, BlueprintReadWrite, Category = "Take Recorder", meta = (NamingTokens))
	FString TakeSaveDir;
...
struct FTakeRecorderParameters
{
	...
	/** If sequencer should open when starting a recording. */
	UPROPERTY(BlueprintReadWrite, Category = "Take Recorder")
...
struct TAKERECORDER_API FTakeRecorderSequenceParameters
{
	GENERATED_BODY()
	
	/** A preset asset to base the recording off */
	UPROPERTY(BlueprintReadWrite, Category = TakeRecorder)
	TObjectPtr<UTakePreset> BasePreset;

	/** A level sequence asset to base the recording off */
	UPROPERTY(BlueprintReadWrite, Category = TakeRecorder)
	TObjectPtr<ULevelSequence> BaseSequence;

	/** A level sequence asset to record into */
	UPROPERTY(BlueprintReadWrite, Category = TakeRecorder)
	TObjectPtr<ULevelSequence> RecordIntoSequence;

	/** A sequence that should be shown directly on the take recorder UI */
	UPROPERTY(BlueprintReadWrite, Category = TakeRecorder)
	TObjectPtr<ULevelSequence> SequenceToView;

	/** Checks whether we have any valid data present. */
	bool IsValid() const;
	
	bool operator==(const FTakeRecorderSequenceParameters& Other) const;
	bool operator!=(const FTakeRecorderSequenceParameters& Other) const;
};
