...
void UMovieScene3DTransformTrackRecorder::FinalizeTrackImpl()
{
	...
	if (ActorToRecord)
	{
		...
		else if (BufferedTransforms.Times.Num() > 0)
		{
			BufferedTransforms.PostMultTransform(OwningTakeRecorderSource->GetTrackRecorderSettings().TransformOrigin.Inverse());
		}
	}
	...
	if (DefaultTransform.IsSet())
	{
 	...
 	BufferedTransforms = UE::TrackRecorders::FBufferedTransformKeys();
...
void UMovieScene3DTransformTrackRecorder::PostProcessAnimationData(UMovieSceneAnimationTrackRecorder* AnimTrackRecorder)
{
	...
	if (AnimTrackRecorder->RootWasRemoved())
	{
		...
		UE::TrackRecorders::FBufferedTransformKeys  AnimationKeys;
		...
		if (SkeletalMeshComponent)
		{
			...
			if (AnimSequence && SkinnedAsset && AnimSequence->GetSkeleton())
			{
				...
				for (const FName& TrackName : TrackNames)
				{
					...
					const int32 BoneTreeIndex = AnimSequence->GetSkeleton()->GetReferenceSkeleton().FindBoneIndex(TrackName);
