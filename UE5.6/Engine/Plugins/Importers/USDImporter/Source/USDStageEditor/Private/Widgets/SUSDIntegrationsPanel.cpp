...
TSharedRef<SWidget> SUsdIntegrationsPanelRow::GenerateWidgetForColumn(const FName& ColumnName)
{
	...
	const static TMap<FName, FText> DisplayTextForProperties({
		DisplayTextForPropertiesEntry(
			*UsdToUnreal::ConvertToken(UnrealIdentifiers::UnrealAnimBlueprintPath),
			LOCTEXT("AnimBlueprintPathText", "Anim Blueprint asset")
		),
		DisplayTextForPropertiesEntry(
			*UsdToUnreal::ConvertToken(UnrealIdentifiers::UnrealControlRigPath),
			LOCTEXT("ControlRigPathText", "Control Rig asset")
		),
		DisplayTextForPropertiesEntry(
			*UsdToUnreal::ConvertToken(UnrealIdentifiers::UnrealUseFKControlRig),
			LOCTEXT("UseFKControlRigText", "Use FKControlRig")
		),
		DisplayTextForPropertiesEntry(
			*UsdToUnreal::ConvertToken(UnrealIdentifiers::UnrealControlRigReduceKeys),
			LOCTEXT("ControlRigReduceKeysText", "Control Rig key reduction")
		),
		DisplayTextForPropertiesEntry(
			*UsdToUnreal::ConvertToken(UnrealIdentifiers::UnrealControlRigReductionTolerance),
			LOCTEXT("ControlRigReduceToleranceText", "Control Rig key reduction tolerance")
		),
		DisplayTextForPropertiesEntry(
			*UsdToUnreal::ConvertToken(UnrealIdentifiers::UnrealLiveLinkSubjectName),
			LOCTEXT("SubjectNameText", "Live Link subject name")
		),
		DisplayTextForPropertiesEntry(
			*UsdToUnreal::ConvertToken(UnrealIdentifiers::UnrealLiveLinkEnabled),
			LOCTEXT("LiveLinkEnabledText", "Enable LiveLink")
		),
	});
	...
	const static TMap<FName, FText> ToolTipTextForProperties({
		DisplayTextForPropertiesEntry(
			*UsdToUnreal::ConvertToken(UnrealIdentifiers::UnrealAnimBlueprintPath),
			LOCTEXT("AnimBlueprintPathToolTip", "Anim Blueprint asset to use on the component to connect with Live Link")
		),
		DisplayTextForPropertiesEntry(
			*UsdToUnreal::ConvertToken(UnrealIdentifiers::UnrealControlRigPath),
			LOCTEXT("ControlRigPathToolTip", "Control Rig Blueprint asset to use")
		),
		DisplayTextForPropertiesEntry(
			*UsdToUnreal::ConvertToken(UnrealIdentifiers::UnrealUseFKControlRig),
			LOCTEXT("UseFKControlRigToolTip", "Whether to use a generated FKControlRig instead of the Control Rig Blueprint asset")
		),
		DisplayTextForPropertiesEntry(
			*UsdToUnreal::ConvertToken(UnrealIdentifiers::UnrealControlRigReduceKeys),
			LOCTEXT("ControlRigReduceKeysToolTip", "Whether to enable key reduction when generating Control Rig tracks from the USD animation data")
		),
		DisplayTextForPropertiesEntry(
			*UsdToUnreal::ConvertToken(UnrealIdentifiers::UnrealControlRigReductionTolerance),
			LOCTEXT("ControlRigReduceToleranceToolTip", "Tolerance to use for the key reduction pass")
		),
		DisplayTextForPropertiesEntry(
			*UsdToUnreal::ConvertToken(UnrealIdentifiers::UnrealLiveLinkSubjectName),
			LOCTEXT("SubjectNameToolTip", "Which Live Link subject to use for this component")
		),
		DisplayTextForPropertiesEntry(
			*UsdToUnreal::ConvertToken(UnrealIdentifiers::UnrealLiveLinkEnabled),
			LOCTEXT(
				"LiveLinkEnabledToolTip",
				"If checked will cause the component to be animated with the pose data coming from the Anim Blueprint. If unchecked will cause the "
				"component to be animated with the regular skeleton animation data coming from the USD Stage"
			)
		),
	});
	...
	else
	{
				...
				FSimpleDelegate PropertyChanged = FSimpleDelegate::CreateLambda(
					[AttributeCopy]()
					{
						...
						FScopedTransaction Transaction(
							FText::Format(LOCTEXT("AnimBluepringTransaction", "Changed AnimBluepring path to '{0}'"), FText::FromString(Path))
						);
			...
			if (UUsdIntegrationsPanelPropertyDummy* Dummy = GetMutableDefault<UUsdIntegrationsPanelPropertyDummy>())
			{
				...
				FSimpleDelegate PropertyChanged = FSimpleDelegate::CreateLambda(
					[AttributeCopy]()
					{
						...
						FScopedTransaction Transaction(
							FText::Format(LOCTEXT("ControlRigPathTransaction", "Changed ControlRig blueprint path to '{0}'"), FText::FromString(Path))
						);
		...
		else if (AttributeName == *UsdToUnreal::ConvertToken(UnrealIdentifiers::UnrealLiveLinkEnabled) || AttributeName == *UsdToUnreal::ConvertToken(UnrealIdentifiers::UnrealUseFKControlRig))
		{
			// clang-format off
			SAssignNew(ColumnWidget, SBox)
			.HeightOverride(FUsdStageEditorStyle::Get()->GetFloat("UsdStageEditor.ListItemHeight"))
