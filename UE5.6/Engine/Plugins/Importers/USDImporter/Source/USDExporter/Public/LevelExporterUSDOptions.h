...
struct USDEXPORTER_API FLevelExporterUSDOptionsInner
{
	...
	FString RootPrimName = TEXT("Root");
	...
	/** If true, and if we have a level sequence animating the level during export, it will revert any actor or component to its unanimated state
	 * before writing to USD */
	UPROPERTY(EditAnywhere, config, BlueprintReadWrite, Category = "Export options")
	...
	/** If true, will export sub-levels as separate layers (referenced as sublayers). If false, will collapse all sub-levels in a single exported root
	 * layer */
...
namespace UsdUtils
{
	USDEXPORTER_API void AddAnalyticsAttributes(const FLevelExporterUSDOptionsInner& Options, TArray<FAnalyticsEventAttribute>& InOutAttributes);

	USDEXPORTER_API void AddAnalyticsAttributes(const ULevelExporterUSDOptions& Options, TArray<FAnalyticsEventAttribute>& InOutAttributes);

	USDEXPORTER_API void HashForLevelExport(const FLevelExporterUSDOptionsInner& Options, FSHA1& HashToUpdate);

	USDEXPORTER_API void HashForLevelExport(const ULevelExporterUSDOptions& Options, FSHA1& HashToUpdate);
}
