...
public:
	...
	UPROPERTY(
		...
		meta = (EditCondition = "bExportLevel", DisplayName = "Use Exported Level as USD Sublayer")
	)
...
namespace UsdUtils
{
	USDEXPORTER_API void AddAnalyticsAttributes(const ULevelSequenceExporterUsdOptions& Options, TArray<FAnalyticsEventAttribute>& InOutAttributes);

	USDEXPORTER_API void HashForLevelSequenceExport(const ULevelSequenceExporterUsdOptions& Options, FSHA1& HashToUpdate);
}
