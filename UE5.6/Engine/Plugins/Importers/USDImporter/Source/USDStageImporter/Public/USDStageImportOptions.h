...
public:
	...
	UPROPERTY(
		...
		EditAnywhere,
		Category = "Processing",
		meta = (DisplayName = "Interpret LOD variant sets", ScriptName="InterpretLods;InterpretLODs", EditCondition = bImportGeometry)
	)
	bool bInterpretLODs;

public:
	UE_DEPRECATED(5.6, "Use FScopedSuppressActorImport instead.")
...
namespace UsdUtils
{
	struct FScopedSuppressActorImport
	{
		FScopedSuppressActorImport(UUsdStageImportOptions* InOptions);
		~FScopedSuppressActorImport();

		FScopedSuppressActorImport(const FScopedSuppressActorImport& Other) = delete;
		FScopedSuppressActorImport(FScopedSuppressActorImport&& Other) = delete;
		FScopedSuppressActorImport& operator=(const FScopedSuppressActorImport& Other) = delete;
		FScopedSuppressActorImport& operator=(FScopedSuppressActorImport&& Other) = delete;

	private:
		bool bOldImportActorsValue = true;
		TWeakObjectPtr<UUsdStageImportOptions> Options;
	};
