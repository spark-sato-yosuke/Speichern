...
namespace PluginUtils
{
	...
	TSharedPtr<IPlugin> LoadPluginInternal(const FString& PluginName, const FString& PluginLocation, const FString& PluginFilePath, FPluginUtils::FLoadPluginParams& LoadParams, const bool bIsNewPlugin)
	{
		...
		if (LoadParams.bUpdateProjectPluginSearchPath && !PluginLocation.IsEmpty())
		{
	...
	void InitializeNewPluginParamsWithDescriptor(const FString& PluginName, const FPluginUtils::FNewPluginParams& CreationParams, FPluginUtils::FNewPluginParamsWithDescriptor& ExCreationParams)
	{
		...
		ExCreationParams.Descriptor.bEnableSceneGraph = CreationParams.bEnableSceneGraph;
	...
	TSharedPtr<IPlugin> CreateAndLoadNewPluginInternal(const FString& PluginName, const FString& NameToReplace, const FString& PluginLocation, const FString& PluginFilePath, const FPluginUtils::FNewPluginParamsWithDescriptor& CreationParams, FPluginUtils::FLoadPluginParams& LoadParams)
	{
		if (!ensure(!PluginFilePath.IsEmpty()))
		{
			return nullptr;
		}
		...
		const FString PluginFolder = FPluginUtils::GetPluginFolder(PluginFilePath, /*bFullPath*/ true);
		...
		do
		{
			...
			{
				const FString PluginContentFolder = FPluginUtils::GetPluginContentFolder(PluginFilePath, /*bFullPath*/ true);
			...
			{
				const FString ResourcesFolder = FPluginUtils::GetPluginResourcesFolder(PluginFilePath, /*bFullPath*/ true);
	...
	bool ValidateNewPluginFilePath(const FString& PluginFilePath, FText* FailReason)
	{
		// Check if a .uplugin file exists
	...
	bool ValidateNewPluginName(const FString& PluginName, FText* FailReason)
	{
...
FString FPluginUtils::GetPluginFolder(const FString& PluginFilePath, bool bFullPath)
{
...
FString FPluginUtils::GetPluginName(const FString& PluginFilePath)
{
	return FPaths::GetBaseFilename(PluginFilePath);
}
...
FString FPluginUtils::GetPluginContentFolder(const FString& PluginFilePath, bool bFullPath)
{
	FString PluginContentFolder = FPaths::Combine(FPaths::GetPath(PluginFilePath), TEXT("Content"));
	if (bFullPath)
	{
		PluginContentFolder = FPaths::ConvertRelativePathToFull(PluginContentFolder);
	}
...
FString FPluginUtils::GetPluginResourcesFolder(const FString& PluginFilePath, bool bFullPath)
{
	FString PluginResourcesFolder = FPaths::Combine(FPaths::GetPath(PluginFilePath), TEXT("Resources"));
	if (bFullPath)
	{
		PluginResourcesFolder = FPaths::ConvertRelativePathToFull(PluginResourcesFolder);
	}
...
{
	FPluginUtils::FNewPluginParamsWithDescriptor ExCreationParams;
	PluginUtils::InitializeNewPluginParamsWithDescriptor(PluginName, CreationParams, ExCreationParams);
...
{
	PRAGMA_DISABLE_DEPRECATION_WARNINGS
	return CreateAndLoadNewPlugin(PluginName, !CreationParams.NameToReplace.IsEmpty() ? CreationParams.NameToReplace : PluginName, PluginLocation, CreationParams, LoadParams);
	PRAGMA_ENABLE_DEPRECATION_WARNINGS
}
...
TSharedPtr<IPlugin> FPluginUtils::CreateAndLoadNewPlugin(const FString& PluginName, const FString& NameToReplace, const FString& PluginLocation, const FNewPluginParamsWithDescriptor& CreationParams, FLoadPluginParams& LoadParams)
{
	...
	return PluginUtils::CreateAndLoadNewPluginInternal(PluginName, NameToReplace, PluginLocation, PluginFilePath, CreationParams, LoadParams);
}

TSharedPtr<IPlugin> FPluginUtils::CreateAndLoadNewPlugin(const FString& PluginFilePath, const FNewPluginParams& CreationParams, FLoadPluginParams& LoadParams)
{
	FPluginUtils::FNewPluginParamsWithDescriptor ExCreationParams;
	PluginUtils::InitializeNewPluginParamsWithDescriptor(GetPluginName(PluginFilePath), CreationParams, ExCreationParams);

	return CreateAndLoadNewPlugin(PluginFilePath, ExCreationParams, LoadParams);
}

TSharedPtr<IPlugin> FPluginUtils::CreateAndLoadNewPlugin(const FString& PluginFilePath, const FNewPluginParamsWithDescriptor& CreationParams, FLoadPluginParams& LoadParams)
{
	// Early validations on new plugin params
	if (PluginFilePath.IsEmpty())
	{
		if (LoadParams.OutFailReason)
		{
			*LoadParams.OutFailReason = LOCTEXT("CreateNewPluginParam_NoPluginFilePath", "Missing plugin file name");
		}
		return nullptr;
	}

	if (!FPluginUtils::ValidateNewPluginFilePath(PluginFilePath, LoadParams.OutFailReason))
	{
		return nullptr;
	}

	const FString PluginName = GetPluginName(PluginFilePath);

	FString FullPluginFilePath = FPaths::ConvertRelativePathToFull(PluginFilePath);
	FPaths::MakePlatformFilename(FullPluginFilePath);

	return PluginUtils::CreateAndLoadNewPluginInternal(PluginName, !CreationParams.NameToReplace.IsEmpty() ? CreationParams.NameToReplace : PluginName, FString(), FullPluginFilePath, CreationParams, LoadParams);
}
...
TSharedPtr<IPlugin> FPluginUtils::LoadPlugin(const FString& PluginFilePath, FLoadPluginParams& LoadParams)
{
...
bool FPluginUtils::ValidateNewPluginNameAndLocation(const FString& PluginName, const FString& PluginLocation /*= FString()*/, FText* FailReason /*= nullptr*/)
{
	...
	if (!PluginLocation.IsEmpty())
	{
		if (!PluginUtils::ValidateNewPluginFilePath(FPluginUtils::GetPluginFilePath(PluginLocation, PluginName), FailReason))
		{
			return false;
		}
		...
		if (!PluginUtils::ValidateNewPluginFolder(FPluginUtils::GetPluginFolder(PluginLocation, PluginName, true /*bFullPath*/), FailReason))
		{
			return false;
		}
	}

	if (!PluginUtils::ValidateNewPluginName(PluginName, FailReason))
	{
		return false;
	}
...
bool FPluginUtils::ValidateNewPluginFilePath(const FString& PluginFilePath, FText* FailReason /*= nullptr*/)
{
	const FString PluginName = GetPluginName(PluginFilePath);
	...
	if (!PluginUtils::ValidateNewPluginFilePath(PluginFilePath, FailReason))
	{
		return false;
	}

	if (!PluginUtils::ValidateNewPluginFolder(FPluginUtils::GetPluginFolder(PluginFilePath, true /*bFullPath*/), FailReason))
	{
		return false;
	}
