...
void FConcertPropertySelection::DiscoverAndAddImplicitParentProperties()
{
    ...
    for (FConcertPropertyChain Current : TSet(ReplicatedProperties))
    {
       while (!Current.IsEmpty())
       {
          ReplicatedProperties.Add(Current);
...
{
    const TSet<FConcertPropertyChain> Intersection = First.Intersect(Second);
    for (const FConcertPropertyChain& ThisChain : Intersection)
    {
       if (Callback(ThisChain) == EBreakBehavior::Break) 
       {
          return true;
       }
    }
