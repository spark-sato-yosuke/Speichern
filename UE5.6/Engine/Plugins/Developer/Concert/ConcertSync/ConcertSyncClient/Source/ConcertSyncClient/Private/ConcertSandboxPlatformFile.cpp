...
IAsyncReadFileHandle* FConcertSandboxPlatformFile::OpenAsyncRead(const TCHAR* Filename, bool bAllowWrite /*= false*/)
{
	const FConcertSandboxPlatformFilePath ResolvedPath = ToSandboxPath(Filename);
...
void FConcertSandboxPlatformFile::MigrateFileToSandbox(const FConcertSandboxPlatformFilePath& InPath) const
{
	...
	{
		UE_LOG(LogConcert, Verbose, TEXT("MigrateFileToSandbox: Sandbox has the file %s already at this location. Nothing to do."), *InPath.GetSandboxPath() );
	...
	{
		UE_LOG(LogConcert, Verbose, TEXT("MigrateFileToSandbox: There is no file %s at this location. Nothing to do."), *InPath.GetNonSandboxPath() );
		// Non-sandbox has no file at this location - nothing to do
		return;
	}

	UE_LOG(LogConcert, Verbose, TEXT("MigrateFileToSandbox: Copying file %s into the sandbox."), *InPath.GetNonSandboxPath() );
