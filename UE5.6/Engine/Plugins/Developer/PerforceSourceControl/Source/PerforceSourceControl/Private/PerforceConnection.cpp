...
bool bDebugBreakConnection = false;
static FAutoConsoleVariableRef CVarDebugBreakConnection(
	TEXT("SourceControl.Perforce.Debug.BreakConnection"),
	bDebugBreakConnection,
	TEXT("Fails any attempt to run a Perforce command."),
	ECVF_Default);
...
FPerforceConnection::FPerforceConnection(const FPerforceConnectionInfo& InConnectionInfo, FPerforceSourceControlProvider& InSCCProvider, FSourceControlResultInfo* OutResultInfo)
	: bEstablishedConnection(false)
	, bIsUnicode(false)
	, LatestCommunicateTime(0.0)
	, SCCProvider(InSCCProvider)
{
	EstablishConnection(InConnectionInfo, OutResultInfo);
}
...
bool FPerforceConnection::AutoDetectWorkspace(const FPerforceConnectionInfo& InConnectionInfo, FPerforceSourceControlProvider& SCCProvider, FString& OutWorkspaceName, TArray<FText>& OutErrorMessages)
{
	...
	FPerforceConnection Connection(InConnectionInfo, SCCProvider, nullptr);
		...
		if (bCommandOK)
		{
			...
			for (const FP4Record& ClientRecord : Records)
			{
				...
				if( bHostNameMatches || (bHostNameWild && bAllowWildHosts) )
				{
					...
					if (bIsNullClientRootPath || FPathViews::IsParentPathOf(ClientRootPath, ApplicationPath))
					{
                                        ...
                                        FOnIsCancelled InIsCancelled, bool& OutConnectionDropped, ERunCommandFlags RunFlags)
{
	...
	if (!bEstablishedConnection || bDebugBreakConnection)
	{
		return false;
	}
...
void FPerforceConnection::EstablishConnection(const FPerforceConnectionInfo& InConnectionInfo, FSourceControlResultInfo* OutResultInfo)
{
	...
	if (P4Error.Test())
	{
		...
		if (OutResultInfo != nullptr)
		{
			OutResultInfo->ErrorMessages.Add(FText::FromString(TEXT("P4ERROR: Invalid connection to server.")));
			OutResultInfo->ErrorMessages.Add(FText::FromString(ANSI_TO_TCHAR(ErrorMessage.Text())));
		}
		else
		{
	...
	{
		TArray<FString> Params;
		FSourceControlResultInfo DummyResultInfo;
		FP4RecordSet Records;
		bool bConnectionDropped = false;
		const ERunCommandFlags RunFlags = ERunCommandFlags::DisableCommandLogging;
		...
		if (RunCommand(TEXT("info"), Params, Records, nullptr, DummyResultInfo, FOnIsCancelled(), bConnectionDropped, RunFlags))
		{
...
{
	FSourceControlResultInfo* ResultInfo = InCommand.Operation->ShouldLogErrors() ? nullptr : &InCommand.ResultInfo;

	Initialize(InCommand.ConnectionInfo, InCommand.Worker->GetSCCProvider(), ResultInfo);
	if (IsValid())
	{
...
{
	Initialize(SCCProvider.AccessSettings().GetConnectionInfo(), SCCProvider, nullptr);
}

void FScopedPerforceConnection::Initialize( const FPerforceConnectionInfo& InConnectionInfo, FPerforceSourceControlProvider& SCCProvider, FSourceControlResultInfo* OutResultInfo)
{
	if(Concurrency == EConcurrency::Synchronous)
	{
		...
		if (SCCProvider.EstablishPersistentConnection(OutResultInfo) )
		{
	...
	else
	{
		...
		FPerforceConnection* NewConnection = new FPerforceConnection(InConnectionInfo, SCCProvider, OutResultInfo);
