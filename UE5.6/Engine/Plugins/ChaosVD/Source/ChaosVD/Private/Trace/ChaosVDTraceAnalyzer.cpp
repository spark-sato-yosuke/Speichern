...
void FChaosVDTraceAnalyzer::PushSimulatedSolverTrackForGTData(const UE::Trace::IAnalyzer::FOnEventContext& Context, const UE::Trace::IAnalyzer::FEventData& EventData)
{
	...
	NewFrameData.SolverID = CurrentGTTrackID;
	NewFrameData.FrameCycle = EventData.GetValue<uint64>("Cycle");
	NewFrameData.bIsKeyFrame = true;
	NewFrameData.bIsResimulated = false;
	NewFrameData.StartTime = Context.EventTime.AsSeconds(NewFrameData.FrameCycle);

	static FString GeneratedStageName = TEXT("Stage 0");
	NewFrameData.SolverSteps.AddDefaulted_GetRef().StepName = GeneratedStageName;

	NewFrameData.GetCustomData().GetOrAddDefaultData<FChaosVDGameFrameDataWrapper>()->FrameData = MakeShared<FChaosVDGameFrameData>();

	NewFrameData.DebugFName = FName(TEXT("Additional Game Frame Data Track"));
	
	NewFrameData.AddAttributes(EChaosVDSolverFrameAttributes::HasGTDataToReRoute);
