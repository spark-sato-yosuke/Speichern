...
TSharedRef<SWidget> SMediaFrameworkCapture::CreateSettingsMenu()
{
...

	{
		...
		SettingsMenuBuilder.BeginSection("ViewportCaptureSection", LOCTEXT("ViewportCaptureSectionHeader", "Viewport Capture"));
		{
			typedef SNumericVectorInputBox<int32, FIntPoint, 2> SNumericVectorInputBoxInt2;

			const TSharedRef<SWidget> InputBox = SNew(SNumericVectorInputBoxInt2)
				.Font(FAppStyle::Get().GetFontStyle("PropertyWindow.NormalFont"))
				.AllowSpin(false)
				.bColorAxisLabels(false)
				.X(this, &SMediaFrameworkCapture::GetDefaultViewportSize, 0)
				.OnXChanged(this, &SMediaFrameworkCapture::DefaultViewportSizeChanged, 0)
				.OnXCommitted(this, &SMediaFrameworkCapture::DefaultViewportSizeCommitted, 0)
				.Y(this, &SMediaFrameworkCapture::GetDefaultViewportSize, 1)
				.OnYChanged(this, &SMediaFrameworkCapture::DefaultViewportSizeChanged, 1)
				.OnYCommitted(this, &SMediaFrameworkCapture::DefaultViewportSizeCommitted, 1);
			
			SettingsMenuBuilder.AddWidget(InputBox,
				LOCTEXT("DefaultViewportCaptureSize_Label", "Default Size"),
				false /*bInNoIndent*/,
				true /*bInSearchable*/,
				LOCTEXT("DefaultViewportCaptureSize_Tooltip", "Default resolution to use for viewport capture if it is not specified by the media output.")
				);
		}
...
TOptional<int32> SMediaFrameworkCapture::GetDefaultViewportSize(int32 InValueIndex) const
{
	if (InValueIndex >= 0 && InValueIndex < 2)
	{
		return GetMutableDefault<UMediaFrameworkMediaCaptureSettings>()->DefaultViewportCaptureSize(InValueIndex);
	}
	return TOptional<int32>();
}
	
void SMediaFrameworkCapture::DefaultViewportSizeChanged(int32 InValue, int32 InValueIndex)
{
	if (InValueIndex >= 0 && InValueIndex < 2)
	{
		GetMutableDefault<UMediaFrameworkMediaCaptureSettings>()->DefaultViewportCaptureSize(InValueIndex) = InValue;
	}
}
	
void SMediaFrameworkCapture::DefaultViewportSizeCommitted(int32 InValue, ETextCommit::Type InCommitInfo, int32 InValueIndex)
{
	if (InCommitInfo == ETextCommit::OnEnter || InCommitInfo == ETextCommit::OnUserMovedFocus)
	{
		if (InValueIndex >= 0 && InValueIndex < 2)
		{
			const FScopedTransaction Transaction(LOCTEXT("DefaultViewportCaptureSize_Transaction", "Default Viewport Capture Size"));
			GetMutableDefault<UMediaFrameworkMediaCaptureSettings>()->Modify();
			GetMutableDefault<UMediaFrameworkMediaCaptureSettings>()->DefaultViewportCaptureSize(InValueIndex) = InValue;
			GetMutableDefault<UMediaFrameworkMediaCaptureSettings>()->SaveConfig();
		}
