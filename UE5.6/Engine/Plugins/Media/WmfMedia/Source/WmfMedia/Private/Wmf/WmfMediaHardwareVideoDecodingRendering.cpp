...

	{
		...
		if (InSample->GetFormat() == EMediaTextureSampleFormat::CharNV12)
		{
			...
			FShaderResourceViewRHIRef Y_SRV = RHICmdList.CreateShaderResourceView(
				SampleDestinationTexture, 
				FRHIViewDesc::CreateTextureSRV()
					.SetDimensionFromTexture(SampleDestinationTexture)
					.SetMipRange(0, 1)
					.SetFormat(PF_G8));
			...
			SetShaderParameters(RHICmdList, PixelShader, VertexShader.GetPixelShader(), Parameters);
		}
		...
		else if (InSample->GetFormat() == EMediaTextureSampleFormat::YCoCg_DXT5_Alpha_BC4)
		{
			...
			FShaderResourceViewRHIRef SRV = RHICmdList.CreateShaderResourceView(
				...
				FRHIViewDesc::CreateTextureSRV()
					.SetDimensionFromTexture(SampleDestinationTexture)
					.SetFormat(PF_DXT5));
			FShaderResourceViewRHIRef SRVAlpha = RHICmdList.CreateShaderResourceView(
				SampleDestinationAlphaTexture, 
				FRHIViewDesc::CreateTextureSRV()
					.SetDimensionFromTexture(SampleDestinationAlphaTexture)
					.SetFormat(PF_BC4));
