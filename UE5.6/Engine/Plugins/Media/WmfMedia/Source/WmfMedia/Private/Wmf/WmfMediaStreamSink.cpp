...
void FWmfMediaStreamSink::CopyTextureAndEnqueueSample(IMFSample* pSample)
{
	...
	if (!IsSampleStillDesired())
	{
		UE_LOG(LogWmfMedia, VeryVerbose, TEXT("Sample is from previous sequence, dropping sample"));
		return;
	}

	if (IsVideoSampleQueueFull())
	{
		UE_LOG(LogWmfMedia, VeryVerbose, TEXT("Queue is full, dropping sample"));
		return;
	}
