...
public:
	 ...
	 */
	WMFMEDIA_API ID3D11Texture2D* InitializeSourceTexture(const TRefCountPtr<ID3D11Device>& InD3D11Device, FMediaTimeStamp InTime, FTimespan InDuration, const FIntPoint& InDim, EPixelFormat InFormat, EMediaTextureSampleFormat InMediaTextureSampleFormat);
	...
	{
		FWmfMediaHardwareVideoDecodingParameters::ConvertTextureFormat_RenderThread(RHICmdList, this, InDstTexture);
		return true;
	}
	...
	FTextureRHIRef GetOrCreateDestinationTexture()
	{
		...
		FRHITextureCreateDesc Desc =
			...
			.SetFlags(ETextureCreateFlags::DisableSRVCreation);
	 ...
	 */
#if !UE_SERVER
	WMFMEDIA_API virtual void ShutdownPoolable() override;
#endif
