...
void FWebMVideoDecoder::ConvertYUVToRGBAndSubmit(const FConvertParams& Params)
{
...

	{
		...
		{
			uint32 Stride = 0;
			unsigned char* TextureMemory = (unsigned char*)InCommandList.LockTexture2D(RHITexture, 0, RLM_WriteOnly, Stride, false);
			...
			if (TextureMemory)
			{
				...
				InCommandList.UnlockTexture2D(RHITexture, 0, false);
			}
		};
