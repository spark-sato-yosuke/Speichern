...
#if ANDROIDMEDIAPLAYER_USE_NATIVELOGGING
		FPlatformMisc::LowLevelOutputDebugStringf(TEXT("Fetch RT: Created VideoTexture for Player: %s"), *PlayerGuid.ToString());
#endif
	}
}
...
static void DoUpdateMediaFrameDataMediaSampleExecute(TWeakPtr<FJavaAndroidMediaPlayer, ESPMode::ThreadSafe> JavaMediaPlayerPtr, TWeakPtr<FMediaSamples, ESPMode::ThreadSafe> SamplesPtr,
	TSharedRef<FAndroidMediaTextureSample, ESPMode::ThreadSafe> VideoSample)
{
	auto PinnedJavaMediaPlayer = JavaMediaPlayerPtr.Pin();
	auto PinnedSamples = SamplesPtr.Pin();
	...
	jobject FrameDataGlobalRef = PinnedJavaMediaPlayer->GetVideoLastMediaFrameData();
	if (FrameDataGlobalRef == nullptr)
	{
		return;
	}

	VideoSample->InitializeMediaFrameData(FrameDataGlobalRef, Time);

	PinnedSamples->AddVideo(VideoSample);
}

struct FRHICommandUpdateMediaFrameDataMediaSample final : public FRHICommand<FRHICommandUpdateMediaFrameDataMediaSample>
{
	TWeakPtr<FJavaAndroidMediaPlayer, ESPMode::ThreadSafe> JavaMediaPlayerPtr;
	TWeakPtr<FMediaSamples, ESPMode::ThreadSafe> SamplesPtr;
	TSharedRef<FAndroidMediaTextureSample, ESPMode::ThreadSafe> VideoSample;

	FORCEINLINE_DEBUGGABLE FRHICommandUpdateMediaFrameDataMediaSample(TWeakPtr<FJavaAndroidMediaPlayer, ESPMode::ThreadSafe> InJavaMediaPlayerPtr, TWeakPtr<FMediaSamples, ESPMode::ThreadSafe> InSamplesPtr,
		TSharedRef<FAndroidMediaTextureSample, ESPMode::ThreadSafe> InVideoSample)
		: JavaMediaPlayerPtr(InJavaMediaPlayerPtr)
		, SamplesPtr(InSamplesPtr)
		, VideoSample(InVideoSample)
	...
	{
		QUICK_SCOPE_CYCLE_COUNTER(STAT_FRHICommandUpdateTextureMediaSample_Execute);
		DoUpdateMediaFrameDataMediaSampleExecute(JavaMediaPlayerPtr, SamplesPtr, VideoSample);
	}
};
...
void FAndroidMediaPlayer::TickFetch(FTimespan DeltaTime, FTimespan /*Timecode*/)
{
	...
	if (FAndroidMisc::ShouldUseVulkan())
	{
		...
		if (!JavaMediaPlayer->IsUsingBitmapRendererImageReader())
		{
			ENQUEUE_RENDER_COMMAND(AndroidMediaPlayerWriteVideoSample)(
		...
		else
		{
			ENQUEUE_RENDER_COMMAND(AndroidMediaPlayerWriteVideoSample)(
				[Params](FRHICommandListImmediate& RHICmdList)
				{
					DoUpdateMediaFrameDataMediaSampleExecute(Params.JavaMediaPlayerPtr, Params.SamplesPtr, Params.VideoSample);
				});
		}
	}
#if ANDROIDMEDIAPLAYER_USE_EXTERNALTEXTURE
	else if (GSupportsImageExternal)
	{
		...
		if (!JavaMediaPlayer->IsUsingBitmapRendererImageReader())
		{
			ENQUEUE_RENDER_COMMAND(AndroidMediaPlayerWriteVideoSample)(
				[Params](FRHICommandListImmediate& RHICmdList)
				{
					...
					Params.VideoSample->InitializeExternalTexture(Time);
		...
		else
		{
			ENQUEUE_RENDER_COMMAND(AndroidMediaPlayerWriteVideoSample)(
				[Params](FRHICommandListImmediate& RHICmdList)
				{
					DoUpdateMediaFrameDataMediaSampleExecute(Params.JavaMediaPlayerPtr, Params.Samples, Params.VideoSample);
				});
		}
	}
#endif // ANDROIDMEDIAPLAYER_USE_EXTERNALTEXTURE
	...
	else
	{
		ENQUEUE_RENDER_COMMAND(AndroidMediaPlayerWriteVideoSample)(
			[Params](FRHICommandListImmediate& RHICmdList)
			{
				if (IsRunningRHIInSeparateThread())
				{
					new (RHICmdList.AllocCommand<FRHICommandUpdateHardwareBufferMediaSample>()) FRHICommandUpdateHardwareBufferMediaSample(Params.JavaMediaPlayerPtr, Params.SamplesPtr, Params.VideoSample);
				}
				else
				{
					DoUpdateMediaFrameDataMediaSampleExecute(Params.JavaMediaPlayerPtr, Params.SamplesPtr, Params.VideoSample);
