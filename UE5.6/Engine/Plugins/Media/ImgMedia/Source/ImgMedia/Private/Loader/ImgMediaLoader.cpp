...
#include "ImgMediaLoaderUtils.h"
#include "Readers/IImgMediaReader.h"
#include "Readers/DdsImgMediaReader.h"
...
FImgMediaLoader::FImgMediaLoader(const TSharedRef<FImgMediaScheduler, ESPMode::ThreadSafe>& InScheduler,
	...
	const TSharedPtr<FNativeMediaSourceColorSettings, ESPMode::ThreadSafe>& InSourceColorSettings,
...
{
	ResetFetchLogic();
	QueuedSampleFetch.LastTimeStamp.SetTimeAndIndexValue(FTimespan::Zero(), 0);
...
TSharedPtr<FImgMediaTextureSample, ESPMode::ThreadSafe> FImgMediaLoader::GetFrameSample(FTimespan Time)
{
	...
	Sample->Initialize(*Frame->Get(), SequenceDim, FMediaTimeStamp(FrameStartTime, 0), NextStartTime - FrameStartTime, GetNumMipLevels(), TilingDescription, SourceColorSettings);
...
void FImgMediaLoader::Flush()
{
	...
	QueuedSampleFetch.LastTimeStamp.SetSequenceIndex(0);
	QueuedSampleFetch.LastTimeStamp.SetLoopIndex(0);
}

void FImgMediaLoader::Seek(const FMediaTimeStamp& SeekTarget, bool bReverse)
{
	ResetFetchLogic();
	QueuedSampleFetch.LastTimeStamp.SetTimeAndIndexValue(SeekTarget.Time, SeekTarget.GetIndexValue());
	QueuedSampleFetch.LastDuration = FTimespan::Zero();
}
...
IMediaSamples::EFetchBestSampleResult FImgMediaLoader::FetchBestVideoSampleForTimeRange(const TRange<FMediaTimeStamp>& InTimeRange, TSharedPtr<IMediaTextureSample, ESPMode::ThreadSafe>& OutSample, bool bIsLoopingEnabled, float PlayRate)
{
	if (IsInitialized() && InTimeRange.HasLowerBound() && InTimeRange.HasUpperBound())
	{
		...
		check(InTimeRange.GetLowerBoundValue().GetSequenceIndex() == InTimeRange.GetUpperBoundValue().GetSequenceIndex());
		check(QueuedSampleFetch.LastTimeStamp.IsValid());
		...
		{
			TimeRange.SetLowerBoundValue(FMediaTimeStamp(FMath::Clamp(TimeRange.GetLowerBoundValue().Time, FTimespan::Zero(), SequenceDuration), TimeRange.GetLowerBoundValue().GetIndexValue()));
			TimeRange.SetUpperBoundValue(FMediaTimeStamp(FMath::Clamp(TimeRange.GetUpperBoundValue().Time, FTimespan::Zero(), SequenceDuration), TimeRange.GetUpperBoundValue().GetIndexValue()));
		}
		FTimespan StartTime = TimeRange.GetLowerBoundValue().Time;
		int32 StartLoopIndex = TimeRange.GetLowerBoundValue().GetLoopIndex();
		...
		{
			EndTime -= FTimespan(1);
			EndLoopIndex = TimeRange.GetUpperBoundValue().GetLoopIndex();
		}
		else
		{
			EndTime = SequenceDuration - FTimespan(1);
			EndLoopIndex = TimeRange.GetUpperBoundValue().GetLoopIndex() - 1;
		}
		...
		if (Frame)
		{
			...
			*/
			...
			QueuedSampleFetch.LastTimeStamp.SetTime(SampleTime);
			QueuedSampleFetch.LastTimeStamp.SetSequenceIndex(TimeRange.GetLowerBoundValue().GetSequenceIndex());
			QueuedSampleFetch.LastTimeStamp.SetLoopIndex(QueuedSampleFetch.LoopIndex);
			...
			Sample->Initialize(*Frame->Get(), SequenceDim, QueuedSampleFetch.LastTimeStamp, QueuedSampleFetch.LastDuration, GetNumMipLevels(), TilingDescription, SourceColorSettings);

			OutSample = Sample;
			CSV_EVENT(ImgMedia, TEXT("LoaderFetchHit %" INT64_FMT " %" INT64_FMT "-%" INT64_FMT), MaxIdx, UnwrappedStartIndex% NumFrames, UnwrappedEndIndex% NumFrames);
		...
		CSV_EVENT(ImgMedia, TEXT("LoaderFetchMiss %" INT64_FMT "-%" INT64_FMT), UnwrappedStartIndex % NumFrames, UnwrappedEndIndex % NumFrames);
	}
...
{
	// When playing (i.e not seeking), we don't actually return a "frame" here as we do NOT involve any requests to the cache,
	// we simply assume we could deliver what a "queue" would contain on demand later (in FetchBestVideoSampleForTimeRange).
	// However, when seeking, we need to check the cache here so the seek status properly reflect the state of the cache and allow
	// for seek throttling to work as intended.
	if (IsInitialized())
	{
		check(QueuedSampleFetch.LastTimeStamp.IsValid());
		...
		if (PlayRate >= 0.0f)
		{
			...
			while(TimeStamp.Time >= SequenceDuration)
			{
		...
		// If waiting for a seek and there is no other playback blocking,
		// properly indicate if the sample has arrived to allow for seek throttling.
		if (QueuedSampleFetch.LastDuration.IsZero() && !bIsPlaybackBlocking)
		{
			const uint32 PeekFrameNumber = TimeToFrameNumber(TimeStamp.Time);

			bool bHasFrame;
			if (UseGlobalCache)
			{
				bHasFrame = GlobalCache->Contains(SequenceName, PeekFrameNumber);
			}
			else
			{
				FScopeLock ScopeLock(&CriticalSection);
				bHasFrame = Frames.Contains(PeekFrameNumber);
			}

			if (!bHasFrame)
			{
				// Request the missing frame to avoid blocking.
				RequestFrame(PeekFrameNumber, PlayRate, bIsLoopingEnabled);

				// Need to invalidate the time stamp because FMediaPlayerFacade::UpdateSeekStatus ignores the return value.
...
void FImgMediaLoader::Initialize(const FString& SequencePath, const FFrameRate& FrameRateOverride,
	bool Loop, bool bIsSequencerEnvironment)
{
	...
	{
		WarmupSequence(FirstFrameInfo, Loop, bIsSequencerEnvironment);
	}
...
bool FImgMediaLoader::LoadSequence(const FString& SequencePath, const FFrameRate& FrameRateOverride, bool Loop, FImgMediaFrameInfo& FirstFrameInfo)
{
	...
	else if (FirstExtension == TEXT("dds"))
	{
		Reader = MakeShared<FDdsImgMediaReader>(AsShared());
	}
	else
	{
...
void FImgMediaLoader::WarmupSequence(const FImgMediaFrameInfo& InFirstFrameInfo, bool Loop, bool bIsSequencerEnvironment)
{
	/**
	* With sequencer, the warmup pre-allocations had negative consequences outweighting any potential benefits:
	* - Prevented asynchronous work in pre-roll section due to spike in render thread.
	* - In cases where only a few mips or tiles are visible, the allocations were overly greedy.
	* - Increased risk of blocking playback disabling if warmup allocations exceed timeout.
	*/
	if (!bIsSequencerEnvironment)
	{
...
void FImgMediaLoader::FindFiles(const FString& SequencePath, TArray<FString>& OutputPaths)
{
	...
	const TSet<FString> SupportedImageFileExtensions = { TEXT("exr"), TEXT("jpg"), TEXT("jpeg"), TEXT("png"), TEXT("bmp"), TEXT("dds") };
...
uint32 FImgMediaLoader::TimeToFrameNumber(FTimespan Time) const
{
	...
	*/
	...
	return UE::ImgMediaLoaderUtils::TimeToFrameNumber(Time, SequenceFrameRate);
}

FTimespan FImgMediaLoader::FrameNumberToTime(uint32 FrameNumber) const
{
	// Note: this version is reversible with TimeToFrameNumber.
	return UE::ImgMediaLoaderUtils::GetFrameStartTime(FrameNumber, SequenceFrameRate);
}

int64 FImgMediaLoader::TimeToFrameNumberUnbound(FTimespan Time) const
{
	return UE::ImgMediaLoaderUtils::TimeToFrameNumberUnbound(Time, SequenceFrameRate);
}
