...
struct FNativeMediaSourceColorSettings;
...
public:
	 ...
	 * @param bIsSequencerEnvironment Whether image sequence playback comes from sequencer, to decide on warmup behavior.
	 * @see IsInitialized
	 */
	void Initialize(const FString& SequencePath, const FFrameRate& FrameRateOverride, bool Loop, bool bIsSequencerEnvironment);
	...
	/**
	 * Source color settings getter.
	 */
	TSharedPtr<FNativeMediaSourceColorSettings, ESPMode::ThreadSafe> GetSourceColorSettings() const { return SourceColorSettings; }
...
protected:
	 ...
	 * @param Loop Whether the cache should loop around.
	 * @param bIsSequencerEnvironment Whether image sequence playback comes from sequencer.
	 */
	void WarmupSequence(const FImgMediaFrameInfo& InFirstFrameInfo, bool Loop, bool bIsSequencerEnvironment);
...
private:
	...
	/** Container for media source color settings. */
...
private:
	...
	FQueuedSampleFetch QueuedSampleFetch;
