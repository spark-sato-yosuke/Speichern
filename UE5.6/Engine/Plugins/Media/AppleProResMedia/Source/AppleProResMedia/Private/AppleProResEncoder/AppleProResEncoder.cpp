...
bool FAppleProResEncoder::Initialize()
{
	...
	CurrentVideoTime.value = Options.FrameNumberOffset * Options.FrameRate.Denominator;
	CurrentVideoTime.timescale = Options.FrameRate.Numerator;
	CurrentVideoTime.flags = kPRTimeFlags_Valid;
	CurrentVideoTime.epoch = 0;
	...
	// Add a Audio stream
	{
		if (Options.bIncludeAudio)
		{
			...
			// Sound tracks must match the sample rate, not frame rate.
			if(!ensureMsgf(Options.AudioSampleRate > 0, TEXT("AudioSampleRate must be greater than zero!")))
			{
				return false;
			}
		
			PRTimeScale ProResAudioTimescale = Options.AudioSampleRate;
			Status = ProResFileWriterSetTrackMediaTimescale(FileWriter, AudioTrackId, ProResAudioTimescale);
			if (Status != 0)
			{
	...
	Status = ProResFileWriterBeginSession(FileWriter, CurrentVideoTime);
	if (Status != 0)
	{
...
bool FAppleProResEncoder::InitializeAudioTrack()
{
	...
	AudioSettings.mSampleRate = Options.AudioSampleRate; 
	...
	// To figure out how much audio data there is per sample, we take our AudioSampleRate (48000) and divide it by our framerate, ie: 48'000 / 30 = 1600
	AudioSamplesPerFrame = FMath::FloorToInt(Options.AudioSampleRate * Options.FrameRate.AsInterval());

	// Audio is uncompressed lpcm, and for Apple ProRes 1 frame is 1 sample across all channels, and 1 packet is 1 frame in compressed files.
	// Then when it's submitted, we can submit one video-frame's worth of data at a time.
	AudioSettings.mBytesPerPacket = Options.AudioChannelCount * sizeof(int16); // 2 channels of signed int16 PCM
	AudioSettings.mFramesPerPacket = 1;
	AudioSettings.mBytesPerFrame = Options.AudioChannelCount * sizeof(int16);
	AudioSettings.mChannelsPerFrame = Options.AudioChannelCount;
	AudioSettings.mBitsPerChannel = sizeof(int16)*8;
	AudioSettings.mReserved = 0; // Padding

	CurrentAudioTime.value = Options.FrameNumberOffset * Options.FrameRate.Denominator;
	CurrentAudioTime.timescale = Options.AudioSampleRate;
	CurrentAudioTime.flags = kPRTimeFlags_Valid;
	CurrentAudioTime.epoch = 0;
...
void FAppleProResEncoder::Finalize()
{
	...
	if (FileWriter)
	{
		...
		CurrentVideoTime.value += Options.FrameRate.Denominator;
		Status = ProResFileWriterEndSession(FileWriter, CurrentVideoTime);
		if (Status != 0)
		{
...
bool FAppleProResEncoder::WriteFrame(const FImagePixelData* InPixelData)
{
	...
	SampleTimingInfo.timeStamp = CurrentVideoTime;
...
#if PLATFORM_MAC
	// This API is more efficient on Mac
	PRStatus Status = ProResFileWriterAddEncoderSampleBufferToTrack(
		FileWriter,
		VideoTrackId,
		Encoder,
		SampleTimingInfo.timeStamp);
#else
...
bool FAppleProResEncoder::WriteAudioSample(const TArrayView<int16>& InAudioSamples)
{
	...
	int32 NumFramesOfData = SampleCountPerChannel / AudioSamplesPerFrame;

	int16* SourceDataPtr = (int16*)InAudioSamples.GetData();
	for (int32 SampleIdx = 0; SampleIdx < NumFramesOfData; SampleIdx++)
	{
		// The number of bytes per frame is channel count (due to interleaved audio) * sizeof(int16) (signed 16-bit audio) * AudioSamplesPerFrame.
		const size_t BufferSize = AudioSamplesPerFrame * Options.AudioChannelCount * sizeof(int16);
		int32 NumSamplesInBuffer = AudioSamplesPerFrame;
		PRStatus Status = ProResFileWriterAddAudioSampleBufferToTrack(
			FileWriter,
			AudioTrackId,
			(void*)SourceDataPtr,
			BufferSize,
		...
		// Increment our source pointer to the next frame's data.
		SourceDataPtr += AudioSamplesPerFrame * Options.AudioChannelCount;

		// Increment our CurrentAudioTime (which is in hz, ie: 48'000) by 1 frame's worth of samples.
		CurrentAudioTime.value += AudioSamplesPerFrame;

		if (Status != 0)
		{
			UE_LOG(LogAppleProResMedia, Error, TEXT("Failed to add sample to Audio Track for file writer at path: \"%s\"."), *Options.OutputFilename);
			return false;
		}
