...
UCLASS(config = Game, defaultconfig, meta = (DisplayName = "PixelStreaming2"))
class PIXELSTREAMING2SETTINGS_API UPixelStreaming2PluginSettings : public UDeveloperSettings
{
	GENERATED_BODY()

	virtual ~UPixelStreaming2PluginSettings();
...
public:
	...
	static TAutoConsoleVariable<FString> CVarEpicRtcLogFilter;
	UPROPERTY(config, EditAnywhere, Category = "PixelStreaming", meta = (
	...
	static TAutoConsoleVariable<float> CVarSignalingMaxReconnectAttempts;
	UPROPERTY(config, EditAnywhere, Category = "PixelStreaming", meta = (
	...
	static TAutoConsoleVariable<FString> CVarDefaultStreamerType;
	UPROPERTY(config, EditAnywhere, Category = "PixelStreaming", meta = (
	...
	static TAutoConsoleVariable<bool> CVarAutoStartStream;
	UPROPERTY(config, EditAnywhere, Category = "PixelStreaming", meta = (
	...
	static TAutoConsoleVariable<FString> CVarConnectionURL;
	UPROPERTY(config, EditAnywhere, Category = "PixelStreaming", meta = (
	...
	static FAutoConsoleVariableDeprecated CVarSignallingURL;
	...
	static TAutoConsoleVariable<bool> CVarEncoderDebugDumpFrame;
	...
	static TAutoConsoleVariable<FString> CVarWebRTCPortAllocatorFlags;
	UPROPERTY(config, EditAnywhere, Category = "WebRtcSettings", meta = (
		Bitmask,
	...
	static EPortAllocatorFlags GetPortAllocationFlags();
...
private:
	...
	UFUNCTION()
	TArray<FString> GetVideoCodecOptions() const;

	UFUNCTION()
	TArray<FString> GetScalabilityModeOptions() const;

	UFUNCTION()
	TArray<FString> GetWebRTCCodecPreferencesOptions() const;

	UFUNCTION()
	TArray<FString> GetDefaultStreamerTypeOptions() const;
};