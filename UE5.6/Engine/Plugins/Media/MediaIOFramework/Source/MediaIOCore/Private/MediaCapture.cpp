...
UMediaCapture::UMediaCapture()
	...
	, bPendingOutputResourcesInitialized(false)
...
bool UMediaCapture::StartSourceCapture(TSharedPtr<UE::MediaCapture::Private::FCaptureSource> InSource)
{
	...
	if (bInitialized)
	{
		...
		bInitialized = GetState() != EMediaCaptureState::Stopped && GetState() != EMediaCaptureState::Error;
	}
	...
	{
		StopCapture(/*bAllowPendingFrameToBeProcess*/ false);
		MediaCaptureDetails::ShowSlateNotification();
	}
...
void UMediaCapture::StopCapture(bool bAllowPendingFrameToBeProcess)
{
	...
	else
	{
		...
		if (GetState() != EMediaCaptureState::Stopped)
		{
			...
			while (WaitingForRenderCommandExecutionCounter.load() > 0 || bPendingOutputResourcesInitialized)
			{
...
bool UMediaCapture::CaptureImmediate_RenderThread(const UE::MediaCaptureData::FCaptureFrameArgs& Args)
{
	...
	if (bPendingOutputResourcesInitialized)
	{
		...
		if (CaptureSource)
		{
				...
				[InMediaCapture, CapturingFrame, InDesiredSize, SourceGPUMask](FRHICommandListImmediate& RHICmdList)
			{
				...
				// Capture frame
				{
					...
					FTextureRHIRef SourceTexture = InMediaCapture->CaptureSource->GetSourceTextureForInput_RenderThread();
			...
			if (DesiredCaptureOptions.bAutostopOnCapture && GetState() == EMediaCaptureState::Capturing && CaptureRequestCount == DesiredCaptureOptions.NumberOfFramesToCapture)
			{
				StopCapture(true);
			}
		}
...
void UMediaCapture::InitializeOutputResources(int32 InNumberOfBuffers)
{
	...
	bPendingOutputResourcesInitialized = true;
	...
	[This](FRHICommandListImmediate& RHICmdList)
		{
			for (int32 Index = 0; Index < This->NumberOfCaptureFrame; ++Index)
			{
				if (This->DesiredOutputResourceType == EMediaCaptureResourceType::Texture)
				{
					...
					if (This->GetDesiredCaptureOptions().bAutostopOnCapture)
					{
						// Don't preroll if capturing a set number of frames since we discard the first frames.
						NewFrame->bPrerollFrame = false;
					}
			...
			This->bPendingOutputResourcesInitialized = false;
		});
}
...
void UMediaCapture::OnBeginFrame_GameThread()
{
	...
	if (CaptureSource->GetCaptureType() == UE::MediaCapture::Private::ECaptureType::Immediate && (GetState() == EMediaCaptureState::Capturing))
	{
		...
		TRACE_BOOKMARK(TEXT("MediaCapture Push frame %d"), GFrameCounter);
...
bool UMediaCapture::ProcessCapture_RenderThread(const TSharedPtr<UE::MediaCaptureData::FCaptureFrame>& CapturingFrame, UE::MediaCaptureData::FCaptureFrameArgs Args)
{
	...
	RDG_EVENT_SCOPE_STAT(Args.GraphBuilder, MediaCapture_ProcessCapture, "MediaCapture_ProcessCapture", FrameNumber % 10);
	...
	if (CapturingFrame)
	{
		...
		if (Args.RDGResourceToCapture || Args.ResourceToCapture)
		{
			...
			if (CapturingFrame->bPrerollFrame)
			{
				// Make sure we're not doing prerolling when capturing a certain number of frames.
				check(!GetDesiredCaptureOptions().bAutostopOnCapture);
			}

			if (CapturingFrame->bPrerollFrame || GetDesiredCaptureOptions().bAutostopOnCapture)
			{
			...
			const bool bIsReadyToStartOutput = Args.MediaCapture->IsReadyToOutput();

			if (!CapturingFrame->bPrerollFrame && !bIsReadyToStartOutput && !bLoggedOutputNotReady)
			{
				bLoggedOutputNotReady = true;
				UE_LOG(LogMediaIOCore, Display, TEXT("Skipping frame since capture implementation is not ready to output yet."));
			}

			if (!CapturingFrame->bPrerollFrame && bIsReadyToStartOutput)
			{
			...
			{
				bHasCaptureSuceeded = true;
				CapturingFrame->bPrerollFrame = false;
				FrameManager->MarkAvailable(*CapturingFrame);
			}
		}
		else
		{
