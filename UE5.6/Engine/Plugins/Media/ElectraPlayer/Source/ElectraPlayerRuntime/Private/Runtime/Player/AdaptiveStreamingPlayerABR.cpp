...
namespace Electra
{
	...
	public:
		...
		void PathwayChanged(const FString& InNewPathway) override;
		...
		void ReportReceivedMainPlaylist(const FString& EffectiveURL) override {}
	...
	private:
		...
		void UpdateCurrentPlaybackPeriods();
	...
	void FAdaptiveStreamSelector::UpdateCurrentPlaybackPeriods()
	{
		...
		if (CurrentPlayPeriodVideo.IsValid())
		{
			...
			if (Asset.IsValid())
			{
				CurrentVideoAdaptationSetID = CurrentPlayPeriodVideo->GetSelectedAdaptationSetID(EStreamType::Video);
				StreamInformationVideo.Empty();
				// Get video stream information
				if (!CurrentVideoAdaptationSetID.IsEmpty())
				{
					for(int32 nA=0, maxA=Asset->GetNumberOfAdaptationSets(EStreamType::Video); nA<maxA; ++nA)
					{
						TSharedPtrTS<IPlaybackAssetAdaptationSet> Adapt = Asset->GetAdaptationSetByTypeAndIndex(EStreamType::Video, nA);
		...
		if (CurrentPlayPeriodAudio.IsValid())
		{
			...
			if (Asset.IsValid())
			{
				CurrentAudioAdaptationSetID = CurrentPlayPeriodAudio->GetSelectedAdaptationSetID(EStreamType::Audio);
				StreamInformationAudio.Empty();
				// Get audio stream information
				if (!CurrentAudioAdaptationSetID.IsEmpty())
				{
					for(int32 nA=0, maxA=Asset->GetNumberOfAdaptationSets(EStreamType::Audio); nA<maxA; ++nA)
					{
						TSharedPtrTS<IPlaybackAssetAdaptationSet> Adapt = Asset->GetAdaptationSetByTypeAndIndex(EStreamType::Audio, nA);
		...
		{
			ABRMethod->RepresentationsChanged(EStreamType::Video, CurrentPlayPeriodVideo);
			ABRMethod->RepresentationsChanged(EStreamType::Audio, CurrentPlayPeriodAudio);
		}
	}
	...
	//-----------------------------------------------------------------------------
	/**
	 * Called when content steering has changed to a different pathway (HLS)
	 * This _could_ mean that a whole different set of streams is suddenly available
	 * to select from, including their IDs, so we need to update our internal
	 * knowledge about what is available.
	 */
	void FAdaptiveStreamSelector::PathwayChanged(const FString& InNewPathway)
	{
		UpdateCurrentPlaybackPeriods();
	}
	...
	IAdaptiveStreamSelector::ESegmentAction FAdaptiveStreamSelector::SelectSuitableStreams(FTimeValue& OutDelay, TSharedPtrTS<const IStreamSegment> CurrentSegment)
	{
		...
		{
			// Initial request. If there are no streams at all we fail.
			if (StreamInformationVideo.IsEmpty() && StreamInformationAudio.IsEmpty())
			{
	...
	IAdaptiveStreamSelector::ESegmentAction FAdaptiveStreamSelector::SelectSuitableStreamByType(FTimeValue& OutDelay, TSharedPtrTS<const IStreamSegment> CurrentSegment, TSharedPtrTS<IManifest::IPlayPeriod> CurrentPlayPeriod, EStreamType StreamType)
	{
		...
		// Check if the state of a stream has become unusable.
		TSharedPtrTS<ITimelineMediaAsset> Asset = CurrentPlayPeriod->GetMediaAsset();
		if (Asset.IsValid())
		{
			FString AdaptationSetId = CurrentPlayPeriod->GetSelectedAdaptationSetID(StreamType);
			if (!AdaptationSetId.IsEmpty())
			{
				for(int32 nA=0, maxA=Asset->GetNumberOfAdaptationSets(StreamType); nA<maxA; ++nA)
				{
					TSharedPtrTS<IPlaybackAssetAdaptationSet> Adapt = Asset->GetAdaptationSetByTypeAndIndex(StreamType, nA);
					if (!AdaptationSetId.Equals(Adapt->GetUniqueIdentifier()))
					{
						continue;
					}
					for(int32 i=0; i<CandidateRepresentations.Num(); ++i)
					{
						TSharedPtrTS<IPlaybackAssetRepresentation> Repr = Adapt->GetRepresentationByUniqueIdentifier(CandidateRepresentations[i]->RepresentationUniqueID);
						if (Repr.IsValid() && !Repr->CanBePlayed())
						{
