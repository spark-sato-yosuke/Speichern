...
void FABROnDemandPlus::RepresentationsChanged(EStreamType InStreamType, TSharedPtrTS<IManifest::IPlayPeriod> InCurrentPlayPeriod)
{
	...
	if (WorkVars)
	{
		...
		{
			InCurrentPlayPeriod->GetAverageSegmentDuration(WorkVars->AverageSegmentDuration, Representations[0]->AdaptationSetUniqueID, Representations[0]->RepresentationUniqueID);
	...
	if (CurrentSegment.IsValid())
	{
		...
		if (CurrentStreamInfo.IsValid())
		{
			...
			if (!Stats.bWasSuccessful)
			{
				...
				{
					// Switching to another representation that needs to be made ready resets the consecutive errors so we get
					// a chance to switch to a different representation if several are being blocked by the CDN to force
					// multiple downswitches.
