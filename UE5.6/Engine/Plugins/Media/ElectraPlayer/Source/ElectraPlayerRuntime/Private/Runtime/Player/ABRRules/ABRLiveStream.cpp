			...
			if (WorkVars->bIsLowLatencyEnabled)
			{
				...
				if (SegmentDownloadStats.TimeToDownload > CheckTime)
				{
					if (SegmentDownloadStats.TimeToFirstByte <= 0.0)
					{
						...
						if (QualityIndex > 0)
						{
							Info->LogMessage(IInfoLog::ELevel::Verbose, FString::Printf(TEXT("Not connected to server after %.3fs for \"%s\""), SegmentDownloadStats.TimeToDownload, *GetFilenameFromURL(SegmentDownloadStats.URL.URL)));
