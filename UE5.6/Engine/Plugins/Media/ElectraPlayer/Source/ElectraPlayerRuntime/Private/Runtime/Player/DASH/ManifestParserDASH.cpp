...
DEF_ATTR(defaultServiceLocation);
DEF_ATTR(queryBeforeStart);
DEF_ATTR(clientRequirement);
DEF_ATTR(proxyServerURL);

// DVB DASH attributes have a colon in them.
const TCHAR* const Attr_dvb_priority = TEXT("dvb:priority");
const TCHAR* const Attr_dvb_weight = TEXT("dvb:weight");
...
bool IDashMPDElement::ProcessAttribute(FManifestParserDASH* Builder, const TCHAR* AttributeName, const TCHAR* AttributeValue)
{
	...
	if (GetElementType() == EType::Descriptor || GetElementType() == EType::ContentSteering || GetElementType() == EType::Other)
	{
		bEmitWarning = false;
	}
...
bool FDashMPD_MPDType::ProcessElement(FManifestParserDASH* Builder, const TCHAR* ElementName, const TCHAR* ElementData, int32 XmlFileLineNumber)
{
	...
	else if (StringHelpers::StringEquals(ElementName, ELEMENT(ContentSteering)))
	{
		ContentSteering = MakeSharedTS<FDashMPD_ContentSteeringType>(ElementName, ElementData);
		return Builder->PushNewElement(ContentSteering, XmlFileLineNumber);
	}
	else
	{
...
bool FDashMPD_BaseURLType::ProcessAttribute(FManifestParserDASH* Builder, const TCHAR* AttributeName, const TCHAR* AttributeValue)
{
	...
	// DVB attributes?
	else if (AttrEquals(dvb_priority))
	{
		return ParseIntAttribute(DVBPriority, Builder, AttributeName, AttributeValue, GetXMLLineNumber());
	}
	else if (AttrEquals(dvb_weight))
	{
		return ParseIntAttribute(DVBWeight, Builder, AttributeName, AttributeValue, GetXMLLineNumber());
	}
	// Any other attribute.
	else
	{
...
bool FDashMPD_ContentSteeringType::ProcessAttribute(FManifestParserDASH* Builder, const TCHAR* AttributeName, const TCHAR* AttributeValue)
{
	if (AttrEquals(defaultServiceLocation))
	{
		return ParseStringAttribute(DefaultServiceLocation, Builder, AttributeName, AttributeValue, GetXMLLineNumber());
	}
	else if (AttrEquals(proxyServerURL))
	{
		return ParseStringAttribute(ProxyServerURL, Builder, AttributeName, AttributeValue, GetXMLLineNumber());
	}
	else if (AttrEquals(queryBeforeStart))
	{
		return ParseBooleanAttribute(bQueryBeforeStart, Builder, AttributeName, AttributeValue, GetXMLLineNumber());
	}
	else if (AttrEquals(clientRequirement))
	{
		return ParseBooleanAttribute(bClientRequirement, Builder, AttributeName, AttributeValue, GetXMLLineNumber());
	}
	else
	{
		return Super::ProcessAttribute(Builder, AttributeName, AttributeValue);
