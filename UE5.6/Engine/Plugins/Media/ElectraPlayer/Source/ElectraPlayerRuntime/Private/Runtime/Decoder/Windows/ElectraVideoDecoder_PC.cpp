...
FElectraPlayerVideoDecoderOutputPC::~FElectraPlayerVideoDecoderOutputPC()
{
	...
	if (GIsRunning && (Texture.IsValid() || SharedTexture.IsValid() || D3D11Device.IsValid()))
	{
		...
		rco->Texture = MoveTemp(Texture);
		rco->SharedTexture = MoveTemp(SharedTexture);
																...
																TSharedPtr<FElectraMediaDecoderOutputBufferPool_DX12>& InOutD3D12ResourcePool, uint32 MaxWidth, uint32 MaxHeight, uint32 MaxOutputBuffers)
{
	...
	const FIntPoint TextureDim = bIsTexture ? FIntPoint{ static_cast<int>(MaxWidth), static_cast<int>(MaxHeight) } : SampleDim;
	...
	*/
	if (!TextureDX12.IsValid() || TextureDim != TextureDX12Dim)
	{
		...
		if (!D3D12ResourcePool->AllocateOutputDataAsTexture(OutputData, TextureDim.X, TextureDim.Y, DXGIFmt))
		{
