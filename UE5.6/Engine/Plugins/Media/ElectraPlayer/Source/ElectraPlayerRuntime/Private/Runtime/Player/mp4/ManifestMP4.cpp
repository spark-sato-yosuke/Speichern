 ...
 */
void FManifestMP4Internal::FPlayPeriodMP4::GetAverageSegmentDuration(FTimeValue& OutAverageSegmentDuration, const FString& AdaptationSetID, const FString& RepresentationID)
{
	TSharedPtrTS<FTimelineAssetMP4> ma = MediaAsset.Pin();
	if (ma.IsValid())
	{
...
{
	// Limit the segment size by content duration and/or size.
	...
	const int64 MaxSegmentByteCount = 1024 * 1024 * 64;
	if (InOutSegment.IsValid())
	{
		...
		while(AllTrackIterator.IsValid())
		{
			...
			if (CurrentTrackIt)
			{
				...
				bool bSizeLimitReached = (MaxSegmentByteCount > 0) && (CurrentTrackOffset - StartOffset >= MaxSegmentByteCount);
				if (CurrentTrackOffset >= EndOffset || bDurationLimitReached || bSizeLimitReached)
				{
					// Limit reached.
			...
			for(int32 nTrk=0; nTrk<nTracks; ++nTrk)
			{
				...
				if (TrkIt)
				{
					...
					for(ItErr = TrkIt->StartAtFirst(false); ItErr == UEMEDIA_ERROR_OK; ItErr = TrkIt->Next())
					{
						...
						if (TrkIt->GetDTS() >= TrackLocalTime && TrkIt->GetPTS() >= TrackLocalTime)
						{
							break;
						}
					}
...
void FManifestMP4Internal::FTimelineAssetMP4::GetAverageSegmentDuration(FTimeValue& OutAverageSegmentDuration, const FString& AdaptationSetID, const FString& RepresentationID)
{
...
FErrorDetail FManifestMP4Internal::FAdaptationSetMP4::CreateFrom(const IParserISO14496_12::ITrack* InTrack, const FString& URL)
{
	...
	if (err.IsOK())
	{
		...
		LanguageTag		 = InTrack->GetLanguageTag();
	}
	return err;
}
