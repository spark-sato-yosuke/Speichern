...
void FPlaylistReaderDASH::CheckForMPDUpdate()
{
	...
	if (bRequestNow)
	{
		...
		if (MPDRoot.IsValid())
		{
			...
			RootDocument.CDN = MPDRoot->IsDVBDash() ? RootDocument.URL : RootDocURL.GetHost();
			...
			for(int32 i=0; i<Locations.Num(); ++i)
			{
				...
				if (sc.MediaURL.CDN.IsEmpty())
				{
					if (!MPDRoot->IsDVBDash())
					{
						FURL_RFC3986 tmp;tmp.Parse(sc.MediaURL.URL);
						sc.MediaURL.CDN = tmp.GetHost();
				}
					else
					{
