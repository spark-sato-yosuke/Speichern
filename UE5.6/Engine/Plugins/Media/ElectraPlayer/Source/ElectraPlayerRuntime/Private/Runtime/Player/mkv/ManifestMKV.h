...
/**
 * This class represents the internal "manifest" or "playlist" of an mkv file.
 * All metadata of the tracks it is composed of are maintained by this class.
 */
class FManifestMKVInternal : public IManifest, public TSharedFromThis<FManifestMKVInternal, ESPMode::ThreadSafe>
{
public:
	FManifestMKVInternal(IPlayerSessionServices* InPlayerSessionServices);
	virtual ~FManifestMKVInternal();

	FErrorDetail Build(TSharedPtrTS<IParserMKV> MP4Parser, const FString& URL, const HTTP::FConnectionInfo& InConnectionInfo);
	...
	class FRepresentationMKV : public IPlaybackAssetRepresentation
	{
	public:
		virtual ~FRepresentationMKV()
		{ }

		FErrorDetail CreateFrom(const IParserMKV::ITrack* InTrack, const FString& URL);
	...
	class FAdaptationSetMKV : public IPlaybackAssetAdaptationSet
	{
	public:
		virtual ~FAdaptationSetMKV()
		{ }
	...
	private:
		TSharedPtrTS<FRepresentationMKV> Representation;
		BCP47::FLanguageTag LanguageTag;
		FString CodecRFC6381;
		FString UniqueIdentifier;
	};

	class FTimelineAssetMKV : public ITimelineMediaAsset, public TSharedFromThis<FTimelineAssetMKV, ESPMode::ThreadSafe>
	{
	public:
		FTimelineAssetMKV()
		...
		virtual ~FTimelineAssetMKV()
		{ }

		FErrorDetail Build(IPlayerSessionServices* InPlayerSessionServices, TSharedPtrTS<IParserMKV> InMKVParser, const FString& InURL);
		...
		{ return FString("mkv-asset.0"); }
		virtual FString GetUniqueIdentifier() const override
		{ return FString("mkv-media.0"); }
		...
		virtual void GetMetaData(TArray<FTrackMetadata>& OutMetadata, EStreamType StreamType) const override
		{
			...
			{
				TSharedPtrTS<FAdaptationSetMKV> AdaptSet = StaticCastSharedPtr<FAdaptationSetMKV>(GetAdaptationSetByTypeAndIndex(StreamType, i));
				if (AdaptSet.IsValid())
				{
					FTrackMetadata tm;
					...
					{
						TSharedPtrTS<FRepresentationMKV> Repr = StaticCastSharedPtr<FRepresentationMKV>(AdaptSet->GetRepresentationByIndex(j));
						if (Repr.IsValid())
						{
							FStreamMetadata sd;
		...
		FResult GetLoopingSegment(TSharedPtrTS<IStreamSegment>& OutSegment, const FPlayerSequenceState& InSequenceState, FString SelectedStreamID, const FPlayStartPosition& StartPosition, ESearchType SearchType);
		...
		TSharedPtrTS<IParserMKV> GetMKVParser();
	...
	private:
		...
		FString MediaURL;
		FTimeValue MovieDuration;
		TSharedPtrTS<IParserMKV> MKVParser;
		TArray<TSharedPtrTS<FAdaptationSetMKV>> VideoAdaptationSets;
		TArray<TSharedPtrTS<FAdaptationSetMKV>> AudioAdaptationSets;
		TArray<TSharedPtrTS<FAdaptationSetMKV>> SubtitleAdaptationSets;
		TArray<uint64> UsableTrackIDs;
	};


	class FPlayPeriodMKV : public IManifest::IPlayPeriod
	{
	public:
		FPlayPeriodMKV(TSharedPtrTS<FTimelineAssetMKV> InMediaAsset);
		virtual ~FPlayPeriodMKV();
	...
	private:
		...
		TWeakPtrTS<FTimelineAssetMKV> MediaAsset;
	...
	IPlayerSessionServices* PlayerSessionServices;
	TSharedPtrTS<FTimelineAssetMKV> MediaAsset;
