...
namespace ElectraDecodersUtil
{
	namespace MPEG
	{
		namespace H265
		{
			...
			{
				return GetFormatInfo(profile_tier_level.general_profile_idc, profile_tier_level.general_level_idc, GetConstraintFlags());
			}

			FString FSequenceParameterSet::GetFormatInfo(uint32 InProfileIDC, uint32 InLevelIDC, uint64 InConstraintFlags)
			{
				FString fi;
				if (InProfileIDC == 1)
				{
				...
				if (InLevelIDC)
				{
					fi.Append(FString::Printf(TEXT(", level %d.%d"), InLevelIDC/30, InLevelIDC%30));
				}
				else
				{
			...
			bool ParseVideoParameterSet(FVideoParameterSet& OutVideoParameterSet, const uint8* InBitstream, uint64 InBitstreamLenInBytes)
			{
				TMap<uint32, FVideoParameterSet> vpss;
				if (!ParseVideoParameterSet(vpss, InBitstream, InBitstreamLenInBytes))
				{
					return false;
				}
				OutVideoParameterSet = vpss.CreateConstIterator().Value();
				return true;
			}
			...
			bool FHEVCDecoderConfigurationRecord::Parse(const TConstArrayView<uint8>& InDCR)
			{
				CSD.Empty();
				RawData.Empty();
				...
				if (InDCR.Num() >= 23)
				{
					...
					FElectraBitstreamReader BitReader(InDCR.GetData(), InDCR.Num());
					...
					for(int32 i=0; i<Arrays.Num(); ++i)
					{
						...
						for(int32 j=0; j<a.NALUs.Num(); ++j)
						{
							...
							CSD.Add(0);
							CSD.Add(1);
							CSD.Append(a.NALUs[j]);
						}

						// VPS nut?
						if (a.NALUnitType == 32)
						{
							TArray<uint8>& vps(VideoParameterSets.Emplace_GetRef());
							for(int32 k=0; k<a.NALUs.Num(); ++k)
							{
								vps.Append(a.NALUs[k]);
								if (!ParseVideoParameterSet(ParsedVideoParameterSets.Emplace_GetRef(), a.NALUs[k].GetData(), a.NALUs[k].Num()))
								{
									ParsedVideoParameterSets.RemoveAt(ParsedVideoParameterSets.Num()-1);
								}
							}
						}
						// SPS nut?
						else if (a.NALUnitType == 33)
						{
							TArray<uint8>& sps(SequenceParameterSets.Emplace_GetRef());
							for(int32 k=0; k<a.NALUs.Num(); ++k)
							{
								sps.Append(a.NALUs[k]);
								if (!ParseSequenceParameterSet(ParsedSequenceParameterSets.Emplace_GetRef(), a.NALUs[k].GetData(), a.NALUs[k].Num()))
								{
									ParsedSequenceParameterSets.RemoveAt(ParsedSequenceParameterSets.Num()-1);
								}
							}
						}
						// Prefix nut?
						else if (a.NALUnitType == 39)
						{
							TArray<uint8>& Prefix(PrefixNUTs.Emplace_GetRef());
							for(int32 k=0; k<a.NALUs.Num(); ++k)
							{
								Prefix.Append(a.NALUs[k]);
							}
						}
						// Suffix nut?
						else if (a.NALUnitType == 40)
						{
							TArray<uint8>& Suffix(SuffixNUTs.Emplace_GetRef());
							for(int32 k=0; k<a.NALUs.Num(); ++k)
							{
								Suffix.Append(a.NALUs[k]);
							}
						}
					}
					return true;
			...
			FString FHEVCDecoderConfigurationRecord::GetCodecSpecifierRFC6381(const TCHAR* InPrefix) const
			{
				/*
				// If we have a parsed SPS we use that.
				if (ParsedSequenceParameterSets.Num())
				{
					return ParsedSequenceParameterSets[0].GetRFC6381(InPrefix);
				}
				*/
				// Construct one from the general information as per ISO/IEC 14496-15:2022 Annex E.3
				FString cfs;
				uint64 ConstraintFlags = GeneralConstraintIndicatorFlags;
				bool bNonZero = false;
				for(int32 i=5; i>=0; --i, ConstraintFlags >>= 8)
				{
					if ((ConstraintFlags & 255) == 0 && !bNonZero)
				...
				const uint32 cf = BitReverse32(GeneralProfileCompatibilityFlags);
				FString rfc;
				if (GeneralProfileSpace == 0)
				{
					rfc = FString::Printf(TEXT("%s.%d.%X.%c%d%s"), InPrefix, GeneralProfileIDC, cf, GeneralTierFlag ? TCHAR('H') : TCHAR('L'), GeneralLevelIDC, *cfs);
				}
				else
				{
					rfc = FString::Printf(TEXT("%s.%c%d.%X.%c%d%s"), InPrefix, TCHAR(TCHAR('A')+GeneralProfileSpace-1), GeneralProfileIDC, cf, GeneralTierFlag ? TCHAR('H') : TCHAR('L'), GeneralLevelIDC, *cfs);
				}
				return rfc;
			}

			FString FHEVCDecoderConfigurationRecord::GetFormatInfo() const
			{
				FString fi(TEXT("HEVC, "));
				fi.Append(FSequenceParameterSet::GetFormatInfo(GeneralProfileIDC, GeneralLevelIDC, GeneralConstraintIndicatorFlags));
				return fi;
			}

			bool FHEVCDecoderConfigurationRecord::CreateFromCodecSpecificData(const TConstArrayView<uint8>& InCSD)
			{
				CSD.Empty();
				RawData.Empty();
				if (InCSD.IsEmpty())
				...
				for(int32 i=0; i<Nalus.Num(); ++i)
				{
					...
					{
						aVPS.NALUnitType = 32;
						aVPS.NALUs.Emplace_GetRef() = MakeArrayView<const uint8>(InCSD.GetData() + Nalus[i].Offset + Nalus[i].UnitLength, Nalus[i].Size);
					}
					else if (Nalus[i].Type == 33)
					{
						...
						if (ParseSequenceParameterSet(sps, InCSD.GetData() + Nalus[i].Offset + Nalus[i].UnitLength, Nalus[i].Size))
						{
							...
							ParsedSequenceParameterSets.Emplace(MoveTemp(sps));
						}
						else
						{
							return false;
