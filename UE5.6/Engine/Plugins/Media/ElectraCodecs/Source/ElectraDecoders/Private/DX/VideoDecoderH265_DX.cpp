...
TSharedPtr<ElectraDecodersUtil::MPEG::H265::FSequenceParameterSet, ESPMode::ThreadSafe> FElectraVideoDecoderH265_DX::GetSPSFromOptions(const TMap<FString, FVariant>& InOptions)
{
...
bool FElectraVideoDecoderH265_DX::InternalDecoderCreate(const TMap<FString, FVariant>& InAdditionalOptions)
{
	...
	if (dcr.Parse(SidebandData) && !dcr.GetParsedSequenceParameterSets().IsEmpty())
	{
		VERIFY_HR(MFSetAttributeSize(MediaType, MF_MT_FRAME_SIZE, dcr.GetParsedSequenceParameterSets()[0].GetWidth(), dcr.GetParsedSequenceParameterSets()[0].GetHeight()), "Failed to set video decoder input media type resolution", ERRCODE_INTERNAL_FAILED_TO_CREATE_DECODER);
	}
	else
	{
