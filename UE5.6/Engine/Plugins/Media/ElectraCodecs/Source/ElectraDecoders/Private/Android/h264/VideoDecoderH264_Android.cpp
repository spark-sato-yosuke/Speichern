...
private:
	...
	struct FDecoderInput
	{
		...
		TSharedPtr<ElectraDecodersUtil::MPEG::H264::FSequenceParameterSet, ESPMode::ThreadSafe> SPS;
	};
	...
	TSharedPtr<ElectraDecodersUtil::MPEG::H264::FAVCDecoderConfigurationRecord, ESPMode::ThreadSafe> GetDCRFromOptions(const TMap<FString, FVariant>& InOptions);
	...
	TSharedPtr<ElectraDecodersUtil::MPEG::H264::FAVCDecoderConfigurationRecord, ESPMode::ThreadSafe> ConfigRecord;
...
TSharedPtr<ElectraDecodersUtil::MPEG::H264::FAVCDecoderConfigurationRecord, ESPMode::ThreadSafe> FElectraVideoDecoderH264_Android::GetDCRFromOptions(const TMap<FString, FVariant>& InOptions)
{
	TArray<uint8> SidebandData = ElectraDecodersUtil::GetVariantValueUInt8Array(InOptions, TEXT("dcr"));
	if (SidebandData.Num())
	{
		TSharedPtr<ElectraDecodersUtil::MPEG::H264::FAVCDecoderConfigurationRecord, ESPMode::ThreadSafe> NewConfigRecord;
		NewConfigRecord = MakeShared<ElectraDecodersUtil::MPEG::H264::FAVCDecoderConfigurationRecord, ESPMode::ThreadSafe>();
...
TSharedPtr<ElectraDecodersUtil::MPEG::H264::FSequenceParameterSet, ESPMode::ThreadSafe> FElectraVideoDecoderH264_Android::GetSPSFromOptions(const TMap<FString, FVariant>& InOptions)
{
	...
	if (SidebandData.Num())
	{
		...
		ElectraDecodersUtil::MPEG::H264::ParseBitstreamForNALUs(NALUs, SidebandData.GetData(), SidebandData.Num());
		for(int32 i=0; i<NALUs.Num(); ++i)
		{
			if (NALUs[i].Type == 7)
			{
				...
				if (ElectraDecodersUtil::MPEG::H264::ParseSequenceParameterSet(*NewSPS, ElectraDecodersUtil::AdvancePointer(SidebandData.GetData(), NALUs[i].Offset + NALUs[i].UnitLength), NALUs[i].Size))
				{
					return NewSPS;
				}
				else
...
bool FElectraVideoDecoderH264_Android::InternalFinishDecoderCreate()
{
	if (CurrentSurfaceRequest.IsValid())
	{
		...
		if (ConfigRecord.IsValid())
		{
			if (ensure(!ConfigRecord->GetSequenceParameterSets().IsEmpty()))
			{
				ElectraDecodersUtil::MPEG::H264::FSequenceParameterSet SPS;
				ElectraDecodersUtil::MPEG::H264::ParseSequenceParameterSet(SPS, ConfigRecord->GetSequenceParameterSets()[0]);
