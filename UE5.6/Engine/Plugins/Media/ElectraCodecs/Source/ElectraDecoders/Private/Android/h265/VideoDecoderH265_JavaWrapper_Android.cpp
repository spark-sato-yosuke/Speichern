...
class FElectraH265VideoDecoderAndroidJava : public IElectraH265VideoDecoderAndroidJava, public FJavaClassObject
{
public:

	FElectraH265VideoDecoderAndroidJava();
	virtual ~FElectraH265VideoDecoderAndroidJava();
...
private:
	...
	{
		return FName("com/epicgames/unreal/ElectraDecoderVideoH265");
	}
 ...
 */
TSharedPtr<IElectraH265VideoDecoderAndroidJava, ESPMode::ThreadSafe> IElectraH265VideoDecoderAndroidJava::Create()
{
	return MakeShared<FElectraH265VideoDecoderAndroidJava>();
}
 ...
 * CTOR
 */
FElectraH265VideoDecoderAndroidJava::FElectraH265VideoDecoderAndroidJava()
	...
	, ConfigureDecoderFN(GetClassMethod("ConfigureDecoder", "(Lcom/epicgames/unreal/ElectraDecoderVideoH265$FCreateParameters;)I"))
	...
	, GetDecoderInformationFN(GetClassMethod("GetDecoderInformation", "()Lcom/epicgames/unreal/ElectraDecoderVideoH265$FDecoderInformation;"))
	, GetOutputFormatInfoFN(GetClassMethod("GetOutputFormatInfo", "(I)Lcom/epicgames/unreal/ElectraDecoderVideoH265$FOutputFormatInfo;"))
	, DequeueOutputBufferFN(GetClassMethod("DequeueOutputBuffer", "(I)Lcom/epicgames/unreal/ElectraDecoderVideoH265$FOutputBufferInfo;"))
	...
	, ReleaseOutputBufferFN(GetClassMethod("ReleaseOutputBuffer", "(IZJ)I"))
{
	...
	jclass localCreateParametersClass = AndroidJavaEnv::FindJavaClass("com/epicgames/unreal/ElectraDecoderVideoH265$FCreateParameters");
	...
	jclass localDecoderInformationClass = AndroidJavaEnv::FindJavaClass("com/epicgames/unreal/ElectraDecoderVideoH265$FDecoderInformation");
	...
	jclass localOutputFormatInfoClass = AndroidJavaEnv::FindJavaClass("com/epicgames/unreal/ElectraDecoderVideoH265$FOutputFormatInfo");
	...
	jclass localOutputBufferInfoClass = AndroidJavaEnv::FindJavaClass("com/epicgames/unreal/ElectraDecoderVideoH265$FOutputBufferInfo");
 ...
 */
void FElectraH265VideoDecoderAndroidJava::SetupDecoderInformation()
{
 ...
 */
int32 FElectraH265VideoDecoderAndroidJava::CreateDecoder()
{
	FScopeLock lock(&Lock);
	int32 result = -1;
 ...
 * @return 0 if successful, 1 on error.
 */
int32 FElectraH265VideoDecoderAndroidJava::InitializeDecoder(const FCreateParameters& InCreateParams)
{
	FScopeLock lock(&Lock);
	if (bHaveDecoder)
	{
		...
		if (InCreateParams.CodecData && !InCreateParams.CodecData->GetParsedSequenceParameterSets().IsEmpty())
		{
			JEnv->SetIntField(CreateParams, FCreateParameters_Width, InCreateParams.CodecData->GetParsedSequenceParameterSets()[0].GetWidth());
			JEnv->SetIntField(CreateParams, FCreateParameters_Height, InCreateParams.CodecData->GetParsedSequenceParameterSets()[0].GetHeight());
 ...
 * @return 0 if successful, 1 on error.
 */
int32 FElectraH265VideoDecoderAndroidJava::SetOutputSurface(jobject InNewOutputSurface)
{
	FScopeLock lock(&Lock);
	if (bHaveDecoder)
	{
 ...
 * @return 0 if successful, 1 on error.
 */
int32 FElectraH265VideoDecoderAndroidJava::ReleaseDecoder()
{
	FScopeLock lock(&Lock);
	if (bHaveDecoder)
	{
 ...
 * @return 0 if successful, 1 on error.
 */
int32 FElectraH265VideoDecoderAndroidJava::release()
{
	Stop();
 ...
 */
const IElectraH265VideoDecoderAndroidJava::FDecoderInformation* FElectraH265VideoDecoderAndroidJava::GetDecoderInformation()
{
 ...
 * @return 0 if successful, 1 on error.
 */
int32 FElectraH265VideoDecoderAndroidJava::Start()
{
	FScopeLock lock(&Lock);
 ...
 * @return 0 if successful, 1 on error.
 */
int32 FElectraH265VideoDecoderAndroidJava::Stop()
{
	FScopeLock lock(&Lock);
	if (bHaveDecoder && bIsStarted)
	{
 ...
 * @return 0 if successful, 1 on error.
 */
int32 FElectraH265VideoDecoderAndroidJava::Flush()
{
	FScopeLock lock(&Lock);
 ...
 * @return 0 if successful, 1 on error.
 */
int32 FElectraH265VideoDecoderAndroidJava::Reset()
{
	FScopeLock lock(&Lock);
 ...
 */
int32 FElectraH265VideoDecoderAndroidJava::DequeueInputBuffer(int32 InTimeoutUsec)
{
	FScopeLock lock(&Lock);
	if (bHaveDecoder)
	{
 ...
 * @return 0 if successful, 1 on error.
 */
int32 FElectraH265VideoDecoderAndroidJava::QueueInputBuffer(int32 InBufferIndex, const void* InPrefixData, int32 InPrefixSize, const void* InAccessUnitData, int32 InAccessUnitSize, int64 InTimestampUSec)
{
	FScopeLock lock(&Lock);
	if (bHaveDecoder)
	{
 ...
 * @return 0 if successful, 1 on error.
 */
int32 FElectraH265VideoDecoderAndroidJava::QueueCSDInputBuffer(int32 InBufferIndex, const void* InCSDData, int32 InCSDSize, int64 InTimestampUSec)
{
	FScopeLock lock(&Lock);
	if (bHaveDecoder)
	{
 ...
 * @return 0 if successful, 1 on error.
 */
int32 FElectraH265VideoDecoderAndroidJava::QueueEOSInputBuffer(int32 InBufferIndex, int64 InTimestampUSec)
{
	FScopeLock lock(&Lock);
	if (bHaveDecoder)
	{
 ...
 * @return 0 if successful, 1 on error.
 */
int32 FElectraH265VideoDecoderAndroidJava::GetOutputFormatInfo(FOutputFormatInfo& OutFormatInfo, int32 InOutputBufferIndex)
{
	FScopeLock lock(&Lock);
	if (bHaveDecoder)
	{
 ...
 */
int32 FElectraH265VideoDecoderAndroidJava::DequeueOutputBuffer(FOutputBufferInfo& OutBufferInfo, int32 InTimeoutUsec)
{
	FScopeLock lock(&Lock);
	if (bHaveDecoder)
	{
 ...
 */
int32 FElectraH265VideoDecoderAndroidJava::GetOutputBuffer(void*& OutBufferDataPtr, int32 OutBufferDataSize, const FOutputBufferInfo& InOutBufferInfo)
{
	FScopeLock lock(&Lock);
	if (bHaveDecoder)
	{
 ...
 * @return 0 on success, 1 on failure.
 */
int32 FElectraH265VideoDecoderAndroidJava::ReleaseOutputBuffer(int32 BufferIndex, int32 ValidCount, bool bRender, int64 releaseAt)
{
	FScopeLock lock(&Lock);
	if (bHaveDecoder)
	{
