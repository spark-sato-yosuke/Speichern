	...
	namespace MPEG
	{
		namespace H264
		{
			...
			TUniquePtr<FRBSP> MakeRBSP(const uint8* InData, uint64 InDataSize)
			{
				...
				uint64 AdjustedSize = EBSPtoRBSP(Ptr->RemovedAtSrc, Ptr->Data.GetData(), InData, InDataSize);
				Ptr->Data.SetNum((int32)AdjustedSize);
				return Ptr;
			}
			...
			void FSequenceParameterSet::GetDisplaySize(uint32& OutWidth, uint32& OutHeight) const
			{
				int32 w,h;
				GetDisplaySize(w, h);
				OutWidth = (uint32) w;
			...
			{
				if (vui_parameters_present_flag && timing_info_present_flag && fixed_frame_rate_flag)
				{
					return FFractionalValue(time_scale, num_units_in_tick * 2 /* rate per field, we assume progressive frames, so 2 fields */);
				}
				return FFractionalValue();
			}

			FString FSequenceParameterSet::GetFormatInfo() const
			{
				return GetFormatInfo(profile_idc, level_idc, constraint_set0_flag | (constraint_set1_flag <<1) | (constraint_set2_flag << 2) | (constraint_set3_flag << 3) | (constraint_set4_flag << 4) | (constraint_set5_flag << 5));
			}
			FString FSequenceParameterSet::GetFormatInfo(uint32 InProfileIDC, uint32 InLevelIDC, uint32 InConstraintFlags)
			{
				FString fi;
				if (InProfileIDC == 44)
				{
					fi = TEXT("CAVLC 4:4:4 Intra");
				}
				else if (InProfileIDC == 66 && (InConstraintFlags & 2) == 2)
				{
					fi = TEXT("Constrained Baseline");
				}
				else if (InProfileIDC == 66)
				{
					fi = TEXT("Baseline");
				}
				else if (InProfileIDC == 77)
				{
					fi = TEXT("Main");
				}
				else if (InProfileIDC == 83 && (InConstraintFlags & 32) == 32)
				{
					fi = TEXT("Scalable Constrained Baseline");
				}
				else if (InProfileIDC == 83)
				{
					fi = TEXT("Scalable Baseline");
				}
				else if (InProfileIDC == 86 && (InConstraintFlags & 32) == 32)
				{
					fi = TEXT("Scalable Constrained High");
				}
				else if (InProfileIDC == 86 && (InConstraintFlags & 8) == 8)
				{
					fi = TEXT("Scalable High Intra");
				}
				else if (InProfileIDC == 86)
				{
					fi = TEXT("Scalable High");
				}
				else if (InProfileIDC == 88)
				{
					fi = TEXT("Extended");
				}
				else if (InProfileIDC == 100 && (InConstraintFlags & 48) == 48)
				{
					fi = TEXT("Constrained High");
				}
				else if (InProfileIDC == 100 && (InConstraintFlags & 16) == 16)
				{
					fi = TEXT("Progressive High");
				}
				else if (InProfileIDC == 100)
				{
					fi = TEXT("High");
				}
				else if (InProfileIDC == 110 && (InConstraintFlags & 8) == 8)
				{
					fi = TEXT("High 10 Intra");
				}
				else if (InProfileIDC == 110)
				{
					fi = TEXT("High 10");
				}
				else if (InProfileIDC == 118)
				{
					fi = TEXT("Multiview High");
				}
				else if (InProfileIDC == 122 && (InConstraintFlags & 8) == 8)
				{
					fi = TEXT("High 4:2:2 Intra");
				}
				else if (InProfileIDC == 122)
				{
					fi = TEXT("High 4:2:2");
				}
				else if (InProfileIDC == 128)
				{
					fi = TEXT("Stereo High");
				}
				else if (InProfileIDC == 134)
				{
					fi = TEXT("MFC High");
				}
				else if (InProfileIDC == 135)
				{
					fi = TEXT("MFC Depth High");
				}
				else if (InProfileIDC == 138)
				{
					fi = TEXT("Multiview Depth High");
				}
				else if (InProfileIDC == 139)
				{
					fi = TEXT("Enhanced Multiview Depth High");
				}
				else if (InProfileIDC == 244 && (InConstraintFlags & 8) == 8)
				{
					fi = TEXT("High 4:4:4 Intra");
				}
				else if (InProfileIDC == 244)
				{
					fi = TEXT("High 4:4:4 Predictive");
				}
				else
				{
					fi = TEXT("Unkown");
				...
				if (InLevelIDC)
				{
					fi.Append(FString::Printf(TEXT(", level %d.%d"), InLevelIDC/10, InLevelIDC%10));
				}
				else
				{
			...
			bool ParseSequenceParameterSet(TMap<uint32, FSequenceParameterSet>& InOutSequenceParameterSets, const uint8* InBitstream, uint64 InBitstreamLenInBytes)
			{
				...
				FBitstreamReader br(RBSP->Data.GetData(), RBSP->Data.Num());
				...
				RBSP->TrimToBitstream(br);
				sps.ProcessedBitstream = MoveTemp(RBSP->Data);
				return true;
			...
			bool ParseSliceHeader(TUniquePtr<FRBSP>& OutRBSP, FBitstreamReader& OutRBSPReader, FSliceHeader& OutSlice, const TMap<uint32, FSequenceParameterSet>& InSequenceParameterSets, const TMap<uint32, FPictureParameterSet>& InPictureParameterSets, const uint8* InBitstream, uint64 InBitstreamLenInBytes)
			{
				...
				OutRBSPReader.SetData(OutRBSP->Data.GetData(), OutRBSP->Data.Num());
			...
			int32 ELECTRADECODERS_API CalcNumBytesInNAL(int32 InProfileLevel, int32 InFrameWidth, int32 InFrameHeight, double InFrameRate)
			{
				int32 MaxMBPS = 0;
				int32 MinCR = InProfileLevel == 31 || InProfileLevel == 32 || InProfileLevel == 40 ? 4 : 2;
				double fR = InProfileLevel >= 60 ? 1/300.0 : 1/172.0;
				if (InProfileLevel <= 10) MaxMBPS = 1485;
				else if (InProfileLevel == 11) MaxMBPS = 3000;
				else if (InProfileLevel == 12) MaxMBPS = 6000;
				else if (InProfileLevel == 13) MaxMBPS = 11880;
				else if (InProfileLevel == 20) MaxMBPS = 11880;
				else if (InProfileLevel == 21) MaxMBPS = 19800;
				else if (InProfileLevel == 22) MaxMBPS = 20250;
				else if (InProfileLevel == 30) MaxMBPS = 40500;
				else if (InProfileLevel == 31) MaxMBPS = 108000;
				else if (InProfileLevel == 32) MaxMBPS = 216000;
				else if (InProfileLevel == 40) MaxMBPS = 245760;
				else if (InProfileLevel == 41) MaxMBPS = 245760;
				else if (InProfileLevel == 42) MaxMBPS = 522240;
				else if (InProfileLevel == 50) MaxMBPS = 589824;
				else if (InProfileLevel == 51) MaxMBPS = 983040;
				else if (InProfileLevel == 52) MaxMBPS = 2073600;
				else if (InProfileLevel == 60) MaxMBPS = 4177920;
				else if (InProfileLevel == 61) MaxMBPS = 8355840;
				else if (InProfileLevel == 62) MaxMBPS = 16711680;
				if (MaxMBPS == 0 || InFrameWidth <= 0 || InFrameHeight <= 0 || InFrameRate <= 1.0)
				{
					return 1536 << 10;
				}
				int32 PicSizeInMbs = Align(InFrameWidth, 16) * Align(InFrameHeight, 16) / 256;
				int32 NumBytesInNAL = (int32)(384.0 * (FMath::Max((double)PicSizeInMbs, fR * (double)MaxMBPS) + ((double)MaxMBPS * 1.0 / InFrameRate)) / (double)MinCR);
				return NumBytesInNAL;
			}


			bool FAVCDecoderConfigurationRecord::Parse(const TConstArrayView<uint8>& InDCR)
			{
				CSD.Empty();
				RawData.Empty();
				...
				RawData = InDCR;
				FElectraByteReader ByteReader(InDCR.GetData(), InDCR.Num());

				if (!ByteReader.ReadByte(ConfigurationVersion) || ConfigurationVersion != 1)
				{
					return false;
				}
				...
				{
					// At least according to the standard these values must appear.
					// However, there are files that are of profile 100 but omit these values.
					...
					if (ByteReader.BytesRemaining() >= 4)
					{
						...
						uint8 nSPSExt;
						if (!ByteReader.ReadByte(nSPSExt))
						{
							return false;
						}
						if (nSPSExt)
						{
							SequenceParameterSetsExt.Reserve(nSPSExt);
						}
						for(int32 i=0; i<nSPSExt; ++i)
						{
							uint16 spsLenExt = 0;
				...
				if (TotalCSDSize)
				{
					const TArray<uint8> StartCode { 0,0,0,1 };
					CSD.Reserve(TotalCSDSize);
					for(int32 i=0; i<SequenceParameterSets.Num(); ++i)
					{
						CSD.Append(StartCode);
						CSD.Append(SequenceParameterSets[i]);
					}
					for(int32 i=0; i<PictureParameterSets.Num(); ++i)
					{
						CSD.Append(StartCode);
						CSD.Append(PictureParameterSets[i]);
					}
				}
				return true;
			}

			bool FAVCDecoderConfigurationRecord::CreateFromCodecSpecificData(const TConstArrayView<uint8>& InCSD)
			{
				CSD.Empty();
				RawData.Empty();
				if (InCSD.IsEmpty())
				...
				CSD = InCSD;
				TArray<FNaluInfo> Nalus;
				if (!ParseBitstreamForNALUs(Nalus, InCSD.GetData(), InCSD.Num()))
				{
					return false;
				}
				...
				for(int32 i=0; i<Nalus.Num(); ++i)
				{
					if (Nalus[i].Type == 7)
					{
						FSequenceParameterSet sps;
						if (ParseSequenceParameterSet(sps, InCSD.GetData() + Nalus[i].Offset + Nalus[i].UnitLength, Nalus[i].Size))
						{
							...
							TArray<uint8>& NewSPS(SequenceParameterSets.Emplace_GetRef());
							NewSPS.Append(InCSD.GetData() + Nalus[i].Offset + Nalus[i].UnitLength, Nalus[i].Size);
						}
						else
						{
							return false;
						}
						aSPS.NALUs.Emplace_GetRef() = MakeArrayView<const uint8>(InCSD.GetData() + Nalus[i].Offset + Nalus[i].UnitLength, Nalus[i].Size);
					}
					else if (Nalus[i].Type == 8)
					{
						aPPS.NALUs.Emplace_GetRef() = MakeArrayView<const uint8>(InCSD.GetData() + Nalus[i].Offset + Nalus[i].UnitLength, Nalus[i].Size);
					}
				}
			...
			FString FAVCDecoderConfigurationRecord::GetFormatInfo() const
			{
				FString fi(TEXT("AVC, "));
				fi.Append(FSequenceParameterSet::GetFormatInfo(AVCProfileIndication, AVCLevelIndication, 0));
				return fi;
			}
