...
namespace ElectraDecodersUtil
{
	namespace MPEG
	{
		namespace H265
		{
			...
			class FBitstreamReader : public FElectraBitstreamReader
			{
			public:
			...
			struct FProfileTierLevel
			{
				FProfileTierLevel()
				{
					Reset();
				}
				UE_API bool Parse(FBitstreamReader& br, bool profilePresentFlag, uint32 maxNumSubLayersMinus1);
				void Reset()
				{
			...
			{
				UE_API bool Parse(FBitstreamReader& br, bool commonInfPresentFlag, uint32 maxNumSubLayersMinus1);
				void Reset()
				{
			...
			struct FVUIParameters
			{
				...
				UE_API bool Parse(FBitstreamReader& br, uint32 In_sps_max_sub_layers_minus1);
				void Reset()
				{
			...
			struct FScalingListData
			{
				UE_API bool Parse(FBitstreamReader& br);
			...
			struct FStRefPicSet
			{
				UE_API bool Parse(FBitstreamReader& br, int32 stRpsIdx, const TArray<FStRefPicSet>& InReferencePicSetList, int32 InReferencePicSetListNum);
				void Reset()
				{
				...
				UE_API void SortDeltaPOC();
			};

			struct FVideoParameterSet
			{
				FVideoParameterSet()
				{
					Reset();
			...
			struct FSequenceParameterSet
			{
				...
				UE_API int32 GetMaxDPBSize() const;
				UE_API int32 GetDPBSize() const;
				UE_API int32 GetMinCbSizeY() const;
				UE_API int32 GetWidth() const;
				UE_API int32 GetHeight() const;
				UE_API void GetDisplaySize(int32& OutWidth, int32& OutHeight) const;
				UE_API void GetDisplaySize(uint32& OutWidth, uint32& OutHeight) const;
				UE_API void GetCrop(int32& OutLeft, int32& OutRight, int32& OutTop, int32& OutBottom) const;
				UE_API void GetAspect(int32& OutSarW, int32& OutSarH) const;
				UE_API FFractionalValue GetTiming() const;
				UE_API uint64 GetConstraintFlags() const;
				UE_API FString GetRFC6381(const TCHAR* SampleTypePrefix) const;
				UE_API FString GetFormatInfo() const;
				static UE_API FString GetFormatInfo(uint32 InProfileIDC, uint32 InLevelIDC, uint64 InConstraintFlags);
			...
			public:
				...
				UE_API void Update(const FSliceSegmentHeader& InFromSliceHeader);
			...
			class FDecodedPictureBuffer : public FSliceSegmentHeaderPOCVars
			{
			public:
				...
				UE_API void ProcessFirstSliceOfFrame(TArray<FDPBOutputFrame>& OutFrames, const FSliceSegmentHeader& InFromSliceHeader, bool bIsFirstInSequence);
				UE_API bool AddDecodedFrame(TArray<FDPBOutputFrame>& OutFrames, const FOutputFrameInfo& InNewDecodedFrame, const FSliceSegmentHeader& InFromSliceHeader);
				...
				UE_API void GetReferenceFramesFromDPB(TArray<FReferenceFrameListEntry>& OutReferenceFrames, TArray<int32> OutDPBIndexLists[eMAX]);
				UE_API const FReferenceFrameListEntry* GetDPBEntryAtIndex(int32 InIndex) const;

				// Call this to get all pending frame infos and clear the structure.
				UE_API void Flush(TArray<FDPBOutputFrame>& OutRemainingFrameInfos);
			...
			private:
				UE_API bool UpdatePOCandRPS(const FSliceSegmentHeader& InFromSliceHeader);
				UE_API FReferenceFrameListEntry* GenerateMissingFrame(int32 InPOC, bool bIsLongterm);
				UE_API void EmitNoLongerNeeded(TArray<FDPBOutputFrame>& OutFrames);
				UE_API void EmitAllRemainingAndEmpty(TArray<FDPBOutputFrame>& OutFrames);
				UE_API void BumpPictures(TArray<FDPBOutputFrame>& OutFrames, const FSliceSegmentHeader& InFromSliceHeader, bool bAdditionalBumping);

				UE_API bool SetLastError(const FString& InLastErrorMsg);
			...
			bool ELECTRADECODERS_API ParseVideoParameterSet(FVideoParameterSet& OutSequenceParameterSet, const uint8* InBitstream, uint64 InBitstreamLenInBytes);
			inline bool ParseVideoParameterSet(FVideoParameterSet& OutSequenceParameterSet, const TArray<uint8>& InData)
			{ return ParseVideoParameterSet(OutSequenceParameterSet, InData.GetData(), InData.Num()); }
			...
			inline bool ParseSequenceParameterSet(FSequenceParameterSet& OutSequenceParameterSet, const TArray<uint8>& InData)
			{ return ParseSequenceParameterSet(OutSequenceParameterSet, InData.GetData(), InData.Num()); }
			...
			public:
				...
				ELECTRADECODERS_API bool CreateFromCodecSpecificData(const TConstArrayView<uint8>& InCSD);
				...
				inline const TArray<TArray<uint8>>& GetVideoParameterSets() const
				{ return VideoParameterSets; }
				inline const TArray<TArray<uint8>>& GetSequenceParameterSets() const
				{ return SequenceParameterSets; }
				inline const TArray<FSequenceParameterSet>& GetParsedSequenceParameterSets() const
				{ return ParsedSequenceParameterSets; }
				inline uint8 GetGeneralProfileSpace() const
				{ return GeneralProfileSpace; }
				inline uint8 GetGeneralTierFlag() const
				{ return GeneralTierFlag; }
				inline uint8 GetGeneralProfileIDC() const
				{ return GeneralProfileIDC; }
				inline uint32 GetGeneralProfileCompatibilityFlags() const
				{ return GeneralProfileCompatibilityFlags; }
				inline uint64 GetGeneralConstraintIndicatorFlags() const
				{ return GeneralConstraintIndicatorFlags; }
				inline uint8 GetGeneralLevelIDC() const
				{ return GeneralLevelIDC; }
				inline uint8 GetChromaFormat() const
				{ return ChromaFormat; }
				inline uint8 GetBitDepthLumaMinus8() const
				{ return BitDepthLumaMinus8; }
				inline uint8 GetBitDepthChromaMinus8() const
				{ return BitDepthChromaMinus8; }
				inline const TArray<TArray<uint8>>& GetPrefixNUTs() const
				{ return PrefixNUTs; }
				inline const TArray<TArray<uint8>>& GetSuffixNUTs() const
				{ return SuffixNUTs; }

				ELECTRADECODERS_API FString GetCodecSpecifierRFC6381(const TCHAR* InPrefix) const;
				ELECTRADECODERS_API FString GetFormatInfo() const;
			...
			private:
				...
				TArray<TArray<uint8>> VideoParameterSets;
				TArray<TArray<uint8>> SequenceParameterSets;
				TArray<TArray<uint8>> PrefixNUTs;
				TArray<TArray<uint8>> SuffixNUTs;
				TArray<FVideoParameterSet> ParsedVideoParameterSets;
				TArray<FSequenceParameterSet> ParsedSequenceParameterSets;
