	...
	namespace MPEG
	{
		namespace H264
		{
			...
			struct FRBSP
			{
				...
				{
					Data.SetNumUninitialized((int32)InDataSize);
				}
				void TrimToBitstream(const FElectraBitstreamReader& InBSR)
				{
					if (InBSR.IsByteAligned())
					{
						Data.SetNum((int32) InBSR.GetBytePosition());
					}
					else
					{
						const uint8 Mask = 0xffU << (8 - InBSR.GetBitPosition());
						Data.SetNum((int32) InBSR.GetBytePosition() + 1);
						Data.Last() &= Mask;
					}
				}
				TArray<uint8> Data;
			...
			struct FSequenceParameterSet
			{
				...
				ELECTRADECODERS_API int32 GetMaxDPBSize() const;
				ELECTRADECODERS_API int32 GetDPBSize() const;
				ELECTRADECODERS_API int32 GetWidth() const;
				ELECTRADECODERS_API int32 GetHeight() const;
				ELECTRADECODERS_API void GetDisplaySize(int32& OutWidth, int32& OutHeight) const;
				ELECTRADECODERS_API void GetDisplaySize(uint32& OutWidth, uint32& OutHeight) const;
				ELECTRADECODERS_API void GetCrop(int32& OutLeft, int32& OutRight, int32& OutTop, int32& OutBottom) const;
				ELECTRADECODERS_API void GetAspect(int32& OutSarW, int32& OutSarH) const;
				ELECTRADECODERS_API FFractionalValue GetTiming() const;
				ELECTRADECODERS_API FString GetFormatInfo() const;
				ELECTRADECODERS_API static FString GetFormatInfo(uint32 InProfileIDC, uint32 InLevelIDC, uint32 InConstraintFlags);

				TArray<uint8> ProcessedBitstream;
			...
			public:
				...
				UE_API void GetCurrentReferenceFrames(TArray<FReferenceFrameListEntry>& OutCurrentReferenceFrames);

				// Call this per slice to get the list of references needed for decoding the slice.
				UE_API bool GetReferenceFrameLists(TArray<FReferenceFrameListEntry>& OutReferenceFrameList0, TArray<FReferenceFrameListEntry>& OutReferenceFrameList1, const FSliceHeader& InSliceHeader);

				// Call this on the first slice of an image to be decoded.
				UE_API bool BeginFrame(uint8 InNalUnitType, uint8 InNalRefIdc, const FSliceHeader& InSliceHeader, const FSequenceParameterSet& InSequenceParameterSet, const FPictureParameterSet& InPictureParameterSet);
				// Call this next to see if there are any missing frames between this and the last.
				UE_API bool HandleMissingFrames(TArray<FOutputFrameInfo>& OutOutputFrameInfos, TArray<FOutputFrameInfo>& OutUnrefFrameInfos, uint8 InNalUnitType, uint8 InNalRefIdc, const FSliceHeader& InSliceHeader, const FSequenceParameterSet& InSequenceParameterSet);
				// Update the POC values for the current frame.
				UE_API bool UpdatePOC(uint8 InNalUnitType, uint8 InNalRefIdc, const FSliceHeader& InSliceHeader, const FSequenceParameterSet& InSequenceParameterSet);
				// Undo the POC value updates in case the decoding process could not run.
				UE_API void UndoPOCUpdate();
				...
				UE_API bool EndFrame(TArray<FOutputFrameInfo>& OutOutputFrameInfos, TArray<FOutputFrameInfo>& OutUnrefFrameInfos, const FOutputFrameInfo& InOutputFrameInfo, uint8 InNalUnitType, uint8 InNalRefIdc, const FSliceHeader& InSliceHeader, bool bIsNonExisting);

				// Call this to get all pending frame infos and clear the structure.
				UE_API void Flush(TArray<FOutputFrameInfo>& OutRemainingFrameInfos, TArray<FOutputFrameInfo>& OutUnrefFrameInfos);
			...
			private:
				...
				UE_API void UpdatePOCInternal(uint8 InNalUnitType, uint8 InNalRefIdc, const FSliceHeader& InSliceHeader, const FSequenceParameterSet& InSequenceParameterSet);
				UE_API void UpdateRefLists();
				UE_API FSmallestPOC GetSmallestPOC();
				UE_API bool ReorderRefPicList(TArray<FReferenceFrameListEntry>& InOutReferenceFrameList, const FSliceHeader& InSliceHeader, int32 InListNum);
				UE_API void FlushDPB(TArray<FOutputFrameInfo>& InOutInfos, TArray<FOutputFrameInfo>& InUnrefInfos);
			...
			public:
				...
				inline const TArray<TArray<uint8>>& GetPictureParameterSets() const
				{ return PictureParameterSets; }

				inline uint8 GetAVCProfileIndication() const
				{ return AVCProfileIndication; }
				inline uint8 GetProfileCompatibility() const
				{ return ProfileCompatibility; }
				inline uint8 GetAVCLevelIndication() const
				{ return AVCLevelIndication; }

				inline FString GetCodecSpecifierRFC6381(const TCHAR* InPrefix) const
				{ return FString::Printf(TEXT("%s.%02x%02x%02x"), InPrefix, GetAVCProfileIndication(), GetProfileCompatibility(), GetAVCLevelIndication()); }
