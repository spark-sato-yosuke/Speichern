...
namespace ElectraDecodersUtil
{
	...
	{
	public:
		UE_API FMP4AtomReader(const void* InDataPtr, int32 InDataSize);

		UE_API int32 GetCurrentOffset() const;
		UE_API int32 GetNumBytesRemaining() const;
		UE_API const uint8* GetCurrentDataPointer() const;
		UE_API void SetCurrentOffset(int32 InNewOffset);
		...
		UE_API bool ReadString(FString& OutString, uint16 NumBytes);
		UE_API bool ReadStringUTF8(FString& OutString, int32 NumBytes);
		UE_API bool ReadStringUTF16(FString& OutString, int32 NumBytes);
		UE_API bool ReadBytes(void* Buffer, int32 NumBytes);
