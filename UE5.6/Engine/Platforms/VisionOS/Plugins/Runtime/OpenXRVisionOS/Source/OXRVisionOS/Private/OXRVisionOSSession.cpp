...
FOXRVisionOSSession::FOXRVisionOSSession(const XrSessionCreateInfo* createInfo, FOXRVisionOSInstance* InInstance)
{
	...
	GripSurfacePoseAdjustment = FQuat(Settings->OXRVisionOSGripSurfacePoseAdjustment);
	...
	XrReferenceSpaceType*						OutSpaces)
{
	...
	const int NumSupportedSpaceTypes = 3;//4;
	...
	*SpaceCountOutput = NumSupportedSpaceTypes;
	if (SpaceCapacityInput != 0)
	{
		...
		OutSpaces[2] = XR_REFERENCE_SPACE_TYPE_LOCAL_FLOOR;
		//OutSpaces[3] = XR_REFERENCE_SPACE_TYPE_STAGE;
	}
	return XrResult::XR_SUCCESS;
}
...
void FOXRVisionOSSession::OnBeginRendering_RenderThread_PreDeviceLocationUpdate()
{
	UE_LOG(LogOXRVisionOS, VeryVerbose, TEXT("%s FC_%i SLF_0x%x FOXRVisionOSSession::OnBeginRendering_RenderThread_PreDeviceLocationUpdate"), VOSThreadString(), PipelinedFrameStateRendering.FrameCounter, PipelinedFrameStateRendering.SwiftFrame);
...
//	bool bGaze = false;
	...
	case EOXRVisionOSControllerButton::GripSurfaceL:
	...
	case EOXRVisionOSControllerButton::GripSurfaceR:
	...
	// else
	{
		...
		if ((OutLocationFlags & (XR_SPACE_LOCATION_POSITION_VALID_BIT | XR_SPACE_LOCATION_ORIENTATION_VALID_BIT)))
		{
			...
			else if (Button == EOXRVisionOSControllerButton::GripSurfaceL || Button == EOXRVisionOSControllerButton::GripSurfaceR)
			{
				OutTransform.SetRotation(OutTransform.TransformRotation(GripSurfacePoseAdjustment));
			}
		}
	}
}
