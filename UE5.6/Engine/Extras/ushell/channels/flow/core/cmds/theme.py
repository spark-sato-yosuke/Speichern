...
class Random(_ThemeCmd):
    ...
    mode = flow.cmd.Arg("", "Constrain randomness to dark/mid/light themes")
    one  = flow.cmd.Opt(False, "Pick one theme at random and exit")

    complete_mode = ("dark", "light", "mid")

    def main(self):
        is_mid = lambda x: x.get_luminance() > 0.023 and x.get_luminance() < 0.3
        schemes = self.get_schemes()
        names = list(schemes.keys())
        what = self.args.mode.lower()
        if   what == "dark":  names = [x for x in names if not schemes[x].is_light()]
        elif what == "light": names = [x for x in names if     schemes[x].is_light()]
        elif what == "mid":   names = [x for x in names if     is_mid(schemes[x].get_bg())]
