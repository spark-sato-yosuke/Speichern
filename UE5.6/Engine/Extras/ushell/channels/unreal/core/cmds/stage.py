...
class _Impl(unrealcmd.Cmd):
    ...
    style    = unrealcmd.Arg("auto", "Type of result to stage")
    uatargs  = unrealcmd.Arg([str], "Additional arguments to pass to UAT")
    debug    = unrealcmd.Opt(False, "Use a debug variant of UAT")
    ...
    def main(self, *, skipstage=False):
        ...
        dot_uat_args = (
            "BuildCookRun",
            "--debug" if self.args.debug else None,
            "--", # NB. NOT a UAT arg!
...
class Stage(_Impl):
    ...
    Available staging styles:
      ...
      pak   - stage to pak/utoc/ucas files
      nopak - stage for zen streaming if enabled, pak otherwise
      zen   - stage for zen streaming, fail otherwise
      auto  - [default] equivalent to nopak
