...
class _Builder(object):
    ...
    def read_actions(self):
        ...
        """
        if platform.lower() == unreal.Platform.get_host().lower():
            from platform import machine as get_machine
            if get_machine() in ("arm64", "ARM64"):
                args = (*args, "-Architecture=arm64")
        """
...
class _BuildCmd(unrealcmd.Cmd):
    ...
    module or source file. It accepts one of the following forms as input;
      ...
      .                        - fuzzy find the file/modules to build (a dot)
    """
    ...
    noshowcfg = unrealcmd.Opt(False, "Bypass the printing of configuration from XML files")
    ...
    def show_build_config(self):
        if not self.is_interactive() or self.args.noshowcfg:
            return
...
class _Runtime(_BuildCmd, unrealcmd.MultiPlatformCmd):
    ...
    def _main_impl(self, target):
        ...
        ue_context = self.get_unreal_context()
        has_project = ue_context.get_project() is not None

        builder = self.get_builder()
        builder.set_platform(platform)
        builder.set_targets(target)
        builder.set_variant(self.args.variant)
        builder.set_projected(has_project)
