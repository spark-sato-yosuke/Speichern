...
class Platform(object):
    ...
    def get_autosdk_name(self):
        self.get_version() # we're cheating; see SDK.json loading path
    ...
    def _get_version_json(self):
        ...
        def read_sdk_json(path):
            ...
            # patch the AutoSDK name while we're here...
            if autosdk_name := sdk_info.get("AutoSDKPlatform", None):
                type(self).autosdk_name = autosdk_name
            ...
            # ...or traverse up the platform forest
    ...
    def get_stage_name(self, target):
        if hasattr(self, "_get_stage_name"):
            return self._get_stage_name(target)
        return self.get_cook_form(target)
