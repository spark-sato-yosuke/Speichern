    ...
    def _launch(self, exec_context, stage_dir, binary_path, args):
        if stage_dir:
            ...
            if project := self.get_unreal_context().get_project():
                midfixes = (project.get_name(), *midfixes)
            for midfix in midfixes:
                base_dir = stage_dir + midfix + "/Binaries/Win64"
                if os.path.isdir(base_dir):
                    break
            else:
                raise EnvironmentError(f"Failed to find binaries directory from '{stage_dir}'")
            args = (*args, *unreal.cmdline.read_ueified("-basedir=" + base_dir))
