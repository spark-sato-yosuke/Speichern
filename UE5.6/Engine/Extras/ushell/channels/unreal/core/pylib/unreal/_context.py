...
class _Project(_UfsNode):
    ...
    def get_target_by_type(self, target_type:TargetType) -> _Target|None:
        ...
        if not (self.get_dir() / "Source").is_dir():
            ret = self.get_parent_node().get_target_by_type(target_type)
            if ret:
                ret._ufs_mount = self.get_root_mount()
            return ret
        ...
        return _Target(target_name, target_type, self.get_root_mount())
...
class Context(object):
    ...
    def __init__(self, primary_dir:str):
        ...
        if sys.platform == "darwin":
            # On Mac, we want to use the case-correct path, and the fcntl.F_GETPATH operation is the most direct way to get that
            # Note that if we encounter an exception, the path will be left in its original form
            try:
                import fcntl
                with os.fdopen(os.open(primary_dir, os.O_RDONLY)) as primary_dir_object:
                    canonnical_path = fcntl.fcntl(primary_dir_object.fileno(), fcntl.F_GETPATH, bytes(os.fpathconf(primary_dir_object.fileno(), 'PC_PATH_MAX')))
                    primary_dir:Path = Path(canonnical_path.decode('utf-8').rstrip('\x00')).resolve()
            except:
                pass
